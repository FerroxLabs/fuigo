...
//! harness implements [`CompactionItem`] for its `FuigoTurn`;
//! fuigo-build implements it for `fuigo_sampling_types::ConversationItem`.
//!
...
//! `<fuigo_user_queries>` blocks, drop tool content from assistant turns,
//! wrap an LLM summary into a carrier item).

/// Harness-agnostic role of a single conversation item.
///
/// This is the common denominator of `FuigoRole` (Fuigo chat) and the
/// `ConversationItem` variants (fuigo-build).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CompactionRole {
    /// System prompt.
    System,
    /// Developer prompt (Fuigo chat) — maps to System on harnesses without a
...
/// extraction (`<fuigo_file id=".." name=".." />` lines in the
/// `<fuigo_user_queries>` preamble).
...
///
/// Implementors:
/// - Fuigo chat: `FuigoTurn`
/// - fuigo-build: `ConversationItem`
pub trait CompactionItem {
    ...
    ///
    /// Returns an owned `String` because some harnesses (Fuigo chat's
    /// `FuigoTurn`) compute the flattened text on demand rather than storing a
    /// borrowable slice.
    ...
    /// Whether this item carries a *prior compaction summary* (Fuigo chat: a
    ...
    /// strips their `<fuigo_user_queries>` blocks before sampling.
    ///
    ...
    /// File attachments on a (user) item, for the `<fuigo_file>` lines in the
    /// `<fuigo_user_queries>` preamble. Empty for items without attachments.
    ///
...
pub trait CompactionItemBuilder: CompactionItem + Clone {
    ...
    /// conversation (Fuigo chat: a `Developer` turn with category
    ...
    /// requests/results (Fuigo chat: keep only `Channel` contents of an
    ...
    /// `max_tokens` (fuigo-build style: `max_bytes = max_tokens * 4`, prefix
...
/// constructs the typed harness items that make up fuigo-build's rebuilt
/// history.
///
/// This is a sibling of [`CompactionItemBuilder`], not a part of it, on
/// purpose. `CompactionItemBuilder` is already implemented by Fuigo chat's
/// `FuigoTurn`; adding these constructors to it as required methods would break
/// that impl. They are also fuigo-build-specific (Fuigo chat's tail-keep path
...
///
/// The fuigo-build implementor (`ConversationItem`) maps each constructor to the
...
/// operate over `&[Arc<T>]` (Fuigo chat stores turns as `Arc<FuigoTurn>`).
