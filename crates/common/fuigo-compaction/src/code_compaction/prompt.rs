//! fuigo-build's session-level summarization prompt.
//!
//! Split out of the crate-root `prompt` module so fuigo-build's full-replace
//! prompt lives alongside the rest of its [`code_compaction`](crate::code_compaction)
//! subsystem. The Fuigo chat's step-level intra prompt
//! ([`format_compaction_prompt`](crate::prompt::format_compaction_prompt))
//! stays at the crate root.

/// Build fuigo-build's session-level summarization prompt (no chat history).
///
...
/// `fuigo-shell::session::helpers::session_compact::build_compaction_prompt`
...
/// (mirrors `fuigo-shell`'s `SELF_SUMMARIZATION_PROMPT`). Framed
...
/// so the structured (fuigo-build) and short self-summary prompts stay
/// shared instead of duplicated per harness.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum SummaryPromptKind {
    /// fuigo-build's detailed, numbered-section summary prompt.
    #[default]
    Structured,
...
mod tests {
    ...
    fn kind_structured_matches_build_summary_prompt() {
        ...
        // so routing through the selector never changes fuigo-build's prompt.
        assert_eq!(
