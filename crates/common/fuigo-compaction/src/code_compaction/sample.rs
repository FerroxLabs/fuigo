...
//!
//! The canonical `sample → classify → retry` loop, used by **both** fuigo-build's
//! full-replace pass ([`sample_full_replace_summary`](super::sample_full_replace_summary))
//! and Fuigo chat's intra `Shared` summarizer
...
//! the winning summary (fuigo-build cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the fuigo-build host uses to step down its input size).
...
/// A rendered HTTP 429 (TPM rate limit): its "Request too large ... tokens per
/// min" wording matches the size heuristic, but the request is retryable once
/// the window passes.
