//! Compacted-history assembly (fuigo-build's rebuild structure, generic).
//!
//! Moved from `fuigo-chat-state::compaction_utils::build_compacted_history` and
...
//! ```
//!
//! fuigo-build is the canonical harness. The summary carrier text is built by
...
pub fn assemble_compacted_history<T: CompactionItemFactory>(
    ...
    // fuigo-build keeps the legacy `<user_query>`-wrapped continuation text and
...
mod tests {
    ...
    #[test]
    fn fuigo_build_order_recent_before_summary() {
