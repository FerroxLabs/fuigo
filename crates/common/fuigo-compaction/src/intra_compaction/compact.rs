...
// The `Shared` summarizer reuses fuigo-build's full-replace summarization core
...
/// `FullReplace` strategy (default): fuigo-build's full-replace — summarize the
...
/// `<fuigo_user_queries>` preamble: the shared `code_compaction` summarizer
/// (always [`IntraSummarizer::Shared`] here, regardless of `policy.summarizer`)
/// preserves user intent itself, matching fuigo-build. The reduction and
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 2. Summarize (possibly fitted) turns through fuigo-build's shared core.
    ...
    //     Shared with Fuigo chat inter-compaction via `append_reminder_block` so
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    ...
    // 3a. For `History` target, split prior `<fuigo_user_queries>` blocks
    //     out of any prior compaction summary items before sampling — same
    //     primitive inter-compaction uses, so the LLM never sees
    //     `<fuigo_user_queries>` and won't re-emit it (which would snowball
    ...
    //     and the `<fuigo_user_queries>` preamble below — stays intra's.
    let summary_text = match policy.summarizer {
        ...
        // New (default): fuigo-build's shared summarization core from
    ...
    // 3c. For `History` target, prepend a `<fuigo_user_queries>` preamble so
...
/// — fuigo-build's summarization core (`build_summary_prompt` + bounded retry +
...
/// intra and fuigo-build stay in lock-step. Outcome mapping:
...
where
    ...
    P: CompactionSampler<Item = T> + ?Sized,
{
    // fuigo-build appends the summarization prompt as the final user message;
    ...
    .await
    {
        // fuigo-build returns the raw summary and cleans it in its assembler;
...
/// metric label. Keep these in sync with the doc string on Fuigo chat's
...
/// the literal error messages produced by the Fuigo chat sampler —
...
mod tests {
    ...
    fn compaction_sample_error_to_intra_maps_empty_response() {
        // The literal message emitted by the Fuigo chat sampler when the
    ...
    /// — guards the forwarding that Fuigo chat (`Arc<FuigoTurn>`) relies on.
    #[test]
