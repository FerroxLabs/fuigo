...
pub enum CompactionTarget {
    ...
    /// with a single summary — fuigo-build's full-replace strategy. No tail is
...
/// stream processor. Implemented by Fuigo chat's
/// `StreamProcessor` (`Item = Arc<FuigoTurn>`).
///
...
pub trait CompactionStreamProc: Send + Sync {
    ...
    /// item type (`Arc<FuigoTurn>`) already satisfies it.
