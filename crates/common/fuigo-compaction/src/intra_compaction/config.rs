...
///
/// - `FullReplace` (default): fuigo-build's full-replace strategy — summarize
...
///
/// Uses **percentage** thresholds (borrowed from fuigo-shell's
...
/// time (token threshold only, matching fuigo-build) and also ignores
...
pub struct IntraCompactionConfig {
    ...
    /// as fuigo-build full-replace auto-compact). Worthless early passes are
    ...
    /// `<fuigo_user_queries>` preamble prepended to the history compaction
