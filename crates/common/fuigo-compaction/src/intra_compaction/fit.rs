...
//!                          that alone exceed budget (fuigo-build style:
...
//!   (one-shot, same `tokens * 4` budget as fuigo-build)
...
/// One-shot payload shrink (fuigo-build: `max_bytes = max_tokens * 4`).
///
...
///
/// Matches fuigo-build's focus: in-place truncate is for oversized tool
...
pub fn fit_turns_for_summarizer<T: CompactionItemBuilder>(
    ...
    // One-shot prefix clip on tool results only (fuigo-build style).
    ...
    // fuigo-build: when even the newest unit alone exceeds budget, keep it
...
///
/// Mirrors fuigo-build `truncate_text_to_bytes` / `truncate_item_to_tokens`:
