...
//! stay in each product host (for example `fuigo-shell`).
//!
//! The crate depends on **neither** a conversation-type crate nor
//! `fuigo-sampling-types`. It is decoupled from both Fuigo chat and
//! fuigo-build hosts through a small set of trait seams:
//!
...
//!
//! - [`code_compaction`] — fuigo-build's whole-session **full-replace**
//!   subsystem (prompt/summary/failure/config, assemble, orchestration).
//! - [`intra_compaction`] — Fuigo chat's tail-keep, per-step pass.
//! - [`inter_compaction`] — Fuigo chat's chunked, between-turn pass.
//!
...
//! (active-agent-state `<system-reminder>` formatting shared by Fuigo chat and
//! fuigo-build; hosts still own snapshotting and host-only sections).
...
// fuigo-build's full-replace subsystem now lives under `code_compaction`;
