...
//!
//! - Fuigo chat has a real tokenizer (`TextTokenizer` / `ImageTokenizer`) and
//!   counts whole turns via `FuigoTurn::get_num_tokens`.
//! - fuigo-build estimates with `bytes / 4`.
//!
...
//! (Fuigo chat: `ItemTokenCounter<Arc<FuigoTurn>>`), so exactly one mechanism
//! is in play.
