...
impl BackgroundNoticeNaming<'static> {
    /// Canonical fuigo-build names, for hosts without renaming.
...
/// Hosts with a shorter transport deadline set `FUIGO_MAX_WAIT_BLOCK_MS`.
pub const MAX_WAIT_BLOCK_MS_DEFAULT: u64 = 3_600_000;

/// The blocking-wait ceiling in effect, honoring `FUIGO_MAX_WAIT_BLOCK_MS`.
///
...
pub fn max_wait_block_ms() -> u64 {
    std::env::var("FUIGO_MAX_WAIT_BLOCK_MS")
        .ok()
...
/// A built-in subagent type shared by the CLI (`fuigo-agent`) and other
...
mod tests {
    ...
    // match what the fuigo-shell MiniJinja templates render for the default
    // fuigo-build toolset (monitor + task + bash + read present, POSIX). The
