...
use fuigo_tool_protocol::{ToolCapabilities, ToolId};
...
/// `filename`, `path`, `metadata`) that the Fuigo SLOP converter uses to
...
pub enum ContentBlock {
    ...
    ///
    /// Optional metadata fields are used by the Fuigo SLOP converter to
    ...
    /// - `path`: file path on the Fuigo Computer filesystem
