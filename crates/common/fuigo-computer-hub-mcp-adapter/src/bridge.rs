...
use fuigo_tool_protocol::{McpBlock, SessionId, ToolId, ToolOutputWire};
use fuigo_tool_runtime::{ToolCallContext, ToolError, ToolStream, TypedToolOutput, terminal_only};
...
/// [`fuigo_computer_hub_sdk::ToolServerBuilder`] to register them
/// with the hub.
///
...
impl McpBridge {
    ...
    /// Handlers to register with a [`fuigo_computer_hub_sdk::ToolServerBuilder`].
    ///
...
#[async_trait]
impl fuigo_computer_hub_sdk::ToolServerHandler for McpToolHandler {
...
mod tests {
    ...
    async fn bridge_handles_transport_error() {
        ...
        let item = stream.next().await.unwrap();
        match item {
            fuigo_tool_runtime::ToolStreamItem::Terminal(Err(ref e))
                if e.kind == fuigo_tool_runtime::ToolErrorKind::Execution =>
            {
                assert!(
