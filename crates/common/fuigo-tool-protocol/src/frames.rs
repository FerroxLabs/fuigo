...
pub struct ToolCallResult {
    ...
    /// this crate must not depend on `fuigo-tool-runtime`. Sampler-side wire
...
/// `fuigo_computer_hub_core::registry::ToolSessionBindOutcome`, with one
...
pub struct ServerIdentityMetadata {
    ...
    /// Logical sandbox-service session UUID (from `FUIGO_SESSION_ID` in the
...
pub struct ToolsListResult {
    pub tools: Vec<fuigo_tool_types::ToolDescription>,
...
pub enum IdleWithholdReason {
    ...
    PreviewRouted,
    /// Only the preview pane's own `/__fuigo-preview/status` liveness poll.
    PreviewStatusOnly,
