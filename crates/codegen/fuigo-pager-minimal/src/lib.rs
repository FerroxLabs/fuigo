//! Minimal (scrollback-native) render mode: `fuigo --minimal`.
//!
//! In this mode finalized conversation blocks are printed once into the terminal's *native* scrollback.
//! The printing goes through `fuigo_ratatui_inline::Terminal::insert_before` and reuses `EntryRenderer`.
...
//! # Wiring
//!
//! `fuigo-pager` (the lib) does **not** depend on this crate: this crate reads deeply into the pager's [`AppView`] / view model.
//! A reverse dependency would be a cargo cycle.
//! Instead the pager exposes function-pointer hooks ([`fuigo_pager::minimal_hook`]).
//! The composition-root binary (`fuigo-pager-bin`) calls [`install`] once at startup to register this crate's [`draw`] entry point.
...
use fuigo_pager::app::app_view::AppView;
...
/// Idempotent: subsequent calls are ignored (see [`fuigo_pager::minimal_hook`]).
pub fn install() {
    fuigo_pager::minimal_hook::install(fuigo_pager::minimal_hook::MinimalHooks { draw });
}
