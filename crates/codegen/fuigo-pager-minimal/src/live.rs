...
use fuigo_pager::app::PagerTerminal;
use fuigo_pager::app::app_view::{ActiveView, AppView};
use fuigo_pager::minimal_api;
...
pub(super) fn live_left_inset(_appearance: &fuigo_pager::appearance::AppearanceConfig) -> u16 {
    0
}
...
fn clear_btw_geometry(agent: &mut fuigo_pager::app::agent_view::AgentView) {
    agent.last_btw_selection_model =
        fuigo_pager::scrollback::text_selection::ResolvedSelectionModel::default();
...
pub(super) fn prompt_style(
    appearance: &fuigo_pager::appearance::AppearanceConfig,
    input_mode: fuigo_pager::app::agent_view::PromptInputMode,
    theme: &Theme,
    multiline: bool,
) -> PromptStyle {
    PromptStyle {
...
pub fn draw_live(app: &mut AppView, terminal: &mut PagerTerminal) {
    ...
    fuigo_pager::render::draw::draw_frame(terminal, cursor, |frame, _link_spans| {
        ...
        agent.active_pane = fuigo_pager::app::agent_view::AgentPane::Prompt;
        ...
        if minimal_api::extensions_modal(agent).is_some() {
            ...
            if let Some(state) = minimal_api::extensions_modal_mut(agent) {
                fuigo_pager::views::extensions_modal::render_extensions_modal(
                    frame.buffer_mut(),
                    area,
                    state,
                    None,
        ...
        let raw_btw = if minimal_api::minimal_btw_surface_available(agent) {
            fuigo_pager::views::btw_overlay::btw_panel_height(
        ...
        if let (Some(btw), Some(btw_area)) = (agent.btw_state.as_ref(), btw_area) {
            ...
            fuigo_pager::views::btw_overlay::render_btw_panel(
                frame.buffer_mut(),
                btw,
                btw_area,
                tick,
        ...
        (
            ...
            result
                .post_flush_escapes
                .map(fuigo_pager::terminal::overlay::PostFlush::from),
        )
    });
}
fn live_tail_renderer<'a>(
    entry: &'a fuigo_pager::scrollback::entry::ScrollbackEntry,
    theme: &'a Theme,
    appearance: &fuigo_pager::appearance::AppearanceConfig,
    cwd: &'a std::path::Path,
    tick: u64,
...
fn minimal_advance_phase_timer(
    agent: &mut fuigo_pager::app::agent_view::AgentView,
) -> Option<fuigo_pager::acp::tracker::TurnActivity> {
...
fn render_minimal_status(
    buf: &mut Buffer,
    area: Rect,
    agent: &fuigo_pager::app::agent_view::AgentView,
    activity: &Option<fuigo_pager::acp::tracker::TurnActivity>,
...
fn render_config_status_line(
    ...
    area: Rect,
    agent: &mut fuigo_pager::app::agent_view::AgentView,
    frame: &fuigo_pager::views::status_line::StatusLineFrame,
    theme: &Theme,
) {
    if area.height == 0 || area.width == 0 {
        return;
    ...
    if let Some(width) = minimal_api::status_line_inner_width(area.width, padding) {
        agent.last_status_line_size = Some(fuigo_pager::views::status_line::RowSize {
            cols: width,
            lines: area.height,
        });
    }
    if let Some(display) = frame.display() {
        let _ = fuigo_pager::views::status_line::render_status_line(
...
fn render_idle_hint(buf: &mut Buffer, area: Rect, theme: &Theme) {
    ...
    buf.set_style(area, style);
    let auto = fuigo_pager::app::minimal_auto_set_for_mouse_leak();
    let switch_back = fuigo_pager::app::minimal_show_switch_back_to_fullscreen();
...
fn render_prompt_info(
    ...
    use fuigo_pager::views::context_bar::fmt_tokens;
    ...
    if let Some(label) = agent.prompt_input_mode.prompt_info_override() {
        ...
        if let (Some(used), Some(total)) = (used, total)
            && total > 0
        {
            let pct = fuigo_token_estimation::usage_percentage(used, total);
            segs.push((
...
fn minimal_pending_hint(
    pending: &Option<fuigo_pager::app::app_view::PendingAction>,
) -> Option<String> {
...
mod tests {
    use super::*;
    fn agent() -> fuigo_pager::app::agent_view::AgentView {
    ...
    fn tail_height_uses_owning_session_cwd_for_tool_paths() {
        ...
        use fuigo_pager::scrollback::types::DisplayMode;
    ...
    fn minimal_status_shows_rich_activity_and_idle_hint() {
        ...
        fuigo_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(false);
        let a = agent();
        let mut buf = Buffer::empty(area);
        ...
        fuigo_pager::app::set_minimal_show_switch_back_to_fullscreen_for_test(true);
        let mut buf = Buffer::empty(area);
        render_minimal_status(&mut buf, area, &a, &None, None, &theme);
    ...
    fn minimal_status_shows_idle_watching_cue() {
        ...
        a.session.scheduled_tasks.insert(
            "loop-1".to_string(),
            fuigo_pager::app::agent::ScheduledTaskInfo {
    ...
    fn prompt_info_shows_session_mode_flag() {
        ...
        let render = |a: &fuigo_pager::app::agent_view::AgentView| -> String {
            let mut buf = Buffer::empty(area);
    ...
    fn pending_hint_formats_press_again() {
        ...
        use fuigo_pager::app::app_view::PendingAction;
        use fuigo_pager::input::key::KeyShortcut;
