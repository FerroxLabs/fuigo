use regex::Regex;
use fuigo_tools::types::{claude_names_for, fuigo_names_for};
...
/// - a "simple" pattern (a plain name or `|`-list) is an **exact** match against each name (after external-to-Fuigo alias expansion), NOT a regex;
...
/// Expand a simple-form pattern into the exact set of names it matches: each `|`-term plus any Fuigo tool names that term aliases.
/// The alias mapping comes from the shared external-name registry in `fuigo-tools`, so `"Bash"` also matches `run_terminal_command`.
...
fn exact_names(pattern: &str) -> Vec<String> {
    ...
    for term in pattern.split('|') {
        push(term);
        for fuigo_name in fuigo_names_for(term) {
            push(fuigo_name);
        }
    }
    names
...
mod tests {
    ...
    #[test]
    fn claude_bash_matches_fuigo_tool() {
    ...
    #[test]
    fn claude_edit_write_matches_fuigo_tool_exactly() {
    ...
    #[test]
    fn regex_against_claude_alias_matches_fuigo_tool() {
        let m = HookMatcher::new("^Bash$").unwrap();
        assert!(m.is_match("run_terminal_command"));
        assert!(m.is_match("Bash"));
    }
