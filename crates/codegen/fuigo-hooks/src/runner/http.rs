...
pub async fn run_http_hook(
    ...
    for (k, v) in [
        ("FUIGO_HOOK_EVENT", envelope.hook_event_name.to_string()),
        ("FUIGO_HOOK_NAME", spec.name.clone()),
        ("FUIGO_SESSION_ID", ctx.session_id.to_string()),
        ("FUIGO_WORKSPACE_ROOT", ctx.workspace_root.to_string()),
...
mod tests {
    ...
    async fn url_unresolved_var_fails_validation() {
        let key = "FUIGO_HOOKS_HTTP_TEST_UNRESOLVED";
