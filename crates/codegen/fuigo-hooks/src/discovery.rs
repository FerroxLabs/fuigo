...
/// Rank ordering also settles managed-vs-managed pairs (`$FUIGO_HOME/requirements.toml` arrives before `/etc/fuigo`).
...
fn load_hooks_from_directory(dir: &Path) -> (Vec<HookSpec>, Vec<HookError>) {
    ...
    // (Sandbox fail-closed listing lives in fuigo_config.)
    ...
    for entry in entries {
        ...
        if !fuigo_config::is_direct_hook_json_name(name) || !path.is_file() {
            continue;
        }
...
fn is_valid_hook_file(path: &Path) -> bool {
    ...
    fuigo_config::is_direct_hook_json_name(name) && path.is_file()
}
...
mod tests {
    ...
    fn dedup_keeps_the_managed_policy_copy_regardless_of_order() {
        ...
        // Managed-vs-managed pair: `$FUIGO_HOME/requirements.toml` arrives before `/etc/fuigo`, but the root-owned tier outranks it
