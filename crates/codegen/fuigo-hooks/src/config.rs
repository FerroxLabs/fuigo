...
pub use fuigo_config::HookProvenance;
...
fn resolve_session_end_default(value: Option<&str>) -> u64 {
    ...
    match value.trim().parse::<u64>() {
        ...
        _ => {
            tracing::warn!(
                value,
                "FUIGO_SESSION_END_HOOKS_TIMEOUT_MS must be a positive integer; using default {}ms",
...
fn session_end_default_timeout_ms() -> u64 {
    ...
    *VALUE.get_or_init(|| {
        resolve_session_end_default(
            std::env::var("FUIGO_SESSION_END_HOOKS_TIMEOUT_MS")
                .ok()
                .as_deref(),
        )
    })
...
pub fn hook_origin(spec: &HookSpec) -> HookOrigin {
    match spec.layer {
        ...
        // Both requirements tiers display as the requirements origin; only the policy exemption distinguishes root-owned from `$FUIGO_HOME`
...
/// How a hook is named to the user: a real name, or tier copy for config-tier sources.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
pub fn parse_hooks_from_config_layers(
    ...
    let home = fuigo_config::user_fuigo_home();
...
mod tests {
    ...
    fn hook_display_name_tracks_stamped_names() {
        ...
        for (source_name, provenance, expected) in tiers {
            let layer = fuigo_config::HookConfigLayer::new(
                provenance,
                source_name,
    ...
    fn config_layer(source_name: &str, toml_src: &str) -> fuigo_config::HookConfigLayer {
        let value: toml::Value = toml::from_str(toml_src).unwrap();
        let hooks = value.get("hooks").cloned().unwrap();
        fuigo_config::HookConfigLayer::new(
            fuigo_config::HookProvenance::Managed,
            source_name,
            hooks,
        )
    }
    ...
    fn config_layers_additive_and_dedup_keeps_higher_authority() {
        ...
        use fuigo_config::HookProvenance::{Managed, User};
    ...
    fn source_dir_from_file_path() {
        ...
        let (specs, _) = parse_hook_file(json, Path::new("/home/user/.fuigo/hooks/safety.json"));
        assert_eq!(specs[0].source_dir, PathBuf::from("/home/user/.fuigo/hooks"));
    }
    ...
    fn parse_hook_file_expands_env_var_in_command_from_process_env() {
        let key = "FUIGO_HOOKS_PARSE_TEST_CMD_PROC_ENV";
    ...
    fn parse_hook_file_expands_env_var_in_url_from_process_env() {
        let key = "FUIGO_HOOKS_PARSE_TEST_URL_PROC_ENV";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_command() {
        let key = "FUIGO_HOOKS_PARSE_TEST_NEVER_SET_AT_LOAD_TIME";
    ...
    fn parse_hook_file_preserves_unresolved_env_refs_in_url() {
        let key = "FUIGO_HOOKS_PARSE_TEST_URL_NEVER_SET_AT_LOAD_TIME";
        with_env_var(key, None, || {
            let json = format!(
                r#"{{
                    "hooks": {{
    ...
    fn parse_hook_file_matcher_is_not_env_expanded() {
        let key = "FUIGO_HOOKS_PARSE_TEST_MATCHER_VAR";
                        ...
                        "hooks": [
                            {
                                ...
                                "command": "echo hi",
                                "env": {
                                    "FUIGO_HOOK_EVENT": "spoofed",
                                    "FUIGO_HOOK_NAME": "spoofed",
                                    "FUIGO_SESSION_ID": "spoofed",
                                    "FUIGO_WORKSPACE_ROOT": "/etc",
