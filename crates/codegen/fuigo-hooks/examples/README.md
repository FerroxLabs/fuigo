...
Sample hooks for Fuigo. Copy to `~/.fuigo/hooks/` to enable globally, or to `<project>/.fuigo/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.fuigo/hooks/bin
cp examples/hooks/safe-shell.json ~/.fuigo/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.fuigo/hooks/bin/
chmod +x ~/.fuigo/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.fuigo/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.fuigo/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.fuigo/hooks/bin/
chmod +x ~/.fuigo/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.fuigo/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.fuigo/hooks/bin
cp examples/hooks/session-log.json ~/.fuigo/hooks/
cp examples/hooks/bin/session-log.sh ~/.fuigo/hooks/bin/
chmod +x ~/.fuigo/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.fuigo/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.fuigo/hooks/bin
cp examples/hooks/tool-logger.json ~/.fuigo/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.fuigo/hooks/bin/
chmod +x ~/.fuigo/hooks/bin/tool-logger.sh
```
...
```sh
mkdir -p ~/.fuigo/hooks/bin
cp examples/hooks/stop-verify.json ~/.fuigo/hooks/
cp examples/hooks/bin/stop-verify.sh ~/.fuigo/hooks/bin/
chmod +x ~/.fuigo/hooks/bin/stop-verify.sh
```
...
- **Event names:** `SessionStart`, `PreToolUse`, `PostToolUse`, `Stop`, `SubagentStop`, `SessionEnd` (see the [user guide](../../fuigo-pager/docs/user-guide/10-hooks.md) for the full set)
- **Matcher:** regex on tool name. Claude names like `Bash`, `Read`, `Edit` are auto-expanded to also match Fuigo names (`run_terminal_cmd`, `read_file`, `search_replace`)
...
Remove the JSON file from `~/.fuigo/hooks/`. The hook stops running on the next session.