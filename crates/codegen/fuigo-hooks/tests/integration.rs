...
use fuigo_hooks::discovery::load_hooks;
use fuigo_hooks::dispatcher;
use fuigo_hooks::event::*;
use fuigo_hooks::result::HookDecision;
use fuigo_hooks::runner::RunContext;
    ...
    let cases = vec![
        ...
        Case {
            ...
            payload: HookPayload::StopFailure {
                error: fuigo_hooks::event::StopFailureKind::RateLimit,
        ...
        Case {
            ...
            payload: HookPayload::StopCancelled {
                reason: fuigo_hooks::event::StopCancelledReason::UserInterrupt,
                cancelled_by: fuigo_hooks::event::CancelledBy::User,
...
/// The reserved keys are `FUIGO_HOOK_EVENT`, `FUIGO_HOOK_NAME`, `FUIGO_SESSION_ID`, `FUIGO_WORKSPACE_ROOT`, and `CLAUDE_PROJECT_DIR`.
...
async fn runner_injected_vars_override_extra_env_at_spawn() {
    ...
    let cmd = format!(
        r#"echo "EVENT=$FUIGO_HOOK_EVENT" > {f}; echo "NAME=$FUIGO_HOOK_NAME" >> {f}; echo "SESSION=$FUIGO_SESSION_ID" >> {f}; echo "ROOT=$FUIGO_WORKSPACE_ROOT" >> {f}; echo "PROJ=$CLAUDE_PROJECT_DIR" >> {f}; echo "USER_KEY=$USER_KEY" >> {f}; echo '{{"decision":"allow"}}'"#,
                    ...
                    "hooks": [
                        {
                            ...
                            "env": {
                                "FUIGO_HOOK_EVENT": "spoofed_event",
                                "FUIGO_HOOK_NAME": "spoofed_name",
                                "FUIGO_SESSION_ID": "spoofed_session",
                                "FUIGO_WORKSPACE_ROOT": "/spoofed/root",
    ...
    assert!(
        ...
        "FUIGO_HOOK_EVENT must reflect the real event, got:\n{captured}"
    );
    assert!(
        !captured.contains("EVENT=spoofed_event"),
        "spoofed FUIGO_HOOK_EVENT must NOT leak through, got:\n{captured}"
    );
    assert!(
        captured.contains(&format!("SESSION={real_session}")),
        "FUIGO_SESSION_ID must reflect the real session, got:\n{captured}"
    );
    assert!(
        !captured.contains("SESSION=spoofed_session"),
        "spoofed FUIGO_SESSION_ID must NOT leak through"
    );
    assert!(
        captured.contains(&format!("ROOT={real_workspace}")),
        "FUIGO_WORKSPACE_ROOT must reflect the real workspace root, got:\n{captured}"
    );
    assert!(
        !captured.contains("ROOT=/spoofed/root"),
        "spoofed FUIGO_WORKSPACE_ROOT must NOT leak through"
    );
    assert!(
