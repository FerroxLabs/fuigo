//! fuigo-chat-state — Actor-based chat state management for Ferrox Labs agents.
//!
//! This crate extracts conversation state management from `fuigo-shell`'s
//! `acp_session.rs` into a standalone actor. It follows the same actor pattern
//! as `fuigo-hunk-tracker`:
//!
//! ```text
//! ┌────────────────┐                  ┌──────────────────────────────────────┐
...
// Re-exported so `fuigo_chat_state::CompactionDetail` stays a working path for
// existing callers.
pub use fuigo_compaction_transcript::CompactionDetail;
