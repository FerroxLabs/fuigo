...
///
/// Stored here (rather than only in `fuigo-shell`) so that query-extraction
...
/// `ActiveSubagentSummary` in fuigo-tools. The mapping between them
/// happens in `run_compact_inner()` (fuigo-shell).
#[derive(Clone)]
...
/// A dependency-free mirror of `TodoStatus` (fuigo-tools), kept here so
...
impl TodoSummaryStatus {
    ...
    /// Mirrors `TodoStatus::tag()` in fuigo-tools.
...
/// handled by the consumer (e.g. `fuigo-shell`), which has access to
...
/// `fuigo-chat-state` and `fuigo-shell` share the same format.
...
pub struct CompactedHistoryInput<'a> {
    ...
    /// When `false` (the default), recent messages come first (fuigo-build
    /// ordering).
    ...
    /// carrier and the fuigo-build summary.
    ...
    /// "Total summaries generated so far …" footer. Ignored by the fuigo-build
...
/// `run_compact_inner` in `fuigo-shell` assembles inline, but is
...
pub fn repair_history(items: &mut Vec<ConversationItem>) -> HistoryRepairReport {
    let duplicates_removed = fuigo_sampling_types::dedup_duplicate_tool_results(items);
    let stripped_tool_result_ids = strip_displaced_tool_results(items);
    let synthetic_results_inserted = fuigo_sampling_types::repair_dangling_tool_calls(
        items,
        fuigo_sampling_types::DanglingToolCallReason::HarnessHalted {
