...
//! lossy summary dropped. In `fuigo-chat-state` so flag resolution and the
//! transcript-hint builder share one definition.

use fuigo_compaction_transcript::CompactionDetail;
...
impl CompactionMode {
    ...
    /// resolver attach the separately-resolved `FUIGO_COMPACTION_DETAIL`.
    ...
    pub fn transcript_hint(self, location: Option<&str>) -> Option<String> {
        use fuigo_compaction_transcript::INDEX_FILE;
        let loc = location?;
        Some(match self {
