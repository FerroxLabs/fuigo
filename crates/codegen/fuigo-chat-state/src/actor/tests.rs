...
async fn provider_counted_model_output_persists_without_bumping_estimate() {
    ...
    h.handle.push_model_output(ConversationItem::Reasoning(
        fuigo_sampling_types::synthesized_reasoning_item("r".repeat(4_000)),
    ));
...
async fn strip_conversation_images_replaces_only_listed_urls_and_persists() {
    ...
    assert!(
        u.content
            .iter()
            .all(|p| !matches!(p, fuigo_sampling_types::ContentPart::Image { .. })),
    ...
    assert!(
        survivor
            .content
            .iter()
            .any(|p| matches!(p, fuigo_sampling_types::ContentPart::Image { .. })),
...
async fn build_request_includes_all_messages() {
    ...
    assert_eq!(request.x_fuigo_conv_id, Some("conv-1".to_string()));
    assert_eq!(request.x_fuigo_req_id, Some("req-1".to_string()));
}
...
/// Simulates the exact sequence that `fuigo-shell`'s `execute_tool_calls`
...
async fn parallel_tool_calls_accept_first_reject_second_skip_third() {
    ...
    let assistant_with_tools =
        ConversationItem::Assistant(fuigo_sampling_types::AssistantItem {
...
async fn get_trailing_assistant_report_skips_reasoning_between_segments() {
    ...
    h.handle.push_tool_result(ConversationItem::Reasoning(
        fuigo_sampling_types::synthesized_reasoning_item("r1"),
    ));
    h.handle
        .push_assistant_response(ConversationItem::assistant("seg1"));
    h.handle
    ...
    h.handle.push_tool_result(ConversationItem::Reasoning(
        fuigo_sampling_types::synthesized_reasoning_item("r2"),
    ));
    h.handle
        .push_assistant_response(ConversationItem::assistant("seg2"));
        ...
        !matches!(
            ...
            Some(ConversationItem::User(u))
                if u.synthetic_reason
                    == Some(fuigo_sampling_types::SyntheticReason::LengthContinue)
        ),
...
async fn get_trailing_assistant_report_joins_later_reminderless_segments() {
    ...
    h.handle.push_tool_result(ConversationItem::Reasoning(
        fuigo_sampling_types::synthesized_reasoning_item("r"),
    ));
    h.handle
...
async fn get_first_user_text_image_first_returns_none() {
    use fuigo_sampling_types::{ContentPart, UserItem};
...
async fn sampling_config_survives_compaction_replacement() {
    use fuigo_sampling_types::ApiBackend;
    ...
    assert_eq!(pre.model, "fuigo-build");
...
/// If that model slug is e.g. "fuigo-build" and not in the ModelState
...
async fn model_metadata_lost_after_compaction_then_recovered_on_next_turn() {
    ...
    h.handle
        .push_assistant_response(ConversationItem::Assistant(
            fuigo_sampling_types::AssistantItem {
...
fn serialize_via_public_api(
    req: &fuigo_sampling_types::ConversationRequest,
) -> serde_json::Value {
    ...
    fuigo_sampling_types::patch_reasoning_text_types(&mut body);
...
fn assert_prefix_stable_pair(
    base: &fuigo_sampling_types::ConversationRequest,
    extended: &fuigo_sampling_types::ConversationRequest,
    label: &str,
) {
...
async fn prefix_stable_after_image_pruning() {
    ...
    let h = TestHarness::with_conversation(vec![
        ConversationItem::system("sys"),
        ConversationItem::User(fuigo_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
    ...
    h.handle
        .push_assistant_response(ConversationItem::assistant("a2"));
    h.handle
        .push_user_message(ConversationItem::User(fuigo_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
                    text: "new image".into(),
                },
...
async fn build_request_budgets_tool_images_on_request_copy_only() {
    use fuigo_sampling_types::{ContentPart, ToolCall};
...
async fn prefix_stable_with_backend_tool_calls() {
    use fuigo_sampling_types::{BackendToolCallItem, BackendToolKind, rs};
