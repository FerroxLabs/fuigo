...
impl ChatStateActor {
    ...
    pub(super) fn truncate_to_prompt_index(&mut self, target_prompt_index: usize) {
        ...
        for (i, item) in self.state.conversation.iter().enumerate() {
            if matches!(item, fuigo_sampling_types::ConversationItem::User(_)) {
    ...
    pub(super) fn check_auto_compact_needed(
        ...
        if fuigo_token_estimation::exceeds_threshold(self.state.total_tokens, cw, threshold_percent) {
            let utilization_percent =
                fuigo_token_estimation::usage_percentage_truncated_u8(self.state.total_tokens, cw);
    ...
    pub(super) fn get_last_model_metadata(&self) -> crate::commands::ModelMetadata {
        self.state
            ...
            .rev()
            .find_map(|item| {
                if let fuigo_sampling_types::ConversationItem::Assistant(a) = item {
    ...
    pub(super) fn has_dangling_tool_calls(&self) -> bool {
        fuigo_sampling_types::has_dangling_tool_calls(&self.state.conversation)
    }
    ...
    pub(super) fn get_trailing_assistant_report(&self) -> Option<String> {
        ...
        for item in items.by_ref() {
            match item {
                fuigo_sampling_types::ConversationItem::Assistant(a)
                    if !a.content.trim().is_empty() =>
                {
                    segments.push(a.content.as_ref());
                    break;
                }
                fuigo_sampling_types::ConversationItem::User(u)
        ...
        for item in items {
            match item {
                ...
                fuigo_sampling_types::ConversationItem::Reasoning(_) => {}
    ...
    ///
    /// [`SyntheticReason::starts_prompt_turn`]: fuigo_sampling_types::SyntheticReason::starts_prompt_turn
    ...
    pub(super) fn get_first_user_text(&self) -> Option<String> {
        self.state.conversation.iter().find_map(|item| {
            if let fuigo_sampling_types::ConversationItem::User(u) = item {
                ...
                u.content.first().and_then(|part| {
                    if let fuigo_sampling_types::ContentPart::Text { text } = part {
    ...
    pub(super) fn get_conversation_item_at(
        &self,
        index: usize,
    ) -> Option<fuigo_sampling_types::ConversationItem> {
    ...
    pub(super) fn get_conversation_counts(&self) -> ConversationCounts {
        ...
        for item in &self.state.conversation {
            match item {
                fuigo_sampling_types::ConversationItem::User(_) => counts.user += 1,
                fuigo_sampling_types::ConversationItem::Assistant(_) => {
                    counts.assistant += 1;
                }
                fuigo_sampling_types::ConversationItem::ToolResult(_) => {
                    counts.tool_result += 1;
                }
                fuigo_sampling_types::ConversationItem::System(_) => {}
                fuigo_sampling_types::ConversationItem::BackendToolCall(_) => {}
                fuigo_sampling_types::ConversationItem::Reasoning(_) => {}
            }
        }
        counts
    ...
    pub(super) fn get_system_message(&self) -> Option<fuigo_sampling_types::ConversationItem> {
        self.state
            .conversation
            .iter()
            .find(|item| matches!(item, fuigo_sampling_types::ConversationItem::System(_)))
            .cloned()
    }
}
