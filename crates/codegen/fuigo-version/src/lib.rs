//! Installed fuigo CLI version, kept in sync with the shipping binaries.

use std::sync::OnceLock;

use semver::Version;

pub const TEST_VERSION_ENV: &str = "FUIGO_TEST_VERSION";

pub const VERSION: &str = match option_env!("FUIGO_VERSION") {
    Some(v) => v,
    None => env!("CARGO_PKG_VERSION"),
};

/// The release pipeline always injects `FUIGO_VERSION`; without it the build is from source.
pub const IS_DEV_BUILD: bool = option_env!("FUIGO_VERSION").is_none();
...
/// `channel_label` is pre-formatted by `fuigo_update::channel_label()`: `" [alpha]"`, `" [stable]"`, or `""` when no pointer is cached.
