...
//! `fuigo-workspace` consumes it cross-crate.
...
/// The markers are `.mcp.json`, `.fuigo/config.toml`, `.claude/settings.json`, the project plugin/agent dirs, and more.
...
impl RepoDirChain {
    ...
    pub fn resolve(cwd: &Path) -> Self {
        let git_root = git2::Repository::discover(cwd)
            ...
            // Otherwise home-level `.fuigo`/`.mcp.json`/plugins would look repo-local
