...
//!
//! Rendering is done by `ToolBridge::render_prompt()` which delegates to `TemplateRenderer` in `fuigo-tools`.
...
pub struct PromptContext {
    ...
    /// Identity in the primary fuigo-build system prompt (`You are <label>…`).
    /// Not the UI picker name. Defaults to [`DEFAULT_SYSTEM_PROMPT_LABEL`].
    #[serde(default = "default_system_prompt_label")]
    pub system_prompt_label: String,
}
/// Default identity on trim-tool-descriptions (`You are Fuigo released by Ferrox Labs`).
pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = "Fuigo";
...
mod tests {
    ...
    fn test_placeholders_system_prompt_label_override() {
        let mut ctx = test_context();
        ctx.system_prompt_label = "Fuigo Internal".into();
        let p = ctx.placeholders();
        assert_eq!(p["system_prompt_label"], "Fuigo Internal");
    }
    #[test]
    ...
    fn test_agents_md_paths_use_display_cwd_in_rendered_section() {
        ...
        let overlay_path = "/root/.fuigo/worktrees/my-project/ab-123-a-overlay";
        let ctx = PromptContext {
