...
//!
//! When `FUIGO_ROOT` and `FUIGO_USER` are set and the resolved directory exists, that path can contribute AGENTS.md / rules / skills discovery.
...
pub fn optional_workspace_user_dir() -> Option<PathBuf> {
    let root = std::env::var("FUIGO_ROOT").ok()?;
    let user = std::env::var("FUIGO_USER").ok()?;
    resolve_workspace_user_dir(&root, &workspace_user_relpath(&user))
}

/// Map `$FUIGO_USER` to a path relative to the workspace root.
///
