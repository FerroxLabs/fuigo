...
//!
//! Searches from cwd to repo root, plus `~/.fuigo/`. Also discovers
//! `*.md` files in rules directories: vendor-prefixed `.fuigo/rules/`,
...
//! (`~/.fuigo/rules/`, `~/.claude/rules/`, `~/.cursor/rules/`).
...
/// Find `*.md` files in `.fuigo/rules/`, `.claude/rules/`, and `.cursor/rules/`, sorted alphabetically.
...
/// Read Agents.md from ~/.fuigo/, git repo root, and session cwd.
///
...
/// `project_trusted` is the folder-trust verdict for `working_directory`; when false, project-scope files are omitted (home scope still loads).
...
/// All vendors count regardless of runtime compat settings, so the folder-trust gate cannot be sidestepped by a compat toggle.
...
async fn read_agents_config_with_roots(
    ...
    add_discovery_root(&mut home_roots, fuigo_home, true, HOME_RULES_DIRS);
    ...
    for (root, is_project) in roots {
        // Folder trust gates project-scope instructions; home-scope roots always load
    ...
    candidates
        ...
        .filter_map(|candidate| {
            ...
            let content = if candidate.is_rule {
                fuigo_tools::implementations::skills::skill::extract_skill_body(&content)
            } else {
                content
            };
...
/// Used by `fuigo-shell` to structurally detect legacy untagged AGENTS.md copies (pre-`SyntheticReason::ProjectInstructions`) on resumed sessions.
pub const LEGACY_AGENTS_MD_REMINDER_PREFIX: &str =
    "\n\n<system-reminder>\nAs you answer the user's questions, you can use the following context";

/// Open/close `system-reminder` (Fuigo) or `system_reminder` (Cursor/IDE), case-insensitive.
...
mod tests {
    ...
    /// Folder trust gates project scope only: an untrusted project omits its AGENTS.md and rules while home-scope rules still load.
    #[tokio::test]
    async fn untrusted_project_instructions_are_omitted() {
        let tmp = tempfile::tempdir().unwrap();
        let fuigo_home = tmp.path().join("fuigo-home");
        let repo = tmp.path().join("repo");
        fs::create_dir_all(fuigo_home.join("rules")).unwrap();
        fs::create_dir_all(repo.join(".fuigo/rules")).unwrap();
        init_git_repo(&repo);
        fs::write(fuigo_home.join("rules/home.md"), "home-rule").unwrap();
        fs::write(repo.join("AGENTS.md"), "project-agents").unwrap();
        fs::write(repo.join(".fuigo/rules/style.md"), "project-rule").unwrap();
        ...
        for trusted in [true, false] {
            ...
            let contents: Vec<&str> = configs
                ...
                .collect();
            assert!(
                contents.contains(&"home-rule"),
                "home scope must always load: {contents:?}"
            );
            for project in ["project-agents", "project-rule"] {
                assert_eq!(
                    contents.contains(&project),
                    trusted,
                    "trusted={trusted} must decide {project}: {contents:?}"
                );
            }
        }
        let chain = crate::repo::RepoDirChain::resolve(&subdir);
        assert!(has_project_instruction_markers_in(
    ...
    async fn home_and_project_rules_have_stable_order_without_doubled_paths() {
        let tmp = tempfile::tempdir().unwrap();
        let fuigo_home = tmp.path().join("custom-fuigo-home");
        ...
        for (path, content) in [
            (fuigo_home.join("rules/b.md"), "fuigo-b"),
            (fuigo_home.join("rules/a.md"), "fuigo-a"),
            (home.join(".claude/rules/a.md"), "claude-a"),
            (home.join(".cursor/rules/a.md"), "cursor-a"),
            (repo.join("AGENTS.md"), "repo-named"),
            (repo.join(".fuigo/rules/a.md"), "repo-fuigo"),
        ...
        for path in [
            fuigo_home.join(".fuigo/rules/doubled.md"),
    ...
    #[tokio::test]
    async fn nested_fuigo_home_keeps_project_role_in_repo_order() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        let nested = repo.join("nested");
        fs::create_dir_all(nested.join("rules")).unwrap();
        fs::create_dir_all(nested.join(".fuigo/rules")).unwrap();
        init_git_repo(&repo);
        ...
        fs::write(nested.join(".fuigo/rules/project.md"), "nested-project-rule").unwrap();
    ...
    #[tokio::test]
    async fn overlapping_fuigo_home_and_project_root_merges_roles() {
        let tmp = tempfile::tempdir().unwrap();
        let repo = tmp.path().join("repo");
        ...
        fs::write(repo.join(".fuigo/rules/project.md"), "project-fuigo-rule").unwrap();
        fs::write(repo.join(".claude/rules/project.md"), "project-claude-rule").unwrap();
        fs::create_dir_all(repo.join(".fuigo/.fuigo/rules")).unwrap();
        fs::write(repo.join(".fuigo/.fuigo/rules/doubled.md"), "doubled").unwrap();
        ...
        .await;
        for expected in ["home-rule", "project-fuigo-rule", "project-claude-rule"] {
            assert_eq!(
                configs
                    .iter()
    ...
    async fn rule_frontmatter_is_stripped_but_named_frontmatter_is_preserved() {
        ...
        for (path, body) in [
            (fuigo_home.join("rules/global.md"), "custom-home-body"),
            (home.join(".claude/rules/global.md"), "claude-body"),
            (home.join(".cursor/rules/global.md"), "cursor-body"),
            (repo.join(".fuigo/rules/project.md"), "fuigo-project-body"),
