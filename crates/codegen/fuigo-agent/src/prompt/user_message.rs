...
//! - `Default`: the legacy Fuigo prefix (built by the shell layer).
...
use fuigo_tools::types::skill_discovery_tracker::{XmlRenderMode, format_announcement_xml};
...
pub enum UserMessageTemplate {
    /// Legacy Fuigo prefix (`<user_info>` and optional `<git_status>`), built directly by the shell layer.
...
/// One discovered rule file (AGENTS.md / Claude.md / .fuigo/rules/*.md).
///
...
pub struct UserMessageContext {
    ...
    /// Callers that need the full string should use `fuigo_shell::util::uname::os_kernel_and_release` (or equivalent).
    ...
    /// User-scoped rule files (~/.fuigo/, ~/.claude/).
    ...
    /// (`~/.fuigo/projects/<encoded-cwd>/mcps`). Surfaced in
...
mod tests {
    ...
    fn format_rules_section_workspace_then_user() {
        ...
        let user = [
            ...
            RuleEntry {
                path: "/home/dev/.fuigo/AGENTS.md".into(),
    ...
    fn format_rules_section_keeps_markdown_heading_off_the_open_tag() {
        let user = [RuleEntry {
            path: "/home/dev/.fuigo/rules/personal.md".into(),
