...
//!
//! A cloned repository could contain plugins (`.fuigo/plugins/`, `.claude/plugins/`) whose hook scripts or MCP server commands run arbitrary code.
//!
...
//!
//! **Trust storage**: `~/.fuigo/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.fuigo/`.
const TRUST_FILE_NAME: &str = fuigo_config::TRUSTED_PLUGINS_FILENAME;
...
impl TrustStore {
    /// If `~/.fuigo/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_fuigo_home() so a project's `.fuigo/trusted-plugins` is never read as the user trust store
        // That happens when neither FUIGO_HOME nor a home dir resolves
        let Some(fuigo) = fuigo_config::user_fuigo_home() else {
            return Self {
                trusted: HashSet::new(),
                file_path: PathBuf::new(),
            };
        };
        let file_path = fuigo.join(TRUST_FILE_NAME);
    ...
    /// Canonicalizes the path and appends it to `~/.fuigo/trusted-plugins`.
    ...
    /// rewrites `~/.fuigo/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.fuigo/trusted-plugins`.
