//! Plugin hooks adapter: pre-filter plugin hook JSON, then feed it to `fuigo-hooks`' parser and inject plugin env vars.

use std::collections::HashMap;
use std::path::{Path, PathBuf};

use fuigo_hooks::config::{HookSpec, parse_hook_file};
...
fn process_hooks_content(
    ...
    // Native `FUIGO_PLUGIN_*` vars plus their vendor-compat aliases.
    let plugin_env: HashMap<String, String> = HashMap::from([
        ("FUIGO_PLUGIN_ROOT".to_string(), plugin_root.to_string()),
        ("CLAUDE_PLUGIN_ROOT".to_string(), plugin_root.to_string()),
        ("FUIGO_PLUGIN_DATA".to_string(), plugin_data.to_string()),
    ...
    for spec in &mut specs {
        ...
        spec.layer = fuigo_hooks::config::HookProvenance::Plugin;
        spec.name = format!(
            "{}{}/{}",
            fuigo_hooks::config::PLUGIN_HOOK_PREFIX,
            plugin_name,
            spec.name
        );
        ...
        if let Some(cmd) = &spec.command {
            ...
            let expanded = fuigo_hooks::config::expand_env_skipping_runner_vars(&substituted);
            if expanded != cmd_str {
...
mod tests {
    ...
    fn parse_plugin_hooks_from_file() {
        ...
        assert_eq!(
            specs[0].extra_env.get("FUIGO_PLUGIN_ROOT").unwrap(),
            "/path/to/plugin"
        );
        assert_eq!(
        ...
        assert_eq!(
            specs[0].extra_env.get("FUIGO_PLUGIN_DATA").unwrap(),
            "/path/to/data"
        );
    ...
    fn parse_plugin_hooks_substitutes_plugin_root_in_command() {
            ...
            "hooks": {
                "PreToolUse": [
                    {"hooks": [
                        ...
                        {"type": "command", "command": "${FUIGO_PLUGIN_ROOT}/hooks/alias.sh"},
        ...
        assert!(
            raws.contains(&"${FUIGO_PLUGIN_ROOT}/hooks/alias.sh"),
            "command_raw must preserve the source string verbatim, got {raws:?}"
        );
        assert!(
    ...
    fn parse_plugin_hooks_user_env_merged_with_plugin_precedence() {
                    ...
                    {"hooks": [
                        {
                            ...
                            "env": {
                                ...
                                "FUIGO_PLUGIN_ROOT": "/user/wins?",
                                "CLAUDE_PLUGIN_DATA": "/user/wins?",
                                "FUIGO_PLUGIN_DATA": "/user/wins?"
                            }
                        }
                    ]}
                ]
        ...
        for (key, expected) in [
            ...
            ("FUIGO_PLUGIN_ROOT", "/actual/plugin/root"),
            ("CLAUDE_PLUGIN_DATA", "/actual/plugin/data"),
            ("FUIGO_PLUGIN_DATA", "/actual/plugin/data"),
        ] {
            assert_eq!(
