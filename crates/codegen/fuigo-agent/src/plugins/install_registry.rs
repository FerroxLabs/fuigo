...
pub struct MarketplaceProvenance {
    ...
    /// Plugin subdirectory within marketplace (e.g., "plugins/fuigo-code-review").
...
impl InstallRegistry {
    ...
    /// 2. Default: `~/.fuigo/installed-plugins/`
    pub fn resolve_install_dir() -> PathBuf {
        if let Some(dir) = Self::read_install_dir_from_config() {
            return dir;
        }

        fuigo_config::fuigo_home().join(DEFAULT_INSTALL_DIR_NAME)
    }
    ...
    fn read_install_dir_from_config() -> Option<PathBuf> {
        ...
        let expanded = if let Some(stripped) = value.strip_prefix("~/") {
            fuigo_dirs::home_dir()?.join(stripped)
        } else {
            PathBuf::from(value)
        };
        Some(expanded)
...
pub enum InstallError {
    ...
    #[error(
        "refusing unpinned remote plugin code for '{plugin}' from {url}: \
         marketplace.require_sha / FUIGO_MARKETPLACE_REQUIRE_SHA is enabled and \
