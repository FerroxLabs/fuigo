//! Agent definition types, parsed from `.fuigo/agents/*.md` files.
...
use fuigo_tools::implementations::codex;
use fuigo_tools::implementations::fuigo_build;
use fuigo_tools::implementations::fuigo_build_concise;
use fuigo_tools::implementations::memory;
use fuigo_tools::implementations::opencode;
use fuigo_tools::implementations::search_tool;
use fuigo_tools::implementations::use_tool;
...
/// Orchestrator-specific prompt body appended to the standard FuigoBuild system prompt (`prompt.md`).
...
fn bash_tool_config() -> ToolConfig {
    ToolConfig::from(&fuigo_build::BashTool)
...
fn task_tool_config() -> ToolConfig {
    ToolConfig::from(&fuigo_build::TaskTool)
...
fn task_output_tool_config() -> ToolConfig {
    ToolConfig::from(&fuigo_build::TaskOutputTool).with_name("get_command_or_subagent_output")
}
/// `wait_tasks` becomes `wait_commands_or_subagents`.
fn wait_tasks_tool_config() -> ToolConfig {
    ToolConfig::from(&fuigo_build::WaitTasksTool).with_name("wait_commands_or_subagents")
}
/// `kill_task` becomes `kill_command_or_subagent`.
fn kill_task_tool_config() -> ToolConfig {
    ToolConfig::from(&fuigo_build::KillTaskTool).with_name("kill_command_or_subagent")
}
/// Complete workspace-executable toolset for hub registration.
///
/// Extends `default_fuigo_build_toolset()` with tools that are dynamically injected by `AgentBuilder::build()` or only available in specific modes.
/// In proxy mode, the workspace server executes ALL tools; the shell has zero local dispatch.
pub fn workspace_fuigo_build_toolset() -> ToolServerConfig {
    let mut tools = default_fuigo_build_toolset().tools;
    tools.push((&opencode::OpenCodeWriteTool).into());
    tools.push((&fuigo_build::EnterPlanModeTool).into());
    tools.push((&fuigo_build::ExitPlanModeTool).into());
    tools.push((&fuigo_build::AskUserQuestionTool).into());
    tools.push((&fuigo_build::WebSearchTool).into());
    tools.push((&fuigo_build::ImageGenTool).into());
    tools.push((&fuigo_build::ImageToVideoTool).into());
    tools.push((&fuigo_build::ReferenceToVideoTool).into());
    tools.push((&fuigo_build::WebFetchTool).into());
    tools.push((&memory::search_tool::MemorySearchImpl).into());
    tools.push((&memory::get_tool::MemoryGetImpl).into());
    tools.push((&fuigo_build::LspTool).into());
    ToolServerConfig {
        tools,
        behavior_preset: None,
    }
}
/// Toolset for the `fuigo-computer` (workspace/sandbox) preset.
fn fuigo_computer_toolset() -> ToolServerConfig {
    #[allow(unused_mut)]
    let mut tools = vec![
        bash_tool_config(),
        (&fuigo_build::ReadFileTool).into(),
        (&fuigo_build::SearchReplaceTool).into(),
        (&opencode::OpenCodeWriteTool).into(),
        (&fuigo_build::ListDirTool).into(),
        (&fuigo_build::GrepTool).into(),
        (&fuigo_build::KillTerminalCommandTool).into(),
        (&fuigo_build::GetTerminalCommandOutputTool).into(),
        (&fuigo_build::SchedulerCreateTool).into(),
        (&fuigo_build::SchedulerDeleteTool).into(),
        (&fuigo_build::SchedulerListTool).into(),
    ];
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn native_toolset_presets() -> Vec<(&'static str, ToolServerConfig)> {
    vec![
        ("fuigo-build", workspace_fuigo_build_toolset()),
        ("fuigo-build-concise", fuigo_build_concise_toolset()),
        ("fuigo-build-plan", fuigo_build_plan_toolset()),
        ("codex", codex_toolset()),
        ("explore", explore_toolset()),
        ("plan", plan_toolset()),
        ("fuigo-computer", fuigo_computer_toolset()),
    ]
}
...
fn default_fuigo_build_toolset() -> ToolServerConfig {
    fuigo_build_core_toolset(true)
}
/// Same as the parent fuigo-build list, without `workflow`.
/// The usual `general-purpose` spawn path must not add that tool and then strip it.
fn general_purpose_toolset() -> ToolServerConfig {
    fuigo_build_core_toolset(false)
}
fn fuigo_build_core_toolset(include_workflow: bool) -> ToolServerConfig {
    let mut tools = vec![
        bash_tool_config(),
        (&fuigo_build::ReadFileTool).into(),
        (&fuigo_build::SearchReplaceTool).into(),
        ...
        (&fuigo_build::SchedulerDeleteTool).into(),
        (&fuigo_build::SchedulerListTool).into(),
        (&fuigo_build::MonitorTool).into(),
        (&search_tool::SearchTool).into(),
        (&use_tool::UseTool).into(),
        (&fuigo_build::UpdateGoalTool).into(),
    ];
    if include_workflow {
        tools.push((&fuigo_build::WorkflowTool).into());
    }
    ToolServerConfig {
        tools,
        behavior_preset: None,
...
fn fuigo_build_concise_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            (&fuigo_build_concise::BashConciseTool).into(),
            (&fuigo_build_concise::ReadFileConciseTool).into(),
            (&fuigo_build_concise::SearchReplaceConciseTool).into(),
            (&fuigo_build::ListDirTool).into(),
            (&fuigo_build::GrepTool).into(),
            kill_task_tool_config(),
            (&fuigo_build::TodoWriteTool).into(),
...
pub fn fuigo_build_hashline_toolset(
    hashline_tools: Vec<fuigo_tools::registry::types::ToolConfig>,
) -> ToolServerConfig {
    let mut tools: Vec<fuigo_tools::registry::types::ToolConfig> = vec![bash_tool_config()];
    ...
    tools.extend([
        ...
        wait_tasks_tool_config(),
        task_tool_config(),
        (&fuigo_build::WebSearchTool).into(),
        (&fuigo_build::SchedulerCreateTool).into(),
        (&fuigo_build::SchedulerDeleteTool).into(),
        (&fuigo_build::SchedulerListTool).into(),
        (&fuigo_build::MonitorTool).into(),
...
/// Extends the default `fuigo-build` toolset with plan mode tools.
/// This allows the agent to enter a structured planning phase before writing code, with user-approved plans.
fn fuigo_build_plan_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard fuigo-build tools
            bash_tool_config(),
            (&fuigo_build::ReadFileTool).into(),
            (&fuigo_build::SearchReplaceTool).into(),
            (&fuigo_build::ListDirTool).into(),
            ...
            (&fuigo_build::WorkflowTool).into(),
            // Plan mode tools
            (&fuigo_build::EnterPlanModeTool).into(),
            (&fuigo_build::ExitPlanModeTool).into(),
            (&fuigo_build::AskUserQuestionTool).into(),
        ],
        behavior_preset: None,
    }
}
...
fn orchestrator_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            ...
            // Web tools
            (&fuigo_build::WebSearchTool).into(),
            (&fuigo_build::WebFetchTool).into(),
            // Imagine
            (&fuigo_build::ImageGenTool).into(),
            (&fuigo_build::ImageToVideoTool).into(),
            (&fuigo_build::ReferenceToVideoTool).into(),
            // Memory
...
/// Same as `fuigo_build_plan_toolset` but excludes `TaskTool`, `TaskOutputTool`, and `KillTaskTool`.
/// Use this when the shell does not have subagent infrastructure wired up.
fn fuigo_build_plan_no_subagents_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            // Standard fuigo-build tools, minus TaskTool only
...
/// Same as `default_fuigo_build_toolset` with the `AskUserQuestionTool` added, allowing the agent to ask structured questions without full plan mode.
fn fuigo_build_ask_user_toolset() -> ToolServerConfig {
    ToolServerConfig {
        tools: vec![
            bash_tool_config(),
            (&fuigo_build::ReadFileTool).into(),
...
pub enum BuiltinAgentName {
    ...
    Plan,
    BrowserUse,
    #[strum(serialize = "fuigo-build-orchestrator")]
    FuigoBuildOrchestrator,
}
...
impl BuiltinAgentName {
    pub fn definition(self) -> AgentDefinition {
        match self {
            Self::FuigoBuild => AgentDefinition::default_fuigo_build(),
            Self::FuigoBuildConcise => AgentDefinition::fuigo_build_concise(),
            Self::FuigoBuildPlan => AgentDefinition::fuigo_build_plan(),
            Self::FuigoBuildPlanNoSubagents => AgentDefinition::fuigo_build_plan_no_subagents(),
            Self::FuigoBuildAskUser => AgentDefinition::fuigo_build_ask_user(),
            ...
            Self::FuigoBuildOrchestrator => AgentDefinition::fuigo_build_orchestrator(),
        }
    }
...
/// Portable agent identity, parsed from .fuigo/agents/*.md.
...
pub struct AgentDefinition {
    ...
    #[serde(default = "default_fuigo_build_toolset")]
    ...
    pub capability_mode: Option<fuigo_tool_types::SubagentCapabilityMode>,
    #[serde(default)]
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub tool_overrides: Option<fuigo_sampling_types::ToolOverrides>,
...
pub enum AgentScope {
    /// .fuigo/agents/ (project-level, highest priority)
    Project,
    /// ~/.fuigo/agents/ (user-level)
    User,
    /// ~/.fuigo/bundled/agents/ (lowest-priority bundled cache)
    Bundled,
    /// Built-in agent (e.g., default_fuigo_build(), browser_use()).
    #[default]
    BuiltIn,
}
impl AgentScope {
...
pub enum MemoryScope {
    /// `~/.fuigo/agent-memory/<name>/`
    User,
    /// `<project>/.fuigo/agent-memory/<name>/`
    Project,
    /// `<project>/.fuigo/agent-memory-local/<name>/`
    Local,
}
impl MemoryScope {
    ...
    pub fn resolve_dir(self, agent_name: &str, project_cwd: &std::path::Path) -> ResolvedMemoryDir {
        match self {
            Self::User => ResolvedMemoryDir {
                path: fuigo_config::fuigo_home()
                    .join("agent-memory")
                    .join(agent_name),
                is_project_scoped: false,
            },
            Self::Project => ResolvedMemoryDir {
                path: project_cwd.join(".fuigo/agent-memory").join(agent_name),
                is_project_scoped: true,
            },
            Self::Local => ResolvedMemoryDir {
                path: project_cwd
                    .join(".fuigo/agent-memory-local")
                    .join(agent_name),
                is_project_scoped: true,
            },
        }
...
/// The `AgentBuilder` maps these into `fuigo_tools::registry::types::ToolsetConfig.bash` which uses the tools crate's `BashToolConfig` type.
#[derive(Debug, Clone, Deserialize)]
#[serde(rename_all = "camelCase")]
...
impl AgentDefinition {
    ...
    fn scope_from_path(path: &Path) -> AgentScope {
        ...
        for (dir, scope) in crate::discovery::user_agent_dirs(home.as_deref(), fuigo.as_deref()) {
            if path.starts_with(&dir) {
                return scope;
            }
        }
        if path_str.contains(".fuigo/agents/") || path_str.contains(".fuigo\\agents\\") {
            return AgentScope::Project;
        }
        if path_str.contains(".fuigo/bundled/agents/")
            || path_str.contains(".fuigo\\bundled\\agents\\")
        {
...
impl AgentDefinition {
    ...
    pub fn include_browser_verification(&self) -> bool {
        matches!(
            self.builtin_name,
            Some(BuiltinAgentName::FuigoBuildPlan | BuiltinAgentName::FuigoBuildPlanNoSubagents)
        )
    }
    ...
    /// Stock `fuigo-build*` agents leave all three at defaults and are non-strict.
    ...
    pub fn override_file_tools(
        &mut self,
        file_tools: Vec<fuigo_tools::registry::types::ToolConfig>,
    ) {
        const FILE_TOOL_SLOTS: &[[&str; 2]] = &[
            ["FuigoBuild:read_file", "FuigoBuildHashline:hashline_read"],
            [
                "FuigoBuild:search_replace",
                "FuigoBuildHashline:hashline_edit",
            ],
            ["FuigoBuild:grep", "FuigoBuildHashline:hashline_grep"],
        ];
    ...
    pub fn builtin_defaults(name: &str, description: &str) -> Self {
        Self {
            ...
            tool_config: default_fuigo_build_toolset(),
    ...
    pub fn default_fuigo_build() -> Self {
        Self::base(
            BuiltinAgentName::FuigoBuild,
            "Fuigo agent for software engineering tasks.",
        )
    }
    /// Concise output format for SFT/RL.
    pub fn fuigo_build_concise() -> Self {
        Self {
            tool_config: fuigo_build_concise_toolset(),
            agents_md: false,
            ..Self::base(
                BuiltinAgentName::FuigoBuildConcise,
                "Fuigo agent with concise output format.",
            )
        }
    }
    pub fn fuigo_build_plan() -> Self {
        Self {
            tool_config: fuigo_build_plan_toolset(),
            ..Self::base(
                BuiltinAgentName::FuigoBuildPlan,
                "Fuigo agent with plan mode support.",
            )
        }
    }
    pub fn fuigo_build_plan_no_subagents() -> Self {
        Self {
            tool_config: fuigo_build_plan_no_subagents_toolset(),
            ..Self::base(
                BuiltinAgentName::FuigoBuildPlanNoSubagents,
                "Fuigo agent with plan mode (no subagents).",
            )
        }
    }
    pub fn fuigo_build_ask_user() -> Self {
        Self {
            tool_config: fuigo_build_ask_user_toolset(),
            ..Self::base(
                BuiltinAgentName::FuigoBuildAskUser,
                "Fuigo agent with ask-user-question tool.",
            )
        }
    }
    pub fn codex() -> Self {
    ...
    pub fn general_purpose() -> Self {
        ...
        Self {
            description: fuigo_tool_types::GENERAL_PURPOSE_SUBAGENT
                .description
                .to_string(),
    ...
    pub fn explore() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: fuigo_tool_types::EXPLORE_SUBAGENT.description.to_string(),
    ...
    pub fn plan() -> Self {
        use crate::prompt::subagent_prompts;
        Self {
            description: fuigo_tool_types::PLAN_SUBAGENT.description.to_string(),
    ...
    /// GBL model with full FuigoBuild tools (skills, MCPs, plan mode) that delegates coding/exploration to subagents.
    ///
    ...
    pub fn fuigo_build_orchestrator() -> Self {
        Self {
            tool_config: orchestrator_toolset(),
            inject_default_tools: false,
            prompt_body: Some(ORCHESTRATOR_PROMPT_BODY.to_string()),
            ..Self::base(
                BuiltinAgentName::FuigoBuildOrchestrator,
                "FuigoBuild orchestrator that delegates coding to specialized subagents",
            )
        }
    }
    ...
    pub fn from_json(value: &serde_json::Value) -> Result<Self, AgentBuildError> {
        ...
        if !value.get("toolConfig").is_some_and(|v| v.is_object()) {
            def.tool_config = default_fuigo_build_toolset();
        }
...
mod tests {
    ...
    fn task_tool_rename_matches_task_tool_id_predicate() {
        ...
        assert!(fuigo_tools::is_task_tool_id(&name));
    }
    ...
    fn bash_tool_rename_matches_writing_tool_kind() {
        ...
        assert_eq!(
            fuigo_tools::tool_taxonomy::writing_tool_kind(&name),
            Some(fuigo_tools::types::tool::ToolKind::Execute)
        );
    }
    /// Native presets only.
    #[test]
    ...
    fn presets_select_distinct_toolsets_by_size() {
        let gb = toolset_for_preset("fuigo-build").unwrap();
    ...
    fn fuigo_computer_exclusive_ids() -> Vec<String> {
        #[allow(unused_mut)]
        let mut ids: Vec<String> = vec![
            ToolConfig::from(&fuigo_build::GetTerminalCommandOutputTool).id,
            ToolConfig::from(&fuigo_build::KillTerminalCommandTool).id,
        ];
        ids
    }
    #[test]
    fn fuigo_computer_preset_is_curated_fuigo_build_subset() {
        let gc = toolset_for_preset("fuigo-computer").unwrap();
        let gb = toolset_for_preset("fuigo-build").unwrap();
        let gb_ids: std::collections::HashSet<&str> =
            gb.tools.iter().map(|t| t.id.as_str()).collect();
        let exclusive_ids = fuigo_computer_exclusive_ids();
        ...
        for t in &gc.tools {
            ...
            assert!(
                ...
                "fuigo-computer tool `{}` must also ship in the fuigo-build preset",
                t.id
            );
        }
        assert!(
            gc.tools.len() < gb.tools.len(),
            "fuigo-computer should be a curated subset of fuigo-build"
        );
    }
    #[test]
    fn fuigo_computer_uses_subagent_free_background_task_tools() {
        let gc = toolset_for_preset("fuigo-computer").unwrap();
        let ids: std::collections::HashSet<&str> = gc.tools.iter().map(|t| t.id.as_str()).collect();
        assert!(
            ids.contains(
                ToolConfig::from(&fuigo_build::GetTerminalCommandOutputTool)
                    .id
                    .as_str()
            )
        );
        assert!(
            ids.contains(
                ToolConfig::from(&fuigo_build::KillTerminalCommandTool)
                    .id
                    .as_str()
            )
        );
        assert!(!ids.contains(ToolConfig::from(&fuigo_build::TaskOutputTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&fuigo_build::KillTaskTool).id.as_str()));
        assert!(!ids.contains(ToolConfig::from(&fuigo_build::TaskTool).id.as_str()));
        for t in &gc.tools {
            if t.id == ToolConfig::from(&fuigo_build::GetTerminalCommandOutputTool).id
                || t.id == ToolConfig::from(&fuigo_build::KillTerminalCommandTool).id
            {
                assert!(t.name_override.is_none(), "tool `{}` must not rename", t.id);
            }
        }
    }
    /// The fuigo-computer preset must ship a full-file write tool (legacy `write_file` parity), the same OpenCode `write` tool fuigo-build uses.
    ...
    #[test]
    fn fuigo_computer_preset_includes_write_tool() {
        let gc = toolset_for_preset("fuigo-computer").unwrap();
        let write_id = ToolConfig::from(&opencode::OpenCodeWriteTool).id;
        assert!(
            gc.tools.iter().any(|t| t.id == write_id),
            "fuigo-computer preset must include the `{write_id}` tool"
        );
    }
    #[test]
    fn fuigo_computer_preset_excludes_plan_and_lsp() {
        let gc = toolset_for_preset("fuigo-computer").unwrap();
        let gc_ids: std::collections::HashSet<&str> =
            gc.tools.iter().map(|t| t.id.as_str()).collect();
        for excluded in [
            ToolConfig::from(&fuigo_build::LspTool).id,
            ToolConfig::from(&fuigo_build::EnterPlanModeTool).id,
            ToolConfig::from(&fuigo_build::ExitPlanModeTool).id,
        ] {
            assert!(
                !gc_ids.contains(excluded.as_str()),
                "fuigo-computer preset must not advertise `{excluded}`"
            );
        }
        let full = workspace_fuigo_build_toolset();
        ...
        for present in [
            ...
            assert!(
                ...
                "workspace_fuigo_build_toolset must ship `{present}`"
            );
        }
    }
    ...
    fn expected_strict_harness(name: BuiltinAgentName) -> bool {
        match name {
            BuiltinAgentName::Codex | BuiltinAgentName::FuigoBuildOrchestrator => true,
            BuiltinAgentName::FuigoBuild
            | BuiltinAgentName::FuigoBuildConcise
            | BuiltinAgentName::FuigoBuildPlan
            | BuiltinAgentName::FuigoBuildPlanNoSubagents
    ...
    fn is_strict_harness_agent_type_classifies_by_name() {
        for strict in ["codex", "fuigo-build-orchestrator"] {
            assert!(
...
"#;
    ...
    fn memory_scope_resolve_dir() {
        ...
        assert_eq!(
            proj.path,
            std::path::PathBuf::from("/project/.fuigo/agent-memory/a")
        );
        assert!(proj.is_project_scoped);
        let local = MemoryScope::Local.resolve_dir("a", cwd);
        assert_eq!(
            local.path,
            std::path::PathBuf::from("/project/.fuigo/agent-memory-local/a")
        );
...
"#;
    ...
    #[test]
    fn test_default_tool_config_has_fuigo_build_tools() {
        let content = r#"---
name: default-tools
...
"#;
        ...
        assert!(
            ...
            "default tool_config should have fuigo_build tools"
        );
    }
    #[test]
    ...
    fn same_name_acp_definition_does_not_enable_browser_verification() {
        let def = AgentDefinition::from_json(&serde_json::json!({
            "name": "fuigo-build-plan",
            "description": "Custom plan agent"
        }))
        .unwrap();
        assert!(!def.include_browser_verification());
        assert!(AgentDefinition::fuigo_build_plan().include_browser_verification());
        assert!(AgentDefinition::fuigo_build_plan_no_subagents().include_browser_verification());
    }
    #[test]
    fn test_from_json_has_default_toolset_with_task_tool() {
        ...
        let def = AgentDefinition::from_json(&json).unwrap();
        let task_tool_id = "FuigoBuild:task";
        assert!(
    ...
    fn test_model_override_in_json() {
        let json = serde_json::json!({
            "name": "test",
            "description": "Test",
            "model": "fuigo-code-fast-1"
        });
        let def = AgentDefinition::from_json(&json).unwrap();
        assert_eq!(
            def.model,
            ModelOverride::Override("fuigo-code-fast-1".to_string())
        );
    }
    #[test]
    fn test_builtin_agent_name_strum_round_trip() {
        use std::str::FromStr;
        for (s, expected) in [
            ("fuigo-build", BuiltinAgentName::FuigoBuild),
            ("fuigo-build-concise", BuiltinAgentName::FuigoBuildConcise),
            ("fuigo-build-ask-user", BuiltinAgentName::FuigoBuildAskUser),
    ...
    fn def_with_template(tpl: crate::prompt::context::TemplateOverride) -> AgentDefinition {
        let mut def = AgentDefinition::default_fuigo_build();
