...
use std::sync::Arc;
use fuigo_tools::bridge::ToolBridge;
...
/// The Fuigo [`ToolKind`] a vendor-compat `tools:` allowlist entry resolves to, so a plugin's upstream allowlist still binds.
/// Backed by the shared vendor-to-Fuigo tool registry in `fuigo-tools` (also used by the hook matcher).
fn claude_tool_kind(name: &str) -> Option<ToolKind> {
    fuigo_tools::types::kind_for(name)
}
...
pub struct AgentBuilder {
    ...
    /// path (e.g., `~/.fuigo/worktrees/project/fork-...-overlay`) that must stay
    ...
    memory_backend: Option<Arc<dyn fuigo_tools::types::memory_backend::MemoryBackend>>,
    ...
    backend_search: bool,
    web_fetch_config: fuigo_tools::implementations::fuigo_build::web_fetch::WebFetchConfig,
    lsp: Option<std::sync::Arc<dyn fuigo_tools::implementations::lsp::LspBackend>>,
    image_gen_config: fuigo_tools::implementations::fuigo_build::image_gen::ImageGenConfig,
    video_gen_config: fuigo_tools::implementations::fuigo_build::video_gen::VideoGenConfig,
    app_builder_deployer_config:
        fuigo_tools::implementations::fuigo_build::app_builder::AppBuilderDeployerConfig,
    ...
    attribution_callback: Option<fuigo_tools::SharedAttributionCallback>,
    ...
    /// See `resolve_max_mcp_output_bytes_for_cwd` in fuigo-shell.
...
fn ensure_plan_mode_tools(tool_config: &mut fuigo_tools::registry::types::ToolServerConfig) {
    ...
    if missing_enter {
        tool_config
            .tools
            .push((&fuigo_build::EnterPlanModeTool).into());
    }
    if missing_exit {
        tool_config
            .tools
            .push((&fuigo_build::ExitPlanModeTool).into());
    }
    if missing_ask {
        tool_config
            .tools
            .push((&fuigo_build::AskUserQuestionTool).into());
    }
}
...
impl AgentBuilder {
    pub fn new(
        ...
        Self {
            ...
            terminal_backend,
            fs_backend: Arc::new(fuigo_tools::computer::local::LocalFs),
            notification_handle,
    ...
    pub fn with_preloaded_skills(
        mut self,
        skills: Vec<fuigo_tools::implementations::skills::types::SkillInfo>,
    ) -> Self {
    ...
    pub fn with_memory_backend(
        mut self,
        backend: Arc<dyn fuigo_tools::types::memory_backend::MemoryBackend>,
    ) -> Self {
    ...
    pub fn with_parent_scheduler_handle(
        mut self,
        handle: fuigo_tools::implementations::fuigo_build::scheduler::types::SchedulerHandle,
    ) -> Self {
    ...
    pub fn with_web_search_config(
        mut self,
        config: fuigo_tools::implementations::web_search::WebSearchConfig,
    ) -> Self {
    ...
    /// Feature-flagged via remote settings `web_fetch_enabled` and `FUIGO_WEB_FETCH` env var.
    pub fn with_web_fetch_config(
        mut self,
        config: fuigo_tools::implementations::fuigo_build::web_fetch::WebFetchConfig,
    ) -> Self {
    ...
    pub fn with_lsp(
        mut self,
        handle: std::sync::Arc<dyn fuigo_tools::implementations::lsp::LspBackend>,
    ) -> Self {
    ...
    /// The tool calls the Ferrox Labs Imagine API with session credentials.
    /// When `Disabled` (default), the tool is not registered.
    pub fn with_image_gen_config(
        mut self,
        config: fuigo_tools::implementations::fuigo_build::image_gen::ImageGenConfig,
    ) -> Self {
    ...
    /// The tool calls the Ferrox Labs Video Generation API with session credentials.
    /// When `Disabled` (default), the tool is not registered.
    pub fn with_video_gen_config(
        mut self,
        config: fuigo_tools::implementations::fuigo_build::video_gen::VideoGenConfig,
    ) -> Self {
    ...
    pub fn with_app_builder_deployer_config(
        mut self,
        config: fuigo_tools::implementations::fuigo_build::app_builder::AppBuilderDeployerConfig,
    ) -> Self {
    ...
    pub fn with_api_key_provider(
        mut self,
        provider: fuigo_tools::types::SharedApiKeyProvider,
    ) -> Self {
    ...
    /// Set public model slugs advertised in the FuigoBuild Task description.
    ...
    /// Otherwise, when disabled, `FuigoBuild:ask_user_question` is stripped from the agent's tool config after `ensure_plan_mode_tools` injection.
    ...
    pub async fn build(mut self) -> Result<Agent, AgentBuildError> {
        ...
        if definition.inject_default_tools {
            ...
            if self.web_search_config.is_enabled() {
                use fuigo_tools::implementations::fuigo_build;
                tool_config.tools.push((&fuigo_build::WebSearchTool).into());
            }
            if self.web_fetch_config.is_enabled() {
                use fuigo_tools::implementations::fuigo_build;
                tool_config.tools.push((&fuigo_build::WebFetchTool).into());
            }
            if self.lsp.is_some() {
                tool_config
                    .tools
                    .push((&fuigo_tools::implementations::fuigo_build::LspTool).into());
            }
            if self.image_gen_config.image_gen_enabled() {
                tool_config
                    .tools
                    .push((&fuigo_tools::implementations::fuigo_build::ImageGenTool).into());
            }
            if self.image_gen_config.image_edit_enabled() {
                tool_config
                    .tools
                    .push((&fuigo_tools::implementations::fuigo_build::ImageEditTool).into());
            }
            if self.video_gen_config.is_enabled() {
                tool_config
                    .tools
                    .push((&fuigo_tools::implementations::fuigo_build::ImageToVideoTool).into());
                tool_config.tools.push(
                    (&fuigo_tools::implementations::fuigo_build::ReferenceToVideoTool).into(),
                );
            }
        ...
        >();
        let is_active_agent_message = |tool: &fuigo_tools::registry::types::ToolConfig| {
        ...
        if self.memory_backend.is_none() {
            let fuigo_build_ns = fuigo_tools::types::tool::ToolNamespace::FuigoBuild.to_string();
            let mem_search_id = format!(
                "{fuigo_build_ns}:{}",
                fuigo_tools::implementations::memory::MEMORY_SEARCH_TOOL_NAME
            );
            let mem_get_id = format!(
                "{fuigo_build_ns}:{}",
                fuigo_tools::implementations::memory::MEMORY_GET_TOOL_NAME
            );
            tool_config
                .tools
        ...
        if self.prompt_audience == crate::prompt::context::PromptAudience::Subagent {
            tool_config
                .tools
                .retain(|tool| tool.kind != Some(fuigo_tools::types::tool::ToolKind::AskUser));
        ...
        if task_stripped {
            use fuigo_tools::types::tool::ToolNamespace;
            ...
            if !has_satisfier(ToolNamespace::FuigoBuild, "run_terminal_cmd", true)
                && !has_satisfier(ToolNamespace::FuigoBuildConcise, "run_terminal_cmd", true)
        ...
        if let fuigo_tools::implementations::fuigo_build::web_fetch::WebFetchConfig::Enabled {
            ref params,
        } = self.web_fetch_config
            && let Ok(params_value) = serde_json::to_value(params)
            && let Some(obj) = params_value.as_object()
        {
            merge_tool_params(&mut tool_config, &["FuigoBuild:web_fetch"], obj);
        }
        if let Some(ref bash_params) = self.bash_params_json {
            merge_tool_params(
                ...
                &[
                    ...
                    "FuigoBuildConcise:run_terminal_cmd",
                ],
                bash_params,
            );
        }
        ...
        if self.is_non_interactive {
            ...
            merge_tool_params(&mut tool_config, &["FuigoBuild:ask_user_question"], &ni);
        }
        ...
        if self.prompt_audience == crate::prompt::context::PromptAudience::Subagent {
            tool_config.tools.retain(|tool| {
                !fuigo_tools::implementations::fuigo_build::is_workflow_tool(tool.kind, &tool.id)
            });
        }
        ...
        if let Some(bytes) = self.mcp_max_output_bytes {
            tool_bridge.toolset().resources.lock().await.insert(
                fuigo_tools::types::resources::TruncationCfg(
        ...
        if use_backend_search {
            if web_search_enabled && definition.hosted_tool_allowed("web_search") {
                hosted_tools.push(fuigo_sampling_types::HostedTool::WebSearch { options: None });
            }
            if definition.hosted_tool_allowed("x_search") {
                hosted_tools.push(fuigo_sampling_types::HostedTool::XSearch { options: None });
            }
            fuigo_sampling_types::apply_tool_overrides(
                &mut hosted_tools,
...
const TASK_TOOL_NAMING: fuigo_tool_types::TaskToolNaming<'static> = fuigo_tool_types::TaskToolNaming {
...
${{ params.task.run_in_background }}: defaults to false (the call blocks and returns the child's report). \n\
Set it true only for independent work you can continue alongside; collect the report later with the task output tool using one long timeout_ms.";
...
const SUBAGENT_TOOL_NAMING: fuigo_tool_types::SubagentToolNaming<'static> =
    fuigo_tool_types::SubagentToolNaming {
...
fn builtin_tools_fragment(name: BuiltinAgentName) -> String {
    let subagent = match name {
        ...
        BuiltinAgentName::Explore => fuigo_tool_types::EXPLORE_SUBAGENT,
...
///
/// Maps each [`SubagentEntry`] to the shared [`fuigo_tool_types::SubagentDescriptor`].
/// Defers to [`fuigo_tool_types::build_task_description`] so the CLI and the prod chat stack share one builder.
...
pub(crate) fn build_task_description(
    ...
    let descriptors: Vec<fuigo_tool_types::SubagentDescriptor> = subagents
        .iter()
        .map(|entry| {
            ...
            fuigo_tool_types::SubagentDescriptor {
        ...
        .collect();
    let mut description = fuigo_tool_types::build_task_description(&descriptors, &TASK_TOOL_NAMING);
...
mod tests {
    ...
    async fn active_agent_message_tool_count(enabled: Option<bool>, predeclared: bool) -> usize {
        ...
        let mut definition = crate::config::AgentDefinition::default_fuigo_build();
        if predeclared {
    ...
    async fn active_agent_messages_are_absent_from_child_toolsets() {
        ...
        definition
            .tool_config
            .tools
            .push(fuigo_tools::registry::types::ToolConfig::for_tool::<
                fuigo_tools::implementations::fuigo_build::SendSubagentMessageTool,
            >());
    ...
    fn entry(name: &str, desc: &str, source: SubagentSource) -> SubagentEntry {
        SubagentEntry {
            ...
            config_source: fuigo_tools::types::config_source::ConfigSource::Builtin,
        }
    }
    #[test]
    ...
    fn build_task_description_shadowed_builtin_uses_user_desc() {
        let subagents = vec![SubagentEntry {
            ...
            config_source: fuigo_tools::types::config_source::ConfigSource::Project {
    ...
    async fn discovery_snapshot_records_gated_and_preloaded_skills() {
        ...
        let write_skill = |dir: &str, content: &str| {
            let d = tmp.path().join(".fuigo/skills").join(dir);
    ...
    async fn pager_flag_combinations_satisfy_tool_invariants() {
        ...
        let cases: &[PagerFlagCase] = &[
            PagerFlagCase {
                label: "fuigo-build / subagents+ask_user",
                profile: AgentDefinition::default_fuigo_build,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "fuigo-build / subagents / no-ask-user",
                profile: AgentDefinition::default_fuigo_build,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "fuigo-build / no-subagents / ask_user",
                profile: AgentDefinition::default_fuigo_build,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "fuigo-build / no-subagents / no-ask-user",
                profile: AgentDefinition::default_fuigo_build,
                subagents: false,
                ask_user: false,
            },
            PagerFlagCase {
                label: "fuigo-build-ask-user / subagents",
                profile: AgentDefinition::fuigo_build_ask_user,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "fuigo-build-ask-user / no-subagents",
                profile: AgentDefinition::fuigo_build_ask_user,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "fuigo-build-plan",
                profile: AgentDefinition::fuigo_build_plan,
                subagents: true,
                ask_user: true,
            },
            PagerFlagCase {
                label: "fuigo-build-plan / no-ask-user",
                profile: AgentDefinition::fuigo_build_plan,
                subagents: true,
                ask_user: false,
            },
            PagerFlagCase {
                label: "fuigo-build-plan-no-subagents",
                profile: AgentDefinition::fuigo_build_plan_no_subagents,
                subagents: false,
                ask_user: true,
            },
            PagerFlagCase {
                label: "fuigo-build-plan-no-subagents / no-ask-user",
                profile: AgentDefinition::fuigo_build_plan_no_subagents,
                subagents: false,
                ask_user: false,
            },
    ...
    async fn subagent_audience_never_receives_ask_user_question() {
        ...
        .from_definition(crate::config::AgentDefinition::fuigo_build_ask_user())
    ...
    async fn top_level_session_still_receives_workflow() {
        let names = workflow_tool_names(
            ...
            crate::config::AgentDefinition::default_fuigo_build(),
        )
        .await;
        assert!(
    ...
    async fn custom_child_toolset_cannot_reintroduce_workflow() {
        use fuigo_tools::implementations::fuigo_build::{ReadFileTool, WorkflowTool};
        let mut definition = crate::config::AgentDefinition::general_purpose();
        definition.inject_default_tools = false;
        definition.tool_config.tools = vec![
            (&ReadFileTool).into(),
            (&WorkflowTool).into(),
            fuigo_tools::registry::types::ToolConfig::from_id("FuigoBuild:workflow"),
        ];
        let names =
    ...
    async fn curated_empty_toolset_fails_agent_build() {
        use fuigo_tools::computer::local::LocalTerminalBackend;
        use fuigo_tools::notification::ToolNotificationHandle;
        let mut profile = crate::config::AgentDefinition::default_fuigo_build();
    ...
    async fn plan_mode_injected_ask_user_question_receives_params() {
        ...
        let profile = crate::config::AgentDefinition::default_fuigo_build();
        assert!(
            !profile
                .tool_config
                .tools
                .iter()
                .any(|tc| tc.id == "FuigoBuild:ask_user_question"),
    ...
    async fn non_interactive_build_stamps_ask_user_question_params() {
        ...
        .from_definition(crate::config::AgentDefinition::default_fuigo_build())
    ...
    async fn build_with_tools(tools: Vec<String>, disallowed: Vec<String>) -> crate::agent::Agent {
        use fuigo_tools::computer::local::LocalTerminalBackend;
        use fuigo_tools::notification::ToolNotificationHandle;
        let mut def = crate::config::AgentDefinition::default_fuigo_build();
        def.tools = tools;
    ...
    fn session_tools_allowed_clamp() {
        ...
        assert!(def.session_tools_allowed("FuigoBuild:read_file"));
    ...
    async fn spawning_blocked_disables_all_background_bash_modes() {
        use fuigo_tools::computer::local::LocalTerminalBackend;
        use fuigo_tools::implementations::fuigo_build::bash::BashParams;
        use fuigo_tools::notification::ToolNotificationHandle;
        use fuigo_tools::types::resources::Params;
        let mut definition = crate::config::AgentDefinition::default_fuigo_build();
    ...
    /// Compat allowlist names (`Read`, `Bash`, `Grep`) map to their Fuigo equivalents by `ToolKind`: a real restricted toolset, not zero tools.
    #[tokio::test]
    async fn claude_tool_names_map_to_fuigo_equivalents() {
    ...
    /// Shell, LSP, ask, and task-lifecycle tool names resolve to their fuigo `ToolKind`, so those allowlists are honored instead of failing open.
    #[test]
    ...
    /// Fuigo keeps the MCP meta-tools (`search_tool`/`use_tool`) regardless of what the allowlist names.
    #[tokio::test]
    ...
    async fn requested_enabled_web_tools_survive_allowlist() {
        use fuigo_tools::computer::local::LocalTerminalBackend;
        use fuigo_tools::implementations::fuigo_build::web_fetch::WebFetchConfig;
    ...
    /// fuigo-build toolsets have no Skill tool; skills are read from `SKILL.md` via `read_file`.
    ...
    /// Compat `ToolSearch` meta-tool maps to fuigo's `search_tool` (MCP discovery).
    ...
    async fn build_with_web_search(
        ...
        tool_overrides: Option<fuigo_sampling_types::ToolOverrides>,
    ) -> crate::agent::Agent {
        use fuigo_tools::computer::local::LocalTerminalBackend;
        use fuigo_tools::implementations::web_search::WebSearchConfig;
        use fuigo_tools::notification::ToolNotificationHandle;
    ...
    async fn disallowed_web_search_strips_function_and_hosted_tools() {
        ...
        assert!(
            !hosted
                .iter()
                .any(|t| matches!(t, fuigo_sampling_types::HostedTool::WebSearch { .. })),
            "hosted WebSearch must be removed when web_search is disallowed, got: {hosted:?}"
        );
        assert!(
            hosted
                .iter()
                .any(|t| matches!(t, fuigo_sampling_types::HostedTool::XSearch { .. })),
    ...
    async fn hosted_tools_bake_definition_tool_overrides_into_options() {
        let x_search = fuigo_sampling_types::XSearchOptions {
            date_bound: Some(
                fuigo_sampling_types::SearchDateBound::new(None, Some("2024-03-15".into()))
                    .unwrap(),
            ),
        };
        let agent = build_with_web_search(
            ...
            true,
            &[],
            Some(fuigo_sampling_types::ToolOverrides {
        ...
        assert!(
            agent
                .hosted_tools()
                .contains(&fuigo_sampling_types::HostedTool::XSearch {
