...
This crate extracts a first-class `Agent` type from `fuigo-shell`.
...
`fuigo-shell`, another in-process host, or a headless batch runner.
...
in `.fuigo/agents/` (project-level) or `~/.fuigo/agents/` (user-level).

```rust
use fuigo_agent::{AgentDefinition, AgentBuilder};
use fuigo_tools::notification::ToolNotificationHandle;

// 1. Parse the definition file
let def = AgentDefinition::from_file(".fuigo/agents/code-reviewer.md")?;
...
```rust
use fuigo_agent::discovery;

// Find all .md files in .fuigo/agents/ directories
...
└────────▲─────────┘
         │ depends on
┌────────┴─────────┐
│  fuigo-shell  │  uses AgentBuilder to create
│  (session host)  │  Agent during session setup
└──────────────────┘
```

- **`fuigo-tools`**: Provides `ToolBridge`, `ToolRegistry`,
  `ToolState`, `SystemReminderLayer`, and tool implementations.
  `fuigo-agent` depends on it for tool setup.
- **`fuigo-shell`**: The application shell. Uses `AgentBuilder`
  to construct an `Agent` during session creation. The shell
  re-exports some modules from `fuigo-agent` (AGENTS.md
...
|---|---|---|
| `fuigo-build` | extend | Default agent for software engineering tasks |
...
```bash
# Check
cargo check -p fuigo-agent

# Test
cargo test -p fuigo-agent

# Clippy
cargo clippy -p fuigo-agent --fix --allow-dirty
