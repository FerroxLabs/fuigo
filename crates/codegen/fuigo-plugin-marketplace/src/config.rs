//! Parse marketplace sources from `~/.fuigo/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// Tighten-only: either `[marketplace] require_sha = true` in config.toml or `FUIGO_MARKETPLACE_REQUIRE_SHA=1` enables it; neither can turn it off.
...
pub fn env_require_sha() -> bool {
    fuigo_config::env_bool("FUIGO_MARKETPLACE_REQUIRE_SHA").unwrap_or(false)
}
...
/// instead of `~/.fuigo`/`~/.claude`. Each root is checked for
...
mod tests {
    ...
    /// Serializes every test that touches the process-global `FUIGO_MARKETPLACE_REQUIRE_SHA`, so they cannot race each other.
    ...
    fn require_sha_policy_composition() {
        ...
        unsafe { std::env::remove_var("FUIGO_MARKETPLACE_REQUIRE_SHA") };
        assert!(!load_require_sha(&empty), "absent everywhere → off");
        assert!(load_require_sha(&enabled), "config alone can enable");

        unsafe { std::env::set_var("FUIGO_MARKETPLACE_REQUIRE_SHA", "1") };
        assert!(load_require_sha(&empty), "env alone can enable");

        unsafe { std::env::set_var("FUIGO_MARKETPLACE_REQUIRE_SHA", "0") };
        assert!(
