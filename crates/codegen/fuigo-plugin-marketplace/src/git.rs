...
//! Cache root: `~/.fuigo/marketplace-cache/<url-hash>/`
...
pub fn sync_source_cache_with_mode(
    ...
    let url = fuigo_agent::plugins::git_install::validate_git_url(url)?;
    let branch = branch
        .map(fuigo_agent::plugins::git_install::validate_git_ref)
        .transpose()?;
...
pub fn default_cache_root() -> PathBuf {
    fuigo_config::fuigo_home().join("marketplace-cache")
}
...
pub use fuigo_tty_utils::{GIT_AUTH_SUPPRESSION_ENVS, git_command, git_command_locking};
...
fn run_git_timed(cmd: &mut Command, what: &str, timeout: Duration) -> Result<(), String> {
    ...
    match fuigo_tty_utils::wait_child_bounded(&mut child, timeout) {
        ...
        Err(error) => {
            let identity = if fuigo_tty_utils::is_child_wait_identity_uncertain(&error) {
...
fn finish_stderr_or_reap(what: &str, mut stderr: StderrReader) -> Option<Vec<u8>> {
    match stderr.finish_bounded(fuigo_tty_utils::KILL_REAP_TIMEOUT) {
...
struct GitReaperOwners {
    child: Option<(Child, Option<Arc<fuigo_tty_utils::ProcessGroup>>)>,
    stderr: Option<StderrReader>,
}
...
fn transfer_git_cleanup(what: &str, mut owners: GitReaperOwners) {
        ...
        && let Some((child, group)) = owners.child.take()
    {
        if let Err((error, child, group)) =
            fuigo_tty_utils::spawn_child_reaper("marketplace-git-reaper", child, group)
        {
...
mod tests {
    ...
    #[test]
    fn default_cache_root_under_fuigo() {
