...
//!
//! Directory precedence mirrors `index::load_index`: `.fuigo-plugin/plugin-index.json` is preferred, then `.claude-plugin/plugin-index.json`.
...
use fuigo_hooks_plugins_types::PluginComponents;
...
mod tests {
    ...
    #[test]
    fn load_catalog_parses_fuigo_plugin_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".fuigo-plugin", BASIC);
    ...
    #[test]
    fn load_catalog_prefers_fuigo_dir_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".fuigo-plugin", BASIC);
        write_catalog(
            dir.path(),
    ...
    fn load_catalog_malformed_returns_none() {
        let dir = tempfile::tempdir().unwrap();
        write_catalog(dir.path(), ".fuigo-plugin", "not json");
        assert!(load_catalog(dir.path()).is_none());
    }
