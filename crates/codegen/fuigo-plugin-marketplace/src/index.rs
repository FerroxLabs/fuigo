...
//!
//! Catalog files are probed in order: `.fuigo-plugin/marketplace.json` (preferred), then `.fuigo-plugin/plugin.json`.
...
///
/// Checks (in order):
/// 1. `.fuigo-plugin/marketplace.json` (preferred Ferrox Labs convention)
/// 2. `.fuigo-plugin/plugin.json`
...
mod tests {
    ...
    #[test]
    fn load_index_valid_fuigo_dir() {
        let dir = tempfile::tempdir().unwrap();
        let fuigo_dir = dir.path().join(".fuigo-plugin");
        std::fs::create_dir_all(&fuigo_dir).unwrap();
        std::fs::write(
            fuigo_dir.join("marketplace.json"),
            r#"{"name": "fuigo", "plugins": []}"#,
        )
        .unwrap();
        let result = load_index(dir.path()).unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap().name, "fuigo");
    }

    #[test]
    fn load_index_fuigo_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".fuigo-plugin", "fuigo"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "fuigo");
    }
