...
//! 1. **Indexed:** if an index file exists, use it (see `index::load_index` for lookup order; `.fuigo-plugin/marketplace.json` is preferred).
...
fn scan_single_plugin(plugin_dir: &Path, relative_path: &str) -> MarketplaceEntry {
    ...
    let manifest_result = fuigo_agent::plugins::manifest::load_manifest(plugin_dir);
    let manifest = match &manifest_result {
        Ok(fuigo_agent::plugins::manifest::ManifestLoadResult::Found(m)) => Some(m.as_ref()),
        _ => None,
    };
...
mod tests {
    ...
    fn url_sourced_entry_carries_keywords() {
        let dir = tempfile::tempdir().unwrap();
        let fuigo_dir = dir.path().join(".fuigo-plugin");
        std::fs::create_dir_all(&fuigo_dir).unwrap();
        std::fs::write(
            fuigo_dir.join("marketplace.json"),
            r#"{
                "name": "kw-marketplace",
                "plugins": [{
                    "name": "remote-plugin",
    ...
    #[test]
    fn fuigo_plugin_dir_index_drives_scan_end_to_end() {
        let dir = tempfile::tempdir().unwrap();
        make_plugin(dir.path(), "fuigo-plugin", "1.0.0");
        ...
        std::fs::write(
            fuigo_dir.join("marketplace.json"),
            r#"{
                "name": "fuigo-marketplace",
                "plugins": [{
                    "name": "fuigo-plugin",
                    "description": "From the .fuigo-plugin index",
                    "category": "design",
                    "source": { "type": "local", "path": "./plugins/fuigo-plugin" },
                    "tags": ["fuigo"]
                }]
            }"#,
        )
        ...
        let plugins = scan_marketplace(dir.path()).entries;
        assert_eq!(plugins.len(), 1);
        assert_eq!(plugins[0].name, "fuigo-plugin");
        assert_eq!(plugins[0].category.as_deref(), Some("design"));
        assert_eq!(plugins[0].tags, vec!["fuigo"]);
        assert!(plugins[0].keywords.is_empty());
    }
    ...
    fn write_fuigo_file(dir: &Path, file: &str, content: &str) {
        let fuigo_dir = dir.join(".fuigo-plugin");
        std::fs::create_dir_all(&fuigo_dir).unwrap();
        std::fs::write(fuigo_dir.join(file), content).unwrap();
    }
    ...
    fn url_entry_gets_components_when_sha_matches() {
        ...
        write_fuigo_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn url_entry_without_pinned_sha_gets_no_components() {
        let dir = tempfile::tempdir().unwrap();
        write_fuigo_file(dir.path(), "marketplace.json", &url_marketplace_index(""));
        write_fuigo_file(dir.path(), "plugin-index.json", URL_CATALOG);
    ...
    fn malformed_catalog_degrades_to_no_components() {
        ...
        write_fuigo_file(dir.path(), "plugin-index.json", "not json");
