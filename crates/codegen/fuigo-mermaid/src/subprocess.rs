...
//!
//! The caller builds the [`Command`] (stdio, env, and the TTY/console detach via `fuigo_tty_utils::detach_std_command`).
...
///
/// `fuigo_tty_utils::detach_std_command` runs `setsid` (EPERM fallback `setpgid(0,0)`).
/// The child is therefore its own group leader and its pgid equals its pid.
/// We send the signal directly because `fuigo_tty_utils::ProcessGroup` only wraps tokio children.
#[cfg(unix)]
