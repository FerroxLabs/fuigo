...
use fuigo_workspace::file_system::TargetClientId;
...
/// Mirrors `fuigo_shell::extensions::routing::send_routed_notification` so this crate does not depend on the shell.
...
const NOTIFICATION_METHOD: &str = "fuigo/terminal/pty/notification";
...
enum Shell {
    Running {
        ...
        group: Option<Arc<fuigo_tty_utils::ProcessGroup>>,
    },
...
impl Shell {
    ...
    fn attach_group(&mut self, enrolled: Arc<fuigo_tty_utils::ProcessGroup>) {
    ...
    fn reap_now(&mut self) {
        if self.hangup() && self.wait_exit(fuigo_tty_utils::HANGUP_GRACE) {
            return;
        }
        self.kill();
...
pub async fn create_pty(
    ...
    if let Some(pid) = shell.pid() {
        ...
        let enrolled = tokio::task::spawn_blocking(move || {
            fuigo_tty_utils::global_process_scope().enroll_terminal_pid(pid)
        })
        .await
    ...
    if fuigo_tty_utils::global_process_scope().is_closed() {
...
fn reap(entry: &Arc<Mutex<PtySession>>) {
    ...
    if hung_up && wait_for_exit(entry, fuigo_tty_utils::HANGUP_GRACE) {
        return;
    }
...
fn resolve_pty_shell(shell: Option<&str>) -> (String, Vec<String>) {
    ...
    #[cfg(not(unix))]
    {
        use fuigo_config::shell::{WindowsShell, detect_windows_shell};
