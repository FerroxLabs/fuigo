//! `fuigo update` is a recovery command: a config failure must not block it.
//!
...
/// Run `fuigo update` in an isolated home against the local pointer base.
fn run_update(base: &str, config_toml: &str, extra_args: &[&str]) -> std::process::Output {
    ...
    Command::new(pager_binary())
        ...
        .env("FUIGO_CLI_BASE_URL", base)
        .output()
        .expect("spawn fuigo update")
}
...
fn corrupt_config_never_changes_update_outcome() {
    ...
    *body.lock().unwrap_or_else(|e| e.into_inner()) = current;
    ...
    assert!(
        ...
        "healthy fuigo update against the local base must exit 0\nstdout:\n{}\nstderr:\n{}",
    ...
    assert!(
        ...
        "a corrupt config.toml must not block fuigo update\nstdout:\n{}\nstderr:\n{}",
