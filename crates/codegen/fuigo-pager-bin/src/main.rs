...
/// True when this command later boots an agent (`spawn_fuigo_shell` / agent subcommand) that heals managed policy after `apply_sandbox`.
...
use std::env;
use fuigo_update::{UpdateConfig, auto_update, enforce_version_policy_or_exit};
...
fn apply_agent_endpoint_args(
    agent_args: &fuigo_pager::app::AgentArgs,
    config: &mut AgentConfig,
) {
    if let Some(v) = &agent_args.cli_chat_proxy_base_url {
        config.endpoints.cli_chat_proxy_base_url = Some(v.clone());
    }
    if let Some(v) = &agent_args.fuigo_api_base_url {
        config.endpoints.fuigo_api_base_url = v.clone();
    }
}
...
fn init_tracing_simple(app_entrypoint: &'static str) {
    ...
    let registry = tracing_subscriber::registry()
        ...
        .with(fuigo_telemetry::span_profile::layer(app_entrypoint))
        .with(fuigo_telemetry::instrumentation::layer())
        .with(fuigo_telemetry::hooks_log::layer())
        .with(fuigo_telemetry::otel_layer::build_otel_layer(
            fuigo_telemetry::otel_layer::OtelClientInfo {
                client_name: "fuigo-pager",
                client_version: fuigo_version::VERSION,
                service_version: env!("VERSION_WITH_COMMIT"),
                app_entrypoint,
            },
            fuigo_shell::auth::credential_provider::build_default_otel_layer_config(),
        ));
    fuigo_telemetry::debug_log::install_firehose(registry, app_entrypoint);
...
/// `fuigo setup`: rendering and exit codes only; fetch logic lives in `fuigo_shell::managed_config`.
...
fn resolve_target(args: &LeaderTargetArgs) -> LeaderTarget {
    match args.pid {
        ...
        None => LeaderTarget::Environment(fuigo_shell::env::FuigoBuildEnvironment::Production),
    }
}
#[tracing::instrument(skip_all)]
async fn connect_to_leader(
    args: &LeaderTargetArgs,
) -> Result<(LeaderDescriptor, fuigo_shell::leader::LeaderClient)> {
    let target = resolve_target(args);
    let selection = fuigo_shell::leader::resolve_leader_target(target)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e.message))?;
    let socket_path = selection
        .socket_path()
        .ok_or_else(|| anyhow::anyhow!("resolved leader target did not include a socket path"))?;
    let client = fuigo_shell::leader::LeaderClient::connect(
...
fn leader_info_json(
    ...
    info: Option<&fuigo_shell::leader::ControlPayload>,
...
/// Env override for the `fuigo workspace` gate: any truthy value enables the command locally, a falsy one disables it.
/// Either way it bypasses the remote settings flag.
const WORKSPACE_COMMAND_ENV: &str = "FUIGO_WORKSPACE_COMMAND";
/// Resolution of the `fuigo workspace` gate.
...
/// The `FUIGO_WORKSPACE_COMMAND` override, if set (`Some(true)`/`Some(false)`); `None` defers to the remote settings flag.
...
fn workspace_command_gate(
    ...
    remote_settings: Option<&fuigo_shell::util::config::RemoteSettings>,
) -> WorkspaceGate {
...
/// Truthy parse for fuigo on/off env vars: everything enables except the common falsy spellings (`0`, `false`, `off`, `no`, empty).
...
fn fetch_remote_settings() -> Option<fuigo_shell::util::config::RemoteSettings> {
    fuigo_shell::agent::models::startup_prefetch::begin(None);
    fuigo_shell::agent::models::startup_prefetch::wait_settings(EARLY_PREFETCH_WAIT)
}
#[tracing::instrument(level = "debug", skip_all)]
async fn run_workspace_mgmt(args: WorkspaceMgmtArgs) -> Result<()> {
    ...
    ) && let Some(profile) = fuigo_sandbox::requested_confinement_profile()
    {
        anyhow::bail!(
            "`fuigo workspace` start/restart/resume is unavailable under sandbox profile '{profile}': \
    ...
    match workspace_command_gate(env_override, remote_settings.as_ref()) {
        ...
        WorkspaceGate::Disabled => {
            anyhow::bail!(
                "`fuigo workspace` is not enabled for this account \
             (gated by a server-side feature flag that is currently off)."
            )
        }
        WorkspaceGate::Unknown => {
            anyhow::bail!(
                "Could not load your settings for `fuigo workspace`. Check your \
             network connection (run `fuigo login` if you are signed out), then \
             try again."
            )
        }
    }
...
async fn connect_workspace_control(
    ...
    let ws_url = &agent_config.fuigo_com_config.fuigo_ws_url;
    ...
    .map_err(|e| {
        anyhow::anyhow!(
            "no running leader for this environment ({e}). \
             Start a fuigo session, or run `fuigo workspace start`."
        )
    })
}
#[tracing::instrument(level = "debug", skip_all)]
...
async fn workspace_start(
    ...
    use fuigo_shell::auth::ensure_authenticated;
    ...
    let (use_leader, _) = resolve_use_leader(
        ...
        true,
        fuigo_sandbox::requested_confinement_profile(),
    );
    if !use_leader {
        anyhow::bail!(
            "`fuigo workspace` requires leader mode (the workspace is shared via the leader).\n\
             Enable it with `[cli] use_leader = true` in ~/.fuigo/config.toml, or pass --leader."
        );
    }
    ensure_authenticated(
        &agent_config.fuigo_com_config,
        false,
        Some("No cached credentials found. Run `fuigo login` first."),
    )
    .await?;
    let env_urls = LeaderEnvUrls::from(&agent_config.fuigo_com_config);
...
const PLUGIN_DIR_LEADER_WARNING: &str = "fuigo: --plugin-dir is ignored in leader mode; run with --no-leader to \
...
async fn run_agent_command(
    ...
    let _signal_flush = tokio::spawn(async {
        #[cfg(unix)]
        {
            ...
            use fuigo_pager::app::signal_handler::next_signal_code;
    ...
    if matches!(
        ...
        fuigo_shell::agent::app::suppress_otel();
    }
    ...
    let had_prefetch = fuigo_shell::agent::models::startup_prefetch::begin(None);
    ...
    agent_config.reasoning_effort_override = agent_args
        .reasoning_effort
        .as_deref()
        .and_then(fuigo_shell::sampling::types::parse_canonical_effort_token);
    let launch_yolo = fuigo_shell::util::config::effective_yolo_for_launch(
        agent_args.yolo,
    ...
    let requested_confinement = fuigo_sandbox::requested_confinement_profile();
    let LeaderMode {
        use_leader,
    ...
    use fuigo_telemetry::process_info::LeaderMode::{Attached, Standalone};
        ...
        match mode {
            ClientMode::Stdio => {
                ...
                let stdin_task = tokio::spawn(async move {
                    let mut stdin_lines = fuigo_acp_lib::spawn_stdin_line_reader();
                    loop {
                        tokio::select! {
                            biased;
    ...
    match agent_args.mode {
        ...
        Some(AgentCmd::Serve(a)) => {
            ...
            let server_config = fuigo_shell::agent::ServerConfig {
                bind_addr: a.bind,
                secret: secret.clone(),
            };
            print_serve_startup_info(a.bind, &secret);
            fuigo_shell::agent::run_agent_server(server_config, agent_config).await
        }
        Some(AgentCmd::Leader(a)) => {
            ...
            let leader_auto_update = if !should_check_for_updates(
                ...
                Some(fuigo_shell::agent::app::LeaderAutoUpdateConfig {
                    check_interval: std::time::Duration::from_secs(60 * 60),
                    check_fn: Box::new(move || {
                        let uc = update_config_for_leader.clone();
                        Box::pin(async move {
                            let current_config = fuigo_shell::util::config::load_config().await;
                            ...
                            match auto_update::ensure_latest_on_disk(&uc).await {
                                Ok(outcome) => {
                                    if let Some(v) = &outcome.installed {
                                        if let Err(e) = fuigo_shell::managed_config::sync().await
                                        {
                                            tracing::warn!(
...
/// Sets `FUIGO_OPEN_DASHBOARD_AT_STARTUP=1` if the user asked for `fuigo dashboard`.
...
/// So `fuigo dashboard` does NOT force leader mode and is compatible with `--no-leader`.
///
/// The only gate is the feature flag: a disabled dashboard (`[dashboard].enabled = false` / `FUIGO_AGENT_DASHBOARD=0`) is a CLI error.
...
fn flag_dashboard_at_startup_if_requested(args: &mut PagerArgs) -> Result<()> {
    ...
    if !fuigo_pager::views::dashboard::dashboard_enabled() {
        anyhow::bail!(
            "the Agent Dashboard is disabled. Enable it by removing \
             `[dashboard] enabled = false` from ~/.fuigo/config.toml and \
             unsetting FUIGO_AGENT_DASHBOARD=0."
        );
    }
    args.command = None;
    unsafe { std::env::set_var("FUIGO_OPEN_DASHBOARD_AT_STARTUP", "1") };
    Ok(())
}
const RUNTIME_SHUTDOWN_GRACE: std::time::Duration = std::time::Duration::from_secs(2);
const FUIGO_WORKER_THREADS_ENV: &str = "FUIGO_WORKER_THREADS";
...
/// How `FUIGO_WORKER_THREADS` resolved.
#[derive(Debug, PartialEq, Eq)]
enum WorkerCount {
    ...
    fn notice(&self) -> Option<String> {
        match self {
            ...
            Self::Clamped {
                ...
                "fuigo: clamped {FUIGO_WORKER_THREADS_ENV}={requested} to {used} (valid range is 1..={cores})"
            )),
            Self::Ignored { value, .. } => Some(format!(
                "fuigo: ignoring {FUIGO_WORKER_THREADS_ENV}={value:?} (not a valid integer)"
            )),
        }
    }
}
fn cli_worker_threads() -> NonZeroUsize {
    ...
    let resolved = match std::env::var(FUIGO_WORKER_THREADS_ENV) {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_allocator_stats() -> Option<fuigo_pager::memory_trace::AllocatorStats> {
    ...
    unsafe {
        ...
        Some(fuigo_pager::memory_trace::AllocatorStats {
...
#[cfg(all(feature = "jemalloc", unix))]
fn jemalloc_heap_stats() -> Option<fuigo_shell::heap_profile::JemallocStats> {
    unsafe {
        tikv_jemalloc_ctl::raw::write(b"epoch\0", 1u64).ok()?;
        ...
        Some(fuigo_shell::heap_profile::JemallocStats {
            allocated,
            resident,
        })
    }
...
fn install_heap_profile_hooks() {
    fuigo_shell::heap_profile::install(fuigo_shell::heap_profile::HeapProfileHooks {
...
fn main() {
    ...
    if let Some(code) = fuigo_pager::voice::maybe_run_capture_subprocess() {
        std::process::exit(code);
    }
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fuigo_pager::memory_release::install_release_hook(purge_jemalloc_retained_pages);
    #[cfg(all(feature = "jemalloc", unix))]
    {
        fuigo_pager::memory_trace::install_allocator_stats_provider(jemalloc_allocator_stats);
        fuigo_pager::memory_trace::install_allocator_dump_provider(jemalloc_stats_dump);
    }
    #[cfg(all(feature = "jemalloc", unix))]
    ...
    let _sentry_guard = fuigo_telemetry::sentry::init(fuigo_telemetry::sentry::Config {
        ...
        disabled: fuigo_shell::agent::config::is_error_reporting_disabled_sync(),
    });
...
async fn async_main(args: PagerArgs) -> Result<()> {
    ...
    if let Some(ref mode) = args.compaction_mode {
        unsafe { std::env::set_var("FUIGO_COMPACTION_MODE", mode) };
    }
    if let Some(ref detail) = args.compaction_detail {
        unsafe { std::env::set_var("FUIGO_COMPACTION_DETAIL", detail) };
    }
    if args.chat() {
        unsafe {
            std::env::set_var(fuigo_shell::agent::chat_modes::FUIGO_CHAT_MODE_ENV, "1");
        }
    }
    if let Some(ref socket) = args.leader_socket {
        unsafe { std::env::set_var(fuigo_shell::leader::LEADER_SOCKET_ENV, socket) };
    }
    if let Some(ref path) = args.debug_file {
        unsafe {
            std::env::set_var("FUIGO_DEBUG_LOG", path);
            std::env::remove_var("FUIGO_LOG_FILE");
        }
    }
    if args.debug || args.debug_file.is_some() {
        ...
        set_if_unset("FUIGO_DEBUG_LOG", "1");
        set_if_unset("FUIGO_HOOKS_LOG", "1");
    }
    ...
    if command_needs_pre_sandbox_policy_heal(args.command.as_ref()) {
        match fuigo_shell::config::load_agent_config_disk_only() {
            Ok(agent_cfg) => {
                let auth_manager = std::sync::Arc::new(fuigo_shell::auth::AuthManager::new(
                    &fuigo_shell::util::fuigo_home::fuigo_home(),
                    agent_cfg.fuigo_com_config.clone(),
                ));
                auth_manager.configure_refresher(
                    agent_cfg.fuigo_com_config.auth_provider_command.clone(),
                    None,
                );
                fuigo_shell::managed_config::ensure_managed_policy_present(&auth_manager).await;
            }
            Err(e) => {
                tracing::warn!(
                    error = %e,
    ...
    fuigo_shell::config::apply_sandbox(
        None,
    ...
    fuigo_shell::http::set_client_name(if is_interactive {
        fuigo_workspace::permission::ClientType::FuigoPager
    } else {
                ...
                if args.leader || args.no_leader {
                    ...
                    anyhow::bail!(
                        "top-level {flag} applies to the pager TUI, not the agent subcommand. \
                         Use `fuigo-pager agent {flag}` instead."
                    );
                }
    ...
    let headless_prompt = fuigo_pager::headless::HeadlessPrompt::from_args(
    ...
    if headless_prompt.is_some() || args.memory_flush {
        ...
        {
            args.output_format = fuigo_pager::headless::OutputFormat::Json;
        }
        let memory_enabled_override = args.memory_enabled_override();
        let memory_flush = args.memory_flush;
        return fuigo_pager::headless::run_single_turn(
            headless_prompt,
            args.verbatim,
            fuigo_pager::headless::HeadlessOptions {
    ...
    let result = fuigo_pager::app::run(args, bg_update_rx).await;
    fuigo_sandbox::flush();
    match result {
        Ok(true) => {
...
///
/// Prefers awaiting the parked waiter for the background `fuigo update` child spawned at startup; the download is usually already done or in flight.
/// It falls back to a fresh blocking `fuigo update` only when there is no waiter or the child failed.
...
fn build_update_config() -> UpdateConfig {
    let environment = fuigo_shell::env::FuigoBuildEnvironment::from_flags(false, false);
    let mut config = UpdateConfig::from_environment(&environment);
    cryptify::flow_stmt!({
        {
            config.deployment_key =
                fuigo_shell::agent::config::EndpointsConfig::default().deployment_key;
        }
    });
    config.npm_registry = std::env::var(obfstr::obfstr!("FUIGO_NPM_REGISTRY"))
        .ok()
        .or_else(fuigo_shell::util::config::load_npm_registry_sync);
    if let Ok(root) = fuigo_shell::config::load_effective_config_disk_only()
        && let Some(ch) = fuigo_shell::util::config::channel_from_toml_opt(&root)
    {
        config.channel = ch;
    }
    config
...
fn should_check_for_updates(no_auto_update_flag: bool) -> bool {
    ...
    !std::env::var_os("FUIGO_DISABLE_AUTOUPDATER")
...
/// True when `exe` is the binary `<fuigo_home>/bin/fuigo` resolves to, the
...
/// npm shim hardcodes `~/.fuigo`, so a custom `FUIGO_HOME` skips here too.
fn is_managed_install(exe: Option<std::path::PathBuf>, fuigo_home: &std::path::Path) -> bool {
    if fuigo_home.as_os_str().is_empty() {
        return false;
    }
    let Some(exe) = exe else {
        return false;
    };
    let managed = fuigo_config::fuigo_application_in(fuigo_home);
...
/// Handle `fuigo-pager update [--check] [--json] [--force-reinstall] [--version X] [--alpha|--stable|--enterprise]`.
...
async fn run_update_command(
    ...
    let telemetry_cfg = fuigo_shell::config::load_agent_config_disk_only()
        .map_err(|e| tracing::warn!("fuigo update: telemetry init skipped (agent config: {e})"))
        .ok();
    if let Some(agent_cfg) = telemetry_cfg {
        ...
        fuigo_shell::agent::init::update_telemetry_config(&agent_cfg, &auth_manager);
    }
...
/// After a successful `fuigo update`, ask any running leader on this machine that is older than `installed_version` to relaunch onto the new binary.
...
async fn signal_leaders_to_relaunch(installed_version: &str) {
    for d in fuigo_shell::leader::discover_leaders().await {
        ...
        let client = match fuigo_shell::leader::LeaderClient::connect(
            socket_path,
            "fuigo-pager-update",
            ClientMode::Stdio,
            ClientCapabilities::default(),
        ...
        match client
            ...
            .await
        {
            Ok(Ok(fuigo_shell::leader::ControlPayload::Relaunching {
                from_version,
                to_version,
                ..
            })) => {
...
mod tests {
    ...
    fn embedded_agent_commands_heal_managed_policy_before_sandboxing() {
        for args in [
            vec!["fuigo"],
            vec!["fuigo", "agent", "stdio"],
            vec!["fuigo", "dashboard"],
            vec!["fuigo", "models"],
            vec!["fuigo", "worktree", "list"],
        ] {
    ...
    fn override_out_of_range_is_clamped_with_a_notice() {
        ...
        assert_eq!(
            ...
            "fuigo: clamped FUIGO_WORKER_THREADS=100000 to 360 (valid range is 1..=360)"
        );
    }
    #[test]
    fn override_unparseable_is_ignored_with_a_notice() {
        ...
        assert_eq!(
            ...
            "fuigo: ignoring FUIGO_WORKER_THREADS=\"abc\" (not a valid integer)"
        );
    }
    #[test]
    fn version_output_writer_preserves_channel_aware_contract() {
        ...
        for (label, expected_suffix) in [
            ...
            assert!(output.starts_with("fuigo "));
    ...
    fn version_flags_and_doctor_are_distinct_early_intents() {
        ...
        let short = PagerArgs::try_parse_from(["fuigo", "-v"]).unwrap();
        assert!(short.version);
        assert!(short.command.is_none());
        let subcommand = PagerArgs::try_parse_from(["fuigo", "version"]).unwrap();
    ...
    #[cfg(all(feature = "jemalloc", unix))]
    fn assert_stats_sane(stats: fuigo_shell::heap_profile::JemallocStats) {
    ...
    fn jemalloc_dump_rejects_interior_nul_path() {
        ...
        let path = Path::new(OsStr::from_bytes(b"/tmp/fuigo-jemalloc-\0.heap"));
    ...
    fn install_heap_profile_hooks_wires_shell_apis() {
        install_heap_profile_hooks();
        assert_stats_sane(
            fuigo_shell::heap_profile::stats().expect("shell stats after install"),
        );
        if !require_opt_prof() {
            assert!(!fuigo_shell::heap_profile::prof_available());
            return;
        }
        assert!(fuigo_shell::heap_profile::prof_available());
        assert_prof_active(false);
        {
            let _guard = ProfActiveGuard::set(true);
            assert_prof_active(true);
            assert!(fuigo_shell::heap_profile::set_prof_active(true));
            assert_prof_active(true);
        }
        assert_prof_active(false);
        assert!(fuigo_shell::heap_profile::set_prof_active(false));
        assert_prof_active(false);
        let dump = TempHeapDump::new("shell");
        fuigo_shell::heap_profile::dump_to_path(dump.path()).expect("shell dump");
        dump.assert_nonempty_dump();
    }
    #[cfg(unix)]
    #[test]
    fn is_managed_install_matches_only_the_bin_fuigo_target() {
        let home =
            std::env::temp_dir().join(format!("fuigo-pager-managed-install-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&home);
        std::fs::create_dir_all(home.join("bin")).unwrap();
        std::fs::create_dir_all(home.join("downloads")).unwrap();
        assert!(!is_managed_install(
            Some(home.join("bin").join("fuigo")),
            &home
        ));
        ...
        std::os::unix::fs::symlink(&target, home.join("bin").join("fuigo")).unwrap();
        assert!(is_managed_install(
            Some(home.join("bin").join("fuigo")),
            &home
        ));
    ...
    /// `fuigo dashboard` flags the startup hook without forcing leader mode.
    ...
    fn dashboard_subcommand_flags_startup_without_forcing_leader() {
        let mut args = PagerArgs::try_parse_from(["fuigo", "dashboard"]).unwrap();
        ...
        assert_eq!(
            std::env::var("FUIGO_OPEN_DASHBOARD_AT_STARTUP").as_deref(),
            Ok("1"),
            "startup hook flag must be set",
        );
        unsafe { std::env::remove_var("FUIGO_OPEN_DASHBOARD_AT_STARTUP") };
    }
    /// `fuigo dashboard --no-leader` is allowed.
    ...
    fn dashboard_subcommand_allows_no_leader() {
        let mut args = PagerArgs::try_parse_from(["fuigo", "--no-leader", "dashboard"]).unwrap();
        flag_dashboard_at_startup_if_requested(&mut args)
    ...
    /// `FUIGO_AGENT_DASHBOARD=0` disables the feature; the subcommand must error visibly before the TUI starts.
    #[serial_test::serial(FUIGO_AGENT_DASHBOARD)]
    #[test]
    fn dashboard_subcommand_errors_when_disabled() {
        unsafe { std::env::set_var("FUIGO_AGENT_DASHBOARD", "0") };
        let mut args = PagerArgs::try_parse_from(["fuigo", "dashboard"]).unwrap();
        let result = flag_dashboard_at_startup_if_requested(&mut args);
        unsafe { std::env::remove_var("FUIGO_AGENT_DASHBOARD") };
        let err = result.expect_err("disabled dashboard must error");
        assert!(err.to_string().contains("disabled"), "got: {err}");
        assert!(
            std::env::var("FUIGO_OPEN_DASHBOARD_AT_STARTUP").is_err(),
            "failure path must not flag the startup hook",
        );
    }
    #[test]
    fn workspace_command_gate_resolution() {
        use fuigo_shell::util::config::RemoteSettings;
    ...
    #[serial_test::serial(FUIGO_WORKSPACE_COMMAND)]
    #[test]
    fn workspace_command_env_override_parsing() {
        unsafe { std::env::remove_var("FUIGO_WORKSPACE_COMMAND") };
        assert_eq!(workspace_command_env_override(), None);
        unsafe { std::env::set_var("FUIGO_WORKSPACE_COMMAND", "1") };
        assert_eq!(workspace_command_env_override(), Some(true));
        unsafe { std::env::set_var("FUIGO_WORKSPACE_COMMAND", "off") };
