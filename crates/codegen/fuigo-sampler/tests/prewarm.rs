//! Wire-level tests for the sampler transport prewarm: one credential-less dial per origin per pool-idle window,
//! reuse by the first sampling request, and claim release on failure/truncation.
//! Own integration binary so the pinned env cannot leak into, or be poisoned by, other tests.
...
use fuigo_sampler::{PrewarmOutcome, SamplingClient, prewarm_transport};
use fuigo_test_support::counting_server::spawn_http_server;
use fuigo_test_support::spawn_counting_server;
