...
/// Kill switch: `FUIGO_SAMPLER_SHARED_CLIENT=0` (or `false`, any case) builds a fresh `reqwest::Client` per `SamplingClient` instead.
...
fn sharing_disabled() -> bool {
    ...
    *DISABLED.get_or_init(|| {
        let disabled = match std::env::var("FUIGO_SAMPLER_SHARED_CLIENT") {
            Ok(v) => v == "0" || v.eq_ignore_ascii_case("false"),
            Err(_) => false,
        };
        if disabled {
            tracing::info!("sampler HTTP client sharing disabled via FUIGO_SAMPLER_SHARED_CLIENT");
        }
        disabled
    })
}
