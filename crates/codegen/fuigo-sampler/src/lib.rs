//! Actor-based sampling layer for Ferrox Labs fuigo.
//!
//! This crate holds the HTTP streaming and retry logic extracted from `fuigo-shell`'s session actor.
//! It is built on the same actor pattern as `fuigo-hunk-tracker`.
//!
//! ## Layered API
//!
...
pub use retry::{
    DEFAULT_MAX_RETRIES, EMPTY_RESPONSE_MAX_ATTEMPTS, EMPTY_RESPONSE_RETRY_BACKOFF, MAX_RETRY_BACKOFF,
    RATE_LIMIT_RETRY_DISABLED, RATE_LIMIT_RETRY_THRESHOLD, REASONING_ONLY_MAX_ATTEMPTS,
    RetryDecision, classify_error, effective_max_retries, format_sampling_error, jitter_backoff,
