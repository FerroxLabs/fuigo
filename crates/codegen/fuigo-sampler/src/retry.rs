...
pub fn resolve_max_retries(model_max_retries: Option<u32>) -> u32 {
    let env_override = std::env::var("FUIGO_MAX_RETRIES").ok();
...
mod tests {
        ...
        for err in [
            ...
            SamplingError::EmptyResponse {
                context: fuigo_sampling_types::EmptyResponseContext {
                    reason: fuigo_sampling_types::EmptyReason::NoVisibleContent,
                    had_reasoning: false,
                    content_len: 0,
                    tool_call_count: 0,
    ...
    fn classify_cloudflare_522_is_retryable() {
        let err = api_err(
            ...
            "Connection to Fuigo timed out or was interrupted. (HTTP 522).",
        );
    ...
    fn classify_cloudflare_525_is_fatal_even_with_should_retry_true() {
        for should_retry in [None, Some(true)] {
            let err = SamplingError::Api {
                ...
                message: "Secure connection to Fuigo failed. (HTTP 525).".into(),
                model_metadata: None,
                retry_after_secs: None,
                should_retry,
                error_code: None,
