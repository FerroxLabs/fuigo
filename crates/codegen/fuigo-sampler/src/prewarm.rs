...
async fn dial_and_drain(
    ...
    origin: &str,
) -> Result<(reqwest::StatusCode, DrainOutcome), fuigo_extra_ca::dispatch::DispatchError> {
    // Redirects may be followed, but hyper pools the origin's connection before following, so the dial still warms it.
    // Checked dispatch: the egress/destination policy runs before any contact, exactly as for a sampling request.
    let mut response = fuigo_extra_ca::dispatch::send(client.get(origin)).await?;
    ...
    while let Some(chunk) = response
        .chunk()
        .await
        .map_err(fuigo_extra_ca::dispatch::DispatchError::Transport)?
    {
        drained += chunk.len();
