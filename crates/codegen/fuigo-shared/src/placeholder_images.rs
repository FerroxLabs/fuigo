...
//!
//! The TUI ([`fuigo_pager::prompt_images`]) and the server-side ingestion path ([`crate::session::acp_session`]) need to recover image bytes.
...
//!
//! Wire format: `[Image #<n>: <absolute_path>]`; the producer is [`fuigo_pager::prompt_images::display_text`].
...
/// The server resolves `[Image #N]` tokens by this number rather than by list position (the two diverge; see `AttachedImages` in `fuigo-tools`).
pub const IMAGE_DISPLAY_NUMBER_META_KEY: &str = "fuigo.dev/imageDisplayNumber";
...
/// See [`AttachedImages`](fuigo_tools::types::resources::AttachedImages).
///
...
///
/// * The producer emits exactly `": "` (colon, single space) as the separator; see [`fuigo_pager::prompt_images::display_text`].
...
pub fn default_allowed_prefixes(workspace_cwd: &Path) -> Vec<PathBuf> {
    default_allowed_prefixes_with_home(workspace_cwd, fuigo_dirs::home_dir())
}
...
/// That is strictly stronger than the legacy [`fuigo_pager::prompt_images::read_image_at_path`], which has no prefix allowlist at all.
...
fn decode_image_mime(data: &[u8]) -> Option<&'static str> {
    fuigo_tools::util::image_validate::validate_image_bytes_with(data, false)
        .ok()
        .map(|(_, _, mime)| mime)
}
...
///
/// This wrapper reads the ambient process `$HOME` via `fuigo_dirs::home_dir()` to construct [`HOME_IMAGE_SUBDIRS`] prefixes.
...
mod tests {
    ...
    /// The placeholder text wire format is the unencoded path produced by `fuigo_pager::prompt_images::display_text`.
    ...
    fn attached_image_references_prefers_file_path_over_data() {
        ...
        let img = agent_client_protocol::ImageContent::new("AAAA", "image/png")
            .uri(Some(
                "file:///Users/me/.fuigo/sessions/s/images/image-1.png".into(),
            ))
        ...
        assert_eq!(
            ...
            vec![(
                1,
                "/Users/me/.fuigo/sessions/s/images/image-1.png".to_string()
            )]
        );
    }
