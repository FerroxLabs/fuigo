...
use fuigo_status_line::StatusLineConfig;
...
pub struct UiConfig {
    ...
    /// Written by the pager's settings modal; read at connect time (CLI `--hunk-tracker-mode` / `FUIGO_HUNK_TRACKER` override it).
    ...
    /// A Fuigo STT catalog code (`en`, `es`, `ja`, …; see Ferrox Labs STT supported languages) or `auto` (system locale, resolved at connect).
    ...
    /// Written by the pager's settings modal; the `FUIGO_PROMPT_SUGGESTIONS` env var overrides at runtime.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct ContextualHints {
    ...
    /// SSH wrap session-load tip (recommend `fuigo wrap ssh` when the session runs over SSH without an OSC 52 sink).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl Default for UiConfig {
    fn default() -> Self {
        Self {
            ...
            theme: None,
            fork_secondary_model: fuigo_models::default_model().to_string(),
            yolo: false,
            ui_theme: None,
            compact_mode: false,
            simple_mode: None,
