...
//! `FUIGO_CLIPBOARD_NO_NATIVE_READ=1` disables the in-process read entirely (kill switch if a future macOS gates `dataForType:` behind a prompt).
//!
...
/// Always `false` off Linux, off Wayland, or when the `FUIGO_CLIPBOARD_NO_DATA_CONTROL` kill-switch env var is set.
...
mod platform {
    ...
    /// `None` also when `FUIGO_CLIPBOARD_NO_NATIVE_READ` is set, the kill switch if a future macOS gates `dataForType:` behind a privacy prompt.
    ...
    pub(super) fn native_image_read() -> Option<super::ImageData> {
        if std::env::var_os("FUIGO_CLIPBOARD_NO_NATIVE_READ").is_some() {
            return None;
        }
        let _guard = PASTEBOARD_LOCK.lock();
        objc2::rc::autoreleasepool(|_| {
    ...
    fn attachments_probe_temp_paths() -> (std::path::PathBuf, std::path::PathBuf, std::path::PathBuf)
    {
        ...
        (
            temp_dir.join("fuigo-clipboard-probe.png"),
            temp_dir.join("fuigo-clipboard-probe.tiff"),
            temp_dir.join("fuigo-clipboard-probe.jpg"),
        )
    }
...
mod platform {
    ...
    /// On Wayland the `FUIGO_CLIPBOARD_NO_DATA_CONTROL` kill switch has to stop arboard from speaking data-control at all.
        ...
        LEASE
            .get_or_init(|| {
                if arboard_wayland_bypassed() {
                    tracing::debug!("arboard leg disabled (FUIGO_CLIPBOARD_NO_DATA_CONTROL)");
                    return None;
                }
    ...
    fn arboard_read_with_deadline<T: Send + 'static>(
        ...
        if arboard_wayland_bypassed() {
            anyhow::bail!("arboard leg disabled (FUIGO_CLIPBOARD_NO_DATA_CONTROL)");
        }
