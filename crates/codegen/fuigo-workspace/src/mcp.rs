...
use fuigo_computer_hub_mcp_adapter::{
...
/// [`ToolServer`](fuigo_computer_hub_sdk::ToolServer). The seam exists so the
...
pub(crate) trait HubToolRegistry: Send + Sync {
    ...
    fn register_tool_dynamic(
        ...
        life: u64,
    ) -> impl Future<Output = Result<(), fuigo_computer_hub_sdk::ClientError>> + Send;
    ...
    fn unregister_tool_dynamic(
        ...
        life: u64,
    ) -> impl Future<Output = Result<bool, fuigo_computer_hub_sdk::ClientError>> + Send;
}

impl HubToolRegistry for fuigo_computer_hub_sdk::ToolServer {
...
impl McpTransport for McpClientTransportAdapter {
    async fn initialize(&self) -> Result<McpServerInfo, fuigo_computer_hub_mcp_adapter::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
        ...
        let info = service.peer_info().ok_or_else(|| {
            fuigo_computer_hub_mcp_adapter::McpError::Transport("no peer info after init".into())
        })?;
    ...
    async fn list_tools(
        &self,
    ) -> Result<Vec<McpToolDefinition>, fuigo_computer_hub_mcp_adapter::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
    ...
    async fn call_tool(
        ...
        name: &str,
        arguments: Value,
    ) -> Result<McpCallResult, fuigo_computer_hub_mcp_adapter::McpError> {
        let service = self
            .client
            .ensure_initialized()
            .await
...
///
/// [`McpState`]: fuigo_mcp::servers::McpState
pub(crate) async fn drive_server_starts(
    ...
    let mut remaining_names: HashSet<String> = configs
        .iter()
        .map(|config| fuigo_mcp::servers::mcp_server_name(config).to_owned())
        .collect();
    {
    ...
    let mut pending: FuturesUnordered<_> = configs
        .into_iter()
        .map(|config| {
            let server_name = fuigo_mcp::servers::mcp_server_name(&config).to_owned();
            ...
            async move {
                let client =
                    fuigo_mcp::servers::start_mcp_server(config, Some(overrides), None, None, ctx)
                        .await
...
pub(crate) fn dedupe_servers_last_wins(servers: &mut Vec<agent_client_protocol::McpServer>) {
    ...
    for index in (0..servers.len()).rev() {
        let name = fuigo_mcp::servers::mcp_server_name(&servers[index]).to_owned();
        if !seen.insert(name) {
...
pub(crate) async fn converge_session(
    ...
    if !plan.start.is_empty() {
        ...
        let configs = desired
            .servers()
            .iter()
            .filter(|config| starting.contains(fuigo_mcp::servers::mcp_server_name(config)))
            .cloned()
            .collect();
        let drive_scope = begin_mcp_drive(session).await?;
...
mod tests {
    ...
    fn cap_servers_keeps_the_first_entries_in_config_order() {
        ...
        assert_eq!(
            ...
            fuigo_mcp::servers::mcp_server_name(
    ...
    fn dedupe_servers_last_wins_keeps_last_definition_in_place() {
        ...
        let named: Vec<(&str, &str)> = servers
            ...
            .map(|server| {
                ...
                (
                    fuigo_mcp::servers::mcp_server_name(server),
                    http.url.as_str(),
                )
            })
            .collect();
