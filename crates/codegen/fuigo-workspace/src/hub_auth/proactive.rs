...
static REFRESH_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    ...
    .expect("register fuigo_workspace_oidc_proactive_refresh_total")
});

static REFRESH_DURATION: LazyLock<Histogram> = LazyLock::new(|| {
    register_histogram!(
        "fuigo_workspace_oidc_proactive_refresh_duration_seconds",
    ...
    .expect("register fuigo_workspace_oidc_proactive_refresh_duration_seconds")
});

static REFRESH_LEAD: LazyLock<Histogram> = LazyLock::new(|| {
    register_histogram!(
        "fuigo_workspace_oidc_refresh_lead_seconds",
    ...
    .expect("register fuigo_workspace_oidc_refresh_lead_seconds")
});

static REFRESH_JITTER: LazyLock<Histogram> = LazyLock::new(|| {
    register_histogram!(
        "fuigo_workspace_oidc_refresh_jitter_seconds",
    ...
    .expect("register fuigo_workspace_oidc_refresh_jitter_seconds")
});
...
async fn do_refresh(inner: &Inner, refresh_token: &str) -> Result<RefreshOutcome, RefreshError> {
    ...
    let client = fuigo_extra_ca::build_reqwest_client(|builder| builder).map_err(refresh_err)?;
