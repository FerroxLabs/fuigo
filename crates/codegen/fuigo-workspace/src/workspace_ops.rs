...
use fuigo_computer_hub_sdk::ToolHarness;
use fuigo_tools::types::output::ToolRunResult;
use fuigo_workspace_client::{WorkspaceClient, is_transport_fatal};
pub use fuigo_workspace_types::rpc::agents_md::DiscoverAgentsMdReq;
pub use fuigo_workspace_types::rpc::code_nav::{
...
pub use fuigo_workspace_types::rpc::export_github::ExportGithubReq;
pub use fuigo_workspace_types::rpc::fs::{
...
pub use fuigo_workspace_types::rpc::git::{
...
pub use fuigo_workspace_types::rpc::hooks::{
    HookEventNameWire, HookRegistryReq, HookRegistryWire, HookSpecWire,
};
pub use fuigo_workspace_types::rpc::hunks::{
...
pub use fuigo_workspace_types::rpc::repos::{
    ProvisionedRepo, RepoManifest, ReposListReq, ReposListResponse,
};
pub use fuigo_workspace_types::rpc::search::{FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq};
pub use fuigo_workspace_types::rpc::session::{BeginPromptReq, EndPromptReq, RewindToReq};
pub use fuigo_workspace_types::rpc::skills::DiscoverSkillsReq;
pub use fuigo_workspace_types::rpc::workspace::WorkspaceInfoReq;
pub use fuigo_workspace_types::rpc::worktree::{
...
pub use fuigo_workspace_types::rpc::{RpcActivityClass, WorkspaceRpc};
...
fn hunk_line_info_to_wire(info: &fuigo_hunk_tracker::types::HunkLineInfo) -> HunkLineInfoWire {
    HunkLineInfoWire {
...
fn hunk_source_to_wire(source: fuigo_hunk_tracker::types::HunkSource) -> HunkSourceWire {
    use fuigo_hunk_tracker::types::HunkSource as S;
    match source {
...
fn hunk_to_wire(hunk: &fuigo_hunk_tracker::types::Hunk) -> HunkWire {
    HunkWire {
...
fn file_content_status_to_wire(
    status: fuigo_hunk_tracker::types::FileContentStatus,
) -> FileContentStatusWire {
    use fuigo_hunk_tracker::types::FileContentStatus as S;
    match status {
...
fn file_content_view_to_wire(
    view: fuigo_hunk_tracker::types::FileContentView,
) -> FileContentViewWire {
    FileContentViewWire {
...
fn file_content_entry_to_wire(entry: fuigo_hunk_tracker::FileContentEntry) -> FileContentEntryWire {
...
fn session_stats_to_wire(stats: &fuigo_hunk_tracker::types::SessionStats) -> SessionStatsWire {
    SessionStatsWire {
...
fn turn_summary_to_wire(turn: fuigo_hunk_tracker::types::TurnSummary) -> TurnSummaryWire {
    TurnSummaryWire {
...
fn session_summary_to_wire(summary: fuigo_hunk_tracker::SessionSummary) -> SessionSummaryWire {
    SessionSummaryWire {
...
fn tracker_action(kind: HunkActionKind) -> fuigo_hunk_tracker::types::HunkAction {
    match kind {
        HunkActionKind::Accept => fuigo_hunk_tracker::types::HunkAction::Accept,
        HunkActionKind::Reject => fuigo_hunk_tracker::types::HunkAction::Reject,
    }
}
...
fn session_tracker(
    ws: &WorkspaceHandle,
    session_id: Option<&str>,
) -> WorkspaceResult<fuigo_hunk_tracker::HunkTrackerHandle> {
    let sid = session_id
...
/// Ancestor hop budget when locating `.fuigo/repos.json`.
/// Grove rewrite is one hop (`/workspace/app` to `/workspace`); desktop workspaces can sit deeper, so this is a backstop only.
/// Primary bounds are the sandbox root (`/workspace`) and the user-global fuigo home.
...
///
/// Does not escape the sandbox workspace or load `~/.fuigo/repos.json` /
/// `$FUIGO_HOME/repos.json` (user-global, not a provisioned workspace).
fn repos_manifest_search_dirs(start: &std::path::Path) -> Vec<std::path::PathBuf> {
    let rel = fuigo_workspace_types::rpc::repos::REPOS_MANIFEST_RELATIVE_PATH;
    let home = fuigo_dirs::home_dir();
    let mut global_manifests = Vec::with_capacity(2);
    if let Some(v) = std::env::var_os("FUIGO_HOME")
        && !v.is_empty()
    {
        global_manifests.push(std::path::PathBuf::from(v).join("repos.json"));
    }
    if let Some(user_home) = fuigo_config::user_fuigo_home() {
...
async fn load_repos_manifest(root: &std::path::Path) -> WorkspaceResult<RepoManifest> {
    let path = root.join(fuigo_workspace_types::rpc::repos::REPOS_MANIFEST_RELATIVE_PATH);
    match tokio::fs::read(&path).await {
...
impl WorkspaceOp for HunkSingleActionReq {
    async fn execute(
        ...
        let hunk_id = fuigo_hunk_tracker::types::HunkId::from_string(self.action.hunk_id.clone());
    ...
    async fn execute(
        ...
        for h in &all_hunks {
            ...
            let is_agent = matches!(
                h.source,
                fuigo_hunk_tracker::types::HunkSource::AgentEdit { .. }
            );
...
fn hook_registry_to_wire(
    registry: &fuigo_hooks::discovery::HookRegistry,
...
fn wire_to_hook_registry(
...
) -> WorkspaceResult<fuigo_hooks::discovery::HookRegistry> {
    ...
    let mut registry: fuigo_hooks::discovery::HookRegistry =
        serde_json::from_value(value).map_err(|e| WorkspaceError::HubError(e.to_string()))?;
...
fn resolve_index_for_workspace(
    ...
    std::sync::Arc<fuigo_codebase_graph::IndexManagerHandle>,
    std::path::PathBuf,
)> {
...
fn query_result_to_response(
    result: Result<fuigo_codebase_graph::QueryResult, fuigo_codebase_graph::QueryError>,
) -> CodeNavResponse {
    match result {
...
fn symbol_locations_to_response(
    locations: Vec<fuigo_codebase_graph::SymbolLocation>,
) -> CodeNavResponse {
    CodeNavResponse {
        locations: locations
            .into_iter()
...
///   Tool calls dispatch through the workspace session's [`FinalizedToolset`](fuigo_tools::registry::types::FinalizedToolset).
...
impl WorkspaceOps {
    ...
    pub fn bind_local_session(
        ...
        hunk_tracker: fuigo_hunk_tracker::HunkTrackerHandle,
    ...
    pub fn server_version(&self) -> Option<String> {
        match self {
            Self::Local { .. } => Some(fuigo_version::VERSION.to_owned()),
    ...
    pub async fn hook_registry(&self) -> WorkspaceResult<fuigo_hooks::discovery::HookRegistry> {
    ...
    ///
    /// - **Local**: dispatches through the workspace session's [`FinalizedToolset`](fuigo_tools::registry::types::FinalizedToolset) (in-process).
        ...
        match self {
            ...
            Self::Proxy { client } => {
                if !client.is_connected() {
                    return Err(fuigo_tool_runtime::ToolError::network_error(
                        "The workspace server connection was lost. \
                         Please restart your session to reconnect.",
                    ));
                }
                let tool_id = fuigo_tool_protocol::ToolId::new(name).map_err(|e| {
                    fuigo_tool_runtime::ToolError::custom(
                        "hub_proxy_error",
                ...
                ctx.call_id =
                    fuigo_tool_protocol::ToolCallId::new(call_id.to_owned()).unwrap_or(ctx.call_id);
...
mod tests {
    ...
    /// The request types live in `fuigo-workspace-types`, re-exported here for existing call sites.
    ...
    async fn git_op_cwd_uses_explicit_git_root_per_window() {
        ...
        let window_a = std::path::PathBuf::from("/repos/fuigo-main");
        let window_b = std::path::PathBuf::from("/repos/fuigo-main-2");
        assert_eq!(
    ...
    async fn repos_list_reads_real_manifest_empty_one_and_many() {
        ...
        assert_eq!(
            empty.version,
            fuigo_workspace_types::rpc::repos::REPOS_MANIFEST_VERSION
        );
        ...
        std::fs::write(
            tmp.path()
                .join(fuigo_workspace_types::rpc::repos::REPOS_MANIFEST_RELATIVE_PATH),
    ...
    async fn repos_list_walks_up_from_rewritten_cwd() {
        ...
        std::fs::create_dir_all(sandbox_ws.join(".fuigo")).unwrap();
        std::fs::write(
            sandbox_ws.join(fuigo_workspace_types::rpc::repos::REPOS_MANIFEST_RELATIVE_PATH),
            one.to_json_bytes().unwrap(),
        )
        .unwrap();
    ...
    fn repos_manifest_search_dirs_stops_at_sandbox_workspace_root() {
        ...
        let _unset_fuigo = crate::TestEnvGuard::unset("FUIGO_HOME");
    ...
    fn repos_manifest_search_dirs_skips_user_global_fuigo_home() {
        ...
        assert!(
            ...
            "must not probe $HOME/.fuigo/repos.json: {dirs:?}"
        );
    }
    ...
    fn repos_list_does_not_load_user_global_manifest() {
        ...
        std::fs::create_dir_all(home.path().join(".fuigo")).unwrap();
        std::fs::write(
            home.path().join(".fuigo").join("repos.json"),
        ...
        assert!(
            ...
            "missing workspace manifest must not fall back to ~/.fuigo/repos.json: {:?}",
            listed.repos
        );
    }
    ...
    async fn end_local_session_drops_bound_toolset() {
        ...
        let toolset = std::sync::Arc::new(
            fuigo_tools::registry::types::FinalizedToolset::empty_for_test(),
        );
    ...
    fn hunk_to_wire_serializes_identically() {
        use fuigo_hunk_tracker::types::{Hunk, HunkSource};
    ...
    fn file_content_entry_to_wire_serializes_identically() {
        use fuigo_hunk_tracker::FileContentEntry;
        use fuigo_hunk_tracker::types::FileContentView;
    ...
    fn session_summary_to_wire_serializes_identically() {
        use std::sync::Arc;
        use fuigo_hunk_tracker::SessionSummary;
        use fuigo_hunk_tracker::types::{Hunk, HunkSource, TurnSummary};
    ...
    fn hook_registry_wire_round_trip_both_directions() {
        let spec = fuigo_hooks::config::HookSpec {
            ...
            event: fuigo_hooks::event::HookEventName::PreToolUse,
            ...
            url_raw: None,
            timeout_ms: 5000,
            source_dir: std::path::PathBuf::from("/home/u/.fuigo/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
            layer: fuigo_hooks::config::HookProvenance::File,
        };
        let mut registry = fuigo_hooks::discovery::HookRegistry::default();
    ...
    fn hook_event_name_wire_covers_all_upstream_variants() {
        use fuigo_hooks::event::HookEventName as E;
    ...
    /// The lean types crate can't depend on `fuigo-hooks`, and `hook_registry_to_wire` only couples the two via a serde round-trip.
    ...
    fn hook_spec_wire_covers_all_upstream_fields() {
        ...
        let spec = HookSpec {
            ...
            source_dir: std::path::PathBuf::from("/home/u/.fuigo/hooks"),
            extra_env: std::collections::HashMap::from([("FOO".to_string(), "bar".to_string())]),
            layer: fuigo_hooks::config::HookProvenance::Managed,
        };
        assert_eq!(
