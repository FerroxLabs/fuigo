//! Advisory image capability tokens, declared by the sandbox image as marker files under `/usr/share/fuigo/capabilities.d/<token>`.
//!
//! ADVISORY ONLY: never an authorization input.
//! Guest processes run as root and can create files here at will.
//! The directory deliberately sits outside `/etc/fuigo`, the root-owned permission-policy tier.

use std::sync::OnceLock;

use tracing::{info, warn};

/// Default in-image declaration directory (image content tier, matching `/usr/share/fuigo/bundled-skills`).
pub const DEFAULT_CAPABILITIES_DIR: &str = "/usr/share/fuigo/capabilities.d";
/// Override for tests / local dev / non-Linux hosts.
pub const CAPABILITIES_DIR_ENV: &str = "FUIGO_IMAGE_CAPABILITIES_DIR";
/// Re-exported so the reader and the bind reply that carries its output cannot drift on the spelling.
pub use fuigo_tool_protocol::IMAGE_CAPABILITIES_V1;
// The gate and the caps live beside the wire field they bound: every hop that validates the set must apply identical rules
use fuigo_tool_protocol::{MAX_IMAGE_CAPABILITIES, is_image_capability_token};
...
mod tests {
    ...
    use std::fs;
    use std::path::Path;
    use fuigo_tool_protocol::MAX_IMAGE_CAPABILITY_LEN;
    ...
    fn missing_dir_is_unknown() {
        ...
        assert_eq!(caps.state("fuigo-files.occ"), None);
        assert!(!caps.has("fuigo-files.occ"));
    }
    ...
    fn self_token_makes_the_read_authoritative() {
        ...
        touch(tmp.path(), "fuigo-files.occ");
        let caps = load(tmp.path());
        assert!(caps.is_declared());
        assert_eq!(caps.state("fuigo-files.occ"), Some(true));
        assert!(caps.has("fuigo-files.occ"));
        // Declared-and-absent, not unknown.
        assert_eq!(caps.state("vercel.cli"), Some(false));
        assert!(!caps.has("vercel.cli"));
        assert_eq!(caps.wire(), ["capabilities.v1", "fuigo-files.occ"]);
    }
    ...
    fn only_plausible_tokens_are_noteworthy() {
        for name in ["Grove.Daemon", "notes.txt", "fuigo_files.occ", "trailing."] {
