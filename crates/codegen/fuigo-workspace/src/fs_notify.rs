...
//! FsNotify adapters that forward [`fuigo_fsnotify`] events to workspace subsystems (hunk tracker, codebase graph, workspace event broadcast).
//!
//! [`fs_event_to_codebase_graph_event`] converts FsNotify file-change events for the [`IndexManagerHandle`](fuigo_codebase_graph::IndexManagerHandle).
...
/// Hidden-directory paths (relative to `cwd`) are filtered out so the hunk tracker never sees `.git/`, `.fuigo/`, etc.
...
pub(crate) fn fs_event_to_codebase_graph_event(
    paths: &[PathBuf],
    kind: FsEventKind,
) -> fuigo_codebase_graph::FileEvent {
    use fuigo_codebase_graph::{FileEvent, FileEventKind};
...
/// Convert [`fuigo_fsnotify::FsEventKind`] to the wire-type [`fuigo_workspace_types::FsEventKind`].
...
pub(crate) fn to_workspace_event_kind(kind: FsEventKind) -> fuigo_workspace_types::FsEventKind {
    match kind {
        FsEventKind::Created => fuigo_workspace_types::FsEventKind::Created,
        FsEventKind::Modified => fuigo_workspace_types::FsEventKind::Modified,
        FsEventKind::Removed => fuigo_workspace_types::FsEventKind::Removed,
        FsEventKind::Renamed => fuigo_workspace_types::FsEventKind::Renamed,
        // `#[non_exhaustive]` fallback.
        _ => fuigo_workspace_types::FsEventKind::Modified,
    }
}

/// Spawn a background task that reads [`FsEvent`]s from a broadcast receiver and forwards `FilesChanged` to the hunk tracker.
/// Each affected path is re-broadcast as [`WorkspaceEvent::FsChanged`](fuigo_workspace_types::WorkspaceEvent::FsChanged) on the event bus.
///
/// The task exits when:
                        ...
                        Ok(FsEvent::FilesChanged { ref paths, kind }) => {
                            ...
                            for path in paths {
                                let _ = events_tx.send(
                                    fuigo_workspace_types::WorkspaceEvent::FsChanged {
                                        path: path.clone(),
                                        kind: ws_kind,
                                    },
                                );
...
fn parse_diff_name_status_line(
    line: &str,
    repo_root: &Path,
) -> Option<fuigo_codebase_graph::FileEvent> {
    ...
    match status.chars().next()? {
        ...
        _ => Some(fuigo_codebase_graph::FileEvent::modified(
            repo_root.join(path),
        )),
    }
}
...
pub(crate) async fn refresh_codebase_graph_after_head_change(
    idx: &fuigo_codebase_graph::IndexManagerHandle,
    repo_root: &Path,
    events_tx: &tokio::sync::broadcast::Sender<fuigo_workspace_types::WorkspaceEvent>,
) {
    ...
    fuigo_tools::util::detach_command(&mut diff_cmd);
    diff_cmd.envs(fuigo_tools::util::pager_env());
    ...
    if let Some(count) = files_updated {
        let _ = events_tx.send(
            fuigo_workspace_types::WorkspaceEvent::CodebaseIndexUpdated {
                files_indexed: count,
            },
        );
    }
...
pub(crate) fn ws_event_to_codebase_graph_event(
    path: &std::path::Path,
    kind: fuigo_workspace_types::FsEventKind,
) -> fuigo_codebase_graph::FileEvent {
    use fuigo_codebase_graph::{FileEvent, FileEventKind};
    let graph_kind = match kind {
        fuigo_workspace_types::FsEventKind::Created => FileEventKind::Created,
        fuigo_workspace_types::FsEventKind::Modified => FileEventKind::Modified,
        fuigo_workspace_types::FsEventKind::Removed => FileEventKind::Removed,
        fuigo_workspace_types::FsEventKind::Renamed => FileEventKind::Renamed,
    };
...
mod tests {
    ...
    fn hidden_dir_positive() {
        assert!(is_under_hidden_dir(
            &PathBuf::from("/workspace/.fuigo/worktrees/abc/src/main.rs"),
    ...
    fn workspace_event_kind_round_trip() {
        use fuigo_workspace_types::FsEventKind as WsKind;
        assert_eq!(
    ...
    fn codebase_graph_event_mapping() {
        use fuigo_codebase_graph::FileEventKind;
