...
//!
//! Delegates to the existing discovery implementations in `fuigo-agent` and `fuigo-tools` rather than duplicating them.
//! The workspace stores configuration on [`WorkspaceShared`] and the [`MpscChannel`] methods call these helpers.
//!
//! Re-exports [`AgentsMdTracker`] from `fuigo-tools` for external consumers that need per-session project-instruction tracking.

use std::path::Path;

use serde_json::Value;

// Re-export AgentsMdTracker so consumers can reference it via the workspace crate without a direct fuigo-tools dependency
pub use fuigo_tools::types::agents_md_tracker::AgentsMdTracker;

// Re-export the config types that callers pass into WorkspaceConfig.
pub use fuigo_agent::plugins::discovery::DiscoveryConfig as PluginDiscoveryConfig;
pub use fuigo_agent::plugins::trust::TrustStore as PluginTrustStore;
pub use fuigo_agent::prompt::skills::SkillsConfig;
...
///
/// Delegates to [`fuigo_agent::plugins::discover_plugins`].
...
/// Load the project config from `<root_cwd>/.fuigo/config.toml`.
///
...
pub fn load_project_config(root_cwd: &Path) -> Value {
    let config_path = root_cwd.join(".fuigo").join("config.toml");
    match fuigo_config::load_config_file(&config_path) {
...
mod tests {
    ...
    // `~/.fuigo/skills/`, so on a developer machine the result may be
    ...
    async fn discover_skills_finds_skill_md() {
        ...
        let skills_dir = tmp.path().join(".fuigo").join("skills").join("my-skill");
    ...
    async fn discover_skills_respects_ignore_config() {
        let tmp = tempfile::tempdir().unwrap();
        let skills_dir = tmp.path().join(".fuigo").join("skills").join("ignored");
        fs::create_dir_all(&skills_dir).unwrap();
        fs::write(
            skills_dir.join("SKILL.md"),
            "---\nname: ignored\ndescription: Ignored\n---\n",
    ...
    fn agent_config_file_wire_matches_workspace_types_mirror() {
        // The RPC serializes fuigo-build's AgentConfigFile and the remote consumer deserializes the workspace-types mirror
        // Pin the cross-crate serde shape so a rename/attr drift on either side can't silently break discovery
        let src = fuigo_agent::prompt::agents_md::AgentConfigFile {
            file_name: "AGENTS.md".to_string(),
            file_path: "/repo/AGENTS.md".to_string(),
            content: "# Instructions\n".to_string(),
        };
        let json = serde_json::to_value(&src).unwrap();
        let mirror: fuigo_workspace_types::rpc::agents_md::AgentConfigFile =
    ...
    // Discovery also scans the real `~/.fuigo`, so fixtures use test-unique names.
    #[tokio::test]
    ...
    // from `~/.fuigo/plugins/`, so tests check for specific plugins.

    #[test]
    fn discover_plugins_finds_manifest_plugin() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join(".fuigo").join("plugins").join("test-plugin");
    ...
    fn discover_plugins_json_has_expected_fields() {
        let tmp = tempfile::tempdir().unwrap();
        let plugins_dir = tmp.path().join(".fuigo").join("plugins").join("field-test");
        fs::create_dir_all(plugins_dir.join("skills")).unwrap();
        fs::write(
            plugins_dir.join("plugin.json"),
    ...
    // On a developer machine with Fuigo installed it may return non-Null even for a temp directory
