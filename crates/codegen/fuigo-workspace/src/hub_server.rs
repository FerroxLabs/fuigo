...
use fuigo_computer_hub_sdk::ToolServerHandler;
use fuigo_tool_protocol::{HookEvent, HookFrame, SessionId, ToolId, ToolServerEvictParams};
...
use fuigo_tools::computer::types::KillOutcome;
use fuigo_tools::computer::types::TaskKind;
use fuigo_tools::implementations::fuigo_build::scheduler::interval::interval_to_human;
use fuigo_tools::implementations::fuigo_build::scheduler::types::{
    SchedulerCommand, SchedulerHandle,
};
use fuigo_tools::registry::types::FinalizedToolset;
use fuigo_tools::types::resources::Terminal;
...
static WORKSPACE_RPC_CALLER_MISMATCH_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "fuigo_workspace_rpc_caller_mismatch_total",
...
static WORKSPACE_RPC_MUTATION_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "fuigo_workspace_rpc_mutation_total",
...
static WORKSPACE_RPC_REQUESTS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "fuigo_workspace_rpc_requests_total",
...
static WORKSPACE_RPC_ERRORS_TOTAL: std::sync::LazyLock<IntCounterVec> =
    std::sync::LazyLock::new(|| {
        register_int_counter_vec!(
            "fuigo_workspace_rpc_errors_total",
...
static WORKSPACE_RPC_DURATION_SECONDS: std::sync::LazyLock<HistogramVec> =
    std::sync::LazyLock::new(|| {
        register_histogram_vec!(
            "fuigo_workspace_rpc_duration_seconds",
...
) -> Vec<fuigo_workspace_types::rpc::workspace::BackgroundTaskSummaryWire> {
    use fuigo_tools::computer::types::TaskKind;
    use fuigo_tools::types::resources::Terminal;
    use fuigo_tools::types::tool::ToolKind;
    use fuigo_workspace_types::rpc::workspace::BackgroundTaskSummaryWire;
    let terminal = {
...
async fn list_session_todos(
    toolset: &fuigo_tools::registry::types::FinalizedToolset,
) -> Vec<fuigo_workspace_types::rpc::workspace::TodoSummaryWire> {
    use fuigo_tools::implementations::fuigo_build::todo::{TodoState, TodoStatus};
    use fuigo_tools::types::resources::State;
    use fuigo_workspace_types::rpc::workspace::TodoSummaryWire;
    let res = toolset.resources.lock().await;
...
impl WorkspaceRpcHandler {
    ...
    async fn dispatch(
        ...
        use fuigo_workspace_types::rpc::git::{GitBranchInfoReq, GitMetadataReq};
        use fuigo_workspace_types::rpc::presence::PresenceNoteReq;
        use fuigo_workspace_types::rpc::search::FuzzyStatusReq;
        use fuigo_workspace_types::rpc::skills::DiscoverPluginsReq;
        use fuigo_workspace_types::rpc::workspace::{
        ...
        use fuigo_workspace_types::rpc::worktree::WorktreeCreateSyncReq;
        ...
        match method {
            <WorkspaceInfoReq as WorkspaceRpc>::METHOD => {
                ...
                let info = WorkspaceInfo {
                    ...
                    version: Some(fuigo_version::VERSION.to_owned()),
                };
...
impl ToolServerHandler for WorkspaceRpcHandler {
    ...
    async fn handle_call(&self, ctx: ToolCallContext, args: Value) -> ToolStream<TypedToolOutput> {
        ...
        let bound_session = ctx.extensions.get::<fuigo_tool_runtime::SessionContext>();
    ...
    async fn handle_hook(&self, session_id: SessionId, frame: HookFrame) {
        match frame.event {
            ...
            HookEvent::Custom { kind, payload } => {
                use fuigo_tool_protocol::turn_hook::{
    ...
    async fn handle_hook_request(&self, session_id: SessionId, frame: HookFrame) -> Option<Value> {
        use fuigo_tool_protocol::turn_hook::{self, TurnHookRequest};
