...
// Not re-exported: the `ClientFs*` types live in `fuigo_workspace_types::rpc::fs` and would collide with the shell-facing `ext_fs` names above
...
pub use fuigo_workspace_types::rpc::fs::FsReadEncoding;
...
pub use fuigo_fuzzy_file_search::{
...
// These types are canonical in fuigo-workspace-types; the re-export keeps existing import paths working
pub use fuigo_workspace_types::rpc::search::{ClientId, ContentSearchRequest, TargetClientId};
