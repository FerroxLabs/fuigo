...
use fuigo_tools::registry::types::{SessionContext, ToolRegistryBuilder, ToolServerConfig};
...
impl SessionTerminalBackend {
    ...
    pub fn local(backend: fuigo_tools::computer::local::LocalTerminalBackend) -> Self {
    ...
    pub fn backend(&self) -> &Arc<dyn fuigo_tools::computer::types::TerminalBackend> {
        &self.backend
    }
...
pub struct WorkspaceBindConfig {
    ...
    pub viewer_ctx: Option<fuigo_tool_runtime::WorkspaceViewerContext>,
    ...
    pub tools: Option<Vec<fuigo_tools_api::ToolConfigEntry>>,
...
pub enum ResolvedToolset {
    ...
    InvalidToolConfig(fuigo_tools::registry::proto_convert::ToolConfigEntryError),
}
...
impl WorkspaceBindConfig {
    ...
    /// The envelope is the shared [`fuigo_tool_runtime::WorkspaceBindMetadata`], the same type the emitter serializes.
    /// `tool_config` is a consumer-only raw escape hatch read separately.
    pub fn from_metadata(metadata: &serde_json::Value) -> Self {
        let wire: fuigo_tool_runtime::WorkspaceBindMetadata =
    ...
    pub fn resolve(
        ...
        if let Some(cfg) = &self.tool_config {
            for (idx, tool) in cfg.tools.iter().enumerate() {
                if let Err(err) = fuigo_tools_api::config_validation::validate_name_override(
                    idx,
                    &tool.id,
        ...
        if let Some(tools) = &self.tools {
            ...
            for (idx, entry) in tools.iter().enumerate() {
                ...
                match fuigo_tools::registry::proto_convert::tool_config_from_entry(
                    idx,
                    entry.clone(),
                ) {
            ...
            if !unserved_tool_ids.is_empty() {
                tracing::warn!(
                    ...
                    running_version = fuigo_version::VERSION,
...
mod bind_config_tests {
    ...
    fn presets_are_never_resolved() {
        for preset in ["explore", "fuigo-computer", "bogus"] {
    ...
    fn tools_entries_resolve_to_tool_server_config() {
        ...
        assert_eq!(grep.id, "FuigoBuild:grep");
        assert_eq!(
            grep.params,
        ...
        assert_eq!(toolset.tools[1].id, "FuigoBuild:read_file");
    }
    #[test]
...
impl BindMcpConfig {
    ...
    /// ([`fuigo_tool_protocol::SESSION_BIND_ACK_TIMEOUT`], which the hub's ws
    ...
    pub fn bind_converge_grace_within(&self, elapsed_since_bind_start: Duration) -> Duration {
        let remaining = fuigo_tool_protocol::SESSION_BIND_ACK_TIMEOUT
...
pub struct WorkspaceConfig {
    ...
    /// Global hook sources (e.g. `~/.claude/settings.json`, `~/.fuigo/hooks/`).
    pub hook_global_sources: Vec<HookSourceConfig>,
    /// Project-scoped hook sources (e.g. `<project>/.fuigo/hooks/`).
    ...
    /// Auth provider for Ferrox Labs service calls made from workspace-scoped code.
    ...
    /// Folder-trust verdict for repo-local (project-scoped) LSP servers from `<cwd>/.fuigo/lsp.json`.
    ...
    /// Confine `fuigo/fs/*` / `workspace.fs_*` resolution to the workspace root (reject `..`, absolute-outside-root, symlink escapes).
...
pub use fuigo_tool_protocol::ServerIdentityMetadata as WorkspaceServerMetadata;
/// Merge an env-sourced logical session id into caller-supplied tool-server metadata (`None` on the restore/local path).
///
/// `env_session_id` is the raw `FUIGO_SESSION_ID`; empty is normalized to absent.
...
impl WorkspaceConfig {
    ...
    pub fn new_for_proxy(
        ...
        auth_provider: fuigo_computer_hub_sdk::SharedAuthProvider,
...
/// Maps 1:1 to [`fuigo_hooks::discovery::HookSource`] but uses owned `PathBuf` so the config struct is `'static`.
#[derive(Debug, Clone)]
pub enum HookSourceConfig {
    /// A single JSON settings file (e.g. `~/.claude/settings.json`).
    SettingsFile(PathBuf),
    /// A directory of `*.json` hook files (e.g. `~/.fuigo/hooks/`).
    Directory(PathBuf),
}
...
mod bind_mcp_config_tests {
    ...
    const HUB_BIND_ACK_TIMEOUT: Duration = fuigo_tool_protocol::SESSION_BIND_ACK_TIMEOUT;
    ...
    fn duplicate_server_names_dedupe_last_wins() {
        ...
        let names: Vec<&str> = config
            .servers()
            .iter()
            .map(fuigo_mcp::servers::mcp_server_name)
            .collect();
        assert_eq!(
            names,
            ["other", "dup"],
    ...
    fn server_list_is_capped_deterministically() {
        ...
        let first = fuigo_mcp::servers::mcp_server_name(&config.servers()[0]);
        let last =
            fuigo_mcp::servers::mcp_server_name(&config.servers()[BindMcpConfig::MAX_SERVERS - 1]);
