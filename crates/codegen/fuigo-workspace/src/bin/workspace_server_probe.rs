...
async fn call_tool(
    harness: &fuigo_computer_hub_sdk::ToolHarness,
    name: &str,
    args: Value,
) -> anyhow::Result<Value> {
...
async fn connect_and_bind(
...
) -> anyhow::Result<fuigo_computer_hub_sdk::ToolHarness> {
    ...
    let metadata = json!({
        "tools": [
            {"id": "FuigoBuild:run_terminal_cmd", "name_override": "run_terminal_command"},
