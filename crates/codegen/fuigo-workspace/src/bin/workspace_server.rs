...
use fuigo_workspace_daemon::daemonize;
...
fn server_id_startup_error(id: &str) -> Option<String> {
    id.parse::<fuigo_tool_protocol::ServerId>()
        .err()
...
#[derive(Parser)]
#[command(name = "fuigo-workspace-server")]
...
struct Args {
    ...
    /// Pass `--upload-queue-enabled false` (or set `FUIGO_WORKSPACE_UPLOAD_QUEUE_ENABLED=false`) to fall back to the legacy inline path.
    #[arg(
        long,
    ...
    /// Trust project-scoped LSP servers from `<repo>/.fuigo/lsp.json`.
    ...
    /// Force `FUIGO_TOOLS_RESET_CHILD_OOM` so shell/pty children reset to 0.
...
/// Forwarded 1:1 to the `/usr/local/bin/fuigo-preview-proxy` child (see `cli.rs` for the proxy's flag names).
...
/// `fuigo-workspace-daemon` deliberately does not depend on `fuigo-workspace`, so this binary owns the one adapter between them.
struct TrackerSink(std::sync::Arc<fuigo_workspace::activity::ActivityTracker>);
...
fn main() -> anyhow::Result<()> {
    ...
    if should_set_reset_child_oom(oom_protection.is_ok(), args.oom_protect) {
        unsafe { std::env::set_var(fuigo_tty_utils::RESET_CHILD_OOM_ENV, "1") };
    }
...
async fn run(
    ...
    let donating = fuigo_computer_hub_sdk::DonatingLogLayer::new_inert();
    ...
    let direct_otlp = match std::env::var("FUIGO_WORKSPACE_OTLP_ENDPOINT") {
        Ok(endpoint) if !endpoint.is_empty() => {
            match fuigo_tracing::init_fastrace(endpoint.clone(), SERVICE_NAME.to_owned(), None) {
                Ok(()) => {
            ...
            Some(val) => {
                ...
                if matches!(parsed, ProfileName::Custom(_)) {
                    tracing::warn!(value = %val,
                        "Unrecognized FUIGO_SANDBOX_PROFILE, defaulting to workspace");
                    ProfileName::Workspace
                } else {
                    parsed
                }
            }
            None if fuigo_sandbox::trust_bwrap_marker_for_devbox() => ProfileName::Devbox,
            None => ProfileName::Workspace,
        };
        let profile_name = profile.to_string();
        if profile == ProfileName::Off {
            tracing::info!(profile = %profile_name, "Sandbox explicitly disabled via FUIGO_SANDBOX_PROFILE=off");
        } else {
            ...
            sandbox.install();
            let active = fuigo_sandbox::is_active();
            ...
            tracing::info!(
                ...
                active,
                restrict_network_at_known_linux_launches = fuigo_sandbox::should_restrict_child_network(),
                "{status_msg}"
            );
        }
    }
    let mut status_config = fuigo_workspace::StatusConfig::from_env();
    status_config.preview_control_port = args.preview.preview_control_port;
    let auth_provider = fuigo_workspace::hub_auth::provider(
        &url,
    ...
    let session_id = std::env::var("FUIGO_SESSION_ID").ok();
    ...
    {
        let caps = fuigo_workspace::image_capabilities::image_capabilities();
    ...
    fuigo_workspace::init_metrics();
    let ws_handle = match fuigo_workspace::handle::connect_local_workspace(
        cwd,
        url,
        auth_provider,
        metadata,
    ...
    fuigo_computer_hub_sdk::flush_log_layer();
...
mod tests {
    ...
    fn classify_from_client_error_display_round_trip() {
        let handshake = WorkspaceError::HubError(
            fuigo_computer_hub_sdk::ClientError::HandshakeAuthFailed { status: 401 }.to_string(),
        );
        ...
        let auth = WorkspaceError::HubError(
            fuigo_computer_hub_sdk::ClientError::AuthError("token rejected".into()).to_string(),
        );
        assert_eq!(
        ...
        let network = WorkspaceError::HubError(
            fuigo_computer_hub_sdk::ClientError::NetworkError("connection refused".into())
                .to_string(),
        );
        assert_eq!(
    ...
    fn capabilities_flag_parses_and_defaults_off() {
        ...
        let args = Args::try_parse_from(["fuigo-workspace-server", "--capabilities"]).unwrap();
        assert!(args.capabilities);
    }
    #[test]
    ...
    fn capabilities_probe_of_legacy_binary_exits_nonzero() {
        ...
        let err = LegacyArgs::try_parse_from(["fuigo-workspace-server", "--capabilities"])
    ...
    fn oom_protect_flag_parses() {
        let args = Args::try_parse_from(["fuigo-workspace-server", "--oom-protect"]).unwrap();
        assert!(args.oom_protect);
    }
    #[test]
    ...
    fn argv_rejection_exit_code_is_distinct_from_server_id_exit_code() {
        let err = Args::try_parse_from(["fuigo-workspace-server", "--flag-from-the-future"])
            .err()
    ...
    /// The scraper's own behavior is tested in `fuigo-workspace-daemon`, and the tracker's in `fuigo_workspace::activity`.
    /// This test covers only the adapter.
    #[test]
    fn tracker_sink_forwards_every_signal_to_the_activity_tracker() {
        use fuigo_workspace::activity::ActivityTracker;
