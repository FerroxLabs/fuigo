...
use dashmap::DashMap;
use fuigo_file_utils::queue::UploadQueueStats;
use fuigo_session_events::{Event, EventWriter, ToolCompletedSource, ToolOutcome};
use fuigo_tool_protocol::{IdleWithholdReason, ToolServerLifecycleStatus, ToolServerStatusPayload};
...
/// Overridable via `FUIGO_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
...
pub(crate) const PRESENCE_ACTIVITY_WINDOW_MS: u64 =
    fuigo_workspace_types::rpc::presence::PRESENCE_ACTIVITY_WINDOW_MS;
...
impl ActivityTracker {
    ...
    /// The durability idle-hold cap comes from `FUIGO_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS` (default [`DEFAULT_DURABILITY_IDLE_HOLD_MAX_MS`]).
    ...
    pub fn snapshot_session(&self, session_id: &str) -> ToolServerStatusPayload {
        ...
        ToolServerStatusPayload {
            status,
            session_id: fuigo_tool_protocol::SessionId::new(session_id).ok(),
            connection_id: None,
...
/// The durability idle-hold cap from `FUIGO_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS`.
fn durability_idle_hold_max_from_env() -> u64 {
    durability_idle_hold_from_raw(std::env::var("FUIGO_WORKSPACE_DURABILITY_IDLE_HOLD_MAX_MS").ok())
}
