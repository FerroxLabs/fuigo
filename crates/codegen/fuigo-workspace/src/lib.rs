...
pub use file_system::*;
pub use fuigo_hunk_tracker::HunkTrackerHandle;
pub use fuigo_workspace_client::WorkspaceClient;
pub use fuigo_workspace_types::WorkspaceEvent;
pub use handle::{
...
/// Crate-wide lock serializing every test that mutates the process-global environment (`FUIGO_HOME`, `HOME`, …).
...
mod init_metrics_tests {
    ...
    fn init_metrics_is_idempotent_and_registers_baselines() {
        ...
        assert!(has(
            "fuigo_workspace_toolset_swap_rejected_total",
        ...
        for outcome in [
            ...
            assert!(
                has(
                    "fuigo_workspace_oidc_proactive_refresh_total",
                    &[("outcome", outcome)]
                ),
        ...
        assert!(has(
            "fuigo_workspace_orphan_lost_total",
            &[("reason", "sha_mismatch")]
        ));
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "fuigo_workspace_env_capture_panic_total")
        );
        assert!(
            families
                .iter()
                .any(|mf| mf.name() == "fuigo_workspace_permission_timeout_total")
        );
    }
}
