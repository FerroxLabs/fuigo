...
impl HubHandle {
    ...
    pub(crate) async fn connect(
        ...
        let mut server_builder = ToolServerBuilder::default()
            ...
            .binary_version(fuigo_version::VERSION)
            .image_capabilities(
...
pub(crate) struct SessionRoutedToolHandler {
    ...
    semantic_kind: Option<fuigo_tools::types::tool::ToolKind>,
    schema: Option<Value>,
...
struct CallCompletedGuard {
    ...
    outcome: fuigo_session_events::ToolOutcome,
}
impl CallCompletedGuard {
    fn new(
        ...
        Self {
            ...
            call_id,
            session_id,
            outcome: fuigo_session_events::ToolOutcome::Cancelled,
        }
    }
    fn set_outcome(&mut self, outcome: fuigo_session_events::ToolOutcome) {
            ...
            while let Some(item) = inner.next().await {
                match item {
                    ...
                    ToolStreamItem::Terminal(Ok(run_result)) => {
                        _guard.set_outcome(fuigo_session_events::ToolOutcome::Success);
                    ...
                    ToolStreamItem::Terminal(Err(e)) => {
                        ...
                        _guard.set_outcome(fuigo_session_events::ToolOutcome::Error);
                        let e = match &virt {
...
fn parse_server_id(id: &str) -> Result<fuigo_tool_protocol::ServerId, ClientError> {
    fuigo_tool_protocol::ServerId::new(id)
...
mod tests {
    ...
    async fn renamed_active_message_handler_keeps_semantic_hitl_classification() {
        let handle = crate::handle::tests::make_handle();
        let mut config = fuigo_tools::registry::types::ToolConfig::for_tool::<
            fuigo_tools::implementations::fuigo_build::SendSubagentMessageTool,
        >();
        config.name_override = Some("relay_to_subagent".to_owned());
        assert_eq!(
            config.kind,
            Some(fuigo_tools::types::tool::ToolKind::ActiveAgentMessage)
        );
    ...
    async fn handle_call_terminal_matches_non_streaming_call() {
        ...
        match reference {
            Ok(run_result) => {
                ...
                assert_eq!(
                    typed.model_output,
                    vec![fuigo_tool_runtime::ContentBlock::Text {
    ...
    use fuigo_tools::types::tool_metadata::ToolMetadata as FuigoToolMetadata;
    #[derive(Debug)]
    struct GateStreamingStub;
    impl FuigoToolMetadata for GateStreamingStub {
    ...
    impl fuigo_tool_runtime::Tool for GateStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> fuigo_tool_protocol::ToolId {
            fuigo_tool_protocol::ToolId::new("gate_streaming_stub").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::fuigo_tool_runtime::ListToolsContext,
        ) -> fuigo_tool_types::ToolDescription {
            fuigo_tool_types::ToolDescription::new("gate_streaming_stub", "gate streaming stub")
        }
        async fn run(
            &self,
        ...
        async fn execute(
            ...
            use fuigo_tool_runtime::{ToolProgress, ToolStreamItem};
    ...
    fn bg_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("FuigoBuild:monitor", Some(ToolKind::Monitor)),
            ],
    ...
    fn task_completed_notif(task_id: &str) -> ToolNotification {
        use fuigo_tools::computer::types::{TaskKind, TaskSnapshot};
    ...
    impl FuigoToolMetadata for PathEchoStub {
        fn kind(&self) -> ToolKind {
            ToolKind::Other
        }
        fn tool_namespace(&self) -> fuigo_tools::types::tool::ToolNamespace {
    ...
    impl fuigo_tool_runtime::Tool for PathEchoStub {
        type Args = serde_json::Value;
        type Output = serde_json::Value;
        fn id(&self) -> fuigo_tool_protocol::ToolId {
            fuigo_tool_protocol::ToolId::new("path_echo").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::fuigo_tool_runtime::ListToolsContext,
        ) -> fuigo_tool_types::ToolDescription {
            fuigo_tool_types::ToolDescription::new("path_echo", "path echo stub")
        }
        async fn run(
            &self,
            _ctx: fuigo_tool_runtime::ToolCallContext,
        ...
        async fn execute(
            ...
            input: serde_json::Value,
        ) -> fuigo_tool_runtime::ToolStream<serde_json::Value> {
            use fuigo_tool_runtime::{ToolProgress, ToolStreamItem};
    ...
    async fn drain_terminal(
        stream: fuigo_tool_runtime::ToolStream<fuigo_tool_runtime::TypedToolOutput>,
    ) -> (
        Vec<fuigo_tool_runtime::ToolProgress>,
    ...
    async fn handle_call_virtualizes_inbound_outbound_and_progress() {
        ...
        match progress.first() {
            Some(fuigo_tool_runtime::ToolProgress::Text { text }) => {
    ...
    impl FuigoToolMetadata for PathErrorStub {
        fn kind(&self) -> ToolKind {
            ToolKind::Other
        }
        fn tool_namespace(&self) -> fuigo_tools::types::tool::ToolNamespace {
    ...
    impl fuigo_tool_runtime::Tool for PathErrorStub {
        type Args = serde_json::Value;
        type Output = serde_json::Value;
        fn id(&self) -> fuigo_tool_protocol::ToolId {
            fuigo_tool_protocol::ToolId::new("path_error").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::fuigo_tool_runtime::ListToolsContext,
        ) -> fuigo_tool_types::ToolDescription {
            fuigo_tool_types::ToolDescription::new("path_error", "path error stub")
        }
        async fn run(
            &self,
            _ctx: fuigo_tool_runtime::ToolCallContext,
            ...
            Err(fuigo_tool_runtime::ToolError::new(
    ...
    impl FuigoToolMetadata for BashCcoPathStub {
        fn kind(&self) -> ToolKind {
            ToolKind::Execute
        }
        fn tool_namespace(&self) -> fuigo_tools::types::tool::ToolNamespace {
    ...
    impl fuigo_tool_runtime::Tool for BashCcoPathStub {
        type Args = serde_json::Value;
        type Output = fuigo_tools::types::output::ToolOutput;
        fn id(&self) -> fuigo_tool_protocol::ToolId {
            fuigo_tool_protocol::ToolId::new("bash_cco_path").expect("valid tool id")
        }
        fn description(
            &self,
            _ctx: &::fuigo_tool_runtime::ListToolsContext,
        ) -> fuigo_tool_types::ToolDescription {
            fuigo_tool_types::ToolDescription::new("bash_cco_path", "bash cco path stub")
        }
        async fn run(
            &self,
            _ctx: fuigo_tool_runtime::ToolCallContext,
            ...
            Ok(fuigo_tools::types::output::ToolOutput::Bash(
                fuigo_tools::types::output::BashOutput {
                    ...
                    output_for_prompt:
                        fuigo_tools::types::output::BashOutput::make_output_for_prompt(stdout),
                    exit_code: 0,
