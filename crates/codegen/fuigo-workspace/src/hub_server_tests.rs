...
) -> Option<fuigo_tool_runtime::ToolStreamItem<TypedToolOutput>> {
...
async fn dispatch_workspace_info_reports_server_version() {
    use fuigo_workspace_types::rpc::workspace::{WorkspaceInfo, WorkspaceInfoReq};
    let handler = WorkspaceRpcHandler::new(make_handle());
    let value = handler
        .dispatch(
    ...
    assert_eq!(Some(fuigo_version::VERSION.to_owned()), info.version);
}
#[tokio::test]
...
async fn handle_evict_triggers_two_phase_drain() {
    use fuigo_tool_protocol::ToolServerLifecycleStatus;
    let handle = make_handle();
...
async fn list_background_tasks_rpc_stays_truthful_across_rebinds() {
    ...
    use fuigo_workspace_types::rpc::workspace::ListBackgroundTasksResponse;
    let handle = make_handle();
    ...
    let read_only = ToolServerConfig {
        tools: vec![tc(
            ...
            Some(fuigo_tools::types::tool::ToolKind::Read),
        )],
...
async fn delete_scheduled_task_rpc_reports_honestly() {
    use fuigo_workspace_types::rpc::workspace::DeleteScheduledTaskResponse;
    let handle = make_handle();
    let cfg = background_capable_cfg();
    let session = handle
        .create_session_with_config(
...
async fn kill_task_rpc_terminates_outstanding_background_task() {
    use fuigo_workspace_types::rpc::workspace::{KillTaskOutcome, KillTaskResponse};
    let handle = make_handle();
    let cfg = background_capable_cfg();
    let session = handle
        .create_session_with_config(
...
async fn handle_call_wraps_in_envelope_with_value() {
    ...
    ctx.extensions
        .insert(fuigo_tool_runtime::SessionContext("main".to_owned()));
...
async fn handle_call_records_rpc_metrics_and_collapses_unknown_method() {
    ...
    let has_bogus_series = prometheus::gather()
        .iter()
        .filter(|mf| mf.name() == "fuigo_workspace_rpc_requests_total")
...
async fn handle_hook_cancel_marks_call_completed() {
    use fuigo_tool_protocol::ToolCallId;
    let handle = make_handle();
    let handler = WorkspaceRpcHandler::new(handle.clone());
    let tracker = handle.activity_tracker();
...
async fn dispatch_knows_every_typed_method() {
    ...
    let methods = [
        ...
        <fuigo_workspace_types::rpc::presence::PresenceNoteReq as WorkspaceRpc>::METHOD,
...
async fn dispatch_stamps_client_rpc_activity_for_mutations_only() {
    ...
    use fuigo_workspace_types::rpc::workspace::DropSessionReq;
    use fuigo_tool_protocol::IdleWithholdReason;
    let handler = WorkspaceRpcHandler::new(make_handle());
