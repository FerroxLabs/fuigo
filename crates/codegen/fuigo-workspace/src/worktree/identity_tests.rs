...
fn cwd_at_slug_level_or_at_worktrees_dir_itself_has_no_identity() {
    let worktrees = Path::new("/home/user/.fuigo/worktrees");
    assert_eq!(
        worktree_identity_in(worktrees, "/home/user/.fuigo/worktrees"),
        None
    );
    assert_eq!(
        worktree_identity_in(worktrees, "/home/user/.fuigo/worktrees/fuigo"),
        None
    );
}
...
fn cwd_outside_worktrees_dir_has_no_identity() {
    let worktrees = Path::new("/home/user/.fuigo/worktrees");
    assert_eq!(
        worktree_identity_in(worktrees, "/home/user/projects/fuigo"),
        None
    );
}
...
fn db_recorded_source_wins_over_git_discovery() {
    ...
    let db = fuigo_fast_worktree::WorktreeDb::open(&fixture.home).unwrap();
    db.register(&fuigo_fast_worktree::WorktreeRecord {
        id: "db-wins".to_owned(),
        path: wt.clone(),
...
fn standalone_clone_behind_symlinked_worktrees_dir_reports_no_source() {
    ...
    fuigo_test_utils::git::init_git_repo(&standalone);
    std::fs::write(standalone.join("tracked.txt"), "x").unwrap();
    fuigo_test_utils::git::git_commit_all(&standalone, "initial");
    let link_home = root.join("link-home");
    std::os::unix::fs::symlink(&real_home, &link_home).unwrap();
    let _env = LockedTestEnv::lock().set("FUIGO_HOME", &link_home);
...
fn standalone_clone_with_source_marker_and_no_db_derives_marker_source() {
    ...
    fuigo_test_utils::git::git_commit_all(&standalone, "initial");
    std::fs::write(
        standalone.join(".git").join("fuigo-worktree-source"),
        "/marker-source\n",
    )
    .unwrap();
