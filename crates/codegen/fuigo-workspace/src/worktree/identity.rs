//! Path-derived identity of a fuigo-managed worktree.
//!
//! Every creation path resolves its destination to `<fuigo home>/worktrees/<repo slug>/<label>`, with the label as the last path component.
...
/// Identity of the fuigo-managed worktree containing a cwd.
...
/// [`worktree_identity_in`] against the default `<fuigo home>/worktrees`.
pub fn worktree_identity_for_cwd(cwd: &str) -> Option<WorktreeIdentity> {
    worktree_identity_in(&super::fuigo_home().join("worktrees"), cwd)
}
...
pub fn worktree_identity_in(worktrees_dir: &Path, cwd: &str) -> Option<WorktreeIdentity> {
    ...
    // FUIGO_HOME (and so `worktrees_dir`) is often the unresolved env spelling
    ...
    let source_workspace_dir = super::source_repo_for_cwd(cwd)
        ...
        .or_else(|| {
            // NO_SEARCH, not `discover`: every fuigo-created worktree has `.git` at its root
            // An upward walk would let a stray non-repo directory here inherit a repository enclosing fuigo home, like a git-managed home directory
            ...
            // Otherwise a symlinked fuigo home lets a standalone clone report itself as the source
...
fn standalone_source_marker(worktree_root: &Path) -> Option<PathBuf> {
    let contents =
        std::fs::read_to_string(worktree_root.join(".git").join("fuigo-worktree-source")).ok()?;
