...
use fuigo_fast_worktree::{BtrfsDelegate, IgnoredFilesMode, WorkingTreeMode, WorktreeBuilder};
...
fn enabled_grove_opts() -> fuigo_fast_worktree::NfsWorktreeOpts {
    fuigo_fast_worktree::NfsWorktreeOpts {
        enabled: true,
        ..fuigo_fast_worktree::NfsWorktreeOpts::default()
    }
}
...
fn resolve_grove_fuse_creation_type(
    ...
    let linked = grove_enabled
        && fuigo_fast_worktree::source_is_linked_local_view(&enabled_grove_opts(), source);
...
pub(crate) fn to_creation_mode(t: WorktreeType) -> fuigo_fast_worktree::CreationMode {
    match t {
        WorktreeType::Linked => fuigo_fast_worktree::CreationMode::Linked,
        WorktreeType::Standalone => fuigo_fast_worktree::CreationMode::Standalone,
        WorktreeType::Git => fuigo_fast_worktree::CreationMode::GitCheckout,
    }
}
...
pub use fuigo_fast_worktree::META_KEY_LABEL;
...
/// Fuigo home for worktree paths: the same resolver as `worktrees.db`, with a `temp_dir()/.fuigo` last resort.
/// This is not fuigo-config's cwd-relative `.fuigo`: worktree paths need an absolute, always-writable anchor that does not move with the process cwd.
fn fuigo_home() -> std::path::PathBuf {
    fuigo_fast_worktree::resolve_fuigo_home().unwrap_or_else(|_| std::env::temp_dir().join(".fuigo"))
}

/// Returns `~/.fuigo/worktrees/<repo_slug>` for the given git root.
pub fn worktree_base_dir(git_root: &Path) -> std::path::PathBuf {
    let slug = repo_slug(git_root);
    fuigo_home().join("worktrees").join(slug)
}

/// Resolves the worktree base directory (`~/.fuigo/worktrees/<repo_name>`)
/// for a given source path, correctly handling fuigo-managed worktrees.
///
/// When `source_path` is already under `~/.fuigo/worktrees/<repo>/...`, the
...
///
/// For paths outside the fuigo worktree directory, falls back to `find_main_repo_root_from_path` then `worktree_base_dir`.
pub fn worktree_base_dir_for_source(source_path: &Path) -> Result<std::path::PathBuf> {
    let worktrees_dir = fuigo_home().join("worktrees");
...
/// Walk up from `cwd` (staying within `~/.fuigo/worktrees/`) to its registered
/// worktree record.
///
...
/// The recorded source repo of the fuigo-managed worktree containing `cwd`, if any.
///
/// Thin wrapper over [`worktree_record_for_cwd`] that drops the DB handle;
/// returns `None` (without DB I/O) for paths outside `~/.fuigo/worktrees/`.
...
pub async fn create_worktree_streaming<N: WorktreeNotificationSender>(
    ...
    let report = match tokio::task::spawn_blocking(move || {
        let mut builder = WorktreeBuilder::new(&source_path, &dest_path)
            ...
            .worktree_kind(fuigo_fast_worktree::WorktreeKind::Session)
...
pub async fn remove_worktree(
    ...
    match tokio::task::spawn_blocking(move || {
        fuigo_fast_worktree::remove_worktree_with_delegate(&wt_path, delegate)
    })
    .await
    {
        Ok(Ok(report)) => {
...
pub async fn rehydrate_subagent_worktree(
    ...
    let report = tokio::task::spawn_blocking(move || {
        fuigo_fast_worktree::rehydrate_worktree_from_ref(
            &dest,
            &source_repo,
            &snapshot_ref,
            session_id.as_deref(),
...
pub async fn snapshot_subagent_worktree(
    ...
    tokio::task::spawn_blocking(move || -> Result<String> {
        ...
        fuigo_fast_worktree::snapshot_worktree_to_ref(&worktree_path, &ref_name, &message)?;
        fuigo_fast_worktree::transfer_snapshot_to_repo(&worktree_path, &source_repo, &ref_name)?;
        Ok(ref_name)
    })
    .await
...
pub async fn remove_subagent_worktree(worktree_path: &Path) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || {
        fuigo_fast_worktree::remove_worktree_with_delegate(&worktree_path, delegate)
    })
    .await
...
///
/// When the source path is already inside `~/.fuigo/worktrees/<repo>/`, the
...
async fn cleanup_cancelled_worktree(worktree_path: &str) {
    ...
    let delegate = btrfs_delegate_from_env();
    match tokio::task::spawn_blocking(move || {
        fuigo_fast_worktree::remove_worktree_with_delegate(&path, delegate)
    })
    .await
    {
        Ok(Ok(_)) => {}
    ...
    let builder_result = {
        ...
        tokio::task::spawn_blocking(move || {
            let mut builder = WorktreeBuilder::new(&source_worktree_path, &dest_path)
                ...
                .ignored_files_mode(IgnoredFilesMode::Skip)
                .creation_mode(to_creation_mode(creation_mode))
                .worktree_kind(fuigo_fast_worktree::WorktreeKind::Fork)
                .session_id(session_id_for_builder)
                .metadata(label_metadata);
...
///
/// ACP equivalent of `fuigo -w -r <session_id>` (optionally with `--ref`).
...
use fuigo_fast_worktree::{
    DbStats, GcOptions, GcReport, ListFilter, WorktreeAutoGcLayer, WorktreeDb, WorktreeKind,
    WorktreeRecord, gc_worktrees as fw_gc_worktrees, rebuild_worktree_db, resolve_fuigo_home,
...
pub fn detach_worktree_mgmt(
    id_or_path: &str,
    allow_copy: bool,
) -> Result<fuigo_fast_worktree::DetachReply> {
    let path = resolve_mgmt_path(id_or_path)?;
    let client = fuigo_fast_worktree::NfsWorktreeClient::from_opts(
        &fuigo_fast_worktree::NfsWorktreeOpts::default(),
    );
...
pub fn salvage_worktree_mgmt(
    id_or_path: &str,
    out: &str,
) -> Result<fuigo_fast_worktree::SalvageReply> {
    let path = resolve_mgmt_path(id_or_path)?;
    let client = fuigo_fast_worktree::NfsWorktreeClient::from_opts(
        &fuigo_fast_worktree::NfsWorktreeOpts::default(),
    );
    match client.salvage_worktree(&path, std::path::Path::new(out)) {
        ...
        Err(e) if e.to_string().contains("unreachable") => {
            fuigo_fast_worktree::local_salvage(&path, std::path::Path::new(out))
        }
        Err(e) => Err(e),
    }
}

pub fn clean_artifacts_mgmt(id_or_path: &str) -> Result<fuigo_fast_worktree::CleanArtifactsReply> {
    let path = resolve_mgmt_path(id_or_path)?;
    let client = fuigo_fast_worktree::NfsWorktreeClient::from_opts(
        &fuigo_fast_worktree::NfsWorktreeOpts::default(),
    );
    match client.clean_artifacts(&path) {
        Ok(r) => Ok(r),
        Err(e) if e.to_string().contains("unreachable") => {
            fuigo_fast_worktree::local_clean_artifacts(&path)
        }
        Err(e) => Err(e),
    }
}
...
pub fn worktree_auto_gc_layer_from_settings(
    s: &fuigo_config_types::WorktreeAutoGcSettings,
) -> WorktreeAutoGcLayer {
    use std::collections::BTreeMap;
    use fuigo_config_types::WorktreeKindMaxAge;
    ...
    let fuigo_config_types::WorktreeAutoGcSettings {
        enabled,
        max_age_secs,
        min_interval_secs,
        dry_run,
...
/// Parse `[worktree.auto_gc]` out of the workspace's `$FUIGO_HOME/config.toml`.
///
/// Returns `None` when the table is absent or fails to deserialize.
fn worktree_auto_gc_settings_from_toml(
    root: &toml::Value,
) -> Option<fuigo_config_types::WorktreeAutoGcSettings> {
    root.get("worktree")
        .and_then(|w| w.get("auto_gc"))
        // toml::Value only deserializes by value (no &Value Deserializer).
        .and_then(|v| fuigo_config_types::WorktreeAutoGcSettings::deserialize(v.clone()).ok())
}

/// Remote-blind (env and `$FUIGO_HOME/config.toml` only): opts in only when local `[worktree.auto_gc] enabled = true`, else returns `None`.
...
fn resolve_worktree_auto_gc_local() -> Option<fuigo_fast_worktree::ResolvedWorktreeAutoGc> {
    let local = if let Ok(home) = resolve_fuigo_home() {
...
/// Split out of `resolve_worktree_auto_gc_local` so the fail-safe is testable without touching `$FUIGO_HOME`.
fn local_auto_gc_policy(
    local: Option<&fuigo_config_types::WorktreeAutoGcSettings>,
) -> Option<fuigo_fast_worktree::ResolvedWorktreeAutoGc> {
...
pub fn worktree_db_rebuild() -> Result<fuigo_fast_worktree::RebuildReport> {
    let home = resolve_fuigo_home()?;
...
pub fn candidate_worktree_cwds_for_same_repo(current_cwd: &std::path::Path) -> Result<Vec<String>> {
    ...
    let db_records = match open_db() {
        Ok(db) => {
            let filter = fuigo_fast_worktree::ListFilter {
...
/// Scan `~/.fuigo/worktrees/<repo_name>/` for subdirectories not tracked
...
pub fn build_candidate_list(
    current_cwd: &str,
    main_repo_root: &str,
    db_records: &[fuigo_fast_worktree::WorktreeRecord],
    fs_paths: &[String],
) -> Vec<String> {
...
mod tests {
    ...
    fn local_auto_gc_policy_opts_in_only_when_enabled() {
        use fuigo_config_types::WorktreeAutoGcSettings;
    ...
    async fn snapshot_and_remove_captures_then_deletes() {
        ...
        let ref_name = "refs/fuigo/subagents/dispose-1";
    ...
    async fn snapshot_and_remove_then_rehydrate_round_trips() {
        ...
        let ref_name = "refs/fuigo/subagents/dispose-2";
    ...
    async fn snapshot_standalone_worktree_durable_after_removal_round_trips() {
        ...
        let ref_name = "refs/fuigo/subagents/standalone-1";
    ...
    async fn snapshot_failure_preserves_worktree() {
        ...
        let result = snapshot_and_remove_subagent_worktree(
            &not_a_worktree,
            &not_a_worktree,
            "refs/fuigo/subagents/dispose-3",
        )
        .await;
    ...
    /// Point `FUIGO_HOME` at an isolated tempdir (`resolve_fuigo_home` re-reads the env per call by design).
    ...
    fn worktree_db_fixture(
        ...
        let record = WorktreeRecord {
            ...
            status: fuigo_fast_worktree::WorktreeStatus::Alive,
    ...
    async fn cleanup_cancelled_worktree_removes_dir_and_deregisters() {
        ...
        // (FuigoHomeFixture is not visible across crates.)
