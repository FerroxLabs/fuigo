...
pub struct ClaudeSettings {
    ...
    /// Raw `defaultMode` string when present (canonical under `permissions`, or fuigo-only root legacy).
...
/// Non-string array entries are skipped with warnings, and the canonical `permissions.*` keys win over the fuigo-legacy root keys.
...
///
/// Root `defaultMode` is fuigo-only back-compat for older tests / hand-written configs.
...
pub(crate) fn extract_default_mode(value: &serde_json::Value, path: &Path) -> Option<String> {
    ...
    // Nested key absent, fall back to the optional fuigo legacy root
        ...
        Some(dm) => match dm.as_str() {
            ...
            None => {
                warn!(
                    path = %path.display(),
                    actual_type = %dm.type_of(),
                    "root defaultMode (fuigo legacy): expected string, ignoring"
                );
                None
            }
        },
...
fn extract_additional_directories(value: &serde_json::Value, path: &Path) -> Option<Vec<String>> {
    ...
    // A nested key of the wrong type does *not* resurrect the fuigo-legacy root value
...
// permissions), consider extracting to a shared helper (e.g., in fuigo-hooks
...
///
/// `fuigo_dirs::home_dir()` matches Node's `os.homedir()` (`USERPROFILE` on Windows), used by both the settings' authoring tool and the import scanner.
...
// =============================================================================
//
// `fuigo-shell::claude_import` writes the marker
...
pub fn is_claude_import_marked() -> bool {
    // Test escape hatch: shell tests call `refresh_marker_cache(true)`, which lives in fuigo-shell (inaccessible from here at runtime)
    // They also set this env var so the gate in this crate honours the override without a cross-crate dependency
    if std::env::var("_FUIGO_CLAUDE_MARKER_OVERRIDE").as_deref() == Ok("1") {
        return true;
    }
    let Some(config_path) = fuigo_config::user_fuigo_home().map(|g| g.join("config.toml")) else {
        return false;
    };
