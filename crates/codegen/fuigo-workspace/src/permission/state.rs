...
pub struct PermissionState {
    ...
    /// Exact MCP tool names (e.g. `"fuigo_com_notion__notion-fetch"`) the user has granted "always allow" for.
    /// Lookup is exact.
    pub allowed_mcp_tools: HashSet<String>,
    /// Server components of valid qualified MCP IDs (e.g. `"fuigo_com_notion"`) for which the user has granted "always allow" to every tool.
...
fn permission_scope_root(cwd: &AbsPathBuf) -> std::path::PathBuf {
    match fuigo_agent::repo::RepoDirChain::resolve(cwd.as_path()).git_root {
...
fn state_dir_for_cwd(cwd: &AbsPathBuf) -> std::path::PathBuf {
    fuigo_config::sessions_cwd_dir(&permission_scope_root(cwd).to_string_lossy())
}
...
fn legacy_state_dir(cwd: &AbsPathBuf, dir: &std::path::Path) -> Option<std::path::PathBuf> {
    let legacy = fuigo_config::sessions_cwd_dir(cwd.as_str());
...
async fn resolve_store_dirs(cwd: &AbsPathBuf, ensure: bool) -> StoreDirs {
    let cwd = cwd.clone();
    let fallback_dir = fuigo_config::sessions_cwd_dir(cwd.as_str());
    tokio::task::spawn_blocking(move || {
        ...
        let dir = if ensure {
            ...
            fuigo_config::ensure_sessions_cwd_dir(&root).unwrap_or_else(|e| {
                tracing::warn!(?e, "failed ensuring sessions cwd dir for permission state");
                fuigo_config::sessions_cwd_dir(&root)
            })
        } else {
            fuigo_config::sessions_cwd_dir(&root)
        };
...
pub async fn cleanup_stale_permission_state(max_age: std::time::Duration) {
    let sessions_dir = fuigo_home().join("sessions");
...
"#,
    ...
    fn roundtrip_with_allowed_mcp_tools() {
        ...
        state
            .allowed_mcp_tools
            .insert("fuigo_com_notion__notion-fetch".to_string());
        state
            .allowed_mcp_tools
        ...
        assert!(
            restored
                .allowed_mcp_tools
                .contains("fuigo_com_notion__notion-fetch")
        );
    ...
    fn roundtrip_with_allowed_mcp_servers() {
        ...
        state
            .allowed_mcp_servers
            .insert("fuigo_com_slack".to_string());
        ...
        assert!(restored.allowed_mcp_servers.contains("fuigo_com_slack"));
...
"#,
    ...
    fn scope_root_outside_a_repo_stays_the_cwd() {
        ...
        if fuigo_agent::repo::RepoDirChain::resolve(&dir)
            .git_root
            .is_none()
        {
