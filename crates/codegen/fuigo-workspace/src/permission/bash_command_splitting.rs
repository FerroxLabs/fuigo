...
///
/// ```bash
/// FUIGO_API_KEY='fuigo-some-key' cargo run --bin fuigo-pager
/// ```
///
...
///   prefix: ["FUIGO_API_KEY=fuigo-some-key"]
///   highlighted_words: ["cargo", "run", "--bin", "fuigo-pager"]
///   suffix: []
...
mod tests {
    ...
    fn test_parse_plain_commands_from_script() {
        ...
        let environment_key_command = "FUIGO_API_KEY='fuigo-some-key' cargo run --bin fuigo-pager";
        assert_eq!(
            primary_command_from_script(environment_key_command),
            Some(BashCommandHighlights {
                prefix: vec!["FUIGO_API_KEY=fuigo-some-key".to_owned()],
                highlighted_words: vec![
                    "cargo".to_owned(),
                    "run".to_owned(),
                    "--bin".to_owned(),
                    "fuigo-pager".to_owned()
                ],
                suffix: vec![],
            })
        );
        ...
        let redirection_command = "cargo build --bin fuigo-pager 2>&1";
        assert_eq!(
        ...
        let another_long_command = "cargo test --package fuigo-shell --lib -- permission::bash_command_splitting::tests::test_parse_plain_commands_from_script --exact --nocapture 2>&1";
        assert_eq!(
