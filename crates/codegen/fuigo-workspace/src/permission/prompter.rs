...
use fuigo_session_events::{Event, EventWriter, PermissionDecision};
use fuigo_tools::implementations::fuigo_build::web_fetch::domain_from_url;

const REJECT_ONCE_LABEL: &str = "No, and tell Fuigo what to do differently";
...
/// Only `FuigoTUI`, `FuigoPager`, and `Desktop` wire the option id through to their YOLO toggle; other clients keep their existing option set.
fn client_supports_enable_always_approve(client_type: ClientType) -> bool {
    matches!(
        client_type,
        ClientType::FuigoTUI | ClientType::FuigoPager | ClientType::Desktop
    )
}
...
/// Defined in `fuigo_workspace_types`; re-exported here because callers historically reached it through this module.
...
pub struct McpToolPermission {
    ...
    pub prompt_prefix: String,
    /// Full tool name as the agent called it (e.g. `"fuigo_com_notion__notion-fetch"`).
    pub tool_name: String,
    /// Server component of a valid qualified MCP ID (e.g. `"fuigo_com_notion"`).
...
impl AcpPrompter {
    pub fn new(
        ...
        // Bash options for FuigoTUI: interactive selection with expandable/contractable terms
...
mod tests {
    ...
    fn gate_off_strips_bash_always_allow_keeps_yes_no() {
        let p = prompter_with_gate(ClientType::FuigoPager, false);
    ...
    fn gate_on_includes_bash_always_allow() {
        let p = prompter_with_gate(ClientType::FuigoPager, true);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
        assert!(
            has_option(&opts, "allow-always-command"),
    ...
    fn gate_off_strips_generic_bash_always_and_reject_always() {
        let p = prompter_with_gate(ClientType::FuigoWeb, false);
        let access = AccessKind::Bash("kubectl get pods".to_owned());
        let opts = p.build_options(&access);
    ...
    fn bash_meta_present_only_when_gate_on_for_fancy_clients() {
        ...
        let on = prompter_with_gate(ClientType::FuigoPager, true);
    ...
    fn bash_reject_always_command_maps_selected_words() {
        let p = prompter(ClientType::FuigoPager);
    ...
    fn generic_client_multi_command_keeps_broad_options() {
        let p = prompter(ClientType::FuigoWeb);
    ...
    fn mcp_prompt_includes_allow_always_with_meta() {
        let p = prompter(ClientType::FuigoTUI);
        for (name, server) in [
            ("linear__list", "linear"),
            ("123__lookup", "123"),
    ...
    fn mcp_fallback_client_returns_plain_allow_always() {
        // non-TUI clients (Generic / FuigoWeb / Extension / …) see `fallback_options`
    ...
    fn mcp_titleize_segment_handles_snake_camel_kebab() {
        ...
        assert_eq!(mcp_titleize_segment("fuigo_com_notion"), "Fuigo Com Notion");
    ...
    /// Same pin as above for `FuigoTUI` and `Desktop`: both client types route the option id through to the YOLO toggle, so both must see it.
    ...
    fn enable_always_approve_is_first_for_tui_and_desktop() {
        for ct in [ClientType::FuigoTUI, ClientType::Desktop] {
            let p = prompter(ct);
