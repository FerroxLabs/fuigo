...
/// They map onto `ToolFilter::Mcp` patterns over Fuigo's qualified `<server>__<tool>` names, which carry no `mcp__` prefix:
...
pub fn parse_permission_rule(
    ...
    if let Some(open_paren) = find_first_unescaped(rule, b'(') {
        ...
        // Fuigo qualifies MCP tools as `<server>__<tool>` with no `mcp__` prefix, so strip it and rewrite into a glob over the qualified name
            ...
            && !rest.is_empty()
        {
            let pattern = if rest == "*" {
                ...
                // The rest is already `<server>__<tool>` (or `<server>__*`), the Fuigo qualified name, so use it verbatim
