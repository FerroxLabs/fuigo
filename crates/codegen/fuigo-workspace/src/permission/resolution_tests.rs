...
// Shared so `FUIGO_HOME`/`HOME` mutations here also serialize against the other env-mutating test modules under single-process `cargo test --lib`
...
fn load_claude_env_merges_with_precedence() {
    ...
    let _lock = ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    let home = tempfile::tempdir().unwrap();
    let _home_guard = EnvVarGuard::set("FUIGO_HOME", home.path());
    let _marker_guard = EnvVarGuard::unset("_FUIGO_CLAUDE_MARKER_OVERRIDE");
    let tmp = tempfile::tempdir().unwrap();
    let claude_dir = tmp.path().join(".claude");
    std::fs::create_dir_all(&claude_dir).unwrap();
...
fn mcp_name_matches_strips_managed_prefix_both_sides_exactly() {
    ...
    assert!(mcp_name_matches("foo", "fuigo_com_foo"));
    assert!(mcp_name_matches("fuigo_com_foo", "foo"));
    assert!(mcp_name_matches("fuigo_com_foo", "fuigo_com_foo"));
    assert!(!mcp_name_matches("foo", "foobar"));
    assert!(!mcp_name_matches("foo", "fuigo_com_foobar"));
...
fn mcp_name_matches_is_case_and_space_insensitive() {
    ...
    assert!(mcp_name_matches("Slack", "fuigo_com_slack"));
    assert!(mcp_name_matches("My Server", "fuigo_com_my_server"));
    assert!(mcp_name_matches("fuigo_com_my_server", "My Server"));
    ...
    assert!(!mcp_name_matches("fuigo_com_", "fuigo_com_anything"));
}
...
/// Truncation applies only to `fuigo_com_*` names; long plain names sharing a prefix must not match.
#[test]
...
/// A long `fuigo_com_*` POLICY entry must not become a prefix grant over attacker-chosen plain runtime names.
...
fn long_managed_allow_entry_does_not_grant_plain_prefix_names() {
    ...
    // An attacker-chosen `fuigo_com_*` decoy that is longer or shorter than the cap must not prefix-match the long entry
...
fn denied_by_server_name_matches_bare_and_managed_prefix() {
    ...
    let managed = http_named("fuigo_com_foo", "https://foo.example.com/mcp");
    assert!(al.is_server_denied(&managed));
    assert!(!al.is_server_allowed(&managed));

    // Name match is transport-agnostic.
    let stdio = stdio_named("fuigo_com_foo", "npx");
    ...
    for unrelated in ["foobar", "fuigo_com_foobar", "barfoo", "bar"] {
...
fn allowed_by_server_name_restricts_across_transports() {
    ...
    assert!(al.is_server_allowed(&http_named("fuigo_com_foo", "https://evil.example.com/x")));
    assert!(al.is_server_allowed(&stdio_named("fuigo_com_foo", "/usr/bin/whatever")));
...
fn server_name_deny_beats_allow() {
    ...
    for s in [
        ...
        http_named("fuigo_com_foo", "https://foo.example.com/x"),
    ] {
...
fn server_name_prefix_edge_cases_vice_versa() {
    ...
    let al = allowlist_from(serde_json::json!({
        "deniedMcpServers": [ { "serverName": "fuigo_com_foo" } ]
    }));

    assert!(al.is_server_denied(&http_named("foo", "https://x.example.com/mcp")));
    assert!(al.is_server_denied(&http_named("fuigo_com_foo", "https://x.example.com/mcp")));
    assert!(!al.is_server_denied(&http_named("foobar", "https://x.example.com/mcp")));
    assert!(!al.is_server_denied(&http_named("fuigo_com_foobar", "https://x.example.com/mcp")));
}
...
fn untrusted_project_claude_permissions_are_not_honored() {
    ...
    let home = tempfile::tempdir().unwrap();
    let _home_guard = EnvVarGuard::set("HOME", home.path());
    let _fuigo_guard = EnvVarGuard::set("FUIGO_HOME", home.path());
    let _marker_guard = EnvVarGuard::unset("_FUIGO_CLAUDE_MARKER_OVERRIDE");
...
/// Untrusted clone must not contribute project `.fuigo/config.toml` [permission].
///
/// The test is sync and uses `block_on` so `ENV_LOCK` is not held across `.await` (clippy `await_holding_lock`).
/// It does not assert exact global rule counts: `fuigo_config::fuigo_home()` is a process-wide `OnceLock`.
/// Under single-process `cargo test` an earlier test may have already pinned `FUIGO_HOME`.
...
fn untrusted_project_config_toml_permissions_are_not_honored() {
    ...
    // Global allow (survives untrusted project when FUIGO_HOME resolves here).
    std::fs::write(
...
"#,
    ...
    let rt = tokio::runtime::Builder::new_current_thread()
        ...
        .build()
        .expect("test runtime");
    // Untrusted may be None when no global rules load (FUIGO_HOME OnceLock already pinned by another test); empty after dropping project is OK
    ...
    let global_live = fuigo_config::user_fuigo_home()
        .is_some_and(|g| g == home.path() || g.starts_with(home.path()));
    if global_live {
        let untrusted = untrusted.expect("global rules present when FUIGO_HOME is live");
        assert!(
            untrusted
                .config
                .rules
...
fn non_bool_lock_key_warns_and_does_not_lock() {
    ...
    let p = Path::new("/etc/fuigo/requirements.toml");
    let bad = layer("[ui]\ndisable_bypass_permissions_mode = \"true\"\n");
    ...
    assert!(
        out.contains("/etc/fuigo/requirements.toml"),
...
fn admin_source_trusts_only_root_owned_tiers() {
    ...
    // the user-writable `~/.fuigo/requirements.toml` is not, despite its path.
    ...
    assert!(is_admin_source(&RequirementSource::SystemRequirements {
        path: "/etc/fuigo/requirements.toml".into(),
    }));
    assert!(!is_admin_source(&RequirementSource::Requirements {
        path: "/home/u/.fuigo/requirements.toml".into(),
    }));
    assert!(!is_admin_source(&RequirementSource::ManagedConfig {
        path: "/etc/fuigo/managed_config.toml".into(),
    }));
...
fn drop_untrusted_catchall_allows_is_source_aware() {
    ...
    let rules = vec![
        ...
        // Managed config: defaults tier, untrusted even from /etc/fuigo.
        sourced(
            allow_any(Some("*")),
...
fn unrecognized_project_mode_claims_scope_over_global_accept_edits() {
    ...
    assert!(
        ...
        "typo should be recorded for fuigo inspect"
    );
}
...
async fn managed_config_toml_rules_resolve_as_non_admin_defaults() {
    ...
    let layers = fuigo_config::managed_config_layers_at(Some(system.path()), Some(user.path()));
