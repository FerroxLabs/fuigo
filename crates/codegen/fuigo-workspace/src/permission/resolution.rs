//! Merges native `.fuigo/config.toml`, managed/enterprise settings, and `.claude` settings into the effective `PermissionConfig`.
...
/// Parse a defaultMode string; an unknown value fails safe to [`DefaultPermissionMode::Default`] with a warn and a skip record for `fuigo inspect`.
...
/// user `~/.fuigo` → `Requirements`, so [`is_admin_source`] trusts only the root tier.
fn load_requirements_permissions() -> Vec<Sourced<PermissionRule>> {
    fuigo_config::requirements_layers()
        .into_iter()
        .flat_map(|layer| {
...
/// Load `[permission]` rules from native Fuigo TOML config files:
///
///   * `~/.fuigo/config.toml` (lowest priority)
///   * Each `.fuigo/config.toml` from the git repo root down to `cwd` (highest priority last).
...
fn load_config_toml_permissions(cwd: &Path, project_trusted: bool) -> Vec<Sourced<PermissionRule>> {
    ...
    // Global `~/.fuigo/config.toml` first (lowest priority within this layer).
    // Gated on user_fuigo_home() so a project's .fuigo/config.toml is never read as global permissions when neither FUIGO_HOME nor a home dir resolves
    if let Some(global_path) = fuigo_config::user_fuigo_home().map(|g| g.join("config.toml"))
        && global_path.is_file()
    {
        match fuigo_config::load_config_file(&global_path) {
    ...
    // An untrusted clone must not contribute allow/deny/ask rules via `.fuigo/config.toml` (same gate as project `.claude/settings.json`)
    if project_trusted {
        for path in crate::project_config::find_project_configs(cwd) {
            match fuigo_config::load_config_file(&path) {
                Ok(value) => rules.extend(extract_toml_permissions(&value, || {
...
fn managed_config_permissions(
    layers: &[fuigo_config::ManagedConfigLayer],
) -> Vec<Sourced<PermissionRule>> {
    layers
        .iter()
        .flat_map(|layer| {
...
/// Resolve permission config, merging native Fuigo and Claude sources.
...
///
/// `project_trusted` gates project-tier `.claude/settings.json` and `.fuigo/config.toml` permission rules (mirrors [`load_claude_env_with_project`]).
...
/// Records each drop for `fuigo inspect`.
...
struct ResolveInputs<'a> {
    ...
    /// When false, project-tier `.claude/settings.json` / `.fuigo/config.toml` permission rules are dropped (global/user/admin tiers still load).
    project_trusted: bool,
}
    ...
    fn live(project_trusted: bool, policy_block: Option<&'static str>) -> Self {
        Self {
            ...
            managed_config_rules: managed_config_permissions(
                &fuigo_config::managed_config_layers(),
            ),
            project_trusted,
        }
    }
...
/// The exception: bypass pinned off via fuigo `requirements.toml` (`[ui] disable_bypass_permissions_mode = true`).
/// Pair managed `dontAsk` with that pin when org policy must not be bypassable by `--always-approve`.
///
/// `project_trusted` gates project-tier Claude settings and `.fuigo/config.toml` permission rules just as [`load_claude_env_with_project`] gates env.
...
/// [`resolve_permissions_with_provenance`] with a caller-supplied pin, so the `fuigo inspect` report reads the pin once and cannot contradict itself.
...
async fn resolve_permissions_with_provenance_inner(
    ...
    // Keep skip-only resolutions alive so the drop reaches `fuigo inspect`
...
fn resolve_claude_settings_inner(
    ...
    for path in claude_settings_paths_for_trust(cwd, project_trusted) {
        ...
        if let Some(perms) = settings.permissions {
            ...
            // Rules *or* skip-only parse failures still own provenance for `fuigo inspect`
    ...
    // Provenance then reaches `fuigo inspect` via the outer resolver
    if all_rules.is_empty()
        && prompt_policy == PromptPolicy::Ask
        && !bypass_blocked
        && default_mode_source.is_none()
...
/// fuigo must not inherit a host-wide always-approve lockdown from that file.
/// fuigo still honors that file's permission rules and MCP / marketplace allowlists.
/// The user's own `--yolo` / `[ui] permission_mode` / runtime toggle drive always-approve.
/// To disable it in fuigo, use a root-owned `requirements.toml`.
...
pub fn yolo_policy_lock() -> Option<YoloPolicyLock> {
    let layers = fuigo_config::requirements_layers();
...
/// Namespace prefix for legacy injected MCP server names (`fuigo_com_*`).
/// Policy matching still uses this spelling.
pub const MANAGED_MCP_PREFIX: &str = "fuigo_com_";

/// Max `char` length of a managed runtime name (`fuigo_com_` + normalized display name), sized to the 64-char tool-name budget.
...
/// Both sides reduce to one key (strip `fuigo_com_`, [`normalize_managed_name`]) and compare by exact equality, never substring.
...
fn mcp_name_matches(pattern: &str, name: &str) -> bool {
    ...
    // Mirror the legacy injected-name truncation: `fuigo_com_*` runtime names were capped at MANAGED_MCP_NAME_MAX_CHARS total
