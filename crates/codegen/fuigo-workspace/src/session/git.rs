...
/// String prefix matching would treat `HOME=/Users/u` as a prefix of `/Users/user/fuigo`.
...
pub const RESTORE_CODE_LOG: &str = "fuigo_restore_code";
...
///
/// 1. `supplied_cwd` is a fuigo-managed worktree (`~/.fuigo/worktrees/...`).
...
pub fn restore_code_checkout_allowed(supplied_cwd: &Path, persisted_cwd: Option<&str>) -> bool {
    let worktrees_dir = fuigo_tools::util::fuigo_home::fuigo_home().join("worktrees");
...
const REWIND_GIT_ENV: &str = "FUIGO_WORKSPACE_REWIND_GIT";
...
pub fn git_rewind_enabled() -> bool {
    fuigo_config::env_bool(REWIND_GIT_ENV).unwrap_or(false)
}
...
const DEFAULT_EXCLUDES_MARKER: &str = "fuigo default excludes";
...
# fuigo default excludes (local-only; seeded by the workspace git_commit op)
.fuigo/
node_modules/
.env
.env.*
...
async fn seed_default_gitignore(git_root: &Path) -> Result<()> {
    ...
    tokio::fs::write(&path, fuigo_workspace_types::binding::DEFAULT_GITIGNORE).await?;
