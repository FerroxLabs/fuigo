...
use std::sync::Arc;
use fuigo_tools::registry::types::{
...
pub(crate) fn resolve_session_toolset(
    ...
    local_registry: Option<fuigo_computer_hub_sdk::LocalRegistry>,
    ...
    notification_handle: Option<fuigo_tools::notification::types::ToolNotificationHandle>,
) -> WorkspaceResult<(
    ToolServerConfig,
...
pub(crate) fn resolve_session_toolset_rebuild(
    ...
    viewer_ctx: Option<fuigo_tool_runtime::WorkspaceViewerContext>,
    notification_handle: Option<fuigo_tools::notification::types::ToolNotificationHandle>,
    terminal_backend: Arc<dyn fuigo_tools::computer::types::TerminalBackend>,
    ...
    let toolset = builder
        .finalize_with_trunc_config(
            finalize_config,
            ctx,
            fuigo_tools::types::context::TruncationConfig::default(),
            viewer_ctx,
        )
        .map_err(|errs| {
...
/// Only `FUIGO_WORKSPACE_TOOL_STATE_ENABLED=true` enables it; any other value keeps legacy behavior.
pub fn tool_state_enabled() -> bool {
    std::env::var("FUIGO_WORKSPACE_TOOL_STATE_ENABLED").as_deref() == Ok("true")
}
...
fn sanitize_session_id(session_id: &str) -> String {
    ...
    if modified {
        let digest = fuigo_file_utils::sha256_hex(session_id.as_bytes());
        safe.push('-');
...
/// Serializes tests (across modules) that mutate the process-global `FUIGO_WORKSPACE_TOOL_STATE_ENABLED`.
...
/// [`LocalTerminalBackend`]: fuigo_tools::computer::local::LocalTerminalBackend
pub struct WorkspaceSessionContextFactory {
    auth: Option<fuigo_computer_hub_sdk::SharedAuthProvider>,
    api_base_url: Option<String>,
    /// Resolved `$FUIGO_WORKSPACE_HOME` when tool-state persistence is enabled; `None` disables it.
...
impl WorkspaceSessionContextFactory {
    ...
    pub fn with_auth(auth: fuigo_computer_hub_sdk::SharedAuthProvider, api_base_url: String) -> Self {
        Self {
            auth: Some(auth),
    ...
    /// Enable session-keyed tool-state persistence rooted at `home` (`$FUIGO_WORKSPACE_HOME`).
...
impl SessionContextFactory for WorkspaceSessionContextFactory {
    fn build_session_context(
        ...
        let fs = Arc::new(fuigo_tools::computer::local::LocalFs)
            as Arc<dyn fuigo_tools::computer::types::AsyncFileSystem>;
        let notification_handle = fuigo_tools::notification::ToolNotificationHandle::noop();
    ...
    fn build_terminal_backend(&self) -> crate::config::SessionTerminalBackend {
        crate::config::SessionTerminalBackend::local(
            fuigo_tools::computer::local::LocalTerminalBackend::new(),
        )
    }
...
/// Enabled with default params unless `FUIGO_DISABLE_WEB_FETCH=1` is set.
fn build_web_fetch_config() -> fuigo_tools::implementations::fuigo_build::web_fetch::WebFetchConfig
{
    use fuigo_tools::implementations::fuigo_build::web_fetch::{WebFetchConfig, WebFetchParams};
    if std::env::var("FUIGO_DISABLE_WEB_FETCH").is_ok_and(|v| v == "1" || v == "true") {
        return WebFetchConfig::Disabled;
    }
    let mut params = WebFetchParams::default();
    if let Ok(proxy) = std::env::var("FUIGO_WEB_FETCH_PROXY") {
        params.proxy_endpoint = Some(proxy);
    }
    if fuigo_config::env_bool("FUIGO_WEB_FETCH_ALLOW_LOCAL") == Some(true) {
        params.allow_local = Some(true);
    }
    WebFetchConfig::Enabled { params }
}
fn default_web_search_model() -> String {
    std::env::var("FUIGO_WEB_SEARCH_MODEL").unwrap_or_else(|_| "grok-4.5".to_string())
}
...
pub mod test_support {
    ...
    impl TestSessionContextFactory {
        ...
        /// Matches production, where `FUIGO_WORKSPACE_TOOL_STATE_ENABLED` is unset and the real factory returns an empty path.
    ...
    pub fn baseline_config() -> ToolServerConfig {
        ToolServerConfig {
            tools: vec![
                ...
                tc("FuigoBuild:search_replace", Some(ToolKind::Edit)),
                tc("FuigoBuild:grep", Some(ToolKind::Search)),
...
mod tests {
    ...
    async fn resolve_session_toolset_mcp_merge_dedup_by_id_baseline_wins() {
        ...
        let mut mcp_dup = test_support::tc("FuigoBuild:read_file", Some(ToolKind::Read));
    ...
    fn backfill_tool_kinds_fills_known_kindless_ids_only() {
        let kinds = HashMap::from([
            ("FuigoBuild:search_replace".to_owned(), ToolKind::Edit),
            ("FuigoBuild:read_file".to_owned(), ToolKind::Read),
        ]);
        let config = ToolServerConfig {
            tools: vec![
                test_support::tc("FuigoBuild:search_replace", None),
                test_support::tc("adhoc.opaque", None),
                // Pre-set kinds must never be overwritten by the registry.
                test_support::tc("FuigoBuild:read_file", Some(ToolKind::Search)),
            ],
        ...
        assert_eq!(kind_of("FuigoBuild:search_replace"), Some(ToolKind::Edit));
        assert_eq!(
    ...
    async fn resolve_session_toolset_readonly_filters_kindless_pinned_tools() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("FuigoBuild:read_file", None),
                test_support::tc("FuigoBuild:grep", None),
                test_support::tc("FuigoBuild:list_dir", None),
                test_support::tc("FuigoBuild:search_replace", None),
                test_support::tc("FuigoBuild:run_terminal_cmd", None),
            ],
            behavior_preset: None,
        };
        let (eff, ts, _backend) = resolve_session_toolset(
    ...
    async fn resolve_session_toolset_mcp_kind_none_dropped_under_readonly() {
        ...
        let baseline = ToolServerConfig {
            tools: vec![
                test_support::tc("FuigoBuild:read_file", Some(ToolKind::Read)),
        ...
        assert!(
            kept_ids.contains(&"FuigoBuild:read_file"),
    ...
    fn tool_state_enabled_only_true_enables() {
        ...
        let var = "FUIGO_WORKSPACE_TOOL_STATE_ENABLED";
    ...
    async fn tool_state_rehydrates_same_session_and_cold_starts_other() {
        use fuigo_tools::types::resources::{State, WebCitationCounter};
