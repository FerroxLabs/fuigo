...
use fuigo_computer_hub_mcp_adapter::McpBridgeHandle;
use fuigo_mcp::servers::McpState;
use fuigo_tools::notification::AcknowledgedToolNotification;
use fuigo_tools::notification::types::ToolNotificationHandle;
use fuigo_tools::registry::types::{FinalizedToolset, ToolConfig, ToolServerConfig};
use fuigo_hunk_tracker::HunkTrackerHandle;
use fuigo_tool_protocol::ToolId;
use fuigo_tool_runtime::WorkspaceViewerContext;
...
pub struct WorkspaceSession {
    ...
    ///
    /// [`HunkTrackerActor`]: fuigo_hunk_tracker::HunkTrackerActor
    ...
    pub(crate) hunk_checkpoints:
        Arc<tokio::sync::Mutex<HashMap<usize, fuigo_hunk_tracker::HunkTurnDelta>>>,
...
impl WorkspaceSession {
    ...
    pub(crate) async fn set_cwd_for_virtualization(&self, cwd: PathBuf) -> Result<(), String> {
        ...
        res.insert(fuigo_tools::types::resources::Cwd(cwd));
        Ok(())
    }
    ...
    pub(crate) async fn toolset_terminal_is_session_owned(&self) -> bool {
        ...
        match res.get::<fuigo_tools::types::resources::Terminal>() {
...
pub struct WorkspaceShared {
    ...
    pub(crate) events: tokio::sync::broadcast::Sender<fuigo_workspace_types::WorkspaceEvent>,
    pub(crate) respect_gitignore: bool,
    pub(crate) memory_config: Option<MemoryConfig>,
    pub(crate) hook_registry: Arc<parking_lot::RwLock<fuigo_hooks::discovery::HookRegistry>>,
    pub(crate) hook_load_errors: Vec<fuigo_hooks::error::HookError>,
    ...
    pub(crate) local_registry: fuigo_computer_hub_sdk::LocalRegistry,
    ...
    pub(crate) lsp: Option<std::sync::Arc<dyn fuigo_tools::implementations::lsp::LspBackend>>,
    pub(crate) codebase_indexes:
        std::sync::Arc<parking_lot::Mutex<crate::file_system::CodebaseIndexManager>>,
    /// Finalize the FS rewind checkpoint on non-`Completed` turn-end outcomes (from `FUIGO_WORKSPACE_REWIND_ALL_OUTCOMES`, default off).
    pub(crate) workspace_rewind_all_outcomes: bool,
    /// Resolved `$FUIGO_WORKSPACE_HOME`, the workspace-owned on-disk state root (`<fuigo_home>/workspace` by default).
    /// The upload queue spills here.
    pub(crate) workspace_home: std::path::PathBuf,
    pub(crate) upload_queue: Option<std::sync::Arc<fuigo_file_utils::queue::UploadQueue>>,
    ...
    pub(crate) session_event_writers:
        Arc<dashmap::DashMap<String, fuigo_session_events::EventWriter>>,
    ...
    pub(crate) inflight_enqueues: dashmap::DashMap<
        (String, u64),
        tokio::task::JoinHandle<fuigo_file_utils::queue::EnqueueOutcome>,
    >,
...
impl WorkspaceShared {
    ...
    /// Resolved `$FUIGO_WORKSPACE_HOME`, the workspace-owned on-disk state root.
    ...
    pub fn upload_queue(&self) -> Option<&std::sync::Arc<fuigo_file_utils::queue::UploadQueue>> {
    ...
    ///
    /// When `events_enabled` is `false` this returns [`EventWriter::noop()`](fuigo_session_events::EventWriter::noop).
    ...
    pub(crate) fn session_event_writer(
        &self,
        session_id: &str,
    ) -> fuigo_session_events::EventWriter {
    ...
    pub(crate) fn session_event_writer_cached(
        &self,
        session_id: &str,
    ) -> Option<fuigo_session_events::EventWriter> {
    ...
    /// Auth provider used for Ferrox Labs service calls.
    pub fn auth_provider(&self) -> Option<&fuigo_computer_hub_sdk::SharedAuthProvider> {
    ...
    ///
    /// Returns a clone of the [`ToolServer`](fuigo_computer_hub_sdk::ToolServer) which is cheap (`Arc` bump).
    ...
    pub fn subscribe_events(
        &self,
    ) -> tokio::sync::broadcast::Receiver<fuigo_workspace_types::WorkspaceEvent> {
...
pub(crate) fn get_or_open_session_writer(
    enabled: bool,
    writers: &dashmap::DashMap<String, fuigo_session_events::EventWriter>,
    workspace_home: &Path,
    session_id: &str,
) -> fuigo_session_events::EventWriter {
    ...
    let create = fuigo_config::create_dir_all_owner_only(&dir);
    fuigo_config::set_dir_owner_only(&sessions_root);
    if let Err(e) = create {
        tracing::warn!(
...
mod tests {
    ...
    use dashmap::DashMap;
    use fuigo_session_events::{Event, EventWriter};
    ...
    /// Session-derived content outside ~/.fuigo/sessions: same owner-only rule,
