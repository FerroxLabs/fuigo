...
const ENVRC_TIMEOUT_ENV: &str = "FUIGO_ENVRC_TIMEOUT_SECS"; // seconds; 0 disables
...
const OUTPUT_SENTINEL: &str = "__FUIGO_ENVRC_COMPLETE__";
...
fn run_with_deadline(mut cmd: Command, deadline: Instant, label: &str) -> RunOutcome {
    ...
    let mut group = fuigo_tools::util::ProcessGroup::new().ok();
    ...
    if let Some(g) = &group
        && !fuigo_tools::util::global_process_scope().register(g)
    {
...
fn reap_with_timeout(child: &mut std::process::Child, label: &str) {
    if let Ok(None) = wait_timeout::ChildExt::wait_timeout(child, fuigo_tty_utils::KILL_REAP_TIMEOUT)
    {
        tracing::warn!(
            label,
            pid = child.id(),
...
mod tests {
    ...
    fn sentinel_named_variable_does_not_truncate_env() {
        ...
        fs::write(
            dir.path().join(".envrc"),
            "export __FUIGO_ENVRC_COMPLETE__=decoy\nexport ZZ_AFTER_DECOY=survives\n",
        )
        .unwrap();
