...
impl WorkspaceIdentity {
    ...
    /// The standalone server's `AuthEntry` never carries the shell's separate `FuigoAuth.team_id`, so `team_id` derives from `principal_id`.
...
/// Derive the workspace owner identity from the server auth provider's [`AuthIdentity`](fuigo_computer_hub_sdk::AuthIdentity).
...
impl From<fuigo_computer_hub_sdk::AuthIdentity> for WorkspaceIdentity {
    fn from(id: fuigo_computer_hub_sdk::AuthIdentity) -> Self {
...
pub struct WorkspaceEnvironment {
    ...
    /// Version of the `fuigo-workspace` crate that produced the record.
    ...
    /// Sandbox profile name (`$FUIGO_SANDBOX_PROFILE`), e.g. `"devbox"`.
...
impl WorkspaceEnvironment {
    ...
    pub(crate) fn capture(
        ...
        Self::assemble(
            ...
            server_id,
            sandbox_id,
            std::env::var("FUIGO_SANDBOX_PROFILE").ok(),
...
mod tests {
    ...
    fn assemble_populates_every_field() {
        let env = WorkspaceEnvironment::assemble(
            ...
            Some("git@github.com:fuigo-org/example.git".to_string()),
        );
        ...
        assert_eq!(
            ...
            Some("git@github.com:fuigo-org/example.git")
        );
    ...
    fn serializes_with_all_fields_present() {
        let env = WorkspaceEnvironment::assemble(
            ...
            Some("host-1".to_string()),
            Some("/work/repo".to_string()),
            Some("https://github.com/fuigo-org/example".to_string()),
        );
    ...
    fn git_repo_facts_strips_remote_url_credentials() {
        ...
        assert_eq!(url, "https://github.com/fuigo-org/example.git");
    }
}
