...
use fuigo_announcements::RemoteAnnouncement;
...
pub struct DoomLoopRecoverySettings {
    /// Send the `x-fuigo-doom-loop-check` header, parse the reported triggers, and resample confident loops.
    ...
    /// Detector window sent as the value of `x-fuigo-doom-loop-check` (honored in 512..=4096, otherwise 4096; absent uses the client default, 1024).
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct WorktreeAutoGcSettings {
    ...
    /// Also rebuild discovery and scrub fuigo's stale `.git/worktrees/` entries (default off).
    #[serde(
        default,
        deserialize_with = "de_opt_bool_tolerant",
        skip_serializing_if = "Option::is_none"
...
/// Consent notice from `fuigo_build_settings.consent_gate`.
...
pub struct RemoteSettings {
    ...
    /// It arrives from the `fuigo_build_settings` remote settings flag via the cli-chat-proxy `/settings` flatten catch-all.
    #[serde(default)]
    ...
    /// When `Some(true)`, enables fuigo's default OAuth2 (Ferrox Labs auth.x.ai).
    /// Enterprise OIDC (user's own IdP via `oidc` config) always wins.
    /// The `--oauth` CLI flag overrides it.
    #[serde(default)]
    pub fuigo_oauth_enabled: Option<bool>,
    #[serde(default)]
    ...
    /// It sits below env `FUIGO_FOLDER_TRUST`, user `[folder_trust] enabled`, and managed config in the resolver chain.
    ...
    /// Global default MCP tool-result inline cap (bytes), from remote settings `fuigo_build_settings.max_mcp_output_bytes`.
    ...
    /// The gate ships disabled; set this to `Some(true)` (via the `fuigo_build_settings` remote settings key) to enable it.
    ...
    /// It arrives as `non_git_warning` in `fuigo_build_settings`.
    ...
    /// Remote settings gate for first-run auto-registration of the official Ferrox Labs marketplace source.
    ...
    /// Server-side pin for the next-prompt suggestion model (tab-autocomplete ghost text), from the `fuigo_build_settings` remote settings flag.
    /// It sits below env (`FUIGO_PROMPT_SUGGESTIONS_MODEL`) and `[models] prompt_suggestion` in config.toml.
    ...
    /// It comes from `fuigo_build_settings`; `None` or `0` means it never re-shows after dismiss.
    #[serde(default)]
    ...
    /// User's subscription tier from remote settings `fuigo_build_access_gate`.
    /// E.g. "free", "premium", "superfuigo", "superfuigo_heavy".
    ...
    /// Whether the user is allowed to use Fuigo. Remote settings `fuigo_build_access_gate` targeting rules set it.
    ...
    /// The remote settings `fuigo_build_usage_redirect_url` feature flag controls it (target it at personal-team users).
    ...
    /// Global auto-compact threshold percent (0-100) from remote settings `fuigo_build_settings`.
    /// A per-model override on `ModelInfo` (`fuigo_build_models`) takes precedence; user config and env var further override per the resolver chain.
    #[serde(default)]
    pub auto_compact_threshold_percent: Option<u8>,
    /// Max subagent nesting depth (`fuigo_build_settings.subagents_max_depth`).
    #[serde(default)]
    ...
    /// Max parallel `image_gen` and `image_edit` tool calls in one model step (`fuigo_build_settings.max_parallel_image_gen_calls`).
    #[serde(default)]
    pub max_parallel_image_gen_calls: Option<u32>,
    /// Max parallel video-gen tool calls in one model step (`fuigo_build_settings.max_parallel_video_gen_calls`).
    #[serde(default)]
    ...
    /// Env (`FUIGO_COMPACTION_WALL_CLOCK_SECS`) overrides it. `resolve_compaction_wall_clock_budget_secs` resolves it.
    #[serde(default)]
    pub compaction_wall_clock_budget_secs: Option<u64>,
    /// Compaction mode (`summary`, `transcript`, or `segments`) from remote settings.
    /// Env (`FUIGO_COMPACTION_MODE`) and user config override it.
    #[serde(default)]
    pub compaction_mode: Option<String>,
    /// Segments verbatim detail (`none`, `minimal`, `balanced`, or `verbose`) from remote settings.
    /// Env (`FUIGO_COMPACTION_DETAIL`) and config override it.
    #[serde(default)]
    pub compaction_detail: Option<String>,
    /// remote settings verbatim-input flag; env (`FUIGO_COMPACTION_VERBATIM_INPUT`) and config override it. `None` = default (true).
    #[serde(default)]
    ...
    /// Remote settings gate for the `fuigo workspace` CLI command (Computer Hub workspace exposure).
    /// It comes from `fuigo_build_settings.workspace_command_enabled`.
    ...
    /// Soft default for `keep_text_selection` (`"flash"`, `"hold"`, or `"word_select"`), from `fuigo_build_settings.keep_text_selection_default`.
...
/// The pair is the atomic configurable unit because a model is only guaranteed to work with a compatible harness (cursor vs fuigo-build).
...
pub struct GoalRoleModel {
    ...
    pub model: String,
    /// Harness `agent_type` (e.g. "cursor", "fuigo-build-plan") whose `AgentDefinition` decides the role subagent's harness flavor.
    /// The flavor (system prompt and cursor-vs-fuigo-build toolset) applies regardless of the session or parent agent.
