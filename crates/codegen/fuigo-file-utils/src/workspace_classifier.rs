...
const EXCLUDED_DIR_NAMES: &[&str] = &[
    ".fuigo", ".cache", ".daemon", ".config", ".npm", ".cargo", ".rustup", ".vscode", ".gemini",
    ".hermes", ".claude",
];
...
fn has_excluded_component(path: &Path) -> bool {
    for component in path.components() {
        if let std::path::Component::Normal(name) = component {
            ...
            if name_lower.starts_with(".fuigo-") {
                return true;
            }
        }
    }
...
mod tests {
    ...
    mod config_and_cache {
        ...
        #[test]
        fn fuigo_dirs_are_unsafe() {
            if let Some(home) = fuigo_dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".fuigo")));
                assert!(!is_project_dir(&home.join(".fuigo/bin")));
            }
        }

        #[test]
        fn fuigo_prefixed_dirs_are_unsafe() {
            if let Some(home) = fuigo_dirs::home_dir() {
                assert!(!is_project_dir(&home.join(".fuigo-proxy-work")));
            }
        }
