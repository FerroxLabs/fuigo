//! Provisioned-repo listing (`workspace.repos_list`) and the on-disk in-sandbox manifest contract (`{workspace}/.fuigo/repos.json`).
//!
...
pub const REPOS_MANIFEST_RELATIVE_PATH: &str = ".fuigo/repos.json";
...
impl RepoManifest {
    ...
    /// Parse bytes from `{workspace}/.fuigo/repos.json`.
    ...
    pub fn materialized_mounts(&self, workspace_root: &std::path::Path) -> Vec<std::path::PathBuf> {
        ...
        for repo in &self.repos {
            ...
            // A compromised/malicious `.fuigo/repos.json` must not point prompt/graph/fs-notify walks at paths outside the sandbox workspace
