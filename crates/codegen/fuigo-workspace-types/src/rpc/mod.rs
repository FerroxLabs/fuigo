...
//! They are shared by the server (hub_server), the shell proxy client (`WorkspaceOps`), and clients that cannot depend on `fuigo-workspace`.
...
/// Tool ID used for workspace-originated client ext-notification frames (e.g. `fuigo/search/fuzzy/status`). Carries `{ method, params }`.
