...
//!
//! SYNC: [`SkillInfo`] / [`SkillScope`] mirror the serde shape the server serializes in `fuigo-tools/src/implementations/skills/types.rs`.
...
pub enum SkillScope {
    /// cwd/.fuigo/skills
    Local,
    /// repo_root/.fuigo/skills
    Repo,
    /// ~/.fuigo/skills
    User,
    /// ~/.fuigo/server-skills (synced from the skill store)
    Server,
...
mod tests {
    ...
    fn skill_info_deserializes_minimal_payload() {
        let raw = serde_json::json!({
            ...
            "path": "/workspace/.fuigo/skills/my-skill/SKILL.md",
            "scope": "local",
        });
    ...
    // Fixture mirrored field-for-field from the fuigo-tools SkillInfo serialization
    ...
    fn skill_info_deserializes_full_payload() {
        let raw = serde_json::json!({
            ...
            "metadata": {"team": "infra"},
            "path": "/root/.fuigo/server-skills/deploy/SKILL.md",
            "scope": "server",
            "config_source": {"type": "user", "path": "/root/.fuigo/skills"},
            "plugin_name": "infra-plugin",
            "plugin_version": "1.0.0",
            "plugin_root": "/root/.fuigo/plugins/infra-plugin",
            "plugin_data": "/root/.fuigo/plugin-data/infra-plugin",
            "allowed_tools": ["bash"],
            "model": "grok-4",
            "effort": "high",
            "user_invocable": true,
