...
//!   Adding it here would force every consumer of `fuigo-workspace-types` (including the eventual WASM browser SDK) to pull in tokio.
...
//! The runtime crate (`fuigo-workspace`) wraps this in its own `Request<T>` that adds the cancellation token and extensions map.
//!
//! Splitting the envelope this way keeps `fuigo-workspace-types` tokio-free.
