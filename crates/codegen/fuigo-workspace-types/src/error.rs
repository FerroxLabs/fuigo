...
//!
//! - **`Tool`**: `Tool(#[from] fuigo_tools::ToolError)` would make this crate depend on `fuigo-tools`, defeating the wire-types-only goal.
...
/// Conversion from non-serializable runtime errors (`std::io::Error`, `fuigo_tools::ToolError`) happens at the workspace-crate boundary.
