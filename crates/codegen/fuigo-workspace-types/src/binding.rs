...
/// Kept in lockstep with the export seed (`.project_id`, `.github_repo`) and local `info/exclude` (`.fuigo/`).
/// That keeps machine state out of BYO remotes and user commits.
pub const DEFAULT_GITIGNORE: &str = "\
# Seeded by Fuigo app workspaces. Secrets and machine state never belong in git;
...
mod tests {
    ...
    fn default_gitignore_excludes_secrets_and_deps() {
        ...
        assert!(DEFAULT_GITIGNORE.contains(".fuigo/"));
