...
pub(crate) fn ancestors_within_writable_roots(
    ...
    let mut out = Vec::new();
    for anc in fuigo_config::existing_ancestor_chain(path) {
...
mod tests {
    ...
    fn ancestors_pin_under_writable_root_not_home() {
        ...
        let fuigo = tmp.join("fuigo");
        let sessions = fuigo.join("sessions");
        let leaf = sessions.join("extra-hooks");
        std::fs::create_dir_all(&leaf).unwrap();
        let ws = tmp.join("ws");
        std::fs::create_dir_all(&ws).unwrap();

        let roots = [fuigo.clone(), ws.clone()];
        let pin = ancestors_within_writable_roots(&leaf, &roots);
        assert!(
            pin.iter().any(|p| p == &sessions),
            "must pin sessions under FUIGO_HOME: {pin:?}"
        );
        assert!(
            pin.iter().any(|p| p == &fuigo),
            "must pin FUIGO_HOME grant root: {pin:?}"
        );
        assert!(
