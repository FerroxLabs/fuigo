...
//! `off`. Custom profiles via `~/.fuigo/sandbox.toml` or `.fuigo/sandbox.toml`.
...
use fuigo_config::{GlobalHookSource, SANDBOX_CONFIG_FILENAME};
...
/// Load sandbox config from `~/.fuigo/sandbox.toml` and `.fuigo/sandbox.toml`.
///
...
pub fn load_sandbox_config(workspace: &Path) -> SandboxConfig {
    ...
    // Global config: ~/.fuigo/sandbox.toml
    let global_path = fuigo_home().join(SANDBOX_CONFIG_FILENAME);
    if let Some(global) = load_config_file(&global_path) {
        config = global;
    }

    // Project config: <workspace>/.fuigo/sandbox.toml (additive only)
    let project_path = workspace.join(".fuigo").join(SANDBOX_CONFIG_FILENAME);
...
pub fn sandbox_profile_conflicts(workspace: &Path) -> Vec<String> {
    let global = load_config_file(&fuigo_home().join(SANDBOX_CONFIG_FILENAME)).unwrap_or_default();
    let project = load_config_file(&workspace.join(".fuigo").join(SANDBOX_CONFIG_FILENAME))
        .unwrap_or_default();
...
impl ProfileName {
        ...
        match std::fs::symlink_metadata(path) {
            Ok(meta) if meta.file_type().is_symlink() => {
                ...
                let default_sessions =
                    fuigo_dirs::home_dir().map(|user_home| user_home.join(".fuigo").join("sessions"));
    ...
    pub(crate) fn capability_set_from_profile(
        ...
        // applied. Pre-create directories like ~/.fuigo/ that may not exist
        // on first run. Symlink children of fuigo_home fail closed unless the
        // canonical dir is this home's same-named child or default ~/.fuigo/sessions.
        let home = fuigo_home();
        ...
        if !profile.write_deny.is_empty() {
            ...
            {
                let files =
                    fuigo_config::validated_hook_json_files_for_sources(&profile.write_deny)
    ...
    fn resolve(&self, workspace: &Path, config: &SandboxConfig) -> anyhow::Result<SandboxProfile> {
        match self {
            ...
            Self::Strict => {
                let home = fuigo_dirs::home_dir().unwrap_or_else(|| PathBuf::from("/root"));
                ...
                // Landlock cannot carve children out of a write grant, so fuigo_home itself stays off read_write
                // Only sessions/ is writable; events JSONL lives there
                .chain(std::iter::once(fuigo_home()))
                .collect();
                ...
                let profile_config = config.profiles.get(name).ok_or_else(|| {
                    anyhow::anyhow!(
                        "Custom sandbox profile '{name}' not found. \
                         Define it in ~/.fuigo/sandbox.toml or .fuigo/sandbox.toml:\n\n\
                         [profiles.{name}]\n\
                         extends = \"workspace\"\n\
                         read_only = [\"/data\"]\n"
                    )
...
mod tests {
    ...
    #[test]
    fn strict_reads_fuigo_home_but_only_sessions_writable() {
        if skip_if_host_hook_write_deny_unresolvable() {
            return;
        }
        let workspace = std::env::temp_dir();
        ...
        for (label, resolved) in cases {
            ...
            assert!(
                ...
                "{label} read_only must include fuigo_home: {:?}",
                profile.read_only
            );
            ...
            assert!(
                ...
                "{label} read_write must not include fuigo_home itself: {:?}",
                profile.read_write
            );
            ...
            for p in &profile.read_write {
                ...
                assert!(
                    ...
                    "{label} read_write path under fuigo_home must be under sessions/: {p:?}"
                );
            }
            assert!(
    ...
    #[test]
    fn workspace_and_read_only_still_include_fuigo_home() {
        if skip_if_host_hook_write_deny_unresolvable() {
            return;
        }
        let workspace = std::env::temp_dir();
        ...
        for name in [ProfileName::Workspace, ProfileName::ReadOnly] {
            ...
            assert!(
                profile.read_write.iter().any(|p| p == &home),
                "{name} read_write must include fuigo_home: {:?}",
                profile.read_write
            );
        }
    }
...
"#;
    ...
    fn enxio_device_file_is_skipped_but_directory_is_not() {
        ...
        let dir = std::env::temp_dir().join(format!("fuigo-sbx-test-{}", std::process::id()));
