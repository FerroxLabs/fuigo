...
//! The `__FUIGO_INSIDE_BWRAP` marker and sentinel are reproducible by a caller, so neither proves identity.
...
pub(crate) fn ensure_bwrap_sentinel_dir() -> Result<PathBuf, String> {
    let parent = crate::paths::fuigo_home();
...
pub(crate) fn verify_bwrap_sentinel() -> Result<(), String> {
    verify_sentinel_under(&crate::paths::fuigo_home())
}
