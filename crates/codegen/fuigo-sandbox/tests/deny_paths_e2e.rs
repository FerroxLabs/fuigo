...
const FUIGO_HOME_ENV: &str = "SANDBOX_E2E_FUIGO_HOME";
...
fn apply_fixture_env(cmd: &mut Command, home: &Path, fuigo_home: &Path, workspace: &Path) {
    cmd.env(WORKSPACE_ENV, workspace.as_os_str())
        .env(HOME_ENV, home.as_os_str())
        .env(FUIGO_HOME_ENV, fuigo_home.as_os_str())
        .env("HOME", home.as_os_str())
        .env("FUIGO_HOME", fuigo_home.as_os_str());
}
...
fn run_scenario(
    ...
    apply_fixture_env(&mut cmd, home, fuigo_home, workspace);
    let output = cmd
...
fn profile_from_env() -> fuigo_sandbox::ProfileName {
    fuigo_sandbox::ProfileName::Custom(std::env::var(PROFILE_ENV).expect(PROFILE_ENV))
}
...
fn subprocess_entry() {
    ...
    let fuigo_home = PathBuf::from(std::env::var(FUIGO_HOME_ENV).expect(FUIGO_HOME_ENV));
    unsafe {
    ...
    match scenario.as_str() {
        ...
        "hook_write_deny_marker_spoof" => subprocess_hook_write_deny_marker_spoof(&fuigo_home),
...
fn subprocess_profile_and_bwrap_reexec(profile: &fuigo_sandbox::ProfileName, workspace: &Path) {
    #[cfg(target_os = "linux")]
    {
        if !fuigo_sandbox::is_inside_bwrap() {
            match fuigo_sandbox::bwrap_reexec_for_profile(profile, workspace) {
                Some(mut cmd) => {
...
fn subprocess_block_deny(workspace: &Path) {
    ...
    let mut sandbox = fuigo_sandbox::SandboxManager::new(profile.clone(), workspace);
    ...
    #[cfg(target_os = "linux")]
    match fuigo_sandbox::verify_read_deny_enforced(&profile, workspace) {
...
fn subprocess_hook_write_deny_marker_spoof(_fuigo_home: &Path) {
    #[cfg(not(target_os = "linux"))]
    {
    ...
    {
        unsafe {
            std::env::set_var("__FUIGO_INSIDE_BWRAP", "1");
        }
        match fuigo_sandbox::verify_hook_write_deny_enforced() {
            Ok(()) => {
...
fn subprocess_devbox_marker_spoof(workspace: &Path) {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let verified = fuigo_sandbox::verify_data_write_deny_enforced(
            &fuigo_sandbox::ProfileName::Devbox,
            workspace,
        );
        ...
        let mut sandbox =
            fuigo_sandbox::SandboxManager::new(fuigo_sandbox::ProfileName::Devbox, workspace);
        if let Err(e) = sandbox.apply(workspace) {
            eprintln!("sandbox apply failed: {e}");
            std::process::exit(3);
        }
...
fn subprocess_devbox_genuine(workspace: &Path) {
    ...
    #[cfg(target_os = "linux")]
    if let Err(e) = fuigo_sandbox::verify_data_write_deny_enforced(
        &fuigo_sandbox::ProfileName::Devbox,
        workspace,
    ) {
...
/// Workspace-profile Fuigo-owned hook write-deny probes (existing sources and first-run).
fn subprocess_hook_write_deny(workspace: &Path, first_run: bool) {
    let home = PathBuf::from(std::env::var(FUIGO_HOME_ENV).expect(FUIGO_HOME_ENV));
    ...
    let trust_boundary_files: Vec<(&str, PathBuf)> = fuigo_config::TRUST_BOUNDARY_FILENAMES
        .iter()
        .copied()
    ...
    if first_run {
        ...
        eprintln!("OK: first-run Fuigo hook slots denied");
    } else {
...
/// Create isolated HOME and FUIGO_HOME fixture dirs for a scenario.
fn fixture_homes(
    tag: &str,
) -> (
    PathBuf,
    ...
    fs::write(fuigo.join(fuigo_config::SANDBOX_CONFIG_FILENAME), "")
...
fn run_deny_case(
    ...
    let (home, fuigo, tmp, _ch, _cg, _cw) = fixture_homes(tag);
    let deny_list = deny_entries
        .iter()
        .map(|p| format!("\"{p}\""))
        .collect::<Vec<_>>()
        .join(", ");
    fs::create_dir_all(tmp.join(".fuigo")).expect("mkdir .fuigo");
    fs::write(
        tmp.join(".fuigo")
    ...
    fs::create_dir_all(fuigo.join("hooks")).expect("mkdir fixture hooks");
    fs::write(fuigo.join("hooks-paths"), b"").expect("write fixture hooks-paths");
    for rel in targets {
    ...
    let (status, stderr) = run_scenario(&home, &fuigo, &tmp, profile, targets, controls, postlaunch);
    assert!(
        status.success(),
...
/// Spoofing `__FUIGO_INSIDE_BWRAP` must not pass read-deny verification while a denied path is readable.
...
fn read_deny_marker_spoof_refused() {
    let (home, fuigo, workspace, _ch, _cg, _cw) = fixture_homes("read-deny-spoof");
    fs::create_dir_all(workspace.join(".fuigo")).expect("mkdir .fuigo");
    fs::write(
            workspace.join(".fuigo").join(fuigo_config::SANDBOX_CONFIG_FILENAME),
    ...
    let exe = std::env::current_exe().expect("current_exe");
    let mut cmd = Command::new(exe);
    apply_fixture_env(&mut cmd, &home, &fuigo, &workspace);
    let output = cmd
...
fn read_deny_forged_mounts_are_refused() {
    ...
    let (home, fuigo, workspace, _ch, _cg, _cw) = fixture_homes("read-deny-forged");
    fs::create_dir_all(workspace.join(".fuigo")).expect("mkdir .fuigo");
    fs::write(
        workspace
            .join(".fuigo")
    ...
    let sentinel = fuigo.join("sandbox-bwrap-sentinel");
...
fn read_deny_empty_set_verifies_inside_bwrap() {
    ...
    let (home, fuigo, workspace, _ch, _cg, _cw) = fixture_homes("read-deny-empty");
    fs::create_dir_all(workspace.join(".fuigo")).expect("mkdir .fuigo");
    fs::write(
        workspace
            .join(".fuigo")
...
/// The complete unprivileged forgery: a caller-run bwrap carrying the marker and a read-only sentinel self-bind, but no fuigo-managed deny mounts.
...
fn devbox_marker_spoof_does_not_skip_enforcement() {
    ...
    let (home, fuigo, workspace, _ch, _cg, _cw) = fixture_homes("devbox-spoof");
    let sentinel = fuigo.join("sandbox-bwrap-sentinel");
    fs::create_dir_all(&sentinel).expect("mkdir sentinel");
...
fn devbox_genuine_reexec_applies_enforcement() {
    ...
    let (home, fuigo, workspace, _ch, _cg, _cw) = fixture_homes("devbox-genuine");
    let exe = std::env::current_exe().expect("current_exe");
    let mut cmd = Command::new(exe);
    apply_fixture_env(&mut cmd, &home, &fuigo, &workspace);
    let output = cmd
...
fn hardlinked_hooks_paths_refuses_startup() {
    ...
    let (home, fuigo, workspace, _ch, _cg, _cw) = fixture_homes("hook-hl");
    fs::create_dir_all(fuigo.join("hooks")).unwrap();
    let reg = fuigo.join("hooks-paths");
    let alias = fuigo.join("hooks-paths-alias");
    fs::write(&reg, b"").unwrap();
    fs::hard_link(&reg, &alias).unwrap();
    let (status, stderr) =
        run_hook_write_deny_scenario(&home, &fuigo, &workspace, "hook_write_deny");
    assert!(
        !status.success(),
...
/// Workspace profile: Fuigo-owned direct hook sources are write-denied but readable.
/// Create / overwrite / unlink / rename / mkdir fail; absolute hooks-paths targets are denied; parent rename is blocked.
/// Fuigo/CWD/temp siblings stay writable.
#[test]
fn workspace_protects_direct_hook_sources() {
    if skip_if_enforcement_unavailable() {
        return;
    }
    let (home, fuigo, workspace, _ch, _cg, _cw) = fixture_homes("hook");
    fs::create_dir_all(fuigo.join("hooks")).expect("mkdir hooks");
    fs::write(fuigo.join("hooks").join("keep.json"), r#"{"keep-me":true}"#)
        .expect("write keep.json");
    let dynamic = fuigo.join("sessions").join("extra-hooks");
    ...
    for needle in [
        ...
        "OK: fuigo runtime sibling writable",
    ...
    for name in fuigo_config::TRUST_BOUNDARY_FILENAMES {
...
fn hardlinked_hooks_json_refuses_startup() {
    ...
    let (home, fuigo, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-hl");
    fs::create_dir_all(fuigo.join("hooks")).unwrap();
    fs::write(fuigo.join("hooks-paths"), b"").unwrap();
    let active = fuigo.join("hooks").join("active.json");
    let alias = fuigo.join("hooks").join("active-alias.json");
...
fn symlinked_hooks_json_refuses_startup() {
    ...
    let (home, fuigo, workspace, _ch, _cg, _cw) = fixture_homes("hook-json-sym");
    fs::create_dir_all(fuigo.join("hooks")).unwrap();
    fs::write(fuigo.join("hooks-paths"), b"").unwrap();
    let real = fuigo.join("real-active.json");
    let active = fuigo.join("hooks").join("active.json");
...
/// First-run: missing fixed slots are created as real Fuigo state before apply, then write-denied.
...
fn workspace_protects_direct_hook_sources_first_run() {
    ...
    let (home, fuigo, workspace, _ch, _cg, _cw) = fixture_homes("hook-fr");
    let (status, stderr) =
        run_hook_write_deny_scenario(&home, &fuigo, &workspace, "hook_write_deny_first_run");
    assert!(
        status.success(),
    ...
    for needle in [
        "OK: first-run Fuigo hook slots denied",
...
fn hook_write_deny_refuses_marker_spoof() {
    ...
    #[cfg(target_os = "linux")]
    {
        let (home, fuigo, workspace, _ch, _cg, _cw) = fixture_homes("hook-spoof");
        fs::create_dir_all(fuigo.join("hooks")).unwrap();
        fs::write(fuigo.join("hooks").join("x.json"), b"{}").unwrap();
        fs::write(fuigo.join("hooks-paths"), b"").unwrap();
        let (status, stderr) =
            run_hook_write_deny_scenario(&home, &fuigo, &workspace, "hook_write_deny_marker_spoof");
        assert!(
            status.success(),
