...
fn trailing_glob_read_write_grants_parent_directory() {
    ...
    let fuigo_home = root.join("fuigo-home");
    ...
    for dir in [&workspace, &fuigo_home, &cache.join("registry-a1b2")] {
        fs::create_dir_all(dir).expect("create fixture dir");
    }

    let cache_abs = dunce::canonicalize(&cache).expect("canonicalize cache");
    fs::write(
        fuigo_home.join(fuigo_config::SANDBOX_CONFIG_FILENAME),
        format!(
            "[profiles.cargo]\nextends = \"workspace\"\nread_write = [{:?}]\n",
...
fn subprocess_entry() {
    ...
    let profile: fuigo_sandbox::ProfileName = "cargo".parse().expect("profile name");
    let mut mgr = fuigo_sandbox::SandboxManager::new(profile, &workspace);
