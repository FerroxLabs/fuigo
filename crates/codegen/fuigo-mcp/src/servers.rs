...
/// MCP server name used as the key in client/tool maps (e.g. `"github"`, `"fuigo_com_linear"`).
...
pub struct McpState {
    ...
    /// Persisted to `~/.fuigo/config.toml` under `[mcp_servers.<name>].disabled_tools`.
...
impl McpState {
    ...
    pub fn new_with_meta(configs: Vec<acp::McpServer>, meta_config_map: McpMetaConfigMap) -> Self {
        Self {
            ...
            event_writer: fuigo_session_events::EventWriter::noop(),
    ...
    /// The `FuigoClientHandler` cloned during `try_handshake` then reads through the same Arc.
    ...
    pub fn set_event_writer(&mut self, writer: fuigo_session_events::EventWriter) {
        self.event_writer = writer;
    }

    pub fn event_writer(&self) -> &fuigo_session_events::EventWriter {
        &self.event_writer
    }
...
impl McpError {
    ...
    pub fn error_category(&self) -> fuigo_session_events::McpErrorCategory {
        use fuigo_session_events::McpErrorCategory;
        match self {
...
///
/// MCP tools are already untyped (JSON in, JSON out), so they implement `fuigo_tool_runtime::Tool` directly instead of going through typed wrappers.
...
impl fuigo_tool_runtime::Tool for McpErasedTool {
    ...
    fn description(
        ...
        fuigo_tool_types::ToolDescription::new(&self.tool.name, &self.tool.description)
    }
    ...
    async fn run(
    ...
    ) -> Result<ToolOutput, fuigo_tool_runtime::ToolError> {
        let call_span = fuigo_telemetry::region::Region::from_span(tracing::info_span!(
            "mcp.tool_call",
        ...
        event_writer.emit(fuigo_session_events::Event::McpToolCallStarted {
            ...
            .await
        {
            ...
            Err(first_err) if client.has_auth() => {
                ...
                ew.emit(fuigo_session_events::Event::McpAuthRetry {
                    server_name: server.clone(),
                ...
                if reauth_ok {
                    self.try_call_tool(&client, &raw, &mut reconnect_attempted, &mut is_timeout, ew)
                        .await
                        .map_err(|e| {
                            fuigo_tool_runtime::ToolError::custom("process_manager", e.to_string())
                        })
                } else {
                    Err(first_err)
                }
        ...
        let call_result = match dispatch_result {
            Ok(result) => result,
            Err(e) => {
                ew.emit(fuigo_session_events::Event::McpToolCallCompleted {
                    server_name: server.clone(),
                    tool_name: tool.clone(),
        ...
        event_writer.emit(fuigo_session_events::Event::McpToolCallCompleted {
            server_name: server.clone(),
            tool_name: tool.clone(),
            call_id: qualified_name.clone(),
            duration_ms,
        ...
        fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::McpToolCalled {
            server_name: server.clone(),
            tool_name: tool.clone(),
            qualified_name,
            success,
...
impl McpErasedTool {
    async fn try_call_tool(
        ...
        is_timeout: &mut bool,
        ew: &fuigo_session_events::EventWriter,
    ) -> Result<rmcp::model::CallToolResult, fuigo_tool_runtime::ToolError> {
    ...
    async fn recover_and_retry(
        ...
        *reconnect_attempted = true;
        ...
        ew.emit(fuigo_session_events::Event::McpTransportError {
        ...
        let mcp_service = match client.recover().await {
            Ok(service) => {
                ew.emit(fuigo_session_events::Event::McpTransportReconnect {
                    server_name: self.tool.server_name.clone(),
                    success: true,
                    error: None,
                });
...
async fn decide_http_auth_over_network(
    ...
    let outcome = {
        let _auth_discovery_timer =
            fuigo_telemetry::instrumentation::timer("mcp_http_auth_discovery");
        ...
        .await
        {
            ...
            Err(_) => {
                ...
                ctx.event_writer
                    .emit(fuigo_session_events::Event::McpOAuthDiscoveryTimeout {
    ...
    match outcome {
        ...
        OauthProbeOutcome::Inconclusive => {
            ...
            ctx.event_writer
                .emit(fuigo_session_events::Event::McpOAuthProbeResolved {
                    server_name: server_name.to_string(),
    ...
    W: AsyncWrite + Send + Unpin + 'static,
{
    ...
    fn record_decode_error(&self, line: &[u8], err: &serde_json::Error) {
        ...
        self.event_writer
            .emit(fuigo_session_events::Event::McpTransportDecodeError {
...
/// Uses [`FuigoClientHandler`] rather than rmcp's default `ClientInfo` handler.
...
pub type McpService = Arc<RunningService<RoleClient, FuigoClientHandler>>;
...
enum ClientState {
    ...
    Ready {
        service: McpService,
        _connected: fuigo_telemetry::activity::ActivityGaugeGuard,
    },
}
...
/// 2. [`FuigoClientHandler`] when the server pushes a notification we care about.
...
pub struct McpClient {
    ...
    /// The `Arc<Mutex<...>>` is **shared with [`FuigoClientHandler`]** constructed by [`Self::make_client_handler`].
...
/// Shared sender slot: the same Arc lives on the [`McpClient`] and the [`FuigoClientHandler`] it constructs during [`McpClient::try_handshake`].
...
/// Surfaced to fuigo-mcp from whatever loader the host crate uses (e.g. the host's `config.toml` parser).
///
...
impl McpClient {
    ...
    fn make_client_info(server_name: &str, advertise_elicitation: bool) -> ClientInfo {
        ...
        ClientInfo::new(
            capabilities,
            Implementation::new(
                format!("fuigo-shell-{server_name}"),
    ...
    /// The [`FuigoClientHandler`] handed to `client.serve` and the [`crate::liveness::spawn_transport_liveness`] task read through the same Arc.
    ...
    pub async fn get_tool_registrations(
        ...
        let _list_tools_timer = fuigo_telemetry::instrumentation::timer("mcp_list_tools");
        loop {
...
fn is_figma_mcp(server_name: &str, url: &str) -> bool {
    ...
    // Legacy direct managed name (`fuigo_com_figma`); newer clients use gateway tools (`managed_mcp_gateway_tools_enabled`).
    const MANAGED_PREFIX: &str = "fuigo_com_";
...
fn ensure_figma_user_agent(headers: &mut reqwest::header::HeaderMap, server_name: &str, url: &str) {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("fuigo-cli"),
    );
}

static DEFAULT_USER_AGENT: LazyLock<reqwest::header::HeaderValue> = LazyLock::new(|| {
    format!("fuigo-cli/{}", fuigo_version::VERSION)
        .parse()
        .unwrap_or_else(|_| reqwest::header::HeaderValue::from_static("fuigo-cli"))
});
...
/// Figma keeps its pinned bare `fuigo-cli` attribution token; every other server gets the versioned default.
...
pub struct McpSpawnCtx<'a> {
    ...
    pub(crate) event_writer: &'a fuigo_session_events::EventWriter,
...
impl<'a> McpSpawnCtx<'a> {
    pub fn for_session(
        session_id: &'a str,
        event_writer: &'a fuigo_session_events::EventWriter,
        mode: OauthInteractivity,
    ...
    pub fn standalone(event_writer: &'a fuigo_session_events::EventWriter) -> Self {
        Self {
            session_id: None,
            event_writer,
...
pub async fn start_mcp_server(
    ...
    let _per_server_timer = fuigo_telemetry::instrumentation::timer("mcp_start_one_server");
    match mcp_server {
...
pub async fn start_mcp_servers(
    ...
    let _mcp_start_timer = fuigo_telemetry::instrumentation::timer("mcp_start_servers");
...
impl ClientHandler for FuigoClientHandler {
    ...
    // See the [`FuigoClientHandler`] doc-comment for the full RPIT contract
    ...
    async fn on_url_elicitation_notification_complete(
        ...
        if !fuigo_tools::mcp_elicitation::chars_within(
            &params.elicitation_id,
            fuigo_tools::mcp_elicitation::MAX_ELICIT_ID_CHARS,
        ) {
            tracing::warn!(
                server = %self.server_name,
