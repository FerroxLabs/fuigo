...
fn is_figma_mcp_matches_name_and_host() {
    ...
    assert!(is_figma_mcp("Figma", "https://example.com/mcp"));
    assert!(is_figma_mcp("fuigo_com_figma", "https://example.com/mcp"));
    assert!(is_figma_mcp("FUIGO_COM_FIGMA", "https://example.com/mcp"));
    assert!(is_figma_mcp("fuigo_com_FIGMA", "https://example.com/mcp"));
    ...
    assert!(!is_figma_mcp("fuigo_com_linear", "https://example.com/mcp"));
...
#[test]
fn ensure_figma_user_agent_sets_fuigo_cli_when_missing() {
...
#[test]
fn apply_user_agent_policy_sets_versioned_fuigo_cli() {
    let mut headers = reqwest::header::HeaderMap::new();
    apply_user_agent_policy(&mut headers, "linear", "https://mcp.linear.app/mcp");
    let expected = format!("fuigo-cli/{}", fuigo_version::VERSION);
    assert_eq!(
        headers.get(reqwest::header::USER_AGENT).unwrap(),
        expected.as_str()
    );
...
fn test_same_raw_name_different_servers_no_local_registry_collision() {
    use fuigo_computer_hub_sdk::LocalRegistry;
    use fuigo_tool_runtime::Tool;
...
async fn http_transport_sends_default_user_agent_on_initialize() {
    ...
    assert_eq!(
        *handles.init_user_agents.lock(),
        vec![format!("fuigo-cli/{}", fuigo_version::VERSION)]
    );
}
...
async fn try_call_tool_http_mcperror_recovers_then_retry_succeeds() {
    ...
    let ew = fuigo_session_events::EventWriter::open(tmp.path());
...
async fn try_call_tool_mrtr_form_elicitation_round_trip() {
    ...
    let answer = tokio::spawn(async move {
        ...
        assert!(
            matches!(
                job.fields.mode,
                fuigo_tools::mcp_elicitation::McpElicitModeFields::Form {
...
async fn try_call_tool_mrtr_url_elicitation_with_state_only_round() {
    ...
    let answer = tokio::spawn(async move {
        let job = inbox
            ...
            .await
            .expect("elicitation job reaches the inbox");
        let fuigo_tools::mcp_elicitation::McpElicitModeFields::Url {
            url,
            elicitation_id,
        } = &job.fields.mode
        else {
...
fn is_auth_rejection_message_matches_auth_signals() {
    assert!(is_auth_rejection_message(
        "MCP server 'fuigo_com_notion' handshake failed: Auth required, when send initialize request"
    ));
...
fn session_test_ctx(event_writer: &fuigo_session_events::EventWriter) -> McpSpawnCtx<'_> {
    crate::isolate_fuigo_home_for_tests();
...
async fn inconclusive_oauth_probe_emits_timeout_and_verdict_events() {
    let (url, _handles) = spawn_fake_streamable_http(axum::http::StatusCode::OK).await;
    let tmp = tempfile::tempdir().unwrap();
    let event_writer = fuigo_session_events::EventWriter::open(tmp.path());
