...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer the live `$FUIGO_HOME` over the `fuigo_home()` OnceLock
        // A home injected by the PTY e2e harness must beat a path some earlier init cached in the same process
        Self::from_env_home()
    }

    /// Resolve cache paths from the live process environment (not the `fuigo_home()` OnceLock).
    /// A seeded `$FUIGO_HOME` set on the pager process is always honoured even if some earlier init path cached a different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("FUIGO_HOME")
            .map(std::path::PathBuf::from)
            .filter(|p| !p.as_os_str().is_empty())
            .unwrap_or_else(crate::util::fuigo_home::fuigo_home);
        Self {
    ...
    /// When `FUIGO_CHANGELOG_OFFLINE` is set (PTY / integration tests), the CDN is skipped and only the disk cache is read.
    ...
    /// Mutating `FUIGO_HOME` / `FUIGO_CHANGELOG_OFFLINE` races across the parallel test harness.
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // If the CDN is unreachable (CI sandboxes, airplane mode), fall back to any on-disk seed under `$FUIGO_HOME`
...
fn changelog_offline() -> bool {
    std::env::var_os("FUIGO_CHANGELOG_OFFLINE").is_some_and(|v| !v.is_empty() && v != "0")
}
...
mod tests {
    ...
    /// Build a manager pointing at `home` directly, bypassing the global `$FUIGO_HOME` env so tests never race the parallel harness.
    ...
    fn cdn_miss_falls_back_to_env_home_disk_cache() {
        let tmp = tempfile::tempdir().unwrap();
        let home = tmp.path().join("fuigo-home-fallback");
        std::fs::create_dir_all(&home).unwrap();
