...
pub use fuigo_dirs::{default_fuigo_home, fuigo_home, user_fuigo_home};
...
/// Canonical fuigo application path: `$FUIGO_HOME/bin/fuigo` (Unix) or `fuigo.exe` (Windows).
pub fn fuigo_application() -> PathBuf {
    fuigo_application_in(&fuigo_home())
}

/// [`fuigo_application`] under an explicit home instead of `$FUIGO_HOME`.
pub fn fuigo_application_in(home: &std::path::Path) -> PathBuf {
    let name = if cfg!(windows) { "fuigo.exe" } else { "fuigo" };
...
pub fn sessions_cwd_dir(cwd: &str) -> PathBuf {
    sessions_cwd_dir_in(&fuigo_home(), cwd)
}

/// [`sessions_cwd_dir`] with an injectable fuigo home, the single source of truth for the `sessions/<encoded-cwd>` path shape.
pub fn sessions_cwd_dir_in(fuigo_home: &std::path::Path, cwd: &str) -> PathBuf {
    fuigo_home.join("sessions").join(encode_cwd_dirname(cwd))
}
...
pub fn ensure_sessions_cwd_dir(cwd: &str) -> std::io::Result<PathBuf> {
    ensure_sessions_cwd_dir_in(&fuigo_home(), cwd)
}

/// [`ensure_sessions_cwd_dir`] with an injectable fuigo home.
pub fn ensure_sessions_cwd_dir_in(
    fuigo_home: &std::path::Path,
    cwd: &str,
) -> std::io::Result<PathBuf> {
    let encoded_name = encode_cwd_dirname(cwd);
    let dir = sessions_cwd_dir_in(fuigo_home, cwd);
    ...
    set_dir_owner_only(&fuigo_home.join("sessions"));
...
mod tests {
    ...
    fn ensure_sessions_cwd_dir_retightens_existing_loose_dirs() {
        ...
        let dir = ensure_sessions_cwd_dir_in(home.path(), "/some/project").unwrap();
        // Simulate dirs created by an older fuigo with umask-default perms.
        std::fs::set_permissions(&dir, std::fs::Permissions::from_mode(0o755)).unwrap();
