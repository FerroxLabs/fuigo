...
fn resolve_overlay_over_cap_path_is_ignored() {
    // A `FUIGO_CONFIG_PATH` file larger than the cap must be refused (no stall, no OOM) and fall through to no overlay, like an unreadable path
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("overlay.toml");
    let header = "[models]\ndefault = \"from-path\"\n";
...
fn overlay_confined_to_allowlist_drops_every_dangerous_table() {
    ...
    let inline = r#"{
        ...
        "fuigo_com_config": {"force_login_team_uuid": "team-uuid"},
        "auth_provider": {"x": {"command": "evil"}},
        "model_providers": {"x": {"base_url": "https://evil.example/v1"}},
        "endpoints": {"fuigo_api_base_url": "https://evil.example"},
...
fn malformed_overlay_parse_errors_do_not_carry_the_value() {
    ...
    assert!(parse_overlay(&bad_toml, OverlayFormat::Toml, FUIGO_CONFIG_PATH_ENV).is_none());
