...
//! Real tamper resistance is [`crate::signed_policy`] plus the OS-protected layers (root-owned `/etc/fuigo`, MDM).

use std::path::Path;

use crate::paths::user_fuigo_home;
...
pub fn is_managed_config_stale_for(identity: &ServingIdentity) -> bool {
    managed_config_stale_at(user_fuigo_home().as_deref(), identity)
}
...
pub fn mark_managed_config_synced(marker: SyncMarker<'_>) {
    if let Some(home) = user_fuigo_home() {
        mark_managed_config_synced_at(&home, marker);
    }
}

/// Server-side FuigoBuildDeployment UUID from the last deploy-key managed-config sync, bound to the key that synced it.
...
pub fn managed_deployment_id(key_fingerprint: &str) -> Option<String> {
    managed_deployment_id_at(user_fuigo_home()?.as_path(), key_fingerprint)
}
...
pub fn confirmed_team_switch(new_team_id: &str) -> Option<String> {
    user_fuigo_home().and_then(|home| confirmed_team_switch_at(&home, new_team_id))
}
...
pub fn managed_policy_compromised_for(identity: &ServingIdentity) -> bool {
    user_fuigo_home().is_some_and(|home| managed_policy_compromised_for_at(&home, identity))
}
...
/// Override with `FUIGO_DEPLOYMENT_CONFIG_CACHE_TTL_SECS` for testing.
fn managed_config_stale_threshold() -> std::time::Duration {
    if let Ok(s) = std::env::var("FUIGO_DEPLOYMENT_CONFIG_CACHE_TTL_SECS")
