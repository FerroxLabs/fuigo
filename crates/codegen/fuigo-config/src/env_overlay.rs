//! `FUIGO_CONFIG` / `FUIGO_CONFIG_PATH` config overlay resolution.
...
pub const FUIGO_CONFIG_ENV: &str = "FUIGO_CONFIG";

/// Path to an additional JSON or TOML config-overlay file (read by extension).
pub const FUIGO_CONFIG_PATH_ENV: &str = "FUIGO_CONFIG_PATH";

/// Hard cap on a `FUIGO_CONFIG_PATH` overlay read.
...
/// A resolved overlay for the merge and for `fuigo inspect`.
#[derive(Debug, Clone)]
...
fn env_overlay_inputs() -> (Option<String>, Option<PathBuf>) {
    let inline = match std::env::var_os(FUIGO_CONFIG_ENV) {
        Some(raw) => match raw.into_string() {
            Ok(s) => Some(s),
            Err(_) => {
                tracing::warn!("FUIGO_CONFIG is not valid UTF-8; ignoring the overlay");
                None
            }
        },
        None => None,
    };
    let path = std::env::var_os(FUIGO_CONFIG_PATH_ENV)
        .filter(|v| !v.is_empty())
        .map(PathBuf::from);
    (inline, path)
}
...
pub(crate) fn load_env_overlay() -> Option<toml::Value> {
    ...
    let source_label = match source {
        OverlaySource::Inline => FUIGO_CONFIG_ENV,
        OverlaySource::Path(_) => FUIGO_CONFIG_PATH_ENV,
    };
    tracing::trace!(
        source = source_label,
        ?sections,
        "resolved FUIGO_CONFIG overlay"
    );
    Some(overlay)
}

/// The resolved overlay for `fuigo inspect`.
...
fn resolve_overlay_detailed(
    ...
    // Inline (`FUIGO_CONFIG`) wins only when it fully succeeds
...
fn resolve_inline_overlay(inline: &str) -> Option<(toml::Value, OverlaySource, Vec<String>)> {
    ...
    let overlay = parse_overlay(trimmed, OverlayFormat::Json, FUIGO_CONFIG_ENV)?;
    finalize_overlay(overlay, FUIGO_CONFIG_ENV, OverlaySource::Inline)
}

/// Read, parse, and run the full pipeline for the `FUIGO_CONFIG_PATH` candidate.
fn resolve_path_overlay(path: &Path) -> Option<(toml::Value, OverlaySource, Vec<String>)> {
    ...
    let overlay = parse_overlay(&raw, format, FUIGO_CONFIG_PATH_ENV)?;
    finalize_overlay(
        overlay,
        FUIGO_CONFIG_PATH_ENV,
        OverlaySource::Path(path.to_path_buf()),
    )
}

/// Read `FUIGO_CONFIG_PATH` with a hard byte cap ([`MAX_OVERLAY_BYTES`]).
...
fn read_capped_overlay_file(path: &Path) -> Option<String> {
    ...
    let file = match std::fs::File::open(path) {
        Ok(f) => f,
        Err(e) => {
            tracing::warn!(path = %path.display(), error = %e, "FUIGO_CONFIG_PATH is unreadable; ignoring the overlay");
            return None;
        }
    };
    ...
    match file.metadata() {
        Ok(meta) if !meta.file_type().is_file() => {
            tracing::warn!(path = %path.display(), "FUIGO_CONFIG_PATH is not a regular file; ignoring the overlay");
            return None;
        }
        Ok(_) => {}
        Err(e) => {
    ...
    if raw.len() as u64 > MAX_OVERLAY_BYTES {
        tracing::warn!(path = %path.display(), max = MAX_OVERLAY_BYTES, "FUIGO_CONFIG_PATH exceeds the max overlay size; ignoring the overlay");
        return None;
    }
    Some(raw)
}
