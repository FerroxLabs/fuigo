//! Fuigo-owned direct global hook paths shared by shell discovery and sandbox write-deny.
//! These are `$FUIGO_HOME/hooks`, `hooks-paths`, and absolute registry targets.
...
pub enum GlobalHookSourceKind {
    /// `$FUIGO_HOME/hooks/` (discovered and protected).
    HookDirectory,
    /// `$FUIGO_HOME/hooks-paths` (protected; never loaded as hook JSON).
    RegistryFile,
    /// Absolute registry target (must exist before sandbox apply).
    ConfiguredSource,
    /// `$FUIGO_HOME` trust-boundary file (protected; never hook JSON or a discovery source).
    TrustBoundaryFile,
}
...
pub enum GlobalHookSourceError {
    ...
    #[error("symlinked FUIGO_HOME is not allowed under sandbox write-deny: {path}")]
    SymlinkedFuigoHome { path: PathBuf },
    ...
    #[error("Fuigo hooks directory has wrong type (expected real directory): {path}")]
    InvalidHooksDir { path: PathBuf },
    #[error("Fuigo hooks-paths registry has wrong type (expected real file): {path}")]
    InvalidRegistryFile { path: PathBuf },
    #[error("cannot create Fuigo hooks directory {path}: {source}")]
    CreateHooksDir {
        path: PathBuf,
        #[source]
        source: io::Error,
    },
    #[error("cannot create Fuigo hooks-paths registry {path}: {source}")]
    CreateRegistryFile {
        path: PathBuf,
        #[source]
        source: io::Error,
...
/// Ensure real `$FUIGO_HOME/hooks` dir and `hooks-paths` file (create if missing).
...
pub fn ensure_fuigo_hook_slots(fuigo_home: &Path) -> Result<(), GlobalHookSourceError> {
    if path_has_symlink_component(fuigo_home) {
        return Err(GlobalHookSourceError::SymlinkedFuigoHome {
            path: fuigo_home.to_path_buf(),
        });
    }

    match std::fs::create_dir(fuigo_home) {
        Ok(()) => {}
        Err(e) if e.kind() == io::ErrorKind::AlreadyExists => {}
        Err(e) if e.kind() == io::ErrorKind::NotFound => {
            std::fs::create_dir_all(fuigo_home).map_err(|source| {
                GlobalHookSourceError::CreateHooksDir {
                    path: fuigo_home.to_path_buf(),
                    source,
                }
    ...
    let fuigo_meta = std::fs::symlink_metadata(fuigo_home).map_err(|source| {
        GlobalHookSourceError::CreateHooksDir {
            path: fuigo_home.to_path_buf(),
            source,
        }
    })?;
    if fuigo_meta.file_type().is_symlink() || !fuigo_meta.file_type().is_dir() {
        return Err(GlobalHookSourceError::SymlinkedFuigoHome {
            path: fuigo_home.to_path_buf(),
        });
    }

    let hooks = fuigo_home.join("hooks");
    ...
    ensure_real_file_slot(&fuigo_home.join("hooks-paths"))?;

    ensure_fuigo_trust_boundary_slots(fuigo_home)?;
    Ok(())
}

/// `$FUIGO_HOME` files that are always-trusted or trust-granting if writable.
...
pub(crate) fn ensure_fuigo_trust_boundary_slots(
    fuigo_home: &Path,
    ...
    for name in TRUST_BOUNDARY_FILENAMES {
        ensure_real_file_slot(&fuigo_home.join(name))?;
    }
    Ok(())
}

/// Resolve `$FUIGO_HOME` trust-boundary files (symlink-rejected for sandbox).
...
/// Resolve Fuigo-owned direct global hook sources (`reject_symlinks` for sandbox).
pub fn resolve_global_hook_sources(
    ...
    if let Some(fuigo) = fuigo_home {
        if reject_symlinks && path_has_symlink_component(fuigo) {
            return Err(GlobalHookSourceError::SymlinkedFuigoHome {
                path: fuigo.to_path_buf(),
            });
        }

        let hooks = fuigo.join("hooks");
        let hooks_paths = fuigo.join("hooks-paths");
        if reject_symlinks {
