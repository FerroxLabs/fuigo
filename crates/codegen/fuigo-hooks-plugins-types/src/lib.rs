...
/// Plugin scope.
///
/// Maps from `PluginScope` in `fuigo-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `fuigo-agent`. Optional on [`PluginInfo`]
...
pub enum PluginOrigin {
    ...
    /// Fuigo's install registry (marketplace or direct git/local install).
    MarketplaceInstall {
...
/// Hook event type.
///
/// Maps from `HookEventName` in `fuigo-hooks`. The source type's
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.fuigo/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
...
mod tests {
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.fuigo/hooks".into(),
            disabled: false,
            pinned: false,
            removable: true,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.fuigo/plugins/test-plugin".into(),
            ...
            origin: Some(PluginOrigin::UserFuigo),
            conflict: None,
        };
