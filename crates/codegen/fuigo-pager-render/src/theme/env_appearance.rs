...
//!
//! `LC_FUIGO_APPEARANCE` is the SSH-surviving alias (`AcceptEnv LC_*`).
...
/// Ordered lookup: `FUIGO_APPEARANCE`, then `LC_FUIGO_APPEARANCE`, then `COLORFGBG`.
#[must_use]
...
pub fn detect_explicit_from_env_map(env: &HashMap<String, String>) -> Option<SystemAppearance> {
    parse_appearance_var(env_nonempty(env, "FUIGO_APPEARANCE"))
        .or_else(|| parse_appearance_var(env_nonempty(env, "LC_FUIGO_APPEARANCE")))
}
...
mod tests {
    ...
    #[test]
    fn fuigo_appearance_dark_and_light() {
        assert_eq!(
            detect_from_env_map(&env(&[("FUIGO_APPEARANCE", "dark")])),
            Some(SystemAppearance::Dark)
        );
        assert_eq!(
            detect_from_env_map(&env(&[("FUIGO_APPEARANCE", "Light")])),
            Some(SystemAppearance::Light)
        );
        assert_eq!(
            detect_from_env_map(&env(&[("FUIGO_APPEARANCE", "night")])),
            Some(SystemAppearance::Dark)
        );
        assert_eq!(
            detect_from_env_map(&env(&[("FUIGO_APPEARANCE", "day")])),
            Some(SystemAppearance::Light)
        );
    }
    ...
    fn lc_alias_used_when_canonical_absent() {
        assert_eq!(
            detect_from_env_map(&env(&[("LC_FUIGO_APPEARANCE", "light")])),
            Some(SystemAppearance::Light)
        );
    }
    ...
    fn canonical_wins_over_lc_and_colorfgbg() {
        assert_eq!(
            detect_from_env_map(&env(&[
                ("FUIGO_APPEARANCE", "dark"),
                ("LC_FUIGO_APPEARANCE", "light"),
                ("COLORFGBG", "0;15"),
            ])),
            Some(SystemAppearance::Dark)
        );
    ...
    fn unknown_or_empty_appearance_falls_through_to_colorfgbg() {
        assert_eq!(
            detect_from_env_map(&env(&[
                ("FUIGO_APPEARANCE", "solarized"),
                ("COLORFGBG", "15;0"),
            ])),
            Some(SystemAppearance::Dark)
        );
        assert_eq!(
            detect_from_env_map(&env(&[("FUIGO_APPEARANCE", ""), ("COLORFGBG", "0;15"),])),
            Some(SystemAppearance::Light)
        );
    }
    ...
    #[test]
    fn unknown_or_empty_lc_falls_through_to_colorfgbg_when_fuigo_absent() {
        assert_eq!(
            detect_from_env_map(&env(&[
                ("LC_FUIGO_APPEARANCE", "solarized"),
                ("COLORFGBG", "15;0"),
            ])),
            Some(SystemAppearance::Dark)
        );
        assert_eq!(
            detect_from_env_map(&env(&[("LC_FUIGO_APPEARANCE", ""), ("COLORFGBG", "0;15"),])),
            Some(SystemAppearance::Light)
        );
    }
    ...
    fn colorfgbg_map_ignores_explicit_hints() {
        ...
        assert_eq!(
            detect_colorfgbg_from_env_map(&env(&[("FUIGO_APPEARANCE", "dark")])),
            None
        );
    }
}
