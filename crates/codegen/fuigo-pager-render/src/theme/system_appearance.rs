...
//! 2. Explicit env stamps, `FUIGO_APPEARANCE` / `LC_FUIGO_APPEARANCE` (SSH with tmux, wrap, headless).
...
///
/// Used by `fuigo wrap` to stamp the *local* OS theme into the child env before SSH.
...
/// When `None`, defaults to `FuigoNight` / `FuigoDay`.
#[must_use]
pub fn to_theme_kind(
    ...
    match appearance {
        SystemAppearance::Light => light_theme.unwrap_or(ThemeKind::FuigoDay),
        SystemAppearance::Dark => dark_theme.unwrap_or(ThemeKind::FuigoNight),
    }
}
