...
//! The default theme is FuigoNight (neutral gray base with TokyoNight accents).
//!
//! ## Color support
//!
//! FuigoNight is defined in `Color::Rgb` (truecolor).
...
impl ThemeKind {
    ...
    /// FuigoNight uses neutral grays that survive quantization cleanly.
    pub fn requires_truecolor(self) -> bool {
        match self {
            Self::FuigoNight => false,
...
/// Human-friendly display name for a canonical theme value (e.g. `"fuigonight"` becomes `"Fuigo Night"`).
/// Falls back to `value` verbatim.
pub fn display_name_for_canonical(value: &str) -> &str {
    match value {
        "auto" => "Auto",
        "fuigonight" => "Fuigo Night",
        "fuigoday" => "Fuigo Day",
...
impl Theme {
    ...
    ///
    /// Reads the active theme kind (loaded from `~/.fuigo/config.toml` on
    ...
    pub fn current() -> Self {
        ...
        //   1. Any terminal that only advertises 16-color support (e.g., `TERM=xterm`, `TERM=ansi`, or `FUIGO_FORCE_COLOR_LEVEL=basic`).
    ...
    ///    An audit on fuigonight found 18 of 27 fields became DarkGray or silver, erasing every state signal.
    ...
    /// FuigoNight on a white terminal, for example, would show white "holes" through every sunken surface.
    fn ansi16_chrome_overrides(self, dark: bool) -> Self {
        ...
        Self {
            ...
            // Without this every FuigoNight bg field quantizes to Color::Black and these become invisible
            ...
            // Markdown content: naive Basic quantize lands FuigoNight md_code / md_muted / h4-h6 on DarkGray (ANSI 8)
...
pub fn apply_cursor_color() {
    ...
    fuigo_shared::stderr::with_locked_stderr(|stderr| {
...
mod tests {
    ...
    fn is_auto_returns_false_for_concrete_variants() {
        assert!(!ThemeKind::FuigoNight.is_auto());
        assert!(!ThemeKind::FuigoDay.is_auto());
    ...
    fn ansi16_overrides_light_inverts_high_contrast_and_elevated_bg() {
        ...
        use ratatui::style::Color;
        let t = Theme::fuigoday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_state_accents_pin_to_polarity_aware_hue() {
        ...
        use ratatui::style::Color;
        let t_dark = Theme::fuigonight().ansi16_chrome_overrides(true);
        ...
        let t_light = Theme::fuigoday().ansi16_chrome_overrides(false);
    ...
    fn ansi16_overrides_canvas_matching_surfaces_use_theme_polarity() {
        ...
        // Color::Reset would defer to the user's terminal canvas, which can disagree with the theme (e.g. FuigoNight on a white-canvas terminal).
        use ratatui::style::Color;
        let t_dark = Theme::fuigonight().ansi16_chrome_overrides(true);
    ...
    fn scrollbar_thumb_contrasts_with_track_in_all_themes() {
        ...
        for &kind in ThemeKind::ALL {
            let theme = match kind {
                ThemeKind::FuigoNight => Theme::fuigonight(),
                ThemeKind::FuigoDay => Theme::fuigoday(),
    ...
    #[test]
    fn ansi16_quantize_without_override_collapses_fuigonight_backgrounds() {
        // Regression-ratchet for the override gate in `Theme::current`
        // Naive `Basic` quantization maps every dark FuigoNight bg field to `Color::Black`, erasing the hierarchy
        ...
        use ratatui::style::Color;
        let q = Theme::fuigonight().quantized(color_support::ColorLevel::Basic);
        for (name, color) in [
            ("bg_base", q.bg_base),
            ("bg_light", q.bg_light),
            ("bg_dark", q.bg_dark),
    ...
    fn from_name_concrete_variants_still_work() {
        assert_eq!(
            ThemeKind::from_name("fuigonight"),
            Some(ThemeKind::FuigoNight)
        );
        assert_eq!(ThemeKind::from_name("dark"), Some(ThemeKind::FuigoNight));
        assert_eq!(ThemeKind::from_name("fuigoday"), Some(ThemeKind::FuigoDay));
        assert_eq!(ThemeKind::from_name("light"), Some(ThemeKind::FuigoDay));
        assert_eq!(
    ...
    fn from_str_matches_from_name_for_all_canonicals() {
        ...
        let cases = [
            ...
            ("fuigonight", ThemeKind::FuigoNight),
            ("fuigo-night", ThemeKind::FuigoNight),
            ("dark", ThemeKind::FuigoNight),
            ("tokyonight", ThemeKind::TokyoNight),
            ("tokyo-night", ThemeKind::TokyoNight),
            ("tokyo", ThemeKind::TokyoNight),
            ("fuigoday", ThemeKind::FuigoDay),
            ("fuigo-day", ThemeKind::FuigoDay),
            ("light", ThemeKind::FuigoDay),
            ("day", ThemeKind::FuigoDay),
