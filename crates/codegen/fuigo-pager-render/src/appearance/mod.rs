...
//!
//! - **`config` + `watcher`**: dev-only `~/.fuigo/pager.toml` RenderConfig
...
//!   Disk writes happen in `fuigo_shell::util::config::set_<field>()` via `Effect::PersistSetting`, NOT here; this is a read-cache only.
