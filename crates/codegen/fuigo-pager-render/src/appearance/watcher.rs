//! In dev mode, watches ~/.fuigo/pager.toml for changes and hot-reloads.
//! In prod mode, returns static defaults (no file operations).
use super::config::AppearanceConfig;
use std::io;
use std::path::PathBuf;
use tokio::sync::watch;
/// In dev mode: reads from ~/.fuigo/pager.toml, watches for changes.
...
impl ConfigWatcher {
    /// - In dev mode: reads/creates ~/.fuigo/pager.toml, watches for changes
    ...
    fn start_static() -> io::Result<Self> {
        let config = fuigo_config::user_fuigo_home()
