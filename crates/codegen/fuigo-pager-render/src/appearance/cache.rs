...
//!
//! Disk writes live in `fuigo_shell::util::config::set_<field>()`; this module only caches in memory.
//!
...
use fuigo_shared::ui_config::UiConfig;
...
/// `FUIGO_SCROLL_SPEED` overrides config.toml.
pub fn load_scroll_speed() -> u8 {
    SCROLL_SPEED_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("FUIGO_SCROLL_SPEED")
                .ok()
...
/// `FUIGO_SCROLL_MODE` overrides `[ui].scroll_mode`, like `FUIGO_SCROLL_SPEED` does for speed; unrecognized values fall back to `auto`.
pub fn load_scroll_mode() -> ScrollMode {
    SCROLL_MODE_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("FUIGO_SCROLL_MODE")
                .ok()
...
/// `FUIGO_INVERT_SCROLL` (`1`/`true`/`0`/`false`) overrides `[ui].invert_scroll`.
pub fn load_invert_scroll() -> bool {
    INVERT_SCROLL_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("FUIGO_INVERT_SCROLL")
                .ok()
...
/// `FUIGO_SCROLL_LINES` overrides `[ui].scroll_lines`.
pub fn load_scroll_lines() -> Option<u8> {
    SCROLL_LINES_LOADED.with(|loaded| {
        if !loaded.get() {
            let from_env = std::env::var("FUIGO_SCROLL_LINES")
                .ok()
...
fn load_keep_text_selection_from_effective_config() -> TextSelection {
    let root = match fuigo_config::load_effective_config_disk_only() {
        Ok(r) => r,
