...
pub(crate) fn shorten_location_path(path: &str) -> Cow<'_, str> {
    ...
    const FUIGO_HOME_PREFIX: &str = "$FUIGO_HOME";
    ...
    let (prefix, remainder, restore_root) = if path == FUIGO_HOME_PREFIX
        || path
            .strip_prefix(FUIGO_HOME_PREFIX)
            .is_some_and(|rest| rest.starts_with(['/', '\\']))
    {
        (
            Cow::Borrowed(FUIGO_HOME_PREFIX),
            path.get(FUIGO_HOME_PREFIX.len()..)
                .unwrap_or("")
                .trim_start_matches(['/', '\\']),
            false,
        )
