...
pub enum TerminalName {
    ...
    JetBrains,
    /// Fuigo Desktop (Electron app).
    #[strum(to_string = "Fuigo Desktop")]
    FuigoDesktop,
...
impl TerminalName {
    ...
    pub fn is_xtermjs_embed(self) -> bool {
        matches!(
            self,
            Self::VsCode | Self::Cursor | Self::Windsurf | Self::FuigoDesktop
        )
    }
...
pub struct TerminalContext {
    ...
    /// Which embedded editor `:terminal` fuigo is running inside, if any.
...
impl TerminalContext {
    ...
    /// A heal keyed off this only fires when a FocusGained actually reaches fuigo.
    ...
    pub fn telemetry_snapshot(&self) -> fuigo_telemetry::events::TerminalTelemetry {
        ...
        fuigo_telemetry::events::TerminalTelemetry {
            ...
            clipboard_native_tool: fuigo_shared::clipboard::native_tool_name().to_owned(),
            clipboard_data_control: crate::clipboard::wayland_data_control_label().to_owned(),
        }
    }

    /// Extract terminal info for feedback submissions.
    pub fn feedback_info(&self) -> fuigo_shared::session::FeedbackTerminalInfo {
        use fuigo_shared::session::FeedbackTerminalInfo;
        ...
        FeedbackTerminalInfo {
            ...
            clipboard_native_tool: Some(fuigo_shared::clipboard::native_tool_name().to_owned()),
...
fn detect_terminal_context() -> TerminalContext {
    ...
    // Revisit when `fuigo ssh` can forward env vars
    let mut ctx = build_terminal_context_from_env(&env);
...
/// That list lives in `fuigo-pager-pty-harness/src/pty.rs`.
...
fn terminal_name_from_term_program(value: &str) -> Option<TerminalName> {
    ...
    match normalized.as_str() {
        ...
        "fuigodesktop" => Some(TerminalName::FuigoDesktop),
...
///
/// Parsed from `[terminal] alt_screen` in `~/.fuigo/pager.toml` and
