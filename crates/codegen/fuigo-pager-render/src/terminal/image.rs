...
/// The scrollback-native minimal mode (`fuigo --minimal`) sets this once at startup because it never runs the interactive draw loop.
...
pub fn kitty_format_from_bytes(image_data: &[u8]) -> Option<KittyImageFormat> {
    match fuigo_shared::clipboard::mime_from_bytes(image_data) {
...
fn convert_via_sips(image_data: &[u8]) -> Option<Vec<u8>> {
    ...
    let ts = std::time::SystemTime::now()
        ...
        .unwrap_or_default()
        .as_nanos();
    let src = tmp_dir.join(format!("fuigo-sips-{id}-{ts}.dat"));
    let dst = tmp_dir.join(format!("fuigo-sips-{id}-{ts}.png"));
    ...
    fuigo_tty_utils::detach_std_command(&mut sips_cmd);
