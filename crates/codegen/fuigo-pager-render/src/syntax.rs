...
//! Dark themes (FuigoNight, TokyoNight) share `fuigo-night.tmTheme`; FuigoDay uses `fuigo-day.tmTheme` with deepened colors for light backgrounds.
//!
...
pub use fuigo_markdown::Syntect;
...
static SYNTECT_FUIGONIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_TOKYONIGHT: OnceLock<Syntect> = OnceLock::new();
static SYNTECT_FUIGODAY: OnceLock<Syntect> = OnceLock::new();
...
///
/// While the terminal-native lock is engaged, [`Theme::current_kind`] reports a nominal `FuigoNight`, so this returns the night theme.
...
pub fn get_syntect() -> &'static Syntect {
    match crate::theme::Theme::current_kind() {
        ...
        ThemeKind::FuigoDay => SYNTECT_FUIGODAY
            .get_or_init(|| Syntect::new(include_bytes!("../assets/fuigo-day.tmTheme"))),
    }
}
