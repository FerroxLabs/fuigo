...
pub enum OpenUrlResult {
    /// Opener was launched (or `FUIGO_TEST_OPEN_URL_FILE` recorded the URL).
    Opened,
...
///
/// Returns `true` when the opener was launched (or `FUIGO_TEST_OPEN_URL_FILE` recorded the URL).
...
pub fn open_url(url: &str) -> bool {
    ...
    if let Ok(path) = std::env::var("FUIGO_TEST_OPEN_URL_FILE") {
        use std::io::Write;
        ...
        {
            tracing::warn!(error = %e, path, "FUIGO_TEST_OPEN_URL_FILE write failed");
            return false;
        }
        return true;
    }
...
fn spawn_url_opener(url: &str) -> bool {
    ...
    fuigo_tools::util::detach_std_command(&mut command);
    match command.spawn() {
        Ok(_) => true,
        Err(e) => {
...
///
/// [`fuigo_tty_utils::detach_std_command`] (`setsid`/`setpgid`) keeps the spawned GUI helper and its children from grabbing the TUI's `/dev/tty`.
...
mod tests {
    ...
    fn open_path_command_passes_path_as_a_single_arg() {
        ...
        let path = std::path::Path::new("/tmp/fuigo session/image 1.jpg");
