...
pub use fuigo_config::fuigo_home;
pub use fuigo_tools::util::format_bytes;

/// A closed stdout (`fuigo du | head`) is a clean stop, not a failure.
...
/// Path to `$FUIGO_HOME/pager.toml`.
pub fn pager_toml_path() -> PathBuf {
    fuigo_home().join("pager.toml")
}

/// `~/.fuigo` or `$FUIGO_HOME`, decided by the resolved home rather than by
/// whether `FUIGO_HOME` is set in the environment.
pub fn display_fuigo_home_prefix() -> String {
    display_fuigo_home_prefix_for(&fuigo_home())
}

pub fn display_fuigo_home_prefix_for(home: &Path) -> String {
    let default = fuigo_config::default_fuigo_home();
    if home == default || home == dunce::canonicalize(&default).unwrap_or(default) {
        "~/.fuigo".to_string()
    } else {
        "$FUIGO_HOME".to_string()
    }
}

/// User-facing path under [`fuigo_home()`], e.g. ``~/.fuigo/config.toml``.
pub fn display_user_fuigo_path(relative: impl AsRef<Path>) -> String {
    display_user_fuigo_path_for(&fuigo_home(), relative)
}

fn display_user_fuigo_path_for(home: &Path, relative: impl AsRef<Path>) -> String {
    let rel = relative.as_ref();
    let prefix = display_fuigo_home_prefix_for(home);
...
/// True when `path` is under user [`fuigo_home()`] (not project `{cwd}/.fuigo`).
pub fn is_under_user_fuigo_home(path: &Path) -> bool {
    path.starts_with(fuigo_home())
}
...
mod tests {
    ...
    #[test]
    fn display_fuigo_home_prefix_default_install() {
        if std::env::var("FUIGO_HOME").is_ok() {
            return;
        }
        assert_eq!(display_fuigo_home_prefix(), "~/.fuigo");
    }

    #[test]
    fn display_user_fuigo_path_joins_relative() {
        let path = display_user_fuigo_path(fuigo_config::USER_CONFIG_FILENAME);
        assert!(path.ends_with("/config.toml") || path.ends_with("\\config.toml"));
        assert!(path.contains(".fuigo") || path.contains("$FUIGO_HOME"));
    }

    #[test]
    fn display_user_fuigo_path_for_custom_home_uses_override_label() {
        let custom = std::env::temp_dir().join("fuigo-home-display-regression");
        assert_eq!(
            display_user_fuigo_path_for(&custom, fuigo_config::USER_CONFIG_FILENAME),
            "$FUIGO_HOME/config.toml"
        );
        assert_eq!(
            display_user_fuigo_path_for(&custom, fuigo_config::SANDBOX_CONFIG_FILENAME),
            format!("$FUIGO_HOME/{}", fuigo_config::SANDBOX_CONFIG_FILENAME)
        );
    }

    #[test]
    fn abbreviate_path_uses_home_when_under_default_fuigo() {
        ...
        let full = format!("{home}/.fuigo/memory/MEMORY.md");
    ...
    fn abbreviate_path_collapses_home_with_native_separator() {
        ...
        // Stay outside fuigo_home so this hits the $HOME branch, not ~/.fuigo.
        let full = home.join("not-fuigo-home").join("file.txt");
        let full_str = full.to_string_lossy();
        let abbreviated = abbreviate_path(&full_str);
        let expected = format!(
            "~/{}",
            Path::new("not-fuigo-home").join("file.txt").display()
        );
