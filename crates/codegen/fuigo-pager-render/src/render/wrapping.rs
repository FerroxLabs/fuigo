...
///
/// The selection layer's stricter, style-aware twin lives in fuigo-pager scrollback/blocks/quote_bar.rs (`rendered_quote_prefix_len`).
...
mod tests {
    ...
    fn real_markdown_link_underline_does_not_leak_after_wrap() {
        ...
        let (out, _) = fuigo_markdown::render_markdown_ratatui_full(md, md_style, true, None);
