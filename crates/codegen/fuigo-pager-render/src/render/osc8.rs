...
//! The frame diff (`fuigo_ratatui_inline::Terminal::flush_with_links`) emits them as OSC 8 hyperlinks.
...
/// One path segment without spaces (`main.rs`, `.fuigo`, `@scope`). Leading `.`
/// matches dot-directories and `%` matches percent-encoded segments; fuigo
/// session media lives under `~/.fuigo/sessions/%2F…/images/1.jpg`.
...
/// Relative file path (`images/1.png`, `.fuigo/x.txt`): one or more `/`-joined directory segments plus a filename that has an extension.
...
fn relative_link_target(
    ...
    let cwd = cwd?;
    let joined = fuigo_paths::normalize_lexically(&cwd.join(path));
    joined
        .starts_with(fuigo_paths::normalize_lexically(cwd))
        .then_some(joined)
}
...
/// media lives at `~/.fuigo/sessions/%2F…/images/1.jpg`, which wraps in
...
mod tests {
    ...
    #[test]
    fn scan_detects_fuigo_session_media_path() {
        // The shape of `image_gen` output prose: a dot-directory (`.fuigo`), a percent-encoded session segment, and a trailing sentence period
        let line = make_line("Saved to /Users/alice/.fuigo/sessions/%2Fabc/00000000/images/1.jpg.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            ...
            "file:///Users/alice/.fuigo/sessions/%252Fabc/00000000/images/1.jpg",
        );
    }
    ...
    fn scan_detects_media_path_soft_wrapped_across_rows() {
        ...
        let row0 =
            make_line("Image generated and saved to /Users/alice/.fuigo/sessions/%2FUsers%2Fali");
        let row1 = make_line("ce%2Fcode%2Ffuigo/00000000-0000-0000-0000-000000000001/images/1.jpg");
        let rows: Vec<(u16, &Line<'static>, Option<&str>)> =
        ...
        let expected_url = "file:///Users/alice/.fuigo/sessions/%252FUsers%252Fali\
                            ce%252Fcode%252Ffuigo/00000000-0000-0000-0000-000000000001/images/1.jpg";
        ...
        assert_eq!(
            ...
            2 + UnicodeWidthStr::width(
                "Image generated and saved to /Users/alice/.fuigo/sessions/%2FUsers%2Fali"
            ) as u16
        );
        ...
        assert_eq!(
            l1.col_end,
            2 + UnicodeWidthStr::width(
                "ce%2Fcode%2Ffuigo/00000000-0000-0000-0000-000000000001/images/1.jpg"
            ) as u16
        );
    }
    ...
    fn scan_wrapped_path_trailing_sentence_period_excluded() {
        ...
        let row0 = make_line("Saved to /Users/me/.fuigo/sessions/%2Fabc/019f3a86/ima");
        ...
        for link in overlay.links() {
            assert_eq!(
                &*resolve_link_target(&link.target)
                    .and_then(|resolved| resolved.osc8_url)
                    .expect("url"),
                "file:///Users/me/.fuigo/sessions/%252Fabc/019f3a86/images/1.jpg"
            );
        }
    ...
    fn scan_file_path_with_dots_and_hyphens() {
        let line = make_line("Reading /tmp/fuigo-impl-summary.md now.");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
        ...
        assert_eq!(
            &*resolve_link_target(&overlay.links()[0].target)
                .and_then(|resolved| resolved.osc8_url)
                .expect("url"),
            "file:///tmp/fuigo-impl-summary.md"
        );
    }
    ...
    fn scan_detects_tilde_file_path() {
        ...
        let raw = "~/Desktop/fuigo-pager-retention-findings-2026-06-06.md";
    ...
    fn scan_relative_path_not_partially_linkified() {
        // A relative path like `crates/codegen/fuigo-pager/src/render` should NOT produce a link for the `/fuigo-pager/src/render` substring
        let line = make_line("find crates/codegen/fuigo-pager/src/render -name '*.rs'");
        let mut overlay = LinkOverlay::new();
        scan_unjoined(std::iter::once((0, &line)), 0, &[], &mut overlay);
