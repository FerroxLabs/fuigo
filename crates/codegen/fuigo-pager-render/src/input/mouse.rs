...
//! A user can force wheel or trackpad behavior when the heuristic is wrong: `scroll_mode` (or `FUIGO_SCROLL_MODE`) pins [`ScrollInputMode`].
...
// The harness has no pager dependency, so six constants below are duplicated in `fuigo-pager-pty-harness/src/scroll_matrix/gestures.rs`:
...
// Runtime may override via `set_redraw_cadence` (`FUIGO_SCROLL_CADENCE_MS` from the event loop); the harness still models the 16ms default
...
pub struct ScrollDebugSnapshot {
    ...
    pub flush_cap: i32,
    /// Effective scroll flush cadence in ms (`FUIGO_SCROLL_CADENCE_MS` / default 16).
    pub cadence_ms: u64,
}
...
/// - **Coalescing**: trackpad-like streams are flushed at most every redraw cadence (default 16ms / ~60Hz; `FUIGO_SCROLL_CADENCE_MS`) to avoid floods.
...
pub struct MouseScrollState {
    ...
    /// `FUIGO_SCROLL_LOG` flight recorder ([`crate::input::scroll_log`]).
...
impl MouseScrollState {
    ...
    /// Inject flush cadence (`FUIGO_SCROLL_CADENCE_MS`); Default stays 16ms.
    ...
    /// Emit a flight-recorder line; a single branch when `FUIGO_SCROLL_LOG` is off.
