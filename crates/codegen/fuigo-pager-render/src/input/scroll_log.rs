//! `FUIGO_SCROLL_LOG` enables a JSONL log of scroll-stream transitions for offline analysis of real gestures.
//!
...
//!
//! Enablement: `FUIGO_SCROLL_LOG=1` (or set-but-empty) logs to
//! `~/.fuigo/logs/scroll-log-<timestamp>.jsonl`; any other non-`0` value is
//! used as the target path.
//! Unset (or `0`, matching `FUIGO_SCROLL_DEBUG`) disables: [`super::mouse::MouseScrollState`] holds `None` and each emission point costs one branch.
//!
...
/// Appends [`ScrollLogRecord`]s to the `FUIGO_SCROLL_LOG` file.
...
impl ScrollLogRecorder {
    /// Build from `FUIGO_SCROLL_LOG` (the module docs describe the accepted values); `None` when unset or `0`.
    pub(crate) fn from_env_at(base: Instant) -> Option<Self> {
        let raw = std::env::var("FUIGO_SCROLL_LOG").ok()?;
        let value = raw.trim();
        if value == "0" {
            return None;
        }
...
/// `~/.fuigo/logs/scroll-log-<utc-ts>.jsonl` — the input-debug dump's dir
