...
fn scroll_log_records_flood_flushes_and_capped_finalize_drop() {
    // FUIGO_SCROLL_LOG: a trackpad flood on the synthetic clock must produce parseable JSONL ordered stream_start, then flushes, then finalize
...
/// Producer-side twin of the harness's `scroll_matrix::log::ScrollLogLine` parser (`fuigo-pager-pty-harness/src/scroll_matrix/log.rs`).
