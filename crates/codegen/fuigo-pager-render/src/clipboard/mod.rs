//! Re-exports [`ClipboardProvider`] and [`InternalClipboard`] from `fuigo-ratatui-textarea`, and adds [`SystemClipboard`] backed by `arboard`.
//!
...
pub use fuigo_ratatui_textarea::{ClipboardProvider, InternalClipboard};
...
/// Documented in `fuigo-pager/docs/internal/22-environment-variables.md`.
pub const FUIGO_COPY_FILE_ENV: &str = "FUIGO_COPY_FILE";

/// Cached result of the remote-session check (env vars don't change at runtime).
fn is_remote() -> bool {
    static REMOTE: OnceLock<bool> = OnceLock::new();
    *REMOTE.get_or_init(fuigo_shared::clipboard::is_remote_session)
}
...
fn is_container_no_display() -> bool {
    ...
    *CONTAINER.get_or_init(fuigo_shared::clipboard::is_containerized_without_display)
}

/// Cached result of the "an upstream OSC 52 sink is capturing our output" check.
///
/// `fuigo wrap` runs a command inside a local PTY and scans its output for OSC 52 clipboard sequences (see `fuigo-pager`'s `pty_wrap` module).
...
/// The inner `fuigo` then knows its OSC 52 writes are reliably intercepted and copied, even when the inner terminal brand is misdetected.
/// Over SSH only `TERM` propagates, so Apple Terminal and unknown brands look OSC-52-incapable.
///
/// Two names are accepted: the canonical `FUIGO_OSC52_SINK` (inherited by local children) and the `LC_`-prefixed `LC_FUIGO_OSC52_SINK`.
/// Default OpenSSH configs forward it (`SendEnv LANG LC_*` / `AcceptEnv LANG LC_*`), so the signal survives the hop into a remote `fuigo`.
pub fn osc52_sink_active() -> bool {
    static SINK: OnceLock<bool> = OnceLock::new();
    *SINK.get_or_init(|| {
        std::env::var_os("FUIGO_OSC52_SINK").is_some()
            || std::env::var_os("LC_FUIGO_OSC52_SINK").is_some()
    })
}

/// Kill switch: never emit OSC 52 clipboard sequences.
///
/// Set `FUIGO_CLIPBOARD_NO_OSC52` (any value) before starting Fuigo.
/// Presence forces the OSC 52 leg off for the whole process, including Linux "always emit", tmux, SSH, container, and `FUIGO_OSC52_SINK` paths.
...
pub fn osc52_disabled() -> bool {
    ...
    *DISABLED.get_or_init(|| std::env::var_os("FUIGO_CLIPBOARD_NO_OSC52").is_some())
}
...
pub fn wayland_data_control_label() -> &'static str {
    match crate::host::DisplayServer::current() {
        crate::host::DisplayServer::Wayland => {
            if fuigo_shared::clipboard::wayland_data_control_supported() {
                "yes"
            } else {
                "no"
            }
...
/// `FUIGO_CLIPBOARD_NO_OSC52` is the exception: it forces OSC 52 off everywhere.
...
fn resolve_clipboard_route_with(ctx: &TerminalContext, opts: ClipboardRouteOpts) -> ClipboardRoute {
    ...
    // macOS/Windows: emit only in tmux/SSH/container contexts, or when an upstream `fuigo wrap` sink is capturing our output
    // The sink forwards captured OSC 52 to the real clipboard
    // `FUIGO_CLIPBOARD_NO_OSC52` wins over every automatic path.
...
fn write_tmux_buffer(text: &str) -> bool {
    ...
    let result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let stdin = fuigo_shared::clipboard::spool_for_stdin(text.as_bytes())?;
        ...
        let status = fuigo_shared::clipboard::wait_with_deadline(
            &mut child,
...
/// Delegates to [`fuigo_shared::clipboard`], which uses `pbcopy`/`pbpaste` on macOS (avoiding AppKit GPU overhead) and `arboard` elsewhere.
///
...
impl ClipboardProvider for SystemClipboard {
    fn get(&mut self) -> Option<String> {
        fuigo_shared::clipboard::get_text().ok().flatten()
    }
...
fn clipboard_write_with_route(text: &str, route: &ClipboardRoute) -> ClipboardWriteLegs {
    ...
    if route.native {
        let outcome = fuigo_shared::clipboard::set_text_with_outcome(text);
    ...
    if route.osc52 {
        match fuigo_shared::clipboard::set_text_osc52(text, route.osc52_tmux_passthrough) {
...
impl ClipboardFeedback {
    ...
    fn message(self) -> &'static str {
        match self {
            ...
            Self::UnverifiedOscRemote | Self::UnverifiedOscContainer => {
                "Copy sent. If paste fails, use fuigo wrap or /minimal."
            }
...
/// Write text and return a toast; emits `fuigo-shell-clipboard_copy` when enabled.
...
///
/// Override with [`FUIGO_COPY_FILE_ENV`] (supports `~`). Otherwise
/// `~/.fuigo/last-copy.txt` (fuigo's per-user home — short, stable, and
/// readable in a toast, unlike macOS's `/var/folders/...` temp dir).
///
/// `None` when no fuigo home resolves and the env var is unset.
...
pub fn default_copy_fallback_path() -> Option<std::path::PathBuf> {
    if let Ok(raw) = std::env::var(FUIGO_COPY_FILE_ENV) {
    ...
    fuigo_config::user_fuigo_home().map(|fuigo_home| fuigo_home.join("last-copy.txt"))
}

/// Render a backup-file path for user-facing messages using the codebase-wide
/// abbreviation convention ([`crate::util::abbreviate_path`]): a fuigo-home
/// prefix collapses to `~/.fuigo` (or `$FUIGO_HOME` when overridden), and a
...
///
/// A pre-existing file (e.g. a `last-copy.txt` created `0644` by an older fuigo) is tightened via `set_permissions`.
...
///
/// Errors with `NotFound` when no fallback path resolves (no home and no `FUIGO_COPY_FILE`).
...
/// `FUIGO_COPY_FILE` may point at a not-yet-created private directory;
/// `~/.fuigo` normally already exists).
pub fn write_copy_fallback(text: &str) -> std::io::Result<std::path::PathBuf> {
    let Some(path) = default_copy_fallback_path() else {
        return Err(std::io::Error::new(
            ...
            "no home directory resolves; set FUIGO_COPY_FILE to enable the copy backup file",
        ));
    };
    #[cfg(unix)]
...
/// clipboard over SSH (notably Apple Terminal without `fuigo wrap`); a failed
...
fn log_clipboard_copy_event(
    ...
    if !fuigo_telemetry::client::is_enabled() {
        return;
    }
    fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::ClipboardCopy {
...
pub fn system_clipboard_read_text() -> Result<Option<String>, ClipboardTextReadError> {
    ...
    fuigo_shared::clipboard::get_text().map_err(|error| {
...
pub fn system_primary_selection_get() -> Option<String> {
    ...
    if !fuigo_shared::clipboard::x11_display_env_present() {
        return None;
    }
    fuigo_shared::clipboard::get_primary_text()
        .ok()
        .flatten()
        .filter(|text| !text.is_empty())
}
...
pub fn x11_primary_guidance_available() -> bool {
    #[cfg(target_os = "linux")]
    {
        is_native_x11(crate::host::DisplayServer::current())
            && fuigo_shared::clipboard::x11_display_env_present()
    }
...
pub fn log_paste_key_empty_host_clipboard(surface: &str) {
    ...
    fuigo_telemetry::session_ctx::log_event(
        fuigo_telemetry::events::PasteKeyEmptyHostClipboard {
            terminal,
...
fn log_clipboard_paste_event(
    ...
    fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::ClipboardImagePaste {
        terminal: crate::terminal::terminal_context().telemetry_snapshot(),
...
fn system_clipboard_get_attachments() -> Result<AttachmentsProbeResult, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match fuigo_shared::clipboard::get_attachments() {
        Ok(att) => {
...
pub use fuigo_shared::clipboard::ImageData;
...
pub fn clipboard_image_snapshot() -> (Option<u64>, bool) {
    ...
    fuigo_shared::clipboard::clipboard_image_snapshot()
}
...
pub fn clipboard_change_count() -> Option<u64> {
    ...
    fuigo_shared::clipboard::clipboard_change_count()
}
...
pub fn clipboard_image_probe_supported() -> bool {
    ...
    fuigo_shared::clipboard::clipboard_image_probe_supported()
}
...
pub fn prewarm_image_probe() {
    ...
    WARMED.call_once(|| {
        std::thread::spawn(fuigo_shared::clipboard::clipboard_prewarm);
    });
}
...
fn system_clipboard_get_image_result() -> Result<Option<ImageData>, ClipboardProbeError> {
    let started = std::time::Instant::now();
    match fuigo_shared::clipboard::get_image() {
        Ok(img) => {
...
mod tests {
    ...
    fn clipboard_route_matrix() {
        ...
        for case in cases {
            // Pure helper with kill switch off so ambient FUIGO_CLIPBOARD_NO_OSC52 cannot flake CI (route() itself still reads the real env)
    ...
    fn clipboard_route_osc52_when_wrap_sink_active() {
        ...
        assert!(
            on.osc52,
            "fuigo wrap sink must emit OSC 52 so the local PTY can intercept it"
        );
        ...
        assert!(
            !killed.osc52,
            "FUIGO_CLIPBOARD_NO_OSC52 still wins over the wrap sink"
        );
    }
    ...
    fn clipboard_route_no_osc52_kill_switch_forces_off() {
        // FUIGO_CLIPBOARD_NO_OSC52 must win over Linux/tmux/SSH automatic emit.
        for ctx in [
            plain_terminal_ctx(),
            plain_tmux_ctx(),
            byobu_tmux_ctx(),
    ...
    /// That includes a pre-existing `0644` file left by an older fuigo.
    #[cfg(unix)]
    #[test]
    ...
    #[test]
    #[serial_test::serial(fuigo_copy_file)]
    fn default_copy_fallback_path_respects_fuigo_copy_file() {
        let dir = tempfile::tempdir().expect("tempdir");
        let custom = dir.path().join("custom-copy.txt");
        // SAFETY: test-only env mutation; serialized on the fuigo_copy_file key.
        unsafe {
            std::env::set_var(FUIGO_COPY_FILE_ENV, &custom);
        }
        let resolved = default_copy_fallback_path();
        unsafe {
            std::env::remove_var(FUIGO_COPY_FILE_ENV);
        }
    ...
    /// Without `FUIGO_COPY_FILE`, the default is `~/.fuigo/last-copy.txt`
    /// (fuigo home) — short and toast-friendly, unlike macOS's temp dir.
    #[test]
    #[serial_test::serial(fuigo_copy_file)]
    fn default_copy_fallback_path_is_fuigo_home() {
        unsafe {
            std::env::remove_var(FUIGO_COPY_FILE_ENV);
        }
        let path = default_copy_fallback_path();
        // Test envs always resolve a home (or set FUIGO_HOME).
        let expected = fuigo_config::user_fuigo_home()
    ...
    /// Fuigo-home paths go through the shared `abbreviate_path` convention.
    /// The `FUIGO_HOME`-override integration test in `fuigo-pager` covers that further.
    #[test]
    fn display_copy_path_abbreviates_home() {
        if std::env::var_os("FUIGO_HOME").is_none() {
            let home = fuigo_dirs::home_dir().expect("home resolves in tests");
            assert_eq!(
                display_copy_path(&home.join(".fuigo").join("last-copy.txt")),
                "~/.fuigo/last-copy.txt"
            );
        }
