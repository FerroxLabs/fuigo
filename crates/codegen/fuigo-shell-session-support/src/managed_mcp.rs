//! Managed MCP gateway catalog and tool calls via the Fuigo API.
//!
...
async fn get_authenticated_json<T: serde::de::DeserializeOwned>(
    ...
    let resp = match fuigo_http::shared_client()
        .get(url)
...
/// Fetch the managed MCP gateway tool catalog from the Fuigo API (`GET /v1/mcp/tools/list`).
///
