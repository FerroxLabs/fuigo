...
/// Client-facing tool names for a scheduled-task wakeup footer. Each half is
/// omitted when its tool name (or child id) is not resolvable.
...
impl<'a> ScheduledWakeupTools<'a> {
    fn canonical(poll_id: Option<&'a str>) -> Self {
        Self {
            child: child_poll(Some(task_completion::DEFAULT_TASK_OUTPUT_TOOL), poll_id),
            schedule: schedule_tool_names(
                Some(task_completion::SCHEDULER_DELETE_REGISTRY_ID),
                Some(fuigo_tools_api::slash_commands::SCHEDULER_CREATE_TOOL_NAME),
            ),
        }
    }
}
...
///
/// `poll_id` is the child subagent whose output the model should check; an
/// in-conversation fire has none, so its footer carries only the schedule
/// tools.
...
pub fn format_loop_iteration_prompt(
    ...
    format_loop_iteration_prompt_with_tools(
        ...
        ScheduledWakeupTools::canonical(Some(poll_id)),
    )
}
...
mod tests {
    ...
    fn format_scheduled_task_prompt_includes_framing() {
        let out = format_scheduled_task_prompt("do stuff", "task-1", Some("child-1"), "every 5m");
        ...
        let no_child = format_scheduled_task_prompt("do stuff", "task-1", None, "every 5m");
        assert!(!no_child.contains("Check the subagent output"));
        assert!(no_child.contains(
            "If this schedule is no longer relevant, run scheduler_delete(\"task-1\"). If it is outdated, you can update it with scheduler_create(new_prompt, interval, \"task-1\")."
        ));
        assert!(no_child.ends_with("do stuff"));
    }
