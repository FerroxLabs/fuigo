...
crate::register_resource!("fuigo_build", "TaskWakeSuppressed", TaskWakeSuppressed);
...
/// events from the structured `fuigo/monitor_event` notification, never by
/// parsing this text.
///
...
pub fn format_monitor_events(
    events: &[crate::implementations::fuigo_build::monitor::types::MonitorEventNotification],
    task_output_name: Option<&str>,
) -> Option<String> {
    use std::fmt::Write as _;
    ...
    match events {
        ...
        [event] => {
            ...
            let label =
                crate::implementations::fuigo_build::monitor::event::sanitize_monitor_description(
                    label,
                );
            Some(format!(
        ...
        _ => {
            type Event =
                crate::implementations::fuigo_build::monitor::types::MonitorEventNotification;
            ...
            for (task_id, group) in &groups {
                let description = group
                    ...
                    .filter(|d| !d.is_empty())
                    .unwrap_or("event");
                let description = crate::implementations::fuigo_build::monitor::event::sanitize_monitor_description(
                    description,
                );
                let _ = write!(
                    buf,
...
/// scope only when it has no recorded owner (legacy / non-fuigo-build backends)
...
/// - `fuigo-shell`'s `SessionActor` to sweep matching synthetic
    ...
    async fn collect_reminders(
        ...
        if let Some(terminal) = terminal {
            ...
            let goal_loop_active = res
                .get::<crate::implementations::fuigo_build::task::types::GoalLoopActive>()
                .is_some_and(|g| g.0);
            let surface_reminders = !goal_loop_active
                && res
                    .get::<crate::types::resources::Params<
                        crate::implementations::fuigo_build::bash::BashParams,
                    >>()
...
mod tests {
    ...
    use fuigo_tool_types::{MultiTaskOutputResult, TaskOutputResult};
    struct MockTerminal {
    ...
    fn shared_with_reminders_disabled(tasks: Vec<TaskSnapshot>) -> SharedResources {
        ...
        res.insert(Terminal(backend));
        res.register_state::<ReportedTaskCompletions>();
        let params = crate::implementations::fuigo_build::bash::BashParams {
    ...
    async fn completions_suppressed_when_goal_loop_active() {
        ...
        res.insert(Terminal(backend));
        res.register_state::<ReportedTaskCompletions>();
        res.insert(crate::implementations::fuigo_build::task::types::GoalLoopActive(true));
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel();
        res.insert(SubagentEventSender(tx));
        tokio::spawn(async move {
        ...
        shared
            .lock()
            .await
            .insert(crate::implementations::fuigo_build::task::types::GoalLoopActive(false));
        let second = reminder.collect_reminders(shared, &output).await;
        assert!(
            second.is_empty(),
    ...
    fn format_subagent_completion_caps_inline_output_next_to_poll_tool() {
        ...
        let digest = format_between_turn_completions(&[c], Some("get_task_output"), None, None);
        assert!(
    ...
    fn format_subagent_completion_inlines_output_when_no_poll_tool() {
        ...
        let wake = format_subagent_completion(&big, None, None, None);
        let digest = format_between_turn_completions(std::slice::from_ref(&big), None, None, None);
    ...
    fn between_turn_batch_shares_one_inline_budget() {
        ...
        let completions: Vec<SubagentCompletionSummary> = (0..BATCH)
            ...
            .collect();
        let msg = format_between_turn_completions(&completions, Some("get_task_output"), None, None);
    ...
    fn between_turn_batch_budget_leaves_a_fitting_batch_byte_identical() {
        let small: Vec<SubagentCompletionSummary> = (0..8)
            ...
            .collect();
        let msg = format_between_turn_completions(&small, Some("get_task_output"), None, None);
        assert!(!msg.contains("[output truncated:"));
        for c in &small {
    ...
    fn between_turn_batch_budget_never_cuts_when_there_is_no_poll_tool() {
        let completions: Vec<SubagentCompletionSummary> = (0..4)
            ...
            .collect();
        let msg = format_between_turn_completions(&completions, None, None, None);
        assert!(!msg.contains("[output truncated:"));
        for c in &completions {
            assert!(
    ...
    async fn reminder_pipeline_ignores_monitor_event_buffer() {
        use crate::implementations::fuigo_build::monitor::types::{
    ...
    fn format_monitor_events_single_vs_batched() {
        use crate::implementations::fuigo_build::monitor::types::MonitorEventNotification;
        ...
        let bare = crate::implementations::fuigo_build::monitor::types::MonitorEventNotification {
            task_id: "task-9".into(),
    ...
    fn wrap_monitor_event_round_trips_through_split() {
        use crate::implementations::fuigo_build::monitor::event::wrap_monitor_event;
