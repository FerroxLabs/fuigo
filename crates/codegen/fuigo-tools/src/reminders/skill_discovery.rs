...
//! after every tool call to check for SKILL.md files in `.fuigo/skills/`,
...
/// Directories that contain skill definitions (`.fuigo/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".fuigo", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.fuigo/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.fuigo/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.fuigo/) are outside the git root so the walk breaks early.
