...
/// writer from filling the disk. Env override: `FUIGO_MAX_OUTPUT_FILE_BYTES`.
const MAX_OUTPUT_FILE_BYTES: u64 = 5 * 1024 * 1024 * 1024;

fn output_file_cap_from_env() -> u64 {
    std::env::var("FUIGO_MAX_OUTPUT_FILE_BYTES")
        .ok()
        .and_then(|s| s.parse::<u64>().ok())
...
impl LocalTerminalActor {
    ...
    async fn spawn_static_command(
        ...
        unsafe {
            cmd.pre_exec(fuigo_tty_utils::detach_pre_exec_hook());
        }
                ...
                && !process.lifecycle.has_exited()
            {
                ...
                if process.kind == crate::computer::types::TaskKind::Monitor {
                    ...
                    let start_offset = std::fs::metadata(&pipeline_output_file)
                        ...
                        .unwrap_or(0);
                    tokio::spawn(async move {
                        crate::implementations::fuigo_build::monitor::tool::run_monitor_pipeline(
                            &pipeline_task_id,
...
#[cfg(unix)]
const ENV_LOGIN_ENV: &str = "FUIGO_LOGIN_ENV";
...
/// active policy filters out and those already set in fuigo's own environment.
#[cfg(unix)]
fn layer_login_env_vars(
    ...
    if let Some(login) = login_env {
        for (key, value) in login {
            // `var_os` reads fuigo's own env (not the possibly-cleared child env);
...
/// Fixed layer order: policy base, login capture (filtered), fuigo control vars,
/// request env (filtered), pager vars, login `PATH`, agent marker last. Applied
/// incrementally, not via `env_clear`: the no-op-policy path must inherit fuigo's
...
fn apply_child_env(
    ...
    crate::util::apply_fuigo_agent_marker(cmd);
}
...
fn spawn_shell_command(
    ...
    let mut build_cmd = |with_breakaway: bool| {
        ...
        let inv = fuigo_config::shell::shell_command_argv(command);
        ...
        cmd.envs(crate::util::pager_env());
        crate::util::apply_fuigo_agent_marker(&mut cmd);
...
mod tests {
    ...
    fn apply_child_env_layers_in_fixed_order() {
        ...
        let policy = ShellEnvironmentPolicy {
            ...
            set: HashMap::from([("FUIGO_TEST_BASE".to_string(), "1".to_string())]),
            ..Default::default()
        };
        let login = HashMap::from([
            ("FUIGO_TEST_LOGIN".to_string(), "l".to_string()),
            ("PATH".to_string(), "/login/bin".to_string()),
        ]);
        let request = HashMap::from([
            ("FUIGO_TEST_REQ".to_string(), "r".to_string()),
            ("PATH".to_string(), "/req/bin".to_string()),
            ("FUIGO_TEST_SECRET".to_string(), "s".to_string()),
        ]);
        ...
        assert_eq!(env.get("FUIGO_TEST_BASE").map(String::as_str), Some("1"));
        assert_eq!(env.get("FUIGO_TEST_LOGIN").map(String::as_str), Some("l"));
        assert_eq!(env.get("FUIGO_TEST_REQ").map(String::as_str), Some("r"));
        assert!(!env.contains_key("FUIGO_TEST_SECRET"));
    ...
    async fn test_persistent_shell_env_var_persists() {
        ...
        let result = backend
            .run(make_request("export FUIGO_PERSIST_TEST=hello123"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));

        let result = backend
            .run(make_request("echo $FUIGO_PERSIST_TEST"))
            .await
            .unwrap();
        assert_eq!(result.exit_code, Some(0));
        assert_eq!(
    ...
    async fn test_persistent_shell_clears_gpg_tty() {
        ...
        req.env
            .insert("GPG_TTY".to_string(), "/fuigo-sentinel-tty".to_string());
