...
pub use fuigo_config::shell::UnixShellKind;

const INIT_MARKER: &str = "__FUIGO_STATIC_SHELL_MARKER__";
...
fn shell_binary(shell: UnixShellKind) -> &'static str {
    fuigo_config::shell::unix_shell_path(shell)
}
...
mod tests {
    ...
    async fn replays_aliases_and_functions() {
        ...
        let output = run_static(
            "alias fuigo_alias_probe='echo ALIAS_OK'\nfuigo_fn_probe() { echo FN_OK; }\n",
            "fuigo_alias_probe && fuigo_fn_probe",
        )
        .await;
