...
//! config helper `fuigo-shell::util::config::resolve_search_tools_enabled`
//! (requirements > env `FUIGO_TOOLS_FIND_BFS` / `FUIGO_TOOLS_GREP_UGREP` (+
//! `FUIGO_FIND_BFS` / `FUIGO_GREP_UGREP` aliases, `DISABLE_EMBEDDED_SEARCH_TOOLS`
...
//! (release builds, self-extracted to `~/.fuigo/vendor/<name>-<ver>-<target>`) →
//! `~/.fuigo/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
//! function (which tags itself with a `__fuigo_shadow_{name}` marker) or a
...
const BFS_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-bfs/bfs-",
    env!("FUIGO_TOOLS_BFS_VER"),
    "-",
    env!("FUIGO_TOOLS_BFS_TARGET"),
    ".bin.zst"
));
...
const UGREP_BYTES: &[u8] = include_bytes!(concat!(
    env!("OUT_DIR"),
    "/bundle-ugrep/ugrep-",
    env!("FUIGO_TOOLS_UGREP_VER"),
    "-",
    env!("FUIGO_TOOLS_UGREP_TARGET"),
    ".bin.zst"
));
...
/// OS binary again. Gated on the `__fuigo_shadow_{name}` marker that
...
fn restore_command(name: &str) -> String {
    format!(
        "if [ -n \"${{__fuigo_shadow_{name}-}}\" ]; then \
           unalias {name} 2>/dev/null || true; \
           unset -f {name} 2>/dev/null || true; \
           unset __fuigo_shadow_{name} 2>/dev/null || true; \
         fi"
    )
}
...
fn resolved_tools() -> &'static ResolvedTools {
    ...
    TOOLS.get_or_init(|| ResolvedTools {
        bfs: resolve_tool("bfs", "FUIGO_TOOLS_BFS_PATH", bundled_bfs()),
        ugrep: resolve_tool("ugrep", "FUIGO_TOOLS_UGREP_PATH", bundled_ugrep()),
    })
}
...
fn resolve_tool(
    ...
    resolve_tool_from(
        ...
        bundled,
        crate::util::fuigo_home().join("vendor").join(bin_name),
        bin_name,
    )
}

/// Resolution order: explicit env path → bundled (self-extracted) →
/// `~/.fuigo/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
/// `__fuigo_shadow_{name}=1` marks this as a harness shadow so `restore_command`
...
fn shell_function(
    ...
    // `local __fuigo_bin` is re-resolved every call. The host hint is trusted
    ...
    // binary. `|| __fuigo_bin=''` keeps the lookup `set -e`-safe (a failed
    ...
    format!(
        "unalias {name} 2>/dev/null || true; \
         {name}() {{ \
           local __fuigo_bin={qpref}; \
           [ -x \"$__fuigo_bin\" ] || __fuigo_bin=$(command -v {bin_name} 2>/dev/null) || __fuigo_bin=''; \
           if [ -z \"$__fuigo_bin\" ]; then command {name} \"$@\"; return; fi; \
           if [[ -z ${{ZSH_VERSION-}} ]] && (( BASH_SUBSHELL > 0 )); then \
             exec -a {name} \"$__fuigo_bin\" {prepend}\"$@\"; \
           else \
             (exec -a {name} \"$__fuigo_bin\" {prepend}\"$@\"); \
           fi; \
         }}; \
         __fuigo_shadow_{name}=1"
    )
}
...
mod tests {
    ...
    fn shell_function_shape() {
        ...
        // Preferred path is the fast-path hint; the shadow execs `$__fuigo_bin`.
        assert!(fn_body.contains("local __fuigo_bin=/tmp/bfs"));
        assert!(fn_body.contains("exec -a find \"$__fuigo_bin\" \"$@\""));
        // Hint is trusted only when executable (`[ -x ]`, not `[ -f ]`), so a
        // non-exec hint falls through instead of hard-failing exec.
        assert!(fn_body.contains("[ -x \"$__fuigo_bin\" ]"));
        assert!(!fn_body.contains("[ -f \"$__fuigo_bin\" ]"));
        ...
        assert!(fn_body.contains("__fuigo_shadow_find=1"));
    ...
    fn shell_function_unresolved_uses_empty_hint() {
        ...
        assert!(fn_body.contains("local __fuigo_bin=''"));
        assert!(fn_body.contains("command -v bfs"));
        assert!(fn_body.contains("command find \"$@\""));
    }
    ...
    fn grep_prepends_ugrep_defaults() {
        ...
        assert!(fn_body.contains("local __fuigo_bin=/tmp/ugrep"));
        assert!(fn_body.contains("\"$__fuigo_bin\" -G --ignore-files --hidden -I"));
    ...
    fn bash_safe_quote_escapes_metacharacters() {
        ...
        assert!(body.contains("local __fuigo_bin='/tmp/evil$(id)'"), "{body}");
    ...
    fn restore_command_is_marker_gated() {
        ...
        assert!(r.contains("if [ -n \"${__fuigo_shadow_find-}\" ]"));
        assert!(r.contains("unalias find"));
        assert!(r.contains("unset -f find"));
        assert!(r.contains("unset __fuigo_shadow_find"));
    }
    ...
    fn build_injection_off_emits_marker_gated_restore_not_function() {
        ...
        assert!(inject.contains("if [ -n \"${__fuigo_shadow_find-}\" ]"));
        assert!(inject.contains("if [ -n \"${__fuigo_shadow_grep-}\" ]"));
    ...
    fn build_injection_on_shadows_resolved_tools() {
        ...
        assert!(inject.contains("__fuigo_shadow_find=1"));
    }
    ...
    fn resolve_tool_precedence() {
        ...
        let dir = std::env::temp_dir().join(format!("fuigo-resolve-{}-{:?}", std::process::id(), {
    ...
    /// `FUIGO_TOOLS_BUNDLE_{BFS,UGREP}_PATH` at build time). Verifies the embedded
    /// bytes self-extract under `~/.fuigo/vendor` and the extracted `bfs` runs.
    #[cfg(all(bundle_bfs, bundle_ugrep))]
    #[test]
    fn bundled_binaries_extract_and_run() {
        let vendor = crate::util::fuigo_home().join("vendor");
        let bfs = bundled_bfs()
            .expect("bfs resolves")
    ...
    /// mode-0644 `FUIGO_TOOLS_*_PATH` / vendor copy) must fall through to the OS
