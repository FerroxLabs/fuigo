//! ToolBridge: adapter that wraps `fuigo-tools`'s `ToolRegistry` and
...
impl ToolBridge {
    ...
    /// `Some("get_task_output")` for the fuigo_build agent and `None` for
    ...
    pub async fn kill_background_task(
        ...
        task_id: &str,
        source: KillSource,
    ) -> Result<KillOutcome, fuigo_tool_runtime::ToolError> {
        if let Some(terminal) = &self.terminal {
    ...
    pub async fn list_scheduled_tasks(
        &self,
    ) -> Vec<crate::implementations::fuigo_build::scheduler::types::ScheduledTask> {
    ...
    pub async fn delete_scheduled_task(
        &self,
        task_id: &str,
    ) -> Result<bool, fuigo_tool_runtime::ToolError> {
        use crate::implementations::fuigo_build::scheduler::types::{
            SchedulerCommand, SchedulerHandle,
        };
        let sender = {
            ...
            res.get::<SchedulerHandle>()
                .ok_or_else(|| {
                    fuigo_tool_runtime::ToolError::custom("missing_resource", "SchedulerHandle")
                })?
                .0
                .clone()
        };
        ...
        sender
            ...
            .map_err(|_| {
                fuigo_tool_runtime::ToolError::custom("process_manager", "Scheduler actor stopped")
            })?;
        reply_rx
            .await
            .map_err(|_| {
            ...
            .map_err(crate::implementations::fuigo_build::scheduler::types::scheduler_tool_error)
    }
...
mod tests {
    ...
    impl fuigo_tool_runtime::Tool for KindFixture {
        type Args = serde_json::Value;
        type Output = String;

        fn id(&self) -> fuigo_tool_protocol::ToolId {
            fuigo_tool_protocol::ToolId::new(self.id).expect("valid id")
        }
        fn description(
            &self,
            _ctx: &::fuigo_tool_runtime::ListToolsContext,
        ) -> fuigo_tool_types::ToolDescription {
            fuigo_tool_types::ToolDescription::new(self.id, "kind fixture")
        }
        async fn run(
            &self,
    ...
    fn tool_kind_returns_registered_kind_per_namespace_and_none_for_unknown() {
        ...
        // PascalCase + fuigo_build's snake_case in one registry
