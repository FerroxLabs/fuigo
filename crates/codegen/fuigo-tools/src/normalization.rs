...
/// Resolve `wire_name` in `toolset` and merge the canonical `fuigo/tool` object
...
/// the `fuigo/tool` `_meta` object. Equivalent tools across toolsets emit the
...
mod tests {
    ...
    fn canonical_omits_absent_options_not_null() {
        let fuigo = parse(serde_json::json!({"variant":"ReadFile","target_file":"/a"}));
        let g = canonical_input(&fuigo).unwrap();
