...
impl<'a> SkillEntry<'a> {
    ...
    // ── Budgeted XML rendering (fuigo build harness) ─────────────
...
///
/// `"/path/.fuigo/skills/my-skill/SKILL.md"` -> `"/path/.fuigo/skills/"`
...
/// Used by the budgeted (fuigo build) XML rendering path.
...
pub enum XmlRenderMode {
    ...
    Verbatim,
    /// Fuigo build harness: budget-capped three-tier rendering with full
    /// XML entity escaping.
    Budgeted {
        ...
        skills
            ...
            .filter(|s| {
                s.enabled
                    ...
                    // Compat mode renders all skills verbatim; only the fuigo
...
mod tests {
    ...
    fn verbatim_bypasses_is_listable_filter() {
        // fuigo-build drops a description-less skill; compat mode renders all.
    ...
    // ── budgeted mode: fuigo build harness (budgeted XML) ───────────
