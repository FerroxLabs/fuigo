//! Canonical external-settings tool name ↔ Fuigo tool correspondence: one table
//! replacing two that drifted apart.
//!
//! Two consumers read it independently. The hook matcher (`fuigo-hooks`) needs the
//! Fuigo tool **names** an external settings term maps to (and the reverse, for regex
//! matchers); the agent builder (`fuigo-agent`) needs the [`ToolKind`] a `tools:`
...
//!
//! The `fuigo` names are test-checked against the live registry.

use super::tool::ToolKind;
use ToolKind::*;

/// One Claude tool's correspondence to Fuigo, read via the accessor functions below.
struct ClaudeTool {
    claude: &'static str,
    /// Fuigo [`ToolKind`] for allowlist resolution; `None` for names that are matchable
    /// (spawn/plan-mode directives) but must not resolve an allowlist.
    kind: Option<ToolKind>,
    /// Fuigo tool names this Claude tool maps to (empty when there is no direct
    /// Fuigo tool — the entry then only contributes a `kind`).
    fuigo: &'static [&'static str],
}

/// Row that resolves an allowlist (carries a [`ToolKind`]) — the common case.
const fn k(claude: &'static str, kind: ToolKind, fuigo: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: Some(kind),
        fuigo,
...
const fn match_only(claude: &'static str, fuigo: &'static [&'static str]) -> ClaudeTool {
    ClaudeTool {
        claude,
        kind: None,
        fuigo,
...
const CLAUDE_TOOLS: &[ClaudeTool] = &[
    ...
    k("Skill",           Read,                 &["skill"]),                           // matcher: opencode's `skill` tool; allowlist Read (fuigo-build reads SKILL.md)
...
/// The Fuigo [`ToolKind`] a Claude allowlist entry resolves to, if any.
...
/// The Fuigo tool names a Claude matcher term fires on.
pub fn fuigo_names_for(claude: &str) -> impl Iterator<Item = &'static str> {
    CLAUDE_TOOLS
        .iter()
        .find(|t| t.claude == claude)
        .map(|t| t.fuigo)
...
/// The Claude names that map to `fuigo_name` (reverse lookup, for regex matchers).
pub fn claude_names_for(fuigo_name: &str) -> impl Iterator<Item = &'static str> + '_ {
    CLAUDE_TOOLS
        .iter()
        .filter(move |t| t.fuigo.contains(&fuigo_name))
        .map(|t| t.claude)
}

/// Every distinct Fuigo name the table references, for the `fuigo-agent` drift-check
/// test that asserts each is a real client tool name.
pub fn fuigo_names() -> impl Iterator<Item = &'static str> {
    let mut seen = std::collections::HashSet::new();
    CLAUDE_TOOLS
        .iter()
        .flat_map(|t| t.fuigo.iter().copied())
...
mod tests {
    ...
    fn every_row_contributes() {
        // A row with neither a kind nor a Fuigo name is dead weight (and signals a typo).
        for t in CLAUDE_TOOLS {
            assert!(
                t.kind.is_some() || !t.fuigo.is_empty(),
                "dead row: {}",
                t.claude
            );
        }
