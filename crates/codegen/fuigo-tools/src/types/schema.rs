...
///
/// Use with `#[schemars(with = "FuigoIntegerSchema")]` on `Option<usize>` fields.
pub struct FuigoIntegerSchema;
impl schemars::JsonSchema for FuigoIntegerSchema {
    fn schema_name() -> std::borrow::Cow<'static, str> {
        "fuigo_integer_schema".into()
    }
...
/// `z.coerce.string()` used by the TypeScript fuigo-computer tools, where
...
/// Lenient boolean deserializers (shared via `fuigo-tool-types`), re-exported so
/// fields reference them under the same `crate::types::schema::` path as above.
pub use fuigo_tool_types::{deserialize_lenient_bool, deserialize_lenient_option_bool};
#[cfg(test)]
mod tests {
    use super::*;
