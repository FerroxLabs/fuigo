...
//!
//! `MemoryBackend` is defined in `fuigo-tools` to keep the tool crate
...
///
/// Use `tracing::info!(target: MEMORY_LOG_TARGET, ...)` in `fuigo-tools`.
/// Mirrors `fuigo_shell::session::memory_log::TARGET`.
pub const MEMORY_LOG_TARGET: &str = "fuigo_memory";
