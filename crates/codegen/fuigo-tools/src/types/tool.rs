...
//!
//! The tool runtime contract (`Tool` trait) lives in `fuigo_tool_runtime`.
...
///
/// Serializes to snake_case (`fuigo_build`, `mcp`, …) for the
...
/// (e.g. `"FuigoBuild:read_file"`); only the serde form goes on the wire.
#[derive(
    Debug,
    Clone,
    Copy,
...
pub enum ToolNamespace {
    #[serde(alias = "FuigoBuild")]
    FuigoBuild,
    #[serde(alias = "FuigoBuildConcise")]
    FuigoBuildConcise,
    #[serde(alias = "FuigoBuildHashline")]
    FuigoBuildHashline,
    #[serde(alias = "Codex")]
    Codex,
