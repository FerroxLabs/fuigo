use crate::implementations::fuigo_build::send_subagent_message::SendSubagentMessageOutput;
use serde::{Deserialize, Serialize};
use std::path::PathBuf;
use strip_ansi_escapes::strip_str;
use fuigo_tool_types::SubagentCompletedOutput;
...
use crate::implementations::fuigo_build::todo::{TodoItem, TodoState};
...
pub(crate) fn typed_tool_output_preserving_cco(
    ...
    source: &impl fuigo_tool_runtime::ToolOutput,
) -> fuigo_tool_runtime::TypedToolOutput {
    let cco = source.chat_completion_output();
    let value = serde_json::to_value(payload).unwrap_or(serde_json::Value::Null);
    fuigo_tool_runtime::TypedToolOutput::from_value(tool_id, value).with_chat_completion_output(cco)
}
...
/// protocol boundary in `fuigo-shell`.
#[derive(Debug, Clone, Serialize, Deserialize, schemars::JsonSchema)]
...
pub struct BashOutput {
    ...
    /// Set by the fuigo_build `run_terminal_cmd` implementation when the
    /// command was detected as a bare `echo "<msg>"` (or close variant:
    /// echo -n, echo -e, simple printf for literal output, etc.).
    ///
    /// Used for:
    /// - Telemetry / statistics on this pattern for the fuigo_build backend.
...
pub struct WebFetchContent {
    ...
    pub bytes: usize,
    /// Internal path to the complete converted body when FuigoBuild persisted overflow.
    #[serde(skip)]
    #[schemars(skip)]
...
use fuigo_tool_types::TaskOutputOutput;
...
impl fuigo_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<fuigo_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => fuigo_tool_runtime::ToolOutput::chat_completion_output(bash),
...
pub enum ToolOutput {
    ...
    Monitor(crate::implementations::fuigo_build::monitor::types::MonitorOutput),
    SchedulerCreate(crate::implementations::fuigo_build::scheduler::create::SchedulerCreateOutput),
    SchedulerDelete(crate::implementations::fuigo_build::scheduler::delete::SchedulerDeleteOutput),
    SchedulerList(crate::implementations::fuigo_build::scheduler::list::SchedulerListOutput),
    UpdateGoal(crate::implementations::fuigo_build::update_goal::UpdateGoalOutput),
    Workflow(crate::implementations::fuigo_build::workflow::WorkflowToolOutput),
    ...
    pub fn is_error(&self) -> bool {
        match self {
            ...
            ToolOutput::SendSubagentMessage(output) => {
                matches!(
                output.disposition(),
                crate::implementations::fuigo_build::send_subagent_message::SendSubagentMessageDisposition::Rejected
            )
            }
            ToolOutput::Todo(
...
impl fuigo_tool_runtime::ToolOutput for ToolOutput {
    fn chat_completion_output(&self) -> Option<fuigo_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Bash(bash) => fuigo_tool_runtime::ToolOutput::chat_completion_output(bash),
            _ => None,
...
impl fuigo_tool_runtime::ToolOutput for BashOutput {
    fn chat_completion_output(&self) -> Option<fuigo_tool_runtime::ToolChatCompletionResponse> {
        ...
        Some(fuigo_tool_runtime::ToolChatCompletionResponse {
            result: Some(fuigo_tool_runtime::ToolChatCompletion {
                sender: "assistant".into(),
                message_tag: Some("raw_function_result".into()),
                code_execution_result: Some(fuigo_tool_runtime::ToolCodeExecutionResult {
                    stdout,
                    stderr: String::new(),
...
impl fuigo_tool_runtime::ToolOutput for GrepSearchOutput {}
impl fuigo_tool_runtime::ToolOutput for ReadFileOutput {}
impl fuigo_tool_runtime::ToolOutput for ListDirOutput {}
impl fuigo_tool_runtime::ToolOutput for SearchReplaceOutput {}
impl fuigo_tool_runtime::ToolOutput for TodoWriteOutput {}
impl fuigo_tool_runtime::ToolOutput for WebSearchOutput {}
impl fuigo_tool_runtime::ToolOutput for WebFetchOutput {}
impl fuigo_tool_runtime::ToolOutput for SkillOutput {}
impl fuigo_tool_runtime::ToolOutput for ApplyPatchOutput {}
impl fuigo_tool_runtime::ToolOutput for CodexGrepFilesOutput {}
impl fuigo_tool_runtime::ToolOutput for SearchToolOutput {}
impl fuigo_tool_runtime::ToolOutput for EnterPlanModeOutput {}
impl fuigo_tool_runtime::ToolOutput for ExitPlanModeOutput {}
impl fuigo_tool_runtime::ToolOutput for AskUserQuestionOutput {}
impl fuigo_tool_runtime::ToolOutput for MCPOutput {}
#[cfg(test)]
mod tests {
    use super::*;
    use crate::implementations::fuigo_build::todo::{TodoPriority, TodoStatus};
    use serde_json::json;
    use fuigo_tool_types::KillTaskResult;
    use fuigo_tool_types::TaskOutputResult;
    #[test]
    ...
    fn subagent_completed_prompt_format_with_worktree() {
        let output = ToolOutput::SubagentCompleted(SubagentCompletedOutput {
            ...
            turns: 1,
            duration_ms: 2000,
            worktree_path: Some("/tmp/fuigo-worktree/wt-agent".into()),
            persona: None,
        ...
        let rendered = output.to_prompt_format();
        assert!(
            rendered.contains("<worktree_path>/tmp/fuigo-worktree/wt-agent</worktree_path>"),
    ...
    fn assert_cer(
        resp: &fuigo_tool_runtime::ToolChatCompletionResponse,
        stdout: &str,
        exit_code: i32,
        timed_out: bool,
    ) {
    ...
    fn bash_output_chat_completion_carries_exit_and_stdout() {
        let resp = fuigo_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(
            0, b"hello\n", false,
        ))
        .unwrap();
    ...
    fn bash_output_chat_completion_empty_stdout_still_emits() {
        let resp =
            fuigo_tool_runtime::ToolOutput::chat_completion_output(&sample_bash(0, b"", false))
                .unwrap();
    ...
    fn bash_output_chat_completion_truncation_marker_and_extra() {
        ...
        let resp = fuigo_tool_runtime::ToolOutput::chat_completion_output(&bash).unwrap();
        let result = resp.result.as_ref().unwrap();
    ...
    fn bash_tool_output_foreground_delegates_background_skips() {
        let resp = fuigo_tool_runtime::ToolOutput::chat_completion_output(&BashToolOutput::Bash(
            sample_bash(0, b"ok", false),
        ))
        .unwrap();
        assert_cer(&resp, "ok", 0, false);
        assert!(
            fuigo_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn aggregate_tool_output_bash_delegates_background_skips() {
        let resp = fuigo_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Bash(
        ...
        assert!(
            fuigo_tool_runtime::ToolOutput::chat_completion_output(&ToolOutput::Text(
    ...
    fn bash_tool_id() -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("bash").unwrap()
    }
    #[test]
    fn into_typed_tool_output_preserves_bash_foreground_cco() {
        ...
        let dropped = fuigo_tool_runtime::TypedToolOutput::from_value(typed.tool_id, expected_value);
    ...
    fn into_typed_tool_output_non_bash_cco_is_none() {
        ...
        let typed = run.into_typed_tool_output(fuigo_tool_protocol::ToolId::new("text").unwrap());
