...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.fuigo/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.fuigo/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.fuigo/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.fuigo/ or ~/.claude/.
    User { path: PathBuf },
