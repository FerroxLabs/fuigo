use crate::implementations::fuigo_build::bash::BashError;
use crate::implementations::fuigo_build::todo::TodoError;
use crate::implementations::fuigo_build::web_fetch::WebFetchError;
...
/// Generate `impl From<$err> for fuigo_tool_runtime::ToolError` that wraps the
/// error as an execution failure tagged with the given tool ID.
macro_rules! impl_runtime_error_from {
    ($($err:ty => $tool_id:literal),+ $(,)?) => {
        $(
            impl From<$err> for fuigo_tool_runtime::ToolError {
                fn from(err: $err) -> Self {
                    fuigo_tool_runtime::ToolError::execution(
                        fuigo_tool_protocol::ToolId::new($tool_id).expect("valid static tool id"),
                        err.to_string(),
                    )
                }
            }
