...
/// `"namespace.Name"` (e.g., `"fuigo_build.ReadFile"`). The ID is used as
...
pub trait ResourceType: Any + 'static {
    /// Unique identifier, e.g. `"fuigo_build.ReadFile"`.
    const ID: &'static str;
...
///
/// ```ignore
/// register_resource!("fuigo_build", "ReadFile", ReadHistory);
/// ```
///
/// This generates:
/// ```ignore
/// impl ResourceType for ReadHistory {
///     const ID: &'static str = "fuigo_build.ReadFile";
/// }
/// ```
#[macro_export]
...
struct ResourceEntry {
    type_id: TypeId,
    /// The `ResourceType::ID` string (e.g., `"fuigo_build.ReadFile"`).
    id: String,
...
impl Resources {
    ...
    pub fn require<T: Send + Sync + 'static>(&self) -> Result<&T, fuigo_tool_runtime::ToolError> {
...
/// tool falls back to `Cwd/.fuigo/plan.md`.
#[derive(Debug, Clone)]
...
pub const PLAN_FILE_RELATIVE_PATH: &str = ".fuigo/plan.md";
...
/// [`Cwd`]`/.fuigo/plan.md`, else the bare relative `.fuigo/plan.md`.
...
pub(crate) fn require_plan_file_path(
    res: &Resources,
) -> Result<(PathBuf, String), fuigo_tool_runtime::ToolError> {
...
/// Newtype wrapper for `Arc<dyn fuigo_tool_runtime::ToolDispatch>` so it can
...
#[derive(Clone)]
pub struct InnerDispatch(pub std::sync::Arc<dyn fuigo_tool_runtime::ToolDispatch>);
#[derive(Debug, Clone)]
...
pub trait ManagedGatewayToolCaller: Send + Sync {
    async fn call_tool(
        ...
        caller: &str,
    ) -> Result<ManagedGatewayToolCallResponse, fuigo_tool_runtime::ToolError>;
}
#[derive(Clone)]
...
/// `FUIGO_SCHEDULER_BACKGROUND_LOOPS` env var, or the
...
///
/// Stamped onto [`fuigo_tool_runtime::ToolCallContext::extensions`] by
...
register_resource!("fuigo_build", "WebCitation", WebCitationCounter);
...
/// depending on `fuigo-mcp` directly.  Follows the same pattern as
...
mod tests {
    ...
    register_resource!("fuigo_build", "Edit", EditConfig);
    #[derive(Debug, Clone, Default, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
    struct ReadHistory {
    ...
    register_resource!("fuigo_build", "Todo", TodoData);
    #[test]
    ...
    fn ephemeral_types_silently_skipped_during_serialization() {
        ...
        assert!(state.get("fuigo_build.ReadFile").is_some());
    ...
    fn load_from_populates_registered_types() {
        ...
        state_map.insert(
            "fuigo_build.ReadFile".to_string(),
            serde_json::json!({"files_read": ["loaded.rs"]}),
        );
        let mut params_map = HashMap::new();
        params_map.insert(
            "fuigo_build.Edit".to_string(),
    ...
    fn get_json_returns_registered_value() {
        ...
        let val = res.get_json("params", "fuigo_build.Edit").unwrap();
    ...
    fn get_json_returns_none_for_missing_value() {
        let mut res = Resources::new();
        res.register_params::<EditConfig>();
        assert!(res.get_json("params", "fuigo_build.Edit").is_none());
    }
    #[test]
    ...
    fn resolve_model_path_tilde_expands_to_home() {
        let home = fuigo_dirs::home_dir().expect("test requires home_dir");
        let cwd = std::path::Path::new("/worktree/abc");
