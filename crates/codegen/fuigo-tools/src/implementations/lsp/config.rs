//! LSP server configuration from `.fuigo/lsp.json`.
...
/// This is the canonical merge function — both session startup and `fuigo inspect` call it.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let user_path = crate::util::fuigo_home::fuigo_home().join("lsp.json");
    let project_path = cwd.join(".fuigo").join("lsp.json");
...
/// Load LSP server configs from `~/.fuigo/lsp.json` and `<cwd>/.fuigo/lsp.json`.
