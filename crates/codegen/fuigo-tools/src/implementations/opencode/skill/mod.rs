...
${%- endfor %}
${%- else %}
(No skills available. Skills can be added in ~/.fuigo/skills/ or .fuigo/skills/)
${%- endif %}
</available_skills>"#;
...
impl fuigo_tool_runtime::Tool for SkillTool {
    type Args = SkillInput;
    type Output = SkillOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("skill").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: SkillInput,
    ) -> Result<SkillOutput, fuigo_tool_runtime::ToolError> {
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        let tool = SkillTool;
        assert_eq!(fuigo_tool_runtime::Tool::id(&tool).as_str(), "skill");
