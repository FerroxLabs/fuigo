...
use crate::implementations::fuigo_build::search_replace::CONTEXT_LINES;
use crate::implementations::fuigo_build::search_replace::helpers::{
...
// the snake_case `params.edit.old_string` keys of the fuigo_build twin resolve
...
impl From<EditInput> for crate::types::tool_io::ToolInput {
    fn from(value: EditInput) -> Self {
        crate::implementations::fuigo_build::search_replace::SearchReplaceInput {
...
impl fuigo_tool_runtime::Tool for EditTool {
    type Args = EditInput;
    type Output = SearchReplaceOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("edit").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: EditInput,
    ) -> Result<SearchReplaceOutput, fuigo_tool_runtime::ToolError> {
...
async fn ensure_parent_dirs(path: &std::path::Path) -> Result<(), fuigo_tool_runtime::ToolError> {
    if let Some(parent) = path.parent()
        && !parent.as_os_str().is_empty()
    {
        tokio::fs::create_dir_all(parent).await.map_err(|e| {
            fuigo_tool_runtime::ToolError::execution(
                fuigo_tool_protocol::ToolId::new("edit").expect("valid"),
                e.to_string(),
            )
        })?;
    }
...
// are imported from `fuigo_build::search_replace::helpers` — shared across
// both the fuigo_build and opencode edit tools.
...
mod tests {
    ...
    fn tool_id_and_kind() {
        ...
        let tool = EditTool;
        assert_eq!(fuigo_tool_runtime::Tool::id(&tool).as_str(), "edit");
