...
//! fuigo-tools-server serves this tool on the stateless local backend
...
impl fuigo_tool_runtime::Tool for BashTool {
    ...
    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("bash").expect("valid tool id")
    }
        ...
        let result = match backend.run(request).await {
            ...
            Err(e) => {
                ...
                return Err(fuigo_tool_runtime::ToolError::execution(
                    fuigo_tool_protocol::ToolId::new("run_terminal_command").expect("valid"),
                    e.to_string(),
                ));
            }
        };
...
mod tests {
    ...
    fn rt_ctx_with_call_id(
        ...
        let id = fuigo_tool_protocol::ToolCallId::new(call_id).unwrap();
    ...
    async fn tool_metadata() {
        use crate::types::tool_metadata::ToolMetadata;
        let tool = BashTool;
        assert_eq!(fuigo_tool_runtime::Tool::id(&tool).as_str(), "bash");
    ...
    //
    // Same shape as fuigo_build/bash: the opencode tool inherits the same
