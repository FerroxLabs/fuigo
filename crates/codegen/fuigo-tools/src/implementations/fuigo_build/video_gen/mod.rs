...
//! the Ferrox Labs Video Generation API and save them to the local filesystem so the
...
const DEFAULT_ZDR_VIDEO_KEY_PREFIX: &str = "fuigo-videos/";
...
pub use fuigo_tools_api::slash_commands::{
...
impl S3AccessCredentials {
    ...
    fn to_static(&self) -> fuigo_file_utils::s3::S3StaticCredentials {
        fuigo_file_utils::s3::S3StaticCredentials {
...
/// HTTP client for Ferrox Labs Video Generation API. Cloned per-request; shares `Arc` state.
#[derive(Clone)]
...
impl VideoGenClient {
    pub fn new(
        ...
        api_key_provider: Option<SharedApiKeyProvider>,
    ) -> Result<Self, fuigo_tool_runtime::ToolError> {
    ...
    ) -> Result<VideoOutcome, fuigo_tool_runtime::ToolError> {
    ...
    async fn presign_zdr_output_urls(
    ...
    ) -> Result<ZdrPresignedUrls, fuigo_tool_runtime::ToolError> {
        ...
        let upload_url = fuigo_file_utils::s3::presign_put_url(
            &config.region,
            endpoint,
    ...
    async fn presign_and_download(
        ...
        urls: &ZdrPresignedUrls,
        request_id: &str,
    ) -> Result<Vec<u8>, fuigo_tool_runtime::ToolError> {
        let get_url = self
    ...
    async fn presign_zdr_get_url(
        ...
        let url = fuigo_file_utils::s3::presign_get_url(
            &config.region,
            endpoint,
            &creds.to_static(),
            &config.bucket,
...
fn video_http_error(status: reqwest::StatusCode, body: &str) -> fuigo_tool_runtime::ToolError {
    if is_zdr_upload_url_error(body) {
...
fn validate_one_of(
    ...
    Err(fuigo_tool_runtime::ToolError::invalid_arguments(format!(
        "`{field}` must be one of: {}. Got {value}.",
        allowed.join(", ")
    )))
}

fn validate_imagine_duration(duration: Option<u32>) -> Result<(), fuigo_tool_runtime::ToolError> {
...
fn validate_r2v_duration(duration: Option<u32>) -> Result<(), fuigo_tool_runtime::ToolError> {
    if let Some(secs) = duration
...
) -> Result<(VideoGenClient, std::path::PathBuf), fuigo_tool_runtime::ToolError> {
...
async fn save_video_bytes(
    ...
    video_bytes: &[u8],
) -> Result<std::path::PathBuf, fuigo_tool_runtime::ToolError> {
...
) -> Result<MediaGenOutput, fuigo_tool_runtime::ToolError> {
    match outcome {
...
impl fuigo_tool_runtime::Tool for ImageToVideoTool {
    type Args = ImageToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new(IMAGE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
...
impl fuigo_tool_runtime::Tool for ReferenceToVideoTool {
    type Args = ReferenceToVideoInput;
    type Output = ToolOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new(REFERENCE_TO_VIDEO_TOOL_NAME).expect("valid tool id")
    }
