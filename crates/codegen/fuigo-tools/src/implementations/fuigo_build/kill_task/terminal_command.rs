...
use crate::implementations::fuigo_build::task_output::background_bash_requires_exprs;
use crate::types::requirements::{Expr, ToolRequirement};
use crate::types::tool::{ToolKind, ToolNamespace};
use fuigo_tool_types::{KillTaskOutput, KillTaskToolInput};
...
impl fuigo_tool_runtime::Tool for KillTerminalCommandTool {
    type Args = KillTaskToolInput;
    type Output = KillTaskOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("kill_terminal_command").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        fuigo_tool_runtime::Tool::run(&KillTaskTool, ctx, input).await
    }
}
