...
use regex::Regex;
use fuigo_config::shell::AmpersandSemantics;
...
static BASH_CAPABILITIES: LazyLock<fuigo_tool_protocol::ToolCapabilities> =
...
pub struct BashParams {
    ...
    /// fuigo-build opts up to 10h via fuigo-shell's `BashToolConfig`.
    ///
    /// When set:
...
impl crate::types::resources::ResourceType for BashParams {
    const ID: &'static str = "fuigo_build.Bash";
...
impl fuigo_tool_runtime::ToolOutput for BashToolOutput {
    fn chat_completion_output(&self) -> Option<fuigo_tool_runtime::ToolChatCompletionResponse> {
        match self {
            Self::Foreground(bash) => fuigo_tool_runtime::ToolOutput::chat_completion_output(bash),
...
// particular production fuigo-build sets it to 10h in fuigo-shell's
...
/// [`fuigo_config::shell::ampersand_semantics`] reports POSIX `&` semantics
/// (Unix + Git Bash).
...
// ───────────────────────────────────────────────────────────────────────────
// Bare `echo "<msg>"` detection (for statistics + hints in fuigo_build bash)
// ───────────────────────────────────────────────────────────────────────────
...
/// communication. We want statistics on this pattern for the fuigo_build
...
impl BashTool {
    ...
    /// (fuigo-cursor) uses this ceiling to report the true FG wait in
...
${%- endif %}"#
    ...
    fn get_prefixed_command(cmd_prefix: &Option<String>, command: &str) -> String {
        match cmd_prefix {
            Some(prefix) => {
                let sep = fuigo_config::shell::chain_separator();
...
impl fuigo_tool_runtime::Tool for BashTool {
    type Args = BashToolInput;
    type Output = BashToolOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("run_terminal_cmd").expect("valid tool id")
    }
    ...
    async fn execute(
        &self,
        mut ctx: fuigo_tool_runtime::ToolCallContext,
        input: BashToolInput,
    ) -> fuigo_tool_runtime::ToolStream<BashToolOutput> {
        ...
        let Some(spec) = BASH_CAPABILITIES.streaming.as_ref() else {
            return Box::pin(async_stream::stream! {
                yield fuigo_tool_runtime::ToolStreamItem::Terminal(Err(
            ...
            loop {
                tokio::select! {
                    ...
                    result = &mut run_fut => {
                        ...
                        yield fuigo_tool_runtime::ToolStreamItem::Terminal(result);
                        break;
                    }
                }
            }
    ...
    async fn run(
        ...
        input: BashToolInput,
    ) -> Result<BashToolOutput, fuigo_tool_runtime::ToolError> {
        ...
        let ampersand = fuigo_config::shell::ampersand_semantics();
        if let Some(violation) = should_reject_background_op(
            ...
            return Err(fuigo_tool_runtime::ToolError::invalid_arguments(message));
        }
        ...
        if input.is_background {
            ...
            // We tag it for statistics (fuigo_build backend) and can surface an educational
...
mod tests {
    ...
    fn read_chunk_progress(p: &fuigo_tool_runtime::ToolProgress) -> (String, usize, bool, bool) {
        match p {
    ...
    fn bash_output_chunk_progress_caps_oversized_delta() {
        ...
        while last < total {
            let p = bash_output_chunk_progress(spec, &chunk, &mut last).unwrap();
            let fuigo_tool_runtime::ToolProgress::Custom { payload, .. } = p else {
    ...
    async fn bash_streaming_progress_suppressed_when_gate_off() {
        ...
        let tool = BashTool;
        let mut stream = fuigo_tool_runtime::Tool::execute(
            &tool,
            ctx,
        ...
        let mut terminal: Option<Result<BashToolOutput, fuigo_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                fuigo_tool_runtime::ToolStreamItem::Progress(_) => {
                    progress += 1;
                }
    ...
    async fn background_injects_python_unbuffered() {
        ...
        let _ = fuigo_tool_runtime::Tool::run(
            &tool,
            test_ctx(resources.into_shared()),
    ...
    async fn errors_when_terminal_not_in_resources() {
        ...
        let result =
            fuigo_tool_runtime::Tool::run(&tool, test_ctx(resources.into_shared()), make_input("ls"))
                .await;
        assert!(result.is_err());
        assert!(
            result
    ...
    fn foreground_chat_completion_emits_code_execution_result() {
        let resp = fuigo_tool_runtime::ToolOutput::chat_completion_output(
    ...
    fn background_chat_completion_is_none() {
        ...
        assert!(fuigo_tool_runtime::ToolOutput::chat_completion_output(&out).is_none());
    }
    ...
    mod resolve_effective_timeout_tests {
        ...
        fn foreground_positive_clamped_to_default_max() {
            ...
            // fuigo-shell opts up to 10h via max_timeout_secs. Background unaffected.
    ...
    mod versioned_background_check {
        ...
        ) -> fuigo_tool_runtime::ToolCallContext {
            let mut ctx =
                fuigo_tool_runtime::ToolCallContext::new(fuigo_tool_protocol::ToolCallId::new_v7());
