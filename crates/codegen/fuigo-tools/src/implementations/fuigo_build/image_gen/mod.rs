//! `image_gen` tool — generates images via the Ferrox Labs Imagine API and saves
...
pub struct ImageGenClient {
    ...
    /// [`FUIGO_IMAGINE_MODEL`]). `image_edit` uses its own model and is
    /// unaffected.
    model: String,
    edit_model: String,
...
impl ImageGenClient {
    pub fn new(
        ...
        let model = model_override
            ...
            .unwrap_or_else(|| FUIGO_IMAGINE_MODEL.to_owned());
        let edit_model = edit_model_override
            .clone()
            .filter(|m| !m.trim().is_empty())
            .unwrap_or_else(|| super::image_edit::FUIGO_IMAGINE_EDIT_MODEL.to_owned());
...
pub enum ImageGenConfig {
    ...
    Enabled {
        ...
        /// ([`FUIGO_IMAGINE_MODEL`]). Driven by the remote
...
impl ImageGenConfig {
    ...
    /// default quality model ([`FUIGO_IMAGINE_MODEL`]) is used.
...
impl fuigo_tool_runtime::Tool for ImageGenTool {
    type Args = ImageGenInput;
    type Output = ToolOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("image_gen").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(fuigo_tool_runtime::Tool::id(&tool).as_str(), "image_gen");
    }
