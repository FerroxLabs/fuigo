...
//!
//! - **`fuigo-tools`** — tool blocks on a oneshot, formats the result.
//! - **`fuigo-shell`** — coordinator receives requests over mpsc, calls the
//!   client via ACP `ext_method`, sends results back over the oneshot.
//! - **`fuigo-pager`** — handles the `ExtMethod`, renders UI, returns a
//!   typed response.
//!
//! All three crates import these types from `fuigo-tools`.
...
register_resource!("fuigo_build", "UserQuestionSender", UserQuestionSender);
