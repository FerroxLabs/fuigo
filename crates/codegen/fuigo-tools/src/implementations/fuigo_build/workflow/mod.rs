...
pub use fuigo_tools_api::slash_commands::WORKFLOW_TOOL_NAME;

/// Short name of a tool id (`FuigoBuild:workflow` → `workflow`).
...
impl fuigo_tool_runtime::ToolOutput for WorkflowToolOutput {}
...
impl fuigo_tool_runtime::Tool for WorkflowTool {
    type Args = WorkflowToolInput;
    type Output = WorkflowToolOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new(WORKFLOW_TOOL_NAME).expect("valid tool id")
    }
    ...
    async fn run(
    ...
    ) -> Result<WorkflowToolOutput, fuigo_tool_runtime::ToolError> {
        ...
        match ack_rx.await {
            ...
            Ok(WorkflowLaunchAck::Rejected { code, detail }) => {
                Err(fuigo_tool_runtime::ToolError::custom(code, detail))
            }
...
mod tests {
    ...
    fn workflow_id_matches_kind_or_short_name() {
        ...
        assert!(is_workflow_tool_id("FuigoBuild:workflow"));
        assert!(!is_workflow_tool_id("web_search"));
        assert!(is_workflow_tool(Some(ToolKind::Workflow), "anything"));
        assert!(is_workflow_tool(None, "FuigoBuild:workflow"));
