...
impl fuigo_tool_runtime::ToolOutput for SchedulerCreateOutput {}
...
impl fuigo_tool_runtime::Tool for SchedulerCreateTool {
    type Args = SchedulerCreateInput;
    type Output = SchedulerCreateOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new(SCHEDULER_CREATE_TOOL_NAME).expect("valid tool id")
    }
    ...
    ) -> Result<SchedulerCreateOutput, fuigo_tool_runtime::ToolError> {
...
mod tests {
    use super::*;
    use crate::implementations::fuigo_build::scheduler::actor::SchedulerActor;
