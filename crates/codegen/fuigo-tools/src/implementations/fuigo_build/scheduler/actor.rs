...
mod tests {
    ...
    async fn non_durable_expiry_persists_task_absence() {
        ...
        assert_eq!(
            persisted["state"]["fuigo_build.Scheduler"]["tasks"],
            serde_json::json!([]),
    ...
    fn make_test_actor_with_subagents_at(
        ...
        resources.insert(
            crate::implementations::fuigo_build::task::types::SessionIdResource(
    ...
    async fn durable_delete_retries_persistence_and_reuses_version() {
        ...
        .unwrap();
        assert_eq!(persisted["state"]["fuigo_build.WebCitation"]["counter"], 7);
        assert_eq!(
            persisted["state"]["fuigo_build.Scheduler"]["tasks"],
            serde_json::json!([])
        );
    ...
    async fn durable_expiry_persists_before_ack_and_commits_version() {
...

        {
            ...
            assert_eq!(
                snapshot["state"]["fuigo_build.Scheduler"]["tasks"],
                serde_json::json!([])
            );
            assert!(notifications.try_recv().is_err());
