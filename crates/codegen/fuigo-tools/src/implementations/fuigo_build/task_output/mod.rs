...
pub(crate) fn max_wait_block() -> Duration {
    Duration::from_millis(fuigo_tool_types::max_wait_block_ms())
}
...
pub(crate) fn background_bash_requires_exprs() -> Vec<Expr<ToolRequirement>> {
    ...
    let fuigo_build_bash = Expr::Value(ToolRequirement::Tool {
        namespace: ToolMetadata::tool_namespace(&BashTool).to_string(),
        id: fuigo_tool_runtime::Tool::id(&BashTool).as_str().to_string(),
    ...
    let fuigo_build_concise_bash = Expr::Value(ToolRequirement::Tool {
        namespace: ToolMetadata::tool_namespace(&BashConciseTool).to_string(),
        id: fuigo_tool_runtime::Tool::id(&BashConciseTool)
            .as_str()
            .to_string(),
        if_params: Some(Expr::Value(ToolParamsRequirement {
            key: "enabled_background".to_string(),
    ...
    let opencode_bash = Expr::Value(ToolRequirement::Tool {
        ...
        id: fuigo_tool_runtime::Tool::id(&OpenCodeBashTool)
            .as_str()
            .to_string(),
        if_params: None,
    });
    vec![fuigo_build_bash, fuigo_build_concise_bash, opencode_bash]
}
...
pub(crate) fn task_output_requires_expr() -> Expr<ToolRequirement> {
    ...
    let task_tool = Expr::Value(ToolRequirement::Tool {
        ...
        id: fuigo_tool_runtime::Tool::id(&TaskTool).as_str().to_string(),
        if_params: None,
    });
...
impl TaskOutputTool {
    async fn run_single_task(
        ...
        let contract_version = ctx
            .extensions
            .get::<fuigo_tool_runtime::BehaviorVersion>()
            .map(|v| v.0.clone());
...
pub(crate) use fuigo_tool_types::MAX_MULTI_WAIT_IDS;
...
fn format_subagent_snapshot(snap: &SubagentSnapshot, wait_hint: WaitHint) -> TaskOutputOutput {
    ...
    match &snap.status {
        ...
        SubagentSnapshotStatus::Completed {
            ...
            output.push_str("\n\n");
            output.push_str(&fuigo_tool_types::format_resume_footer(
                &snap.subagent_id,
                &snap.subagent_type,
...
impl crate::types::tool_metadata::ToolMetadata for TaskOutputTool {
    ...
    fn description_template(&self) -> &str {
        ...
        // fallback mirrors the default fuigo-build toolset.
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            fuigo_tool_types::build_task_output_description(&fuigo_tool_types::TaskOutputToolNaming {
...
/// [`fuigo_tool_types::build_task_output_description`] builder so the CLI and
...
impl fuigo_tool_runtime::Tool for TaskOutputTool {
    type Args = TaskOutputToolInput;
    type Output = TaskOutputOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("get_task_output").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        // The static fallback is the shared builder's default fuigo-build
    ...
    fn resources_with_backend_query() -> (
        ...
        tokio::sync::mpsc::UnboundedReceiver<
            crate::implementations::fuigo_build::task::types::SubagentEvent,
        >,
    ) {
        use crate::implementations::fuigo_build::task::backend::{
            ChannelBackend, SubagentBackendResource,
        };
        use crate::implementations::fuigo_build::task::types::SubagentEvent;
    ...
    fn unwrap_query(
        event: crate::implementations::fuigo_build::task::types::SubagentEvent,
    ) -> crate::implementations::fuigo_build::task::types::SubagentQueryRequest {
        match event {
            crate::implementations::fuigo_build::task::types::SubagentEvent::Query(q) => q,
