//! `image_edit` tool — edits or transforms images via the Ferrox Labs Imagine
...
use crate::implementations::fuigo_build::image_gen::{ImageGenClient, ImageGenResponse};
...
pub(crate) const FUIGO_IMAGINE_EDIT_MODEL: &str = "fuigo-imagine-image-quality";
...
fn compress_reference(
    raw_bytes: Vec<u8>,
) -> Result<(Vec<u8>, &'static str), fuigo_tool_runtime::ToolError> {
    ...
    let img = ImageReader::new(Cursor::new(&raw_bytes))
        ...
        .ok_or_else(|| {
            fuigo_tool_runtime::ToolError::invalid_arguments("failed to decode image reference")
        })?;
...
impl fuigo_tool_runtime::Tool for ImageEditTool {
    type Args = ImageEditInput;
    type Output = ToolOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("image_edit").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_name_and_description() {
        ...
        assert_eq!(fuigo_tool_runtime::Tool::id(&tool).as_str(), "image_edit");
    }
