...
//! `LspTool` (Tool trait impl) under the `FuigoBuild` namespace.
...
impl fuigo_tool_runtime::ToolOutput for LspToolOutput {}
...
impl fuigo_tool_runtime::Tool for LspTool {
    type Args = LspToolInput;
    type Output = LspToolOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("lsp").expect("valid tool id")
    }
    ...
    async fn run(
        ...
        input: LspToolInput,
    ) -> Result<LspToolOutput, fuigo_tool_runtime::ToolError> {
            ...
            handle = res
                ...
                .ok_or_else(|| {
                    fuigo_tool_runtime::ToolError::custom(
                        "process_manager",
                        "LSP tool is unavailable. Configure ~/.fuigo/lsp.json or <cwd>/.fuigo/lsp.json and ensure the language server can start.",
                    )
                })?
                .clone();
        }
