...
use educe::Educe;
use fuigo_tool_types::{SubagentCapabilityMode, SubagentIsolationMode, WaitMode};
...
pub struct SubagentRuntimeOverrides {
    ...
    /// `"fuigo-build-plan"`) whose `AgentDefinition` decides the child's harness
...
/// Re-export of [`fuigo_tool_types::is_not_sentinel`] for existing call sites.
pub use fuigo_tool_types::is_not_sentinel;
...
/// defense-in-depth check in `fuigo-shell`'s subagent coordinator.
...
fn is_background_capable_bash_tool(tc: &crate::registry::types::ToolConfig) -> bool {
    match tc.id.as_str() {
        "FuigoBuild:run_terminal_cmd" | "FuigoBuildConcise:run_terminal_cmd" => tc
            .params
            .as_ref()
...
register_resource!("fuigo_build", "SubagentEventSender", SubagentEventSender);
...
register_resource!("fuigo_build", "SubagentDepthCounter", SubagentDepthCounter);

/// Host-injected max nesting depth; absent → [`super::MAX_SUBAGENT_DEPTH`].
#[derive(Debug, Clone, Copy)]
pub struct MaxSubagentDepth(pub u32);

register_resource!("fuigo_build", "MaxSubagentDepth", MaxSubagentDepth);
...
register_resource!("fuigo_build", "TaskModelValidator", TaskModelValidator);
...
register_resource!("fuigo_build", "SessionIdResource", SessionIdResource);
...
///
/// Set by fuigo-shell immediately before a prompt turn begins executing so
...
/// True while a `/goal` loop is active. Set by fuigo-shell at turn start.
...
register_resource!("fuigo_build", "GoalLoopActive", GoalLoopActive);
...
mod tests {
    ...
    fn read_only_filter_prunes_orphaned_background_task_tools() {
        let mut config = ToolServerConfig {
            tools: vec![
                tc("FuigoBuild:run_terminal_cmd", ToolKind::Execute),
                tc("FuigoBuild:read_file", ToolKind::Read),
                tc("FuigoBuild:list_dir", ToolKind::List),
                tc("FuigoBuild:grep", ToolKind::Search),
                tc("FuigoBuild:kill_task", ToolKind::KillTaskAction),
                tc("FuigoBuild:get_task_output", ToolKind::BackgroundTaskAction),
            ],
    ...
    fn read_only_filter_keeps_background_task_tools_when_task_tool_remains() {
        let mut config = ToolServerConfig {
            tools: vec![
                ...
                tc("FuigoBuild:kill_task", ToolKind::KillTaskAction),
                tc("FuigoBuild:get_task_output", ToolKind::BackgroundTaskAction),
                tc("FuigoBuild:task", ToolKind::Task),
            ],
            behavior_preset: None,
        };
    ...
    fn read_write_filter_keeps_background_capable_bash_when_explicitly_enabled() {
        let mut bash = tc("FuigoBuild:run_terminal_cmd", ToolKind::Execute);
        bash.params = Some(
