...
use fuigo_tool_types::{SubagentCompletedOutput, SubagentIsolationMode, TaskToolInput};
use regex::Regex;
...
async fn detect_continue_parent_work(
    ...
    fuigo_tool_types::should_continue_parent_work(&asks, description, prompt)
}
...
async fn resolve_background_notice_names(resources: &SharedResources) -> (String, String, String) {
    let canonical = fuigo_tool_types::BackgroundNoticeNaming::CANONICAL;
...
fn flatten_spawn_join(
    joined: Result<Result<SubagentResult, fuigo_tool_runtime::ToolError>, tokio::task::JoinError>,
) -> Result<SubagentResult, fuigo_tool_runtime::ToolError> {
    match joined {
        Ok(result) => result,
        Err(_) => Err(fuigo_tool_runtime::ToolError::custom(
            "channel_closed",
...
fn background_spawn_reject_error(
    id: &str,
    subagent_type: &str,
    result: Result<SubagentResult, fuigo_tool_runtime::ToolError>,
...
impl crate::types::tool_metadata::ToolMetadata for TaskTool {
    ...
    fn description_template(&self) -> &str {
        // Fuigo normally supplies the description via
        // `ToolConfig::with_description(...)` using `build_task_description()`
        // in fuigo-agent/src/builder.rs (live subagent roster). But a
        ...
        static DESC: std::sync::LazyLock<String> = std::sync::LazyLock::new(|| {
            let subagents: Vec<fuigo_tool_types::SubagentDescriptor> =
                fuigo_tool_types::BUILTIN_SUBAGENTS
                    .iter()
                    .map(|b| fuigo_tool_types::SubagentDescriptor {
                        name: b.name.to_owned(),
                        description: b.description.to_owned(),
                        tools: Some(guard_kind_tokens(b.tools_template)),
                    })
                    .collect();
            fuigo_tool_types::build_task_description(
                &subagents,
                &fuigo_tool_types::TaskToolNaming {
...
impl fuigo_tool_runtime::Tool for TaskTool {
    type Args = TaskToolInput;
    type Output = ToolOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new(TASK_TOOL_NAME).expect("valid tool id")
    }
    ...
    async fn run(
        ...
        let tool_cancellation = ctx
            .get::<fuigo_tool_runtime::Cancellation>()
        ...
        let model = fuigo_tool_types::sanitize_optional_arg(input.model);
            ...
            && resume_from.is_none()
        {
            ...
            if !p.is_dir() {
                ...
                return Err(fuigo_tool_runtime::ToolError::invalid_arguments(detail));
            }
        }
        ...
        if let Some(ref requested) = model {
            ...
            if let Some(error) = validator.error_for(requested) {
                return Err(fuigo_tool_runtime::ToolError::invalid_arguments(error));
            }
        }
        ...
        if input.run_in_background {
            ...
            let naming = fuigo_tool_types::BackgroundNoticeNaming {
            ...
            return Ok(ToolOutput::Text(
                fuigo_tool_types::format_subagent_started_background(
                    &id,
                    &input.subagent_type,
                    &input.description,
                    &naming,
        ...
        if result.backgrounded {
            ...
            let text = fuigo_tool_types::format_subagent_auto_backgrounded(
                &id,
                &input.subagent_type,
                &input.description,
                &naming,
        ...
        if result.success {
            ...
            Err(fuigo_tool_runtime::ToolError::invalid_arguments(
                result
                    .error
...
mod tests {
    use super::*;
    use crate::implementations::fuigo_build::task::backend::{
    ...
    async fn auto_backgrounded_result_returns_task_id_text() {
        ...
        match result {
            ToolOutput::Text(text) => {
                ...
                assert!(
                    text.text
                        .contains(fuigo_tool_types::BACKGROUND_SUBAGENT_CONTINUE_PARENT_WORK),
    ...
    async fn invalid_model_returns_error_before_background_spawn() {
        ...
        let result =
            fuigo_tool_runtime::Tool::run(&TaskTool, test_ctx(resources.into_shared()), input)
                .await;
    ...
    async fn validation_unavailable_returns_custom_error_not_invalid_arguments() {
        ...
        assert!(
            matches!(err.kind, fuigo_tool_runtime::ToolErrorKind::Custom),
    ...
    fn task_tool_id_predicate_accepts_all_wire_spellings() {
        assert!(is_task_tool_id(
            fuigo_tool_runtime::Tool::id(&TaskTool).as_str()
        ));
    ...
    async fn model_threads_to_runtime_overrides() {
        ...
        let result = fuigo_tool_runtime::Tool::run(&TaskTool, test_ctx(shared), input)
            .await
            .unwrap();
        handle.await.unwrap();
        match result {
    ...
    async fn model_whitespace_is_trimmed() {
        ...
        let mut input = task_input("general-purpose", false);
        input.model = Some("  test-model  ".into());
        let _ = fuigo_tool_runtime::Tool::run(&TaskTool, test_ctx(shared), input)
            .await
            .unwrap();
        handle.await.unwrap();
    }
