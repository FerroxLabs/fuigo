...
use crate::implementations::fuigo_build::task::types::{SubagentOwner, SubagentRequest};
...
fn limits_accept_plain_positive_digits_and_ignore_everything_else() {
    let from = |concurrent: Option<&str>, behavior: Option<&str>| {
        SubagentLimits::from_lookup(|var| {
            match var {
                "FUIGO_MAX_CONCURRENT_SUBAGENTS" => concurrent,
                "FUIGO_SUBAGENT_LIMIT_BEHAVIOR" => behavior,
