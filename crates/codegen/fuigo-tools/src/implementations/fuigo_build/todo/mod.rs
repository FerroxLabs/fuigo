...
crate::register_resource!("fuigo_build", "Todo", TodoState);
...
impl fuigo_tool_runtime::Tool for TodoWriteTool {
    ...
    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("todo_write").expect("valid tool id")
    }
...
mod tests {
    ...
    fn name_and_description() {
        ...
        assert_eq!(fuigo_tool_runtime::Tool::id(&tool).as_str(), "todo_write");
    }
    ...
    async fn state_serialization_roundtrip() {
        ...
        assert!(
            state_map.get("fuigo_build.Todo").is_some(),
            "TodoState should serialize under 'fuigo_build.Todo'"
        );
