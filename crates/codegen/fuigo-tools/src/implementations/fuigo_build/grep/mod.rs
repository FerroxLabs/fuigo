...
// Re-export the shared FuigoIntegerSchema from types module
pub use crate::types::FuigoIntegerSchema;
use ripgrep::rg_path;
...
crate::register_resource!("fuigo_build", "Grep", GrepParams);
...
fn grep_timeout() -> Duration {
    Duration::from_secs(grep_timeout_secs(fuigo_tty_utils::is_wsl()))
}
...
static GREP_CAPABILITIES: LazyLock<fuigo_tool_protocol::ToolCapabilities> =
...
impl fuigo_tool_runtime::Tool for GrepTool {
    type Args = GrepSearchInput;
    type Output = GrepSearchOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("grep").expect("valid tool id")
    }
    ...
    async fn execute(
    ...
    ) -> fuigo_tool_runtime::ToolStream<GrepSearchOutput> {
        ...
        let Some(spec) = admitted_spec else {
            return fuigo_tool_runtime::terminal_only(self.run(ctx, input).await);
        };
    ...
    async fn run(
        ...
        ctx: fuigo_tool_runtime::ToolCallContext,
        input: GrepSearchInput,
    ) -> Result<GrepSearchOutput, fuigo_tool_runtime::ToolError> {
...
fn grep_progress_stream(
    ctx: fuigo_tool_runtime::ToolCallContext,
    input: GrepSearchInput,
    spec: &'static fuigo_tool_protocol::StreamingSpec,
    span: tracing::Span,
) -> fuigo_tool_runtime::ToolStream<GrepSearchOutput> {
    Box::pin(async_stream::stream! {
        ...
        let GrepReady {
            ...
            Ok(GrepStep::Early(out)) => {
                ...
                yield fuigo_tool_runtime::ToolStreamItem::Terminal(Ok(out));
                return;
            }
            Err(e) => {
        ...
        if timed_out {
            ...
            if stdout_buf.is_empty() {
                yield fuigo_tool_runtime::ToolStreamItem::Terminal(Ok(grep_timeout_output(secs)));
            } else {
...
async fn prepare_grep(
    ctx: &fuigo_tool_runtime::ToolCallContext,
    input: &GrepSearchInput,
) -> Result<GrepStep, fuigo_tool_runtime::ToolError> {
...
struct BodyStreamer<'a> {
    spec: &'a fuigo_tool_protocol::StreamingSpec,
...
impl<'a> BodyStreamer<'a> {
    fn new(spec: &'a fuigo_tool_protocol::StreamingSpec, config: &'a GrepFormatConfig) -> Self {
        Self {
            spec,
            config,
            body: String::new(),
    ...
    fn feed(&mut self, bytes: &[u8]) -> Vec<fuigo_tool_runtime::ToolProgress> {
    ...
    fn finish(&mut self) -> Option<fuigo_tool_runtime::ToolProgress> {
    ...
    fn push_line(&mut self, line: &[u8]) -> Option<fuigo_tool_runtime::ToolProgress> {
        ...
        fuigo_tool_runtime::stream_chunk(
            self.spec,
            self.body.as_bytes(),
...
mod tests {
    ...
    async fn deny_read_globs_exclude_denied_files() {
        ...
        let run = |resources: Resources, input| async {
            let out =
                fuigo_tool_runtime::Tool::run(&GrepTool, test_ctx(resources.into_shared()), input)
                    .await
                    .unwrap();
    ...
    fn read_grep_delta(p: &fuigo_tool_runtime::ToolProgress) -> String {
        match p {
    ...
    async fn grep_streaming_body_matches_card_body() {
        ...
        let mut terminal: Option<Result<GrepSearchOutput, fuigo_tool_runtime::ToolError>> = None;
    ...
    async fn grep_streaming_suppressed_when_gate_off() {
        ...
        let tool = GrepTool;
        let mut stream =
            fuigo_tool_runtime::Tool::execute(&tool, ctx, make_grep_input("findme")).await;

        let mut progress = 0usize;
        let mut terminal: Option<Result<GrepSearchOutput, fuigo_tool_runtime::ToolError>> = None;
        while let Some(item) = stream.next().await {
            match item {
                fuigo_tool_runtime::ToolStreamItem::Progress(_) => progress += 1,
                fuigo_tool_runtime::ToolStreamItem::Terminal(r) => {
                    assert!(terminal.is_none(), "more than one Terminal yielded");
                    terminal = Some(r);
                }
    ...
    async fn grep_streaming_body_matches_card_body_when_truncated() {
        ...
        let tool = GrepTool;
        let mut stream =
            fuigo_tool_runtime::Tool::execute(&tool, test_ctx(resources.into_shared()), input).await;
