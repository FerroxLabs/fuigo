...
/// name (e.g. `~/.fuigo/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Fuigo.
...
/// visits known config roots (`.fuigo`, `.agents`, `.claude`, …); those are
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.fuigo is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.fuigo/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.fuigo/ → kept (user content).
        let fuigo_shell = tmp.path().join(".fuigo").join("skills").join("shell");
        std::fs::create_dir_all(&fuigo_shell).unwrap();
        std::fs::write(
            fuigo_shell.join("SKILL.md"),
        ...
        let skills = parse_skill_files(vec![
            ...
            (fuigo_shell.join("SKILL.md"), SkillScope::User),
        ]);
        assert_eq!(skills.len(), 1, "cursor builtin must be dropped");
        assert!(skills[0].path.contains("/.fuigo/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        // A .claude skill and a .fuigo skill in an intermediate dir.
        ...
        .unwrap();
        let fuigo_skill = sub.join(".fuigo").join("skills").join("fuigo-dyn");
        std::fs::create_dir_all(&fuigo_skill).unwrap();
        std::fs::write(fuigo_skill.join("SKILL.md"), "---\nname: fuigo-dyn\n---\n").unwrap();
        ...
        assert!(
            names_on.contains(&"fuigo-dyn"),
            "fuigo-dyn missing: {names_on:?}"
        );
        assert!(
        ...
        // claude.skills OFF → only fuigo-dyn discovered.
        ...
        assert!(
            names_off.contains(&"fuigo-dyn"),
            "fuigo-dyn missing: {names_off:?}"
        );
        assert!(
