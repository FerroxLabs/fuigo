...
//! absolute paths. See the plan document for the full diff vs the fuigo-build
//! `ListDirTool`.
...
impl fuigo_tool_runtime::Tool for CodexListDirTool {
...
mod tests {
    ...
    async fn tool_lists_directory() {
        ...
        let ctx = fuigo_tool_runtime::ToolCallContext::default();
    ...
    async fn empty_directory_returns_success() {
        ...
        let output = fuigo_tool_runtime::Tool::run(&tool, ctx, input)
            .await
            .unwrap();
        match output {
            ListDirOutput::Content(content) => {
