...
//!
//! Reuses the core file-reading logic from [`fuigo_build::read_file::run_read_file`]
...
use crate::implementations::fuigo_build::read_file::{ReadFileInput, run_read_file};
...
impl fuigo_tool_runtime::Tool for HashlineReadTool {
    type Args = ReadFileInput;
    type Output = ReadFileOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("hashline_read").expect("valid tool id")
    }
...
mod tests {
    ...
    fn tool_metadata() {
        ...
        assert_eq!(fuigo_tool_runtime::Tool::id(&tool).as_str(), "hashline_read");
    ...
    /// file content independently of fuigo_build `read_file`).
    #[tokio::test]
