...
impl fuigo_tool_runtime::Tool for SearchTool {
    type Args = SearchToolInput;
    type Output = ToolOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new(SEARCH_TOOL_NAME).expect("valid tool id")
    }
...
mod tests {
    ...
    fn hash_value_pinned_output() {
        ...
        let h = hash_value(&"fuigo-mcp-fingerprint-stability-test");
        assert_eq!(h, hash_value(&"fuigo-mcp-fingerprint-stability-test"));
