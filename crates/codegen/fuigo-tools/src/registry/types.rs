use crate::{
    ...
    implementations::{
        codex, fuigo_build, fuigo_build_concise, fuigo_build_hashline, opencode,
...
/// this crate: instead of `fuigo-tools` referencing an out-of-tree tool
/// pack, the pack calls
...
impl ToolConfig {
    ...
    /// `fuigo_tool_runtime::Tool::id()`. Use this for built-in tools known
    ...
    where
        T: crate::types::tool_metadata::ToolMetadata + fuigo_tool_runtime::Tool + Default,
    {
        Self::from(&T::default())
    }
...
impl<T: crate::types::tool_metadata::ToolMetadata + fuigo_tool_runtime::Tool> From<&T>
    for ToolConfig
{
    fn from(tool: &T) -> Self {
        Self {
...
pub struct SubagentSessionResources {
    pub backend: crate::implementations::fuigo_build::task::backend::SubagentBackendResource,
    ...
    pub event_sender: crate::implementations::fuigo_build::task::types::SubagentEventSender,
    pub depth: crate::implementations::fuigo_build::task::types::SubagentDepthCounter,
    pub session_id: crate::implementations::fuigo_build::task::types::SessionIdResource,
}
...
pub struct SessionContext {
    ...
    pub parent_scheduler_handle:
        Option<crate::implementations::fuigo_build::scheduler::types::SchedulerHandle>,
    ...
    ///
    /// Empty means this registry gives the session a handle that reads and writes nothing. `fuigo-agent` reads
    ...
    pub web_fetch_config: crate::implementations::fuigo_build::web_fetch::WebFetchConfig,
    ...
    /// call the Ferrox Labs Imagine API. When `Disabled` (default), the tool is not
    /// registered and image generation is unavailable.
    pub image_gen_config: crate::implementations::fuigo_build::image_gen::ImageGenConfig,
    ...
    /// call the Ferrox Labs Video Generation API. When `Disabled` (default), the tool is not
    /// registered and video generation is unavailable.
    pub video_gen_config: crate::implementations::fuigo_build::video_gen::VideoGenConfig,
    ...
    pub app_builder_deployer_config:
        crate::implementations::fuigo_build::app_builder::AppBuilderDeployerConfig,
    ...
    /// Auth provider which returns a fuigo_computer_hub_sdk::AuthCredential. Can be used by
    ...
    pub auth_provider: Option<fuigo_computer_hub_sdk::SharedAuthProvider>,
...
pub async fn drain_value_stream(
    mut stream: fuigo_tool_runtime::ToolStream<fuigo_tool_runtime::TypedToolOutput>,
) -> Result<serde_json::Value, fuigo_tool_runtime::ToolError> {
    use futures::StreamExt;
    while let Some(item) = stream.next().await {
        match item {
            fuigo_tool_runtime::ToolStreamItem::Progress(_) => continue,
            fuigo_tool_runtime::ToolStreamItem::Terminal(result) => {
...
fn stream_no_terminal_error() -> fuigo_tool_runtime::ToolError {
...
struct DispatchParts {
    ...
    lr_handle: Arc<dyn fuigo_computer_hub_core::ToolHandle>,
...
struct ToolEntry {
    ...
    parse_input: Box<
        dyn Fn(serde_json::Value) -> Result<ToolInput, fuigo_tool_runtime::ToolError> + Send + Sync,
    >,
    /// Registers this tool into a `LocalRegistry` using the concrete type.
    /// Captured at `register::<T>()` time when T is known.
    register_in_local: Box<dyn Fn(&fuigo_computer_hub_sdk::LocalRegistry) + Send + Sync>,
}
...
pub struct FinalizedToolset {
    ...
    local_registry: fuigo_computer_hub_sdk::LocalRegistry,
    ...
    workspace_viewer_ctx: Option<fuigo_tool_runtime::WorkspaceViewerContext>,
}
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
...
pub struct ToolRegistryBuilder {
    ...
    shared_local_registry: Option<fuigo_computer_hub_sdk::LocalRegistry>,
...
impl ToolRegistryBuilder {
            ...
            + 'static,
    {
        ...
        let id = fuigo_tool_runtime::Tool::id(&tool).as_str().to_string();
        let kind = tool.kind();
        ...
        self.tools.insert(
            ...
            ToolEntry {
                ...
                register_in_local: Box::new(|lr: &fuigo_computer_hub_sdk::LocalRegistry| {
    ...
    /// Fully-qualified tool id (`"FuigoBuild:read_file"`) → declared
    ...
    pub fn new() -> Self {
        ...
        b.register_with_params::<fuigo_build::BashTool, fuigo_build::bash::BashParams>();
        b.register_with_params::<fuigo_build::ReadFileTool, fuigo_build::read_file::ReadFileParams>();
        b.register_with_params::<
                fuigo_build::SearchReplaceTool,
                fuigo_build::search_replace::SearchReplaceParams,
            >();
        b.register_with_params::<fuigo_build::ListDirTool, fuigo_build::list_dir::ListDirParams>();
        b.register_with_params::<fuigo_build::GrepTool, fuigo_build::grep::GrepParams>();
        b.register::<fuigo_build::KillTaskTool>();
        b.register::<fuigo_build::KillTerminalCommandTool>();
        b.register::<fuigo_build::TodoWriteTool>();
        b.register::<fuigo_build::UpdateGoalTool>();
        b.register::<fuigo_build::WorkflowTool>();
        b.register::<fuigo_build::TaskOutputTool>();
        b.register::<fuigo_build::GetTerminalCommandOutputTool>();
        b.register::<fuigo_build::WaitTasksTool>();
        b.register::<fuigo_build::TaskTool>();
        b.register::<fuigo_build::SendSubagentMessageTool>();
        b.register::<fuigo_build::WebSearchTool>();
        b.register_with_params::<fuigo_build::WebFetchTool, fuigo_build::web_fetch::WebFetchParams>();
        b.register::<fuigo_build::LspTool>();
        b.register::<fuigo_build::ImageGenTool>();
        b.register::<fuigo_build::ImageEditTool>();
        b.register::<fuigo_build::ImageToVideoTool>();
        b.register::<fuigo_build::ReferenceToVideoTool>();
        b.register::<fuigo_build::EnterPlanModeTool>();
        b.register::<fuigo_build::ExitPlanModeTool>();
        b.register_with_params::<
                fuigo_build::AskUserQuestionTool,
                fuigo_build::ask_user_question::AskUserQuestionParams,
            >();
        b.register::<fuigo_build::MonitorTool>();
        b.register::<fuigo_build::SchedulerCreateTool>();
        b.register::<fuigo_build::SchedulerDeleteTool>();
        b.register::<fuigo_build::SchedulerListTool>();
        ...
        b.register_with_params::<
            ...
            >();
        b.register_with_params::<
                fuigo_build_concise::ReadFileConciseTool,
                fuigo_build::read_file::ReadFileParams,
            >();
        b.register_with_params::<
                fuigo_build_concise::SearchReplaceConciseTool,
                fuigo_build::search_replace::SearchReplaceParams,
            >();
        b.register_with_params::<
                fuigo_build_concise::BashConciseTool,
                fuigo_build::bash::BashParams,
            >();
        b.register_with_params::<
                fuigo_build_hashline::HashlineReadTool,
                fuigo_build_hashline::config::HashlineSchemeParams,
            >();
        b.register_with_params::<
                fuigo_build_hashline::HashlineEditTool,
                fuigo_build_hashline::config::HashlineSchemeParams,
            >();
        b.register_with_params::<
                fuigo_build_hashline::HashlineGrepTool,
                fuigo_build_hashline::config::HashlineSchemeParams,
            >();
    ...
    pub fn with_local_registry(mut self, registry: fuigo_computer_hub_sdk::LocalRegistry) -> Self {
    ...
    pub fn validate_config(&self, config: &ToolServerConfig) -> Vec<RequirementError> {
        ...
        }
        {
            let standard_file_ids: &[&str] = &[
                "FuigoBuild:read_file",
                "FuigoBuild:search_replace",
                "FuigoBuild:grep",
            ];
            let hashline_file_ids: &[&str] = &[
                "FuigoBuildHashline:hashline_read",
                "FuigoBuildHashline:hashline_edit",
                "FuigoBuildHashline:hashline_grep",
            ];
    ...
    pub fn finalize_with_trunc_config(
        ...
        if image_gen_config.has_credentials() {
            match crate::implementations::fuigo_build::image_gen::ImageGenClient::new(
                &image_gen_config,
                ctx.api_key_provider.clone(),
            ) {
                Ok(client) => {
        ...
        if video_gen_config.is_enabled() {
            match crate::implementations::fuigo_build::video_gen::VideoGenClient::new(
                &video_gen_config,
                ctx.api_key_provider.clone(),
            ) {
                Ok(client) => {
        ...
        if let crate::implementations::fuigo_build::web_fetch::WebFetchConfig::Enabled { params } =
            &ctx.web_fetch_config
        {
            match crate::implementations::fuigo_build::web_fetch::WebFetchClient::new(params) {
                Ok(client) => {
                    resources.insert(client);
                }
                Err(e) => {
        ...
        let concise_ns = crate::types::tool::ToolNamespace::FuigoBuildConcise.to_string();
        ...
        resources.register_state::<crate::implementations::fuigo_build::todo::TodoState>();
        ...
        resources
            ...
            >();
        resources
            .register_state::<crate::implementations::fuigo_build::scheduler::types::SchedulerState>(
            );
        ...
        for tool_config in &config.tools {
            ...
            if let Some(desc) = &definition.function.description {
                definition.function.description = Some(truncation_config.interpolate_description(
                    ...
                    fuigo_tool_types::max_wait_block_ms(),
                ));
            }
        ...
        let (scheduler_cmd_rx, scheduler_cancel_token) =
            if let Some(parent_handle) = ctx.parent_scheduler_handle {
                ...
                resources.insert(
                    crate::implementations::fuigo_build::scheduler::types::SchedulerHandle(
                        scheduler_cmd_tx,
                    ),
                );
        ...
        if let (Some(cmd_rx), Some(cancel_token)) = (scheduler_cmd_rx, &scheduler_cancel_token) {
            let actor = crate::implementations::fuigo_build::scheduler::actor::SchedulerActor {
...
///
/// Implements the canonical `fuigo_tool_runtime::ToolDispatch` trait so the
...
#[async_trait::async_trait]
impl fuigo_tool_runtime::ToolDispatch for InnerDispatchForToolset {
    async fn call(
        &self,
        tool_id: fuigo_tool_protocol::ToolId,
        args: serde_json::Value,
        ctx: fuigo_tool_runtime::ToolCallContext,
    ) -> fuigo_tool_runtime::ToolStream<fuigo_tool_runtime::TypedToolOutput> {
        let result = self
            .toolset
            ...
            .and_then(|output| {
                let value = serde_json::to_value(&output).map_err(|e| {
                    fuigo_tool_runtime::ToolError::custom("output_encoding", e.to_string())
                })?;
                Ok(fuigo_tool_runtime::TypedToolOutput::from_value(
                    tool_id.clone(),
                    value,
                ))
            });
        fuigo_tool_runtime::terminal_only(result)
    }
}
impl FinalizedToolset {
    ...
    pub fn empty_for_test() -> Self {
        Self {
            ...
            local_registry: fuigo_computer_hub_sdk::LocalRegistry::new(),
    ...
    pub fn local_registry(&self) -> &fuigo_computer_hub_sdk::LocalRegistry {
        &self.local_registry
    }
    ...
    fn tool_not_found_error(tool_name: &str) -> fuigo_tool_runtime::ToolError {
        let tid = fuigo_tool_protocol::ToolId::new(tool_name)
            .unwrap_or_else(|_| fuigo_tool_protocol::ToolId::new("unknown").expect("valid"));
        fuigo_tool_runtime::ToolError::not_found(tid, format!("Tool not found: {tool_name}"))
    }
    pub async fn try_parse(
        &self,
        tool_name: &str,
        tool_params: &serde_json::Value,
    ) -> Result<ToolInput, fuigo_tool_runtime::ToolError> {
    ...
    async fn call_raw(
        ...
        parent_ctx: fuigo_tool_runtime::ToolCallContext,
    ) -> Result<crate::types::output::ToolOutput, fuigo_tool_runtime::ToolError> {
        ...
        let mut ctx = fuigo_tool_runtime::ToolCallContext::new(parent_ctx.call_id.clone());
        ctx.extensions.insert(self.resources.clone());
        ctx.extensions.insert_arc(Arc::clone(&self.renderer));
        if let Some(cancellation) = parent_ctx.get::<fuigo_tool_runtime::Cancellation>() {
        ...
        let lr_handle = self.local_registry.find(&tool_id).ok_or_else(|| {
            fuigo_tool_runtime::ToolError::not_found(
                tool_id,
        ...
        (output_converter)(value)
            .map_err(|e| fuigo_tool_runtime::ToolError::custom("output_decoding", e.to_string()))
    }
    ...
    pub async fn call_with_cancellation(
        ...
        while let Some(item) = stream.next().await {
            match item {
                fuigo_tool_runtime::ToolStreamItem::Progress(_) => continue,
                fuigo_tool_runtime::ToolStreamItem::Terminal(result) => return result,
            }
        }
        Err(stream_no_terminal_error())
    }
    ...
    ///
    /// [`ToolStream`]: fuigo_tool_runtime::ToolStream
    /// [`ToolStreamItem::Progress`]: fuigo_tool_runtime::ToolStreamItem::Progress
    ...
    pub fn call_streaming_with_cancellation(
        ...
        Box::pin(async_stream::stream! {
            let parts = match this.prepare_dispatch(
                ...
                Ok(parts) => parts,
                Err(e) => {
                    yield fuigo_tool_runtime::ToolStreamItem::Terminal(Err(e));
                    return;
                }
            };
            let DispatchParts {
            ...
            while let Some(item) = inner.next().await {
                match item {
                    ...
                    fuigo_tool_runtime::ToolStreamItem::Terminal(Err(e)) => {
                        yield fuigo_tool_runtime::ToolStreamItem::Terminal(Err(e));
                        return;
                    }
                    fuigo_tool_runtime::ToolStreamItem::Terminal(Ok(typed)) => {
                        let run_result = this
                            .finalize_output(typed.value, &output_converter, effective_tool_name)
                            .await;
                        yield fuigo_tool_runtime::ToolStreamItem::Terminal(run_result);
                        return;
                    }
                }
            }
            yield fuigo_tool_runtime::ToolStreamItem::Terminal(Err(stream_no_terminal_error()));
        })
    }
    ...
    fn prepare_dispatch(
        ...
        let rt_call_id = fuigo_tool_protocol::ToolCallId::new(tool_call_id)
            .unwrap_or_else(|_| fuigo_tool_protocol::ToolCallId::new_v7());
        let mut ctx = fuigo_tool_runtime::ToolCallContext::new(rt_call_id);
        ctx.extensions.insert(self.resources.clone());
        ctx.extensions.insert_arc(Arc::clone(&self.renderer));
        ctx.extensions.insert(
            crate::types::resources::InvokingToolParamNames::from_reverse_params(&reverse_params),
        ...
        if let Some(cwd) = cwd_override {
            ctx.extensions.insert(fuigo_tool_runtime::Cwd(cwd));
        }
        ...
        if let Some(ref version) = contract_version {
            ctx.extensions
                .insert(fuigo_tool_runtime::BehaviorVersion(version.clone()));
        }
    ...
    ///
    /// The tool must implement `fuigo_tool_runtime::Tool + ToolMetadata`.
    ...
    where
        T: fuigo_tool_runtime::Tool + ToolMetadata + std::fmt::Debug + Send + Sync + 'static,
        T::Output: serde::Serialize,
    {
        ...
        let kind = tool.kind();
        let registry_id = fuigo_tool_runtime::Tool::id(&tool).as_str().to_owned();
    ...
    pub fn unregister_tools_by_prefix(&self, prefix: &str) -> usize {
        ...
        let to_remove: Vec<_> = tools
            ...
            .filter_map(|t| fuigo_tool_protocol::ToolId::new(&t.registry_id).ok())
            .collect();
    ...
    pub fn unregister_tool_by_name(&self, name: &str) -> bool {
        ...
        let tool_id = tools
            ...
            .and_then(|t| fuigo_tool_protocol::ToolId::new(&t.registry_id).ok());
        let before = tools.len();
...
fn explain_requirement_failure(
    ...
    match fq_tool_id.as_str() {
        ...
        "FuigoBuild:get_task_output" => {
            let has_fuigo_build_bash = has_tool_with_bool_param(
                proposed,
                "FuigoBuild",
                "run_terminal_cmd",
                "enabled_background",
            ...
            let has_fuigo_build_concise_bash = has_tool_with_bool_param(
                proposed,
                "FuigoBuildConcise",
                "run_terminal_cmd",
                "enabled_background",
            ...
            let has_task = has_tool(proposed, "FuigoBuild", "task");
            let mut notes = vec![];
            if has_tool(proposed, "FuigoBuild", "run_terminal_cmd")
                && !has_fuigo_build_bash
            {
                notes
                    .push(
                        "FuigoBuild:run_terminal_cmd is present but enabled_background=false",
                    );
            }
            if has_tool(proposed, "FuigoBuildConcise", "run_terminal_cmd")
                && !has_fuigo_build_concise_bash
            {
                notes
                    .push(
                        "FuigoBuildConcise:run_terminal_cmd is present but enabled_background=false",
                    );
            }
            let mut message = "get_task_output requires a background-capable bash tool (FuigoBuild:run_terminal_cmd or FuigoBuildConcise:run_terminal_cmd with enabled_background=true), OpenCode:bash, or FuigoBuild:task"
                .to_string();
            let has_provider = has_fuigo_build_bash || has_fuigo_build_concise_bash
            ...
            RequirementError::new(fq_tool_id, message)
                .with_field_path("tools")
                .with_expected(
                    "include a background-capable bash tool, OpenCode:bash, or FuigoBuild:task",
                )
                .with_category("requirements")
        }
        "FuigoBuild:search_replace" if !params
            ...
            RequirementError::new(
                ...
                .with_expected(
                    "set skip_read_before_edit=true or include a Read tool such as FuigoBuild:read_file",
                )
                .with_bad_value(serde_json::Value::Bool(false))
                .with_category("requirements")
        }
        "FuigoBuild:enter_plan_mode" => {
            RequirementError::new(
                    fq_tool_id,
                    "enter_plan_mode requires FuigoBuild:exit_plan_mode so plan mode can always be exited",
                )
                .with_field_path("tools")
                .with_expected("include FuigoBuild:exit_plan_mode")
                .with_category("requirements")
        }
        "FuigoBuild:exit_plan_mode" => {
            RequirementError::new(
                    fq_tool_id,
                    "exit_plan_mode requires FuigoBuild:enter_plan_mode so plan mode can be entered before exiting",
                )
                .with_field_path("tools")
                .with_expected("include FuigoBuild:enter_plan_mode")
                .with_category("requirements")
        }
        _ => {
...
mod tests {
    ...
    fn test_session_context(tmp: &TempDir) -> SessionContext {
        SessionContext {
            ...
            web_fetch_config:
                crate::implementations::fuigo_build::web_fetch::WebFetchConfig::default(),
            lsp: None,
            image_gen_config:
                crate::implementations::fuigo_build::image_gen::ImageGenConfig::default(),
            video_gen_config:
                crate::implementations::fuigo_build::video_gen::VideoGenConfig::default(),
            app_builder_deployer_config:
                crate::implementations::fuigo_build::app_builder::AppBuilderDeployerConfig::default(),
    ...
    /// appeared before `fuigo_build:search_replace` (`ToolKind::Edit`, input:
    ...
    async fn kind_params_merged_across_multiple_tools_of_same_kind() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "FuigoBuild:search_replace".to_string(),
                    params: Some(
    ...
    async fn search_replace_description_renders_minimum_anchor_guidance() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig {
                    id: "FuigoBuild:read_file".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
    ...
    /// Smoke test: finalize the full FuigoBuild toolset and verify every
    ...
    async fn full_toolset_descriptions_render_cleanly() {
        use crate::implementations::fuigo_build::{
        ...
        let config = ToolServerConfig {
            ...
            .into_iter()
            .map(|id| ToolConfig::from_id(format!("FuigoBuild:{id}")))
    ...
    /// name: `run_terminal_cmd` (fuigo).
    #[tokio::test]
    async fn tool_name_for_kind_resolves_execute() {
        use crate::types::tool::ToolKind;
        let tmp = TempDir::new().unwrap();
        let fuigo = ToolRegistryBuilder::new()
            .finalize(
                ToolServerConfig {
                    tools: vec![
                        ToolConfig::from_id("FuigoBuild:run_terminal_cmd".to_string()),
                        ToolConfig::from_id("FuigoBuild:get_task_output".to_string()),
                        ToolConfig::from_id("FuigoBuild:kill_task".to_string()),
                    ],
                    behavior_preset: None,
                },
                test_session_context(&tmp),
            )
            .expect("fuigo toolset should finalize");
        assert_eq!(
            fuigo.tool_name_for_kind(ToolKind::Execute).as_deref(),
    ...
    fn capabilities_is_read_only_matches_metadata() {
        ...
        let mismatches: Vec<String> = builder
            ...
            .filter_map(|(name, entry)| {
                let lr = fuigo_computer_hub_sdk::LocalRegistry::new();
                (entry.register_in_local)(&lr);
                let id = fuigo_tool_protocol::ToolId::new(&entry.id)
    ...
    fn tool_config_kind_sinks_unknown_to_other() {
        ...
        let known = parse(serde_json::json!({"id": "FuigoBuild:read_file", "kind": "read"}));
        assert_eq!(known.kind, Some(ToolKind::Read));
        let typo = parse(serde_json::json!({"id": "FuigoBuild:read_file", "kind": "raed"}));
        assert_eq!(typo.kind, Some(ToolKind::Other));
        let absent = parse(serde_json::json!({"id": "FuigoBuild:read_file"}));
    ...
    async fn bash_descriptions_track_system_reminders_setting() {
        ...
        let ids = [
            "FuigoBuild:run_terminal_cmd",
            "FuigoBuild:get_task_output",
            "FuigoBuild:kill_task",
        ];
        let tmp = TempDir::new().unwrap();
        let toolset = ToolRegistryBuilder::new()
    ...
    /// Verify FuigoBuildConcise tools can be finalized and produce concise output.
    #[tokio::test]
    async fn test_concise_namespace_tools() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig {
                    id: "FuigoBuildConcise:read_file".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
                ...
                ToolConfig {
                    id: "FuigoBuildConcise:search_replace".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
                ...
                ToolConfig {
                    id: "FuigoBuildConcise:run_terminal_cmd".to_string(),
                    params: Some(
                ...
                ToolConfig::for_tool::<fuigo_build::GrepTool>(),
                ToolConfig::for_tool::<fuigo_build::KillTaskTool>(),
                ToolConfig::for_tool::<fuigo_build::TaskOutputTool>(),
                ToolConfig {
                    id: "FuigoBuild:list_dir".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
    ...
    fn has_tool_id_knows_pinned_tool_config_ids() {
        ...
        for id in [
            ...
            "FuigoBuild:list_dir",
            "FuigoBuild:grep",
            "FuigoBuild:get_terminal_command_output",
            "FuigoBuild:kill_terminal_command",
        ] {
            assert!(
        ...
        assert!(
            !builder.has_tool_id("FuigoBuild:does_not_exist"),
    ...
    fn known_tool_kinds_maps_pinned_tool_config_ids() {
        ...
        for (id, expected) in [
            ("FuigoBuild:run_terminal_cmd", ToolKind::Execute),
            ("FuigoBuild:read_file", ToolKind::Read),
            ("FuigoBuild:search_replace", ToolKind::Edit),
            ("FuigoBuild:grep", ToolKind::Search),
            ("FuigoBuild:list_dir", ToolKind::List),
        ] {
            assert_eq!(
                kinds.get(id),
                Some(&expected),
        ...
        assert!(
            !kinds.contains_key("FuigoBuild:does_not_exist"),
    ...
    /// (e.g. `"read_file"`). If both `FuigoBuild:read_file` and
    ...
    fn validate_config_reports_param_field_path_and_value() {
        ...
        let config = ToolServerConfig {
            tools: vec![ToolConfig {
                id: "FuigoBuild:run_terminal_cmd".to_string(),
                params: Some(
        ...
        let error = &errors[0];
        assert_eq!(error.tool, "FuigoBuild:run_terminal_cmd");
        assert_eq!(
    ...
    fn validate_config_reports_semantic_hashline_param_error() {
        ...
        let config = ToolServerConfig {
            tools: vec![ToolConfig {
                id: "FuigoBuildHashline:hashline_read".to_string(),
                params: Some(
                    serde_json::from_value(serde_json::json!({
                        "hash_len": 0
                    }))
    ...
    impl fuigo_tool_runtime::Tool for FakeMcpTool {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> fuigo_tool_protocol::ToolId {
            fuigo_tool_protocol::ToolId::new("fake_mcp").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::fuigo_tool_runtime::ListToolsContext,
        ) -> fuigo_tool_types::ToolDescription {
            fuigo_tool_types::ToolDescription::new("fake_mcp", &self.description)
        }
        async fn run(
            &self,
    ...
    impl fuigo_tool_runtime::Tool for NonStreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> fuigo_tool_protocol::ToolId {
            fuigo_tool_protocol::ToolId::new("non_streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::fuigo_tool_runtime::ListToolsContext,
        ) -> fuigo_tool_types::ToolDescription {
            fuigo_tool_types::ToolDescription::new("non_streaming_stub", "non-streaming stub")
        }
        async fn run(
            &self,
            _ctx: fuigo_tool_runtime::ToolCallContext,
    ...
    impl fuigo_tool_runtime::Tool for StreamingStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> fuigo_tool_protocol::ToolId {
            fuigo_tool_protocol::ToolId::new("streaming_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::fuigo_tool_runtime::ListToolsContext,
        ) -> fuigo_tool_types::ToolDescription {
            fuigo_tool_types::ToolDescription::new("streaming_stub", "streaming stub")
        }
        async fn run(
            &self,
            _ctx: fuigo_tool_runtime::ToolCallContext,
        ...
        async fn execute(
            ...
            _ctx: fuigo_tool_runtime::ToolCallContext,
            _input: serde_json::Value,
        ) -> fuigo_tool_runtime::ToolStream<String> {
            Box::pin(futures::stream::iter(vec![
                fuigo_tool_runtime::ToolStreamItem::Progress(fuigo_tool_runtime::ToolProgress::Text {
                    text: "progress-1".into(),
                }),
                fuigo_tool_runtime::ToolStreamItem::Terminal(Ok("terminal-value".to_string())),
            ]))
        }
    }
        ...
        while let Some(item) = stream.next().await {
            match item {
                fuigo_tool_runtime::ToolStreamItem::Progress(p) => {
                    ...
                    assert!(matches!(p, fuigo_tool_runtime::ToolProgress::Text { .. }));
                    progress_count += 1;
                }
                fuigo_tool_runtime::ToolStreamItem::Terminal(result) => {
    ...
    impl fuigo_tool_runtime::Tool for NoTerminalStub {
        type Args = serde_json::Value;
        type Output = String;
        fn id(&self) -> fuigo_tool_protocol::ToolId {
            fuigo_tool_protocol::ToolId::new("no_terminal_stub").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::fuigo_tool_runtime::ListToolsContext,
        ) -> fuigo_tool_types::ToolDescription {
            fuigo_tool_types::ToolDescription::new("no_terminal_stub", "no-terminal stub")
        }
        async fn run(
            &self,
            _ctx: fuigo_tool_runtime::ToolCallContext,
    ...
    async fn non_pi_finalized_contract_snapshot_is_unchanged() {
        ...
        let toolset = ToolRegistryBuilder::new()
            .finalize(
                ToolServerConfig {
                    tools: vec![
                        ToolConfig::for_tool::<fuigo_build::TodoWriteTool>(),
...
"##,
    ...
    async fn tool_definitions_builtins_only_hides_mcp_tools() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig::for_tool::<fuigo_build::ReadFileTool>(),
                ToolConfig::for_tool::<fuigo_build::GrepTool>(),
            ],
            behavior_preset: None,
        };
    ...
    fn task_tool_rejected_without_get_task_output_and_kill_task() {
        ...
        assert!(
            errors.iter().any(|e| e.tool == "FuigoBuild:task"
                && e.message.contains("FuigoBuild:get_task_output")
                && e.message.contains("FuigoBuild:kill_task")),
    ...
    fn task_tool_rejected_with_only_get_task_output() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "FuigoBuild:get_task_output".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
        ...
        assert!(
            errors
                .iter()
                .any(|e| e.tool == "FuigoBuild:task" && e.message.contains("FuigoBuild:kill_task")),
    ...
    fn task_tool_rejected_with_only_kill_task() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "FuigoBuild:kill_task".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
    ...
    /// fuigo-build agent config (all tools present) and that the new examples
    ...
    async fn background_param_templates_reference_real_schema_keys() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                tool("FuigoBuild:run_terminal_cmd"),
                tool("FuigoBuild:task"),
                tool("FuigoBuild:get_task_output"),
                tool("FuigoBuild:wait_tasks"),
                tool("FuigoBuild:kill_task"),
            ],
            behavior_preset: None,
        };
        let tmp = TempDir::new().unwrap();
    ...
    fn bash_with_enabled_background_reports_missing_task_tools() {
        ...
        assert!(error.message.contains("FuigoBuild:get_task_output"));
        assert!(error.message.contains("FuigoBuild:kill_task"));
        assert!(
            error
                .expected
                .as_deref()
    ...
    fn task_requirement_error_lists_missing_background_tools() {
        ...
        let error = &errors[0];
        assert_eq!(error.tool, "FuigoBuild:task");
        assert!(error.message.contains("FuigoBuild:get_task_output"));
        assert!(error.message.contains("FuigoBuild:kill_task"));
    ...
    fn search_replace_requirement_error_mentions_read_tool() {
        ...
        let error = errors
            .iter()
            .find(|error| error.tool == "FuigoBuild:search_replace")
    ...
    fn ask_user_question_validates_standalone() {
        ...
        let config = ToolServerConfig {
            tools: vec![ToolConfig {
                id: "FuigoBuild:ask_user_question".to_string(),
                params: None,
                name_override: None,
                params_name_overrides: None,
                description_override: None,
    ...
    async fn task_description_renders_with_default_agent_config() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ...
                ToolConfig {
                    id: "FuigoBuild:web_search".to_string(),
                    params: None,
                    name_override: None,
                    params_name_overrides: None,
                    description_override: None,
        ...
        let toolset = builder
            .finalize(config, ctx)
            .expect("finalize should succeed with default fuigo-build tools");
        let defs = toolset.tool_definitions();
        let task_def = defs
            .iter()
    ...
    fn hashline_tools_registered_in_builder() {
        ...
        assert!(
            builder
                .tools
                .contains_key("FuigoBuildHashline:hashline_read"),
            "hashline_read should be registered"
        );
        assert!(
            builder
                .tools
                .contains_key("FuigoBuildHashline:hashline_edit"),
            "hashline_edit should be registered"
        );
        assert!(
            builder
                .tools
                .contains_key("FuigoBuildHashline:hashline_grep"),
    ...
    async fn hashline_config_finalizes_successfully() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                hashline_tool_config("FuigoBuildHashline:hashline_read"),
                hashline_tool_config("FuigoBuildHashline:hashline_edit"),
                hashline_tool_config("FuigoBuildHashline:hashline_grep"),
            ],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
    ...
    async fn standard_and_hashline_each_finalize_independently() {
        ...
        let standard_config = ToolServerConfig {
            tools: vec![
                hashline_tool_config("FuigoBuild:read_file"),
                hashline_tool_config("FuigoBuild:search_replace"),
                hashline_tool_config("FuigoBuild:grep"),
            ],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
    ...
    async fn empty_struct_schema_has_properties_and_required() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig::for_tool::<fuigo_build::EnterPlanModeTool>(),
                ToolConfig::for_tool::<fuigo_build::ExitPlanModeTool>(),
            ],
            behavior_preset: None,
        };
        let ctx = test_session_context(&tmp);
    ...
    async fn hashline_toolset_e2e_with_utilities() {
        ...
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig {
                    id: "FuigoBuildHashline:hashline_read".to_owned(),
                    params: Some(
                ...
                ToolConfig::for_tool::<fuigo_build_hashline::HashlineEditTool>(),
                ToolConfig::for_tool::<fuigo_build_hashline::HashlineGrepTool>(),
                ToolConfig::for_tool::<fuigo_build::ListDirTool>(),
            ],
            behavior_preset: None,
        };
        let errors = builder.validate_config(&config);
    ...
    fn bash_config_with_background() -> ToolConfig {
        ToolConfig {
            id: "FuigoBuild:run_terminal_cmd".to_owned(),
            params: Some(
                serde_json::json!({ "enabled_background": true })
                    .as_object()
                    .unwrap()
    ...
    async fn fuigo_build_bridge(tmp: &TempDir) -> crate::bridge::ToolBridge {
        let builder = ToolRegistryBuilder::new();
        let config = ToolServerConfig {
            tools: vec![
                ToolConfig::for_tool::<fuigo_build::ListDirTool>(),
                ToolConfig::for_tool::<fuigo_build::ReadFileTool>(),
                ToolConfig::for_tool::<fuigo_build::SearchReplaceTool>(),
    ...
    async fn hub_dispatch_list_dir() {
        ...
        let bridge = fuigo_build_bridge(&tmp).await;
        let result = bridge
            .call(
                "list_dir",
    ...
    async fn hub_dispatch_parity_list_dir() {
        ...
        let hub_bridge = fuigo_build_bridge(&tmp).await;
        ...
        let builder = ToolRegistryBuilder::new();
        let config = ToolServerConfig {
            tools: vec![ToolConfig::for_tool::<fuigo_build::ListDirTool>()],
            behavior_preset: None,
        };
    ...
    fn generate_schema_strips_root_title_and_description() {
        let schema = generate_schema::<crate::implementations::fuigo_build::bash::BashToolInput>();
        assert!(
    ...
    fn toolset_with_viewer_ctx(
        viewer_ctx: Option<fuigo_tool_runtime::WorkspaceViewerContext>,
    ...
    async fn prepare_dispatch_stamps_workspace_viewer_ctx_when_present() {
        let (toolset, _tmp) =
            toolset_with_viewer_ctx(Some(fuigo_tool_runtime::WorkspaceViewerContext {
    ...
    async fn bash_streaming_progress_emitted_via_finalized_toolset_when_gate_on() {
        ...
        let toolset = Arc::new(
            builder
                .finalize_with_trunc_config(
                    ...
                    ctx,
                    crate::types::context::TruncationConfig::default(),
                    Some(fuigo_tool_runtime::WorkspaceViewerContext {
                        stream_tool_progress: true,
                    }),
                )
                .expect("finalize"),
        ...
        while let Some(item) = stream.next().await {
            match item {
                fuigo_tool_runtime::ToolStreamItem::Progress(_) => progress_count += 1,
