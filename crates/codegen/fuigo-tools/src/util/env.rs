...
//!
//! All implementations now live in the lightweight [`fuigo_tty_utils`] crate
//! so that every crate in the workspace can use them without pulling in the
//! heavyweight `fuigo-tools` dependency. This module re-exports the public
//! API for backward compatibility.

pub use fuigo_tty_utils::{detach_from_tty, pager_env};
...
/// Note: the CLI also uses `FUIGO_AGENT` as an
/// optional agent-definition selector for launching `fuigo` itself; child terminal
/// processes only need the sentinel value `"1"`.
pub const FUIGO_AGENT_ENV: &str = "FUIGO_AGENT";

/// Sentinel value for [`FUIGO_AGENT_ENV`] on agent tool terminals.
pub const FUIGO_AGENT_ENV_VALUE: &str = "1";

/// Force `FUIGO_AGENT=1` on an agent terminal child so request/login env cannot
/// clear the agent marker.
pub fn apply_fuigo_agent_marker(cmd: &mut tokio::process::Command) {
    cmd.env(FUIGO_AGENT_ENV, FUIGO_AGENT_ENV_VALUE);
}

/// Expand the four plugin-path tokens (`${CLAUDE_PLUGIN_ROOT}` / `${FUIGO_PLUGIN_ROOT}`
/// and `${CLAUDE_PLUGIN_DATA}` / `${FUIGO_PLUGIN_DATA}`) in `s`. Each pair is expanded
...
pub fn substitute_plugin_tokens(
    ...
    if let Some(root) = plugin_root {
        out = out
            ...
            .replace("${FUIGO_PLUGIN_ROOT}", root);
    }
    if let Some(data) = plugin_data {
        out = out
            .replace("${CLAUDE_PLUGIN_DATA}", data)
            .replace("${FUIGO_PLUGIN_DATA}", data);
    }
    out
}
...
#[cfg(test)]
mod tests {
    use super::{FUIGO_AGENT_ENV, FUIGO_AGENT_ENV_VALUE, substitute_plugin_tokens};

    const ALL_TOKENS: &str = "${CLAUDE_PLUGIN_ROOT}/a ${FUIGO_PLUGIN_ROOT}/b ${CLAUDE_PLUGIN_DATA}/c ${FUIGO_PLUGIN_DATA}/d";
    ...
    fn expands_only_root_when_data_none() {
        ...
        assert_eq!(
            out,
            "/root/a /root/b ${CLAUDE_PLUGIN_DATA}/c ${FUIGO_PLUGIN_DATA}/d"
        );
    }

    #[test]
    fn agent_marker_constants_match_cursor_parity() {
        assert_eq!(FUIGO_AGENT_ENV, "FUIGO_AGENT");
        assert_eq!(FUIGO_AGENT_ENV_VALUE, "1");
    }
}
