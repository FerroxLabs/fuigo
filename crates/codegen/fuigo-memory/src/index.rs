...
impl MemoryIndex {
    ...
    /// `fuigo memory doctor` uses this to detect stuck states.
    ...
    ///
    /// `fuigo memory doctor` uses this to detect orphaned chunks (chunks whose source file has since been deleted).
...
mod tests {
    ...
    fn test_open_or_create_uses_wal_on_local_fs() {
        // FUIGO_SQLITE_JOURNAL_MODE overrides the journal-mode decision; skip when it is set
        if std::env::var("FUIGO_SQLITE_JOURNAL_MODE").is_ok() {
            return;
        }
        let tmp = TempDir::new().unwrap();
    ...
    /// 3. Run the same orphan-removal logic as `fuigo memory reindex`:
    ...
    ///
    /// This proves that `fuigo memory reindex`'s Phase 1 fixes the state that `fuigo memory doctor` warns about.
    #[test]
    fn test_reindex_maintenance_removes_orphaned_chunks() {
        ...
        // Simulate `fuigo memory reindex` Phase 1: compare indexed vs current.
    ...
    ///
    /// Verifies that `fuigo memory reindex` Phase 0 bails when a live session holds a fresh claim.
    ...
    /// `fuigo memory reindex` Phase 3 resets the stale reindex claim.
    ///
    /// Verifies that `release_claim()` clears `meta.reindex_claim` so that `fuigo memory doctor` no longer reports a stale lock after reindex runs.
    #[test]
    fn test_reindex_maintenance_resets_stale_claim() {
        ...
        // Simulate `fuigo memory reindex` Phase 3: release the claim.
        idx.release_claim();
