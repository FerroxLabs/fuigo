...
pub(crate) async fn require_bearer(auth: &SharedVoiceAuth) -> Result<String, VoiceError> {
    auth.bearer().await.ok_or_else(|| {
        VoiceError::Auth(
            "not signed in — run `fuigo login`, set FUIGO_API_KEY, or set a model api_key/env_key"
                .into(),
        )
    })
}
...
/// Used by the standalone `voice-probe` binary and tests, where there is no `AuthManager`, only a raw `FUIGO_API_KEY`.
