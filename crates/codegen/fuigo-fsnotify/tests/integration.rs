...
async fn shared_dedupes_by_directory() {
    ...
    let Ok(a) = fuigo_fsnotify::shared(path.clone(), FsConfig::default()) else {
        eprintln!("skipping: OS watcher unavailable (resource limit?)");
        return;
    };
    let before = fuigo_fsnotify::stats();
    let b = fuigo_fsnotify::shared(path.clone(), FsConfig::default()).unwrap();
    ...
    let after = fuigo_fsnotify::stats();
    assert_eq!(
    ...
    let c = fuigo_fsnotify::shared(other.path().to_path_buf(), FsConfig::default()).unwrap();
    assert!(!Arc::ptr_eq(&a, &c), "different dirs must not share");
    assert_eq!(
        fuigo_fsnotify::stats().created_total - after.created_total,
        1,
    ...
    drop(a);
    drop(b);
    let d = fuigo_fsnotify::shared(path, FsConfig::default()).unwrap();
    assert_eq!(
        Arc::strong_count(&d),
        1,
