...
/// Per-dir on Linux and fan-out elsewhere, overridden by `FUIGO_FSNOTIFY_PER_DIR`.
pub(crate) fn watch_strategy() -> WatchStrategy {
    match std::env::var("FUIGO_FSNOTIFY_PER_DIR").ok().as_deref() {
...
/// Per-dir mode's total watch budget (`FUIGO_FSNOTIFY_MAX_WATCHES` overrides).
const DEFAULT_MAX_WATCHES: usize = 49_152;

pub(crate) fn max_watch_budget() -> usize {
    std::env::var("FUIGO_FSNOTIFY_MAX_WATCHES")
        .ok()
