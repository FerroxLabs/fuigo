...
use fuigo_tracing_macros::teprintln;
...
mod helper_tests {
    ...
    fn external_ancestor_sl_arms_in_recursive_root_mode() {
        ...
        // (e.g. `fuigo` run in `crates/codegen`): the production guard must
    ...
    fn sapling_enabled_respects_kill_switch() {
        ...
        impl Drop for Restore {
            fn drop(&mut self) {
                ...
                unsafe { std::env::remove_var("FUIGO_FSNOTIFY_SAPLING") };
            }
        }
        ...
        for off in ["0", "false"] {
            unsafe { std::env::set_var("FUIGO_FSNOTIFY_SAPLING", off) };
            assert!(!sapling_enabled(), "{off:?} must disable Sapling");
        }
        unsafe { std::env::set_var("FUIGO_FSNOTIFY_SAPLING", "1") };
