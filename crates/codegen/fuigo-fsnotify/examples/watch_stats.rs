...
//! startup latency, under either strategy (`FUIGO_FSNOTIFY_PER_DIR=0|1`).
//!
//! ```bash
//! # Generate a synthetic tree, then measure both strategies against it:
//! cargo run --release -p fuigo-fsnotify --example watch_stats -- gen js /tmp/js-repo
//! FUIGO_FSNOTIFY_PER_DIR=0 cargo run --release -p fuigo-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! FUIGO_FSNOTIFY_PER_DIR=1 cargo run --release -p fuigo-fsnotify --example watch_stats -- run /tmp/js-repo 5
//! ```
//!
...
//!   "fuigo holds 55k inotify watches" report).
...
use fuigo_fsnotify::{FsConfig, FsEventSource};
...
fn main() {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("run") => {
            ...
            let strategy = std::env::var("FUIGO_FSNOTIFY_PER_DIR").unwrap_or_default();
