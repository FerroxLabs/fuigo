...
/// This is a private copy of `fuigo_workspace::util::is_lock_contended`, so this crate does not depend on `fuigo-workspace`.
...
///
/// This is far below the server's own SIGTERM drain budget (`FUIGO_WORKSPACE_TERMINATION_GRACE_MS`, default 45s) on purpose.
...
mod tests {
    ...
    fn process_name_matches_own_argv0() {
        ...
        // Different test runners name the binary differently (e.g. Cargo uses `fuigo_workspace_daemon-<hash>`).
    ...
    fn basename_contains_ignores_directory_components() {
        assert!(basename_contains(
            "/usr/local/bin/fuigo-workspace-server",
            "workspace-server"
        ));
        assert!(basename_contains(
            "C:\\Program Files\\FUIGO-Workspace-Server.exe",
            "workspace-server"
        ));
        assert!(
            !basename_contains(
