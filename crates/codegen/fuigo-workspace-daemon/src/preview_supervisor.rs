...
//!
//! After the workspace-server self-daemonizes (see [`crate::daemonize`]) it spawns the unchanged `/usr/local/bin/fuigo-preview-proxy` binary.
...
pub const PREVIEW_PROXY_BIN_PATH: &str = "/usr/local/bin/fuigo-preview-proxy";
...
/// `fuigo_workspace_preview_proxy_restart_total{reason}` counts the (re)start events the in-sandbox supervisor emits, by reason.
...
static PREVIEW_PROXY_RESTART_TOTAL: LazyLock<IntCounterVec> = LazyLock::new(|| {
    register_int_counter_vec!(
        "fuigo_workspace_preview_proxy_restart_total",
...
impl PreviewArgs {
    /// Map the forwarded fields to the proxy's exact CLI flag names (see `fuigo-preview-proxy/src/cli.rs`).
...
/// Secrets (`FUIGO_SERVER_KEY` / `FUIGO_SESSION_ID`) reach the proxy by env inheritance, never argv.
fn build_preview_command(cfg: &PreviewArgs) -> io::Result<tokio::process::Command> {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        // Raw pre_exec, NOT fuigo_tty_utils::detach_command
        ...
        let oom_protect = std::env::var_os(fuigo_tty_utils::RESET_CHILD_OOM_ENV).is_some();
        ...
        unsafe {
            cmd.pre_exec(move || {
                ...
                if oom_protect {
                    ...
                    fuigo_tty_utils::reset_oom_score_adj()?;
                }
                Ok(())
            });
        }
...
/// The proxy's control path for the last-activity stamp; mirrors `fuigo-preview-proxy`'s `/__control/activity` route.
const PREVIEW_ACTIVITY_PATH: &str = "/__control/activity";

/// Effective proxy `--control-port` when the supervisor didn't set one (mirrors the default in `fuigo-preview-proxy/src/cli.rs`).
...
/// It is a trait rather than the concrete tracker so this crate stays free of `fuigo-workspace`.
...
pub async fn supervise_preview_metrics(control_port: Option<u16>, shutdown: watch::Receiver<bool>) {
    scrape_metrics_loop(
        ...
        shutdown,
        |body| {
            if let Some(sink) = fuigo_computer_hub_sdk::metric_donate::active_metrics_sink() {
...
mod tests {
    ...
    /// The tracker's own idle-withhold accounting is covered by `fuigo_workspace::activity`.
    /// The wiring between the two is covered by the `fuigo-workspace-server` binary's own tests.
    struct TestSink {
        window_ms: u64,
    ...
    fn to_argv_maps_every_flag_to_the_proxy_cli_names() {
        // Flag names must match fuigo-preview-proxy/src/cli.rs exactly.
        assert_eq!(
            sample_cfg().to_argv(),
            vec![
                "--preview-port",
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_HELPER_ENV: &str = "FUIGO_PDEATHSIG_HELPER";
    #[cfg(target_os = "linux")]
