...
/// The tail, not the head: JWTs share a base64 header and Ferrox Labs keys a prefix.
...
mod tests {
    ...
    fn bearer_suffix_semantics() {
        for (input, expected) in [
            // The tail, not the head: JWT headers and Ferrox Labs key prefixes are shared.
            ("eyJ0eXAiOiJh.shared-head.tail-distinct", "ail-distinct"),
            ("fuigo-key-aaaaaaaaaaadistinct1", "aaadistinct1"),
