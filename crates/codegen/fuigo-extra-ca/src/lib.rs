//! TLS policy for the fuigo CLI: OS roots, Mozilla roots, and opt-in extra roots from `FUIGO_EXTRA_CA_BUNDLE` (fallback: `SSL_CERT_FILE`).
...
/// `FUIGO_EXTRA_CA_BUNDLE` wins over `SSL_CERT_FILE`; an empty value disables both, since `SSL_CERT_FILE` is often set process-wide (Nix, conda).
fn configured_ca_bundle() -> Option<(&'static str, std::path::PathBuf)> {
    select_bundle(
        std::env::var_os(ENV_FUIGO_EXTRA_CA_BUNDLE),
...
fn select_bundle(
    ...
    match bundle {
        Some(p) if !p.is_empty() => Some((ENV_FUIGO_EXTRA_CA_BUNDLE, p.into())),
        Some(_) => None,
        None => match ssl {
