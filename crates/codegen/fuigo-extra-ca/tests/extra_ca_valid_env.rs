//! Process-isolated: valid FUIGO_EXTRA_CA_BUNDLE loads one root via OnceLock.
...
-----END CERTIFICATE-----\n";
    ...
    unsafe {
        std::env::set_var(
            fuigo_extra_ca::ENV_FUIGO_EXTRA_CA_BUNDLE,
            path.as_os_str(),
        );
    }

    assert_eq!(fuigo_extra_ca::extra_root_ders().len(), 1);
