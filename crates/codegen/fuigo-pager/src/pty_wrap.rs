//! Local PTY wrapper: the engine behind `fuigo wrap` (see [`crate::wrap_cmd`]).
//!
...
fn apply_wrap_child_env(
    ...
    if let Some(appearance) = appearance {
        ...
        cmd.env("FUIGO_APPEARANCE", value);
        cmd.env("LC_FUIGO_APPEARANCE", value);
    }
}

/// Run an arbitrary command inside a local PTY with OSC 52 output filtering.
///
/// This is the engine behind `fuigo wrap`: it spawns `program` (with `args`) attached to a local pseudo-terminal.
...
pub(crate) fn run_wrapped_command(program: &str, args: &[String]) -> Result<i32> {
    ...
    // Handles are intentionally detached: `fuigo wrap` is short-lived and exits with the child
...
mod tests {
    ...
    fn apply_wrap_child_env_dark_overrides_parent_light_on_both_names() {
        ...
        cmd.env("FUIGO_APPEARANCE", "light");
    ...
    fn apply_wrap_child_env_light_overrides_parent_dark_on_both_names() {
        let mut cmd = portable_pty::CommandBuilder::new("true");
        cmd.env("FUIGO_APPEARANCE", "dark");
        cmd.env("LC_FUIGO_APPEARANCE", "dark");
        apply_wrap_child_env(&mut cmd, Some(SystemAppearance::Light));
        assert_eq!(env_str(&cmd, "FUIGO_APPEARANCE").as_deref(), Some("light"));
        assert_eq!(
    ...
    fn apply_wrap_child_env_none_does_not_stamp_from_parent_snapshot() {
        let mut cmd = portable_pty::CommandBuilder::new("true");
        cmd.env("FUIGO_APPEARANCE", "dark");
        cmd.env_remove("LC_FUIGO_APPEARANCE");
