...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    ...
    sources.extend(fuigo_plugin_marketplace::load_extra_sources_from_settings(&sources));
    ...
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            fuigo_telemetry::session_ctx::log_event(
                fuigo_telemetry::events::PluginUninstalled {
                    confirmed: true,
                    success: true,
                },
            );
...
mod tests {
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir = fuigo_plugin_marketplace::git::sync_source_cache(
            &url,
            Some("main"),
            cache_root.path(),
        )
