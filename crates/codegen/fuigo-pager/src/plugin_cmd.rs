//! `fuigo plugin` CLI subcommand: manage plugins and marketplace sources.
//!
...
//!
//! Business logic lives in `fuigo_shell::plugin` and lower crates (`fuigo-agent`, `fuigo-plugin-marketplace`).
...
use fuigo_agent::plugins::install_registry::{InstallKind, InstallRegistry};
use fuigo_agent::plugins::manifest::{ManifestLoadResult, PluginManifest, load_manifest};
use fuigo_plugin_marketplace::SourceKind;
use fuigo_plugin_marketplace::git::SourceCacheLease;
use fuigo_shell::plugin::{self, RepoUpdateOutcome, UninstallError};

// ── JSON output types ───────────────────────────────────────────────

/// Typed entry for `fuigo plugin list --json`.
...
/// Typed entry for `fuigo plugin marketplace list --json`.
#[derive(Serialize)]
...
pub enum PluginCommand {
    ...
    Uninstall {
        /// Plugin name (as shown by `fuigo plugin list`).
        name: String,
...
fn installed_plugins(
    repos: &[(&str, &fuigo_agent::plugins::install_registry::InstalledRepo)],
) -> Vec<PluginEntry> {
    repos
        .iter()
...
fn available_plugins(registry: &InstallRegistry) -> Vec<PluginEntry> {
    let config = fuigo_shell::config::load_effective_config()
        .ok()
        .unwrap_or(toml::Value::Table(toml::map::Map::new()));
    let mut sources = fuigo_plugin_marketplace::load_sources(&config);
    sources.extend(fuigo_plugin_marketplace::load_extra_sources_from_settings(
        &sources,
    ));
    ...
    for source in &sources {
        ...
        for plugin in fuigo_plugin_marketplace::scan_marketplace(&root).entries {
            let already_installed =
                fuigo_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    registry,
                    &identity,
...
fn source_identity(source: &fuigo_plugin_marketplace::MarketplaceSource) -> String {
    match &source.kind {
...
fn resolve_marketplace_root(
    source: &fuigo_plugin_marketplace::MarketplaceSource,
    ...
    match &source.kind {
        ...
        SourceKind::Git { url, branch } => {
            let cache = fuigo_plugin_marketplace::git::default_cache_root();
            fuigo_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache,
                fuigo_plugin_marketplace::git::SyncMode::UseTtl,
            )
...
fn install_kind(is_git: bool) -> fuigo_telemetry::events::InstallKind {
    if is_git {
        fuigo_telemetry::events::InstallKind::Git
    } else {
        fuigo_telemetry::events::InstallKind::Local
    }
}

fn log_plugin_installed(
    install_kind: fuigo_telemetry::events::InstallKind,
    success: bool,
    error_category: Option<String>,
) {
    fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::PluginInstalled {
        install_kind,
        success,
        trust: true,
        error_category,
...
fn cmd_install(source: &str, trust: bool) -> Result<()> {
    if let Some(mref) = fuigo_plugin_marketplace::install_resolve::parse_marketplace_ref(source) {
    ...
    if !trust {
        use fuigo_agent::plugins::git_install::{self, InstallSource};
    ...
    match plugin::install_plugin(source, &cwd) {
        ...
        Err(e) => {
            ...
            log_plugin_installed(fuigo_telemetry::events::InstallKind::Git, false, Some(cat));
            bail!("{e}");
        }
    }
}
...
fn cmd_install_marketplace(
    source: &str,
    mref: &fuigo_plugin_marketplace::install_resolve::MarketplaceRef,
    trust: bool,
) -> Result<()> {
    if !trust {
...
fn cmd_uninstall(name: &str, confirm: bool, keep_data: bool) -> Result<()> {
    match plugin::uninstall_plugin(name, confirm, keep_data) {
        Ok(outcome) => {
            fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::PluginUninstalled {
                confirmed: true,
                success: true,
            });
        ...
        Err(UninstallError::NeedsConfirm {
            ...
            "Plugin \"{name}\" belongs to repo \"{repo_key}\" which also contains:\n\
             ...
             Uninstalling will remove all {total} plugin(s). To proceed:\n\
               fuigo plugin uninstall {name} --confirm",
            other_plugins
                .iter()
                .map(|p| format!("  - {p}"))
                .collect::<Vec<_>>()
...
fn cmd_disable(name: &str) -> Result<()> {
    ...
    if let Err(e) = fuigo_shell::config::remove_enabled_plugin(name) {
...
fn cmd_tag(path: &str, push: bool, force: bool, dry_run: bool) -> Result<()> {
    ...
    let version = match load_manifest(&root) {
        Ok(ManifestLoadResult::Found(m)) => m.version.ok_or_else(|| {
            anyhow::anyhow!(
                "No `version` field in plugin.json. Set a version to use `fuigo plugin tag`."
            )
        })?,
...
fn marketplace_add(
    ...
    use fuigo_shell::plugin::MarketplaceAddInput;
    ...
    if !force && let MarketplaceAddInput::GitUrl(git_url) = &input {
        fuigo_plugin_marketplace::git::probe_git_remote(git_url).map_err(|e| {
            anyhow::anyhow!(
...
fn find_removal_source<'a>(
    sources: &'a [fuigo_plugin_marketplace::MarketplaceSource],
    input: &str,
    cwd: &Path,
) -> Result<&'a fuigo_plugin_marketplace::MarketplaceSource, String> {
    ...
    let local_input = match plugin::classify_marketplace_add_input(input, cwd) {
        fuigo_shell::plugin::MarketplaceAddInput::LocalPath(p) => Some(p),
        _ => None,
    };
...
fn marketplace_update(
    ...
    marketplace_update_with_cache_root(
        sources,
        name,
        &fuigo_plugin_marketplace::git::default_cache_root(),
    )
}
...
fn marketplace_update_with_cache_root(
    ...
    for source in sources {
        ...
        if let SourceKind::Git { url, branch } = &source.kind {
            match fuigo_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                cache_root,
            ) {
...
mod tests {
    use super::*;
    use fuigo_plugin_marketplace::MarketplaceSource;
    ...
    fn removal_fixture() -> Vec<MarketplaceSource> {
        vec![
            ...
            MarketplaceSource {
                name: "official".into(),
                kind: SourceKind::Git {
                    url: "https://github.com/fuigo-org/plugin-marketplace.git".into(),
                    branch: None,
                },
            },
            MarketplaceSource {
    ...
    fn trust_prompt_marketplace_has_no_error_framing() {
        let msg = trust_prompt(
            "\"sentry\" from marketplace \"Ferrox Labs Official\"",
            "sentry@fuigo-org/plugin-marketplace",
        );
        assert!(
            msg.starts_with(
                "Installing \"sentry\" from marketplace \"Ferrox Labs Official\" requires confirmation."
            ),
            "{msg}"
        );
        assert!(msg.contains("hooks, MCP servers, and skills"));
        assert!(msg.contains(
            "To proceed, re-run with --trust:\n  fuigo plugin install sentry@fuigo-org/plugin-marketplace --trust"
        ));
    ...
    fn trust_prompt_git_and_local_subjects() {
        ...
        assert!(git.ends_with("  fuigo plugin install u/r --trust"), "{git}");
    ...
    fn marketplace_update_force_syncs_fresh_git_cache() {
        ...
        let cache_dir =
            fuigo_plugin_marketplace::git::sync_source_cache(&url, Some("main"), cache_root.path())
                .unwrap();
    ...
    fn current_head(repo: &Path) -> String {
        let output = fuigo_plugin_marketplace::git::git_command()
            .current_dir(repo)
            .args(["rev-parse", "HEAD"])
            .output()
            .unwrap();
