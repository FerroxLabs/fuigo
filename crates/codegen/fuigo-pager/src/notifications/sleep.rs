...
impl SleepInhibitor {
    ...
    fn platform_inhibit(&self) -> bool {
        ...
        let reason = core_foundation::string::CFString::new("fuigo: agent turn in progress");
        let assertion_type =
    ...
    fn platform_inhibit(&self) -> bool {
        ...
        // Bind that pid to us so a crashed or killed fuigo (SIGKILL, `panic=abort` SIGABRT, no Drop runs) cannot leave it running forever
        // A leaked `systemd-inhibit` holds the idle lock and pid slots on shared hosts
        fuigo_tty_utils::kill_on_parent_death_std(&mut cmd);
