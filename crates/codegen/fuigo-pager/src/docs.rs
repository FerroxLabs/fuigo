...
pub static USER_GUIDE: &[Doc] = &[
    ...
    Doc {
        filename: "27-fuigo-clone.md",
        title: "fuigo clone",
        description: "Depth-1 Grove clone, --full-history, and safe deepen/switch commands",
        content: include_str!("../docs/user-guide/27-fuigo-clone.md"),
    },
];
...
/// Extract user-guide docs to `<fuigo_home>/docs/user-guide/`.
///
/// Called from the pager binary startup so the model can read them from disk.
pub fn extract_user_guide_docs(fuigo_home: &std::path::Path) {
    let docs_dir = fuigo_home.join("docs").join("user-guide");
