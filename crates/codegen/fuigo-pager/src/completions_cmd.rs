//! `fuigo completions <shell>`: generate shell completion scripts.
//!
...
/// `fuigo worktree <TAB>` then re-offers every top-level command.
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            r#"curcontext="${curcontext%:*:*}:fuigo-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:fuigo-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("fuigo");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "fuigo", &mut buf);
    ...
    // Dispatch happens on `$line[2]`, so `fuigo worktree <TAB>` re-offered every top-level command (upstream clap-rs/clap#6282)
    #[test]
