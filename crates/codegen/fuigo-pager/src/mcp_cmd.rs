//! `fuigo mcp`: manage MCP server configurations from the command line.
...
Examples:
  ...
  # Add to the project config (./.fuigo/config.toml) instead of ~/.fuigo/config.toml
  fuigo mcp add --scope project github -- npx -y @modelcontextprotocol/server-github";
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
pub enum McpScope {
    /// `~/.fuigo/config.toml`, available in all your projects
    User,
    /// `./.fuigo/config.toml`, shared with everyone working in this directory
    Project,
}
...
pub struct AddArgs {
    ...
    /// Place them after `--` so flags such as `-y` are passed to the server instead of fuigo.
    #[arg(value_name = "ARGS")]
    args: Vec<String>,
    ...
    /// Config to write to: user (~/.fuigo/config.toml) or project (./.fuigo/config.toml)
...
fn run_list(json: bool) -> Result<()> {
    ...
    let servers = fuigo_shell::util::config::load_mcp_server_configs_with_project(&cwd);
    let disabled = fuigo_shell::util::config::disabled_mcp_server_names(&cwd);
    ...
    match transport {
        McpTransport::Stdio => {
            let Some(command) = source else {
                bail!(
                    "A command is required for stdio servers. Usage: fuigo mcp add <name> -- <command> [args...]"
                );
            };
        ...
        McpTransport::Http | McpTransport::Sse => {
            ...
            let Some(url) = source else {
                bail!(
                    "A URL is required for {label} servers. Usage: fuigo mcp add --transport {label} <name> <url>"
                );
            };
...
fn scope_target(scope: McpScope) -> PathBuf {
    match scope {
        McpScope::User => fuigo_shell::util::config::user_config_path(),
        McpScope::Project => fuigo_shell::util::config::project_config_path(&current_dir_or_exit()),
    }
}
...
fn scope_display(scope: McpScope, path: &Path) -> String {
    match scope {
        McpScope::User => display_user_fuigo_path(fuigo_config::USER_CONFIG_FILENAME),
...
fn select_remove_site(
    ...
    use fuigo_shell::util::config::user_config_path;
    ...
    project_site
        ...
        .or_else(|| {
            user_defined.then(|| {
                (
                    McpScope::User,
                    fuigo_shell::util::config::user_config_path(),
                )
            })
        })
}
...
fn mcp_server_is_known(name: &str, cwd: &Path) -> bool {
    fuigo_shell::util::config::cli_known_mcp_server_names(cwd).contains(name)
}
...
fn available_mcp_server_names(cwd: &Path) -> Vec<String> {
    let mut names: Vec<String> = fuigo_shell::util::config::cli_known_mcp_server_names(cwd)
        .into_iter()
        .collect();
    names.sort();
    names
...
async fn run_set_enabled(name: &str, enabled: bool) -> Result<()> {
    ...
    let modified =
        fuigo_shell::util::config::save_mcp_server_enabled_in(name, enabled, &cwd).await?;

    let now_disabled = fuigo_shell::util::config::disabled_mcp_server_names(&cwd).contains(name);
...
async fn run_remove(name: &str, requested_scope: Option<McpScope>) -> Result<()> {
    ...
    let find_project_site = || {
        fuigo_shell::config::find_project_configs(&cwd)
            .into_iter()
            .rev()
...
async fn run_doctor(json: bool, name: Option<String>) -> Result<()> {
    let cwd = current_dir_or_exit();
    let report = fuigo_shell::mcp_doctor::run_doctor(&cwd, name.as_deref()).await;
    ...
    if json {
        ...
        fuigo_shell::mcp_doctor::print_report(&report);
    }
...
mod tests {
    ...
    fn add_hyphen_flag_without_double_dash_is_rejected() {
        let err = PagerArgs::try_parse_from(["fuigo", "mcp", "add", "fs", "npx", "-y"])
    ...
    fn add_infers_http_for_bare_positional_url() {
        for url in ["https://mcp.example.com/mcp", "http://mcp.example.com/mcp"] {
            let add = parse_add(&["fuigo", "mcp", "add", "api", url]);
    ...
    fn add_default_transport_warns_on_url_looking_command() {
        ...
        let add = parse_add(&["fuigo", "mcp", "add", "local", "localhost:3000"]);
    ...
    fn add_scope_project_parses_and_invalid_scope_is_rejected() {
        let add = parse_add(&[
            "fuigo", "mcp", "add", "-s", "project", "fs", "--", "npx", "pkg",
        ]);
        assert_eq!(add.scope, McpScope::Project);

        let err = PagerArgs::try_parse_from([
            "fuigo", "mcp", "add", "-s", "local", "fs", "--", "npx", "pkg",
        ])
        .expect_err("local is not a fuigo scope");
    ...
    fn add_legacy_multi_value_env_is_rejected() {
        ...
        let add = parse_add(&[
            "fuigo", "mcp", "add", "pg", "--env", "A=1", "B=2", "--", "npx", "-y", "server",
        ]);
    ...
    fn add_validates_name_env_and_headers() {
        let add = parse_add(&["fuigo", "mcp", "add", "fs", "-e", "NOT_A_PAIR", "--", "npx"]);
        ...
        let add = parse_add(&["fuigo", "mcp", "add", "bad name!", "--", "npx"]);
    ...
    fn add_rejects_mismatched_options_per_transport() {
        let add = parse_add(&["fuigo", "mcp", "add", "fs", "-H", "X: y", "--", "npx"]);
    ...
    fn add_requires_a_source_for_each_transport() {
        let add = parse_add(&["fuigo", "mcp", "add", "fs"]);
        ...
        let add = parse_add(&["fuigo", "mcp", "add", "--transport", "http", "api"]);
    ...
    fn remove_accepts_optional_scope() {
        let args = PagerArgs::try_parse_from(["fuigo", "mcp", "remove", "fs", "-s", "project"])
    ...
    fn gateway_cli_toggle_names_are_rejected() {
        ...
        assert!(!is_gateway_cli_toggle_name("fuigo_com_slack"));
        assert!(!is_gateway_cli_toggle_name("user-slack"));
    }

    #[test]
    fn fuigo_com_known_only_with_toml_definition() {
        // Unique name: `fuigo_home()` is process-wide OnceLock, so FUIGO_HOME
        // EnvGuard is a no-op if another test already resolved it. A leftover
        // `fuigo_com_*` in the real ~/.fuigo disabled list would fail an orphan
        // assertion on a well-known name.
        let name = format!("fuigo_com_orphan_{}", uuid::Uuid::new_v4().as_simple());

        let orphan = tempfile::tempdir().unwrap();
        git2::Repository::init(orphan.path()).unwrap();
        assert!(
            !mcp_server_is_known(&name, orphan.path()),
            "orphan fuigo_com_* must not be known by prefix"
        );

        let defined = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(defined.path().join(".fuigo")).unwrap();
        std::fs::write(
            defined
                .path()
                .join(".fuigo")
                .join(fuigo_config::USER_CONFIG_FILENAME),
            format!(
                r#"
[mcp_servers.{name}]
...
"#
    ...
    fn enable_and_disable_parse_name() {
        let args = PagerArgs::try_parse_from(["fuigo", "mcp", "enable", "user-grafana"])
        ...
        let args = PagerArgs::try_parse_from(["fuigo", "mcp", "disable", "user-slack"])
    ...
    fn enable_disable_require_name() {
        let err = PagerArgs::try_parse_from(["fuigo", "mcp", "enable"])
            .expect_err("enable without name must fail");
        assert_eq!(err.kind(), clap::error::ErrorKind::MissingRequiredArgument);

        let err = PagerArgs::try_parse_from(["fuigo", "mcp", "disable"])
    ...
    fn select_remove_site_covers_scope_presence_matrix() {
        let user = fuigo_shell::util::config::user_config_path();
        let project = PathBuf::from("/repo/.fuigo/config.toml");
