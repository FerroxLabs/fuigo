...
//!
//! - **Samples**: footprint/RSS and allocator gauges every `FUIGO_MEMTRACE_INTERVAL_SECS` (default 30s) from a detached thread.
...
//! - **Thresholds**: the physical footprint crossing a bucket (`FUIGO_MEMTRACE_THRESHOLD_MB`, default 1 GiB, doubling) fires the *threshold hook*.
...
//! ## Files
//!
//! `$FUIGO_HOME/memtrace/<start-ts>-<pid>.jsonl` (and a `.1` after 4 MiB rotation) plus `<stem>-jemalloc-<seq>.txt` threshold dumps.
...
//!
//! The lib cannot depend on jemalloc; `fuigo-pager-bin` installs:
...
pub use fuigo_tty_utils::{ProcessResources, sample_process_memory};
...
/// Env: disable with `FUIGO_MEMTRACE=0|false|off`.
fn enabled_by_env() -> bool {
    !matches!(
        std::env::var("FUIGO_MEMTRACE").ok().as_deref(),
        Some("0") | Some("false") | Some("off")
    )
}

fn interval_from_env() -> Duration {
    let secs = std::env::var("FUIGO_MEMTRACE_INTERVAL_SECS")
        .ok()
...
fn first_threshold_from_env() -> u64 {
    std::env::var("FUIGO_MEMTRACE_THRESHOLD_MB")
        .ok()
        .and_then(|v| v.parse::<u64>().ok())
        .unwrap_or(1024)
        .saturating_mul(1 << 20)
...
/// Start memory tracing: install the process-global sink under `dir` (e.g. `$FUIGO_HOME/memtrace/`) and spawn the detached sampler thread.
...
/// Inert when `FUIGO_MEMTRACE=0`.
///
/// The trace file is created lazily on the first event, and the first sample is taken after one full interval.
/// Short-lived CLI invocations (`fuigo --version`, `fuigo trace …`) therefore leave no files behind.
...
pub fn default_dir() -> PathBuf {
    fuigo_shell::util::fuigo_home::fuigo_home().join("memtrace")
}
