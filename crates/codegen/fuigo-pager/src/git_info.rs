...
pub struct CwdGitInfo {
    ...
    /// Whether `cwd` is a worktree rather than the primary checkout (linked `git worktree`, fuigo standalone clone, or worktree DB hit).
...
fn compute_snapshot(cwd: &Path) -> GitSnapshot {
    ...
    // Standalone fuigo worktrees are CoW clones (`.git` is a directory), so `path != commondir` is false; the source marker is the back-pointer
    let mut marker_main_repo = None;
    for ancestor in cwd.ancestors() {
        let git = ancestor.join(".git");
        if let Ok(contents) = std::fs::read_to_string(git.join("fuigo-worktree-source"))
...
pub fn worktree_label_index() -> std::collections::HashMap<PathBuf, String> {
    ...
    let Ok(db) = fuigo_fast_worktree::db::WorktreeDb::open_default() else {
        return map;
    };
...
/// Tilde-collapse with a path-component prefix, so `HOME=/Users/u` does not match `/Users/user/fuigo`.
...
///
/// `FUIGO_NERD_FONTS=1` forces Powerline; `FUIGO_NERD_FONTS=0` forces the fallback.
pub(crate) fn branch_icon() -> &'static str {
    static ICON: OnceLock<&str> = OnceLock::new();
    ICON.get_or_init(|| {
        decide_branch_icon(
            std::env::var("FUIGO_NERD_FONTS").ok().as_deref(),
            HostOs::current(),
...
///
/// An explicit `FUIGO_NERD_FONTS` override always wins: `0`/`false` means off, anything else means on.
...
pub(crate) fn home_dir() -> Option<String> {
    fuigo_dirs::home_dir().map(|home| home.to_string_lossy().into_owned())
}
...
mod tests {
    ...
    fn cwd_git_info_lazy_non_repo_is_none() {
        let p = Path::new("/nonexistent-fuigo-git-info-lazy-test-zzz");
    ...
    fn collapse_home_requires_whole_path_component() {
        ...
        assert_eq!(
            collapse_home_path(Path::new("/Users/u/fuigo"), Some(home)),
            "~/fuigo"
        );
        assert_eq!(
    ...
    #[test]
    fn compute_cwd_git_info_standalone_fuigo_worktree() {
    ...
    fn update_from_notification_ors_cached_label_into_is_worktree() {
        let dir = PathBuf::from("/nonexistent-fuigo-notif-label-wt");
        {
