...
/// Passive input-device probe for `fuigo doctor` / `/doctor`.
///
...
pub fn apply_voice_probe(report: &mut DiagnosticReport, emit_missing_issue: bool) {
    ...
    match fuigo_voice::input_device_info() {
        Ok(device) => {
        ...
        Err(err) => {
            let error = match err {
                fuigo_voice::VoiceError::Config(message) => message,
    ...
    DiagnosticFinding {
        ...
        note: Some(
            "Connect or select a microphone in your system sound settings. On Linux, install a \
             supported audio recorder if none was found on PATH. Then run `/doctor` or `fuigo \
...
pub enum WarningCategory {
    ...
    /// The session runs over SSH without `fuigo wrap` on the local end.
...
pub(crate) fn collect_startup_warnings_from(
    ...
    if ctx.brand == TerminalName::AppleTerminal && ctx.is_ssh {
        ...
        warning.note = Some(
            "Fuigo also saves each copy to the backup file shown in the copy message. To copy \
             directly, run `fuigo wrap ssh <host>` on your local computer or use a terminal that \
    ...
    if ctx.is_tmux_backed() && matches!(tmux.control_mode, probes::TmuxProbeResult::Available(true))
    {
        let message = match fullscreen_active {
            ...
            Some(false) => "Fuigo is using inline mode because tmux control mode limits fullscreen",
...
pub(crate) fn wezterm_kitty_keyboard_warning_from(
    ...
    if shape == WezTermShape::SshXtversion {
        ...
        warning.note = Some(
            "For this session, type `\\` and then press Enter. Fuigo can't negotiate the Kitty \
...
pub fn sandbox_profile_conflict_warning(workspace: &Path) -> Option<TerminalWarning> {
    sandbox_profile_conflict_warning_from(fuigo_sandbox::sandbox_profile_conflicts(workspace))
}
...
fn sandbox_profile_conflict_warning_from(conflicts: Vec<String>) -> Option<TerminalWarning> {
    ...
    Some(TerminalWarning {
        ...
        config_path: None,
        note: Some(format!(
            "Fuigo is using the user profile. Compare `.fuigo/sandbox.toml` with {}, then rename \
             or remove the conflicting project profile. Project settings can add profile names \
             but can't redefine a user profile.",
            crate::util::display_user_fuigo_path(fuigo_config::SANDBOX_CONFIG_FILENAME)
        )),
    })
}

/// Pure SSH `fuigo wrap` recommendation: suggests launching the session through `fuigo wrap ssh <host>` on the user's local machine.
...
///   `fuigo wrap` advertises its OSC 52 sink through the SSH hop via an env var (see `clipboard::osc52_sink_active`).
...
pub(crate) fn collect_notification_warnings_with_method(
        ...
        && ctx.brand == TerminalName::Unknown
    {
        let mut warning = TerminalWarning::new(
            ...
            "Fuigo is using the terminal bell because the terminal was not recognized",
            None,
            None,
        );
    ...
    if condition == NotificationCondition::Unfocused && !supports_focus_tracking(ctx.brand) {
        let mut warning = TerminalWarning::new(
            ...
            Some(&crate::util::display_user_fuigo_path(
...
pub fn color_support_warning(
    ...
    if level == probes::RuntimeEvidence::Available(ColorLevel::None) {
        ...
        warning.note = Some("Unset `NO_COLOR`, then restart Fuigo.".to_string());
        return Some(warning);
    }

    // Checked before the detected level is consulted at all: the level says what Fuigo emits, which is a different question from what survives tmux
    // A truecolor detection is not evidence that truecolor reaches the terminal
    // A session with no color evidence (piped `fuigo doctor`) still has a clamping client worth reporting
    if color_passthrough == TmuxColorPassthrough::Reduced {
        ...
        warning.note = Some(format!(
            "Run `tmux source-file {tmux_config_path}`, then detach and reattach: the server \
             ...
             If Fuigo still reports less than truecolor afterwards, also add `set -g \
    ...
    if is_tmux_backed {
        ...
        warning.note = Some(format!(
            "In the same tmux config, also add `set -g default-terminal \"tmux-256color\"`. Add \
             ...
             `tmux source-file {tmux_config_path}`, then detach and reattach, and restart Fuigo."
        ));
        return Some(warning);
    }
...
mod tests {
    ...
    fn sandbox_profile_conflict_warning_reports_conflicts() {
        ...
        assert!(w.note.as_deref().is_some_and(|note| {
            note.contains("rename or remove")
                && note.contains(".fuigo/sandbox.toml")
                && note.contains(&crate::util::display_user_fuigo_path(
    ...
    // -- ssh_wrap_hint: `fuigo wrap ssh` recommendation --------------------------
    ...
    fn ssh_wrap_hint_fires_over_plain_ssh() {
        ...
        assert_eq!(w.fix.as_deref(), Some("fuigo wrap ssh <host>"));
        assert!(
    ...
    fn ssh_wrap_hint_suppressed_when_sink_active() {
        // An active OSC 52 sink means the session already runs under `fuigo wrap`; adoption silences the hint by itself
    ...
    fn notification_none_protocol_no_warnings() {
        let ctx = TerminalContext {
            brand: TerminalName::FuigoDesktop,
            ..Default::default()
        };
        let query = FakeTmuxQuery::healthy_modern();
        let w = collect_notification_warnings(
    ...
    fn supports_focus_tracking_known_terminals() {
        ...
        assert!(supports_focus_tracking(TerminalName::FuigoDesktop));
    ...
    /// Regression: a tmux client that reduces color used to be invisible to Doctor whenever Fuigo's own detection reported truecolor.
    ...
    /// Piped `fuigo doctor` has no color evidence, but the tmux client is still measurable, and `doctor fix` needs the finding to plan against.
    #[test]
