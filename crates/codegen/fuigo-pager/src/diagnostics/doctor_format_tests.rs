...
fn tmux_config_and_reload_notes_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Automatic setup: `fuigo doctor fix tmux-clipboard`\n",
            ...
            "      Automatic setup: `fuigo doctor fix dcs-passthrough`\n",
            ...
            "      Automatic setup: `fuigo doctor fix tmux-extended-keys`\n",
...
fn limited_color_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "  ssh          no\n",
            "  color        256\n",
            "  themes       2/5: fuigonight, fuigoday\n",
            "\n",
            "Clipboard\n",
            "  native       local (pbcopy)\n",
            "  tmux         off\n",
            ...
            "      Note: Add this export to your shell startup file, such as `~/.zshrc` or `~/.bashrc`, then restart Fuigo.\n",
        )
    );
}
...
fn unwrapped_ssh_recommendation_with_no_issues_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Automatic setup: `fuigo doctor fix ssh-wrap`\n",
            "      One-off: `fuigo wrap ssh <host>`\n",
...
fn wezterm_xtversion_runtime_evidence_output_is_stable() {
    ...
    assert_eq!(
        ...
        concat!(
            ...
            "      Note: For this session, type `\\` and then press Enter. Fuigo can't negotiate the Kitty keyboard protocol over SSH yet. `enable_kitty_keyboard = true` applies only to local WezTerm sessions.\n",
        )
    );
}
...
fn runtime_startup_findings_are_visible_with_useful_doctor_content() {
    ...
    assert!(output.contains("Fuigo is using the terminal bell"));
    assert!(output.contains("If the bell works for you"));
    assert!(output.contains("may not report focus changes"));
    assert!(output.contains(&crate::util::display_user_fuigo_path("config.toml")));
...
fn runtime_findings_merge_before_single_formatter_orders_issues_before_recommendations() {
    ...
    let issue = output.find("Fuigo is using the terminal bell").unwrap();
