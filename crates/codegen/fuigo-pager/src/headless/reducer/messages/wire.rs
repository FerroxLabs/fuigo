...
/// Map a Fuigo permission mode to the Messages `permissionMode` enum; Fuigo-only values become `default`.
...
/// The `usage.server_tool_use` sub-object (Fuigo counts backend `web_search` only).
#[derive(Clone, Default, Serialize, Deserialize)]
