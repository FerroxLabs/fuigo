...
use fuigo_acp_lib::{AcpArgsBox, AcpResult};
...
pub(crate) fn handle_ext_notification(
    notif: &fuigo_acp_lib::AcpArgsBox<acp::ExtNotification>,
) -> ExtEvent {
...
fn decode_session_notification(method: &str, params: &str) -> ExtEvent {
    ...
    enum FuigoUpdate {
        ...
        ResponseCompleted {
            ...
            #[serde(default)]
            usage: Option<fuigo_shell::extensions::notification::ResponseUsage>,
            #[serde(default)]
            signature: Option<String>,
            #[serde(default)]
    ...
    let fuigo_notif = match serde_json::from_str::<FuigoNotif>(params) {
        Ok(n) => n,
        Err(e) => {
            tracing::warn!(
                method,
    ...
    match fuigo_notif.update {
        FuigoUpdate::AutoCompactStarted { percentage } => {
            ExtEvent::Lifecycle(Lifecycle::CompactStarted { percentage })
        }
        FuigoUpdate::AutoCompactCompleted { tokens_before } => {
            ExtEvent::Lifecycle(Lifecycle::CompactCompleted {
                pre_tokens: tokens_before.unwrap_or(0),
            })
        }
        FuigoUpdate::AutoCompactFailed { error } => {
            ExtEvent::Lifecycle(Lifecycle::CompactFailed { error })
        }
        FuigoUpdate::AutoCompactCancelled {} => ExtEvent::Lifecycle(Lifecycle::CompactCancelled),
        FuigoUpdate::AutoContinueCompleted { total_tokens } => {
        ...
        FuigoUpdate::SubagentSpawned { subagent_id } => ExtEvent::SubagentSpawned { subagent_id },
        FuigoUpdate::SubagentFinished { subagent_id, .. } => {
            ExtEvent::SubagentFinished { subagent_id }
        }
        FuigoUpdate::ResponseStarted {
            message_id,
            model,
            input_tokens,
