//! Terminal-mode tracking and restore emission for `fuigo wrap`.
//!
//! `fuigo wrap` cannot tell a clean child exit from a connection drop: an ssh transport death reaches it as a plain PTY EOF plus an exit code.
...
//!
//! The tracked set mirrors the canonical teardown table in [`fuigo_crash_handler::terminal`] (`RESTORE_SEQ`), pinned by a unit test below.
...
/// Mouse/paste/focus disables in the relative order pinned by `fuigo_crash_handler::terminal::RESTORE_SEQ`'s ordering tests.
...
/// The emission order matches `fuigo_crash_handler::terminal::RESTORE_SEQ` for every element the two share (pinned by a unit test below).
...
mod tests {
    ...
    fn covers_and_orders_every_crash_handler_restore_seq_element() {
        let elements: Vec<Vec<u8>> = fuigo_crash_handler::terminal::RESTORE_SEQ
            .split(|&b| b == 0x1b)
