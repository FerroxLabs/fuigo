...
fn auth_provider_env_set() -> bool {
    std::env::var("FUIGO_AUTH_PROVIDER_COMMAND")
        .ok()
        .is_some_and(|s| !s.trim().is_empty())
}

fn auth_provider_config_set() -> bool {
    let Ok(raw) = fuigo_shell::config::load_effective_config() else {
        return false;
    };
    let Ok(cfg) = fuigo_shell::agent::config::Config::new_from_toml_cfg(&raw) else {
        return false;
    };
