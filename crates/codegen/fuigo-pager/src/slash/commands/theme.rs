...
/// Only the picker's `match_text` carries the aliases; `insert_text` stays the canonical display name.
...
mod tests {
    ...
    fn suggest_args_explicit_active_when_not_auto() {
        with_test_env(|| {
            ...
            let items = cmd.suggest_args(&ctx, "").expect("should return items");
            let fuigonight = items
                .iter()
                .find(|i| i.insert_text == "fuigonight")
                .expect("fuigonight should be in list");
            assert!(
                fuigonight.description.contains("(active)"),
                "explicit theme should show (active), got: {}",
                fuigonight.description
            );
        });
    }
    ...
    fn run_explicit_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            let result = cmd.run(&mut ctx, "fuigonight");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "fuigonight");
                }
                other => panic!("expected Action::SetTheme(\"fuigonight\"), got {other:?}"),
            }
        });
    }
    ...
    fn run_toggle_dispatches_set_theme_action() {
        with_test_env(|| {
            ...
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    // available[0] is FuigoNight; next is available[1]
    ...
    fn run_alias_normalises_to_canonical() {
        with_test_env(|| {
            ...
            // "dark" is an alias for FuigoNight.
            let result = cmd.run(&mut ctx, "dark");
            match result {
                CommandResult::Action(Action::SetTheme(name)) => {
                    assert_eq!(name, "fuigonight", "alias must normalise to canonical");
                }
                other => panic!("expected Action::SetTheme(\"fuigonight\"), got {other:?}"),
            }
        });
    ...
    fn preview_auto_applies_resolved_theme() {
        with_test_env(|| {
            ...
            // The default auto config maps Light to FuigoDay
            assert_eq!(Theme::current_kind(), ThemeKind::FuigoDay);
        });
    }
    ...
    fn cancel_preview_unknown_theme_is_no_op() {
        with_test_env(|| {
            theme_cache::set(ThemeKind::FuigoDay);
            let cmd = ThemeCommand;
