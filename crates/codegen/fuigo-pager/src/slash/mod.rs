...
mod tests {
    ...
    fn parses_invocation_with_args() {
        let inv = parse_invocation("/model fuigo-code-fast-1").expect("parsed");
        assert_eq!(inv.token, "model");
        assert_eq!(inv.args, "fuigo-code-fast-1");
    }
    ...
    fn acp_and_skill_commands_tag_from_map() {
        ...
        let skill_meta = serde_json::json!({
            "scope": "local",
            "path": "/home/user/.fuigo/skills/skill-cmd/SKILL.md",
        })
        .as_object()
        .cloned()
        .expect("skill meta is an object");
