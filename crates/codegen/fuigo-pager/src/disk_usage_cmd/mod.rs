//! `fuigo du`: what the user's fuigo home uses on disk.
//! It creates no fuigo home, registry file, or schema.
...
use fuigo_fast_worktree::{
    ...
    managed_worktree_roots, path_under_worktree_roots, resolve_fuigo_home,
};
use serde::Serialize;
...
#[command(
    after_help = "Lists every top-level directory in the fuigo home, largest first, then every \
worktree under `worktrees/` and `worktree_pool/` with its size, age, and label. To reclaim space, preview a sweep with \
`fuigo worktree gc --max-age 7d --dry-run`: without `--max-age`, gc expires nothing, it \
...
pub fn run(args: DiskUsageArgs) -> Result<()> {
    ...
    if !present {
        if args.json {
            return write_report(&empty_report(&fuigo_home), args.json, &mut out);
        }
        let written = display::print_missing_home(&fuigo_home.to_string_lossy(), &mut out);
        return Ok(crate::util::ignore_broken_pipe(written)?);
    }
    // Rows store canonical paths, so the home must match to strip-prefix.
    let fuigo_home = dunce::canonicalize(&fuigo_home).unwrap_or(fuigo_home);
    write_report(&collect_report(&fuigo_home)?, args.json, &mut out)
}
...
fn empty_report(fuigo_home: &Path) -> DiskUsageReport {
    DiskUsageReport {
        fuigo_home: fuigo_home.to_string_lossy().into_owned(),
...
fn collect_report(fuigo_home: &Path) -> Result<DiskUsageReport> {
    let mut top_level_dirs = Vec::new();
    let mut root_files_bytes = 0u64;
    let mut issues = WalkIssues::default();
    let mut unfollowed_dir_symlinks = 0u64;
    let volume = Volume::of(fuigo_home);
    let mut worktree_sizes: HashMap<PathBuf, Measure> = HashMap::new();
    let children = std::fs::read_dir(fuigo_home)
        .with_context(|| format!("cannot read {}", fuigo_home.display()))?;
    for child in children {
        let child = match child {
            Ok(child) => child,
            Err(e) => {
                tracing::debug!(path = %fuigo_home.display(), error = %e, "du: home entry could not be read");
    ...
    let (registry, records, registry_path) = load_registry(fuigo_home);
    let rows = collect_worktrees(fuigo_home, records, &worktree_sizes, volume);
    issues.merge(rows.issues);
    let (volume_capacity_bytes, volume_available_bytes) = volume_bytes(fuigo_home).unzip();
    Ok(DiskUsageReport {
        schema_version: SCHEMA_VERSION,
        fuigo_home: fuigo_home.to_string_lossy().into_owned(),
        total_bytes,
...
fn load_registry(fuigo_home: &Path) -> (RegistryState, Vec<WorktreeRecord>, PathBuf) {
    classify(WorktreeDb::open_read_only(fuigo_home))
}
...
fn collect_worktrees(
    ...
    for found in discover_worktrees(fuigo_home).found {
