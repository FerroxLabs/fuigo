...
fn untracked_row(bytes: u64) -> WorktreeUsage {
    WorktreeUsage {
        ...
        path: "/wt-home/worktrees/fuigo/wt-1".into(),
    }
}
...
fn collect_report_joins_registry_and_flags_untracked() {
    ...
    let home = base.join("fuigo-home");
    let tracked = home.join("worktrees/fuigo/wt-tracked");
    let untracked = home.join("worktrees/fuigo/wt-untracked");
...
fn record_registered_via_symlinked_home_joins_as_one_row() {
    ...
    let wt = real_home.join("worktrees/fuigo/wt-a");
    ...
    db.register(&make_record(
        "wt-a",
        &link_home.join("worktrees/fuigo/wt-a"),
        "via-link",
    ))
    .unwrap();
...
fn duplicate_discovered_dirs_size_once() {
    ...
    let wt = home.join("worktrees/fuigo/wt-a");
    std::fs::create_dir_all(&wt).unwrap();
    std::fs::write(wt.join("f.bin"), vec![b'x'; 4096]).unwrap();
    std::os::unix::fs::symlink(&wt, home.join("worktrees/fuigo/wt-alias")).unwrap();
...
fn escape_symlink_is_counted_not_sized() {
    ...
    let base = dunce::canonicalize(tmp.path()).unwrap();
    let home = base.join("fuigo-home");
    std::fs::create_dir_all(home.join("worktrees/fuigo")).unwrap();
    let external = base.join("external");
    std::fs::create_dir_all(&external).unwrap();
    std::fs::write(external.join("huge.bin"), vec![b'x'; 65536]).unwrap();
    std::os::unix::fs::symlink(&external, home.join("worktrees/fuigo/escape")).unwrap();
...
fn record_at_missing_path_is_omitted() {
    ...
    db.register(&make_record(
        "wt-gone",
        &home.join("worktrees/fuigo/wt-gone"),
        "gone",
    ))
    .unwrap();
...
fn a_row_off_the_anchor_reports_no_size() {
    let tmp = tempfile::TempDir::new().unwrap();
    let wt = tmp.path().join("worktrees/fuigo/wt-a");
    std::fs::create_dir_all(&wt).unwrap();
...
fn every_open_outcome_maps_to_its_state() {
    ...
    db.register(&make_record(
        "wt-a",
        &home.join("worktrees/fuigo/wt-a"),
        "lbl",
    ))
    .unwrap();
...
fn worktrees_dominate_at_half_of_total() {
    ...
    for case in cases {
        let report = DiskUsageReport {
            fuigo_home: "/home/user/.fuigo".into(),
...
fn json_shape_is_frozen() {
    let report = DiskUsageReport {
        ...
        registry: RegistryState::Read,
        registry_path: "/home/user/.fuigo/worktrees.db".into(),
        worktrees: vec![
            WorktreeUsage {
                last_modified_at: Some(1_700_005_000),
                path: "/home/user/.fuigo/worktrees/fuigo/wt-1".into(),
                ..tracked_row(
                    90,
                    TrackedRow {
                        last_accessed_at: Some(1_700_009_999),
                        label: Some("my-feature".into()),
                        repo_name: "fuigo".into(),
            ...
            WorktreeUsage {
                ...
                path: "/home/user/.fuigo/worktree_pool/inst/wt-2".into(),
                ..untracked_row(10)
            },
        ],
    };
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "worktrees_outside_managed_roots": 0,
            "registry": "read",
            "registry_path": "/home/user/.fuigo/worktrees.db",
            "worktrees": [
                {
                    "bytes": 90,
                    "kind": "session",
                    ...
                    "repo_name": "fuigo",
                    "git_ref": "brian/fix",
                    "path": "/home/user/.fuigo/worktrees/fuigo/wt-1",
                },
                {
                    "bytes": 10,
                    "kind": "pool",
                    ...
                    "repo_name": null,
                    "git_ref": null,
                    "path": "/home/user/.fuigo/worktree_pool/inst/wt-2",
                },
            ],
        })
    );
...
fn missing_home_json_is_valid_and_empty() {
    let mut out = Vec::new();
    write_report(
        &empty_report(Path::new("/nonexistent/.fuigo")),
        /*json*/ true,
        &mut out,
    )
    .unwrap();
...
fn print_report_truncates_long_labels_and_keeps_columns_aligned() {
    ...
    let report = DiskUsageReport {
        ...
        worktrees: vec![
            WorktreeUsage {
                path: "/wt-home/worktrees/fuigo/wt-long".into(),
                ..tracked_row(
                    150,
                    TrackedRow {
            ...
            WorktreeUsage {
                path: "/wt-home/worktrees/fuigo/wt-dead".into(),
                ..tracked_row(
                    100,
                    TrackedRow {
            ...
            WorktreeUsage {
                path: "/wt-home/worktrees/fuigo/wt-short".into(),
                ..untracked_row(50)
            },
        ],
        ..DiskUsageReport::default()
    ...
    crate::test_util::assert_path_column_aligned(&text, "worktrees/fuigo/wt-");
}
...
fn print_report_renders_registry_notices() {
    ...
    let cases = [
        ...
        Case {
            ...
            absent: &["db rebuild", "damaged", "Remove $FUIGO_HOME/worktrees.db"],
        },
        Case {
...
fn reclaim_hint_names_a_sequence_that_frees_space() {
    const AGE: &str = "run `fuigo worktree gc --max-age 7d --dry-run`";
    const RM: &str = "Remove one with `fuigo worktree rm --dry-run <path>`";
...
fn symlinked_worktrees_dir_is_surfaced_not_silently_dropped() {
    ...
    let wt = elsewhere.join("fuigo/wt-a");
    std::fs::create_dir_all(&wt).unwrap();
...
fn symlinked_default_home_keeps_home_label() {
    ...
    std::fs::create_dir_all(&real_fuigo).unwrap();
    std::os::unix::fs::symlink(&real_fuigo, fake_home.join(".fuigo")).unwrap();
    let _home = crate::test_util::EnvVarGuard::set("HOME", &fake_home);

    let resolved = dunce::canonicalize(&fake_home).unwrap().join(".fuigo");
    let canonical = dunce::canonicalize(&resolved).unwrap();
    assert_ne!(canonical, resolved, "the symlink must actually resolve");
    assert_eq!(
        crate::util::display_fuigo_home_prefix_for(&canonical),
        "~/.fuigo"
    );
}
