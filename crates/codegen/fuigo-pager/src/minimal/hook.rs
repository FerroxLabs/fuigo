...
//!
//! A dependency on `fuigo-pager-minimal` would be a cargo cycle: that crate reads this crate's `AppView`, `views::*` widgets, and `scrollback`.
...
//!
//! The `fuigo-pager-bin` binary calls `fuigo_pager_minimal::install()` once at startup.
...
/// Renders one minimal-mode frame; the installed implementation is `fuigo_pager_minimal::draw`.
