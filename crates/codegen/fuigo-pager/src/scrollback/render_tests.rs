...
fn make_hyperlink(
    ...
    url: &str,
    id: u32,
) -> fuigo_markdown::HyperlinkTarget {
    fuigo_markdown::HyperlinkTarget {
        line_index: line,
        column_range: cols,
        url: url.to_string(),
...
fn overlay_empty_hyperlinks_produces_nothing() {
    ...
    let links: &[fuigo_markdown::HyperlinkTarget] = &[];
    let mut overlay = LinkOverlay::new();
    map_hyperlinks_to_overlay(links, &output, 0, 0, 10, 0, 0, &[], None, &mut overlay);
...
fn markdown_wrapped_project_media_path_fully_linkified() {
    ...
    let path = "/Users/alice/.fuigo/projects/%2FUsers%2Falice%2Fcode%2Ffuigo/\
...
#[test]
fn basename_headers_stay_fuigo_owned_for_duplicate_and_outside_targets() {
    let cwd = std::path::PathBuf::from("/worktree");
    let terminal = official_vscode_remote_context();
    let cases = [
