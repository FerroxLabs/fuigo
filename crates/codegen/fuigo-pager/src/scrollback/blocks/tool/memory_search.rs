...
fn shorten_path(path: &str) -> &str {
    let memory_root = fuigo_config::fuigo_home().join("memory");
...
**File:** /root/.fuigo/memory/MEMORY.md (lines 0-10)
```
## Project Conventions
...
"#;
        ...
        assert_eq!(results[0].path, "/root/.fuigo/memory/MEMORY.md");
...
**File:** /root/.fuigo/memory/ws/MEMORY.md (lines 1-5)
```
workspace content
```

### Result 2 (score: 0.42, source: session)
**File:** /root/.fuigo/memory/ws/sessions/2026-05-01.md (lines 10-20)
```
session content
```
"#;
    ...
    fn shorten_memory_path() {
        // Paths under the configured fuigo memory root drop the root and the first directory below it
        let memory_root = fuigo_config::fuigo_home().join("memory");
        let session = memory_root.join("fuigo-50aa78f0/sessions/2026-05-01.md");
