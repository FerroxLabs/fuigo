...
//! This module detects those blocks in an agent message via the generic [`CodeBlockSpan`](fuigo_markdown::CodeBlockSpan) API.
...
pub struct MermaidBlock {
    ...
    /// Taken from [`CodeBlockSpan::body`](fuigo_markdown::CodeBlockSpan::body).
    ...
    /// Mirrors [`CodeBlockSpan::output_line_range`](fuigo_markdown::CodeBlockSpan::output_line_range).
...
) -> impl Iterator<Item = &'a fuigo_markdown::CodeBlockSpan> {
    view.code_blocks
        .iter()
...
///
/// `FuigoDay` is the only light theme.
/// Every other concrete theme (and the `FuigoNight` default that `Auto` resolves to before it reaches the cache) is dark.
/// The render worker maps this to `fuigo_mermaid::MermaidTheme`.
...
pub fn theme_is_dark(theme: ThemeKind) -> bool {
    !matches!(theme, ThemeKind::FuigoDay)
}
...
mod tests {
    ...
    #[test]
    fn theme_is_dark_maps_fuigoday_to_light_only() {
        assert!(
            !theme_is_dark(ThemeKind::FuigoDay),
            "FuigoDay is the light theme"
        );
        for dark in [
            ThemeKind::FuigoNight,
