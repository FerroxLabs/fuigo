...
use fuigo_shell::session::{ContextInfo, count_detail};
...
impl ContextInfoBlock {
    ...
    fn build_lines(&self, theme: &Theme, bar: BarLayout) -> Vec<Line<'static>> {
        ...
        // Uses the *live* value from the session snapshot (it comes from fuigo-shell's model config resolution)
        // The “Auto-compact at X%” line and the tip therefore match whatever the current model has configured
        // Remote settings, user TOML, and env all feed that value (e.g. 65 for fuigo-build).
        //
...
mod tests {
    use super::*;
    use fuigo_shell::session::TokenUsageCategory;
    ...
    fn bar_and_legend_reconcile_used_with_overhead_excluding_tools() {
        ...
        let block = ContextInfoBlock::new(snap, "fuigo-build");
        let theme = test_theme();
        let lines = block.build_lines(&theme, BarLayout::WIDE);
