...
//!
//! A client that sends `session/cancel` without `_meta.cancelTrigger` (Murage does) ends the turn with
//! `cancellationCategory: "MidTurnAbort"` and no trigger, so a viewer of that session sees the plain banner.
...
mod tests {
    ...
    fn missing_or_unknown_is_unspecified() {
        ...
        // A trigger-less MidTurnAbort is what a client without `_meta.cancelTrigger` (Murage) produces
        assert_eq!(
            CancelledBy::from_meta(
                None,
    ...
    #[test]
    fn every_cause_has_a_banner_and_only_user_says_by_user() {
        for cause in [
            CancelledBy::User,
            CancelledBy::SessionClosed,
            CancelledBy::Shutdown,
            ...
            assert!(cause.phrase().starts_with("Turn cancelled"), "{cause:?}");
            assert_eq!(
                cause.phrase().contains("by user"),
