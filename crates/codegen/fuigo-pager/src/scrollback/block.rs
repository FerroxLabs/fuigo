...
impl RenderBlock {
    ...
    pub fn context_info(
        snapshot: fuigo_shell::session::ContextInfo,
    ...
    pub fn with_hyperlinks<R>(&self, f: impl FnOnce(&[fuigo_markdown::HyperlinkTarget]) -> R) -> R {
        match self {
    ...
    pub fn with_table_copy_meta<R>(
        &self,
        f: impl FnOnce(&[fuigo_markdown::TableCopyMeta]) -> R,
    ) -> R {
        match self {
...
mod searchable_text_tests {
    ...
    fn subagent_failed_indexes_metadata_and_error() {
        ...
        assert!(text.contains("fuigo-test"), "got: {text:?}");
