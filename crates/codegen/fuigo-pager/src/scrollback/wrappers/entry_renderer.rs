...
impl<'a> EntryRenderer<'a> {
    ...
    /// Lets the layout cache size off-screen entries on a bulk load (`fuigo -r`) without word-wrapping or markdown-rendering every entry (O(history)).
...
mod tests {
    ...
    fn affordance_row_adds_one_row_per_diagram() {
        ...
        // That is the invariant a bulk load (`fuigo -r`) relies on to avoid clipping
