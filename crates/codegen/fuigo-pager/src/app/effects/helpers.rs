...
use fuigo_acp_lib::{AcpAgentTx, acp_send};
...
use fuigo_shell::session::unified_list::ListScope;
...
/// Reads `FUIGO_ENVRC_TIMEOUT_SECS` in this process; the agent inherits the same environment.
...
where
    T: fuigo_acp_lib::AcpRequest,
    R: From<fuigo_acp_lib::AcpArgs<T>> + std::fmt::Debug,
{
...
pub(crate) fn parse_session_scheduler_background_loops(
    ...
    resp_meta
        .and_then(|m| {
            m.get(fuigo_shell::session::SCHEDULER_BACKGROUND_LOOPS_META_KEY)
        })
        .and_then(|v| v.as_bool())
}
...
pub(crate) fn sanitize_user_error(raw: &str) -> String {
    ...
    let mut result = fuigo_shell::sampling::error::rewrite_service_names(raw);
...
/// |-------|-----------|----------|--------------------------------|--------------------|
/// | false | false     | false    | `fuigo-build` (default)         | `false`            |
/// | false | true      | false    | `fuigo-build` (default)         | `false`            |
/// | false | false     | true     | `fuigo-build-ask-user`          | omitted (shell gate) |
/// | false | true      | true     | `fuigo-build-ask-user`          | omitted (shell gate) |
/// | true  | false     | false    | `fuigo-build-plan-no-subagents` | `false`            |
/// | true  | true      | false    | `fuigo-build-plan`              | `false`            |
/// | true  | false     | true     | `fuigo-build-plan-no-subagents` | omitted (shell gate) |
...
impl SessionFlags {
    ...
    ///
    /// Returns `None` for the default `fuigo-build` profile (no `_meta` needed; it already includes TaskTool).
    ...
    pub(super) fn agent_profile(&self) -> Option<&'static str> {
        ...
        match (self.plan_mode, self.subagents, self.ask_user) {
            (true, true, _) => Some("fuigo-build-plan"),
            (true, false, _) => Some("fuigo-build-plan-no-subagents"),
            (false, _, true) => Some("fuigo-build-ask-user"),
            (false, _, false) => None,
        }
    }
    ...
    pub(crate) fn to_meta(&self) -> Option<acp::Meta> {
        ...
        if self.chat_mode {
            if self.plan_mode || self.agent_override.is_some()
                || std::env::var("FUIGO_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
            {
                tracing::warn!(
        ...
        } else if std::env::var("FUIGO_AGENT").ok().is_some_and(|s| !s.trim().is_empty())
...
pub(super) fn extract_first_user_prompt(
    info: &fuigo_shell::session::info::Info,
) -> Option<String> {
    use std::io::BufRead;
    let history_path = fuigo_shell::session::persistence::session_dir(info)
...
pub(super) fn count_chat_history_stats(history_path: &Path) -> (usize, usize) {
    use std::io::BufRead;
    use fuigo_shell::sampling::{AssistantItem, ConversationItem, UserItem};
...
pub(super) fn parse_session_picker_entries(
    ...
    entries
        ...
        .filter_map(|v| {
            ...
            use fuigo_tools::implementations::skills::skill::extract_skill_display_text;
            let display = if let Some(ref fp) = first_prompt {
                ...
                let info_cwd = v
                    ...
                    .unwrap_or_default()
                    .to_string();
                let info = fuigo_shell::session::info::Info {
...
pub(crate) async fn persist_setting(
    ...
    match key {
        "compact_mode" => {
            ...
            fuigo_shell::util::config::set_compact_mode(b)
                .await
        ...
        "trace_upload" => {
            ...
            fuigo_shell::util::config::set_trace_upload(b)
                .await
                .map_err(|e| e.to_string())
        }
        "feedback_trace_card" => {
            ...
            fuigo_shell::util::config::set_feedback_trace_card(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_timestamps" => {
            ...
            fuigo_shell::util::config::set_show_timestamps(b)
                .await
                .map_err(|e| e.to_string())
        }
        "page_flip_on_send" => {
            ...
            fuigo_shell::util::config::set_page_flip_on_send(b)
                .await
                .map_err(|e| e.to_string())
        }
        "confirm_before_rewind" => {
            ...
            fuigo_shell::util::config::set_confirm_before_rewind(b)
                .await
                .map_err(|e| e.to_string())
        }
        "combine_queued_prompts" => {
            ...
            fuigo_shell::util::config::set_combine_queued_prompts(b)
                .await
                .map_err(|e| e.to_string())
        }
        "follow_up_behavior" => {
            ...
            fuigo_shell::util::config::set_follow_up_behavior(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "show_timeline" => {
            ...
            fuigo_shell::util::config::set_show_timeline(b)
                .await
                .map_err(|e| e.to_string())
        }
        "simple_mode" => {
            ...
            fuigo_shell::util::config::set_simple_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.undo" => {
            ...
            fuigo_shell::util::config::set_contextual_hint_undo(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.plan_mode" => {
            ...
            fuigo_shell::util::config::set_contextual_hint_plan_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.image_input" => {
            ...
            fuigo_shell::util::config::set_contextual_hint_image_input(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.send_now" => {
            ...
            fuigo_shell::util::config::set_contextual_hint_send_now(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.small_screen" => {
            ...
            fuigo_shell::util::config::set_contextual_hint_small_screen(b)
                .await
                .map_err(|e| e.to_string())
        }
        "contextual_hints.word_select" => {
            ...
            fuigo_shell::util::config::set_contextual_hint_word_select(b)
                .await
                .map_err(|e| e.to_string())
        }
        ...
        "theme" => {
            ...
            fuigo_shell::util::config::set_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_dark_theme" => {
            ...
            fuigo_shell::util::config::set_auto_dark_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "auto_light_theme" => {
            ...
            fuigo_shell::util::config::set_auto_light_theme(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "default_model" => {
            ...
            fuigo_shell::util::config::set_default_model(s)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_speed" => {
            ...
            fuigo_shell::util::config::set_scroll_speed(i)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_mode" => {
            ...
            fuigo_shell::util::config::set_scroll_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "invert_scroll" => {
            ...
            fuigo_shell::util::config::set_invert_scroll(b)
                .await
                .map_err(|e| e.to_string())
        }
        "display_refresh_auto_cadence" => {
            ...
            fuigo_shell::util::config::set_display_refresh_auto_cadence(b)
                .await
                .map_err(|e| e.to_string())
        }
        "scroll_lines" => {
            ...
            fuigo_shell::util::config::set_scroll_lines(i)
                .await
                .map_err(|e| e.to_string())
        }
        "default_selected_permission" => {
            ...
            fuigo_shell::util::config::set_default_selected_permission(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "cancel_subagents_on_turn_cancel" => {
            ...
            fuigo_shell::util::config::set_cancel_subagents_on_turn_cancel(
                    s.to_string(),
                )
                .await
                .map_err(|e| e.to_string())
        ...
        "vim_mode" => {
            ...
            fuigo_shell::util::config::set_vim_mode(b)
                .await
                .map_err(|e| e.to_string())
        }
        "remember_tool_approvals" => {
            ...
            fuigo_shell::util::config::set_remember_tool_approvals(b)
                .await
                .map_err(|e| e.to_string())
        }
        "toolset.ask_user_question.timeout_enabled" => {
            ...
            fuigo_shell::util::config::set_ask_user_question_timeout_enabled(b)
                .await
                .map_err(|e| e.to_string())
        }
        "show_thinking_blocks" => {
            ...
            fuigo_shell::util::config::set_show_thinking_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "group_tool_verbs" => {
            ...
            fuigo_shell::util::config::set_group_tool_verbs(b)
                .await
                .map_err(|e| e.to_string())
        }
        "collapsed_edit_blocks" => {
            ...
            fuigo_shell::util::config::set_collapsed_edit_blocks(b)
                .await
                .map_err(|e| e.to_string())
        }
        "prompt_suggestions" => {
            ...
            fuigo_shell::util::config::set_prompt_suggestions(b)
                .await
                .map_err(|e| e.to_string())
        }
        "keep_text_selection" => {
            ...
            fuigo_shell::util::config::set_keep_text_selection(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        ...
        "render_mermaid" => {
            ...
            fuigo_shell::util::config::set_render_mermaid(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "hunk_tracker_mode" => {
            ...
            fuigo_shell::util::config::set_hunk_tracker_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "screen_mode" => {
            ...
            fuigo_shell::util::config::set_screen_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_keybind_enabled" => {
            ...
            fuigo_shell::util::config::set_voice_keybind_enabled(b)
                .await
                .map_err(|e| e.to_string())
        }
        "voice_capture_mode" => {
            ...
            fuigo_shell::util::config::set_voice_capture_mode(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "voice_stt_language" => {
            ...
            fuigo_shell::util::config::set_voice_stt_language(s.to_string())
                .await
                .map_err(|e| e.to_string())
        }
        "max_thoughts_width" => {
            ...
            fuigo_shell::util::config::set_max_thoughts_width(i)
                .await
                .map_err(|e| e.to_string())
        }
        "show_tips" => {
            ...
            fuigo_shell::util::config::set_show_tips(b)
                .await
                .map_err(|e| e.to_string())
        }
        "auto_update" => {
            ...
            fuigo_shell::util::config::set_auto_update(b)
                .await
                .map_err(|e| e.to_string())
        }
        "fork_secondary_model" => {
            ...
            fuigo_shell::util::config::set_fork_secondary_model(s)
                .await
                .map_err(|e| e.to_string())
        }
...
pub(crate) async fn persist_permission_mode_and_notify(
    ...
    let disk_result = fuigo_shell::util::config::update_config(|cfg| {
...
pub(super) fn marketplace_outcome_succeeded(
    outcome: &fuigo_hooks_plugins_types::ActionOutcome,
) -> bool {
...
/// Those DTOs are `fuigo_shell::extensions::task::KillTaskResponse` and `fuigo_shell::session::result::ExtMethodResult`.
...
pub(super) fn parse_kill_outcome(
    resp: &str,
) -> Option<fuigo_tools::types::KillOutcome> {
    use fuigo_shell::extensions::task::KillTaskResponse;
    use fuigo_shell::session::result::ExtMethodResult;
...
pub(super) fn parse_subagent_kill_outcome(resp: &str) -> SubagentKillOutcome {
    use fuigo_shell::extensions::task::{
...
pub(super) fn credit_balance_from_config(
    c: fuigo_shell::extensions::billing::BillingConfig,
...
pub(super) fn parse_auto_topup_response(
    ...
    use fuigo_shell::extensions::billing::GetAutoTopupRuleResponse;
...
/// A blocking flock on the shared, possibly-network `~/.fuigo` lock must never
...
pub(super) fn unregister_active_session_best_effort_in(
    ...
    match fuigo_active_sessions::try_unregister_in(root, session_id) {
        Ok(true) => {}
        Ok(false) => {
            tracing::debug!(
