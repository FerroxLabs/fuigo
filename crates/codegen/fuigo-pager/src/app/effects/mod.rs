...
use fuigo_telemetry::startup::{self, StartupPhase};
use actions::{
...
use fuigo_shell::sampling::error::http_status_from_error;
use fuigo_shell::session::{ExtMethodResult, SessionInfoResponse};
...
pub(crate) fn execute(
    ...
    match effect {
        Effect::RegisterActiveSession { session_id, cwd } => {
            ...
            if let Err(e) = fuigo_active_sessions::register(fuigo_active_sessions::ActiveSession {
                session_id,
                pid: std::process::id(),
                cwd,
        ...
        Effect::ScanForeignSessions { cwd, compat, fuigo_home, coordinator, seq } => {
            ...
            let abort_handle = tasks
                .spawn(async move {
                    ...
                    let summaries = tokio::task::spawn_blocking(move || {
                            let _permit = permit;
                            fuigo_foreign_sessions::scan_foreign_sessions(
                                &cwd,
                                enabled,
                            )
                        })
            ...
            tasks
                .spawn(async move {
                    ...
                    let recent = crate::app::foreign_sessions::with_gated_sources_async(
                            ...
                            &fuigo_home,
                            |enabled| async move {
                                tokio::task::spawn_blocking(move || fuigo_foreign_sessions::most_recent_foreign_session(
                                        &cwd_for_scan,
                                        enabled,
        ...
        Effect::FetchDashboardSessions => {
            ...
            tasks
                .spawn(async move {
                    ...
                    "cwd": cwd.to_string_lossy(),
                    "limit": 30,
                    "headless": fuigo_shell::session::unified_list::HeadlessPolicy::Exclude
                        .as_wire_str(),
                });
                    let request = acp::ExtRequest::new(
                        "fuigo/session/list",
            ...
            tasks
                .spawn(async move {
                    match tokio::task::spawn_blocking(move || {
                            let store = fuigo_dashboard_store::WorkspaceStore::open(
                                &db_path,
                            )?;
                            let snapshot = store.snapshot()?;
                            Ok::<
                                _,
                                fuigo_dashboard_store::StoreError,
                            >((store, snapshot))
                        })
                        .await
                    {
                ...
                .spawn(async move {
                    match tokio::task::spawn_blocking(move || {
                            ...
                            for member in members.iter().cloned() {
                                ...
                                &error,
                                fuigo_dashboard_store::StoreError::Busy { .. }
                            );
                                    failures
        ...
        Effect::RestoreAndLoadSession { agent_id, session_id, session_cwd: _ } => {
            use fuigo_shell::agent::session_registry_client::SessionRegistryClient;
            use fuigo_shell::session::restore::restore_session_with_storage;
            let setup_started = std::time::Instant::now();
            let raw_config = fuigo_shell::config::load_effective_config();
            let setup = raw_config
                .ok()
                .and_then(|raw| {
                    let cfg = fuigo_shell::agent::config::Config::new_from_toml_cfg(
                            &raw,
                        )
                        .ok()?;
                    ...
                    let storage = fuigo_shell::auth::credential_provider::build_storage_client_for_proxy(
                        &proxy_base,
                        deployment_key,
                        alpha_test_key,
            ...
            tasks
                .spawn(async move {
                    ...
                    let progress: Option<
                        fuigo_shell::session::restore::ProgressCallback,
                    > = {
                        use fuigo_shell::session::restore::{PhaseStep, RestorePhase};
                        Some(
                            Box::new(move |event| {
                    ...
                    match restore_session_with_storage(
                            ...
                            &session_id,
                            &cwd_str,
                            fuigo_shell::session::restore::RestoreSessionOpts {
                                turn_override: None,
                                progress,
                                restore_code: true,
                            },
            ...
            tasks
                .spawn(async move {
                    ...
                    let detail = tokio::task::spawn_blocking(move || {
                            ...
                            let history_path = fuigo_shell::session::persistence::session_dir(
                                    &info,
                                )
        ...
        Effect::SendBashCommand { agent_id, session_id, command, prompt_id } => {
            ...
            tasks
                .spawn(async move {
                    use fuigo_shell::extensions::prompt_meta::PromptBlockMeta;
                    ulog::info(
                        "prompt.acp_send.start",
                        Some(&session_id.0),
                        Some(
        ...
        Effect::KillBgTask { session_id, task_id, source } => {
            ...
            tasks
                .spawn(async move {
                    let params = fuigo_shell::extensions::task::KillTaskRequest {
            ...
            tasks
                .spawn(async move {
                    ...
                    let result = acp_send(req, &tx)
                        ...
                        .map(|_| ())
                        .map_err(|e| {
                            use fuigo_shell::agent::config::ModelSwitchIncompatibleAgentError;
            ...
            tasks
                .spawn(async move {
                    let changelog = tokio::task::spawn_blocking(|| {
                            fuigo_shell::util::changelog::ChangelogManager::new()
                                .fetch()
                        })
                        .await
                        .unwrap_or_else(|e| {
                            tracing::warn!(error = %e, "changelog fetch task failed");
                            fuigo_shell::util::changelog::Changelog {
                                markdown: None,
                                entries: None,
                            }
                        });
        ...
        Effect::PersistPrivacyBannerAcked { acked_at } => {
            tasks
                .spawn(async move {
                    if let Err(e) = fuigo_shell::util::config::set_privacy_banner_acked(
                            acked_at,
                        )
                        .await
                    {
        ...
        Effect::PersistConsentAnswer { account, notice_id, version, acked } => {
            tasks
                .spawn(async move {
                    match fuigo_shell::util::config::set_consent_answer(
                            account,
                            notice_id,
                            version,
                            acked,
        ...
        Effect::PersistPreferredModel { model_id, reasoning_effort } => {
            ...
            tasks
                .spawn(async move {
                    let result = fuigo_shell::util::config::persist_models_default(
                            Some(model_id_str),
                            reasoning_effort,
                        )
                        .await
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                fuigo_hooks_plugins_types::HooksListResponse,
                            >(inner.clone())
                ...
                .spawn(async move {
                    ...
                    let result = match acp_send(req, &tx).await {
                        Ok(resp) => {
                            ...
                            let inner = wrapper.get("result").unwrap_or(&wrapper);
                            serde_json::from_value::<
                                fuigo_hooks_plugins_types::PluginsListResponse,
                            >(inner.clone())
        ...
        Effect::HooksAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = fuigo_hooks_plugins_types::HooksActionRequest {
                        session_id: session_id.0.to_string(),
                        action,
                    };
                    let req = acp::ExtRequest::new(
        ...
        Effect::PluginsAction { agent_id, session_id, action } => {
            ...
            tasks
                .spawn(async move {
                    let req_body = fuigo_hooks_plugins_types::PluginsActionRequest {
                        session_id: session_id.0.to_string(),
                        action,
                    };
                    let req = acp::ExtRequest::new(
            ...
            tasks
                .spawn(async move {
                    ...
                    for (name, old, new, source_url, rel_path) in outdated {
                        let action = fuigo_hooks_plugins_types::MarketplaceAction::Update {
                            source_url_or_path: source_url.clone(),
                            plugin_relative_path: rel_path.clone(),
                        };
                        let req_body = fuigo_hooks_plugins_types::MarketplaceActionRequest {
                            session_id: session_id.0.to_string(),
                            action,
                        };
            ...
            tasks
                .spawn(async move {
                    let plugin_name = plugin_relative_path
                        ...
                        .to_string();
                    let action = fuigo_hooks_plugins_types::MarketplaceAction::Install {
                        source_url_or_path,
                        plugin_relative_path,
                    };
                    let req_body = fuigo_hooks_plugins_types::MarketplaceActionRequest {
            ...
            tasks
                .spawn(async move {
                    ...
                    struct McpUpsertRequest {
                        ...
                        server_name: String,
                        #[serde(flatten)]
                        config: fuigo_shell::util::config::McpServerConfig,
                    }
        ...
        Effect::ShareSession { agent_id, session_id } => {
            use fuigo_shell::session::{ShareSessionRequest, ShareSessionResponse};
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    let request = acp::ExtRequest::new(
        ...
        Effect::ShowSessionInfo { agent_id, session_id, show_resolved_model, nonce } => {
            let is_api_key_auth = session_flags.is_api_key_auth;
            let api_key_env_set = fuigo_shell::agent::auth_method::has_fuigo_api_key_env();
            let tx = acp_tx.clone();
            tasks
                .spawn(async move {
                    match fetch_session_info(&session_id, &tx).await {
        ...
        Effect::SendFeedback { agent_id, session_id, feedback_text, images } => {
            use fuigo_shell::session::ClientType;
            use fuigo_shell::session::acp_types::ClientFeedbackInput;
                            ...
                            Ok(Ok(resp)) => {
                                ...
                                if let Some(hits) = payload.get("results") {
                                    results = serde_json::from_value::<
                                        Vec<
                                            fuigo_shell::extensions::session_search::SearchSessionHit,
                                        >,
                                    >(hits.clone())
                                        .unwrap_or_default();
                                }
        ...
        Effect::HydrateSessionMetaFromDisk {
            ...
            tasks
                .spawn(async move {
                    ...
                    let path = fuigo_shell::session::persistence::session_dir(&info)
                        .join("summary.json");
                    ...
                    let (title, last_turn_summary) = tokio::task::spawn_blocking(move || -> Option<
                        ...
                        > {
                            ...
                            let summary: fuigo_shell::session::persistence::Summary = serde_json::from_str(
                                    &raw,
                                )
                                .ok()?;
            ...
            tasks
                .spawn(async move {
                    let settings = tokio::task::spawn_blocking(|| {
                            if !fuigo_shell::util::config::resolve_remote_fetch_enabled() {
                                return None;
                            }
                            let fuigo_home = fuigo_shell::util::fuigo_home::fuigo_home();
                            let store = fuigo_shell::auth::read_auth_json(
                                    &fuigo_home.join("auth.json"),
                                )
                                .ok()?;
                            let scope = fuigo_shell::auth::FuigoComConfig::default()
                                .auth_scope();
                            let auth = fuigo_shell::auth::lookup_auth(
                                &store,
                                &scope,
                            )?;
                            let proxy_base = std::env::var(
                                ...
                                .unwrap_or_else(|_| {
                                    fuigo_shell::agent::config::CLI_CHAT_PROXY_BASE_URL_DEFAULT
                                        .to_owned()
                                });
                            fuigo_shell::remote::fetch_settings_blocking(
                                    &proxy_base,
                                    &auth,
                                    None,
                                )
...
fn session_info_fields(
    ...
    push(
        "Shell version",
        fuigo_version::display_version(fuigo_update::channel_label()),
        false,
    );
    ...
    let model_display = fuigo_shell::session::model_display_name(
    ...
    if let Some(profile) = fuigo_sandbox::profile_name() {
...
fn format_auth_lines(is_api_key_auth: bool, api_key_env_set: bool) -> String {
    if is_api_key_auth {
        let method = if api_key_env_set {
            "  Auth method: API key (FUIGO_API_KEY)\n"
        } else {
            "  Auth method: API key\n"
        };
        return format!(
