...
use super::*;
use fuigo_shell::extensions::billing::{BillingConfig, Cent, UsagePeriod};
...
fn parse_kill_outcome_reads_result_envelope() {
    use fuigo_tools::types::KillOutcome;
...
fn parse_worktree_restore_payload_full() {
    use fuigo_workspace::session::git::RestoreDegree;
...
fn register_session_in(root: &std::path::Path, id: &str) -> acp::SessionId {
    use fuigo_active_sessions::{ActiveSession, register_in};
...
fn unregister_best_effort_removes_entry_when_lock_free() {
    ...
    assert!(
            fuigo_active_sessions::list_in(dir.path())
                .expect("list")
                .is_empty(),
...
async fn check_marketplace_updates_dispatches_update_and_skips_failed_notifications() {
    ...
    use fuigo_hooks_plugins_types::{ActionOutcome, MarketplaceAction, OutcomeStatus};
...
async fn foreign_scan_task_echoes_sequence_without_enabled_sources() {
    ...
    execute(
        Effect::ScanForeignSessions {
            ...
            compat: fuigo_foreign_sessions::EnabledForeignSessionSources::default(),
            fuigo_home: PathBuf::from("/path/that/must/not/be-read"),
...
async fn deep_search_sessions_echoes_routing_and_policy() {
    ...
    execute(
        Effect::DeepSearchSessions {
            ...
            query: "abc".into(),
            seq: 4,
            headless_policy: fuigo_shell::session::unified_list::HeadlessPolicy::Only,
        },
        &mut tasks,
        &tx,
        Path::new("."),
...
/// Default flags produce no agent profile (uses fuigo-build default).
#[test]
fn default_flags_produce_no_profile() {
    let flags = SessionFlags::default();
    assert_eq!(flags.agent_profile(), None);
}
/// --subagents alone produces no profile (fuigo-build already has TaskTool).
#[test]
...
/// Neutralize `FUIGO_AGENT` for the profile-matrix tests below.
...
/// Callers must be `#[serial_test::serial(FUIGO_AGENT)]` (process-global env).
fn without_fuigo_agent() -> crate::test_util::EnvVarGuard {
    crate::test_util::EnvVarGuard::set("FUIGO_AGENT", "")
}
...
#[serial_test::serial(FUIGO_AGENT)]
#[test]
fn runtime_default_flags_produce_plan_meta() {
    let _env = without_fuigo_agent();
    let flags = SessionFlags {
        plan_mode: true,
        subagents: true,
        ask_user: true,
    ...
    assert_eq!(meta["agentProfile"], "fuigo-build-plan");
...
fn plan_only_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "fuigo-build-plan-no-subagents");
...
/// --ask-user alone selects the fuigo-build-ask-user profile.
#[serial_test::serial(FUIGO_AGENT)]
#[test]
fn ask_user_alone_meta() {
    ...
    let meta = flags.to_meta().unwrap();
    assert_eq!(meta["agentProfile"], "fuigo-build-ask-user");
    assert!(meta.get("askUserQuestion").is_none());
    assert_eq!(meta["yoloMode"], false);
}
...
/// Mirrors the runtime: the `subagents` toggle alone does not need an `agentProfile` (default `fuigo-build` already has it).
#[test]
...
/// All three flags on at the runtime default produce fuigo-build-plan and no `askUserQuestion` field.
#[serial_test::serial(FUIGO_AGENT)]
#[test]
fn all_flags_meta() {
    let _env = without_fuigo_agent();
...
fn make_session_info(
    ...
    used: u64,
    total: u64,
) -> fuigo_shell::session::SessionInfoResponse {
    use fuigo_shell::session::acp_types::{ContextInfo, SessionInfoData};
    fuigo_shell::session::SessionInfoResponse {
...
fn format_session_info_session_auth_ignores_api_key_env() {
    ...
    assert!(!text.contains("fuigo login"), "{text}");
}
#[test]
...
#[test]
fn format_session_info_api_key_auth_suggests_fuigo_login() {
    let info = make_session_info("auto", None, 1000, 10000);
...
async fn lookup_session_title_loads_single_summary_by_cwd() {
    ...
    let dir = root
        .path()
        .join("sessions")
        .join(fuigo_shell::util::fuigo_home::encode_cwd_dirname(cwd))
        .join("sess-1");
...
fn sanitize_user_error_rewrites_shared_service_names() {
    for (pattern, replacement) in fuigo_shell::sampling::error::SERVICE_NAME_REWRITES {
