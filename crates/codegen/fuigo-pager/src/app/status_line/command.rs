...
use fuigo_status_line::StatusLineContext;
...
impl std::fmt::Display for RunError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            // Fuigo's own bug rather than the script's, so it says whose it is.
            RunError::Json(e) => write!(f, "could not encode Fuigo's payload: {e}"),
...
struct GroupGuard(Option<std::sync::Arc<fuigo_tty_utils::ProcessGroup>>);
...
async fn run_command(
    ...
    let expanded = match command.strip_prefix("~/") {
        Some(rest) => match fuigo_dirs::home_dir() {
    ...
    let configure = |cmd: &mut Command| {
        ...
        fuigo_tty_utils::detach_command(cmd);
    ...
    let group = match fuigo_tty_utils::global_process_scope().enroll(&child) {
        Ok(group) => Some(group),
        Err(error) => {
