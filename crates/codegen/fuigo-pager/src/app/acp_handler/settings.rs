...
pub(super) fn handle_settings_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    if let Some(campaigns) = update.campaigns.clone() {
        ...
        fuigo_shell::util::config::set_remote_campaigns_from_settings(Some(&rs));
    }
    ...
    if let Some(v) = update.auto_permission_mode_enabled {
        ...
        fuigo_shell::util::config::cache_remote_auto_permission_mode_enabled(Some(v));
        app.auto_mode_gate = fuigo_shell::util::config::auto_permission_mode_enabled_from_disk();
    ...
    if let Some(v) = update.prompt_suggestions_enabled {
        fuigo_shell::util::config::cache_remote_prompt_suggestions_enabled(Some(v));
    }
        ...
        && app.permission_mode_from_soft_default
    {
        ...
        let root = fuigo_shell::config::load_effective_config()
    ...
    if let Some(v) = update.privacy_notice_rollout {
        app.privacy_notice_rollout =
            fuigo_config::env_bool("FUIGO_PRIVACY_NOTICE_ROLLOUT").unwrap_or(v);
    }
    if let Some(v) = update.privacy_banner_reshow_days {
        app.privacy_banner_reshow_days = Some(
            std::env::var("FUIGO_PRIVACY_BANNER_RESHOW_DAYS")
                .ok()
    ...
    if let Some(remote_val) = update.session_picker_grouped {
        let resolved = std::env::var("FUIGO_SESSION_PICKER_GROUPED")
            .ok()
        ...
        && !msg.is_empty()
    {
        ...
        let effs = app.impose_gate(fuigo_shell::auth::GateInfo {
            message: msg.clone(),
    ...
    let (requirements, user_config, managed_config) = (
        fuigo_shell::config::load_merged_requirements(),
        fuigo_shell::config::load_from_disk().ok(),
        fuigo_shell::config::load_managed_config().ok(),
    );
    ...
    let remote = fuigo_shell::util::config::RemoteSettings {
        group_tool_verbs: update.group_tool_verbs,
        ..Default::default()
    };
    let resolved = fuigo_shell::util::config::resolve_group_tool_verbs(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    let resolved = fuigo_shell::util::config::resolve_collapsed_edit_blocks(
        requirements.as_ref(),
        user_config.as_ref(),
        managed_config.as_ref(),
        Some(&remote),
    ...
    if let Some(remote_tips) = update.tips {
        use fuigo_shell::util::config::resolve_tips;
    ...
    if let Some(remote_tags) = update.slash_command_tags.as_ref() {
        use fuigo_shell::util::config::resolve_slash_command_tags;
        let effective_config = fuigo_shell::config::load_effective_config().ok();
...
pub(super) fn apply_soft_default_permission_mode(
    ...
    let mode = fuigo_shell::util::config::selected_permission_mode(effective_ui, remote)
        .unwrap_or_else(fuigo_shell::util::config::default_interactive_permission_mode);
    ...
    app.current_ui.permission_mode = Some(if auto {
        ...
        fuigo_shell::util::config::resolved_display_permission_mode(effective_ui, remote)
            .to_string()
    });
}
...
pub(super) fn notify_sessions_leave_auto(app: &AppView, session_ids: &[acp::SessionId]) {
    ...
    let args = fuigo_acp_lib::AcpArgs {
...
pub(super) fn handle_announcements_update(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) =
        serde_json::from_str::<fuigo_announcements::AnnouncementsRefreshed>(notif.params.get())
    else {
        return false;
    };
...
/// `resolve_announcements` honors `FUIGO_ANNOUNCEMENTS_OVERRIDE` first, so a backend push can't reintroduce announcements when the override is set.
pub(super) fn apply_announcements_update(
    app: &mut AppView,
    next_gen: u64,
    remote: &[fuigo_announcements::RemoteAnnouncement],
    requirements: Option<&toml::Value>,
    user_config: Option<&toml::Value>,
    managed_config: Option<&toml::Value>,
) {
    let merged = fuigo_shell::util::config::resolve_announcements(
        requirements,
        user_config,
        managed_config,
        Some(remote),
    );
    let announcements = fuigo_announcements::filter_expired(merged);
...
) -> Option<fuigo_announcements::RemoteAnnouncement> {
...
pub(super) struct PagerSettingsUpdate {
    ...
    #[serde(default)]
    campaigns: Option<Vec<fuigo_shell::util::config::CampaignOverride>>,
    #[serde(default)]
    ...
    #[serde(
        default,
        deserialize_with = "fuigo_shell::util::config::deserialize_tolerant"
    )]
    consent_gate: Option<fuigo_shell::util::config::ConsentGate>,
}
