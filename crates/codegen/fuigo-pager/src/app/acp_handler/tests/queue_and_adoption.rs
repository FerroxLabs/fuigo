#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn queue_changed_adopts_running_prompt_id_only_when_unset() {
        // Shell and pager share the fuigo-prompt-queue type
        // Serializing the payload as the shell emits it pins the wire shape the handler consumes, not cross-crate compat
        let shell_payload = fuigo_shell::session::prompt_queue::QueueChanged {
    ...
    fn queue_changed_still_adopts_running_prompt_not_terminal_in_replay() {
        ...
        app.agents.get_mut(&id).unwrap().session.loading_replay = true;
        let _ = handle_ext_notification(
            &fuigo_turn_completed_notif("sess-1", "p-old", "end_turn", true),
            &mut app,
        );
        app.agents.get_mut(&id).unwrap().session.loading_replay = false;
    ...
    fn adopted_running_prompt_lets_subsequent_chunk_through_gate() {
        ...
        let msg = AcpClientMessage::SessionNotification(fuigo_acp_lib::AcpArgs {
            request,
            response_tx: tx,
        });
        handle(msg, &mut app);
    ...
    fn bash_kind_round_trips_and_adoption_sets_bash_turn() {
        // Shell and pager share the fuigo-prompt-queue type; pin kind through a serde cycle.
        let shell = fuigo_shell::session::prompt_queue::QueueChanged {
            session_id: "sess-1".to_string(),
            entries: vec![fuigo_shell::session::prompt_queue::QueueEntryWire {
                id: "b1".to_string(),
                version: 0,
                owner: None,
                last_editor: None,
    ...
    fn bind_session_id_resets_cursor_only_on_id_change() {
        ...
        agent.last_applied_fuigo_event_seq = Some(8);
        agent.deferred_subagent_finishes.insert(
            "child-stale".into(),
            crate::app::agent_view::DeferredSubagentFinish {
                notification: fuigo_shell::extensions::notification::SessionNotification {
        ...
        assert_eq!(agent.last_applied_fuigo_event_seq, Some(8));
        ...
        assert!(agent.last_applied_fuigo_event_seq.is_none());
        assert!(
