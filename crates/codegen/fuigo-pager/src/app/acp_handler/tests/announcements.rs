#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.fuigo` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.fuigo` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.fuigo` reads).
    #[test]
