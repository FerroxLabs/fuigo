#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn apply_compaction_started_sets_activity() {
        ...
        let update = FuigoSessionUpdate::AutoCompactStarted {
            tokens_used: 90000,
    ...
    fn apply_compaction_failed_keeps_held_prompt() {
        ...
        for error in [
            ...
            let update = FuigoSessionUpdate::AutoCompactFailed {
                error: error.into(),
            };
            assert!(apply_session_event(&update, &mut session, &mut scrollback, false));
            assert_eq!(
    ...
    fn apply_image_dropped_pushes_scrollback_block() {
        ...
        let update = FuigoSessionUpdate::ImageDropped {
            notes: notes.clone(),
        };
    ...
    /// Legacy WebLogin auth keeps its verbose message (with `fuigo logout` / `fuigo login` guidance), not the generic re-auth prompt.
    #[test]
    ...
    fn apply_retry_state_context_length_shows_context_too_large() {
        use fuigo_shell::extensions::notification::CONTEXT_LENGTH_ERROR_TYPE;
        let mut session = make_session(Some("s1"));
        let mut scrollback = ScrollbackState::new();
        apply_retry_state(
            &RetryState::Failed {
    ...
    fn apply_compaction_completed_defers_message_until_turn_end() {
        ...
        let update = FuigoSessionUpdate::AutoCompactCompleted {
    ...
    fn compaction_lifecycle_refreshes_context_bar_from_trigger_counts() {
        ...
        let started = FuigoSessionUpdate::AutoCompactStarted {
            tokens_used: 460_231,
        ...
        let completed = FuigoSessionUpdate::AutoCompactCompleted {
        ...
        let failed = FuigoSessionUpdate::AutoCompactFailed { error: "e".into() };
        assert_eq!(compaction_context_refresh(&failed), None);
        let cancelled = FuigoSessionUpdate::AutoCompactCancelled {
            reason: fuigo_shell::extensions::notification::AutoCompactCancelReason::UserCancelled,
        };
    ...
    fn child_compact_started_refreshes_context_used() {
        ...
        child_view.context_state = Some(fuigo_shell::session::ContextInfo::from_notification(
            90_000, 131_072,
        ));
        agent
            .subagent_views
    ...
    fn tool_call_delta_chunk_ignored_during_replay() {
        ...
        let payload = SessionNotification {
            ...
            update: FuigoSessionUpdate::ToolCallDeltaChunk {
                tool_call_id: Some("call_1".into()),
                tool_index: 0,
                name: Some("write".into()),
    ...
    fn summary_generated_ext(
        ...
        let meta = if title_is_manual {
            Some(fuigo_shell::extensions::notification::title_is_manual_meta())
        } else {
            None
        };
    ...
    fn auto_title_notification_strips_controls_and_caps() {
        use fuigo_shell::session::persistence::MAX_TITLE_SCALARS;
        let mut app = make_app_with_agent("sess-1");
        let dirty = format!(
    ...
    fn hooks_changed_ext(
        ...
        let notif = SessionNotification {
            session_id: acp::SessionId::new(session_id),
            update: FuigoSessionUpdate::HooksChanged {
                hooks,
