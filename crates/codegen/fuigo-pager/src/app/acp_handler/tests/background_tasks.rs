#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// Regression (resume sync): the on-disk replay stream re-emits persisted notifications through the generic `fuigo/session/update` envelope.
    ...
    fn ext_session_update_replay_restores_bg_task() {
        ...
        let update = FuigoSessionUpdate::TaskBackgrounded {
        ...
        handle(
            make_ext_session_notification_with_method("sess-1", "fuigo/session/update", update),
            &mut app,
        );
    ...
    fn completed_before_backgrounded_does_not_resurrect_running() {
        ...
        let raw = serde_json::value::to_raw_value(&notif).unwrap();
        let late = acp::ExtNotification::new("fuigo/task_backgrounded", std::sync::Arc::from(raw));
    ...
    fn unknown_completed_monitor_kind_marks_is_monitor() {
        ...
        snapshot.kind = fuigo_tools::computer::types::TaskKind::Monitor;
        let done = completed_notif_from_snapshot("sess-1", snapshot, false);
        assert!(handle_task_completed(&done, &mut app));
