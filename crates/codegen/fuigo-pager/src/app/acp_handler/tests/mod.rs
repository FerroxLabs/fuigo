...
use fuigo_shell::extensions::notification::SessionUpdate as FuigoSessionUpdate;
...
pub(super) fn compressed_entry(
    index: usize,
) -> fuigo_shell::extensions::notification::ImageCompressedEntry {
    fuigo_shell::extensions::notification::ImageCompressedEntry {
        index,
...
pub(super) fn make_exit_plan_ext(
    ...
    fuigo_acp_lib::AcpArgs<acp::ExtRequest>,
    tokio::sync::oneshot::Receiver<fuigo_acp_lib::AcpResult<acp::ExtResponse>>,
) {
...
pub(super) fn make_exit_plan_ext_with_tool_call_id(
    ...
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    let request = acp::ExtRequest::new("fuigo/exit_plan_mode", raw.into());
...
/// Build a `fuigo/queue/changed` notification carrying `runningPromptId`.
...
pub(super) fn make_fired_notif(
    ...
    let notif = SessionNotification {
        ...
        update: FuigoSessionUpdate::ScheduledTaskFired {
            task_id: task_id.into(),
            prompt: prompt.into(),
    ...
    acp::ExtNotification::new("fuigo/scheduled_task_fired", std::sync::Arc::from(raw))
}
...
pub(super) fn announcements_update_notif(
    r#gen: u64,
    announcements: &[fuigo_announcements::RemoteAnnouncement],
) -> acp::ExtNotification {
    acp::ExtNotification::new(
...
pub(super) fn make_created_ext_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: FuigoSessionUpdate::ScheduledTaskCreated {
            task_id: task_id.into(),
            prompt: prompt.into(),
            human_schedule: human_schedule.into(),
            next_fire_at: next_fire_at.map(str::to_string),
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("fuigo/scheduled_task_created", std::sync::Arc::from(raw))
}
pub(super) fn make_deleted_ext_notif(
    ...
    make_deleted_ext_notif_with_reason(
        session_id,
        task_id,
        fuigo_tools::notification::ScheduledTaskRemovedReason::Unknown,
        false,
    )
}
pub(super) fn make_deleted_ext_notif_with_reason(
    session_id: &str,
    task_id: &str,
    reason: fuigo_tools::notification::ScheduledTaskRemovedReason,
    is_replay: bool,
) -> acp::ExtNotification {
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: FuigoSessionUpdate::ScheduledTaskDeleted {
            task_id: task_id.into(),
            reason,
        },
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("fuigo/scheduled_task_deleted", std::sync::Arc::from(raw))
}
...
pub(super) fn fuigo_model_switch_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: FuigoSessionUpdate::ModelAutoSwitched {
...
pub(super) fn fuigo_unhandled_notif(
    session_id: &str,
    event_id: &str,
) -> acp::ExtNotification {
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: FuigoSessionUpdate::MemoryFlushStarted,
        meta: Some(serde_json::json!({ "eventId": event_id })),
    };
    acp::ExtNotification::new(
        "fuigo/session/update",
...
/// Build an `fuigo/session/prompt_complete` ext-notification for `session_id`.
pub(super) fn prompt_complete_ext(session_id: &str) -> acp::ExtNotification {
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("fuigo/session/prompt_complete", std::sync::Arc::from(raw))
}
...
/// Build an `fuigo/session/prompt_complete` ext-notification with an explicit `stopReason` and optional `agentResult`.
...
/// Failed `fuigo/session/prompt_complete` carrying the typed `errorKind`.
...
/// Build an `fuigo/session/prompt_complete` ext-notification carrying a `promptId` (shells with the lost-response fix).
/// Built through the typed [`PromptCompletePayload`] so the test wire shape can never drift from what `handle_prompt_complete` parses.
...
/// Build a durable `TurnCompleted` update on the `fuigo/session/update` rail, optionally stamped `isReplay`.
/// Built through the typed `SessionNotification` so the wire shape can't drift from what the dispatch parses.
pub(super) fn fuigo_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
    stop_reason: &str,
    is_replay: bool,
...
pub(super) fn fuigo_turn_completed_replay(
    session_id: &str,
    prompt_id: &str,
    stop_reason: &str,
...
/// Callers pass an `agent_result` with no canonical truncation text.
/// A rail test therefore fails if its typed-kind read is deleted; the text fallback cannot mask it.
pub(super) fn fuigo_turn_completed_failed_with_error_kind(
    session_id: &str,
    prompt_id: &str,
    agent_result: &str,
    error_kind: &str,
...
pub(super) fn fuigo_turn_completed_notif_with_cancel_trigger(
    session_id: &str,
    prompt_id: &str,
    stop_reason: &str,
    cancel_trigger: &str,
...
pub(super) fn fuigo_wake_turn_completed_notif(
    session_id: &str,
    prompt_id: &str,
...
/// Build a `HookExecution` update (one successful run) on the `fuigo/session/update` rail, optionally stamped `isReplay`.
/// `prompt_id == None` models pre-attribution shells.
pub(super) fn fuigo_hook_execution_notif_for_prompt(
    session_id: &str,
    event_name: &str,
...
pub(super) fn fuigo_hook_execution_notif_with_runs(
    session_id: &str,
    event_name: &str,
    prompt_id: Option<&str>,
    is_replay: bool,
...
pub(super) fn fuigo_hook_execution_notif(
    session_id: &str,
    event_name: &str,
    is_replay: bool,
) -> acp::ExtNotification {
...
/// Build an `fuigo/session/interjection` ext-notification (no id).
pub(super) fn interjection_ext(session_id: &str, text: &str) -> acp::ExtNotification {
    interjection_ext_with_id(session_id, text, None)
}
/// Build an `fuigo/session/interjection` ext-notification with an optional `interjectionId` (the originator-dedup key).
pub(super) fn interjection_ext_with_id(
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("fuigo/session/interjection", std::sync::Arc::from(raw))
}
...
/// Build an `ExtNotification` envelope for `fuigo/session_notification`.
...
/// Build an `ExtNotification` envelope with an explicit Ferrox Labs session method.
...
pub(super) fn test_subagent_finished(child_sid: &str) -> FuigoSessionUpdate {
    FuigoSessionUpdate::SubagentFinished {
        subagent_id: child_sid.into(),
        child_session_id: child_sid.into(),
...
pub(super) fn test_subagent_progress(
    ...
    FuigoSessionUpdate::SubagentProgress {
        subagent_id: child_sid.into(),
        parent_session_id: parent_sid.into(),
        child_session_id: child_sid.into(),
        duration_ms: 100,
...
/// Shared temp `FUIGO_HOME` for disk-replay tests.
/// `fuigo_home()` uses a process-wide `OnceLock`, so parallel tests must not each set `FUIGO_HOME` to a different tempdir.
...
/// Runs `f` with a thread-local fuigo home override.
/// Disk replay tests then do not depend on process-wide `fuigo_home()` cache order when the full suite runs.
pub(super) fn with_replay_disk_home<R>(f: impl FnOnce(&std::path::Path) -> R) -> R {
    let home = replay_disk_test_home();
    crate::app::subagent::set_replay_fuigo_home_for_tests(Some(home.to_path_buf()));
    let out = f(home);
    crate::app::subagent::set_replay_fuigo_home_for_tests(None);
    out
}
pub(super) fn write_child_updates_jsonl(
    ...
    write_child_updates_jsonl_under_cwd(fuigo_home, "/tmp", child_sid, content);
}
pub(super) fn write_child_updates_jsonl_under_cwd(
    ...
    let sessions_dir = fuigo_home
        .join("sessions")
        .join(fuigo_config::encode_cwd_dirname(cwd))
        .join(child_sid);
...
pub(super) fn make_permission_message(
    ...
    let msg = AcpClientMessage::RequestPermission(fuigo_acp_lib::AcpArgs {
        request,
        response_tx: tx,
    });
    (msg, rx)
}
/// Build an `fuigo/session_notification` carrying `InteractionResolved{tool_call_id}`.
...
pub(super) fn interaction_resolved_ext(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: FuigoSessionUpdate::InteractionResolved {
...
pub(super) fn make_git_head_changed_notif(
    ...
    let payload = fuigo_workspace::session::git::GitHeadChanged {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("fuigo/git_head_changed", std::sync::Arc::from(raw))
}
pub(super) fn make_task_backgrounded_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("fuigo/task_backgrounded", std::sync::Arc::from(raw))
}
...
pub(super) fn make_replayed_task_backgrounded_notif(
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("fuigo/session/update", std::sync::Arc::from(raw))
}
...
pub(super) fn task_completed_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: FuigoSessionUpdate::TaskCompleted {
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("fuigo/task_completed", std::sync::Arc::from(raw))
}
pub(super) fn make_monitor_event_notif(
    ...
    let notif = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: FuigoSessionUpdate::MonitorEvent {
            task_id: task_id.into(),
    ...
    let raw = serde_json::value::to_raw_value(&notif).unwrap();
    acp::ExtNotification::new("fuigo/monitor_event", std::sync::Arc::from(raw))
}
...
pub(super) fn make_models_update_notif(
    ...
    acp::ExtNotification::new("fuigo/models/update", std::sync::Arc::from(raw))
}
/// `fuigo/models/update` carrying a single reasoning-capable model whose catalog-default effort is `default_effort`.
...
pub(super) fn model_changed_ext(
    ...
    let payload = SessionNotification {
        session_id: acp::SessionId::new(session_id),
        update: FuigoSessionUpdate::ModelChanged {
...
/// Helper: build an `fuigo/mcp/init_progress` notification.
pub(super) fn make_mcp_init_progress_notif(
    ...
    let raw = serde_json::value::to_raw_value(
        ...
        .unwrap();
    acp::ExtNotification::new("fuigo/mcp/init_progress", std::sync::Arc::from(raw))
}
...
pub(super) fn make_server_status_notif(
    session_id: &str,
    name: &str,
    status: fuigo_shell::extensions::mcp::McpServerStatus,
    tools: Option<serde_json::Value>,
) -> acp::ExtNotification {
    use fuigo_shell::extensions::mcp::{
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("fuigo/mcp/server_status", std::sync::Arc::from(raw))
}
...
pub(super) fn make_servers_updated_notif() -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("fuigo/mcp/servers_updated", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_post_h2(
    ...
    let payload = fuigo_shell::extensions::mcp::McpToolsChanged {
        session_id: session_id.to_string(),
        server_name: "fuigo_com_linear".to_string(),
        tools: Vec::new(),
    };
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("fuigo/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_tools_changed_notif_pre_h2() -> acp::ExtNotification {
    let payload = serde_json::json!({ "serverName": "fuigo_com_linear", "tools": [] });
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("fuigo/mcp/tools_changed", std::sync::Arc::from(raw))
}
...
pub(super) fn make_mcp_initialized_notif(session_id: &str) -> acp::ExtNotification {
    ...
    let raw = serde_json::value::to_raw_value(&payload).unwrap();
    acp::ExtNotification::new("fuigo/mcp_initialized", std::sync::Arc::from(raw))
}
