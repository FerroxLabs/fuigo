#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn late_unique_subagent_lifecycle_event_is_not_dropped() {
        ...
        // A persisted active goal update can arrive ahead of a lower-ID spawn; it advances the Ferrox Labs highwater without adding a scrollback block
        assert!(handle(
            notification(
                serde_json::json!({
        ...
        assert_eq!(
            (
                app.agents[&AgentId(0)].last_applied_fuigo_event_seq,
        ...
        assert_eq!(
            app.agents[&AgentId(0)].last_applied_fuigo_event_seq,
            Some(100),
            "a late lower-ID lifecycle event must not regress the scalar Ferrox Labs highwater"
        );
    ...
    fn finish_before_spawn_is_applied_after_later_cursor_progress() {
        let mut app = make_app_with_agent("sess-parent");
        let notification = |update: FuigoSessionUpdate, event_id: &str| {
    ...
    fn a_notification_reaches_its_target_agent_even_when_another_is_active() {
        // AutoCompactCompleted on the Ferrox Labs ext path resets the context bar numerator via refresh_context_used
