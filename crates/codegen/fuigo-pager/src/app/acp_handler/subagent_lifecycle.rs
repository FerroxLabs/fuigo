...
//!
//! Each transition happens once per child, but transitions can arrive out of order with other Ferrox Labs updates and even with each other.
    ...
    match delivery {
        ...
        LifecycleDelivery::DropDuplicate => {
            tracing::debug!(
                ...
                event_id,
                "fuigo/session lifecycle update DROPPED as a duplicate"
            );
            LifecycleDelivery::DropDuplicate
        }
...
pub(super) fn defer_subagent_finish(
    ...
    tracing::debug!(
        ...
        transition = %SubagentLifecycle::Finished,
        event_id,
        "fuigo/session lifecycle update DEFERRED until spawn"
    );
...
pub(super) fn redispatched_subagent_finish(
    ...
    serde_json::value::to_raw_value(&payload)
        .ok()
        .map(|params| acp::ExtNotification::new("fuigo/session/update", params.into()))
}

fn strip_deferred_finish_output(notification: &mut SessionNotification) {
    if let FuigoSessionUpdate::SubagentFinished { output, .. } = &mut notification.update {
        *output = None;
    }
}
        ...
        assert!(
            deferred
                ...
                .all(|entry| match &entry.notification.update {
                    FuigoSessionUpdate::SubagentFinished { output, .. } => output.is_none(),
                    _ => false,
                })
        );
