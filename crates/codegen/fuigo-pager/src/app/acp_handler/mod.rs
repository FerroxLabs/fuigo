...
use fuigo_shell::extensions::notification::{
    SessionNotification, SessionUpdate as FuigoSessionUpdate, is_reauthable_failure,
};
use fuigo_shell::tools::todo::todo_item_from_plan_entry;
use fuigo_tools::notification::ScheduledTaskRemovedReason;
...
fn handle_ext_notification(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    match method {
        "fuigo/follow_ups" => handle_follow_ups(notif, app),
        "fuigo/task_backgrounded" => handle_task_backgrounded(notif, app),
        "fuigo/task_completed" => handle_task_completed(notif, app),
        "fuigo/models/update" => handle_models_update(notif, app),
        "fuigo/settings/update" => handle_settings_update(notif, app),
        "fuigo/sessions/changed" => handle_sessions_changed(notif, app),
        "fuigo/queue/changed" => handle_queue_changed(notif, app),
        "fuigo/session/prompt_complete" => handle_prompt_complete(notif, app),
        "fuigo/session/interjection" => handle_interjection(notif, app),
        "fuigo/monitor_event" => handle_monitor_event(notif, app),
        "fuigo/scheduled_task_created" => handle_scheduled_task_created(notif, app),
        "fuigo/scheduled_task_fired" => handle_scheduled_task_fired(notif, app),
        "fuigo/scheduled_task_deleted" => handle_scheduled_task_deleted(notif, app),
        ...
        "fuigo/mcp/elicit_complete" => handle_mcp_elicit_complete(notif, app),
        "fuigo/mcp/servers_updated" => handle_mcp_servers_updated(notif, app),
        _ => false,
    }
}
...
fn handle_interjection(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    let Ok(parsed) = serde_json::from_str::<serde_json::Value>(notif.params.get()) else {
        tracing::warn!("Failed to parse fuigo/session/interjection");
        return false;
    };
...
fn handle_ext_method(ext: fuigo_acp_lib::AcpArgs<acp::ExtRequest>, app: &mut AppView) -> bool {
    match ext.request.method.as_ref() {
        "fuigo/ask_user_question" => handle_ask_user_question(ext, app),
        "fuigo/exit_plan_mode" => handle_exit_plan_mode(ext, app),
        "fuigo/mcp/elicit" => handle_mcp_elicit(ext, app),
        unknown => {
