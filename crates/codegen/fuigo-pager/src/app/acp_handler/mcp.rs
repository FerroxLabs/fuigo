...
/// Default `true`; set `FUIGO_MCP_PUSH_SERVER_STATUS=0` to disable.
pub(super) fn push_server_status_enabled() -> bool {
    use std::sync::OnceLock;
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        fuigo_shell::util::config::resolve_mcp_push_server_status(
...
///    All `tools_changed` emit sites carry `sessionId` via the typed [`fuigo_shell::extensions::mcp::McpToolsChanged`] struct.
...
/// The types: [`fuigo_shell::extensions::mcp::McpServerStatusPayload`] and [`fuigo_shell::extensions::mcp::McpServerStatus`].
...
pub(super) fn handle_mcp_server_status(notif: &acp::ExtNotification, app: &mut AppView) -> bool {
    ...
    use fuigo_shell::extensions::mcp::{McpServerStatus, McpServerStatusPayload, McpToolEntry};
