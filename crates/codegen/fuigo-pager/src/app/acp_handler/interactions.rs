...
fn cancel_elicitation_request(
    response_tx: tokio::sync::oneshot::Sender<fuigo_acp_lib::AcpResult<acp::ExtResponse>>,
) {
    let cancelled = fuigo_tools::mcp_elicitation::McpElicitExtResponse::Cancel;
...
pub(crate) fn handle_mcp_elicit(
    ext: fuigo_acp_lib::AcpArgs<acp::ExtRequest>,
    app: &mut AppView,
) -> bool {
    use crate::views::elicitation_view::ElicitationViewState;
    use fuigo_tools::mcp_elicitation::McpElicitExtRequest;
...
/// Handle `fuigo/ask_user_question` ext-method.
///
...
/// Handle an `fuigo/exit_plan_mode` ext_method request.
///
