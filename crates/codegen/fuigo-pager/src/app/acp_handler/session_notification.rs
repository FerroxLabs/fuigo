...
pub(super) fn failed_hook_line(
    event_name: &str,
    run: &fuigo_shell::extensions::notification::HookRunEntryDto,
) -> Option<String> {
    use fuigo_hooks::config::HookDisplayName;
    ...
    let subject = match fuigo_hooks::config::hook_display_label(&run.name) {
