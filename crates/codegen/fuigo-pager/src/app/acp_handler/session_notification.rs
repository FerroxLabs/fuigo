...
pub(super) fn compaction_context_refresh(update: &FuigoSessionUpdate) -> Option<u64> {
    match update {
        FuigoSessionUpdate::AutoCompactStarted { tokens_used, .. } => Some(*tokens_used),
        FuigoSessionUpdate::AutoCompactCompleted { tokens_after, .. } => Some(*tokens_after),
        _ => None,
    }
}
...
/// Replay gate shared by the ACP and Ferrox Labs session-update paths. Returns `true` when the update must be dropped.
///
...
pub(super) fn handle_session_notification_with_origin(
    ...
    match &session_notif.update {
        FuigoSessionUpdate::TaskBackgrounded { .. } => {
            return handle_task_backgrounded(notif, app);
        }
        FuigoSessionUpdate::TaskCompleted { .. } => {
            return handle_task_completed(notif, app);
        }
        FuigoSessionUpdate::ScheduledTaskCreated { .. } => {
            return handle_scheduled_task_created(notif, app);
        }
        FuigoSessionUpdate::ScheduledTaskDeleted { .. } => {
    ...
    let is_workflow_update = matches!(
        &session_notif.update,
        FuigoSessionUpdate::WorkflowUpdated { .. }
    );
    ...
    let changed = match session_notif.update {
        ref update @ (FuigoSessionUpdate::AutoCompactStarted { .. }
        ...
        | FuigoSessionUpdate::MemorySessionSaved { .. }) => {
            ...
            if let FuigoSessionUpdate::AutoCompactCompleted { .. } = update {
        ...
        FuigoSessionUpdate::TurnCompleted {
            prompt_id,
            stop_reason,
            agent_result,
            error_kind,
        ...
        FuigoSessionUpdate::SubagentSpawned {
            ...
            let retained_terminal_finish = agent
                ...
                .map(|info| SessionNotification {
                    ...
                    update: FuigoSessionUpdate::SubagentFinished {
        ...
        FuigoSessionUpdate::HookAnnotation { message } => {
        ...
        FuigoSessionUpdate::HookExecution {
            ...
            let hook_entries: Vec<HookRunEntry> = runs
                ...
                .map(|r| {
                    let status = match r.status {
                        fuigo_shell::extensions::notification::HookRunStatusDto::Success {
                            elapsed_ms,
                        } => HookRunStatus::Success {
                            elapsed: std::time::Duration::from_millis(elapsed_ms),
                        },
                        fuigo_shell::extensions::notification::HookRunStatusDto::Skipped => {
                            HookRunStatus::Skipped
                        }
                        fuigo_shell::extensions::notification::HookRunStatusDto::Failed {
                            error,
                            elapsed_ms,
                            blocked: true,
            ...
            let is_stop_hook =
                fuigo_hooks_plugins_types::HookEvent::from_wire(&event_name).is_turn_end();
            if is_tool_hook {
        ...
        FuigoSessionUpdate::HooksChanged {
            ...
            if let Some(ref mut modal) = agent.extensions_modal {
                ...
                modal.hooks_data =
                    TabDataState::Loaded(fuigo_hooks_plugins_types::HooksListResponse {
                        hooks,
                        project_trusted,
                        load_errors,
                    });
        ...
        FuigoSessionUpdate::PluginsChanged { plugins } => {
            if let Some(ref mut modal) = agent.extensions_modal {
                use crate::views::extensions_modal::TabDataState;
        ...
        FuigoSessionUpdate::SessionSummaryGenerated { session_summary } => {
            let title_is_manual = session_notif.meta.as_ref().and_then(|v| {
                v.get(fuigo_shell::extensions::notification::TITLE_IS_MANUAL_META_KEY)
                    .and_then(|v| v.as_bool())
            });
            match title_is_manual {
                Some(true) => {
                    if let Some(clean) =
                        fuigo_shell::session::persistence::sanitize_and_cap_title(
                            &session_summary,
                        )
                    {
                ...
                other => {
                    ...
                    if let Some(clean) =
                        fuigo_shell::session::persistence::sanitize_and_cap_title(&decoded)
                    {
                        agent.generated_session_title = Some(clean);
        ...
        FuigoSessionUpdate::LastTurnSummary {
            summary,
            prompt_id: _,
        } => {
        ...
        FuigoSessionUpdate::SessionRecap { summary, auto } => {
        ...
        FuigoSessionUpdate::SessionRecapUnavailable => {
            if meta.is_replay {
                false
        ...
        FuigoSessionUpdate::ModelAutoSwitched {
            previous_model_id,
            new_model_id,
            reason,
        } => {
        ...
        FuigoSessionUpdate::ModelChanged {
            model_id,
            reasoning_effort,
        } => {
            ...
            if !agent.session.models.available.contains_key(&new_model_id) {
                if fuigo_shell::agent::chat_modes::process_chat_mode_enabled() {
        ...
        FuigoSessionUpdate::MemoryFiles { files } => {
        ...
        update @ FuigoSessionUpdate::WorkflowUpdated { .. } => ingest_workflow_update(agent, update),
        FuigoSessionUpdate::GoalUpdated {
            goal_id,
            objective,
            status,
            phase,
        ...
        FuigoSessionUpdate::InteractionResolved { tool_call_id } => {
            agent.dismiss_resolved_interaction(&tool_call_id)
        }
        FuigoSessionUpdate::SessionStatus(status) => {
    ...
    if let Some(agent) = app.agents.get_mut(&parent_id) {
        if let Some(seq) = meta.event_seq
            ...
            && !is_workflow_update
        {
            agent.last_applied_fuigo_event_seq = Some(
                agent
                    .last_applied_fuigo_event_seq
...
/// Handle an Ferrox Labs session notification that targets a child (subagent) session.
///
                ...
                && let Some(info) = agent.subagent_sessions.get_mut(child_sid)
            {
                ...
                if let Some(cw) = info.context_window_tokens.filter(|&cw| cw > 0) {
                    info.context_usage_pct =
                        Some(fuigo_token_estimation::usage_percentage_u8(tokens_after, cw));
                }
            }
            changed
        }
...
/// Apply one Ferrox Labs session event to a child view.
...
pub(super) fn apply_session_event(
    ...
    match update {
        FuigoSessionUpdate::AutoCompactStarted { percentage, .. } => {
        ...
        FuigoSessionUpdate::AutoCompactFailed { error } => {
...
pub(super) fn apply_image_compressed(
    agent: &mut AgentView,
    images: &[fuigo_shell::extensions::notification::ImageCompressedEntry],
    message: &str,
) -> bool {
    if images.is_empty() {
...
pub(super) fn apply_retry_state(
    retry: &fuigo_shell::extensions::notification::RetryState,
    session: &mut AgentSession,
    scrollback: &mut crate::scrollback::state::ScrollbackState,
    is_api_key_auth: bool,
) {
        ...
        RetryState::Exhausted {
            ...
            if *rate_limited {
                fuigo_telemetry::session_ctx::log_event(
                    fuigo_telemetry::events::RateLimitHit {
                        model_id: session
                            .models
                            .current
                            .as_ref()
    ...
    if is_credit_limit {
        fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::CreditLimitHit {
            model_id: session
                .models
                .current
                .as_ref()
...
pub(super) fn detect_plan_mode_change(update: &acp::SessionUpdate, agent: &mut AgentView) -> bool {
    use fuigo_tools::types::SessionMode;
