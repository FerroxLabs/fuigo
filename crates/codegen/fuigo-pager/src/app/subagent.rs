...
/// Fuigo home for the replay path (overridable in tests).
#[cfg(not(test))]
fn effective_fuigo_home() -> std::path::PathBuf {
    fuigo_shell::util::fuigo_home::fuigo_home()
}

#[cfg(test)]
thread_local! {
    static REPLAY_FUIGO_HOME: std::cell::RefCell<Option<std::path::PathBuf>> =
        const { std::cell::RefCell::new(None) };
}

/// Override fuigo home for disk-replay unit tests (thread-local).
#[cfg(test)]
pub(crate) fn set_replay_fuigo_home_for_tests(home: Option<std::path::PathBuf>) {
    REPLAY_FUIGO_HOME.with(|h| *h.borrow_mut() = home);
}

#[cfg(test)]
fn effective_fuigo_home() -> std::path::PathBuf {
    if let Some(home) = REPLAY_FUIGO_HOME.with(|h| h.borrow().clone()) {
        return home;
    }
    fuigo_shell::util::fuigo_home::fuigo_home()
}
...
pub(crate) fn enrich_from_meta(
    ...
    enrich_from_meta_with_home(info, &effective_fuigo_home(), parent_cwd, parent_session_id);
}
...
pub(crate) fn evict_finished_child_view(
    ...
    if !info.transcript.evictable() && had_content {
        ...
        if !replay_would_emit(child_sid, &effective_fuigo_home(), hint).unwrap_or(false) {
            return EvictOutcome::Retained;
        }
    }
    if let Some(info) = parent.subagent_sessions.get_mut(child_sid) {
