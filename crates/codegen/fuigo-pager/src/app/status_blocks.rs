...
fn format_cost(m: &fuigo_shell::extensions::notification::PromptUsageModel) -> String {
    use fuigo_shell::extensions::notification::ticks_to_usd;
...
mod tests {
    use super::*;
    use fuigo_shell::extensions::notification::{PromptUsage, PromptUsageModel};
    ...
    fn session_usage_block_lists_models_when_multiple() {
        ...
        usage
            .model_usage
            .insert("fuigo-build".into(), model_row(100, 10, None));
        usage
            .model_usage
            .insert("grok-4".into(), model_row(50, 5, None));
        let text = session_usage_block_text(&usage);
        assert!(text.contains("By model:"), "{text}");
        assert!(text.contains("fuigo-build: 100 in / 10 out"), "{text}");
