...
//! This remains for the startup env override, the `FUIGO_SCREEN_MODE_SWITCH=exec` escape hatch, and unrecoverable transitions.
//!
...
pub(crate) const FUIGO_SCREEN_MODE_ENV: &str = "FUIGO_SCREEN_MODE";
...
/// That way a cold-start `fuigo "do the thing"` does not re-submit on resume.
...
pub(crate) fn build_screen_mode_relaunch_args(
    ...
    while let Some(arg) = iter.next() {
        ...
        // Bare positional prompt (e.g. `fuigo "fix the bug"`), which must not re-fire on resume.
    ...
    // Keep a CLI mode flag for hand-pasted resume hints that omit FUIGO_SCREEN_MODE.
    if want_minimal {
...
/// `FUIGO_SCREEN_MODE_SWITCH=exec` forces the legacy re-exec switch.
pub(crate) const SCREEN_MODE_SWITCH_ENV: &str = "FUIGO_SCREEN_MODE_SWITCH";
...
pub(crate) fn screen_mode_relaunch_resume_hint(session_id: &str, want_minimal: bool) -> String {
    ...
    format!("{FUIGO_SCREEN_MODE_ENV}={mode} fuigo {flag} --resume {session_id}")
}
...
pub(crate) fn exec_screen_mode_relaunch(session_id: &str, want_minimal: bool) -> io::Result<()> {
    ...
    cmd.env(FUIGO_SCREEN_MODE_ENV, screen_mode_env_value(want_minimal));
...
/// Parse a [`FUIGO_SCREEN_MODE_ENV`] or config `[ui] screen_mode` value (pure; unit-tested directly).
///
...
/// Consume the one-shot screen-mode override env (see [`FUIGO_SCREEN_MODE_ENV`]).
///
/// Reads **and removes** the variable so the override is truly one-shot.
/// Every spawned child (tool shells, workers, nested `fuigo` invocations) would otherwise inherit a forced screen mode the user never asked for.
///
...
pub(crate) fn take_screen_mode_env_override() -> Option<super::ScreenMode> {
    let raw = std::env::var_os(FUIGO_SCREEN_MODE_ENV);
    if raw.is_some() {
        ...
        unsafe { std::env::remove_var(FUIGO_SCREEN_MODE_ENV) };
    }
...
mod tests {
    ...
    fn adds_minimal_and_resume() {
        let out = build_screen_mode_relaunch_args(args(&["fuigo", "--no-leader"]), "abc", true);
        assert_eq!(
            as_strs(&out),
    ...
    fn adds_fullscreen_and_resume() {
        let out = build_screen_mode_relaunch_args(args(&["fuigo", "--no-leader"]), "abc", false);
        assert_eq!(
            as_strs(&out),
    ...
    fn strips_session_id_flag() {
        ...
        let out = build_screen_mode_relaunch_args(
            args(&["fuigo", "-s", "11111111-1111-1111-1111-111111111111"]),
            "new",
            false,
        );
    ...
    fn strips_prior_minimal_and_resume() {
        let out = build_screen_mode_relaunch_args(
            args(&["fuigo", "--minimal", "--resume", "old", "--no-leader"]),
            "new",
            false,
        );
        assert_eq!(
    ...
    fn strips_prior_fullscreen_flag() {
        let out = build_screen_mode_relaunch_args(
            args(&["fuigo", "--fullscreen", "--resume", "old", "--no-leader"]),
            "new",
            true,
        );
        assert_eq!(
    ...
    fn strips_resume_equals_form() {
        let out = build_screen_mode_relaunch_args(
            args(&["fuigo", "--resume=old-id", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn strips_positional_prompt() {
        let out = build_screen_mode_relaunch_args(
            args(&["fuigo", "--no-leader", "fix the bug"]),
            "sid",
            true,
        );
        assert_eq!(
    ...
    fn double_dash_and_following_positionals_dropped() {
        // `fuigo --no-leader -- "fix the bug"`: everything after `--` is the prompt
    ...
    fn keeps_equals_form_and_short_model_flag() {
        let out = build_screen_mode_relaunch_args(
            args(&["fuigo", "-m", "grok-4", "--cwd=/tmp/proj", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn resume_without_value_then_flag_is_not_eaten() {
        // `fuigo --resume --no-leader` (resume most-recent; next token is a flag).
        let out = build_screen_mode_relaunch_args(
            args(&["fuigo", "--resume", "--no-leader"]),
            "sid",
            false,
        );
        assert_eq!(
    ...
    fn failed_relaunch_hint_includes_screen_mode_env() {
        // Recovery command must carry FUIGO_SCREEN_MODE so following the hint after a failed `/fullscreen` does not reopen minimal/inline
        // The explicit flag keeps the resume in the right mode if the env is dropped
        assert_eq!(
            screen_mode_relaunch_resume_hint("abc-sid", false),
            "FUIGO_SCREEN_MODE=fullscreen fuigo --fullscreen --resume abc-sid"
        );
        assert_eq!(
            screen_mode_relaunch_resume_hint("abc-sid", true),
            "FUIGO_SCREEN_MODE=minimal fuigo --minimal --resume abc-sid"
        );
    }
