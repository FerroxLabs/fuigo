...
/// [`fuigo_shell::util::config::resolve_hints`]; this type is the pager's
/// in-memory mirror.
...
impl From<fuigo_shell::util::config::WorktreeHintMode> for WorktreeMode {
    fn from(mode: fuigo_shell::util::config::WorktreeHintMode) -> Self {
        use fuigo_shell::util::config::WorktreeHintMode;
        match mode {
...
impl WorktreeMode {
    ...
    pub fn from_config_str(s: &str) -> Self {
        fuigo_shell::util::config::WorktreeHintMode::from_config_str(s).into()
    }
...
/// Seeded once before the first render from the pure [`fuigo_workspace::folder_trust::decide`] verdict.
...
/// Cap for the `FUIGO_ESC_DOUBLE_PRESS_MS` override; the pty_e2e suite sets exactly this value.
pub const ESC_DOUBLE_PRESS_TEST_MS: u64 = 60_000;
/// Idle-Esc double-press confirm window, `FUIGO_ESC_DOUBLE_PRESS_MS`-overridable (read once, bounded).
...
pub(crate) fn esc_double_press_ttl() -> Duration {
    ...
    *TTL.get_or_init(|| parse_esc_ttl(std::env::var("FUIGO_ESC_DOUBLE_PRESS_MS").ok()))
}
...
///
/// The string classification is shared with the shell's capability (toolset) gate via [`fuigo_shell::tier::is_restricted_tier_name`].
...
fn is_restricted_tier(tier: Option<&str>) -> bool {
    match tier {
        None => true,
        Some(t) => fuigo_shell::tier::is_restricted_tier_name(t),
    }
}
...
pub struct AppView {
    ...
    pub pending_startup: Option<fuigo_telemetry::startup::PendingStartup>,
    ...
    pub current_ui: fuigo_shell::agent::config::UiConfig,
    pub cwd: PathBuf,
    ...
    /// Current appearance config (hot-reloadable from ~/.fuigo/pager.toml).
    ...
    /// Scroll-diagnostics HUD (`FUIGO_SCROLL_DEBUG` env / `/scroll-debug`).
    ...
    /// Release-safe FPS HUD (`/debug fps`; `FUIGO_FPS` env on release builds, where the dev overlay is compiled out); see the module doc.
    pub fps_hud: crate::views::fps_hud::FpsHud,
    pub active_announcements: Vec<fuigo_announcements::RemoteAnnouncement>,
    ...
    /// Env `FUIGO_PLUGIN_CTA` overrides `RemoteSettings.plugin_cta` (remote settings); defaults to `false`.
    ...
    /// `None` keeps the default Ferrox Labs Official source.
    ...
    /// The Agent Dashboard entry points (`/dashboard`, `Ctrl+\`, `fuigo dashboard`, the startup hook) are gated on this flag.
    ...
    pub workspace_store: Option<fuigo_dashboard_store::WorkspaceStore>,
    /// Initial workspace view read from [`Self::workspace_store`].
    pub workspace_snapshot: Option<fuigo_dashboard_store::WorkspaceSnapshot>,
    ...
    pub workspace_retry_metadata: std::collections::HashMap<
        fuigo_dashboard_store::SessionId,
    ...
    /// Gated by `FUIGO_SESSION_PICKER_GROUPED` env var or remote settings `session_picker_grouped`; defaults to `false`.
    ...
    /// Gated by `FUIGO_CANCEL_REWIND` env / `[features] cancel_rewind` config / remote settings flag.
    ...
    /// Default-empty and inert outside `--minimal`; the `fuigo-pager-minimal` crate reads/mutates it through the `crate::minimal_api` accessors.
    ...
    pub session_picker_content_results:
        Option<Vec<fuigo_shell::extensions::session_search::SearchSessionHit>>,
    ...
    pub(crate) foreign_session_compat: fuigo_foreign_sessions::EnabledForeignSessionSources,
    ...
    /// See `fuigo_shell::util::config::resolve_auto_permission_mode_enabled`.
    ...
    /// Precedence: `FUIGO_CONTEXTUAL_HINTS` (master) > `[ui.contextual_hints]` user config > remote tier > default.
    pub contextual_hints: fuigo_shell::util::config::ResolvedContextualHints,
    ...
    pub remote_contextual_hints: Option<fuigo_shell::util::config::ContextualHintsRemote>,
    ...
    /// One-shot gate for the SSH `fuigo wrap` tip: set after the first evaluation at a stable agent-view draw.
    ...
    pub announcement_cta_impressions_logged:
        std::collections::BTreeSet<(String, fuigo_telemetry::events::AnnouncementCtaSurface)>,
    /// Access gate from `fuigo_build_access_gate`. `Some` means blocked.
    pub gate: Option<fuigo_shell::auth::GateInfo>,
    ...
    pub pending_gate_verification: Option<fuigo_shell::auth::GateInfo>,
    ...
    /// Loaded once on startup from `~/.fuigo/config.toml`.
    ...
    /// When false (remote kill switch or `FUIGO_VOICE_MODE=0`) the STT pipeline is not started and session voice mode cannot turn on.
    ...
    pub voice_config: fuigo_voice::VoiceConfig,
    /// Auth for STT (OAuth session via shell `AuthManager`, or `FUIGO_API_KEY`).
    /// `None` until the pipeline is first started (lazy on `/voice`).
    pub voice_auth: Option<fuigo_voice::SharedVoiceAuth>,
    /// Commands into the voice pipeline (start/stop capture; toggle, not hold).
    pub voice_cmd_tx: Option<tokio::sync::mpsc::Sender<fuigo_voice::VoiceCommand>>,
...
impl AppView {
    ...
    pub fn gate_from_settings(
        rs: &fuigo_shell::util::config::RemoteSettings,
    ) -> Option<fuigo_shell::auth::GateInfo> {
    ...
    pub fn apply_auth_meta(&mut self, meta: &fuigo_shell::auth::AuthMeta) {
    ...
    pub fn new(
        ...
        Self {
            ...
            current_ui: fuigo_shell::agent::config::UiConfig::default(),
            ...
            auto_mode_gate: fuigo_shell::util::config::auto_permission_mode_enabled_from_disk(),
            ...
            voice_config: fuigo_voice::VoiceConfig::default(),
            voice_auth: None,
            voice_cmd_tx: None,
    ...
    pub fn voice_can_start_pipeline(&self) -> bool {
        self.voice_mode_enabled && fuigo_voice::AUDIO_SUPPORTED
    }
    ...
    /// Rows the dev `FUIGO_FPS` overlay occupies (0 in non-dev builds), so runtime debug overlays stack below instead of overpainting it.
        ...
        if matches!(ctx.auth_state, AuthState::Done) {
            if ctx.upgrade_cta_keyboard && key!('o', CONTROL).matches(key) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    fuigo_telemetry::events::AnnouncementCtaSurface::Keyboard,
                ));
            }
    ...
    if let Event::Mouse(mouse) = ev {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                {
                    return InputOutcome::Action(Action::AnnouncementsOpenCta(
                        fuigo_telemetry::events::AnnouncementCtaSurface::Welcome,
                    ));
                }
...
impl AppView {
    ...
    pub fn draw(&mut self, terminal: &mut PagerTerminal) {
        ...
        fuigo_telemetry::startup::record_first_frame();
    ...
    /// Log [`fuigo_telemetry::events::AnnouncementCtaShown`] for each surface whose CTA button is painted this frame.
    ...
    pub(crate) fn log_announcement_cta_impressions(&mut self) {
        ...
        let key = fuigo_announcements::announcement_hide_key(owner);
        ...
        for (surface, _) in surfaces.into_iter().filter(|(_, painted)| *painted) {
            ...
            {
                fuigo_telemetry::session_ctx::log_event(
                    fuigo_telemetry::events::AnnouncementCtaShown {
                        id: id.clone(),
                        source: surface,
                    },
                );
...
impl AppView {
    ...
    pub fn apply_contextual_hints(
        &mut self,
        resolved: fuigo_shell::util::config::ResolvedContextualHints,
    ) {
    ...
    /// One-shot SSH `fuigo wrap` tip trigger, run at the top of every `draw` right after [`Self::maybe_trigger_small_screen_tip`].
    ...
    fn apply_clipboard_probe(
        ...
        if agent.show_ephemeral_tip(
            ...
            fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::ContextualTip {
                tip: fuigo_telemetry::events::ContextualTipKind::ImageInput,
