...
impl PagerArgs {
    ...
    pub fn saved_resume_profile_for_cwd(&self, cwd: Option<&str>) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                fuigo_shell::session::persistence::resumed_session_sandbox_profile(Some(&id), cwd)
            }
            ResumeTarget::MostRecentForCwd => {
...
mod tests {
    ...
    fn leader_mgmt_list_info_kill_parse() {
        ...
        let kill =
            PagerArgs::try_parse_from(["fuigo", "leader", "kill"]).expect("fuigo leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
    ...
    fn initial_prompt_trims_and_ignores_whitespace_only() {
        let args =
            PagerArgs::try_parse_from(["fuigo", "  spaced  "]).expect("padded prompt parses");
    ...
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        ...
        let alias = PagerArgs::try_parse_from(["fuigo", "--effort", "high"])
    ...
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        ...
        let deep = PagerArgs::try_parse_from(["fuigo", "--reasoning-effort", "deep"])
            .expect("deep parses");
