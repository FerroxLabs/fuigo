...
pub enum Command {
    /// Run Fuigo without the interactive UI
    Agent(Box<AgentArgs>),
    /// Show the configuration Fuigo discovers for this directory
    Inspect {
        /// Emit machine-readable JSON output.
        #[arg(long)]
        json: bool,
    },
    /// Check terminal, clipboard, color, and input support without starting Fuigo
    ...
    Setup {
        /// Print the fetched configuration as JSON instead of installing it; writes nothing to ~/.fuigo.
        #[arg(long)]
        json: bool,
    },
...
Examples:
  fuigo wrap docker exec -it my-container bash
  fuigo wrap kubectl exec -it my-pod -- bash

See ~/.fuigo/README.md for more information.
")]
    Wrap(WrapArgs),
    ...
    Update {
        ...
        enterprise: bool,
        /// Internal: what spawned this `fuigo update` (`user_command`, `auto_background`, `leader_converge`). Hidden.
        #[arg(long, hide = true)]
    ...
    /// Show what the fuigo home (~/.fuigo) uses on disk
    ...
    ///
    /// Disabled by default and enabled server-side per account; set `FUIGO_WORKSPACE_COMMAND=1` to enable it locally for testing.
    #[command(hide = true)]
    ...
    /// Disabled when `[dashboard].enabled = false` in `~/.fuigo/config.toml` or when the `FUIGO_AGENT_DASHBOARD=0` env var is set.
    Dashboard,
}
...
/// Targets a running leader process by PID (used by `fuigo leader` / `fuigo workspace`).
#[derive(Debug, clap::Args, Clone, Default)]
pub struct LeaderTargetArgs {
    /// Leader process ID from `fuigo leader list`.
    #[arg(long)]
    pub pid: Option<u32>,
}
#[derive(Debug, clap::Args, Clone)]
...
pub struct AgentArgs {
    ...
    /// Override the public Ferrox Labs API base URL.
    #[arg(long = "fuigo-api-base-url")]
    pub fuigo_api_base_url: Option<String>,
    /// Agent runtime mode
    #[command(subcommand)]
...
pub enum AgentCmd {
    ...
    Stdio,
    /// Run the agent headlessly over the Fuigo WebSocket relay
...
#[command(
    name = "fuigo",
    version = fuigo_version::full_version(),
    about = "Fuigo TUI",
...
pub struct PagerArgs {
    ...
    #[arg(long)]
    pub cwd: Option<PathBuf>,
    /// Use a custom leader socket path instead of the default `~/.fuigo/leader.sock`.
    #[arg(
        long = "leader-socket",
        value_name = "PATH",
        global = true,
    ...
    /// Only affects `--compaction-mode segments`. Sets `FUIGO_COMPACTION_DETAIL`.
    ...
    #[arg(
        ...
        value_parser = clap::builder::PossibleValuesParser::new(
            fuigo_shell::agent::config::PermissionMode::VALID_VALUES
        )
    )]
    ...
    /// Default for all `fuigo -p` runs is to wait (up to `--background-wait-timeout`) so eval harnesses see full task completion.
    ...
    /// To default plain `fuigo` to minimal, set `[ui] screen_mode = "minimal"` in ~/.fuigo/config.toml.
    #[arg(long = "minimal")]
    pub minimal: bool,
    ...
    pub no_leader: bool,
    /// Initial prompt for the interactive session, e.g. `fuigo "fix the bug"` or `fuigo --worktree=feat "create this feature"`.
    #[arg(
        value_name = "PROMPT",
        conflicts_with_all = &["single",
        "prompt_json",
...
impl PagerArgs {
    ...
    pub(crate) fn local_resume_selection(
        &self,
    ) -> fuigo_shell::session::persistence::RecentSessionSelection {
        use fuigo_shell::session::unified_list::HeadlessPolicy;
        ...
        fuigo_shell::session::persistence::RecentSessionSelection::from_headless_policy(policy)
    }
    ...
    /// An explicit `--sandbox`/`FUIGO_SANDBOX` that differs from the saved profile is refused: changing a session's sandbox on resume would be unsafe.
    ...
    pub fn saved_resume_profile_for_cwd(&self, cwd: Option<&str>) -> Option<String> {
        ...
        match self.resume_target() {
            ResumeTarget::SessionId(id) => {
                fuigo_shell::session::persistence::resumed_session_sandbox_profile(
                    Some(&id),
                    cwd,
                )
            }
            ResumeTarget::MostRecentForCwd => {
                fuigo_shell::session::persistence::resolve_recent_session_sandbox_profile(
                    cwd,
    ...
    fn resolve_startup_sandbox(explicit: Option<&str>, saved: Option<String>) -> SandboxStartup {
        match (explicit, saved) {
            (Some(x), Some(s))
                if x.parse::<fuigo_sandbox::ProfileName>().ok()
                    != s.parse::<fuigo_sandbox::ProfileName>().ok() =>
            {
    ...
    /// This is the `fuigo "<prompt>"` launch form; the headless `-p`/`--single` path is handled separately.
...
mod tests {
    ...
    fn version_flags_parse_as_early_intent_without_exiting() {
        for flag in ["--version", "-v", "-V"] {
            let args = PagerArgs::try_parse_from(["fuigo", flag]).expect("version flag parses");
    ...
    fn ordinary_and_doctor_parsing_do_not_set_version_intent() {
        assert!(!PagerArgs::try_parse_from(["fuigo"]).unwrap().version);
        assert!(
            !PagerArgs::try_parse_from(["fuigo", "doctor"])
                .unwrap()
                .version
        );
        assert!(matches!(
            PagerArgs::try_parse_from(["fuigo", "version"])
                .unwrap()
                .command,
    ...
    fn doctor_accepts_report_and_explicit_fix_forms() {
        let bare = PagerArgs::try_parse_from(["fuigo", "doctor"]).expect("bare doctor parses");
        assert!(matches!(
            bare.command,
        ...
        let json =
            PagerArgs::try_parse_from(["fuigo", "doctor", "--json"]).expect("doctor --json parses");
        assert!(matches!(
            json.command,
            Some(Command::Doctor(crate::doctor_cmd::DoctorArgs {
                json: true,
        ...
        for id in [
            ...
            let fix = PagerArgs::try_parse_from(["fuigo", "doctor", "fix", id, "--yes"])
        ...
        let list = PagerArgs::try_parse_from(["fuigo", "doctor", "fix"])
        ...
        for unsupported in [
            vec!["fuigo", "doctor", "all"],
            vec!["fuigo", "doctor", "fix", "ssh-wrap", "extra"],
            vec!["fuigo", "doctor", "fix", "--yes"],
            vec!["fuigo", "doctor", "--json", "fix", "terminal.ssh-wrap"],
        ] {
    ...
    fn resume_target_classifies_flags() {
        assert_eq!(
            PagerArgs::try_parse_from(["fuigo"]).unwrap().resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["fuigo", "-c"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["fuigo", "--resume"])
                .unwrap()
                .resume_target(),
            ResumeTarget::MostRecentForCwd
        );
        assert_eq!(
            PagerArgs::try_parse_from(["fuigo", "--resume", "sess-1"])
                .unwrap()
                .resume_target(),
            ResumeTarget::SessionId("sess-1".to_string())
        );
        assert_eq!(
            PagerArgs::try_parse_from(["fuigo", "-s", "sess-2"])
                .unwrap()
                .resume_target(),
            ResumeTarget::None
        );
        assert_eq!(
            PagerArgs::try_parse_from(["fuigo", "-r", "old", "--fork-session"])
                .unwrap()
                .resume_target(),
    ...
    fn minimal_and_fullscreen_flags_conflict() {
        let args = PagerArgs::try_parse_from(["fuigo", "--minimal"]).unwrap();
        assert!(args.minimal && !args.fullscreen);
        let args = PagerArgs::try_parse_from(["fuigo", "--fullscreen"]).unwrap();
        assert!(args.fullscreen && !args.minimal);
        let err = PagerArgs::try_parse_from(["fuigo", "--minimal", "--fullscreen"]).unwrap_err();
    ...
    fn startup_sandbox_profile_no_resume() {
        assert_eq!(
            PagerArgs::try_parse_from(["fuigo", "--sandbox", "strict"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(Some("strict".to_string()))
        );
        assert_eq!(
            PagerArgs::try_parse_from(["fuigo", "--sandbox", ""])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
        assert_eq!(
            PagerArgs::try_parse_from(["fuigo"])
                .unwrap()
                .startup_sandbox_profile(None),
            SandboxStartup::Apply(None)
        );
    ...
    fn leader_socket_flag_parses_at_root() {
        let args = PagerArgs::try_parse_from(["fuigo", "--leader-socket", "/tmp/leader-x.sock"])
    ...
    fn leader_socket_flag_defaults_to_none() {
        let args = PagerArgs::try_parse_from(["fuigo"]).expect("bare fuigo parses");
        assert!(args.leader_socket.is_none());
    }
    #[test]
    fn leader_mgmt_list_info_kill_parse() {
        let list = PagerArgs::try_parse_from(["fuigo", "leader", "list", "--json"])
            .expect("fuigo leader list --json");
        assert!(matches!(
            list.command,
        ...
        let info = PagerArgs::try_parse_from(["fuigo", "leader", "info", "--pid", "42"])
            .expect("fuigo leader info --pid");
        assert!(matches!(
            info.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        let kill = PagerArgs::try_parse_from(["fuigo", "leader", "kill"]).expect("fuigo leader kill");
        assert!(matches!(
            kill.command,
            Some(Command::Leader(LeaderMgmtArgs {
        ...
        assert!(PagerArgs::try_parse_from(["fuigo", "leader", "profile"]).is_err());
    }
    #[test]
    fn debug_file_flag_parses_and_is_global() {
        let root = PagerArgs::try_parse_from(["fuigo", "--debug-file", "/tmp/fire.txt"])
        ...
        let sub =
            PagerArgs::try_parse_from(["fuigo", "agent", "stdio", "--debug-file", "/tmp/f.txt"])
    ...
    fn positional_prompt_seeds_interactive_session() {
        let args =
            PagerArgs::try_parse_from(["fuigo", "fix the bug"]).expect("positional prompt parses");
        assert_eq!(args.initial_prompt(), Some("fix the bug"));
        assert!(args.command.is_none());
        assert!(args.single.is_none());
    }
    #[test]
    fn bare_fuigo_has_no_initial_prompt() {
        let args = PagerArgs::try_parse_from(["fuigo"]).expect("bare fuigo parses");
        assert_eq!(args.initial_prompt(), None);
    }
    #[test]
    fn initial_prompt_trims_and_ignores_whitespace_only() {
        let args = PagerArgs::try_parse_from(["fuigo", "  spaced  "]).expect("padded prompt parses");
    ...
    fn usage_command_parses_session_and_optional_turn() {
        let session_only = PagerArgs::try_parse_from(["fuigo", "usage", "sess-1"])
            .expect("fuigo usage <session-id>");
        assert!(matches!(
        ...
        let with_turn = PagerArgs::try_parse_from(["fuigo", "usage", "sess-1", "3"])
            .expect("fuigo usage <session-id> <turn>");
        assert!(matches!(
            with_turn.command,
            Some(Command::Usage(crate::usage_cmd::UsageArgs {
                ref session_id,
    ...
    fn worktree_flag_and_initial_prompt_combine() {
        ...
        let c = PagerArgs::try_parse_from(["fuigo", "-w", "x"]).expect("-w x parses");
        assert_eq!(c.worktree.as_deref(), Some("x"));
        assert_eq!(c.initial_prompt(), None);
    }
    #[test]
    fn trust_flag_parses_on_pager_and_alias() {
        let bare = PagerArgs::try_parse_from(["fuigo"]).expect("bare fuigo parses");
        assert!(!bare.trust);
        let long = PagerArgs::try_parse_from(["fuigo", "--trust"]).expect("--trust parses");
        assert!(long.trust);
        let alias =
            PagerArgs::try_parse_from(["fuigo", "--trust-folder"]).expect("--trust-folder parses");
        assert!(alias.trust);
    }
    #[test]
    fn reasoning_effort_and_effort_alias_parse_same_field() {
        let long = PagerArgs::try_parse_from(["fuigo", "--reasoning-effort", "high"])
            .expect("--reasoning-effort parses");
        assert_eq!(long.reasoning_effort.as_deref(), Some("high"));
        let alias =
            PagerArgs::try_parse_from(["fuigo", "--effort", "high"]).expect("--effort alias parses");
        assert_eq!(alias.reasoning_effort.as_deref(), Some("high"));
    }
    #[test]
    fn reasoning_effort_accepts_max_and_remapped_ids() {
        let max = PagerArgs::try_parse_from(["fuigo", "--effort", "max"]).expect("max parses");
        assert_eq!(max.reasoning_effort.as_deref(), Some("max"));
        let deep =
            PagerArgs::try_parse_from(["fuigo", "--reasoning-effort", "deep"]).expect("deep parses");
        assert_eq!(deep.reasoning_effort.as_deref(), Some("deep"));
    }
    #[test]
    fn reasoning_effort_last_flag_wins_when_both_names_set() {
        let args =
            PagerArgs::try_parse_from(["fuigo", "--reasoning-effort", "low", "--effort", "high"])
                .expect("both effort flag names parse");
        assert_eq!(args.reasoning_effort.as_deref(), Some("high"));
        let reverse =
            PagerArgs::try_parse_from(["fuigo", "--effort", "high", "--reasoning-effort", "low"])
    ...
    fn agent_args_effort_alias_parses() {
        let args = PagerArgs::try_parse_from(["fuigo", "agent", "--effort", "max", "stdio"])
