...
impl ForeignPickerSource {
    ...
    fn skill_paths(self, fuigo_home: &Path) -> [PathBuf; 2] {
        ...
        [
            fuigo_home
                ...
                .join(skill)
                .join("SKILL.md"),
            fuigo_home.join("skills").join(skill).join("SKILL.md"),
        ]
    }
}
    ...
    Fut: Future<Output = bool>,
{
    ...
    for source in ForeignPickerSource::ALL {
        ...
        for path in source.skill_paths(fuigo_home) {
...
pub(crate) async fn gated_sources_async(
    ...
    gated_sources_async_with(compat, fuigo_home, |path| async move {
...
{
    let enabled = gated_sources_async_with(compat, fuigo_home, metadata_exists).await;
...
pub(crate) fn scan_effect(
    ...
    (compat.claude || compat.codex || compat.cursor).then(|| Effect::ScanForeignSessions {
        cwd: cwd.to_path_buf(),
        compat,
        fuigo_home: fuigo_home.to_path_buf(),
        coordinator,
        seq,
    })
}
...
mod tests {
    ...
    use fuigo_foreign_sessions::ForeignSessionSource;
    ...
    async fn async_gate_supports_bundled_and_user_skill_locations() {
        let enabled = gated_sources_async_with(compat_all(), Path::new("/fuigo"), |path| {
