...
use super::*;
use fuigo_shell::sampling::error::is_free_usage_exhausted_error;
...
fn upsell_non_max_unified_shows_buy_credits() {
    ...
    assert_eq!(
        ...
        "Purchase credits to keep using Fuigo"
    );
    assert_eq!(q.options[2].label, "Try Again");
}
...
fn complete_session_usage(
    app: &mut AppView,
    session_id: &str,
    usage: fuigo_shell::extensions::notification::PromptUsage,
) -> Vec<Effect> {
    dispatch(
...
fn team_auth_disables_agent_billing_surface() {
    ...
    app.apply_auth_meta(&fuigo_shell::auth::AuthMeta {
...
fn manage_billing_gates_on_consumer_billing_surface() {
    let out = std::env::temp_dir().join(format!("fuigo-manage-billing-{}.txt", std::process::id()));
    let _ = std::fs::remove_file(&out);
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("FUIGO_TEST_OPEN_URL_FILE", &out) };
    let mut app = test_app_with_agent();
...
fn session_usage_complete_pushes_block_and_chains_billing() {
    ...
    app.screen_mode = crate::app::ScreenMode::Minimal;
    let before = agent_scrollback_len(&app);
    let usage = fuigo_shell::extensions::notification::PromptUsage {
        totals: fuigo_shell::extensions::notification::PromptUsageModel {
            input_tokens: 1_000,
            output_tokens: 100,
            total_tokens: 1_100,
            model_calls: 3,
...
fn session_usage_complete_drops_stale_session() {
    ...
    let effects = complete_session_usage(
        &mut app,
        "old-session",
        fuigo_shell::extensions::notification::PromptUsage {
            totals: fuigo_shell::extensions::notification::PromptUsageModel {
                model_calls: 99,
...
fn billing_fetched_updates_subscription_tier() {
    let mut app = test_app_with_agent();
    dispatch_billing(&mut app, None, true, Some("superfuigo_heavy".into()));
    assert_eq!(app.subscription_tier.as_deref(), Some("superfuigo_heavy"));
}
...
/// The opener failure is simulated via a broken `FUIGO_TEST_OPEN_URL_FILE` path.
...
fn open_url_shows_manual_url_when_browser_unavailable() {
    // Point `FUIGO_TEST_OPEN_URL_FILE` at a path whose parent dir does not exist so the write fails and `open_url` returns false (BrowserUnavailable)
    let bad = std::env::temp_dir().join(format!(
        "fuigo-open-url-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("FUIGO_TEST_OPEN_URL_FILE", &bad) };
...
/// A successful open (the `FUIGO_TEST_OPEN_URL_FILE` write succeeds) must not spam a fallback system message.
#[serial_test::serial(FUIGO_TEST_OPEN_URL_FILE)]
#[test]
fn open_url_does_not_show_fallback_when_opener_succeeds() {
    let url_file =
        std::env::temp_dir().join(format!("fuigo-open-url-ok-{}.txt", std::process::id()));
...
fn open_url_welcome_toasts_single_line_url_when_browser_unavailable() {
    let bad = std::env::temp_dir().join(format!(
        "fuigo-open-url-welcome-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test` so no other test races the env var.
    unsafe { std::env::set_var("FUIGO_TEST_OPEN_URL_FILE", &bad) };
...
fn credit_limit_upsell_submit_shows_url_when_browser_unavailable() {
    ...
    let bad = std::env::temp_dir().join(format!(
        "fuigo-open-url-upsell-missing-{}/out.txt",
        std::process::id()
    ));
    // SAFETY: serialized via `serial_test`.
    unsafe { std::env::set_var("FUIGO_TEST_OPEN_URL_FILE", &bad) };
    ...
    let kind = LocalQuestionKind::CreditLimitUpsell {
        choices: vec![
            fuigo_telemetry::events::CreditLimitChoice::UpgradeTier,
            fuigo_telemetry::events::CreditLimitChoice::PurchaseCredits,
            fuigo_telemetry::events::CreditLimitChoice::RetryLastPrompt,
        ],
    };
