...
/// `fuigo dashboard` before login: the startup hook consumes the
/// `FUIGO_OPEN_DASHBOARD_AT_STARTUP` env var and stashes
...
fn auth_complete_opens_deferred_dashboard() {
    ...
    assert!(
        ...
        "deferred `fuigo dashboard` must open the dashboard after login",
    );
    assert!(
...
fn dashboard_confirm_worktree_applies_pending_model_and_plan() {
    ...
    if let Some(d) = app.dashboard.as_mut() {
        d.pending_model = Some(crate::views::dashboard::PendingDispatchModel {
            id: model_id.clone(),
            effort: Some(fuigo_shell::sampling::types::ReasoningEffort::High),
...
/// `fuigo dashboard`, and the startup hook.
#[serial_test::serial(FUIGO_AGENT_DASHBOARD)]
#[test]
...
fn workspace_busy_write_retries_only_once() {
    ...
    let attempted = fuigo_dashboard_store::NewMember {
        key: fuigo_dashboard_store::MemberKey {
            session_id: fuigo_dashboard_store::SessionId::new("saved").unwrap(),
            kind: fuigo_dashboard_store::MemberKind::Build,
        },
        origin: fuigo_dashboard_store::MemberOrigin::Local,
        metadata: fuigo_dashboard_store::MemberMetadata {
            cwd: Some("/tmp".into()),
            title: None,
            model: None,
            last_turn_summary: None,
    ...
    permanent.key.session_id = fuigo_dashboard_store::SessionId::new("permanent").unwrap();
    let _ = dispatch(
    ...
    let _ = dispatch(
        Action::TaskComplete(TaskResult::WorkspaceMembersUpserted {
            store,
            snapshot: Ok(fuigo_dashboard_store::WorkspaceSnapshot {
                grouping: fuigo_dashboard_store::Grouping::State,
                members: vec![],
                data_version: 1,
            }),
...
fn dashboard_attach_subagent_lazily_replays_deferred_transcript() {
    ...
    crate::app::subagent::set_replay_fuigo_home_for_tests(Some(home.path().to_path_buf()));
    let mut app = test_app_with_agent();
    open_dashboard(&mut app);
    let parent = AgentId(0);
    app.agents
...
fn dashboard_overlay_cycle_non_overlay_noop_when_dashboard_disabled() {
    let _guard =
        crate::test_util::EnvVarGuard::set("FUIGO_AGENT_DASHBOARD", std::path::Path::new("0"));
    let mut app = test_app_with_agent();
    let id1 = AgentId(0);
    mark_agent_nonempty(&mut app, id1);
    let id2 = AgentId(1);
        ...
        effects.iter().any(|e| matches!(
            ...
            Effect::RenameSession { agent_id, title, kind, .. }
                if *agent_id == id
                    ...
                    && *kind == fuigo_shell::session::unified_list::SessionKind::Build
        )),
    ...
    assert!(
        effects.iter().any(|e| matches!(
            ...
            Effect::RenameSession { agent_id, title, kind, .. }
                if *agent_id == id
                    && title == "Chat title"
                    && *kind == fuigo_shell::session::unified_list::SessionKind::Chat
        )),
