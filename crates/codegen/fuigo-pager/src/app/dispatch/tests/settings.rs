...
fn plugin_cta_catalog_reload_empty_candidates_resets_matched_phase() {
    ...
    let id = AgentId(0);
    {
        ...
        cta.source_url_or_path = Some(fuigo_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.into());
    ...
    let response = fuigo_hooks_plugins_types::MarketplaceListResponse {
        sources: vec![fuigo_hooks_plugins_types::MarketplaceScanResult {
            ...
            source_url_or_path: fuigo_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.into(),
...
fn clear_default_model_persists_but_keeps_live_current() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("fuigo-test"));
    let info = acp::ModelInfo::new(id.clone(), "Fuigo Test".to_string());
...
fn set_default_model_idempotent_when_already_current() {
    ...
    use std::sync::Arc;
    let mut app = test_app_with_agent();
    let id = acp::ModelId::new(Arc::from("fuigo-already"));
    let info = acp::ModelInfo::new(id.clone(), "Fuigo Already".to_string());
    let agent_id = AgentId(0);
    app.agents
        .get_mut(&agent_id)
        .unwrap()
...
/// `dispatch(Action::SetTheme("fuigoday"), &mut app)` emits exactly one `Effect::PersistSetting` and mutates `app.current_ui.theme`.
/// It also fires a toast and toggles AUTO_MODE off (kind is concrete).
///
/// The test persists `fuigoday` (a non-truecolor theme) because the asserted payload is the registry's CANONICAL, not the live theme cache.
/// `clamp_to_terminal` might fold the cache to FuigoNight in non-truecolor test environments.
...
fn set_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        ...
        let effects = dispatch(Action::SetTheme("fuigoday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*key, "theme");
                assert_eq!(*value, SettingValue::Enum("fuigoday"));
                assert_eq!(*rollback_value, SettingValue::Enum("fuigonight"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(app.current_ui.theme.as_deref(), Some("fuigoday"));
        assert!(
...
/// Uses `fuigoday` to avoid the `clamp_to_terminal` ambiguity in non-truecolor test envs.
...
fn set_auto_dark_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoDarkTheme("fuigoday".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
        ...
        assert_eq!(app.current_ui.auto_dark_theme.as_deref(), Some("fuigoday"));
    });
}
#[test]
fn set_auto_light_theme_emits_persist_setting_with_correct_payload() {
    ...
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let effects = dispatch(Action::SetAutoLightTheme("fuigonight".into()), &mut app);
        assert_eq!(effects.len(), 1);
        match &effects[0] {
            Effect::PersistSetting {
                key,
                ...
                assert_eq!(*value, SettingValue::Enum("fuigonight"));
                assert_eq!(*rollback_value, SettingValue::Enum("fuigoday"));
            }
            other => panic!("expected PersistSetting, got {other:?}"),
        }
        assert_eq!(
...
///
/// Scenario: `theme="fuigonight"` (concrete) while the system is Dark, and the user commits `auto_dark_theme="fuigoday"`.
/// The setting is dormant (parent theme is concrete, not auto), so the live display must stay on FuigoNight.
/// Uses `fuigoday` to avoid `clamp_to_terminal` ambiguity in non-truecolor envs.
#[test]
fn set_auto_dark_theme_does_not_apply_when_theme_is_not_auto() {
    with_theme_test_env(|| {
        ...
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("fuigonight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::FuigoNight,
        );
        let _ = dispatch(Action::SetAutoDarkTheme("fuigoday".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::FuigoNight,
...
///
/// Uses `FuigoDay` (non-truecolor-requiring) for the dark-mode fixture: the test environment may not report truecolor support.
/// `Theme::apply_kind` clamps truecolor-only themes (TokyoNight, RosePineMoon) to FuigoNight, so a non-truecolor theme avoids the clamp.
...
/// Uses `fuigonight` for the auto_dark_theme value to avoid `clamp_to_terminal` ambiguity.
/// We want a concrete kind that's clearly different from FuigoDay, the active resolved theme.
#[test]
fn set_auto_dark_theme_does_not_apply_when_system_is_light() {
    with_theme_test_env(|| {
        ...
        let _ = dispatch(Action::SetAutoDarkTheme("fuigonight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::FuigoDay,
...
/// Uses a non-truecolor theme (`fuigonight`) for the same clamp reason as the dark variant.
#[test]
fn set_auto_light_theme_applies_when_theme_is_auto_and_system_is_light() {
    with_theme_test_env(|| {
        ...
        let _ = dispatch(Action::SetAutoLightTheme("fuigonight".into()), &mut app);
        assert_eq!(
            crate::theme::cache::current_kind(),
            crate::theme::ThemeKind::FuigoNight,
...
fn set_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        let mut app = test_app_with_agent();
        let _ = dispatch(Action::SetTheme("fuigoday".into()), &mut app);
        let toast = read_toast(&app);
        assert!(
        ...
        assert!(
            toast.contains("Fuigo Day"),
            "toast must use display name `Fuigo Day`, not canonical `fuigoday`, got: {toast:?}",
        );
...
fn set_auto_dark_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Fuigo Day"));
        assert!(toast.contains('\u{2713}'));
    });
}
#[test]
fn set_auto_light_theme_toast_format_uses_display_name() {
    with_theme_test_env(|| {
        ...
        assert!(toast.contains("Fuigo Night"));
    });
}
...
///
/// Uses the non-truecolor themes `fuigoday` and `fuigonight` to avoid the `clamp_to_terminal` interaction in test envs without truecolor support.
#[test]
fn rollback_theme_reverts_current_ui_and_cache() {
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                key: "theme",
                rollback_value: SettingValue::Enum("fuigonight"),
    ...
    with_theme_test_env(|| {
        ...
        let _ = dispatch(
            Action::TaskComplete(TaskResult::SettingPersistFailed {
                ...
                rollback_value: SettingValue::Enum("fuigoday"),
                error: "disk full".into(),
            }),
            &mut app,
        );
        assert_eq!(app.current_ui.auto_light_theme.as_deref(), Some("fuigoday"));
    });
}
