...
fn cta_entry(name: &str, status: &str) -> fuigo_hooks_plugins_types::MarketplacePluginEntry {
...
/// Plant a Build session under the process `fuigo_home()` (OnceLock-cached; do not rely on setting `FUIGO_HOME` mid-process).
...
fn plant_local_build_session(cwd: &std::path::Path, session_id: &str) -> std::path::PathBuf {
    ...
    let encoded = fuigo_shell::util::fuigo_home::encode_cwd_dirname(&cwd.to_string_lossy());
...
fn make_ask_user_question_args(
    ...
    let req = AskUserQuestionExtRequest {
        ...
        mode: fuigo_tools::implementations::fuigo_build::ask_user_question::AskUserQuestionMode::Default,
...
fn push_synthetic_permission(
    ...
    let request = fuigo_acp_lib::AcpArgs {
