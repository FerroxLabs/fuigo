...
fn cta_outcome_reload(
    status: fuigo_hooks_plugins_types::OutcomeStatus,
    message: &str,
    ...
    let response = fuigo_hooks_plugins_types::MarketplaceListResponse {
        sources: vec![
            ...
            fuigo_hooks_plugins_types::MarketplaceScanResult {
                ...
                source_kind: "git".into(),
                source_url_or_path: "git@github.com:fuigo-org/plugin-marketplace.git".into(),
...
fn plugin_cta_marketplace_override_naming_official_selects_it() {
    ...
    app.plugin_cta_marketplace =
        Some(fuigo_plugin_marketplace::OFFICIAL_SOURCE_NAME.to_string());
    let id = AgentId(0);
...
fn cta_install_error_category_maps_outcome() {
    use fuigo_hooks_plugins_types::OutcomeStatus;
    assert_eq!(
    ...
    for phase in [
        ...
        let id = AgentId(0);
        {
            ...
            cta.source_url_or_path =
                Some(fuigo_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.into());
            cta.candidates = vec![cta_entry("figma", "not_installed")];
            cta.debounce_generation = 1;
...
fn cta_install_done_non_success_sets_error_with_sanitized_message() {
    ...
    let (pattern, expected) = fuigo_shell::sampling::error::SERVICE_NAME_REWRITES[0];
    let effects = dispatch(
        Action::TaskComplete(TaskResult::CtaPluginInstallDone {
            agent_id: id,
            plugin_name: "figma".into(),
...
mod cta_e2e {
    ...
    fn figma_candidate() -> fuigo_hooks_plugins_types::MarketplacePluginEntry {
        let mut entry = cta_entry("figma", "not_installed");
        entry.keywords = vec!["figma".into()];
    ...
    fn happy_path_with_auth_handoff() {
        ...
        let servers = vec![
            cta_mcp_server("fuigo_com_managed", None, McpServerDisplayStatus::Ready),
    ...
    fn dismissed_plugin_suppressed_on_next_recompute() {
        ...
        let dismissed = fuigo_shell::config::dismissed_plugin_ctas_in_file(&config_path);
        let matched =
        ...
        fuigo_shell::config::add_dismissed_plugin_cta_to_file("figma", &config_path)
        ...
        for (enabled, source_present) in [(false, true), (true, false)] {
            ...
            app.plugin_cta_enabled = enabled;
            {
                ...
                cta.source_url_or_path = source_present
                    .then(|| fuigo_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL.to_string());
                cta.candidates = vec![figma_candidate()];
                cta.debounce_generation = 1;
                cta.phase = CtaPhase::Matched {
                    plugin_relative_path: "plugins/figma".into(),
