...
/// Resume into a cwd with `.git/fuigo-worktree-source` sets `session.is_worktree`.
#[test]
fn load_session_marks_standalone_worktree_cwd() {
    ...
    assert!(
        ...
        "resume into a standalone fuigo worktree must set session.is_worktree"
    );
...
/// A failed `fuigo/prompt_history` fetch arrives as `PromptHistoryLoaded` with an empty list.
#[test]
...
/// The fast foreign fetch has landed (its rows hidden behind the Fuigo-default CTA) while the native fetch is still in flight.
