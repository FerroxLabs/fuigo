...
fn seed_foreign_resume_hint(app: &mut AppView, tool: fuigo_foreign_sessions::ForeignSessionTool) {
    ...
    app.apply_foreign_resume_detection(
        launch_token,
        &canonical_cwd,
        Some(fuigo_foreign_sessions::RecentForeignSession {
            tool,
...
fn critical_announcement(id: &str) -> fuigo_announcements::RemoteAnnouncement {
...
fn promo_announcement(id: &str) -> fuigo_announcements::RemoteAnnouncement {
    fuigo_announcements::RemoteAnnouncement {
        id: Some(id.into()),
        message: Some(format!("{id} message")),
        severity: Some("promo".into()),
        cta: Some(fuigo_announcements::AnnouncementCta {
            label: Some("Go".into()),
...
/// The opens are observed through the file named by `FUIGO_TEST_OPEN_URL_FILE`.
...
fn announcements_open_cta_opens_promo_and_noops_under_critical() {
    ...
    let url_file = std::env::temp_dir().join(format!("fuigo-cta-open-{}.txt", std::process::id()));
...
fn tick_propagates_available_commands_to_bootstrap() {
    ...
    let skill_meta = serde_json::json!({
        "scope": "user",
        "path": "/home/user/.fuigo/skills/pick-best/SKILL.md",
    });
    ...
    assert!(
        matches!(
            ...
            [Effect::RenameSession { kind, .. }]
                if *kind == fuigo_shell::session::unified_list::SessionKind::Chat
        ),
...
fn load_sticky_chat_history_bypass_rename_kind_is_build() {
    ...
    assert_eq!(
        agent.rename_kind(),
        fuigo_shell::session::unified_list::SessionKind::Build
    );
    let rename = dispatch(
        Action::RenameSession {
    ...
    assert!(
        matches!(
            ...
            [Effect::RenameSession { kind, title, .. }]
                if *kind == fuigo_shell::session::unified_list::SessionKind::Build
                    && title == "local title"
        ),
...
fn welcome_expand_skips_conversation_and_routes_build_card_detail() {
    ...
    assert_eq!(
        welcome_card_detail(&app),
        None,
        "Headless must not resurrect a cleared Fuigo row from card detail"
    );
}
