...
///    That is what flips `[ui] permission_mode` on disk AND fires the `fuigo/yolo_mode_changed` ACP notification back to the shell.
...
fn enable_always_approve_sends_response_and_flips_yolo_and_persists() {
    ...
    //     ~/.fuigo/config.toml AND fires fuigo/yolo_mode_changed.
    let persist = effects
        .iter()
        .find_map(|e| match e {
...
/// The `set_yolo_mode_inner` early-return at the `app.active_view` guard MUST precede the `fuigo_telemetry::log_event` call.
