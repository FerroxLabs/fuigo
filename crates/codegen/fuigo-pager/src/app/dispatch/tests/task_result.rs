...
use super::*;
use fuigo_shell::session::helpers::session_compact::COMPACT_CANCELLED_MSG;
...
fn doctor_planning_opens_refuses_remote_and_rejects_stale_identity() {
    ...
    dispatch_task_result(
        TaskResult::DoctorFixPlanned {
            ...
            result: Ok(crate::app::actions::DoctorPlanningOutcome::RunLocally(
                "fuigo doctor fix ssh-wrap".to_owned(),
            )),
        },
        &mut app,
    );
    assert!(
        last_system_text(&app, id)
            .contains("On your local computer, run: fuigo doctor fix ssh-wrap")
    );
...
) -> fuigo_foreign_sessions::RecentForeignSession {
    fuigo_foreign_sessions::RecentForeignSession {
        tool,
...
fn foreign_resume_results_require_launch_token_and_canonical_cwd() {
    ...
    launch.foreign_session_compat = fuigo_foreign_sessions::EnabledForeignSessionSources {
        cursor: true,
        ..Default::default()
    };
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                fuigo_foreign_sessions::ForeignSessionTool::Cursor,
            )),
        }),
        &mut launch,
    );
    assert_eq!(
        ...
        Some(fuigo_foreign_sessions::ForeignSessionTool::Cursor)
    );
    ...
    dispatch(
        Action::TaskComplete(TaskResult::ForeignResumeHintDetected {
            ...
            hint: Some(foreign_resume_hint(
                fuigo_foreign_sessions::ForeignSessionTool::Codex,
            )),
        }),
        &mut stale,
    );
...
fn wrap_host_image_request_eligible_covers_full_miss_and_attachment_error_only() {
    ...
    // That request is how `fuigo wrap` pastes images over headless SSH
...
fn marketplace_list_loaded_sanitizes_components_at_ingestion() {
    ...
    entry.components = Some(fuigo_hooks_plugins_types::PluginComponents {
        skills: vec![fuigo_hooks_plugins_types::ComponentItem {
...
fn plugins_action_success_sets_result_notice_and_autoreload_preserves_it() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::PluginsActionResult {
            agent_id: id,
            result: Ok(fuigo_hooks_plugins_types::ActionOutcome {
...
fn uninstall_result_notice_is_footer_only_not_row_anchored() {
    ...
    let id = AgentId(0);
    {
        ...
        modal.last_plugins_action = Some(fuigo_hooks_plugins_types::PluginsAction::Uninstall {
...
fn confirmation_required_builds_plugins_confirmation_with_confirmed_true() {
    ...
    dispatch(
        Action::TaskComplete(TaskResult::PluginsActionResult {
            agent_id: id,
            result: Ok(fuigo_hooks_plugins_types::ActionOutcome {
                status: fuigo_hooks_plugins_types::OutcomeStatus::ConfirmationRequired,
    ...
    match &modal.modal_message {
        Some(ModalMessage::Confirmation {
            ...
            assert_eq!(
                action,
                &ConfirmationAction::Plugins(fuigo_hooks_plugins_types::PluginsAction::Uninstall {
                    plugin_id: "user/ab12/gone".into(),
                    confirmed: true,
                })
            );
...
fn switch_to_non_reasoning_model_clears_persisted_effort() {
    ...
    app.agents
        ...
        .insert(
            model_id.clone(),
            acp::ModelInfo::new(model_id.clone(), "Fuigo".to_string()),
        );
    app.agents
        .get_mut(&id)
        .unwrap()
...
fn switch_model_incompatible_agent_shows_question_modal() {
    ...
    let err = fuigo_shell::agent::config::ModelSwitchIncompatibleAgentError {
        code: "MODEL_SWITCH_INCOMPATIBLE_AGENT".into(),
        active_agent_type: "fuigo-build".into(),
...
fn same_agent_type_switch_no_modal() {
    ...
    let mut app = test_app_with_agent();
    let id = AgentId(0);
    let model_a = acp::ModelId::new(std::sync::Arc::from("fuigo-build-a"));
    let model_b = acp::ModelId::new(std::sync::Arc::from("fuigo-build-b"));

    // Add both models to the catalog (no agentType, so both use fuigo-build)
    let agent = app.agents.get_mut(&id).unwrap();
    agent.session.models.available.insert(
        model_a.clone(),
        acp::ModelInfo::new(model_a.clone(), "Fuigo A".to_string()),
    );
    agent.session.models.set_current(model_a, None);
    agent.session.models.available.insert(
        model_b.clone(),
        acp::ModelInfo::new(model_b.clone(), "Fuigo B".to_string()),
    );
    agent.session.model_switch_pending = true;
...
fn session_success_arms_finish_startup_obligation() {
    ...
    for result in results {
        let mut app = test_app_with_agent();
        app.agents.get_mut(&id).unwrap().session.session_id = None;
        app.pending_startup = Some(fuigo_telemetry::startup::PendingStartup::new());
...
fn delete_both_session_clears_modal_and_welcome_content_hits() {
    ...
    let hit = fuigo_shell::extensions::session_search::SearchSessionHit {
...
fn gate_refreshed_emits_check_subscription_on_gate_lift() {
    ...
    let settings = fuigo_shell::util::config::RemoteSettings::default();
...
fn verify_check_with_meta_resolves_pending_gate() {
    ...
    let meta = serde_json::to_value(fuigo_shell::auth::AuthMeta::default()).unwrap();
    dispatch_task_result(
...
fn verify_check_with_gated_meta_shows_gate() {
    ...
    let meta = serde_json::to_value(fuigo_shell::auth::AuthMeta {
        gate: Some(test_gate()),
        ..Default::default()
    })
    .unwrap();
...
fn compact_complete_error_surfaces_real_error_in_scrollback() {
    use fuigo_shell::session::helpers::session_compact::{CompactErrorKind, compact_error_data};
