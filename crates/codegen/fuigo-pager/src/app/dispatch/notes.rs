...
pub(crate) fn log_trace_consent_selected(reenables_sharing: bool, choice: FeedbackTraceChoice) {
    use fuigo_telemetry::events::{FeedbackTraceConsentChoice, FeedbackTraceConsentSelected};
    fuigo_telemetry::session_ctx::log_event(FeedbackTraceConsentSelected {
        choice: match choice {
        ...
        Some(FeedbackTraceChoice::AlwaysUpload) => {
            ...
            if app.coding_data_retention_opt_out {
                let (sharing, outcome) = super::status::set_coding_data_sharing_tracked(
                    app,
                    true,
                    fuigo_telemetry::events::CodingDataConsentSource::FeedbackTraceCard,
                );
...
fn encode_feedback_images(
    images: crate::views::prompt_widget::FeedbackImages,
) -> (Vec<fuigo_shell::session::FeedbackImage>, Option<String>) {
    use base64::Engine as _;
    use fuigo_shell::session::{
    ...
    for image in images.as_slice() {
        ...
        encoded.push(fuigo_shell::session::FeedbackImage {
            ...
            Ok(()) => {
                agent
                    ...
                    .push_block(crate::scrollback::block::RenderBlock::system(format!(
                        "Memory saved to {}",
                        crate::util::display_user_fuigo_path("memory/MEMORY.md")
                    )));
            }
            Err(error) => {
                agent
