...
fn log_coding_data_consent_selected(
    source: fuigo_telemetry::events::CodingDataConsentSource,
    opted_in: bool,
    previous_opted_in: bool,
) {
    use fuigo_telemetry::events::{CodingDataConsentChoice, CodingDataConsentSelected};
    fuigo_telemetry::session_ctx::log_event(CodingDataConsentSelected {
        source,
...
/// The shell owns this setting and stores it in auth metadata (persists via ACP ext-request, NOT `~/.fuigo/config.toml`).
...
pub(super) fn append_consumer_billing_surface(app: &mut AppView, agent_id: AgentId) -> Vec<Effect> {
    ...
    // Remote-settings kill switch (`fuigo_build_usage_redirect_url`): link out instead of fetching billing from the backend
...
pub(in crate::app::dispatch) fn dispatch_privacy_banner_opt_in(app: &mut AppView) -> Vec<Effect> {
    ...
    set_coding_data_sharing(
        app,
        true,
        fuigo_telemetry::events::CodingDataConsentSource::PrivacyBanner,
    )
}
