...
use fuigo_workspace::permission::{BashCommandSelectedTerms, McpScopeSelection};
...
///
/// Special case for [`fuigo_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID`]:
...
/// "always-approve"` to `~/.fuigo/config.toml`, and fire the
/// `fuigo/yolo_mode_changed` ACP notification.
...
pub(super) fn dispatch_permission_select(
    ...
    let enable_always_approve =
        option_id.0.as_ref() == fuigo_workspace::permission::ENABLE_ALWAYS_APPROVE_OPTION_ID;
    ...
    let steers_next_cursor = !enable_always_approve
        && option_id.0.as_ref() != fuigo_workspace::permission::ALLOW_EDITS_SESSION_OPTION_ID;
