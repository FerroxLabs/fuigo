...
pub(super) fn plugin_cta_candidates(
    ...
    Vec<fuigo_hooks_plugins_types::MarketplacePluginEntry>,
    Option<String>,
) {
    ...
    let winner = match cta_marketplace {
        ...
        None => sources
            .iter()
            .position(|s| {
                fuigo_plugin_marketplace::is_official_source_url(&s.source_url_or_path)
            })
            .or_else(|| {
                sources.iter().position(|s| {
                    s.source_name == fuigo_plugin_marketplace::OFFICIAL_SOURCE_NAME
                })
            }),
    };
...
pub(super) fn cta_install_relative_path(
    candidates: &[fuigo_hooks_plugins_types::MarketplacePluginEntry],
    name: &str,
) -> String {
    candidates
        .iter()
...
pub(super) fn plugin_cta_phase_for(
    ...
    use fuigo_plugin_marketplace::matcher::{KeywordCandidate, match_plugin_keyword};
...
pub(super) fn handle_cta_plugin_install_done(
    ...
    log_event(fuigo_telemetry::events::PluginCtaInstalled {
    ...
    let install_result = match result {
        Ok(outcome) if outcome.status == fuigo_hooks_plugins_types::OutcomeStatus::Success => {
...
pub(super) fn handle_cta_plugin_reload_done(
    ...
    let reload_result = match result {
        Ok(outcome) if outcome.status == fuigo_hooks_plugins_types::OutcomeStatus::Success => Ok(()),
        Ok(outcome) => Err(crate::app::effects::sanitize_user_error(&outcome.message)),
        Err(e) => Err(e),
    };
    match reload_result {
        ...
        Ok(servers) => {
            ...
            if needs_auth {
                ...
                log_event(fuigo_telemetry::events::ExtensionsModalOpened {
                    trigger: fuigo_telemetry::events::ExtensionsModalTrigger::AuthHandoff,
    ...
    match result {
        Ok(mut response) => {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                ...
                if enabled {
                    agent.plugin_cta.dismissed = fuigo_shell::config::dismissed_plugin_ctas();
                }
                ...
                if matches!(
                    ...
                    {
                        log_event(fuigo_telemetry::events::PluginCtaImpression {
