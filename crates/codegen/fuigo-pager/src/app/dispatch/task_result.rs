...
/// Whether a completed clipboard probe should fall through to the `fuigo wrap` host-image request.
/// A clean `FullMiss` always qualifies; a remote read *error* (`AttachmentRead`) qualifies too.
/// Inside `fuigo wrap` the authoritative pasteboard is the local host's, not the (absent) remote one.
...
/// The request itself still self-gates on `osc52_sink_active()`, so this is inert outside `fuigo wrap`.
    ...
    match result {
        ...
        TaskResult::SessionMetaFromDisk {
            ...
            if let Some(agent) = app.agents.get_mut(&agent_id) {
                if let Some((raw, is_manual)) = title
                    && let Some(t) = fuigo_shell::session::persistence::sanitize_and_cap_title(&raw)
                {
        ...
        TaskResult::WorkspaceSnapshotLoaded { store, snapshot } => {
            app.workspace_writes_disabled = !matches!(
                store.schema_state(),
                fuigo_dashboard_store::SchemaState::Current
            );
        ...
        TaskResult::WorkspaceMembersUpserted {
            ...
            if app.workspace_writes_disabled {
                ...
                app.show_toast("Dashboard workspace is read-only in this Fuigo version");
            } else {
        ...
        TaskResult::ChangelogFetched { markdown, entries } => {
            ...
            app.changelog_bullets = fuigo_shell::util::changelog::bullets_from_entries(&entries, 3);
            vec![]
        }
