...
//!
//! Env sandboxing follows this crate's `serial(FUIGO_HOME)` idiom.
//! `fuigo_home()` is process-cached (OnceLock), so disk assertions always go through [`effective_fuigo_home`] rather than assuming the temp dir won.
//!
//! The scenarios are `#[ignore]`d in the shared lib test binary.
//! The harness mutates process-global env (proxy URLs, `FUIGO_API_KEY`, `FUIGO_LEADER_SOCKET`, `FUIGO_HOME`) for a real agent's whole lifetime.
//! In a several-thousand-test process that mutation poisons concurrently-running tests; `fuigo_home()`'s OnceLock is usually already pinned too.
//! Run on demand:
//!
//! ```bash
//! cargo test -p fuigo-pager --lib -- app::leader_cluster --ignored --test-threads=1
//! ```
//!
...
/// The fuigo home the agent actually persisted under: `fuigo_home()` is process-cached, so an earlier test in this binary may have pinned it.
fn effective_fuigo_home() -> PathBuf {
    fuigo_config::fuigo_home()
}
...
struct PagerLeaderCluster {
    ...
    /// `kill_leader` aborts and drains them so a respawn can never race a still-running old agent on the same FUIGO_HOME.
...
impl PagerLeaderCluster {
    ...
    /// Callers MUST be `#[serial_test::serial(FUIGO_HOME)]` (env mutation) and run inside a current-thread `LocalSet`.
    async fn start() -> Self {
        ...
        let sock_path = fuigo_home.path().join("leader-cluster.sock");

        let env = vec![
            crate::test_util::EnvVarGuard::set("FUIGO_HOME", fuigo_home.path()),
            crate::test_util::EnvVarGuard::set("FUIGO_CLI_CHAT_PROXY_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("FUIGO_API_BASE_URL", server.url()),
            crate::test_util::EnvVarGuard::set("FUIGO_API_KEY", "test-key-for-ci"),
            crate::test_util::EnvVarGuard::set("FUIGO_TELEMETRY_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("FUIGO_FEEDBACK_ENABLED", "false"),
            crate::test_util::EnvVarGuard::set("FUIGO_TRACE_UPLOAD", "false"),
    ...
    async fn spawn_leader_generation(&mut self) {
        ...
        let control_state = LeaderServerControlState::new(LeaderServerMetadata {
            ...
            // MUST be the client-side comparison source (fuigo_version), not this crate's version
        ...
        generation_tasks.extend(fuigo_shell::leader::in_process::spawn_agent(
            acp_rx,
            response_tx,
        ));
    ...
    async fn kill_leader(&mut self) {
        ...
        // Without the drain an old agent task could still run against the same FUIGO_HOME when the next generation's agent starts
    ...
    /// The socket is pinned via `FUIGO_LEADER_SOCKET` and the cluster holds the flock, so reconnects always adopt the in-process server.
