...
fn app_draw_drains_deferred_release_after_flush() {
    ...
    let mut terminal = fuigo_ratatui_inline::Terminal::with_options(
        backend,
        TerminalOptions {
...
/// That holds even after the fast foreign scan lands rows the default Fuigo filter hides; once the native list settles the demand parks again.
#[test]
fn tick_demand_fast_while_modal_session_picker_loads() {
    ...
    assert_eq!(
        app.tick_demand(),
        TickDemand::Fast,
        "foreign rows hidden by the Fuigo filter must not end the loading spinner"
    );
...
fn slash_gate_resyncs_when_critical_expires_between_pushes() {
    ...
    app.active_announcements = vec![fuigo_announcements::RemoteAnnouncement {
...
fn esc_owned_before_agent_covers_app_level_owners() {
    ...
    app.import_claude_modal = Some(
        crate::views::import_claude_modal::ImportClaudeModalState::new(
            fuigo_shell::claude_import::ImportPlan::default(),
...
fn overlay_left_arrow_file_search_open_does_not_exit() {
    let (mut app, id) = neutral_overlay_app();
    {
        ...
        agent.prompt.file_search.set_test_state(
            ctx,
            vec![fuigo_workspace::file_system::FuzzyMatchResult {
...
fn welcome_picker_f_cycle_disabled_under_chat_mode() {
    ...
    assert_eq!(
        ...
        crate::views::session_picker::SourceFilter::Fuigo,
