...
use fuigo_shell::session::unified_list::SessionKind;
...
pub enum SwitchModelError {
    ...
    IncompatibleAgent {
        error: fuigo_shell::agent::config::ModelSwitchIncompatibleAgentError,
...
pub enum Action {
    ...
    ToggleMouseCapture,
    /// Toggle the scroll-diagnostics HUD (hidden `/scroll-debug` command, also `/debug scroll`; `FUIGO_SCROLL_DEBUG=1` enables it from startup).
    ToggleScrollDebugHud,
    /// Toggle the release-safe FPS HUD (`/debug fps`).
    ToggleFpsHud,
    /// Toggle the scroll flight recorder at runtime (`/debug log`; `FUIGO_SCROLL_LOG=1` enables it from startup).
    ToggleScrollLog,
    ...
    OpenTranscriptPager,
    /// Minimal mode (`fuigo --minimal`): re-print the last committed folded block, fully expanded, into native scrollback below the conversation.
    ...
    OpenExtensionsModal {
        ...
        trigger: fuigo_telemetry::events::ExtensionsModalTrigger,
    },
    ...
    ExecuteHooksAction(fuigo_hooks_plugins_types::HooksAction),
    ...
    AnnouncementsOpenCta(fuigo_telemetry::events::AnnouncementCtaSurface),
...
pub enum Effect {
    ...
    ScanForeignSessions {
        cwd: std::path::PathBuf,
        compat: fuigo_foreign_sessions::EnabledForeignSessionSources,
    ...
    UpsertWorkspaceMembers {
        store: fuigo_dashboard_store::WorkspaceStore,
        members: Vec<fuigo_dashboard_store::NewMember>,
    },
    ...
    /// Persist the dashboard's `[dashboard]` configuration to `~/.fuigo/config.toml`.
    ...
    /// Persist a typed setting to `~/.fuigo/config.toml`. On failure,
    ...
    SendFeedback {
        ...
        images: Vec<fuigo_shell::session::FeedbackImage>,
    },
    ...
    /// registry (`~/.fuigo/active_sessions.json`).
...
pub enum TaskResult {
    ...
    ForeignResumeHintDetected {
        canonical_cwd: std::path::PathBuf,
        launch_token: u64,
        hint: Option<fuigo_foreign_sessions::RecentForeignSession>,
    },
    ...
    WorkspaceMembersUpserted {
        store: fuigo_dashboard_store::WorkspaceStore,
        snapshot: Result<fuigo_dashboard_store::WorkspaceSnapshot, String>,
        failures: Vec<WorkspaceMemberUpsertFailure>,
        attempted: Vec<fuigo_dashboard_store::NewMember>,
    },
    ...
    BgTaskKilled {
        session_id: String,
        task_id: String,
        outcome: Option<fuigo_tools::types::KillOutcome>,
    },
    ...
    ChangelogFetched {
        ...
        entries: Vec<fuigo_shell::util::changelog::ChangelogEntry>,
    },
    ...
    HooksListLoaded {
        agent_id: AgentId,
        result: Result<fuigo_hooks_plugins_types::HooksListResponse, String>,
    },
    /// Plugins list fetched from shell.
    PluginsListLoaded {
        agent_id: AgentId,
        result: Result<fuigo_hooks_plugins_types::PluginsListResponse, String>,
    },
    /// Hooks action completed.
    HooksActionResult {
        agent_id: AgentId,
        result: Result<fuigo_hooks_plugins_types::ActionOutcome, String>,
    },
    ...
    MarketplaceListLoaded {
        agent_id: AgentId,
        result: Result<fuigo_hooks_plugins_types::MarketplaceListResponse, String>,
    },
    ...
    SkillsListLoaded {
        agent_id: AgentId,
        result: Result<Vec<fuigo_tools::implementations::skills::types::SkillInfo>, String>,
    },
    WorkflowsListLoaded {
        agent_id: AgentId,
        session_id: acp::SessionId,
    ...
    SessionInfoComplete {
        agent_id: AgentId,
        session_id: acp::SessionId,
        info: Box<fuigo_shell::session::SessionInfoResponse>,
    ...
    SessionUsageComplete {
        agent_id: AgentId,
        session_id: acp::SessionId,
        usage: Box<fuigo_shell::extensions::notification::PromptUsage>,
        nonce: u64,
    },
