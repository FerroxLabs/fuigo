...
use fuigo_shell::extensions::notification::GoalClassifierVerdict;
...
pub enum AgentCommand {
    ...
    /// Drives the spinner shown on the placeholder agent while the `fuigo/session/fork` request is in flight.
    ForkSession,
}
impl AgentCommand {
...
impl BgTaskState {
    ...
    pub fn tombstone_from_snapshot(
        snapshot: &fuigo_tools::types::TaskSnapshot,
        status: BgTaskStatus,
...
pub struct AgentSession {
    ...
    /// Prompt history for the current session, fetched from ACP (`fuigo/prompt_history` scoped via `filter_session_id`). Most-recent-first.
    /// Fetched on session create/load; prompts sent in this session are additionally front-inserted locally on send.
    pub prompt_history: Vec<String>,
    /// True until the session's startup/load `fuigo/prompt_history` fetch completes.
...
pub struct ChipElement {
    ...
    pub kind: fuigo_ratatui_textarea::ElementKind,
...
impl AgentSession {
    ...
    /// Pop the front entry, merging consecutive plain `Prompt` followers via [`fuigo_prompt_queue::combine_prefix_len`].
    ...
    pub fn dequeue_combined_prompt(&mut self, editing_id: Option<u64>) -> Option<QueuedPrompt> {
        use fuigo_prompt_queue::{CombineGate, TEXT_SEPARATOR, combine_prefix_len, join_texts};
