...
pub enum DeferredSessionStartup {
    ...
    /// Fresh plain Fuigo session whose first prompt resumes a foreign tool session.
    ForeignResume {
        tool: fuigo_foreign_sessions::ForeignSessionTool,
        native_id: String,
    },
}
...
pub fn fork_session_params(
    ...
    let source_cwd = fuigo_shell::session::resolve_local_session_any_cwd(parent_session_id)
...
pub fn parent_session_is_worktree(session_id: &str, cwd: &Path) -> bool {
    ...
    let sessions_root = fuigo_shell::util::fuigo_home::fuigo_home().join("sessions");
    let encoded = fuigo_shell::util::fuigo_home::encode_cwd_dirname(&cwd_str);
    ...
    for dir in cwd.ancestors() {
        ...
        if git.is_dir() {
            return std::fs::read_to_string(git.join("fuigo-worktree-source"))
...
pub fn local_build_session_on_disk(session_id: &str, cwd: &Path) -> bool {
    let cwd_str = cwd.to_string_lossy();
    fuigo_shell::session::resolve_local_session(session_id, &cwd_str).is_some()
}
...
pub use fuigo_shell::session::persistence::RecentSessionSelection;
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No session found for current directory. \
                 Use 'fuigo' to start a new session."
            )
        })?;
...
pub(crate) fn pre_acp_auth_manager(
    agent_config: &fuigo_shell::agent::config::Config,
) -> std::sync::Arc<fuigo_shell::auth::AuthManager> {
    let auth = std::sync::Arc::new(fuigo_shell::auth::AuthManager::new(
        &fuigo_shell::util::fuigo_home::fuigo_home(),
        agent_config.fuigo_com_config.clone(),
    ));
    auth.configure_refresher(
        agent_config.fuigo_com_config.auth_provider_command.clone(),
        None,
    );
    auth
}
...
pub fn ensure_session_id_available(session_id: &str, cwd: &str) -> anyhow::Result<()> {
    ...
    if fuigo_shell::session::persistence::session_exists_for_cwd(session_id, cwd) {
...
async fn resolve_existing_session(
    ...
    if let Some(local_id) = fuigo_shell::session::resolve_local_session(session_id, cwd) {
...
async fn restore_session_from_remote(
    ...
    if let Some((false, source)) =
        fuigo_shell::util::config::session_registry_local_override_sourced(Some(&raw_config))
    {
        anyhow::bail!(
    ...
    let agent_config = fuigo_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {}", e))?;
    use fuigo_shell::agent::session_registry_client::SessionRegistryClient;
    use fuigo_shell::auth::{AuthManager, ensure_authenticated_or_noninteractive};
    use fuigo_shell::session::restore::{RestoreSessionOpts, restore_session_with_storage};
    ...
    let storage_client = fuigo_shell::auth::credential_provider::build_storage_client_for_proxy(
    ...
    let progress: fuigo_shell::session::restore::ProgressCallback = Box::new(move |event| {
    ...
    let recovered_local_id = fuigo_shell::session::find_local_child_for_remote(session_id, cwd);
...
mod tests {
    ...
    fn deferred_startup_owner_take_is_atomic() {
        let mut actions = DeferredStartupActions {
            session: Some(DeferredSessionStartup::ForeignResume {
                tool: fuigo_foreign_sessions::ForeignSessionTool::Cursor,
    ...
    fn intent_default_is_new_auto() {
        assert_eq!(
            parse(&["fuigo"]).session_startup_intent().unwrap(),
    ...
    fn intent_continue() {
        assert_eq!(
            parse(&["fuigo", "-c"]).session_startup_intent().unwrap(),
            SessionStartupIntent::Resume {
                session_id: None,
                most_recent_for_cwd: true,
            }
    ...
    fn intent_fork_with_resume() {
        assert_eq!(
            parse(&["fuigo", "-r", "old", "--fork-session"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
    ...
    fn intent_fork_with_resume_and_new_id() {
        assert_eq!(
            parse(&["fuigo", "-r", "old", "--fork-session", "-s", "new"])
                .session_startup_intent()
                .unwrap(),
            SessionStartupIntent::ForkFrom {
                source_session_id: Some("old".into()),
    ...
    fn intent_fork_alone_errors() {
        let err = parse(&["fuigo", "--fork-session"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_fork_with_worktree_errors() {
        let err = parse(&["fuigo", "-r", "a", "--fork-session", "-w"])
            .session_startup_intent()
            .unwrap_err();
    ...
    fn intent_from_flags_matches_pager_args() {
        let args = parse(&["fuigo", "-r", "old", "--fork-session", "-s", "new"]);
    ...
    fn materialize_ctx_recent_selection_follows_surface() {
        assert_eq!(
            MaterializeCtx::from_pager_args(&parse(&["fuigo"])).recent_session_selection,
            RecentSessionSelection::Interactive,
        );
        assert_eq!(
            MaterializeCtx::from_pager_args(&parse(&["fuigo", "-p", "run"]))
    ...
    async fn most_recent_fork_selection_follows_surface() {
        ...
        for (args, expected_parent) in [
            (&["fuigo", "-c", "--fork-session"][..], interactive_id),
            (
                &["fuigo", "-p", "run", "-c", "--fork-session"][..],
                headless_id,
            ),
        ] {
            let args = parse(args);
    ...
    fn materialize_ctx_chat_mode_from_args() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["fuigo"])).chat_mode);
    }
    #[test]
    fn remote_restore_follows_compiled_restore_stack() {
        assert_eq!(
            MaterializeCtx::from_pager_args(&parse(&["fuigo"])).allow_remote_restore,
            false
        );
    }
    #[test]
    fn from_pager_args_does_not_probe_tty_for_progress() {
        assert!(
            !MaterializeCtx::from_pager_args(&parse(&["fuigo"])).restore_progress_on_stdout,
            "stdout vs stderr is decided at the composition root, not from_pager_args"
        );
    }
    #[test]
    fn materialize_ctx_restore_code_follows_cli_flag() {
        assert!(!MaterializeCtx::from_pager_args(&parse(&["fuigo"])).restore_code);
        assert!(!MaterializeCtx::from_pager_args(&parse(&["fuigo", "-r", "abc"])).restore_code);
        assert!(
            MaterializeCtx::from_pager_args(&parse(&["fuigo", "-r", "abc", "--restore-code"]))
                .restore_code
        );
    ...
    async fn chat_resume_passthrough_keeps_cwd_collision_refusal() {
        ...
        let encoded = fuigo_shell::util::fuigo_home::encode_cwd_dirname(&cwd_str);
        let sessions_cwd_dir = fuigo_shell::util::fuigo_home::fuigo_home()
            .join("sessions")
            .join(&encoded);
    ...
    mod resume_by_title {
        use super::*;
        use crate::test_util::FuigoHomeFixture;
