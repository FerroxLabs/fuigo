...
/// Inclusive bounds for motion cadence env knobs (`FUIGO_MIN_DRAW_MS`, `FUIGO_SCROLL_CADENCE_MS`).
...
pub fn start(
    ...
    let (min_draw_env_set, min_draw_env_ms) =
        cadence_ms_from_env("FUIGO_MIN_DRAW_MS", default_cadence_ms);
    let (scroll_env_set, scroll_env_ms) =
        cadence_ms_from_env("FUIGO_SCROLL_CADENCE_MS", default_cadence_ms);
...
fn spawn_terminal_and_display_refresh_telemetry(tel: StartupTel) {
    tokio::task::spawn_blocking(move || {
        ...
        fuigo_telemetry::session_ctx::log_event(t.clone());
        ...
        fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::DisplayRefreshProbe {
            terminal: t,
