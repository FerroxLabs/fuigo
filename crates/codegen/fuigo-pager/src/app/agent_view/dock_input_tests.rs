...
use crate::views::dock::{DockItem, DockLayout, MAX_DOCK_ROWS, Section};
use crossterm::event::{
...
/// A running monitor (`Watchers` section) rather than a plain background run.
...
fn insert_running_subagent(agent: &mut AgentView, child_session_id: &str) {
    ...
    // `open_subagent_fullscreen` returns early without a child view, so a row
    // that paints `openable` could not actually open without one.
    agent
        .subagent_views
        .insert(child_session_id.to_string(), Box::new(make_agent()));
}
...
fn task_row_index(agent: &AgentView) -> usize {
    agent
        .dock_items()
        .iter()
...
fn reveal_row_index(agent: &AgentView, section: Section) -> usize {
    agent
        .dock_items()
        .iter()
        .position(|item| *item == DockItem::RevealRemaining(section))
        .expect("overflow row")
}
...
/// One full `AgentView::draw` pass, so the tests that pin what a *frame* does
/// (spending a pending reveal, clearing the painted kill rect) exercise the
/// real render path rather than a helper.
...
fn ctrl_x_while_dock_focused_is_unchanged() {
    let mut agent = dock_with_task();
    // The cursor must sit on the killable row, not the Header it rests on by
    // default: `dock_stop_action(Header)` is `None` either way, so a header
    // cursor would leave this green with the modifier guard deleted.
    agent.dock_cursor = task_row_index(&agent);
    assert!(
        agent
            .dock_stop_action(DockItem::Row(Section::Tasks, 0))
            .is_some(),
        "the selected row has a kill action for the guard to refuse"
    );
...
// -------------------------------------------------------- U060: collapse and
// expand keys, and the `show N more` reveal they gate.
...
fn enter_on_show_more_gives_the_section_the_spare_rows() {
    ...
    let before = agent.dock_layout().visible_rows(Section::Tasks);
    ...
    let after = agent.dock_layout().visible_rows(Section::Tasks);
    assert!(
        after > before,
    ...
    let items = agent.dock_items();
    assert!(
        items.contains(&DockItem::Header(Section::Subagents)),
        "the other section keeps its header: {items:?}"
    );
    assert!(
        items.len() > MAX_DOCK_ROWS as usize,
...
fn walking_down_reaches_the_show_more_row_and_enter_opens_it() {
    ...
    agent.dock_shown = true;
    agent.pane_areas.scrollback = Rect::new(0, 0, 80, 24);
    agent.pane_areas.dock = Rect::new(0, 24, 80, MAX_DOCK_ROWS);
    agent.active_pane = AgentPane::Dock;
    let items = agent.dock_items();
    let reveal = reveal_row_index(&agent, Section::Tasks);
    let hidden_before = agent.dock_layout().hidden_rows(Section::Tasks);
    ...
    for _ in 0..reveal {
        ...
        assert_eq!(
            agent.dock_layout().hidden_rows(Section::Tasks),
            hidden_before,
    ...
    assert_eq!(
        agent.dock_items().get(agent.dock_cursor).copied(),
        Some(DockItem::RevealRemaining(Section::Tasks))
    );
    ...
    assert!(agent.dock_tasks_show_all);
    assert!(
        agent.dock_layout().visible_rows(Section::Tasks) > items.len() - reveal,
        "the section opened"
    );
}
...
// -------------------------------------------------------- U037: the hovered
// row's `[stop]` maps to that row, and a click there kills exactly it.
...
fn clicking_hover_stop_kills_subagent_but_clicking_row_opens_it() {
    ...
    let item = DockItem::Row(Section::Subagents, 0);
...
fn clicking_hover_stop_kills_task() {
    let mut agent = dock_with_task();
    let item = DockItem::Row(Section::Tasks, 0);
    agent.pane_areas.dock = Rect::new(2, 4, 78, 2);
    let row_y = agent.pane_areas.dock.y + 1;
    let stop_col = agent.pane_areas.dock.right() - 1;
...
fn clicking_hover_stop_kills_monitor() {
    ...
    agent.dock_on = true;
    agent.pane_areas.dock = Rect::new(2, 4, 78, 2);
    let item = DockItem::Row(Section::Watchers, 0);
    let row_y = agent.pane_areas.dock.y + 1;
    let stop_col = agent.pane_areas.dock.right() - 1;
    let _ = agent.handle_mouse(&mouse(MouseEventKind::Moved, stop_col, row_y));
    assert_eq!(agent.dock_hovered, Some(item));
...
fn stop_click_uses_current_row_when_rows_change_after_hover() {
    ...
    let y = agent.pane_areas.dock.y + 1;
    let _ = agent.handle_mouse(&mouse(MouseEventKind::Moved, x, y));
    assert_eq!(agent.dock_hovered, Some(DockItem::Row(Section::Subagents, 0)));
...
fn stale_stop_click_does_not_kill_the_replacement_row() {
    ...
    assert_eq!(
        ...
        Some(super::DockKillId::Subagent("sa-child-1".into()))
    );
    agent.subagent_sessions.remove("child-1");
    agent.subagent_views.remove("child-1");
...
fn every_visible_stop_column_dispatches_and_adjacent_click_opens_row() {
    let stop_width = Section::Subagents.kill_label().width() as u16;
...
fn clicking_selected_stop_kills_without_hover() {
    ...
    agent.dock_cursor = agent
        .dock_items()
        .iter()
        .position(|item| matches!(item, DockItem::Row(Section::Subagents, 0)))
        .expect("subagent row");
...
fn stop_click_after_a_relayout_kills_the_row_now_under_the_pointer() {
    ...
    insert_running_subagent(&mut agent, "child-1");
    agent.active_pane = AgentPane::Prompt;
    agent.pane_areas.dock = Rect::new(0, 4, 80, MAX_DOCK_ROWS);
    let more_row = reveal_row_index(&agent, Section::Tasks) as u16;
    ...
    let (row_y, index) = (0..dock.height)
        .find_map(|offset| match agent.dock_item_at(dock, dock.y + offset) {
            Some(DockItem::Row(Section::Tasks, index)) => Some((dock.y + offset, index)),
            _ => None,
        })
...
// -------------------------------------------------------- U060/U106: hover,
// wheel, and the reveal's effect on the hit-test map.
...
fn click_on_show_more_widens_the_section_in_place() {
    ...
    assert_eq!(agent.active_pane, AgentPane::Dock);
    assert!(agent.dock_tasks_show_all);
    assert!(agent.dock_layout().visible_rows(Section::Tasks) > before);
}
...
fn wheel_scrolls_the_section_under_the_pointer_and_clicks_follow_it() {
    ...
    assert!(matches!(outcome, InputOutcome::Changed));
    assert_eq!(agent.dock_items().get(agent.dock_cursor).copied(), Some(clicked));
}
...
// -------------------------------------------------------- U060: the reveal /
// clamp / reconcile state machine that feeds the layout.
...
fn reveal_aims_the_cursor_when_the_last_painted_dock_is_the_resting_cap() {
    ...
    agent.pane_areas.scrollback = Rect::new(0, 0, 80, 24);
    agent.pane_areas.dock = Rect::new(0, 4, 80, MAX_DOCK_ROWS);
    let first_hidden = agent.dock_layout().visible_rows(Section::Tasks);
    assert!(
        !agent
            .dock_items()
            .contains(&DockItem::Row(Section::Tasks, first_hidden)),
    ...
    assert_eq!(
        agent.dock_items().get(agent.dock_cursor).copied(),
        Some(DockItem::Row(Section::Tasks, first_hidden)),
...
fn reveal_after_scroll_aims_the_cursor_at_the_first_hidden_row() {
    ...
    agent.pane_areas.scrollback = Rect::new(0, 0, 80, 24);
    agent.pane_areas.dock = Rect::new(0, 4, 80, MAX_DOCK_ROWS);
    let visible = agent.dock_layout().visible_rows(Section::Tasks);
    assert!(
        agent.scroll_dock_section(Section::Tasks, 3),
        "test needs a section that can scroll"
    );
    let layout = agent.dock_layout();
    let offset = layout.row_offset(Section::Tasks);
    assert!(offset > 0, "the section must have scrolled");
    let first_hidden = offset + layout.visible_rows(Section::Tasks);
    assert_ne!(
...
fn revealing_a_second_section_leaves_the_first_open() {
    ...
    let tasks_at_rest = agent.dock_layout().visible_rows(Section::Tasks);
    reveal(&mut agent, Section::Tasks);
    let tasks_open = agent.dock_layout().visible_rows(Section::Tasks);
    assert!(
        tasks_open > tasks_at_rest,
        "Tasks opened: {tasks_at_rest} -> {tasks_open}"
    );

    reveal(&mut agent, Section::Watchers);
    ...
    assert!(
        agent.dock_layout().visible_rows(Section::Watchers) > 1,
...
fn reveal_on_a_typical_terminal_does_not_shrink_the_dock() {
    ...
    agent.pane_areas.scrollback = Rect::new(0, 0, 80, 8);
    agent.pane_areas.dock = Rect::new(0, 8, 80, MAX_DOCK_ROWS);
    let assigned = agent.pane_areas.dock.height;
...
fn revealed_scroll_follows_the_assigned_height_not_the_raised_request() {
    ...
    let assigned = DockLayout::with_cap(&agent.dock_counts(), MAX_DOCK_ROWS as usize);
    let layout = agent.dock_layout();
    assert_eq!(
        layout.rows(),
        assigned.rows(),
    ...
    assert!(
        agent.scroll_dock_section(Section::Tasks, 1),
...
fn a_frame_spends_the_pending_reveal() {
    ...
    assert!(
        agent.dock_items().len() > MAX_DOCK_ROWS as usize,
...
/// The subagent takeover returns before the dock block, so it needs its own
/// `take_dock_row_request()`: otherwise the flag survives every frame spent in
/// the child view and `dock_max_rows()` budgets the raised ask on return.
#[test]
fn a_subagent_takeover_frame_spends_the_pending_reveal() {
    let mut agent = agent_with_task_overflow();
    insert_running_subagent(&mut agent, "child-1");
    agent.pane_areas.scrollback = Rect::new(0, 0, 80, 24);
    agent.pane_areas.dock = Rect::new(0, 24, 80, MAX_DOCK_ROWS);
    ...
    agent.open_subagent_fullscreen("child-1".to_owned());
    assert_eq!(agent.active_subagent.as_deref(), Some("child-1"));
    draw_frame(&mut agent);

    assert!(
        !agent.dock_reveal_pending,
        "a takeover frame paints no dock, so it must still spend the reveal"
    );
}

/// The painted kill rect is the one thing a click trusts, so a frame that
/// paints no dock must not leave the previous frame's rect and identity live.
#[test]
fn a_frame_that_paints_no_dock_clears_the_painted_kill_rect() {
    let mut agent = dock_with_task();
    agent.dock_hidden = true;
    agent.dock_stop_button = Some(super::CachedDockStop {
        rect: Rect::new(70, 5, 6, 1),
        id: super::DockKillId::Task("bg-1".into()),
    });

    draw_frame(&mut agent);

    assert!(
        agent.dock_stop_button.is_none(),
        "a Ctrl+G-hidden dock must not keep last frame's kill rect armed"
    );
}

// -------------------------------------------------------- U060: a crowded or
// squeezed dock still reaches every header, and the queue keeps its body.
...
fn a_crowded_dock_keeps_every_section_header_on_screen() {
    ...
    for section in [Section::Subagents, Section::Tasks] {
        assert!(
            items.contains(&DockItem::Header(section)),
            "{section:?} lost its header: {items:?}"
        );
    }
    assert!(
        items.contains(&DockItem::RevealRemaining(Section::Tasks)),
...
fn a_squeezed_dock_keeps_trailing_headers_reachable() {
    ...
    let items = agent.dock_items();
    for section in [Section::Subagents, Section::Tasks, Section::Queued] {
        assert!(
            items.contains(&DockItem::Header(section)),
...
fn dock_item_at_budgets_against_the_passed_rect_height() {
    ...
    let hit: Vec<_> = (0..4)
        ...
        .collect();
    let expected = DockLayout::with_cap(&agent.dock_counts(), 4).rows().to_vec();
    assert_eq!(
        hit, expected,
...
fn a_collapsed_queue_still_keeps_its_header() {
    ...
    let items = crate::views::dock::visible_items(&data);
    assert!(
        items.contains(&DockItem::Header(Section::Queued)),
        "{items:?}"
    );
    assert_eq!(
        crate::views::dock::queue_body_rect(Rect::new(0, 4, 80, MAX_DOCK_ROWS), &data),
        Rect::default(),
