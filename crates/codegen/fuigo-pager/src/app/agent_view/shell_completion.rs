...
mod shell_suggestion_key_tests {
    ...
    /// Same, with the pipeline OFF (`FUIGO_SUGGESTIONS` unset): the always-on Tab surface under test.
    ...
    // -- always-on Tab fetch (no FUIGO_SUGGESTIONS) --------------------------
    ...
    fn tab_mixed_rangeless_and_ranged_rows_open_dropdown() {
        ...
        agent.prompt.suggestions.dropdown.items = vec![
            item("grep", None),
            file_item("ls | fuigofile", "fuigofile", 5..7),
        ];
