...
impl McpInitProgress {
    ...
    ///   `fuigo/mcp_initialized` clears the progress.
...
pub struct BannerSlotParams<'a> {
    ...
    pub(crate) announcements: &'a [fuigo_announcements::RemoteAnnouncement],
...
pub(crate) struct DeferredSubagentFinish {
    pub notification: fuigo_shell::extensions::notification::SessionNotification,
...
pub(crate) struct SessionReload {
    ...
    /// Live dedup highwaters (ACP and Ferrox Labs) as of window open (same restore
    /// rationale).
    last_applied_event_seq: Option<u64>,
    last_applied_fuigo_event_seq: Option<u64>,
...
pub struct PluginCtaState {
    ...
    /// (Ferrox Labs Official, or the configured `plugin_cta_marketplace` override).
    pub candidates: Vec<fuigo_hooks_plugins_types::MarketplacePluginEntry>,
...
/// (`fuigo/follow_ups`). Streaming-only: never persisted, does not survive a
...
pub struct AgentView {
    ...
    ///
    /// ACP stream only; the Ferrox Labs stream keeps its own highwater
    /// ([`Self::last_applied_fuigo_event_seq`]) because the two streams are not
    /// delivered in one id order: ACP lines ride the agent's FIFO event
    /// pipeline while Ferrox Labs lines are emitted direct-to-gateway, so a fresh Ferrox Labs
    ...
    /// Ferrox Labs-stream sibling of [`Self::last_applied_event_seq`] (see there for
    ...
    pub context_state: Option<fuigo_shell::session::ContextInfo>,
    pub status_context: Option<fuigo_status_line::StatusLineContext>,
    ...
    /// (optimistic echo). The shell broadcasts `fuigo/session/interjection` to
    ...
    pub(crate) pending_elicitation: Option<(
        fuigo_tools::mcp_elicitation::McpElicitExtRequest,
        tokio::sync::oneshot::Sender<fuigo_acp_lib::AcpResult<agent_client_protocol::ExtResponse>>,
    )>,
    ...
    pub(crate) deferred_session_mode: Option<fuigo_tools::types::SessionMode>,
    ...
    /// MCP servers, cleared when `fuigo/mcp_initialized` arrives.
    ...
    /// Running agent definition for this session (`fuigo/session/info` `agentName`).
    ...
    /// refreshed by `fuigo/settings/update`: the fire side is pinned for the
    ...
    /// Armed when an `fuigo/session/prompt_complete` broadcast arrives for the
    ...
    /// `fuigo/queue/changed` broadcast has confirmed the row yet. Inserted by
    ...
    /// optimistic echo. Firing `fuigo/queue/interject` then would race the
    ...
    /// Parked here and fired from the confirming `fuigo/queue/changed`
    ...
    /// Follow-up suggestion chips for the latest assistant response
    /// (`fuigo/follow_ups`). `None` when no chips are shown. Set by
    ...
    /// Stamped `fuigo/follow_ups` that arrived for a turn that is NOT yet the
    ...
    match kind {
        ...
        LocalQuestionKind::CreditLimitUpsell { choices } => {
            ...
            let choice = choices
                .get(*idx)
                .copied()
                .unwrap_or(fuigo_telemetry::events::CreditLimitChoice::PayAsYouGo);
            fuigo_telemetry::session_ctx::log_event(
                fuigo_telemetry::events::CreditLimitUpsellClicked {
                    surface: fuigo_telemetry::events::CreditLimitUpsellSurface::QuestionModal,
                    choice,
...
fn question_visible_h(
    ...
    prompt_height: u16,
    question: &fuigo_tools::implementations::fuigo_build::ask_user_question::Question,
    content_w: usize,
    preview: Option<&str>,
    fullscreen: bool,
    desc_cap: u16,
...
pub(crate) mod test_fixtures {
    ...
    -> crate::views::permission_view::PermissionViewState {
        ...
        let perm = fuigo_acp_lib::AcpArgs {
            request,
            response_tx,
        };
        crate::views::permission_view::PermissionViewState {
    ...
    /// After adopting a NEW turn, a buffer-replayed `fuigo/follow_ups`
    ...
    /// Stamped path: a LATE FIRST-TIME (never-seen) `fuigo/follow_ups`
    ...
    /// upstream by the `_meta["fuigo/replayed"]` gate); it is pinned here so the
    ...
    /// Buffer-before-adoption: a stamped `fuigo/follow_ups` for a turn that
