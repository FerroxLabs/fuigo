...
impl AgentView {
    ...
    pub(in crate::app) fn handle_prompt_key(
            ...
            && self.prompt.prompt_suggestion_visible()
        {
            if key!(Tab).matches(key) || key!(Right).matches(key) {
                if self.prompt.accept_prompt_suggestion() {
                    ...
                    fuigo_telemetry::session_ctx::log_event(
                        fuigo_telemetry::events::PromptSuggestion {
                            action: fuigo_telemetry::events::PromptSuggestionAction::Accepted,
                            chars,
                            words,
                            model: None,
                            latency_ms: None,
                ...
                && self.prompt.text().is_empty()
            {
                ...
                fuigo_telemetry::session_ctx::log_event(
                    fuigo_telemetry::events::PromptSuggestion {
                        action: fuigo_telemetry::events::PromptSuggestionAction::Dismissed,
                        chars,
                        words,
                        model: None,
                        latency_ms: None,
    ...
    pub(super) fn try_handle_esc_policy(
        ...
        // While a cancel is already in flight there is no hint: Ctrl+C escalates to quit there, so naming it would mislead
