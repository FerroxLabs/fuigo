    ...
    fn handle_input_inner(
            ...
            && (self.queue.is_visible() || !self.visible_queue_is_empty())
        {
            if self.dock_shown {
                ...
                // The dock replaced the queue pane; an empty dock opens nothing.
                return InputOutcome::Unchanged;
            } else {
            ...
            && registry.lookup(key, When::AgentScreen) == Some(ActionId::OpenExtensions)
        {
            ...
            return InputOutcome::Action(Action::OpenExtensionsModal {
                ...
                trigger: fuigo_telemetry::events::ExtensionsModalTrigger::KeyboardShortcut,
            });
        }
        if let Event::Key(key) = ev
            && key.kind != KeyEventKind::Release
...
mod esc_would_cancel_turn_tests {
    ...
    fn queued_edit_and_inline_edit_steal_esc() {
        ...
        agent.inline_edit = Some(crate::app::inline_edit::InlineEditState {
            ...
            prompt_index: 0,
            original: "sent".into(),
            textarea: fuigo_ratatui_textarea::TextArea::new(),
            textarea_state: fuigo_ratatui_textarea::TextAreaState::default(),
            last_text_area: None,
            last_rect: None,
        });
        assert!(
