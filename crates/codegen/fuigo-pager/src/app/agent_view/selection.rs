...
impl AgentView {
    ...
    fn with_entry_table_copy_source<R>(
        ...
        width_override: Option<u16>,
        geom: &TableGeometry,
        f: impl FnOnce(&dyn Fn(usize) -> Option<String>, Option<&fuigo_markdown::TableCopyMeta>) -> R,
    ) -> Option<R> {
        if entry_idx == BTW_OVERLAY_ENTRY_IDX {
    ...
    /// Geometry resolves once per drag, at arming, into the drag-owned slot so a `/btw` drag
    /// never steals or wipes the held highlight's side-car.
    fn arm_text_drag(
        &mut self,
        mouse: &MouseEvent,
        anchor: RangeHit,
...
mod tests {
    ...
    fn btw_table_drag_detects_geometry_and_copies_cells() {
        ...
        assert!(!text.contains('\u{2502}'), "no border glyphs: {text:?}");
