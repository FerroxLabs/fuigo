...
//!
//! A `session/prompt` RPC has no deadline of its own, so this bounds only the *acknowledgment*: the
//! first `fuigo/queue/changed`, `session/update`, or turn end that names the prompt id. A shell that
//! never takes the prompt therefore stops being an indefinite hang.
//!
//! Invariant: a [`PromptAckWatch`] exists only while the prompt it names is the one in flight;
//! every turn-end path clears it.

use std::time::{Duration, Instant};

use serde::Serialize;

const PROMPT_ACK_TIMEOUT_ENV: &str = "FUIGO_PROMPT_ACK_TIMEOUT_SECS";
/// Notice ("waiting for the agent to accept…") before the hard deadline.
...
/// Whether a `fuigo/queue/changed` payload proves the shell holds `prompt_id` (queued or running).
