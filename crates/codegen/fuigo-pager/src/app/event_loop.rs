...
use fuigo_acp_lib::{AcpClientMessage, acp_send};
...
/// Reads the local store, scans for repo-local code-exec config, and runs the pure [`decide`](fuigo_workspace::folder_trust::decide) precedence.
///
...
pub(crate) fn seed_consent_state_from_gate(
    app: &mut AppView,
    gate: Option<&fuigo_shell::util::config::ConsentGate>,
) {
    use crate::app::consent::{ConsentInputs, consent_verdict};
    let stored = fuigo_shell::config::load_from_disk()
        .ok()
        .map(|root| fuigo_shell::util::config::load_config_from_toml(&root).consent)
        .unwrap_or_default();
...
pub(crate) async fn run(
    ...
    pending_startup: fuigo_telemetry::startup::PendingStartup,
    ...
    let launch_effective_config = fuigo_shell::config::load_effective_config().ok();
    ...
    let toml_owns_mode = launch_effective_ui
        .as_ref()
        .and_then(fuigo_shell::util::config::permission_mode_from_ui_if_set)
        .is_some();
    ...
    app.require_plan_approval = fuigo_shell::util::config::load_require_plan_approval();
    ...
    if let Some(ref agent) = args.agent {
        match crate::headless::resolve_agent_arg(agent) {
            crate::headless::ResolvedAgent::FilePath(path) => {
                match fuigo_shell::agent::config::AgentDefinition::from_file(&path) {
    ...
    app.privacy_notice_rollout = fuigo_config::env_bool("FUIGO_PRIVACY_NOTICE_ROLLOUT")
        .or_else(|| {
            remote_settings
                .as_ref()
        ...
        .unwrap_or(false);
    app.privacy_banner_reshow_days = std::env::var("FUIGO_PRIVACY_BANNER_RESHOW_DAYS")
        .ok()
    ...
    app.workspace_dashboard_enabled = fuigo_config::env_bool("FUIGO_WORKSPACE_DASHBOARD")
        .or_else(|| {
            remote_settings
                .as_ref()
    ...
    app.session_picker_grouped = std::env::var("FUIGO_SESSION_PICKER_GROUPED")
        .ok()
        ...
        .or_else(|| {
            fuigo_shell::config::load_effective_config()
                .ok()
    ...
    if needs_interactive_login {
        ...
        if connection.needs_login {
            ...
            let fuigo_com = connection
                .auth_methods
                .iter()
                .find(|m| m.id().0.as_ref() == "grok.com");
            if let Some(method) = fuigo_com {
    ...
    if let Some(meta) = connection.auth_meta.as_ref() {
        match serde_json::from_value::<fuigo_shell::auth::AuthMeta>(meta.clone()) {
    ...
    app.hidden_announcement_ids = fuigo_announcements::read_hidden_announcement_ids().await;

    // Load config layers once, resolve announcements, tips, and feature flags.
    let requirements = fuigo_shell::config::load_merged_requirements();
    let user_config = fuigo_shell::config::load_from_disk().ok();
    let managed_config = fuigo_shell::config::load_managed_config().ok();

    // Full merge when every layer parses; partial merge below if any layer fails.
    let effective_config = match fuigo_shell::config::load_effective_config() {
        Ok(raw) => Some(raw),
        Err(e) => {
    ...
    let compat = fuigo_shell::agent::config::resolve_compat_sessions_from_raw(
        effective_config.as_ref().ok_or(()),
        remote_settings.as_ref(),
    );
    app.foreign_session_compat = fuigo_foreign_sessions::EnabledForeignSessionSources {
    ...
    if let Some(ref raw) = effective_config {
        ...
        if let Some(table) = raw.as_table() {
            ...
            let endpoints_base =
                fuigo_shell::agent::config::EndpointsConfig::from_config_value(raw)
                    .fuigo_api_base_url;
            app.voice_config =
                fuigo_voice::VoiceConfig::from_config_table(table, Some(&endpoints_base));
        }
    }
    // Stamp request-identity headers so the STT handshake attributes voice usage to fuigo-cli server-side (mirrors sampler / imagine)
    ...
    app.zdr_access_enabled = fuigo_shell::util::config::resolve_zdr_access_enabled(
        requirements.as_ref(),
        user_config.as_ref(),
    ...
    crate::appearance::cache::set_show_thinking_blocks(
        fuigo_shell::util::config::resolve_show_thinking_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_group_tool_verbs(
        fuigo_shell::util::config::resolve_group_tool_verbs(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    crate::appearance::cache::set_collapsed_edit_blocks(
        fuigo_shell::util::config::resolve_collapsed_edit_blocks(
            requirements.as_ref(),
            user_config.as_ref(),
            managed_config.as_ref(),
            remote_settings.as_ref(),
    ...
    app.scheduler_background_loops_seed =
        fuigo_shell::util::config::resolve_scheduler_background_loops(
            remote_settings
                .as_ref()
...

    {
        ...
        app.active_announcements = fuigo_announcements::filter_expired(announcements);
        ...
        if !app.tips.is_empty() {
            ...
            app.tip = fuigo_shell::util::tips::pick_and_advance(&app.tips, &fuigo_home);
        }
    ...
    let hints = fuigo_shell::util::config::resolve_hints(
    ...
    // Apply initial config (may come from existing ~/.fuigo/pager.toml).
    ...
    let display_mode: &'static str = if launch_auto {
        ...
        fuigo_shell::util::config::clamped_display_permission_mode(
            fuigo_shell::util::config::parse_permission_mode_canonical(cli),
        )
    } else {
        fuigo_shell::util::config::resolved_display_permission_mode(
    ...
    let resolved_hints = fuigo_shell::util::config::resolve_contextual_hints(
    ...
    let mouse_toggle = fuigo_shell::util::config::resolve_mouse_reporting_toggle(
        effective_config.as_ref(),
        &app.current_ui,
    );
    ...
    // FUIGO_SCROLL_SPEED/_MODE/_LINES and FUIGO_INVERT_SCROLL env overrides apply on first load
    ...
    let mut voice_rx = None::<tokio::sync::mpsc::Receiver<fuigo_voice::VoiceEvent>>;
    ...
    // Initial prompt from the CLI positional (`fuigo "fix the bug"`)
    ...
    // `fuigo dashboard` startup: open the dashboard view immediately
    // The CLI subcommand wrote a `FUIGO_OPEN_DASHBOARD_AT_STARTUP=1` env var so we don't have to thread a flag through every arg struct
    if std::env::var("FUIGO_OPEN_DASHBOARD_AT_STARTUP").as_deref() == Ok("1") {
    ...
    loop {
        ...
        if let VoiceState::ColdStart { hold, target } = app.voice_state {
            if app.voice_cmd_tx.is_none() && app.voice_can_start_pipeline() {
                ...
                tokio::spawn(fuigo_voice::run_voice_pipeline(
                    voice_config,
                    voice_auth.clone(),
                    cmd_rx,
                    event_tx,
                ...
                app.voice_state = VoiceState::Idle;
                app.voice_ui_active = false;
                app.show_toast("Voice could not start. Restart Fuigo.");
            } else {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.fuigo/config.toml [ui]` rather than `~/.fuigo/pager.toml`
...
pub(crate) fn load_initial_ui_config() -> fuigo_shell::agent::config::UiConfig {
    use fuigo_shell::agent::config::UiConfig;
    static INITIAL_UI: std::sync::OnceLock<UiConfig> = std::sync::OnceLock::new();

    INITIAL_UI
        .get_or_init(|| {
            let Ok(root) = fuigo_shell::config::load_effective_config() else {
...
/// The marketplace source name the plugin CTA draws candidates from instead of Ferrox Labs Official.
...
fn emit_event_loop_stall(window: super::event_loop_stall::StallWindow) {
    fuigo_telemetry::session_ctx::log_event(super::event_loop_stall::event_loop_stall_event(
        window,
    ));
}
...
mod tests {
    ...
    async fn pending_create_uses_auto_selected_before_effect_execution() {
        ...
        let request = match acp_rx.recv().await.expect("session/new request") {
            fuigo_acp_lib::AcpAgentMessage::NewSession(args) => args.request,
    ...
    async fn welcome_paste_preserves_create_and_forwarded_prompt() {
        ...
        assert!(matches!(
            ...
            fuigo_acp_lib::AcpAgentMessage::NewSession(_)
        ));
        assert_eq!(
    ...
    fn plugin_cta_marketplace_from_unset_or_empty_is_none() {
        let unset = fuigo_config::ConfigLayers::default();
        assert_eq!(
            plugin_cta_marketplace_from(&unset.effective_config_base()),
            None
        );
        let empty = fuigo_config::ConfigLayers {
        ...
        let blank = fuigo_config::ConfigLayers {
            user: toml::from_str("[marketplace]\nplugin_cta_marketplace = \"   \"\n").unwrap(),
            ..Default::default()
        };
        assert_eq!(
