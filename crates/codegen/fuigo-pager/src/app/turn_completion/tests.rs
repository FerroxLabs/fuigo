...
fn hook_denied_finalize_requeues_blocked_prompt_and_opens_card() {
    ...
    let context = serde_json::to_value(fuigo_shell::session::commands::CancellationContext {
...
/// One chooser for every rail: only the hook-denied category flips the marker kind.
#[test]
...
/// The banner names the cause from the wire meta; missing or unknown meta is plain "Turn cancelled", never "by user".
#[test]
fn cancelled_turn_event_names_passive_causes() {
    ...
    assert_eq!(banner(Some("esc"), None), "Turn cancelled by user in 10s.");
    assert_eq!(
    ...
    assert_eq!(
        banner(
            None,
            Some(fuigo_shell::session::commands::PERMISSION_REJECTED_CATEGORY)
        ),
    ...
    assert_eq!(
        banner(
            None,
            Some(fuigo_shell::session::commands::MID_TURN_ABORT_CATEGORY)
        ),
...
fn viewer_finalize_truncation_wire_pair_renders_exact_user_copy() {
    ...
    let _ = finalize_turn_from_terminal(
        ...
        TerminalSignal {
            prompt_id: Some("p1"),
            stop_reason: Some("error"),
            agent_result: Some(fuigo_shell::sampling::error::MAX_TOKENS_TRUNCATION_MESSAGE),
            error_kind: Some(WireErrorType::MaxTokensTruncation),
            ..Default::default()
        },
    );
