...
pub(crate) fn resolve_voice_mode_enabled(
    ...
    use fuigo_shell::agent::config::{ConfigSource, Feature, FeatureSources};
...
/// Uses [`fuigo_ratatui_inline::Terminal`] instead of stock `ratatui::Terminal`: our `flush()` returns a `bool` saying whether any cells changed.
...
pub(crate) enum ScreenMode {
    ...
    ///
    /// All minimal-mode rendering lives in the sibling `fuigo-pager-minimal` crate.
...
impl ScreenMode {
    ...
    /// Values are pinned by the telemetry allowlist (`fuigo-telemetry`'s `KNOWN_SCREEN_MODES`).
...
pub fn resolve_leader_mode<'p>(
    ...
    _remote_settings: Option<&fuigo_shell::util::config::RemoteSettings>,
    eligible: bool,
...
async fn bounded_connect(
    ...
    let context = || startup_failure::Context {
        target,
        attempt,
        version: fuigo_version::display_version_with_commit(
            fuigo_version::full_version(),
            fuigo_update::channel_label(),
        ),
        log_path: fuigo_telemetry::unified_log::path(),
    };
    tokio::select! {
        biased;
...
/// The caller should print a message telling the user to relaunch `fuigo`.
pub async fn run(
    mut args: PagerArgs,
    bg_update_rx: Option<
        tokio::sync::oneshot::Receiver<Option<fuigo_update::auto_update::UpdateAvailable>>,
    >,
) -> anyhow::Result<bool> {
    ...
    let fuigo_com_config = match fuigo_shell::agent::config::Config::new_from_toml_cfg(&raw_config)
    {
        ...
        Err(e) => {
            ...
            fuigo_shell::auth::FuigoComConfig::default()
        }
    };
    if matches!(
        fuigo_shell::auth::maybe_run_pre_tui_external_login(
            &fuigo_com_config,
            args.force_login,
            io::stdin().is_terminal(),
        )
        .await?,
        fuigo_shell::auth::PreTuiLoginOutcome::SignedIn(_)
    ) {
        args.force_login = false;
    }
    fuigo_tty_utils::redirect_native_stderr();
    let refreshed_auth = tokio::time::timeout(
        fuigo_shell::http::STARTUP_AUTH_REFRESH_TIMEOUT,
        fuigo_shell::auth::try_ensure_fresh_auth(&fuigo_com_config),
    )
    .await
    .unwrap_or(None);
    ...
    let remote_settings = if had_prefetch {
        ...
        fuigo_telemetry::startup::record_prefetch_wait(prefetch_wait_started.elapsed());
        settings
    } else {
        None
    };
    fuigo_shell::util::config::cache_remote_auto_mode(
        remote_settings.as_ref().and_then(|s| s.auto_mode.clone()),
    );
    fuigo_shell::util::config::cache_remote_prompt_suggestions(
        remote_settings
            .as_ref()
    ...
    if let Some(reason) = policy_disable_reason {
        tokio::spawn(fuigo_shell::leader::kill_stale_reachable_leaders(reason));
    }
    if let Some(err) =
    ...
    {
        let summaries = fuigo_shell::session::persistence::list_summaries(None).await?;
    ...
    let env_hunk_tracker_mode = std::env::var("FUIGO_HUNK_TRACKER").ok();
    ...
    let launch_auto = fuigo_shell::util::config::effective_auto_for_launch(
        args.yolo,
        args.permission_mode_flag.as_deref(),
        remote_permission_mode,
        fuigo_shell::util::config::default_interactive_permission_mode(),
    );
    ...
    let pending_startup = fuigo_telemetry::startup::PendingStartup::new();
...
fn disable_mouse_paste_raw() {
    fuigo_shell::util::with_locked_stderr(|stderr| {
        let _ = stderr.write_all(fuigo_crash_handler::terminal::MOUSE_PASTE_RESET);
        let _ = stderr.flush();
    });
}
...
fn init_terminal(
    ...
    fuigo_crash_handler::enable_terminal_escape_restore();
    ...
    let (terminal, screen_mode) = (|| -> io::Result<(PagerTerminal, ScreenMode)> {
        ...
        fuigo_shell::util::with_locked_stderr(|stderr| {
            if !want_minimal {
                ...
                let _ = stderr.write_all(fuigo_crash_handler::terminal::MOUSE_TRACKING_RESET);
            }
            execute!(
                stderr,
        ...
        if mode.is_fullscreen() {
            ...
            Ok((
                fuigo_ratatui_inline::Terminal::new(backend)?,
            ...
            if let Ok(term) = fuigo_ratatui_inline::Terminal::with_options(
                probe_backend,
            ...
            let term = fuigo_ratatui_inline::Terminal::with_options(
                backend,
                ratatui::TerminalOptions {
...
mod tests {
    ...
    fn terminal_title_strips_control_characters() {
        ...
        assert_eq!(terminal_title_string("\x07\x1b\x00"), "fuigo");
        assert_eq!(terminal_title_string(""), "fuigo");
        assert_eq!(terminal_title_string("My chat"), "My chat - fuigo");
    }
    #[test]
    ...
    fn cli_hidden_memory_compat_flags_conflict() {
        assert!(try_parse_pager(&["fuigo-pager", "--experimental-memory", "--no-memory"]).is_err());
    }
    #[test]
    fn cli_memory_flush_flag_parses() {
        let args = try_parse_pager(&["fuigo-pager", "--memory-flush"]).unwrap();
        assert!(args.memory_flush);
        assert!(!try_parse_pager(&["fuigo-pager"]).unwrap().memory_flush);
    }
    #[test]
    fn cli_neither_leader_flag_defaults_false() {
        let args = try_parse_pager(&["fuigo-pager"]).unwrap();
        assert!(!args.leader);
        assert!(!args.no_leader);
    }
    #[test]
    ...
    fn cli_top_level_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["fuigo-pager", "--leader", "agent"]).unwrap();
        assert!(args.leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    fn cli_top_level_no_leader_with_agent_subcommand_parses_flag() {
        let args = try_parse_pager(&["fuigo-pager", "--no-leader", "agent"]).unwrap();
        assert!(args.no_leader);
        assert!(matches!(args.command, Some(Command::Agent(_))));
    }
    #[test]
    ...
    fn remote_settings_leader_mode_true_enables_leader() {
        let rs = fuigo_shell::util::config::RemoteSettings {
    ...
    fn cli_short_r_no_id_sets_empty_sentinel() {
        let args = try_parse_pager(&["fuigo-pager", "-r"]).unwrap();
        assert_eq!(args.resume_session.as_deref(), Some(""));
        assert!(args.resume_most_recent());
    }
    #[test]
    ...
    fn cli_continue_conflicts_with_resume() {
        let result = try_parse_pager(&["fuigo-pager", "--continue", "--resume", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    fn cli_continue_conflicts_with_load() {
        let result = try_parse_pager(&["fuigo-pager", "--continue", "--load", "abc"]);
        assert!(result.is_err());
    }
    #[test]
    ...
    fn cli_local_workspace_flags_rejected_without_feature() {
        assert!(try_parse_pager(&["fuigo-pager", "--local-workspace-attach=srv"]).is_err());
        assert!(try_parse_pager(&["fuigo-pager", "--local-workspace"]).is_err());
        assert!(try_parse_pager(&["fuigo-pager", "--local-workspace-cwd=/tmp"]).is_err());
    }
    #[test]
    ...
    fn cli_worktree_flag_parses() {
        let args = try_parse_pager(&["fuigo-pager", "--worktree"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_short_w_parses() {
        let args = try_parse_pager(&["fuigo-pager", "-w"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
    }
    #[test]
    fn cli_worktree_with_label() {
        let args = try_parse_pager(&["fuigo-pager", "-w", "my-label"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
    }
    #[test]
    fn cli_worktree_long_with_label() {
        let args = try_parse_pager(&["fuigo-pager", "--worktree", "fix-bug"]).unwrap();
    ...
    fn cli_worktree_with_resume_parses() {
        let args = try_parse_pager(&["fuigo-pager", "-w", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some(""));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    fn cli_worktree_label_with_resume() {
        let args = try_parse_pager(&["fuigo-pager", "-w", "my-label", "--resume", "abc"]).unwrap();
        assert_eq!(args.worktree.as_deref(), Some("my-label"));
        assert_eq!(args.session_to_resume(), Some("abc"));
    }
    #[test]
    ...
    fn cli_session_id_parses() {
        let args = try_parse_pager(&["fuigo-pager", "--session-id", "my-id"]).unwrap();
    ...
    fn cli_session_id_short_s_parses() {
        let args = try_parse_pager(&["fuigo-pager", "-s", "my-id"]).unwrap();
        assert_eq!(args.session_id.as_deref(), Some("my-id"));
    }
    #[test]
    fn cli_session_id_with_resume_requires_fork() {
        let args = try_parse_pager(&["fuigo-pager", "-s", "a", "--resume", "b"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    fn cli_session_id_with_continue_requires_fork() {
        let args = try_parse_pager(&["fuigo-pager", "-s", "a", "--continue"]).unwrap();
        assert!(args.session_startup_intent().is_err());
    }
    #[test]
    ...
    fn cli_no_alt_screen_flag_parses() {
        let args = try_parse_pager(&["fuigo-pager", "--no-alt-screen"]).unwrap();
    ...
    #[test]
    fn cli_command_name_is_fuigo() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "fuigo");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            ...
            vec![
                "Fuigo TUI",
                "",
                "Usage: fuigo [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
    ...
    fn cli_completions_parses() {
        ...
        let args = try_parse_pager(&["fuigo-pager", "completions", "zsh"]).unwrap();
        assert!(matches!(
            args.command,
            Some(Command::Completions { shell: Shell::Zsh })
        ));
        let args = try_parse_pager(&["fuigo-pager", "completions", "bash"]).unwrap();
        assert!(matches!(
            args.command,
