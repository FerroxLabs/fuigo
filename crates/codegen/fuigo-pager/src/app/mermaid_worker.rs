...
//!
//! The shipped CLI profiles build with `panic = "abort"`, so the `catch_unwind` inside [`fuigo_mermaid::render_checked`] is a no-op there.
...
//!
//! 1. The pager re-execs itself as `fuigo-pager __mermaid-render` (see [`maybe_run_render_subprocess`]).
...
//! 2. The worker spawns that child with a wall-clock budget ([`RENDER_TIMEOUT`]) via [`fuigo_mermaid::run_with_timeout`].
...
//!    Those are the source-size limit ([`fuigo_mermaid::RenderLimits`]) and the raster megapixel/height caps (see `fuigo-mermaid`).
...
fn render_source_to_png(
    ...
    target_width_px: u32,
    quality: MermaidRenderQuality,
) -> Result<RenderedDiagram, fuigo_mermaid::MermaidError> {
...
thread_local! {
    ...
    /// `FUIGO_HOME` (whose `fuigo_home()` value is cached first-write-wins, an
    /// isolation hazard under the full parallel suite; PNGs could land in the
    /// real `~/.fuigo`). Thread-local, so each parallel test is independent; the
    /// `TempDir` guard lives here so the dir outlives the view. Mirrors the
    /// `subagent::REPLAY_FUIGO_HOME` test override. Production never sets this.
...
impl AgentView {
    ...
    fn mermaid_out_path(&self, key: &MermaidCacheKey) -> Option<PathBuf> {
        // Test override: a hermetic per-test cache dir (no `FUIGO_HOME` mutation)
        #[cfg(test)]
...
mod tests {
    ...
    fn is_render_subcommand_matches_only_argv1() {
        ...
        assert!(!is_render_subcommand(&argv(&["fuigo"])));
        assert!(!is_render_subcommand(&argv(&["fuigo", "chat"])));
        assert!(!is_render_subcommand(&argv(&[])));
    ...
    fn per_theme_renders_land_in_distinct_files() {
        ...
        let light_key =
            MermaidCacheKey::derive(src, ThemeKind::FuigoDay, 80, MermaidRenderQuality::Terminal);
        assert_ne!(
    ...
    /// Point this test's session `mermaid/` cache dir at a private tempdir: hermetic, with no process-global `FUIGO_HOME` mutation.
    /// The `TempDir` lives in the [`TEST_MERMAID_DIR`] thread-local for the test thread's lifetime (so the dir outlives the view).
    /// Each parallel test gets its own dir, so there is no cross-test contamination and no `fuigo_home()` cache race.
    ...
    fn agent_with_session(name: &str) -> AgentView {
        ...
        let cwd = PathBuf::from("/fuigo-mermaid-test").join(name);
    ...
    fn mermaid_is_rendering_matches_by_source_across_theme_width_change() {
        ...
        let click_key =
            MermaidCacheKey::derive(src, ThemeKind::FuigoNight, 80, MermaidRenderQuality::Open);
        let mut rt = MermaidRuntime::new();
        rt.pending.push(PendingMermaidAction {
            key: click_key.clone(),
            action: MermaidClickAction::Open,
        ...
        let live_key =
            MermaidCacheKey::derive(src, ThemeKind::FuigoDay, 240, MermaidRenderQuality::Open);
        assert_ne!(
            click_key, live_key,
