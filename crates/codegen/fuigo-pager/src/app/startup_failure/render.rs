...
use fuigo_telemetry::startup::{AgentKind, PhaseSnapshot, StartupPhase, format_duration};
...
pub(super) fn render(failure: &StartupFailure) -> String {
    ...
    let mut report = match &failure.reason {
        Reason::TimedOut { waited, timings } => {
            ...
            format!(
                "Couldn't start Fuigo: startup timed out after {}.\n\n{explanation}",
...
impl Advice {
    fn explanation(&self) -> String {
        ...
        if let Some(earlier) = self.earlier {
            ...
            let _ = write!(
                explanation,
                " Fuigo spent the first {} on the {target}.",
...
impl NextStep {
    fn text(self) -> &'static str {
        match self {
            Self::Retry => "Start Fuigo again.",
            Self::CheckNetworkThenRetry => "Check your network connection, then start Fuigo again.",
            Self::RestartSharedLeader => {
                "Stop it with the command below, which also stops any other Fuigo \
                 session using it, then start Fuigo again."
            }
        }
    }
    ...
    fn command(self) -> Option<&'static str> {
        match self {
            ...
            Self::RestartSharedLeader => Some("fuigo leader kill"),
        }
    }
}
