...
/// Overridable via the remote settings `fuigo_build_settings.subscription_watch_interval_secs` field.
...
/// Floor for the `FUIGO_SUBSCRIPTION_WATCH_INTERVAL_SECS` env override (for tests and power users; deliberately below the server floor).
...
impl AppView {
    /// Consumer Ferrox Labs session auth: not an API key, not an enterprise team.
    ...
    pub fn subscription_watch_interval(&self) -> Option<std::time::Duration> {
        if let Ok(v) = std::env::var("FUIGO_SUBSCRIPTION_WATCH_INTERVAL_SECS")
    ...
    #[must_use]
    pub fn impose_gate(&mut self, gate: fuigo_shell::auth::GateInfo) -> Vec<Effect> {
    ...
    #[must_use]
    fn defer_gate_for_verification(&mut self, gate: fuigo_shell::auth::GateInfo) -> Vec<Effect> {
