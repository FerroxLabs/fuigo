...
//!
//! Simplified to only support FuigoShell (in-process) mode.
...
///
/// Hold one from every site that calls [`spawn_fuigo_shell`] (headless, the TUI,
...
fn join_agent_thread(handle: thread::JoinHandle<Result<()>>, timeout: Duration) -> JoinOutcome {
    ...
    let span = fuigo_telemetry::session_end::join_span();
    ...
    fuigo_telemetry::session_end::record_join(&span, outcome_label, elapsed_ms, notice_shown);
...
/// Spawn a FuigoShell agent in a background thread.
pub async fn spawn_fuigo_shell(
    agent_config: AgentConfig,
    cancel: &CancellationToken,
    memory_config: Option<fuigo_shell::config::MemoryConfig>,
) -> Result<SpawnedAgent> {
    ...
    fuigo_shell::agent::app::apply_otel_config(&auth_manager, &agent_config.fuigo_com_config);

    fuigo_shell::agent::models::startup_prefetch::begin_before_policy_gate(&agent_config);
    ...
    let (agent_config, models_manager) =
        fuigo_shell::agent::init::bootstrap(&agent_config, &auth_manager, None)
...
async fn spawn_agent_thread_direct(
    ...
    let rt = tokio::task::spawn_blocking(|| {
        ...
        fuigo_tty_utils::runtime::build_with_blocking_pool(builder.enable_all())
    })
    .await
            ...
            let result = local.block_on(&rt, async move {
                ...
                let _skills_watcher = {
                    ...
                    let workspace_user_dir =
                        fuigo_agent::prompt::workspace_user::optional_workspace_user_dir();
                    fuigo_shell::config::watcher::SkillsFileWatcher::start(
                        Some(cwd.as_path()),
                    ...
                    .map(|(mut watcher, mut skills_rx)| {
                        ...
                        tokio::task::spawn_local(async move {
                            while let Some(change) = skills_rx.recv().await {
                                ...
                                match change {
                                    fuigo_shell::config::watcher::DiscoveryChange::Skills => {
                                        tracing::info!(
                                    ...
                                    fuigo_shell::config::watcher::DiscoveryChange::Workflows => {
                                        tracing::info!(
...
mod tests {
    ...
    fn worker_runtime_teardown_bounded_despite_inflight_blocking_task() {
        let mut builder = tokio::runtime::Builder::new_current_thread();
        let rt = fuigo_tty_utils::runtime::build_with_blocking_pool(builder.enable_all()).unwrap();
