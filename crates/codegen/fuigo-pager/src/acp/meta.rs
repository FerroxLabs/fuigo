...
//! All fields are `Option`, so parsing degrades gracefully when fuigo-shell hasn't been updated or meta is absent.
...
pub struct NotificationMeta {
    ...
    /// Monotonic per-process sequence parsed from `eventId` (`"{sessionId}-{counter}"`, see `fuigo-shell util::event_id`).
...
/// The stamp is `_meta.isReplay`, set by fuigo-shell's `forward_raw_replay_line` during `session/load`.
///
...
pub mod user_prompt_meta {
    ...
    /// See [`fuigo_prompt_queue::COMBINED_DISPLAY_TEXTS_META`].
    pub const COMBINED_DISPLAY_TEXTS: &str = fuigo_prompt_queue::COMBINED_DISPLAY_TEXTS_META;
}
...
mod tests {
    ...
    fn parse_missing_new_fields() {
        // Simulate old fuigo-shell that doesn't send streamStartMs/turnStartMs
        let meta_json = json!({
            "totalTokens": 1000u64,
            "agentTimestampMs": 1700000000000i64,
        });
