...
use fuigo_acp_lib::{AcpAgentTx, AcpClientRx, acp_send};
...
pub struct AcpConnection {
    ...
    /// Whether the agent is a fuigo-shell instance.
    pub is_fuigo_shell: bool,
    ...
    // When available, add field: startup_announcements: Option<Vec<fuigo_announcements::RemoteAnnouncement>>
    ...
    pub auth_manager: std::sync::Arc<fuigo_shell::auth::AuthManager>,
}
...
pub struct ConnectFlags {
    ...
    pub remote_settings: Option<fuigo_shell::util::config::RemoteSettings>,
...
pub async fn connect(cancel: &CancellationToken, flags: ConnectFlags) -> Result<AcpConnection> {
    ...
    let spawned = spawn::spawn_fuigo_shell(agent_config, cancel, memory_config).await?;
...
pub async fn connect_via_leader(
    ...
    startup::set_auth_mode(fuigo_shell::managed_config::classify_auth_mode());
    let mut agent_config = AgentConfig::new_from_toml_cfg(raw_config)
        .map_err(|e| anyhow::anyhow!("Failed to create agent config: {e}"))?;
    ...
    fuigo_shell::agent::init::update_telemetry_config(&agent_config, &auth_manager);
...
/// When `fuigo.api_key` was advertised, non-interactive credentials were available: do not promote to interactive auto-Login.
...
mod tests {
    ...
    fn parse_available_commands_missing_key_returns_empty() {
        let meta = serde_json::json!({ "fuigoShell": true });
        let cmds = parse_available_commands(meta.as_object());
    ...
    fn parse_session_recap_available_defaults_off_when_missing() {
        let meta = serde_json::json!({ "fuigoShell": true, "cancelRewind": true });
        assert!(!parse_session_recap_available(meta.as_object()));
    ...
    #[test]
    fn startup_auth_fuigo_com_no_provider_needs_login_pending() {
    ...
    #[test]
    fn startup_auth_fuigo_com_with_external_provider_command() {
    ...
    #[test]
    fn startup_auth_non_fuigo_com_no_login() {
    ...
    ///
    /// Enterprise/BYOK configs (e.g. an enterprise `~/.fuigo/config.toml` with a
    ...
    /// If a change re-orders `build_auth_methods()` to put `fuigo.api_key` anywhere other than first (the shape of a past regression), this test fails.
    ...
    /// `enterprise_byok_first_method_is_fuigo_api_key` and `enterprise_byok_config_does_not_require_login`.
    ...
    fn shell_built_auth_methods_for_byok_user_skip_login_screen() {
        use fuigo_shell::agent::auth_method::{AuthMethodsBuildInputs, build_auth_methods};
        ...
        assert!(
            ...
            "shell built auth_methods for a BYOK user, but the pager still \
             ...
             fuigo.api_key first or the pager stopped treating fuigo.api_key as \
             a no-login method.",
        );
        assert!(label.is_none());
        assert!(method_id.is_none());
    ...
    /// Inverse direction: when `fuigo.api_key` is NOT in the list, the pager MUST show the login screen.
    /// We assert this with `fuigo.api_key` present LATER in the list (the shape of a past regression) and confirm the pager still requires login.
    /// The pager only inspects `auth_methods.first()`.
    /// This locks the failure mode of the regression.
    /// If a refactor makes the pager scan past `.first()`, this test diverges from `startup_auth_fuigo_com_no_provider_needs_login_pending` above.
    ...
    fn startup_auth_fuigo_api_key_not_first_still_requires_login() {
        ...
        let methods = vec![
            ...
            make_auth_method(FUIGO_API_KEY_METHOD_ID, "fuigo.api_key", None),
        ];
        let (needs, _, _, _) = startup_auth_metadata(&methods);
        assert!(
            needs,
            "with grok.com first, the pager must require login -- pinning \
             the BAD-ordering failure mode (fuigo.api_key not first)",
        );
    }
    ...
    fn client_capabilities_meta_advertises_the_status_line_the_config_asked_for() {
        let key = fuigo_status_line::STATUS_LINE_CAPABILITY;
