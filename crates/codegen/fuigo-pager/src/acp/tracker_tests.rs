use super::*;
use std::sync::Arc;
/// Default meta with no timestamps (simulates old fuigo-shell or tests that don't care about timing).
...
fn test_search_tool_call_flow() {
    use fuigo_tools::types::output::{GrepFileMatch, GrepLineMatch, GrepSearchOutput};
    let mut tracker = AcpUpdateTracker::new();
...
/// No stream_start_ms (old fuigo-shell) should not break anything.
#[test]
...
fn activity_writing_tool_call_copy_covers_taxonomy_map() {
    for (name, _) in fuigo_tools::tool_taxonomy::WRITING_TOOL_WIRE_NAMES {
        let mut tracker = AcpUpdateTracker::new();
        tracker.note_tool_call_arguments_delta(Some(name), 0);
        let Some(TurnActivity::WritingToolCall(writing)) = tracker.activity() else {
            panic!("expected WritingToolCall activity for {name:?}");
...
#[test]
fn is_task_tool_recognizes_fuigo_build_variant() {
...
fn video_tool_variants_use_typed_path_not_generic_scrape() {
    ...
    let cases: &[(&str, ToolOutput)] = &[
        (
            "ImageToVideo",
            ToolOutput::ImageToVideo(fuigo_tools::types::output::MediaGenOutput::new(
                video_path.clone(),
            )),
        ),
        (
            "ReferenceToVideo",
            ToolOutput::ReferenceToVideo(fuigo_tools::types::output::MediaGenOutput::new(
                video_path.clone(),
            )),
        ),
    ];
...
fn media_gen_ref_skips_uploaded_only_video() {
    let output = ToolOutput::ImageToVideo(fuigo_tools::types::output::MediaGenOutput::uploaded(
