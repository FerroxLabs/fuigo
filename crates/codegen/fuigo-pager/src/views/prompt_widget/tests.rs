#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn cmd_a_image_chip_selection_is_path_free() {
        ...
        img.source_path = Some(PathBuf::from("/tmp/fuigo-test-image.png"));
        ...
        assert!(
            !full.contains("/tmp/fuigo-test-image.png"),
            "source path must not appear in the buffer chip: {full:?}"
        );
        assert_eq!(
            pw.images[0].source_path.as_deref(),
            Some(std::path::Path::new("/tmp/fuigo-test-image.png")),
        ...
        assert!(selected.contains("[Image #1]"));
        assert!(
            !selected.contains("/tmp/fuigo-test-image.png"),
    ...
    fn test_image() -> PastedImage {
        PastedImage {
            element_id: fuigo_ratatui_textarea::ElementId::from_raw(0), // overwritten by insert_image
            display_number: 0,
    ...
    fn fuzzy_result(path: &str, is_dir: bool) -> fuigo_workspace::file_system::FuzzyMatchResult {
        fuigo_workspace::file_system::FuzzyMatchResult {
    ...
    /// Widget with an active gate and a loaded suggestion: the state right after a turn ends with `fuigo/suggestPrompt` resolved.
