//! Release-safe FPS readout: `/debug fps`, and `FUIGO_FPS` on release builds.
//!
//! The frame profiler (`render::frame_metrics`, `FUIGO_FPS`) threads per-phase timings through `draw_frame`, so it exists only in debug/dev builds.
...
//!
//! `FUIGO_FPS` ownership: in debug/dev builds the env feeds `FrameMetrics` and this HUD stays toggle-only (no double overlay).
//! On release binaries, where that overlay does not exist, the same env enables this HUD from startup.
//! `FUIGO_FPS=1` is therefore never a silent no-op ([`HONORS_FUIGO_FPS_ENV`]).
//!
...
/// Whether this HUD owns the `FUIGO_FPS` env gate: only where the dev `FrameMetrics` overlay is compiled out.
/// In debug/dev builds the env keeps feeding that overlay alone.
const HONORS_FUIGO_FPS_ENV: bool = true;
/// Runtime state for the FPS HUD.
/// `FUIGO_FPS` enables it at startup on release binaries ([`HONORS_FUIGO_FPS_ENV`]); `/debug fps` toggles it live everywhere.
...
impl FpsHud {
    pub fn new() -> Self {
        Self::with_env(std::env::var("FUIGO_FPS").ok())
    }
    /// `env` is the raw `FUIGO_FPS` value; the truthiness rule (nonempty and not `"0"`) matches `FrameMetrics` and `FUIGO_SCROLL_DEBUG`.
    fn with_env(env: Option<String>) -> Self {
        let env_on = HONORS_FUIGO_FPS_ENV && env.is_some_and(|v| !v.is_empty() && v != "0");
        Self {
            enabled: env_on,
...
pub struct FpsOverlay {
    body: String,
    /// Rows left free for overlays above (the dev `FUIGO_FPS` line).
    pub top_offset: u16,
}
impl FpsOverlay {
...
mod tests {
    ...
    /// Asserting against [`HONORS_FUIGO_FPS_ENV`] keeps the test true under both cfgs.
    /// The dev half is pinned by the constant's shape, the same limitation as the `/debug` visibility test.
    #[test]
    fn fuigo_fps_env_enables_hud_where_dev_overlay_absent() {
        for truthy in ["1", "full", " "] {
            assert_eq!(
                FpsHud::with_env(Some(truthy.into())).enabled(),
                HONORS_FUIGO_FPS_ENV,
                "FUIGO_FPS={truthy:?} must track the env-gate owner"
            );
        }
