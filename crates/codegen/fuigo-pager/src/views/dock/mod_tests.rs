...
fn all_sections_render_with_counts_and_collapse_state() {
    ...
    assert!(first.trim_end().ends_with("fuigo-4.5 2m14s"), "{first}");
    assert!(
...
fn focused_cursor_highlights_and_shows_row_actions() {
    ...
    assert_eq!(buf.cell((0, 1)).map(|c| c.bg), Some(row_hover_bg(&theme)));
