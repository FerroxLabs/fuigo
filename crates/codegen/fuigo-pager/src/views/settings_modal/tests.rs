...
fn editor_render_fixture(buffer: &str, cursor_byte: usize) -> SettingsModalState {
    ...
    let snapshot = PagerLocalSnapshot {
        available_models: vec![(
            "Fuigo Test".to_string(),
            acp::ModelId::new(Arc::from("fuigo-test")),
        )],
        ..PagerLocalSnapshot::default()
    };
...
fn picking_enum_esc_returns_to_browse() {
    let mut s = make_state();
    s.transition_to_picking_enum("theme", 0, SettingValue::Enum("fuigonight"), true);
    let outcome = handle_settings_key(&mut s, &KeyEvent::new(KeyCode::Esc, KeyModifiers::NONE));
    match outcome {
        ...
        other => panic!("expected Action::PreviewTheme(\"fuigonight\") on Esc, got {other:?}"),
    }
    assert!(matches!(s.mode(), SettingsModalMode::Browse));
}
...
fn fork_secondary_model_picker_opens_on_persisted_model() {
    ...
    assert_ne!(slug, fuigo_shell::models::default_model());
    let snapshot = PagerLocalSnapshot {
        available_models: vec![
...
fn picker_long_description_wraps_to_multiple_lines() {
    let entries = vec![SettingMeta {
        ...
        description: "Controls whether Ferrox Labs may retain and train on coding data.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-out",
            choices: &[
                EnumChoice {
                    canonical: "opt-in",
                    display: "Opt in",
                    description: "Allow Ferrox Labs to retain and use coding session data for training and product improvement.",
                },
                EnumChoice {
                    canonical: "opt-out",
                    display: "Opt out",
...
fn picker_multi_line_choice_hit_rect_spans_all_lines() {
    ...
    let entries = vec![SettingMeta {
        ...
        owner: SettingOwner::Shared,
        label: "Coding data sharing",
        description: "Controls whether Ferrox Labs may retain coding data.",
        keywords: &["test"],
        kind: SettingKind::Enum {
            default: "opt-in",
            choices: &[
...
fn string_editor_uses_canonical_edits_policy_and_live_validation() {
    ...
    let mut state = editor_render_fixture("Fuigo Tes", "Fuigo Tes".len());
    ...
    assert_eq!(state.editing_buffer(), Some("Fuigo Test"));
...
//    the "Tip · Ask Fuigo…" docs footer and the keybindings hints.
//
...
fn string_editor_paste_sanitizes_validates_and_consumes_rejected_text() {
    let mut state = editor_render_fixture("Fuigo Tst", "Fuigo T".len());
...
fn docs_footer_tip_is_centered() {
    ...
    // SHORT is "Tip · Ask Fuigo to change a setting" (34 cells); LONG is ~73 cells. width=40 lands in the SHORT band.
    ...
    assert!(
        !row_short.contains("fuigoday"),
        "width=40 must NOT render LONG path (no `fuigoday`): {row_short:?}",
    );
    assert!(
...
fn tip_line_has_blank_row_above() {
    ...
    for y in 0..area.height {
        let txt = buf_row_text(&buf, y, area.x, area.width);
        if txt.contains("Tip") && txt.contains("Ask Fuigo") {
            tip_y = Some(y);
            break;
        }
    }
...
fn click_settings_breadcrumb_collapses_picker_to_browse() {
    ...
    // The default theme's original canonical is `"fuigonight"`
    match outcome {
        ...
        other => panic!(
            "expected Action(PreviewTheme(\"fuigonight\")) — the keyboard \
...
fn click_settings_breadcrumb_after_nav_reverts_to_original() {
    ...
    // The default theme is `fuigonight` (index 1 per the registry); advance to index 0 to ensure we're navigating to a different value
...
fn d_key_in_picking_enum_dispatches_open_reset_confirm() {
    ...
    match outcome {
        SettingsKeyOutcome::ActionPair(
            ...
            // Default theme is `fuigonight`
            // Entering the picker captures `original_value = current value = fuigonight`, so the revert dispatches with that canonical
            assert_eq!(
                orig, "fuigonight",
...
fn consent_chooser_drops_tip_and_reset() {
    ...
    assert!(
        text.contains("d reset") && text.contains("Ask Fuigo"),
...
fn max_thoughts_width_preview_title_styling_distinguishes_from_content() {
    ...
    let raw_theme = match crate::theme::Theme::current_kind() {
        crate::theme::ThemeKind::FuigoNight => crate::theme::Theme::fuigonight(),
        crate::theme::ThemeKind::TokyoNight => crate::theme::Theme::tokyonight(),
        crate::theme::ThemeKind::FuigoDay => crate::theme::Theme::fuigoday(),
