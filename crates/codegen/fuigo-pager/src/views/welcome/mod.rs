...
struct WelcomeLayoutInput<'a> {
    ...
    prompt_compact: bool,
    announcement: Option<&'a fuigo_announcements::RemoteAnnouncement>,
...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **Fuigo** VERSION+channel (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | channel (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **Fuigo**  VERSION (left-aligned).
    HeroInline,
}
...
pub(super) fn render_version_badge(
    ...
    let channel = fuigo_update::channel_label();
    match &mode {
        VersionBadgeMode::Full { .. } => {
            ...
            spans.push(Span::styled(
                format!("{}{}", fuigo_version::VERSION, channel),
                Style::default().fg(theme.gray),
            ));
        }
...
pub struct WelcomeRenderParams<'a> {
    ...
    pub announcement: Option<&'a fuigo_announcements::RemoteAnnouncement>,
    ...
    pub foreign_resume_hint: Option<&'a fuigo_foreign_sessions::RecentForeignSession>,
    pub is_api_key_auth: bool,
    pub session_picker_content_results:
        Option<&'a [fuigo_shell::extensions::session_search::SearchSessionHit]>,
    ...
    pub gate: Option<&'a fuigo_shell::auth::GateInfo>,
    ...
    let mut result = match params.auth_state {
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "Fuigo is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
/// Here the message is a multi-line block showing the workspace path and the warning that Fuigo may run or modify contents in this directory.
...
fn render_announcement_section(
    ...
    buf: &mut Buffer,
    theme: &Theme,
    announcement: &fuigo_announcements::RemoteAnnouncement,
    min_width_hint: u16,
    content_height: u16,
    expanded: bool,
    mouse_pos: Option<(u16, u16)>,
...
fn masked_auth_token_view(input: &str, cursor_byte: usize, width: usize) -> (String, usize) {
    ...
    let buffer =
        fuigo_ratatui_textarea::EditBuffer::from_parts(masked.display.as_str(), masked.cursor_byte);
...
mod tests {
    ...
    fn version_badge_carries_no_release_label() {
        ...
        assert!(full.contains("Fuigo"), "full badge: {full:?}");
        assert!(inline.contains("Fuigo"), "inline badge: {inline:?}");
    ...
    fn foreign_resume_tip_names_each_tool_and_age() {
        use fuigo_foreign_sessions::ForeignSessionTool;
        ...
        for (tool, label) in [
            ...
            let hint = fuigo_foreign_sessions::RecentForeignSession {
                tool,
    ...
    fn grouped_entries_insert_headers() {
        let entries = vec![
            make_entry("s1", "Fix auth", "fuigo"),
            make_entry("s2", "Add streaming", "fuigo"),
    ...
    fn grouped_entries_rows_are_indented() {
        let entries = vec![make_entry("s1", "Fix auth", "fuigo")];
    ...
    fn long_ann() -> fuigo_announcements::RemoteAnnouncement {
...
the usual channels. "
    ...
    fn announcement_equal_for_short_message() {
        let area = Rect::new(0, 0, 120, 60);
        let a = fuigo_announcements::RemoteAnnouncement {
            title: Some("FYI".into()),
