...
pub const WORKSPACE_MODE_LOG: &str = "fuigo.pager.workspace_mode";
...
mod apply_tests {
    use super::*;
    use crate::app::session_startup::{
        FUIGO_CHAT_LOCAL_WORKSPACE_ACK_ENV, LocalWorkspaceMode, set_active_local_workspace,
    };
    ...
    fn welcome_local_one_shot_only_when_agents_alive() {
        let _ack = fuigo_test_support::EnvGuard::set(FUIGO_CHAT_LOCAL_WORKSPACE_ACK_ENV, "1");
        set_active_local_workspace(None).unwrap();
        let tmp = tempfile::tempdir().unwrap();
        let out = prepare_welcome_workspace_for_new_session(
            WelcomeWorkspaceMode::LocalWorkspace,
