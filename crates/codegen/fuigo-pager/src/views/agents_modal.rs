...
/// User-level vs project-level config files (`~/.fuigo` vs `{cwd}/.fuigo`).
...
pub struct AgentsModalState {
    ...
    bundle: BundleState,
    /// Resolved startup agent name (same chain as the shell: `[agent]`, `FUIGO_AGENT`, model `agentType`, then `fuigo-build`).
...
/// FuigoBuildConcise, FuigoBuildPlan, FuigoBuildPlanNoSubagents, FuigoBuildAskUser, Codex, Opencode, CursorExtended, FuigoBuildOrchestrator.
...
impl AgentsModalState {
    ...
    #[cfg(test)]
    fn search_viewport(&self, width: usize) -> fuigo_ratatui_textarea::SingleLineViewport {
...
pub fn build_agent_list(
    ...
    let subagent_names: Vec<String> = BuiltinAgentName::subagent_variants()
        ...
        .collect();
    let discovered = fuigo_agent::discovery::discover(cwd);
    ...
    if let Some(registry) = plugins {
        for agent in fuigo_agent::discovery::plugin_agents(registry) {
...
/// Union bundled personas with local `~/.fuigo/personas` and `{cwd}/.fuigo/personas`.
///
...
pub fn merge_persona_lists(bundle: &BundleState, cwd: &Path) -> Vec<PersonaDetail> {
    ...
    let bundled_dir = fuigo_home.join("bundled").join("personas");
...
pub fn load_agent_toggle() -> HashMap<String, bool> {
    let root = match fuigo_shell::config::load_effective_config() {
        Ok(r) => r,
...
fn personas_dir_for_scope(scope: ConfigFileScope, cwd: &Path) -> PathBuf {
    match scope {
        ConfigFileScope::User => fuigo_config::fuigo_home().join("personas"),
...
/// True when `path` is a deletable local persona file (user or project `.fuigo/personas`).
pub fn persona_path_is_deletable(path: &Path) -> bool {
    config_path_is_user_or_project(path, "personas")
}
/// Shared guard: canonical path under `~/.fuigo/{subdir}` or `{cwd}/.fuigo/{subdir}`, not bundled.
fn config_path_is_user_or_project(path: &Path, subdir: &str) -> bool {
    ...
    let fuigo_home = fuigo_config::fuigo_home();
    let in_user = dunce::canonicalize(fuigo_home.join(subdir))
        .ok()
        .is_some_and(|d| canonical.starts_with(&d));
    let project_suffix = std::path::Path::new(".fuigo").join(subdir);
...
fn load_agent_selection_config() -> AgentSelectionConfig {
    fuigo_shell::config::load_effective_config()
        .ok()
        .and_then(|root| fuigo_shell::agent::config::Config::new_from_toml_cfg(&root).ok())
        .map(|cfg| cfg.agent)
        .unwrap_or_default()
}
...
/// Mirrors `MvpAgent::resolve_agent_definition` in fuigo-shell.
pub fn resolve_default_agent_name(cwd: &Path, model_agent_type: Option<&str>) -> String {
    ...
    fuigo_shell::agent::mvp_agent::MvpAgent::resolve_agent_definition(
        cwd,
        None,
        &agent_config,
        None,
...
pub fn format_agent_detail(entry: &AgentListEntry) -> Vec<String> {
    ...
    let mode_label = match def.prompt_mode {
        fuigo_agent::config::PromptMode::Extend => "extend",
        fuigo_agent::config::PromptMode::Full => "full",
    };
...
mod tests {
    ...
    fn merge_persona_lists_appends_local_only() {
        ...
        let personas_dir = dir.path().join(".fuigo").join("personas");
    ...
    fn persona_is_deletable_local_vs_bundled() {
        let dir = tempfile::tempdir().expect("tempdir");
        let local = dir.path().join(".fuigo").join("personas").join("p.toml");
        ...
        let bundled_detail = PersonaDetail {
            ...
            has_inputs: false,
            has_outputs: false,
            source_path: Some("/home/user/.fuigo/bundled/personas/b.toml".into()),
            scope_label: None,
        };
    ...
    fn delete_persona_file_allows_project_persona() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join(".fuigo").join("personas").join("gone.toml");
