...
pub struct DashboardRow {
    ...
    pub label: String,
    /// Right-of-label subtitle painted after a ` · ` separator in dim text (e.g. `"fuigo my-branch-2 worktree"`).
...
mod tests {
    ...
    fn workspace_member(
        ...
        title: &str,
        summary: Option<&str>,
    ) -> fuigo_dashboard_store::Member {
        fuigo_dashboard_store::Member {
            session_id: fuigo_dashboard_store::SessionId::new(session_id).unwrap(),
            ...
            model: Some("fuigo-test".to_owned()),
    ...
    ) -> fuigo_dashboard_store::WorkspaceSnapshot {
        fuigo_dashboard_store::WorkspaceSnapshot {
    ...
    fn workspace_rows_ignore_non_build_members() {
        ...
        conversation.kind = fuigo_dashboard_store::MemberKind::Conversation;
    ...
    fn unloaded_workspace_members_are_idle_in_snapshot_order() {
        ...
        assert_eq!(rows[1].secondary_line.as_deref(), Some("fuigo-test"));
    ...
    fn subtitle_worktree_shows_label_branch_and_marker() {
        ...
        agent.session.cwd = PathBuf::from("/home/me/.fuigo/worktrees/x/location-picker");
    ...
    fn subtitle_non_worktree_shows_branch_and_folder() {
        let mut agent = make_idle_agent_with_model(None);
        agent.session.cwd = PathBuf::from("/home/me/fuigo/crates/foo");
