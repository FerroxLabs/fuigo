...
use fuigo_shell::session::persistence::MAX_TITLE_SCALARS as MAX_RENAME_SCALARS;
...
/// Persisted dashboard configuration stored under `[dashboard]` in `~/.fuigo/config.toml`.
...
pub struct PendingDispatchModel {
    ...
    pub effort: Option<fuigo_shell::sampling::types::ReasoningEffort>,
    pub display: String,
}
        ...
        match sep {
            ...
            None => {
                if cfg!(windows) && has_windows_drive_prefix(q) && q.len() == 2 {
                    ...
                    let home = fuigo_dirs::home_dir().unwrap_or_else(|| self.base_cwd.clone());
...
fn resolve_dir_prefix(prefix: &str, base: &Path) -> PathBuf {
    ...
    if prefix.starts_with('/') {
        ...
        let mut home = fuigo_dirs::home_dir().unwrap_or_else(|| base.to_path_buf());
        if !rest.is_empty() {
            home.push(rest);
        }
        home
    ...
    fn handle_mouse(&mut self, mouse: &crossterm::event::MouseEvent) -> InputOutcome {
        ...
        if matches!(mouse.kind, MouseEventKind::Down(MouseButton::Left)) {
            ...
            if self.upgrade_cta_hit.contains(mouse.column, mouse.row) {
                return InputOutcome::Action(Action::AnnouncementsOpenCta(
                    fuigo_telemetry::events::AnnouncementCtaSurface::Dashboard,
                ));
            }
...
/// Load the full persisted dashboard from `~/.fuigo/config.toml`.
///
...
fn config_path() -> Option<PathBuf> {
    ...
    Some(home.join(fuigo_config::USER_CONFIG_FILENAME))
}
