...
fn compact_cwd_strips_home() {
    let p = Path::new("/Users/alice/projects/fuigo");
    assert_eq!(compact_cwd(p, Some("/Users/alice")), "~/projects/fuigo");
}
...
fn rename_at_cap_drops_extra_char() {
    assert_eq!(
        MAX_RENAME_SCALARS,
        fuigo_shell::session::persistence::MAX_TITLE_SCALARS
    );
    assert_eq!(fuigo_shell::session::persistence::MAX_TITLE_SCALARS, 100);
...
fn completion_reports_full_miss_for_unreadable_file_url() {
    ...
    let completion = state.complete_clipboard_attachment_paste(
        completion_ctx(None, false),
        crate::app::actions::ProbedAttachment::NoRaster,
        Some("file:///definitely/missing/fuigo-primary-paste.png".to_owned()),
    );
...
/// per-key serial lock. The `FUIGO_AGENT_DASHBOARD` key means this
/// test runs serially with any other test that decorates itself
/// with `#[serial_test::serial(FUIGO_AGENT_DASHBOARD)]` — see the
...
fn env_var_force_disables() {
    ...
    // same `FUIGO_AGENT_DASHBOARD` key reads it concurrently.
