...
/// Env override for the whole feature: `FUIGO_PROMPT_SUGGESTIONS=0/1`.
/// When unset, the persisted `prompt_suggestions` setting applies.
pub const PROMPT_SUGGESTIONS_ENV: &str = "FUIGO_PROMPT_SUGGESTIONS";

/// Env override for the model used by the suggestion call: `FUIGO_PROMPT_SUGGESTIONS_MODEL=<model-id>`.
pub const PROMPT_SUGGESTIONS_MODEL_ENV: &str = "FUIGO_PROMPT_SUGGESTIONS_MODEL";
...
pub struct PromptSuggestionController {
    ...
    shown_logged: bool,
    /// Whether the feature is enabled. Resolved from `FUIGO_PROMPT_SUGGESTIONS`, falling back to the persisted `prompt_suggestions` setting.
    pub enabled: bool,
}
...
pub fn resolve_enabled() -> bool {
    fuigo_shell::util::config::prompt_suggestions_enabled_for_config(
