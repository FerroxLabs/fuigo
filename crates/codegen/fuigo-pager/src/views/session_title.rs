...
pub fn entry_title(agent: &AgentView) -> String {
    ...
    if let Some(title) = agent.generated_session_title.as_deref() {
        ...
        if !trimmed.is_empty() {
            let clean =
                fuigo_tools::implementations::skills::skill::extract_skill_display_text(trimmed);
...
/// Same character class as [`fuigo_shell::session::persistence::is_forbidden_title_char`]; persist drops, display replaces.
///
