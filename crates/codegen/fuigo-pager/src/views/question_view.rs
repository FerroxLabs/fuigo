...
use fuigo_acp_lib::AcpResult;
use fuigo_markdown::StreamingMarkdownRenderer;
pub use fuigo_tools::implementations::fuigo_build::ask_user_question::{
...
/// Pager-internal origin for a locally-opened question (one that was NOT driven by an ACP `fuigo/ask_user_question` request).
///
...
pub enum LocalQuestionKind {
    ...
    AgentTypeMismatch {
        ...
        effort: Option<fuigo_shell::sampling::types::ReasoningEffort>,
    },
    DoctorFix {
...
pub const FEEDBACK_QUESTION_LABEL: &str = "How can we improve Fuigo?";
...
pub const FEEDBACK_TRACE_QUESTION_LABEL: &str = "Opt-in to provide your trace for debugging \
     purposes. This will also provide Ferrox Labs the ability to retain and train on coding data, \
...
///
/// Created when an `fuigo/ask_user_question` ext-method request arrives;
...
pub struct QuestionViewState {
    ...
    /// `Some` when this question was opened locally (e.g. by `/fork`) instead of by an ACP `fuigo/ask_user_question` request.
...
impl QuestionViewState {
    ...
    ///
    /// Called by the `ExtMethod` handler when a blocking `fuigo/ask_user_question` request arrives from the shell coordinator.
...
impl QuestionViewState {
    ...
    pub fn build_accepted_response(
        &self,
    ) -> fuigo_tools::implementations::fuigo_build::ask_user_question::AskUserQuestionExtResponse
    {
    ...
    pub fn send_ext_response(
        &mut self,
        response: fuigo_tools::implementations::fuigo_build::ask_user_question::AskUserQuestionExtResponse,
    ) -> bool {
...
mod tests {
    ...
    fn chrome_height_option_less_question_drops_the_label_gap() {
        ...
        let q = make_question("How can we improve Fuigo?", &[], false);
        assert_eq!(
            chrome_height(
                &q,
                80,
