...
/// For paths with only one normal component (e.g., `/fuigo`), returns that component alone.
/// Does not perform tilde expansion; callers provide absolute paths.
/// Returns `"unknown"` for empty input.
///
/// Examples: `/home/user/fw/1` becomes `"fw-1"`, `/fuigo` becomes `"fuigo"`, `/` becomes `"/"`.
///
...
pub(crate) fn pending_delete_from_selection(
    ...
    content_results: Option<&[fuigo_shell::extensions::session_search::SearchSessionHit]>,
...
/// The filter check (not `entries.is_none()`) matters: the fast foreign scan can land rows the default Fuigo view hides before the native list
/// arrives.
...
///
/// Default is [`Self::Fuigo`]: native Fuigo sessions only (local / remote / conversation).
/// That keeps Claude/Codex/Cursor foreign sessions and `fuigo -p` one-shots out of `/resume`.
/// `f` cycles Fuigo, Headless, External, All, Local, Remote, then back to Fuigo.
...
pub enum SourceFilter {
    /// Native Fuigo sessions only; excludes Claude/Codex/Cursor foreign rows.
    #[default]
    Fuigo,
    /// `fuigo -p` one-shots only (`session_kind == "headless"`).
    Headless,
    Local,
    Remote,
    External,
...
impl SourceFilter {
    ...
    pub fn headless_policy(self) -> fuigo_shell::session::unified_list::HeadlessPolicy {
    ...
    ///
    /// grok.com conversations carry `source == "conversation"` and live remotely, so they pass the `Remote` filter (and `Fuigo` / `All`) but not
    /// `Local`.
    ...
    pub fn matches(self, source: &str, session_kind: Option<&str>) -> bool {
        ...
        match self {
            Self::Fuigo => !crate::app::is_foreign_picker_source(source) && !is_headless,
...
pub(crate) fn build_content_entry_data(
    hits: &[fuigo_shell::extensions::session_search::SearchSessionHit],
    entries_data: &[SessionPickerEntry],
    filtered_indices: &[usize],
    state: &PickerState,
    content_start: usize,
...
mod tests {
    ...
    fn repo_name_from_cwd_standard_path() {
        assert_eq!(repo_name_from_cwd("/home/user/fuigo"), "user-fuigo");
    }
    ...
    fn repo_name_from_cwd_single_dir() {
        assert_eq!(repo_name_from_cwd("/fuigo"), "fuigo");
    }
    ...
    fn source_filter_matches() {
        // Default Fuigo filter: native only (not Claude/Codex/Cursor).
        assert!(SourceFilter::Fuigo.matches("local", None));
        assert!(SourceFilter::Fuigo.matches("remote", None));
        assert!(SourceFilter::Fuigo.matches("both", None));
        assert!(SourceFilter::Fuigo.matches("conversation", None));
        assert!(!SourceFilter::Fuigo.matches("claude", None));
        assert!(!SourceFilter::Fuigo.matches("codex", None));
        assert!(!SourceFilter::Fuigo.matches("cursor", None));
        ...
        // grok.com conversations are remote: visible under Fuigo, All, and Remote, not Local
    ...
    fn source_filter_cycles() {
        assert_eq!(SourceFilter::Fuigo.next(), SourceFilter::Headless);
        ...
        assert_eq!(SourceFilter::Remote.next(), SourceFilter::Fuigo);
        assert_eq!(SourceFilter::Fuigo.label(), "Fuigo");
        assert_eq!(SourceFilter::Headless.label(), "Headless");
        assert_eq!(SourceFilter::External.label(), "External");
        assert_eq!(SourceFilter::default(), SourceFilter::Fuigo);
    }
    ...
    fn source_filter_filters_entries() {
        ...
        let fuigo = filter_session_entries(Some(&entries), "", SourceFilter::Fuigo);
        assert_eq!(fuigo, vec![0, 1, 2]);
    ...
    fn source_filter_empty_and_unknown_source() {
        ...
        // It passes Fuigo and All but never Local, Remote, or External
        assert!(SourceFilter::Fuigo.matches("", None));
        ...
        assert!(SourceFilter::Fuigo.matches("unknown", None));
    ...
    fn source_filter_is_active() {
        assert!(!SourceFilter::Fuigo.is_active());
    ...
    fn hidden_external_hint_visibility() {
        ...
        assert!(hidden_external_hint(Some(&entries), SourceFilter::Fuigo).is_none());
