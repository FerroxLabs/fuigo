...
pub fn fuzzy_matches_hook(hook: &fuigo_hooks_plugins_types::HookInfo, query: &str) -> bool {
    if query.is_empty() {
        return true;
    }
...
fn hook_row_label(hook: &fuigo_hooks_plugins_types::HookInfo) -> String {
    let matcher = hook
        .matcher
        .as_deref()
        .map(|m| format!(" /{m}"))
...
fn is_official_marketplace_source(source: &fuigo_hooks_plugins_types::MarketplaceScanResult) -> bool {
...
pub(crate) fn test_plugin_info(
    name: &str,
    origin: Option<fuigo_hooks_plugins_types::PluginOrigin>,
...
type GroupedPlugins<'a> = std::collections::BTreeMap<
    PluginGroupSortKey,
    Vec<(usize, &'a fuigo_hooks_plugins_types::PluginInfo)>,
>;
...
pub fn plugin_group(plugin: &fuigo_hooks_plugins_types::PluginInfo) -> PluginGroup {
    use fuigo_hooks_plugins_types::{PluginOrigin, PluginScope};

    match &plugin.origin {
        Some(PluginOrigin::ProjectFuigo) => PluginGroup::new(0, "origin:project", "Project"),
        Some(PluginOrigin::ProjectClaude) => {
            PluginGroup::new(1, "origin:project-claude", "Project (Claude)")
        }
        Some(PluginOrigin::UserFuigo) => PluginGroup::new(2, "origin:user", "User"),
...
fn build_hook_groups<'a>(
    hooks: &'a [fuigo_hooks_plugins_types::HookInfo],
    filter: StatusFilter,
    query: &str,
) -> Vec<HookGroupView<'a>> {
...
impl ExtensionsTab {
    ...
    pub fn telemetry_tab(self) -> fuigo_telemetry::events::ExtensionsModalTab {
        use fuigo_telemetry::events::ExtensionsModalTab;
        match self {
...
pub(crate) fn hook_group_any_enabled<'a>(
    hooks: impl Iterator<Item = &'a fuigo_hooks_plugins_types::HookInfo>,
) -> bool {
...
pub(crate) fn hook_source_pinned(
    hooks: &[fuigo_hooks_plugins_types::HookInfo],
    source_dir: &str,
) -> bool {
...
pub enum ButtonAction {
    ...
    HooksAction(fuigo_hooks_plugins_types::HooksAction),
    /// Execute a plugins action via ACP (no args needed).
    PluginsAction(fuigo_hooks_plugins_types::PluginsAction),
    ...
    MarketplaceAction(fuigo_hooks_plugins_types::MarketplaceAction),
...
impl ModalInputField {
    ...
    pub(crate) fn viewport(&self, width: usize) -> fuigo_ratatui_textarea::SingleLineViewport {
...
pub enum ConfirmationAction {
    ...
    Hooks(fuigo_hooks_plugins_types::HooksAction),
    ...
    Plugins(fuigo_hooks_plugins_types::PluginsAction),
    /// Replay a marketplace action (uninstall plugin or remove source).
    Marketplace(fuigo_hooks_plugins_types::MarketplaceAction),
...
pub fn resolve_key(tab: ExtensionsTab, ch: char) -> Option<ButtonAction> {
    use fuigo_hooks_plugins_types::{HooksAction, MarketplaceAction, PluginsAction};
...
pub fn build_action_from_input(
    ...
    use fuigo_hooks_plugins_types::{HooksAction, PluginsAction};
    ...
    match command_prefix {
        ...
        "marketplace_add_source" => Some(ButtonAction::MarketplaceAction(
            fuigo_hooks_plugins_types::MarketplaceAction::AddSource {
                url: first.to_string(),
            },
        )),
        "mcp_add" => {
...
fn parse_mcp_add_fields(name: &str, url_or_cmd: &str) -> Option<ButtonAction> {
    use fuigo_shell::util::config::{McpServerConfig, McpServerTransportConfig};
...
pub struct ExtensionsModalState {
    ...
    pub plugins_data: TabDataState<fuigo_hooks_plugins_types::PluginsListResponse>,
    ...
    pub last_plugins_action: Option<fuigo_hooks_plugins_types::PluginsAction>,
    ...
    pub marketplace_data: TabDataState<fuigo_hooks_plugins_types::MarketplaceListResponse>,
...
impl ExtensionsModalState {
    ...
    pub fn seed_hook_groups_once(&mut self, hooks: &[fuigo_hooks_plugins_types::HookInfo]) {
        seed_groups_once(
    ...
    pub fn seed_plugin_groups_once(&mut self, plugins: &[fuigo_hooks_plugins_types::PluginInfo]) {
        seed_groups_once(
...
fn classify_hook_source(source_dir: &str) -> HookSourceMeta {
    let fuigo = fuigo_config::fuigo_home();
    let source_path = std::path::Path::new(source_dir);
    // Plugin / installed-plugin dirs, under the user fuigo home (FUIGO_HOME-aware) or a project-scoped `{cwd}/.fuigo/<subdir>/`
    ...
    let plugin_name = |subdir: &str| -> Option<String> {
        ...
        // User fuigo home (FUIGO_HOME-aware).
        if let Ok(rest) = source_path.strip_prefix(fuigo.join(subdir))
            && let Some(name) = first_comp(rest)
        {
            return Some(name);
        }
        // Project-scoped `.fuigo/<subdir>/<name>` anywhere in the path.
        ...
        comps
            .windows(3)
            .find(|w| w[0] == ".fuigo" && w[1] == subdir && !w[2].is_empty())
            .map(|w| w[2].clone())
    };
    ...
    // Global hooks under $FUIGO_HOME/hooks
    let global_hooks = fuigo.join("hooks");
    ...
    // Project hooks
    if source_dir.ends_with("/.fuigo/hooks") || source_dir.contains("/.fuigo/hooks/") {
        return HookSourceMeta {
    ...
    let display = {
        if let Ok(rest) = source_path.strip_prefix(&fuigo) {
            let prefix = crate::util::display_fuigo_home_prefix();
...
fn skill_source_str(skill: &SkillInfo) -> String {
    if let Some(ref cs) = skill.config_source {
        match cs {
            fuigo_tools::types::config_source::ConfigSource::User { path } => {
                if crate::util::is_under_user_fuigo_home(path) {
                    crate::util::display_user_fuigo_path("skills")
            ...
            fuigo_tools::types::config_source::ConfigSource::Project { path } => {
            ...
            fuigo_tools::types::config_source::ConfigSource::Plugin { plugin_name, .. } => {
...
fn build_plugin_fields(plugin: &fuigo_hooks_plugins_types::PluginInfo) -> Vec<String> {
    use fuigo_hooks_plugins_types::McpStatus;
...
fn component_categories(
    components: &fuigo_hooks_plugins_types::PluginComponents,
) -> [(&'static str, &[fuigo_hooks_plugins_types::ComponentItem]); 6] {
    use fuigo_hooks_plugins_types::ComponentCategory;
...
pub(crate) fn marketplace_components_summary(
    plugin: &fuigo_hooks_plugins_types::MarketplacePluginEntry,
) -> Option<String> {
    plugin
        .components
        .as_ref()
...
mod tests {
    ...
    fn derive_source_label_detects_project_scoped_plugins() {
        // Regression: project-scoped `{cwd}/.fuigo/plugins/<name>/` must label as a (non-removable) plugin, not a removable "Custom" source
        // The user fuigo-home branch is FUIGO_HOME-aware; this covers the project fallback
        let (label, is_custom) = derive_source_label("/repo/work/.fuigo/plugins/my-plugin/hooks");
        assert_eq!(label, "Plugin: my-plugin");
        assert!(!is_custom);

        let (label, is_custom) =
            derive_source_label("/repo/work/.fuigo/installed-plugins/vendor-abc123/skills");
    ...
    fn mcp_collapsed_managed_section_omits_server_rows() {
        ...
        assert!(
            !rows.labels.iter().any(|l| l == "fuigo_com_linear"),
    ...
    fn make_skill(
        ...
        fuigo_tools::implementations::skills::types::SkillInfo {
            ...
            metadata: None,
            path: "test".to_string(),
            scope: fuigo_tools::implementations::skills::types::SkillScope::User,
            config_source: None,
            plugin_name: None,
            plugin_version: None,
            plugin_root: None,
    ...
    fn make_plugin_skill(
        ...
        skill.scope = fuigo_tools::implementations::skills::types::SkillScope::Plugin;
    ...
    fn make_plugin(name: &str) -> fuigo_hooks_plugins_types::PluginInfo {
        test_plugin_info(name, None)
    }

    fn make_plugin_with_origin(
        name: &str,
        origin: fuigo_hooks_plugins_types::PluginOrigin,
    ) -> fuigo_hooks_plugins_types::PluginInfo {
    ...
    fn make_plugin_with_enabled(name: &str, enabled: bool) -> fuigo_hooks_plugins_types::PluginInfo {
    ...
    fn space_footer_desc_is_contextual_enable_or_disable() {
        ...
        state.picker_state.selected = 0;
        state.plugins_data = TabDataState::Loaded(fuigo_hooks_plugins_types::PluginsListResponse {
            plugins: vec![
    ...
    fn hook_source_pinned_is_source_level() {
        let mut pinned = make_hook("policy/a", "/etc/fuigo", false);
        pinned.pinned = true;
        let sibling = make_hook("user/b", "/etc/fuigo", false);
        let elsewhere = make_hook("user/c", "/home/u/.fuigo", false);
        let hooks = vec![pinned, sibling, elsewhere];

        assert!(hook_source_pinned(&hooks, "/etc/fuigo"));
        assert!(!hook_source_pinned(&hooks, "/home/u/.fuigo"));
    ...
    fn policy_enforced_selection_suppresses_space_hint() {
        let mut pinned = make_hook("policy/a", "/etc/fuigo", false);
        pinned.pinned = true;
        let mut user = make_hook("user/b", "/home/u/.fuigo", false);
        user.removable = true;

        // Entry maps as the picker builds them (headers carry a group key, no data index):
        //   0: header /etc/fuigo, 1: pinned row, 2: header user, 3: user row
        let mut state = ExtensionsModalState::new(ExtensionsTab::Hooks);
        state.hooks_data = TabDataState::Loaded(fuigo_hooks_plugins_types::HooksListResponse {
        ...
        let group_keys = vec![
            Some("/etc/fuigo".to_string()),
            None,
            Some("/home/u/.fuigo".to_string()),
            None,
        ];
        ...
        let mixed_state = {
            ...
            s.hooks_data = TabDataState::Loaded(fuigo_hooks_plugins_types::HooksListResponse {
    ...
    fn mcp_add_url_derives_name() {
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                assert!(matches!(
                    config.transport,
                    fuigo_shell::util::config::McpServerTransportConfig::StreamableHttp { .. }
                ));
            }
        ...
        match action {
            Some(ButtonAction::AddMcpServer { name, config }) => {
                ...
                match config.transport {
                    fuigo_shell::util::config::McpServerTransportConfig::Stdio {
                        command,
                        args,
                        ..
                    } => {
    ...
    fn build_action_single_field_plugins() {
        ...
        assert!(matches!(
            ...
            Some(ButtonAction::PluginsAction(
                fuigo_hooks_plugins_types::PluginsAction::Install { .. }
            ))
        ));
    }
    ...
    fn superpowers_source() -> fuigo_hooks_plugins_types::MarketplaceScanResult {
    ...
    struct TestPlugin {
        ...
        components: Option<fuigo_hooks_plugins_types::PluginComponents>,
    }
    ...
    impl TestPlugin {
        fn build(self) -> fuigo_hooks_plugins_types::MarketplacePluginEntry {
    ...
    fn marketplace_add_source_builds_action() {
        ...
        match action {
            Some(ButtonAction::MarketplaceAction(
                fuigo_hooks_plugins_types::MarketplaceAction::AddSource { url },
            )) => {
    ...
    fn marketplace_key_r_dispatches_refresh() {
        ...
        assert!(matches!(
            action,
            Some(ButtonAction::MarketplaceAction(
                fuigo_hooks_plugins_types::MarketplaceAction::Refresh {
                    source_url_or_path: None
                }
            ))
        ));
    ...
    fn marketplace_modal_state_with_loaded_data() {
        ...
        state.marketplace_data =
            TabDataState::Loaded(fuigo_hooks_plugins_types::MarketplaceListResponse {
    ...
    fn marketplace_error_source_renders_header_with_error_badge() {
        let error_source = fuigo_hooks_plugins_types::MarketplaceScanResult {
    ...
    fn component(name: &str, desc: Option<&str>) -> fuigo_hooks_plugins_types::ComponentItem {
        fuigo_hooks_plugins_types::ComponentItem::new(name, desc.map(str::to_string))
    }

    fn sample_components() -> fuigo_hooks_plugins_types::PluginComponents {
        fuigo_hooks_plugins_types::PluginComponents {
            skills: vec![
    ...
    fn marketplace_summary_empty_components_is_none() {
        let plugin = TestPlugin {
            name: "empty",
            components: Some(fuigo_hooks_plugins_types::PluginComponents::default()),
            ..Default::default()
        }
        .build();
    ...
    fn render_components_fields_caps_names_per_category() {
        let components = fuigo_hooks_plugins_types::PluginComponents {
            skills: (0..12)
    ...
    fn plugins_modal_state(
        plugins: Vec<fuigo_hooks_plugins_types::PluginInfo>,
    ) -> ExtensionsModalState {
        let mut state = ExtensionsModalState::new(ExtensionsTab::Plugins);
        state.plugins_data =
            TabDataState::Loaded(fuigo_hooks_plugins_types::PluginsListResponse { plugins });
        state
    }
    ...
    fn plugin_group_maps_each_origin_variant() {
        use fuigo_hooks_plugins_types::PluginOrigin;
        for (origin, rank, key, label) in [
            (PluginOrigin::ProjectFuigo, 0, "origin:project", "Project"),
            (
            ...
            (PluginOrigin::UserFuigo, 2, "origin:user", "User"),
            (
            ...
            (
                PluginOrigin::MarketplaceInstall {
                    source_name: Some("Ferrox Labs Official".into()),
    ...
    fn plugin_group_fallback_without_origin() {
        ...
        project.scope = fuigo_hooks_plugins_types::PluginScope::Project;
        ...
        cli.scope = fuigo_hooks_plugins_types::PluginScope::Cli;
        assert_eq!(plugin_group(&cli).key, "origin:cli");

        let mut config = make_plugin("cfg-tool");
        config.scope = fuigo_hooks_plugins_types::PluginScope::Config;
    ...
    fn plugin_group_unknown_origin_uses_scope_fallback() {
        let mut unknown = make_plugin_with_origin(
            "future-tool",
            fuigo_hooks_plugins_types::PluginOrigin::Unknown,
        );
    ...
    fn plugins_render_groups_with_headers_in_rank_order() {
        ...
        let mut state = plugins_modal_state(vec![
            ...
            make_plugin_with_origin("user-tool", PluginOrigin::UserFuigo),
    ...
    fn plugins_render_multiple_plugins_under_one_group() {
        use fuigo_hooks_plugins_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("solo-tool", PluginOrigin::UserFuigo),
            make_plugin_with_origin(
                "catalog-tool",
                PluginOrigin::ClaudeMarketplace {
                    marketplace: "claude-market".into(),
    ...
    fn plugins_collapsed_group_hides_rows_and_search_forces_open() {
        use fuigo_hooks_plugins_types::PluginOrigin;
        let mut plugin = make_plugin_with_origin("user-tool", PluginOrigin::UserFuigo);
    ...
    fn plugins_fallback_grouping_without_origin() {
        ...
        assert_eq!(buffer_count(&buf, "User (1 plugin)"), 1);
        assert_eq!(buffer_count(&buf, "Ferrox Labs Official (1 plugin)"), 1);
    ...
    fn marketplace_placeholders_render_only_when_expanded() {
        ...
        source.plugins[0].components = Some(fuigo_hooks_plugins_types::PluginComponents::default());
    ...
    fn plugins_sort_az_by_name_within_group() {
        use fuigo_hooks_plugins_types::PluginOrigin;
        let mut state = plugins_modal_state(vec![
            make_plugin_with_origin("Zebra", PluginOrigin::UserFuigo),
            make_plugin_with_origin("alpha", PluginOrigin::UserFuigo),
            make_plugin_with_origin("MID", PluginOrigin::UserFuigo),
        ]);
    ...
    fn skills_groups_then_az_by_label() {
        ...
        project_z.scope = fuigo_tools::implementations::skills::types::SkillScope::Local;
        project_z.config_source = Some(
            fuigo_tools::types::config_source::ConfigSource::Project {
                path: std::path::PathBuf::from("/repo/.fuigo/skills/zzz"),
            },
        );
        project_z.display_name = Some("zeta-proj".into());

        let mut project_a = make_skill("aaa-proj", "other project");
        project_a.scope = fuigo_tools::implementations::skills::types::SkillScope::Repo;
        project_a.config_source = Some(
            fuigo_tools::types::config_source::ConfigSource::Project {
                path: std::path::PathBuf::from("/repo/.fuigo/skills/aaa"),
            },
        );
    ...
    fn hooks_group_and_row_order() {
        ...
        h_stop.event = fuigo_hooks_plugins_types::HookEvent::Stop;
        let mut h_pre = make_hook("pre-hook", "/tmp/hooks-src", false);
        h_pre.event = fuigo_hooks_plugins_types::HookEvent::PreToolUse;
        h_pre.matcher = Some("Bash".into());
        let mut h_notify = make_hook("notify-hook", "/tmp/hooks-src", false);
        h_notify.event = fuigo_hooks_plugins_types::HookEvent::Notification;

        let hooks = vec![
            make_hook("c", "/zzz/custom", false),
            h_stop,
            make_hook("a", "/repo/.fuigo/hooks", false),
            h_pre,
            h_notify,
