...
//! Recipe: `FUIGO_FPS=1 FUIGO_SCROLL_DEBUG=1 fuigo --resume <session>`.
//! Then flip `scroll_mode`, `scroll_lines`, `invert_scroll`, or `scroll_speed` in `/settings` to compare variants live.
//! This HUD samples once per frame; `FUIGO_SCROLL_LOG=1` additionally records every event as JSONL (`input::scroll_log` in fuigo-pager-render).
//!
...
/// `FUIGO_SCROLL_DEBUG` (nonempty and not `"0"`) enables it at startup, and the hidden `/scroll-debug` command toggles it live.
...
impl ScrollDebugHud {
    pub fn new() -> Self {
        let env_on = std::env::var("FUIGO_SCROLL_DEBUG").is_ok_and(|v| !v.is_empty() && v != "0");
        Self { enabled: env_on }
    }
...
pub struct ScrollDebugPanel {
    ...
    /// Rows left free for FPS overlays stacked above (the dev `FUIGO_FPS` line and/or the release-safe `/debug fps` HUD).
    pub top_offset: u16,
}
