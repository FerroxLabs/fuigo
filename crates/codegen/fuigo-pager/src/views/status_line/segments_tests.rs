...
fn context() -> StatusLineContext {
    ...
    ctx.model.display_name = Some("Fuigo".into());
...
fn composes_every_segment_in_order() {
    assert_eq!(
        ...
        "project │ Fuigo │ 42% ctx │ $0.37 │ 1m23s │ status_line work"
    );
}
...
fn omits_segments_whose_data_is_missing_or_rounds_to_zero() {
    ...
    assert_eq!(
        ...
        "project │ Fuigo │ 42% ctx │ status_line work"
    );
...
/// The turn timer rolls into hours instead of counting minutes past 60.
#[test]
fn turn_timer_values() {
    assert_eq!(turn_timer(59), "59s");
    assert_eq!(turn_timer(59 * 60 + 59), "59m59s");
