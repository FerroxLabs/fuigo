...
//! Separate from `fuigo-pager-render`'s `vte::Perform`, which discards OSC and quantizes colour.
...
fn safe_link_target(uri: &str) -> Option<Arc<str>> {
    ...
    // The shared gate stops at the scheme, enough for Fuigo's own text but not a script's
