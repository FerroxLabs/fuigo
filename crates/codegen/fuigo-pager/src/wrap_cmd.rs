//! `fuigo wrap` runs any command in a local PTY that forwards its clipboard.
//!
//! Generalizes the `fuigo ssh` wrapper: spawns an arbitrary command inside a local pseudo-terminal.
...
//! It also stamps `LC_FUIGO_APPEARANCE` from the local OS theme so a remote `theme = "auto"` can resolve over SSH and tmux.
//!
//! Resolvable programs spawn directly.
//! On Unix, a command a direct spawn cannot run (a single shell-quoted string `fuigo wrap "mycli ssh host"` or a shell alias) goes to `$SHELL -i -c`.
...
pub fn run(args: &WrapArgs) -> Result<()> {
    ...
    let program = args
        .command
        .first()
        .ok_or_else(|| anyhow::anyhow!("fuigo wrap: no command given"))?;
    ...
    let (wrapped, fallback) = {
        ...
        let in_path = fuigo_config::shell::is_command_available(program);
        (
...
/// The program and argv `fuigo wrap` will actually spawn.
#[derive(Clone)]
struct SpawnPlan {
    program: String,
    args: Vec<String>,
...
fn derive_spawn(
    ...
    // A single argument containing whitespace is a shell-quoted command line (`fuigo wrap "mycli ssh host"`), not a program name
    ...
    // An empty one (`fuigo wrap "$PROG" ...` with `$PROG` unset) must keep failing fast instead of silently running the tail
...
/// This deliberately avoids `fuigo_config::shell::unix_shell_path`, which coerces to bash/zsh and would drop fish/other-shell aliases.
#[cfg(unix)]
...
///
/// Unlike `fuigo ssh`, `fuigo wrap` does not gate on the terminal brand.
