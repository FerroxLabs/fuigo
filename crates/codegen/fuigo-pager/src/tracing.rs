...
/// That is the dev pane filter (dev builds) or the firehose (`FUIGO_DEBUG_LOG` / `FUIGO_LOG_FILE`).
pub use fuigo_telemetry::debug_log::ACP_UPDATE_PAYLOAD_TARGET;
/// Target for the always-on compact ACP update summary line (kind, ids, status, payload sizes).
/// Cheap to format at streaming rate.
///
/// Defined in `fuigo-telemetry` so the firehose directives and the pager filter share one constant (re-exported here for callsites).
pub use fuigo_telemetry::debug_log::ACP_UPDATE_TARGET;
...
pub fn init_tracing() -> TracingHandle {
    ...
    let directives = format!(
        "fuigo_shell=info,fuigo_pager=trace,fuigo_tools=info,fuigo_session_search=info,fuigo_acp_lib=info,{RMCP_SSE_NOISE_TARGET}=error,sampling_log=off,{ACP_UPDATE_TARGET}=debug,{ACP_UPDATE_PAYLOAD_TARGET}={payload_level}"
    );
    ...
    let otel_layer = fuigo_telemetry::otel_layer::build_otel_layer(
        fuigo_telemetry::otel_layer::OtelClientInfo {
            ...
            service_version: fuigo_version::full_version(),
            app_entrypoint: "tui",
        },
        fuigo_shell::auth::credential_provider::build_default_otel_layer_config(),
    );
    let instrumentation_layer = fuigo_telemetry::instrumentation::layer();
    let sampling_log_layer = fuigo_telemetry::sampling_log::layer();
    let hooks_log_layer = fuigo_telemetry::hooks_log::layer();
    let registry = tracing_subscriber::registry()
        .with(fmt_layer.with_filter(env_filter))
        .with(instrumentation_layer)
        .with(sampling_log_layer)
        .with(fuigo_telemetry::span_profile::layer("tui"))
