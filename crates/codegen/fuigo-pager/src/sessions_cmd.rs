...
use fuigo_shell::auth::{AuthManager, try_ensure_fresh_auth};
use fuigo_shell::session::merge::MergedSession;
...
pub async fn run(args: SessionsArgs, agent_config: &AgentConfig) -> Result<()> {
    ...
    // Enterprise deployments may configure only a deployment_key and a custom fuigo_api_base_url
    // If the user has previously run the interactive `fuigo` TUI (which succeeds for these setups), any cached credential is used
    // Otherwise we still proceed so the SessionRegistryClient can use the deployment_key when talking to the custom proxy
    let auth = try_ensure_fresh_auth(&agent_config.fuigo_com_config).await;
    ...
    let client = fuigo_shell::agent::session_registry_client::SessionRegistryClient::new(
    ...
    match args.command {
        SessionsCommand::List { limit } => {
            let sessions = fuigo_shell::session::merge::fetch_merged(
                Some(&client),
                cwd.to_str(),
                fuigo_shell::session::merge::CwdScope::WithSiblings,
                None,
                limit,
                // The CLI listing is an inventory, not the resume picker.
                fuigo_shell::session::visibility::HeadlessPolicy::Include,
            )
            .await;
        ...
        SessionsCommand::Delete { id } => {
            ...
            let deletion = fuigo_shell::session::persistence::delete_session_history(
                &id,
                None,
                needs_remote,
                auth_manager.clone(),
