...
pub fn make_worktree_record(
    ...
    use fuigo_fast_worktree::{WorktreeKind, WorktreeRecord, WorktreeStatus};
    WorktreeRecord {
        id: id.to_owned(),
...
/// Shared FUIGO_HOME boundary fixture for the resume-by-title startup and pre-sandbox tests.
///
/// `fuigo_home()` is OnceLock-cached process-wide, so summaries land under the
...
impl Default for FuigoHomeFixture {
    fn default() -> Self {
        Self::new()
    }
}
impl FuigoHomeFixture {
    ...
    fn sessions_cwd_dir(cwd: &str) -> std::path::PathBuf {
        let encoded = fuigo_shell::util::fuigo_home::encode_cwd_dirname(cwd);
...
impl TempGitRepo {
    ...
    /// CoW-style standalone clone: `.git` is a directory plus `fuigo-worktree-source`.
    pub fn standalone_clone(&self, branch: &str) -> Self {
        ...
        std::fs::write(
            path.join(".git").join("fuigo-worktree-source"),
