//! Host clipboard image paste mediated by `fuigo wrap`.
//!
...
//! Any process that can write to the PTY (not only the inner `fuigo`) can solicit the host pasteboard.
//! That is intentional and acceptable for `fuigo wrap`: the user opted into wrap on their own host, and the answer stays inside their session.
//! The remote also only requests when `osc52_sink_active()` (wrap already set `FUIGO_OSC52_SINK` / `LC_FUIGO_OSC52_SINK`).
//! Do not generalize this pattern to untrusted multiplexers without an explicit allowlist.

use base64::Engine as _;
use fuigo_pager_render::clipboard::{ImageData, osc52_sink_active};

/// OSC body after `ESC ]` for a host image request.
pub const REQUEST_BODY: &[u8] = b"999;FuigoWrapClipboardImage?";
...
/// Successful host image frame: `FUIGO_WRAP_IMG\n<mime>\n<base64>`.
pub const MAGIC_IMG: &str = "FUIGO_WRAP_IMG";

/// Host has no image (`FUIGO_WRAP_NONE`; not a prefix of [`MAGIC_IMG`]).
pub const MAGIC_NONE: &str = "FUIGO_WRAP_NONE";
...
mod tests {
    ...
    fn garbage_is_not_wrap_paste() {
        ...
        assert_eq!(try_decode_wrap_host_image_paste("FUIGO_WRAP_IM"), None);
    }

    #[test]
    fn malformed_img_frame_consumed_not_text() {
        assert_eq!(
            try_decode_wrap_host_image_paste("FUIGO_WRAP_IMG"),
            Some(WrapImagePaste::NoImage)
        );
        assert_eq!(
            try_decode_wrap_host_image_paste("FUIGO_WRAP_IMG\nbad"),
    ...
    fn oversized_b64_rejected_before_decode() {
        ...
        let payload = format!("FUIGO_WRAP_IMG\nimage/png\n{huge}");
        assert_eq!(
