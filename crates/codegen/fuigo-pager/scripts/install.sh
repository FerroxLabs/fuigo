#!/bin/bash
#
# Fuigo CLI installer — https://x.ai/cli/install.sh
#
# Auth: FUIGO_DEPLOYMENT_KEY (takes precedence) or ~/.fuigo/auth.json from `fuigo login`.
# Env: FUIGO_CHANNEL (stable|alpha|enterprise, default: stable), FUIGO_BIN_DIR, FUIGO_PROXY_URL
#
# Usage:
#   curl -fsSL https://x.ai/cli/install.sh | bash            # latest stable
#   curl -fsSL https://x.ai/cli/install.sh | bash -s 0.1.42  # specific version
#   FUIGO_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://x.ai/cli/install.sh)
#
...
# Read a token from ~/.fuigo/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_fuigo_token() {
    local auth_file="$HOME/.fuigo/auth.json"
    local scope="$1"
...
# Resolve auth: FUIGO_DEPLOYMENT_KEY > OIDC token > legacy token
OIDC_SCOPE="https://auth.x.ai::b1a00492-073a-47ea-816f-4c329264a828"
LEGACY_SCOPE="https://accounts.x.ai/sign-in"
AUTH_SOURCE=""

if [ -n "$FUIGO_DEPLOYMENT_KEY" ]; then
    AUTH_SOURCE="deployment key"
    echo "Auth: using deployment key." >&2
else
    OIDC_TOKEN=$(read_fuigo_token "$OIDC_SCOPE" 2>/dev/null) || true
    LEGACY_TOKEN=$(read_fuigo_token "$LEGACY_SCOPE" 2>/dev/null) || true
    if [ -n "$OIDC_TOKEN" ]; then
        AUTH_SOURCE="auth.json (oidc)"
        echo "Auth: using OIDC token from ~/.fuigo/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.fuigo/auth.json." >&2
    fi
fi
...
BASE_URL_FALLBACK="https://storage.googleapis.com/fuigo-build-public-artifacts/cli"
DOWNLOAD_DIR="$HOME/.fuigo/downloads"
BIN_DIR="${FUIGO_BIN_DIR:-$HOME/.fuigo/bin}"
mkdir -p "$DOWNLOAD_DIR" "$BIN_DIR"

platform="${os}-${arch}"
CHANNEL="${FUIGO_CHANNEL:-stable}"
case "$CHANNEL" in
    stable|alpha|enterprise) ;;
    *)
        echo "Invalid FUIGO_CHANNEL: '${CHANNEL}' (expected stable, alpha, or enterprise)" >&2
        exit 1
        ;;
esac
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing Fuigo $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing Fuigo $version ($platform)..." >&2
fi

binary_path="$DOWNLOAD_DIR/fuigo-$platform"
artifact_base="${BASE_URL}/fuigo-${version}-${platform}"
...
echo "  Downloading fuigo ${version}..." >&2
if [ "$os" = "windows" ]; then
    if ! fetch_binary "${artifact_base}.exe" "$binary_tmp"; then
        if ! fetch_binary "$artifact_base" "$binary_tmp"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: Fuigo is not yet available for your system ($platform)." >&2
            else
...
if [ "$os" = "windows" ]; then
    ...
    for bin_name in fuigo.exe agent.exe; do
    ...
    echo "  Binary installed to $BIN_DIR/fuigo.exe and $BIN_DIR/agent.exe." >&2
else
    chmod +x "$binary_tmp"
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded fuigo failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.fuigo/bin and ~/.fuigo/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/fuigo"
    ln -sf "$link_target" "$BIN_DIR/agent"
    echo "  Binary linked to $BIN_DIR/fuigo and $BIN_DIR/agent." >&2
fi

# Generate shell completions (best-effort)
mkdir -p "$HOME/.fuigo/completions/bash" "$HOME/.fuigo/completions/zsh"
"$BIN_DIR/fuigo" completions bash > "$HOME/.fuigo/completions/bash/fuigo.bash" 2>/dev/null || true
"$BIN_DIR/fuigo" completions zsh  > "$HOME/.fuigo/completions/zsh/_fuigo"     2>/dev/null || true
...
if mkdir -p "$HOME/.config/fish/completions" 2>/dev/null; then
    "$BIN_DIR/fuigo" completions fish > "$HOME/.config/fish/completions/fuigo.fish" 2>/dev/null || true
fi

# Persist installer source and channel to config
CONFIG_FILE="$HOME/.fuigo/config.toml"
...
if [ -n "$FUIGO_DEPLOYMENT_KEY" ]; then
    PROXY_URL="${FUIGO_PROXY_URL:-https://cli-chat-proxy.grok.com/v1}"
    ...
    if [ -z "$proxy_ok" ]; then
        echo "Error: FUIGO_PROXY_URL must be an https:// URL." >&2
        exit 1
    fi
    ...
    if [ -n "$AUTH_HEADER_FILE" ]; then
        ...
        printf 'Authorization: Bearer %s\n' "$FUIGO_DEPLOYMENT_KEY" > "$AUTH_HEADER_FILE"
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.fuigo/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.fuigo/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.fuigo/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.fuigo/requirements.toml"
        fi
    fi
fi

if [ "$os" = "windows" ]; then
    echo "Fuigo $version installed to $BIN_DIR/fuigo.exe" >&2
else
    echo "Fuigo $version installed to $BIN_DIR/fuigo" >&2
fi

# --- Ensure fuigo is on PATH ---

path_has_dir() {
    case ":$PATH:" in *":$1:"*) return 0 ;; *) return 1 ;; esac
}

# Try to symlink into a directory already on PATH so fuigo works immediately
...
if [ "$os" != "windows" ] && ! path_has_dir "$BIN_DIR"; then
    for candidate in "$HOME/.local/bin" "/usr/local/bin"; do
        if path_has_dir "$candidate" && [ -d "$candidate" ] && [ -w "$candidate" ]; then
            ln -sf "$BIN_DIR/fuigo" "$candidate/fuigo"
            ln -sf "$BIN_DIR/agent" "$candidate/agent"
            SYMLINK_CREATED="$candidate"
            echo "  Symlinked $candidate/fuigo -> $BIN_DIR/fuigo" >&2
...
# Also update shell config so ~/.fuigo/bin is on PATH for future sessions
...
if [ -n "$config_file" ]; then
    ...
    if [ "$user_shell" = "fish" ]; then
        new_block='# >>> fuigo installer >>>
fish_add_path $HOME/.fuigo/bin
# <<< fuigo installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> fuigo installer >>>
export PATH="$HOME/.fuigo/bin:$PATH"
fpath=(~/.fuigo/completions/zsh $fpath)
...
# <<< fuigo installer <<<'
    ...
    if grep -qs "fuigo installer" "$config_file" 2>/dev/null; then
        # Replace existing block in-place (strip old >>> to <<< lines, insert new)
        tmp="$config_file.tmp.$$"
        awk '
            /# >>> fuigo installer >>>/ { skip=1; next }
            /# <<< fuigo installer <<</ { skip=0; next }
            !skip { print }
        ' "$config_file" > "$tmp" && mv "$tmp" "$config_file"
    else
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    echo "Run 'fuigo' or 'agent' to get started!" >&2
elif [ -n "$config_file" ]; then
    echo "Restart your terminal, then run 'fuigo' or 'agent' to get started!" >&2
else
    echo "Add $BIN_DIR to your PATH, then run 'fuigo' or 'agent' to get started:" >&2
    echo '  export PATH="$HOME/.fuigo/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use fuigo from cmd.exe or PowerShell, add %USERPROFILE%\\.fuigo\\bin to your PATH." >&2
fi
