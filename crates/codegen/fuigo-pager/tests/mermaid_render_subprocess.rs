...
use fuigo_pager::app::mermaid_worker::render_via_subprocess;
use fuigo_pager::scrollback::blocks::mermaid_content::MermaidRenderQuality;
...
fn tight_timeout_kills_child_and_returns_err() {
    ...
    // That the kill terminates the child's process group is asserted by the `fuigo_mermaid::subprocess` `reap_terminates_the_process` unit test
