...
//!
//! Uses `fuigo-pager-pty-harness` (`PtyHarness`) and Shift+Tab (CSI Z, compatible with `ptyctl` key injection) to cycle Normal to Plan to Auto.
//! The mode banner or status line must show Auto as its own mode, distinct from Always-Approve.
//!
//! Auth: seeds `HOME/.fuigo/auth.json` from `FUIGO_AUTH_JSON` (path) or the
//! developer's `~/.fuigo/auth.json` so the pager skips device-login when
...
use fuigo_pager_pty_harness::{PtyHarness, pager_binary};
use fuigo_test_support::TestSandbox;
...
/// Prefer explicit path, else the user's real `~/.fuigo/auth.json`.
fn auth_json_source() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("FUIGO_AUTH_JSON") {
...
fn prepare_sandbox(sandbox: &mut TestSandbox, gate_on: bool) -> Vec<(String, String)> {
    ...
    sandbox.remove_env("FUIGO_API_KEY");

    let home = sandbox.home();
    let fuigo = sandbox.fuigo_home();
    let _ = std::fs::create_dir_all(fuigo);
    if let Some(src) = auth_json_source() {
        let dest = fuigo.join("auth.json");
        ...
        eprintln!("pty_auto_mode: no ~/.fuigo/auth.json — may hit device login");
    }
    ...
    // `FUIGO_AUTO_PERMISSION_MODE` is the highest gate layer below requirements; "1" and "0" parse to on and off (`fuigo_config::env_bool`)
    ...
    env.push((
        "FUIGO_AUTO_PERMISSION_MODE".into(),
...
fn require_auth() -> bool {
    std::env::var("FUIGO_PTY_REQUIRE_AUTH").is_ok_and(|v| v == "1" || v == "true")
}
...
fn pty_shift_tab_cycles_to_auto_mode_banner() {
    ...
    let mut harness =
        PtyHarness::new_in_sandbox(&binary, ROWS, COLS, &[], &sandbox, &env_refs, None)
            .expect("spawn pager in PTY (fuigo-pager-pty-harness)");
    ...
    if is_login_screen(&early) {
        assert!(
            !require_auth(),
            "FUIGO_PTY_REQUIRE_AUTH set but pager hit the login screen — auth seeding broke"
        );
