...
use fuigo_pager_pty_harness::StyledLine;
...
/// Seed a user-invocable skill under `dir/.fuigo/skills`. The workspace must
/// be a git repo: the live session advertises workspace-local skills only for git workspaces (offline `fuigo inspect` scans plain dirs too).
/// The mid_text_skill_token_echo.yaml scenario sets `git_init: true` for the same reason.
fn seed_test_skill(dir: &Path) {
    let skill_dir = dir.join(".fuigo").join("skills").join("test-skill");
