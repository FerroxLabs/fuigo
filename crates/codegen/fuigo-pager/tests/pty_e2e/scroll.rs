...
use fuigo_pager_pty_harness::PtyHarness;
pub(crate) use fuigo_pager_pty_harness::{SGR_SCROLL_DOWN, SGR_SCROLL_UP};
...
///
/// Harness mirror: the scroll-matrix runner (`fuigo-pager-pty-harness/src/scroll_matrix/runner.rs`) ports this send loop onto `WheelStep` tables.
...
///
/// Harness mirror: `spawn_settled_marker_session` in `fuigo-pager-pty-harness/src/scroll_matrix/session.rs` is a port of this preamble.
...
/// [`spawn_bottom_pinned_marker_scrollback`] with extra pager env vars (e.g. `FUIGO_SCROLL_SPEED`) appended to the controller's env.
...
///
/// Harness mirror: `spawn_streaming_marker_session` in `fuigo-pager-pty-harness/src/scroll_matrix/session.rs` is a port of this preamble.
/// Fixes must flow both ways.
