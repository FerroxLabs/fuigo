...
async fn run_reasoning_turn(collapse_thinking: bool) -> Turn {
    ...
    std::fs::create_dir_all(content.home().join(".fuigo")).expect("mk .fuigo");
    std::fs::write(
        content.home().join(".fuigo/config.toml"),
        "[ui]\nshow_thinking_blocks = true\n",
    )
    .expect("write config");
    if collapse_thinking {
        let fuigo_home = content.sandbox().fuigo_home().to_path_buf();
        std::fs::create_dir_all(&fuigo_home).expect("mk fuigo home");
        std::fs::write(
