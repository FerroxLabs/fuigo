...
/// `fuigo_telemetry::unified_log::path()` resolves the calling process's own fuigo home and the file-name const is private.
/// So this composes the sandbox fuigo home with the exported `LOG_DIR`.
pub(crate) fn unified_log_path(content: &ContentController) -> PathBuf {
    content
        .sandbox()
        .fuigo_home()
        .join(fuigo_telemetry::unified_log::LOG_DIR)
        .join("unified.jsonl")
}
...
pub(crate) const CONTEXTUAL_HINTS_ENV: &[(&str, &str)] = &[("FUIGO_CONTEXTUAL_HINTS", "1")];
...
// `seed_fake_oauth` / `oauth_credential_ops` live in `fuigo_pager_pty_harness::flows` (re-exported above)
...
/// The harness's default `FUIGO_API_KEY` (ApiKey/BYOK mode, no auth.json entry) would never fetch `/v1/settings`.
/// Spawns WITHOUT `FUIGO_ANNOUNCEMENTS_OVERRIDE` (the env override beats pushed lists in the pager and would mask updates).
...
pub(crate) fn spawn_polling_session_with_env(
    ...
    overrides.push(EnvOp::set("FUIGO_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS", "1"));
...
/// The default model is `"default-model"` (no agent type, so it uses the `fuigo-build` harness).
...
/// A self-built fuigo auto-trusts, so `FUIGO_TEST_VERSION` simulates a release; the gate is pinned both ways.
pub(crate) fn trust_env(feature_on: bool) -> [(&'static str, &'static str); 2] {
    [
        ("FUIGO_TEST_VERSION", "0.0.0-sim"),
        ("FUIGO_FOLDER_TRUST", if feature_on { "1" } else { "0" }),
    ]
}
...
pub(crate) fn folder_is_trusted(content: &ContentController, repo: &std::path::Path) -> bool {
    let store_path = content
        .home()
        .join(".fuigo")
        .join(fuigo_workspace::trust::TRUST_FILE_NAME);
    let store = fuigo_workspace::trust::TrustStore::load_from(store_path);
    store.is_trusted(&fuigo_workspace::trust::workspace_key(repo))
}
...
// `wait_for_labels_absent` lives in `fuigo_pager_pty_harness::flows` (re-exported above)
...
// Deduplication in load_hooks_from_sources is covered by unit tests in fuigo-hooks::discovery::tests
// A PTY e2e test would need careful environment variable setup to avoid static caching issues with FUIGO_HOME
...
/// Seed `~/.fuigo/config.toml` with a `[ui]` section body (e.g. `"vim_mode = true"`).
/// Same `{FUIGO_HOME|HOME}/.fuigo/config.toml` location `seed_mouse_reporting_toggle_config` uses; call before spawning the pager.
pub(crate) fn seed_ui_config(content: &ContentController, ui_body: &str) {
    let fuigo_home = content.home().join(".fuigo");
    std::fs::create_dir_all(&fuigo_home).expect("create .fuigo");
...
pub(crate) fn seed_mouse_reporting_toggle_config(content: &ContentController, enabled: bool) {
    let fuigo_home = content.home().join(".fuigo");
    std::fs::create_dir_all(&fuigo_home).expect("create .fuigo");
    // Minimal opt-in only; matches load_config's `{FUIGO_HOME|HOME}/.fuigo/config.toml`
...
pub(crate) fn spawn_mouse_toggle_pager(content: &ContentController) -> PtyHarness {
    ...
    PtyHarness::spawn_with_content_env_ops(
        ...
        content,
        &[],
        &[EnvOp::set("FUIGO_MOUSE_REPORTING_TOGGLE", "true")],
    )
    .expect("spawn pager")
}
...
pub(crate) const ESC_DOUBLE_PRESS_ENV: &str = "FUIGO_ESC_DOUBLE_PRESS_MS";
...
pub(crate) fn spawn_esc_double_press_pager(content: &ContentController) -> PtyHarness {
    let binary = pager_binary().expect("resolve pager binary");
    let value = fuigo_pager::app::app_view::ESC_DOUBLE_PRESS_TEST_MS.to_string();
    PtyHarness::spawn_with_content_env_ops(
        &binary,
        DEFAULT_ROWS,
        DEFAULT_COLS,
...
/// Locate `<fuigo_home>/sessions/<encoded cwd>/<session id>/`, where the shell keeps the session's `plan.md`.
...
pub(crate) fn session_dir(content: &ContentController, harness: &mut PtyHarness) -> PathBuf {
    let sessions = content.home().join(".fuigo").join("sessions");
    for _ in 0..100 {
...
/// Run `fuigo wrap <wrap_args...>` to completion inside a PTY with an isolated `FUIGO_HOME`.
...
pub(crate) fn run_wrap_driving(
    ...
    let mut env: Vec<(&str, &str)> = vec![("FUIGO_HOME", &home_str), ("NO_COLOR", "1")];
    env.extend_from_slice(extra_env);

    let mut harness =
        PtyHarness::new_inherited_env(&binary, DEFAULT_ROWS, DEFAULT_COLS, &args, &env, None)
            .expect("spawn fuigo wrap");
    ...
    let code = match wait_for_exit_status(&mut harness, WRAP_TIMEOUT) {
        ...
        Ok(PtyExitPoll::Running) => {
            harness.quit().expect("kill fuigo wrap after timeout");
            None
        }
...
/// Tests can then assert the exact argv `fuigo wrap` hands to the user's shell without depending on any real shell's rc files or alias state.
...
/// Dump an asciinema cast of `harness` into `$FUIGO_PTY_CAST_DIR/<file_name>` when the env var is set.
...
pub(crate) fn write_cast_if_requested(harness: &PtyHarness, file_name: &str) {
    let Ok(dir) = std::env::var("FUIGO_PTY_CAST_DIR") else {
        return;
    };
    if dir.is_empty() {
        return;
...
/// Dump the current screen (plain text and HTML) into `$FUIGO_PTY_CAST_DIR/<file_stem>.{txt,html}` when the env var is set.
...
pub(crate) use fuigo_pager_pty_harness::host_clipboard::{
