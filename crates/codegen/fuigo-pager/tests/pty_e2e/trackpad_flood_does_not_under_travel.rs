...
// So this test presents `TERM_PROGRAM=iTerm.app` (ept=1) and floods wheel-up reports with `FUIGO_SCROLL_SPEED=100`
...
async fn trackpad_flood_does_not_under_travel() {
    let (mut harness, _content, top_before) = spawn_bottom_pinned_marker_scrollback_with_env(
        MARKER_COUNT,
        &[("TERM_PROGRAM", "iTerm.app"), ("FUIGO_SCROLL_SPEED", "100")],
    )
    .await;
