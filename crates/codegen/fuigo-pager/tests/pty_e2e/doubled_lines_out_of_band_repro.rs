...
//!
//! The stack is `tmux -> nvim :terminal -> fuigo`: nvim/tmux repaint fuigo's pane out-of-band (no fuigo PTY resize).
//! Fuigo's diff renderer only re-clears on a real size change, so the rows it doesn't own survive.
...
//! We SIMULATE the out-of-band reflow with `feed_screen`, which writes straight into the virtual terminal, bypassing fuigo.
//! Then we check whether fuigo heals it.
//! `NVIM` is set in fuigo's env so it sees the embedded-editor context the fix keys off (mirroring a real nvim `:terminal`).
//!
//! The fix: fuigo forces a full clear and repaint on `FocusGained` in editor/multiplexer contexts, so the injected row is gone after refocus.
//! The final assertion guards that heal.

use super::common::*;

/// Unique sentinel that fuigo would never render on its own.
...
async fn out_of_band_stale_row_heals_on_focus_gained() {
    ...
    // Col 1 is the static left margin fuigo's diff renderer doesn't repaint during the logo shimmer
    ...
    // fuigo must not self-heal out-of-band content via ordinary diff redraws
    // It only rewrites cells whose own model changed, so this row is stranded
    h.update(Duration::from_millis(300));
    assert!(
        h.contains_text(STALE_MARKER),
        "stale row should survive a normal redraw (fuigo's diff renderer doesn't own it)\nscreen:\n{}",
        h.screen_contents()
    );

    // A FocusGained (CSI I) forces a full clear and repaint that re-asserts fuigo's whole screen and removes the out-of-band row
