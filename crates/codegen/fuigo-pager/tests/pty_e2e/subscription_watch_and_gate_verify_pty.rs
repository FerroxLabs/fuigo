...
///
/// Proto `prod_auth.SubscriptionTier`: 5 is `superfuigo_heavy`, the live `/user` string `SuperGrokPro`.
...
/// Like [`seed_fake_oauth`], but under the `FUIGO_LOCAL_AUTH` dev issuer (`http://localhost:22255`). Two reasons:
/// `is_fuigo_oauth2_issuer()` accepts the local issuer, so the subscription gate applies (an enterprise/unknown issuer bypasses it).
...
/// Pair with `FUIGO_LOCAL_AUTH=1` in the spawn env so the shell's scope-key lookup resolves this entry.
fn seed_fake_oauth_local_issuer(content: &ContentController, user: &str) {
    let fuigo_home = content.home().join(".fuigo");
    std::fs::create_dir_all(&fuigo_home).expect("create temp .fuigo");
    std::fs::write(
...
fn spawn_subscription_pager(
    ...
    overrides.push(EnvOp::set("FUIGO_LOCAL_AUTH", "1"));
...
async fn subscription_watch_polls_free_tier_then_goes_dormant_after_upgrade() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subwatch",
        &[EnvOp::set("FUIGO_SUBSCRIPTION_WATCH_INTERVAL_SECS", "1")],
    );
...
async fn stale_gate_push_never_flashes_paywall_for_subscribed_user() {
    ...
    let mut harness = spawn_subscription_session(
        &content,
        "pty-subgate-paid",
        &[EnvOp::set("FUIGO_SUBSCRIPTION_WATCH_INTERVAL_SECS", "0")],
    );
