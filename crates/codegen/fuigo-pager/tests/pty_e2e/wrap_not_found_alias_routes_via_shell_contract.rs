...
/// Real alias expansion is the shell's own contract (and rc-file dependent); the argv shape is the part `fuigo wrap` owns.
...
fn wrap_not_found_alias_routes_via_shell_contract() {
    ...
    let (code, raw) = run_wrap(
        &["fuigo-wrap-e2e-alias-xx", "with space"],
        &[("SHELL", &shell)],
    );
    ...
    assert!(
        raw.contains("ARG:fuigo-wrap-e2e-alias-xx 'with space'"),
