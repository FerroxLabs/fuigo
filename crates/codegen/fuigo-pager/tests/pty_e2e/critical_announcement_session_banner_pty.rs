...
fn spawn_with_announcements(content: &ContentController, override_json: &str) -> PtyHarness {
    ...
    let overrides: Vec<(String, String)> = vec![(
        "FUIGO_ANNOUNCEMENTS_OVERRIDE".into(),
...
/// The URL lands in the `FUIGO_TEST_OPEN_URL_FILE` file; no real browser opens.
...
async fn promo_cta_click_opens_link_and_hide_roundtrip() {
    ...
    let extra_env = [
        ("FUIGO_TEST_OPEN_URL_FILE", url_file_str.as_str()),
...
/// Pinned (`dismissible:false`) promo seeded via `FUIGO_ANNOUNCEMENTS_OVERRIDE`.
...
/// [`spawn_with_announcements`] with extra env pairs appended (e.g. `FUIGO_TEST_OPEN_URL_FILE` and a `TERM_PROGRAM` pin for OSC 8).
fn spawn_with_announcements_and_env(
    ...
    let binary = pager_binary().expect("resolve pager binary");
    let announcement = ("FUIGO_ANNOUNCEMENTS_OVERRIDE", override_json);
