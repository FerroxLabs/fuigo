...
async fn storage_upload_parks_on_401_and_drains_after_recovery() {
    ...
    // Trace uploads are gated on first-party Ferrox Labs OAuth (`is_fuigo_auth()` means AuthMode::Oidc with the Ferrox Labs issuer)
    // The harness's FUIGO_API_KEY is ApiKey mode and never uploads, so seed a fake OAuth entry instead
    ...
    let overrides = [
        ...
        EnvOp::set("FUIGO_TRACE_UPLOAD", "true"),
        EnvOp::set("FUIGO_TELEMETRY_TRACE_UPLOAD", "true"),
        EnvOp::set("FUIGO_UPLOAD_QUEUE_AUTH_PROBE_SECS", "2"),
    ];
    ...
    //
    // Accounting (fuigo-file-utils upload queue):
    ...
    //   `has_usable_credential()` stays true for the seeded entry, so the 2s probe (`FUIGO_UPLOAD_QUEUE_AUTH_PROBE_SECS`) can wake workers early
    //
