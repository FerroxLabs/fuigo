...
async fn word_select_tip_shows_and_ctrl_y_accepts() {
    ...
    let config_path = content.home().join(".fuigo").join("config.toml");
...
async fn word_select_tip_skipped_when_contextual_hint_disabled() {
    ...
    // FUIGO_CONTEXTUAL_HINTS stays unset here; that master switch would force all tips on and defeat the config opt-out
    ...
    let binary = pager_binary().expect("resolve pager binary");
    // Pin FUIGO_CONTEXTUAL_HINTS to empty (parsed as unset) so a value inherited from the runner's shell can't force tips on
    let overrides: Vec<(String, String)> = vec![("FUIGO_CONTEXTUAL_HINTS".into(), String::new())];
