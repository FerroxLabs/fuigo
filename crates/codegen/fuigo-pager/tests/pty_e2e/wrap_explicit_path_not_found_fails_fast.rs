...
/// PTY spawn fails, the exec fallback engages ("wrapped mode failed"), exec ENOENTs ("failed to exec"), and fuigo exits 1.
...
fn wrap_explicit_path_not_found_fails_fast() {
    let (code, raw) = run_wrap(&["/nonexistent-fuigo-wrap-e2e/prog", "arg"], &[]);
