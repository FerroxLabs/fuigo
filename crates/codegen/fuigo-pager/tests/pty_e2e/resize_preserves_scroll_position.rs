...
//
// Bug: the user has scrolled UP into fuigo's scrollback (NOT pinned to the bottom, not following) and the terminal is resized HORIZONTALLY
// Fuigo then loses its scroll position and the view jumps
...
// It only happens when NOT following; in follow/tail mode fuigo re-pins to the bottom (no jump)
//
...
//
// Runs FULLSCREEN (alt-screen): fuigo's mode-independent scroll-anchor logic is what's under test
// `Viewport::Fullscreen` is autoresized on resize with no DSR cursor probe, so fuigo itself re-wraps at the new width
// The alt-screen grid is not reflowed by the terminal, so the marker's position reflects fuigo's own re-layout, with no harness-reflow confound
...
///
/// fuigo renders markdown with SOFT line breaks (a single `\n` becomes a space).
...
async fn resize_preserves_scroll_position() {
    ...
    // Fuigo thus re-wraps the scrollback at the new width and the stale `scroll_offset` shows the jump in fuigo's OWN rendered output
    // The alt-screen grid is not reflowed by the terminal on resize, so this isolates fuigo's scroll-anchor logic with no harness confound
