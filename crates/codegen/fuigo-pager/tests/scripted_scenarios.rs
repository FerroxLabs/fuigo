...
//!
//! These tests exercise the real `fuigo-pager` binary through a PTY using YAML scenarios under `tests/scenarios/`.
...
/// Enterprise deploy report: with `FUIGO_GOAL=1`, `/goal` must show in the slash menu on the welcome screen before the first user turn.
...
/// Counterpart to `scripted_goal_slash_presession`, with the goal flag explicitly off (`FUIGO_GOAL=0`; goal mode defaults on).
...
/// Full folder-trust session: `FUIGO_FOLDER_TRUST=1` and a git repo that ships a repo-local `.mcp.json` (declared via the scenario `workspace`).
...
/// Plan-nudge opt-out edge: contextual hints are off by default, and this scenario also pins `FUIGO_CONTEXTUAL_HINTS=0` to be sure.
