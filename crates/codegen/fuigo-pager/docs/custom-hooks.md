...
Hooks let you run custom scripts or HTTP requests at key moments during a Fuigo session, for example before or after a tool runs, when a session starts or ends, or when the agent sends a notification.
...
2. Create a simple hook file, e.g. `~/.fuigo/hooks/session-start.json`:
   ```json
   {
     "hooks": {
       "SessionStart": [
...
|-----------|-----------------------------------|--------------|-------|
| Global    | `~/.fuigo/hooks/*.json`            | Always       | Best for personal hooks |
...
**Trusting a project**: The first time you open a project with hooks, open the hooks modal (`Ctrl+L` outside the VS Code family, or `/hooks` on any terminal) or run `/hooks-trust`. This is the same folder-trust gate as `--trust`, recorded in `~/.fuigo/trusted_folders.toml`. Trust prevents untrusted repos from running arbitrary code.
...
**Tool name aliases**: Claude-style names like `Bash`, `Edit`, `Read` automatically match Fuigo's internal names (`run_terminal_cmd`, `search_replace`, `read_file`).
...
The `hook_event_name` (snake_case key) carries Claude's PascalCase value; `hookEventName` (camelCase key) carries fuigo's snake_case value.
...
Fuigo injects the following variables into every hook process:

- `FUIGO_HOOK_EVENT`: the event name (e.g. `pre_tool_use`, `session_start`, `post_tool_use`).
- `FUIGO_HOOK_NAME`: the full configured name of this hook.
- `FUIGO_SESSION_ID`: the current session identifier.
- `FUIGO_WORKSPACE_ROOT`: absolute path to the workspace root.

For hooks provided by plugins, the following are also set:

- `FUIGO_PLUGIN_ROOT`: absolute path to the plugin's installation directory.
- `FUIGO_PLUGIN_DATA`: absolute path to the plugin's writable data directory.

These runner- and plugin-injected variables always take precedence. Attempts to override the reserved runner keys via the `env` field are stripped at load time (with a warning logged). For plugin hooks, `FUIGO_PLUGIN_ROOT` and `FUIGO_PLUGIN_DATA` similarly override any user-supplied values for those keys.
...
`FUIGO_PLUGIN_ROOT` and `FUIGO_PLUGIN_DATA`. These keys override any user-declared
...
{
  "type": "command",
  "command": "${HOME}/.config/fuigo-hooks/check.sh"
}
```

Lookup order for each reference:
1. The handler's own `env` map.
2. The current process environment (the env Fuigo itself sees).
...
`FUIGO_WORKSPACE_ROOT`, `FUIGO_HOOK_EVENT`, `FUIGO_HOOK_NAME`,
`FUIGO_SESSION_ID`) are not taken from the Fuigo process environment at
...
`${FUIGO_PLUGIN_ROOT}/check` resolve against the plugin's actual path.
...
Hooks from `~/.fuigo/hooks/` appear under **Global**, project ones under **Project**, etc.
...
- Global hooks (`~/.fuigo/...`) run with your user permissions. Treat them like shell scripts.
...
- **`The argument '/.claude/hooks/….ps1' to the -File parameter does not exist`?** PowerShell treated `$CLAUDE_PROJECT_DIR` as empty. Fuigo rewrites it to `$env:CLAUDE_PROJECT_DIR` unless `FUIGO_SHELL=cmd`.
- **See errors?** Check the pager logs (usually in the tracing pane or `~/.fuigo/logs`).

## More Examples

See the built-in examples in the `fuigo-hooks` crate:

- [Safe Shell Guard](../../../fuigo-hooks/examples/hooks/safe-shell.json)
- [No Recursive Grep](../../../fuigo-hooks/examples/hooks/no-recursive-grep.json): hard-blocks `grep -r`/`grep -R`/`rgrep` (OOM guard)
- [Session Audit Log](../../../fuigo-hooks/examples/hooks/session-log.json)
- [Tool Activity Logger](../../../fuigo-hooks/examples/hooks/tool-logger.json)

Copy them to `~/.fuigo/hooks/` and customize.
