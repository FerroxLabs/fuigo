...
Fuigo asks before doing anything risky — and can plan before it codes.

## Permissions

When Fuigo wants to run a risky command or edit a file, it pauses and asks:
...
For bigger or more ambiguous tasks, use **plan mode**: Fuigo explores the
...
`s` requests changes — Fuigo iterates until you're happy, then implements.
...
background — Fuigo keeps working and you're notified when it finishes
