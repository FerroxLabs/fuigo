...
This file ships with the CLI and is extracted to `~/.fuigo/docs/user-guide/26-config-reference.md` on launch. It is the complete field list for `config.toml`, `managed_config.toml`, and `requirements.toml`. For conceptual guidance see [05-configuration.md](05-configuration.md).

## How to configure

Three files configure Fuigo, and they are written by different people.

| File | Who writes it | Where it lives | Use it to |
| --- | --- | --- | --- |
| `config.toml` | The developer | `~/.fuigo/config.toml`, and `.fuigo/config.toml` in a project | Set personal defaults. Anything here can be changed by the person using the machine. |
| `managed_config.toml` | You, through the console or a deployment tool | `/etc/fuigo/managed_config.toml` | Ship a starting point to a fleet. A developer's own file overrides it. |
| `requirements.toml` | You, signed | `/etc/fuigo/requirements.toml`, or macOS device management | Set values a developer cannot change. Keys marked `pin` below hold against every other file, the environment, and the command line. |
...
Fuigo also reads these layers, later rows winning except where a requirements pin or the Managed column says otherwise.

1. Compiled defaults.
2. `/etc/fuigo/managed_config.toml`, then `$FUIGO_HOME/managed_config.toml` (fleet defaults; console-synced).
3. `$FUIGO_HOME/config.toml` (your settings; `/settings` writes here). Default `$FUIGO_HOME` is `~/.fuigo`.
4. Project `.fuigo/config.toml`: only `[mcp_servers]`, `[plugins]`, `[permission]`, and `[mcp] max_output_bytes`.
5. `FUIGO_CONFIG` (inline JSON) or `FUIGO_CONFIG_PATH` (JSON or TOML file). Allowlisted keys only.
6. `$FUIGO_HOME/requirements.toml`, then `/etc/fuigo/requirements.toml`, then macOS MDM `ai.x.grok`. Admin layer. Keys marked `pin` in the table cannot be overridden; keys marked `yes` are also valid in this file.
7. `FUIGO_*` environment variables.
8. CLI flags such as `--model`, `--sandbox`, `--yolo`.

Run `fuigo inspect` or `fuigo inspect --json` to see which files and values won.

## config.toml

User-level configuration lives in `$FUIGO_HOME/config.toml` (default `~/.fuigo/config.toml`; Windows `%USERPROFILE%\.fuigo\config.toml`). Project-scoped overrides live in `.fuigo/config.toml` and only contribute `[mcp_servers]`, `[plugins]`, `[permission]`, and `[mcp] max_output_bytes`.
...
| `agent.name` | `string` | `yes` | `user` | Built-in or discovered agent definition name. Also FUIGO_AGENT and `--agent-profile`. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `auth` | `table` | `yes` | `user` | Alias of `[fuigo_com_config]`; every `fuigo_com_config.*` key also works as `auth.*`. |
| `auth.auth_provider_command` | `string` | `yes` | `user` | External auth binary; stdout is the token. Also FUIGO_AUTH_PROVIDER_COMMAND; also valid as `fuigo_com_config.auth_provider_command`. |
| `auth.auth_provider_label` | `string` | `yes` | `user` | Login button label for an external auth provider. Also FUIGO_AUTH_PROVIDER_LABEL; also valid as `fuigo_com_config.auth_provider_label`. |
...
| `auth.oauth2.issuer` | `string` | `yes` | `user` | OAuth2 issuer URL. Also FUIGO_OAUTH2_ISSUER; also valid as `fuigo_com_config.oauth2.issuer`. |
| `auth.oauth2.principal_id` | `string` | `yes` | `user` | Required principal id when `principal_type` is set. Also FUIGO_OAUTH2_PRINCIPAL_ID; also valid as `fuigo_com_config.oauth2.principal_id`. |
| `auth.oauth2.principal_type` | `string` | `yes` | `user` | Token principal type, such as Team. Also FUIGO_OAUTH2_PRINCIPAL_TYPE; also valid as `fuigo_com_config.oauth2.principal_type`. |
| `auth.oauth2.referrer` | `string` | `yes` | `user` | Referrer for OAuth usage attribution. Also FUIGO_OAUTH2_REFERRER; also valid as `fuigo_com_config.oauth2.referrer`. |
| `auth.oauth2.scopes` | `string[]` | `yes` | `user` | OAuth2 scopes. Also FUIGO_OAUTH2_SCOPES; also valid as `fuigo_com_config.oauth2.scopes`. |
| `auth.oidc` | `table` | `yes` | `user` | Customer OIDC identity-provider settings; also valid as `fuigo_com_config.oidc`. |
| `auth.oidc.audience` | `string` | `yes` | `user` | Optional OIDC audience. Also FUIGO_OIDC_AUDIENCE; also valid as `fuigo_com_config.oidc.audience`. |
| `auth.oidc.client_id` | `string` | `yes` | `user` | OIDC client id. Also FUIGO_OIDC_CLIENT_ID; also valid as `fuigo_com_config.oidc.client_id`. |
| `auth.oidc.issuer` | `string` | `yes` | `user` | OIDC issuer URL. Also FUIGO_OIDC_ISSUER; also valid as `fuigo_com_config.oidc.issuer`. |
| `auth.oidc.scopes` | `string[]` | `yes` | `user` | OIDC scopes. Also FUIGO_OIDC_SCOPES; also valid as `fuigo_com_config.oidc.scopes`. |
| `auth.preferred_method` | `api_key / oidc` | `yes` | `user` | Pin automatic auth to one method with no fallthrough; also valid as `fuigo_com_config.preferred_method`. |
| `auth.token_header` | `string` | `yes` | `user` | Header name that carries the CLI auth token; default `xai-grok-cli`; also valid as `fuigo_com_config.token_header`. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `cli.auto_update` | `boolean` | `pin` | `user` | Check for CLI updates on launch. Also FUIGO_DISABLE_AUTOUPDATER to suppress. |
...
| `cli.maximum_version` | `string` | `pin` | `user` | Highest CLI version that still runs without a hard block. Also FUIGO_MAXIMUM_VERSION. |
| `cli.minimum_version` | `string` | `pin` | `user` | Lowest CLI version that still runs without a hard block. Also FUIGO_MINIMUM_VERSION. |
| `cli.npm_registry` | `string` | `yes` | `user` | npm registry used by the auto-updater. |
| `cli.required_maximum_version` | `string` | `pin` | `user` | Hard maximum CLI version. Also FUIGO_REQUIRED_MAXIMUM_VERSION. |
| `cli.required_minimum_version` | `string` | `pin` | `user` | Hard minimum CLI version. Also FUIGO_REQUIRED_MINIMUM_VERSION. |
...
| `compat.claude.rules` | `boolean` | `yes` | `user` | Scan Claude rules. Also FUIGO_CLAUDE_RULES_ENABLED. |
| `compat.claude.skills` | `boolean` | `yes` | `user` | Scan Claude skills. Also FUIGO_CLAUDE_SKILLS_ENABLED. |
...
| `compat.cursor.agents` | `boolean` | `yes` | `user` | Scan agent definitions from Cursor compat sources. Also FUIGO_CURSOR_AGENTS_ENABLED. |
| `compat.cursor.hooks` | `boolean` | `yes` | `user` | Scan Cursor hooks. Also FUIGO_CURSOR_HOOKS_ENABLED. |
| `compat.cursor.mcps` | `boolean` | `yes` | `user` | Scan Cursor mcp.json. Also FUIGO_CURSOR_MCPS_ENABLED. |
| `compat.cursor.rules` | `boolean` | `yes` | `user` | Scan `.cursor/rules/`. Also FUIGO_CURSOR_RULES_ENABLED. |
| `compat.cursor.skills` | `boolean` | `yes` | `user` | Scan Cursor skills directories. Also FUIGO_CURSOR_SKILLS_ENABLED. |
...
| `endpoints.deployment_key` | `string` | `pin` | `user` | Management key for enterprise deployments. Also FUIGO_DEPLOYMENT_KEY. |
| `endpoints.feedback_base_url` | `string` | `yes` | `user` | Where feedback submissions go. Also FUIGO_FEEDBACK_BASE_URL. |
| `endpoints.managed_config_url` | `string` | `yes` | `user` | Override managed config endpoint. Also FUIGO_MANAGED_CONFIG_URL. |
| `endpoints.models_base_url` | `string` | `pin` | `user` | Custom inference base URL. Also FUIGO_MODELS_BASE_URL. |
| `endpoints.models_list_url` | `string` | `pin` | `user` | Override model-list URL. Also FUIGO_MODELS_LIST_URL. Alias `models_endpoint`. |
| `endpoints.trace_upload_bucket` | `string` | `yes` | `user` | Direct gs:// or s3:// bucket for traces; bypasses the proxy. Also FUIGO_TRACE_UPLOAD_BUCKET. |
...
| `endpoints.trace_upload_credentials_file` | `string (path)` | `yes` | `user` | Path to a GCS service-account JSON or AWS credentials file for that bucket. Also FUIGO_TRACE_UPLOAD_CREDENTIALS_FILE. |
| `endpoints.trace_upload_endpoint_url` | `string` | `yes` | `user` | Custom S3-compatible endpoint for s3:// bucket uploads. Also FUIGO_TRACE_UPLOAD_ENDPOINT_URL. |
| `endpoints.trace_upload_region` | `string` | `yes` | `user` | AWS region for s3:// bucket uploads; default us-east-1. Also FUIGO_TRACE_UPLOAD_REGION. |
| `endpoints.trace_upload_url` | `string` | `pin` | `user` | Proxy destination for traces when no direct bucket is set. Also FUIGO_TRACE_UPLOAD_URL. |
| `endpoints.fuigo_api_base_url` | `string` | `pin` | `user` | Public Ferrox Labs API base. Also FUIGO_API_BASE_URL. |

### `features`

| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `features.active_agent_messages` | `boolean` | `pin` | `user` | Enable or disable `active_agent_messages`. Default false. Also `FUIGO_ACTIVE_AGENT_MESSAGES`. |
| `features.ask_user_question` | `boolean` | `pin` | `user` | Enable or disable `ask_user_question`. Default true. Also `FUIGO_ASK_USER_QUESTION`. |
| `features.auto_wake` | `boolean` | `pin` | `user` | Enable or disable `auto_wake`. Default true. Also `FUIGO_AUTO_WAKE`. |
| `features.backend_tools` | `boolean` | `pin` | `user` | Enable or disable `backend_tools`. Default true. Also `FUIGO_BACKEND_SEARCH`. |
| `features.campaigns` | `boolean` | `yes` | `user` | Enable remote campaign patches. `FUIGO_CAMPAIGNS=0` still disables even when requirements set this true. |
| `features.cancel_rewind` | `boolean` | `pin` | `user` | Enable or disable `cancel_rewind`. Default true. Also `FUIGO_CANCEL_REWIND`. |
...
| `features.compaction_detail` | `none / minimal / balanced / verbose` | `yes` | `user` | Verbatim detail level for `segments` compaction. Also FUIGO_COMPACTION_DETAIL. |
| `features.compaction_mode` | `summary / transcript / segments` | `yes` | `user` | Compaction strategy. Also FUIGO_COMPACTION_MODE. |
| `features.compaction_tool_choice` | `string` | `yes` | `user` | Tool-choice hint used during compaction. |
| `features.compaction_verbatim_input` | `boolean` | `pin` | `user` | Enable or disable `compaction_verbatim_input`. Default true. Also `FUIGO_COMPACTION_VERBATIM_INPUT`. |
| `features.feedback` | `boolean` | `pin` | `user` | Enable or disable `feedback`. Default true. Also `FUIGO_FEEDBACK_ENABLED`. |
| `features.feedback_trace_card` | `boolean` | `pin` | `user` | Show a trace-upload consent question after `/feedback`. Default false. Also `FUIGO_FEEDBACK_TRACE_CARD`. |
...
| `features.lsp_tools` | `boolean` | `pin` | `user` | Enable or disable `lsp_tools`. Default false. Also `FUIGO_LSP_TOOLS`. |
| `features.managed_config` | `boolean` | `yes` | `user` | Fetch managed_config.toml and requirements.toml from the deployment. |
| `features.mcp_auto_restart` | `boolean` | `yes` | `user` | Auto-restart stdio MCP servers after transport failure. Also FUIGO_MCP_AUTO_RESTART. |
...
| `features.mcp_push_server_status` | `boolean` | `yes` | `user` | Pager subscribes to MCP server_status push. Process env FUIGO_MCP_PUSH_SERVER_STATUS wins at launch. |
| `features.mcp_recursive_config_watch` | `boolean` | `yes` | `user` | Watch `<cwd>/` and `<cwd>/.fuigo/` for project MCP config edits. Name is a misnomer; watches are non-recursive. |
| `features.non_git_warning` | `boolean` | `yes` | `user` | Show a blocking warning when Fuigo starts outside a Git repository. |
...
| `features.repo_status_in_system_prompt` | `boolean` | `pin` | `user` | Enable or disable `repo_status_in_system_prompt`. Default true. Also `FUIGO_REPO_STATUS_IN_SYSTEM_PROMPT`. |
| `features.session_recap` | `boolean` | `pin` | `user` | Enable or disable `session_recap`. Default true. Also `FUIGO_SESSION_RECAP`. |
| `features.session_search` | `boolean` | `pin` | `user` | Enable or disable `session_search`. Default true. Also `FUIGO_SESSION_SEARCH`. |
...
| `features.voice_mode` | `boolean` | `pin` | `user` | Enable or disable `voice_mode`. Default true. Also `FUIGO_VOICE_MODE`. |
| `features.web_fetch` | `boolean` | `pin` | `user` | Enable or disable `web_fetch`. Default false. Also `FUIGO_WEB_FETCH`. |
| `features.write_file` | `boolean` | `pin` | `user` | Enable or disable `write_file`. Default true. Also `FUIGO_WRITE_FILE`. |
| `features.zdr_access_enabled` | `boolean` | `pin` | `user` | Advertise ZDR-incompatible tools when the team is on Zero Data Retention. Also `FUIGO_ZDR_ACCESS_ENABLED`. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `fuigo_com_config` | `table` | `yes` | `user` | Fuigo.com websocket and OAuth/OIDC settings. `[auth]` is an alias. |
| `fuigo_com_config.auth_provider_command` | `string` | `yes` | `user` | External auth binary; stdout is the token. Also FUIGO_AUTH_PROVIDER_COMMAND. |
| `fuigo_com_config.auth_provider_label` | `string` | `yes` | `user` | Login button label for an external auth provider. Also FUIGO_AUTH_PROVIDER_LABEL. |
...
| `fuigo_com_config.oauth2.issuer` | `string` | `yes` | `user` | OAuth2 issuer URL. Also FUIGO_OAUTH2_ISSUER. |
| `fuigo_com_config.oauth2.principal_id` | `string` | `yes` | `user` | Required principal id when `principal_type` is set. Also FUIGO_OAUTH2_PRINCIPAL_ID. |
| `fuigo_com_config.oauth2.principal_type` | `string` | `yes` | `user` | Token principal type, such as Team. Also FUIGO_OAUTH2_PRINCIPAL_TYPE. |
| `fuigo_com_config.oauth2.referrer` | `string` | `yes` | `user` | Referrer for OAuth usage attribution. Also FUIGO_OAUTH2_REFERRER. |
| `fuigo_com_config.oauth2.scopes` | `string[]` | `yes` | `user` | OAuth2 scopes. Also FUIGO_OAUTH2_SCOPES. |
| `fuigo_com_config.oidc` | `table` | `yes` | `user` | Customer OIDC identity-provider settings. |
| `fuigo_com_config.oidc.audience` | `string` | `yes` | `user` | Optional OIDC audience. Also FUIGO_OIDC_AUDIENCE. |
| `fuigo_com_config.oidc.client_id` | `string` | `yes` | `user` | OIDC client id. Also FUIGO_OIDC_CLIENT_ID. |
| `fuigo_com_config.oidc.issuer` | `string` | `yes` | `user` | OIDC issuer URL. Also FUIGO_OIDC_ISSUER. |
| `fuigo_com_config.oidc.scopes` | `string[]` | `yes` | `user` | OIDC scopes. Also FUIGO_OIDC_SCOPES. |
| `fuigo_com_config.preferred_method` | `api_key / oidc` | `yes` | `user` | Pin automatic auth to one method with no fallthrough. |
| `fuigo_com_config.token_header` | `string` | `yes` | `user` | Header name that carries the CLI auth token; default `xai-grok-cli`. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `managed_mcps.enabled` | `boolean` | `pin` | `user` | Fetch managed MCP configs at startup. Also FUIGO_MANAGED_MCPS_ENABLED. |
| `managed_mcps.gateway_tools_enabled` | `boolean` | `yes` | `user` | Expose managed MCP gateway tools. Also FUIGO_MANAGED_MCP_GATEWAY_TOOLS_ENABLED. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `memory.enabled` | `boolean` | `pin` | `user` | Cross-session memory master switch. Also FUIGO_MEMORY. |
...
| `model.<id>.api_base_url` | `string` | `yes` | `user` | Alternate API base used with FUIGO_API_KEY resolution. |
...
| `model.<id>.max_retries` | `number` | `yes` | `user` | Inference retries for this model (attempts, counting the first). `FUIGO_MAX_RETRIES` overrides it. Empty responses are capped lower whatever this says: every empty reply of one request — reasoning-only (reasoning tokens, no text, no tool call) and fully empty alike — shares one budget of 3 attempts, so at most two resends, 2 s apart, however many retries of other errors came first; a lower value still wins, and the progress line's denominator drops with it. |
...
| `models.web_search` | `string` | `pin` | `user` | Model used by the client `web_search` tool. Also `FUIGO_WEB_SEARCH_MODEL`. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `sandbox.auto_allow_bash` | `boolean` | `pin` | `user` | Skip bash permission prompts when a sandbox profile is active. Also FUIGO_SANDBOX_AUTO_ALLOW_BASH. |
| `sandbox.profile` | `off / workspace / read-only / strict / string` | `pin` | `user` | Filesystem sandbox profile. Also `--sandbox` and FUIGO_SANDBOX. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `subagents.enabled` | `boolean` | `pin` | `user` | Subagent / task tool master switch. Also FUIGO_SUBAGENTS. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `telemetry.otel_enabled` | `boolean` | `yes` | `user` | External OTEL master switch. Also FUIGO_EXTERNAL_OTEL. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `tools.disable_zdr_incompatible_tools` | `boolean` | `yes` | `user` | Restrict tools that need Ferrox Labs-hosted output under ZDR. Also FUIGO_DISABLE_ZDR_INCOMPATIBLE_TOOLS. |
| `tools.media_gen.max_parallel_image_gen_calls` | `integer` | `yes` | `user` | Cap parallel image_gen/image_edit calls in one model step. Also FUIGO_MAX_PARALLEL_IMAGE_GEN_CALLS. |
| `tools.media_gen.max_parallel_video_gen_calls` | `integer` | `yes` | `user` | Cap parallel video_gen calls in one model step. Also FUIGO_MAX_PARALLEL_VIDEO_GEN_CALLS. |
| `tools.respect_gitignore` | `boolean` | `pin` | `user` | When true, search and read tools skip gitignored files. Also FUIGO_RESPECT_GITIGNORE. |
...
| `toolset.web_fetch.proxy_endpoint` | `string` | `yes` | `user` | Egress proxy URL for web_fetch. Also FUIGO_WEB_FETCH_PROXY. |
...
| `ui.collapsed_edit_blocks` | `boolean` | `yes` | `user` | Show edits as one-line +N/-M summaries. Also FUIGO_COLLAPSED_EDIT_BLOCKS. |
...
| `ui.default_selected_permission` | `string` | `yes` | `user` | Preselected approval row on the first prompt of a session. Also FUIGO_DEFAULT_SELECTED_PERMISSION. |
| `ui.display_refresh.auto_cadence_enabled` | `boolean` | `yes` | `user` | Match stream/scroll cadence to display refresh rate. Also FUIGO_DISPLAY_REFRESH_AUTO_CADENCE. |
...
| `ui.group_tool_verbs` | `boolean` | `yes` | `user` | Fold consecutive read/search/list tool rows. Also FUIGO_GROUP_TOOL_VERBS. |
| `ui.hunk_tracker_mode` | `agent_only / all_dirty / off` | `yes` | `user` | File-change hunk tracking. Also FUIGO_HUNK_TRACKER and `--hunk-tracker-mode`. |
| `ui.invert_scroll` | `boolean` | `yes` | `user` | Reverse vertical scroll direction. Also FUIGO_INVERT_SCROLL. |
...
| `ui.mouse_reporting_toggle` | `boolean` | `yes` | `user` | Ctrl+R in scrollback toggles terminal mouse capture. Also FUIGO_MOUSE_REPORTING_TOGGLE. |
...
| `ui.prompt_suggestions` | `boolean` | `yes` | `user` | Next-prompt ghost text after each turn. Also FUIGO_PROMPT_SUGGESTIONS; a remote kill-switch can disable it fleet-wide. |
...
| `ui.remember_tool_approvals` | `boolean` | `yes` | `user` | Show per-tool Always allow options. Also FUIGO_REMEMBER_TOOL_APPROVALS. |
| `ui.render_mermaid` | `auto / on / off` | `yes` | `user` | How mermaid fences render: clickable open row or raw source. |
| `ui.screen_mode` | `fullscreen / minimal` | `yes` | `user` | Default render mode for plain `fuigo`. Restart required. |
| `ui.scroll_lines` | `integer` | `yes` | `user` | Lines per scroll tick (1–10). Also FUIGO_SCROLL_LINES. |
| `ui.scroll_mode` | `auto / wheel / trackpad` | `yes` | `user` | Scroll input classification. Also FUIGO_SCROLL_MODE. |
| `ui.scroll_speed` | `integer` | `yes` | `user` | Mouse/trackpad scroll speed multiplier (1–100). Also FUIGO_SCROLL_SPEED. |
| `ui.show_thinking_blocks` | `boolean` | `yes` | `user` | Show thinking/reasoning blocks while streaming. Also FUIGO_SHOW_THINKING_BLOCKS. |
...
| `ui.theme` | `string` | `yes` | `user` | Color theme name, or `auto`/`system` to follow the OS. Also `/theme` and FUIGO_THEME. |
...
| Key | Type / Values | Requirements | Managed | Details |
| --- | --- | --- | --- | --- |
| `voice.api_base` | `string` | `yes` | `user` | HTTPS API root for speech-to-text. Unset inherits `[endpoints].fuigo_api_base_url`. |
...
Fuigo reads `/etc/fuigo/managed_config.toml` first, then `$FUIGO_HOME/managed_config.toml`, which the console keeps in sync. Values in the second replace values in the first.
...
`requirements.toml` is an admin-enforced file. Locations: `$FUIGO_HOME/requirements.toml` (signed cache) then `/etc/fuigo/requirements.toml`, then macOS MDM `ai.x.grok`. The **Requirements** column on the `config.toml` tables lists every `config.toml` key this file accepts (`pin` or `yes`). Omitted keys stay unconstrained.
...
| --- | --- |
| A developer sets a key you pinned | The pinned value applies. `fuigo inspect` lists the requirements file that contributed. |
...
Run `fuigo inspect` on the developer's machine. It lists every config file that contributed, including requirements and managed layers, so a policy that is not applying is visible in one command.
