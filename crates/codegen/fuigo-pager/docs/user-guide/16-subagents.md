...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.fuigo/agents/` or `~/.fuigo/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.fuigo/personas/`; applied during subagent resolution |
...
| **Examples** | `fuigo-build`, `explore`, `plan` | `researcher`, `concise` |
...
Fuigo discovers file-based personas from these locations, in priority order:

- `.fuigo/personas/*.toml` (project)
- `~/.fuigo/personas/*.toml` (user)
...
> **Note:** Fuigo applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Fuigo resolves the effective model and reasoning effort in this order, highest priority first:
...
# instructions_file = ".fuigo/personas/concise.md"  # or load from a file
```

Fuigo also discovers roles from `.fuigo/roles/*.toml` and personas from `.fuigo/personas/*.toml`. Inline `config.toml` definitions take precedence over files.

---

## The Tasks Pane (TUI)

Fuigo shows running and finished work in side panes on the agent screen:
