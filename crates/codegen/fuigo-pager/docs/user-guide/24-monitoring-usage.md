...
> **Status: alpha.** The schema below is versioned (`fuigo_code.schema.version = v1`);
> additive changes may occur without notice, renames/removals will bump the
> version and be called out in the changelog.

Fuigo CLI can export usage **metrics** and **events** to your organization's
...
any data flowing through Ferrox Labs.
...
|---------|---------------|
| Telemetry master switch | `[features] telemetry` / `FUIGO_TELEMETRY_ENABLED` |
| Coding data, retention, and training | Settings — `/privacy` opens the row |
| Trace upload | `[telemetry] trace_upload` / `FUIGO_TELEMETRY_TRACE_UPLOAD` |
| External OpenTelemetry | `FUIGO_EXTERNAL_OTEL` / `[telemetry] otel_*` (this guide) |
...
- **Structurally separate** from Ferrox Labs-internal telemetry: its exporters carry
  only the headers you configure, never Ferrox Labs credentials.
- **Independent of Ferrox Labs data-retention opt-outs**: it works even when
  `telemetry` is disabled and for ZDR (zero-data-retention) teams. Those
  settings govern Ferrox Labs-side retention; the external stream is governed solely
  by your own OTEL configuration.

## Quick start

```bash
export FUIGO_EXTERNAL_OTEL=1                  # master switch
...
`FUIGO_EXTERNAL_OTEL=1` alone enables **nothing** — you must also select at
...
|---|---|---|
| `FUIGO_EXTERNAL_OTEL` | `0` | Master switch. Distinct from `FUIGO_TELEMETRY_ENABLED`, which controls Ferrox Labs-internal product analytics — the two govern opposite-pointing data flows. |
...
| `OTEL_LOG_USER_PROMPTS` | `0` | Content gate: prompt text on `fuigo_code.user_prompt` (60 KB cap, secret-scrubbed). |
...
> `FUIGO_EXTERNAL_OTEL` is set, product analytics ignores those vars, and the
...
standard OTEL names** (`FUIGO_EXTERNAL_OTEL`, `OTEL_*`) for ecosystem
...
the `[telemetry]` `otel_*` keys through `fuigo setup` managed config /
...
Because Ferrox Labs can force-disable this stream fleet-wide, the CLI holds emission
...
**The wait is bounded**, so a deployment that cannot reach Ferrox Labs still exports:

- If no fleet policy can apply at all — `[features] remote_fetch = false`, or
  `[endpoints] cli_chat_proxy_base_url` points somewhere other than Ferrox Labs — the
...
(`fuigo --debug`) for `external otel:` lines — they record whether the stream
...
## Metrics (meter scope `ai.fuigo.code`)

| Metric | Unit | Attributes |
|---|---|---|
| `fuigo_code.session.count` | `{session}` | base attrs only |
| `fuigo_code.token.usage` | `{token}` | `type` = `input` \| `output` \| `reasoning` \| `cache_read`; `model` |
| `fuigo_code.turn.count` | `{turn}` | `outcome` = `completed` \| `cancelled` \| `error`; `model` |
| `fuigo_code.tool.decision` | `{decision}` | `tool_name`, `decision` = `allow` \| `deny` \| `cancelled` \| `followup`, `access_kind`, `permission_mode` |
| `fuigo_code.tool.usage` | `{call}` | `tool_name`, `outcome` |
| `fuigo_code.error.count` | `{error}` | `error_category`, `model` |
| `fuigo_code.startup.total` | `ms` | `outcome` = `ok` \| `timeout` \| `error`; `auth_mode` |
| `fuigo_code.startup.phase_duration` | `ms` | `phase`, `outcome`, `auth_mode` |
| `fuigo_code.startup.timeout` | `{timeout}` | `stuck_in`, `auth_mode` |
...
message Fuigo prints names the longest step instead, so the two can name
...
There is no `cost.usage` metric: join `fuigo_code.token.usage` with your own
...
|---|---|
| `fuigo_code.session_start` | `model`, `permission_mode`, `mcp_server_count`, `plugin_count`, `skill_count`, `hook_count`, `memory_enabled`, `is_git_repo`, `client_identifier` |
| `fuigo_code.session_end` | `duration_secs`, `turn_count`, `tool_call_count`, `compaction_count`, `model` |
| `fuigo_code.user_prompt` | `prompt_length`, `model`, `screen_mode?` (`fullscreen` \| `inline` \| `minimal` \| `headless` \| `other`); `prompt` (**prompts**) |
| `fuigo_code.turn_completed` | `outcome`, `duration_ms`, `tool_call_count`, `model`, `error_category?`, `cancellation_category?` |
| `fuigo_code.api_request` | `model`, `duration_ms`, `stop_reason?`, `input_tokens`, `output_tokens`, `reasoning_tokens`, `cache_read_tokens` |
| `fuigo_code.api_error` | `error_category`, `model`, `status_code?`, `duration_ms?` |
| `fuigo_code.tool_result` | `tool_name`, `outcome`, `success`, `duration_ms`, `file_extension`; `tool_parameters`, `file_path` (**details**) |
| `fuigo_code.tool_decision` | `tool_name`, `decision`, `access_kind`, `permission_mode`, `source` |
| `fuigo_code.mcp_server_connection` | `status`, `transport_type`, `duration_ms`, `tool_count?`, `error_type?`; `mcp_server.name` (**details**; collapsed to `mcp_server` otherwise) |
| `fuigo_code.permission_mode_changed` | `to_mode`, `trigger` |
| `fuigo_code.skill_activated` | `skill_source`, `trigger` = `slash_command` \| `skill_md_read` \| `skill_tool`; `skill.name` (**details**) |
| `fuigo_code.plugin_loaded` | `install_kind?`, `success`, `error_category?`; `plugin_name` (**details**) |
| `fuigo_code.compaction` | `duration_ms`, `tokens_before`, `tokens_after`, `model?` |
| `fuigo_code.subagent` | `phase` = `launched` \| `completed`, `subagent_type?`, `outcome?`, `duration_ms?` |
| `fuigo_code.auth` | `auth_method` |
| `fuigo_code.internal_error` | `error_type` (class only — no message, no location) |
| `fuigo_code.model_switched` | `from_model`, `to_model`, `success`, `error_code?` |
...
sum by (model, type) (rate(fuigo_code_token_usage_total[1h]))

# Sessions per team per day
sum by (team_id) (increase(fuigo_code_session_count_total[1d]))

# Tool-permission denial ratio
sum(rate(fuigo_code_tool_decision_total{decision="deny"}[1h]))
  / sum(rate(fuigo_code_tool_decision_total[1h]))
```
