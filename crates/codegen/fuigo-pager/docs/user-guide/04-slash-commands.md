...
Not `/config-agents` (alias `/agents`), which manages agent *definitions* and personas. Hidden in minimal mode; disable with `FUIGO_AGENT_DASHBOARD=0` or `[dashboard].enabled = false`.

### `/compact [context]`

Compress conversation history to reclaim context-window space. Pass a note to tell Fuigo what to keep:
...
Fuigo also auto-compacts once the context window hits 85% (tune it with `[session] auto_compact_threshold_percent`).
...
Every copy is also written to a backup file — `~/.fuigo/last-copy.txt` by default, or `FUIGO_COPY_FILE` if set. Confirmed copies toast briefly (e.g. `Copied!`). Unverified OSC 52 deliveries and clipboard-unreachable fallbacks name the backup path so you can recover the text.
...
`/flush`, `/dream`, and `/memory` require memory enabled through `FUIGO_MEMORY=1`, `[memory] enabled = true`, or managed remote settings; `/memory` also needs a configured memory backend. `/remember` is always available.
...
Run a prompt on a recurring interval. Give the interval as `30m`, `1 hour`, or `every 2 days`; leave it out and Fuigo will ask.
...
Set, manage, or check an autonomous goal. Fuigo works across rounds and only marks the goal complete after an independent evidence review confirms the claim; if that review can't reproduce the result or has no usable evidence, the goal stays active or pauses with concrete gaps.
...
Project workflows live in `.fuigo/workflows/*.rhai`; user workflows live in `~/.fuigo/workflows/*.rhai`. A same-process pause/resume continues the original immutable script, args, and `agent_budget` cap from committed host-call results — to iterate, edit the returned script copy and launch it as a new run.
...
Open the extensions modal on the **Workflows** tab — a browse-only catalog of the saved workflows Fuigo discovered (built-ins, project `.fuigo/workflows/`, and user `~/.fuigo/workflows/`), with each entry's source, description, and path. The same catalog is listed for the model under the skill listing in the session preamble. Launch one with `/workflow <name>` (or its own slash command), then watch it in `/workflow runs`.
...
Browse the built-in How-to Guides, open the online FluxRouter docs, or jump straight to a guide by title. Aliases: `/howto`, `/guides`.
...
- `/docs web` opens https://fluxrouter.ai/docs in your browser.
...
For persisted per-turn token and cost totals of any local session, use `fuigo usage <session-id> [turn]` from the shell. See [Session Management](17-sessions.md#the-fuigo-usage-subcommand).
...
Any enabled skill with `user-invocable: true` in its SKILL.md frontmatter shows up as a slash command. (Turn a skill off via `/skills` and it stops being advertised.) So a skill at `~/.fuigo/skills/commit/SKILL.md` runs as:
