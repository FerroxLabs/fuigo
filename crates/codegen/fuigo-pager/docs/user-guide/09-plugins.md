...
A plugin bundles skills, slash commands, agents, hooks, and MCP servers into one installable unit. You get plugins from a marketplace, install the ones you want, and Fuigo loads what they add. To build and share your own, see [Create your own marketplace](#create-your-own-marketplace).
...
1. **Add the marketplace** so Fuigo can show what it offers. Nothing installs yet.
...
```bash
fuigo plugin marketplace add my-org/team-plugins                  # GitHub shorthand (owner/repo)
fuigo plugin marketplace add https://gitlab.com/acme/plugins.git  # any git host, include https:// and .git
fuigo plugin marketplace add ./my-marketplace                     # a local folder
```

List, refresh, and remove sources with `fuigo plugin marketplace list`, `fuigo plugin marketplace update [<name>]`, and `fuigo plugin marketplace remove <url>`.
...
Place this file at `~/.fuigo/settings.json` or `~/.claude/settings.json`.
...
```bash
fuigo plugin install deploy-tools --trust
```
...
Run `fuigo plugin install <source>` without `--trust` and Fuigo shows the source, warns that installing activates the plugin's hooks, MCP servers, and skills, then stops. Add `--trust` to go ahead. Only install plugins from sources you trust (see [Trust and security](#trust-and-security)).

A plugin's skills appear in the slash menu. When a skill name is ambiguous, Fuigo shows the qualified form prefixed by the plugin name, for example `/deploy-tools:release`. To pick up a newly installed plugin, press `r` in the Plugins tab or start a new session.
...
```bash
fuigo plugin list [--json] [--available]   # installed plugins (--available requires --json)
fuigo plugin uninstall <name> [--confirm] [--keep-data]   # aliases: rm, remove
fuigo plugin update [<name>]               # omit the name to update every plugin
fuigo plugin enable <name>
fuigo plugin disable <name>
fuigo plugin details <name>                # show the plugin's component inventory
```
...
In the **Workflows** tab (open it directly with `/workflows`, or `Tab` from the commands above), browse the saved workflows Fuigo discovered: built-ins, project `.fuigo/workflows/`, and user `~/.fuigo/workflows/`. Each row shows the workflow's name, source, and description; press `Enter` to expand its path and when-to-use notes, `r` to reload the list, and `/` to search. Rows are browse-only — run one with `/workflow <name>` or its own slash command.

### Turn plugins on or off in config

Set these in `~/.fuigo/config.toml`:
...
Plugins are off by default, so list one in `enabled` to turn it on, or in `disabled` to discover it but skip loading it. Each entry is a plain plugin name (from `fuigo plugin list`) or a full ID (`<scope>/<hash>/<name>`).

To hide the plugins and hooks interface entirely, set `disable_plugins = true` in `~/.fuigo/pager.toml`.
...
3. **List the plugins in `.fuigo-plugin/marketplace.json`.** This is the index Fuigo reads.
...
my-org-plugins/
  .fuigo-plugin/
    marketplace.json      # the index Fuigo reads (required)
...
Fuigo reads the index from `.fuigo-plugin/marketplace.json`. It also accepts `.fuigo-plugin/plugin.json` and the `.claude-plugin/` equivalents.
...
Validate a plugin before publishing with `fuigo plugin validate [<path>]`, and tag a release from the manifest version with `fuigo plugin tag [<path>] [--push]`. Then point people at the repository. They add it once and install the plugins they want:

```bash
fuigo plugin marketplace add my-org/my-org-plugins   # GitHub shorthand, a git URL, or a local path
fuigo plugin install gdrive --trust
```
...
# Plugins stay off until enabled. List plugin names (from `fuigo plugin list`)
# or full IDs (`<scope>/<hash>/<name>`).
[plugins]
enabled = ["gdrive"]
```

For a hands-off install with no per-person step, also place the plugin's files where Fuigo discovers and trusts them automatically: `~/.fuigo/plugins/`, or a directory your device-management tool manages that you point to with `[plugins].paths`. Then enable them with `[plugins].enabled`.
...
Marketplaces distribute Fuigo content: skills, commands, agents, hooks, and MCP server configurations. They do not install a program onto a machine. A skill or MCP server that runs a helper binary (for example a custom sign-in tool) still needs that binary delivered separately, bundled with your deployment or pushed through your device-management tool.

---

## Troubleshooting

**A plugin you installed isn't showing up.** Plugins are off until enabled. Check `fuigo plugin list`, then add the plugin's name or ID to `[plugins].enabled`, or press `Space` on it in the Plugins tab. Reload with `r` in the Plugins tab or start a new session.

**A plugin's hooks or MCP servers don't run.** They stay inactive until the plugin is trusted. Reinstall with `--trust`, or place the plugin under `~/.fuigo/plugins/` (auto-trusted). See [Trust and security](#trust-and-security).

**A skill or MCP server from a marketplace is missing.** Refresh the source with `fuigo plugin marketplace update`, confirm the plugin is installed and enabled, and, if your organization restricts sources, check that the marketplace is still allowed (see [Distribute across an organization](#distribute-across-an-organization)). Some MCP servers require a sign-in and will not appear until you authenticate.
...
**See exactly what loaded.** Run `fuigo inspect` (add `--json` for machine-readable output) to list every discovered plugin and the skills, agents, hooks, and MCP servers it provides, each labeled with its `plugin: <name>` source.
...
An optional `plugin.json` manifest can override paths or add metadata; without one, Fuigo discovers components from these standard directories. For example, a `team-tools` plugin might bundle a deploy skill, a code-review agent, pre-commit hooks, and a Linear MCP server, installed together in one step.
...
| `FUIGO_PLUGIN_DATA` | Absolute path to the plugin's writable data directory, for state, caches, and logs. |

Fuigo sets these and overrides any same-named value in the hook's `env` map (the `CLAUDE_PLUGIN_ROOT` and `CLAUDE_PLUGIN_DATA` aliases are set too). See the [Hooks guide](10-hooks.md) for every variable passed to hooks.
