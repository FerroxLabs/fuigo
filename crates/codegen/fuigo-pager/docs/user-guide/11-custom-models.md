...
Fuigo connects to custom model endpoints for alternative providers, self-hosted models, and overriding built-in settings. This guide explains how to select models, configure endpoints, and integrate third-party providers.
...
Fuigo's model requests to Flux Router (`api.fluxrouter.ai`) carry the body field `"cache": {"no-cache": true, "no-store": true}`: every turn on all three wire formats, plus the side calls built on the same sampling client — titles, recaps and compaction. The web search tool posts through its own HTTP client rather than that one, and applies the same host rule itself, so its searches carry the field too. No other provider receives that field, because a strict provider rejects an unknown body field with a 400. Fuigo's remaining Flux Router calls are not model turns and do not carry it either: `/imagine` posts `/v1/images/generations` and `/v1/images/edits`, video generation posts `/v1/videos/generations`, and batch speech-to-text posts `/v1/audio/transcriptions` as multipart form data, which has no JSON body to put a field in. Flux Router honours it on `/v1/chat/completions`, where it is what stops a retried turn being answered with a stored copy of an earlier reply. Its `/v1/responses` surface and both of its Anthropic Messages mounts — the bare `/v1/messages`, which is the path the default base URL reaches with `api_backend = "messages"`, and the prefixed `/anthropic/v1/messages` — rebuild each upstream request from a fixed field list, so the field never reaches the cache and they drop it. What protects a retry on those surfaces is the router's own cache instead: the `api.fluxrouter.ai` deployment released ahead of this version of Fuigo stops storing agent traffic at all. That is a property of that deployment rather than of the wire formats, so a self-hosted or older Flux Router may still replay a retry; Fuigo sends the field on every surface regardless, and the explicit opt-out takes effect the moment a surface honours it.
...
Set a persistent default in `~/.fuigo/config.toml`:
...
Fuigo supports three API backends. Set `api_backend` in your `[model.*]` config to choose which protocol the model uses:
...
Add custom model endpoints in `~/.fuigo/config.toml` under `[model.<name>]` sections:
...
env_key = "FUIGO_API_KEY"                   # Env var holding the API key (optional; string or array)
...
Fuigo resolves the API key in this order:
...
3. Your signed-in session token (from `fuigo login`), for a model with no `api_key`/`env_key` of its own
4. The `FUIGO_API_KEY` environment variable (global fallback; Fuigo also accepts `FUIGO_CODE_API_KEY` for backward compatibility)

### Context Window

The `context_window` value tells Fuigo when to trigger auto-compaction. When you override a known model, Fuigo inherits that model's context window. When you define a new model and omit `context_window`, Fuigo defaults to 200,000 tokens, so set it explicitly to match your provider.
...
Some gateways route or version on the query string. `query_params` appends percent-encoded query parameters to every request Fuigo makes for a model. For example, a gateway that selects an API version this way:
...
When you override a built-in model, Fuigo starts with the default configuration (including the correct `base_url`), then applies only the fields you specify. Unspecified fields inherit from the default.
...
Point Fuigo at a custom OpenAI-compatible `/v1/models` endpoint instead of the default. Use this when your models sit behind a corporate gateway or a self-hosted inference service.

### Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `FUIGO_MODELS_BASE_URL` | Yes | Base URL for inference. Fuigo fetches the model list from `{base_url}/models`. |
| `FUIGO_API_KEY` | Yes | API key sent as `Authorization: Bearer`. Fuigo also accepts `FUIGO_CODE_API_KEY`. |
| `FUIGO_MODELS_LIST_URL` | No | Override the model-list URL when it differs from `{base_url}/models`. |
...
When you use `[endpoints]` with partial model overrides, Fuigo inherits the `base_url` from the endpoints config, so you do not need to specify it in each `[model.*]` section.

### Auth Behavior

When you set `models_base_url`, Fuigo uses API key auth (`Authorization: Bearer`) instead of session auth. You do not need `fuigo login` -- the API key is enough.
...
```bash
export FUIGO_WEB_SEARCH_MODEL="grok-4.5"
```

If you point web search at a custom model, you also need a `[model.*]` entry so Fuigo can reach it. Server-side ("backend") web search runs only when the model sets `supports_backend_search = true` (and the build enables backend search); it does not depend on `api_backend`:
...
curl -s https://api.example.com/v1/models \
  -H "Authorization: Bearer $FUIGO_API_KEY"
```
