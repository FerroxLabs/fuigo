...
A skill is a directory that contains a `SKILL.md` file. Its markdown body tells Fuigo how to handle a specific type of task: step-by-step instructions, conventions, and tool-usage patterns.

Use a skill for a repeatable procedure that's too specific for AGENTS.md but too long to retype. Fuigo activates a skill only when it applies to your current task.
...
|----------|-------|----------|-------|
| `./.fuigo/skills/`, `./.fuigo/commands/` | Local (CWD) | Highest | Current directory skills / legacy command markdown |
| `<repo_root>/.fuigo/skills/`, `…/commands/` | Repo | Medium | Shared across the repo |
| `~/.fuigo/skills/`, `~/.fuigo/commands/` | User | Lowest | Personal skills for all projects |
...
Fuigo deduplicates skills by name -- a higher-priority location overrides a lower one. Fuigo also scans `.agents/skills/` (and `commands/`) at each tier (alongside `.fuigo/`) and walks every directory between your working directory and the repo root.
...
Add directories, exclude paths, or disable individual skills via `[skills]` in `~/.fuigo/config.toml`:
...
Each entry in `paths` is a `SKILL.md` file or a directory that Fuigo walks recursively. `ignore` hides a skill completely; `disabled` keeps it in the list but excludes it from the system prompt and from invocation. `paths` and `ignore` take filesystem paths and support `~` expansion; `disabled` takes skill names.
...
| Field | Description |
|-------|-------------|
| `name` | Skill identifier. Use lowercase letters, digits, and hyphens, up to 64 characters. Fuigo normalizes spaces and underscores to hyphens. If you omit `name`, Fuigo uses the skill's directory name. |
| `description` | What the skill does and when to use it. Fuigo reads this to decide whether to invoke the skill. If you omit it, Fuigo uses the first paragraph of the body. |

Write a specific `description`. It determines when Fuigo invokes the skill automatically. Name the trigger phrases and use cases.
...
| `metadata` | Arbitrary string key-value pairs. Fuigo promotes `metadata.author` and `metadata.short-description` for display. |

---

## Creating Skills with /create-skill

The `/create-skill` command walks you through building a new skill interactively. Fuigo asks what you want, drafts the files, and writes them to disk.

### How It Works

When you run `/create-skill`, Fuigo:

1. **Gathers requirements.** Fuigo asks for the skill name, the scope to save it under, and a description of the workflow you want to capture. Use a name with lowercase letters, digits, and hyphens (2–64 characters, starting and ending with a letter or digit).

2. **Drafts the description.** Fuigo writes a `description` that states what the skill does, the phrases that trigger it, and the slash command name. You approve or edit the draft before continuing.

3. **Creates the skill directory.** Fuigo creates the `<scope>/.fuigo/skills/<name>/` directory, plus `scripts/` or `references/` subdirectories when the skill needs them.

4. **Writes SKILL.md.** Fuigo writes the frontmatter (`name` and `description`) and a markdown body of instructions, along with any supporting files.

5. **Verifies and confirms.** Fuigo reads the file back, confirms it wrote correctly, and tells you how to run the skill.

### Choosing a Scope

Fuigo asks where to save the skill:

- **Project** (`<repo_root>/.fuigo/skills/<name>/`) -- available only in this repository and shareable with teammates through version control. Fuigo recommends this scope inside a git repository.
- **User** (`~/.fuigo/skills/<name>/`) -- available across all your projects.
...
The new skill appears in the slash menu within a few seconds, because Fuigo reloads skills when files change on disk.
...
To browse your skills, type `/` to open the slash-command menu. Fuigo lists every built-in command and skill and filters them as you type. To list skills from the command line instead, run `fuigo inspect` (see [Viewing Skill Details](#viewing-skill-details)).

### Qualified Names

When a skill's name collides with another skill or a built-in command, Fuigo keeps **both** invocable. The built-in keeps the bare name (`/login`, `/compact`, …). The skill is advertised under a qualified name prefixed by its scope -- `local:`, `repo:`, `user:`, or the plugin name:

```
/local:commit        # The "commit" skill from ./.fuigo/skills/
/user:commit         # The "commit" skill from ~/.fuigo/skills/
...
`fuigo inspect` tags colliding skills with `[collides with /login → /acme:login]`.

### Automatic Invocation

Fuigo can invoke a skill on its own when it recognizes a relevant task. Fuigo matches your prompt against the skill's `description` and `when-to-use` fields, so write both to describe the triggering situation.
...
Run `fuigo inspect` to see every skill Fuigo discovers, along with the rest of your configuration:

```bash
fuigo inspect          # Human-readable summary
fuigo inspect --json   # Machine-readable report
```

In the human-readable output, the Skills section lists each skill's name and its source -- `project`, `user`, `bundled`, `config` (a `[skills].paths` entry), `server` (skills synced from the skill store in managed workspaces), or `plugin: <name>`. Fuigo tags any skill disabled via `[skills].disabled` or from a disabled vendor surface with `[disabled]`.
...
Fuigo distributes platform skills separately from your personal skills. Bundled skills are cached under `~/.fuigo/bundled/skills/`; Fuigo never writes them into `~/.fuigo/skills/`. A same-named local, repo, or user skill overrides the bundled copy. `fuigo inspect` labels each definition by its actual source. (A plugin skill of the same name does not override a native skill; it stays available under its qualified `plugin:name` form.)

Skills can also come from plugins. When you install a plugin that includes skills, they appear alongside your user and project skills. `fuigo inspect` labels each plugin-provided skill with its source as `plugin: <name>`.
...
2. **Include concrete steps.** Skills work best when they give Fuigo a clear, ordered procedure to follow.
...
5. **Version-control project skills.** Commit `.fuigo/skills/` to your repository so the whole team benefits. User skills in `~/.fuigo/skills/` stay personal and unshared.
