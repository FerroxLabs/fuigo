...
Reference for key bindings in the Fuigo TUI. Bindings are built in and cannot currently be remapped.

---

## Input Modes

Fuigo has two input modes that control how you navigate the scrollback:
...
Simple mode is active by default. To switch to Vim mode, set `vim_mode = true` under `[ui]` in `~/.fuigo/config.toml`, or toggle it at runtime with `/vim-mode`. See [Configuration](05-configuration.md) for details.
...
> in `~/.fuigo/config.toml` (or `/vim-mode` to toggle). Arrow keys, `Tab`,
...
elicitation card** (`fuigo/mcp/elicit`), the **permission prompt**, and the
...
### MCP elicitation card (`fuigo/mcp/elicit`)
...
|--------|---------|--------|
| Turn running (every mode and pane) | `Esc` | Does **not** cancel. Shows a "Press Ctrl+c to cancel the turn" toast (a one-time scrollback line per turn in minimal mode); the draft is untouched. Use `Ctrl+C` (or palette / other cancel entry points). |
...
**Note:** External editing works in every render mode: minimal mode binds `Ctrl+G`, and the full TUI uses `/edit-prompt` or the command palette. Fuigo resolves `$VISUAL`, then `$EDITOR`, then `vi`. Values may include quoted arguments. Saving replaces only the draft (the final newline editors append on save is stripped); an empty file clears it. Drafts with pasted/file/image chips must be edited in the composer so attachments are not flattened.

**Note:** `Ctrl+'` is a Windows alt for `Ctrl+;` — some Windows consoles drop the `Ctrl` modifier on punctuation keys.

**Note:** `Ctrl+.` needs the Kitty keyboard protocol (or tmux `extended-keys on` so that protocol can pass through). On VS Code / Cursor / Windsurf / Zed integrated terminals, VTE, Apple Terminal, Windows Terminal, JetBrains, tmux with `extended-keys off`, screen, and similar no-KKP setups, Fuigo advertises **`Ctrl+X`** as the primary shortcuts-cheatsheet key instead. **`Ctrl+X` always works** as a classic control character even when `Ctrl+.` does not. Run `/doctor` if modified keys misbehave in tmux.
...
- An unmodified middle click in Fuigo reads **PRIMARY**, the current mouse selection, only when `DISPLAY` is non-empty. Pure X11 can use its native reader fallback; XWayland requires `xclip` or `xsel` on `PATH` so Fuigo reads the X11 selection rather than Wayland PRIMARY. The press is handled once; the release does not paste again.
...
Over SSH, the remote Fuigo process usually cannot access the terminal's local X11 selection. Use terminal-native `Shift+Insert` or `Shift+middle click` so the local terminal sends the selected text through the PTY.
...
**VS Code family terminal** (VS Code, Cursor, Windsurf, Zed integrated terminals): `Ctrl+Q` is captured by the host, so Fuigo makes **`Ctrl+D` the sole quit key** (`Ctrl+Q` is not bound). Half-page-down is rebound to bare **`Shift+D`**. Mid-turn interject uses **`Ctrl+L`** (no alternates) because `Ctrl+Enter` / `Ctrl+I` do not reliably reach the PTY; extensions are opened via `/plugins` instead of `Ctrl+L`.
...
> **Cmd+A is gated to Ghostty.** Fuigo's in-app `Cmd+A` handler is only
