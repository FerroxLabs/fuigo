...
Fuigo runs as a full-screen TUI. It relies on terminal support for color,
...
Run `/doctor` in Fuigo to check the current session and see available fixes. If
Fuigo cannot start, run `fuigo doctor` in your shell. Use `fuigo doctor --json`
...
`fuigo doctor --json` reports the same color capability when piped. Microphone
...
`/doctor fix tmux-clipboard` or `fuigo doctor fix dcs-passthrough --yes`.
...
to guess if that directory is unavailable or unsafe. Fuigo preserves the file's
...
Fuigo deliberately does **not** run `tmux source-file` or change the live tmux
...
Fuigo detects these terminal emulators from environment variables:
...
- Inside tmux, variables that identify the outer terminal may not reach Fuigo.
...
Inside tmux there are two separate questions: what color Fuigo emits, and what
...
Fuigo writes through up to three routes, shown in `/doctor` under **Clipboard**:

- **native** — the local operating-system clipboard.
- **tmux** — the tmux paste buffer when Fuigo runs inside tmux.
...
terminal focused. Older compositors may require Fuigo to remain focused until
the copy message appears. Fuigo shows a startup warning when this applies; run
`/doctor` for the detected status and steps.

`FUIGO_CLIPBOARD_NO_DATA_CONTROL=1` is an advanced fallback that disables the
data-control route. Copies then use command-line clipboard tools.

#### OSC 52 kill switch

Fuigo emits OSC 52 on Linux and across tmux, SSH, or displayless containers when
that route is enabled. A terminal that does not implement OSC 52 may display the
encoded payload as text. Set `FUIGO_CLIPBOARD_NO_OSC52=1` before starting Fuigo to
...
A remote Fuigo process normally cannot read the local terminal's selection. Use
...
When Fuigo cannot identify the outer terminal over SSH, it predicts that OSC 52
...
clipboard. Each copy is still saved to a backup file (`~/.fuigo/last-copy.txt` by
default; override with `FUIGO_COPY_FILE`); the toast names that path when delivery
...
through `fuigo wrap`, for example `fuigo wrap ssh user@host`. The same command can
wrap container and pod shells. It also restores terminal modes after a dropped
connection.

When an SSH session is not using `fuigo wrap`, Fuigo shows the one-time tip
...
`[ui.contextual_hints]` in `$FUIGO_HOME/config.toml`. This setting does not hide
the Doctor recommendation.

For repeated SSH use, Doctor offers `fuigo doctor fix ssh-wrap`. It also shows
...
> **Warning**: `fuigo wrap` is experimental and may not work in every setup.
...
Zellij and tmux control mode can limit the alternate screen. Fuigo normally uses
inline mode in those environments. Run `/doctor` to see the detected condition.
You can configure `[terminal] alt_screen` in `~/.fuigo/pager.toml`, or run
`fuigo --no-alt-screen` to confirm inline mode works.

### Zellij keybindings interfere with Fuigo

Zellij can intercept Ctrl/Alt keys before they reach Fuigo. On Zellij 0.41 or
...
mode, if `Ctrl+G` still does not reach Fuigo, open the command palette and select
...
WezTerm ships with the Kitty keyboard protocol disabled. Run `/doctor` in Fuigo.
...
keys. Fuigo therefore does not negotiate the protocol there, and Shift+Enter can
...
If Fuigo stops receiving mouse input, re-enable mouse reporting in the terminal:
...
After about 10 seconds without a transcript, Fuigo stops capture and shows
...
to the terminal hosting Fuigo. Open **System Settings → Privacy & Security →
...
Run `fuigo doctor`, or run `/doctor` while voice mode is on. The **Voice** section
shows the microphone Fuigo would use. If no input device is available, Doctor
...
problem, set `FUIGO_VOICE_CAPTURE=inprocess` to use the in-process fallback for
comparison.
...
terminals, Terminal.app, Konsole, mlterm, and others). Fuigo therefore
**does not** reorder RTL by default.

If Arabic or Persian in **scrollback** (or list content) reads backwards,
enable app-side reordering in `~/.fuigo/pager.toml` (or project config):
