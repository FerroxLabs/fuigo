...
Fuigo reads settings from config files, environment variables, and CLI flags. This page covers the common options. The field list for `config.toml`, `managed_config.toml`, and `requirements.toml` is [26-config-reference.md](26-config-reference.md) (extracted to `~/.fuigo/docs/user-guide/` on launch).
...
2. **Environment variables** (e.g. `FUIGO_API_KEY`, `FUIGO_MEMORY`)
3. **`requirements.toml` / MDM** (org-enforced; clamps every config layer below, including the overlay)
4. **`FUIGO_CONFIG` / `FUIGO_CONFIG_PATH` overlay** (above `config.toml` and managed, below `requirements.toml` / MDM)
5. **config.toml** (`~/.fuigo/config.toml`)
6. **`managed_config.toml`** (org-deployed defaults; below `config.toml`)
7. **Built-in defaults**

Within the config-file tier, the layers merge lowest-to-highest: `managed_config.toml` → `config.toml` → `FUIGO_CONFIG` overlay → `requirements.toml` / MDM. So `requirements.toml` and MDM clamp **both** your `config.toml` and the overlay.

`FUIGO_CONFIG` / `FUIGO_CONFIG_PATH` (tier 4) are config **overlays**: a merged config layer, not direct-setting environment variables like `FUIGO_API_KEY` (tier 2). They set config keys (subject to the allowlist below), so read them as part of the config-file tier rather than the env-var tier.

### Injecting config with `FUIGO_CONFIG`

A harness or ACP client that launches `fuigo agent stdio` can inject settings without writing a `config.toml` or relocating `$FUIGO_HOME`:
...
Location: `~/.fuigo/config.toml`. If the file is missing, Fuigo uses its built-in defaults, so you only need to set the values you want to override.
...
# Env vars FUIGO_MAX_PARALLEL_IMAGE_GEN_CALLS / FUIGO_MAX_PARALLEL_VIDEO_GEN_CALLS
...
You can also flip it from the settings pane (`/settings` → **Disable vim input mode**); Fuigo writes your choice to `[ui] simple_mode`. `simple_mode` and `vim_mode` are independent — one governs the prompt editor, the other governs scrollback navigation. See [Keyboard Shortcuts](03-keyboard-shortcuts.md) for the full binding reference.
...
Each setting also has an environment-variable override, applied on first load only (again, handy for headless / test runs): `FUIGO_SCROLL_SPEED`, `FUIGO_SCROLL_MODE`, `FUIGO_INVERT_SCROLL` (`1`/`true`/`0`/`false`), and `FUIGO_SCROLL_LINES`. Precedence: env var → `config.toml` → default. Unrecognized values fall back to the default, and out-of-range numbers clamp.
...
`allow_local` is off by default (SSRF fail-closed). Turn it on (or set `FUIGO_WEB_FETCH_ALLOW_LOCAL=1`) and `web_fetch` may reach **explicit** loopback hosts only — private, link-local, and cloud-metadata ranges stay blocked. Resolution: TOML > env > default off.
...
env_key = "FUIGO_API_KEY"               # env var(s) holding the API key; string or array (first set, non-empty wins)
...
Remote (HTTP/SSE) servers receive a default `User-Agent: fuigo-cli/<version>` header; a
valid `User-Agent` entry in `headers` overrides it (Figma servers receive bare
`fuigo-cli`). See [MCP servers](07-mcp-servers.md) for details.

MCP servers can also be set per-project in `.fuigo/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; every other section loads only from `~/.fuigo/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.fuigo/config.toml` (current dir) > `<repo-root>/.fuigo/config.toml` > `~/.fuigo/config.toml`. `[permission]` rules aren't overridden by priority — they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).

### Memory

Persist knowledge across sessions. Enable memory with `FUIGO_MEMORY=1`, `[memory] enabled = true`, or managed remote settings.
...
[subagents]
enabled = true
sampling_limit = 12                   # concurrent in-flight subagent sampling calls per process; defaults to max_concurrent (32) when unset (FUIGO_SUBAGENT_SAMPLING_LIMIT)
...
Background workflows — the `workflow` tool, named `.fuigo/workflows/*.rhai` scripts, `/deep-research`, and `/workflow` launches — are **on by default**. Disable with config, env, or remote settings.

```toml
[workflows]
enabled = false                       # disable background workflows (or FUIGO_WORKFLOWS=0)
```

Project workflows are discovered from `<repo-root>/.fuigo/workflows/`; user workflows from `~/.fuigo/workflows/`. Discovery and invocation key off the script's `meta.name`, so keep each filename aligned with its `meta.name`. Built-ins win over project names, and project names win over user names, so keep names unique across scopes.
...
`fuigo inspect` reports cells that still need session-start resolution as `?` until a value is available; cells with an explicit env or TOML value use that value. Affected discovery entries report `compatibilityStatus: "unresolved"` in JSON and `[compat unresolved]` in human output.
...
`[hints]` holds small persisted UI preferences: remembered answers and modal layout. Fuigo writes these for you as you use the TUI, but you can edit or delete them by hand; removing a key restores the default.

`[hints]` is read from the **effective config merge**, with the usual precedence: system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`, higher layers winning. The TUI only ever **writes** these to your user `~/.fuigo/config.toml`.
...
enabled = true
items = ["action-required", "spinner", "activity", "session-name", "fuigo"]
```
...
| `title.items` | array | (see above) | Items shown in the title bar. Options: `action-required`, `spinner`, `activity`, `session-name`, `cwd`, `model`, `turn-timer`, `fuigo`. |
...
| Fuigo Desktop | None (native) | N/A | N/A |
| Unknown | BEL | No | No |

With `method = "auto"`, Fuigo detects the terminal brand and picks the best protocol. Set `method` explicitly to override that.

#### Notification hooks

Run your own commands when events fire. Hooks receive `$FUIGO_EVENT`, `$FUIGO_MESSAGE`, and `$FUIGO_SESSION_ID` in the environment.

```toml
# macOS native notification
[[ui.notifications.hooks]]
command = "terminal-notifier -title 'Fuigo' -message '$FUIGO_MESSAGE'"
events = ["turn_complete", "approval_required"]
only_unfocused = true
timeout_secs = 10
...
# Push to ntfy server
[[ui.notifications.hooks]]
command = "curl -s -d '$FUIGO_MESSAGE' ntfy.sh/my-fuigo-alerts"
...
- **`[features] telemetry`** / `FUIGO_TELEMETRY_ENABLED` — the product-analytics master switch. `/privacy` doesn't change it.
- **Coding data, retention, and training** — the Settings row `/privacy` opens; coding-data sharing, separate from telemetry.
- **`[telemetry] trace_upload`** / `FUIGO_TELEMETRY_TRACE_UPLOAD` — session traces; follows telemetry when unset.
- **`[telemetry] otel_*`** / `FUIGO_EXTERNAL_OTEL` — external OTEL to your own collector (below).
...
Set these only to point telemetry at your own infrastructure or to switch parts off. The built-in endpoint and credentials are managed by Fuigo — leave them unset to use the defaults.
...
```toml
[telemetry]
otel_enabled = true                                       # external OTEL master switch (= FUIGO_EXTERNAL_OTEL)
...
- `minimum_version` (`FUIGO_MINIMUM_VERSION`) is a soft anti-downgrade floor. The
  updater skips a target below it and keeps the current version. It never blocks
  startup.
- `maximum_version` (`FUIGO_MAXIMUM_VERSION`) is a soft ceiling. The updater caps
  its target at it and never installs above it.
- `required_minimum_version` (`FUIGO_REQUIRED_MINIMUM_VERSION`) and
  `required_maximum_version` (`FUIGO_REQUIRED_MAXIMUM_VERSION`) are hard bounds. If
  the running version is outside the range, the CLI exits at startup and instructs
  the user to install an approved version. `fuigo update` and `fuigo --version` keep
...
- An explicit `fuigo update --version X` is allowed above the ceiling, to recover
...
Location: `~/.fuigo/pager.toml`. This controls the TUI's look and feel. Changes apply on restart.
...
| `FUIGO_AUTH_EARLY_INVALIDATION_SECS` | Seconds before expiry to refresh (default: 300) |
| `FUIGO_OIDC_ISSUER` | OIDC issuer URL |
| `FUIGO_OIDC_CLIENT_ID` | OIDC client ID |

### Endpoints

| Variable | Description |
|----------|-------------|
| `FUIGO_CLI_CHAT_PROXY_BASE_URL` | Override API proxy base URL |
...
| `FUIGO_SUBAGENTS` | Enable (`1`) or disable (`0`) subagents |
| `FUIGO_WORKFLOWS` | Enable (`1`) or disable (`0`) background workflows and select the `/goal` driver (default on: host-owned workflow driver; off: legacy `update_goal`) |
| `FUIGO_WEB_FETCH` | Enable (`1`) or disable (`0`) the web_fetch tool |
| `FUIGO_WEB_FETCH_ALLOW_LOCAL` | Allow `web_fetch` to explicit loopback hosts only (`localhost` / `127.0.0.0/8` / `::1`). Same as `[toolset.web_fetch] allow_local`. Default off; private/metadata stay blocked. |
| `FUIGO_AGENT` | Custom agent definition path or name |
| `FUIGO_SANDBOX` | Sandbox profile (off, workspace, devbox, read-only, strict; or a custom profile name) |
| `FUIGO_EXIT_TIMEOUT_SECS` | Seconds after a quit is requested before the process is force-exited if teardown hangs (default: 20, `0` disables; a hard exit follows 5s later) |

### Logging

| Variable | Description |
|----------|-------------|
| `FUIGO_LOG_FILE` | Write logs to this file path (used verbatim as the path) |
| `RUST_LOG` | Log level filter (e.g. `debug`); controls the `FUIGO_LOG_FILE` log and headless stderr output |
...
| `FUIGO_RESPECT_GITIGNORE` | Force gitignore filtering on (`1`) or off (`0`); overrides `[tools] respect_gitignore` |

### Telemetry

| Variable | Description |
|----------|-------------|
| `FUIGO_TELEMETRY_ENABLED` | Enable/disable telemetry |
| `FUIGO_TELEMETRY_TRACE_UPLOAD` | Enable/disable session trace upload |
| `FUIGO_TELEMETRY_MIXPANEL_ENABLED` | Enable/disable Mixpanel specifically |
| `FUIGO_EXTERNAL_OTEL` | External OTEL to your collector (see [24-monitoring-usage.md](24-monitoring-usage.md)) |
| `FUIGO_FEEDBACK_ENABLED` | Enable/disable feedback system |
| `FUIGO_DEPLOYMENT_KEY` | Management API key for enterprise |
...
| Path | Description |
|------|-------------|
| `~/.fuigo/config.toml` | Main configuration file |
| `~/.fuigo/pager.toml` | TUI appearance configuration |
...
| `~/.fuigo/lsp.json` | LSP server configuration (user-scoped) |
| `~/.fuigo/logs/` | Internal log files (e.g. `unified.jsonl`, MCP server logs) |
| `.fuigo/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
...
Some settings can be set per-project by placing files in `.fuigo/` inside your repository:

| File | What it configures |
|------|--------------------|
| `.fuigo/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.fuigo/config.toml`) |
| `.fuigo/skills/` | Project-specific skills |
| `.fuigo/hooks/` | Project-specific lifecycle hooks |
| `.fuigo/agents/` | Project-specific agent definitions |
| `.fuigo/lsp.json` | LSP server configuration |
| `.fuigo/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.fuigo/lsp.json` | All projects |
| Project | `.fuigo/lsp.json` | Current repository |
...
1. **Project** — `.fuigo/lsp.json`
2. **User** — `~/.fuigo/lsp.json`
