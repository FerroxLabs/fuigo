...
Fuigo is a terminal-based AI coding assistant from Ferrox Labs. It runs as a TUI (Terminal User Interface) that understands your codebase, executes shell commands, edits files, searches the web, and manages tasks.
...
for a complete clone. See [fuigo clone](27-fuigo-clone.md).

---

## First Launch

Start Fuigo by running:
...
Once authenticated, Fuigo presents a full-screen TUI with two main areas:

- **Scrollback** -- the conversation history showing your prompts, Fuigo's responses, tool calls, file edits, and more.
- **Prompt** -- the input area at the bottom where you type messages.

Type a message and press `Enter` to send it. Fuigo reads files, runs commands, and edits code as needed. Each tool run streams into the scrollback in real time.
...
By default, Fuigo asks for permission before executing shell commands or editing files. You can approve individually or toggle always-approve mode:

- Press `Ctrl+O` to toggle always-approve mode
- Use the `--yolo` flag at launch: `fuigo --yolo`
...
Every conversation is a **session**. Sessions are automatically saved to `~/.fuigo/sessions/` and can be resumed later. Each session tracks the full conversation history, tool calls, file edits, and task state.

- Start a new session: `Ctrl+N` or `/new`
- Resume a previous session: `/resume` in the TUI, or `--resume <ID>` from the CLI
- Continue the most recent session: `fuigo -c`
...
- **Agent messages** -- Fuigo's responses with full markdown rendering and syntax highlighting
- **Thinking blocks** -- Fuigo's reasoning process (collapsible)
...
fuigo --resume <session-id>

# Continue the most recent session
fuigo -c

# Experimental scrollback-native render mode. Sticky: plain `fuigo` reopens in
...
Run Fuigo non-interactively for scripting, CI/CD, and automation:
...
```bash
fuigo -p "Review changes for bugs" --output-format json --yolo | jq -r '.text'
```
...
Add per-project instructions by creating an `AGENTS.md` file in your repository. Fuigo reads these files and injects their contents as a project-instructions message at the start of the conversation:

```
~/.fuigo/AGENTS.md           # Global rules (apply to all projects)
<repo-root>/AGENTS.md       # Repository-level rules
<cwd>/AGENTS.md             # Directory-level rules (highest priority)
```

Deeper files take precedence. Fuigo also reads `CLAUDE.md` files for compatibility.
