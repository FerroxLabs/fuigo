...
Fuigo supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.fuigo/hooks/`
- **Project hooks** — from `.fuigo/hooks/` in your repo
...
Plugin hooks automatically receive `FUIGO_PLUGIN_ROOT` and `FUIGO_PLUGIN_DATA` environment variables (see the [Plugins guide](../user-guide/09-plugins.md#environment-variables-in-plugin-hooks)).
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.fuigo/settings.json` or `~/.claude/settings.json`
...
Press `a` on the Marketplace tab (or run `fuigo plugin marketplace add <source>`)
...
Sources land in `~/.fuigo/config.toml`:
...
Or in `~/.fuigo/settings.json` / `~/.claude/settings.json`:
...
- [Hook Examples](../../../fuigo-hooks/examples/README.md) — ready-to-use sample hooks
