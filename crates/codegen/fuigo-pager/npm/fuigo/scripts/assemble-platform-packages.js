...
//   1. Brotli-compresses the built binary into `../fuigo-<platform>/bin/<bin>.br`
...
// (`@fuigo-official/fuigo`) lists all six as `optionalDependencies` pinned to
// the same version; npm installs only the one matching the host's
// `os` + `cpu` filters.
//
// Why brotli? npm's tarball ceiling is ~200 MB and the raw fuigo binary is
...
const fuigoRoot = process.env.FUIGO_ROOT || path.resolve(__dirname, '..', '..', '..', '..', '..');
const npmRoot = path.resolve(__dirname, '..', '..');

const NOTICES_SOURCE = path.resolve(
    npmRoot, '..', '..', 'fuigo-tools', 'THIRD_PARTY_NOTICES.md');
...
async function packPlatform({ platform, arch, envVar, defaultSource, binName }) {
    const pkgDir = path.join(npmRoot, `fuigo-${platform}-${arch}`);
    ...
    console.log(
        `[assemble] fuigo-${platform}-${arch}@${VERSION}: ` +
    ...
    const targets = [
        {
            ...
            envVar: 'FUIGO_DARWIN_ARM64',
            defaultSource: path.join(fuigoRoot, 'target', 'release', 'fuigo-pager'),
        },
        {
            platform: 'darwin', arch: 'x64', binName: 'fuigo',
            envVar: 'FUIGO_DARWIN_X64',
            defaultSource: path.join(fuigoRoot, 'target', 'x86_64-apple-darwin', 'release', 'fuigo-pager'),
        },
        {
            platform: 'linux', arch: 'x64', binName: 'fuigo',
            envVar: 'FUIGO_LINUX_X64',
            defaultSource: path.join(fuigoRoot, 'target',
                'explorer_cross_x86_64-unknown-linux-gnu',
                'x86_64-unknown-linux-gnu', 'release', 'fuigo-pager'),
        },
        {
            platform: 'linux', arch: 'arm64', binName: 'fuigo',
            envVar: 'FUIGO_LINUX_ARM64',
            defaultSource: path.join(fuigoRoot, 'target',
                'explorer_cross_aarch64-unknown-linux-gnu',
                'aarch64-unknown-linux-gnu', 'release', 'fuigo-pager'),
        },
        {
            platform: 'win32', arch: 'x64', binName: 'fuigo.exe',
            envVar: 'FUIGO_WIN32_X64',
            defaultSource: path.join(fuigoRoot, 'target', 'x86_64-pc-windows-msvc', 'release', 'fuigo-pager.exe'),
        },
        {
            platform: 'win32', arch: 'arm64', binName: 'fuigo.exe',
            envVar: 'FUIGO_WIN32_ARM64',
            defaultSource: path.join(fuigoRoot, 'target', 'aarch64-pc-windows-msvc', 'release', 'fuigo-pager.exe'),
        },
    ];
