...
Terminal UI (TUI) for Fuigo. Provides the interactive full-screen interface
...
| `Esc` | Turn running | Does not cancel (every mode): shows a reminder to press `Ctrl+C`, which cancels |
...
- [Hook Examples](../fuigo-hooks/examples/README.md) — sample hooks for common workflows
- [Hooks Crate (`fuigo-hooks`)](../fuigo-hooks/) — hook runtime, event types, and execution engine
- [Plugin Marketplace Crate (`fuigo-plugin-marketplace`)](../fuigo-plugin-marketplace/) — marketplace source loading, scanning, and install
