...
//!
//! > After any install attempt, `$BIN_DIR/fuigo` resolves to a binary that runs, OR is still the previous-good binary, never a partial/garbage binary.
//!
...
fn script_path(name: &str) -> Option<PathBuf> {
    dunce::canonicalize(
        Path::new(env!("CARGO_MANIFEST_DIR")).join(format!("../fuigo-pager/scripts/{name}")),
    )
    .ok()
    .filter(|p| p.exists())
}
...
fn desktop_install_sh_path() -> Option<PathBuf> {
    dunce::canonicalize(
        Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../../frontend/apps/fuigo-desktop/scripts/install.sh"),
    )
    .ok()
    .filter(|p| p.exists())
}
...
const INSTALLER_BLOCK_START: &str = "# >>> fuigo installer >>>";
...
fn seed_previous_good(home: &Path, platform: &str) -> PathBuf {
    let downloads = home.join(".fuigo").join("downloads");
    let bin = home.join(".fuigo").join("bin");
    std::fs::create_dir_all(&downloads).unwrap();
    std::fs::create_dir_all(&bin).unwrap();
    let prev = downloads.join(format!("fuigo-{platform}"));
    ...
    std::os::unix::fs::symlink(format!("../downloads/fuigo-{platform}"), &link).unwrap();
    dunce::canonicalize(&prev).unwrap()
}

/// Re-resolve `$BIN_DIR/fuigo` from disk and re-run it: the active fuigo must always execute, and never be a `.tmp`/partial file.
fn assert_active_fuigo_runs(home: &Path) {
    let link = home.join(".fuigo").join("bin").join("fuigo");
    assert!(link.is_symlink(), "fuigo must remain a symlink");
    let resolved =
        dunce::canonicalize(&link).unwrap_or_else(|e| panic!("fuigo symlink dangles: {e}"));
    let name = resolved.file_name().unwrap().to_string_lossy().to_string();
    assert!(
        !name.contains(".tmp"),
        "active fuigo must not be a temp file: {name}"
    );
    let ok = Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ok, "active fuigo must run: {}", resolved.display());
}
...
fn run_installer(install_sh: &Path, home: &Path, fakebin: &Path, mode: &str, shell: &str) -> bool {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", shell)
        .env("FUIGO_BIN_DIR", home.join(".fuigo").join("bin"))
        .env("FUIGO_CHANNEL", "stable")
        .env("FAKE_MODE", mode)
        .status()
...
fn assert_single_installer_block(path: &Path, preserved: Option<&str>) {
    ...
    assert_eq!(
        n,
        1,
        "{} must contain exactly one fuigo installer block, got {n}:\n{body}",
        path.display()
    );
...
fn run_shell_rc_case(case: &ShellRcCase) {
    ...
    assert_active_fuigo_runs(&home_path);
}

#[test]
fn install_sh_blitz_keeps_fuigo_runnable_under_corruption() {
    ...
    for (mode, expect_ok) in cases {
        ...
        // The invariant holds on every path: the active fuigo always runs (new good binary on success, previous-good on rejection)
        assert_active_fuigo_runs(home.path());
    }
}
...
/// The enterprise script requires auth, provided via a dummy `FUIGO_DEPLOYMENT_KEY`.
fn install_urls_on_fake_host(script: &str, host: FakeHost) -> Option<String> {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("PATH", path_env)
        .env("SHELL", "/bin/bash")
        .env("FUIGO_BIN_DIR", home.path().join(".fuigo").join("bin"))
        .env("FUIGO_CHANNEL", "stable")
        .env("FUIGO_DEPLOYMENT_KEY", "test-deployment-key")
        .env("FAKE_MODE", "full")
...
fn run_with_proxy_url(script: &Path, proxy_url: &str) -> (bool, String, bool) {
    ...
    let status = Command::new("/bin/bash")
        ...
        .env("FUIGO_BIN_DIR", home.path().join(".fuigo").join("bin"))
        .env("FUIGO_CHANNEL", "stable")
        .env("FUIGO_DEPLOYMENT_KEY", "test-deployment-key-must-not-leak")
        .env("FUIGO_PROXY_URL", proxy_url)
        .env("FAKE_MODE", "full")
        .env("FAKE_URL_LOG", &url_log)
        .status()
        .expect("spawn bash install script");
    let urls = std::fs::read_to_string(&url_log).unwrap_or_default();
    let managed = home.path().join(".fuigo/managed_config.toml").exists();
...
fn assert_no_credentialed_proxy_request(label: &str, proxy_url: &str, urls: &str) {
    assert!(
        !urls.contains("/deployment/config")
            && !urls.contains("/fuigo-cli/update")
...
fn install_scripts_refuse_bad_proxy_url_for_deployment_key() {
    ...
    for (label, script_file) in &scripts {
        for proxy_url in BAD_PROXY_URLS {
            ...
            assert!(
                !ok,
                "{label}: FUIGO_PROXY_URL={proxy_url:?} must fail closed"
            );
...
#[test]
fn install_sh_rejects_hostile_fuigo_channel() {
    let Some(install_sh) = install_sh_path() else {
        eprintln!("skipping: install.sh not found relative to crate; run under cargo");
        return;
    };
    ...
    assert!(
        ...
        "unlisted FUIGO_CHANNEL must fail closed"
    );
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("FUIGO_CHANNEL"),
        "must name FUIGO_CHANNEL in the error, stderr:\n{stderr}"
    );
    let config = home.path().join(".fuigo/config.toml");
    if config.exists() {
...
fn install_scripts_allow_custom_https_proxy_url() {
    ...
    for (label, script_file) in &scripts {
        for proxy_url in GOOD_PROXY_URLS {
            ...
            assert!(
                ok,
                "{label}: custom https FUIGO_PROXY_URL={proxy_url:?} must succeed"
            );
            assert!(
