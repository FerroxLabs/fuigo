//! These tests must run serially: they touch `FUIGO_HOME` (a `OnceLock` in `fuigo-config`), `FUIGO_TEST_VERSION`, and `NPM_TOKEN`.
//! Once `FUIGO_HOME` is initialized for a process, it can't be changed.
...
use fuigo_update::write_version_cache;
...
#[tokio::test]
#[serial]
async fn write_version_cache_creates_file_at_fuigo_home() {
    let _ = test_home();
    reset();
...
// ─────────────────────────────────────────────────────────────────────────────
// version.json wire format — the on-disk file is read by every fuigo launch.
// ─────────────────────────────────────────────────────────────────────────────
#[tokio::test]
#[serial]
...
// ─────────────────────────────────────────────────────────────────────────────
// get_installed_fuigo_version env override
//
// The function honors `FUIGO_TEST_VERSION` for testing. We exercise it
// via the public re-export only — no private items leaked.
// ─────────────────────────────────────────────────────────────────────────────
//
// `get_installed_fuigo_version` is not re-exported from `lib.rs`, but it's `pub` from `version` module and accessible via `version::`
#[tokio::test]
#[serial]
async fn get_installed_version_falls_back_to_cargo_pkg_version_when_env_unset() {
    ...
    let v = fuigo_update::version::get_installed_fuigo_version();
...
async fn get_installed_version_with_env_var_takes_precedence() {
    ...
    let real = {
        ...
        fuigo_update::version::get_installed_fuigo_version()
    };

    unsafe {
        std::env::set_var("FUIGO_TEST_VERSION", "0.0.0-test");
    }
    let overridden = fuigo_update::version::get_installed_fuigo_version();
...
async fn get_installed_version_does_not_validate_env_var_format() {
    ...
    unsafe {
        std::env::set_var("FUIGO_TEST_VERSION", "not-a-version");
    }
    let v = fuigo_update::version::get_installed_fuigo_version();
