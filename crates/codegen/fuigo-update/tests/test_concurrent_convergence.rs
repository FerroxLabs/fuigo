...
//!
//! 1. TUI startup: `check_update_background` spawns a detached `fuigo update` (the Ctrl+U path adopts this child instead of spawning a second).
//! 2. Explicit `fuigo update` (including the Ctrl+U fallback when there is no live child).
...
use fuigo_update::version::installed_on_disk_version;

/// Assert the active `~/.fuigo/bin/fuigo` resolves to the expected versioned
...
fn assert_active_binary(home: &Path, version: &str, platform: &str, expected_content: &[u8]) {
    let link = home.join("bin").join("fuigo");
    assert!(link.is_symlink(), "fuigo must be a symlink");
    let resolved = dunce::canonicalize(&link)
        .unwrap_or_else(|e| panic!("active fuigo symlink does not resolve: {e}"));
    assert_eq!(
        resolved.file_name().unwrap().to_string_lossy(),
        format!("fuigo-{version}-{platform}"),
        "active fuigo must be the expected version"
    );
    assert_eq!(
    ...
    let ran_ok = std::process::Command::new(&resolved)
        ...
        .map(|s| s.success())
        .unwrap_or(false);
    assert!(ran_ok, "active fuigo must pass the smoke-test");
}

/// Lay down what `install_internal_from_base` produces in the test FUIGO_HOME: `bin/fuigo -> ../downloads/fuigo-<version>-<platform>`.
...
// ─────────────────────────────────────────────────────────────────────────────
// Convergence: explicit `fuigo update` (the Ctrl+U fallback path) finds the binary another process already installed and skips the download
...
// installers that actually maintain the managed `~/.fuigo/bin/fuigo` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.fuigo/downloads cleanup), the probe must report None — otherwise
...
async fn concurrent_different_version_installs_do_not_corrupt_each_other() {
    ...
    for version in ["0.1.181", "0.1.182"] {
        let path = home
            .join("downloads")
            .join(format!("fuigo-{version}-{platform}"));
        assert_eq!(
    ...
    let resolved = dunce::canonicalize(home.join("bin").join("fuigo")).unwrap();
    assert_eq!(std::fs::read(&resolved).unwrap(), artifact);
    let name = resolved.file_name().unwrap().to_string_lossy().to_string();
    assert!(
        !name.contains(".tmp"),
        "active fuigo must never be a temp file: {name}"
    );
