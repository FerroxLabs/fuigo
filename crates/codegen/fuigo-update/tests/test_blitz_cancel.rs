...
//!
//! > `~/.fuigo/bin/fuigo` resolves to a binary that passes the smoke-test, OR it
...
use fuigo_update::auto_update::install_internal_from_base;
...
/// Seed a previous-good versioned binary and both managed symlinks (`fuigo` and `agent`; see `swap_managed_bin_links`).
...
/// What the active `fuigo` should resolve to after an install attempt.
...
/// Applied to both `fuigo` and `agent`; `swap_managed_bin_links` moves them together.
...
/// Gated behind `#[ignore]`; invoke via `just blitz-stress` or `cargo nextest run -p fuigo-update --run-ignored all`.
#[tokio::test(flavor = "multi_thread")]
#[serial]
...
async fn blitz_fuzz_stress() {
    ...
    let iterations: usize = std::env::var("FUIGO_BLITZ_ITERS")
        .ok()
