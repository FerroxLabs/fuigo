...
async fn mount_gcs_with_channels(
    ...
    Mock::given(method("GET"))
        .and(path(format!("/fuigo-{binary_version}-{platform}")))
...
async fn internal_install_stable_upgrade_0_2_5_to_0_2_7() {
    ...
    let symlink = test_home().join("bin").join("fuigo");
    let target = std::fs::read_link(&symlink).unwrap();
...
//
// The TUI background download, the leader hourly checker, and explicit `fuigo update` can run concurrently
...
//
// These test the race between a user opening fuigo (which caches the version) and a pointer flip happening
...
async fn npm_user_upgraded_then_stable_rolled_back_stays_on_newer() {
    // User ran `fuigo update` and got 0.2.7. Then stable was rolled back to 0.2.5.
