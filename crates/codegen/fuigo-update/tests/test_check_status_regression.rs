//! End-to-end regression tests for `check_update_status`, locking in the exact JSON shape produced by `fuigo update --check --json`.
...
use fuigo_update::auto_update::check_update_status;

/// Set up a fake `npm` on PATH and set `FUIGO_INSTALLER=npm` so the auto-update code dispatches to npm without consulting config.
