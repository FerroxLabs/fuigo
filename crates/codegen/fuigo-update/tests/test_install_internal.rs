...
//!
//! Wires together a wiremock-mocked GCS bucket and an isolated `FUIGO_HOME` tempdir to verify the full install pipeline:
//! fetch version, download the fuigo binary, chmod, atomic symlink, cleanup_old_downloads, persist installer config.
//!
//! The function reads `fuigo_home()` (a process-wide `OnceLock`).
//! All tests in this binary therefore share one `FUIGO_HOME` and run serially via `#[serial]`.
...
use fuigo_telemetry::events::CliUpdateErrorKind;
use fuigo_update::UpdateConfig;
...
async fn mount_gcs(version: &str, platform: &str) -> MockServer {
    ...
    // Main fuigo binary download.
    Mock::given(method("GET"))
        .and(path(format!("/fuigo-{version}-{platform}")))
...
async fn install_internal_pinned_version_writes_binary_and_symlink() {
    ...
    assert!(symlink.is_symlink(), "fuigo symlink created");
    ...
    assert_eq!(agent_target, target, "agent and fuigo point at same target");
}
...
/// Rollback regression: if `agent` swap fails after `fuigo` succeeded, `fuigo` must roll back to its prior target (all-or-nothing).
#[tokio::test]
#[serial]
async fn install_internal_rolls_back_fuigo_when_agent_swap_fails() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
...
/// Rollback regression for a fresh install (no prior `fuigo` or `agent`): a sabotaged `agent` swap must *remove* the just-created `fuigo` link.
/// Otherwise `fuigo` would stay on the new binary while `agent` is absent.
#[tokio::test]
#[serial]
async fn install_internal_rollback_removes_absent_prior_fuigo_link() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    // No prior `fuigo`. Sabotage the `agent` swap: a non-empty directory fails the rename with EISDIR.
    let agent_dir = bin_dir.join("agent");
    std::fs::create_dir(&agent_dir).unwrap();
    std::fs::write(agent_dir.join("blocker"), b"x").unwrap();
    assert!(
        !bin_dir.join("fuigo").exists() && !bin_dir.join("fuigo").is_symlink(),
        "precondition: fuigo must not exist before install",
    );
    ...
    let fuigo_path = bin_dir.join("fuigo");
    assert!(
        !fuigo_path.is_symlink() && !fuigo_path.exists(),
        "fuigo must be removed on rollback when there was no prior link",
    );
}
...
async fn install_internal_cleans_up_stale_pager_symlink() {
    // Old installations shipped a separate fuigo-pager binary. Verify the
    // update removes the stale symlink from ~/.fuigo/bin/.
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = mount_gcs("0.1.181", &platform).await;
    ...
    let bin_dir = home.join("bin");
    std::fs::create_dir_all(&bin_dir).unwrap();
    let pager_link = bin_dir.join("fuigo-pager");
    ...
    assert!(
        ...
        "stale fuigo-pager symlink should be removed"
    );
}
...
#[tokio::test]
#[serial]
async fn install_internal_fails_on_fuigo_binary_404() {
    let _ = test_home();
    reset_home();
    let platform = host_platform();
    let server = MockServer::start().await;
...
async fn install_internal_cleans_up_old_versions_keeping_n_minus_one() {
    ...
    let target = std::fs::read_link(home.join("bin").join("fuigo")).unwrap();
    assert!(
        target
            .file_name()
            .unwrap()
...
async fn install_internal_idempotent_for_same_version() {
    ...
    let target = std::fs::read_link(test_home().join("bin").join("fuigo")).unwrap();
    assert!(target.to_string_lossy().contains("0.1.181"));
}

#[tokio::test]
#[serial]
async fn install_internal_creates_fuigo_home_subdirs_if_missing() {
    let _ = test_home();
    reset_home();
