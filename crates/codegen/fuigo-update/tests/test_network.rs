...
use fuigo_update::auto_update::{download_silent, download_with_progress};
use fuigo_update::version::fetch_gcs_version_from_base;
...
async fn download_silent_to_nonexistent_parent_dir_fails() {
    ...
    let dest = tmp.path().join("missing-subdir").join("fuigo");
    let err = download_silent(&format!("{}/x", server.uri()), &dest)
        .await
        .unwrap_err();
    let msg = format!("{err:#}").to_lowercase();
...
async fn download_with_progress_writes_body_with_content_length() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let dest = tmp.path().join("fuigo");
    download_with_progress(&format!("{}/fuigo", server.uri()), &dest)
        .await
        .unwrap();
...
async fn download_silent_parallel_path_reassembles_bytes() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let dest = tmp.path().join("fuigo-binary");
    download_silent(&format!("{}/big", server.uri()), &dest)
        .await
        .unwrap();
