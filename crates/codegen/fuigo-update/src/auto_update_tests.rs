...
fn test_tmp_download_path_is_unique_per_version_and_per_attempt() {
    ...
    let dest_181 = std::path::Path::new("/home/u/.fuigo/downloads/grok-0.1.181-linux-x86_64");
    let dest_182 = std::path::Path::new("/home/u/.fuigo/downloads/grok-0.1.182-linux-x86_64");
    ...
    assert_eq!(
        a.parent(),
        std::path::Path::new("/home/u/.fuigo/downloads").into(),
...
async fn test_atomic_symlink_swap_replaces_regular_file() {
    ...
    let link = dir.path().join("fuigo");
    // Simulate an old installation where fuigo is a regular file.
...
async fn test_reconcile_agent_repoints_diverged_agent() {
    ...
    std::os::unix::fs::symlink("../downloads/grok-0.2.101-macos-aarch64", bin.join("fuigo"))
        .unwrap();
    std::os::unix::fs::symlink("../downloads/grok-0.1.199-macos-aarch64", bin.join("agent"))
        .unwrap();

    reconcile_agent_to_fuigo(&bin).await;
...
async fn test_reconcile_agent_heals_legacy_unversioned_agent() {
    let (_dir, bin, downloads) = managed_layout();
    std::fs::write(downloads.join("grok-0.2.101-macos-aarch64"), "new").unwrap();
    std::fs::write(downloads.join("fuigo-macos-aarch64"), "legacy").unwrap();

    std::os::unix::fs::symlink("../downloads/grok-0.2.101-macos-aarch64", bin.join("fuigo"))
        .unwrap();
    std::os::unix::fs::symlink("../downloads/fuigo-macos-aarch64", bin.join("agent")).unwrap();
...
async fn test_reconcile_agent_noop_when_consistent() {
    ...
    std::os::unix::fs::symlink(target, bin.join("fuigo")).unwrap();
...
#[cfg(unix)]
#[tokio::test]
async fn test_reconcile_agent_skips_when_fuigo_dangling() {
    let (_dir, bin, downloads) = managed_layout();
    std::os::unix::fs::symlink("../downloads/grok-0.2.101-macos-aarch64", bin.join("fuigo"))
        .unwrap();
    std::fs::write(downloads.join("grok-0.1.199-macos-aarch64"), "old").unwrap();
...
#[cfg(unix)]
#[tokio::test]
async fn test_reconcile_agent_skips_when_fuigo_not_symlink() {
    let (_dir, bin, downloads) = managed_layout();
    std::fs::write(bin.join("fuigo"), "copy-binary").unwrap();
    std::fs::write(downloads.join("grok-0.1.199-macos-aarch64"), "old").unwrap();
    std::os::unix::fs::symlink("../downloads/grok-0.1.199-macos-aarch64", bin.join("agent"))
        .unwrap();
...
async fn test_sweep_stale_tmp_links_removes_stale_keeps_fresh_and_active() {
    ...
    let leftover_old = dir.path().join("fuigo.tmp-link");
    let leftover_new = dir.path().join("fuigo.123-0.tmp-link");
    std::os::unix::fs::symlink(&target, &leftover_old).unwrap();
    std::os::unix::fs::symlink(&target, &leftover_new).unwrap();

    // max_age = ZERO: every leftover is stale and removed; the active `fuigo` link (no `.tmp-link` suffix) is untouched
    ...
    let fresh = dir.path().join("fuigo.999-9.tmp-link");
...
fn test_relative_symlink_target_sibling_dirs() {
    // bin/fuigo -> ../downloads/grok-0.1.203
    let target = std::path::Path::new("/home/alice/.fuigo/downloads/grok-0.1.203");
    let link = std::path::Path::new("/home/alice/.fuigo/bin/fuigo");
...
fn test_relative_symlink_target_same_dir() {
    // downloads/fuigo-latest -> grok-0.1.203 (same directory)
    let target = std::path::Path::new("/home/alice/.fuigo/downloads/grok-0.1.203");
    let link = std::path::Path::new("/home/alice/.fuigo/downloads/fuigo-latest");
    let result = relative_symlink_target(target, link);
...
fn test_relative_symlink_target_cross_tree_stays_absolute() {
    // /usr/local/bin/fuigo -> /home/alice/.fuigo/downloads/grok-0.1.203
    // Different grandparents, so the target should stay absolute
    let target = std::path::Path::new("/home/alice/.fuigo/downloads/grok-0.1.203");
    let link = std::path::Path::new("/usr/local/bin/fuigo");
    let result = relative_symlink_target(target, link);
    assert_eq!(
        result,
        std::path::PathBuf::from("/home/alice/.fuigo/downloads/grok-0.1.203")
    );
}
...
async fn test_relative_symlink_survives_directory_move() {
    // Simulates Docker bind-mount: create ~/.fuigo/ layout at path A,
    // then move it to path B and verify the symlink still resolves.
    let dir = tempfile::tempdir().unwrap();

    // Create alice's layout
    let alice = dir.path().join("alice").join(".fuigo");
    ...
    let bob_link = bob.join("bin").join("fuigo");
...
async fn test_cleanup_old_downloads_keeps_current_plus_one() {
    ...
    // Simulate 5 old fuigo binaries in downloads dir.
...
async fn test_cleanup_old_downloads_version_prefix_collision() {
    ...
    cleanup_old_downloads(d, "fuigo", "0.1.14").await;
...
async fn test_cleanup_old_downloads_pager_multi_version() {
    // Verify cleanup works for fuigo-pager with multiple old versions.
    let dir = tempfile::tempdir().unwrap();
    let d = dir.path();

    for v in ["0.1.148", "0.1.149", "0.1.150"] {
        std::fs::write(d.join(format!("fuigo-pager-{}-linux-x64", v)), v).unwrap();
    }
    std::fs::write(d.join("fuigo-pager-0.1.151-linux-x64"), "current").unwrap();

    make_all_stale(d);

    cleanup_old_downloads(d, "fuigo-pager", "0.1.151").await;

    assert!(d.join("fuigo-pager-0.1.151-linux-x64").exists(), "current");
    assert!(d.join("fuigo-pager-0.1.150-linux-x64").exists(), "N-1 kept");
    assert!(
        !d.join("fuigo-pager-0.1.149-linux-x64").exists(),
        "0.1.149 deleted"
    );
    assert!(
        !d.join("fuigo-pager-0.1.148-linux-x64").exists(),
        "0.1.148 deleted"
    );
}
...
async fn test_cleanup_old_downloads_npm_layout() {
    // npm layout: files are just `fuigo-{version}` (no platform suffix).
    let dir = tempfile::tempdir().unwrap();
    let d = dir.path();
...
async fn test_cleanup_old_downloads_alpha_versions() {
    ...
    cleanup_old_downloads(d, "fuigo", "0.1.150-alpha.1").await;
...
async fn test_cleanup_old_downloads_files_with_non_digit_suffix_skipped() {
    ...
    // fuigo-latest and fuigo-pager-* must be untouched.
    assert!(d.join("fuigo-latest").exists());
    assert!(d.join("fuigo-pager-0.1.141-macos-aarch64").exists());
}
...
fn test_user_facing_constants_are_stable() {
    ...
    assert_eq!(
        ...
        "Run `fuigo update` to get the latest version."
    );
}
...
//   1. FUIGO_INSTALLER (npm | internal | gh-release | gh)
//   2. FUIGO_MANAGED_BY_NPM       → npm
//   3. FUIGO_MANAGED_BY_INTERNAL  → internal
...
impl InstallerEnvGuard {
    fn isolate() -> Self {
        const VARS: &[&str] = &[
            "FUIGO_INSTALLER",
            "FUIGO_MANAGED_BY_NPM",
            "FUIGO_MANAGED_BY_INTERNAL",
...
fn test_env_installer_explicit_npm() {
    let _g = InstallerEnvGuard::isolate();
    unsafe { std::env::set_var("FUIGO_INSTALLER", "npm") };
...
fn test_env_installer_explicit_internal() {
    let _g = InstallerEnvGuard::isolate();
    unsafe { std::env::set_var("FUIGO_INSTALLER", "internal") };
...
fn test_env_installer_explicit_gh_alias() {
    ...
    let _g = InstallerEnvGuard::isolate();
    unsafe { std::env::set_var("FUIGO_INSTALLER", "gh") };
    assert_eq!(env_installer(), Some("gh-release"));
}
...
fn test_env_installer_explicit_unknown_value_returns_none() {
    ...
    // So `FUIGO_INSTALLER=brew` disables the env-installer detection entirely
    let _g = InstallerEnvGuard::isolate();
    unsafe { std::env::set_var("FUIGO_INSTALLER", "brew") };
    // Even if MANAGED_BY_NPM is also set, the explicit var wins (and rejects).
    unsafe { std::env::set_var("FUIGO_MANAGED_BY_NPM", "1") };
    assert_eq!(
        env_installer(),
        None,
        "explicit FUIGO_INSTALLER=brew must early-return None, not fall through"
    );
}
...
fn test_env_installer_explicit_empty_returns_none() {
    let _g = InstallerEnvGuard::isolate();
    unsafe { std::env::set_var("FUIGO_INSTALLER", "") };
    assert_eq!(env_installer(), None);
}
...
fn test_env_installer_managed_by_npm_any_value() {
    ...
    let _g = InstallerEnvGuard::isolate();
    unsafe { std::env::set_var("FUIGO_MANAGED_BY_NPM", "") };
    assert_eq!(env_installer(), Some("npm"));
}
...
fn test_env_installer_managed_by_internal() {
    let _g = InstallerEnvGuard::isolate();
    unsafe { std::env::set_var("FUIGO_MANAGED_BY_INTERNAL", "1") };
    assert_eq!(env_installer(), Some("internal"));
}
...
fn test_env_installer_explicit_internal_wins_over_npm_managed() {
    // FUIGO_INSTALLER=internal must override an inherited MANAGED_BY_NPM.
    let _g = InstallerEnvGuard::isolate();
    unsafe {
        std::env::set_var("FUIGO_INSTALLER", "internal");
        std::env::set_var("FUIGO_MANAGED_BY_NPM", "1");
    }
    assert_eq!(env_installer(), Some("internal"));
}
...
async fn test_windows_replace_exe_creates_dest_when_missing() {
    ...
    let dest = dir.path().join("fuigo.exe");
...
async fn test_windows_replace_exe_cleans_stale_old_backup() {
    ...
    let old = dir.path().join("fuigo.exe.old");
...
async fn test_windows_replace_exe_rollback_on_copy_failure() {
    // No stale .old: the aside IS fuigo.exe.old, so this pins the non-diverted rollback branch (rename .old back onto dest)
    use std::os::windows::fs::OpenOptionsExt;
    const FILE_SHARE_READ: u32 = 0x00000001;
    const FILE_SHARE_DELETE: u32 = 0x00000004;
...
async fn test_windows_replace_exe_locked_stale_old_does_not_block_update() {
    ...
    let asides: Vec<std::path::PathBuf> = std::fs::read_dir(dir.path())
        ...
        .filter(|p| {
            p.file_name()
                .and_then(|n| n.to_str())
                .is_some_and(|n| n.starts_with("fuigo.exe.old.") && n.ends_with(".old"))
        })
        .collect();
    assert_eq!(
        asides.len(),
...
async fn test_windows_replace_exe_rollback_restores_from_diverted_aside() {
    ...
    let leftover_asides = std::fs::read_dir(dir.path())
        ...
        .filter(|e| {
            ...
            name.starts_with("fuigo.exe.old.") && name.ends_with(".old")
        })
        .count();
...
async fn test_windows_replace_exe_sweeps_accumulated_asides() {
    ...
    let aside_a = dir.path().join("fuigo.exe.old.1234-0.old");
    let aside_b = dir.path().join("fuigo.exe.old.1234-1.old");
...
async fn download_and_decode_round_trips_each_codec() {
    ...
    let payload = b"\x7fELF fuigo binary payload".to_vec();
