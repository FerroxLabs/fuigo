...
fn executable_is_in_use_is_false_for_missing_binary() {
    assert!(!executable_is_in_use(Path::new("/no/such/fuigo-binary")));
}
...
async fn cleanup_keeps_an_older_binary_marked_in_use() {
    ...
    let live = d.join("fuigo-0.1.140-macos-aarch64");
    cleanup_old_downloads_with(d, "fuigo", "0.1.145", |path| path == live).await;
    ...
    assert!(!d.join("fuigo-0.1.143-macos-aarch64").exists(), "idle");
}
