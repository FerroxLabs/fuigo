...
use fuigo_shell::util::fuigo_home::{fuigo_application, fuigo_home};
pub use fuigo_telemetry::events::CliUpdateTrigger;
...
pub async fn check_update_status(update_config: &UpdateConfig) -> UpdateStatus {
    ...
    let current_version = get_installed_fuigo_version();
...
pub async fn auto_update_target(update_config: &UpdateConfig) -> Option<(&'static str, String)> {
    ...
    let current = get_installed_fuigo_version();
...
/// A binary already installed by another process (TUI background download, explicit `fuigo update`) is reused as-is.
...
pub async fn ensure_latest_on_disk(update_config: &UpdateConfig) -> Result<EnsureLatestOutcome> {
    ...
    let effective_current =
        disk_version_for_installer(installer).unwrap_or_else(get_installed_fuigo_version);
    if needs_update(
        &effective_current,
        &target,
        &update_config.channel,
...
/// managed `~/.fuigo/bin/fuigo` symlink.
///
...
fn env_installer() -> Option<&'static str> {
    if let Ok(v) = std::env::var("FUIGO_INSTALLER") {
    ...
    if std::env::var_os("FUIGO_MANAGED_BY_NPM").is_some() {
        return Some("npm");
    }
    if std::env::var_os("FUIGO_MANAGED_BY_INTERNAL").is_some() {
...
pub struct BackgroundUpdateCheck {
    ...
    /// Handle to the background `fuigo update` child, `Some` only when a download was actually started (the on-disk install was behind the pointer).
...
/// If `auto_update` is enabled **and the on-disk install is also behind the pointer**, kicks off a download (a detached `fuigo update` child).
...
pub async fn check_update_background(update_config: &UpdateConfig) -> BackgroundUpdateCheck {
    ...
    // The quit-for-update path's `fuigo update` child resolves to "Already up to date" against the same disk state
...
/// Launch "fuigo update" in blocking or non-blocking mode.
///
...
async fn run_update_subcommand(
    ...
    if let Some(mode) = fuigo_telemetry::client::current_mode() {
        cmd.env("FUIGO_TELEMETRY_ENABLED", mode.to_string());
    }
    match run_mode {
        UpdateRunMode::Blocking => {
            ...
            if !status.success() {
                anyhow::bail!("fuigo update failed with {}", status);
            }
            Ok(None)
        }
...
/// Restart fuigo with the original command-line arguments to pick up the update.
pub fn restart_fuigo() -> Result<()> {
...
pub async fn run_install_script(
    ...
    fuigo_telemetry::session_ctx::log_event(CliUpdate {
        outcome,
        trigger,
        from_version,
        to_version,
...
/// That covers interactive `fuigo update`, background `--auto` children, the leader's hourly converge, and forced minimum-version installs.
...
/// Delete `~/.fuigo/models_cache.json` after a successful update.
///
...
async fn remove_stale_models_cache() {
    let cache = fuigo_home().join("models_cache.json");
...
/// Remove the stale `fuigo-pager` symlink/binary from `~/.fuigo/bin/` left by
...
/// config and writes to `~/.fuigo/bin/`, so callers must isolate
/// `FUIGO_HOME`.
#[doc(hidden)]
...
/// A downloaded and smoke-tested binary in `~/.fuigo/downloads/`, not yet
/// activated as the managed `fuigo`/`agent`.
...
async fn download_verified_from_base(
    ...
    let fuigo_home = fuigo_home();
    let download_dir = fuigo_home.join("downloads");
    ...
    // A truncated or corrupt download is caught here and never becomes the active fuigo
...
async fn activate_verified_download(download: &VerifiedDownload) -> Result<()> {
    let fuigo_home = fuigo_home();
    let download_dir = fuigo_home.join("downloads");
    let bin_dir = fuigo_home.join("bin");
    ...
    cleanup_old_downloads(&download_dir, "fuigo-pager", &download.version).await;
    ...
    regenerate_completions(&link_path, &fuigo_home).await;
...
async fn regenerate_completions(binary: &std::path::Path, fuigo_home: &std::path::Path) {
    // Derive $HOME independently: fuigo_home may be overridden via FUIGO_HOME env var, so fuigo_home.parent() isn't necessarily the user's home dir
    let user_home = fuigo_dirs::home_dir().unwrap_or_default();
...
fn relative_symlink_target(target: &std::path::Path, link: &std::path::Path) -> std::path::PathBuf {
    ...
    // Sibling directories: ../target_dir/filename (e.g. bin/fuigo -> ../downloads/fuigo-…)
...
/// Swap `~/.fuigo/bin/{fuigo,agent}` to point at `binary_path`. Returns the
/// `fuigo` link path (for [`regenerate_completions`]).
///
/// The bootstrap installers (`install.sh`, `install.ps1`, `install-enterprise.sh`) maintain `fuigo` and `agent` in lockstep, and so must the updater.
/// Otherwise `fuigo update` leaves `agent` pinned at the previous version.
///
/// Unix: atomic symlink swap with relative target (survives Docker
/// bind-mounts of `~/.fuigo/`). Windows: [`windows_replace_exe`].
///
...
async fn swap_managed_bin_links(
    ...
    let fuigo_name = if cfg!(windows) { "fuigo.exe" } else { "fuigo" };
    let agent_name = if cfg!(windows) { "agent.exe" } else { "agent" };
    let fuigo_link = bin_dir.join(fuigo_name);
    let agent_link = bin_dir.join(agent_name);
    let link_paths: [std::path::PathBuf; 2] = [fuigo_link.clone(), agent_link];
    ...
    rename_result.map_err(|e| {
        anyhow::anyhow!(
            "cannot rename locked executable {}: {e}\n\
             Close all running fuigo sessions and retry.",
            dest.display(),
        )
    })?;
    match tokio::fs::copy(src, dest).await {
...
async fn heal_managed_install(installer: &str) {
    ...
    #[cfg(any(unix, windows))]
    {
        ...
        #[cfg(unix)]
        reconcile_agent_to_fuigo(&bin_dir).await;

        #[cfg(windows)]
        reconcile_agent_exe_to_fuigo(&bin_dir).await;
    }
}

#[cfg(unix)]
async fn reconcile_agent_to_fuigo(bin_dir: &std::path::Path) {
    let fuigo_link = bin_dir.join("fuigo");
    let agent_link = bin_dir.join("agent");

    let Ok(fuigo_target) = tokio::fs::read_link(&fuigo_link).await else {
        return;
    };
    if tokio::fs::metadata(&fuigo_link).await.is_err() {
        return;
    }
    if let Ok(agent_target) = tokio::fs::read_link(&agent_link).await
        && agent_target == fuigo_target
    {
        return;
    }
    match atomic_symlink_swap(&fuigo_target, &agent_link).await {
        Ok(()) => tracing::info!(
            fuigo_target = %fuigo_target.display(),
            "reconciled agent bin symlink to fuigo target"
        ),
...
#[cfg(windows)]
async fn reconcile_agent_exe_to_fuigo(bin_dir: &std::path::Path) {
    let fuigo_exe = bin_dir.join("fuigo.exe");
    let agent_exe = bin_dir.join("agent.exe");

    if tokio::fs::metadata(&fuigo_exe).await.is_err() {
        return;
    }
    match agent_exe_differs(&fuigo_exe, &agent_exe).await {
        Ok(true) => {}
        Ok(false) => return,
        Err(e) => {
    ...
    match windows_replace_exe(&fuigo_exe, &agent_exe).await {
        Ok(()) => tracing::info!("reconciled agent.exe to fuigo.exe"),
        Err(e) => tracing::warn!("failed to reconcile agent.exe to fuigo.exe: {e:#}"),
    }
}
...
async fn agent_exe_differs(
    ...
    let fuigo_len = tokio::fs::metadata(fuigo).await?.len();
    match tokio::fs::metadata(agent).await {
        Ok(m) if m.len() != fuigo_len => return Ok(true),
        Ok(_) => {}
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => return Ok(true),
        Err(e) => return Err(e),
    }
    let mut rg = BufReader::new(tokio::fs::File::open(fuigo).await?);
...
async fn install_gh_release(target: Option<&str>) -> Result<()> {
    ...
    // Update fuigo-latest -> versioned binary so any existing symlinks that route
    // through it (e.g. /usr/local/bin/fuigo -> ~/.fuigo/downloads/fuigo-latest)
    // resolve to the newly installed version.
    #[cfg(unix)]
    {
        let latest_path = download_dir.join("fuigo-latest");
        ...
        if let Err(e) = atomic_symlink_swap(&rel_target, &latest_path).await {
            tracing::warn!("Failed to update fuigo-latest symlink: {e}");
        }
    }

    // Also update /usr/local/bin/{fuigo,agent} if either points directly into
    // ~/.fuigo/downloads/ (legacy layout — skips the fuigo-latest indirection).
    // Permission errors are ignored
    #[cfg(unix)]
    for name in ["fuigo", "agent"] {
        ...
        if let Ok(existing_target) = tokio::fs::read_link(&system_link).await {
            ...
            if target_str.contains(".fuigo/downloads/") && !target_str.ends_with("fuigo-latest") {
    ...
    cleanup_old_downloads(&download_dir, "fuigo-pager", &version).await;
...
/// Check if other fuigo processes are running (macOS only).
///
/// On macOS, `npm i -g` replaces the vendored binary in node_modules in-place.
/// Any fuigo process running from that vendored path will be SIGKILL'd by the kernel.
...
///
/// While our postinstall.js now uses versioned binaries under ~/.fuigo/bin/
...
#[cfg(target_os = "macos")]
fn warn_if_other_fuigo_processes_running() {
...
async fn refresh_deployment_config() {
    if !fuigo_shell::managed_config::has_principal() {
        return;
    }
    if !fuigo_shell::managed_config::is_fetch_enabled() {
        return;
    }
    // Clear a logged-out team's files before deciding to fetch (mirrors the loop).
    fuigo_shell::managed_config::clear_orphan();
    if !fuigo_shell::config::is_managed_config_stale_for(
        &fuigo_shell::managed_config::current_serving_identity(),
    ) {
        return;
    }
