...
use crate::version::get_installed_fuigo_version;
use semver::Version;
use tracing::warn;
use fuigo_shell::util::config::VersionPolicy;
...
pub(crate) enum VersionPolicyError {
    #[error(
        "Cannot install Fuigo {target}: the minimum allowed version is {minimum}. \
         Run `fuigo update` to install the latest allowed version."
    )]
