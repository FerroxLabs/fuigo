//! Unified PTY harness for fuigo-pager.
//!
...
//!    They run via `tests/` in this crate and via `pty-scenario` YAML under `fuigo-pager/tests/scenarios/`.
...
//! - **`scroll_matrix`**  — `FUIGO_SCROLL_LOG` JSONL ingestion for the scroll validation matrix.
...
impl PtyHarness {
    /// Inherit the parent environment for terminal/shell behavior tests (XTVERSION probes and fuigo wrap).
    ...
    /// Spawn from a canonical [`fuigo_test_support::TestSandbox`] baseline plus Set-only convenience overrides.
    pub fn new_in_sandbox(
        binary: &Path,
        rows: u16,
        cols: u16,
        args: &[&str],
        sandbox: &fuigo_test_support::TestSandbox,
        env: &[(&str, &str)],
        cwd: Option<&Path>,
    ) -> Result<Self> {
    ...
    /// # use fuigo_pager_pty_harness::{PtyHarness, ContentController, pager_binary};
    ...
    /// Feed bytes **directly into the virtual screen only**, bypassing the child (fuigo).
    ///
    /// Simulates an outer layer (tmux, or an nvim/vim `:terminal`) repainting the screen without going through fuigo's stdout.
    /// Used to reproduce the doubled-line class of bugs where fuigo's diff renderer never re-asserts a region it didn't write itself.
