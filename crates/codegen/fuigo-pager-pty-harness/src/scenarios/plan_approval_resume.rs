...
//! On `--continue` the shell re-issues the `fuigo/exit_plan_mode` reverse-request, a real live ACP waiter.
...
fn seed_parked_approval(home: &Path) -> Result<usize> {
    let sessions_root = home.join(".fuigo").join("sessions");
...
/// The shape mirrors `fuigo_shell::session::plan_mode::PlanModeSnapshot`.
