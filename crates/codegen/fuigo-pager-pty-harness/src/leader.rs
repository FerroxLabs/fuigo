...
//! One [`ContentController`] gives one shared `$HOME` (hence one elected leader) plus a fixed leader socket beneath its `FUIGO_HOME`.
...
impl LeaderCluster {
    /// Start the cluster: one [`ContentController`] (one shared `$HOME`, so one leader) and a fixed leader socket under its `FUIGO_HOME`.
    pub async fn start(rows: u16, cols: u16) -> Result<Self> {
        let content = ContentController::start()
            .await
            .context("start content controller")?;
        // One shared FUIGO_HOME means one leader; the socket lives beneath it so every client (sharing the same env) elects/attaches to the same one
        let fuigo_home = content.home().join(".fuigo");
        std::fs::create_dir_all(&fuigo_home).context("create fuigo home")?;
        let socket = fuigo_home.join("leader-e2e.sock");
    ...
    /// The cluster's sessions root: `FUIGO_HOME/sessions` (layout below is `sessions/<encoded-cwd>/<session-id>/updates.jsonl`).
    fn sessions_dir(&self) -> PathBuf {
        self.content.home().join(".fuigo").join("sessions")
    }
...
mod tests {
    ...
    fn envelope(update_json: &str) -> String {
        format!(
            r#"{{"timestamp":1,"method":"_fuigo/session/update","params":{{"sessionId":"s","update":{update_json}}}}}"#
        )
    }
    ...
    fn parse_update_payloads_unwraps_and_tolerates_torn_trailing_line() {
        let body = format!(
            "{}\n{}\n{{\"timestamp\":2,\"method\":\"_fuigo/sess",
            envelope(
