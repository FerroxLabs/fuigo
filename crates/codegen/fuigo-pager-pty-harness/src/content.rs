...
//! [`ContentController`] wraps the shared [`MockInferenceServer`] from `fuigo-test-support`.
...
pub use fuigo_test_support::mock_server::LogEntry;
pub use fuigo_test_support::mock_server::MockModelEntry as MockModel;
pub use fuigo_test_support::mock_server::StorageUpload;
pub use fuigo_test_support::sse;
pub use fuigo_test_support::{
...
impl ContentController {
    ...
    pub async fn start_with_models(models: Vec<MockModel>) -> Result<Self> {
        ...
        sandbox.set_env("FUIGO_PROMPT_SUGGESTIONS", "false");
    ...
    /// Isolated `$HOME` directory that the pager should use (keeps its ~/.fuigo
...
mod tests {
    ...
    async fn read_foreground(url: String, endpoint: InferenceEndpoint) -> String {
        ...
        reqwest::Client::new()
            ...
            .header("x-fuigo-turn-idx", "1")
            .header("x-fuigo-req-id", format!("direct-{endpoint:?}"))
            .json(&body)
            .send()
            .await
