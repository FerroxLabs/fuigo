//! Scripted TUI scenario runner for fuigo-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$FUIGO_HOME` before spawn.
...
impl ScriptedScenarioRunner {
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&fuigo_home)
                .with_context(|| format!("create scenario FUIGO_HOME {}", fuigo_home.display()))?;
            fs::write(fuigo_home.join("config.toml"), config_toml)
...
mod tests {
    ...
    fn workspace_rejects_paths_outside_the_tempdir() {
        ...
        let sandbox = fuigo_test_support::TestSandbox::new();
