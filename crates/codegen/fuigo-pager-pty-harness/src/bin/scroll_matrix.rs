...
use fuigo_pager_pty_harness::pager_binary;
use fuigo_pager_pty_harness::scroll_matrix::{
...
#[command(
    name = "scroll-matrix",
    about = "Run the scroll validation matrix against fuigo-pager",
    long_about = None,
)]
struct Cli {
    /// Cell tier to run.
    ...
    /// Pager binary.
    /// Defaults to PAGER_BINARY, CARGO_BIN_EXE_fuigo-pager, or a locally-built debug binary.
...
async fn run() -> Result<ExitCode> {
    ...
    // The pager child resolves FUIGO_SCROLL_LOG against ITS cwd (the harness's temp workspace)
