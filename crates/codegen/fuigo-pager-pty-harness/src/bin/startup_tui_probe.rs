...
enum BootMode {
    /// A first boot into an empty fuigo home.
    Cold,
...
async fn run(args: &Args) -> Result<Observation> {
    ...
    if args.mode == BootMode::Resume {
        ...
        resume_updates_bytes = largest_updates_log(content.sandbox().fuigo_home());
    }
    ...
    let log = content
        .sandbox()
        .fuigo_home()
        // `fuigo_telemetry::unified_log::LOG_DIR` is unreachable without a telemetry dependency.
        .join("logs")
        .join("unified.jsonl");
...
fn largest_updates_log(fuigo_home: &Path) -> u64 {
    let mut largest = 0;
    let mut pending = vec![fuigo_home.join("sessions")];
