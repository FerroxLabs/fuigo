...
/// Seed a fake Ferrox Labs OAuth entry into the isolated home's `auth.json` so the shell has session auth.
/// The harness's `FUIGO_API_KEY` is ApiKey/BYOK mode and never enters the auth manager.
...
/// `team_blocked_reasons` carries `BLOCKED_REASON_NO_LOGS`, the shell's `FuigoAuth::is_zdr_team` trigger.
...
/// Field names must match the shell's `FuigoAuth` serde names in `fuigo-shell/src/auth/model.rs`.
...
pub fn oauth_credential_ops() -> [crate::EnvOp<'static>; 1] {
    [crate::EnvOp::remove("FUIGO_API_KEY")]
}
