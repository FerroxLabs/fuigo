...
impl PtyController {
    /// Inherit the parent environment for terminal-brand probes, fuigo-wrap tests, and other fixtures that test inherited host env.
    ...
    fn spawn_inner(
        ...
        let process_tree = process_pid.map(|pid| TestProcessTree::attach(pid, "fuigo PTY child"));
...
const CLIPBOARD_SINK_ENV_VARS: &[&str] = &["FUIGO_OSC52_SINK", "LC_FUIGO_OSC52_SINK"];
...
///
/// The pager's terminal detection (`fuigo-pager-render/src/terminal/mod.rs` and `embedded_editor.rs`) reads all of these.
...
/// Content-backed callers provide a [`fuigo_test_support::TestSandbox`], which always clears inheritance.
/// The explicitly named inherited-env path is reserved for terminal probing and fuigo-wrap fixtures.
...
fn apply_child_env(cmd: &mut CommandBuilder, sandbox: Option<&TestSandbox>, env: &[EnvOp<'_>]) {
    ...
    // A harness launched under `fuigo wrap` must not silently confirm clipboard delivery for no-sink scenarios
...
mod tests {
    ...
    fn apply_child_env_strips_all_host_terminal_markers() {
        ...
        cmd.env("FUIGO_SCROLL_LOG", "/tmp/scroll.jsonl");
        let sandbox = TestSandbox::new();
        ...
        assert_eq!(
            cmd.get_env("FUIGO_SCROLL_LOG").and_then(|v| v.to_str()),
            None,
            "hermetic baseline must remove unrelated inherited vars"
        );
        assert_eq!(
            cmd.get_env("FUIGO_HOME").and_then(|v| v.to_str()),
            sandbox.fuigo_home().to_str()
        );
    }
    ...
    fn apply_child_env_uses_sandbox_baseline() {
        ...
        assert_eq!(cmd.get_env("FUIGO_LEADER_SOCKET"), None);
    }
    ...
    fn apply_child_env_remove_deletes_sandbox_credential() {
        ...
        apply_child_env(&mut cmd, Some(&sandbox), &[EnvOp::remove("FUIGO_API_KEY")]);
    ...
    fn apply_child_env_caller_env_overrides_survive_strips() {
        ...
        assert_eq!(
            cmd.get_env("FUIGO_OSC52_SINK").and_then(|v| v.to_str()),
            Some("1"),
