//! The scroll matrix drives the pager binary in a PTY with `FUIGO_SCROLL_LOG` pointed at a tempfile.
//! It then validates the pager's flight-recorder JSONL (producer: `fuigo-pager/src/input/scroll_log.rs`) against gesture invariants.
//!
...
//! The producer-side twin lives in `fuigo-pager/src/input/mouse/tests.rs` (a wire-format fixture test asserting the same key set on raw JSON).
