//! `FUIGO_SCROLL_LOG` JSONL parsing, per-stream grouping, and finalize synchronization.
//!
//! Wire schema source of truth: the pager's `ScrollLogRecord` in `fuigo-pager/src/input/scroll_log.rs`.
...
/// Parse a `FUIGO_SCROLL_LOG` JSONL file into records.
///
...
///
/// Expects the shape the pager emits when the recorder exists for the whole session (`FUIGO_SCROLL_LOG` set at spawn).
...
mod tests {
    ...
    // The lines are copied from `scroll_log_records_flood_flushes_and_capped_finalize_drop` in `fuigo-pager/src/input/mouse/tests.rs`
