...
pub struct ExpectedProfile {
    ...
    pub invert: bool,
    /// Speed multiplier (NOT the 1-100 setting): `FUIGO_SCROLL_SPEED=100` echoes 6.0 via the pager's `speed_to_multiplier`.
    pub speed: f32,
}
...
pub struct MatrixCell {
    ...
    /// The runner appends `FUIGO_SCROLL_LOG`; the harness's env strips guarantee the host terminal can't leak competing markers underneath these.
...
const SPEED100: (&str, &str) = ("FUIGO_SCROLL_SPEED", "100");
const MODE_WHEEL: (&str, &str) = ("FUIGO_SCROLL_MODE", "wheel");
const MODE_TRACKPAD: (&str, &str) = ("FUIGO_SCROLL_MODE", "trackpad");
const LINES1: (&str, &str) = ("FUIGO_SCROLL_LINES", "1");
const INVERT: (&str, &str) = ("FUIGO_INVERT_SCROLL", "1");
...
mod tests {
    ...
    /// Re-derive the expected profile from a cell's env the way the pager would: `from_terminal_context` plus the `FUIGO_SCROLL_*` env overrides.
    ...
    fn derive_expected(env: &'static [(&'static str, &'static str)]) -> ExpectedProfile {
        ...
        if let Some(lines) = get("FUIGO_SCROLL_LINES") {
        ...
        let speed = match get("FUIGO_SCROLL_SPEED") {
            None | Some("50") => 1.0,
            Some("100") => 6.0,
            Some(other) => panic!("unmapped FUIGO_SCROLL_SPEED {other:?}"),
        };
        ExpectedProfile {
            mode: get("FUIGO_SCROLL_MODE").unwrap_or("auto"),
            ept,
            wheel_lpt,
            trackpad_lpt,
            invert: get("FUIGO_INVERT_SCROLL") == Some("1"),
            speed,
        }
    }
