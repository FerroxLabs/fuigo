...
//! The pipeline runs auth.json, shell `FuigoAuth`, auth meta, `AppView::coding_data_sharing_lock()`, `PagerLocalSnapshot`, then the render:
//!
...
//!
//! Both accounts also suppress the welcome privacy banner even with `FUIGO_PRIVACY_NOTICE_ROLLOUT=1`.
//! That is asserted on the authenticated welcome screen before opening settings.
//! Row/input details are unit-tested in `fuigo-pager` (`views/settings_modal/tests.rs`, `locked_coding_*`).
//! This suite covers the auth-to-render pipeline.
//!
//! ```bash
//! cargo test -p fuigo-pager-pty-harness --test settings_locked_row_e2e \
//!   -- --ignored --nocapture
//! ```
...
const ROWS: u16 = 50;
const COLS: u16 = 120;
const BANNER_TITLE: &str = "Help improve Fuigo";
...
const DESCRIPTION_PREFIX: &str = "Opt-in to provide Ferrox Labs";
...
/// The sandbox's fake `FUIGO_API_KEY` is removed so the seeded team OAuth entry is the active auth.
...
fn locked_row_env_ops() -> [EnvOp<'static>; 3] {
    [
        EnvOp::set("FUIGO_PRIVACY_NOTICE_ROLLOUT", "1"),
        EnvOp::set("FUIGO_ZDR_ACCESS_ENABLED", "1"),
        EnvOp::remove("FUIGO_API_KEY"),
    ]
}
