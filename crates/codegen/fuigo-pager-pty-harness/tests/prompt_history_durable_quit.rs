...
//!
//! ```bash
//! cargo test -p fuigo-pager-pty-harness --test prompt_history_durable_quit \
//!   -- --ignored --nocapture
//! ```
...
/// The per-CWD history file lives at `<home>/.fuigo/sessions/<enc-cwd>/prompt_history.jsonl`.
fn find_prompt_history(home: &Path) -> Result<PathBuf> {
    let root = home.join(".fuigo").join("sessions");
