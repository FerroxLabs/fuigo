...
//! `#` notes need `[features] remember_mode = true`, which the run seeds into the sandbox `$FUIGO_HOME/config.toml`.

use std::time::Duration;

use anyhow::{Context, Result};
use fuigo_pager_pty_harness::{ContentController, PtyHarness, keys, pager_binary};
...
async fn run() -> Result<()> {
    ...
    std::fs::create_dir_all(&fuigo_home).context("create sandbox .fuigo")?;
    std::fs::write(
