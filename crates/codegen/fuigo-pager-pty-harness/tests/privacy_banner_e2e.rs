...
//! A seeded opted-out OAuth entry is the active auth (`FUIGO_API_KEY` removed) and the rollout is forced on via `FUIGO_PRIVACY_NOTICE_ROLLOUT=1`.
//!
//! ```bash
//! cargo test -p fuigo-pager-pty-harness --test privacy_banner_e2e \
//!   -- --ignored --nocapture
//! ```
...
/// The banner's two preconditions: force the rollout flag on (the env override beats remote settings) and remove the sandbox's fake `FUIGO_API_KEY`.
...
/// Poll `<home>/.fuigo/config.toml` for the async `privacy_banner_acked` write.
...
fn wait_for_ack_on_disk(pager: &mut PtyHarness, home: &Path, timeout: Duration) -> Result<()> {
    let path = home.join(".fuigo").join("config.toml");
