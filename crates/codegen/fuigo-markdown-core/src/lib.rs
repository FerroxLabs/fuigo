//! Headless markdown analysis sharing Fuigo's exact `pulldown-cmark` config.
//!
...
//! `fuigo-markdown` uses the same options, so analysis matches what Fuigo renders.
//!
//! After parsing, Fuigo applies [`offset_events`]: only `~~…~~` counts as strikethrough.
...
/// The exact `pulldown-cmark` option set Fuigo uses to render markdown.
///
...
/// Returns Fuigo's parser events with source byte ranges, single-tilde strikethrough already demoted.
///
...
/// Parse `text` with Fuigo's options; count elements and flag structural issues.
pub fn analyze(text: &str) -> MarkdownAnalysis {
    ...
    // `offset_events` attaches byte ranges and demotes single-tilde strike, so counts match what Fuigo renders
