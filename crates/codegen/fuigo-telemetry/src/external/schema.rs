...
impl ExternalEventName {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            Self::SessionStart => "fuigo_code.session_start",
            Self::SessionEnd => "fuigo_code.session_end",
            Self::UserPrompt => "fuigo_code.user_prompt",
            Self::TurnCompleted => "fuigo_code.turn_completed",
            Self::ApiRequest => "fuigo_code.api_request",
            Self::ApiError => "fuigo_code.api_error",
            Self::ToolResult => "fuigo_code.tool_result",
            Self::ToolDecision => "fuigo_code.tool_decision",
            Self::McpServerConnection => "fuigo_code.mcp_server_connection",
            Self::PermissionModeChanged => "fuigo_code.permission_mode_changed",
            Self::SkillActivated => "fuigo_code.skill_activated",
            Self::PluginLoaded => "fuigo_code.plugin_loaded",
            Self::Compaction => "fuigo_code.compaction",
            Self::Subagent => "fuigo_code.subagent",
            Self::Auth => "fuigo_code.auth",
            Self::InternalError => "fuigo_code.internal_error",
            Self::ModelSwitched => "fuigo_code.model_switched",
            Self::ContextualTip => "fuigo_code.contextual_tip",
        }
    }
}
...
pub enum MetricIncrement {
    ...
    /// `fuigo_code.turn.count`.
    TurnCount {
        outcome: &'static str,
        model: String,
    },
    /// `fuigo_code.tool.decision`.
    ToolDecision {
        tool_name: String,
        decision: &'static str,
    ...
    /// `fuigo_code.tool.usage`.
    ToolUsage {
        tool_name: String,
        outcome: &'static str,
    },
    /// `fuigo_code.error.count`.
    ErrorCount {
        error_category: String,
        model: String,
    },
    /// `fuigo_code.startup.timeout` (`stuck_in` is the phase label).
    StartupTimeout {
        stuck_in: String,
        auth_mode: String,
    },
    /// `fuigo_code.startup.phase_duration` (ms; `phase` is the phase label).
    ...
    /// `fuigo_code.startup.total` (ms from process start to a usable session).
    StartupTotal {
        duration_ms: u64,
        outcome: String,
        auth_mode: String,
...
pub(crate) const METRIC_SESSION_COUNT: &str = "fuigo_code.session.count";
pub(crate) const METRIC_TOKEN_USAGE: &str = "fuigo_code.token.usage";
pub(crate) const METRIC_COST_USAGE: &str = "fuigo_code.cost.usage";
pub(crate) const METRIC_TURN_COUNT: &str = "fuigo_code.turn.count";
pub(crate) const METRIC_TOOL_DECISION: &str = "fuigo_code.tool.decision";
pub(crate) const METRIC_TOOL_USAGE: &str = "fuigo_code.tool.usage";
pub(crate) const METRIC_ERROR_COUNT: &str = "fuigo_code.error.count";
pub(crate) const METRIC_STARTUP_PHASE_DURATION: &str = "fuigo_code.startup.phase_duration";
pub(crate) const METRIC_STARTUP_TIMEOUT: &str = "fuigo_code.startup.timeout";
pub(crate) const METRIC_STARTUP_TOTAL: &str = "fuigo_code.startup.total";
...
/// Built-in tool names (a closed enum in `fuigo-tools`) that pass verbatim through `tool_name` sanitization.
...
/// `SessionHarness` maps to `fuigo_code.session_start`.
...
/// `SessionNew` increments `fuigo_code.session.count` (metric only; the `session_start` log record comes from the richer `SessionHarness`).
...
/// `SessionEnded` maps to `fuigo_code.session_end`.
...
/// `PromptSubmitted` maps to `fuigo_code.user_prompt`.
...
/// `TurnCompleted` maps to `fuigo_code.turn_completed` and increments `turn.count` (and `error.count` on error outcomes).
...
/// `ModelResponseReceived` maps to `fuigo_code.api_request` and increments `token.usage`.
...
/// `RateLimitHit` maps to `fuigo_code.api_error` (`error_category = rate_limit`).
...
/// `ApiError` maps to `fuigo_code.api_error`.
...
/// `ToolCallCompleted` maps to `fuigo_code.tool_result` and increments `tool.usage`.
...
/// `PermissionDecisionPayload` maps to `fuigo_code.tool_decision` and increments `tool.decision`.
...
/// `McpServerConnected` maps to `fuigo_code.mcp_server_connection` (`status=connected`).
...
/// `McpServerFailed` maps to `fuigo_code.mcp_server_connection` (`status=failed`).
...
/// `PlanModeToggled` maps to `fuigo_code.permission_mode_changed`.
...
/// `ContextualTip` maps to `fuigo_code.contextual_tip`.
...
/// `YoloToggled` maps to `fuigo_code.permission_mode_changed`.
...
/// `SkillDispatched` maps to `fuigo_code.skill_activated`.
...
/// `PluginInstalled` maps to `fuigo_code.plugin_loaded`.
...
/// `PluginUsed` maps to `fuigo_code.plugin_loaded` (plugin name details-gated).
...
/// `CompactionCompleted` maps to `fuigo_code.compaction`.
...
/// `CompactionTriggered` is not mapped; the `fuigo_code.compaction` trigger attrs ride on the completion event instead (one event per compaction).
/// `SubagentLaunched` maps to `fuigo_code.subagent` (`phase=launched`).
...
/// `SubagentCompleted` maps to `fuigo_code.subagent` (`phase=completed`).
...
/// `InternalError` maps to `fuigo_code.internal_error`.
...
/// `ModelSwitched` maps to `fuigo_code.model_switched`.
