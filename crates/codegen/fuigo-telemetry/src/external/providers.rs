...
fn build_resource(cfg: &ExternalOtelConfig) -> opentelemetry_sdk::Resource {
    let mut attrs = vec![
        ...
        opentelemetry::KeyValue::new("fuigo_code.schema.version", super::schema::SCHEMA_VERSION),
    ];
    ...
    opentelemetry_sdk::Resource::builder_empty()
        // `fuigo-cli` is a wire commitment
...
fn grpc_tls_candidates(
    ...
    // Process-wide `FUIGO_EXTRA_CA_BUNDLE` roots (fail-open by that crate's contract), matching the HTTP transport's client policy
    // The same corporate CA must work on both transports
    if let Some(extra_pem) = ders_to_pem_bundle(fuigo_extra_ca::extra_root_ders()) {
...
mod tests {
    ...
    /// The DER-to-PEM re-encode used for `FUIGO_EXTRA_CA_BUNDLE` must produce a bundle other PEM parsers can read back, one block per DER.
    #[test]
