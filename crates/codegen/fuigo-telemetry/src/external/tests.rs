...
fn event_names_are_pinned() {
    ...
    let expected: &[(E, &str)] = &[
        (E::SessionStart, "fuigo_code.session_start"),
        (E::SessionEnd, "fuigo_code.session_end"),
        (E::UserPrompt, "fuigo_code.user_prompt"),
        (E::TurnCompleted, "fuigo_code.turn_completed"),
        (E::ApiRequest, "fuigo_code.api_request"),
        (E::ApiError, "fuigo_code.api_error"),
        (E::ToolResult, "fuigo_code.tool_result"),
        (E::ToolDecision, "fuigo_code.tool_decision"),
        (E::McpServerConnection, "fuigo_code.mcp_server_connection"),
        (
            E::PermissionModeChanged,
            "fuigo_code.permission_mode_changed",
        ),
        (E::SkillActivated, "fuigo_code.skill_activated"),
        (E::PluginLoaded, "fuigo_code.plugin_loaded"),
        (E::Compaction, "fuigo_code.compaction"),
        (E::Subagent, "fuigo_code.subagent"),
        (E::Auth, "fuigo_code.auth"),
        (E::InternalError, "fuigo_code.internal_error"),
        (E::ModelSwitched, "fuigo_code.model_switched"),
        (E::ContextualTip, "fuigo_code.contextual_tip"),
    ];
...
fn client_identifier_allowlist_is_pinned() {
    ...
    assert_eq!(
        schema::sanitize_client_identifier("fuigo-pager"),
        "fuigo-pager"
    );
    assert_eq!(
...
fn sentinel_session_harness() -> events::SessionHarness {
    events::SessionHarness {
        ...
        client_identifier: Some("fuigo-pager".into()),
        model_id: "grok-4".into(),
        agent_name: "fuigo-build-plan".into(),
...
fn session_start_snapshot_counts_not_names() {
    ...
    let ev = &events[0];
    assert_eq!(ev.0, "fuigo_code.session_start");
...
fn session_new_increments_session_count_only() {
    ...
    assert_eq!(names, vec!["fuigo_code.session.count".to_owned()]);
}
...
fn agent_connect_timeout_emits_phase_histogram_and_timeout_counter() {
    ...
    assert_eq!(
        names,
        vec![
            "fuigo_code.startup.phase_duration".to_owned(),
            "fuigo_code.startup.timeout".to_owned(),
        ]
    );
}
...
fn startup_completed_records_the_total_histogram_only() {
    ...
    assert_eq!(
        ...
        vec!["fuigo_code.startup.total".to_owned()]
    );
}
...
fn api_request_snapshot_and_token_usage() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "fuigo_code.api_request");
...
fn api_request_cost_and_cache_creation_export_attrs_and_metrics() {
    ...
    assert_eq!(
        names,
        vec!["fuigo_code.cost.usage", "fuigo_code.token.usage"],
...
fn one_failed_turn_increments_error_count_exactly_once() {
    ...
    let names: Vec<String> = exported_events(&stream)
        ...
        .collect();
    assert_eq!(
        names.iter().filter(|n| *n == "fuigo_code.api_error").count(),
        2
    );
    ...
    let total: u64 = stream
        ...
        .filter(|m| m.name() == "fuigo_code.error.count")
        .map(|m| match m.data() {
...
fn turn_error_increments_error_count() {
    ...
    let mut names = exported_metric_names(&stream);
    names.sort();
    assert_eq!(names, vec!["fuigo_code.error.count", "fuigo_code.turn.count"]);
}

#[test]
fn tool_result_hook_rewrote_is_content_free() {
    use fuigo_session_events::types::ToolOutcome;
...
fn tool_result_gates_off_collapses_and_reduces() {
    ...
    emit_event_into(
        ...
        &events::ToolCallCompleted {
            ...
            outcome: fuigo_session_events::types::ToolOutcome::Success,
            hook_rewrote: false,
            duration_ms: 42,
            tool_result_size_bytes: None,
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "fuigo_code.tool_result");
...
fn user_prompt_gates_off_drops_text() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "fuigo_code.user_prompt");
...
fn mcp_connection_collapses_server_name_by_default() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "fuigo_code.mcp_server_connection");
...
fn tool_decision_snapshot() {
    ...
    let events = exported_events(&stream);
    let ev = &events[0];
    assert_eq!(ev.0, "fuigo_code.tool_decision");
...
fn contextual_tip_maps_every_tip_and_action() {
    ...
    for (tip, action, tip_label, action_label) in cases {
        ...
        let events = exported_events(&stream);
        let ev = &events[0];
        assert_eq!(ev.0, "fuigo_code.contextual_tip");
...
/// The workspace-only events today are the fuigo-workspace sampler events.
...
fn validating_metric_exporter_drops_export_on_bad_attr_key() {
    ...
    let rogue = meter.u64_counter("fuigo_code.session.count").build();
    rogue.add(
        1,
...
fn redacting_log_exporter_drops_record_with_closed_gate_key() {
    ...
    record.set_event_name("fuigo_code.user_prompt");
...
fn redacting_log_exporter_drops_record_with_unknown_key() {
    ...
    let logger = stream.ext.logger.as_ref().unwrap();
    let mut record = logger.create_log_record();
    record.set_event_name("fuigo_code.api_request");
...
fn metric_increments_pass_model_through_scrub_and_attach_session_id() {
    ...
    let exported = stream.metrics.get_finished_metrics().unwrap();
    let blob = format!("{exported:?}");
    assert!(blob.contains("fuigo_code.token.usage"));
    assert!(
