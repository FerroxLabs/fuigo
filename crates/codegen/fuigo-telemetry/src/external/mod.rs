...
//!
//! Enterprise customers point the Fuigo CLI at *their own* OpenTelemetry collector through the standard `OTEL_*` env vars.
//! `FUIGO_EXTERNAL_OTEL` is the master switch.
...
//! - Default **off**: with `FUIGO_EXTERNAL_OTEL` unset (or no exporter selected) nothing is constructed; zero allocation, zero threads, zero sockets.
//! - Independent of `TelemetryMode`, GCS trace upload, and the user-confirmed data-collection and data-retention opt-outs.
//!   Those govern Ferrox Labs-side retention; this stream ships only to the customer's own collector under the customer's own explicit double opt-in.
//!
...
impl IdentityAttrs {
    pub fn from_snapshot(snapshot: &fuigo_auth::CredentialSnapshot) -> Self {
        Self {
    ...
    if cfg.internal_pipeline_consumed_otel_vars {
        tracing::warn!(
            "external otel: refusing to activate — the internal trace pipeline consumed \
             ...
             FUIGO_INTERNAL_OTLP_* to use the external stream."
        );
        return None;
    }
