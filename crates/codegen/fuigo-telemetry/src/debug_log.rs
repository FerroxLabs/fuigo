...
//! - PerSession (`FUIGO_DEBUG_LOG=1`): a routing layer fans each session's
//!   firehose to `~/.fuigo/debug/<session_id>.txt` (one file per session), with a
...
//! - SingleFile (explicit path via `FUIGO_LOG_FILE` or `FUIGO_DEBUG_LOG=<path>`): one flat `fmt` file, routing bypassed.
...
impl DebugSource {
    fn label(self) -> &'static str {
        match self {
            Self::FuigoLogFile => "FUIGO_LOG_FILE",
            Self::FuigoDebugLog => "FUIGO_DEBUG_LOG",
        }
    }
}
...
///
/// Lives here (not in `fuigo-pager`) so the firehose directives below and the pager's own filter are built from the same constants.
...
/// See `fuigo-pager/src/tracing.rs` for the consumer side.
...
const FIREHOSE_BASE_DIRECTIVES: &str = "info,fuigo_pager=debug,fuigo_shell=debug,fuigo_tools=debug,fuigo_telemetry=debug,fuigo_agent=debug,fuigo_mcp=debug,fuigo_session_search=debug,fuigo_acp_lib=debug,sampling_log=off";
...
// The broad firehose filter, used by both the routing layer and the FUIGO_DEBUG_LOG single-file source (mirrors `default_file_filter`)
fn firehose_filter() -> EnvFilter {
    EnvFilter::new(firehose_directives())
}

// RUST_LOG-respecting filter for the FUIGO_LOG_FILE source: DEBUG default, honor RUST_LOG, silence sampling_log
// This preserves FUIGO_LOG_FILE back-compat
...
/// SingleFile installs a flat `fmt` file picking the filter by source (FUIGO_LOG_FILE respects RUST_LOG).
...
pub(crate) enum DebugTarget {
    /// `FUIGO_DEBUG_LOG=1` → route per session into `<dir>` (`~/.fuigo/debug`).
    PerSession { dir: PathBuf },
    /// An explicit path writes one flat `fmt` file, routing bypassed.
    SingleFile { path: PathBuf, src: DebugSource },
}

/// Resolve the debug target, honoring precedence: explicit FUIGO_LOG_FILE wins
/// (single file, RUST_LOG filter); else FUIGO_DEBUG_LOG — a truthy bool routes
/// per session into `~/.fuigo/debug`, an explicit path writes a single file.
///
/// Read via `var_os` (not `var`) so a non-UTF-8 path isn't silently dropped.
pub(crate) fn resolve_debug_target() -> Option<DebugTarget> {
    let fuigo_log_file = std::env::var_os("FUIGO_LOG_FILE");
    let fuigo_debug_log = std::env::var_os("FUIGO_DEBUG_LOG");
    resolve_debug_target_inner(
        fuigo_log_file.as_deref(),
        fuigo_debug_log.as_deref(),
        &fuigo_home().join("debug"),
    )
}
...
fn resolve_debug_target_inner(
    fuigo_log_file: Option<&OsStr>,
    fuigo_debug_log: Option<&OsStr>,
    debug_dir: &Path,
) -> Option<DebugTarget> {
    if let Some(raw) = fuigo_log_file
        && !is_blank(raw)
    {
        return Some(DebugTarget::SingleFile {
            path: os_path(raw),
            src: DebugSource::FuigoLogFile,
        });
    }
    let raw = fuigo_debug_log?;
...
/// `~/.fuigo/debug` older than [`LOG_RETENTION`] so the dir doesn't grow
...
pub(crate) fn sweep_old_logs() {
    prune_old_logs(&fuigo_home().join("debug"), LOG_RETENTION);
}
...
mod tests {
        ...
        for v in ["0", "false", "off", "no", "", "  "] {
            assert!(
                resolve_debug_target_inner(None, Some(OsStr::new(v)), Path::new("/debug"))
                    .is_none(),
                "expected None for FUIGO_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_debug_log_enabled_is_per_session_dir() {
        for v in ["1", "true", "on", "yes"] {
            ...
            assert_eq!(
                ...
                "expected PerSession for FUIGO_DEBUG_LOG={v:?}"
            );
        }
    }
    ...
    fn resolve_target_empty_log_file_falls_through_to_debug_log() {
        // Empty or whitespace FUIGO_LOG_FILE is treated as unset (mirrors FUIGO_DEBUG_LOG)
        for blank in ["", "   "] {
            let target = resolve_debug_target_inner(
    ...
    fn resolve_target_non_utf8_debug_log_path_is_single_file() {
        ...
        // A non-UTF-8 FUIGO_DEBUG_LOG value is a path, not a bool keyword, and its bytes must round-trip (not be silently dropped)
        ...
        match target {
            DebugTarget::SingleFile { path, src } => {
                assert_eq!(src, DebugSource::FuigoDebugLog);
    ...
    fn layer_routes_event_by_session_span() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            tracing::info_span!("session", session_id = %"sess-xyz").in_scope(|| {
                tracing::info!(target: "fuigo_shell", "inside session");
            });
            tracing::info!(target: "fuigo_shell", "outside session");
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_under_real_firehose_filter() {
        ...
        // Pins at the unit level that the `session` span (INFO, target `fuigo_telemetry::session_ctx`) survives the real filter
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!(
                target: "fuigo_telemetry::session_ctx",
                "session",
                session_id = %"sid-real"
            )
            .in_scope(|| {
                tracing::debug!(target: "fuigo_shell", "filtered routing works");
            });
        });
        crate::appender::flush_file_log_guards();
    ...
    fn layer_routes_two_sessions_to_distinct_files() {
        ...
        tracing::subscriber::with_default(subscriber, || {
            tracing::info_span!("session", session_id = %"sid-one").in_scope(|| {
                ...
                tracing::info!(target: "fuigo_shell", "one first");
                tracing::info!(target: "fuigo_shell", "one second");
            });
            tracing::info_span!("session", session_id = %"sid-two").in_scope(|| {
                tracing::info!(target: "fuigo_shell", "two only");
            });
        });
        crate::appender::flush_file_log_guards();
