...
//! Pager and desktop forward entries over ACP (`fuigo/log` notifications).
...
pub const LOG_METHOD: &str = "fuigo/log";
...
pub enum LogSource {
    ...
    #[serde(rename = "shell")]
    Shell,
    #[strum(serialize = "fuigo-pager")]
    #[serde(rename = "fuigo-pager")]
    FuigoPager,
    #[strum(serialize = "fuigo-desktop")]
    #[serde(rename = "fuigo-desktop")]
    FuigoDesktop,
}
...
/// Wire format for the `fuigo/log` ACP notification params.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// `~/.fuigo/logs/unified.jsonl` (those bursts inflate exactly the counters
...
fn log_path() -> PathBuf {
    ...
    fuigo_home().join(LOG_DIR).join(LOG_FILE)
}
...
///
/// Split from [`open_writer`] so a writer re-points at **its own** path when healing a stale handle rather than re-resolving `$FUIGO_HOME`.
...
///
/// Called by the `fuigo/log` notification handler.
...
mod tests {
    ...
    /// writer onto the developer's real `~/.fuigo/logs/unified.jsonl`.
    #[ctor::ctor]
    fn redirect_for_tests() {
        redirect_to_temp_for_tests();
    }

    /// The redirect must cover both the writer and the snapshot readers: an emit lands in a per-process temp file, never under `fuigo_home()`.
    #[test]
