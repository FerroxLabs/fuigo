...
//!
//! Extracted from `fuigo-shell` so binaries (TUI, sampler) can reuse them without the shell's HTTP product-analytics client.
...
///
/// Volume caveat: one-shot `fuigo update` resolves telemetry from disk and env only.
...
pub enum CliUpdateTrigger {
    /// A human ran `fuigo update` or accepted an update prompt.
    UserCommand,
...
/// `method` is "fuigo" or "api_key"; `mode` is "device", "loopback", or "api_key".
#[derive(Serialize)]
...
/// `method` is "fuigo" or "api_key"; `mode` is the resolved auth mode.
...
impl CompactionScope {
    pub fn begin(params: CompactionBeginParams) -> Self {
        ...
        let percentage = fuigo_token_estimation::usage_percentage_u8(tokens_used, context_window);
...
pub struct SubagentCompleted {
    ...
    // Spawn-phase durations (`crate::subagent_spawn`, the `fuigo_code_subagent_spawn_*` taxonomy); absent when a phase did not run
...
pub struct ToolCallCompleted {
    pub tool_name: String,
    pub outcome: fuigo_session_events::types::ToolOutcome,
...
// ---------------------------------------------------------------------------
// Pager events (called from fuigo-pager via log_event)
// ---------------------------------------------------------------------------
...
pub struct ClipboardCopy {
    ...
    /// An explicit `fuigo wrap` OSC 52 sink was active.
    pub osc52_sink: bool,
...
///
/// Alerting contract: the event lands under the Shell-origin name `fuigo-shell-manual_auth`.
/// The `manual_auth` binding gets the `fuigo-shell-` prefix at emit.
...
/// One attempt to download and activate a new `fuigo` binary.
/// Analytics name: `fuigo-shell-cli_update`.
...
/// Whether `fuigo clone` finished the mount.
#[derive(Serialize, Clone, Copy, Debug, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
...
/// Where a failed `fuigo clone` stopped.
...
/// One `fuigo clone` attempt. Content-free: no URL, dest, store, or repo name.
#[derive(Serialize, Debug, Clone, PartialEq)]
