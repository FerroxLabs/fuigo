...
//! file logger that writes to `~/.fuigo/logs/hooks.log`.
//!
//! ## When to use
//!
//! Use regular `tracing::info!` / `tracing::debug!` / `tracing::warn!` with targets `fuigo_hooks` or `fuigo_agent::plugins`.
//! Log at discovery, dispatch, execution, and error points.
//!
//! ## Enabling
//!
//! ```bash
//! FUIGO_HOOKS_LOG=1 fuigo              # enable, write to ~/.fuigo/logs/hooks.log
//! FUIGO_HOOKS_LOG=/tmp/h.log fuigo     # write to custom path
//! FUIGO_HOOKS_LOG=0 fuigo              # explicitly disable
//! tail -f ~/.fuigo/logs/hooks.log     # watch in another terminal
//! ```
...
const ENV_HOOKS_LOG: &str = "FUIGO_HOOKS_LOG";
...
/// Writes to `~/.fuigo/logs/hooks.log` (or custom path via `FUIGO_HOOKS_LOG`).
/// Filters to hooks (`fuigo_hooks`) and plugins (`fuigo_agent::plugins`) targets.
/// Set `FUIGO_HOOKS_LOG=0` to disable, `FUIGO_HOOKS_LOG=/path` to redirect.
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let filter = tracing_subscriber::filter::EnvFilter::new(
        "fuigo_hooks=debug,fuigo_agent::plugins=debug",
    );
...
fn resolve_log_path() -> Option<PathBuf> {
    let default_path = || fuigo_home().join("logs").join("hooks.log");
