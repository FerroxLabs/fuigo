...
pub enum EmitterOrigin {
    /// `fuigo-shell` (and the pager/TUI that emit through it).
    Shell,
    /// `fuigo-workspace` (remote sampler / workspace server).
    Workspace,
}
...
impl EmitterOrigin {
    ...
    pub fn event_prefix(self) -> &'static str {
        match self {
            EmitterOrigin::Shell => "fuigo-shell-",
            EmitterOrigin::Workspace => "fuigo-workspace-",
        }
    }
}
...
/// Fires in both `Enabled` and `SessionMetrics` modes; the origin selects the analytics event-name prefix (`fuigo-shell-*` vs `fuigo-workspace-*`).
...
mod tests {
    ...
    /// What a command exiting right after emitting (`fuigo login`) relies on.
    ...
    fn event_prefix_is_stable_per_origin() {
        assert_eq!(EmitterOrigin::Shell.event_prefix(), "fuigo-shell-");
        assert_eq!(EmitterOrigin::Workspace.event_prefix(), "fuigo-workspace-");
    }

    /// The `Shell` reroute must reproduce the historical `format!("fuigo-shell-{suffix}")` event name byte-for-byte.
    ...
    fn shell_origin_event_name_matches_legacy_format() {
        ...
        let legacy = format!("fuigo-shell-{suffix}");
    ...
    fn workspace_origin_event_name_uses_workspace_prefix() {
        ...
        assert_eq!(name, "fuigo-workspace-turn");
    }
