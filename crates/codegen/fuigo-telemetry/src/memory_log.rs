//! Provides a dedicated tracing target (`fuigo_memory`) with an optional
//! file logger that writes to `~/.fuigo/logs/memory.log`.
//!
//! ## When to use
//!
...
//! FUIGO_MEMORY_LOG=0 fuigo                # disable even when enabled
//! tail -f ~/.fuigo/logs/memory.log      # watch in another terminal
//! ```

/// Tracing target for all memory system operations.
pub const TARGET: &str = "fuigo_memory";
...
mod inner {
    ...
    const ENV_MEMORY_LOG: &str = "FUIGO_MEMORY_LOG";
    ...
    /// Writes to `~/.fuigo/logs/memory.log`. Filters to `fuigo_memory=trace`.
    /// Set `FUIGO_MEMORY_LOG=0` to disable, `FUIGO_MEMORY_LOG=/path` to redirect.
    ...
    fn resolve_log_path() -> Option<PathBuf> {
        let default_path = || fuigo_home().join("logs").join("memory.log");
