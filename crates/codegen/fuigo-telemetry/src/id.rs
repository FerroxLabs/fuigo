...
const ENV_AGENT_ID: &str = "FUIGO_AGENT_ID";

static AGENT_ID: OnceLock<String> = OnceLock::new();
static AGENT_INSTANCE_ID: OnceLock<String> = OnceLock::new();

/// Returns the stable agent ID: `FUIGO_AGENT_ID` if set, else the value cached in `$FUIGO_HOME/agent_id`.
...
fn load_or_compute_agent_id() -> String {
    ...
    let cache_path = fuigo_config::fuigo_home().join("agent_id");
...
fn write_agent_id_cache(path: &std::path::Path, id: &str) -> std::io::Result<()> {
    ...
    fuigo_config::fs_atomic::write_atomically(path, id, Some(0o600))
}
...
/// Coarse gate for features that need a full workspace checkout; external installs leave `FUIGO_ROOT` and `FUIGO_USER` unset.
pub fn has_workspace_env_markers() -> bool {
    std::env::var("FUIGO_ROOT").is_ok() && std::env::var("FUIGO_USER").is_ok()
}

/// Opt-in special-user gate for telemetry (`FUIGO_TELEMETRY_SPECIAL_USER`).
pub fn is_special_user() -> bool {
    matches!(
        std::env::var("FUIGO_TELEMETRY_SPECIAL_USER").as_deref(),
