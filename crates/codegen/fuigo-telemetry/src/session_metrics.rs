...
impl TraceUploadReason {
    ...
    pub fn from_upload_method(method: &Option<fuigo_file_utils::UploadMethod>) -> Self {
        match method {
            Some(fuigo_file_utils::UploadMethod::Proxy { .. }) => Self::Proxy,
            Some(fuigo_file_utils::UploadMethod::S3 { .. }) => Self::DirectS3,
            Some(fuigo_file_utils::UploadMethod::Direct { .. }) => Self::DirectGcs,
...
mod tests {
    ...
    /// The `fuigo-shell-doom_loop_recovery` Mixpanel event's name and property keys are dashboard contracts; pin them.
    #[test]
