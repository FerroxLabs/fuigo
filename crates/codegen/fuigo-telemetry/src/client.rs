...
/// Handles both CCP display names ("SuperGrok Heavy") and JWT-derived keys ("superfuigo_heavy").
fn normalize_tier(tier: &str) -> String {
    match tier {
        "SuperGrok Heavy" | "superfuigo_heavy" => "superfuigo_heavy",
        "SuperGrok Plus" | "superfuigo_plus" => "superfuigo_plus",
        "SuperGrok" | "superfuigo" => "superfuigo",
        "SuperGrok Lite" | "superfuigo_lite" => "superfuigo_lite",
...
impl EventEnrichment {
    fn capture() -> Self {
        ...
        Self {
            ...
            dev_build: fuigo_version::IS_DEV_BUILD,
...
mod tests {
    ...
    /// Shell events must still strip to their bare suffix, byte-for-byte identical to the previous `strip_prefix("fuigo-shell-")` behavior.
    #[test]
    fn event_value_strips_shell_prefix() {
        assert_eq!(event_value("fuigo-shell-turn"), "turn");
        assert_eq!(
            event_value("fuigo-shell-trace_upload_attempted"),
    ...
    fn event_value_strips_workspace_prefix() {
        assert_eq!(event_value("fuigo-workspace-turn"), "turn");
    }
    ...
    fn event_value_strips_only_leading_prefix() {
        assert_eq!(event_value("fuigo-shell-workspace-x"), "workspace-x");
    }
    ...
    fn normalize_tier_maps_display_and_claim_names() {
        ...
        assert_eq!(normalize_tier("free"), "free");
        assert_eq!(normalize_tier("SuperGrok"), "superfuigo");
        assert_eq!(normalize_tier("SuperGrok Heavy"), "superfuigo_heavy");
        assert_eq!(normalize_tier("superfuigo_heavy"), "superfuigo_heavy");
        ...
        assert_eq!(normalize_tier("SuperGrok Lite"), "superfuigo_lite");
        assert_eq!(normalize_tier("SuperGrok Plus"), "superfuigo_plus");
        assert_eq!(normalize_tier("superfuigo_plus"), "superfuigo_plus");
    ...
    /// A future origin like `"fuigo-shell-ext-"` would let an earlier `ALL` entry strip the shorter prefix first and yield the wrong `event_value`.
