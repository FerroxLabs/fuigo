...
struct CpuSample {
    now: Instant,
    cpu: fuigo_tty_utils::ProcessCpu,
...
pub fn snapshot() -> ProcessMetrics {
    ...
    let sample = {
        ...
        let now = Instant::now();
        let cpu = fuigo_tty_utils::sample_process_cpu();
    ...
    let first = FIRST_SNAPSHOT.get_or_init(|| FirstSnapshot {
        at: sample.now,
        process_age_secs: fuigo_tty_utils::process_start_time()
    ...
    let memory = fuigo_tty_utils::sample_process_memory();
    ...
    ProcessMetrics {
        ...
        memory_limit_bytes: fuigo_tty_utils::process_memory_limit(),
