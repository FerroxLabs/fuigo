//! Tracing layer for `target: "sampling_log"` → `~/.fuigo/logs/sampling.jsonl`.
//! Enable with `--log-sampling` or `FUIGO_LOG_SAMPLING=1`.
...
const ENV_VAR: &str = "FUIGO_LOG_SAMPLING";
    ...
    S: Subscriber + for<'span> LookupSpan<'span> + Send + Sync + 'static,
{
    ...
    let path = fuigo_home().join(crate::unified_log::LOG_DIR).join(LOG_FILE);
