...
//!
//! `fuigo-pager` uses this module to set up OTLP trace export.
...
const ENV_OTEL_FILTER: &str = "FUIGO_OTEL_FILTER";
const DEFAULT_OTEL_FILTER: &str = "info";
/// Runtime values for [`build_otel_layer`].
/// Passing them in keeps this crate free of shell-internal types (`AuthManager`, `EndpointsConfig`, `FuigoComConfig`).
///
/// The binaries (`fuigo-pager`) build this from their own configuration.
/// The credentials provider is constructed by shell's `fuigo_shell::auth::credential_provider::build_otel_credential_provider`.
...
pub struct OtelClientInfo {
    /// Binary name (`fuigo-pager`), exported as `client.name`.
...
///
/// - `client_name`: binary name (e.g. `"fuigo-tui"`, `"fuigo-pager"`), stored as `client.name` for dashboards to distinguish client types.
/// - `client_version`: `CARGO_PKG_VERSION`, sent in the `x-fuigo-client-version` header.
    ...
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    let tracer = provider.tracer("fuigo-cli");
...
fn build_server_provider(client: OtelClientInfo, config: OtelLayerConfig) -> SdkTracerProvider {
    ...
    if config.exporter.enabled {
        ...
        static_headers.insert(
            "x-fuigo-client-version".to_string(),
