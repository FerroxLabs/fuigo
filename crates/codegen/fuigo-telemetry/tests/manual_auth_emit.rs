...
//! Wire test: `log_event(ManualAuth)` must POST to the product events endpoint as `fuigo-shell-manual_auth`.
...
use fuigo_telemetry::client;
use fuigo_telemetry::config::{TelemetryConfig, TelemetryMode};
...
async fn manual_auth_posts_to_events_endpoint_as_fuigo_shell_manual_auth() {
    ...
    fuigo_telemetry::log_event(ManualAuth {
    ...
    let event = loop {
        let found = bodies.lock().unwrap().iter().find_map(|b| {
            ...
            (e.get("event_name")?.as_str()? == "fuigo-shell-manual_auth").then(|| e.clone())
        });
        if let Some(e) = found {
            break e;
        }
        assert!(
            ...
            "no fuigo-shell-manual_auth POST received"
        );
    ...
    for (key, expected) in [
        ...
        (
            "dev_build",
            serde_json::json!(fuigo_version::IS_DEV_BUILD),
        ),
    ...
    for key in fuigo_telemetry::client::RESERVED_EVENT_KEYS {
        assert!(
