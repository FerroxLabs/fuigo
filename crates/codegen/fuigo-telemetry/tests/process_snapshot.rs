...
use fuigo_telemetry::events::ShellTrueNoop;
use fuigo_telemetry::{process_metrics, session_ctx};

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn a_gated_emit_takes_no_snapshot_and_the_second_snapshot_reports_cpu() {
    assert!(
        !fuigo_telemetry::is_enabled(),
        "this binary must never install a telemetry client"
    );
    fuigo_telemetry::log_event(ShellTrueNoop {
        tool_name: "bash".into(),
    });
