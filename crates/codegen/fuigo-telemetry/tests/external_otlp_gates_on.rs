...
use fuigo_telemetry::external::{self, ExternalOtelRemotePolicy, IdentityAttrs};
...
fn external_stream_gates_on_end_to_end() {
    ...
    assert!(!fuigo_telemetry::is_enabled());

    fuigo_telemetry::log_event(fuigo_telemetry::events::SessionHarness {
        ...
        memory_retrieval_mode: fuigo_telemetry::events::MemoryRetrievalMode::Disabled,
        is_git_repo: true,
        auto_update: None,
    });
    fuigo_telemetry::log_event(fuigo_telemetry::events::PromptSubmitted {
        prompt_length: 100,
        model_id: "grok-4".into(),
    ...
    fuigo_telemetry::log_event(fuigo_telemetry::events::ModelResponseReceived {
    ...
    fuigo_telemetry::log_event(fuigo_telemetry::events::ToolCallCompleted {
    ...
    let harness = col::find_event(&collected, "fuigo_code.session_start")
        .expect("session_start must be present");
    assert_eq!(harness.scope_name, "ai.fuigo.code");
    assert_eq!(
        harness
            .resource
            .get("service.name")
            .and_then(|v| v.as_str()),
        Some("fuigo-cli"),
        "service.name=fuigo-cli is a wire commitment"
    );
    assert_eq!(
        harness
            .resource
            .get("fuigo_code.schema.version")
            .and_then(|v| v.as_str()),
        Some("v1")
    );
    ...
    let prompt = col::find_event(&collected, "fuigo_code.user_prompt").expect("user_prompt present");
    ...
    let tool = col::find_event(&collected, "fuigo_code.tool_result").expect("tool_result present");
    assert_eq!(
    ...
    let tokens = col::find_metric(&collected, "fuigo_code.token.usage");
    ...
    let sessions = col::find_metric(&collected, "fuigo_code.session.count");
