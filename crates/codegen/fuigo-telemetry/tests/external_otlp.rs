...
fn external_stream_end_to_end() {
    ...
    let mut cfg = fuigo_telemetry::external::ExternalOtelConfig::resolve_with(
        |name| match name {
    ...
    cfg.client = fuigo_telemetry::external::config::ExternalClientInfo {
        service_version: "0.0.0-test".into(),
        client_version: "0.0.0-test".into(),
        app_entrypoint: "cli".into(),
    };

    fuigo_telemetry::external::init(Some(cfg));
    assert!(fuigo_telemetry::external::is_active());
    ...
    assert!(
        resource_service_name
            .as_deref()
            .is_some_and(|s| s.contains("fuigo-cli")),
        "service.name=fuigo-cli is a wire commitment: {resource_service_name:?}"
    );
    for expected in [
        "fuigo_code.session_start",
        "fuigo_code.user_prompt",
        "fuigo_code.api_request",
    ] {
        assert!(
    ...
    assert_eq!(
        event_names
            .iter()
            .filter(|n| *n == "fuigo_code.session_start")
            .count(),
        1
    );
            ...
            for sm in &rm.scope_metrics {
                for metric in &sm.metrics {
                    ...
                    if let Some(Data::Sum(sum)) = &metric.data {
                        ...
                        if metric.name == "fuigo_code.session.count" {
    ...
    assert!(
        metric_names.iter().any(|n| n == "fuigo_code.session.count"),
        "missing session.count in {metric_names:?}"
    );
    assert!(metric_names.iter().any(|n| n == "fuigo_code.token.usage"));
    assert_eq!(
    ...
    fuigo_telemetry::external::shutdown();
    assert!(
