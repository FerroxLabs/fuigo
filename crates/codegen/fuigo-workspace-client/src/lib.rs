...
//! `WorkspaceOps` proxy mode and consumers that cannot depend on `fuigo-workspace` both use it.
//! Wire types live in `fuigo_workspace_types::rpc`.
...
use fuigo_computer_hub_sdk::harness::ToolHarness;
use fuigo_workspace_types::rpc::agents_md::{AgentConfigFile, DiscoverAgentsMdReq};
use fuigo_workspace_types::rpc::code_nav::{
...
use fuigo_workspace_types::rpc::export_github::{ExportGithubReq, ExportGithubResponse};
use fuigo_workspace_types::rpc::fs::{
...
use fuigo_workspace_types::rpc::git::{
...
use fuigo_workspace_types::rpc::hunks::{
...
use fuigo_workspace_types::rpc::search::{
    ContentSearchData, ContentSearchRequest, FuzzyChangeReq, FuzzyCloseReq, FuzzyOpenReq,
    FuzzyStatusReq,
};
use fuigo_workspace_types::rpc::session::{
    BeginPromptReq, EndPromptReq, FileRewindResponse, RewindToReq,
};
use fuigo_workspace_types::rpc::skills::{DiscoverPluginsReq, DiscoverSkillsReq, SkillInfo};
use fuigo_workspace_types::rpc::workspace::{
...
use fuigo_workspace_types::rpc::worktree::{
...
use fuigo_workspace_types::rpc::{RpcEnvelope, RpcError, WORKSPACE_RPC_TOOL_ID, WorkspaceRpc};
...
pub async fn consume_stream_terminal(
    stream: &mut fuigo_tool_runtime::ToolStream<TypedToolOutput>,
...
/// Check whether a [`ToolError`](fuigo_tool_runtime::ToolError) indicates a fatal transport failure that should mark the hub as disconnected.
///
/// Returns `true` for:
...
pub fn is_transport_fatal(err: &fuigo_tool_runtime::ToolError) -> bool {
    match err.kind {
        fuigo_tool_runtime::ToolErrorKind::NetworkError => true,
        fuigo_tool_runtime::ToolErrorKind::Custom => err
            .details
            .as_ref()
...
fn is_non_retryable_workspace_unavailable(err: &fuigo_tool_runtime::ToolError) -> bool {
    if !matches!(err.kind, fuigo_tool_runtime::ToolErrorKind::Custom) {
        return false;
    }
    err.details
        .as_ref()
        .and_then(|d| {
            ...
            fuigo_tool_protocol::WorkspaceUnavailableDetails::deserialize(d).ok()
        })
        .is_some_and(|d| d.code == fuigo_tool_protocol::WORKSPACE_UNAVAILABLE_SUBCODE && !d.retryable)
}
...
impl WorkspaceClient {
    ...
    pub async fn rpc_raw(
        ...
        let tool_id = fuigo_tool_protocol::ToolId::new(WORKSPACE_RPC_TOOL_ID)
...
mod tests {
    ...
    use fuigo_computer_hub_sdk::harness::LocalRegistry;
    use fuigo_workspace_types::rpc::RpcActivityClass;
    use fuigo_tool_protocol::{SessionId, ToolId};
    use fuigo_tool_runtime::{Tool, ToolError};
    ...
    impl fuigo_tool_runtime::ToolOutput for RawOut {}
    #[derive(Debug)]
        ...
        async fn run(&self, _ctx: ToolCallContext, args: Self::Args) -> Result<RawOut, ToolError> {
            ...
            match args.method.as_str() {
                ...
                "workspace.hibernated" => Err(workspace_gone_tool_error(
                    fuigo_tool_protocol::WorkspaceGoneReason::Hibernated,
                    fuigo_tool_protocol::WorkspaceGonePhase::RouteMissing,
                )),
                "workspace.gone_retryable" => Err(workspace_gone_tool_error(
                    fuigo_tool_protocol::WorkspaceGoneReason::NotBound,
                    fuigo_tool_protocol::WorkspaceGonePhase::RouteMissing,
                )),
    ...
    fn workspace_gone_tool_error(
        reason: fuigo_tool_protocol::WorkspaceGoneReason,
        phase: fuigo_tool_protocol::WorkspaceGonePhase,
    ) -> ToolError {
        let fuigo_tool_protocol::ToolErrorWire::Custom {
            subcode,
            message,
            details,
        } = fuigo_tool_protocol::workspace_unavailable_wire(reason, phase)
        else {
    ...
    async fn consume_stream_terminal_returns_err() {
        let mut stream: fuigo_tool_runtime::ToolStream<TypedToolOutput> =
            fuigo_tool_runtime::terminal_only(Err(ToolError::network_error("oops")));
    ...
    async fn consume_stream_terminal_exhausted_stream_is_network_error() {
        ...
        let mut stream = fuigo_tool_runtime::terminal_only::<TypedToolOutput>(Ok(typed));
