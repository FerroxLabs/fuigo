...
const CHILD_WORKTREE: &str = "FUIGO_FAST_WORKTREE_SAFETY_CHILD_WORKTREE";
const CHILD_SOURCE: &str = "FUIGO_FAST_WORKTREE_SAFETY_CHILD_SOURCE";
...
fn no_combination_of_working_tree_shapes_loses_uncarried_work() {
    ...
    for subset in 0..(1u8 << shapes.len()) {
        ...
        let ref_name = format!("refs/fuigo/subagents/{name}");
...
fn reclaiming_a_worktree_runs_none_of_its_hooks() {
    ...
    snapshot_into(&worktree, &fixture.source, "refs/fuigo/subagents/has-hooks");
...
fn snapshot_under_a_foreign_clean_filter() {
    ...
    let ref_name = "refs/fuigo/subagents/inherits-a-filter";
