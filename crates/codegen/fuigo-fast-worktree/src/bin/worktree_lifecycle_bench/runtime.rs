...
impl CgroupV2 {
    pub fn create(label: &str) -> Result<Option<Arc<Self>>> {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            let path = parent.join(format!("fuigo-lifecycle-{label}"));
    ...
    pub fn support_available() -> bool {
        ...
        let ok = wait_for_stopped_child(pid).is_ok()
            ...
            && fuigo_tty_utils::wait_child_bounded(&mut child, CHILD_KILL_REAP_TIMEOUT)
        ...
        if !ok {
            ...
            let _ = fuigo_tty_utils::wait_child_bounded(&mut child, CHILD_KILL_REAP_TIMEOUT);
        }
        ok
    }
...
struct ActiveCommand {
    group: Arc<fuigo_tty_utils::ProcessGroup>,
...
fn run_contained_command_impl(
    ...
    let scope = fuigo_tty_utils::ProcessScope::new();
    ...
    let status = match fuigo_tty_utils::wait_child_bounded(&mut child, deadline) {
        Ok(Some(status)) => status,
        Ok(None) => {
            let mut cleanup_errors = Vec::new();
            attempt_command_teardown(cgroup.as_deref(), &group, &mut cleanup_errors);
            let reap = fuigo_tty_utils::wait_child_bounded(&mut child, CHILD_KILL_REAP_TIMEOUT);
            if !matches!(reap, Ok(Some(_))) {
                ...
                let _ = fuigo_tty_utils::spawn_child_reaper(
        ...
        Err(error) => {
            let mut cleanup_errors = Vec::new();
            attempt_command_teardown(cgroup.as_deref(), &group, &mut cleanup_errors);
            if !fuigo_tty_utils::is_child_wait_identity_uncertain(&error) {
                let reap = fuigo_tty_utils::wait_child_bounded(&mut child, CHILD_KILL_REAP_TIMEOUT);
                if !matches!(reap, Ok(Some(_))) {
                    cleanup_errors.push(match &reap {
...
fn kill_and_reap_unenrolled_child(child: &mut std::process::Child) {
    ...
    let _ = fuigo_tty_utils::wait_child_bounded(child, CHILD_KILL_REAP_TIMEOUT);
}
