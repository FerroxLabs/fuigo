...
pub const BACKING_MARKER_FILE: &str = "fuigo-nfs-worktree.json";
...
pub fn candidate_data_dirs() -> Vec<PathBuf> {
    ...
    if let Some(fuigo_home) = fuigo_dirs::resolve_fuigo_home() {
        push(fuigo_home.join("grove"));
    }
    dirs
}
...
pub fn gc_orphan_pins(
    ...
    for idn in identities.values() {
        ...
        let pin = format!("refs/fuigo/worktrees/{}", idn.worktree_id);
...
mod tests {
    ...
    fn identities_prefer_metadata_grove_over_legacy_nfs() {
        ...
        rec.metadata = Some(serde_json::json!({
            "grove": {
                ...
                "source_pin": "refs/fuigo/worktrees/wt-grove"
            },
            "nfs": {
                "backing": "/legacy/should-not-win",
                "source_pin": "refs/fuigo/worktrees/legacy"
            }
        }));
        ...
        assert_eq!(
            ...
            Some("refs/fuigo/worktrees/wt-grove")
        );
    }
    #[test]
    ...
    fn db_loss_then_source_gc_keeps_pin_via_union_liveness() {
        ...
        fuigo_tty_utils::detach_std_command(&mut reset);
        assert!(
            reset
                .current_dir(&repo)
        ...
        let pin = "refs/fuigo/worktrees/wt-live";
        let mut uref = std::process::Command::new("git");
        fuigo_tty_utils::detach_std_command(&mut uref);
        assert!(
        ...
        fuigo_tty_utils::detach_std_command(&mut gc);
        assert!(
            gc.current_dir(&repo)
                .args(["gc", "--prune=now"])
                .status()
        ...
        fuigo_tty_utils::detach_std_command(&mut cat);
        let cat_out = cat
            .current_dir(&repo)
    ...
    fn aborted_partial_removal_prunes_after_grace() {
        ...
        let pin = "refs/fuigo/worktrees/wt-orphan";
        let mut uref = std::process::Command::new("git");
        fuigo_tty_utils::detach_std_command(&mut uref);
        assert!(
            uref.current_dir(&repo)
    ...
    fn in_flight_create_pin_survives_gc() {
        ...
        git_commit_all(&repo, "c");
        let oid = git_rev_parse(&repo, "HEAD");
        let pin = "refs/fuigo/worktrees/wt-fly";
        let mut uref = std::process::Command::new("git");
        fuigo_tty_utils::detach_std_command(&mut uref);
        assert!(
            uref.current_dir(&repo)
    ...
    fn aborted_journal_does_not_mask_marker_or_mounts_toml() {
        ...
        let pin = "refs/fuigo/worktrees/wt-mask";
        let mut uref = std::process::Command::new("git");
        fuigo_tty_utils::detach_std_command(&mut uref);
        assert!(
            uref.current_dir(&repo)
    ...
    fn planted_orphan_state_does_not_delete_heads_main() {
        ...
        fuigo_tty_utils::detach_std_command(&mut check);
        assert!(
            check
                .current_dir(&repo)
