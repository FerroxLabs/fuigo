...
fn register_worktree_writes_correct_fields() {
    // Isolate FUIGO_HOME so register_worktree's open_default write lands
    ...
    // (FUIGO_HOME is process-global) can't INSERT-OR-REPLACE our row.
...
fn gc_removes_an_expired_path_with_no_repo_and_counts_it_apart() {
    ...
    // `remove_worktree` reclaims either. FUIGO_HOME is the gc DB dir so
...
fn gc_asks_the_source_repository_about_a_standalone_worktree() {
    ...
    fuigo_test_utils::git::run_git(&dir, &["commit", "-am", "work no remote holds"]);
    ...
    fuigo_test_utils::git::run_git(&dir, &["push", "origin", "HEAD:refs/heads/main"]);
    // The gate asks the source repository, not the remote, so the source
    // is what has to hold the commit.
    fuigo_test_utils::git::run_git(&source, &["fetch", "origin"]);
...
fn gc_with_delegate_removes_expired_and_unregisters() {
    ...
    // FUIGO_HOME == the gc DB dir so remove_worktree's open_default
