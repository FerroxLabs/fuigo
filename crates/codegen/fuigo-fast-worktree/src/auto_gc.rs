...
use crate::db::{ListFilter, WorktreeDb, WorktreeKind, now_epoch_secs, resolve_fuigo_home};
...
pub const ENV_AUTO_GC: &str = "FUIGO_WORKTREE_AUTO_GC";
/// `1` / `true` / `on` forces age-count without delete.
pub const ENV_AUTO_GC_DRY_RUN: &str = "FUIGO_WORKTREE_AUTO_GC_DRY_RUN";
/// Default max age in seconds (overrides TOML/remote when set and parseable).
pub const ENV_AUTO_GC_MAX_AGE: &str = "FUIGO_WORKTREE_AUTO_GC_MAX_AGE";
/// `1` / `true` / `on` enables optional discovery rebuild + stale git prune.
pub const ENV_AUTO_GC_REBUILD: &str = "FUIGO_WORKTREE_AUTO_GC_REBUILD";

/// Remove every `FUIGO_WORKTREE_AUTO_GC*` env var so a test starts from a clean
...
pub struct WorktreeAutoGcLayer {
    ...
    /// Optional discovery rebuild + fuigo-scoped stale `.git/worktrees/` scrub (default off).
...
/// Parse `FUIGO_WORKTREE_AUTO_GC_MAX_AGE` as seconds; invalid/absent → None.
...
fn maybe_run_rebuild(
    ...
    let home = match resolve_fuigo_home() {
        Ok(h) => h,
        Err(e) => {
            tracing::warn!(error = %e, "auto worktree rebuild skipped: fuigo home unresolved");
            return (None, false);
        }
    };
...
/// Scrub stale fuigo-owned registrations from each known source repo,
/// scoped to worktrees under the fuigo home to prove ownership (see
...
fn prune_stale_git_worktree_registrations(repos: &BTreeSet<PathBuf>) -> u64 {
    let Ok(fuigo_home) = resolve_fuigo_home() else {
        tracing::warn!("auto worktree registration scrub skipped: fuigo home unresolved");
        return 0;
    };
    let cleaned: u64 = repos
        .iter()
        .filter(|repo| repo.is_dir())
        .map(|repo| crate::git::remove_stale_worktree_registrations_under(repo, &fuigo_home))
...
mod tests {
    ...
    use fuigo_test_utils::git::{init_git_repo, run_git};
    ...
    /// Kill switch: env `FUIGO_WORKTREE_AUTO_GC=0` or `opts.enabled=false` both
    ...
    #[test]
    fn include_rebuild_true_registers_untracked_under_fuigo_home() {
        let _g = env_guard();
        clear_auto_gc_env();
        let fx = crate::db::FuigoHomeFixture::new();
        ...
        assert!(
            ...
            "untracked dir under fuigo_home/worktrees must be registered"
        );
    }
    ...
    /// A real rebuild pass prunes a stale fuigo-owned git registration. The
