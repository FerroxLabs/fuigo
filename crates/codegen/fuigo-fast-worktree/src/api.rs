...
/// handle `~/.fuigo/worktrees/<repo>/<session>/`) and calls `remove_worktree()`
...
fn try_btrfs_remove(
    ...
    if worktree_path.is_symlink() {
        ...
        if let Ok(Some(_)) = btrfs::is_btrfs_subvolume(&resolved) {
            ...
            // `btrfs` refuses non-subvolumes, the snapshot dir is fuigo-owned, and
    ...
    if btrfs_info.bind_mount_source.is_some() {
        ...
        fuigo_tty_utils::detach_std_command(&mut umount_cmd);
...
mod tests {
    ...
    fn remove_with_delegate_deregisters_plain_worktree_without_calling_delegate() {
        fuigo_test_utils::require_git!();
        use fuigo_test_utils::git::{git_commit_all, init_git_repo};
        // Isolate FUIGO_HOME so the post-removal unregister writes to a private DB.
        #[cfg(feature = "metadata")]
        let _fx = crate::db::FuigoHomeFixture::new();
    ...
    fn delete_fallback_invokes_delegate_when_direct_delete_fails() {
        ...
        let report = delete_snapshot_with_delegate_fallback(
            ...
            Path::new("/home/u/.fuigo/worktrees/repo/wt"),
            Some(&delegate),
    ...
    fn test_cleanup_orphaned_btrfs_skips_when_mount_target_parent_missing() {
        ...
        let mount_target = unrestored_home.join(".fuigo/worktrees/x/wt-live");
        assert!(
    ...
    fn test_cleanup_orphaned_btrfs_skips_active() {
        ...
        let mount_target = std::path::PathBuf::from("/home/user/.fuigo/worktrees/active-wt");
