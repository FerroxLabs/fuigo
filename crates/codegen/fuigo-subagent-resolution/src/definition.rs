...
use std::path::Path;
use fuigo_agent::config::{AgentDefinition, IsolationMode};
use fuigo_agent::plugins::PluginRegistry;
use fuigo_agent::prompt::context::{PromptAudience, PromptContext};
...
use fuigo_tool_types::{SubagentCapabilityMode, SubagentIsolationMode};
...
pub fn discover_agent_definition(
    ...
    fuigo_agent::discovery::by_name_in_cwd_with_plugins(
        subagent_type,
        context.cwd,
        context.plugins,
    )
...
pub fn available_agent_names(context: &DefinitionResolutionContext<'_>) -> Vec<String> {
    let mut available: Vec<String> = fuigo_agent::discovery::all_subagents_with_plugins(
        context.cwd,
        context.toggles,
        context.plugins,
    )
...
mod tests {
    ...
    fn general_purpose_definition_omits_workflow() {
        ...
        assert!(
            ...
            "general-purpose must keep the rest of the fuigo-build child tools"
        );
    }
    #[test]
    fn child_tool_policy_strips_workflow_and_keeps_other_tools() {
        ...
        definition
            .tool_config
            .tools
            .push((&fuigo_tools::implementations::fuigo_build::WorkflowTool).into());
    ...
    fn kindless_workflow_id_is_stripped_by_child_policy() {
        ...
        definition
            .tool_config
            .tools
            .push(ToolConfig::from_id("FuigoBuild:workflow"));
        apply_child_tool_policy(&mut definition, None, true);
        assert!(definition.tool_config.tools.iter().all(|tool| {
            tool.kind != Some(ToolKind::Workflow)
                && tool.id.rsplit(':').next()
                    != Some(fuigo_tools::implementations::fuigo_build::WORKFLOW_TOOL_NAME)
        }));
    }
    #[test]
