...
//!
//! Extracted from `fuigo-shell/src/agent/subagent/` `normalize_forked_context()`.
...
///
/// See also: `fuigo-chat-state::compaction_utils::strip_system_tags`, which strips a related (but different) tag set for compaction.
...
/// The other is `fork_filter_chat` in `fuigo-shell/src/session/storage/jsonl.rs`; it truncates to the last complete turn before this counts them.
...
///
/// See also: `fuigo-chat-state::compaction_utils::strip_system_tags`, which also leaves unclosed tags untouched for a different tag set.
...
mod tests {
    use super::*;
    use fuigo_sampling_types::conversation::{ConversationItem, ToolCall, ToolResultItem};
    ...
    fn reasoning_item(text: &str) -> ConversationItem {
        ConversationItem::Reasoning(fuigo_sampling_types::synthesized_reasoning_item(text))
    }
