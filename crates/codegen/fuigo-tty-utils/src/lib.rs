...
//!
//! ```rust,no_run
//! use fuigo_tty_utils::{detach_command, pager_env};
...
//!
//! ```rust,no_run
//! use fuigo_tty_utils::{detach_std_command, pager_env};
//! use std::process::Stdio;
//!
...
#[cfg(unix)]
pub const RESET_CHILD_OOM_ENV: &str = "FUIGO_TOOLS_RESET_CHILD_OOM";
...
/// fallback exists to avoid. Mirrors `os_pipe` in fuigo-tools' shell_state,
...
/// killed fuigo and pile up on shared hosts. No-op on non-Linux platforms
...
/// the agent SDKs, `fuigo-desktop` all spawn `fuigo agent … stdio`): the
...
mod tests {
    ...
    #[cfg(target_os = "linux")]
    const PDEATHSIG_INTERMEDIATE_ENV: &str = "__FUIGO_TTY_UTILS_PDEATHSIG_INTERMEDIATE";
    ...
    fn stderr_redirect_roundtrip_subprocess() {
        let status = std::process::Command::new(std::env::current_exe().unwrap())
            ...
            .env("__FUIGO_STDERR_REDIRECT_SUBPROCESS", "1")
            .status()
    ...
    fn stderr_redirect_roundtrip_body() {
        if std::env::var("__FUIGO_STDERR_REDIRECT_SUBPROCESS").is_err() {
    ...
    fn null_fd_outlives_its_path() {
        let path = std::env::temp_dir().join(format!("fuigo-tty-utils-null-{}", std::process::id()));
