...
//! against per-user ceilings (`pids.max` / `RLIMIT_NPROC`). Fuigo runtimes
...
//! runtime (the `fuigo` binary, `workspace_server`) derives its worker count
//! from here.
...
/// Maximum runtime worker threads for any fuigo process.
#[allow(
    clippy::unwrap_used,
