...
//! The SQLite DB is shared with other fuigo processes (older binaries may wipe or downgrade it on open).
...
/// Manages background session indexing for every fuigo home this process touches.
///
...
impl SearchIndexManager {
    ...
    /// `source_factory` opens the session store for a fuigo home and `extract` pulls searchable text out of one transcript.
