...
impl SessionSearchIndex {
    ...
    fn open_with_journal_mode(
        ...
        // Multiple fuigo generations share this DB (stable vs alpha)
    ...
    /// So `/resume` search by id returned nothing while `fuigo --resume <id>` still loaded the session.
    pub fn query(
        &self,
        query: &str,
        cwd: Option<&str>,
...
mod tests {
    ...
    fn test_open_or_create_uses_wal_on_local_fs() {
        // The `FUIGO_SQLITE_JOURNAL_MODE` env var would override the decision; skip if set
    ...
    fn test_newer_version_index_is_tolerated_not_dropped() {
        let tmp = TempDir::new().unwrap();
        {
            ...
            index
                .upsert_doc(&test_doc("s1", "Rust debugging", "borrow checker"))
                .unwrap();
            // Simulate an index owned by a newer fuigo generation that has completed a bootstrap
