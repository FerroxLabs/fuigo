//! Hermetic filesystem and child-environment owner for fuigo integration tests.
...
/// Child commands start from `env_clear()` and receive only platform essentials, sandbox paths, fuigo network kill switches, and explicit overrides.
...
impl TestSandbox {
    ...
    /// Explicit fuigo state root.
    pub fn fuigo_home(&self) -> &Path {
        &self.fuigo_home
    }
    ...
    pub fn git_command(&self) -> Command {
        ...
        cmd.stdin(Stdio::null()).envs(fuigo_tty_utils::pager_env());
        for &(key, value) in &fuigo_tty_utils::GIT_AUTH_SUPPRESSION_ENVS {
            cmd.env(key, value);
        }
    ...
    pub fn diagnostic_summary(&self) -> String {
        let mut summary = format!(
            "root={} home={} fuigo_home={} workspace={} temp={}",
            self.root().display(),
            self.home.display(),
            self.fuigo_home.display(),
...
impl TestSandboxBuilder {
    /// Wire fuigo API, models, feedback, trace, conversation, and web traffic to a loopback mock endpoint and install the fake CI API key.
    ...
    pub fn build(self) -> TestSandbox {
        ...
        for path in [&home, &fuigo_home, &workspace, &temp] {
        ...
        let mut env = baseline_env(&home, &fuigo_home, &temp, &parent_cwd);
...
impl TestSandbox {
    fn init_git_workspace(&self) {
        ...
        run_git(self, &["config", "user.name", "Fuigo Test"]);
...
fn apply_mock_url(env: &mut BTreeMap<OsString, OsString>, url: String) {
    ...
    env.insert("FUIGO_API_KEY".into(), TEST_API_KEY.into());
}
...
fn baseline_env(
    ...
    baseline_env_from_parent(home, fuigo_home, temp, parent_cwd, &parent_env)
}
...
fn baseline_env_from_parent(
    ...
    for (key, value) in [
        ("FUIGO_TELEMETRY_ENABLED", "false"),
        ("FUIGO_TELEMETRY_TRACE_UPLOAD", "false"),
        ("FUIGO_FEEDBACK_ENABLED", "false"),
        ("FUIGO_TRACE_UPLOAD", "false"),
        ("FUIGO_INSTRUMENTATION", "disabled"),
        ("OTEL_SDK_DISABLED", "true"),
        ("DISABLE_TELEMETRY", "1"),
        ("DISABLE_FEEDBACK_COMMAND", "1"),
        ("FUIGO_DISABLE_AUTOUPDATER", "1"),
        ("FUIGO_PROMPT_SUGGESTIONS", "false"),
        // Pin so a developer-exported override cannot flake empty-home launch tests.
        ("FUIGO_DEFAULT_PERMISSION_MODE", "ask"),
        // The post-turn summary would send unscripted requests to the mock server and break exact wire-traffic assertions
        ("FUIGO_TURN_SUMMARY", "0"),
    ...
    env.insert(
        ...
        fuigo_home.join("gitconfig").into_os_string(),
    );
    env
}
...
fn diagnostic_value_is_sensitive(key: &OsStr) -> bool {
    ...
    is_secret_key(&key)
        ...
        || matches!(
            ...
            "HOME" | "USERPROFILE" | "FUIGO_HOME" | "TMPDIR" | "TMP" | "TEMP" | "GIT_CONFIG_GLOBAL"
        )
}
...
mod tests {
    ...
    fn owns_distinct_isolated_paths() {
        ...
        assert_eq!(sandbox.fuigo_home(), sandbox.home().join(".fuigo"));
    }
    ...
    fn git_command_uses_sandbox_state_without_process_global_mutation() {
        ...
        let sandbox = TestSandbox {
            ...
            fuigo_home: root.path().join("home/.fuigo"),
    ...
    fn baseline_is_hermetic_and_network_quiet() {
        ...
        assert_eq!(
            env_value(&sandbox, "FUIGO_HOME"),
            Some(sandbox.fuigo_home().into())
        );
        assert_eq!(
        ...
        assert_eq!(
            env_value(&sandbox, "FUIGO_API_KEY").as_deref(),
            Some(OsStr::new(TEST_API_KEY))
        );
        assert_eq!(
            env_value(&sandbox, "FUIGO_DISABLE_AUTOUPDATER").as_deref(),
            Some(OsStr::new("1"))
        );
        assert_eq!(
            env_value(&sandbox, "FUIGO_TELEMETRY_TRACE_UPLOAD").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "FUIGO_LEADER_SOCKET"), None);
        assert_eq!(env_value(&sandbox, "FUIGO_DISABLE_WEB_FETCH"), None);
        assert_eq!(env_value(&sandbox, "FUIGO_WEB_FETCH"), None);
    }
    ...
    fn command_application_clears_ambient_env_and_command_override_wins() {
        ...
        cmd.env("AMBIENT_SECRET", "must-disappear")
            .env("FUIGO_PROMPT_SUGGESTIONS", "ambient");
        sandbox.apply_to_std_command(&mut cmd);
        cmd.env("FUIGO_PROMPT_SUGGESTIONS", "command");
        ...
        assert_eq!(
            env.get(OsStr::new("FUIGO_PROMPT_SUGGESTIONS"))
                .map(OsString::as_os_str),
    ...
    fn explicit_overrides_win_and_can_remove_baseline_entries() {
        ...
        sandbox
            ...
            .set_env("FUIGO_PROMPT_SUGGESTIONS", "true")
            .set_env("NO_PROXY", "override.invalid")
            .remove_env("FUIGO_DISABLE_AUTOUPDATER");
        assert_eq!(
            env_value(&sandbox, "TERM_PROGRAM").as_deref(),
            Some(OsStr::new("vscode"))
        );
        assert_eq!(
            env_value(&sandbox, "FUIGO_PROMPT_SUGGESTIONS").as_deref(),
            Some(OsStr::new("true"))
        );
        assert_eq!(
            env_value(&sandbox, "NO_PROXY").as_deref(),
        ...
        assert_eq!(env_value(&sandbox, "FUIGO_DISABLE_AUTOUPDATER"), None);
    }
    ...
    fn diagnostics_fail_closed_for_credential_keys() {
        ...
        for (key, value) in [
            ...
            ("FUIGO_DEPLOYMENT_KEY", "deployment-key-do-not-print"),
            ("FUIGO_EXTRA_AUTH_KEY", "alpha-test-key-do-not-print"),
