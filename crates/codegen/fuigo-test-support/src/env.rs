...
fn local_fuigo_binary_path() -> PathBuf {
    target_dir()
        .join("debug")
        .join(format!("fuigo-pager{}", std::env::consts::EXE_SUFFIX))
}

fn ensure_local_fuigo_binary(binary: &Path) {
    if binary.exists() {
        return;
    }
    ...
    let output = cmd
        .output()
        .unwrap_or_else(|e| panic!("failed to spawn {cargo} to build fuigo-pager: {e}"));

    assert!(
        output.status.success(),
        "failed to build fuigo-pager for lifecycle tests (exit {:?})\nstdout:\n{}\nstderr:\n{}",
        output.status.code(),
...
/// Resolve fuigo binary: `FUIGO_BINARY` env (CI) or a locally built `fuigo-pager` binary.
pub fn fuigo_binary() -> PathBuf {
    ...
    let binary = local_fuigo_binary_path();
    ensure_local_fuigo_binary(&binary);
    binary
}
