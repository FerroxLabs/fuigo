//! Headless mode (`fuigo -p`) test runner.
...
/// Timeout for one headless fuigo invocation: 60 seconds, multiplied by [`crate::scaled`]'s `FUIGO_TEST_TIMEOUT_SCALE`.
...
/// Run `fuigo` with the given args against the mock server, bounded by the scaled headless timeout.
...
mod tests {
    ...
    async fn custom_headless_env_is_explicit_and_wins_after_sandbox_baseline() {
        ...
        cmd.args([
            "-c",
            "printf '%s|%s|%s|%s' \"${AMBIENT_ONLY-unset}\" \"$FUIGO_PROMPT_SUGGESTIONS\" \"$FEATURE_TEST_VAR\" \"$HOME\"",
        ])
        .env("AMBIENT_ONLY", "discarded")
        .env("FUIGO_PROMPT_SUGGESTIONS", "command-level-discarded");

        let result = run_headless_in_sandbox_borrowed_with_env(
            cmd,
            &sandbox,
            &[
                ("FUIGO_PROMPT_SUGGESTIONS", "explicit-override"),
