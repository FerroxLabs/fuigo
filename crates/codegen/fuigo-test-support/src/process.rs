...
pub struct TestProcessTree {
    pid: u32,
    label: String,
    group: Option<fuigo_tty_utils::ProcessGroup>,
...
impl TestProcessTree {
    ...
    pub fn attach(pid: u32, label: impl Into<String>) -> Self {
        let label = label.into();
        let (group, attachment_error) = match fuigo_tty_utils::ProcessGroup::new() {
    ...
    fn from_group(pid: u32, label: String, group: fuigo_tty_utils::ProcessGroup) -> Self {
        Self {
            pid,
            label,
            group: Some(group),
