//! Leader-mode (`fuigo agent --leader stdio`) test harness.
//!
...
/// Falls back to [`fuigo_binary`]'s resolution.
pub const LEADER_BINARY_ENV: &str = "FUIGO_BINARY_LEADER";

/// Env var naming the binary for the second (usually newer) client in a two-binary test.
/// Falls back to [`fuigo_binary`]'s resolution.
pub const CLIENT_BINARY_ENV: &str = "FUIGO_BINARY_CLIENT";
...
/// A `fuigo agent --leader stdio` client subprocess speaking ACP over pipes.
...
impl LeaderFixture {
    ...
    pub async fn start(
        ...
        Self::start_with_binary(&fuigo_binary(), server, cwd, sandbox).await
    }
    ...
    async fn start_with_binary_base_url_timeout(
        ...
        let socket = sandbox.fuigo_home().join("leader.sock");
        let lock = sandbox.fuigo_home().join("leader.lock");
        ...
        cmd.envs(fuigo_tty_utils::pager_env())
            .env("FUIGO_CLI_CHAT_PROXY_BASE_URL", base_url)
            .env("FUIGO_API_BASE_URL", base_url)
            .env("FUIGO_MODELS_BASE_URL", base_url)
            .env("FUIGO_FEEDBACK_BASE_URL", base_url)
            .env("FUIGO_TRACE_UPLOAD_URL", base_url)
            .env("FUIGO_API_KEY", "test-key-for-ci")
            .env("FUIGO_LEADER_SOCKET", &socket)
            .env("RUST_LOG", "fuigo_shell=debug");
        let log_path = sandbox.fuigo_home().join("leader.log");
        ...
        let pid = child.id();
        let tree = match TestProcessTree::try_attach(pid, "persistent fuigo test leader") {
            Ok(tree) => tree,
            Err(error) => {
                let _ = child.kill();
...
impl LeaderStdioClient {
    async fn spawn_with_binary_and_socket(
        ...
        let mut process = TestProcess::spawn(
            ...
            TestProcessConfig::new()
                .label("fuigo leader stdio client")
                ...
                .env("FUIGO_TRACE_UPLOAD_URL", base_url)
                .env("FUIGO_API_KEY", "test-key-for-ci")
                .env("FUIGO_LEADER_SOCKET", leader_socket)
                .env("RUST_LOG", "fuigo_shell=debug"),
        )
        .map_err(|error| {
            io::Error::new(
    ...
    pub async fn initialize(&self) -> acp::InitializeResponse {
        ...
        let api_key_method = init
            .auth_methods
            .iter()
            .find(|method| &*method.id().0 == "fuigo.api_key")
            .expect("fuigo.api_key auth method");
        self.conn
            .authenticate(
...
pub fn leader_lock_path(home: &Path) -> PathBuf {
    home.join(".fuigo").join("leader.lock")
}
...
pub fn leader_log(home: &Path) -> String {
    std::fs::read_to_string(home.join(".fuigo").join("leader.log")).unwrap_or_default()
}
