...
/// The canonical store path under a fuigo home.
/// The caller resolves the fuigo home (nothing in this crate reads `FUIGO_HOME`); tests pass a temp dir.
...
pub fn default_db_path(fuigo_home: &Path) -> PathBuf {
    fuigo_home.join("dashboard").join("workspace.db")
}
