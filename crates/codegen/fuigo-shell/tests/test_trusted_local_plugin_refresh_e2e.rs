...
//! 4. After exit, check the session JSON under `$FUIGO_HOME/sessions/` parses.
//!
//! Requires a built `fuigo` binary (`FUIGO_BINARY` or cargo-built pager) for the ignored headless test.
//!
//! ```bash
//! cargo test -p fuigo-shell --test test_trusted_local_plugin_refresh_e2e
//! cargo test -p fuigo-shell --test test_trusted_local_plugin_refresh_e2e -- --ignored
//! ```
...
fn trusted_local_refresh_surfaces_new_agent_via_discovery() {
    ...
    let _fuigo_guard = EnvVarGuard::set("FUIGO_HOME", &fuigo_home);
    ...
    let mut registry = InstallRegistry::empty(fuigo_home.join("installed-plugins"));
    ...
    // This runs in the same test because fuigo_home() caches the first FUIGO_HOME per process
...
#[tokio::test]
#[ignore = "requires pre-built fuigo binary; run with --ignored"]
#[serial]
async fn headless_session_refreshes_trusted_local_plugin_and_writes_session_json() {
    ...
    let home = dunce::canonicalize(home_tmp.path()).unwrap();
    let fuigo_home = home.join(".fuigo");
    std::fs::create_dir_all(&fuigo_home).unwrap();
    ...
    // The spawned binary gets HOME and FUIGO_HOME via `cmd.env` below
    // This global env only serves the discovery assertion run in-process after the binary exits; it resolves the registry via fuigo_home()
    ...
    // Session storage under FUIGO_HOME/sessions must hold JSON files that parse
