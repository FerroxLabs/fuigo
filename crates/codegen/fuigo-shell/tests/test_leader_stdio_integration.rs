...
async fn setup_control_test_server(
    temp: &TempDir,
) -> (std::path::PathBuf, fuigo_shell::leader::ServerHandle) {
...
/// The agent emitting it from `model_switch::apply` is covered by the `extensions::notification` tests in `fuigo-shell`.
/// The pager applying it silently on followers is covered by the `model_changed_*` tests in `fuigo-pager`'s `acp_handler` tests.
...
async fn test_session_ownership_cleanup_on_disconnect() {
    ...
    tokio::spawn(async move {
        ...
        let (shutdown_tx, _shutdown_rx) =
            tokio::sync::watch::channel(fuigo_shell::leader::ShutdownReason::Manual);
...
async fn test_auto_update_shutdown_reason_reaches_client() {
    use fuigo_shell::leader::{ClientCapabilities, ClientMode, LeaderClient, ShutdownReason};
    ...
    tokio::time::timeout(
        ...
        disconnect_rx.wait_for(|r| *r != fuigo_shell::leader::DisconnectReason::Connected),
    )
    .await
    ...
    assert_eq!(
        *disconnect_rx.borrow(),
        fuigo_shell::leader::DisconnectReason::LeaderShutdown
    );
}
...
///
/// This binary does not sandbox FUIGO_HOME, so on a dev machine these entries
/// land in the real `~/.fuigo` log — accepted: the server already writes
...
fn orphan_log_count(request_id: &str) -> usize {
    let Some(bytes) = fuigo_telemetry::unified_log::snapshot_log() else {
        return 0;
    };
...
async fn wait_for_client_disconnected_log(client_id: u64) -> bool {
    let deadline = tokio::time::Instant::now() + Duration::from_secs(5);
    loop {
        let seen = fuigo_telemetry::unified_log::snapshot_log().is_some_and(|bytes| {
...
async fn test_hung_agent_leaves_transport_healthy_and_forwards_cancel() {
    ...
    assert_eq!(
        *client.disconnect_reason().borrow(),
        fuigo_shell::leader::DisconnectReason::Connected,
