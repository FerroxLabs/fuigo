...
//!
//! Operator binaries live outside this repo and read `FUIGO_AUTH_EXPIRED=1` as "headless, don't prompt".
...
use chrono::Utc;
use fuigo_shell::auth::{
    AuthMode, FuigoAuth, FuigoComConfig, ensure_authenticated, try_ensure_fresh_auth,
};
...
fn write_conforming_provider(home: &Path) -> String {
    ...
    std::fs::write(
        ...
        format!(
            "#!/bin/sh\n\
             echo \"expired=${{FUIGO_AUTH_EXPIRED:-unset}}\" >> {log}\n\
...
async fn a_provider_that_declines_the_headless_run_can_still_sign_the_user_in() {
    ...
    // environment is spawned. `fuigo_home()` memoizes, so this must stay the
    // only test in the binary.
    unsafe {
        std::env::set_var("FUIGO_HOME", home.path());
        std::env::set_var("FUIGO_CLI_CHAT_PROXY_BASE_URL", &dead);
        std::env::set_var("FUIGO_API_BASE_URL", &dead);
