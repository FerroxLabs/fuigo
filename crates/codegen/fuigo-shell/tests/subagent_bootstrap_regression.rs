...
//! Knobs: `FUIGO_BOOTSTRAP_REGRESSION_SOFT=1` reports without asserting; `FUIGO_SWEEP_LOG` adjusts tracing.
...
fn regression_bootstrap_cost() {
    ...
    if std::env::var_os("FUIGO_BOOTSTRAP_REGRESSION_SOFT").is_some() {
        return;
    }
    if boot_ratio > SESSBOOT_MAX_RATIO || seed_ratio > BRIDGE_MAX_RATIO {
        ...
        eprintln!(
            "reproduce: cargo test -p fuigo-shell --test subagent_bootstrap_regression -- --exact regression_bootstrap_cost --ignored --nocapture"
        );
    }
    assert!(
