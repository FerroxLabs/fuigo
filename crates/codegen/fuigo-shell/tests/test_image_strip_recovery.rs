...
fn chat_completion_bodies(server: &fuigo_test_support::MockInferenceServer) -> Vec<String> {
    server
        .requests()
        .into_iter()
        .filter(|r| r.path == "/v1/chat/completions")
        .filter(|r| {
            !r.header("x-fuigo-req-id")
                .is_some_and(|id| id.starts_with("fuigo-turn-summary-"))
        })
...
fn session_chat_jsonl() -> std::path::PathBuf {
    let sessions = std::path::PathBuf::from(std::env::var("FUIGO_HOME").expect("FUIGO_HOME set"))
        .join("sessions");
