...
//!
//! One `#[test]`: the phases share one process-global `FUIGO_HOME` and env, so nothing else may run concurrently.
#![cfg(unix)]
...
fn seed_credential(fuigo_home: &Path, expires_at: chrono::DateTime<chrono::Utc>) {
    let auth = json!({
    ...
    std::fs::create_dir_all(fuigo_home).expect("create fuigo home");
    std::fs::write(
...
fn write_interactive_only_provider(fuigo_home: &Path) -> String {
    use std::os::unix::fs::PermissionsExt;

    let script = fuigo_home.join("acme-auth.sh");
    std::fs::write(
        &script,
        "#!/bin/sh\n\
...
fn provider_runs(fuigo_home: &Path) -> usize {
    std::fs::read_to_string(fuigo_home.join("provider-runs"))
        .map(|s| s.lines().count())
        .unwrap_or(0)
}
...
fn expired_external_credential_routes_to_the_provider_login_flow() {
    ...
    let provider = write_interactive_only_provider(fuigo_home.path());
    ...
    agent_rt.block_on(local.run_until(async move {
        ...
        std::fs::remove_file(fuigo_home.path().join("auth.json")).expect("remove auth.json");
