...
Launches fuigo agent stdio in isolated environments ($HOME override)
...
Usage:
    ...
    python3 crates/codegen/fuigo-shell/tests/memory_integration/run_tests.py

    # Run only fast (no-model) tests:
    python3 crates/codegen/fuigo-shell/tests/memory_integration/run_tests.py --fast

    # Run a single test by name:
    python3 crates/codegen/fuigo-shell/tests/memory_integration/run_tests.py test_fts_search_quality

    # Skip build (use pre-built binary):
    FUIGO_BINARY=/path/to/fuigo-pager python3 run_tests.py --fast
"""
...
class AcpClient:
    """Talks ACP (JSON-RPC over NDJSON) to a fuigo agent stdio subprocess."""
...
class IsolatedEnv:
    """Creates a fully isolated fuigo environment with custom $HOME.

    The agent subprocess gets a fake $HOME with:
      ~/.fuigo/auth.json   (copied from real home)
      ~/.fuigo/memory/     (pre-populated by tests)
      ~/.fuigo/logs/       (memory.log appears here)

    And a workspace directory used as cwd when spawning the agent.
    """

    def __init__(self, workspace_name="test-project"):
        self.root = tempfile.mkdtemp(prefix="fuigo-memtest-")
        ...
        # Create .fuigo dirs in fake home
        self.fuigo_home = os.path.join(self.fake_home, ".fuigo")
        self.memory_dir = os.path.join(self.fuigo_home, "memory")
        self.logs_dir = os.path.join(self.fuigo_home, "logs")
        os.makedirs(self.memory_dir)
        os.makedirs(self.logs_dir)

        # Copy auth from real home
        real_auth = os.path.expanduser("~/.fuigo/auth.json")
        if os.path.isfile(real_auth):
            shutil.copy2(real_auth, os.path.join(self.fuigo_home, "auth.json"))

    def write_config(self, toml_str):
        """Write global ~/.fuigo/config.toml (where the agent reads config)."""
        with open(os.path.join(self.fuigo_home, "config.toml"), "w") as f:
            f.write(toml_str)
    ...
    def spawn_agent(self, extra_env=None):
        binary = os.environ.get("FUIGO_BINARY", "fuigo")
        if not shutil.which(binary) and not os.path.isfile(binary):
            print(f"{R}fuigo binary not found: {binary}{N}")
            sys.exit(1)
        env = os.environ.copy()
...
def _default_config() -> str:
    embed_model = os.environ.get("FUIGO_MEMORY_EMBEDDING_MODEL", "").strip()
...
""")
        ...
        client = env.spawn_agent(extra_env={"FUIGO_MEMORY": "0"})
        init_session(client)
        time.sleep(2)
...
def test_memory_enabled_env_override():
    """FUIGO_MEMORY=1 env var enables memory even without config."""
    section("13. FUIGO_MEMORY Env Var Override")
    env = IsolatedEnv()
    try:
        # Config enables memory
        env.write_config(INDEXING_ONLY_CONFIG)
        env.write_global_memory("# Env test content\n* Key value here\n")
        client = env.spawn_agent(extra_env={"FUIGO_MEMORY": "1"})
        init_session(client)
        time.sleep(3)

        db = env.get_db_path()
        if db:
            ok("index created with FUIGO_MEMORY=1")
        else:
            fail("no index despite FUIGO_MEMORY=1 and config enabled")
...
def test_multiple_workspace_isolation():
    ...
    try:
        ...
        # Config is already in global ~/.fuigo/config.toml from env1.write_config

        # Start agent in workspace beta (reuse env1's fake home)
        binary = os.environ.get("FUIGO_BINARY", "fuigo")
        env_vars = os.environ.copy()
        env_vars["HOME"] = env1.fake_home
        env_vars["FUIGO_MEMORY"] = "1"
        env_vars["RUST_LOG"] = "warn"
        env_vars["FUIGO_NO_LEADER"] = "1"
...
def build_binary():
    """Build fuigo-pager with --features dev --release. Returns binary path."""
    repo = find_repo_root()
    if not repo:
        print(f"{R}Could not find repo root (no Cargo.toml + crates/ above cwd){N}")
        sys.exit(1)

    binary = os.path.join(repo, "target", "release", "fuigo-pager")

    # Find rg for FUIGO_SHELL_BUNDLE_RG_PATH
    rg = shutil.which("rg")
    if not rg:
    ...
    env = os.environ.copy()
    env["FUIGO_SHELL_BUNDLE_RG_PATH"] = rg

    print(f"{B}Building fuigo-pager (release + dev)...{N}")
    result = subprocess.run(
        ["cargo", "build", "-p", "fuigo-pager", "--features", "dev", "--release"],
        cwd=repo,
        env=env,
    )
...
def main():
    ...
    if "FUIGO_BINARY" in os.environ:
        binary = os.environ["FUIGO_BINARY"]
        print(f"Binary: {binary} (from $FUIGO_BINARY, skipping build)")
    else:
        binary = build_binary()
        os.environ["FUIGO_BINARY"] = binary
