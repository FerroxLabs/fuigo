...
/// All tests share this as FUIGO_HOME (the `OnceLock` in fuigo-config only allows one value per process).
fn test_home() -> &'static PathBuf {
    ...
    HOME.get_or_init(|| {
        ...
        unsafe { std::env::set_var("FUIGO_HOME", &path) };
        path
    })
}
...
async fn update_config_does_not_leak_requirements_into_user_config() {
    ...
    let effective = fuigo_shell::config::load_effective_config().unwrap();
    let effective_cfg = fuigo_shell::util::config::load_config_from_toml(&effective);
    assert_eq!(
    ...
    fuigo_shell::util::config::update_config(|cfg| {
    ...
    let user_cfg = fuigo_shell::util::config::load_config_from_toml(&user_toml);
