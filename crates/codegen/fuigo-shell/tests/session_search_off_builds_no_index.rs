//! Its own binary: the fuigo home resolves once per process.
...
async fn saved_session_is_neither_indexed_nor_found_with_search_off() {
    ...
    let root = dir.path();
    let _home = EnvGuard::set("FUIGO_HOME", root);
    let _off = EnvGuard::set("FUIGO_SESSION_SEARCH", "0");

    let config = fuigo_shell::config::load_agent_config_disk_only().expect("config loads");
    let search = fuigo_shell::session::storage::search::start_if_enabled(&config);
    assert!(
    ...
    assert_eq!(
        search.off_reason(),
        Some("the FUIGO_SESSION_SEARCH environment variable"),
