...
//! It is hermetic: a throwaway `FUIGO_HOME`, no network, and a provider command that needs no binary beyond what the platform shell already provides.

use std::collections::BTreeMap;
use std::path::Path;

use chrono::Utc;
use fuigo_shell::auth::{AuthMode, FuigoAuth, FuigoComConfig, try_ensure_fresh_auth};

const SEED_TOKEN: &str = "stale-token-that-must-be-replaced";

/// `fuigo_home()` memoizes into a `OnceLock`, so every phase below shares this one directory.
/// That is why the phases live in a single test rather than racing each other as separate ones.
fn use_temp_fuigo_home(dir: &Path) {
    // SAFETY: single-threaded test entry, before any thread that reads the
    // environment is spawned.
    unsafe {
        std::env::set_var("FUIGO_HOME", dir);
    }
}
...
fn seed_expired_credential(home: &Path, scope: &str) {
    ...
    let store: BTreeMap<String, FuigoAuth> = [(scope.to_owned(), expired)].into_iter().collect();
    std::fs::write(
        home.join("auth.json"),
...
async fn auth_provider_command_mints_the_session_credential() {
    ...
    use_temp_fuigo_home(home.path());

    // `echo <token>` is valid in both `sh -c` and `cmd /C`, so this phase needs no external binary and runs identically on every platform
    let token = mint_with_provider(home.path(), "echo fuigo-ext-token").await;
    assert_eq!(token, "fuigo-ext-token");
