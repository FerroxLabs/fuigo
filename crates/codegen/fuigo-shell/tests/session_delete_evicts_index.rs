//! One binary, one home: `fuigo_home()` memoizes the first read for the process, so tests that need a temp home have to share one.
//! `#[serial]` keeps their env writes apart.

use std::sync::{Arc, OnceLock};

use agent_client_protocol as acp;
use fuigo_shell::auth::{AuthManager, FuigoComConfig};
use fuigo_shell::session::info::Info;
use fuigo_shell::session::persistence::delete_session_history;
...
fn home() -> &'static std::path::Path {
    ...
    HOME.get_or_init(|| {
        ...
        let guard = EnvGuard::set("FUIGO_HOME", dir.path());
        (dir, guard)
    })
    .0
    .path()
...
/// `start_if_enabled` is the only way to get a manager, so the test calls it with `FUIGO_SESSION_SEARCH` left at its default.
fn start_index() -> SearchIndexManager {
    let _default_on = EnvGuard::unset("FUIGO_SESSION_SEARCH");
    match start_if_enabled(&fuigo_shell::agent::config::Config::default()) {
...
async fn deleting_a_session_clears_only_its_own_search_row() {
    ...
    let auth = Arc::new(AuthManager::new(root, FuigoComConfig::default()));

    let session_dir =
        fuigo_shell::util::fuigo_home::sessions_cwd_dir_in(root, "/ws-a").join("orphan");
...
fn loading_config_applies_requirement_pins() {
    ...
    let loaded = fuigo_shell::config::load_agent_config_disk_only();
    std::fs::remove_file(&pin).unwrap();
    let config = loaded.expect("config loads");

    let resolved = config.feature(fuigo_shell::agent::config::Feature::SessionSearch);
    assert!(
        !resolved.value,
    ...
    assert_eq!(
        resolved.source,
        fuigo_shell::agent::config::ConfigSource::Requirement
    );
}
