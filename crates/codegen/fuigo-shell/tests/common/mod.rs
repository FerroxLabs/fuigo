...
pub fn isolated_home() -> tempfile::TempDir {
    let home = tempfile::TempDir::new().expect("fuigo home tempdir");
    // SAFETY: single-test binary; no other thread reads or writes the environment.
    unsafe { fuigo_test_support::isolate_fuigo_env(home.path()) };
    home
}
...
pub async fn start_seeded_mock(home: &std::path::Path) -> fuigo_test_support::MockInferenceServer {
    ...
    let auth = serde_json::json!({
        scope: {
            ...
            "auth_mode": "oidc",
            "oidc_issuer": fuigo_shell::auth::fuigo_oauth2_issuer(),
...
#[allow(dead_code)]
pub async fn run_bootstrap() -> Result<fuigo_shell::agent::config::Config, String> {
    tokio::task::spawn_blocking(|| {
        let cfg = fuigo_shell::agent::config::Config::default();
        let auth_manager = std::sync::Arc::new(cfg.create_auth_manager());
        fuigo_shell::agent::init::bootstrap(&cfg, &auth_manager, None)
...
pub fn test_sampler_config(
    ...
    // Shell `Client` is `fuigo_sampler::SamplingClient`, which takes a `SamplerConfig` directly
    SamplerConfig {
