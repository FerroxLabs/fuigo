...
pub fn build_repo(files: usize) -> TempDir {
    ...
    let wd = dir.path();
    let git = |args: &[&str]| fuigo_test_utils::git::run_git(wd, args);
    git(&["init"]);
    git(&["config", "user.name", "Sweep User"]);
    git(&["config", "user.email", "sweep@test.com"]);
    fuigo_test_utils::git::write_fanout_tree(wd, files, 100);
    git(&["add", "."]);
...
pub fn run_burst(
    ...
    let repo_files = env_usize("FUIGO_SWEEP_REPO_FILES", 50);
...
pub fn sweep_env_init() -> SweepEnv {
    ...
    let filter = std::env::var("FUIGO_SWEEP_LOG").unwrap_or_else(|_| "warn".to_string());
...
pub fn run_sweep(ns: &[usize], isolation: &str) {
    ...
    for &n in ns {
        ...
        if std::env::var_os("FUIGO_SWEEP_ASSERT_NO_FAILURES").is_some() {
