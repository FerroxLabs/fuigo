...
- **Using Fuigo**
  ...
  - [SSH Passthrough](#ssh-passthrough-fuigo-ssh) — Apple Terminal clipboard support
- **Configuration**
...
- **Reference**
  - [Introspection (`fuigo inspect`)](#introspection)
...
- [Building with Fuigo](#building-with-fuigo) — headless API, ACP SDK integration
...
Credentials are stored in `~/.fuigo/auth.json` and persist across sessions. Tokens expire after 7 days; Fuigo will prompt you to re-authenticate when needed.
...
**3. Run `fuigo`.** The CLI discovers endpoints via `{issuer}/.well-known/openid-configuration`, opens the IdP login page, and stores tokens in `~/.fuigo/auth.json`. The OIDC token is sent as `Authorization: Bearer` to the configured proxy. Tokens auto-refresh silently via the stored `refresh_token`.
...
Fuigo is provider-agnostic — it doesn't know or care how your binary authenticates. It just runs the command, reads a token from stdout, and stores it. Your binary is a black box that handles the entire auth flow.
...
4. **stdout** → captured by Fuigo and saved to `~/.fuigo/auth.json` as the access token
5. exit 0 → success; exit non-zero → Fuigo falls through to interactive login
...
**Do not print anything to stdout except the token.** No progress messages, no debug output, no "Login successful!" text. Fuigo reads stdout verbatim and tries to parse it as a token. Any extra text will break parsing.
...
Use JSON if your tokens expire and you want Fuigo to automatically re-run the binary before expiry. The `expires_in` field (seconds until expiry) tells Fuigo when to proactively refresh. Without it, Fuigo assumes tokens last 30 days.
...
# Print ONLY the token to stdout (Fuigo captures this)
...
export FUIGO_AUTH_PROVIDER_LABEL="Acme Corp"   # optional
export FUIGO_AUTH_TOKEN_TTL=3600               # optional
```

If your binary outputs a bare token string (not JSON with `expires_in`), set `auth_token_ttl` to the token's expected lifetime in seconds. Without it, Fuigo cannot detect expiry proactively and will only refresh after a 401.
...
> **Windows:** write the path as a TOML *literal* string (single quotes) so backslashes survive: `auth_provider_command = 'C:\corp\fuigo-auth.exe'`. Inside a double-quoted TOML string `\t`, `\n`, `\r`, `\b` and `\f` are escape sequences, so `"C:\temp\auth.exe"` parses into a path containing a tab character and the provider fails to start — after which Fuigo falls back to browser login as if the setting were ignored.

When `auth_provider_label` is set, the TUI welcome screen shows **"Login with Acme Corp"** instead of "Login with grok.com". In headless mode (`fuigo -p`), the label has no effect — stderr from your binary is printed directly to the terminal.
...
RESP=$(curl -s -X POST https://auth.acme.com/device/code -d "client_id=fuigo-cli")
...
Fuigo runs your binary on two different contracts, and `FUIGO_AUTH_EXPIRED` is how it tells them apart:

| | `FUIGO_AUTH_EXPIRED=1` | unset |
|---|---|---|
| **What it is** | A headless refresh over a credential Fuigo already holds — near-expiry rotation, or a token the server rejected | A sign-in: `fuigo login`, the sign-in screen, or the escalation after a headless run couldn't mint |
...
| **Budget** | A few seconds (7s), then Fuigo kills the process | 300s — enough for a browser round trip or a device code |
...
if [ "$FUIGO_AUTH_EXPIRED" = "1" ]; then
    ...
    # lapsed, say — exit non-zero rather than block. Fuigo then shows the
...
Exiting promptly on `FUIGO_AUTH_EXPIRED=1` is what makes the handover to the sign-in screen fast: a binary that blocks instead pays the whole refresh timeout on every start with an expired token.

One case stays ambiguous, and only in **leader mode** (`--leader`, or `[cli] use_leader = true`; off by default): with no credential at all, the leader makes one extra attempt in the background just after startup, and that run has the variable unset, like a sign-in. A binary that mints without help (service account, keytab, mounted token) succeeds there and the session heals itself. One that must prompt just sits, up to the 300s sign-in ceiling — nothing waits on it, the sign-in screen is already up, and its stderr goes to `~/.fuigo/leader.log` rather than to the user.

`FUIGO_AUTH_EXPIRED` is optional — if your binary ignores it, Fuigo still works. It just runs the same flow for both login and refresh, and a flow that prompts will be killed on the headless run before it can finish.

### Automatic Credential Refresh

Fuigo supports automatic credential refresh for external auth providers and OIDC. When Fuigo detects that your token is expired (either locally based on `expires_in`, or when the server returns a 401), it automatically re-runs your `auth_provider_command` to obtain new credentials before retrying the request.

This is transparent — you don't need to do anything. Fuigo handles it in the background during your session.

**When does refresh happen?**

- **Before expiry:** If your binary returned `expires_in` in its JSON output, or you set `auth_token_ttl` in config, Fuigo re-runs the binary ~5 minutes before the token expires, so you never see an auth error.
- **On auth error:** If the server rejects a request with 401/403 (e.g. token was revoked or expired), Fuigo re-runs the binary and retries the request once.
- **When the refresh run can't mint:** refreshes are headless (no stdin, short timeout), so a binary that needs you to complete an SSO flow cannot succeed there. Fuigo then stops treating the stored credential as usable and runs your binary in its interactive mode instead — at startup that is the same sign-in flow a machine with no credentials gets; mid-session the turn fails with a re-auth prompt and `/login` re-runs the binary.
- **OIDC:** If you're using OIDC and have a `refresh_token`, Fuigo silently refreshes via your IdP without re-opening the browser.

**Tuning the refresh buffer:**

```bash
# Fuigo refreshes tokens 5 minutes before expiry by default.
# Set to 0 to only refresh on 401. Set higher for very short-lived tokens.
export FUIGO_AUTH_EARLY_INVALIDATION_SECS=300
```

**Keep in mind:**
- When using `auth_provider_command`, you don't need to run `fuigo login` before starting — on first launch Fuigo runs your binary on the real terminal (URL and progress on stderr), then opens the UI already signed in. You _can_ run `fuigo login` to explicitly hydrate `auth.json` ahead of time if you prefer. Mid-session `/login` still uses the in-TUI copy-link overlay.
- If both OIDC and `auth_provider_command` are configured: at **login** time, Fuigo tries OIDC silent refresh first (if a `refresh_token` exists), then the external binary, then browser-based login. During a **session**, whichever method is configured is used exclusively — if `auth_provider_command` is set it handles all mid-session refreshes; otherwise OIDC silent refresh is used.
...
```bash
fuigo --debug-file /tmp/fuigo-auth.log -p "hello"
tail -f /tmp/fuigo-auth.log
```
...
|-------------|---------------|
| `auth: running external auth provider (headless refresh)` | Your binary is being called with `FUIGO_AUTH_EXPIRED=1` and a few seconds to work |
| `auth: running external auth provider (interactive login)` | Your binary is being called on the sign-in contract: no `FUIGO_AUTH_EXPIRED`, stderr shown, 300s |
...
| `auth: external auth provider timed out (likely needs interactive auth), killing` | Binary didn't exit before the 7s headless-refresh timeout and was killed. Exiting non-zero on `FUIGO_AUTH_EXPIRED=1` avoids this wait entirely |
| `auth: failed to start external auth provider` | The command couldn't be spawned (e.g. binary not found) |

### Per-Model Auth Providers

`auth_provider_command` above replaces Fuigo's *session* auth: it mints the token sent to Ferrox Labs's backend. If you instead want Ferrox Labs models on normal Ferrox Labs login while **other models** route through a gateway (LiteLLM, corporate proxy) whose bearer tokens rotate, use a named auth provider — the rotating-token analogue of a per-model `api_key`/`env_key`.
...
- `FUIGO_AUTH_EXPIRED=1` is set whenever Fuigo re-mints over a token still cached in memory, whether from near-expiry rotation or a rejection. The first mint on a cold cache runs without it.

**Token lifecycle:**

- Tokens are cached in memory per provider and shared by every model referencing the provider; nothing is written to disk. The command is a credential helper: it owns durable storage and OAuth2 refresh (keychain, its own dotdir, etc.), exactly like `gcloud auth print-access-token` or a git credential helper. On an in-session re-mint the last credential is handed back via `FUIGO_AUTH_PROVIDER_ACCESS_TOKEN` (and, when present, `FUIGO_AUTH_PROVIDER_REFRESH_TOKEN` / `FUIGO_AUTH_PROVIDER_EXPIRES_AT`), so a refresh-grant command can refresh instead of re-authenticating. The command must be non-interactive and fast; do any interactive login out of band, and Fuigo re-runs the command on restart to re-mint.
- Fuigo runs the command before a chat turn when the token is missing or within about a minute of expiring, and once more after the server rejects a token. A token rejected within 30 seconds of being fetched is not refetched again, so a broken helper surfaces one clear error instead of looping.
...
**Interaction with other credentials:** a literal `api_key`/`env_key` on the model wins over its `auth_provider`. Provider-backed models are BYOK: your Ferrox Labs session token is never sent to their endpoints, and a failing provider command fails the request rather than falling back to the session token.

**Security:** provider commands execute code, so they are honored only from trusted config layers (`~/.fuigo/config.toml`, managed config, requirements). A project's `.fuigo/config.toml` can never define one. Whatever layer sets a model's `base_url` decides where that model's minted token is sent, and `base_url` (unlike the provider table) is not stripped from remote or campaign patches, the same as for a static `env_key`. Keep provider tables and the model `base_url` in layers you trust. The command inherits Fuigo's environment (so it sees `PATH`, `HOME`, and any other secrets there), but Fuigo's own first-party credentials (`FUIGO_API_KEY`, `FUIGO_DEPLOYMENT_KEY`, and related keys) are removed so a BYOK helper never receives them; write helpers that read only what they need, and prefer the `FUIGO_AUTH_PROVIDER_*` handback for the prior credential.
...
curl -s -N -X POST "https://cli-chat-proxy.grok.com/v1/chat/completions" \
  ...
  -H "x-fuigo-model-override: fuigo-build" \
  -d '{
    "model": "fuigo-build",
...
| -------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Authorization: Bearer <token>`  | Yes      | Session token from `~/.fuigo/auth.json` (set by `fuigo login`)                                                                                                                              |
| `X-XAI-Token-Auth: xai-grok-cli` | Yes      | Tells the auth middleware to validate as a CLI session token                                                                                                                              |
| `x-fuigo-model-override: <model>` | Yes\*    | The proxy uses this header (not the JSON body) to route to the correct backend. \*Can be omitted for `fuigo-build` which is on the default route, but always safe to include. |
...
| --------------------- | -------------- | ------------ |
| `fuigo-build`    | ✅ Supported   | ✅ Supported |

> **Note:** `auth.json` tokens expire after 7 days. Run `fuigo login` to refresh.
...
fuigo --cwd ~/projects/my-app

# Start with an initial task
fuigo --prompt "Review this codebase and suggest improvements"

# Add project-specific rules
fuigo --rules "Always use TypeScript. Prefer functional components."

# Auto-approve mode for trusted tasks
fuigo --always-approve --prompt "Format all files"
```
...
Run Fuigo non-interactively from the command line. Use headless mode when you need to:
...
| `-m, --model <MODEL>`   | Model to use (e.g., `fuigo-build`)               |
...
fuigo -p "Refactor the code" --deny "Edit(/etc/**)"
...
fuigo --allow "WebFetch(domain:docs.rs)" --deny "WebFetch(*)"
```
...
```bash
# Simple question
fuigo -p "What does this project do?"

# Use a specific model
fuigo -p "Optimize this function" -m fuigo-build

# Get JSON output for parsing
fuigo -p "List all TODO comments in the codebase" --output-format json

# Streaming JSON for real-time processing
fuigo -p "Explain the architecture" --output-format streaming-json

# Multi-turn conversation (session ID is returned in JSON output)
fuigo -p "Remember: the secret number is 42" --output-format json
fuigo -p "What's the secret number?" --resume <sessionId>

# Resume most recent session
fuigo -p "Continue where we left off" -c

# Run in a different directory
fuigo -p "Run the tests" --cwd ~/projects/other-app --always-approve
```
...
fuigo -p "Review the changes in this PR" -s "critique-myrepo-pr-123"

# Continue in the same session
fuigo -p "Now check for security issues" -s "critique-myrepo-pr-123"
```
...
git diff --staged | fuigo -p "Write a concise commit message for these changes"

# Batch: process multiple files
for file in src/*.js; do
  fuigo -p "Migrate $file from CommonJS to ES modules." --always-approve
done
...
Run Fuigo as an ACP (Agent Client Protocol) agent for integration with IDEs, editors, and custom tooling.
...
| Flag                  | Description                                                                         |
| --------------------- | ----------------------------------------------------------------------------------- |
| `-m, --model <MODEL>` | Override the default model ID (e.g., `fuigo-build`)                           |
...
```bash
fuigo agent headless --fuigo-ws-url wss://your-relay.example.com/ws
```
...
## SSH Passthrough (`fuigo ssh`)

Use `fuigo ssh` instead of plain `ssh` when connecting to remote hosts in terminals that lack native support (e.g. Apple Terminal) for local OSC 52 clipboard interception.
...
# With SSH flags
fuigo ssh -t user@host
fuigo ssh -L 8080:localhost:8080 user@host

# With remote command
fuigo ssh user@host -- tmux attach
```

On macOS, if the terminal doesn't natively handle OSC 52, `fuigo ssh` runs SSH inside a local PTY that intercepts clipboard sequences and writes them to `pbcopy`. Both plain OSC 52 and tmux DCS passthrough are handled. Terminals with native OSC 52 (iTerm2, Ghostty, Kitty, WezTerm, Alacritty) get a plain `ssh` exec with no wrapper.
...
Fuigo can be used as an OpenAI-compatible chat completion backend. Choose between two integration modes:
...
Use headless mode for simple integrations. Spawns `fuigo -p` and parses JSON output.
...
class FuigoChat:
    ...
    async def create(self, messages, model="fuigo-build", stream=False):
...
class FuigoChat {
  ...
  async create(
    ...
    { model = "fuigo-build", stream = false } = {},
  ) {
    const prompt =
      messages.length === 1
        ? messages[0].content
  ...
  async *streamResponse(prompt: string, model: string) {
    const proc = execa("fuigo", this.buildArgs(prompt, model, true));
...
# Usage
async def main():
    client = await FuigoACPChat(cwd=".").init()
...
class FuigoACPChat {
  ...
  async init() {
    this.proc = spawn("fuigo", ["agent", "stdio"]);
...
Fuigo implements the [Agent Client Protocol (ACP)](https://agentclientprotocol.com), a standard for AI agent communication.
...
Fuigo reads configuration from `~/.fuigo/config.toml`. If the file doesn't exist, Fuigo uses sensible defaults. You only need to specify values you want to override.
...
telemetry = false                      # anonymous usage telemetry (env: FUIGO_TELEMETRY_ENABLED)
feedback = false                       # feedback system (env: FUIGO_FEEDBACK_ENABLED)
...
[tools]
respect_gitignore = true               # filter gitignored files from tools (env: FUIGO_RESPECT_GITIGNORE)
...
```toml
[telemetry]
events_url = "https://example.com/events"  # env: FUIGO_TELEMETRY_EVENTS_URL
events_api_key = "..."                      # env: FUIGO_TELEMETRY_EVENTS_API_KEY
mixpanel_token = "..."                      # env: FUIGO_TELEMETRY_MIXPANEL_TOKEN
mixpanel_enabled = true                     # env: FUIGO_TELEMETRY_MIXPANEL_ENABLED
trace_upload = true                         # env: FUIGO_TELEMETRY_TRACE_UPLOAD
```

When building from source, defaults can also be baked into the binary at compile time by setting `FUIGO_TELEMETRY_BUILD_EVENTS_URL`, `FUIGO_TELEMETRY_BUILD_EVENTS_API_KEY`, and `FUIGO_TELEMETRY_BUILD_MIXPANEL_TOKEN` in the build environment (providing a Mixpanel token this way also enables Mixpanel by default). Config-file and runtime env values override build-time defaults.

### LSP Servers

Fuigo can connect to Language Server Protocol (LSP) servers configured in JSON files. LSP integration gives Fuigo language-aware code intelligence while it works in your repository.

LSP support is used in two ways:

- **Passive diagnostics** — after edits, Fuigo can surface language-server diagnostics such as errors and warnings.
- **The `lsp` tool** — Fuigo can actively query the language server for `goToDefinition`, `findReferences`, `hover`, `goToImplementation`, `documentSymbol`, and `workspaceSymbol`.

Reference: [Language Server Protocol](https://microsoft.github.io/language-server-protocol/)

#### Config locations

Fuigo looks for server definitions in:

- project config: `<repo>/.fuigo/lsp.json`
- user config: `~/.fuigo/lsp.json`
...
- LSP tools are enabled (`FUIGO_LSP_TOOLS=1` or `[features] lsp_tools = true`)
- the merged LSP configuration is non-empty

Enable the tool for one run:

```bash
FUIGO_LSP_TOOLS=1 fuigo
```
...
If LSP tools are enabled but no usable server config is found, Fuigo emits a non-fatal warning in logs and continues without the `lsp` tool. If config exists but every server fails to start, the tool may still be present and will fail on first use with a startup error.
...
Fuigo does not bundle language server binaries. You must install the server yourself and make sure the configured `command` is runnable on your machine.
...
- Passive diagnostics do **not** require `FUIGO_LSP_TOOLS=1`; they run whenever an applicable server is configured and starts successfully.
...
[models]
default = "company-fuigo"

[model.company-fuigo]
model = "fuigo-build"
base_url = "https://fuigo-proxy.acme.com/"
name = "Fuigo Latest (Proxy)"
...
With this config, `fuigo` runs your auth binary, stores the token, and routes inference through your corporate proxy. See [Authentication](#authentication) for full auth setup details.

---

## AGENTS.md

Add project-specific instructions by creating an agent rules file (e.g., `AGENTS.md`). Fuigo reads these files and appends their contents to the system prompt.

Fuigo scans for agent rules in this order:

1. `~/.fuigo/` (global rules)
...
Within each directory, Fuigo checks for these filenames:
...
Skills are reusable prompt packages that extend Fuigo with specialized workflows, domain knowledge, and tool integrations. Use them to encode repeatable procedures that would otherwise require re-explaining each session.

### Skill Locations

Fuigo discovers skills from these directories (in priority order):

| Location                    | Scope | Priority |
| --------------------------- | ----- | -------- |
| `./.fuigo/skills/`           | Local | Highest  |
| `<repo_root>/.fuigo/skills/` | Repo  | Medium   |
| `~/.fuigo/skills/`           | User  | Lowest   |
| `~/.claude/skills/`         | User  | Lowest   |

Skills with the same name are deduplicated — higher priority locations override lower ones.

Repo-scoped skills (Local and Repo) respect `.gitignore` and are filtered out if ignored. User-scoped skills (`~/.fuigo/skills/`) are outside the repo and never filtered.
...
| `description` | What the skill does and when to use it—this is how Fuigo decides to invoke it |
...
Users can reference skills as `/skill-name` (e.g., `/commit`). When you see this pattern, Fuigo invokes the corresponding skill.

> **Tip:** The `description` field is critical — it determines when Fuigo automatically invokes the skill. Be specific about trigger phrases and use cases.
...
Fuigo discovers agent definitions from `.fuigo/agents/` (project), `~/.fuigo/agents/` (user), and built-in agents. Priority (highest wins):

1. `--agent-profile <PATH>` CLI flag
2. `[agent]` section in `config.toml`
3. `FUIGO_AGENT` env var
4. Default `fuigo-build` agent
...
```bash
fuigo --agent-profile ./my-agent.md
# or
export FUIGO_AGENT="my-custom-agent"
```
...
```bash
export FUIGO_SUBAGENTS=0              # Environment variable
```
...
[subagents.models]
explore = "fuigo-build"              # route explore to a lighter model
```
...
model = "fuigo-build"
prompt_file = ".fuigo/prompts/researcher.md"

[subagents.personas.concise]
instructions = "Be extremely concise. No filler words."
# instructions_file = ".fuigo/personas/concise.md"  # or load from file
```

Both are also discovered from `.fuigo/roles/*.toml` and `.fuigo/personas/*.toml` files respectively. If a requested persona is not found, the spawn fails (fail-closed).

---

## Plugins

Plugins extend Fuigo with additional tools, skills, and MCP servers from external packages.

### Plugin Locations

| Location                    | Scope   |
| --------------------------- | ------- |
| `.fuigo/plugins/`            | Project |
| `~/.fuigo/plugins/`          | User    |
...
Fuigo discovers hooks from `.fuigo/hooks/` in the project directory. Manage them with:
...
`/etc/fuigo/requirements.toml`, or `/etc/fuigo/managed_config.toml`) are enforced:
...
`$FUIGO_HOME` layers (`requirements.toml`, `managed_config.toml`, `config.toml`)
...
**Credential resolution order:** `api_key` → `env_key` → cached `auth_provider` token (terminal: a cache miss resolves to no credential, never the session token) → session token → `FUIGO_API_KEY`. See [Per-Model Auth Providers](#per-model-auth-providers).

The `context_window` parameter is used to calculate when auto-compact should trigger. If not specified, Fuigo falls back to built-in defaults for known models.
...
**How it works:** When you override a built-in model, Fuigo starts with the default configuration (including the correct `base_url` from your `[endpoints]` setting), then applies only the fields you specify. Unspecified fields inherit from the default.
...
**Web search model:** Set `[models] web_search`, `FUIGO_WEB_SEARCH_MODEL`, or `--web-search-model` to point the `web_search` tool at a different model. The target endpoint must support the Responses API and web search.
...
> `fuigo models` output). You also need a `[model.*]` entry so Fuigo knows
...
>
> [model.my-custom-model]              # 2. tell Fuigo how to reach it
...
Point Fuigo at a custom OpenAI-compatible `/v1/models` endpoint instead of the default cli-chat-proxy. Useful when models are served behind a corporate gateway or self-hosted inference stack.

**Environment variables:**

| Variable | Required | Description |
|----------|----------|-------------|
| `FUIGO_MODELS_BASE_URL` | Yes | Base URL for inference / chat completions (e.g. `https://api.acme.com/v1`). The model list is fetched from `{base_url}/models` automatically |
| `FUIGO_API_KEY` | Yes | API key sent as `Authorization: Bearer` to the custom endpoint |
| `FUIGO_MODELS_LIST_URL` | No | Override the model list URL if it differs from `{base_url}/models` |

**Setup:**

```bash
export FUIGO_MODELS_BASE_URL="https://api.acme.com/v1"
export FUIGO_API_KEY="fuigo-..."
fuigo
```

Fuigo fetches the model list from `{FUIGO_MODELS_BASE_URL}/models` on startup and sends inference requests to `FUIGO_MODELS_BASE_URL`. This follows the standard OpenAI-compatible convention used by OpenAI, Anthropic, OpenRouter, Groq, Together.ai, and others.

If your model list endpoint differs from `{base_url}/models`, set `FUIGO_MODELS_LIST_URL` explicitly.

**Combining with `[endpoints]` config:** You can also set endpoints in `~/.fuigo/config.toml`:
...
**Auth behavior:** When `models_base_url` is set, Fuigo uses API key auth (`Authorization: Bearer`) instead of session auth. `fuigo login` is not required — only the API key.

---

## MCP Servers

Extend Fuigo's capabilities with [Model Context Protocol](https://modelcontextprotocol.io) servers.

### Configuration

MCP servers are configured in `~/.fuigo/config.toml`:
...
MCP servers can also be configured per-project in `.fuigo/config.toml`. Fuigo walks from the current directory up to the git repo root, loading `.fuigo/config.toml` at each level:

| Location                        | Scope             | Priority |
| ------------------------------- | ----------------- | -------- |
| `~/.fuigo/config.toml`           | All projects      | Lowest   |
| `<repo-root>/.fuigo/config.toml` | This repository   | ↑        |
| `<cwd>/.fuigo/config.toml`       | Current directory | Highest  |
...
**Example:** commit a `.fuigo/config.toml` in your repo to share MCP servers across the team:
...
If you also have a `linear` server in `~/.fuigo/config.toml`, the project version replaces it entirely.

> **Note:** Only `[mcp_servers]` is supported in project-scoped `.fuigo/config.toml`. Other config sections (models, etc.) are only read from `~/.fuigo/config.toml`.
...
> **Experimental:** enable with `FUIGO_MEMORY=1`, `[memory] enabled = true`, or managed remote settings.

Cross-session memory lets Fuigo remember facts, decisions, code patterns, and debugging workflows across separate sessions in the same project.

### How it works

Memory is stored as Markdown files under `~/.fuigo/memory/`:
- **Global** (`~/.fuigo/memory/MEMORY.md`) — facts that apply across all your projects
- **Workspace** (`~/.fuigo/memory/<project-slug>-<hash8>/MEMORY.md`) — project-specific conventions and context
- **Session logs** (`~/.fuigo/memory/<project-slug>-<hash8>/sessions/`) — per-session summaries

Workspace directories are suffixed with a short hash for uniqueness (e.g. `fuigo-a3f7b2c9/`). The hash is derived from the git remote URL so all clones and worktrees of the same repository share the same memory directory.
...
At the end of each session, Fuigo saves a **structured metadata summary** to the daily session log:
...
For richer capture — decisions, patterns, debugging workflows, API discoveries — use `/flush` in the TUI. This triggers an LLM-generated summary of the current session's most important content and writes it to a dated session log under `~/.fuigo/memory/<project-slug>-<hash8>/sessions/`, where it is indexed and searchable in future sessions.
...
Fuigo searches memory automatically on the first turn of each session and after compaction. The first-turn injection can be disabled or given its own score threshold under `[memory.initial_injection]`. You can also invoke `memory_search` and `memory_get` directly via the model prompt:
...
Key options under `[memory]` in `~/.fuigo/config.toml`:
...
| `watcher.enabled` | `true` | Watch `~/.fuigo/memory/` for external edits and reindex on search |
...
When first-turn memory injection runs, Fuigo emits the `fuigo-shell-memory_injection`
...
Fuigo can restrict what the agent process and its spawned commands can access on
...
| `workspace`     | Everywhere         | CWD + `/tmp` + `~/.fuigo/` | Allowed       | Normal development       |
| `read-only`     | Everywhere         | `~/.fuigo/` only           | Blocked       | Exploration, code review |
| `strict`        | CWD + system paths | CWD + `/tmp` + `~/.fuigo/` | Blocked       | Untrusted code           |

Sensitive paths (`~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.fuigo/auth/`) are always
write-protected regardless of profile.

### Custom Profiles

Create `~/.fuigo/sandbox.toml` (global) or `.fuigo/sandbox.toml` (per-project):
...
```bash
fuigo --sandbox devbox
```

### How It Works

The sandbox is applied to the **entire fuigo process** at startup using kernel
...
- **Platform support**: Sandbox enforcement uses Landlock on Linux (kernel ≥ 5.13)
  ...
  kernel, missing entitlements), Fuigo logs a warning and continues without
  enforcement.
...
Sandbox events (profile applied, violations) are logged to `~/.fuigo/sandbox-events.jsonl`
...
Use `fuigo inspect` to see everything Fuigo discovers in the current directory:

```bash
fuigo inspect          # human-readable output
fuigo inspect --json   # machine-readable JSON
```
...
- **Skills** — from `.fuigo/skills/`, `~/.fuigo/skills/`, plugins, and config paths
...
Fuigo automatically discovers configuration from Claude Code directories alongside native `.fuigo/` paths. No extra setup is needed.

### What is picked up

| Component         | Claude Code location                                 | How Fuigo uses it                 |
| ----------------- | ---------------------------------------------------- | -------------------------------- |
| **Skills**        | `.claude/skills/`, `~/.claude/skills/`               | Loaded as skills (same as `.fuigo/skills/`) |
...
Claude Code plugins can provide skills (`skills/`), commands (`commands/`), agents (`agents/`), hooks (`hooks/hooks.json`), MCP servers (`.mcp.json`), and LSP servers (`.lsp.json`). All component types are discovered and used by Fuigo at runtime.

---

## Built-in Tools

Fuigo includes these tools by default:
...
Fetch a specific URL and return its content as markdown. **Disabled by default** — enable with `FUIGO_WEB_FETCH=1`. 

When no custom `allowed_domains` is set, the tool permits a default allowlist of useful documentation sites (Ferrox Labs, language docs, frameworks, cloud providers, databases, etc.). Domains not on the allowlist prompt the user for approval; `--always-approve` auto-approves all. Domain matching is case-insensitive, strips `www.` prefixes, and supports path-scoped entries (e.g. `fuigo/company`).

---

## Session Persistence

Fuigo automatically persists conversations to disk. This works across all modes: TUI, headless, and agent stdio.

### Storage Layout

Sessions are stored under `~/.fuigo/sessions/`, organized by URL-encoded working directory:

```
~/.fuigo/sessions/<encoded-cwd>/<session-id>/
...
fuigo -p "Remember: X=42" -s my-session
fuigo -p "What is X?" -s my-session

# Resume existing session (errors if not found)
fuigo -p "Continue" -r my-session

# Continue most recent session in current directory
fuigo -p "What were we doing?" -c
```
...
| Path                  | Description                                         |
| --------------------- | --------------------------------------------------- |
| `~/.fuigo/config.toml` | Configuration file                                  |
| `~/.fuigo/sessions/`   | Persisted sessions (organized by working directory) |
| `~/.fuigo/auth.json`   | Authentication credentials (auto-managed)           |
| `~/.fuigo/memory/`     | Cross-session memory files and index                |
| `~/.fuigo/skills/`     | User-scoped skill definitions                       |
| `~/.fuigo/plugins/`    | User-scoped plugins                                 |
| `~/.fuigo/agents/`     | User-scoped agent definitions                       |
| `.fuigo/config.toml`   | Project-scoped config (MCP servers)                 |
| `.fuigo/skills/`       | Project-scoped skill definitions                    |
| `.fuigo/plugins/`      | Project-scoped plugins                              |
| `.fuigo/agents/`       | Project-scoped agent definitions                    |
| `.fuigo/hooks/`        | Project-scoped hooks                                |
| `.fuigo/lsp.json`      | LSP server configuration                            |
...
| `FUIGO_AUTH_TOKEN_TTL`            | Token lifetime in seconds for external auth providers that output bare tokens. See [External Auth Provider](#external-auth-provider) |
| `FUIGO_AUTH_EARLY_INVALIDATION_SECS` | Seconds before `expires_at` to consider a token expired (default: `300`). See [Automatic Credential Refresh](#automatic-credential-refresh) |
| `FUIGO_OIDC_ISSUER`              | OIDC issuer URL (alternative to config file). See [OIDC](#oidc-customer-sso)                             |
| `FUIGO_OIDC_CLIENT_ID`           | OIDC client ID (alternative to config file). See [OIDC](#oidc-customer-sso)                              |
| `FUIGO_HOME`                     | Override config directory (default: `~/.fuigo`)                                                           |
| `FUIGO_SUBAGENTS`                | Enable (`1`) or disable (`0`) subagent/task tool support                                                 |
| `FUIGO_MEMORY`                   | Enable (`1`) or disable (`0`) cross-session memory                                                       |
| `FUIGO_AGENT`                    | Custom agent definition path or name (see [Agent Profiles](#agent-profiles))                             |
| `FUIGO_WEB_FETCH`                | Enable (`1`) or disable (`0`) the `web_fetch` tool                                                       |
| `FUIGO_WEB_FETCH_PROXY`          | Egress proxy URL for `web_fetch` requests (overridden by `[toolset.web_fetch] proxy_endpoint`)           |
| `FUIGO_RESPECT_GITIGNORE`        | Disable `.gitignore` filtering in tools when set to `0`                                                  |
| `FUIGO_FEEDBACK_ENABLED`         | Enable (`1`) or disable (`0`) feedback system independently from telemetry                               |
| `FUIGO_DEPLOYMENT_KEY`           | Management API key for enterprise deployments                                                            |
| `FUIGO_LOG_FILE`                 | Enable file logging by providing a file path (the value is used verbatim as the path)                    |
| `FUIGO_DEBUG_LOG`                | Debug firehose (set by `--debug`): truthy routes per-session logs to `~/.fuigo/debug/<sessionId>.txt`, a path writes that one file |
| `RUST_LOG`                      | Log filter for stderr (headless `-p` defaults to `off`, other non-TUI modes to `error`; TUI captures stderr) and for the `FUIGO_LOG_FILE` log; the `--debug` firehose ignores it |

---

## Shell Completions

Generate completions for your shell and install them to enable tab completion for `fuigo` commands and flags.
...
fuigo completions bash > ~/.local/share/bash-completion/completions/fuigo
```

Reload your shell or run `source ~/.bashrc`.

Alternative (Fuigo-managed location):

```bash
mkdir -p ~/.fuigo/completions/bash
fuigo completions bash > ~/.fuigo/completions/bash/fuigo.bash
```

Add to `~/.bashrc`:

```bash
[[ -r "$HOME/.fuigo/completions/bash/fuigo.bash" ]] && source "$HOME/.fuigo/completions/bash/fuigo.bash"
```
...
fuigo completions zsh > ~/.zsh/completions/_fuigo
```
...
```bash
mkdir -p ~/.fuigo/completions/zsh
fuigo completions zsh > ~/.fuigo/completions/zsh/_fuigo
```

Add to `~/.zshrc`:

```zsh
fpath=("$HOME/.fuigo/completions/zsh" $fpath)
autoload -Uz compinit
compinit
```
...
Regenerate completions after upgrading `fuigo` — the script reflects the CLI of the installed version.
...
Write logs to a file for debugging. The TUI captures stderr, so `RUST_LOG` alone won't produce visible output in production — use `fuigo --debug` or `FUIGO_LOG_FILE` instead:

```bash
# Per-session debug log (~/.fuigo/debug/<sessionId>.txt)
fuigo --debug

# Log to a custom path
FUIGO_LOG_FILE=/tmp/fuigo-debug.log fuigo

# Tail the most-recently-opened session's log in another terminal (Unix symlink)
tail -f ~/.fuigo/debug/latest.txt
```

The `--debug` firehose uses a fixed filter (first-party crates at `debug`) and is not narrowed by `RUST_LOG`. A `FUIGO_LOG_FILE` log defaults to `debug` and honors `RUST_LOG`, so you can set module-level filters for targeted debugging:

```bash
# Debug auth, info for everything else
FUIGO_LOG_FILE=/tmp/fuigo-debug.log RUST_LOG="info,fuigo_shell::auth=debug" fuigo
```
...
cat ~/.fuigo/sessions/<encoded-cwd>/<session-id>/summary.json | jq .

# View conversation history
cat ~/.fuigo/sessions/<encoded-cwd>/<session-id>/updates.jsonl | head -20

# Count turns in a session
wc -l ~/.fuigo/sessions/<encoded-cwd>/<session-id>/chat_history.jsonl
```
