//! Build script for bundling ripgrep for the fuigo-shell crate.
//!
//! - If `FUIGO_SHELL_BUNDLE_RG_PATH` is set, always bundle it
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("cargo:rerun-if-env-changed=FUIGO_SHELL_BUNDLE_RG_PATH");
    println!("cargo:rerun-if-env-changed=FUIGO_SHELL_RG_DOWNLOAD_BASE");
    ...
    let path_override = env::var("FUIGO_SHELL_BUNDLE_RG_PATH").ok();
    let is_release = env::var("PROFILE").as_deref() == Ok("release");
    if path_override.is_none() && !is_release {
        return Ok(());
    }

    // In Bazel builds, write into OUT_DIR; FUIGO_ROOT/target/tmp is read-only inside the sandbox
    // Outside Bazel, prefer FUIGO_ROOT's shared cache dir and fall back to OUT_DIR for standalone checkouts where FUIGO_ROOT is unset
    ...
    } else if let Ok(fuigo_root) = env::var("FUIGO_ROOT") {
        PathBuf::from(fuigo_root).join("target/tmp/fuigo-shell-bundle-rg")
    } else {
        PathBuf::from(env::var("OUT_DIR")?)
    };
    ...
    // An explicit FUIGO_SHELL_BUNDLE_RG_PATH still bundles on Windows; the override branch below copies any binary regardless of target
    ...
    println!("cargo:rustc-env=FUIGO_SHELL_RG_VER={}", RG_VER);
    println!(
        "cargo:rustc-env=FUIGO_SHELL_RG_GEN_DIR={}",
        gen_dir.display()
    );
    ...
    if let Some(path) = path_override {
        ...
        println!("cargo:rustc-env=FUIGO_SHELL_RG_TARGET=override");
        let _ = fs::remove_file(&dest);
        fs::copy(PathBuf::from(path.clone()), &dest).map_err(|e| {
            format!(
                "Failed copying FUIGO_SHELL_BUNDLE_RG_PATH: {e} from path {path} to dest {}",
                dest.display()
            )
        })?;
        return Ok(());
    ...
    let asset_triple = match (target_os.as_str(), target_arch.as_str()) {
        ...
        _ => {
            return Err(format!(
                "Unsupported target for ripgrep bundling: {os}-{arch}. Set FUIGO_SHELL_BUNDLE_RG_PATH to a local rg binary for offline or unsupported builds.",
                os = target_os,
                arch = target_arch
            ).into());
        }
    ...
    println!("cargo:rustc-env=FUIGO_SHELL_RG_TARGET={}", asset_triple);
    ...
    // Example: FUIGO_SHELL_RG_DOWNLOAD_BASE=http://<mirror>/github/BurntSushi/ripgrep/releases/download
    let download_base = env::var("FUIGO_SHELL_RG_DOWNLOAD_BASE")
    ...
    let bytes: Vec<u8> = {
        let resp = reqwest::blocking::get(&url).map_err(|e| {
            format!(
                "Failed to download ripgrep: {}\nSet FUIGO_SHELL_BUNDLE_RG_PATH to a local rg for offline builds.",
                e
            )
        })?;
        if !resp.status().is_success() {
            return Err(format!(
                "HTTP {} downloading ripgrep. Set FUIGO_SHELL_BUNDLE_RG_PATH for offline builds.",
                resp.status()
            )
            .into());
        }
    ...
    if !found {
        return Err(format!(
            "Could not find 'rg' in ripgrep archive {}. Set FUIGO_SHELL_BUNDLE_RG_PATH for offline builds.",
            url
        )
        .into());
    }
