...
impl Fixture {
    ...
    fn assert_correct(&self, runtime: &tokio::runtime::Runtime) {
        let discovered = fuigo_shell::session::worktree::candidate_worktree_cwds_for_same_repo(
...
fn create_same_repo_cwds(home: &Path) -> SameRepoTopology {
    ...
    repo.remote(
        "origin",
        "https://github.com/fuigo-org/session-list-benchmark.git",
    )
    ...
    let worktree_base = fuigo_shell::session::worktree::worktree_base_dir(&main);
    ...
    for (index, (path, label)) in linked.iter().enumerate().take(DB_OVERLAP_WORKTREE_COUNT) {
        db.register(&WorktreeRecord {
            ...
            metadata: Some(fuigo_shell::session::worktree::build_label_metadata(
                label, false,
            )),
        })
...
fn write_summary(
    ...
    let summary = Summary {
        ...
        git_remotes: vec!["git@github.com:fuigo-org/benchmark.git".to_owned()],
...
fn bench_session_list(c: &mut Criterion) {
    ...
    assert_eq!(fuigo_shell::util::fuigo_home::fuigo_home(), home.path());
