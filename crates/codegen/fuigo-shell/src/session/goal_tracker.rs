...
/// Generate a short opaque identifier scoping the per-goal scratch root (`<temp_dir>/fuigo-goal-<id>`) and the verifier verdict/details files in it.
///
...
/// Private per-goal scratch root: `<temp_dir>/fuigo-goal-<verifier_id>`.
///
...
pub(crate) fn goal_scratch_root(verifier_id: &str) -> PathBuf {
    std::env::temp_dir().join(format!("fuigo-goal-{verifier_id}"))
}
