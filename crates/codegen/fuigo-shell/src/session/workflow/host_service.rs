...
pub(crate) const WORKFLOW_MAX_AGENT_RUNS: u32 =
    (fuigo_workflow::MAX_AGENT_BUDGET as u32) * (SCHEMA_CONTRACT_RETRIES + 1);
...
pub(crate) struct WorkflowHostParams {
    ...
    pub effort: Option<fuigo_sampling_types::ReasoningEffort>,
...
impl HostService {
        ...
        match self.agent_slots.try_acquire() {
            ...
            Err(tokio::sync::TryAcquireError::NoPermits) => {
                ...
                fuigo_telemetry::session_ctx::log_event(
                    fuigo_telemetry::events::SubagentLimitHit::workflow_run_concurrent(
    ...
    async fn spawn_agent(&self, mut opts: AgentOpts) -> Result<AgentResult, HostError> {
        ...
        let isolation = opts
            .isolation_worktree
            .then_some(fuigo_tool_types::SubagentIsolationMode::Worktree);
    ...
    async fn git_diff_since(&self, commit: &str) -> Result<String, HostError> {
        ...
        fuigo_tty_utils::detach_command(&mut cmd);
...
mod tests {
    ...
    async fn a_run_keeps_at_most_max_concurrent_agents_live() {
        ...
        let succeed = |spawn: fuigo_tools::implementations::fuigo_build::task::types::SubagentSpawnRequest| {
            spawn
