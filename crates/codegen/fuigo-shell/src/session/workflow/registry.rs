...
use fuigo_workflow::{WorkflowMeta, extract_meta};
...
pub(crate) enum ResolveError {
    ...
    Meta(#[from] fuigo_workflow::MetaError),
}

pub(crate) fn project_root(session_cwd: &Path) -> PathBuf {
    fuigo_workspace::session::git::find_git_root_from_path(session_cwd)
        .unwrap_or_else(|_| session_cwd.to_path_buf())
}

pub(crate) fn user_workflow_dir() -> PathBuf {
    crate::util::fuigo_home::fuigo_home().join("workflows")
}

/// Runtime-updated builtins from the GCS subagent bundle (`~/.fuigo/bundled/workflows`).
...
impl WorkflowRegistry {
    pub(crate) fn scan(session_cwd: Option<&Path>) -> Self {
        ...
        {
            dirs.push((project_root(cwd).join(".fuigo").join("workflows"), "project"));
        }
...
pub(crate) fn resolve_by_path(
    ...
    if !in_project && !in_user_or_session {
        return Err(ResolveError::UntrustedPath {
            path: candidate.display().to_string(),
            reason: "outside the project, fuigo home, and session workflow runs".into(),
        });
    }
...
pub(crate) fn save_project_workflow(
    ...
    let dir = canonical_root.join(".fuigo").join("workflows");
...
mod tests {
    ...
    fn inline_resolution_validates_name_and_size() {
        ...
        assert!(matches!(
            ...
            Err(ResolveError::Meta(fuigo_workflow::MetaError::InvalidName))
        ));
        assert!(matches!(
    ...
    fn deterministic_scan_uses_git_root_and_skips_invalid_filename() {
        ...
        let wf_dir = dir.path().join(".fuigo").join("workflows");
    ...
    fn discovery_and_explicit_paths_reject_symlink_files() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let project = dir.path().join("project");
        let workflows = project.join(".fuigo/workflows");
