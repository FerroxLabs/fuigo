//! Re-exports of the crate-internal event types that live in `fuigo-session-events`.
//! The orphan-rule items (`From<&permission::Decision>` and the doom-loop categorizer) stay here since they need shell-local types.

pub(crate) use fuigo_session_events::tracker::EventTracker;
pub(crate) use fuigo_session_events::types::{
...
) -> Option<fuigo_sampling_types::PriorTurnInterrupt> {
    use fuigo_sampling_types::PriorTurnInterrupt;
    match category {
...
/// Committing it would silently run fuigo-build flavor.
