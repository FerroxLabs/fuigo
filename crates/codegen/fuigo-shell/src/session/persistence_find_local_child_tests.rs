...
fn returns_none_when_no_child_exists() {
    let tmp = TempDir::new().unwrap();
    let root = tmp.path().join("sessions");
    let encoded = crate::util::fuigo_home::encode_cwd_dirname("/work");
...
/// Regression: a second `fuigo -r <remote_id>` must return the existing child without creating a new restore, not return `None`.
#[test]
