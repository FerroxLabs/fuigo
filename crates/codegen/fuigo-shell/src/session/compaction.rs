...
use fuigo_sampling_types::{ApiBackend, ConversationItem};
use std::sync::Arc;
...
fn prefire_lead_percent() -> u64 {
    std::env::var("FUIGO_PREFIRE_LEAD_PERCENT")
        .ok()
...
impl SessionActor {
    ...
    pub(crate) async fn should_prefire_two_pass(&self) -> bool {
        ...
        fuigo_token_estimation::exceeds_threshold(estimated_total, cw, start_pct as u8)
    }
    ...
    async fn run_prefire_pass1_inner(self: &Arc<Self>) -> PrefirePass1Run {
        ...
        if std::env::var("FUIGO_DEBUG_TWO_PASS_FAIL_PASS1")
            .is_ok_and(|v| matches!(v.trim(), "1" | "true" | "yes" | "on"))
        {
            tracing::info!(
                target: "two_pass",
                "two_pass: DEBUG FUIGO_DEBUG_TWO_PASS_FAIL_PASS1 — prefire pass1 produces no cache"
            );
        ...
        let prefix_est_tokens = prefix_prepared
            .iter()
            .map(fuigo_chat_state::estimate_item_tokens)
            .sum::<u64>();
...
/// This only APPROXIMATES the reseed done by `fuigo-chat-state` `replace_conversation`, the authority.
...
impl SessionActor {
    ...
    async fn maybe_pre_compaction_flush(
        ...
        if fuigo_memory::flush::should_flush(
            total_tokens,
            context_window,
    ...
    pub(crate) async fn run_compact(
        ...
        self.send_fuigo_notification(FuigoSessionUpdate::AutoCompactCompleted {
    ...
    async fn emit_compact_cancelled(&self, auto_trigger: bool) -> Result<(), acp::Error> {
        if auto_trigger {
            use crate::extensions::notification::SessionUpdate as FuigoSessionUpdate;
            self.send_fuigo_notification(FuigoSessionUpdate::AutoCompactCancelled {
    ...
    async fn suppress_auto_compaction(
            ...
            .is_ok()
        {
            ...
            fuigo_telemetry::session_ctx::log_event(
                fuigo_telemetry::events::AutoCompactSuppressed {
    ...
    fn user_facing_compact_error(raw: &str) -> String {
        ...
        const INTERNAL_PREFIXES: &[&str] = &[
            ...
            fuigo_compaction::sampler::SAMPLER_BUILD_FAILED_PREFIX,
            fuigo_compaction::sampler::SAMPLER_START_FAILED_PREFIX,
        ];
        ...
        match preserve_inherited_prefix(&full_conv, compacted_history, prefix_len) {
            Ok(preserved) => {
                let projected_preserved = project_preserved_reseed_tokens(
                    fuigo_chat_state::estimate_conversation_tokens(&preserved),
                    tokens_before,
                    fuigo_chat_state::estimate_conversation_tokens(&full_conv),
                );
    ...
    async fn run_compact_inner(
        ...
        let trigger_str = match trigger {
            fuigo_telemetry::events::CompactionTrigger::Manual => "manual",
            fuigo_telemetry::events::CompactionTrigger::Auto => "auto",
        };
        let sampling_config = self.chat_state_handle.get_sampling_config().await;
        let context_window = sampling_config
            .as_ref()
        ...
        let compaction = fuigo_telemetry::events::CompactionScope::begin(
            fuigo_telemetry::events::CompactionBeginParams {
                trigger,
                ...
                compaction_mode: match self.compaction.compaction_mode {
                    fuigo_chat_state::CompactionMode::Summary => {
                        fuigo_telemetry::events::CompactionModeLabel::Summary
                    }
                    fuigo_chat_state::CompactionMode::Transcript => {
                        fuigo_telemetry::events::CompactionModeLabel::Transcript
                    }
                    fuigo_chat_state::CompactionMode::Segments(_) => {
                        fuigo_telemetry::events::CompactionModeLabel::Segments
                    }
                },
        ...
        self.dispatch_hook(
            fuigo_hooks::event::HookEventName::PreCompact,
            fuigo_hooks::event::HookPayload::PreCompact {
        ...
        let mut simplified_messages = if verbatim_input_enabled {
            fuigo_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                full_conversation,
        ...
        if lossy_input {
            simplified_messages = fuigo_chat_state::compaction_utils::fit_conversation_to_budget(
                simplified_messages,
        ...
        let estimated_input_tokens =
            fuigo_chat_state::estimate_conversation_tokens(&simplified_messages);
        let auto_trigger = matches!(trigger, fuigo_telemetry::events::CompactionTrigger::Auto);
        let wall_clock_budget_secs = self
            .agent
            .borrow()
            .compaction_policy()
        ...
        let fr_config = fuigo_compaction::FullReplaceConfig {
        ...
        while compact_summary.is_none() {
            match fuigo_compaction::sample_full_replace_summary(
                &sampler,
                &request_turns,
            ...
            .await
            {
                ...
                Err(fuigo_compaction::FullReplaceError::Sampler {
                    message,
                    deterministic,
                    context_overflow,
                }) => {
                    ...
                    if context_overflow {
                        ...
                        if let Some(stage) = next_stage {
                            ...
                            fuigo_telemetry::session_ctx::log_event(
                                fuigo_telemetry::events::CompactionRetryDegraded {
                                    trigger,
                            ...
                            request_turns = match stage {
                                InputStage::VerbatimFitted => {
                                    ...
                                    let verbatim = fuigo_chat_state::compaction_utils::prepare_conversation_for_verbatim_summarization(
                                        conv,
                                        summary_strips_reasoning,
                                    );
                                    fuigo_chat_state::compaction_utils::fit_conversation_to_budget(
                                        verbatim,
                                        budget,
                                    )
                                }
                    ...
                    let running_tasks: Vec<_> = pending_tasks
                        ...
                        .map(|t| {
                            let tool_name = match t.kind {
                                fuigo_tools::computer::types::TaskKind::Monitor => {
                                    monitor_tool_name.clone()
                                }
                                fuigo_tools::computer::types::TaskKind::Bash => {
        ...
        let memory_ref: Option<&dyn fuigo_tools::types::memory_backend::MemoryBackend> =
            if memory_opt_out {
                None
            } else {
                memory_backend_impl
                    .as_ref()
                    .map(|b| b as &dyn fuigo_tools::types::memory_backend::MemoryBackend)
            };
            ...
            .and_then(|item| match item {
                ConversationItem::User(parts) => {
                    parts.content.into_iter().next().and_then(|p| match p {
                        fuigo_sampling_types::ContentPart::Text { text } => {
        ...
        if self.memory.is_enabled() {
            tracing::info!(target: fuigo_telemetry::memory_log::TARGET, "MEMORY_COMPACT: post-compaction reset, next turn re-checks injection (search only if no block persisted)");
        }
        let _ = self
            .notifications
            .persistence_tx
        ...
        self.dispatch_hook(
            fuigo_hooks::event::HookEventName::PostCompact,
            fuigo_hooks::event::HookPayload::PostCompact {
                source: compact_source.into(),
            },
            None,
            None,
        ...
        compaction.complete(
            fuigo_telemetry::events::CompactionCompleteStats {
                tokens_after,
                two_pass_used,
                segments_queued,
            ...
            fuigo_telemetry::events::CompactionTiming {
    ...
    pub(crate) fn should_auto_compact(
        ...
        if fuigo_token_estimation::exceeds_threshold(
            ...
            let percentage = fuigo_token_estimation::usage_percentage_u8(total_tokens, cw);
    ...
    pub(crate) async fn should_compact_on_error(
        &self,
        err: &fuigo_sampler::SamplingErrorInfo,
    ) -> bool {
        if self.compaction.is_suppressed() {
            return false;
        }
    ...
    pub(crate) async fn check_auto_compact_needed(&self) -> Option<AutoCompactTriggerInfo> {
        ...
        if self
            ...
            .is_ok()
        {
            let percentage = fuigo_token_estimation::usage_percentage_u8(estimated_total, cw);
            tracing::info!(
    ...
    pub(crate) async fn run_compact_only(
        ...
        fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::AutoCompactFired {
            tokens_before: trigger_info.tokens_used,
            percentage: trigger_info.percentage,
        });
        self.signals_handle()
            .record_compaction(trigger_info.tokens_used);
        self.send_fuigo_notification(FuigoSessionUpdate::AutoCompactStarted {
        ...
        let result = self
            .run_compact_inner(
                None,
                None,
                fuigo_telemetry::events::CompactionTrigger::Auto,
                lossy_input,
            )
            .await;
    ...
    fn persist_compaction_request_artifact(
        ...
        tools: Vec<fuigo_sampling_types::ToolSpec>,
        user_context: Option<&str>,
