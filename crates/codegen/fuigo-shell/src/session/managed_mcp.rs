...
//! Managed connectors exist only via the gateway catalog (`GET /v1/mcp/tools/list`), not as injected `fuigo_com_*` HTTP servers.
//!
...
//!
//! The gateway catalog/call core lives in `fuigo_shell_session_support::managed_mcp`.
//! It is re-exported here so `crate::session::managed_mcp::…` paths keep resolving unchanged.

pub use fuigo_shell_session_support::managed_mcp::*;
...
pub(crate) fn merge_managed_mcp_servers_sourced(
    ...
    let config_source = ConfigSource::ConfigToml {
        path: fuigo_tools::util::fuigo_home::fuigo_home().join("config.toml"),
    };
...
pub(crate) struct McpDiscoveryInputs<'a> {
    ...
    pub plugin_registry: Option<&'a fuigo_agent::plugins::PluginRegistry>,
    pub compat: &'a fuigo_tools::types::compat::CompatConfig,
}
...
fn load_plugin_mcp_servers_from_config(
    ...
    let sub = |s: &str| -> String {
        let s = fuigo_agent::plugins::manifest::substitute_env_vars(s, plugin_root, plugin_data);
...
mod tests {
    ...
    fn toml_claim_survives_when_client_cursor_insert_skipped() {
        ...
        std::fs::write(
            cwd.path().join(".fuigo").join("config.toml"),
            r#"
...
"#,
    ...
    fn merge_drops_server_denied_by_name_including_managed_prefix() {
        ...
        let tagged = apply_mcp_server_policy(
            vec![
                acp::McpServer::Http(
                    acp::McpServerHttp::new("fuigo_com_slack", "https://mcp.slack.com/sse")
                        .headers(vec![]),
                ),
        ...
        let slack = tagged
            .iter()
            .find(|s| mcp_server_name(&s.server) == "fuigo_com_slack")
    ...
    #[test]
    fn policy_server_name_matches_legacy_fuigo_com_runtime_spelling() {
        use fuigo_workspace::permission::resolution::{AllowedMcpServer, McpServerAllowlist};
        ...
        for (display, runtime) in [
            ("Slack", "fuigo_com_slack"),
            ("My Server", "fuigo_com_my_server"),
        ] {
...
"#,
    ...
    fn load_plugin_mcp_creates_stdio_server_with_env_substitution() {
        let config: crate::util::config::McpConfig = serde_json::from_value(serde_json::json!({
            "mcpServers": {
                "echo-mcp": {
                    "command": "python3",
                    "args": ["${FUIGO_PLUGIN_ROOT}/mcp-echo-server.py"]
                }
            }
        }))
        ...
        let (servers, _) = load_plugin_mcp_servers_from_config(
            &config,
            "team-tool",
            "/home/user/.fuigo/plugins/team-tool",
            "/home/user/.fuigo/plugin-data/team-tool",
        );
        ...
        match &servers[0] {
            acp::McpServer::Stdio(acp::McpServerStdio {
                ...
                assert_eq!(
                    args.as_slice(),
                    &["/home/user/.fuigo/plugins/team-tool/mcp-echo-server.py"]
                );
            }
    ...
    fn plugin_server_deduped_across_file_and_inline() {
        use fuigo_agent::plugins::PluginRegistry;
        use fuigo_agent::plugins::PluginScope;
        use fuigo_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use fuigo_agent::plugins::manifest::{PathOrInline, PluginManifest};
        ...
        let dp = DiscoveredPlugin {
            ...
            origin: fuigo_agent::plugins::PluginOrigin::UserFuigo,
            trusted: true,
            skill_dirs: vec![],
            command_dirs: vec![],
            agent_dirs: vec![],
    ...
    fn collect_plugin_oauth_configs_reads_byo_client_id_from_mcp_json() {
        ...
        use fuigo_agent::plugins::PluginScope;
        use fuigo_agent::plugins::discovery::{DiscoveredPlugin, PluginId};
        use fuigo_agent::plugins::manifest::PluginManifest;
