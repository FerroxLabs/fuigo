...
//! When a user message contains image content blocks, the session calls a vision-capable Fuigo model.
...
impl ImageDescribeCache {
    ...
    pub(crate) async fn get_or_describe(
        &self,
        client: fuigo_sampler::SamplingClient,
        model: &str,
        raw_bytes: &[u8],
        mime_type: &str,
        outline: Option<&str>,
...
mod tests {
    use super::*;
    use fuigo_sampling_types::conversation::{ConversationItem, UserItem};
    #[test]
    ...
    fn user(text: &str) -> ConversationItem {
        ConversationItem::User(UserItem {
            content: vec![fuigo_sampling_types::conversation::ContentPart::Text {
                text: text.into(),
            }],
            synthetic_reason: None,
            ..Default::default()
