//! ACP conversion functions for `fuigo-tools`'s `ToolOutput`.
//!
//! These standalone functions convert `fuigo_tools::types::output::ToolOutput`
...
///
/// In forked sessions, tools produce output containing the worktree directory (e.g., `/root/.fuigo/worktrees/project/fork-019cb252-...`).
...
impl PathRewriter {
    ...
    ///
    /// Handles both plain paths (e.g., `/root/.fuigo/worktrees/project/fork-...`)
    /// and URL-encoded paths (e.g., `%2Froot%2F.fuigo%2Fworktrees%2F...`) that appear in session directory structures and `output_file` references.
...
pub(crate) fn acp_tool_update(
    ...
    match output {
        ToolOutput::ReadFile(read_file_output) => {
            let (content, status) = match read_file_output {
                ...
                ReadFileOutput::ImageContent(image_content) => {
                    ...
                    // through a `From` impl on the tools crate, so that `fuigo-tools` stays free of an `agent-client-protocol` dependency
        ...
        ToolOutput::WebFetch(web_fetch_output) => {
            use fuigo_tools::types::output::WebFetchOutput;
        ...
        ToolOutput::Todo(todo_output) => {
            use fuigo_tools::types::output::TodoWriteOutput;
        ...
        ToolOutput::AskUserQuestion(ask) => {
            let message = match ask {
                fuigo_tools::types::output::AskUserQuestionOutput::UserAnswered { message } => {
                    message.clone()
                }
                fuigo_tools::types::output::AskUserQuestionOutput::QuestionsSent {
                    message,
                    ..
                } => message.clone(),
            };
        ...
        ToolOutput::EnterPlanMode(enter) => {
            let message = match enter {
                fuigo_tools::types::output::EnterPlanModeOutput::Entered { message, .. } => {
                    message.clone()
                }
            };
            Some(acp::ToolCallUpdate::new(
...
///
/// This converts `fuigo-tools`' TodoItem (which has `id`, `content: Option<String>`,
...
mod tests {
    ...
    fn test_acp_tool_update_send_subagent_message_outcomes_are_terminal() {
        use fuigo_tools::implementations::fuigo_build::send_subagent_message::SendSubagentMessageOutput::*;
    ...
    fn test_path_rewriter_new_returns_some_when_different() {
        let rw = PathRewriter::new(
            "/root/.fuigo/worktrees/project/ab-123",
    ...
    fn test_path_rewriter_rewrite_text() {
        let rw = PathRewriter::new(
            "/root/.fuigo/worktrees/myproject/ab-123",
            Some("/testbed/myproject"),
        )
        .unwrap();
        let input = "File at /root/.fuigo/worktrees/myproject/ab-123/src/main.rs";
    ...
    fn test_path_rewriter_rewrite_path() {
        ...
        .unwrap();
        let path = Path::new("/root/.fuigo/worktrees/myproject/ab-123/src/lib.rs");
    ...
    fn test_path_rewriter_rewrites_raw_output_json() {
        ...
        let output = ToolOutput::ReadFile(ReadFileOutput::FileContent(FileContent {
            content: "content".to_string(),
            content_concise: None,
            absolute_path: PathBuf::from("/root/.fuigo/worktrees/myproject/ab-123/src/main.rs"),
            offset: None,
            limit: None,
            raw_output: "content".to_string(),
            total_lines: 100,
    ...
    fn test_path_rewriter_rewrites_list_dir_raw_output() {
        ...
        let output = ToolOutput::ListDir(ListDirOutput::Content(ListDirContent {
            content: "file1.rs\nfile2.rs".to_string(),
            absolute_root_path: PathBuf::from("/root/.fuigo/worktrees/myproject/ab-123/src"),
        }));
        let json = raw_output_json(&output, Some(&rw)).unwrap();
        let round_tripped: ToolOutput = serde_json::from_value(json).unwrap();
        match round_tripped {
    ...
    fn test_path_rewriter_rewrites_bash_command_and_output() {
        ...
        let output = ToolOutput::Bash(BashOutput {
            output: b"listing /root/.fuigo/worktrees/myproject/ab-123/src".to_vec(),
            output_for_prompt: String::new(),
            exit_code: 0,
            command: "ls /root/.fuigo/worktrees/myproject/ab-123/src".to_string(),
            truncated: false,
            signal: None,
            timed_out: false,
            description: None,
            current_dir: "/root/.fuigo/worktrees/myproject/ab-123".to_string(),
    ...
    fn test_path_rewriter_rewrites_search_replace_diff_path() {
        ...
        let output = ToolOutput::SearchReplace(SearchReplaceOutput::EditsApplied(
            SearchReplaceEditsApplied {
                ...
                absolute_path: PathBuf::from("/root/.fuigo/worktrees/myproject/ab-123/src/lib.rs"),
        ...
        assert!(
            !raw_str.contains("/root/.fuigo/worktrees/myproject/ab-123"),
    ...
    fn test_rewrite_handles_url_encoded_paths() {
        let rw = PathRewriter::new(
            "/root/.fuigo/worktrees/project/ab-123-a-overlay",
            Some("/home/user/project"),
        )
        .unwrap();
    ...
    fn test_rewrite_handles_plain_paths() {
        ...
        .unwrap();
        let input = "file: /root/.fuigo/worktrees/project/ab-123-a-overlay/src/main.rs";
        let result = rw.rewrite(input);
    ...
    fn test_rewrite_json_handles_url_encoded_paths() {
        ...
        .unwrap();
        let encoded = urlencoding::encode("/root/.fuigo/worktrees/project/ab-123");
    ...
    fn test_maybe_rewrite_with_rewriter_sanitizes_error_text() {
        let rw = PathRewriter::new(
            "/root/.fuigo/worktrees/project/ab-999",
            Some("/home/user/project"),
        )
        .unwrap();
        let error_text = "Tool `read_file` failed: IO error reading /root/.fuigo/worktrees/project/ab-999/src/lib.rs".to_string();
    ...
    fn test_no_rewriter_preserves_original_paths() {
        ...
        match round_tripped {
            ToolOutput::ReadFile(ReadFileOutput::FileContent(fc)) => {
                assert_eq!(
                    fc.absolute_path,
                    PathBuf::from("/root/.fuigo/worktrees/myproject/ab-123/src/main.rs")
                );
            }
            other => panic!("Expected ReadFile, got {:?}", other),
        }
