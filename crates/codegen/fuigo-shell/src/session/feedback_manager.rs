...
pub(crate) fn new_submission(
    ...
    s.shell_version = Some(fuigo_version::VERSION.to_string());
    s
}
...
pub(crate) async fn submit_feedback_workflow(
    ...
    if let Some(user_meta) = crate::agent::mvp_agent::parse_json_object_env("FUIGO_USER_METADATA") {
    ...
    if telemetry_enabled {
        ...
        fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::UserFeedback {
...
pub struct FeedbackManagerConfig {
    ...
    /// Process exit clamps this under [`SHUTDOWN_DRAIN_CAP`] (or `FUIGO_SESSION_EXIT_DRAIN_SECS`) so a hung upload cannot exceed the agent join grace.
...
pub struct FeedbackManager {
    ...
    upload_queue_stats: std::sync::OnceLock<Arc<fuigo_file_utils::queue::UploadQueueStats>>,
}
...
impl FeedbackManager {
    ...
    pub(crate) fn set_upload_queue_stats(
        &self,
        stats: Arc<fuigo_file_utils::queue::UploadQueueStats>,
    ) {
    ...
    /// Non-empty drains use `min(config.drain_timeout, cap)` (default cap 5s, `FUIGO_SESSION_EXIT_DRAIN_SECS` up to hard max 7s).
    /// Incomplete drains leave durable pairs on disk for next-session recovery.
    pub async fn shutdown(&self, queue: Option<&fuigo_file_utils::queue::UploadQueue>) {
        let pending = queue
...
/// Override with `FUIGO_SESSION_EXIT_DRAIN_SECS` (still hard-capped by [`SHUTDOWN_DRAIN_HARD_MAX`]).
...
/// Fleets on slow networks may raise it with `FUIGO_SESSION_EXIT_DRAIN_SECS`, up to [`SHUTDOWN_DRAIN_HARD_MAX`].
/// Raising it does not slow the empty-session fast path.
fn nonempty_drain_budget(config_timeout: Duration) -> Duration {
    let cap = std::env::var("FUIGO_SESSION_EXIT_DRAIN_SECS")
        .ok()
...
mod tests {
    ...
    fn test_shutdown_budgets_fit_under_session_flush_grace() {
        ...
        let _unset = fuigo_test_support::env::EnvGuard::unset("FUIGO_SESSION_EXIT_DRAIN_SECS");
        assert!(
    ...
    fn test_nonempty_drain_budget_env_raises_cap() {
        {
            let _guard =
                fuigo_test_support::env::EnvGuard::set("FUIGO_SESSION_EXIT_DRAIN_SECS", "7");
            assert_eq!(
                nonempty_drain_budget(Duration::from_secs(30)),
        ...
        {
            let _guard =
                fuigo_test_support::env::EnvGuard::set("FUIGO_SESSION_EXIT_DRAIN_SECS", "99");
            assert_eq!(
                nonempty_drain_budget(Duration::from_secs(30)),
...
mod author_identity_tests {
    ...
    async fn env_var_identity_reaches_the_wire_end_to_end() {
        ...
        let mut value = toml::from_str::<toml::Value>(
            r#"
[feedback.user]
name = ["$FUIGO_TEST_WORK_NAME"]
email = ["$FUIGO_TEST_WORK_EMAIL"]
"#,
        )
        .unwrap();
    ...
    /// `FUIGO_USER_METADATA` is merged into the submission and travels with it: onto the wire body for triage and onto the local feedback.jsonl entry.
    #[tokio::test]
    #[serial_test::serial]
