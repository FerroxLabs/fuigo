...
fn switch_target_config(model: &str, base_url: String) -> fuigo_sampler::SamplerConfig {
    fuigo_sampler::SamplerConfig {
    ...
    local
        .run_until(async {
            ...
            actor.chat_state_handle.replace_conversation(vec![
                ...
                ConversationItem::Reasoning(fuigo_sampling_types::rs::ReasoningItem {
                ...
                ConversationItem::assistant_tool_calls(vec![fuigo_sampling_types::ToolCall {
                    id: std::sync::Arc::<str>::from("call_fuigo_minted_id"),
                    name: "run_terminal_command".to_string(),
                    arguments: std::sync::Arc::<str>::from(r#"{"command":"ls"}"#),
                }]),
                ConversationItem::ToolResult(fuigo_sampling_types::ToolResultItem {
                    tool_call_id: "call_fuigo_minted_id".to_string(),
...
fn user_facing_compact_error_strips_prefix_single_lines_and_caps() {
    ...
    let nested = fuigo_compaction::sampler::CompactionSampleError::Build(format!(
...
fn classify_suppress_reason_maps_error_text() {
    ...
    assert_eq!(
        classify("API error (status 402 Payment Required): Fuigo usage balance exhausted"),
        SuppressReason::CreditBlock
    );
    assert_eq!(
        classify("Fuigo usage limit reached"),
        SuppressReason::CreditBlock
    );
    assert_eq!(
...
fn api_error_with_context_window(context_window: u64) -> fuigo_sampler::SamplingErrorInfo {
...
async fn get_transcript_path_returns_some_when_file_exists() {
    ...
    local
        .run_until(async {
            ...
            actor.compaction.compaction_mode = fuigo_chat_state::CompactionMode::Transcript;
            ...
            actor.compaction.compaction_mode = fuigo_chat_state::CompactionMode::Summary;
