...
//!
//! Receives [`fuigo_mcp::servers::McpClientEvent`]s emitted by:
//! - per-client transport-liveness watchers ([`fuigo_mcp::liveness`]),
//! - the [`fuigo_mcp::servers::FuigoClientHandler`] (server-pushed `tools/list_changed` and `resources/list_changed`),
...
pub enum McpServerStatus {
    /// Client is in [`fuigo_mcp::servers::ClientStateKind::Ready`] and the transport is healthy.
    Ready,
...
fn flush_elicitation_completes(
    ...
    for (server, elicitation_id) in completes {
        let payload = fuigo_tools::mcp_elicitation::McpElicitCompletePayload {
            session_id: session_id.to_string(),
            elicitation_id,
        ...
        match serde_json::value::to_raw_value(&payload) {
            Ok(raw) => {
                gateway.forward_fire_and_forget(acp::ExtNotification::new(
                    fuigo_mcp::wire::MCP_ELICIT_COMPLETE,
                    raw.into(),
                ));
            }
            Err(e) => {
...
mod tests {
    ...
    async fn run_dispatcher_forwards_completes_when_buf_is_empty() {
        ...
        let gateway = fuigo_acp_lib::AcpAgentGatewaySender::new(gw_tx);
    ...
    /// Gateway catalog ids are `Managed`; `fuigo_com_*` spawned names are not.
    #[test]
    fn classify_source_gateway_is_managed() {
        ...
        assert_eq!(classify_source("fuigo_com_linear"), McpServerSource::Local);
    ...
    fn recoverable_http_servers_excludes_stdio_and_disabled() {
        ...
        let want: HashSet<String> = ["http-mcp-server".to_string(), "fuigo_com_slack".to_string()]
            .into_iter()
            .collect();
        assert_eq!(got, want);
    }
    ...
    fn discard_gateway() -> fuigo_acp_lib::AcpAgentGatewaySender {
