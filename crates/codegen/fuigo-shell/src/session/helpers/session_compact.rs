...
///
/// All other agents (fuigo-build, etc.) continue to use the detailed structured prompt built inline in `generate_session_compact`.
...
fn classify_response_event_error(code: Option<&str>, message: &str) -> CompactFailure {
    ...
    if code.is_some_and(fuigo_sampling_types::is_size_overflow_error_code)
...
pub(crate) fn build_compaction_prompt(
    ...
    if use_short_prompt {
        ...
        // Default (fuigo-build, codex, ...): the concise summarize prompt the fuigo-build models are RL-trained on
...
pub(crate) async fn generate_session_compact(
    ...
    let output = match sampling_config.api_backend {
        ApiBackend::ChatCompletions => {
            ...
            message.x_fuigo_conv_id = Some(sid.clone());
            message.x_fuigo_req_id = Some(format!("fuigo-compact-{}", uuid::Uuid::new_v4()));
            message.x_fuigo_session_id = Some(sid);
            message.x_fuigo_agent_id = Some(fuigo_telemetry::id::agent_id());
            ...
            loop {
                ...
                match chunk_result {
                    Ok(event) => {
                        ...
                        fuigo_sampling_types::messages::MessageStreamEvent::MessageDelta { delta, .. } => {
                            if let Some(sr) = delta.stop_reason {
                                truncated = matches!(
                                    sr,
                                    fuigo_sampling_types::messages::StopReason::MaxTokens
                                        | fuigo_sampling_types::messages::StopReason::ModelContextWindowExceeded
                                );
