//! Rendering helpers for [`CompactionStateContext`] that depend on shell-specific types (`fuigo_tools::MemoryBackend`, memory context).
//!
//! The core [`CompactionStateContext`] struct and its builder live in `fuigo_chat_state::compaction_utils`.
//! This module adds system-reminder rendering that requires dependencies not available in `fuigo-chat-state`.
//!
//! The **common** active-agent section (Running Background Tasks: commands, loops, workflows, subagents) plus TODO is formatted by [`fuigo_compaction::reminder`].
//! That keeps fuigo-chat and fuigo-build in lockstep.
...
pub fn to_system_reminder_sync(
    ...
    skills: &[fuigo_tools::implementations::skills::types::SkillInfo],
...
pub async fn to_system_reminder(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[fuigo_tools::implementations::skills::types::SkillInfo],
    memory_backend: Option<&dyn fuigo_tools::types::memory_backend::MemoryBackend>,
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
    workflow_listing: Option<&str>,
    memory_storage: Option<&fuigo_memory::storage::MemoryStorage>,
) -> Option<String> {
...
fn to_system_reminder_inner(
    ...
    discovered_agents_md: &[PathBuf],
    skills: &[fuigo_tools::implementations::skills::types::SkillInfo],
    memory_results: &[fuigo_tools::types::memory_backend::MemorySearchResult],
    subagent_tool_names: Option<&SubagentToolNames>,
    mcp_tool_names: Option<&McpToolNames>,
    workflow_listing: Option<&str>,
    memory_storage: Option<&fuigo_memory::storage::MemoryStorage>,
    ...
    if let Some(listing) =
        fuigo_tools::types::skill_discovery_tracker::format_compaction_skill_listing(skills)
    {
    ...
    if !ctx.connected_mcp_servers.is_empty() {
        use fuigo_tools::implementations::search_tool::format_compaction_server_line;
    ...
    if !memory_results.is_empty()
        && let Some(storage) = memory_storage
        && let Some(reminder) =
            super::memory_context::format_memory_reminder_with_storage(memory_results, storage)
    {
...
- linear (12 tools)
    ...
    fn system_reminder_places_workflows_below_skills() {
        ...
        let skills = [fuigo_tools::implementations::skills::types::SkillInfo {
            name: "commit".into(),
