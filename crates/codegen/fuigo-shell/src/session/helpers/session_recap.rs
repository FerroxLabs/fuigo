...
use fuigo_chat_state::{compaction_utils, estimate_conversation_tokens, estimate_item_tokens};
...
pub(crate) fn build_instruction_items(
    ...
    let mut items = if strip_reasoning {
        fuigo_chat_state::compaction_utils::strip_reasoning_blocks(conversation)
    } else {
        conversation
    };
...
/// This is the verified `max_prompt_length` for current `fuigo-build` / `grok-4.5` product backends (`500000`).
...
/// (The 85% threshold, the headroom, and the 500k cap make that unlikely for normal fuigo-build sessions.)
///
/// * Fast path: if the whole snapshot already fits, returns `build_instruction_items(...)` verbatim.
///   This keeps the fuigo prefix KV cache warm and honors the caller's `strip_reasoning`.
...
pub(crate) fn budget_instruction_items(
    ...
    // Un-stripped estimate is a safe upper bound (stripping only shrinks); the verbatim path keeps the fuigo prefix cache warm
...
mod tests {
    ...
    fn main_turn_count_counts_real_users_only() {
        use fuigo_sampling_types::{ContentPart, SyntheticReason, ToolCall, UserItem};
        use std::sync::Arc;
    ...
    fn mk_tool_call(id: &str, args: &str) -> fuigo_sampling_types::ToolCall {
        use std::sync::Arc;
        fuigo_sampling_types::ToolCall {
            id: Arc::from(id),
            name: "read_file".into(),
    ...
    #[test]
    fn budget_over_budget_strips_reasoning_even_on_fuigo() {
        let conv = vec![
            mk_reasoning("r1"),
            ConversationItem::assistant("did stuff"),
            ConversationItem::user("z".repeat(40_000)),
        ];
        // The fuigo backend passes strip_reasoning=false, but the over-budget branch must strip reasoning anyway
    ...
    #[test]
    fn budget_fast_path_keeps_reasoning_on_fuigo() {
        let conv = vec![
            mk_reasoning("r1"),
            ConversationItem::assistant("did stuff"),
            ConversationItem::user("small"),
        ];
        // The snapshot fits under a large window on fuigo (strip_reasoning=false), so it is returned verbatim
        // Reasoning is kept so the prefix KV cache stays warm
        let out = budget_recap_items(conv, "system-reminder", false, 256_000);
        assert!(
            out.iter()
                .any(|i| matches!(i, ConversationItem::Reasoning(_))),
            "fits path on fuigo must keep reasoning verbatim"
        );
    }
