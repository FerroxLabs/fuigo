...
/// `~/.fuigo/worktree_pool/`, reads each `.pid` file, and checks
...
pub fn cleanup_stale_pool_worktrees(source_git_root: Option<&Path>) {
    ...
    if let Some(git_root) = source_git_root {
        ...
        REGISTRATION_CLEANUP_ONCE.call_once(move || {
            let removed = fuigo_fast_worktree::remove_stale_worktree_registrations_under(
                &root,
                &pool_base_directory(),
            );
            tracing::info!(
...
/// The base pool directory under `~/.fuigo/`.
fn pool_base_directory() -> PathBuf {
    fuigo_home().join("worktree_pool")
}
