...
use fuigo_workspace::file_system::{
    FileReference, render_embedded_resource, render_file_reference,
};
use serde::Deserialize;
use std::path::PathBuf;
/// Some templates put `<user_query>` last (context first); Fuigo puts it first.
...
pub struct ParsedPrompt {
    ...
    /// Fuigo mode may include editor open/focus metadata; the compat mode does not.
...
impl ParsedPrompt {
    ...
    /// Layout:
    /// - **Fuigo mode:** `<user_query>`, then `<skill_information>`, then context
...
///
/// When `is_cursor` is false, produces original Fuigo-format output:
...
{}
    ...
    if !resource_links.is_empty() {
        ...
        context.push_str(&render_resource_links_fuigo(resource_links));
    }
    (context, query)
}
...
/// Fuigo-format resource links: `<focused_files>` / `<open_files>` with metadata inside a `<system-reminder>` wrapper.
fn render_resource_links_fuigo(resource_links: &[acp::ResourceLink]) -> String {
...
mod tests {
    ...
    /// Shorthand: render and assemble for fuigo mode.
    fn render_fuigo(
        message: &str,
        embedded: Vec<String>,
    ...
    #[test]
    fn test_fuigo_render_plain_message() {
        let result = render_fuigo("hello", vec![], vec![], &[], false);
        assert_eq!(result, "<user_query>\nhello\n</user_query>");
    }
    #[test]
    fn test_fuigo_render_with_attachments_uses_system_reminder_wrapper() {
        let result = render_fuigo(
            "check this",
        ...
        assert!(
            ...
            "Fuigo should start with <user_query>, got: {result}"
        );
    }
    #[test]
    fn test_fuigo_render_user_query_first() {
        let link = acp::ResourceLink::new("doc.md", "file:///doc.md")
            .title(Some("My Doc".into()))
            .size(Some(1024));
        let result = render_fuigo("hello", vec![], vec![], &[link], false);
        let uq_pos = result.find("<user_query>").unwrap();
        let rr_pos = result.find("Referenced resources:").unwrap();
        assert!(
            uq_pos < rr_pos,
            "Fuigo: <user_query> ({uq_pos}) should come before resource links ({rr_pos})\ngot: {result}"
        );
        assert!(result.contains("<system-reminder>"));
    }
    #[test]
    fn test_fuigo_render_resource_links_use_focused_files_format() {
        let links = vec![
        ...
        let result = render_fuigo("hello", vec![], vec![], &links, false);
