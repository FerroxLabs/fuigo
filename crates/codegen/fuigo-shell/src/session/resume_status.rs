...
pub(crate) fn resume_task_description(
    ...
    display_command.and_then(monitor_label).map(str::to_string)
}

/// The human label of a monitor's `[monitor] <label>` pseudo-command; `None` for ordinary bash tasks.
fn monitor_label(display_command: &str) -> Option<&str> {
    display_command
...
pub(crate) fn loops_from_resources_state(session_dir: &Path) -> Vec<ResumeLoop> {
    ...
    let Some(tasks) = value
        .pointer("/state/fuigo_build.Scheduler/tasks")
