...
use crate::extensions::notification::SessionNotification as FuigoSessionNotification;
...
/// The two variants stay separate because ACP and Ferrox Labs chunks merge under different rules and wire envelopes.
/// One-shot Ferrox Labs events (RetryState, ImageCompressed, HookExecution, etc.) instead take `send_fuigo_notification` for per-event hooks and persistence.
#[derive(Debug, Clone)]
pub(crate) enum SessionNotification {
    Acp(Box<AcpSessionNotification>),
    Fuigo(Box<FuigoSessionNotification>),
}
...
impl From<FuigoSessionNotification> for SessionNotification {
    fn from(n: FuigoSessionNotification) -> Self {
        Self::Fuigo(Box::new(n))
    }
}
...
impl SessionNotification {
    pub(crate) fn expect_acp(&self) -> &AcpSessionNotification {
        match self {
            Self::Acp(n) => n,
            Self::Fuigo(_) => panic!("expected Acp notification, got Fuigo"),
        }
    }
