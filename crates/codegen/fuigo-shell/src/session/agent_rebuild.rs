//! `AgentRebuildSpec` is the canonical recipe for constructing an [`fuigo_agent::Agent`] for a given session.
//!
//! INVARIANT: This is the **only** place in the shell crate that calls [`fuigo_agent::AgentBuilder::new`].
...
//! ## Why this exists
//!
//! [`fuigo_agent::Agent`] owns an [`fuigo_tools::bridge::ToolBridge`] that carries session-scoped channels.
...
//!
//! ## WHEN ADDING A NEW [`fuigo_agent::AgentBuilder`]`::with_*` KNOB
//!
...
use fuigo_agent::{Agent, AgentBuilder, CompactionPolicy, ReminderPolicy};
...
use fuigo_tools::implementations::fuigo_build::ask_user_question::types::UserQuestionRequest;
...
use fuigo_tools::implementations::fuigo_build::task::types::{SubagentEvent, TaskModelValidator};
...
use fuigo_tools::implementations::lsp::LspBackend;
...
pub(crate) struct AgentRebuildSpec {
    ...
    pub web_search_domains: Option<fuigo_sampling_types::WebSearchOptions>,
...
impl AgentRebuildSpec {
    ...
    async fn build_agent_inner(
        ...
        let session_env = {
            ...
            env.insert("FUIGO_SESSION_ID".to_string(), session_id_str.clone());
            Arc::new(env)
        };
        ...
        agent
            ...
            .update_resources_with(|resources| {
                ...
                if let Some(event_tx) = subagent_event_tx.clone() {
                    use fuigo_tools::implementations::fuigo_build::task::backend::{
                ...
                resources
                    .insert(
                        fuigo_tools::types::resources::SchedulerBackgroundLoops(
                            *scheduler_background_loops,
                        ),
                    );
                resources
                    .insert(
                        fuigo_tools::types::resources::PathNotFoundHints(
                            *path_not_found_hints,
                        ),
                    );
                ...
                {
                    use fuigo_tools::implementations::fuigo_build::ask_user_question::UserQuestionSender;
...
pub(crate) fn test_rebuild_spec_default() -> Arc<AgentRebuildSpec> {
    ...
    Arc::new(AgentRebuildSpec {
        ...
        terminal_backend: Arc::new(
            fuigo_tools::computer::local::LocalTerminalBackend::new_local(
                fuigo_tools::computer::local::SearchShadowConfig::default(),
            ),
        ),
...
mod tests {
    ...
    fn task_description(agent: &Agent) -> String {
        ...
        let task_name = toolset
            .tool_name_for_kind(fuigo_tools::types::tool::ToolKind::Task)
            .expect("FuigoBuild Task tool should be present");
        toolset
            .tool_definitions()
            .into_iter()
    ...
    async fn config_web_search_domains_beat_agent_frontmatter() {
        use fuigo_sampling_types::{HostedTool, ToolOverrides, WebSearchOptions};
        ...
        let definition = || {
            let mut d = AgentDefinition::default_fuigo_build();
