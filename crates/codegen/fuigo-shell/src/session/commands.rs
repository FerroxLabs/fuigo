...
pub enum SideQuestionError {
    ...
    Sampling(#[from] fuigo_sampling_types::SamplingError),
...
pub fn meta_category_str(
    category: fuigo_session_events::types::CancellationCategory,
) -> &'static str {
...
pub enum SessionCommand {
    ...
    GetToolOverrides {
        respond_to: oneshot::Sender<Option<fuigo_sampling_types::ToolOverrides>>,
    },
    ...
    SetToolOverrides {
        overrides: fuigo_sampling_types::ToolOverrides,
    },
    Prompt {
        prompt_id: String,
        ...
        tool_overrides_update: Option<fuigo_sampling_types::ToolOverridesUpdate>,
    ...
    ParentAgentMessage {
        delivery:
            fuigo_tools::implementations::fuigo_build::task::types::ActiveAgentMessageDelivery,
        #[allow(private_interfaces)]
        receipt_sink: tokio::sync::mpsc::Sender<crate::agent::subagent::PromptTurnReceipt>,
        parent_telemetry_ctx: fuigo_telemetry::TelemetryCtx,
        respond_to: oneshot::Sender<
            fuigo_tools::implementations::fuigo_build::task::coordinator::ActiveMessageAdmission,
        >,
    },
    SessionMode {
    ...
    /// Keeps the existing base_url, api_key, and other config; only the `model` field in the `x-fuigo-model-override` header changes.
    ...
    GetModelMetadata {
        responds_to: oneshot::Sender<fuigo_chat_state::ModelMetadata>,
    },
    ...
    ReloadPlugins {
        registry: Option<std::sync::Arc<fuigo_agent::plugins::PluginRegistry>>,
    },
    /// Re-discover the session's own project hooks (`.fuigo/hooks`, `.cursor/hooks.json`, …) mid-session, re-evaluating folder trust.
    ...
    /// Ferrox Labs extension session notification: client-side events to store in persistence
    FuigoSessionNotification {
    ...
    RecordSubagentUsage {
        by_model: Vec<(String, fuigo_chat_state::UsageTotals)>,
    ...
    SnapshotToolDefinitions {
        respond_to: oneshot::Sender<Vec<fuigo_sampling_types::ToolSpec>>,
    },
    ...
    KillBackgroundTask {
        ...
        respond_to: oneshot::Sender<Result<fuigo_tools::types::KillOutcome, String>>,
    },
    DeleteScheduledTask {
        task_id: String,
    ...
    ListTasks {
        respond_to: oneshot::Sender<Option<Vec<fuigo_tools::types::TaskSnapshot>>>,
    },
    ...
    GetHooksList {
        respond_to: oneshot::Sender<fuigo_hooks_plugins_types::HooksListResponse>,
    },
    ...
    PluginsList {
        respond_to: oneshot::Sender<Option<std::sync::Arc<fuigo_agent::plugins::PluginRegistry>>>,
    },
    ...
    /// Dispatch a compat `Notification` hook (e.g. `task_complete` from the notification bridge, which does not go through `send_fuigo_notification`).
    ...
    TakeTurnMessages {
        respond_to: oneshot::Sender<Option<fuigo_chat_state::TurnCapture>>,
    },
    ...
    TakeHarnessTraceTurns {
        respond_to: oneshot::Sender<Vec<Vec<fuigo_sampling_types::conversation::ConversationItem>>>,
    },
