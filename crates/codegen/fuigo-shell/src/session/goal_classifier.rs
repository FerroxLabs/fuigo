...
/// `FUIGO_GOAL_CLASSIFIER_MAX` or remote `goal_classifier_max_runs` can raise it arbitrarily.
pub(crate) const GOAL_CLASSIFIER_MAX_RUNS_DEFAULT: u32 = 10;

/// Floor for `FUIGO_GOAL_CLASSIFIER_MAX` / remote `goal_classifier_max_runs`.
...
/// Override via `FUIGO_GOAL_VERIFIER_N` (clamped 1..=5) or the remote `goal_verifier_count` setting.
...
/// Lower/upper bounds for `FUIGO_GOAL_VERIFIER_N` / remote `goal_verifier_count`.
...
///
/// The tool_result MUST carry the real task tool's `<subagent_result>` footer (via [`fuigo_tool_types::format_resume_footer`]).
...
pub(crate) fn build_subagent_trace_items(
    ...
    prompt: &str,
    output: &str,
) -> Vec<fuigo_sampling_types::conversation::ConversationItem> {
    use fuigo_sampling_types::conversation::{ConversationItem, ToolCall};
    ...
    let footer = fuigo_tool_types::format_resume_footer(subagent_id, subagent_type, None);
...
/// The caller seals the phase via [`fuigo_chat_state::ChatStateHandle::flush_harness_trace_turn`] so it uploads as its own sibling `turn_{N}` artifact.
...
pub(crate) fn record_subagent_trace(
    sink: Option<&(fuigo_chat_state::ChatStateHandle, String)>,
    subagent_id: &str,
    subagent_type: &str,
    description: &str,
    prompt: Option<&str>,
...
pub(crate) struct ChannelSpawner {
    ...
    pub(crate) foreground_wait:
        Option<fuigo_tools::implementations::fuigo_build::task::types::SubagentForegroundWait>,
    ...
    pub(crate) trace_sink: Option<(fuigo_chat_state::ChatStateHandle, String)>,
