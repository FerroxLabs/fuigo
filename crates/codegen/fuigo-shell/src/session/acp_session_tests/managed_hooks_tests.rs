...
//! The dispatcher-level exemption and the display predicate are covered with a sandboxed `FUIGO_HOME` in `fuigo_hooks::dispatcher` tests.
...
/// A temp `FUIGO_HOME` alone cannot redirect it: `fuigo_home()` is `OnceLock`-cached and another test in this binary may have resolved it first.
...
impl DisabledHooksGuard {
    fn capture() -> Self {
        let path = fuigo_config::user_fuigo_home().map(|home| home.join("disabled-hooks"));
...
fn managed_registry() -> fuigo_hooks::discovery::HookRegistry {
    ...
    local
        .run_until(async {
            ...
            *actor.hook_registry.borrow_mut() = Some(Arc::new(managed_registry()));
            ...
            let outcome = actor
                .handle_hooks_action(fuigo_hooks_plugins_types::HooksAction::Disable {
            ...
            let outcome = actor
                .handle_hooks_action(fuigo_hooks_plugins_types::HooksAction::ToggleSource {
