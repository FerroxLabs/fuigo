...
fn main_turn_input(items: Vec<ConversationItem>) -> Vec<serde_json::Value> {
    let request = fuigo_sampling_types::ConversationRequest {
        items: fuigo_chat_state::compaction_utils::ModelRequestHistory::from_raw(items).into_items(),
...
fn assert_messages_rides_parent_prefix(
    ...
    let request = fuigo_sampling_types::ConversationRequest {
        items: fuigo_chat_state::compaction_utils::ModelRequestHistory::from_raw(parent).into_items(),
        model: Some("test".to_string()),
        reasoning_effort: Some(fuigo_sampling_types::ReasoningEffort::High),
        ..Default::default()
    };
    let expected = serde_json::to_value(fuigo_sampling_types::build_messages_request(&request))
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            let body = requests
                ...
                .to_string();
            assert!(body.contains(fuigo_chat_state::compaction_utils::AGENT_MESSAGE_MODEL_LABEL));
            assert_eq!(
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            cfg.reasoning_effort = Some(fuigo_sampling_types::ReasoningEffort::Low);
            actor.chat_state_handle.update_sampling_config(cfg);
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            let mut cfg = actor.chat_state_handle.get_sampling_config().await.unwrap();
            cfg.base_url = server.url();
            cfg.api_backend = fuigo_sampling_types::ApiBackend::ChatCompletions;
            actor.chat_state_handle.update_sampling_config(cfg);
...
fn drained_session_recap(rx: &mut tokio::sync::mpsc::UnboundedReceiver<PersistenceMsg>) -> bool {
    ...
    while let Ok(msg) = rx.try_recv() {
        if let PersistenceMsg::Update(crate::session::storage::SessionUpdate::Fuigo(n)) = msg
            && matches!(
                n.update,
...
/// Drain the persistence channel and report whether a `SessionRecapUnavailable` Ferrox Labs update was emitted.
        ...
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    ...
                    assert!(
                        artifact.x_fuigo_req_id.starts_with("fuigo-recap-"),
                        "req id: {}",
                        artifact.x_fuigo_req_id
                    );
...
async fn manual_recap_over_budget_trims_persisted_request_and_is_display_only() {
    ...
    local
        .run_until(async {
            ...
            while let Ok(msg) = persistence_rx.try_recv() {
                if let PersistenceMsg::RecapRequest(artifact) = msg {
                    let est = fuigo_chat_state::estimate_conversation_tokens(&artifact.chat_history);
                    assert!(
                        est <= PROMPT_BUDGET,
...
/// shape, so it also covers the laxer fuigo ChatCompletions/Responses shapes.)
#[test]
fn over_budget_recap_serializes_to_well_formed_messages_request() {
    use crate::session::helpers::session_recap;
    use fuigo_sampling_types::messages::{ContentBlock, MessageContent, MessageRole};
    use fuigo_sampling_types::{ConversationRequest, ToolCall, rs};
    ...
    // The fuigo backend sets `strip_reasoning` to false; the over-budget branch strips anyway
    let items = session_recap::budget_recap_items(conv, "system-reminder", false, 8_000);
    let req = ConversationRequest::from_items(items);
    let msg = fuigo_sampling_types::build_messages_request(&req);
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            let conv_id = recap_req
                .header("x-fuigo-conv-id")
                .expect("recap must send x-fuigo-conv-id");
            assert!(
...
async fn recap_request_sends_hosted_tools_under_backend_search() {
    use fuigo_sampling_types::HostedTool;
    use fuigo_test_support::MockInferenceServer;
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            // Backend-search fixture: agent carries hosted tools and both gates are on.
            {
                ...
                *agent_slot = fuigo_agent::Agent::new(
...
async fn recap_hosted_tools_reflect_the_active_per_turn_override() {
    use fuigo_sampling_types::{HostedTool, SearchDateBound, ToolOverrides, XSearchOptions};
    use fuigo_test_support::MockInferenceServer;
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            let conv_id = btw_req
                .header("x-fuigo-conv-id")
                .expect("side question must send x-fuigo-conv-id");
            assert!(
...
async fn messages_side_calls_preserve_completed_reasoning() {
    use fuigo_sampling_types::{ReasoningEffort, rs};
    use fuigo_test_support::MockInferenceServer;
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_goal_tool().await;
            ...
            let mut cfg = actor.chat_state_handle.get_sampling_config().await.unwrap();
            cfg.base_url = server.url();
            cfg.api_backend = fuigo_sampling_types::ApiBackend::Messages;
...
async fn messages_side_calls_strip_reasoning_without_supported_thinking_effort() {
    use fuigo_sampling_types::{ReasoningEffort, synthesized_reasoning_item};
    use fuigo_test_support::MockInferenceServer;
...
async fn side_question_trims_reasoning_orphaned_by_mid_turn_truncation() {
    use fuigo_sampling_types::conversation::{AssistantItem, ToolCall};
    use fuigo_test_support::MockInferenceServer;
