...
use fuigo_hooks::event::{HookEventName, StopCancelledReason, StopFailureKind};
...
impl fuigo_agent_lifecycle::LocalTurnLifecycleContributor for RecordingLifecycle {
    async fn on_turn_abort(&self, _input: &fuigo_agent_lifecycle::TurnAbortInput) {
        self.aborts.set(self.aborts.get() + 1);
    }
}

#[async_trait::async_trait(?Send)]
impl fuigo_agent_lifecycle::LocalSessionLifecycleContributor for RecordingLifecycle {
    async fn on_session_idle(&self, _input: &fuigo_agent_lifecycle::SessionIdleInput) {
...
struct Harness {
    ...
    gateway: Option<tokio::sync::mpsc::UnboundedReceiver<fuigo_acp_lib::AcpClientMessage>>,
...
impl Harness {
    ...
    async fn build(is_subagent: bool) -> Self {
        ...
        let mut builder = fuigo_agent_lifecycle::LocalExtensionRegistryBuilder::default();
...
async fn a_turn_announces_its_abort_once() {
    run(async {
        let h = Harness::new().await;
        let interrupted = fuigo_agent_lifecycle::TurnAbortReason::Interrupted;
...
async fn a_long_assistant_message_is_clipped() {
    run(async {
        let over = fuigo_hooks::event::MAX_ASSISTANT_MESSAGE_CHARS + 500;
        ...
        let assert_clipped = |text: &str| {
            ...
            assert!(
                text.chars().count() <= fuigo_hooks::event::MAX_ASSISTANT_MESSAGE_CHARS + 32,
                "{} chars",
                text.chars().count()
            );
        };
        ...
        let fuigo_hooks::event::HookPayload::SubagentStop {
    ...
    run(async {
        ...
        for (field, prefix, max) in [
            (
                "cancelTrigger",
                "ctrl_c",
                fuigo_hooks::event::MAX_CANCEL_TRIGGER_CHARS,
            ),
            (
                "reasonDetails",
                "read_file: ",
                fuigo_hooks::event::MAX_STOP_ENTRY_TEXT_CHARS,
            ),
        ] {
