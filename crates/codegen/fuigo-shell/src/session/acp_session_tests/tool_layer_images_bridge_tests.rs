...
use fuigo_sampling_types::{ContentPart, ConversationItem};
    ...
    local
        .run_until(async {
            ...
            let (persistence_tx, _) = tokio::sync::mpsc::unbounded_channel::<
                PersistenceMsg,
            >();
            let (_acp_updates, fuigo_updates) = spawn_capturing_gateway_loop(gateway_rx);
            ...
            let updates = fuigo_updates.lock().unwrap();
