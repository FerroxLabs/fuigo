...
async fn salvage_test_actor_with_context(
    ...
    salvage_test_actor_on_backend(
        ...
        total_tokens,
        context_window,
        fuigo_sampling_types::ApiBackend::ChatCompletions,
    )
    .await
}
...
async fn salvage_test_actor_on_backend(
    ...
    total_tokens: u64,
    context_window: u64,
    backend: fuigo_sampling_types::ApiBackend,
) -> Arc<SessionActor> {
...
fn default_agent_gate_off_hard_fails_on_length() {
    if std::env::var_os("FUIGO_LENGTH_SALVAGE").is_some() {
        eprintln!("skipping: ambient FUIGO_LENGTH_SALVAGE would flip the gate under test");
        return;
    }
    block_on_session(|| {
