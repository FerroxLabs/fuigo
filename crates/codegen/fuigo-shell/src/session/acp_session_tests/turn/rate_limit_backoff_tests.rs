...
pub(super) fn sampler_surfaces_429() -> fuigo_sampler::RetryPolicy {
    fuigo_sampler::RetryPolicy {
        max_retries: 5,
        rate_limit_retry_threshold: fuigo_sampler::RATE_LIMIT_RETRY_DISABLED,
        ..Default::default()
    }
}

fn sampler_retries_429() -> fuigo_sampler::RetryPolicy {
    fuigo_sampler::RetryPolicy {
        max_retries: 5,
        rate_limit_retry_threshold: fuigo_sampler::RATE_LIMIT_RETRY_THRESHOLD,
        ..Default::default()
    }
}
...
/// F023: a per-model `rate_limit_retry_threshold` survives session request reconstruction and hands 429s to the sampler,
/// so the separate subagent wait loop is switched off (one 429 layer per role, never stacked).
