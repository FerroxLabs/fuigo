...
fn prompt_gate_registry(script: &str) -> fuigo_hooks::discovery::HookRegistry {
    ...
    registry.append_specs(vec![fuigo_hooks::config::HookSpec {
        ...
        event: fuigo_hooks::event::HookEventName::UserPromptSubmit,
        ...
        while let Some(msg) = rx.recv().await {
            match msg {
                ...
                PersistenceMsg::Update(crate::session::storage::SessionUpdate::Fuigo(n)) => {
                    if let FuigoSessionUpdate::HookAnnotation { message } = n.update {
