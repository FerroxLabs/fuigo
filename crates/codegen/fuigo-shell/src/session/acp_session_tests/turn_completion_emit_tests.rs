...
//! They assert on the notification the real `send_fuigo_notification` persists.
...
fn is_durable_turn_completed(m: &PersistenceMsg) -> bool {
    matches!(
        ...
        PersistenceMsg::AppendUpdateDurablyAndAck {
            update: crate::session::storage::SessionUpdate::Fuigo(n),
            ..
        } if matches!(n.update, FuigoSessionUpdate::TurnCompleted { .. })
    )
}
...
fn is_buffered_turn_completed(m: &PersistenceMsg) -> bool {
    matches!(
        m,
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Fuigo(n))
            if matches!(n.update, FuigoSessionUpdate::TurnCompleted { .. })
    )
}
...
fn turn_completed_fields(
    ...
    msgs.iter().find_map(|m| {
        let (PersistenceMsg::Update(crate::session::storage::SessionUpdate::Fuigo(n))
        | PersistenceMsg::AppendUpdateDurablyAndAck {
            update: crate::session::storage::SessionUpdate::Fuigo(n),
            ..
        }) = m
...
fn turn_completed_error_kind(msgs: &[PersistenceMsg]) -> Option<String> {
    msgs.iter().find_map(|m| {
        ...
        match &n.update {
            FuigoSessionUpdate::TurnCompleted { error_kind, .. } => error_kind.clone(),
            _ => None,
        }
    })
}
...
fn turn_completed_meta(msgs: &[PersistenceMsg]) -> Option<serde_json::Value> {
    msgs.iter().find_map(|m| {
        ...
        matches!(n.update, FuigoSessionUpdate::TurnCompleted { .. })
            .then(|| n.meta.clone())
            .flatten()
    })
}
