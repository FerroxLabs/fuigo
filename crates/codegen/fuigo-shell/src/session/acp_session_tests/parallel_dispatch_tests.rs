//! These tests verify the parallel dispatch path (FUIGO_PARALLEL_TOOL_DISPATCH):
...
fn active_message_outputs_distinguish_uncertain_from_proved_rejection() {
    ...
    for (label, output, expected) in [
        ...
        let result: Result<ToolRunResult, fuigo_tool_runtime::ToolError> = Ok(ToolRunResult {
...
async fn generic_tool_completion_chokepoint_has_exact_active_message_cardinality() {
    use fuigo_tools::implementations::fuigo_build::task::backend::SubagentBackendResource;
    use fuigo_tools::implementations::fuigo_build::task::types::{
...
/// The compat toolset input types use `path`, and fuigo_build's `read_file` uses `target_file`.
...
#[test]
fn lock_path_for_args_matches_fuigo_build_file_path() {
    // fuigo_build search_replace / opencode EditTool / WriteTool / etc.
...
#[test]
fn lock_path_for_args_matches_fuigo_build_target_file() {
    // fuigo_build read_file uses #[serde(rename = "target_file")].
    let args = serde_json::json!({
...
#[test]
fn lock_path_for_args_buckets_fuigo_build_and_compat_to_same_lock_for_same_file() {
    // A mixed batch of fuigo_build search_replace and compat StrReplace in the same turn must still serialize on the shared file path
    ...
    assert_eq!(
        lock_path_for_args(&fuigo, Path::new("/cwd")),
...
fn test_skill_discovery_deferred_during_parallel_batch() {
    use fuigo_sampling_types::{ConversationItem, SyntheticReason};
