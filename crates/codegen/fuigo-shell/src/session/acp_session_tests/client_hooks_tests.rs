...
fn post_tool_use_envelope(actor: &SessionActor) -> fuigo_hooks::event::HookEventEnvelope {
    actor.make_hook_envelope(
        fuigo_hooks::event::HookEventName::PostToolUse,
        None,
        fuigo_hooks::event::HookPayload::PostToolUse {
...
fn spawn_run_responder(
    mut gateway_rx: tokio::sync::mpsc::UnboundedReceiver<fuigo_acp_lib::AcpClientMessage>,
...
async fn client_hooks_fire_without_file_registry() {
    ...
    local
        .run_until(async {
            ...
            install_client_hook(&actor, fuigo_hooks::event::HookEventName::Stop, &["cb_0"]);

            actor.fire_hook(
                fuigo_hooks::event::HookEventName::Stop,
                None,
                fuigo_hooks::event::HookPayload::Stop {
        ...
        .run_until(async {
            ...
            *actor.agent.borrow_mut() =
                test_agent_with_tools(vec![fuigo_tools::registry::types::ToolConfig::for_tool::<
                    fuigo_tools::implementations::use_tool::UseTool,
                >()])
                .await;
            ...
            client_hooks.insert(
                ...
                vec![crate::extensions::hooks::ClientHookGroup {
                    matcher: Some(
                        fuigo_hooks::matcher::HookMatcher::new("linear__save_issue").unwrap(),
                    ),
...
impl fuigo_tools::types::tool_metadata::ToolMetadata for McpErrorResultTool {
    fn kind(&self) -> fuigo_tools::types::tool::ToolKind {
        fuigo_tools::types::tool::ToolKind::Other
    }
...
impl fuigo_tool_runtime::Tool for McpErrorResultTool {
    type Args = serde_json::Value;
    type Output = fuigo_tools::types::output::ToolOutput;

    fn id(&self) -> fuigo_tool_protocol::ToolId {
        fuigo_tool_protocol::ToolId::new("mock_error_tool").expect("valid tool id")
    }
    ...
    fn description(
        ...
        fuigo_tool_types::ToolDescription::new("mock_error_tool", "stub MCP error tool")
    }
    ...
    ) -> Result<Self::Output, fuigo_tool_runtime::ToolError> {
        Ok(fuigo_tools::types::output::ToolOutput::MCP(
            fuigo_tools::types::output::MCPOutput::errored(
...
async fn post_tool_use_dispatch_merges_file_then_client_contributions() {
    use fuigo_tools::types::output::{MCPOutput, ToolOutput, ToolRunResult};
...
fn file_registry(
    ...
    registry.append_specs(vec![fuigo_hooks::config::HookSpec {
...
async fn subagent_session_gates_on_subagent_stop() {
    ...
    local
        .run_until(async {
            ...
            *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(file_registry_with_spec(
                fuigo_hooks::event::HookEventName::SubagentStop,
...
async fn alias_envelope_serializes_canonical_event_name() {
    ...
    local
        .run_until(async {
            ...
            let envelope = actor.make_hook_envelope(
                fuigo_hooks::event::HookEventName::SubagentEnd,
                None,
                fuigo_hooks::event::HookPayload::SubagentStop {
                    phase: fuigo_hooks::event::SubagentStopPhase::Observe,
...
async fn an_unanswered_client_gate_leaves_the_report_unspent() {
    ...
    local
        .run_until(async {
            ...
            tokio::task::spawn_local(async move {
                while let Some(msg) = gateway_rx.recv().await {
                    if let fuigo_acp_lib::AcpClientMessage::ExtMethod(args) = msg {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(file_registry(
                ...
                let reported = actor.claim_and_queue(
                    ...
                    super::turn_end_hooks::TurnEnd::Failed {
                        error: fuigo_hooks::event::StopFailureKind::Unknown,
                        error_details: None,
                        last_assistant_message: None,
                    },
                );
