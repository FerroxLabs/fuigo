...
async fn persist_ack_waits_for_disk_flush_before_success() {
    ...
    local
        .run_until(async {
            ...
            let sampling_client = crate::sampling::Client::new(fuigo_sampler::SamplerConfig {
...
async fn cancel_running_task_teardown_clears_running_and_pending_work() {
    ...
    local
        .run_until(async {
            ...
            let cwd = AbsPathBuf::new(std::path::PathBuf::from("/tmp")).unwrap();
            let fs = Arc::new(
                fuigo_workspace::file_system::MockFs::new(cwd.to_path_buf()),
            );
            let terminal = Arc::new(DummyTerminal {});
            let (hunk_tx, _hunk_rx) = tokio::sync::mpsc::unbounded_channel();
            let hunk_tracker_handle = fuigo_hunk_tracker::HunkTrackerActor::spawn(
    ...
    local
        .run_until(async {
            ...
            *actor
            ...
            let cancelled_strip = fuigo_sampler::RequestId::from("cancelled-strip");
            let timed_out_strip = fuigo_sampler::RequestId::from("older-timeout-strip");
        ...
        .run_until(async {
            ...
            let request = ConversationRequest {
                items: vec![ConversationItem::User(
                        fuigo_sampling_types::UserItem {
                            content: vec![fuigo_sampling_types::ContentPart::Text {
                                text: "hi".into(),
                            }],
...
async fn skill_reminder_deferred_while_turn_running_flushed_when_idle() {
    use fuigo_tools::types::skill_discovery_tracker::{SkillUpdateEffects, SkillUpdateKind};
        ...
        actor
            ...
            .filter(|item| {
                matches!(item, ConversationItem::User(u) if u.content.iter().any(|p| matches!(
                    p,
                    fuigo_sampling_types::ContentPart::Text { text } if text.contains("pdf-tools")
                )))
            })
            .count()
    }
