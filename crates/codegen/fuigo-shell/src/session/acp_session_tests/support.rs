...
#[cfg(test)]
pub(crate) fn noop_observability_bridge() -> fuigo_computer_hub_sdk::ObservabilityBridge {
    fuigo_computer_hub_sdk::ObservabilityBridge::new(
        None,
        fuigo_tool_protocol::SessionId::new("test").expect("valid"),
    )
}
#[cfg(test)]
pub(crate) async fn test_agent_default() -> fuigo_agent::Agent {
    test_agent_with_tools(vec![]).await
}
#[cfg(test)]
pub(crate) async fn test_agent_backend_search(
    hosted_tools: Vec<fuigo_sampling_types::HostedTool>,
) -> fuigo_agent::Agent {
    let base = test_agent_default().await;
    fuigo_agent::Agent::new(
        base.definition().clone(),
        fuigo_agent::PromptContext::default(),
        String::new(),
        base.tool_bridge().clone(),
        fuigo_agent::ReminderPolicy::default(),
        fuigo_agent::CompactionPolicy::default(),
        hosted_tools,
        true,
    )
}
#[cfg(test)]
pub(crate) async fn test_agent_with_goal_tool() -> fuigo_agent::Agent {
    use fuigo_tools::implementations::fuigo_build::update_goal::UpdateGoalTool;
    use fuigo_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<UpdateGoalTool>()]).await
}
#[cfg(test)]
pub(crate) async fn test_fuigo_build_agent_with_todo() -> fuigo_agent::Agent {
    use fuigo_tools::implementations::fuigo_build::todo::TodoWriteTool;
    use fuigo_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![ToolConfig::for_tool::<TodoWriteTool>()]).await
}
#[cfg(test)]
pub(crate) async fn test_agent_with_active_message_tool() -> fuigo_agent::Agent {
    use fuigo_tools::implementations::fuigo_build::send_subagent_message::SendSubagentMessageTool;
    use fuigo_tools::implementations::fuigo_build::todo::TodoWriteTool;
    use fuigo_tools::registry::types::ToolConfig;
...
#[cfg(test)]
pub(crate) async fn test_agent_with_plan_tools() -> fuigo_agent::Agent {
    use fuigo_tools::implementations::fuigo_build::enter_plan_mode::EnterPlanModeTool;
    use fuigo_tools::implementations::fuigo_build::exit_plan_mode::ExitPlanModeTool;
    use fuigo_tools::registry::types::ToolConfig;
    test_agent_with_tools(vec![
...
pub(crate) async fn test_agent_with_tools(
    tools: Vec<fuigo_tools::registry::types::ToolConfig>,
) -> fuigo_agent::Agent {
...
pub(crate) async fn test_agent_with_user_message_template(
    template: fuigo_agent::prompt::user_message::UserMessageTemplate,
) -> fuigo_agent::Agent {
...
async fn test_agent_from_config(
    ...
    backend: std::sync::Arc<dyn fuigo_tools::computer::types::TerminalBackend>,
) -> fuigo_agent::Agent {
    use fuigo_tools::computer::local::LocalFs;
    use fuigo_tools::computer::types::AsyncFileSystem;
    ...
    let ctx = SessionContext {
        ...
        session_folder: std::env::temp_dir().join("fuigo-test"),
...
pub(crate) async fn create_test_actor_with_terminal(
    ...
    let fs = Arc::new(fuigo_workspace::file_system::MockFs::new(
        cwd.to_path_buf(),
    ));
    ...
    let hunk_tracker_handle = fuigo_hunk_tracker::HunkTrackerActor::spawn(
        ...
        cwd.to_path_buf(),
        hunk_tx,
        fuigo_hunk_tracker::TrackingMode::AgentOnly,
        tokio_util::sync::CancellationToken::new(),
    );
    let mut tool_context =
        ToolContext::new(cwd.clone(), None, None, fs, terminal, hunk_tracker_handle);
    tool_context.task_completion_reservations =
        Some(fuigo_tools::reminders::task_completion::TaskCompletionReservations::default());
    tool_context.task_wake_suppressed =
        Some(fuigo_tools::reminders::task_completion::TaskWakeSuppressed::default());
    ...
    let actor = SessionActor {
        ...
        compaction: crate::session::compaction_config::CompactionConfig {
            ...
            compaction_mode: fuigo_chat_state::CompactionMode::Transcript,
            verbatim_input: true,
        ...
        extension_registry: fuigo_agent_lifecycle::LocalExtensionRegistry::default(),
        ...
        vcs_kind: fuigo_workspace::session::git::VcsKind::Git,
        ...
        sampler_handle: fuigo_sampler::SamplerHandle::noop(),
        sampling_gate: None,
...
#[cfg(test)]
pub(crate) fn read_and_edit_toolset() -> Vec<fuigo_tools::registry::types::ToolConfig> {
    use fuigo_tools::registry::types::ToolConfig;
    vec![
        ToolConfig::from_id("FuigoBuild:read_file"),
        ToolConfig {
            id: "FuigoBuild:search_replace".into(),
            params: Some(
...
pub(crate) fn pre_tool_use_spec(
    ...
    fuigo_hooks::config::HookSpec {
        ...
        matcher: matcher.map(|m| fuigo_hooks::matcher::HookMatcher::new(m).unwrap()),
        enabled: true,
...
pub(crate) fn post_tool_use_spec(
    ...
    fuigo_hooks::config::HookSpec {
        event: fuigo_hooks::event::HookEventName::PostToolUse,
...
pub(crate) fn post_tool_use_failure_spec(
    ...
    fuigo_hooks::config::HookSpec {
        event: fuigo_hooks::event::HookEventName::PostToolUseFailure,
        ..pre_tool_use_spec(name, matcher, script)
    }
}
#[cfg(test)]
pub(crate) fn install_pre_tool_use_hooks(
    actor: &mut SessionActor,
    specs: Vec<fuigo_hooks::config::HookSpec>,
) {
    let (mut registry, _) = fuigo_hooks::discovery::load_hooks(None, None);
...
pub(crate) async fn tool_result_text(actor: &SessionActor, call_id: &str) -> String {
    ...
    conversation
        ...
        .find_map(|item| match item {
            fuigo_sampling_types::ConversationItem::ToolResult(result)
...
pub(crate) fn spawn_gateway_loop(
    gateway_rx: tokio::sync::mpsc::UnboundedReceiver<fuigo_acp_lib::AcpClientMessage>,
...
pub(crate) fn drain_gateway(
    ...
    tokio::task::spawn_local(async move {
        while let Some(msg) = rx.recv().await {
            if let fuigo_acp_lib::AcpClientMessage::SessionNotification(args) = msg {
                let _ = args.response_tx.send(Ok(()));
            }
        }
    });
...
pub(crate) fn spawn_capturing_gateway_loop(
    ...
    let fuigo_updates: Arc<std::sync::Mutex<Vec<serde_json::Value>>> = Arc::default();
    let acp_captured = acp_updates.clone();
    let fuigo_captured = fuigo_updates.clone();
    let mut gateway_rx = gateway_rx;
    tokio::task::spawn_local(async move {
        while let Some(msg) = gateway_rx.recv().await {
            match msg {
