        ...
        .run_until(async {
            ...
            let result = actor
                .handle_sampling_failure(
                    error_of_kind(fuigo_sampler::SamplingErrorKind::IdleTimeout, None),
                    0,
            ...
            match result {
                Ok(SamplerFailureRecovery::RetryTransient { kind, .. }) => {
                    assert_eq!(kind, fuigo_sampler::SamplingErrorKind::IdleTimeout);
                }
...
async fn server_error_first_failure_requests_resubmit() {
    ...
    local
        .run_until(async {
            ...
            let result = actor
                .handle_sampling_failure(
                    error_of_kind(fuigo_sampler::SamplingErrorKind::Api, Some(503)),
                    0,
                    transient_state(0, true),
                    false,
                )
...
async fn empty_response_stays_terminal_with_full_budget() {
    ...
    local
        .run_until(async {
            ...
            let result = actor
                .handle_sampling_failure(
                    error_of_kind(fuigo_sampler::SamplingErrorKind::EmptyResponse, None),
                    0,
                    transient_state(0, true),
                    false,
                )
...
async fn invalid_image_code_is_never_transient_retried() {
    ...
    local
        .run_until(async {
            let (actor, _rx) = make_actor().await;
            let mut error = error_of_kind(fuigo_sampler::SamplingErrorKind::Api, Some(500));
            error.error_code = Some(fuigo_sampling_types::ApiErrorCode::InvalidImage);
            let result = actor
