...
use fuigo_tools::implementations::fuigo_build::exit_plan_mode::ExitPlanModeExtRequest;
        ...
        while let Some(msg) = gateway_rx.recv().await {
            match msg {
                fuigo_acp_lib::AcpClientMessage::ExtMethod(args) => {
                    if args.request.method.as_ref() == "fuigo/exit_plan_mode" {
...
async fn assert_mixed_batch_snapshot(write_first: bool) {
    ...
    let snapshot = captured
        ...
        .unwrap()
        .clone()
        .expect("gateway must receive fuigo/exit_plan_mode with plan content");
...
async fn mixed_permission_cancel_skips_exit_reverse_request() {
    ...
    local
        .run_until(async {
            ...
            *actor.agent.borrow_mut() = test_agent_with_tools(vec![
                ToolConfig {
                    id: "FuigoBuild:run_terminal_cmd".into(),
                    params: Some(
                        serde_json::from_value(serde_json::json!({
