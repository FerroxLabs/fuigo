...
fn make_ico_content(width: u32, height: u32) -> ImageContent {
    let png = make_test_png(width, height);
    let buf = fuigo_test_utils::image::ico_with_png_frame(&png, width as u8, height as u8);
    ImageContent::new(
...
fn re_encode_fallback_notice_picks_tag_per_harness() {
    ...
    let fuigo = render_re_encode_fallback_notice(&notes, false);
    assert!(fuigo.contains("<system-reminder>"));
    assert!(fuigo.contains("</system-reminder>"));
    assert!(!fuigo.contains("<system_reminder>"));
    assert!(fuigo.contains("<image_re_encode_fallback>"));
}
#[test]
fn display_format() {
...
fn persisted_image_reject_reason_verdicts() {
    ...
    let ico = fuigo_test_utils::image::ico_with_png_frame(&make_test_png(16, 16), 16, 16);
...
fn image_dropped_notice_picks_tag_per_harness() {
    ...
    let fuigo = render_image_dropped_notice(&notes, false);
    assert!(fuigo.contains("<system-reminder>"));
    assert!(fuigo.contains("</system-reminder>"));
    assert!(!fuigo.contains("<system_reminder>"));
    assert!(fuigo.contains("<image_dropped_notice>"));
    assert!(fuigo.contains("Image 5"));
