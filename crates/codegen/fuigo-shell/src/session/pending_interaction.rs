...
/// Never persisted.
fn broadcast(gateway: &GatewaySender, session_id: &acp::SessionId, update: FuigoSessionUpdate) {
...
impl PendingInteractionGuard {
    ...
    pub(crate) fn new(
        ...
        broadcast(
            &gateway,
            &session_id,
            FuigoSessionUpdate::PendingInteraction {
    ...
    fn drop(&mut self) {
        ...
        if removed {
            broadcast(
                &self.gateway,
                &self.session_id,
                FuigoSessionUpdate::InteractionResolved {
