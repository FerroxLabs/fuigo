//! The mechanism (OS watch, coalesce, refcount) lives in `fuigo_fsnotify`.
...
fn fs_event_to_delta(
    ...
    kind: FsEventKind,
    root: &Path,
) -> fuigo_workspace::file_system::FileIndexDelta {
...
impl GitHead {
    ...
    async fn emit(&self) {
        let branch = fuigo_workspace::session::git::get_branch(&self.cwd).await;
        let commit = fuigo_workspace::session::git::get_current_commit(&self.cwd).await;
        let worktree = fuigo_workspace::session::git::get_worktree_info(&self.cwd).await;
        ...
        if changed {
            let params = fuigo_workspace::session::git::GitHeadChanged {
...
pub(crate) struct FsWatchPlan {
    ...
    fs_config: fuigo_fsnotify::FsConfig,
    cwd: PathBuf,
}
...
pub(crate) fn spawn(plan: FsWatchPlan) -> FsWatchHandle {
    ...
    tokio::task::spawn_local(async move {
        ...
        let source = {
            ...
            let result =
                tokio::task::spawn_blocking(move || fuigo_fsnotify::shared(init_cwd, fs_config))
                    .await;
            let ws = fuigo_fsnotify::stats();
...
mod tests {
    ...
    async fn run_rebase_cadence(
        ...
        let settle = Duration::from_millis(fuigo_fsnotify::SETTLE_MS);
    ...
    async fn debounce_armed_pre_op_defers_mid_op_and_fires_once_after_completed() {
        ...
        match on_event(
            FsEvent::GitMetaChanged {
                kind: fuigo_fsnotify::GitMetaKind::RefsChanged,
            },
            &mut in_op,
            &mut op_buffer,
        ) {
