...
/// Drift here would silently invalidate the `fuigo-goal-<12 hex chars>` scratch-root format and the 12-hex restore validation in `from_snapshot`.
#[test]
...
fn scratch_path_helpers_derive_pinned_layout() {
    ...
    assert_eq!(root, std::env::temp_dir().join("fuigo-goal-vid123"));
    assert_eq!(
        implementer_scratch_dir("vid123"),
        std::env::temp_dir()
            .join("fuigo-goal-vid123")
            .join("implementer"),
    );
    assert_eq!(
