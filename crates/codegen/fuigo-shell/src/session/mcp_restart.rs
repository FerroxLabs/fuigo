...
//!    In its `acp::McpServer::Stdio` arm, [`fuigo_mcp::servers::start_mcp_server`] sets `kill_on_drop(true)` on the child it spawns.
...
//! 5. **Already-Empty**: see the [`fuigo_mcp::servers::ClientStateKind::Empty`] doc: a previous handshake exhausted attempts.
...
use fuigo_mcp::servers::{McpClientEventKind, McpServerName};
