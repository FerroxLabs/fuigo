//! Core worktree lifecycle logic lives in [`fuigo_workspace::worktree`].
...
///
/// When `git_ref` is set, forces a clean checkout of that ref (same as the manual `create_from_worktree_sync` path used by `fuigo -w --ref`).
...
async fn create_worktree_for_resume(
    ...
    if find_git_root_from_path(source)
        .ok()
        .is_some_and(|root| fuigo_workspace::session::git::detect_vcs_kind(&root).is_jj())
    {
...
pub(crate) async fn checkout_persisted_head_in_worktree(
    ...
    session_id: &str,
) -> fuigo_workspace::session::git::CheckoutSessionOutcome {
    let sha = match head_commit {
        Some(s) if !s.is_empty() => s,
        _ => return fuigo_workspace::session::git::CheckoutSessionOutcome::default(),
    };
    fuigo_workspace::session::git::checkout_session_commit(
...
pub(crate) struct WorktreeRestoreDecision {
    ...
    pub restore_degree: Option<fuigo_workspace::session::git::RestoreDegree>,
}
/// Thin wire-format adapter over the shared [`fuigo_workspace::session::git::build_restore_decision`] helper.
pub(crate) fn build_worktree_restore_outcome(
    head_commit: Option<&str>,
    outcome: &fuigo_workspace::session::git::CheckoutSessionOutcome,
    kind: fuigo_workspace::session::git::RestoreKind,
) -> WorktreeRestoreDecision {
    let d = fuigo_workspace::session::git::build_restore_decision(head_commit, outcome, kind);
    WorktreeRestoreDecision {
...
pub(crate) async fn resume_session_in_worktree(
    ...
    ops: &fuigo_workspace::WorkspaceOps,
...
async fn restore_remote_session_into_worktree(
    req: &ResumeSessionInWorktreeRequest,
    #[allow(unused_variables)] ops: &fuigo_workspace::WorkspaceOps,
    client: &crate::agent::session_registry_client::SessionRegistryClient,
    restore_code_default: bool,
    turn: i32,
    wt_resp: CreateWorktreeFromWorktreeResponse,
) -> Result<ResumeSessionInWorktreeResponse> {
    use fuigo_workspace::session::git::effective_worktree_path;
    ...
    let restore_degree = if codebase_ok {
        Some(fuigo_workspace::session::git::RestoreDegree::Full)
    } else {
        None
    };
...
async fn resume_local_session_in_worktree(
    ...
    if req.restore_code.unwrap_or(restore_code_default) {
        let is_jj = find_git_root_from_path(std::path::Path::new(resolved_source_cwd))
            ...
            .is_some_and(|root| fuigo_workspace::session::git::detect_vcs_kind(&root).is_jj());
        if !is_jj {
            if fuigo_workspace::session::git::should_warn_registry_disabled(
                is_jj,
            ...
            .await;
            use fuigo_workspace::session::git::RestoreKind;
...
pub(crate) async fn rehydrate_session_in_worktree(
    ...
    let session_summary_exists = crate::util::fuigo_home::sessions_cwd_dir(&req.source_cwd)
        .join(&req.session_id)
        .join("summary.json")
        .exists();
...
mod tests {
    ...
    fn resume_response_round_trips() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(fuigo_workspace::session::git::RestoreDegree::Full),
        };
    ...
    fn resume_response_serializes_degree_when_set() {
        let resp = ResumeSessionInWorktreeResponse {
            ...
            restore_degree: Some(fuigo_workspace::session::git::RestoreDegree::HeadOnly),
        };
        let json = serde_json::to_string(&resp).unwrap();
        ...
        assert_eq!(
            deser.restore_degree,
            Some(fuigo_workspace::session::git::RestoreDegree::HeadOnly)
        );
    ...
    fn make_wt_record(path: &str, source_repo: &str) -> fuigo_fast_worktree::WorktreeRecord {
        fuigo_fast_worktree::WorktreeRecord {
            id: format!("wt-{}", path.replace('/', "-")),
            path: std::path::PathBuf::from(path),
            source_repo: std::path::PathBuf::from(source_repo),
            repo_name: "repo".into(),
            kind: fuigo_fast_worktree::WorktreeKind::Session,
    ...
    fn repo_wide_resolve_finds_session_in_sibling_cwd_skipping_remote() {
        ...
        let encoded = crate::util::fuigo_home::encode_cwd_dirname(sibling_cwd);
    ...
    async fn checkout_persisted_head_stashes_dirty_worktree_state() {
        ...
        assert!(
            list_out.contains("fuigo: pre-restore-code sess-dirty-wt"),
