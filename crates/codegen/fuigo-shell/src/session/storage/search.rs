//! Binds the `fuigo-session-search` index to this crate's JSONL session store.
...
use fuigo_session_search::{IndexableSession, SessionSource};

pub use fuigo_session_search::{
...
/// One live manager per fuigo home, at most.
...
pub fn notify_session_updated(index: Option<&SearchIndexManager>, session_id: &str, cwd: &str) {
    ...
    let root = crate::util::fuigo_home::fuigo_home();
...
pub(crate) async fn evict_session(root_dir: &Path, session_id: &str) {
    fuigo_session_search::evict_session(root_dir, session_id).await;
}
...
pub async fn execute_search(
    ...
    fuigo_session_search::execute_search(index, root_dir, req).await
}
