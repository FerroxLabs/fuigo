...
async fn workflow_run_manifest_round_trips_and_clear_tombstone_wins() {
    ...
    assert_eq!(loaded_with_effort.workflow_runs[0].effort,
            Some(fuigo_sampling_types::ReasoningEffort::High));
...
async fn reads_never_modify_rewind_or_updates_files() {
    use fuigo_workspace::session::file_state::{FileStateTracker, RewindPoint};
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
...
#[tokio::test]
async fn test_fuigo_session_update_round_trip() {
    ...
    adapter
        .append_update(&info, &SessionUpdate::Fuigo(Box::new(fuigo_notification.clone())))
        .await
        .unwrap();
    ...
    match &loaded.updates[0] {
        ...
        _ => panic!("Expected Ferrox Labs update as first item"),
    }
...
async fn test_subagent_notifications_round_trip() {
    use crate::extensions::notification::{
        SessionNotification as FuigoSessionNotification,
        SessionUpdate as FuigoSessionUpdateType,
    };
    let temp_dir = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    let info = create_test_info();
    adapter.init_session(&info, default_model_id()).await.unwrap();
    let spawned = FuigoSessionNotification {
        session_id: acp::SessionId::new("parent-session"),
        update: FuigoSessionUpdateType::SubagentSpawned {
    ...
    adapter.append_update(&info, &SessionUpdate::Fuigo(Box::new(spawned))).await.unwrap();
    let finished = FuigoSessionNotification {
        session_id: acp::SessionId::new("parent-session"),
        update: FuigoSessionUpdateType::SubagentFinished {
            subagent_id: "child-001".to_string(),
            child_session_id: "child-001".to_string(),
    ...
    adapter.append_update(&info, &SessionUpdate::Fuigo(Box::new(finished))).await.unwrap();
    let loaded = adapter.load_session(&info).await.unwrap();
    assert_eq!(loaded.updates.len(), 2);
    match &loaded.updates[0] {
        SessionUpdate::Fuigo(notification) => {
            match &notification.update {
                FuigoSessionUpdateType::SubagentSpawned {
                    subagent_id,
                    child_session_id,
                    description,
                    subagent_type,
    ...
    match &loaded.updates[1] {
        SessionUpdate::Fuigo(notification) => {
            match &notification.update {
                FuigoSessionUpdateType::SubagentFinished {
                    subagent_id,
                    status,
                    tool_calls,
                    turns,
    ...
    assert_eq!(spawned_json["method"], "_fuigo/session/update");
    ...
    assert_eq!(finished_json["method"], "_fuigo/session/update");
...
async fn test_load_prompts_only_applies_rewind_truncation() {
    ...
    for update in [
        ...
        SessionUpdate::Fuigo(Box::new(rewind)),
...
async fn init_session_stamps_configured_profile_on_new_session() {
    let tmp = TempDir::new().unwrap();
    let adapter = JsonlStorageAdapter::with_root(tmp.path().to_path_buf());
    fuigo_sandbox::set_configured_profile("workspace");
    let expected = fuigo_sandbox::configured_profile_name().map(String::from);
    let info = Info {
...
fn scan_session_dirs_finds_all_sessions() {
    ...
    let cwd = crate::util::fuigo_home::encode_cwd_dirname("/home/user/project");
...
fn scan_session_dirs_filters_by_cwd() {
    let tmp = TempDir::new().unwrap();
    let now = chrono::Utc::now();
    let cwd_a = crate::util::fuigo_home::encode_cwd_dirname("/home/user/project-a");
    let cwd_b = crate::util::fuigo_home::encode_cwd_dirname("/home/user/project-b");
...
fn scan_session_dirs_skips_non_directory_entries() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::fuigo_home::encode_cwd_dirname("/project");
...
async fn list_sessions_sorts_by_last_active_at_over_updated_at() {
    ...
    let cwd = crate::util::fuigo_home::encode_cwd_dirname(cwd_path);
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd,
...
async fn list_sessions_recent_sorts_by_updated_at() {
    let tmp = TempDir::new().unwrap();
    let cwd = crate::util::fuigo_home::encode_cwd_dirname("/ws");
    let now = chrono::Utc::now();
...
async fn list_sessions_recent_spans_multiple_workspaces() {
    let tmp = TempDir::new().unwrap();
    let cwd_a = crate::util::fuigo_home::encode_cwd_dirname("/project-a");
    let cwd_b = crate::util::fuigo_home::encode_cwd_dirname("/project-b");
    let now = chrono::Utc::now();
    write_test_summary(
        tmp.path(),
        &cwd_a,
...
/// Exercises the real on-read upgrade path end-to-end (loader, serde, and fuigo_sampling_types::upgrade_legacy_reasoning).
...
/// The assistant carries `reasoning: { text, encrypted, id }` inline, the legacy fuigo-build / Opus / chat-completions shape.
...
fn read_chat_history_upgrades_legacy_singular_reasoning_to_sibling() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"results...","reasoning":{"text":"the results are about cats","encrypted":"enc-blob","id":"rs_legacy"},"model_id":"fuigo-build"}"#,
        ],
    );
    assert_eq!(
            items.len(),
    ...
    match &items[3] {
        ConversationItem::Reasoning(r) => {
            ...
            let fuigo_sampling_types::rs::SummaryPart::SummaryText(s) = &r.summary[0];
...
fn read_chat_history_handles_hybrid_legacy_and_post_pr_lines() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a1","reasoning":{"text":"legacy thinking","encrypted":"enc","id":"rs_legacy"},"model_id":"fuigo-build"}"#,
            ...
            r#"{"type":"assistant","content":"a2","model_id":"fuigo-build"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
    ...
    let fuigo_sampling_types::rs::SummaryPart::SummaryText(s) = &reconstructed
        .summary[0];
...
fn read_chat_history_is_idempotent_on_post_pr_sessions() {
    let items = load_lines(
        &[
            ...
            r#"{"type":"assistant","content":"a","model_id":"fuigo-build"}"#,
        ],
    );
    let kinds: Vec<&'static str> = items
        .iter()
...
fn read_chat_history_skips_merged_line_from_interrupted_append() {
    ...
    let good_2 = r#"{"type":"assistant","content":"after","model_id":"fuigo-build"}"#;
    let raw = format!("{good_1}\n{partial}{merged_onto}\n{good_2}\n");
    let temp_dir = TempDir::new().unwrap();
...
async fn retry_after_lost_ack_converges_memory_and_disk_to_authoritative_item() {
    ...
    let chat = fuigo_chat_state::ChatStateActor::spawn(
        vec![],
    ...
    assert!(matches!(
            ...
            .await,
            Err(fuigo_chat_state::StrictAppendError::Indeterminate(_))
        ));
...
async fn acknowledged_chat_append_preserves_existing_file_bytes_and_appends_once() {
    ...
    assert!(matches!(
            adapter
                ...
                .await
                .unwrap(),
            fuigo_chat_state::StrictAppendAck::Appended
        ));
...
async fn init_session_creates_owner_only_session_and_parent_dirs() {
    ...
    let session_dir = temp_dir
        .path()
        .join("sessions")
        .join(crate::util::fuigo_home::encode_cwd_dirname(&info.cwd))
...
/// Dirs loosened on disk (e.g. by an older fuigo) re-tighten on next touch.
#[tokio::test]
#[cfg(unix)]
...
async fn copy_session_data_creates_owner_only_target_dir() {
    ...
    let target_dir = temp_dir
        .path()
        .join("sessions")
        .join(crate::util::fuigo_home::encode_cwd_dirname(&target.cwd))
...
async fn usage_json_rewrites_session_and_appends_turns() {
    ...
    use fuigo_sampling_types::TokenUsage;
    let temp_dir = TempDir::new().unwrap();
    let info = create_test_info();
    let adapter = JsonlStorageAdapter::with_root(temp_dir.path().to_path_buf());
    adapter.init_session(&info, default_model_id()).await.unwrap();
