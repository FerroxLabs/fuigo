...
/// Desktop hides local rows whose cwd sits under a fuigo worktree unless they carry a label (or `session_kind == "worktree"`).
...
/// Read-site repair for a `summary` that is untagged, or kinded but still missing `worktree_label`, when its cwd is inside a fuigo-managed worktree.
