...
/// `~/.fuigo/sessions` RelocationView scan.
...
/// Ferrox Labs events alone never count as `Emitted`, so an eviction decision can't settle on a file the client cannot rebuild content from.
...
/// The stream carries ACP transcript updates plus persisted Ferrox Labs child events (compaction, retry, memory lifecycle) in file order.
...
/// Like [`load_updates_for_replay`], but resolves the session under a specific fuigo home.
...
pub fn load_updates_for_replay_at(
    ...
    let Some(updates_path) =
        resolve_replay_updates_path(session_id, fuigo_home, ReplayPathHint::default())?
    else {
        return Ok(None);
    };
    Ok(Some(collect_replay_updates(&updates_path)?))
...
fn try_fast_replay_updates_path(
    ...
    for cwd in [hint.child_cwd, hint.parent_cwd].into_iter().flatten() {
        let encoded = fuigo_config::encode_cwd_dirname(&cwd.to_string_lossy());
        let candidate = fuigo_home.join("sessions").join(encoded).join(session_id);
...
/// Resolve `updates.jsonl` for `session_id` under `fuigo_home`, or `None` when the session directory or the file is missing.
...
pub(crate) fn resolve_replay_updates_path(
    ...
    match try_fast_replay_updates_path(session_id, fuigo_home, hint) {
...
/// Invoke `f` once per client-replay ACP update for a session under `fuigo_home`, never building the full typed `Vec`.
...
/// Ferrox Labs-only, torn, or catalog-only content replays `Empty`.
...
pub fn replay_would_emit(
    ...
    let Some(path) = resolve_replay_updates_path(session_id, fuigo_home, hint)? else {
        return Ok(false);
    };
...
/// Persisted Ferrox Labs child events are forwarded in file order (see [`ReplayedUpdate`]).
pub fn stream_replay_updates_at_hinted<F: FnMut(ReplayedUpdate)>(
    ...
    let Some(updates_path) = resolve_replay_updates_path(session_id, fuigo_home, hint)? else {
...
pub(crate) fn collect_unfinished_subagents(filtered: &[&str]) -> Vec<(String, String)> {
    ...
    for line in filtered {
        ...
        match notification.update {
            ...
            FuigoUpdate::SubagentFinished { subagent_id, .. } => {
