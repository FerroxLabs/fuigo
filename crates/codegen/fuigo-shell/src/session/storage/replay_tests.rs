...
fn stream_replay_updates_at_missing_session_is_empty() {
    ...
    std::fs::create_dir_all(fuigo_home.path().join("sessions")).unwrap();

    let mut count = 0usize;
    let emission =
        stream_replay_updates_at("does-not-exist", fuigo_home.path(), |_| count += 1).unwrap();
...
fn stream_replay_updates_at_surfaces_read_errors() {
    let fuigo_home = tempfile::tempdir().unwrap();
    let session_dir = fuigo_home.path().join("sessions").join("cwd").join("sess");
    std::fs::create_dir_all(&session_dir).unwrap();
    std::fs::write(session_dir.join(SUMMARY_FILE), "{}").unwrap();
    std::fs::create_dir(session_dir.join(UPDATES_FILE)).unwrap();

    let result = stream_replay_updates_at("sess", fuigo_home.path(), |_| {});
    assert!(
        result.is_err(),
...
fn prepare_replay_cursor_refused_when_tail_has_event_id_less_line() {
    ...
    // Ferrox Labs-style line persisted by an older binary: no _meta at all.
    let old_fuigo = r#"{"timestamp":2,"method":"_fuigo/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"}}}"#;
    let raw = format!("{a1}\n{old_fuigo}\n");
    ...
    let new_fuigo = r#"{"timestamp":2,"method":"_fuigo/session/update","params":{"sessionId":"s","update":{"sessionUpdate":"hook_annotation","message":"trailing"},"_meta":{"eventId":"ev2"}}}"#;
    let raw = format!("{a1}\n{new_fuigo}\n");
    let prepared = prepare_replay_lines(&raw, Some("ev1"));
    assert!(!prepared.mark_replay);
    assert_eq!(prepared.lines.len(), 1);
...
fn fast_reject_rewind_marker_in_content() {
    ...
    // (b) an ACP (non-fuigo) update carrying rewind_marker in content is NOT a real fuigo rewind_marker; it yields NotUserMessage (no RewindTo)
...
fn prepare_replay_reports_spawn_without_finish() {
    let spawn = |id: &str, child: &str| {
        format!(
            r#"{{"method":"_fuigo/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"subagent_spawned","subagent_id":"{id}","parent_session_id":"s","child_session_id":"{child}","subagent_type":"general-purpose","description":"task"}},"_meta":{{"eventId":"s-1"}}}}}}"#
        )
    };
    let finish = |id: &str| {
        format!(
            r#"{{"method":"_fuigo/session/update","params":{{"sessionId":"s","update":{{"sessionUpdate":"subagent_finished","subagent_id":"{id}","child_session_id":"c{id}","status":"completed","tool_calls":0,"turns":0,"duration_ms":0}},"_meta":{{"eventId":"s-2"}}}}}}"#
        )
    };
...
fn persist_fuigo_update(update: crate::extensions::notification::SessionUpdate) -> String {
    ...
    let envelope =
        SessionUpdateEnvelope::from_update(&SessionUpdate::Fuigo(Box::new(notif))).unwrap();
...
fn stream_replay_forwards_completed_tool_call_update_without_base() {
    ...
    let encoded = fuigo_config::encode_cwd_dirname(cwd);
...
/// Persisted Ferrox Labs child events (compaction, retry) are forwarded in file order alongside the ACP stream.
/// A rebuilt child view thus keeps its non-ACP markers, but they never count toward `Emitted`.
#[test]
fn stream_replay_forwards_fuigo_updates_in_file_order() {
    let home = tempfile::tempdir().unwrap();
    let sid = "child-fuigo";
    let dir = home.path().join("sessions").join("cwd").join(sid);
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(dir.join(SUMMARY_FILE), "{}").unwrap();
    let compact = persist_fuigo_update(
    ...
    let emission =
        ...
        .unwrap();
    assert_eq!(emission, ReplayEmission::Emitted);
    assert_eq!(kinds, vec![true, false], "fuigo then acp, in file order");
}
...
/// A transcript holding only Ferrox Labs events replays them but stays `Empty`.
/// Eviction decisions must not settle on a file the client cannot rebuild transcript content from.
#[test]
fn fuigo_only_transcript_forwards_but_stays_empty() {
    let home = tempfile::tempdir().unwrap();
    let sid = "child-fuigo-only";
    let dir = home.path().join("sessions").join("cwd").join(sid);
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(dir.join(SUMMARY_FILE), "{}").unwrap();
    let retry = persist_fuigo_update(crate::extensions::notification::SessionUpdate::RetryState(
...
/// Non-empty content that replays `Empty` (Ferrox Labs-only lines, a torn/unparseable line, a start-only ToolCallUpdate) must report false.
#[test]
fn replay_would_emit_requires_an_emitting_acp_line() {
    ...
    let fuigo_only = persist_fuigo_update(
        crate::extensions::notification::SessionUpdate::AutoCompactCompleted {
            tokens_before: Some(1_000),
            tokens_after: 100,
            elapsed_ms: Some(5),
    ...
    assert!(
        !probe(&format!("{fuigo_only}\n")),
        "Ferrox Labs events alone never count as Emitted"
    );
    let acu =
        acp_envelope(r#"{"sessionUpdate":"available_commands_update","availableCommands":[]}"#);
...
fn child_fast_path_finds_updates_under_child_cwd() {
    ...
    let encoded = fuigo_config::encode_cwd_dirname(child_cwd);
    let sid = "child-wt";
    let dir = home.path().join("sessions").join(&encoded).join(sid);
    std::fs::create_dir_all(&dir).unwrap();
    std::fs::write(dir.join(SUMMARY_FILE), "{}").unwrap();
...
fn child_lookup_falls_back_when_fast_path_misses() {
    let home = tempfile::tempdir().unwrap();
    let other = fuigo_config::encode_cwd_dirname("/other/cwd");
