    ...
    for item in items.iter().rev() {
        let (role, warning) = match item {
                ...
                if user.synthetic_reason == Some(SyntheticReason::AgentMessage) =>
            {
                (
                    "agent_message",
                    Some(fuigo_chat_state::compaction_utils::AGENT_MESSAGE_MODEL_LABEL),
                )
            }
        ...
        let capped = fuigo_tools::util::truncate_str(trimmed, ITEM_MAX_BYTES);
...
pub(crate) fn build_goal_evaluator_request(
    ...
    ConversationRequest {
        ...
        x_fuigo_conv_id: Some(session_id.to_owned()),
        x_fuigo_req_id: Some(format!("fuigo-goal-eval-{}", uuid::Uuid::new_v4())),
        x_fuigo_session_id: Some(session_id.to_owned()),
