...
/// Unconditional reservations for `fuigo inspect`.
...
pub(crate) struct ListCommandsRequest {
    ...
    /// Product lane: `"chat"` filters to Fuigo Chat / Fuigo Computer first-party skills only.
...
fn product_skills_cache_entry_after_fetch(
    primary: &crate::auth::FuigoAuth,
    skills: Vec<SkillInfo>,
    _used_untagged_recovery: bool,
) -> ProductSkillsCacheEntry {
    product_skills_cache_entry(primary, skills)
}
fn product_skills_negative_stamp(auth: &crate::auth::FuigoAuth) -> ProductSkillsIdentityStamp {
...
fn clear_degraded_cache_for_auth(auth: &crate::auth::FuigoAuth) {
...
fn clear_negative_cache_for_auth(auth: &crate::auth::FuigoAuth) {
...
pub(crate) async fn product_skill_infos(
    ...
    let fuigo_auth = match auth.auth().await {
        Ok(a) => a,
        Err(err) => {
    ...
    if let Some(skills) = product_skills_cache_lookup(&fuigo_auth) {
        return skills;
    }
    ...
    match client.try_list_catalog(PRODUCT_SKILLS_LOCALE).await {
        Ok((catalog, used_untagged_recovery)) if catalog.user_list_failed => {
            let skills = catalog.to_skill_infos();
            {
                ...
                if let Some(entry) = guard.as_ref()
                    && product_skills_cache_matches(entry, &fuigo_auth)
                {
                    tracing::warn!(
            ...
            *PRODUCT_SKILLS_DEGRADED_CACHE.lock() = Some(product_skills_cache_entry_after_fetch(
            ...
            clear_negative_cache_for_auth(&fuigo_auth);
            tracing::warn!(
        ...
        Ok((catalog, used_untagged_recovery)) => {
            ...
            *PRODUCT_SKILLS_CACHE.lock() = Some(product_skills_cache_entry_after_fetch(
            ...
            clear_degraded_cache_for_auth(&fuigo_auth);
            clear_negative_cache_for_auth(&fuigo_auth);
            Some(skills)
        }
        Err(err) => {
...
fn product_skills_cache_lookup(
    fuigo_auth: &crate::auth::FuigoAuth,
    ...
    }
    {
        ...
        if let Some(entry) = guard.as_ref()
            && product_skills_negative_matches(entry, fuigo_auth)
...
/// - `kind == Some("chat")` (feature `chat` only): **product Skills REST catalog** (same as fuigo-web) plus builtins, not Build disk skills.
...
///
/// - `RewriteToRun` (default): replace `/foo args` with `"run /foo args"`, matching today's Fuigo flow that calls our dedicated `skill` tool.
...
pub(super) async fn build_skill_information_for_refs(
    ...
    use fuigo_tools::implementations::skills::skill::{
...
/// The wording (usage hint and scheduling instruction) is sourced from `fuigo-tools`.
