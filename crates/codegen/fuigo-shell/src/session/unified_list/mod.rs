...
/// Env lane (desktop `FUIGO_SESSION_LIST_CONVERSATIONS`) OR process-wide `--chat` (`FUIGO_CHAT_MODE`); hard-off in release builds.
...
mod tests {
    ...
    fn local(session_id: &str, updated_at: &str) -> MergedSession {
        MergedSession {
            ...
            cwd: "/Users/me/fuigo".into(),
            hostname: Some("devbox".into()),
            source: "local".into(),
            model_id: Some("fuigo-build".into()),
            num_messages: 7,
            last_active_at: Some(updated_at.into()),
            branch: Some("main".into()),
            repo_name: Some("fuigo".into()),
            worktree_label: Some("wt".into()),
            git_root_dir: Some("/Users/me/fuigo".into()),
            git_remotes: vec!["git@github.com:example/repo.git".into()],
            source_workspace_dir: Some("/Users/me/fuigo-src".into()),
    ...
    fn facets_carry_kind_and_cwd() {
        ...
        assert!(matches!(
            ...
            Some(FacetValue::One(serde_json::Value::String(c))) if c == "/Users/me/fuigo"
        ));
    }
    #[test]
    ...
    fn fuigo_auth_manager(dir: &std::path::Path) -> std::sync::Arc<crate::auth::AuthManager> {
    ...
    fn conversations_lane_env_gating_matrix() {
        {
            let _off = fuigo_test_support::EnvGuard::unset("FUIGO_SESSION_LIST_CONVERSATIONS");
            assert!(!conversations_lane_enabled());
        }
        {
            let _on = fuigo_test_support::EnvGuard::set("FUIGO_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_enabled(), false);
        }
        {
            let _off = fuigo_test_support::EnvGuard::set("FUIGO_SESSION_LIST_CONVERSATIONS", "0");
            assert!(!conversations_lane_enabled());
        }
    }
    ...
    fn conversations_lane_active_truth_table() {
        ...
        {
            let _desktop =
                fuigo_test_support::EnvGuard::set("FUIGO_SESSION_LIST_CONVERSATIONS", "1");
            assert_eq!(conversations_lane_active(), false);
        }
        {
            let _chat = fuigo_test_support::EnvGuard::set(FUIGO_CHAT_MODE_ENV, "1");
            assert_eq!(
                conversations_lane_active(),
                false,
    ...
    fn parse_list_req_forces_kind_under_process_chat_mode_only() {
        ...
        .to_string();
        {
            let _off = fuigo_test_support::EnvGuard::unset(FUIGO_CHAT_MODE_ENV);
    ...
    fn relax_rows_scopes_to_repo_and_requires_messages() {
        ...
        let repo_url = fuigo_workspace::session::git::normalize_repo_url(this_repo).unwrap();
        ...
        let rows = relax_rows(
            ...
            vec![
                ...
                summary("theirs", Some("git@github.com:fuigo-org/other.git"), 9),
            ],
            30,
            facet_registry(),
            HeadlessPolicy::Exclude,
