...
pub(crate) struct RoleToolNames {
    ...
    /// `ToolKind::Edit` is the default fuigo-build host's `search_replace` mutator.
    pub write: String,
...
impl RoleToolNames {
    ...
    pub(crate) fn from_summary(
        summary: &fuigo_tools::implementations::fuigo_build::task::types::SubagentTypeSummary,
    ) -> Self {
        ...
        Self::from_parts(
            ...
            get(ToolKind::Search),
            // The default fuigo-build host's pre-spawn describe probe exposes only `Edit` (`search_replace`) as the file mutator
...
pub(crate) mod tests {
    ...
    #[test]
    fn from_summary_uses_fuigo_build_names() {
        // A fuigo-build toolset: client names match the literal defaults.
    ...
    fn web_tools_fall_back_when_absent_from_the_toolset() {
        ...
        // So the planner prompt still names a real tool on the default fuigo-build host (the stock `web_search`/`web_fetch`)
    ...
    #[test]
    fn from_summary_write_falls_back_to_edit_on_default_fuigo_build_host() {
        // Default fuigo-build host: the pre-spawn describe probe exposes only `Edit` (`search_replace`); `Write` is injection-only and absent
    ...
    fn from_parent_write_falls_back_to_edit_when_bridge_has_no_write() {
        // Default fuigo-build parent bridge: no `Write`, only `Edit` (`search_replace`)
        let tn = RoleToolNames::from_parent(
            Some("read_file".into()),
            Some("list_dir".into()),
            Some("grep".into()),
    ...
    #[test]
    fn parent_and_summary_renders_agree_on_default_fuigo_build_mutator() {
        // The explicit-pair `primary` render comes from `from_summary`; the inherit/fail-open `fallback` render comes from `from_parent`
        // Both must name the SAME mutator on the default fuigo-build host (Edit-only toolset)
