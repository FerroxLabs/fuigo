...
pub struct SessionHandle {
    ...
    pub chat_state_handle: fuigo_chat_state::ChatStateHandle,
...
impl SessionHandle {
    ...
    pub(crate) async fn get_model_metadata(&self) -> fuigo_chat_state::ModelMetadata {
            ...
            .is_ok()
        {
            ...
            fuigo_chat_state::ModelMetadata::default()
        }
    }
    ...
    /// Routes through the session actor to the ToolBridge's TerminalBackend.
    pub async fn list_tasks(&self) -> Option<Vec<fuigo_tools::types::TaskSnapshot>> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
    ...
    pub(crate) async fn snapshot_tool_definitions(&self) -> Vec<fuigo_sampling_types::ToolSpec> {
        let (tx, rx) = oneshot::channel();
        if self
            .cmd_tx
