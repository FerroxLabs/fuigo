...
fn save_session_titles(
    server: &fuigo_test_support::MockInferenceServer,
    session_id: &str,
) -> Vec<SaveTitle> {
...
fn request_path_summary(server: &fuigo_test_support::MockInferenceServer) -> Vec<String> {
    server
        .requests()
        .iter()
...
mod prompt_file_tests {
    ...
    fn hash_encoded_cwd_marker_is_synced_before_parent_dir_sync() {
        ...
        let cwd_dir = crate::util::fuigo_home::sessions_cwd_dir_in(home.path(), &info.cwd);
