...
impl GoalNotifySender {
    ...
    /// Those must not close the rewind window, which `send_fuigo_notification` does as a side effect.
    pub(crate) fn send_update(&self, update: FuigoSessionUpdate) {
    ...
    fn dispatch_update(&self, update: FuigoSessionUpdate, persist: bool) {
...
pub(crate) fn build_goal_cleared() -> FuigoSessionUpdate {
    FuigoSessionUpdate::GoalUpdated {
        goal_id: String::new(),
...
mod tests {
    ...
    fn planning_field(update: &FuigoSessionUpdate) -> Option<bool> {
        match update {
            FuigoSessionUpdate::GoalUpdated { planning, .. } => *planning,
            _ => panic!("expected GoalUpdated"),
        }
    }
    ...
    fn build_goal_updated_verifying_flag_reflects_latch() {
        ...
        let mut o = make_base_orchestration();
        let verifying = |u: &FuigoSessionUpdate| match u {
            FuigoSessionUpdate::GoalUpdated {
                verifying_completion,
                ..
            } => *verifying_completion,
