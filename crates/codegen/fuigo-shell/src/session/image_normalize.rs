...
/// Resolve the active reminder tag via the canonical constants in `fuigo_tools::reminders`.
...
fn compute_normalized_blocking(
    ...
    let (orig_w, orig_h, orig_mime) =
        fuigo_tools::util::image_validate::validate_image_bytes_with(&raw_bytes, false)
            .map_err(|e| NormalizeError(format!("validate: {e}")))?;
    if !fuigo_tools::util::image_validate::image_structurally_complete(&raw_bytes) {
    ...
    if !exceeded_size && !exceeded_dimensions {
        if let Err(e) = fuigo_tools::util::image_validate::validate_image_bytes(&raw_bytes) {
