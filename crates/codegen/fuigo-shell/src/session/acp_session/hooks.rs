...
const CLIENT_VERIFICATION_GATE_TIMEOUT: Duration =
    Duration::from_secs(fuigo_hooks::config::DEFAULT_VERIFICATION_GATE_TIMEOUT_SECS);

fn default_client_gate_timeout(gate: fuigo_hooks::event::GateKind) -> Duration {
    use fuigo_hooks::event::GateKind;
    match gate {
...
impl SessionActor {
    ...
    > {
        ...
        groups
            .iter()
            .filter(move |group| {
                fuigo_hooks::matcher::matcher_allows(group.matcher.as_ref(), tool_name)
            })
            .flat_map(move |group| {
            ...
            .map(move |(callback_id, timeout)| {
                ...
                async move {
                    ...
                    fuigo_telemetry::session_ctx::log_event(
                        fuigo_telemetry::events::ClientHookGate {
    ...
    pub(super) async fn run_stop_client_hooks(
        &self,
        envelope: &HookEventEnvelope,
    ) -> fuigo_hooks::dispatcher::StopDispatchResult {
        use fuigo_hooks::result::HookRunResult;

        let mut out = fuigo_hooks::dispatcher::StopDispatchResult::default();
        let Some(groups) = self
            .client_hooks
            .borrow()
        ...
        } in ordered
        {
            ...
            let detail = fuigo_hooks::dispatcher::stop_detail(
                stop_reason.is_some(),
            ...
            out.absorb(
                &hook_name,
                fuigo_hooks::dispatcher::StopSignals {
                    block_reason,
                    stop_reason,
    ...
    pub(super) async fn run_post_tool_use_client_hooks(
        &self,
        envelope: &HookEventEnvelope,
    ) -> fuigo_hooks::dispatcher::PostToolUseResult {
        use fuigo_hooks::result::HookRunResult;

        let mut out = fuigo_hooks::dispatcher::PostToolUseResult::default();
        let Some(groups) = self
            .client_hooks
            .borrow()
            .get(&envelope.hook_event_name.canonical())
        ...
        } in ordered
        {
            ...
            if let Some(context) = response.additional_context.filter(|c| !c.trim().is_empty()) {
                out.additional_context
                    .push(fuigo_hooks::dispatcher::AdditionalContext {
                        hook_name,
        ...
        local
            .run_until(async {
                ...
                *actor.hook_registry.borrow_mut() = Some(std::sync::Arc::new(
                    fuigo_hooks::discovery::HookRegistry::default(),
                ));
                assert!(actor.may_have_hooks_for(HookEventName::Stop));
