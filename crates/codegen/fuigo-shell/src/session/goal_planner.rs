...
///
/// [`SubagentRuntimeOverrides::harness_agent_type`]: fuigo_tools::implementations::fuigo_build::task::types::SubagentRuntimeOverrides::harness_agent_type
...
pub(crate) struct RoleSpawnOverride {
    ...
    /// Resolved harness `agent_type` (e.g. `"fuigo-build-plan"`), applied REGARDLESS of the parent agent.
...
mod tests {
    ...
    async fn fail_open_retry_emits_spawn_failed_event() {
        ...
        let writer = fuigo_session_events::EventWriter::open(dir.path());
        let ov = RoleSpawnOverride {
            model: Some("m".into()),
            agent_type: Some("t".into()),
        };
