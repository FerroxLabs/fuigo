...
use fuigo_agent_lifecycle::LocalExtensionRegistryBuilder;
use fuigo_agent_lifecycle::{LocalSessionLifecycleContributor, LocalTurnLifecycleContributor};
use fuigo_agent_lifecycle::{
...
/// `FUIGO_IDLE_NOTIFICATION_DELAY_MS` overrides it (used by E2E tests).
fn idle_notification_delay() -> Duration {
    resolve_idle_notification_delay(std::env::var("FUIGO_IDLE_NOTIFICATION_DELAY_MS").ok())
}
...
/// The headless E2E covers this via `FUIGO_IDLE_NOTIFICATION_DELAY_MS`.
...
mod idle_notification_delay_tests {
    ...
    /// Pins the public `FUIGO_IDLE_NOTIFICATION_DELAY_MS` contract: a valid override is interpreted as milliseconds (the E2E tests depend on this).
    #[test]
...
mod idle_after_interrupt_tests {
    use super::*;
    use fuigo_agent_lifecycle::TurnAbortReason;
