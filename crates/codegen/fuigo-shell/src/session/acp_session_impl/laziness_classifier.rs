...
/// Prefix on `x_fuigo_req_id` for laziness-classifier sampler calls, used by the production producer (`maybe_fire_laziness_check`).
pub(crate) const LAZINESS_REQ_ID_PREFIX: &str = "fuigo-laziness-";
        ...
        match item {
            ...
            ConversationItem::User(user) => {
                ...
                for part in &user.content {
                    if let fuigo_sampling_types::ContentPart::Text { text: t } = part {
                        if !text.is_empty() {
                            text.push(' ');
                        }
                        text.push_str(t);
                ...
                if user.synthetic_reason == Some(SyntheticReason::AgentMessage) {
                    let _ = writeln!(
                        out,
                        "[agent_message] {} {}",
                        fuigo_chat_state::compaction_utils::AGENT_MESSAGE_MODEL_LABEL,
                        truncate(&text)
                    );
                } else {
            ...
            ConversationItem::Reasoning(r) => {
                if include_reasoning {
                    let text = fuigo_sampling_types::reasoning_item_text(r);
...
const CLASSIFIER_TURN_MAX_LEN: usize = fuigo_workspace::permission::CLASSIFIER_TURN_MAX_LEN;
...
pub(crate) fn build_classifier_turns(
    items: &[ConversationItem],
) -> Vec<fuigo_workspace::permission::ClassifierTurn> {
    use fuigo_workspace::permission::ClassifierTurn;
        ...
        match item {
            ConversationItem::User(user) => {
                ...
                let text = if user.synthetic_reason == Some(SyntheticReason::Interjection) {
                    item_text
                } else if fuigo_chat_state::compaction_utils::is_real_user_turn(item)
                    && !is_project_instructions(item)
                    && !is_legacy_tool_image
                {
                    fuigo_chat_state::compaction_utils::extract_user_query(&item_text)
                } else {
                    continue;
                };
                if !text.is_empty() {
                    ...
                    let text =
                        fuigo_tools::util::truncate_str_with_marker(&text, CLASSIFIER_TURN_MAX_LEN)
                            .into_owned();
            ...
            ConversationItem::Assistant(assistant) => {
                for tc in &assistant.tool_calls {
                    ...
                    let args =
                        fuigo_tools::util::truncate_str_with_marker(&args, CLASSIFIER_TURN_MAX_LEN)
                            .into_owned();
                    let tool = neutralize_transcript_user_text(&tc.name);
                    let tool =
                        fuigo_tools::util::truncate_str_with_marker(&tool, CLASSIFIER_TURN_MAX_LEN)
                            .into_owned();
