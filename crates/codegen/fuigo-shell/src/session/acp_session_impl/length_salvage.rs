...
/// Kill switches outrank the always-on cursor tier: an explicit `FUIGO_LENGTH_SALVAGE=0` locally, and the remote `length_salvage_budget = 0`.
...
impl SessionActor {
    ...
    /// Always on when [`SessionActor::is_cursor_agent`]; the `FUIGO_LENGTH_SALVAGE` env var is the local debug override.
    ...
    pub(super) fn length_salvage_budget(&self) -> Option<u32> {
        resolve_length_salvage_budget(
            ...
            fuigo_config::env_bool("FUIGO_LENGTH_SALVAGE"),
            None,
        )
    }
}
