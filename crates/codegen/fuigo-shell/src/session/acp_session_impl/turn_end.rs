...
impl SessionActor {
    ...
    pub(super) async fn maybe_notify_git_branch(&self) {
        ...
        let (worktree_info, branch, commit) = tokio::join!(
            fuigo_workspace::session::git::get_worktree_info(cwd),
            fuigo_workspace::session::git::get_branch(cwd),
            fuigo_workspace::session::git::get_current_commit(cwd),
        );
    ...
    pub(super) async fn outstanding_reply_for_prompt(
        &self,
        prompt_id: &str,
    ) -> Option<fuigo_tools::implementations::fuigo_build::task::types::SubagentOutstandingReply>
    {
    ...
    pub(super) fn classify_install_error(
        err: &fuigo_agent::plugins::install_registry::InstallError,
    ) -> String {
