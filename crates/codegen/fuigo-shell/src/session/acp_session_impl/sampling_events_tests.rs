...
fn own_request(actor: &SessionActor, request_id: &fuigo_sampler::RequestId) {
...
async fn channel_tokens_accumulate_into_streaming_capture() {
    use fuigo_sampler::{RequestId, SamplingChannel, SamplingEvent};
...
async fn completed_event_clears_slot_keeps_prior_uncommitted_segments() {
    use fuigo_sampler::{InferenceLatencyStats, RequestId, SamplingChannel, SamplingEvent};
            ...
            *actor
            ...
            actor
                .handle_sampling_event(SamplingEvent::Retrying {
                    ...
                    attempt: 1,
                    max_retries: 2,
                    kind: fuigo_sampler::SamplingErrorKind::DoomLoopDetected,
...
async fn unowned_retry_does_not_notify_the_next_turn() {
    use fuigo_sampler::{RequestId, SamplingErrorKind, SamplingEvent};
...
async fn observe_only_confident_completion_stays_warn_only() {
    ...
    local
        .run_until(async {
            let mut fixture = make_replay_send_update_fixture().await;
            fixture.actor.doom_loop_recovery = Some(fuigo_sampling_types::DoomLoopRecoveryPolicy {
                max_threshold: 8,
                max_retries: 0,
                ..Default::default()
            });
            ...
            *actor
            ...
            let response = fuigo_sampling_types::ConversationResponse {
...
async fn exact_repetition_completion_is_tracked_for_incidence_only() {
    ...
    local
        .run_until(async {
            ...
            let actor = Arc::new(fixture.actor);
            let response = fuigo_sampling_types::ConversationResponse {
                items: vec![fuigo_sampling_types::ConversationItem::assistant("answer")],
                stop_reason: None,
                usage: None,
                cost_usd_ticks: None,
                message_chunks_emitted: 1,
...
async fn doom_loop_recovery_stamps_capture_segments_and_counters() {
    use fuigo_sampler::{RequestId, SamplingChannel, SamplingErrorKind, SamplingEvent};
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            let mut fixture = make_replay_send_update_fixture().await;
            ...
            fixture.actor.doom_loop_recovery =
                Some(fuigo_sampling_types::DoomLoopRecoveryPolicy::default());
            let actor = Arc::new(fixture.actor);
...
async fn reasoning_only_doomloop_turn_captures_every_generation_as_segments() {
    ...
    use fuigo_sampling_types::{EmptyReason, EmptyResponseContext};
    let local = tokio::task::LocalSet::new();
    local
        .run_until(async {
            ...
            *actor
            ...
            let (_chat_tx, chat_rx) = mpsc::unbounded_channel::<fuigo_chat_state::ChatStateEvent>();
