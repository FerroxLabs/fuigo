...
use fuigo_telemetry::session_end::{self, Phase};
...
pub(super) struct MemoryFlushSnapshot {
    counts: fuigo_chat_state::ConversationCounts,
...
impl SessionActor {
    ...
    pub(super) async fn register_memory_tools(
        ...
        bridge
            .register_mcp_tools(
                ...
                fuigo_tools::implementations::memory::search_tool::MemorySearchImpl,
                None,
            )
            .await
        ...
        bridge
            .register_mcp_tools(
                ...
                fuigo_tools::implementations::memory::get_tool::MemoryGetImpl,
                None,
            )
            .await
    ...
    pub(super) fn emit_memory_session_summary(
        ...
        fuigo_telemetry::session_ctx::log_event(
            fuigo_telemetry::memory_telemetry::MemorySessionSummary {
    ...
    pub(super) async fn run_session_end_memory_pipeline(
        &self,
        log_suffix: &str,
        timer: &fuigo_telemetry::session_end::SharedSessionEndTimer,
    ) {
            ...
            match &result {
                crate::session::memory::hooks::SessionEndResult::Written(path_str) => {
                    ...
                    self.reindex_and_embed(std::path::Path::new(path_str), "session")
                        .await;
                    self.send_fuigo_notification(FuigoSessionUpdate::MemorySessionSaved {
                        path: path_str.clone(),
                    })
                    .await;
                }
    ...
    fn dream_context(
        ...
        let sid8 = MemoryStorage::session_suffix(sid);
    ...
    async fn run_dream_inner(
        ...
        let generation = match storage.generation() {
            Ok(epoch) => epoch,
            Err(_) => return DreamAttempt::Skipped("memory generation could not be read"),
        };
        let existing_memory = match std::fs::read_to_string(storage.workspace_memory_file()) {
        ...
        let result = match storage.with_generation(generation, || {
            Ok(execute_dream(
                storage,
                &model_response,
                sessions.len(),
                existing_memory.as_deref().unwrap_or(""),
            ))
        }) {
            Ok(result) => result,
            Err(_) => return DreamAttempt::Skipped("memory was cleared during consolidation"),
        };
        ...
        let dream_path = match &result.status {
            DreamStatus::Completed { .. } => {
                ...
                let indexed = self
                    .memory
                    .open_index(storage)
                    .and_then(|mut index| index.reindex_file(&path, "dream").ok())
                    .is_some();
                if !indexed {
                    self.memory.record_dream_result(false);
                    self.send_fuigo_notification(FuigoSessionUpdate::MemoryDreamCompleted {
                ...
                // Raw sources stay on disk. The success marker records exact source hashes.
                ...
                if storage
                    .with_generation(generation, || {
                        Ok(guard.commit_sources(&dream_msg.source_snapshots))
                    })
                    .unwrap_or(false)
                {
        ...
        let dream_result_str = if marker_failed {
            "failed: success marker could not be saved; retry remains open".into()
        } else {
            match &result.status {
                DreamStatus::Completed { chars_written } => {
                    format!("written ({chars_written} chars)")
                }
        ...
        tracing::info!(
            ...
            sessions_retained = dream_msg.processed_stems.len(),
    ...
    async fn run_dream_model_call(&self, user_message: &str) -> Result<String, acp::Error> {
        ...
        let request = ConversationRequest {
            ...
            model: Some(model),
            x_fuigo_conv_id: Some(format!("dream-{}", uuid::Uuid::new_v4())),
            x_fuigo_req_id: Some(format!("fuigo-dream-{}", uuid::Uuid::new_v4())),
    ...
    pub(super) async fn run_memory_flush(
        ...
        tracing::info!(target: fuigo_telemetry::memory_log::TARGET, "MEMORY_FLUSH: starting");
        let flush_start = std::time::Instant::now();
        let Some(flush_generation) = self
            .memory
            .storage()
            .and_then(|storage| storage.generation().ok())
        else {
            self.memory.release_flush_lock();
            return false;
        };

        let flush_turn = self.chat_state_handle.get_conversation_counts().await.total;
        self.send_fuigo_notification(FuigoSessionUpdate::MemoryFlushStarted)
            .await;
        ...
        let result = async {
            ...
            fuigo_telemetry::session_ctx::log_event(
                fuigo_telemetry::memory_telemetry::MemoryFlushStart {
                    session_id: self.session_info.id.to_string(),
            ...
            items.extend(
                fuigo_chat_state::compaction_utils::ModelRequestHistory::from_raw(recent).into_items(),
            );
            ...
            let request = ConversationRequest {
                ...
                x_fuigo_req_id: Some(format!("fuigo-flush-{}", uuid::Uuid::new_v4())),
                x_fuigo_session_id: Some(session_id.clone()),
                x_fuigo_agent_id: Some(fuigo_telemetry::id::agent_id()),
                ..Default::default()
            };
                ...
                match process_flush_response(&response_text, &self.memory.flush_config) {
                    ...
                    FlushResult::Accepted(content) => {
                        ...
                        if is_sem_dup {
                            ...
                            let persisted = fuigo_memory::safety::capture_record(
                                &content,
                                session_id,
                            ...
                            match storage.with_generation(flush_generation, || {
                                let persisted = persisted.ok_or_else(|| {
                                    std::io::Error::other("unsafe flush content rejected")
                                })?;
                                storage
                                    .write_daily_log(&date, trigger, session_id, &persisted, true)
                            }) {
                                Ok(path) => {
        ...
        tracing::info!(target: fuigo_telemetry::memory_log::TARGET, outcome = %outcome, "MEMORY_FLUSH: completed");
        ...
        fuigo_telemetry::session_ctx::log_event(
            fuigo_telemetry::memory_telemetry::MemoryFlushComplete {
                session_id: self.session_info.id.to_string(),
                trigger: trigger.to_owned(),
        ...
        let flush_trigger = match trigger {
            "slash_command" => fuigo_telemetry::events::MemoryFlushTrigger::SlashCommand,
            "interval" => fuigo_telemetry::events::MemoryFlushTrigger::Interval,
            "pre_compaction" => fuigo_telemetry::events::MemoryFlushTrigger::PreCompaction,
            _ => fuigo_telemetry::events::MemoryFlushTrigger::UserRequested,
        };
        fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::MemoryFlushed {
        ...
        self.memory.release_flush_lock();
        self.send_fuigo_notification(FuigoSessionUpdate::MemoryFlushCompleted {
            result: outcome,
            path: flush_path,
        })
        .await;
