...
impl SessionActor {
    ...
    /// The client sees no "Fuigo is thinking", no streaming token chunks, and no other session update from a fire.
    ...
    pub(crate) async fn maybe_fire_laziness_check(self: Arc<Self>) {
        ...
        let items = vec![
            ConversationItem::System(fuigo_sampling_types::SystemItem {
        ...
        let request = ConversationRequest {
            ...
            x_fuigo_conv_id: Some(format!("trace-classifier-{}", uuid::Uuid::new_v4())),
            x_fuigo_req_id: Some(format!("{LAZINESS_REQ_ID_PREFIX}{}", uuid::Uuid::new_v4())),
            x_fuigo_session_id: Some(session_id_str),
