...
use fuigo_telemetry::region::Parent as SpanParent;
use fuigo_telemetry::subagent_spawn::phase_region_under;
...
fn drop_cli_catchall_allows(
    rules: Vec<fuigo_workspace::permission::types::PermissionRule>,
    ...
    for rule in rules {
        if fuigo_workspace::permission::resolution::is_catchall_allow(&rule) {
            dropped.push(rule);
        } else {
            kept.push(rule);
        }
...
/// This only caps the blocking pool; pre-warming is reserved for process-lifetime runtimes (`fuigo_tty_utils::runtime`).
...
) -> fuigo_telemetry::events::MemoryRetrievalMode {
    use fuigo_telemetry::events::MemoryRetrievalMode::*;
...
fn subagent_sampler_rate_limit_threshold(is_subagent: bool, pacer_max_attempts: u32) -> u32 {
    if is_subagent && pacer_max_attempts > 0 {
        fuigo_sampler::RATE_LIMIT_RETRY_DISABLED
    } else {
...
mod cli_catchall_drop_tests {
    ...
    use fuigo_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    use fuigo_workspace::permission::rules::parse_permission_rule;
    use fuigo_workspace::permission::types::{PermissionRule, RuleAction, ToolFilter};
...
pub(crate) async fn spawn_session_actor(
    ...
    credentials: fuigo_chat_state::Credentials,
    ...
    compaction_mode: fuigo_chat_state::CompactionMode,
    ...
    web_search_sampling_config: Option<fuigo_sampler::SamplerConfig>,
    ...
    app_builder_deployer_config: fuigo_tools::implementations::fuigo_build::app_builder::AppBuilderDeployerConfig,
    ...
    media_gen_batch_limits: fuigo_tools::media_gen_limits::MediaGenBatchLimits,
    ...
    prompt_audience: fuigo_agent::prompt::context::PromptAudience,
    ...
    plugin_registry_handle: Option<fuigo_agent::plugins::SharedPluginRegistryHandle>,
    models_manager: crate::agent::models::ModelsManager,
    inherited_permission_handle: Option<fuigo_workspace::permission::PermissionHandle>,
    api_key_provider: Option<fuigo_tools::types::SharedApiKeyProvider>,
    image_description_model: String,
    hook_registry_override: Option<std::sync::Arc<fuigo_hooks::discovery::HookRegistry>>,
    workspace_ops: fuigo_workspace::WorkspaceOps,
    cli_permission_rules: Vec<fuigo_workspace::permission::types::PermissionRule>,
    todo_gate: bool,
    ...
    parent_terminal_backend: Option<
        std::sync::Arc<dyn fuigo_tools::computer::types::TerminalBackend>,
    >,
    parent_scheduler_handle: Option<
        fuigo_tools::implementations::fuigo_build::scheduler::types::SchedulerHandle,
    >,
    ...
    fuigo_agent::AgentBuildError,
> {
    if max_turns == Some(0) {
        return Err(fuigo_agent::AgentBuildError::InvalidConfig(
        ...
        inherited_permission_handle
    {
        ...
        let mut permission_config =
            fuigo_workspace::permission::resolution::resolve_permission_config_with_fallback(
                tool_context.cwd.as_path(),
                project_trusted,
            )
            .await;
        let yolo_pin = fuigo_workspace::permission::resolution::yolo_disabled_by_policy();
        ...
        fuigo_workspace::permission::resolution::apply_permission_mode_hint(
        ...
        let deny_read_globs = permission_config
            .as_ref()
            .map(fuigo_workspace::permission::resolution::deny_read_globs_from_config)
            .unwrap_or_default();
        let hub_permission = if fuigo_workspace::permission::hitl_permission_live_enabled() {
            ...
            let transport = server
                .and_then(|server| {
                    fuigo_workspace::permission::ToolServerPermissionTransport::from_session_id(
                        server,
        ...
        let (permissions, permission_events_rx) =
            fuigo_workspace::permission::spawn_permission_manager_with_hub(
    ...
    let web_search_config = if disable_web_search {
        ...
        if let Some(api_key) = cfg.api_key {
            fuigo_tools::implementations::WebSearchConfig::Enabled {
                api_key,
    ...
    let context_window_override = std::env::var("FUIGO_DEBUG_CONTEXT_WINDOW")
        .ok()
    ...
    let chat_state_handle = fuigo_chat_state::ChatStateActor::spawn_with_pruning(
        conversation.clone(),
    ...
    *synthetic_trace_tx_shared.lock().unwrap() = tool_context.synthetic_trace_tx.clone();
    ...
    let index_root_for_session =
        fuigo_workspace::session::git::find_git_root_from_path(tool_context.cwd.as_path())
    ...
    let resolve_search_shadows = || {
        ...
        fuigo_tools::computer::local::SearchShadowConfig {
            find_bfs,
            grep_ugrep,
        }
    };
        ...
        match terminal_backend_kind {
            ...
            TerminalBackendKind::AcpClient => {
                std::sync::Arc::new(crate::terminal::AcpTerminalAdapter::new(
                    ...
                    as std::sync::Arc<dyn fuigo_tools::computer::types::TerminalBackend>
            }
    ...
    let fs_backend: std::sync::Arc<dyn fuigo_tools::computer::types::AsyncFileSystem> =
        if client_fs_capable && tool_context.gateway.is_some() {
            std::sync::Arc::new(fuigo_workspace::file_system::AcpFsAdapter::new(
                tool_context.gateway.clone().unwrap(),
                tool_context.session_id.clone().unwrap(),
            ))
        } else {
            std::sync::Arc::new(fuigo_tools::computer::local::LocalFs)
        };
    ...
    let compaction_policy = fuigo_agent::CompactionPolicy {
    ...
    let (user_question_tx, user_question_rx) = tokio::sync::mpsc::unbounded_channel::<
        fuigo_tools::implementations::fuigo_build::ask_user_question::types::UserQuestionRequest,
    >();
    ...
    let memory_backend_for_spec: Option<
        std::sync::Arc<dyn fuigo_tools::types::memory_backend::MemoryBackend>,
    > = if let Some(ref storage) = memory_storage_for_session {
        ...
        let backend: std::sync::Arc<dyn fuigo_tools::types::memory_backend::MemoryBackend> =
        ...
        fuigo_telemetry::session_ctx::log_event(
            fuigo_telemetry::memory_telemetry::MemorySessionInit {
                session_id: session_info.id.to_string(),
                memory_enabled: true,
                watcher_config_enabled: watcher_config.enabled,
                watcher_started,
    ...
    let managed_gateway_tool_client = auth_manager.as_ref().map(|am| {
        fuigo_tools::types::resources::ManagedGatewayToolClient(Arc::new(
    ...
    use fuigo_telemetry::subagent_spawn::SubagentSpawnPhase;
    ...
    let harness_metrics = if !startup_hints.is_subagent
        && (telemetry_enabled || fuigo_telemetry::external::is_active())
    {
    ...
    let scheduler_handle_for_handle = {
        ...
        res.get::<fuigo_tools::implementations::fuigo_build::scheduler::types::SchedulerHandle>()
            .cloned()
    };
    ...
    let feedback_client_type = match client_type {
        ClientType::FuigoTUI => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
        ClientType::FuigoWeb => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Web,
        ...
        ClientType::FuigoPager => prod_mc_cli_chat_proxy_types::feedback_types::ClientType::Tui,
    };
    ...
    let resolved_workspace_root = fuigo_workspace::session::git::find_git_root_from_path(
    ...
    let (goal_update_tx, goal_update_rx) = tokio::sync::mpsc::unbounded_channel::<
        fuigo_tools::implementations::fuigo_build::update_goal::UpdateGoalEnvelope,
    >();
    ...
    let (workflow_launch_tx, mut workflow_launch_rx) = tokio::sync::mpsc::unbounded_channel::<
        fuigo_tools::implementations::fuigo_build::workflow::WorkflowLaunchEnvelope,
    >();
    {
        ...
        tokio::spawn(async move {
            ...
            use fuigo_tools::implementations::fuigo_build::workflow::WorkflowLaunchAck;
            while let Some((req, ack)) = workflow_launch_rx.recv().await {
                if !background_workflows_enabled {
                    let _ = ack.send(WorkflowLaunchAck::Rejected {
                        ...
                        detail: "Background workflows are disabled for this session \
                                 ([workflows] enabled = false / FUIGO_WORKFLOWS=0 / remote flag)."
                            .into(),
                    });
                    continue;
                }
                ...
                if input.validate_only {
                    ...
                    let agent_budget = input
                        .agent_budget
                        .unwrap_or(fuigo_workflow::DEFAULT_AGENT_BUDGET);
                    tokio::spawn(async move {
                        let verdict = tokio::task::spawn_blocking(move || {
                            fuigo_workflow::validate_script_with_agent_budget(
                                &script,
                                probe_args,
                                agent_budget,
                            )
    ...
    let obs_bridge = {
        let sid = fuigo_tool_protocol::SessionId::new(&*session_info.id.0)
            .unwrap_or_else(|_| fuigo_tool_protocol::SessionId::new("unknown").expect("valid"));
        fuigo_computer_hub_sdk::ObservabilityBridge::new(None, sid)
    };
    ...
    let resolved_tool_overrides: std::sync::Arc<
        arc_swap::ArcSwapOption<fuigo_sampling_types::ToolOverrides>,
    ...
    let vcs_kind = {
        ...
        match fuigo_workspace::session::git::discover_git_root(root) {
            fuigo_workspace::session::git::GitDiscoveryResult::Found(git_root) => {
                fuigo_workspace::session::git::detect_vcs_kind(&git_root)
            }
            _ => fuigo_workspace::session::git::VcsKind::None,
        }
    };
    ...
    let repo_status_plan = if matches!(vcs_kind, fuigo_workspace::session::git::VcsKind::None) {
    ...
    let session = Arc::new_cyclic(|weak: &std::sync::Weak<SessionActor>| SessionActor {
        ...
        programmatic_tool_calling: std::cell::Cell::new(sampling_config.programmatic_tool_calling),
    ...
    if let Some(storage) = session.memory.storage() {
        ...
        tokio::task::spawn_local(async move {
            ...
            ) && let Ok(files) = storage.list_memory_files()
            {
                ...
                fuigo_telemetry::session_ctx::log_event(
                    fuigo_telemetry::memory_telemetry::MemoryReindex {
    ...
    }
    {
        ...
        tokio::task::spawn_local(async move {
            while let Some(mut request) = user_question_rx.recv().await {
                use fuigo_tools::implementations::fuigo_build::ask_user_question::AskUserQuestionMode;
    ...
    if let Some(metrics) = harness_metrics {
        ...
        tokio::spawn(async move {
            ...
            fuigo_telemetry::session_ctx::log_event_dual(telemetry_enabled, ev);
        });
    }
    let hosting = fuigo_telemetry::activity::SESSIONS_ACTIVE.enter();
    tokio::task::spawn_local(async move {
        let _hosting = hosting;
        fuigo_telemetry::session_ctx::with_session_ctx(
            telemetry_ctx,
            run_session(
                session,
                cmd_rx,
        ...
        .spawn(move || {
            ...
            let rt = match build_session_runtime() {
                ...
                Err(e) => {
                    ...
                    let _ = init_tx.send(Err(fuigo_agent::AgentBuildError::RuntimeBuild(e)));
                    return;
                }
            };
            ...
            let actor_main = async move {
                let _trace_span = parent_traceparent.as_ref().map(|tp| {
                    let meta = serde_json::json!({ "traceparent": tp })
                        ...
                        .cloned()
                        .unwrap_or_default();
                    let span = fuigo_file_utils::trace_context::span_from_meta_traceparent(&meta);
                    span.entered()
                });
            ...
            rt.block_on(fuigo_telemetry::session_ctx::drain_at_session_exit());
        });
    ...
    conversation.iter().any(|item| {
        ...
        let contains = |needle: &str| {
            u.content.iter().any(|part| {
                matches!(
                    part,
                    fuigo_sampling_types::conversation::ContentPart::Text { text }
