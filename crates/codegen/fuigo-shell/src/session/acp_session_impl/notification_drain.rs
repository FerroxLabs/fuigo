...
impl SessionActor {
    ...
    pub(super) async fn reconcile_live_orphaned_subagents(&self) {
        ...
        while let Ok(cmd) = cmd_rx.try_recv() {
            if let SessionCommand::FuigoSessionNotification { notification } = cmd {
                self.handle_fuigo_session_notification(notification).await;
            }
        }
    }
    ...
    pub(super) async fn list_running_subagents(
        &self,
    ) -> Vec<fuigo_tools::implementations::fuigo_build::task::types::SubagentInspection> {
        self.reconcile_live_orphaned_subagents().await;
        let Some(event_tx) = self.tool_context.subagent_event_tx.clone() else {
            return Vec::new();
        };
        fuigo_tools::implementations::fuigo_build::task::backend::ChannelBackend::new(event_tx)
    ...
    pub(super) async fn emit_session_idle_if_idle(&self) {
        ...
        for contributor in self.extension_registry.session_lifecycle_contributors() {
            contributor
                .on_session_idle(&fuigo_agent_lifecycle::SessionIdleInput)
                .await;
        }
    }
    ...
    pub(super) fn sweep_monitor_buffer_into_pending(
        ...
        for event in fuigo_tools::implementations::fuigo_build::monitor::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        ) {
            Self::push_pending_notification(
    ...
    fn notification_blocks(
        ...
        use fuigo_tools::implementations::fuigo_build::monitor::types::MonitorEventNotification;
        ...
        if let (Some(index), Some(batch)) = (
            monitor_section_idx,
            fuigo_tools::reminders::task_completion::format_monitor_events(
                &monitor_events,
...
mod live_orphan_hook_tests {
    ...
    fn persisted_cancelled_finishes(
        ...
        while let Ok(msg) = persistence_rx.try_recv() {
            let PersistenceMsg::Update(crate::session::storage::SessionUpdate::Fuigo(notif)) = msg
            else {
                continue;
            };
