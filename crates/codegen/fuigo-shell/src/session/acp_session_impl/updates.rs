    ...
    async fn persist_path_scrubs_title_and_drops_manual_meta() {
        ...
        local
            .run_until(async {
                ...
                actor
                    .handle_fuigo_session_notification(FuigoSessionNotification {
                ...
                loop {
                    match prx.try_recv().expect("inbound summary must be persisted") {
                        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Fuigo(
                            notif,
                        )) => {
                            let FuigoSessionUpdate::SessionSummaryGenerated { session_summary } =
                                &notif.update
                            else {
                                continue;
                            };
...
impl SessionActor {
    ...
    pub(super) async fn record_subagent_usage(
        &self,
        by_model: &[(String, fuigo_chat_state::UsageTotals)],
    ...
    /// Producer for the **high-frequency streaming path** with an Ferrox Labs extension payload.
    /// Routes through `event_tx`, the `ReplayBuffer`, and `emit_buffered`, so chunks are merged, debounced, and emitted.
    ///
    /// For one-shot Ferrox Labs events (RetryState, ImageCompressed, HookExecution, AutoCompactCompleted, etc.), use `send_fuigo_notification` instead.
    ///
    /// The frequency-based split (`send_buffered_fuigo_update` vs `send_fuigo_notification`) mirrors the ACP-side split.
    /// There, `send_update` is the high-frequency buffered path and `emit_notification_direct` the low-frequency direct one.
    pub(super) async fn send_buffered_fuigo_update(&self, update: FuigoSessionUpdate) {
        self.close_rewind_window().await;
    ...
    /// - **Ferrox Labs** (`ToolCallDeltaChunk`) inlines a gateway forward as `ExtNotification` only, with no persistence and no hook dispatch.
    ...
    pub(super) async fn emit_buffered(&self, notification: SessionNotification) {
        match notification {
            ...
            SessionNotification::Fuigo(n) => {
                self.log_outbound_fuigo_buffered(&n);
                if self
                    .notifications
                    .gateway_enabled
    ...
    fn log_outbound_fuigo_buffered(&self, notification: &FuigoSessionNotification) {
        if !matches!(
            notification.update,
            FuigoSessionUpdate::ToolCallDeltaChunk { .. }
        ) {
            return;
        }
        tracing::info!(
            target: "acp_event",
            event = "fuigo_buffered_notification_sent",
            session_id = %self.session_info.id,
            "Sending buffered Ferrox Labs session notification"
        );
    }
    ...
    /// Handle Ferrox Labs session notifications: store them in persistence.
    ...
    pub(super) async fn handle_fuigo_session_notification(
        &self,
        mut notification: FuigoSessionNotification,
    ) {
        if !matches!(
            notification.update,
            FuigoSessionUpdate::SubagentProgress { .. }
        ) {
            tracing::debug!("storing Ferrox Labs session notification");
        }
        {
        ...
        match &notification.update {
            FuigoSessionUpdate::SubagentSpawned {
                ...
                let envelope = self.fire_hook(
                    fuigo_hooks::event::HookEventName::SubagentStart,
                    None,
                    fuigo_hooks::event::HookPayload::SubagentStart {
                ...
                if let Some(registry) = hook_registry_snapshot {
                    ...
                    let _ = fuigo_hooks::dispatcher::dispatch_non_blocking(
                        &registry,
                        fuigo_hooks::event::HookEventName::SubagentStart,
                        &envelope,
                        &ctx,
    ...
    /// Persist an Ferrox Labs extension notification to `updates.jsonl` **without** sending it to the gateway/UI.
    /// Used for internal bookkeeping updates like `CompactionCheckpoint` and `RewindMarker` that are only relevant during replay.
    pub(super) fn persist_fuigo_update_only(&self, update: FuigoSessionUpdate) {
        let notification = FuigoSessionNotification {
            session_id: self.session_info.id.clone(),
            update,
        ...
        if self
            ...
            .is_err()
        {
            tracing::warn!("Failed to send Ferrox Labs update to persistence channel");
        }
    }
    ...
    pub(super) async fn dispatch_notification_hook(
        ...
        let envelope = self.fire_hook(
            fuigo_hooks::event::HookEventName::Notification,
            None,
            fuigo_hooks::event::HookPayload::Notification {
    ...
    pub(super) async fn send_fuigo_notification(&self, update: FuigoSessionUpdate) {
    ...
    /// [`Self::send_fuigo_notification`] with caller-supplied `_meta` keys merged into the standard eventId/timestamp meta.
    ...
    pub(super) async fn send_fuigo_notification_with_extra_meta(
        ...
        let persisted_update =
            crate::session::storage::SessionUpdate::Fuigo(Box::new(notification.clone()));
        match durability {
            crate::session::storage::jsonl::AppendDurability::Buffered => {
                let _ = self
                    .notifications
...
#[cfg(test)]
mod fuigo_event_id_stamping_tests {
    use super::support::create_test_actor;
    use super::*;
    fn persisted_fuigo_event_id(
                ...
                .expect("an Ferrox Labs line must be persisted")
            {
                PersistenceMsg::Update(crate::session::storage::SessionUpdate::Fuigo(notif)) => {
                    return notif
                        ...
                        .and_then(|m| m.get("eventId"))
                        .and_then(|v| v.as_str())
                        .expect("persisted Ferrox Labs lines must carry an eventId")
                        .to_string();
                }
                _ => continue,
            }
    ...
    /// Both persist paths must stamp: `send_fuigo_notification` (own emission) and `handle_fuigo_session_notification` (inbound/forwarded, meta-less).
    /// An id-less line degrades every later cursor reconnect of the session to a full replay.
    #[tokio::test]
    async fn actor_persisted_fuigo_lines_carry_event_id() {
        let local = tokio::task::LocalSet::new();
        local
            .run_until(async {
                let (gateway_tx, _gateway_rx) =
                ...
                let actor = create_test_actor(0, 256_000, 85, gateway_tx, persistence_tx).await;
                actor
                    .send_fuigo_notification(FuigoSessionUpdate::HookAnnotation {
                        message: "own emission".into(),
                    })
                    .await;
                let own_id = persisted_fuigo_event_id(&mut prx);
                assert!(own_id.starts_with("test-actor-"));
                actor
                    .handle_fuigo_session_notification(FuigoSessionNotification {
                        session_id: acp::SessionId::new("test-actor"),
                        update: FuigoSessionUpdate::HookAnnotation {
                            message: "inbound".into(),
                        },
                        meta: None,
                    })
                    .await;
                let inbound_id = persisted_fuigo_event_id(&mut prx);
                assert!(inbound_id.starts_with("test-actor-"));
                assert_ne!(own_id, inbound_id);
                actor.persist_fuigo_update_only(FuigoSessionUpdate::HookAnnotation {
                    message: "persist-only".into(),
                });
                let persist_only_id = persisted_fuigo_event_id(&mut prx);
    ...
    async fn live_user_message_chunk_forwards_when_echo_cap_is_on() {
        ...
        local
            .run_until(async {
                ...
                assert!(matches!(
                    gateway_rx.try_recv(),
                    Ok(fuigo_acp_lib::AcpClientMessage::SessionNotification(_))
                ));
            })
            .await;
    }
