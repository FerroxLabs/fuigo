...
use super::*;
use fuigo_hooks::result::HookDecision;
...
fn record_tool_span_outcome(
    span: tracing::Span,
    result: &Result<ToolRunResult, fuigo_tool_runtime::ToolError>,
) -> bool {
...
pub(super) fn tool_output_span_outcome(
    ...
    use fuigo_tools::implementations::fuigo_build::send_subagent_message::SendSubagentMessageDisposition;
    match result {
...
fn is_interruptible_wait_tool(tool_name: &str, args: &serde_json::Value) -> bool {
    match tool_name {
        ...
        | "get_terminal_command_output" => fuigo_tool_types::task_output_waits_from_json(args),
...
pub(super) fn is_file_backed_exit_plan_kind(
    ...
    matches!(kind, Some(fuigo_tools::types::tool::ToolKind::ExitPlan))
}
...
fn split_exit_plan_tail(
    ...
    kind_of: impl Fn(&str) -> Option<fuigo_tools::types::tool::ToolKind>,
) -> (
...
pub(super) enum PlanEditGate {
    ...
    Allow,
    /// Fuigo-toolset edit outside the plan file (plan-file-only rule).
    RejectNonPlanFile,
}
...
/// - **Compat-toolset `Delete`** is **not** on the markdown carve-out: it maps to `AccessKind::Edit` and is plan-file-only (same as fuigo edits).
...
impl PlanApprovalOutcome {
    fn from_response(
        resp: &fuigo_tools::implementations::fuigo_build::exit_plan_mode::ExitPlanModeExtResponse,
    ) -> Self {
...
fn ext_method_no_client(err: &acp::Error) -> bool {
    matches!(
        fuigo_acp_lib::acp_channel_failure(err),
        Some(fuigo_acp_lib::AcpChannelFailure::SendFailed)
    )
}
...
impl SessionActor {
    ...
    pub(super) fn stamp_tool_meta(
        ...
        fuigo_tools::normalization::merge_tool_meta(
            &toolset,
    ...
    pub(super) fn media_gen_over_cap(
        &self,
        calls: &[fuigo_sampling_types::ToolCall],
    ) -> Vec<fuigo_tools::media_gen_limits::MediaGenOverCap> {
        let kind_of = |name: &str| self.agent.borrow().tool_bridge().tool_kind(name);
        fuigo_tools::media_gen_limits::over_cap_by_name(
    ...
    async fn reject_excess_media_gen_calls(
        ...
        let kind_of = |name: &str| self.agent.borrow().tool_bridge().tool_kind(name);
        let (allowed, rejected) = fuigo_tools::media_gen_limits::partition_media_gen_batch(
            tool_calls,
        ...
        for (name, rejected_n) in &rejected_by_name {
            let max = self
                ...
                .tool_bridge()
                .tool_kind(name)
                .and_then(|k| fuigo_tools::media_gen_limits::max_calls_per_batch(k, limits))
                .unwrap_or(0);
    ...
    async fn execute_tool_calls_batch(
        ...
        for call in tool_calls.into_iter() {
            ...
            self.observability_bridge
                .emit(
                    fuigo_tool_protocol::session_event::SessionEvent::ToolCallStarted {
            ...
            match self.prepare_tool_call(call, deferred_followups).await? {
                ...
                Err(tool_loop) => {
                        ...
                        crate::session::mcp_servers::parse_mcp_tool_name(&call_name)
                    {
                        ...
                        self.emit_event(fuigo_session_events::Event::McpToolCallCompleted {
        ...
        let (
            ...
            (
                param_names.clone(),
                toolset
                    .tool_name_for_kind(fuigo_tools::types::tool::ToolKind::Workflow)
        ...
        while let Some((idx, result, duration_ms)) = dispatch_rx.recv().await {
            ...
            let mut post_tool_use_failure_contexts: Vec<
                fuigo_hooks::dispatcher::AdditionalContext,
            > = Vec::new();
            ...
            for context in &prepared.additional_context {
                deferred_followups.push(self.wrap_hook_note(
                    fuigo_hooks::event::HookEventName::PreToolUse,
            ...
            if let Some(delivery) = post_tool_use_delivery {
                for context in &delivery.additional_context {
                    deferred_followups.push(self.wrap_hook_note(
                        fuigo_hooks::event::HookEventName::PostToolUse,
                        HookNoteKind::Context,
                        &context.hook_name,
                        &context.text,
                    ));
            ...
            for context in &post_tool_use_failure_contexts {
                deferred_followups.push(self.wrap_hook_note(
                    fuigo_hooks::event::HookEventName::PostToolUseFailure,
                    HookNoteKind::Context,
                    &context.hook_name,
                    &context.text,
                ));
            ...
            self.observability_bridge
                .emit(
                    fuigo_tool_protocol::session_event::SessionEvent::ToolCallCompleted {
                ...
                .await;
            let (ext_file_path, ext_parameters) = if fuigo_telemetry::external::is_active() {
            ...
            fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::ToolCallCompleted {
                tool_name: prepared.tool_name.clone(),
                outcome: tool_outcome,
                hook_rewrote,
                duration_ms,
    ...
    async fn apply_pre_tool_use_gate(
        ...
        if let Some(registry) = hook_registry_snapshot {
            ...
            let pre_result =
                fuigo_hooks::dispatcher::dispatch_pre_tool_use(&registry, &envelope, &ctx).await;
    ...
    async fn validate_hook_rewrite(
        ...
        resolved_tool_name: &str,
        dispatch_target_name: &Option<String>,
        rewrite: fuigo_hooks::dispatcher::InputRewrite,
            ...
            .await
        {
            Ok(input) => input,
            Err(err) => {
                let schema_error = fuigo_hooks::event::clip_reason(&err.to_string());
                return Ok(Err(self
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        );
        {
            ...
            let subagent_background =
                fuigo_tools::is_task_tool_id(&call.function.name).then(|| {
                    early_raw_input
                        .as_ref()
        ...
        if self.may_have_hooks_for(fuigo_hooks::event::HookEventName::PreToolUse) {
            let gate = match self
        ...
        if !plan_file_auto_approve {
            ...
            let (telemetry_access_kind, _access_detail) = match &access_kind {
                fuigo_workspace::permission::AccessKind::Read(p) => (
                    fuigo_telemetry::events::AccessKind::Read,
                    p.clone().unwrap_or_default(),
                ),
                fuigo_workspace::permission::AccessKind::Edit(p) => {
                    (fuigo_telemetry::events::AccessKind::Edit, p.clone())
                }
                fuigo_workspace::permission::AccessKind::Bash(cmd) => {
                    (fuigo_telemetry::events::AccessKind::Bash, cmd.clone())
                }
                fuigo_workspace::permission::AccessKind::Grep { path, glob } => (
                    fuigo_telemetry::events::AccessKind::Grep,
                    path.clone().or_else(|| glob.clone()).unwrap_or_default(),
                ),
                fuigo_workspace::permission::AccessKind::MCPTool { name, .. } => {
                    (fuigo_telemetry::events::AccessKind::Mcp, name.clone())
                }
                fuigo_workspace::permission::AccessKind::WebFetch(u) => {
                    (fuigo_telemetry::events::AccessKind::Web, u.clone())
                }
                fuigo_workspace::permission::AccessKind::WebSearch(q) => {
                    (fuigo_telemetry::events::AccessKind::Web, q.clone())
                }
                fuigo_workspace::permission::AccessKind::AgentMessage { subagent_id } => (
                    fuigo_telemetry::events::AccessKind::AgentMessage,
                    subagent_id.clone(),
                ),
                _ => (fuigo_telemetry::events::AccessKind::Other, String::new()),
            };
            ...
            let perm_mode = if self.permissions.is_yolo_mode() {
                fuigo_telemetry::enums::PermissionMode::AlwaysApprove
            } else if self.permissions.is_auto_mode() {
                fuigo_telemetry::enums::PermissionMode::Auto
            } else {
                fuigo_telemetry::enums::PermissionMode::Ask
            };
            fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::PermissionPrompted {
            ...
            let path_context = Some(fuigo_workspace::permission::types::RequestPathContext {
            ...
            self.events.permission_resolved(
                ...
                match &decision {
                    Decision::Allow | Decision::Ask => {
                        fuigo_session_events::types::PermissionDecision::Allow
                    }
                    Decision::Reject(_) | Decision::PolicyDeny(_) => {
                        fuigo_session_events::types::PermissionDecision::Deny
                    }
                    Decision::Cancelled => {
                        fuigo_session_events::types::PermissionDecision::Cancelled
                    }
                    Decision::FollowupMessage(_) => {
                        fuigo_session_events::types::PermissionDecision::Followup
                    }
                },
                perm_start,
            );
            ...
            match decision {
                Decision::PolicyDeny(ref reason) | Decision::Reject(ref reason) => {
                    ...
                    let (tool_input_value, tool_input_truncated) =
                        fuigo_hooks::event::truncate_payload(raw_input.clone());
                    self.dispatch_hook(
                        fuigo_hooks::event::HookEventName::PermissionDenied,
                        fuigo_hooks::event::HookPayload::PermissionDenied {
    ...
    pub(super) async fn request_plan_approval(
        ...
        fuigo_tools::implementations::fuigo_build::exit_plan_mode::ExitPlanModeExtResponse,
        acp::Error,
    > {
        use agent_client_protocol::Client as _;
        use fuigo_tools::implementations::fuigo_build::exit_plan_mode::{
        ...
        if deactivated {
            ...
            *self.turn_prompt_mode.lock() = PromptMode::Agent;
            ...
            self.enqueue_current_mode_update(acp::SessionModeId::new(
                fuigo_tools::types::SessionMode::Default.as_id(),
            ));
        }
    }
        ...
        let (title, kind, locations, content) = match tool_call_input {
                ...
                (
                    ...
                    vec![
                        acp::ToolCallLocation::new(read_file.path)
                            ...
                            .line(
                                fuigo_tools::normalization::norm_offset_i64(read_file.offset)
                                    .map(|l| l as u32),
                            ),
                    ],
                    Vec::new(),
            ...
            ToolInput::WaitTasks(wait) => (
                format!(
                    ...
                    wait.task_ids.len(),
                    match wait.mode {
                        fuigo_tool_types::WaitMode::WaitAny => "wait_any",
                        fuigo_tool_types::WaitMode::WaitAll => "wait_all",
                    }
                ),
                acp::ToolKind::Other,
                vec![],
            ...
            ToolInput::Skill(skill) => {
                fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::SkillDispatched {
                    ...
                    plugin_source: None,
                    trigger: fuigo_telemetry::events::SkillTrigger::SkillTool,
                });
                tracing::info_span!(
                    "skill.activated",
    ...
    async fn skill_for_read_path(
        &self,
        path: &str,
    ) -> Option<fuigo_tools::implementations::skills::types::SkillInfo> {
        if self.is_chat_kind {
            return None;
        }
        let read_path = fuigo_tools::types::resources::resolve_model_path(
            self.tool_context.cwd.as_path(),
    ...
    fn emit_skill_md_read(&self, skill: fuigo_tools::implementations::skills::types::SkillInfo) {
        ...
        fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::SkillDispatched {
            ...
            trigger: fuigo_telemetry::events::SkillTrigger::SkillMdRead,
        });
    }
    ...
    ) -> fuigo_hooks::event::HookEventEnvelope {
        let (tool_input, tool_input_truncated) =
            fuigo_hooks::event::truncate_payload(input.clone());
        self.make_hook_envelope(
            fuigo_hooks::event::HookEventName::PreToolUse,
            None,
            fuigo_hooks::event::HookPayload::PreToolUse {
    ...
    async fn handle_tool_parse_error(
        ...
        let title = (err.kind == fuigo_tool_runtime::ToolErrorKind::NotFound)
    ...
    fn record_git_pr_signals(&self, effective_tool_name: &str, result: &ToolRunResult) {
        use fuigo_telemetry::enums::PrCreationSource;
        use fuigo_tools::util::git_detect;
        match &result.output {
            fuigo_tools::types::output::ToolOutput::Bash(b) if b.exit_code == 0 => {
                ...
                if ops.pr_merged {
                    ...
                    fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::PrMerged {});
                }
            }
            fuigo_tools::types::output::ToolOutput::MCP(m)
    ...
    fn record_pr_created(
        &self,
        pr: fuigo_tools::util::git_detect::PrRef,
        source: fuigo_telemetry::enums::PrCreationSource,
    ) {
        self.signals_handle()
    ...
    pub(super) async fn handle_bridge_tool_success(
        ...
        if matches!(
            ...
            ToolsToolOutput::SearchReplace(
                fuigo_tools::types::output::SearchReplaceOutput::EditsApplied(_)
            ) | ToolsToolOutput::Bash(_)
        ) {
            self.maybe_notify_git_branch().await;
        }
        if let fuigo_tools::types::output::ToolOutput::Bash(ref b) = result.output
            && b.was_bare_echo
        {
            ...
            acp_tool_update(&result.output, call_id, path_rewriter.as_ref(), tool_meta)
        {
            ...
            if matches!(
                &result.output,
                fuigo_tools::types::output::ToolOutput::EnterPlanMode(_)
                    | fuigo_tools::types::output::ToolOutput::ExitPlanMode(_)
            ) {
        ...
        if !extracted_images.is_empty() {
            ...
            if let Some((notice, notes)) = crate::session::image_normalize::dropped_to_envelope(
                ...
                self.send_fuigo_notification(FuigoSessionUpdate::ImageDropped { notes })
                    .await;
            }
    ...
    async fn render_non_replaced_tool_body(
        ...
        tool_layer_images: Vec<fuigo_tools::util::base64_images::ExtractedImage>,
    ) -> (
        String,
        Vec<ContentPart>,
        Vec<fuigo_tools::util::base64_images::ExtractedImage>,
    ) {
        ...
        let extraction = if !self.is_cursor_harness()
            ...
            fuigo_tools::util::base64_images::extract_base64_images(prompt_text)
        } else {
            fuigo_tools::util::base64_images::ExtractionResult {
                text: prompt_text,
                images: Vec::new(),
            }
        };
...
fn execute_tool_call_parts(
    ...
    let display = fuigo_tools::util::strip_redundant_session_cd(command, cwd);
    (
...
mod mcp_error_routing_tests {
    ...
    use fuigo_tools::types::output::{MCPOutput, TodoWriteOutput, ToolOutput};
    #[test]
...
mod exit_plan_tail_predicate_tests {
    ...
    fn exit_plan_kind_is_file_backed_exit() {
        ...
        assert!(is_file_backed_exit_plan_input(&ToolInput::ExitPlanMode(
            fuigo_tools::implementations::fuigo_build::exit_plan_mode::ExitPlanModeInput {}
        )));
    }
...
mod plan_mode_edit_gate_tests {
    ...
    use fuigo_tools::types::ToolInput;
    use fuigo_workspace::permission::AccessKind;
    ...
    /// Fuigo edit tools are plan-file-only while plan mode is active: the enforcement that makes plan mode read-only even under always-approve.
    #[test]
    fn fuigo_edits_outside_plan_file_rejected() {
        ...
        assert_eq!(
            ...
            PlanEditGate::RejectNonPlanFile,
            "fuigo tools get no markdown exception — plan file only"
        );
    }
    ...
    fn non_edit_tools_not_gated() {
        use fuigo_tools::implementations::BashToolInput;
        let t = active_tracker();
        assert_eq!(
            gate(
                &t,
...
mod plan_approval_helper_tests {
    ...
    use fuigo_tools::implementations::fuigo_build::exit_plan_mode::ExitPlanModeExtResponse;
    ...
    fn ext_method_no_client_defaults_false_for_untagged_error() {
        assert!(!ext_method_no_client(&fuigo_acp_lib::acp_internal_error(
...
mod wait_interrupt_tests {
    ...
    async fn pending_interjection_aborts_in_flight_wait() {
        ...
        use fuigo_interjection_core::PendingInterjection;
    ...
    #[test]
    fn interruptible_wait_tool_unless_timeout_is_explicit_zero() {
