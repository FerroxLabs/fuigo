...
fn partition_rules_by_scope(
    files: Vec<fuigo_agent::prompt::agents_md::AgentConfigFile>,
    fuigo_home: &std::path::Path,
    vendor_homes: &[(std::path::PathBuf, bool)],
    workspace_roots: &[&std::path::Path],
) -> (
    Vec<fuigo_agent::prompt::user_message::RuleEntry>,
    Vec<fuigo_agent::prompt::user_message::RuleEntry>,
) {
    ...
    for file in files {
        ...
        let entry = fuigo_agent::prompt::user_message::RuleEntry::from(file);
        if is_user_rule {
            user.push(entry);
        } else {
...
mod partition_rules_by_scope_tests {
    ...
    use std::path::Path;
    use fuigo_agent::prompt::agents_md::AgentConfigFile;
    ...
    fn paths(entries: &[fuigo_agent::prompt::user_message::RuleEntry]) -> Vec<&str> {
        entries.iter().map(|entry| entry.content.as_str()).collect()
    }
    #[test]
    fn partitions_custom_fuigo_and_vendor_home_rules_as_user_scope() {
        let files = vec![
    ...
    #[test]
    fn fuigo_home_nested_in_workspace_keeps_direct_surfaces_user_scoped() {
        let files = vec![
            file("/repo/config/AGENTS.md"),
            file("/repo/config/rules/global.md"),
            file("/repo/config/.fuigo/rules/project.md"),
    ...
    #[test]
    fn nested_fuigo_home_workspace_files_stay_workspace_scoped() {
        let files = vec![
            file("/custom/fuigo/rules/global.md"),
            file("/custom/fuigo/worktrees/repo/.cursor/rules/project.md"),
            file("/custom/fuigo/worktrees/repo/src/AGENTS.md"),
        ];
        let (workspace, user) = partition_rules_by_scope(
            files,
            Path::new("/custom/fuigo"),
            &[],
            &[Path::new("/custom/fuigo/worktrees/repo")],
        );
        assert_eq!(paths(&user), vec!["/custom/fuigo/rules/global.md"]);
        assert_eq!(
            paths(&workspace),
            vec![
    ...
    fn partitioned_snapshot_bodies_match_rules_and_old_reminder() {
        let files = vec![
            ...
            AgentConfigFile {
                file_name: "AGENTS.md".into(),
                file_path: "/home/user/.fuigo/AGENTS.md".into(),
                content: "home-fuigo-body".into(),
            },
            AgentConfigFile {
                file_name: "CLAUDE.md".into(),
            ...
            AgentConfigFile {
                file_name: "x.md".into(),
                file_path: "/repo/.fuigo/rules/x.md".into(),
                content: "repo-fuigo-rules-x".into(),
            },
        ];
        let vendor_homes = vec![(Path::new("/home/user/.claude").to_path_buf(), true)];
        let (workspace, user) = partition_rules_by_scope(
            files.clone(),
            Path::new("/home/user/.fuigo"),
            &vendor_homes,
            &[Path::new("/repo")],
        );
        let rules =
            fuigo_agent::prompt::user_message::format_rules_section(&workspace, &user).unwrap();
        let reminder = fuigo_agent::prompt::agents_md::format_agents_md_section(&files).unwrap();
        for body in [
            "repo-agents-body",
            "repo-claude-body",
            "home-fuigo-body",
        ...
        assert!(rules.contains("name=\"/repo/.fuigo/rules/x.md\""));
    ...
    fn fork_ondisk_and_display_prefixes_both_count_as_workspace() {
        let files = vec![
            file("/home/user/.fuigo/worktrees/repo/AGENTS.md"),
            file("/home/user/repo/crates/foo/AGENTS.md"),
            file("/home/user/.fuigo/AGENTS.md"),
        ];
        let (workspace, user) = partition_rules_by_scope(
            files,
            Path::new("/home/user/.fuigo"),
            &[],
            &[
                Path::new("/home/user/.fuigo/worktrees/repo"),
        ...
        assert_eq!(
            paths(&workspace),
            vec![
                "/home/user/.fuigo/worktrees/repo/AGENTS.md",
                "/home/user/repo/crates/foo/AGENTS.md",
            ]
        );
        assert_eq!(paths(&user), vec!["/home/user/.fuigo/AGENTS.md"]);
    }
}
...
impl SessionActor {
                ...
                !matches!(
                    ...
                    ConversationItem::User(u)
                        if u.synthetic_reason
                            == Some(fuigo_sampling_types::SyntheticReason::SystemReminder)
                )
            });
        }
    }
    ...
    pub(super) async fn build_user_message_prefix(&self) -> String {
        ...
        if matches!(template, UserMessageTemplate::Default) && include_verification {
            ...
            user_rules.splice(
                0..0,
                fuigo_agent::prompt::browser_verification::synthetic_user_rules(),
            );
            fuigo_agent::prompt::user_message::append_rules_section(
                &mut out,
                &workspace_rules,
                &user_rules,
            );
    ...
    fn gather_partitioned_rules(
        ...
        let workspace_roots: Vec<&std::path::Path> = display_root
            ...
            .collect();
        partition_rules_by_scope(files, &fuigo_home, &vendor_homes, &workspace_roots)
    }
    ...
    async fn build_templated_user_message(
        ...
        use fuigo_agent::prompt::user_message::UserMessageContext;
    ...
    ) -> Vec<fuigo_agent::prompt::user_message::McpServerEntry> {
        use fuigo_agent::prompt::user_message::McpServerEntry;
