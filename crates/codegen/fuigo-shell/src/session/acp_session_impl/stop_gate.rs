...
use fuigo_hooks::{dispatcher, result};
...
fn stop_entry_from_task(task: &fuigo_tools::types::TaskSnapshot) -> StopBackgroundTask {
    let command_text =
...
fn stop_entry_from_subagent(
    summary: &fuigo_tools::implementations::fuigo_build::task::types::ActiveSubagentSummary,
) -> StopBackgroundTask {
    StopBackgroundTask {
...
fn stop_cron_from_scheduled(
    task: &fuigo_tools::implementations::fuigo_build::scheduler::types::ScheduledTask,
) -> StopSessionCron {
    StopSessionCron {
        id: task.id.clone(),
        schedule: fuigo_tools::implementations::fuigo_build::scheduler::interval::interval_to_human(
            task.interval_secs,
        ),
...
impl SessionActor {
    ...
    pub(crate) async fn list_active_subagents(
        &self,
    ) -> Vec<fuigo_tools::implementations::fuigo_build::task::types::ActiveSubagentSummary> {
    ...
    pub(super) async fn run_stop_gate(
        ...
        // Client hooks get the awaited `fuigo/hooks/run` request below, not a fire-and-forget event
    ...
    async fn announce_keep_working(
        ...
        for block in blocks {
            ...
            fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::HookBlocked {
                ...
                cause: fuigo_telemetry::events::HookBlockCause::StopBlocked,
            });
        }
        if blocks.is_empty() {
            for context in additional_context {
...
mod stop_gate_snapshot_tests {
    ...
    fn task_snapshot(
        kind: fuigo_tools::computer::types::TaskKind,
    ) -> fuigo_tools::types::TaskSnapshot {
        fuigo_tools::types::TaskSnapshot {
            task_id: "task-1".into(),
    ...
    fn task_snapshot_maps_to_stop_entry() {
        let shell =
            stop_entry_from_task(&task_snapshot(fuigo_tools::computer::types::TaskKind::Bash));
    ...
    fn subagent_summary_maps_to_stop_entry() {
        let summary =
            fuigo_tools::implementations::fuigo_build::task::types::ActiveSubagentSummary {
    ...
    fn scheduled_task_maps_to_stop_cron() {
        let task = fuigo_tools::implementations::fuigo_build::scheduler::types::ScheduledTask::new(
            300,
