...
impl SessionActor {
    ...
    pub(super) async fn inject_baseline_skill_reminder(
    ...
    ) -> Option<fuigo_tools::types::skill_discovery_tracker::SkillUpdateEffects> {
        let bridge = self.agent.borrow().tool_bridge().clone();
        ...
        let skill_text = effects
            ...
            .and_then(|update| {
                if is_cursor
                    && update.kind
                        == fuigo_tools::types::skill_discovery_tracker::SkillUpdateKind::BaselineChange
                {
                    None
                } else {
    ...
    pub(crate) async fn slash_skills_for_resolve(
        &self,
    ) -> Vec<fuigo_tools::implementations::skills::types::SkillInfo> {
        #[cfg(test)]
    ...
    pub(super) fn wrap_skill_reminder(
        &self,
        effects: &fuigo_tools::types::skill_discovery_tracker::SkillUpdateEffects,
    ) -> Option<ConversationItem> {
        use fuigo_tools::types::skill_discovery_tracker::SkillUpdateKind;
        let is_cursor = self.is_cursor_harness();
    ...
    pub(super) async fn maybe_refresh_model_metadata_on_resume(&self) {
        ...
        let (response, stamp) =
            match fuigo_auth::execute_with_stamp(&middleware_client, built).await {
                Ok(r) => r,
                Err(e) => {
    ...
    pub(super) async fn handle_model_metadata_update(
        ...
        let updated_config = fuigo_sampling_types::SamplingConfig {
    ...
    pub(super) async fn build_session_info(&self) -> SessionInfoData {
        ...
        let system_prompt_tokens = system_message
            .as_ref()
            .map(fuigo_chat_state::estimate_system_message_tokens)
            .unwrap_or(0);
        ...
        let tool_definitions_tokens = fuigo_chat_state::estimate_tool_definitions_tokens(&tool_defs);
        ...
        let free_tokens = fuigo_token_estimation::free_tokens(context_window, total_tokens);
        let usage_pct = fuigo_token_estimation::usage_percentage_u8(total_tokens, context_window);
