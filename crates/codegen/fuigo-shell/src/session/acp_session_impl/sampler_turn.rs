...
pub(super) fn transient_retry_eligible(error: &fuigo_sampler::SamplingErrorInfo) -> bool {
    use fuigo_sampler::SamplingErrorKind;
    if error.should_retry == Some(false)
        || fuigo_sampling_types::is_context_length_error(&error.message)
        ...
        || matches!(
            error.error_code,
            Some(fuigo_sampling_types::ApiErrorCode::InvalidImage)
        )
    {
        return false;
    }
        ...
        SamplingErrorKind::Api => error.status_code.is_some_and(|status| {
            status != 429
                && reqwest::StatusCode::from_u16(status)
                    .is_ok_and(fuigo_sampling_types::is_retryable_api_status)
        }),
...
/// Matches strictly on HTTP 401 when the error carries a structured status code, mirroring `SamplingError::is_auth_error` in fuigo-sampling-types.
...
pub(super) fn is_auth_tool_error(err: &fuigo_tool_runtime::ToolError) -> bool {
...
pub(super) async fn call_with_auth_retry<F, Fut>(
    ...
    tool_name: &str,
    mut call: F,
) -> Result<fuigo_tools::types::output::ToolRunResult, fuigo_tool_runtime::ToolError>
where
    F: FnMut() -> Fut,
    Fut: std::future::Future<
            Output = Result<
                fuigo_tools::types::output::ToolRunResult,
...
fn error_after_stream_drain(
    ...
    original: fuigo_sampler::SamplingErrorInfo,
...
impl SessionActor {
    ...
    fn resolve_hosted(
        ...
        Vec<fuigo_sampling_types::HostedTool>,
        fuigo_sampling_types::ToolOverrides,
    ) {
        let mut tools = self.agent.borrow().hosted_tools().to_vec();
        let applied = fuigo_sampling_types::apply_tool_overrides(
            &mut tools,
    ...
    pub(crate) fn effective_hosted_tools(&self) -> Vec<fuigo_sampling_types::HostedTool> {
        self.resolve_hosted().0
    }

    pub(crate) fn hosted_tools_for_turn(&self) -> Vec<fuigo_sampling_types::HostedTool> {
    ...
    pub(crate) fn set_tool_overrides(&self, overrides: fuigo_sampling_types::ToolOverrides) {
    ...
    pub(crate) fn apply_tool_overrides_update(
        &self,
        update: Option<fuigo_sampling_types::ToolOverridesUpdate>,
    ) {
    ...
    pub(super) async fn reconstruct_full_config(&self) -> SamplingConfig {
        ...
        impl fuigo_sampler::HeaderInjector for TraceContextInjector {
            fn inject(&self, headers: &mut reqwest::header::HeaderMap) {
                if let Some(tp) = fuigo_file_utils::trace_context::current_traceparent()
        ...
        let cfg = self
            ...
            .get_sampling_config()
            .await
            .unwrap_or_else(|| fuigo_sampling_types::SamplingConfig {
                base_url: String::new(),
                model: String::new(),
    ...
    pub(crate) async fn wire_permission_auto_llm_classifier(self: &Arc<Self>) {
        ...
        let (tx, mut rx) = tokio::sync::mpsc::unbounded_channel::<(
            Vec<fuigo_workspace::permission::ClassifierMessage>,
            tokio::sync::oneshot::Sender<
                Result<String, fuigo_workspace::permission::ClassifierFailure>,
            >,
        )>();
                ...
                let result = async {
                    let (sampling_client, model, context_window) = match &aux_classifier_sampler {
                        ...
                        None => {
                            ...
                            let client = fuigo_sampler::SamplingClient::new(config).map_err(|e| {
                    ...
                    let items = messages
                        .into_iter()
                        .map(|m| match m.role {
                            fuigo_workspace::permission::ClassifierMessageRole::System => {
                                ConversationItem::system(m.text)
                            }
                            fuigo_workspace::permission::ClassifierMessageRole::User => {
                                ConversationItem::user(m.text)
                            }
                        })
                        .collect::<Vec<_>>();
                    let input_tokens = fuigo_chat_state::estimate_conversation_tokens(&items);
                    ...
                    let request = ConversationRequest {
                        ...
                        json_schema: Some(
                            fuigo_workspace::permission::classifier_output_json_schema(),
                        ),
                        ...
                        x_fuigo_conv_id: Some(format!("perm-classifier-{}", uuid::Uuid::new_v4())),
                        x_fuigo_req_id: Some(format!("fuigo-perm-auto-{}", uuid::Uuid::new_v4())),
                        x_fuigo_session_id: Some(session_id),
                        x_fuigo_agent_id: Some(fuigo_telemetry::id::agent_id()),
                        ..ConversationRequest::default()
                    };
                    let fut = sampling_client.conversation_collect(request);
                    let response = tokio::time::timeout(classify_timeout, fut)
                        .await
                        .map_err(|_| fuigo_workspace::permission::ClassifierFailure::Timeout)?
                        .map_err(|e| {
                            fuigo_workspace::permission::ClassifierFailure::TransportError(
                                e.to_string(),
                            )
        ...
        let clf =
            fuigo_workspace::permission::LlmPermissionClassifier::with_channel(tx, prompt_type);
        debug_assert!(
            clf.has_side_query(),
    ...
    /// The routing is Tier-1 catalog creds / Tier-2 Ferrox Labs-proxy via session token / `FUIGO_API_KEY` / deployment key.
    ...
    async fn resolve_auto_classifier_sampler(
        ...
        let client = fuigo_sampler::SamplingClient::new(cfg)
            .map_err(
    ...
    pub(super) async fn prepare_chat_completion(
        &self,
        force_http1: bool,
    ) -> Result<fuigo_sampler::SamplingClient, acp::Error> {
        ...
        let sampling_client =
            fuigo_sampler::SamplingClient::new(full_config).map_err(|e| self.to_acp_error(e))?;
    ...
    pub(crate) async fn fail_turn_length_salvage_exhausted(&self) -> acp::Error {
        let kind = fuigo_sampler::SamplingErrorKind::MaxTokensTruncation;
        let message = fuigo_sampling_types::SamplingError::MaxTokensTruncation.to_string();
    ...
    pub(crate) async fn handle_sampling_failure(
        ...
        let client_version = fuigo_version::VERSION;
        ...
        if auth_mode == crate::auth::AuthMode::WebLogin {
            let msg = format!(
                "{detailed_message}\n\n\
                 ...
                 To fix: run `fuigo update`, then `fuigo logout`, then `fuigo login` to re-authenticate with OAuth2.\n\n\
        ...
        let error_type = if error
            .error_code
            .as_ref()
            .is_some_and(fuigo_sampling_types::ApiErrorCode::is_size_overflow)
            || fuigo_sampling_types::is_context_length_error(&error.message)
        {
    ...
    async fn wait_for_stream_drain(
        &self,
        request_id: &fuigo_sampler::RequestId,
    ...
    async fn submit_turn_request(
        ...
        match collected.result {
            ...
            Err(rich_err) => {
                ...
                let original = fuigo_sampler::SamplingErrorInfo::from(&rich_err);
    ...
    async fn recover_from_sampling_failure(
        self: &Arc<Self>,
        info: fuigo_sampler::SamplingErrorInfo,
...
fn resolve_configured_cutoff(
    seed: Option<fuigo_sampling_types::ToolOverrides>,
    base: Option<&fuigo_sampling_types::ToolOverrides>,
) -> fuigo_sampling_types::ToolOverrides {
    use fuigo_sampling_types::{ToolOverrides, WebSearchOptions, XSearchOptions};
    let ToolOverrides {
        x_search: seed_x,
        web_search: seed_w,
...
mod stream_drain_tests {
    ...
    fn failed_drain_revocation_supersedes_original_error() {
        let original = fuigo_sampler::SamplingErrorInfo {
            kind: fuigo_sampler::SamplingErrorKind::RateLimited,
