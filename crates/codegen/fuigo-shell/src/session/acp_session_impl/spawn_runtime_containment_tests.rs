...
const CHILD_ENV: &str = "FUIGO_SHELL_RUNTIME_CONTAINMENT_CHILD";
const BLOCKING_POOL_CHILD_ENV: &str = "FUIGO_SHELL_BLOCKING_POOL_CONTAINMENT_CHILD";
...
fn thread_count() -> Option<u64> {
    fuigo_tty_utils::sample_process_resources().threads
}

/// Child: the session runtime must not pre-warm a 16-wide blocking pool (the cap proof lives in `fuigo-tty-utils`).
