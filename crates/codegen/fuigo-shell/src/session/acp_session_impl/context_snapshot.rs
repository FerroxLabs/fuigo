...
//!
//! Token counts come from `POST {fuigo_api_base_url}/tokenize-text` (the model's tokenizer), not the bytes/4 `/context` heuristic.
...
impl SessionActor {
    ...
    pub(super) async fn emit_session_context_snapshot(&self) {
        ...
        fuigo_telemetry::session_ctx::log_session_event(session_context_snapshot(
...
fn fuigo_api_base_url() -> String {
    crate::config::load_effective_config()
        .ok()
        .and_then(|effective| crate::agent::config::Config::new_from_toml_cfg(&effective).ok())
        .map(|cfg| cfg.endpoints.fuigo_api_base_url)
        .filter(|url| !url.is_empty())
        .unwrap_or_else(|| crate::agent::config::EndpointsConfig::default().fuigo_api_base_url)
}
...
mod tests {
    ...
    fn info() -> SessionInfoData {
        SessionInfoData {
            agent_name: Some("fuigo-build".into()),
