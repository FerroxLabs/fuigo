...
impl SessionActor {
    ...
    /// Emit per-server `fuigo/mcp/tools_changed` notifications.
    ///
    ...
    /// Handle explicit auth trigger from the client (fuigo/mcp/auth_trigger).
    ///
    ...
    /// Attempt to respawn MCP servers whose last spawn failed as unreachable ([`fuigo_mcp::servers::McpError::Unreachable`]).
    ...
    ///
    /// Runs on MCP-surface tool batches, `fuigo/mcp/list` refreshes, and the explicit retry command.
        ...
        for result in results {
            match result {
                Ok(client) => {
                    ...
                    let _ = arc
                        .arm_liveness_watcher(fuigo_mcp::liveness::DEFAULT_POLL_INTERVAL)
                        .await;
                    let mut mcp_state = self.mcp_state.lock().await;
    ...
    async fn settle_failed_unreachable_attempt(
        ...
        let detail =
            || fuigo_tools::util::truncate_str_with_marker(&error.to_string(), 200).into_owned();
        let mut state = self.mcp_state.lock().await;
    ...
    pub(super) async fn maybe_inject_mcp_reminder(&self) {
        ...
        use fuigo_tools::implementations::search_tool::fingerprint_servers;
    ...
    fn latch_and_push_mcp_reminder(
        &self,
        server_summaries: &[fuigo_tools::types::tool_index::ServerSummary],
        new_fingerprints: std::collections::HashMap<
            String,
            fuigo_tools::implementations::search_tool::ServerFingerprint,
        >,
    ...
    /// or removed it from `~/.fuigo/config.toml` while we were
    /// sleeping.
    ///
    ...
    /// which parses `~/.fuigo/config.toml` + the project
    /// `.fuigo/config.toml`) on every call.
    ...
    /// The dispatcher's `notify_tx` is wired to its `FuigoClientHandler`, and the liveness watcher is running.
    ...
    ///
    /// The `FuigoClientHandler` constructed inside `try_handshake` holds the shared `Arc<Mutex<Option<Sender>>>` slot (`SharedEventTx`).
    ...
    ///
    /// **Contract:** the [`fuigo_mcp::servers`] test `client_handler_observes_post_handshake_set_event_tx` covers this.
    ...
    pub(crate) async fn respawn_stdio(&self, server: &str) -> Result<(), String> {
        ...
        if let Some(tx) = event_tx {
            ...
            let _ = tx.send(fuigo_mcp::servers::McpClientEvent::ToolsChanged {
        ...
        let mcp_clients: Vec<_> = mcp_results
            ...
            .filter_map(|result| match result {
                ...
                Err(e) => {
                    ...
                    self.events
                        .emit(fuigo_session_events::Event::McpServerFailed {
                            server_name: sname,
            ...
            async move {
                ...
                for client in mcp_clients.iter() {
                    ...
                    futs.push(async move {
                        ...
                        ew.emit(fuigo_session_events::Event::McpServerStarting {
...

                {
                    ...
                    if mcp_state.generation() != generation {
                        ...
                        event_writer.emit(fuigo_session_events::Event::McpInitCancelled {
                            reason: MCP_INIT_CANCELLED_CONFIG_CHANGED.to_string(),
                        });
                        return;
                    }
                        ...
                        match result {
                            Ok((server_name, registrations, elapsed, timeout_sec)) => {
                                ...
                                for reg in registrations {
                                    ...
                                    if reg.model_visible {
                                            ...
                                            .await
                                        {
                                            ...
                                            event_writer.emit(
                                            fuigo_session_events::Event::McpToolRegistrationFailed {
                                                server_name: server_name.clone(),
                                ...
                                let transport_enum = match server_transport_map
                                    ...
                                    .unwrap_or("unknown")
                                {
                                    "stdio" => fuigo_telemetry::events::McpTransport::Stdio,
                                    "sse" => fuigo_telemetry::events::McpTransport::Sse,
                                    _ => fuigo_telemetry::events::McpTransport::Http,
                                };
                                debug_assert!(
                                    fuigo_telemetry::activity::MCP_SERVERS_CONNECTED.get() >= 1,
                                    "McpServerConnected must stamp a self-inclusive count"
                                );
                                fuigo_telemetry::session_ctx::log_event(
                                    fuigo_telemetry::events::McpServerConnected {
                                        server_name: server_name.clone(),
                                        tool_count,
                                ...
                                let transport_str = server_transport_map
                                    ...
                                    .unwrap_or("unknown");
                                event_writer.emit(
                                    fuigo_session_events::Event::McpServerConnected {
                                        server_name: server_name.clone(),
                            ...
                            Err((server_name, ref e, needs_auth, elapsed, timeout_sec)) => {
                                let error_cat = if needs_auth {
                                    fuigo_session_events::McpErrorCategory::AuthRequired
                                } else {
                                    e.error_category()
                                };
                                let error_type_label = match error_cat {
                                    fuigo_session_events::McpErrorCategory::AuthRequired => {
                                        fuigo_telemetry::events::McpErrorType::Auth
                                    }
                                    fuigo_session_events::McpErrorCategory::Timeout => {
                                        fuigo_telemetry::events::McpErrorType::Timeout
                                    }
                                    _ => fuigo_telemetry::events::McpErrorType::HandshakeFailed,
                                };
                                fuigo_telemetry::session_ctx::log_event(
                                ...
                                event_writer.emit(fuigo_session_events::Event::McpServerFailed {
                                    server_name: server_name.clone(),
                                ...
                                let detail = (!needs_auth).then(|| {
                                    fuigo_tools::util::truncate_str_with_marker(&e.to_string(), 200)
                                        .into_owned()
                                });
                    ...
                    fuigo_telemetry::session_ctx::log_event(
                        fuigo_telemetry::events::McpInitCompleted {
                    ...
                    event_writer.emit(fuigo_session_events::Event::McpInitCompleted {
    ...
    pub(crate) fn connected_server_summaries(
        &self,
    ) -> Vec<fuigo_tools::types::tool_index::ServerSummary> {
        use fuigo_tools::types::tool_index::ToolSearchIndex;
    ...
    pub(super) async fn mcp_announcement_snapshot(&self) -> Option<McpAnnouncementSnapshot> {
        let server_summaries = self.connected_server_summaries();
        let mut text =
            fuigo_tools::implementations::search_tool::build_server_reminder(&server_summaries)?;
