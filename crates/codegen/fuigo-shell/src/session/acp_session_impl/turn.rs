...
impl UsageDrainOutcome {
    ...
    pub(super) fn from_outstanding_reply(
        reply: Option<
            &fuigo_tools::implementations::fuigo_build::task::types::SubagentOutstandingReply,
        >,
    ) -> Self {
        match reply {
            None => Self {
...
pub(super) fn record_failed_sample_on_turn_span(
    span: &tracing::Span,
    kind: fuigo_sampler::SamplingErrorKind,
) {
    let stop_reason = match kind {
        fuigo_sampler::SamplingErrorKind::MaxTokensTruncation => {
            fuigo_sampling_types::StopReason::Length.as_str()
        }
...
impl SessionActor {
    ...
    pub(super) async fn notify_turn_abort(
        &self,
        epoch: TurnEpoch,
        reason: fuigo_agent_lifecycle::TurnAbortReason,
    ) {
        if !self.turn_abort.try_mark_announced(epoch) {
            return;
        }
        let input = fuigo_agent_lifecycle::TurnAbortInput::new(reason);
    ...
    pub(crate) async fn normalize_images_with_notices(
        ...
        if !norm_result.compressed.is_empty() {
            ...
            let images = norm_result
                ...
                .collect();
            self.send_fuigo_notification(FuigoSessionUpdate::ImageCompressed { images, message })
                .await;
        }
        if !norm_result.re_encode_fallbacks.is_empty() {
            ...
            self.send_fuigo_notification(FuigoSessionUpdate::ImageCompressed {
                images: vec![],
    ...
    pub(super) async fn handle_turn_input(
        ...
        if let Some(ref tp) = request.traceparent {
            fuigo_file_utils::trace_context::link_span_to_meta(
                &span,
    ...
    async fn handle_turn_input_inner(
        ...
        let _active = fuigo_telemetry::activity::TURNS_ACTIVE.enter();
        ...
        *self.turn_prompt_mode.lock() = prompt_mode;
        ...
        let turn_start_input =
            fuigo_agent_lifecycle::TurnStartInput::new(input_origin.is_synthetic());
        for contributor in self.extension_registry.turn_lifecycle_contributors() {
            contributor
        ...
        let prompt_blocks = match resolved {
            ...
            Err(SlashCommandOutcome::Builtin(action)) => {
                ...
                let slash_used = fuigo_telemetry::events::SlashCommandUsed {
                ...
                match action {
                    BuiltinAction::GoalSet {
                        ...
                        fuigo_telemetry::session_ctx::log_event(slash_used);
            ...
            Err(SlashCommandOutcome::InvokeSkill {
                ...
                for sk in &parsed_skills {
                    ...
                    if let Some(ref pname) = sk.plugin_name {
                        fuigo_telemetry::session_ctx::log_event(
                            fuigo_telemetry::events::PluginUsed {
        ...
        *self.doom_loop_turn_tally.lock() = Default::default();
        ...
        self.observability_bridge
            .emit(
                fuigo_tool_protocol::session_event::SessionEvent::TurnStarted {
                    turn_number,
                    model_id: model_id.clone(),
                    yolo_mode,
                },
            ...
            .await;
        self.send_before_turn_event(fuigo_tool_protocol::turn_hook::BeforeTurnPayload {
            turn_number: self.chat_state_handle.get_prompt_index().await as u64,
            model_id: model_id.clone(),
            yolo_mode: self.permissions.is_yolo_mode(),
            conversation_message_count: msg_count,
            session_relationship: fuigo_tool_protocol::turn_hook::DEFAULT_SESSION_RELATIONSHIP
                .to_string(),
        ...
        fuigo_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::Turn {
        ...
        let prompt_block = if policy.authority != InputAuthority::ModelAuthoredUntrusted {
            let prompt_gate_verdict = self
                .dispatch_prompt_submit_hook(
                    fuigo_hooks::event::HookPayload::UserPromptSubmit {
            ...
            match (
                ...
                (true, fuigo_hooks::result::PromptDecision::Block { reason, hook_name }) => {
                    Some((hook_name, reason))
                }
                (false, fuigo_hooks::result::PromptDecision::Block { reason, hook_name }) => {
                    ...
                    self.send_hook_annotation(
                        ...
                        fuigo_hooks::config::hook_display_name(&hook_name)
                    ),
                        )
                        .await;
                    None
                }
                (_, fuigo_hooks::result::PromptDecision::Allow) => None,
            }
        } else {
            None
        };
        if prompt_block.is_some() {
            ...
            *self.tool_context.prompt_index.lock().await = current_prompt_index;
            ...
            let query = if send_now && !verbatim {
                fuigo_interjection_core::frame_user_turn(
                    fuigo_interjection_core::INTERJECTION_NOTE,
                    &query,
                )
            } else {
                query
            ...
            let (query, extra_images) = if !self.is_cursor_harness() {
                let extraction = fuigo_tools::util::base64_images::extract_base64_images(query);
            ...
            if policy.analytics.is_human_prompt()
                && (self.telemetry_enabled || fuigo_telemetry::external::is_active())
            {
                let effective_client_identifier =
                    prompt_client_identifier.or_else(|| self.client_identifier.clone());
                let ev = fuigo_telemetry::events::PromptSubmitted {
            ...
            self.tool_bridge_handle()
                .update_resource(fuigo_tools::types::resources::AttachedImages(
                    attached_image_refs,
                ))
                .await;
        ...
        let mut result = if let Some((hook_name, reason)) = prompt_block {
            ...
            fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::HookBlocked {
                hook_name: hook_name.clone(),
                cause: fuigo_telemetry::events::HookBlockCause::PromptBlocked,
            });
        ...
        self.observability_bridge
            .emit(
                fuigo_tool_protocol::session_event::SessionEvent::TurnEnded {
        ...
        match &result {
            Ok(TurnOutcome::Completed { stop, .. }) => {
                ...
                self.send_after_turn_event(fuigo_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: fuigo_tool_protocol::turn_hook::TurnHookOutcome::Completed,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                ...
                .await;
                fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::TurnCompleted {
                    outcome: fuigo_telemetry::events::Outcome::Completed,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
            ...
            Ok(TurnOutcome::Cancelled { category, context }) => {
                ...
                self.send_after_turn_event(fuigo_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: fuigo_tool_protocol::turn_hook::TurnHookOutcome::Cancelled,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
                ...
                .await;
                fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::TurnCompleted {
                    outcome: fuigo_telemetry::events::Outcome::Cancelled,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id,
            ...
            Err(err) => {
                ...
                self.send_after_turn_event(fuigo_tool_protocol::turn_hook::AfterTurnPayload {
                    turn_number: current_prompt_index as u64,
                    outcome: fuigo_tool_protocol::turn_hook::TurnHookOutcome::Error,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id.clone(),
                    written_repo_paths: Vec::new(),
                ...
                fuigo_telemetry::session_ctx::log_session_event(
                    fuigo_telemetry::events::ApiError {
                ...
                fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::TurnCompleted {
                    outcome: fuigo_telemetry::events::Outcome::Error,
                    duration_ms: turn_duration_ms,
                    tool_call_count: turn_tool_count,
                    model_id: turn_model_id,
                    cancellation_category: None,
        ...
        match &result {
            Ok(TurnOutcome::Completed { .. }) | Ok(TurnOutcome::StationarityEnded { .. }) => {
                for contributor in self.extension_registry.turn_lifecycle_contributors() {
                    contributor
                        .on_turn_done(&fuigo_agent_lifecycle::TurnDoneInput)
                        .await;
                }
            }
            Ok(TurnOutcome::Cancelled { .. }) | Ok(TurnOutcome::MaxTurnsReached { .. }) => {
                self.notify_turn_abort(
                    ...
                    fuigo_agent_lifecycle::TurnAbortReason::Interrupted,
                )
                .await;
            }
            Err(err) => {
    ...
    pub(crate) async fn inject_pending_monitor_events(&self) {
        ...
        let mine = fuigo_tools::implementations::fuigo_build::monitor::types::drain_owned(
            buffer,
            Some(self.session_info.id.0.as_ref()),
        );
        if mine.is_empty() {
        ...
        let Some(body) = fuigo_tools::reminders::task_completion::format_monitor_events(
            &mine,
            Some(&self.tool_context.task_output_tool_name),
        ) else {
            return;
        };
        let wrapped = fuigo_tools::reminders::wrap_reminder(&body);
        self.chat_state_handle
    ...
    pub(super) async fn process_conversation_turn_with_recovery(
        ...
        loop {
            ...
            if attempt > recovery.max_retries {
                ...
                self.send_fuigo_notification(FuigoSessionUpdate::AutoRecoveryExhausted {
                    attempts: attempt,
                    error: error_desc,
                })
                .await;
    ...
    pub(crate) async fn first_turn_memory_reminder(&self) -> Option<String> {
        ...
        if !self.memory.initial_injection_config.enabled {
            ...
            crate::session::memory_observation::log_memory_injection(
                ...
                fuigo_telemetry::memory_telemetry::MemoryInjectionOutcome::Skipped,
                Default::default(),
            );
            return None;
        }
        ...
        use fuigo_tools::types::memory_backend::MemoryBackend as _;
        ...
        let (outcome, mut inject_results) = match search_result {
            Ok(results) if results.is_empty() => (
                fuigo_telemetry::memory_telemetry::MemoryInjectionOutcome::Empty,
                results,
            ),
            Ok(results) => (
                fuigo_telemetry::memory_telemetry::MemoryInjectionOutcome::Results,
                results,
            ),
            Err(error) => {
                tracing::warn!(
                ...
                (
                    fuigo_telemetry::memory_telemetry::MemoryInjectionOutcome::Error,
                    Vec::new(),
                )
            }
        };
    ...
    async fn handle_structured_output_tool_call(
        &self,
        tool_calls: &mut Vec<fuigo_sampling_types::conversation::ToolCall>,
        validator: &Result<jsonschema::Validator, String>,
        retries: &mut u32,
    ...
    async fn is_run_true_step(
        &self,
        tool_calls: &[fuigo_sampling_types::conversation::ToolCall],
    ) -> bool {
    ...
    async fn finalize_turn_bookkeeping(
        ...
        if let Some(snap) = snapshot.as_mut() {
            ...
            for pr in &snap.delta.prs_created_this_turn {
                fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::PrCreated {
                    source: pr.source,
    ...
    async fn process_conversation_turn(
        ...
        loop {
            ...
            if identical_tool_calls.run_len >= identical_tool_calls.hard_stop_threshold() {
                ...
                fuigo_telemetry::session_ctx::log_event(
                    fuigo_telemetry::events::ActionStationarityStop {
                        true_noop,
            ...
            if identical_tool_calls.take_nudge() {
                ...
                fuigo_telemetry::session_ctx::log_event(
                    fuigo_telemetry::events::ActionStationarityNudge {
                        problematically_repeating,
                        run_len,
                        tool_name: tool_name.clone(),
                    },
            ...
            *self.last_sent_tool_specs.borrow_mut() = Some(effective_tools.clone());
            ...
            self.observability_bridge
                .emit(
                    fuigo_tool_protocol::session_event::SessionEvent::PhaseChanged {
                        phase: fuigo_tool_protocol::session_event::SessionPhase::Sampling,
                    },
                )
                .await;
            fuigo_telemetry::unified_log::info(
                ...
                .await
            {
                ...
                Ok(SamplerTurnOutcome::RetryTransient { kind, status_code }) => {
                    ...
                    let cause = match kind {
                        fuigo_sampler::SamplingErrorKind::IdleTimeout => "Response stalled",
                        fuigo_sampler::SamplingErrorKind::Http => "Connection problem",
                        _ => "Server error",
                    };
            ...
            let model_duration_ms = model_timer.elapsed().as_millis() as u64;
            {
                let model_id = self.current_model_id().await;
                fuigo_telemetry::session_ctx::log_event(
                    fuigo_telemetry::events::ModelResponseReceived {
                        model_id,
            ...
            if fuigo_tools::media_gen_limits::should_resample_egregious(
                ...
                let reminder = fuigo_tools::media_gen_limits::resample_reminder(&egregious);
                tracing::warn!(
                    session_id = %self.session_info.id,
            ...
            let turn_refused = stop_reason == Some(fuigo_sampling_types::StopReason::ContentFilter);
            ...
            match length_salvage_streak.on_sample(
                stop_reason == Some(fuigo_sampling_types::StopReason::Length)
                    && !tool_calls.is_empty(),
            ) {
            ...
            self.send_buffered_fuigo_update(response_completed).await;
            ...
            if stop_reason == Some(fuigo_sampling_types::StopReason::Length)
                && tool_calls.is_empty()
            ...
            if is_true_noop {
                fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::ShellTrueNoop {
            ...
            self.observability_bridge
                .emit(
                    fuigo_tool_protocol::session_event::SessionEvent::PhaseChanged {
                        phase: fuigo_tool_protocol::session_event::SessionPhase::ToolExecution,
                    },
                )
                .await;
...
fn step_signature(tool_calls: &[fuigo_sampling_types::conversation::ToolCall]) -> String {
...
mod identical_tool_call_run_tests {
    ...
    fn step_signature_ignores_key_order_and_call_order() {
        let call = |name: &str, args: &str| fuigo_sampling_types::conversation::ToolCall {
            id: "id".into(),
            name: name.to_string(),
            arguments: args.into(),
        };
...
mod last_sample_span_tests {
    ...
    use fuigo_sampling_types::conversation::{
    ...
    fn non_length_failure_and_unreported_stop_reason_overwrite_last_sample() {
        ...
        let mut totals = TurnSpanTotals::default();
        totals.record(&span, &sample(Some(StopReason::ToolCalls), true, 40));
        record_failed_sample_on_turn_span(&span, fuigo_sampler::SamplingErrorKind::Api);
        assert_eq!(
            fields
                .lock()
                .unwrap()
