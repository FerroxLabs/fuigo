...
fn build_context_window(
    ...
    let used_percentage = used_tokens
        .filter(|_| size > 0)
        .map(|used| fuigo_token_estimation::usage_percentage_u8(used, size));
    StatusLineContextWindow {
...
impl SessionActor {
    pub(super) async fn build_status_context(&self) -> StatusLineContext {
        ...
        StatusLineContext {
            ...
            version: fuigo_version::VERSION.to_string(),
            cost: StatusLineCost {
    ...
    async fn emit_status_snapshot(&self) {
        // `send_fuigo_notification_transient` checks this too; here it skips the build, which an attach re-requests once the gate is open
