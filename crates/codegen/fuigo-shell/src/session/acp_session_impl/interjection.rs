...
// Buffer, entry type, and formatting live in the shared fuigo-interjection-core crate so the server-side agent loop can adopt the same behaviour
        ...
        } in &guard.entries
        {
            ...
            let (skill_information, mut wrapped) = match authority {
                InterjectionAuthority::User => (
                    ...
                    format_interjection(sanitized.clone()),
                ),
                InterjectionAuthority::ParentAgent { .. } => (
                    self.parent_agent_skill_information(&sanitized).await,
                    format_parent_agent_interjection(sanitized.clone()),
                ),
            };
