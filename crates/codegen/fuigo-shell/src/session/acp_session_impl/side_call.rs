...
impl From<&fuigo_sampling_types::TokenUsage> for PromptCacheUsage {
    fn from(usage: &fuigo_sampling_types::TokenUsage) -> Self {
...
pub(crate) struct AuxCall {
    ...
    pub(crate) hosted_tools: Vec<fuigo_sampling_types::HostedTool>,
    pub(crate) model: String,
    /// Must match the main turn's, or the prompt differs before the conversation history even starts.
    pub(crate) reasoning_effort: Option<fuigo_sampling_types::ReasoningEffort>,
...
pub(crate) struct SideCallSetup {
    pub(crate) client: fuigo_sampler::SamplingClient,
...
impl SessionActor {
    ...
    pub(crate) fn parent_cached_request(&self, call: AuxCall) -> ConversationRequest {
        ...
        // The `btw-`/`recap-` label still shows up in `x_fuigo_req_id`
        ...
        ConversationRequest {
            ...
            x_fuigo_conv_id: Some(conv_id),
            x_fuigo_req_id: Some(call.req_id),
            ...
            length_policy: fuigo_sampling_types::LengthPolicy::Fail,
            ..Default::default()
        }
    }
