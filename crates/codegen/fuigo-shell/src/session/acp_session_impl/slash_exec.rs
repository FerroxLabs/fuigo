        ...
        match action {
            ...
            BuiltinAction::HooksTrust => {
                let msg = match Self::do_hooks_trust_project(&self.session_info.cwd) {
                    ...
                    Err(e) => {
                        fuigo_telemetry::session_ctx::log_event(
                            fuigo_telemetry::events::HookTrusted { success: false },
                        );
                        e
                    }
                };
            ...
            BuiltinAction::HooksAdd { path } => {
                if path.is_empty() {
                    self.send_host_turn_slash_command_output(
                        "Usage: /hooks add <path>\nProvide a path to a hook JSON file or directory under ~/.fuigo/.",
                    )
                    .await;
                } else {
                    ...
                    match crate::config::add_hooks_path(&path) {
                        ...
                        Err(e) => {
                            fuigo_telemetry::session_ctx::log_event(
                                fuigo_telemetry::events::HookAdded { success: false },
                            );
                            self.send_host_turn_slash_command_output(&format!(
            ...
            BuiltinAction::HooksRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_hooks_path(&path) {
                        ...
                        Ok(false) => {
                            fuigo_telemetry::session_ctx::log_event(
                                fuigo_telemetry::events::HookRemoved { success: false },
                            );
                            self.send_host_turn_slash_command_output(&format!(
            ...
            BuiltinAction::PluginsReload => {
                match &self.plugin_registry_handle {
                    Some(handle) => {
                        ...
                        fuigo_telemetry::session_ctx::log_event(
                            fuigo_telemetry::events::PluginReloaded { success: true },
                        );
                        self.send_host_turn_slash_command_output(&msg).await;
                    }
                    None => {
                        fuigo_telemetry::session_ctx::log_event(
                            fuigo_telemetry::events::PluginReloaded { success: false },
                        );
                        self.send_host_turn_slash_command_output(
            ...
            BuiltinAction::SessionInfo => {
                ...
                let context_pct = fuigo_token_estimation::usage_percentage(ctx.used, ctx.total);
                        ...
                        Ok(()) => {
                            fuigo_telemetry::session_ctx::log_event(
                                fuigo_telemetry::events::PluginAdded {
                                    source: fuigo_telemetry::events::PluginSource::LocalPath,
                                    success: true,
                                },
                            );
            ...
            BuiltinAction::PluginsRemove { path } => {
                if path.is_empty() {
                    ...
                    match crate::config::remove_plugin_path(&path_str) {
                        ...
                        Err(e) => {
                            fuigo_telemetry::session_ctx::log_event(
                                fuigo_telemetry::events::PluginRemoved { success: false },
                            );
                            self.send_host_turn_slash_command_output(&format!(
                    ...
                    if !trust {
                        ...
                        match crate::plugin::install_plugin(&source, cwd) {
                            Ok(outcome) => {
                                ...
                                fuigo_telemetry::session_ctx::log_event(
                                    fuigo_telemetry::events::PluginInstalled {
                                        install_kind: kind,
                                        success: true,
                                        trust: true,
                                        error_category: None,
            ...
            BuiltinAction::MemoryBrowse => {
                ...
                self.send_fuigo_notification(FuigoSessionUpdate::MemoryFiles { files: file_infos })
                    .await;
                ok_end_turn(0, None)
            }
                        ...
                        if let Err(e) = storage.ensure_initialized() {
                            ...
                            *self.memory.search_counter.borrow_mut() =
                            ...
                            let backend: std::sync::Arc<
                                dyn fuigo_tools::types::memory_backend::MemoryBackend,
