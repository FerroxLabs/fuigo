...
use fuigo_telemetry::session_end::{self, Phase, SharedSessionEndTimer};
...
pub(super) async fn fire_session_end_hooks(
    ...
    let envelope = session.fire_hook(
        ...
        None,
        fuigo_hooks::event::HookPayload::SessionEnd {
...
async fn log_session_ended(session: &SessionActor) {
    ...
    if let Some(signals) = session.signals_handle().snapshot().await {
        fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::SessionEnded {
...
async fn emit_session_end_timings(timer: &SharedSessionEndTimer, is_subagent: bool) {
    ...
    let mut event = fuigo_telemetry::events::SessionEndTimings::default();
    timer.write_event_phases(&mut event);
    event.total_ms = Some(timer.elapsed_ms());
    let _ = tokio::time::timeout(
        SESSION_END_EMIT_BUDGET,
        fuigo_telemetry::session_ctx::log_event_now(event),
    )
    .await;
}
pub(super) async fn run_session(
    ...
    mut chat_state_event_rx: mpsc::UnboundedReceiver<fuigo_chat_state::ChatStateEvent>,
    ...
    let _elicitation_coordinator = if !session.startup_hints.is_subagent {
        let elicit_inbox = fuigo_mcp::elicitation::ElicitationInbox::new();
        {
    ...
    if !session.startup_hints.is_subagent {
        let (event_tx, event_rx) =
            tokio::sync::mpsc::unbounded_channel::<fuigo_mcp::servers::McpClientEvent>();
        {
            let mut mcp_state = session.mcp_state.lock().await;
        ...
        tokio::select! {
                ...
                _ = &mut idle_flush_sleep, if session.idle_flush_timeout.is_some()
                    ...
                    if current_len > last_len {
                        tracing::info!(target: fuigo_telemetry::memory_log::TARGET,
                ...
                event = chat_state_event_rx.recv() => {
                    match event {
                        Some(fuigo_chat_state::ChatStateEvent::ConversationReset { new_len }) => {
                        ...
                        Some(fuigo_chat_state::ChatStateEvent::ImageBudget {
                            body_bytes,
                            trigger_bytes,
                        ...
                        Some(fuigo_chat_state::ChatStateEvent::PromptIndexChanged { .. }) |
                        Some(fuigo_chat_state::ChatStateEvent::TokensUpdated { .. }) => {
                ...
                maybe_cmd = cmd_rx.recv() => {
                    ...
                    match cmd {
                        ...
                        SessionCommand::OverrideModelName { model_name, extra_headers, context_window } => {
                            ...
                            if let Some(mut cfg) = session.chat_state_handle.get_sampling_config().await {
                                ...
                                if let Some(r) = crate::agent::config::try_resolve_model_credentials(model_name.as_str(), existing.api_key.as_deref()) {
                                    session.chat_state_handle.update_credentials(fuigo_chat_state::Credentials {
                                        api_key: r.api_key,
                        ...
                        SessionCommand::GetHooksList { respond_to } => {
                            ...
                            let _ = respond_to.send(fuigo_hooks_plugins_types::HooksListResponse {
                                hooks,
                                project_trusted,
                        ...
                        SessionCommand::NotifyPluginUpdates { updates } => {
                            session
                                .send_fuigo_notification(
                                    FuigoSessionUpdate::PluginUpdatesInstalled { updates },
                                )
                                .await;
                        }
                        ...
                        SessionCommand::InjectNotification { prompt_id, prompt_blocks, priority, source } => {
                            ...
                            if is_turn_active && priority == NotificationPriority::Next {
                                ...
                                if let Some(buffer) = &session.tool_context.monitor_event_buffer {
                                    ...
                                    buffer.push_capped(
                                        fuigo_tools::implementations::fuigo_build::monitor::types::MonitorEventNotification {
                        ...
                        SessionCommand::FuigoSessionNotification { notification } => {
                            session.handle_fuigo_session_notification(notification).await;
                        }
                        ...
                        SessionCommand::UpdateMcpServers { mcp_servers, respond_to } => {
                            ...
                            // also fires for `<cwd>/.fuigo/config.toml` edits,
                            ...
                            {
                                let _ = tx.send(
                                    fuigo_mcp::servers::McpClientEvent::ConfigDiff {
                        ...
                        SessionCommand::ToggleMcpServer { server_name, enabled, server_config, respond_to } => {
                            session.events.emit(fuigo_session_events::Event::McpServerToggled {
                            ...
                            if is_managed_gateway {
                                ...
                                tokio::task::spawn_local(async move {
                                    ...
                                    if let Ok(params) = serde_json::value::to_raw_value(&payload) {
                                        notifications.forward_fire_and_forget(acp::ExtNotification::new("fuigo/mcp/tools_changed", params.into()));
                                    }
                                    let _ = respond_to.send(Ok(()));
                                });
                                continue;
...
pub(super) fn turn_texts_for_feedback(
    conversation: &[fuigo_sampling_types::ConversationItem],
    turn_number: usize,
    ...
    let extracted = fuigo_chat_state::compaction_utils::extract_user_query(&raw);
