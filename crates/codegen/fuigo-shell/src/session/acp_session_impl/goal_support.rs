...
impl DrainSource {
    ...
    pub(super) fn into_parts(
        ...
        fuigo_tools::implementations::fuigo_build::update_goal::UpdateGoalInput,
        Option<
...
pub(super) fn try_send_ack(
    ack_tx: Option<
        ...
        >,
    >,
    ack: fuigo_tools::implementations::fuigo_build::update_goal::UpdateGoalAck,
) {
...
/// `Some` only when the planner is enabled (`FUIGO_GOAL_PLANNER`) and a plan exists.
...
/// Override with `FUIGO_GOAL_REVERIFY_AFTER` (floored at 1).
...
pub(crate) fn goal_slash_and_harness_available(goal_enabled: bool, tool_names: &[String]) -> bool {
    use fuigo_tools::implementations::fuigo_build::UPDATE_GOAL_TOOL_NAME;
