    ...
    fn attached_images_are_inlined_on_the_side_question_item() {
        ...
        assert!(
            user.content.iter().any(|part| matches!(
                part,
                fuigo_sampling_types::ContentPart::Image { url }
...
impl SessionActor {
    ...
    pub(super) async fn handle_side_question(
        ...
        if super::side_call::should_strip_side_call_reasoning(
            ...
            items = fuigo_chat_state::compaction_utils::strip_reasoning_blocks(items);
        }
    ...
    async fn prepare_side_question_images(
        ...
        let images = self
            .normalize_images_with_notices(body, images, is_cursor)
            .await;
        if is_cursor && !images.is_empty() {
            body.push_str(
    ...
    pub(super) async fn handle_recap(&self, auto: bool) {
        ...
        // Applies to manual `/recap` too: spinner-less clients (e.g. Fuigo Desktop) would otherwise append the late recap mid-turn.
    ...
    fn persist_recap_request_artifact(
        ...
        let artifact = RecapRequestFile {
            ...
            x_fuigo_req_id: x_fuigo_req_id.to_owned(),
            x_fuigo_conv_id: x_fuigo_conv_id.to_owned(),
            strip_reasoning,
    ...
    pub(super) async fn handle_suggest_prompt(
        ...
        use fuigo_telemetry::events::{PromptSuggestion, PromptSuggestionAction as PsAction};
        ...
        let Some(model) = prompt_suggest::effective_suggest_model(
            ...
            fuigo_telemetry::session_ctx::log_event(PromptSuggestion {
        ...
        let suggest_reasoning = prompt_suggest::resolve_suggest_reasoning(
            ...
            self.models_manager.model_supports_reasoning_effort_value(
                &model,
                fuigo_sampling_types::ReasoningEffort::None,
            ),
        );
        ...
        let sampling_client = match fuigo_sampler::SamplingClient::new(sampling_config) {
            Ok(client) => client,
            Err(e) => {
        ...
        let request_id = format!("fuigo-promptsuggest-{}", uuid::Uuid::new_v4());
        let request = ConversationRequest {
            items,
            tools: vec![],
            ...
            reasoning_effort,
            x_fuigo_conv_id: Some(format!("promptsuggest-{}", uuid::Uuid::new_v4())),
            x_fuigo_req_id: Some(request_id.clone()),
            x_fuigo_session_id: Some(self.session_info.id.to_string()),
        ...
        tracing::debug!(
            raw_preview = %fuigo_tools::util::truncate_str(raw.trim(), 60),
...
mod tests {
    ...
    fn side_question_attempts_get_fresh_request_ids() {
        let base = ConversationRequest {
            x_fuigo_conv_id: Some("btw-test".into()),
            ..Default::default()
        };
        let a = build_side_question_attempt(&base);
        let b = build_side_question_attempt(&base);
        let (a_id, b_id) = (a.x_fuigo_req_id.unwrap(), b.x_fuigo_req_id.unwrap());
        assert!(a_id.starts_with("fuigo-btw-"));
