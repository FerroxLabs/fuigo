...
use crate::extensions::notification::{
    RetryState, SessionNotification as FuigoSessionNotification, is_reauthable_failure,
};
...
use fuigo_sampling_types::truncate_bytes;
...
use fuigo_workspace::file_system::CodebaseIndexManager;
...
pub(crate) struct InputItem {
    ...
    pub(crate) tool_overrides_update: Option<fuigo_sampling_types::ToolOverridesUpdate>,
...
impl fuigo_tools::types::resources::ManagedGatewayToolCaller for ShellManagedGatewayToolClient {
    async fn call_tool(
        &self,
        call_id: &str,
        ...
        fuigo_tools::types::resources::ManagedGatewayToolCallResponse,
    ...
    match error {
        crate::session::managed_mcp::ManagedMcpFetchError::Status { status, message } => {
            ...
            let mut err = if status == reqwest::StatusCode::UNAUTHORIZED {
                fuigo_tool_runtime::ToolError::unauthorized(detail)
        ...
        crate::session::managed_mcp::ManagedMcpFetchError::Transport(e) => {
            fuigo_tool_runtime::ToolError::network_error(format!(
...
mod managed_gateway_error_tests {
    ...
    fn unauthorized_status_maps_to_unauthorized_and_carries_status() {
        ...
        assert_eq!(err.kind, fuigo_tool_runtime::ToolErrorKind::Unauthorized);
    ...
    fn general_status_maps_to_execution_with_caller_tool_id() {
        ...
        assert_eq!(err.kind, fuigo_tool_runtime::ToolErrorKind::Execution);
        let details = err.details.as_ref().unwrap();
        assert_eq!(
            details.get(HTTP_STATUS_DETAILS_KEY),
    ...
    async fn transport_error_maps_to_network_error_without_url() {
        ...
        assert_eq!(err.kind, fuigo_tool_runtime::ToolErrorKind::NetworkError);
...
pub(crate) struct SessionActor {
    ...
    /// The event fires at each of the six `OaiCompatClient` 401 arms in `fuigo-sampler`.
    ...
    pub(crate) attribution_callback: Option<fuigo_sampler::SharedAttributionCallback>,
    ...
    pub(crate) chat_state_handle: fuigo_chat_state::ChatStateHandle,
    ...
    pub(crate) resolved_tool_overrides:
        std::sync::Arc<arc_swap::ArcSwapOption<fuigo_sampling_types::ToolOverrides>>,
    pub(crate) compactions_remaining:
        std::cell::Cell<Option<fuigo_sampling_types::CompactionsRemaining>>,
    pub(crate) compaction_at_tokens:
        std::cell::Cell<Option<fuigo_sampling_types::CompactionAtTokens>>,
    /// Server-side doom-loop check policy, resolved once at spawn by `Config::resolve_doom_loop_recovery`; `None` means disabled.
    /// `reconstruct_full_config` threads it into the sampler config, and the sampler itself sends the matching `x-fuigo-doom-loop-check` header.
    pub(crate) doom_loop_recovery: Option<fuigo_sampling_types::DoomLoopRecoveryPolicy>,
    ...
    pub(crate) upload_queue:
        std::sync::Arc<std::sync::OnceLock<fuigo_file_utils::queue::UploadQueue>>,
    ...
    /// Kill-switch (`FUIGO_GOAL_USE_CURRENT_MODEL_ONLY` / `[features] goal_use_current_model_only`) resolved at actor build.
    ...
    pub(crate) pending_classifier_completions: parking_lot::Mutex<
        VecDeque<fuigo_tools::implementations::fuigo_build::update_goal::UpdateGoalInput>,
    >,
    ...
    pub(crate) extension_registry: fuigo_agent_lifecycle::LocalExtensionRegistry,
    ...
    /// Used for hook child process cwd, envelope fields, and FUIGO_WORKSPACE_ROOT env var.
    pub(crate) hook_resolved_workspace_root: String,
    /// The detected VCS kind for this session's workspace.
    pub(crate) vcs_kind: fuigo_workspace::session::git::VcsKind,
    ...
    pub(crate) plugin_registry:
        std::cell::RefCell<Option<std::sync::Arc<fuigo_agent::plugins::PluginRegistry>>>,
    ...
    pub(crate) sampler_handle: fuigo_sampler::SamplerHandle,
    ...
    /// Cached recipe for constructing this session's [`fuigo_agent::Agent`].
    ///
    ...
    /// A fresh `Agent` covers the system prompt, the [`fuigo_tools::bridge::ToolBridge`], and the tool registry.
    ...
    pub(crate) workspace_ops: fuigo_workspace::WorkspaceOps,
...
impl SessionActor {
    ...
    fn build_local_command_availability(
        ...
        slash_commands::CommandAvailability {
            ...
            scheduler: tool_names.iter().any(|n| {
                n == fuigo_tools::implementations::fuigo_build::SCHEDULER_CREATE_TOOL_NAME
            }),
            hooks: self.hook_registry.borrow().is_some(),
            plugins: self.plugin_registry.borrow().is_some(),
            goal,
            workflows: tool_names.iter().any(|n| {
                n == fuigo_tools::implementations::fuigo_build::workflow::WORKFLOW_TOOL_NAME
            }),
    ...
    async fn send_feedback_notification(&self, request: crate::session::feedback::FeedbackRequest) {
        self.send_fuigo_notification(FuigoSessionUpdate::FeedbackRequest(request.into()))
            .await;
    }
}
impl SessionActor {
    ...
    fn tool_bridge_handle(&self) -> Arc<fuigo_tools::bridge::ToolBridge> {
...
/// The saved JSON enables deterministic re-rendering and `fuigo prompt --json` inspection.
/// It also enables post-hoc debugging of what went into a session's system prompt.
fn save_prompt_context(session_info: &SessionInfo, prompt_context: &fuigo_agent::PromptContext) {
...
) -> Option<fuigo_agent::PromptContext> {
    let dir = crate::session::persistence::session_dir(session_info);
...
mod managed_gateway_descriptor_tests {
    use super::*;
    use fuigo_tools::types::output::{MCPOutput, ToolOutput};
    use fuigo_tools::types::tool::{ToolKind, ToolNamespace};
    #[derive(Debug, Default)]
    struct FixtureMcpTool;
    impl fuigo_tools::types::tool_metadata::ToolMetadata for FixtureMcpTool {
    ...
    impl fuigo_tool_runtime::Tool for FixtureMcpTool {
        type Args = serde_json::Value;
        type Output = ToolOutput;
        fn id(&self) -> fuigo_tool_protocol::ToolId {
            fuigo_tool_protocol::ToolId::new("server__tool").expect("valid")
        }
        fn description(
            &self,
            _ctx: &::fuigo_tool_runtime::ListToolsContext,
        ) -> fuigo_tool_types::ToolDescription {
            fuigo_tool_types::ToolDescription::new("server__tool", "fixture")
        }
        async fn run(
            &self,
            ...
            .run_until(async {
                ...
                fixture.actor.agent = std::cell::RefCell::new(
                    test_agent_with_tools(vec![ToolConfig::from_id(
                        "FuigoBuild:read_file".to_string(),
                    )])
                    .await,
                );
...
mod managed_gateway_tool_tests {
    ...
    async fn refresh_snapshot_seeds_only_admitted_gateway_catalog_entries() {
        ...
        let catalog = bridge
            .read_resource::<fuigo_tools::types::resources::ManagedGatewayToolCatalog>()
            .await
