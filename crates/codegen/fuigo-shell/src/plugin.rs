...
use fuigo_agent::plugins::discovery::PluginScope;
use fuigo_agent::plugins::git_install::{self, UpdateStatus};
...
/// When `keep_data` is true, `~/.fuigo/plugin-data/<id>/` is preserved.
pub fn uninstall_plugin(
    ...
    if !keep_data {
        ...
        let scope = match fuigo_dirs::home_dir() {
...
fn marketplace_root_for_provenance(
    provenance: &fuigo_agent::plugins::install_registry::MarketplaceProvenance,
                ...
                if registered.is_empty() {
                    write!(
                        ...
                        "Unknown marketplace \"{qualifier}\". No marketplaces are registered; \
                         add one with `fuigo plugin marketplace add`."
                    )
                } else {
            ...
            Self::NameNotFound {
                ...
                write!(
                    ...
                    "No marketplace plugin named \"{name}\" in any registered marketplace.\n\
                     Install a local directory with `fuigo plugin install ./{name}`, or add a \
                     source with `fuigo plugin marketplace add`."
                )?;
            ...
            Self::NameAmbiguous { name, candidates } => {
                ...
                write!(
                    ...
                    "Multiple marketplaces provide a plugin named \"{name}\":\n{list}\n\
                     Pin one with `fuigo plugin install {name}@<qualifier>`."
                )
            }
            Self::PartialScan {
                name,
                ...
                write!(
                    ...
                    "Couldn't scan every marketplace while resolving \"{name}\", so it can't be \
                     ...
                     Retry, or pin the source explicitly with `fuigo plugin install {name}@<qualifier>`."
                )
            }
            Self::Sync {
                source_display,
...
/// Neither a remote campaign nor the `FUIGO_CONFIG` / `FUIGO_CONFIG_PATH` overlay must be able to relax a local security policy.
...
pub(crate) fn marketplace_require_sha() -> bool {
    fuigo_config::ConfigLayers::load()
        .map(|layers| {
            fuigo_plugin_marketplace::load_require_sha(
                &layers.effective_config_base_without_overlay(),
            )
        })
        .unwrap_or_else(|_| fuigo_plugin_marketplace::env_require_sha())
}
...
/// `~/.fuigo/` and `~/.claude/`. Returns `true` if removed from at least one file.
pub fn try_remove_source_from_json_files(source_url_or_path: &str) -> bool {
    // Resolve user fuigo via user_fuigo_home() (None when no home resolves) and home separately
    // Removal then still runs from $FUIGO_HOME when no home dir exists, and never touches a cwd-relative .fuigo
    ...
    if let Some(ref fuigo) = fuigo {
        settings_candidates.push(fuigo.join("settings.local.json"));
        settings_candidates.push(fuigo.join("settings.json"));
    }
    ...
    if let Some(ref fuigo) = fuigo {
        known_candidates.push(fuigo.join("plugins").join("known_marketplaces.json"));
    }
    if let Some(ref home) = home {
...
mod tests {
    ...
    fn registered_source_label_uses_addressable_qualifier() {
        assert_eq!(
            ...
            "Ferrox Labs Official (fuigo-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn candidate_label_includes_pin_hint() {
        assert_eq!(
            ...
            "Ferrox Labs Official (pin: sentry@fuigo-org/plugin-marketplace)"
        );
        assert_eq!(
    ...
    fn unknown_qualifier_error_lists_registered_marketplaces() {
        let err = MarketplaceInstallError::UnknownQualifier {
            ...
            registered: vec![
                "Ferrox Labs Official (fuigo-org/plugin-marketplace)".into(),
        ...
        assert!(
            msg.contains("  - Ferrox Labs Official (fuigo-org/plugin-marketplace)"),
            "{msg}"
        );
    ...
    fn ambiguous_qualifier_error_lists_source_names() {
        let err = MarketplaceInstallError::AmbiguousQualifier {
            qualifier: "fuigo-org/plugin-marketplace".into(),
    ...
    fn name_not_found_error_hints_local_dir_and_add_source() {
        ...
        let msg = err.to_string();
        assert!(msg.contains("fuigo plugin install ./sentry"), "{msg}");
        assert!(msg.contains("fuigo plugin marketplace add"), "{msg}");
    ...
    fn name_ambiguous_error_lists_candidates_and_pin_hint() {
        ...
        assert!(
            msg.contains("  - Ferrox Labs Official (pin: sentry@fuigo-org/plugin-marketplace)"),
            "{msg}"
        );
        assert!(
            msg.contains("fuigo plugin install sentry@<qualifier>"),
            "{msg}"
        );
    }
    ...
    const OFFICIAL_URL: &str = "https://github.com/fuigo-org/plugin-marketplace.git";
        ...
        match err {
            MarketplaceInstallError::UnknownQualifier {
                ...
                assert_eq!(
                    registered,
                    vec![
                        "Ferrox Labs Official (fuigo-org/plugin-marketplace)".to_string(),
    ...
    fn plan_install_qualifier_not_found_when_scan_lacks_name() {
        let sources = [git_source("Ferrox Labs Official", OFFICIAL_URL)];
        let err = plan_install(
            &sources,
            "sentry",
            Some("fuigo-org/plugin-marketplace"),
        ...
        match err {
            MarketplaceInstallError::QualifiedNameNotFound {
                ...
                assert_eq!(source_display, "Ferrox Labs Official");
            }
    ...
    fn resolve_qualified_source_name_with_matches_git_owner_repo() {
        ...
        let name = resolve_qualified_source_name_with(&sources, "fuigo-org/plugin-marketplace")
            .expect("qualifier should match the official source");
        assert_eq!(name, "Ferrox Labs Official");
    }
