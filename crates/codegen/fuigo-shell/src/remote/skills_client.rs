//! grok.com product Skills catalog, served by the same REST sources fuigo-web uses:
...
/// Combined product catalog matching fuigo-web's Skills menu composition.
...
impl ProductSkillsCatalog {
    ...
    ///
    /// Mirrors fuigo-web: enabled user skills first (and hide same-named bundled entries they override), then remaining bundled skills.
...
fn entry_is_untagged(auth: &crate::auth::FuigoAuth) -> bool {
...
fn skills_auth_alt_candidates<'a>(
    primary: &crate::auth::FuigoAuth,
    entries: impl IntoIterator<Item = &'a crate::auth::FuigoAuth>,
...
impl SkillsClient {
    ...
    async fn require_skills_auth(&self) -> Result<crate::auth::FuigoAuth, SkillsError> {
    ...
    fn skills_auth_candidates(&self, primary: &crate::auth::FuigoAuth) -> Vec<SkillsAuthCandidate> {
    ...
    /// Bundled first-party skills (`POST /rest/skills`), gated by backend feature-flag allow-list (same as fuigo-web attach menu).
    ///
