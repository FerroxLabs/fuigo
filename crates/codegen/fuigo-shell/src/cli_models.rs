//! Data APIs for `fuigo models`. Rendering is the client's job.
use crate::agent::config::Config as AgentConfig;
use agent_client_protocol as acp;
use anyhow::Result;
use fuigo_acp_lib::{AcpAgentTx, acp_send};
/// Status for the `fuigo models` banner (the display order is not the sampling priority; see [`AuthStatus::resolve`]).
#[derive(Debug, PartialEq, Eq)]
pub enum AuthStatus {
    ApiKey,
    /// Auth host from `fuigo_ws_origin` (scheme stripped).
    LoggedIn(String),
...
impl AuthStatus {
    ...
    /// BYOK uses [`crate::agent::auth_method::should_advertise_fuigo_api_key`] so `disable_api_key_auth` is honored.
    pub fn resolve(agent_config: &AgentConfig) -> Self {
        ...
        if crate::agent::auth_method::should_advertise_fuigo_api_key(
            agent_config.fuigo_com_config.api_key_auth_disabled(),
            models.values(),
...
mod tests {
    ...
    /// Uses `FUIGO_AUTH_PATH` (not `FUIGO_HOME`) so a OnceLock-cached real home with `auth.json` cannot leak into these tests.
    fn isolate_auth_sources() -> (tempfile::TempDir, [EnvGuard; 7]) {
        let dir = tempfile::tempdir().unwrap();
        let auth_path = dir.path().join("no-auth.json");
        let guards = [
            EnvGuard::unset(FUIGO_API_KEY_ENV_VAR),
            EnvGuard::unset(LEGACY_FUIGO_API_KEY_ENV_VAR),
            EnvGuard::unset("FUIGO_AUTH"),
            EnvGuard::set("FUIGO_AUTH_PATH", auth_path.to_str().unwrap()),
            EnvGuard::unset("FUIGO_DEPLOYMENT_KEY"),
            EnvGuard::unset("FUIGO_WS_ORIGIN"),
            EnvGuard::unset("FUIGO_DISABLE_API_KEY_AUTH"),
        ];
        (dir, guards)
    }
