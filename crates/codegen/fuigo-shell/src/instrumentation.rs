//! Shim; see `fuigo_telemetry::instrumentation` for the implementation.
//!
//! Two pieces stay here:
//! - The [`instrumentation_timer!`] macro is `#[macro_export]`-ed from this crate.
//!   Call sites spell it `crate::instrumentation_timer!` or `fuigo_shell::instrumentation_timer!`.
...
pub use fuigo_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `fuigo_shell::instrumentation::finalize_and_exit`.
...
///
/// The macro stays in shell so `$crate` continues to resolve to `fuigo_shell` for the 12+ existing call sites.
/// Those sites spell it `crate::instrumentation_timer!(...)` or `fuigo_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in `fuigo_telemetry::instrumentation`.
#[macro_export]
