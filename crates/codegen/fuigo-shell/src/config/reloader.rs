...
use crate::auth::{FuigoAuth, read_auth_json};
...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.fuigo/config.toml`
    ///    changed.
    ...
    /// Those are `<cwd>/.fuigo/config.toml`, `<cwd>/.mcp.json`, and the project-level `<cwd>/.claude.json`.
    ///
    ...
    McpServersChanged,
    /// A **project-scoped** MCP config file changed (`<cwd>/.fuigo/config.toml`, `<cwd>/.mcp.json`, or `<cwd>/.claude.json`).
    ...
    ProjectMcpServersChanged {
        /// The project root whose `.fuigo/`, `.mcp.json`, or `.claude.json` file was edited.
    ...
    ModelsChanged,
    /// `~/.fuigo/models_cache.json` was rewritten on disk (possibly by another
    /// fuigo process sharing the home dir). The agent should consult the cache via `ModelsManager::reload_from_disk_cache`.
...
impl ConfigReloader {
    ...
    pub(crate) async fn run(
        ...
        loop {
            ...
            // same tick (e.g. `~/.fuigo/config.toml` AND
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // scoped changes (`<cwd>/.fuigo/config.toml`, `<cwd>/.mcp.json`) go out separately as `ConfigUpdate::ProjectMcpServersChanged { cwd }`
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.fuigo/config.toml` | `<cwd>`           |
    ...
    for evt in batch {
        let cwd = match evt {
            ConfigChangeEvent::ProjectConfigChanged { path } => {
                // <cwd>/.fuigo/config.toml yields <cwd>
                path.parent()
                    .and_then(|p| p.parent())
                    .map(|p| p.to_path_buf())
            }
...
/// It walks ancestors up to the git root as the loaders do: `find_project_configs` for `.fuigo/config.toml`, `find_mcp_json_files` for `.mcp.json`.
...
/// (`~/.fuigo/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
///
/// Consumers: the reload dispatch above (change detection into `ConfigUpdate::Skills`) and `fuigo inspect` (via the `crate::config` re-export).
...
pub(crate) fn parse_skills_config(
    config: &toml::Value,
) -> fuigo_agent::prompt::skills::SkillsConfig {
    config
        .get("skills")
...
fn parse_compat_config(config: &toml::Value) -> fuigo_tools::types::compat::CompatConfigToml {
    config
        .get("compat")
        .and_then(|v| v.clone().try_into().ok())
        .unwrap_or_default()
...
mod tests {
    ...
    fn make_auth(key: &str) -> FuigoAuth {
        FuigoAuth {
            key: key.to_string(),
    ...
    async fn reloader_detects_new_auth_key() {
        ...
        assert!(
            matches!(update, ConfigUpdate::Auth(a) if a.key == "new-key"), // a is Box<FuigoAuth>, Deref coercion
    ...
    /// The hash must reflect ancestor `.fuigo/config.toml` and `.mcp.json` under `cwd`; otherwise an ancestor edit would be wrongly suppressed.
    #[test]
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".fuigo").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .fuigo/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.fuigo/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.fuigo/skills".to_string()]);
    ...
    fn memory_config_diff_detects_enabled_change() {
        ...
        let new = crate::agent::config::Config::new_from_toml_cfg(&disabled)
            .unwrap()
            .resolve_memory(None, None);
...
"#,
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.fuigo/config.toml"),
            },
        ];
    ...
    /// `collect_project_cwds` extracts `<cwd>` from `ProjectConfigChanged` (`<cwd>/.fuigo/config.toml`) and `McpConfigChanged` (`<cwd>/.mcp.json`).
    /// It de-duplicates while preserving order.
    #[test]
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.fuigo/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.fuigo/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
