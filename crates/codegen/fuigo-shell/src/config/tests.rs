...
fn expands_env_vars_in_toml_strings() {
    with_env_var(
        "FUIGO_TEST_CONFIG_EXPAND",
        "expanded",
        || {
            let toml_str = r#"
[mcp_servers.test]
command = "$FUIGO_TEST_CONFIG_EXPAND/bin/server"
args = ["--path", "${FUIGO_TEST_CONFIG_EXPAND}/data"]
"#;
...
fn leaves_missing_env_vars_unchanged() {
    let toml_str = r#"
[mcp_servers.test]
command = "$FUIGO_TEST_CONFIG_MISSING/bin/server"
"#;
    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
    expand_env_vars_in_toml(&mut value);
    let toml::Value::Table(table) = value else {
    ...
    let command = test.get("command").and_then(|v| v.as_str()).unwrap();
    assert_eq!(command, "$FUIGO_TEST_CONFIG_MISSING/bin/server");
}
#[test]
...
/// Mutex to serialize tests that touch the FUIGO_MEMORY env var.
...
/// Run `f` with FUIGO_MEMORY explicitly unset.
fn without_fuigo_memory<T>(f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("FUIGO_MEMORY", None, f)
}
/// Run `f` with FUIGO_MEMORY set to a specific value.
fn with_fuigo_memory<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = MEMORY_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
...
fn memory_config_env_var_zero_does_not_enable() {
    with_fuigo_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(!mem.enabled, "FUIGO_MEMORY=0 should not enable memory");
        },
    );
}
#[test]
fn memory_config_env_var_false_does_not_enable() {
    with_fuigo_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, false, &config, None);
            assert!(!mem.enabled, "FUIGO_MEMORY=false should not enable memory");
        },
    );
}
#[test]
...
fn memory_config_env_zero_force_disables_toml_enabled() {
    with_fuigo_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "FUIGO_MEMORY=0 should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_env_false_force_disables_toml_enabled() {
    with_fuigo_memory(
        ...
        || {
            ...
            assert!(
                !mem.enabled,
                "FUIGO_MEMORY=false should force-disable even when TOML enables memory"
            );
        },
    );
}
...
fn memory_config_cli_flag_overrides_env_disable() {
    with_fuigo_memory(
        ...
        || {
            ...
            assert!(
                mem.enabled,
                "CLI --experimental-memory should override FUIGO_MEMORY=0"
            );
        },
    );
}
...
fn memory_config_no_memory_overrides_env_enable() {
    with_fuigo_memory(
        ...
        || {
            let config = toml::Value::Table(toml::map::Map::new());
            let mem = MemoryConfig::resolve(false, true, &config, None);
            assert!(!mem.enabled, "--no-memory should override FUIGO_MEMORY=1");
        },
    );
}
#[test]
        ...
        || {
            with_env_var(
                ...
                || {
                    let toml_str = r#"
[mcp_servers.test]
command = "$FUIGO_TEST_USER $FUIGO_TEST_ROOT"
"#;
                    let mut value = toml::from_str::<toml::Value>(toml_str).unwrap();
                    expand_env_vars_in_toml(&mut value);
                    let toml::Value::Table(table) = value else {
...
/// Mutex to serialize tests that touch the FUIGO_SUBAGENTS env var.
static SUBAGENTS_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());
/// Run `f` with FUIGO_SUBAGENTS explicitly unset.
fn without_fuigo_subagents<T>(f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("FUIGO_SUBAGENTS", None, f)
}
/// Run `f` with FUIGO_SUBAGENTS set to a specific value.
fn with_fuigo_subagents<T>(value: &str, f: impl FnOnce() -> T) -> T {
    let _guard = SUBAGENTS_ENV_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt("FUIGO_SUBAGENTS", Some(value), f)
}
#[test]
fn subagents_config_default_enabled() {
    without_fuigo_subagents(|| {
        let config = toml::Value::Table(toml::map::Map::new());
...
fn subagent_limit_counts_resolve_env_over_toml_over_remote_over_default() {
    use fuigo_tools::implementations::fuigo_build::task::admission;
...
fn subagent_sampling_limit_applies_precedence_and_clamps() {
    ...
    use fuigo_tools::implementations::fuigo_build::task::admission::DEFAULT_MAX_CONCURRENT;
    ...
    with_fuigo_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str("[subagents]\nenabled = true")
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(!sa.enabled, "FUIGO_SUBAGENTS=0 should override config file");
        },
    );
}
#[test]
...
fn subagents_config_env_var_disables_default() {
    with_fuigo_subagents(
        ...
        || {
            ...
            assert!(
                !sa.enabled,
                "FUIGO_SUBAGENTS=0 should override the enabled default"
            );
        },
    );
}
...
fn subagents_config_cli_flag_overrides_env_var() {
    with_fuigo_subagents(
        ...
        || {
            ...
            assert!(
                sa.enabled,
                "--subagents CLI flag should override FUIGO_SUBAGENTS=0"
            );
        },
    );
}
    ...
    with_fuigo_subagents(
        ...
        || {
            let config: toml::Value = toml::from_str(
                ...
                .unwrap();
            let sa = SubagentsConfig::resolve(false, &config);
            assert!(sa.enabled, "FUIGO_SUBAGENTS=1 should enable");
            assert_eq!(sa.models.get("explore").unwrap(), "grok-3-fast");
        },
    );
}
...
fn with_managed_mcp_env<T>(
    ...
    with_env_var_opt(
        ...
        managed_mcps,
        || with_env_var_opt("FUIGO_MANAGED_MCP_GATEWAY_TOOLS_ENABLED", gateway_tools, f),
    )
}
#[test]
#[serial_test::serial]
...
fn with_model_overrides_env_full<T>(
    ...
    let _guard = LOCK.lock().unwrap_or_else(|e| e.into_inner());
    with_env_var_opt(
        "FUIGO_WEB_SEARCH_MODEL",
        ws,
        || with_env_var_opt(
            "FUIGO_SESSION_SUMMARY_MODEL",
            ss,
            || with_env_var_opt(
                "FUIGO_IMAGE_DESCRIPTION_MODEL",
                id,
                || with_env_var_opt("FUIGO_PROMPT_SUGGESTIONS_MODEL", ps, f),
            ),
        ),
    )
}
...
#[test]
fn model_overrides_default_image_description_is_fuigo_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
#[test]
fn model_overrides_default_session_summary_is_fuigo_build() {
    with_model_overrides_env(
        None,
        None,
        None,
...
fn with_tools_env<T>(
    ...
    with_env_var_opt(
        "FUIGO_RESPECT_GITIGNORE",
        respect_gitignore,
        || with_env_var_opt("FUIGO_DISABLE_ZDR_INCOMPATIBLE_TOOLS", disable_zdr, f),
    )
}
fn without_fuigo_respect_gitignore<T>(f: impl FnOnce() -> T) -> T {
    with_tools_env(None, None, f)
}
fn with_fuigo_respect_gitignore<T>(value: &str, f: impl FnOnce() -> T) -> T {
    with_tools_env(Some(value), None, f)
}
#[test]
fn tools_config_default_disabled() {
    without_fuigo_respect_gitignore(|| {
        let config = toml::Value::Table(toml::map::Map::new());
    ...
    with_fuigo_respect_gitignore(
        ...
        || {
            ...
            assert!(
                ...
                "FUIGO_RESPECT_GITIGNORE=false should override config file"
            );
        },
    );
}
...
fn media_gen_caps_resolve_env_over_toml_over_remote_over_default() {
    use fuigo_tools::media_gen_limits::{
...
fn roles_parse_from_toml() {
    let toml_str = r#"
            ...
            prompt_file = ".fuigo/prompts/impl.md"
        "#;
...
fn validate_roles_accepts_valid_prompt_file() {
    let toml_str = r#"
            ...
            prompt_file = ".fuigo/prompts/ok.md"
        "#;
    let cfg: SubagentsConfig = toml::from_str(toml_str).unwrap();
    assert!(cfg.validate_roles().is_empty());
}
...
fn discover_roles_loads_from_directory() {
    ...
    let roles_dir = tmp.path().join(".fuigo").join("roles");
...
fn personas_parse_from_toml() {
    ...
    assert_eq!(
            ...
            Some(".fuigo/personas/concise.md")
        );
}
#[test]
...
fn discover_personas_loads_from_directory() {
    let tmp = tempfile::TempDir::new().unwrap();
    let dir = tmp.path().join(".fuigo").join("personas");
...
fn bundled_personas_and_roles_have_lowest_priority_in_resolve_order() {
    ...
    let bundled = home.join(".fuigo").join("bundled");
    std::fs::create_dir_all(workspace.join(".fuigo").join("roles")).unwrap();
    std::fs::create_dir_all(workspace.join(".fuigo").join("personas")).unwrap();
    std::fs::create_dir_all(home.join(".fuigo").join("roles")).unwrap();
    std::fs::create_dir_all(home.join(".fuigo").join("personas")).unwrap();
    ...
    std::fs::write(
        ...
        .unwrap();
    std::fs::write(
            home.join(".fuigo/roles/reviewer.toml"),
            r#"description = "User reviewer""#,
        )
        .unwrap();
    std::fs::write(
            home.join(".fuigo/personas/reviewer.toml"),
            r#"instructions = "User persona""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".fuigo/roles/reviewer.toml"),
            r#"description = "Project reviewer""#,
        )
        .unwrap();
    std::fs::write(
            workspace.join(".fuigo/personas/reviewer.toml"),
    ...
    std::fs::remove_file(workspace.join(".fuigo/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(workspace.join(".fuigo/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
    ...
    std::fs::remove_file(home.join(".fuigo/roles/reviewer.toml")).unwrap();
    std::fs::remove_file(home.join(".fuigo/personas/reviewer.toml")).unwrap();
    let config = toml::from_str::<
        toml::Value,
    >(r#"
            [subagents]
...
fn render_io_summary_shows_bundled_for_bundled_personas() {
    let persona = SubagentPersona {
        ...
        source_path: Some("/tmp/home/.fuigo/bundled/personas/reviewer.toml".to_string()),
        ..Default::default()
    };
...
/// (`ConfigLayers` has no project slot, so a repo `.fuigo/config.toml` structurally cannot supply one.)
#[test]
...
[model.fuigo-build]
base_url = "https://inference.acme-corp.example/fuigo/v1"
...
/// RCE guard: a project `.fuigo/config.toml` must never source `[feedback.user]` (its `command` runs `sh -c`).
#[test]
#[serial_test::serial]
...
fn a_repeated_pin_is_reported_against_the_layer_that_decided() {
    ...
    let user = RequirementSource::Requirements {
        path: std::path::PathBuf::from("/home/dev/.fuigo/requirements.toml"),
    };
...
/// An administrator pinning the title off must still outrank a user's FUIGO_TITLE_REFRESH, which a config-tier value would lose to.
#[test]
#[serial_test::serial]
fn apply_requirements_pins_title_refresh_over_the_environment() {
    ...
    assert!(!resolved.value, "the pin lost to FUIGO_TITLE_REFRESH");
    assert_eq!(
            resolved.source,
...
#[test]
fn validate_hooks_path_rejects_outside_fuigo_home() {
    let result = validate_hooks_path("/tmp/evil-hooks");
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
    assert!(
            msg.contains("must be under ~/.fuigo/"),
            "should mention ~/.fuigo/ restriction, got: {msg}"
        );
}
#[test]
fn validate_hooks_path_rejects_traversal_attack() {
    let fuigo_home = crate::util::fuigo_home::fuigo_home();
    let traversal = format!("{}/../evil", fuigo_home.display());
...
#[test]
fn validate_hooks_path_accepts_fuigo_hooks_subdir() {
    let fuigo_home = crate::util::fuigo_home::fuigo_home();
    let valid_path = fuigo_home.join("hooks").join("my-hooks");
    let _ = std::fs::create_dir_all(&valid_path);
    let result = validate_hooks_path(valid_path.to_str().unwrap());
    assert!(result.is_ok(), "path under ~/.fuigo/ should be accepted");
}
#[test]
fn managed_settings_disables_features_and_requirements_overrides() {
    use crate::agent::config::Feature;
    use fuigo_workspace::permission::resolution::ManagedSettingsFeatures;
    let mut cfg = crate::agent::config::Config::default();
...
/// disableBypassPermissionsMode (mapped to features.disable_yolo) must NOT clamp the user's own fuigo yolo.
#[test]
...
fn simulate_release_build() -> fuigo_test_support::EnvGuard {
    fuigo_test_support::EnvGuard::set(fuigo_version::TEST_VERSION_ENV, "0.0.0-sim")
}
#[test]
...
fn base_resolver_without_project_cwd_keeps_project_files_out() {
    let tmp = tempfile::tempdir().unwrap();
    write_subagent_definitions(&tmp.path().join(".fuigo"), &[("project", "Project")]);
    let base = SubagentsConfig::resolve_base_with_sources(
        false,
...
#[test]
fn explicit_fuigo_root_is_the_only_user_source() {
    let tmp = tempfile::tempdir().unwrap();
    let ambient = tmp.path().join("ambient-home/.fuigo");
    let configured = tmp.path().join("configured-fuigo-home");
...
/// The test is FUIGO_HOME-isolated and `#[serial]` for folder-trust store hygiene: an empty store is deterministically untrusted.
/// `EnvGuard` restores FUIGO_HOME even on panic.
...
/// This mirrors the Project-scope analog `discover_real_project_plugin_gated_on_project_trusted` in `fuigo-agent`.
...
/// The test is FUIGO_HOME-isolated and `#[serial]` (`EnvGuard` restores it even on panic).
#[test]
#[serial_test::serial]
...
/// The test is FUIGO_HOME-isolated (empty store); FUIGO_FOLDER_TRUST is unset so the kill-switch is the only signal.
#[test]
#[serial_test::serial]
fn kill_switched_cold_cwd_stays_allowed_through_plugins_config_read() {
    ...
    let fuigo = repo.path().join(".fuigo");
    std::fs::create_dir_all(&fuigo).unwrap();
    std::fs::write(fuigo.join("config.toml"), "[plugins]\npaths = [\"./proj-plugin\"]\n")
        .unwrap();
    let cwd = repo.path();
    let remote = crate::util::config::RemoteSettings {
...
/// Writeback requires grok.com auth: remote may advertise it, but a non-fuigo credential is downgraded to `Local`.
#[test]
#[serial_test::serial]
fn from_remote_gated_requires_fuigo_auth_for_writeback() {
