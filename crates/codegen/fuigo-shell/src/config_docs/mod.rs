...
//!
//! The page `crates/codegen/fuigo-pager/docs/user-guide/26-config-reference.md` is the source; edit that file.
...
//! `~/.fuigo/docs/user-guide/` on launch.

use std::collections::BTreeMap;
use std::path::PathBuf;

use fuigo_config_types::{FEATURES, KNOWN_MCP_SERVER_FIELDS};
...
const FUIGO_COM_CONFIG_LEAVES: &[&str] = &[
    "fuigo_com_config.fuigo_ws_origin",
    "fuigo_com_config.fuigo_ws_url",
    "fuigo_com_config.token_header",
    "fuigo_com_config.auth_provider_label",
    "fuigo_com_config.auth_token_ttl",
    "fuigo_com_config.auth_provider_command",
    "fuigo_com_config.preferred_method",
    "fuigo_com_config.disable_api_key_auth",
    "fuigo_com_config.force_login_team_uuid",
    "fuigo_com_config.oauth2.issuer",
    "fuigo_com_config.oauth2.client_id",
    "fuigo_com_config.oauth2.scopes",
    "fuigo_com_config.oauth2.principal_type",
    "fuigo_com_config.oauth2.principal_id",
    "fuigo_com_config.oauth2.referrer",
    "fuigo_com_config.oidc.issuer",
    "fuigo_com_config.oidc.client_id",
    "fuigo_com_config.oidc.scopes",
    "fuigo_com_config.oidc.audience",
];
...
fn committed_markdown_path() -> PathBuf {
    if let Some(path) = std::env::var_os("FUIGO_CONFIG_REFERENCE_MD") {
        return PathBuf::from(path);
    }
    let root = find_monorepo_root().unwrap_or_else(|| {
        panic!(
            "committed config-reference user-guide not found; set FUIGO_CONFIG_REFERENCE_MD or run from the monorepo (CARGO_MANIFEST_DIR={})",
            env!("CARGO_MANIFEST_DIR")
        )
    });
    root.join(format!(
        "crates/codegen/fuigo-pager/docs/user-guide/{USER_GUIDE_FILENAME}"
    ))
}
...
mod tests {
    ...
    #[test]
    fn fuigo_com_config_nested_fields_and_auth_aliases() {
        let (config, _, _) = page();
        let map = by_key(&config);
        for leaf in FUIGO_COM_CONFIG_LEAVES {
            let row = map.get(*leaf).unwrap_or_else(|| panic!("missing {leaf}"));
            let alias = leaf.replacen("fuigo_com_config.", "auth.", 1);
            let alias_row = map
                .get(alias.as_str())
        ...
        assert_eq!(
            map["fuigo_com_config.disable_api_key_auth"].requirements,
            "pin"
        );
        assert_eq!(
            map["fuigo_com_config.force_login_team_uuid"].requirements,
            "pin"
        );
    }
    ...
    #[test]
    fn overlay_free_gates_do_not_claim_fuigo_config() {
        let (config, req, _) = page();
        for row in config.iter().chain(req.iter()) {
            if matches!(
                ...
                assert!(
                    !row.details.contains("FUIGO_CONFIG"),
                    "{} must not claim FUIGO_CONFIG",
                    row.key
                );
            }
        }
    ...
    fn page_is_the_user_facing_field_list() {
        ...
        assert!(!md.contains("Generated from `fuigo-shell`"));
        for leak in [
            "FEATURES",
