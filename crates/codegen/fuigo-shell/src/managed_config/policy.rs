    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Busy => f.write_str(
                "Managed policy is being updated by another fuigo process or a \
...
) -> fuigo_telemetry::startup::AuthMode {
...
pub(super) fn served_principal_of(
    payload: &fuigo_config::signed_policy::SignedPayload,
) -> Option<&str> {
    payload
        .deployment_id
        .as_deref()
...
pub(super) fn claim_binds_to(
    claim: &fuigo_config::signed_policy::ManagedIdentityClaim,
