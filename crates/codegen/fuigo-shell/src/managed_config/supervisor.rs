...
/// Base 1s; `FUIGO_DEPLOYMENT_CONFIG_BACKOFF_MS` overrides it for tests.
fn retry_backoff(attempt: u32) -> std::time::Duration {
    let base = std::env::var("FUIGO_DEPLOYMENT_CONFIG_BACKOFF_MS")
        .ok()
    ...
    match failure.kind {
        TransportFailureKind::CertificateUntrusted => {
            ManagedConfigError::CertificateUntrusted(policy::certificate_detail(
                failure.detail,
                fuigo_extra_ca::configured_bundle_env(),
                fuigo_extra_ca::extra_root_ders().len(),
            ))
        }
...
async fn fetch_managed_config_once(
    ...
    if let Some(nonce) = fuigo_config::signed_policy::stored_envelope_nonce(
        &crate::util::fuigo_home::fuigo_home(),
        echo_principal,
    ) && let Ok(value) = reqwest::header::HeaderValue::from_str(&nonce)
    {
        request = request.header(
            fuigo_config::signed_policy::MANAGED_CONFIG_NONCE_ECHO_HEADER,
            value,
        );
    }
...
fn managed_config_sync_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("FUIGO_DEPLOYMENT_CONFIG_REFRESH_INTERVAL_SECS")
...
async fn sync_bounded(
    budget: SyncBudget,
    team_override: Option<FuigoAuth>,
...
pub async fn post_login_sync(authenticated: Option<FuigoAuth>) -> ManagedConfigSync {
    store::clear_orphan();
    if !store::is_fetch_enabled() {
...
pub async fn ensure_managed_policy_present(
    ...
    fuigo_telemetry::startup::enter(fuigo_telemetry::startup::StartupPhase::ManagedPolicy);
    let has_deployment_key = store::resolve_deployment_key().is_some();
    let signed_in_team = store::team_principal_signed_in();
    fuigo_telemetry::startup::set_auth_mode(policy::auth_mode(has_deployment_key, &signed_in_team));
...
async fn refreshed_team_principal(
    ...
    tokio::time::timeout(SESSION_START_AUTH_DEADLINE, refresh)
        ...
        .and_then(Result::ok)
        .and_then(Result::ok)
        .filter(FuigoAuth::is_team_principal)
}
