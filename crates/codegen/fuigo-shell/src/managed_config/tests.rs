...
fn certificate_detail_names_the_bundle_env_only_when_set() {
    assert_eq!(
        certificate_detail("UnknownIssuer".into(), Some("FUIGO_EXTRA_CA_BUNDLE"), 2),
        "UnknownIssuer; FUIGO_EXTRA_CA_BUNDLE is set: verify it includes the issuing root CA"
    );
    assert_eq!(
        certificate_detail("UnknownIssuer".into(), Some("FUIGO_EXTRA_CA_BUNDLE"), 0),
        "UnknownIssuer; FUIGO_EXTRA_CA_BUNDLE is set but no usable roots were loaded from it: check that the file is readable, contains PEM certificates, and is under the size cap"
    );
    assert_eq!(
...
fn purge_keeps_marker_when_an_artifact_removal_fails() {
    ...
    std::fs::write(home.join(fuigo_config::MANAGED_CONFIG_CACHE_FILE), "{}").unwrap();
    ...
    remove_managed_config_files(home);
    assert!(
        home.join(fuigo_config::MANAGED_CONFIG_CACHE_FILE)
            .exists(),
    ...
    assert!(
        !home
            .join(fuigo_config::MANAGED_CONFIG_CACHE_FILE)
            .exists(),
...
fn served_principal_prefers_deployment_id() {
    use fuigo_config::signed_policy::SignedPayload;
    let payload = |dep: Option<&str>, team: Option<&str>| SignedPayload {
        typ: fuigo_config::signed_policy::MANAGED_POLICY_TYP.into(),
        version: 1,
...
fn claim_persists_only_when_bound_to_served_principal() {
    let claim = |principal: &str| fuigo_config::signed_policy::ManagedIdentityClaim {
        typ: fuigo_config::signed_policy::MANAGED_IDENTITY_TYP.into(),
...
fn auth_mode_classification() {
    use fuigo_telemetry::startup::AuthMode;
