...
pub const MANAGED_ARTIFACT_FILES: [&str; 4] = [
    ...
    fuigo_config::signed_policy::SIGNATURE_SIDECAR_FILE,
...
pub(super) fn remove_managed_config_files(home: &std::path::Path) {
    ...
    if artifacts_removed {
        remove_synced_file(
            home,
            fuigo_config::MANAGED_CONFIG_CACHE_FILE,
            "removed managed config file",
        );
        let _ = std::fs::remove_file(staged_refresh_path(home));
    }
    let atomic_write_tmp_prefixes = [
        format!("{}.", fuigo_config::MANAGED_CONFIG_CACHE_FILE),
        format!("{}.", fuigo_config::signed_policy::SIGNATURE_SIDECAR_FILE),
        format!(
            "{}.",
            fuigo_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE
        ),
...
pub(super) fn eligible_team_principal(auth: FuigoAuth) -> Option<FuigoAuth> {
...
pub fn clear_orphan() {
    ...
    if fuigo_config::fail_closed_policy_armed_at(&home) {
        tracing::info!(
...
/// `home` must be the process `fuigo_home()`: the identity reads resolve it internally.
pub(super) fn gate_snapshot_locked(home: &std::path::Path) -> GateSnapshot {
    ...
    fuigo_config::bump_rollback_floor(home);
    GateSnapshot {
...
pub(super) fn bump_managed_rollback_floor() {
    ...
    if !fuigo_config::signed_policy::verification_active() {
        return;
    }
    let home = crate::util::fuigo_home::fuigo_home();
    ...
    for (name, content) in artifacts {
        ...
        match content.filter(|s| !s.is_empty()) {
            Some(content) => {
                ...
                match fuigo_config::fs_atomic::write_atomically(&path, content, Some(0o600)) {
                    Ok(()) => changed = true,
                    Err(e) => {
...
pub(super) fn apply_fetched(
    ...
    let verified = if fuigo_config::signed_policy::verification_active() {
    ...
    if let Some(parked_at) = parked_at
        && parked_at < fuigo_config::managed_config_synced_at(&home).unwrap_or(0)
    {
    ...
    if let Some(verified) = verified {
        clear_squatting_dir(&home.join(fuigo_config::signed_policy::SIGNATURE_SIDECAR_FILE));
        fuigo_config::signed_policy::write_sidecar(&home, &verified.sidecar)?;
        if let Some(claim_sidecar) =
            verified_claim_sidecar(body, served_principal_of(&verified.payload))
        {
            clear_squatting_dir(
                &home.join(fuigo_config::signed_policy::MANAGED_IDENTITY_SIDECAR_FILE),
            );
            fuigo_config::signed_policy::write_managed_identity_sidecar(&home, &claim_sidecar)?;
        }
    }
    // Marker last, still under the lock: post-release, a concurrent purge could delete
    // the files it describes.
    clear_squatting_dir(&home.join(fuigo_config::MANAGED_CONFIG_CACHE_FILE));
...
fn stage_refresh(
    ...
    fuigo_config::fs_atomic::write_atomically(&path, &json, Some(0o600))
}
...
pub(super) fn apply_staged_managed_config() {
    ...
    if !is_fetch_enabled() || !fuigo_config::signed_policy::verification_active() {
    ...
    if staged.parked_at == 0
        || staged.parked_at < fuigo_config::managed_config_synced_at(&home).unwrap_or(0)
    {
        let _ = std::fs::remove_file(&path);
        return;
    }
...
pub(super) fn verified_claim_sidecar(
...
) -> Option<fuigo_config::signed_policy::SignatureEnvelope> {
    ...
    let claim = match fuigo_config::signed_policy::verify_fetched_claim(&sidecar, now_unix()) {
        Ok(claim) => claim,
        Err(e) => {
...
/// Overlay-free read: a `FUIGO_CONFIG` overlay must not suppress a policy-enforcement sync.
pub fn is_fetch_enabled() -> bool {
    if let Some(v) = crate::agent::config::env_bool("FUIGO_MANAGED_CONFIG") {
        return v;
    }
...
pub fn classify_auth_mode() -> fuigo_telemetry::startup::AuthMode {
    auth_mode(
