...
impl ManagedConfigSource {
    ...
    /// The 401/403 error tailored to the credential (don't tell a team user to check `FUIGO_DEPLOYMENT_KEY`).
...
pub enum ManagedConfigError {
    ...
    #[error(
        "The deployment key was rejected. Confirm that FUIGO_DEPLOYMENT_KEY is set correctly and hasn't expired."
    )]
    DeploymentKeyRejected,
    #[error(
        "Your team sign-in was rejected. It may have expired or lack access. Run `fuigo login` to sign in again."
    )]
    TeamAuthRejected,
...
pub(super) struct ManagedConfigResponse {
    ...
    #[serde(default)]
    pub(super) signatures: Option<Vec<fuigo_config::signed_policy::SignatureEnvelope>>,
    ...
    pub(super) managed_identity_signatures:
        Option<Vec<fuigo_config::signed_policy::SignatureEnvelope>>,
}
...
impl ManagedConfigResponse {
    ...
    pub(super) fn signature_sidecar(
        ...
        pick_trusted_envelope(
            ...
            fuigo_config::signed_policy::embedded_key_id_trusted,
        )
    }
...
fn pick_trusted_envelope(
    envelopes: Option<&[fuigo_config::signed_policy::SignatureEnvelope]>,
...
pub(super) struct VerifiedEnvelope {
    pub(super) sidecar: fuigo_config::signed_policy::SignatureEnvelope,
    pub(super) payload: fuigo_config::signed_policy::SignedPayload,
}
...
pub(super) fn verify_signed_envelope(
    ...
    use fuigo_config::signed_policy;
...
mod tests {
    ...
    fn pick_trusted_envelope_prefers_trusted_then_falls_back() {
        use fuigo_config::signed_policy::SignatureEnvelope;
