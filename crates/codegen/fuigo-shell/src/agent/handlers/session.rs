...
/// Router for fuigo/session/* and fuigo/session_summaries/* methods.
pub(crate) async fn handle(
    agent: &MvpAgent,
    args: &acp::ExtRequest,
) -> Result<acp::ExtResponse, acp::Error> {
    match args.method.as_ref() {
        "fuigo/session/info" => handle_session_info(agent, args).await,
        "fuigo/session/close" => handle_session_close(agent, args).await,
        "fuigo/session/list" => handle_session_list(agent, args).await,
        "fuigo/sessions/list" => handle_roster_list(agent, args).await,
...
/// `fuigo/sessions/list`, the FleetView roster.
/// Returns every resident session plus on-disk `Dormant` sessions that were touched recently.
/// Clients poll this while the dashboard is open and reconcile against the `fuigo/sessions/changed` broadcast.
