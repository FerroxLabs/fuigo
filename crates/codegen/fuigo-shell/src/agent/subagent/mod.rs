...
//!
//! Lifecycle state and command scheduling live in the shared `fuigo-tools` coordinator actor.
...
use fuigo_agent::config::{McpInheritance, ModelOverride, PermissionMode};
...
use fuigo_subagent_resolution::ResumeSourceData;
use fuigo_tools::implementations::fuigo_build::monitor::types::MonitorEventBuffer;
use fuigo_tools::implementations::fuigo_build::task::types::*;
use fuigo_tools::types::tool::ToolKind;
use fuigo_workspace::file_system::AsyncFileSystem;
...
pub(crate) struct SubagentSpawnContext {
    ...
    /// The Spawn arm of `handle_command` in fuigo-tools task/coordinator.rs re-parents nested Spawn requests to the root parent.
    /// Every subagent therefore resolves back to the root session.
    pub process_scope: Option<fuigo_tty_utils::ProcessScope>,
    ...
    pub sampling_config: fuigo_sampler::SamplerConfig,
    ...
    pub auth: Option<crate::auth::FuigoAuth>,
    ...
    pub inherited_tool_overrides: Option<fuigo_sampling_types::ToolOverrides>,
    pub yolo_mode: bool,
    ...
    pub parent_terminal_backend: Option<Arc<dyn fuigo_tools::computer::types::TerminalBackend>>,
    ...
    pub web_search_sampling_config: Option<fuigo_sampler::SamplerConfig>,
    pub web_fetch_config: fuigo_tools::implementations::fuigo_build::web_fetch::WebFetchConfig,
    pub image_gen_config: fuigo_tools::implementations::fuigo_build::image_gen::ImageGenConfig,
    pub video_gen_config: fuigo_tools::implementations::fuigo_build::video_gen::VideoGenConfig,
    ...
    pub parent_chat_state: Option<fuigo_chat_state::ChatStateHandle>,
    ...
    pub file_tool_overrides: Option<Vec<fuigo_tools::registry::types::ToolConfig>>,
    ...
    pub permission_handle: Option<fuigo_workspace::permission::PermissionHandle>,
    pub worktree_type: crate::util::config::WorktreeType,
    pub api_key_provider: Option<fuigo_tools::types::SharedApiKeyProvider>,
    ...
    pub attribution_callback: Option<fuigo_sampler::SharedAttributionCallback>,
    /// Parent session's agent name (e.g. "fuigo-build").
    ...
    pub parent_tool_definitions: Option<Vec<fuigo_sampling_types::ToolSpec>>,
    /// Pre-discovered skills from the parent session, captured at spawn time.
    pub parent_skills: Option<Vec<fuigo_tools::implementations::skills::types::SkillInfo>>,
    /// Parent's skills config for the child's SkillManager.
    pub parent_skills_config: fuigo_agent::prompt::skills::SkillsConfig,
    ...
    pub parent_compat: fuigo_tools::types::compat::CompatConfig,
...
pub(crate) fn strip_ask_user_question_tool(tools: &mut Vec<fuigo_sampling_types::ToolSpec>) {
    tools.retain(|tool| tool.name != "ask_user_question");
}
pub(crate) fn strip_workflow_tool(tools: &mut Vec<fuigo_sampling_types::ToolSpec>) {
    tools
        .retain(|tool| !fuigo_tools::implementations::fuigo_build::is_workflow_tool_id(&tool.name));
}
impl SubagentSpawnContext {
    /// Would installing a live bearer resolver strip this subagent's only credential?
    /// A wired resolver is the sampler's sole auth source, so with no session key at spawn it must not displace a fallback key (env `FUIGO_API_KEY`).
    ...
    fn apply_session_cli_overrides(&self, def: &mut fuigo_agent::config::AgentDefinition) {
    ...
    pub(crate) fn snapshot_parent_compaction_pins(
        resolved_mode: fuigo_chat_state::CompactionMode,
        ...
        let current = parent_agent_name
            .and_then(|name| fuigo_agent::discovery::by_name_in_cwd(name, parent_cwd))
    ...
    pub(crate) fn compaction_pins_for_child(
        &self,
        child_template: &fuigo_agent::prompt::user_message::UserMessageTemplate,
    ) -> crate::session::CompactionPins {
        crate::session::cursor_compaction_pins(
    ...
    pub(crate) fn resolve_compaction_mode(&self) -> fuigo_chat_state::CompactionMode {
        crate::agent::config::resolve_compaction_mode_from(
            crate::agent::config::env_string("FUIGO_COMPACTION_MODE").as_deref(),
            self.agent_config
                .as_ref()
        ...
        .with_segment_detail(crate::agent::config::resolve_compaction_detail_from(
            crate::agent::config::env_string("FUIGO_COMPACTION_DETAIL").as_deref(),
            self.agent_config
                .as_ref()
...
async fn resolve_subagent_sampling_config(
    agent_name: &str,
    agent_model: &fuigo_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (fuigo_sampler::SamplerConfig, acp::ModelId) {
...
async fn resolve_effective_model_config(
    ...
    subagent_type: &str,
    definition_model: &fuigo_agent::config::ModelOverride,
    ctx: &SubagentSpawnContext,
) -> (fuigo_sampler::SamplerConfig, acp::ModelId) {
...
fn log_subagent_model_resolution(
    agent_name: &str,
    priority: &str,
    resolved: &fuigo_sampler::SamplerConfig,
...
fn session_bearer_resolver(
    ...
    base_url: &str,
) -> Option<fuigo_sampler::SharedBearerResolver> {
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let mut inherited = fuigo_sampler::SamplerConfig {
                ...
                supports_backend_search,
                programmatic_tool_calling: ctx
                    .models_manager
                    .model_programmatic_tool_calling(catalog_model_id.0.as_ref()),
...
fn resolve_model_override_to_config(
    ...
    config.bearer_resolver = if !ctx.would_strip_fallback_key(config.api_key.as_deref())
        && resolved_auth_type == fuigo_chat_state::AuthType::SessionToken
    {
...
struct InitialContext {
    ...
    conversation: Vec<fuigo_sampling_types::conversation::ConversationItem>,
    force_compact: bool,
...
fn forked_initial_context(
    mut items: Vec<fuigo_sampling_types::conversation::ConversationItem>,
) -> InitialContext {
    ...
    let (conversation, prefix_len) =
        fuigo_subagent_resolution::context::normalize_forked_context(items);
    if prefix_len < 2 {
        return InitialContext {
            source: InitialContextSource::New,
...
fn conversation_tail_is_complete(
    items: &[fuigo_sampling_types::conversation::ConversationItem],
) -> bool {
    matches!(
        items.last(),
...
fn verbatim_or_normalize_fork(
    items: Vec<fuigo_sampling_types::conversation::ConversationItem>,
    ...
    let estimated_tokens = fuigo_chat_state::estimate_conversation_tokens(&items);
    ...
    let (conversation, prefix_len) =
        fuigo_subagent_resolution::context::normalize_forked_context(filtered);
    InitialContext {
        source: InitialContextSource::Forked,
        copy_error: None,
        prefix_len: Some(prefix_len),
            ...
            .await
        {
            Ok(result) => {
                ...
                let estimated_tokens =
                    fuigo_chat_state::estimate_conversation_tokens(&conversation);
...
fn resolve_inherited_mcp_pool(
    ...
    inheritance: &fuigo_agent::config::McpInheritance,
...
fn resolve_agent_definition(
    subagent_type: &str,
    ctx: &SubagentSpawnContext,
) -> Option<fuigo_agent::config::AgentDefinition> {
    let cli_agents = ctx
        .agent_config
        .as_ref()
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    let resolution_context = fuigo_subagent_resolution::DefinitionResolutionContext {
        cwd: &ctx.parent_cwd,
    ...
    let mut def =
        fuigo_subagent_resolution::discover_agent_definition(subagent_type, &resolution_context)?;
...
fn available_agent_names(ctx: &SubagentSpawnContext) -> Vec<String> {
    let cli_agents = ctx
        ...
        .map(|config| config.cli_agents.as_slice())
        .unwrap_or_default();
    fuigo_subagent_resolution::available_agent_names(
        &fuigo_subagent_resolution::DefinitionResolutionContext {
            cwd: &ctx.parent_cwd,
            plugins: ctx.plugin_registry.as_deref(),
            cli_agents,
            toggles: &ctx.subagent_toggle,
...
pub(crate) struct SubagentValidationContext {
    pub parent_cwd: PathBuf,
    pub plugin_registry: Option<Arc<fuigo_agent::plugins::PluginRegistry>>,
...
pub(crate) fn validate_subagent_type(
    ...
    let context = fuigo_subagent_resolution::DefinitionValidationContext {
        cwd: &ctx.parent_cwd,
        plugins: ctx.plugin_registry.as_deref(),
    ...
    match fuigo_subagent_resolution::validate_agent_name(subagent_type, &context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(fuigo_subagent_resolution::ResolutionError::Unknown { available, .. }) => {
            SubagentValidateTypeOutcome::Unknown { available }
        }
        Err(fuigo_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        Err(fuigo_subagent_resolution::ResolutionError::NotAllowed { allowed, .. }) => {
            SubagentValidateTypeOutcome::NotAllowed { allowed }
        }
        Err(
            fuigo_subagent_resolution::ResolutionError::PersonaResolution(_)
            | fuigo_subagent_resolution::ResolutionError::ResumeValidation(_),
...
fn gate_subagent_type(
    ...
    match fuigo_subagent_resolution::gate_agent_definition(subagent_type, &resolution_context) {
        Ok(()) => SubagentValidateTypeOutcome::Ok,
        Err(fuigo_subagent_resolution::ResolutionError::Disabled { .. }) => {
            SubagentValidateTypeOutcome::Disabled
        }
        ...
        Err(
            fuigo_subagent_resolution::ResolutionError::Unknown { .. }
            | fuigo_subagent_resolution::ResolutionError::PersonaResolution(_)
            | fuigo_subagent_resolution::ResolutionError::ResumeValidation(_),
        ) => SubagentValidateTypeOutcome::ValidationUnavailable,
    }
...
pub(crate) fn subagent_harness_flavor_is_representable(agent_type: &str) -> bool {
    fuigo_subagent_resolution::subagent_harness_flavor_is_representable(agent_type)
}
/// Apply the harness-dependent toolset/prompt re-selection to a resolved agent definition.
///
/// The harness flavor (alternate vs fuigo-build) normally follows the PARENT: `FuigoBuildOrchestrator` parents give children the alternate harness.
...
/// So a fuigo-build session can run an alternate-harness verifier and vice-versa.
...
fn resolve_subagent_toolset(
    ...
    ctx: &SubagentSpawnContext,
    definition: &mut fuigo_agent::config::AgentDefinition,
) {
    let resolution_context = fuigo_subagent_resolution::HarnessToolsetContext {
    ...
    fuigo_subagent_resolution::apply_harness_toolset(
        subagent_type,
        &resolution_context,
        definition,
    );
...
fn resolve_subagent_permission_mode(
    requested: fuigo_agent::config::PermissionMode,
    is_plugin: bool,
    policy_block: Option<&'static str>,
) -> fuigo_agent::config::PermissionMode {
    if is_plugin {
...
fn resolve_subagent_source_repo(ctx: &SubagentSpawnContext) -> std::path::PathBuf {
    ...
    fuigo_workspace::session::git::find_main_repo_root_from_path(&source_cwd).unwrap_or(source_cwd)
}
...
fn cancellation_error_message(
    category: Option<fuigo_session_events::types::CancellationCategory>,
...
fn telemetry_owner_kind(request: &SubagentRequest) -> fuigo_telemetry::events::SubagentOwnerKind {
    if request.owner.is_workflow() {
        fuigo_telemetry::events::SubagentOwnerKind::Workflow
    } else if request.from_scheduler_loop() {
        fuigo_telemetry::events::SubagentOwnerKind::SchedulerLoop
    } else {
        fuigo_telemetry::events::SubagentOwnerKind::Task
    }
}
    ...
    tokio_util::task::AbortOnDropHandle::new(tokio::spawn(async move {
        ...
        loop {
            ...
            if let Some(ref cmd_tx) = parent_cmd_tx {
                let _ = cmd_tx.send(SessionCommand::FuigoSessionNotification { notification });
            }
...
pub(crate) async fn reconcile_orphaned_subagents_with_backend(
    ...
    backend: &fuigo_tools::implementations::fuigo_build::task::backend::ChannelBackend,
    session_dir: &Path,
    parent_session_id: &str,
    gateway: &GatewaySender,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
