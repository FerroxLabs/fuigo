...
fn build_worker_runtime() -> std::io::Result<tokio::runtime::Runtime> {
    let workers = std::env::var("FUIGO_SUBAGENT_WORKER_THREADS")
        .ok()
        ...
        Box::pin(async move {
            ...
            match this.try_build_subagent_spawn_context(&parent_session_id) {
                ...
                None => {
                    ...
                    fuigo_tools::implementations::fuigo_build::task::types::SubagentDescribeOutcome::Unavailable
                }
            }
        })
    }
...
fn log_limit_notice(notice: coordinator::SubagentLimitNotice) {
    ...
    fuigo_telemetry::session_ctx::log_event(SubagentLimitHit::session_concurrent(
...
pub(crate) fn spawn_subagent_coordinator(
    ...
    let config = coordinator::CoordinatorConfig {
        foreground_budget:
            fuigo_tools::implementations::fuigo_build::task::backend::env_duration_or(
                "FUIGO_SUBAGENT_AWAIT_BUDGET_MS",
...
pub(crate) struct InjectParams<'a> {
    ...
    pub task_completion_reservations:
        &'a Option<fuigo_tools::reminders::task_completion::TaskCompletionReservations>,
...
pub(crate) fn inject_subagent_completed_prompt(params: InjectParams) {
    ...
    let summary =
        fuigo_tools::implementations::fuigo_build::task::completion_summary(request, result);
    let message = fuigo_tools::reminders::task_completion::format_subagent_completion(
        &summary,
...
pub(crate) fn emit_subagent_notification(
    ...
    if let Some(cmd_tx) = parent_cmd_tx {
        let _ = cmd_tx.send(SessionCommand::FuigoSessionNotification {
