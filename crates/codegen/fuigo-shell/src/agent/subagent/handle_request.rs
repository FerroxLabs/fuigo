...
use fuigo_telemetry::subagent_spawn::{SubagentSpawnPhase, phase_region};
use fuigo_telemetry::{instrument_task, region};
use fuigo_tools::implementations::{fuigo_build, opencode};
...
pub(super) async fn reparent_surviving_child_tasks(
    parent_tb: &std::sync::Arc<dyn fuigo_tools::computer::types::TerminalBackend>,
    parent_notif_handle: &fuigo_tools::notification::types::ToolNotificationHandle,
    parent_cmd_tx: Option<&mpsc::UnboundedSender<SessionCommand>>,
...
pub(crate) async fn run_shell_child(
    run: fuigo_build::task::coordinator::ChildRunRequest<ShellChildRuntime>,
    mut ctx: SubagentSpawnContext,
    gateway: GatewaySender,
) -> ChildRunOutput<ShellCompletionData> {
    let fuigo_build::task::coordinator::ChildRunRequest {
        mut request,
    ...
    let spawn_timer = fuigo_telemetry::subagent_spawn::SubagentSpawnTimer::new_shared();
    ...
    let mut effective_runtime = fuigo_subagent_resolution::resolve_runtime_config(
        &request.subagent_type,
    ...
    if let Some(ref source) = resume_source {
        ...
        if let Err(e) = fuigo_subagent_resolution::validate_resume_identity(
            &request.subagent_type,
    ...
    let worktree_path = if let Some(ref source) = resume_source {
    ...
    } else if effective_runtime.isolation != fuigo_tool_types::SubagentIsolationMode::None {
        ...
        let dest = match crate::session::worktree::worktree_base_dir_for_source(&source_cwd) {
            ...
            Err(e) => {
                ...
                std::env::temp_dir()
                    .join("fuigo-subagent-worktrees")
                    .join(&request.id)
            }
        };
        let source_clone = source_cwd;
        let subagent_id = request.id.clone();
        let creation_mode: fuigo_fast_worktree::CreationMode = ctx.worktree_type.into();
        ...
        let created = match tokio::task::spawn_blocking(move || {
            let mut builder = fuigo_fast_worktree::WorktreeBuilder::new(&source_clone, &dest)
                .working_tree_mode(fuigo_fast_worktree::WorkingTreeMode::PreserveWorkingTree)
                .creation_mode(creation_mode)
                .worktree_kind(fuigo_fast_worktree::WorktreeKind::Subagent)
    ...
    effective_runtime.capability_mode = fuigo_subagent_resolution::intersect_capability_modes(
    ...
    fuigo_subagent_resolution::apply_child_tool_policy(
        &mut definition,
        effective_runtime.capability_mode,
    ...
    if let Err(e) = crate::util::fuigo_home::ensure_sessions_cwd_dir(&child_session_info.cwd) {
    ...
    {
        let reminder = fuigo_sampling_types::conversation::ConversationItem::system_reminder(
    ...
    let covered_by_parent = fuigo_fsnotify::watch_root_covers(&ctx.parent_cwd, &child_cwd);
    ...
    let child_cwd_abs = fuigo_paths::AbsPathBuf::new(child_cwd).unwrap_or_else(|_| {
        fuigo_paths::AbsPathBuf::new(std::env::current_dir().unwrap_or_default())
    ...
    let parent_traceparent = fuigo_file_utils::trace_context::current_traceparent();
    let tracker_child_cwd = child_session_info.cwd.clone();
    let tracker_model_id = effective_model_id.0.to_string();
    let initial_child_tokens = fuigo_chat_state::estimate_conversation_tokens(&forked_conversation);
    ...
    if let Some(scope) = agent_memory_scope {
        let memory_tools: Vec<fuigo_tools::registry::types::ToolConfig> = vec![
            ...
            && let Ok(content) = std::fs::read_to_string(&memory_md)
        {
            ...
            let truncated: String = content
                ...
                .join("\n");
            let truncated =
                fuigo_tools::util::truncate::truncate_str(&truncated, MAX_BYTES).to_string();
    ...
    let agent_mcp_servers: Vec<_> = if definition.mcp_servers.is_empty() {
        ...
        definition
                ...
                .filter_map(|entry| match entry {
                    fuigo_agent::config::McpServerRef::Named(name) => {
    ...
    if inherit_skills && ctx.parent_skills.is_none() {
        ...
        ctx.parent_skills = Some(
            fuigo_agent::prompt::skills::list_skills_with_plugins(
                Some(&parent_cwd_str),
    ...
    let _active = fuigo_telemetry::activity::SUBAGENTS_ACTIVE.enter();
    debug_assert!(
        fuigo_telemetry::activity::SUBAGENTS_ACTIVE.get() >= 1,
        "SubagentLaunched must stamp a self-inclusive count"
    );
    fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::SubagentLaunched {
    ...
    let spawn_result = session::spawn_session_on_thread(
        ...
        ctx.yolo_mode
            || matches!(
                ...
                fuigo_agent::config::PermissionMode::BypassPermissions
            ),
        false,
        None,
        ...
        Vec::new(),
        fuigo_agent::prompt::context::PromptAudience::Subagent,
        ...
        false,
        Some(fuigo_telemetry::subagent_spawn::SpawnPhaseContext {
    ...
    result.tokens_used = trace_token_totals
        .as_ref()
        .map(fuigo_chat_state::UsageTotals::total_tokens)
        .unwrap_or(0);
    ...
    let outcome = if result.success {
        fuigo_telemetry::events::Outcome::Completed
    } else if result.cancelled {
        fuigo_telemetry::events::Outcome::Cancelled
    } else {
        fuigo_telemetry::events::Outcome::Error
    };
    let mut completed = fuigo_telemetry::events::SubagentCompleted {
        subagent_id: request.id.clone(),
        parent_session_id: request.parent_session_id.clone(),
        owner: telemetry_owner_kind(&request),
        workflow_run_id: request.owner.workflow_run_id().map(str::to_string),
    ...
    fuigo_telemetry::session_ctx::log_event(completed);
    match (
...
pub(crate) async fn dispose_worktree_after_completion(
    ...
    let ref_name = format!("refs/fuigo/subagents/{subagent_id}");
    ...
    let reclaim = tokio::task::spawn_blocking(move || {
        fuigo_fast_worktree::reclaimable_after_snapshot(
            &checked_path,
    ...
    reclaim_span.close();
    match reclaim {
        Ok(fuigo_fast_worktree::Reclaim::Now { .. }) => {}
        Ok(fuigo_fast_worktree::Reclaim::Keep(reason)) => {
            tracing::info!(
                subagent_id = %subagent_id,
                worktree_path = %worktree.display(),
                %reason,
        ...
        Ok(fuigo_fast_worktree::Reclaim::Unnamed(error)) => {
            tracing::warn!(
                subagent_id = %subagent_id,
                worktree_path = %worktree.display(),
                %error,
