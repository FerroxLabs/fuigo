...
use fuigo_subagent_resolution::resolve_effective_overrides;
...
fn canonical_total_tokens_does_not_double_count_reasoning() {
    let totals = fuigo_chat_state::UsageTotals {
        input_tokens: 100,
        output_tokens: 40,
        reasoning_tokens: 25,
        ..Default::default()
...
async fn usage_ack_precedes_terminal_presentation() {
    ...
    assert!(matches!(
            ...
            Ok(SessionCommand::FuigoSessionNotification {
...
impl fuigo_tools::computer::types::TerminalBackend for StarvedTerminal {
    async fn run(
        &self,
        _request: fuigo_tools::computer::types::TerminalRunRequest,
    ) -> Result<
        fuigo_tools::computer::types::TerminalRunResult,
        fuigo_tools::computer::types::ComputerError,
    > {
    ...
    async fn get_task(
        &self,
        _task_id: &str,
    ) -> Option<fuigo_tools::computer::types::TaskSnapshot> {
        std::future::pending().await
    }
    async fn kill_task(
        &self,
        _task_id: &str,
    ) -> fuigo_tools::computer::types::KillOutcome {
        std::future::pending().await
    }
    ...
    async fn list_tasks(&self) -> Vec<fuigo_tools::computer::types::TaskSnapshot> {
        std::future::pending().await
    }
    async fn reparent_notifications(
        ...
        _new_handle: fuigo_tools::notification::types::ToolNotificationHandle,
        _backend_weak: std::sync::Weak<
            dyn fuigo_tools::computer::types::TerminalBackend,
        >,
    ) {
        std::future::pending().await
    }
...
async fn reparent_is_bounded_when_terminal_actor_never_answers() {
    ...
    let notif_handle = fuigo_tools::notification::types::ToolNotificationHandle::noop();
    let (cmd_tx, mut cmd_rx) = mpsc::unbounded_channel();
    tokio::time::timeout(
            20 * PARENT_ACK_TIMEOUT,
...
async fn subagent_inherits_session_cli_overrides() {
    use fuigo_agent::config::{AgentDefinition, PermissionMode};
...
fn subagent_bypass_permission_mode_gated_by_policy_pin() {
    use fuigo_agent::config::PermissionMode;
    const PIN: &str = fuigo_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
    assert_eq!(
...
async fn emit_subagent_notification_stamps_one_event_id_on_both_paths() {
    ...
    let persisted_id = match cmd_rx.try_recv().expect("persist hop must fire") {
        SessionCommand::FuigoSessionNotification { notification } => {
            notification
                .meta
                .as_ref()
        ...
        _ => panic!("expected FuigoSessionNotification"),
    };
...
fn resume_worktree_action_covers_three_outcomes() {
    ...
    assert_eq!(
            resume_worktree_action(true, Some("refs/fuigo/subagents/x")),
            ResumeWorktreeAction::Rehydrate
        );
    assert_eq!(
            resume_worktree_action(false, Some("refs/fuigo/subagents/x")),
            ResumeWorktreeAction::Rehydrate
        );
    assert_eq!(
...
fn inject_subagent_completed_prompt_sends_prompt_and_marks_delivered() {
    ...
    let reservations = fuigo_tools::reminders::task_completion::TaskCompletionReservations::default();
...
fn partial_override_fills_from_role() {
    ...
    let role = fuigo_subagent_resolution::config::SubagentRole {
    ...
    assert_eq!(
            ...
            Some(fuigo_tool_types::SubagentCapabilityMode::Execute)
        );
}
#[test]
...
fn persona_resolved_from_config() {
    ...
    personas
        .insert(
            ...
            fuigo_subagent_resolution::config::SubagentPersona {
...
fn model_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_model = fuigo_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
...
fn reasoning_effort_precedence_explicit_over_role_over_persona() {
    ...
    let role_no_re = fuigo_subagent_resolution::config::SubagentRole {
        description: "test".into(),
        ..Default::default()
    };
    let r = resolve_effective_overrides(
...
fn forked_initial_context_inherits_parent_across_reasoning() {
    ...
    let items = vec![
            ...
            ConversationItem::Reasoning(fuigo_sampling_types::synthesized_reasoning_item(
                "deliberating",
            )),
...
fn verbatim_fork_falls_back_to_summary_when_oversize() {
    use fuigo_sampling_types::conversation::{ContentPart, ConversationItem};
    let items = vec![
            ConversationItem::system("parent system"),
...
fn summarize_tool_config_uses_name_override_and_strips_namespace() {
    ...
    let mut read = ToolConfig::from_id("FuigoBuild:read_file");
...
/// Regression guard for the DEFAULT fuigo-build host, the primary `/goal` host.
...
fn subagent_keeps_default_flavor_when_parent_model_is_non_strict() {
    ...
    ctx.parent_model_agent_type = Some(
        BuiltinAgentName::FuigoBuildPlan.as_ref().to_string(),
    );
...
async fn cancel_pending_at_promote_removes_fresh_worktree_preserves_resumed() {
    ...
    fuigo_fast_worktree::WorktreeBuilder::new(&repo, &fresh)
        .standalone(true)
        .create()
        .unwrap();
    assert!(fresh.exists());
    ...
    fuigo_fast_worktree::WorktreeBuilder::new(&repo, &resumed)
        .standalone(true)
        .create()
        .unwrap();
...
fn subagent_auth_type_rule() {
    use crate::agent::auth_method::{CACHED_TOKEN_AUTH_METHOD_ID, FUIGO_API_KEY_METHOD_ID};
    use fuigo_chat_state::AuthType;
    let session = acp::AuthMethodId::new(CACHED_TOKEN_AUTH_METHOD_ID);
    let api_key = acp::AuthMethodId::new(FUIGO_API_KEY_METHOD_ID);
    let byok = byok_model_entry("fuigo-byok");
    let plain = test_model_entry("fuigo-plain");
    assert_eq!(
...
fn fresh_tool_model_rejects_unknown_and_nonavailable_entries() {
    ...
    for requested in [
        ...
        assert!(!error.contains("fuigo models"));
    }
    assert!(
            super::handle_request::task_model_override_error(
                Some("oauth-only"),
...
fn test_sampling_config(model_slug: &str) -> fuigo_sampling_types::SamplingConfig {
...
fn spawn_test_parent_chat_state(model_slug: &str) -> fuigo_chat_state::ChatStateHandle {
    let (mock, _persistence_rx) = fuigo_chat_state::MockChatPersistence::new();
