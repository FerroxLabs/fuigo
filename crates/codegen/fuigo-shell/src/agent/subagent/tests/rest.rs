...
use fuigo_tools::implementations::fuigo_build::task::backend::ChannelBackend;
#[test]
fn normalize_forked_context_strips_project_layout() {
    ...
    let (conv, _) = fuigo_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn normalize_forked_context_consecutive_users() {
    ...
    let (conv, prefix_len) = fuigo_subagent_resolution::context::normalize_forked_context(
        items,
    );
...
fn end_to_end_normalized_conversation_shape() {
    ...
    let (mut conv, prefix_len) = fuigo_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
    assert_eq!(prefix_len, 2);
...
fn last_user_message_is_task_after_normalization() {
    ...
    let (mut conv, _) = fuigo_subagent_resolution::context::normalize_forked_context(
        parent_conv,
    );
        ...
        .find_map(|item| {
            if let ConversationItem::User(u) = item {
                let text: String = u
                    ...
                    .iter()
                    .filter_map(|p| match p {
                        fuigo_sampling_types::conversation::ContentPart::Text {
                            text,
                        } => Some(text.as_ref()),
                        _ => None,
                    })
            ...
            if let ConversationItem::User(u) = i {
                u.content
                    ...
                    .any(|p| {
                        matches!(
                    p,
                    fuigo_sampling_types::conversation::ContentPart::Text { text } if text.contains("<background_context>")
                )
                    })
            } else {
                false
...
fn snapshot_ref_field_in_meta_roundtrips() {
    let meta = SubagentMeta {
        ...
        tool_calls: Some(1),
        turns: Some(1),
        worktree_path: Some("/tmp/fuigo-wt/sa-snap".into()),
        snapshot_ref: Some("refs/fuigo/subagent-snapshots/sa-snap".into()),
        ..base_meta()
    };
    let json = serde_json::to_string(&meta).unwrap();
    assert!(json.contains("snapshot_ref"));
    assert!(json.contains("refs/fuigo/subagent-snapshots/sa-snap"));
    let parsed: SubagentMeta = serde_json::from_str(&json).unwrap();
    assert_eq!(
            parsed.snapshot_ref.as_deref(),
            Some("refs/fuigo/subagent-snapshots/sa-snap")
        );
}
#[test]
...
fn snapshot_test_meta(id: &str) -> SubagentMeta {
    SubagentMeta {
        ...
        tool_calls: Some(0),
        turns: Some(1),
        worktree_path: Some("/tmp/fuigo-wt/subagent-x".into()),
        ..base_meta()
    }
}
...
fn update_subagent_meta_snapshot_ref_persists_to_disk() {
    ...
    assert_eq!(
            ...
            Some("refs/fuigo/subagents/sa-write")
        );
    assert_eq!(reread.status, "completed");
    assert_eq!(
            reread.worktree_path.as_deref(),
            Some("/tmp/fuigo-wt/subagent-x")
        );
}
...
fn update_subagent_meta_snapshot_ref_reports_failure_when_meta_missing() {
    ...
    assert!(!update_subagent_meta_snapshot_ref(
            dir.path(),
            "refs/fuigo/subagents/sa-missing",
            "completed"
        ));
}
...
fn snapshot_ref_write_promotes_nonterminal_status_to_terminal() {
    ...
    assert!(update_subagent_meta_snapshot_ref(
            dir.path(),
            "refs/fuigo/subagents/x",
            "completed"
        ));
    let data = std::fs::read_to_string(dir.path().join("meta.json")).unwrap();
    let reread: SubagentMeta = serde_json::from_str(&data).unwrap();
    assert_eq!(
            Some("refs/fuigo/subagents/x"),
            reread.snapshot_ref.as_deref()
        );
...
async fn disposed_linked_worktree_persists_the_pointer_then_removes_the_directory() {
    fuigo_test_utils::require_git!();
    use fuigo_test_utils::git::seed_repo_with_remote;
    let temp = tempfile::TempDir::new().unwrap();
    let (repo, _remote) = seed_repo_with_remote(temp.path());
    let wt = temp.path().join("subagent-reclaim-1");
    fuigo_fast_worktree::WorktreeBuilder::new(&repo, &wt).create().unwrap();
    let meta_dir = temp.path().join("meta");
    ...
    let disposal = crate::agent::subagent::handle_request::dispose_worktree_after_completion(
        ...
        .await;
    let snapshot_ref = "refs/fuigo/subagents/reclaim-1";
...
async fn disposal_names_a_reflog_only_commit_before_removing_the_worktree() {
    fuigo_test_utils::require_git!();
    use fuigo_test_utils::git::{reflog_only_commit, run_git, seed_repo_with_remote};
    let temp = tempfile::TempDir::new().unwrap();
    let (repo, _remote) = seed_repo_with_remote(temp.path());
...
fn resume_source_worktree_reuse() {
    let source_with_worktree = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.fuigo/worktrees/myrepo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
    ...
    assert_eq!(
            worktree.as_deref(),
            Some(Path::new(
                "/home/user/.fuigo/worktrees/myrepo/subagent-sub-wt",
            )),
...
fn resume_inherited_cwd_requires_existing_non_worktree_dir() {
    ...
    let missing = ResumeSourceData {
        child_cwd: "/no/such/dir/fuigo-missing".into(),
        ..present.clone()
    };
...
fn select_override_cwd_resume_never_falls_through_to_request_cwd() {
    let source = ResumeSourceData {
        ...
        worktree_path: Some(
            PathBuf::from("/home/user/.fuigo/worktrees/repo/subagent-sub-wt"),
        ),
        snapshot_ref: None,
        subagent_type: "general-purpose".into(),
        persona: None,
...
fn token_estimation_for_window_safety() {
    ...
    let estimated = fuigo_chat_state::estimate_conversation_tokens(&conversation);
    assert!(estimated > 0, "should produce non-zero estimate");
    assert!(
            estimated < 100,
            "short conversation should have small token estimate"
        );
    assert_eq!(fuigo_chat_state::estimate_conversation_tokens(&[]), 0);
}
#[test]
fn token_estimation_accounts_for_images() {
    ...
    let text_tokens = fuigo_chat_state::estimate_conversation_tokens(&text_only);
    ...
    let image_tokens = fuigo_chat_state::estimate_conversation_tokens(&with_image);
    assert_eq!(
            image_tokens,
            text_tokens + 765,
...
fn durable_fallback_roundtrips_child_cwd_and_worktree() {
    ...
    let meta = SubagentMeta {
        ...
        worktree_path: Some("/tmp/fuigo-wt/sa-dur".into()),
    ...
    assert_eq!(loaded.worktree_path.as_deref(), Some("/tmp/fuigo-wt/sa-dur"));
...
fn durable_fallback_rejects_running_status() {
    let dir = std::env::temp_dir()
        .join("fuigo-test-durable-status")
        .join(uuid::Uuid::now_v7().to_string());
...
fn drain_cancelled_finish_cmds(
    ...
    while let Ok(cmd) = cmd_rx.try_recv() {
        if let SessionCommand::FuigoSessionNotification { notification } = cmd
...
fn drain_cancelled_finish_broadcasts(
    ...
    while let Ok(msg) = gateway_rx.try_recv() {
        ...
        assert_eq!(args.request.method.as_ref(), "fuigo/session_notification");
...
async fn reconcile_reemits_shared_actor_terminal_outcome() {
    ...
    let finish = std::iter::from_fn(|| cmd_rx.try_recv().ok())
        .find_map(|command| {
            let SessionCommand::FuigoSessionNotification { notification } = command else {
                return None;
            };
    ...
    assert_eq!(
            std::iter::from_fn(|| cmd_rx.try_recv().ok())
                .filter(|command| matches!(
                    command,
                    SessionCommand::FuigoSessionNotification {
...
async fn live_reconcile_persists_failed_and_cancelled_inspection() {
    ...
    for (status, expected) in cases {
        ...
        let finish = std::iter::from_fn(|| cmd_rx.try_recv().ok())
            .find_map(|command| {
                let SessionCommand::FuigoSessionNotification { notification } = command
                else {
                    return None;
                };
                let SessionUpdate::SubagentFinished { status, will_wake, .. } = notification
...
fn resume_identity_does_not_gate_on_model() {
    ...
    assert!(
            fuigo_subagent_resolution::validate_resume_identity(
                "general-purpose",
                None,
                &source,
            )
...
fn durable_meta_roundtrips_effective_model_id() {
    let dir = std::env::temp_dir()
        .join("fuigo-test-model-roundtrip")
        .join(uuid::Uuid::now_v7().to_string());
    let _ = std::fs::create_dir_all(&dir);
    let meta = SubagentMeta {
...
fn resume_model_pinning_overrides_default_resolution() {
    ...
    let resolved_model = "fuigo-light";
...
async fn read_parent_sampling_config_keeps_auto_when_catalog_has_slug_key_only() {
    ...
    ctx.parent_chat_state
        .as_ref()
        .unwrap()
        .update_sampling_config(fuigo_sampling_types::SamplingConfig {
...
async fn read_parent_sampling_config_live_never_strips_a_fallback_key() {
    ...
    chat.update_credentials(fuigo_chat_state::Credentials {
        api_key: Some("fuigo-env-fallback".to_string()),
    ...
    assert_eq!(config.api_key.as_deref(), Some("fuigo-env-fallback"));
}
/// `would_strip_fallback_key` on the inherit-fallback path.
/// The baseline keeps the env `FUIGO_API_KEY` even while `auth_type` flips to `SessionToken`, and no resolver may displace it.
#[tokio::test]
async fn read_parent_sampling_config_fallback_never_strips_a_fallback_key() {
    ...
    ctx.sampling_config.model = "grok-4.5".to_string();
    ctx.sampling_config.base_url = "https://api.x.ai/v1".to_string();
    ctx.sampling_config.api_key = Some("fuigo-env-fallback".to_string());
    let (config, _) = read_parent_sampling_config(&ctx).await;
...
/// `FUIGO_API_KEY`'s presence varies by environment, so assert the rule itself rather than one branch of it.
#[test]
...
async fn subagent_override_provider_model_spawns_cache_only_credentials() {
    ...
    ctx.model_id = acp::ModelId::new("grok-4.5");
    ctx.available_models = models;
    ctx.auth = Some(crate::auth::FuigoAuth {
...
fn non_cursor_persona_injected_as_system_reminder() {
    use fuigo_sampling_types::conversation::{ConversationItem, SyntheticReason};
...
fn filter_inheritance_all_passes_everything_through() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &fuigo_agent::config::McpInheritance::All,
    );
...
fn filter_inheritance_none_returns_none() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &fuigo_agent::config::McpInheritance::None,
    );
...
fn filter_inheritance_named_selects_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &fuigo_agent::config::McpInheritance::Named(
...
fn filter_inheritance_except_excludes_specific_servers() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &fuigo_agent::config::McpInheritance::Except(
...
fn filter_inheritance_named_empty_list_gives_empty_pool() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &fuigo_agent::config::McpInheritance::Named(vec![]),
    );
...
fn filter_inheritance_except_empty_list_keeps_all() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &fuigo_agent::config::McpInheritance::Except(vec![]),
    );
    let result = result.expect("Except([]) should return Some");
...
fn filter_inheritance_except_nonexistent_servers_ignored() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &fuigo_agent::config::McpInheritance::Except(vec!["nonexistent".into()]),
    );
    let result = result.expect("Except should return Some");
    assert_eq!(pool_names(&result), vec!["github", "linear"]);
}
...
fn filter_inheritance_named_all_nonexistent_gives_empty() {
    ...
    let result = super::filter_pool_by_inheritance(
        pool,
        &fuigo_agent::config::McpInheritance::Named(vec!["foo".into(), "bar".into()]),
    );
    let result = result.expect("Named should return Some");
    assert_eq!(result.server_names().count(), 0);
}
...
fn resolve_inherited_pool_named_filters() {
    ...
    let result = super::resolve_inherited_mcp_pool(
            Some(pool),
            &fuigo_agent::config::McpInheritance::Named(vec!["atlassian".into()]),
        )
...
fn make_test_skill(
    ...
    fuigo_tools::implementations::skills::types::SkillInfo {
        ...
        scope: fuigo_tools::implementations::skills::types::SkillScope::Local,
        enabled: true,
        user_invocable: true,
    ...
    local
        .run_until(async {
            ...
            let cmd = tokio::time::timeout(
                ...
                .expect("channel open");
            cancel.cancel();
            let SessionCommand::FuigoSessionNotification { notification } = cmd else {
                panic!("expected FuigoSessionNotification");
            };
