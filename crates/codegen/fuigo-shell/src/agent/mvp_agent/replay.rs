...
type ReplayCompletionRx = tokio::sync::oneshot::Receiver<fuigo_acp_lib::AcpResult<()>>;
...
impl MvpAgent {
    ...
    /// - Ferrox Labs updates (`"_fuigo/session/update"`) become an `ExtNotification`.
    ///
    ...
    pub(super) fn forward_raw_replay_line(
        ...
        // updates.jsonl only persists `_fuigo/session/update` and `session/update`.
        ...
        let is_fuigo = method == "_fuigo/session/update";
        ...
        if is_fuigo {
            ...
            let Ok(mut params) = serde_json::from_str::<serde_json::Value>(raw_params.get()) else {
                tracing::debug!("replay: skipping Ferrox Labs update with unparseable params");
                return None;
            };
            if let Some(obj) = params.as_object_mut() {
                ...
                if let Some(m) = meta.as_object_mut() {
                    ...
                    if let Some(pd) = persist_data {
                        m.insert("fuigo/persist".to_string(), pd.clone());
                    }
                    if let Some(tid) = target_client_id {
                        m.insert("fuigo/leaderClientId".to_string(), tid.clone());
                    }
                }
            }
        ...
        if let Some(tid) = target_client_id {
            stamp_meta_value(&mut notification.meta, "fuigo/leaderClientId", tid);
        }
    ...
    pub(super) async fn replay_session_updates(
        ...
        replay_timer.with_subphase(fuigo_telemetry::startup::Subphase::SessionReplay);
...
mod drain_tests {
    ...
    async fn completion_window_awaits_oldest_before_exceeding_cap() {
        ...
        let (overflow_tx, overflow_rx) =
            tokio::sync::oneshot::channel::<fuigo_acp_lib::AcpResult<()>>();
        {
