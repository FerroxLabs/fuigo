...
//! Repo-local `.fuigo/lsp.json` servers therefore start on the next session open (the durable grant makes that re-spawn trusted).
...
impl MvpAgent {
    ...
    pub(crate) fn maybe_spawn_interactive_trust_prompt(
        ...
        let key = fuigo_workspace::trust::workspace_key(cwd);
...
async fn reload_project_servers_after_grant(ctx: ReloadAfterGrant<'_>) {
    ...
    for target in ctx.targets {
        ...
        // This reloads the session's own project hooks (`.fuigo/hooks`, `.cursor/hooks.json`), which `ReloadPlugins` does not touch
