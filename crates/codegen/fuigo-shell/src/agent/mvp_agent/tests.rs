...
fn resolve_subscription_tier_prefers_display_then_api_key_then_jwt() {
    ...
    let api = auth_with_mode(crate::auth::AuthMode::ApiKey, "fuigo-not-a-jwt");
    assert_eq!(
...
fn jwt_claim_matches_user_subscription_tier_known_pairs() {
    let cases = [
        ...
        ("superfuigo_heavy", "SuperGrokPro"),
        ("9", "EnterpriseMystery"),
        ("superfuigo_lite", "SuperGrokLite"),
        ("superfuigo_plus", "SuperGrokPlus"),
    ];
...
mod hunk_tracking_mode {
    ...
    use fuigo_hunk_tracker::TrackingMode;
    #[test]
...
fn harness_pair(id: &str) -> Vec<fuigo_sampling_types::conversation::ConversationItem> {
...
#[test]
#[serial_test::serial]
fn resolve_agent_definition_defaults_to_fuigo_build() {
    let prev = std::env::var("FUIGO_AGENT").ok();
    unsafe {
        std::env::remove_var("FUIGO_AGENT");
    }
...
/// When model_agent_type = Some("codex"), the codex agent is selected even though the default chain would return fuigo-build.
#[test]
#[serial_test::serial]
...
fn resolve_agent_definition_acp_profile_wins_when_model_agent_type_is_default() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let acp_profile = fuigo_agent::AgentDefinition::from_json(&serde_json::json!(
...
/// Regression: `DEFAULT_AGENT_TYPE` flipped to `fuigo-build-plan`.
/// Models in the catalog that still declare `agent_type = "fuigo-build"` explicitly must NOT preempt an ACP profile.
/// Any value in the `fuigo-build*` family is the stock harness with no strict requirement.
#[test]
#[serial_test::serial]
fn resolve_agent_definition_acp_profile_wins_for_explicit_fuigo_build_family() {
    let prev = std::env::var("FUIGO_AGENT").ok();
    unsafe {
        std::env::remove_var("FUIGO_AGENT");
    }
    ...
    .expect("agent definition must parse");
    for family_variant in ["fuigo-build", "fuigo-build-plan", "fuigo-build-concise"] {
        let def = MvpAgent::resolve_agent_definition(
            tmp.path(),
            None,
            &config::AgentSelectionConfig::default(),
        ...
        assert_eq!(
            def.name, "custom-devbox-profile",
            "ACP profile must win for fuigo-build family variant `{family_variant}`"
        );
    }
    if let Some(v) = prev {
        unsafe { std::env::set_var("FUIGO_AGENT", v) }
...
fn resolve_agent_definition_agent_profile_with_model_override() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let agents_dir = tmp.path().join(".fuigo").join("agents");
    ...
    assert_eq!(
        def.model,
        fuigo_agent::config::ModelOverride::Override("test-model-123".to_string()),
        "agent profile model override must be preserved through resolution"
    );
    match prev {
        Some(v) => unsafe { std::env::set_var("FUIGO_AGENT", v) },
        None => unsafe { std::env::remove_var("FUIGO_AGENT") },
    }
}
#[test]
...
/// A zero-turn switch from `fuigo-build` (a client profile name) to `fuigo-build-plan` (the default model agent_type) must be treated as compatible.
...
fn harnesses_are_compatible_for_stock_family_pairs() {
    assert!(harnesses_are_compatible("fuigo-build", "fuigo-build-plan"));
    assert!(harnesses_are_compatible("fuigo-build-plan", "fuigo-build"));
    assert!(harnesses_are_compatible("fuigo-build", "fuigo-build"));
...
fn harnesses_are_compatible_rejects_strict_mismatches() {
    ...
    assert!(!harnesses_are_compatible("fuigo-build-plan", "codex"));
}
#[test]
fn explicit_agent_type_wins_over_session_default() {
    assert_eq!(
        resolve_required_agent_type(Some("cursor"), "fuigo-build-plan"),
        "cursor"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_fuigo_build_plan() {
    assert_eq!(
        resolve_required_agent_type(None, "fuigo-build-plan"),
        "fuigo-build-plan"
    );
}
#[test]
fn null_agent_type_falls_back_to_session_default_fuigo_build() {
    assert_eq!(
        resolve_required_agent_type(None, "fuigo-build"),
        "fuigo-build"
    );
}
#[test]
fn null_agent_type_returns_to_session_default_after_cursor_switch() {
    let session_default = "fuigo-build-plan";
    let required_after_null = resolve_required_agent_type(None, session_default);
    assert_eq!(required_after_null, "fuigo-build-plan");
...
fn agent_name_unchanged_without_harness_rebuild() {
    let unchanged = agent_name_after_model_switch(false, "fuigo-build-plan", "remote-sidebar");
    assert_eq!(
...
async fn file_toolset_override_e2e_to_finalized_toolset() {
    ...
    use fuigo_tools::registry::types::SessionContext;
    let tmp = tempfile::tempdir().unwrap();
    ...
    let builder = fuigo_tools::registry::types::ToolRegistryBuilder::new();
    let ctx = SessionContext {
        ...
        memory_backend: None,
        web_search_config: fuigo_tools::implementations::web_search::WebSearchConfig::default(),
        web_fetch_config: Default::default(),
        lsp: None,
        image_gen_config: fuigo_tools::implementations::fuigo_build::image_gen::ImageGenConfig::default(),
        video_gen_config: fuigo_tools::implementations::fuigo_build::video_gen::VideoGenConfig::default(),
        app_builder_deployer_config: fuigo_tools::implementations::fuigo_build::app_builder::AppBuilderDeployerConfig::default(),
...
fn make_test_handle(
    ...
    let hunk_tracker_handle = fuigo_hunk_tracker::HunkTrackerActor::spawn(
        ...
        hunk_event_tx,
        fuigo_hunk_tracker::TrackingMode::AllDirty,
        hunk_cancel,
    );
    crate::session::SessionHandle {
        cmd_tx,
        ...
        permission_handle: fuigo_workspace::permission::PermissionHandle::allow_all(),
        attribution_callback: None,
        agent_name: "fuigo-build".to_string(),
...
async fn model_state_prefers_session_reasoning_effort_over_global() {
    ...
    use fuigo_sampling_types::{REASONING_EFFORT_META_KEY, ReasoningEffort};
    let agent = build_minimal_agent_for_tests();
...
async fn restore_effort_via_load(
    initial: Option<fuigo_sampling_types::ReasoningEffort>,
    persisted: Option<fuigo_sampling_types::ReasoningEffort>,
...
async fn yolo_toggle_scoped_by_client_identifier() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("grok-3", false, Some("fuigo-tui")),
        ),
        (
            sid_vscode.clone(),
            make_test_handle("grok-3", false, Some("fuigo-code-extension")),
        ),
    ]
    .into();
    let updated =
        apply_yolo_mode_to_matching_sessions(sessions.values_mut(), Some("fuigo-tui"), true);
...
async fn yolo_toggle_can_disable_session_started_with_yolo_enabled() {
    ...
    let mut sessions: HashMap<acp::SessionId, crate::session::SessionHandle> = [
        (
            sid_tui.clone(),
            make_test_handle("grok-3", true, Some("fuigo-tui")),
        ),
        (
            sid_other.clone(),
            make_test_handle("grok-3", true, Some("fuigo-code-extension")),
        ),
    ]
    .into();
    let updated =
        apply_yolo_mode_to_matching_sessions(sessions.values_mut(), Some("fuigo-tui"), false);
...
async fn test_per_session_code_nav_isolation() {
    let web_handle = {
        let mut h = make_test_handle("model", false, Some("fuigo-web"));
        h.code_nav_enabled = true;
        h
    };
    let tui_handle = {
        let mut h = make_test_handle("model", false, Some("fuigo-tui"));
    ...
    let check = |handle: &crate::session::SessionHandle| {
        ...
        if !matches!(ct, ClientType::FuigoWeb) {
...
async fn ext_method_routes_auth_cleared_and_refreshes_resident_sessions() {
    ...
    local
        .run_until(async {
            let agent = build_agent_with_auth(crate::auth::FuigoAuth {
                key: "eligible".into(),
...
fn build_minimal_agent_for_tests() -> MvpAgent {
    ...
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(temp_dir.path(), FuigoComConfig::default()));
...
async fn ensure_plugin_registry_lazily_populates_snapshot() {
    ...
    let _env = EnvGuard::set("FUIGO_HOME", fuigo_home.path());
...
async fn test_web_session_with_capability_is_eligible() {
    ...
    let mut handle = make_test_handle("model", false, Some("fuigo-web"));
...
async fn test_tui_session_is_rejected() {
    ...
    let mut handle = make_test_handle("model", false, Some("fuigo-tui"));
    handle.code_nav_enabled = true;
    let sessions = [(sid.clone(), handle)].into();
    assert_eq!(
...
async fn test_leader_mode_two_sessions_stay_isolated() {
    ...
    let mut web_handle = make_test_handle("model", false, Some("fuigo-web"));
    web_handle.code_nav_enabled = true;
    let mut tui_handle = make_test_handle("model", false, Some("fuigo-tui"));
...
async fn test_unknown_session_id_returns_session_required() {
    ...
    let mut known_handle = make_test_handle("model", false, Some("fuigo-web"));
...
async fn auth_type_session_based_no_current_returns_session_token() {
    for method_id in [
        ...
        crate::agent::auth_method::FUIGO_COM_METHOD_ID,
...
/// BYOK guard. Users with `fuigo.api_key` must continue to report `ApiKey` regardless of live-token state.
...
#[tokio::test(flavor = "current_thread")]
async fn auth_type_fuigo_api_key_no_current_returns_api_key() {
    let agent = build_minimal_agent_for_tests();
    agent.set_auth_method(acp::AuthMethodId::new(
        crate::agent::auth_method::FUIGO_API_KEY_METHOD_ID,
    ));
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(
        agent.auth_type(),
        fuigo_chat_state::AuthType::ApiKey,
        "fuigo.api_key auth must report ApiKey -- BYOK has no session-token \
...
async fn auth_type_session_based_with_current_returns_session_token() {
    ...
    agent.auth_manager.hot_swap(FuigoAuth::test_default());
...
async fn auth_type_no_method_id_no_current_returns_api_key() {
    ...
    assert!(agent.auth_manager.current().is_none());
    assert_eq!(agent.auth_type(), fuigo_chat_state::AuthType::ApiKey,);
}
...
fn build_agent_with_api_key_auth_disabled() -> MvpAgent {
    ...
    let gateway = GatewaySender::new(tx);
    let mut cfg = AgentConfig::default();
    cfg.fuigo_com_config.disable_api_key_auth = Some(true);
    MvpAgent::new(gateway, &cfg, auth_manager, None).expect("valid test config")
}
/// Deployment-key / managed-config user: `FUIGO_API_KEY` resolves and the kill switch is off.
/// A dead `cached_token` MUST then fall through to `fuigo.api_key` (no browser).
/// This is the exact regression the fallthrough fixes.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn cached_token_fallthrough_prefers_api_key_for_deployment_key() {
    use crate::agent::auth_method::{FUIGO_API_KEY_ENV_VAR, FUIGO_API_KEY_METHOD_ID};
    use fuigo_test_support::EnvGuard;
    ...
    assert_eq!(
        ...
        "deployment-key user (FUIGO_API_KEY set, no kill switch) must fall \
         through to fuigo.api_key on a dead cached_token -- not interactive login",
    );
}
/// Forced-IdP deployment: even with `FUIGO_API_KEY` present, the admin kill switch keeps the fallthrough on interactive `grok.com`.
...
async fn cached_token_fallthrough_respects_kill_switch() {
    ...
    assert_eq!(
        ...
        "disable_api_key_auth must keep the cached_token fallthrough on \
         interactive grok.com so FUIGO_API_KEY can't bypass forced IdP login",
    );
}
...
async fn cached_token_fallthrough_falls_to_fuigo_com_without_credentials() {
    ...
    use fuigo_test_support::EnvGuard;
    let _lockdown = EnvGuard::unset("FUIGO_DISABLE_API_KEY_AUTH");
    let _new = EnvGuard::unset(FUIGO_API_KEY_ENV_VAR);
...
async fn prepare_video_gen_config_disabled_when_zdr_flag_set() {
    use fuigo_tools::implementations::fuigo_build::video_gen::{
...
/// The server can only scope the coding data-retention opt-out (`/privacy opt-out`) to Build traffic via the `x-fuigo-client-identifier` header.
...
/// A fuigo home of its own, with the switch left at its registered default.
/// `decide_search_index` stops short of a session store, but do not reach `bootstrap_once`.
/// `bootstrap_once` takes the process-cached `fuigo_home()`, which these guards cannot redirect, so it could index the developer's own store.
fn search_index_env() -> (tempfile::TempDir, [fuigo_test_support::EnvGuard; 2]) {
    use fuigo_test_support::EnvGuard;
    let home = tempfile::tempdir().unwrap();
    let guards = [
        EnvGuard::set("FUIGO_HOME", home.path()),
        EnvGuard::unset("FUIGO_SESSION_SEARCH"),
    ];
    (home, guards)
}
#[tokio::test]
...
async fn exhausted_fetch_decides_on_the_local_layers() {
    ...
    use fuigo_test_support::EnvGuard;
    let (_home, _env) = search_index_env();
    let _no_inline_auth = EnvGuard::unset("FUIGO_AUTH");
    let _no_auth_path = EnvGuard::unset("FUIGO_AUTH_PATH");
    let auth_dir = tempfile::tempdir().unwrap();
    let auth_manager =
        std::sync::Arc::new(AuthManager::new(auth_dir.path(), FuigoComConfig::default()));
    assert!(
...
fn chat_session_spawn_options_matches_thin_profile() {
    ...
    let cwd = fuigo_paths::AbsPathBuf::new(std::env::temp_dir()).expect("temp cwd");
...
async fn remove_session_releases_workspace_binding_and_side_maps() {
    ...
    let ops = fuigo_workspace::WorkspaceOps::for_test();
    let toolset =
        std::sync::Arc::new(fuigo_tools::registry::types::FinalizedToolset::empty_for_test());
    ...
    *agent.workspace_ops.borrow_mut() = Some(ops);
    ...
    let (_permission_tx, permission_rx) =
        tokio::sync::mpsc::unbounded_channel::<fuigo_workspace::permission::PermissionEvent>();
    agent
        .session_registry
...
fn ext_method_rewind_uses_local_dispatch_without_bridge() {
    use acp::Agent as _;
    let _env = crate::env::EnvVarGuard::remove(crate::env::FUIGO_DISABLE_CUSTOM_BRIDGE_ENV);
    run_local_for_bridge_test(|| async {
        let agent = build_minimal_agent_for_tests();
...
/// SDK e2e shape: `test_cancel_ends_in_flight_turn_and_frees_slot` (fuigo-agent-sdk).
#[test]
...
fn make_live_session_handle(
    ...
    let mut handle = make_test_handle("test-model", false, Some("fuigo-tui"));
...
fn post_auth_settings_not_coalesced_by_in_flight_reapply() {
    run_local_for_bridge_test(|| async {
        ...
        agent.spawn_settings_reapply();
        assert!(agent.settings_reapply_in_flight.get());
        agent.spawn_post_auth_settings(crate::auth::FuigoAuth::test_default());
        assert_eq!(
    ...
    run_local_for_bridge_test(|| async {
        ...
        *agent.allow_access_resolved_for.borrow_mut() = Some(auth.user_id.clone());
        ...
        tokio::time::timeout(
            ...
            agent.enforce_fuigo_code_access(&auth),
        )
        .await
...
impl Drop for RestoreOtelGate {
    fn drop(&mut self) {
        fuigo_telemetry::external::mark_external_otel_settings_resolved();
    }
}
...
async fn post_auth_settings_fuigo_upgrades_writeback_emits_and_opens_gate() {
    ...
    assert!(!fuigo_telemetry::external::is_settings_gate_open());
    agent.maybe_fetch_post_auth_settings().await;
    assert_eq!(
        agent.storage_mode(),
        StorageMode::Writeback,
        "fuigo auth + writeback_enabled settings must upgrade storage to Writeback"
    );
    assert!(
        fuigo_telemetry::external::is_settings_gate_open(),
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[serial_test::serial]
async fn post_auth_settings_non_fuigo_keeps_local_but_still_emits() {
    use crate::agent::config::AgentMode;
    ...
    let api_auth = FuigoAuth {
        auth_mode: AuthMode::ApiKey,
        ..FuigoAuth::test_default()
    };
    assert!(
        !api_auth.is_fuigo_auth(),
    ...
    assert_eq!(
        agent.storage_mode(),
        StorageMode::Local,
        "non-fuigo auth must stay Local even when writeback is advertised remotely"
    );
    assert!(
        fuigo_telemetry::external::is_settings_gate_open(),
...
async fn same_credential_refresh_does_not_flap_resolved_gate() {
    ...
    let (agent, _rx) =
        build_agent_with_auth_and_proxy(fuigo_auth.clone(), server.url(), AgentMode::Leader);
    agent.otel_gate.set_resolved_for(&fuigo_auth.user_id);
    fuigo_telemetry::external::mark_external_otel_settings_resolved();
    assert!(fuigo_telemetry::external::is_settings_gate_open());
    agent.refresh_remote_settings(&fuigo_auth).await;
    assert!(
        fuigo_telemetry::external::is_settings_gate_open(),
...
fn write_project_subagent_definitions(cwd: &std::path::Path) {
    let roles = cwd.join(".fuigo/roles");
    let personas = cwd.join(".fuigo/personas");
    ...
    run_local_for_bridge_test(|| async {
        ...
        agent.insert_resident(&sid, handle);
        {
            ...
            cfg.subagent_roles.insert(
                "refreshed".into(),
                fuigo_subagent_resolution::config::SubagentRole {
...
/// End-to-end gate wiring: project `.fuigo/roles` / `personas` alone must drive the real `resolve_and_record` untrusted.
...
fn interactive_trust_prompt_client_error_fails_closed() {
    ...
    run_local_for_bridge_test(|| async {
        ...
        let msg = tokio::time::timeout(std::time::Duration::from_secs(2), gw_rx.recv())
            ...
            .expect("trust request must be sent")
            .expect("gateway channel open");
        assert!(matches!(msg, fuigo_acp_lib::AcpClientMessage::ExtMethod(_)));
        drop(msg);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(300), cmd_rx.recv())
                .await
...
fn ann(id: &str) -> fuigo_announcements::RemoteAnnouncement {
    fuigo_announcements::RemoteAnnouncement {
...
fn settings_with(
    announcements: Option<Vec<fuigo_announcements::RemoteAnnouncement>>,
) -> crate::util::config::RemoteSettings {
    crate::util::config::RemoteSettings {
        announcements,
        ..Default::default()
...
fn announcements_push_gate_ignores_expired_only_addition() {
    let now = test_now();
    let expired = fuigo_announcements::RemoteAnnouncement {
...
fn announcements_push_gate_emits_on_expiry_crossing() {
    let expiring = fuigo_announcements::RemoteAnnouncement {
...
fn status_line_meta(enabled: bool) -> acp::Meta {
    let mut meta = acp::Meta::new();
    meta.insert(
        fuigo_status_line::CLIENT_STATUS_LINE_META.to_string(),
...
fn init_advertising_status_line(enabled: bool) -> acp::InitializeRequest {
    let mut meta = serde_json::Map::new();
    meta.insert(
        fuigo_status_line::STATUS_LINE_CAPABILITY.to_string(),
        serde_json::json!(enabled),
    );
    acp::InitializeRequest::new(acp::ProtocolVersion::V1).client_capabilities(
        acp::ClientCapabilities::new()
