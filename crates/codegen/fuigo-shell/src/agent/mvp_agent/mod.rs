...
use fuigo_telemetry::id::{agent_id, agent_instance_id};
use fuigo_telemetry::session_ctx::log_event;
use fuigo_workspace::file_system::{AcpSessionFs, CodebaseIndexManager, LocalFs};
use fuigo_workspace::permission::{ClientType, PermissionEvent};
...
pub(crate) fn jwt_claim_matches_user_subscription_tier(
    ...
    match user_subscription_tier {
        ...
        "SuperGrokPro" => jwt_claim == "superfuigo_heavy",
        "SuperGrokLite" => jwt_claim == "superfuigo_lite",
        "SuperGrokPlus" => jwt_claim == "superfuigo_plus",
...
pub(crate) struct SessionSpawnOptions<'a> {
    ...
    pub envrc: Option<fuigo_workspace::envrc::EnvrcLoad>,
...
pub(crate) struct PromptResponseMetaArgs<'a> {
    ...
    pub total_tokens: u64,
    pub model_id: &'a str,
    pub last_turn_usage: Option<&'a fuigo_sampling_types::TokenUsage>,
...
fn announcements_push_payload(
    stored: Option<&[fuigo_announcements::RemoteAnnouncement]>,
    last_emitted: &[fuigo_announcements::RemoteAnnouncement],
    now: chrono::DateTime<chrono::Utc>,
    mode: AnnouncementsPushMode,
) -> Option<Vec<fuigo_announcements::RemoteAnnouncement>> {
    let current = fuigo_announcements::filter_expired_at(
...
/// Override with `FUIGO_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS`.
/// Clamped to at least 1s: `tokio::time::interval` panics on a zero period.
fn announcements_refresh_interval() -> std::time::Duration {
    if let Ok(s) = std::env::var("FUIGO_ANNOUNCEMENTS_REFRESH_INTERVAL_SECS")
...
struct ResidentResources {
    ...
    codebase_index: Option<std::sync::Arc<fuigo_codebase_graph::IndexManagerHandle>>,
    is_headless: bool,
...
pub struct MvpAgent {
    ...
    /// That task calls [`crate::config::watcher::ConfigFileWatcher::watch_path`] (a **non-recursive** watch on `<cwd>/` and `<cwd>/.fuigo/`).
    ///
    ...
    /// Local session-registry override: `FUIGO_SESSION_REGISTRY` env, else `[cli] session_registry`.
    ...
    subagent_event_tx: fuigo_tools::implementations::fuigo_build::task::backend::SubagentCoordinatorSender,
    ...
    subagent_event_rx: RefCell<
        Option<
            fuigo_tools::implementations::fuigo_build::task::coordinator::SubagentCoordinatorReceiver,
        >,
    >,
    ...
    monitor_event_buffer: fuigo_tools::implementations::fuigo_build::monitor::types::MonitorEventBuffer,
    ...
    pub(crate) plugin_registry_handle: fuigo_agent::plugins::SharedPluginRegistryHandle,
    ...
    /// The walk (cwd to git root, plus user and marketplace dirs) stalled fuigo-desktop's first `initialize`.
    ...
    /// LEADER-SAFE(shared): one agent-wide push stream.
    last_emitted_announcements: RefCell<Vec<fuigo_announcements::RemoteAnnouncement>>,
...
pub(crate) fn inherited_harness_template(
    current: &fuigo_agent::prompt::user_message::UserMessageTemplate,
    pinned_model_agent_type: Option<&str>,
    cwd: &std::path::Path,
) -> Option<fuigo_agent::prompt::user_message::UserMessageTemplate> {
    ...
    if !fuigo_agent::config::is_strict_harness_agent_type(agent_type) {
        return None;
    }
    let harness = fuigo_agent::discovery::by_name_in_cwd(agent_type, cwd)?;
...
/// Compatible stock switches (e.g. `fuigo-build` to `fuigo-build-plan`) intentionally preserve the session's original ACP `agentProfile`.
...
/// So switching e.g. `fuigo-build` to `fuigo-build-plan` doesn't require rebuilding the harness.
...
pub(crate) fn harnesses_are_compatible(active: &str, required: &str) -> bool {
    use fuigo_agent::config::is_strict_harness_agent_type;
    match (
...
use fuigo_chat_state::conversation_util::replace_or_insert_system_head;
...
pub(crate) fn subagent_rate_limit_max_attempts_env() -> Option<u32> {
    parse_subagent_rate_limit_max_attempts(
        std::env::var("FUIGO_SUBAGENT_RATE_LIMIT_MAX_ATTEMPTS").ok().as_deref(),
    )
}
...
fn parse_subagent_rate_limit_max_attempts(raw: Option<&str>) -> Option<u32> {
    ...
    match value.parse::<u32>() {
        ...
        Err(_) => {
            tracing::warn!(
                value,
                "ignoring invalid FUIGO_SUBAGENT_RATE_LIMIT_MAX_ATTEMPTS"
            );
            None
        }
    }
...
fn resolve_hunk_tracking_mode(
    mode_str: Option<&str>,
) -> Option<fuigo_hunk_tracker::TrackingMode> {
    ...
    Some(
        serde_json::from_value(serde_json::Value::String(mode.to_ascii_lowercase()))
            .unwrap_or(fuigo_hunk_tracker::TrackingMode::AllDirty),
    )
}
...
struct HunkTrackingPlan {
    ...
    actor_mode: Option<fuigo_hunk_tracker::TrackingMode>,
}
...
impl MvpAgent {
    ...
    pub(super) fn reconcile_stale_background_tasks(
        ...
        session_id: &acp::SessionId,
        updates_file_path: &Option<PathBuf>,
    ) -> Vec<tokio::sync::oneshot::Receiver<fuigo_acp_lib::AcpResult<()>>> {
        ...
        for task in &orphaned {
            let snapshot = fuigo_tools::types::TaskSnapshot {
    ...
    ///
    /// Non-Ferrox Labs auth (API keys, enterprise) always passes.
    /// For Ferrox Labs OAuth2 users, reads `allow_access` from remote settings (explicit `false` blocks; absent field fails open).
    /// When settings have not arrived yet the gate is provisionally open and re-resolved on arrival.
    pub(super) async fn enforce_fuigo_code_access(&self, auth: &crate::auth::FuigoAuth) {
        ...
        *self.allow_access_resolved_for.borrow_mut() = Some(auth.user_id.clone());
        if !allow {
            tracing::info!(
                "auth: user blocked by allow_access (remote settings fuigo_build_access_gate)"
            );
    ...
    pub(super) fn start_search_index_once(&self) {
        ...
        if let crate::session::storage::search::IndexDecision::On(index) = self
            .search_index()
        {
            index.bootstrap_once(crate::util::fuigo_home::fuigo_home());
        }
    }
    ...
    pub(super) fn spawn_auto_worktree_gc(&self) {
        ...
        let fuigo_home = fuigo_fast_worktree::resolve_fuigo_home();
        tokio::task::spawn_blocking(move || Self::reclaim_worktrees(
            fuigo_home,
            auto_gc_policy,
        ));
    }
    /// The caller resolves the home: read here, $FUIGO_HOME would be read when the blocking thread starts.
    /// This deletes worktrees under what it finds.
    pub(super) fn reclaim_worktrees(
        fuigo_home: anyhow::Result<std::path::PathBuf>,
        policy: fuigo_fast_worktree::ResolvedWorktreeAutoGc,
    ) {
        if let Err(e) = fuigo_home
            .and_then(|home| fuigo_fast_worktree::WorktreeDb::open(&home))
            .and_then(|db| fuigo_fast_worktree::maybe_auto_gc(&db, &policy))
        {
    ...
    ///    against `~/.fuigo/bundled/` and the manifest.
...
async fn handle_synthetic_turn_trace(
    ...
    let turn_messages: Option<fuigo_chat_state::TurnCapture> = {
