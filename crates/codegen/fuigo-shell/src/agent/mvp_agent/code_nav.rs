...
impl MvpAgent {
    /// Parse the `fuigo/codeNavigation.enabled` capability from an initialize request.
    ...
    pub(crate) fn parse_code_nav_capability(init: &acp::InitializeRequest) -> bool {
        init.client_capabilities
            .meta
            .as_ref()
            .and_then(|m| m.get("fuigo/codeNavigation"))
    ...
    pub(crate) fn start_codebase_index_for_code_nav(
        ...
        cwd: &std::path::Path,
    ) -> Option<(std::sync::Arc<fuigo_codebase_graph::IndexManagerHandle>, bool)> {
    ...
    pub(super) fn code_nav_eligibility_inner(
        ...
        // Gate 1: client type
        if !matches!(client_type, ClientType::FuigoWeb) {
            tracing::info!(
        ...
        if !code_nav_enabled {
            tracing::info!(
                ...
                "code-nav eligibility check: skipping (fuigo/codeNavigation.enabled not advertised)"
            );
    ...
    pub(crate) fn code_nav_eligibility_for_request(
        ...
        let session_id = match session_id {
            ...
            // Callers must provide sessionId for fuigo/code/* requests
    ...
    pub(super) fn indexed_roots_for(&self, cwd: &std::path::Path) -> Vec<String> {
        ...
        if let Ok(git_root) = fuigo_workspace::session::git::find_git_root_from_path(cwd)
    ...
    pub(super) fn get_or_create_codebase_index(
        &self,
        cwd: PathBuf,
    ) -> (std::sync::Arc<fuigo_codebase_graph::IndexManagerHandle>, bool) {
    ...
    pub(crate) fn get_codebase_index(
        &self,
        cwd: &std::path::Path,
    ) -> Option<std::sync::Arc<fuigo_codebase_graph::IndexManagerHandle>> {
