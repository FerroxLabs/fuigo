...
use fuigo_tools::implementations::fuigo_build::task::backend::SubagentBackend;
...
impl MvpAgent {
    ...
    fn has_proxy_credentials(&self) -> bool {
        self.cfg.borrow().endpoints.deployment_key.is_some()
            || self.auth_manager.current_or_expired().is_some_and(|a| a.is_fuigo_auth())
    }
    ...
    pub(super) fn current_or_buffered_auth(&self) -> Option<crate::auth::FuigoAuth> {
        self.auth_manager
            .current()
            .or_else(|| {
    ...
    /// They must not block the ACP response (fuigo-desktop sends `initialize` immediately).
    ...
    /// The cwd-to-git-root and user/marketplace walks stalled fuigo-desktop's first `initialize`.
    ...
    pub(super) fn ensure_telemetry_client(&self) {
        ...
        if !mode.is_disabled() {
            let Some(auth) = self
                ...
                .current()
                .filter(|a| {
                    a.is_fuigo_auth() || a.auth_mode == crate::auth::AuthMode::ApiKey
                }) else {
                return;
            };
            ...
            let (user_id, team_id) = if auth.is_fuigo_auth() {
                (Some(auth.user_id), auth.team_id)
            } else {
                (None, auth.team_id)
            };
            fuigo_telemetry::client::init_if_needed(
                cfg.telemetry.clone(),
                mode,
                user_id,
                team_id,
                ...
                fuigo_version::VERSION.to_owned(),
                subscription_tier,
    ...
    /// Return the current FuigoAuth credentials, if authenticated and not expired.
    pub(crate) fn current_auth(&self) -> Option<crate::auth::FuigoAuth> {
    ...
    pub(crate) fn plugin_registry_handle(
        &self,
    ) -> &fuigo_agent::plugins::SharedPluginRegistryHandle {
    ...
    /// This is the same boot stall as plugin discovery on fuigo-desktop Windows.
    fn ensure_local_workspace_ops(
        &self,
    ) -> Result<fuigo_workspace::WorkspaceOps, acp::Error> {
        ...
        let workspace_identity = self
            ...
            .map(|a| match a.team_id.filter(|t| !t.is_empty()) {
                Some(team) => {
                    fuigo_workspace::WorkspaceIdentity::team(a.user_id, team)
                }
                None => {
                    fuigo_workspace::WorkspaceIdentity::new(
                        a.user_id,
                        a.principal_type,
                        a.principal_id,
                    )
            ...
            .unwrap_or_default();
        let ops = match fuigo_workspace::handle::WorkspaceHandle::new_minimal(
            cwd.to_path_buf(),
            workspace_identity,
            project_lsp_trusted,
        ) {
            Ok(handle) => fuigo_workspace::WorkspaceOps::local(handle),
            Err(e) => {
                tracing::error!(
                    error = %e,
    ...
    ///
    /// Returns `ApiKey` only when the auth method is BYOK (`fuigo.api_key`) or no auth method has been selected yet AND no live credential exists.
    ///
    ...
    pub(crate) fn auth_type(&self) -> fuigo_chat_state::AuthType {
    ...
    /// Fall through to `fuigo.api_key` if the startup probe still allows it, else `grok.com`.
    /// `None` when `preferred_method` is pinned.
    pub(super) fn cached_token_fallthrough_method_id(
        &self,
    ) -> Option<acp::AuthMethodId> {
        let preferred = self.cfg.borrow().fuigo_com_config.preferred_method;
        let id = auth_method::method_id_after_cached_token_unavailable(
            auth_method::should_advertise_fuigo_api_key_with_env_ok(
    ...
    /// Mirrors the `resolve_config` gate: only upgrades from `Local`, writeback needs fuigo auth.
    fn reapply_storage_mode(&self) {
        ...
        let resolved_mode = {
            ...
            let has_fuigo_auth = self
                .auth_manager
                .current_or_expired()
                .is_some_and(|a| a.is_fuigo_auth());
            StorageMode::from_remote_gated(cfg.remote_settings.as_ref(), has_fuigo_auth)
        };
    ...
    pub(super) async fn refresh_remote_settings(&self, auth: &crate::auth::FuigoAuth) {
        if !crate::util::config::resolve_remote_fetch_enabled() {
            tracing::debug!("post-auth settings refresh skipped: remote_fetch disabled");
            return;
        }
        let is_fuigo = auth.is_fuigo_auth();
        ...
        let (
            ...
            let fuigo_user_id = is_fuigo.then(|| user_id.clone());
            let fuigo_team_id = is_fuigo.then(|| team_id.clone()).flatten();
    ...
    pub(super) fn apply_polled_announcements(
        ...
        pre_fetch: Option<Vec<fuigo_announcements::RemoteAnnouncement>>,
    ) {
        let mut cfg = self.cfg.borrow_mut();
    ...
    pub(crate) fn prepare_sampling_config_for_model(
        ...
        if prefers_oidc && !model.has_own_credentials()
            && credentials.auth_type == fuigo_chat_state::AuthType::ApiKey
        {
            credentials.api_key = None;
            credentials.auth_type = fuigo_chat_state::AuthType::SessionToken;
        }
        ...
        if !has_session_key && credentials.auth_type == fuigo_chat_state::AuthType::ApiKey
    ...
    fn is_tier_restricted_capability(&self) -> bool {
        ...
        if !auth.is_fuigo_auth() || auth.team_id.is_some() {
            return false;
        }
        let tier = self
            .cfg
    ...
    pub(super) fn prepare_image_gen_config(
        ...
        headers.insert("user-agent".to_string(), format!("fuigo-build/{version}"));
    ...
    pub(super) fn prepare_app_builder_deployer_config(
        ...
        use fuigo_tools::implementations::fuigo_build::app_builder::AppBuilderDeployerConfig;
        AppBuilderDeployerConfig::Disabled
    }
    /// Video tools call the Ferrox Labs API directly.
    pub(super) fn prepare_video_gen_config(
        &self,
    ) -> fuigo_tools::implementations::fuigo_build::video_gen::VideoGenConfig {
    ...
    ///
    /// Enabled gate: `disable_web_search` kill-switch > `FUIGO_WEB_FETCH` env > remote settings `web_fetch_enabled` > default (false).
    ///
    /// Params resolution (TOML > env > remote settings > default):
    /// - `proxy_endpoint`: `[toolset.web_fetch] proxy_endpoint` > `FUIGO_WEB_FETCH_PROXY` > remote settings > None
    /// - `allowed_domains`: `[toolset.web_fetch] allowed_domains` > remote settings > built-in defaults
    /// - `allow_local`: `[toolset.web_fetch] allow_local` > `FUIGO_WEB_FETCH_ALLOW_LOCAL` > false
    pub(super) fn prepare_web_fetch_config(
        &self,
    ) -> fuigo_tools::implementations::fuigo_build::web_fetch::WebFetchConfig {
    ...
    pub fn with_models(
        ...
        let has_fuigo_auth = auth_manager
            .current_or_expired()
            .is_some_and(|a| a.is_fuigo_auth());
        let relay_sync_enabled = tui_mode && relay_config_enabled && has_fuigo_auth;
        ...
        if relay_sync_enabled {
            tracing::info!("[fuigo] Relay sync: ENABLED");
        } else if tui_mode && relay_config_enabled && !has_fuigo_auth {
            tracing::info!("[fuigo] Relay sync: DISABLED (no auth - run 'fuigo login' first)");
        ...
        let instance = Self {
            ...
            plugin_registry_handle: fuigo_agent::plugins::SharedPluginRegistryHandle::new(
                None,
            ...
            monitor_event_buffer: fuigo_tools::implementations::fuigo_build::monitor::types::MonitorEventBuffer::default(),
        ...
        instance
            .auth_manager
            .configure_refresher(
                instance.cfg.borrow().fuigo_com_config.auth_provider_command.clone(),
    ...
    pub(crate) async fn kill_background_task(
        ...
        session_id: &str,
        task_id: &str,
        source: fuigo_tools::types::KillSource,
    ) -> Result<fuigo_tools::types::KillOutcome, String> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub(crate) async fn cancel_subagent(
        &self,
        subagent_id: &str,
    ) -> fuigo_tools::implementations::fuigo_build::task::types::SubagentCancelOutcome {
    ...
    pub(crate) async fn list_running_subagents(
        ...
        fuigo_tools::implementations::fuigo_build::task::types::SubagentInspection,
    > {
        let backend = fuigo_tools::implementations::fuigo_build::task::backend::ChannelBackend::new(
            self.subagent_event_tx.event_sender().0,
        );
    ...
    pub(crate) async fn query_subagent(
        ...
        fuigo_tools::implementations::fuigo_build::task::types::SubagentSnapshot,
    > {
        fuigo_tools::implementations::fuigo_build::task::backend::ChannelBackend::new(
                self.subagent_event_tx.event_sender().0,
            )
    ...
    pub async fn list_tasks(
        &self,
        session_id: &str,
    ) -> Option<Vec<fuigo_tools::types::TaskSnapshot>> {
        let sid = acp::SessionId::new(session_id);
        if let Some(handle) = self.get_session_handle(&sid) {
    ...
    pub(crate) async fn list_hooks(
        &self,
        session_id: &acp::SessionId,
    ) -> Option<fuigo_hooks_plugins_types::HooksListResponse> {
    ...
    pub(crate) async fn execute_hooks_action(
        ...
        if matches!(action, fuigo_hooks_plugins_types::HooksAction::Untrust)
            && let Some(cwd) = self.get_session_cwd(session_id)
        {
            self.interactive_trust_prompted
                .borrow_mut()
                .remove(&fuigo_workspace::trust::workspace_key(&cwd));
        }
        let handle = self.get_session_handle(session_id)?;
    ...
    pub(crate) async fn execute_plugins_action(
        ...
        let is_reload = matches!(action, fuigo_hooks_plugins_types::PluginsAction::Reload);
        let handle = self.get_session_handle(session_id)?;
        let outcome = handle.execute_plugins_action(action).await;
        let succeeded = matches!(
            outcome.as_ref().map(|o| &o.status),
            Some(fuigo_hooks_plugins_types::OutcomeStatus::Success)
        );
    ...
    pub(super) fn trace_upload_config_snapshot(
        ...
        let auth_token = if cfg.endpoints.deployment_key.is_none() {
            self.auth_manager
                .current_or_expired()
                .filter(|auth| auth.is_fuigo_auth())
                .map(|auth| auth.key)
        } else {
            None
        };
    ...
    pub(super) fn insert_session_config_meta(
        ...
        let detail = session_config::FuigoSessionDetail::build(
    ...
    pub(super) async fn upload_harness_trace_turns(
        ...
        model: &str,
        turns: Vec<Vec<fuigo_sampling_types::conversation::ConversationItem>>,
    ) {
    ...
    pub(super) async fn build_harness_trace_uploads(
        ...
        base: u64,
        turns: Vec<Vec<fuigo_sampling_types::conversation::ConversationItem>>,
    ) -> Vec<(PromptTraceContext, PromptMetadata, fuigo_chat_state::TurnCapture)> {
        ...
        for (offset, items) in turns.into_iter().enumerate() {
            ...
            let metadata = PromptMetadata::new(PromptMetadataParams {
                ...
                shell_version: Some(fuigo_version::VERSION.to_string()),
                sandbox: local_sandbox_telemetry(),
                ..Default::default()
            });
            let capture = fuigo_chat_state::TurnCapture {
                messages: items,
    ...
    pub(crate) async fn get_trace_context(
        ...
        let upload_method = match upload_method {
            Some(method) => method,
            None => {
                fuigo_telemetry::session_ctx::log_session_event(crate::agent::session_metrics::TraceUploadSkipped {
    ...
    /// 5. `FUIGO_AGENT` env var.
    /// 6. Built-in default agent.
    ///
    /// `FUIGO_AGENT` and an explicit `[agent] name` bypass step 1.
    /// Strict-harness classification is structural; see [`fuigo_agent::config::is_strict_harness_agent_type`].
    ///
    ...
    pub fn resolve_agent_definition_for_model(
        ...
        if let Some(ref name) = agent_config.name {
            ...
            if let Some(def) = fuigo_agent::discovery::by_name_in_cwd(name, cwd) {
                return def;
            }
            tracing::warn!(
                agent_name = %name,
        ...
        let agent_name = std::env::var("FUIGO_AGENT").ok();
        let resolved = match agent_name.as_deref() {
            Some("browser-use") | Some("browser_use") => AgentDefinition::browser_use(),
            Some("fuigo-build-concise") | Some("fuigo_build_concise") => {
                AgentDefinition::fuigo_build_concise()
            }
            Some(path) if std::path::Path::new(path).is_absolute() => {
                match AgentDefinition::from_file(path) {
                    ...
                    Err(e) => {
                        ...
                        AgentDefinition::fuigo_build_plan()
                    }
                }
            }
            Some(name) => {
                fuigo_agent::discovery::by_name_in_cwd(name, cwd)
                    .unwrap_or_else(AgentDefinition::fuigo_build_plan)
            }
            None => AgentDefinition::fuigo_build_plan(),
        };
        if !fuigo_agent_env_set && !config_agent_explicitly_set
            && model_requires_strict_harness && let Some(required) = model_agent_type
            && resolved.name != required
        {
            ...
            if let Some(def) = fuigo_agent::discovery::by_name_in_cwd(required, cwd) {
                return def;
            }
            tracing::warn!(
                model_agent_type = %required,
    ...
    pub(super) async fn spawn_and_register_session(
        ...
        let envrc = envrc
            .unwrap_or_else(|| fuigo_workspace::envrc::spawn_envrc_load(
        ...
        let fs: Arc<dyn fuigo_workspace::file_system::AsyncFileSystem> = if use_acp_fs {
        ...
        let (hunk_tracker_handle, hunk_event_rx) = match hunk_plan.actor_mode {
            ...
            None => (fuigo_hunk_tracker::HunkTrackerHandle::noop(), None),
        };
        let has_fuigo_auth = self.auth_manager.current().is_some_and(|a| a.is_fuigo_auth());
        let loc_tracking_enabled = hunk_tracking_enabled && has_fuigo_auth
            && (self
                .cfg
                .borrow()
                .remote_settings
                ...
                .unwrap_or(false)
                || std::env::var("FUIGO_LOC_TRACKING")
        ...
        let loc_aggregate_rx = match hunk_event_rx {
            Some((hunk_event_rx, loc_cancel)) if loc_tracking_enabled => {
                ...
                tokio::spawn(
                    fuigo_hunk_tracker::run_loc_sink(
                        hunk_event_rx,
                        loc_writer,
                        loc_ctx,
                        loc_cancel,
        ...
        let mut session_env = fuigo_workspace::permission::claude_settings::load_claude_env_with_project(
            cwd.as_path(),
            project_env_trusted,
        );
            ...
            .model
        {
            ...
            fuigo_agent::config::ModelOverride::Inherit => None,
        };
        ...
        if lsp_tools_enabled && tool_ctx.lsp.is_none() {
            ...
            let (
                ...
                .unzip();
            let sourced = fuigo_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
            ...
            if servers.is_empty() {
                let user_path = fuigo_tools::util::fuigo_home::fuigo_home()
                    .join("lsp.json");
                let project_path = tool_ctx.cwd.as_path().join(".fuigo").join("lsp.json");
                tracing::debug!(
                    cwd = %tool_ctx.cwd,
        ...
        let (mut handle, permission_events_rx, agent_system_prompt, session_thread) = {
            ...
            let attribution_callback: Option<
                fuigo_sampler::SharedAttributionCallback,
            > = Some(
        ...
        if let Some(mut loc_rx) = loc_aggregate_rx {
            ...
            tokio::spawn(async move {
                while let Some(agg) = loc_rx.recv().await {
                    match agg {
                        fuigo_hunk_tracker::LocAggregate::LinesChanged {
                            author_type,
                            lines_added,
                            lines_removed,
                            file_path,
                            ...
                            let is_agent = author_type
                                == fuigo_hunk_tracker::AuthorType::Agent;
                            signals
                                .record_loc_change(
                                    is_agent,
                                    lines_added,
                        ...
                        fuigo_hunk_tracker::LocAggregate::LinesReverted {
