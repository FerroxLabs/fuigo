...
use super::*;
use fuigo_telemetry::instrument_task;
...
impl acp::Agent for MvpAgent {
    ...
    async fn initialize(
        ...
        fuigo_telemetry::unified_log::info("agent initialized", None, None);
        fuigo_telemetry::startup::mark_agent_serving();
        ...
        CLEANUP_PERMISSIONS_ONCE
            .call_once(|| {
                tokio::task::spawn(
                    fuigo_workspace::permission::cleanup_stale_permission_state(
        ...
        fuigo_workspace::trust::migrate_legacy_hook_trust();
        ...
        *self.buffering_settings.borrow_mut() = buffering_settings;
        ...
        let preferred_method_early = self.cfg.borrow().fuigo_com_config.preferred_method;
        let fuigo_api_base_url = self.cfg.borrow().endpoints.fuigo_api_base_url.clone();
        let has_byok = self
            .models_manager
            .models()
            .values()
        ...
        let first_party_env_ok = if crate::auth::should_probe_first_party_env_key(
            ...
            has_byok,
            auth_method::has_fuigo_api_key_env(),
        ...
        let has_external_api_key = auth_method::should_advertise_fuigo_api_key_with_env_ok(
            disable_api_key_auth,
        ...
        let (
            ...
            let cfg = self.cfg.borrow();
            let issuer = cfg.fuigo_com_config.oidc.as_ref().map(|o| o.issuer.clone());
            (
                cfg.fuigo_com_config.auth_provider_label.clone(),
                cfg.fuigo_com_config.auth_provider_command.is_some(),
                cfg.fuigo_com_config.oidc.is_some(),
                issuer,
            )
        };
        ...
        fuigo_telemetry::unified_log::info(
            ...
            Some(
                serde_json::json!({
                "fuigo_home": crate::util::fuigo_home::fuigo_home().display().to_string(),
        ...
        debug_assert!(
            !has_external_api_key
                || matches!(
                    ...
                    Some(auth_method::AuthMethodKind::FuigoApiKey)
                ),
            "BYOK invariant violated: fuigo.api_key MUST be auth_methods.first() \
        ...
        Ok(
            acp::InitializeResponse::new(acp::ProtocolVersion::V1)
                ...
                .meta({
                    let metadata = parse_json_object_env("FUIGO_AGENT_METADATA");
                    serde_json::json!({
                    "fuigoShell": true,
    ...
    async fn authenticate(
        ...
        if let Some(preferred) = self.cfg.borrow().fuigo_com_config.preferred_method {
        ...
        match arguments.method_id.0.as_ref() {
            auth_method::FUIGO_API_KEY_METHOD_ID => {
                if self.cfg.borrow().fuigo_com_config.api_key_auth_disabled() {
                ...
                log_event(fuigo_telemetry::events::Login {
            ...
            auth_method::CACHED_TOKEN_AUTH_METHOD_ID => {
                ...
                let pin_blocks_oidc_mint = matches!(
                    self.cfg.borrow().fuigo_com_config.preferred_method,
                ...
                self.enforce_fuigo_code_access(&auth).await;
                self.maybe_sync_bundle_in_background(false);
            ...
            auth_method::FUIGO_COM_METHOD_ID | auth_method::OIDC_METHOD_ID => {
                let fuigo_ctx = self.auth_manager.fuigo_com_config();
                let auth_meta = AuthRequestMeta::from_json(arguments.meta.as_ref());
                tracing::info!(
                ...
                let use_oidc = self.cfg.borrow().resolve_fuigo_oauth(cli_oauth);
    ...
    async fn prompt(
            ...
            .and_then(|m| m.get("toolOverrides"))
        {
            None => None,
            Some(value) => {
                match fuigo_sampling_types::ToolOverridesUpdate::parse(value) {
        ...
        let traceparent = fuigo_file_utils::trace_context::current_traceparent();
        let dispatch_result: Result<(), acp::Error> = if send_now {
            ...
            let envelope = fuigo_message_delivery_core::DeliveryEnvelope::from_human(
                fuigo_message_delivery_core::Operation::Queue,
            ...
            Ok(turn_ok) => {
                ...
                if let Some(ctx) = trace_context {
                    ...
                    async fn advance_last_turn(
                        ...
                        let repo_head_at_end = fuigo_tty_utils::git_command()
                            .current_dir(&cwd)
                            .args(["rev-parse", "HEAD"])
                            .output()
                            .ok()
                    ...
                    }
                    {
                        ...
                        tokio::spawn(async move {
                            let head = fuigo_workspace::session::git::get_current_commit(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let branch = fuigo_workspace::session::git::get_branch(
                                    std::path::Path::new(&cwd),
                                )
                                .await;
                            let _ = cmd_tx
    ...
    async fn ext_method(
        ...
        if let Some(meta) = &request_meta {
            fuigo_file_utils::trace_context::link_current_span_to_meta(meta);
        }
    ...
    async fn ext_notification(
        ...
        if args.method.as_ref() == "_x.ai/session/update" {
            ...
            >(args.params.get()) {
                tracing::info!(
                    "Storing Ferrox Labs session notification: session_id={}",
                    notification.session_id.0
                );
                if let Some(handle) = self.resident_handle(&notification.session_id) {
                    let _ = handle
                        .cmd_tx
                        .send(crate::session::SessionCommand::FuigoSessionNotification {
                            notification,
                        });
                } else {
                    tracing::warn!(
                        "Received Ferrox Labs session notification for unknown session: {}",
                        notification.session_id.0
                    );
                }
            } else {
        ...
        if args.method.as_ref() == "x.ai/telemetry/non_git_decision" {
            ...
            >(args.params.get()) {
                ...
                fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::NonGitDecisionEvent {
                    decision: params.decision,
                    session_id: params.session_id,
                    client_version: params.client_version,
                });
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_followup" {
            ...
            >(args.params.get()) {
                ...
                fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::MultiAgentFollowup {
                    ...
                    other_agents: params
                        .other_agents
                        .into_iter()
                        .map(|(l, s, m)| fuigo_telemetry::events::AgentInfo {
                            label: l.to_string(),
                            session_id: s,
                            model_id: m,
                        })
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_apply" {
            ...
            >(args.params.get()) {
                ...
                fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::MultiAgentApply {
        ...
        if args.method.as_ref() == "x.ai/telemetry/multi_agent_discard" {
            ...
            >(args.params.get()) {
                ...
                fuigo_telemetry::session_ctx::log_event(fuigo_telemetry::events::MultiAgentDiscard {
                    discarded_agents: params
                        .discarded_agents
                        .into_iter()
                        .map(|(l, s, m)| fuigo_telemetry::events::AgentInfo {
        ...
        if args.method.as_ref() == fuigo_telemetry::unified_log::LOG_METHOD
            && let Ok(params) = serde_json::from_str::<
                fuigo_telemetry::unified_log::LogNotificationParams,
            >(args.params.get())
        {
            fuigo_telemetry::unified_log::ingest_client_entries(
                params.src,
                &params.entries,
            );
        }
