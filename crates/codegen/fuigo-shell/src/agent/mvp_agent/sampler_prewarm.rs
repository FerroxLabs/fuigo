...
fn prewarm_and_record(base_url: String) -> impl std::future::Future<Output = ()> {
    ...
    async move {
        ...
        let report = fuigo_sampler::prewarm_transport(&base_url).await;
...
fn record_prewarm_report(span: &tracing::Span, report: &fuigo_sampler::PrewarmReport) {
...
mod tests {
    ...
    async fn prewarm_telemetry() {
        ...
        record_prewarm_report(
            &span,
            &fuigo_sampler::PrewarmReport {
                outcome: fuigo_sampler::PrewarmOutcome::Warmed,
                duration_ms: 7,
