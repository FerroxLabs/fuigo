...
struct RestoredSignals {
    ...
    turn_count: u64,
    tool_call_count: u64,
    plan_mode_state: fuigo_telemetry::events::PlanModeState,
...
impl RestoredSignals {
    fn read(
        ...
        use fuigo_telemetry::events::PlanModeState as Reported;
        Self {
...
impl MvpAgent {
    pub(super) async fn new_session_inner(
        ...
        if product_analytics || fuigo_telemetry::external::is_active() {
            ...
            tokio::spawn(async move {
                let git = fuigo_telemetry::context::collect_git_context(&cwd_str);
                let ev = fuigo_telemetry::events::SessionNew {
                    session_id: sid,
                ...
                fuigo_telemetry::session_ctx::log_event_dual(product_analytics, ev);
            });
        }
        ...
        let (git_root, is_git_repo, discovery_failed) =
            match fuigo_workspace::session::git::discover_git_root(cwd.as_path()) {
    ...
    async fn attach_session(
        ...
        persistence_timer.with_subphase(fuigo_telemetry::startup::Subphase::SessionLoad);
        ...
        let envrc = if !load_envrc {
            Some(fuigo_workspace::envrc::spawn_envrc_load(
        ...
        if self.product_analytics_enabled() {
            log_event(fuigo_telemetry::events::SessionLoad {
    ...
    async fn restore_session_code(
        ...
        {
            tracing::warn!(
                target: fuigo_workspace::session::git::RESTORE_CODE_LOG,
                session_id = %session_id.0,
                supplied_cwd = %cwd.as_str(),
                persisted_cwd = %summary.info.cwd,
                target_sha = %target_sha,
                "restore_code: skipping session HEAD checkout — supplied cwd is neither a fuigo worktree nor the session's persisted cwd (refusing to detach the source repo)"
            );
            ...
            && let Some(ref target_sha) = summary.head_commit
        {
            ...
            let outcome = fuigo_workspace::session::git::checkout_session_commit(
                cwd.as_path(),
                target_sha,
                true,
                session_id.0.as_ref(),
    ...
    pub(super) async fn restore_persisted_model(
        ...
        let is_fuigo_build = persisted_model.0.starts_with("fuigo-build");
        let same_family_fallback = if is_fuigo_build {
            available
                .keys()
                .find(|id| id.0.starts_with("fuigo-build"))
                .cloned()
        } else {
            available
                .keys()
    ...
    async fn build_attach_response_meta(
        ...
        if summary.head_commit.is_some()
            ...
            && summary.git_root_dir.as_deref().is_none_or(|root| {
                fuigo_workspace::session::git::find_git_root_from_path(std::path::Path::new(
                    cwd.as_str(),
                ))
                .ok()
            ...
            })
        {
            ...
            git_scan_timer.with_subphase(fuigo_telemetry::startup::Subphase::SessionGitScan);
            let cwd_path = std::path::Path::new(cwd.as_str());
            let current_head =
                fuigo_workspace::session::git::git_cli(cwd_path, &["rev-parse", "HEAD"])
                    .await
                    .ok();
            if let Some(divergence) = fuigo_workspace::session::git::detect_head_divergence(
