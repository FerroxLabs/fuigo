...
use crate::auth::{AuthManager, FuigoComConfig, OidcAuthConfig};
...
pub struct EndpointsConfig {
    ...
    /// Env: `FUIGO_MODELS_LIST_URL`. Overrides the default `{base}/models` list URL.
    #[serde(alias = "models_endpoint", skip_serializing_if = "Option::is_none")]
    pub models_list_url: Option<String>,
    /// Env: `FUIGO_FEEDBACK_BASE_URL`. Where feedback submissions go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub feedback_base_url: Option<String>,
    /// Env: `FUIGO_TRACE_UPLOAD_URL`. Where trace uploads go.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_url: Option<String>,
    /// Env: `FUIGO_TRACE_UPLOAD_BUCKET`. Direct bucket (`gs://` or `s3://`), bypasses proxy.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_bucket: Option<String>,
    /// Env: `FUIGO_TRACE_UPLOAD_REGION`. AWS region (S3 only).
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_region: Option<String>,
    /// Env: `FUIGO_TRACE_UPLOAD_CREDENTIALS_FILE`. Path to GCS SA key or AWS credentials file.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `FUIGO_TRACE_UPLOAD_ENDPOINT_URL`. Custom S3-compatible endpoint.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub trace_upload_endpoint_url: Option<String>,
    /// Env: `FUIGO_DEPLOYMENT_KEY`. Management API key for enterprise deployments.
    /// Sent on telemetry and service requests for deployment-level attribution.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub deployment_key: Option<String>,
    /// Env: `FUIGO_MANAGED_CONFIG_URL`. Override the managed config endpoint.
    ...
    /// Legacy repoint of the INTERNAL trace pipeline, deprecated in favor of `FUIGO_INTERNAL_OTLP_TRACES_ENDPOINT`.
    /// Ignored by the internal pipeline when `FUIGO_EXTERNAL_OTEL` is set (the standard `OTEL_*` vars then route the external stream only).
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Env: `FUIGO_INTERNAL_OTLP_TRACES_ENDPOINT`. Full INTERNAL traces endpoint, used verbatim.
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub fuigo_internal_otlp_traces_endpoint: Option<String>,
    /// Env: `FUIGO_INTERNAL_OTLP_HEADERS`.
    /// `k=v,k2=v2` extra headers for the internal export (debug).
    /// Wins over the legacy `OTEL_EXPORTER_OTLP_HEADERS`.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub fuigo_internal_otlp_headers: Option<String>,
    /// External-OTEL master switch, captured at construction via [`external_otel_master_switch_resolved`].
    /// That resolver applies requirement pin > `FUIGO_EXTERNAL_OTEL` env > `[telemetry].otel_enabled` config, managed layers included.
...
/// Parse a `k=v,k2=v2` OTLP header list (the `OTEL_EXPORTER_OTLP_HEADERS` format, shared with `FUIGO_INTERNAL_OTLP_HEADERS`).
...
impl EndpointsConfig {
    ...
    /// Trace upload endpoint, an auxiliary service, so it defaults to the cli-chat-proxy, never `fuigo_api_base_url`.
    pub(crate) fn resolve_trace_upload_url(&self) -> String {
        blank_as_unset(&self.trace_upload_url).unwrap_or_else(|| self.proxy_url())
    }
    /// Managed deployment-config URL (`fuigo setup`): explicit `managed_config_url`, else `proxy_url` + `/deployment/config`.
    /// Never `fuigo_api_base_url`, so the deployment key reaches the proxy, not the inference host.
    ...
    /// 1. `fuigo_internal_otlp_traces_endpoint` (verbatim)
    ...
    /// Uses the proxy default (not the `fuigo_api_base_url` fallback) so telemetry reports to Ferrox Labs even when inference is overridden.
    pub(crate) fn resolve_otlp_traces_endpoint(&self) -> String {
        if let Some(full) = blank_as_unset(&self.fuigo_internal_otlp_traces_endpoint) {
            ...
            && let Some(legacy) = self.legacy_internal_otlp_traces_endpoint()
        {
            tracing::warn!(
                "Repointing the internal trace pipeline via OTEL_EXPORTER_OTLP_ENDPOINT / \
                 ...
                 FUIGO_INTERNAL_OTLP_TRACES_ENDPOINT instead — the standard OTEL_* vars will \
    ...
    /// Extra headers for the INTERNAL export: `fuigo_internal_otlp_headers` first.
    ...
    pub(crate) fn resolve_otlp_headers(&self) -> Vec<(String, String)> {
        if let Some(headers) = blank_as_unset(&self.fuigo_internal_otlp_headers) {
    ...
    /// True when the master switch is unset, the standard var for that half is non-blank, and no `fuigo_internal_otlp_*` override shadowed it.
    ///
    ...
    pub(crate) fn internal_otlp_consumed_standard_vars(&self) -> bool {
        ...
        let endpoint_consumed = blank_as_unset(&self.fuigo_internal_otlp_traces_endpoint).is_none()
            && self.legacy_internal_otlp_traces_endpoint().is_some();
        let headers_consumed = blank_as_unset(&self.fuigo_internal_otlp_headers).is_none()
    ...
    /// Deliberately still honored by the internal pipeline even with `FUIGO_EXTERNAL_OTEL` set: disabling internal span export is the safe direction.
...
impl Default for EndpointsConfig {
    fn default() -> Self {
        Self {
            cli_chat_proxy_base_url: std::env::var("FUIGO_CLI_CHAT_PROXY_BASE_URL").ok(),
            fuigo_api_base_url: std::env::var("FUIGO_API_BASE_URL")
                .unwrap_or_else(|_| FUIGO_API_BASE_URL_DEFAULT.to_owned()),
            alpha_test_key: None,
            models_base_url: env_string("FUIGO_MODELS_BASE_URL"),
            models_list_url: env_string("FUIGO_MODELS_LIST_URL"),
            feedback_base_url: env_string("FUIGO_FEEDBACK_BASE_URL"),
            trace_upload_url: env_string("FUIGO_TRACE_UPLOAD_URL"),
            trace_upload_bucket: env_string("FUIGO_TRACE_UPLOAD_BUCKET"),
            trace_upload_region: env_string("FUIGO_TRACE_UPLOAD_REGION"),
            trace_upload_credentials_file: env_string("FUIGO_TRACE_UPLOAD_CREDENTIALS_FILE"),
            trace_upload_credentials: None,
            trace_upload_endpoint_url: env_string("FUIGO_TRACE_UPLOAD_ENDPOINT_URL"),
            deployment_key: env_string("FUIGO_DEPLOYMENT_KEY"),
            managed_config_url: env_string("FUIGO_MANAGED_CONFIG_URL"),
            ...
            fuigo_internal_otlp_traces_endpoint: env_string("FUIGO_INTERNAL_OTLP_TRACES_ENDPOINT"),
            fuigo_internal_otlp_headers: env_string("FUIGO_INTERNAL_OTLP_HEADERS"),
...
/// Those live in `auth::manager` (`FUIGO_AUTH`/`FUIGO_AUTH_PATH`) and `auth_method` (`FUIGO_API_KEY`/legacy).
...
pub(crate) const FIRST_PARTY_CREDENTIAL_ENV_VARS: &[&str] = &[
    crate::agent::auth_method::FUIGO_API_KEY_ENV_VAR,
    crate::agent::auth_method::LEGACY_FUIGO_API_KEY_ENV_VAR,
    "FUIGO_AUTH",
    "FUIGO_AUTH_PATH",
...
pub(crate) fn resolve_compaction_mode_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> fuigo_chat_state::CompactionMode {
...
pub(crate) fn resolve_compaction_detail_from(
    ...
    config: Option<&str>,
    remote: Option<&str>,
) -> fuigo_chat_state::CompactionDetail {
    use fuigo_chat_state::CompactionDetail;
...
fn resolve_compat_cell(
    ...
    resolve_compat_cell_with_env(fuigo_config::env_bool(env), cfg, remote, default)
}
...
pub(crate) fn compat_config_cell(
    ...
    cell: fuigo_tools::types::compat::CompatCell,
...
pub(crate) use fuigo_telemetry::config::env_telemetry_mode;
pub use fuigo_telemetry::config::{TelemetryConfig, TelemetryMode};
    ...
    pub(crate) fn merge_claude_enabled_plugins(&mut self, _cwd: Option<&std::path::Path>) {
        ...
        for path in &paths {
            let (claude_enabled, claude_disabled) =
                fuigo_agent::plugins::marketplace::load_enabled_disabled_plugins(path);
...
pub struct CliConfig {
    ...
    /// Env `FUIGO_MAXIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
    pub maximum_version: Option<String>,
    /// Env `FUIGO_REQUIRED_MINIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
    pub required_minimum_version: Option<String>,
    /// Env `FUIGO_REQUIRED_MAXIMUM_VERSION`. See [`crate::util::config::VersionPolicy`].
    #[serde(skip_serializing_if = "Option::is_none")]
...
///
/// Optional default Computer Hub URL for **workspace provider** exposure (`fuigo workspace` / leader `with_default_hub_url`).
...
pub struct HubConfig {
    ...
    /// `fuigo workspace start` when the CLI does not pass `--hub-url`.
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl SandboxSettingsConfig {
    ...
    pub fn resolve_profile(
        ...
        resolve_string_flag(cli_arg, "FUIGO_SANDBOX", self.profile.as_deref(), None)
    ...
    pub(crate) fn resolve_auto_allow_bash(&self, requirement: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("FUIGO_SANDBOX_AUTO_ALLOW_BASH")
...
///
/// These supplement the built-in scan locations (`.fuigo/skills/`,
/// `.agents/skills/`, `~/.fuigo/skills/`). They're written by `/import-claude`
...
pub struct Config {
    ...
    pub fuigo_com_config: FuigoComConfig,
    ...
    /// Typed as `FuigoComConfig` (same schema) so sub-field typos are caught.
    #[serde(default, skip_serializing)]
    pub auth: Option<FuigoComConfig>,
    /// `[desktop]` section: owned by fuigo-desktop (Electron app), opaque to the CLI agent.
    #[serde(default, skip_serializing)]
    pub desktop: Option<toml::Value>,
    /// Top-level `announcements` array: consumed by `resolve_announcements`.
    #[serde(default, skip_serializing)]
    pub announcements: Vec<fuigo_announcements::RemoteAnnouncement>,
    ...
    /// `[marketplace]`: also read by `fuigo_plugin_marketplace::load_sources()`.
    #[serde(default, skip_serializing)]
    ...
    /// Defaults to reading from FUIGO_STORAGE_MODE env var.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub cli_agents: Vec<fuigo_agent::config::AgentDefinition>,
    #[serde(skip)]
    pub cli_agent_overrides: CliAgentOverrides,
    /// Whether subagent (task tool) support is enabled.
    /// Enabled by default; disabled only via `FUIGO_SUBAGENTS=0` or `[subagents] enabled = false`.
    /// Not remotely gated.
    #[serde(skip)]
    ...
    #[serde(skip)]
    pub media_gen_batch_limits: fuigo_tools::media_gen_limits::MediaGenBatchLimits,
    ...
    #[serde(skip)]
    pub subagent_roles:
        std::collections::HashMap<String, fuigo_subagent_resolution::config::SubagentRole>,
    /// Trust-independent personas from inline, user, and bundled sources.
    #[serde(skip)]
    pub subagent_personas:
        std::collections::HashMap<String, fuigo_subagent_resolution::config::SubagentPersona>,
...
pub struct CliAgentOverrides {
    ...
    pub permission_rules: Vec<fuigo_workspace::permission::types::PermissionRule>,
    pub max_turns: Option<u32>,
    pub permission_mode: Option<fuigo_agent::config::PermissionMode>,
}
impl CliAgentOverrides {
    ...
    pub(crate) fn apply_to_definition(&self, def: &mut fuigo_agent::config::AgentDefinition) {
    ...
    pub(crate) fn apply_to_subagent_definition(
        &self,
        def: &mut fuigo_agent::config::AgentDefinition,
    ) {
...
pub use fuigo_agent::config::AgentDefinition;
pub use fuigo_agent::config::Effort;
pub use fuigo_agent::config::PermissionMode;
pub use fuigo_shared::ui_config::{ContextualHints, UiConfig};
/// Set in `config.toml` under `[agent]`:
///
/// ```toml
/// [agent]
/// # Use a named agent (looked up via discovery: .fuigo/agents/, ~/.fuigo/agents/, built-ins)
...
/// 4. `FUIGO_AGENT` env var
/// 5. Default `fuigo-build` agent
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
pub struct AgentSelectionConfig {
    /// Name of a built-in or discovered agent definition.
    /// Looked up via `fuigo_agent::discovery::by_name_in_cwd()`.
    /// Examples: "fuigo-build", "browser-use", or a custom agent name.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub name: Option<String>,
    /// Path to an agent definition file (.md with YAML frontmatter).
    /// Supports environment variable expansion (e.g., `$HOME/.fuigo/agents/my-agent.md`).
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl Default for Config {
    fn default() -> Self {
        ...
        let mut cfg = Self {
            ...
            fuigo_com_config: FuigoComConfig::default(),
...
/// Parse `[auth_provider.<name>]` tables leniently: a malformed entry warns (surfaced by `fuigo inspect`) and is skipped.
...
impl Config {
    ...
    pub fn new_from_toml_cfg(raw_config: &toml::Value) -> Result<Self, String> {
        ...
        if config.fuigo_com_config.oidc.is_none() {
            config.fuigo_com_config.oidc = OidcAuthConfig::from_env();
        }
        if config.fuigo_com_config.oidc.is_none() && config.fuigo_com_config.oauth2.is_none() {
            config.fuigo_com_config.oauth2 = crate::auth::OAuth2ProviderConfig::from_env();
        }
    ...
    pub fn resolve_runtime_fields(&mut self, ctx: &RuntimeResolutionContext<'_>) {
        ...
        self.media_gen_batch_limits = fuigo_tools::media_gen_limits::MediaGenBatchLimits {
    ...
    /// If the TOML contains `[auth]`, copy its contents under `[fuigo_com_config]`.
    /// `[fuigo_com_config]` takes precedence if both are present (explicit wins).
    ///
    /// This lets customers write the shorter `[auth.oidc]` instead of `[fuigo_com_config.oidc]`.
    fn expand_auth_alias(raw_config: &toml::Value) -> toml::Value {
        ...
        {
            if let Some(gcc) = table.get_mut("fuigo_com_config") {
                ...
                table.insert("fuigo_com_config".to_owned(), auth);
            }
        }
        config
    }
    fn apply_env_overrides(&mut self) {
        ...
        if let Some(mode) = env_telemetry_mode("FUIGO_TELEMETRY_ENABLED") {
            self.features.telemetry = Some(mode);
        }
        self.fuigo_com_config.force_login_team_uuid = crate::auth::resolve_force_login_team(
            crate::auth::force_login_team_from_requirements(),
            crate::auth::force_login_team_from_env(),
            self.fuigo_com_config.force_login_team_uuid.take(),
        );
    }
    ...
    pub(crate) fn resolve_trace_upload(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("FUIGO_TELEMETRY_TRACE_UPLOAD")
    ...
    pub(crate) fn trace_upload_decision_debug(&self) -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            "in_env_trace_upload": std::env::var("FUIGO_TELEMETRY_TRACE_UPLOAD").ok(),
            "in_env_telemetry_enabled": std::env::var("FUIGO_TELEMETRY_ENABLED").ok(),
    ...
    /// It covers the `x-fuigo-doom-loop-check` header, trigger parsing, and confident-signal resampling, all applied by the sampler.
    ...
    /// Gate precedence: env `FUIGO_DOOM_LOOP_RECOVERY` > TOML `enabled` > remote `enabled` > default ON.
    ...
    pub(crate) fn resolve_doom_loop_recovery(
        &self,
    ) -> Option<fuigo_sampling_types::DoomLoopRecoveryPolicy> {
        use fuigo_sampling_types::DoomLoopRecoveryPolicy as Policy;
        let remote = self
            .remote_settings
            .as_ref()
            .and_then(|s| s.doom_loop_recovery.as_ref());
        let enabled = BoolFlag::env("FUIGO_DOOM_LOOP_RECOVERY")
    ...
    /// Platform age-expiry (`process_cwd_scan_available`: linux+macos) is enforced inside `fuigo_fast_worktree::maybe_auto_gc`, not here.
    pub(crate) fn resolve_worktree_auto_gc(&self) -> fuigo_fast_worktree::ResolvedWorktreeAutoGc {
    ...
    /// Gate first-run auto-registration of the official Ferrox Labs marketplace source.
    /// Precedence: env `FUIGO_OFFICIAL_MARKETPLACE_AUTO_REGISTER` > remote settings > default off.
    ...
    pub(crate) fn resolve_official_marketplace_auto_register(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("FUIGO_OFFICIAL_MARKETPLACE_AUTO_REGISTER")
            .feature_flag(ff)
            .default(false)
            .resolve()
    }
    ...
    pub(crate) fn resolve_title_refresh(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("FUIGO_TITLE_REFRESH")
    ...
    pub(crate) fn resolve_image_gen(&self) -> Resolved<bool> {
        use fuigo_tools::implementations::fuigo_build::IMAGE_GEN_TOOL_NAME;
    ...
    pub(crate) fn resolve_image_edit(&self) -> Resolved<bool> {
        use fuigo_tools::implementations::fuigo_build::IMAGE_EDIT_TOOL_NAME;
        ...
        BoolFlag::env("FUIGO_IMAGE_EDIT").default(true).resolve()
    }
    ...
    pub(crate) fn resolve_image_gen_model_override(&self) -> Option<String> {
        resolve_string_flag(
            None,
            "FUIGO_IMAGE_GEN_MODEL_OVERRIDE",
    ...
    pub(crate) fn resolve_image_edit_model_override(&self) -> Option<String> {
        resolve_string_flag(
            None,
            "FUIGO_IMAGE_EDIT_MODEL_OVERRIDE",
    ...
    pub(crate) fn resolve_goal(&self) -> Resolved<bool> {
        ...
        BoolFlag::env("FUIGO_GOAL")
            .config(self.goal.enabled)
            .feature_flag(ff)
            .default(true)
            .resolve()
    }
    /// Background workflows (`workflow` tool, `.fuigo/workflows/*.rhai`, `/deep-research`, host-owned `/goal` driver).
    ...
    pub(crate) fn resolve_goal_classifier_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("FUIGO_GOAL_CLASSIFIER")
    ...
    pub(crate) fn resolve_goal_planner_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("FUIGO_GOAL_PLANNER")
    ...
    pub(crate) fn resolve_goal_summary_enabled(&self, goal_enabled: bool) -> Resolved<bool> {
        BoolFlag::env("FUIGO_GOAL_SUMMARY")
    ...
    pub(crate) fn resolve_goal_use_current_model_only(&self) -> Resolved<bool> {
        BoolFlag::env("FUIGO_GOAL_USE_CURRENT_MODEL_ONLY")
    ...
    /// Resolve the mode: env `FUIGO_COMPACTION_MODE` > config > remote settings > default, with unrecognized values falling through.
    /// For `Segments`, attach the separately-resolved detail level.
    pub(crate) fn resolve_compaction_mode(&self) -> fuigo_chat_state::CompactionMode {
        resolve_compaction_mode_from(
            env_string("FUIGO_COMPACTION_MODE").as_deref(),
    ...
    /// Precedence: env `FUIGO_COMPACTION_DETAIL` > config `features.compaction_detail` > remote `compaction_detail` > default (`verbose`).
    /// Drives the `segments` verbatim detail level.
    fn resolve_compaction_detail(&self) -> fuigo_chat_state::CompactionDetail {
        resolve_compaction_detail_from(
            env_string("FUIGO_COMPACTION_DETAIL").as_deref(),
    ...
    /// Resolve whether to use fuigo's default OAuth2 (Ferrox Labs auth.x.ai).
    ///
    /// Enterprise OIDC (`oidc` in config.toml) always wins; this only gates the default Ferrox Labs OAuth2 fallback when no enterprise OIDC is configured.
    ///
    /// Priority: `--oauth` > FUIGO_OAUTH_ENABLED env > default (true, meaning OAuth).
    pub(crate) fn resolve_fuigo_oauth(&self, cli_oidc: Option<bool>) -> Resolved<bool> {
        BoolFlag::env("FUIGO_OAUTH_ENABLED")
            .cli(cli_oidc)
            .default(true)
            .resolve()
    }
...
///
/// `requirement > cli > env (FUIGO_MCP_LIVENESS_WATCHERS) > config > managed > feature_flag > default (true)`.
///
...
pub(crate) fn resolve_mcp_liveness_watchers(
    ...
    BoolFlag::env("FUIGO_MCP_LIVENESS_WATCHERS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// Stacks the full 7-step `BoolFlag` precedence:
///
/// `requirement > cli > env (FUIGO_MCP_AUTO_RESTART) > config > managed > feature_flag > default (true)`.
///
...
///
/// Recovery is on by default; opt out via `FUIGO_MCP_AUTO_RESTART=false`, `[features] mcp_auto_restart`, or `requirements.toml`.
pub(crate) fn resolve_mcp_auto_restart(
    ...
    BoolFlag::env("FUIGO_MCP_AUTO_RESTART")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// Standard `BoolFlag` precedence; env `FUIGO_TURN_TRANSIENT_RETRY`; default on.
pub(crate) fn resolve_turn_transient_retry(
    ...
    BoolFlag::env("FUIGO_TURN_TRANSIENT_RETRY")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
///
/// `requirement > cli > env (FUIGO_MCP_PUSH_SERVER_STATUS) > config > managed > feature_flag > default (true)`.
///
...
pub fn resolve_mcp_push_server_status(
    ...
    BoolFlag::env("FUIGO_MCP_PUSH_SERVER_STATUS")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
/// Stacks the same 7-step `BoolFlag` precedence as [`resolve_mcp_liveness_watchers`]:
///
/// `requirement > cli > env (FUIGO_MCP_RECURSIVE_CONFIG_WATCH) > config > managed > feature_flag > default (true)`.
///
...
/// If FSEvents flakiness on macOS or an inotify-quota issue on Linux causes a regression, operators set `FUIGO_MCP_RECURSIVE_CONFIG_WATCH=0`.
...
pub(crate) fn resolve_mcp_recursive_config_watch(
    ...
    BoolFlag::env("FUIGO_MCP_RECURSIVE_CONFIG_WATCH")
        .requirement(requirement)
        .cli(cli)
        .config(config)
        .managed(managed)
...
impl SyncBoolFlag {
    ...
    /// Either-direction env resolver (typically `FUIGO_*`).
...
pub(crate) fn is_telemetry_disabled_sync() -> bool {
    !SyncBoolFlag::new(telemetry_enabled_from_toml)
        ...
        .enable_env(fuigo_telemetry_env_enabled)
        .resolve()
}
...
pub fn is_error_reporting_disabled_sync() -> bool {
    !SyncBoolFlag::new(error_reporting_enabled_from_toml)
        ...
        .enable_env(|| env_bool("FUIGO_ERROR_REPORTING"))
...
/// `FUIGO_TELEMETRY_ENABLED` resolved through `TelemetryMode::parse` so the extended string forms (e.g. `"session_metrics"`) are accepted.
...
pub(crate) fn external_otel_master_switch_resolved() -> bool {
    external_otel_master_switch_from(
        fuigo_config::load_merged_requirements().as_ref(),
        env_bool("FUIGO_EXTERNAL_OTEL"),
...
/// That config already includes managed-config layers distributed by `fuigo setup`.
...
pub fn resolve_external_otel_config(
    client: fuigo_telemetry::external::config::ExternalClientInfo,
) -> Option<fuigo_telemetry::external::ExternalOtelConfig> {
...
pub(crate) fn resolve_external_otel_config_with(
    ...
    let file_cfg: Option<fuigo_telemetry::external::ExternalOtelFileConfig> = effective_config
        .and_then(|cfg| cfg.get("telemetry"))
        .map(|t| fuigo_telemetry::external::ExternalOtelFileConfig {
    ...
    let getenv_pinned = |name: &str| -> Option<String> {
        let pin = match name {
            fuigo_telemetry::external::config::ENV_MASTER_SWITCH => req_enabled,
    ...
    let mut resolved = fuigo_telemetry::external::ExternalOtelConfig::resolve_with(
        getenv_pinned,
        file_cfg.as_ref(),
    )?;
...
pub(crate) fn apply_external_otel_remote_policy(
    ...
    let policy = fuigo_telemetry::external::ExternalOtelRemotePolicy {
    ...
    if policy.force_disable || policy.lock_content_gates {
        fuigo_telemetry::external::apply_remote_policy(policy);
    }
}
    ...
    for (key, model_override) in &cfg.config_models {
        ...
        let session_bearer_unsafe = !crate::util::is_fuigo_api_bearer_url(&entry.info.base_url)
            || entry
                .api_base_url
                .as_deref()
                .is_some_and(|url| !crate::util::is_fuigo_api_bearer_url(url));
    ...
    entries
        ...
        .map(|m| {
            ...
            let config = ModelEntryConfig {
                ...
                api_base_url: Some(endpoints.fuigo_api_base_url.clone()),
                name: m.name,
...
pub struct ModelEntryConfig {
    ...
    /// If not set, falls back to env_key, then FUIGO_API_KEY.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// If not set, falls back to FUIGO_API_KEY.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// Without this set, Ferrox Labs API models send args as one delta event, defeating the purpose of streaming.
    ///
...
pub struct ModelInfo {
    ...
    /// The routing slug sent in API requests.
    pub model: String,
    /// Provider family that mints this model's conversation items (e.g. "fuigo"); `None` means unknown.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub model_family: Option<String>,
...
/// codebase_indexing = ["/Users/*/fuigo*", "!/Users/*/old-*"]           # globs, ! to exclude
/// ```
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(untagged)]
...
pub struct AutoModeConfig {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prompt_type: Option<fuigo_workspace::permission::ClassifierPromptType>,
...
pub struct Features {
    ...
    /// Show a blocking warning when Fuigo starts outside a Git repository.
    /// Default: false.
    /// Used as the local fallback when the `non_git_warning` remote settings flag in `fuigo_build_settings` is absent.
    ...
    /// `None` defers to CLI / env (`FUIGO_COMPACTION_MODE`).
    /// Parsed via `CompactionMode::parse`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub compaction_mode: Option<String>,
    /// `none` | `minimal` | `balanced` | `verbose` (default).
    /// `None` defers to env (`FUIGO_COMPACTION_DETAIL`).
    ...
    /// `None` defers to env / default (recovery is on by default; set `false` here / via `FUIGO_MCP_AUTO_RESTART` to opt out).
    ///
    /// Not read through this struct: the live resolver re-reads the `[features]` key out-of-band from raw TOML in `util::config::resolve::mcp`.
    /// Declared so `serde_ignored` does not report it as an unrecognized key.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    /// `~/.fuigo/config.toml` will NOT disable the pager's
    /// subscription on a freshly-launched process.
    /// To disable the pager subscription, set `FUIGO_MCP_PUSH_SERVER_STATUS=0` in the env before launch.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub mcp_push_server_status: Option<bool>,
    /// Whether the leader's `ConfigFileWatcher` adds the two narrow non-recursive watches for `<cwd>/` and `<cwd>/.fuigo/`.
    ///
    /// When `true` (default), the watcher sees edits to `<cwd>/.mcp.json`, `<cwd>/.fuigo/config.toml`, or `<cwd>/.claude.json`.
...
pub(crate) struct ResolvedCredentials {
    pub api_key: Option<String>,
    pub base_url: String,
    pub auth_type: fuigo_chat_state::AuthType,
    pub auth_scheme: AuthScheme,
}
...
pub use fuigo_telemetry::config::deployment_id_from_key;
...
pub(crate) fn try_resolve_model_credentials(
    ...
    enforce_disable_api_key_auth(
        &mut credentials,
        cfg.fuigo_com_config.api_key_auth_disabled(),
        session_key,
    );
    Some(credentials)
}
...
pub(crate) fn resolve_aux_model_sampling_config(
    ...
    if let Some(bearer) = fuigo_bearer {
...
/// That avoids forcing `image_description_model` onto the agent endpoint, which 404s on BYOK / non-proxy routes for internal slugs like `fuigo-build`.
...
pub(crate) fn resolve_chat_state_auth_type(
    model_id: &str,
    session_key: Option<&str>,
    fallback: fuigo_chat_state::AuthType,
) -> fuigo_chat_state::AuthType {
...
pub(crate) fn response_include_extensions(
    ...
    let is_trusted_route = crate::util::is_trusted_cli_chat_proxy_url(base_url)
        || crate::util::is_trusted_fuigo_https_url(base_url);
