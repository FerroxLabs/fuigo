...
pub fn bootstrap(
    ...
    fuigo_telemetry::id::prefetch_agent_id();
    fuigo_telemetry::startup::enter(fuigo_telemetry::startup::StartupPhase::Bootstrap);
    ...
    fuigo_telemetry::startup::enter(fuigo_telemetry::startup::StartupPhase::ModelCatalog);
    let models_manager = {
...
fn resolve_config(
    ...
    // env var > remote settings > Local. Skip remote settings for Generic (fuigo -p, subagents).
    let has_fuigo_auth = auth_manager.current().is_some_and(|a| a.is_fuigo_auth());
...
fn init_process(cfg: &AgentConfig, auth_manager: &AuthManager) {
    ...
    INIT.call_once(|| {
        ...
        crate::builtin::extract_builtin_files(&fuigo_home);
        if !cfg!(test) {
            // Deletes dirs; must never touch a unit-test process's real home.
            crate::builtin::purge_stale_extracted_skills(&fuigo_home);
        }

        crate::extensions::marketplace::purge_default_skills_installs(&fuigo_home);
        ...
        if cfg.resolve_official_marketplace_auto_register().value {
            crate::extensions::marketplace::ensure_official_marketplace_source(&fuigo_home);
        }
        ...
        fuigo_telemetry::session_ctx::log_event(limits.into_event());
    });
}
...
pub fn update_telemetry_config(config: &AgentConfig, auth_manager: &AuthManager) {
    ...
    // and that string comes from the FUIGO_CLIENT_NAME env var. Telemetry
    // init must never take down its caller — `fuigo update` is a repair
    ...
    if reqwest::header::HeaderValue::from_str(&user_agent).is_err() {
        tracing::warn!("telemetry init skipped: FUIGO_CLIENT_NAME yields an invalid user agent");
        return;
    }
    let fuigo_auth = auth_manager.current().filter(|a| a.is_fuigo_auth());
    let user_id = fuigo_auth.as_ref().map(|a| a.user_id.clone());
    let team_id = fuigo_auth.as_ref().and_then(|a| a.team_id.clone());
