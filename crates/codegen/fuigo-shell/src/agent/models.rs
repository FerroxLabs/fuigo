...
impl ModelFetchAuth {
    ...
    pub(crate) fn resolve(endpoints: &config::EndpointsConfig, has_cached_session: bool) -> Self {
        if endpoints.has_custom_endpoint() {
            ...
            Self::Deployment
        } else if crate::agent::auth_method::has_fuigo_api_key_env() {
            Self::ApiKey
        } else {
            Self::Session
        }
...
impl Default for ModelsManager {
    fn default() -> Self {
        ...
        let auth_manager = Arc::new(AuthManager::new(&fuigo_home, FuigoComConfig::default()));
        Self::new(
            None,
            IndexMap::new(),
...
impl ModelsManager {
    ...
    pub(crate) fn set_gateway(&self, gateway: fuigo_acp_lib::AcpAgentGatewaySender) {
    ...
    /// Hot-reload the catalog from `~/.fuigo/models_cache.json` after an external write (config-watcher detected).
    ...
    async fn bounded_startup_auth(auth_manager: &Arc<AuthManager>) -> Option<FuigoAuth> {
    ...
    async fn bounded_auth_refresh<F>(fut: F) -> Option<FuigoAuth>
    where
        F: std::future::Future<Output = Option<FuigoAuth>>,
    {
