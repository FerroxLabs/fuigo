...
const SETTINGS_CACHE_HMAC_KEY: &[u8] =
    b"fuigo-shell-settings-cache-hmac-v1-ba6c43d3-404f-4b5c-b0cd-df09b2f5bdf4";
...
impl SettingsCacheManager {
    pub(crate) fn new() -> Self {
        Self {
            path: crate::util::fuigo_home::fuigo_home().join(SETTINGS_CACHE_FILE),
    ...
    pub(crate) fn load_or_fetch(
        ...
        if crate::agent::config::env_bool("FUIGO_SETTINGS_CACHE") == Some(false) {
            return (fetch(), None);
        }
    ...
    fn identity(auth: &crate::auth::FuigoAuth, alpha_test_key: Option<&str>) -> String {
    ...
    fn load_fresh(&self, identity: &str, origin: &str) -> Option<CachedSettings> {
        ...
        if cache.fuigo_version != fuigo_version::VERSION {
...
mod settings_cache_tests {
    ...
    fn hand_written_file_with_stale_version_or_client_misses() {
        ...
        write(&SettingsCache {
            fuigo_version: format!("{}-stale", fuigo_version::VERSION),
            ..base()
        });
