...
fn test_manager() -> ModelsManager {
    let _ = tracing_subscriber::fmt()
        ...
        .with_test_writer()
        .try_init();
    let tmp = std::env::temp_dir().join("fuigo-test-models-manager");
    let auth_manager = Arc::new(AuthManager::new(&tmp, FuigoComConfig::default()));
...
async fn catalog_retry_recovers_after_endpoint_returns() {
    ...
    let tmp = std::env::temp_dir().join("fuigo-test-catalog-retry");
    let auth_manager = Arc::new(AuthManager::new(&tmp, FuigoComConfig::default()));
...
async fn auth_refresh_watcher_refetches_on_notify() {
    ...
    let calls = Arc::new(AtomicUsize::new(0));
    let tmp = std::env::temp_dir().join("fuigo-test-auth-refresh-watcher");
    let auth_manager = Arc::new(AuthManager::new(&tmp, FuigoComConfig::default()));
    let mgr = ModelsManagerBuilder::new(
        None,
        IndexMap::new(),
...
async fn first_catalog_wait_skips_doomed_signed_out_fetch() {
    let _no_key = EnvGuard::unset("FUIGO_API_KEY");
    let _no_legacy_key = EnvGuard::unset("FUIGO_CODE_API_KEY");
    let mgr = cold_manager(config::Config::default(), Arc::new(HangingEndpoint));
    let start = tokio::time::Instant::now();
    mgr.spawn_fetch_inner(None, /*remote_fetch_enabled*/ true);
    assert!(
...
fn current_reasoning_effort_seeded_from_config() {
    let tmp = std::env::temp_dir().join("fuigo-test-models-manager-seed");
    let auth_manager = Arc::new(AuthManager::new(&tmp, FuigoComConfig::default()));
...
fn config_menu_only_model_derives_support_and_default() {
    ...
    let tmp = std::env::temp_dir().join("fuigo-test-models-manager-menu-only");
    let auth_manager = Arc::new(AuthManager::new(&tmp, FuigoComConfig::default()));
    let mgr = ModelsManager::new(
        None,
        catalog,
...
fn apply_config_falls_back_when_preferred_not_in_catalog() {
    ...
    let mut new_cfg = config::Config::default();
    new_cfg.models.default = Some("fuigo-nonexistent".to_string());
    mgr.apply_config(new_cfg);
...
fn a_tampered_cache_cannot_supply_credentials() {
    ...
    let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
    let loaded = cache
        .load_fresh(&identity, &auth_method, &origin)
...
fn reload_from_disk_cache_ignores_stale_cache() {
    ...
    let stale = ModelsCache {
        ...
        identity: Some(mgr.cache_identity()),
        auth_method: Some(auth_method),
        origin: Some(mgr.cache_origin()),
        etag: Some("etag-stale".into()),
        models: make_prefetched(&["fuigo-stale"]),
    };
    cache.atomic_write(&stale);

    mgr.reload_from_cache_manager(&cache);

    assert!(!mgr.models().contains_key("fuigo-stale"));
...
fn reload_from_disk_cache_ignores_auth_method_mismatch() {
    ...
    cache.persist(
        &make_prefetched(&["fuigo-other-auth"]),
        Some("etag-x"),
        &mgr.cache_identity(),
        other,
        &mgr.cache_origin(),
    ...
    assert!(!mgr.models().contains_key("fuigo-other-auth"));
}
...
fn reload_from_disk_cache_ignores_origin_mismatch() {
    ...
    let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
    cache.persist(
        &make_prefetched(&["fuigo-other-origin"]),
        Some("etag-y"),
        &mgr.cache_identity(),
        auth_method,
    ...
    assert!(!mgr.models().contains_key("fuigo-other-origin"));
    assert!(mgr.inner.catalog.read().etag.is_none());
}
...
fn reload_from_disk_cache_ignores_legacy_cache_without_origin() {
    ...
    let legacy = ModelsCache {
        ...
        models: make_prefetched(&["fuigo-legacy"]),
    };
    cache.atomic_write(&legacy);

    mgr.reload_from_cache_manager(&cache);

    assert!(!mgr.models().contains_key("fuigo-legacy"));
}
...
fn resolve_custom_endpoint_always_wins() {
    let _key = EnvGuard::set("FUIGO_API_KEY", "test-key");
...
fn resolve_deployment_key_outranks_ambient_api_key() {
    let _key = EnvGuard::set("FUIGO_API_KEY", "stray-env-key");
    let endpoints = config::EndpointsConfig {
        deployment_key: Some("deploy-key".to_owned()),
        ..config::EndpointsConfig::default()
    };
    assert_eq!(
        ModelFetchAuth::resolve(&endpoints, false),
        ModelFetchAuth::Deployment,
        "managed deployment_key should outrank an ambient FUIGO_API_KEY",
    );
    assert_eq!(
        ModelFetchAuth::resolve(&endpoints, true),
        ModelFetchAuth::Session,
...
fn prefetch_env_none_when_remote_fetch_disabled_despite_credentials() {
    ...
    assert!(
        resolve_prefetch_env_from_parts(Some(FuigoAuth::test_default()), endpoints.clone(), false,)
            .is_none(),
...
fn build_prefetched_map_distinct_ids_same_slug() {
    let entries = vec![
        make_entry_config_with_id(Some("auto"), "fuigo-build", Some("Auto")),
        make_entry_config_with_id(Some("fuigo-build"), "fuigo-build", Some("Fuigo")),
    ...
    assert!(map.contains_key("fuigo-build"));
    assert!(map.contains_key("experimental-fast"));
    assert_eq!(
        map["auto"].info.model, "fuigo-build",
        "auto entry should still route to fuigo-build"
    );
    assert_eq!(map["fuigo-build"].info.model, "fuigo-build");
}
...
fn build_prefetched_map_duplicate_id_overwrites() {
    let entries = vec![
        make_entry_config_with_id(Some("fuigo-build"), "fuigo-build", Some("First")),
        make_entry_config_with_id(Some("fuigo-build"), "fuigo-build", Some("Second")),
    ];
    let map = build_prefetched_map(entries, None);

    assert_eq!(map.len(), 1, "duplicate id: second overwrites first");
    assert_eq!(map["fuigo-build"].info.name.as_deref(), Some("Second"));
}
...
fn resolve_default_model_prefers_id_over_model_slug() {
    ...
    catalog.insert(
        "auto-fuigo-build".to_string(),
        make_model_entry("fuigo-build"),
    );
    catalog.insert("fuigo-build".to_string(), make_model_entry("fuigo-build"));

    let mut cfg = config::Config::default();
    cfg.models.default = Some("fuigo-build".to_string());

    let (key, _, _) = resolve_default_model(&cfg, &catalog, true);
    assert_eq!(key, "fuigo-build", "must match id, not first slug hit");
}
...
fn resolve_catalog_key_maps_routing_slug_to_config_key() {
    ...
    models.insert(
        "enterprise-fuigo-build".to_string(),
    ...
    assert_eq!(key.0.as_ref(), "enterprise-fuigo-build");
}
...
fn resolve_catalog_key_last_slug_match_wins() {
    let mut models = IndexMap::new();
    models.insert(
        "default-fuigo-build".to_string(),
        make_model_entry("grok-4.5"),
    );
    models.insert("user-fuigo-build".to_string(), make_model_entry("grok-4.5"));

    let persisted = acp::ModelId::new("grok-4.5");
    let key = resolve_catalog_key(&models, &persisted).expect("slug must resolve");
    assert_eq!(key.0.as_ref(), "user-fuigo-build");
}
...
fn selectable_prefers_available_identity_over_non_selectable_exact_key() {
    let mut models = IndexMap::new();
    models.insert("fuigo-build".to_string(), make_model_entry("fuigo-build"));
    models.insert(
        "enterprise-fuigo-build".to_string(),
        make_model_entry("fuigo-build"),
    );
    ...
    let available = test_available_keys(&["enterprise-fuigo-build", "grok-4.3"]);

    let persisted = acp::ModelId::new("fuigo-build");
    assert_eq!(
...
fn selectable_prefers_exact_key_over_later_slug_match() {
    let mut models = IndexMap::new();
    models.insert("fuigo-build".to_string(), make_model_entry("grok-4.5"));
    models.insert("other".to_string(), make_model_entry("fuigo-build"));

    let available = test_available_keys(&["fuigo-build", "other"]);

    let persisted = acp::ModelId::new("fuigo-build");
    let key = selectable_catalog_key_for_persisted(&models, &available, &persisted)
        .expect("exact selectable key must win");
    assert_eq!(key.0.as_ref(), "fuigo-build");
}
...
async fn bounded_auth_refresh_times_out_to_none() {
    ...
    let result =
        ModelsManager::bounded_auth_refresh(std::future::pending::<Option<FuigoAuth>>()).await;
...
async fn bounded_auth_refresh_passes_through_ready_value() {
    let result =
        ModelsManager::bounded_auth_refresh(async { Some(FuigoAuth::test_default()) }).await;
    assert!(
        result.is_some(),
...
// ── per-account catalog keying and TTL-renewal safety (upstream 1.0.25) ──

#[test]
fn a_second_account_does_not_read_the_first_accounts_catalog() {
    // `~/.fuigo/models_cache.json` is one file per machine, and every other
    // field `load_fresh` checks is identical for two accounts on one host: the
    // auth METHOD collapses to `Session | ApiKey | Deployment`, the origin is
    // just the models-list URL, and the version is the build's. Without an
    // account discriminator, account B boots on account A's entitlements for
    // the whole 300s TTL.
    let mgr = test_manager();
    let tmp = tempfile::TempDir::new().unwrap();
    let cache = test_cache_manager(tmp.path());
    ...
    cache.persist(
        &make_prefetched(&["account-a-only"]),
        Some("etag-a"),
        "identity-account-a",
        auth_method.clone(),
        &origin,
    ...
    assert!(
        cache
            .load_fresh("identity-account-a", &auth_method, &origin)
            .is_some(),
        "the writing account must still hit its own cache, or this proves nothing",
    );
    assert!(
        cache
            .load_fresh("identity-account-b", &auth_method, &origin)
            .is_none(),
        "a second account must not read the first account's catalog",
    );
}

#[test]
fn a_cache_without_an_identity_is_a_miss() {
    // A catalog written by a build that predates the discriminator has no idea
    // whose it is. One refetch is the right price; serving it is not.
    let mgr = test_manager();
    let tmp = tempfile::TempDir::new().unwrap();
    let cache = test_cache_manager(tmp.path());
    let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
    ...
    let legacy = ModelsCache {
        ...
        etag: Some("etag-legacy".into()),
        models: make_prefetched(&["fuigo-pre-identity"]),
    };
    cache.atomic_write(&legacy);
    ...
    assert!(
        cache
            ...
            .is_none(),
        "an identity-less catalog must never be served to anybody",
    );
}
...
fn models_cache_identity_tracks_the_credential_without_naming_it() {
    ...
    let mut with_alpha_key = base.clone();
    with_alpha_key.alpha_test_key = Some("alpha-key-c".to_string());

    let base_id = models_cache_identity(None, &base);
    let deployment_id = models_cache_identity(None, &with_deployment_key);
    let alpha_id = models_cache_identity(None, &with_alpha_key);

    assert_ne!(base_id, deployment_id, "a deployment key is a new identity");
    assert_ne!(base_id, alpha_id, "an alpha test key is a new identity");
    assert_ne!(deployment_id, alpha_id);
    assert_eq!(
        base_id,
        models_cache_identity(None, &base),
        "the identity must be stable for the same credentials",
    );
    for id in [&deployment_id, &alpha_id] {
        assert!(
            !id.contains("deployment-key-b") && !id.contains("alpha-key-c"),
            "the identity is written to disk in cleartext; it must name no secret: {id}",
        );
    }
}

#[tokio::test]
async fn ttl_renewal_does_not_resurrect_a_catalog_rewritten_since_its_read() {
    // `renew_ttl` is an unlocked read-modify-write: read, stamp `fetched_at`,
    // atomic-rename back. A models fetch that persists a NEW catalog inside
    // that window is silently renamed away by the stale copy the renewal
    // holds -- and the stale catalog then gets a fresh full TTL.
    let mgr = test_manager();
    let tmp = tempfile::TempDir::new().unwrap();
    let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
    let identity = mgr.cache_identity();
    ...
    // The renewal now holds its snapshot of the stale catalog.
    tokio::time::timeout(
    ...
    let on_disk: ModelsCache = serde_json::from_slice(
        &std::fs::read(tmp.path().join(MODELS_CACHE_FILE)).expect("cache file is readable"),
    )
    .expect("cache file parses");
    assert!(
        on_disk.models.contains_key("fresh-catalog"),
        "the TTL renewal renamed a stale catalog back over a freshly persisted one",
    );
    assert!(
        !on_disk.models.contains_key("stale-catalog"),
        "the stale catalog must not be resurrected",
    );
}

#[tokio::test]
async fn ttl_renewal_still_stamps_an_untouched_cache() {
    // The guard above must not turn the renewal into a no-op: with nothing
    // racing it, a renewal still refreshes `fetched_at` and keeps the catalog.
    let mgr = test_manager();
    let tmp = tempfile::TempDir::new().unwrap();
    let cache = test_cache_manager(tmp.path());
    let auth_method = mgr.inner.fetch_auth.read().cache_auth_method();
    ...
    let aged = ModelsCache {
        fetched_at: Utc::now() - ChronoDuration::seconds(240),
        fuigo_version: Some(fuigo_version::VERSION.to_string()),
        identity: Some(identity.clone()),
        auth_method: Some(auth_method.clone()),
        origin: Some(origin.clone()),
        etag: Some("etag-aged".into()),
        models: make_prefetched(&["aged-catalog"]),
    };
    cache.atomic_write(&aged);

    cache.renew_ttl(&identity, &auth_method, &origin).await;
    ...
    .expect("cache file parses");
    assert!(
        on_disk.fetched_at > aged.fetched_at,
        "an uncontended renewal must still stamp the cache fresh",
    );
