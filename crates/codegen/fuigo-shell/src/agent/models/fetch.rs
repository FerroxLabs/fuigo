...
pub(crate) struct PrefetchEnv {
    pub(crate) auth: Option<FuigoAuth>,
...
pub(in crate::agent::models) fn prefetch_env(auth: Option<FuigoAuth>) -> Option<PrefetchEnv> {
...
pub(in crate::agent::models) fn resolve_disk_auth(
    fuigo_com_config: Option<FuigoComConfig>,
) -> Option<FuigoAuth> {
    let fuigo_home = crate::util::fuigo_home::fuigo_home();
    AuthManager::new(&fuigo_home, fuigo_com_config.unwrap_or_default()).current()
}
