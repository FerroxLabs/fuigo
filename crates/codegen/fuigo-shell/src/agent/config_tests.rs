...
fn main_cli_tools_override_preserves_profile_injection_policy() {
    ...
    let mut cases = vec![(AgentDefinition::default_fuigo_build(), true)];
...
fn auto_mode_config_parses_from_toml_and_json_equivalently() {
    use fuigo_workspace::permission::ClassifierPromptType;
    let toml_src = r#"
enabled = true
...
#[test]
fn resolve_aux_model_honors_fuigo_build_override() {
    let endpoints = EndpointsConfig::default();
    let mut catalog = IndexMap::new();
    catalog.insert(
        "fuigo-build".to_string(),
        test_model_entry(
...
/// Cold cache falls back to the session model, never the Ferrox Labs proxy; warm cache serves the provider token at the provider endpoint.
#[tokio::test]
...
fn shell_environment_policy_known_keys_track_the_policy_struct() {
    let fuigo_tools::util::ShellEnvironmentPolicy {
        inherit: _,
        ignore_default_excludes: _,
        exclude: _,
        set: _,
        include_only: _,
    } = fuigo_tools::util::ShellEnvironmentPolicy::default();
...
async fn static_key_shadows_defined_provider_through_pipeline() {
    ...
    assert_eq!(creds.auth_type, fuigo_chat_state::AuthType::ApiKey);
...
async fn set_env_key_shadows_warm_provider_at_resolve_time() {
    use fuigo_test_support::EnvGuard;
    let var = "FUIGO_TEST_ENVKEY_SHADOW";
...
fn default_models_dual_endpoint_routing() {
    ...
    for (model_id, entry) in default_model_entries(&endpoints) {
        ...
        assert_eq!(
            api_key_creds.base_url, endpoints.fuigo_api_base_url,
...
fn env_keys_resolve_first_set_wins() {
    let keys = EnvKeys::new(["FUIGO_TEST_ENV_KEY_PRIMARY", "FUIGO_TEST_ENV_KEY_FALLBACK"]);
    assert_eq!(keys.resolve_value_with(|_| None), None, "none set");
    assert_eq!(
        keys.resolve_value_with(
            |n| (n == "FUIGO_TEST_ENV_KEY_FALLBACK").then(|| "from-fallback".into())
        ),
        Some("from-fallback".into())
    );
    assert_eq!(
        keys.resolve_value_with(|n| match n {
            "FUIGO_TEST_ENV_KEY_PRIMARY" => Some("from-primary".into()),
            "FUIGO_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
            _ => None,
        }),
    ...
    assert_eq!(
        keys.resolve_value_with(|n| match n {
            "FUIGO_TEST_ENV_KEY_PRIMARY" => Some(String::new()),
            "FUIGO_TEST_ENV_KEY_FALLBACK" => Some("from-fallback".into()),
            _ => None,
        }),
        Some("from-fallback".into())
...
fn env_keys_resolve_skips_whitespace_only_value() {
    let keys = EnvKeys::new(["FUIGO_TEST_WS_PRIMARY", "FUIGO_TEST_WS_FALLBACK"]);
    assert_eq!(
        keys.resolve_value_with(|n| match n {
            "FUIGO_TEST_WS_PRIMARY" => Some("   ".into()),
            "FUIGO_TEST_WS_FALLBACK" => Some("real".into()),
            _ => None,
        }),
        Some("real".into())
    );
    assert_eq!(
        EnvKeys::single("FUIGO_TEST_WS_ONLY").resolve_value_with(|_| Some("   ".into())),
        None
    );
    assert_eq!(
        EnvKeys::single("FUIGO_TEST_WS_PAD").resolve_value_with(|_| Some("  tok  ".into())),
        Some("  tok  ".into())
    );
}
#[test]
...
fn first_own_credential_empty_api_key_falls_through_to_env_key() {
    use fuigo_test_support::EnvGuard;
    let var = "FUIGO_TEST_FIRST_OWN_CRED_ENV";
    let _guard = EnvGuard::set(var, "env-token");
...
fn resolve_credentials_multi_env_key_uses_lc_alias() {
    use fuigo_chat_state::AuthType;
    let primary = "FUIGO_TEST_MULTI_ENV_PRIMARY";
    let alias = "FUIGO_TEST_MULTI_ENV_LC_ALIAS";
    unsafe {
...
fn resolve_credentials_empty_env_key_falls_through_to_session() {
    use fuigo_chat_state::AuthType;
    use fuigo_test_support::EnvGuard;
    let primary = "FUIGO_TEST_EMPTY_ENV_PRIMARY";
    let alias = "FUIGO_TEST_EMPTY_ENV_LC_ALIAS";
...
fn resolve_credentials_empty_env_key_falls_through_to_global_key() {
    ...
    let primary = "FUIGO_TEST_EMPTY_ENV_GLOBAL_PRIMARY";
    let alias = "FUIGO_TEST_EMPTY_ENV_GLOBAL_ALIAS";
    let _primary = EnvGuard::set(primary, "");
    let _alias = EnvGuard::set(alias, "");
...
fn api_key_creds(base_url: &str) -> ResolvedCredentials {
    ResolvedCredentials {
        api_key: Some("fuigo-secret".to_string()),
        base_url: base_url.to_string(),
        auth_type: fuigo_chat_state::AuthType::ApiKey,
        auth_scheme: Default::default(),
    }
...
fn enforce_disable_api_key_auth_blocks_first_party_only() {
    ...
    assert_eq!(creds.auth_type, AuthType::ApiKey);
    assert_eq!(creds.api_key.as_deref(), Some("fuigo-secret"));
    let mut creds = api_key_creds("https://api.x.ai/v1");
...
fn try_resolve_model_credentials_swaps_first_party_own_key_under_kill_switch() {
    ...
    assert_eq!(creds.api_key.as_deref(), Some("fuigo-model-key"));
    enforce_disable_api_key_auth(&mut creds, true, Some("session-jwt"));
    assert_eq!(
        creds.auth_type,
...
fn x_api_key_auth_scheme_flows_from_config_to_sampler() {
    ...
    assert_eq!(config.api_backend, ApiBackend::Messages);
    let client = fuigo_sampler::SamplingClient::new(config).expect("client should build");
...
fn compaction_mode_precedence_env_over_config_over_remote_over_default() {
    ...
    assert_eq!(
        ...
        CompactionMode::Segments(fuigo_chat_state::CompactionDetail::default())
    );
    assert_eq!(
...
/// Without this, `--reasoning-effort` is silently dropped in fuigo-shell/src/agent/models.rs:857 for any BYOK Claude config.
#[test]
...
#[test]
fn model_agent_type_defaults_to_fuigo_build() {
    let raw_config: toml::Value = toml::from_str(
        r#"
            [model.my-model]
            model = "my-model"
...
fn invalid_glob_is_rejected_by_validation() {
    ...
    assert!(ModelGlobSet::compile(Some(&vec!["fuigo[".to_string()])).is_err());
    let raw: toml::Value = toml::from_str(
        r#"
            [models]
            allowed_models = ["fuigo["]
            "#,
    )
    .unwrap();
...
#[test]
fn auth_alias_maps_to_fuigo_com_config() {
    let raw: toml::Value = toml::from_str(
        r#"
            [auth.oidc]
    ...
    let oidc = cfg.fuigo_com_config.oidc.expect("oidc should be set");
    assert_eq!(oidc.issuer, "https://example.okta.com");
    assert_eq!(oidc.client_id, "test-id");
}
#[test]
fn fuigo_com_config_still_works() {
    let raw: toml::Value = toml::from_str(
        r#"
...
fn disable_api_key_auth_parses_from_auth_alias() {
    ...
    assert_eq!(absent.fuigo_com_config.disable_api_key_auth, None);
    let raw: toml::Value = toml::from_str(
        r#"
            [auth]
    ...
    .unwrap();
    let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
    assert_eq!(cfg.fuigo_com_config.disable_api_key_auth, Some(true));
}
...
fn force_login_team_uuid_parses_string_and_array() {
    ...
    let _g = crate::env::EnvVarGuard::remove("FUIGO_FORCE_LOGIN_TEAM_ID");
    assert!(
    ...
    let absent = Config::new_from_toml_cfg(&toml::from_str("").unwrap()).unwrap();
    assert_eq!(absent.fuigo_com_config.force_login_team_uuid, None);
    let raw: toml::Value = toml::from_str(
        r#"
            [auth]
    ...
    let cfg = Config::new_from_toml_cfg(&raw).expect("config should parse");
    assert_eq!(
        cfg.fuigo_com_config.force_login_team_uuid,
...
fn force_login_team_id_env_overrides_user_config() {
    use crate::auth::ForceLoginTeam;
    let _guard = crate::env::EnvVarGuard::set("FUIGO_FORCE_LOGIN_TEAM_ID", "env-team");
    assert!(
        crate::auth::force_login_team_from_requirements().is_none(),
        "clear the force_login_team_uuid pin in requirements.toml to run this test",
    );
    ...
    assert_eq!(
        from_env.fuigo_com_config.force_login_team_uuid,
    ...
    assert_eq!(
        overridden.fuigo_com_config.force_login_team_uuid,
        Some(ForceLoginTeam::Single("env-team".into())),
    );
}
...
fn force_login_team_id_env_unset_keeps_config_value() {
    use crate::auth::ForceLoginTeam;
    let _guard = crate::env::EnvVarGuard::remove("FUIGO_FORCE_LOGIN_TEAM_ID");
    assert!(
        crate::auth::force_login_team_from_requirements().is_none(),
        "clear the force_login_team_uuid pin in requirements.toml to run this test",
    );
...
fn force_login_team_uuid_implies_api_key_auth_disabled() {
    use crate::auth::{ForceLoginTeam, FuigoComConfig};
    let base = FuigoComConfig {
...
fn e2e_config_toml_model_overrides_default() {
    ...
    unsafe { std::env::set_var("FUIGO_API_KEY", "fuigo-key") };
    let sampling = resolve_sampling(model, None);
    assert_eq!(sampling.base_url, "https://inference.example.com/v1");
...
fn config_models_default_is_not_overwritten_by_default_models_json() {
    ...
    let resolved = resolve_string_flag(
        None,
        "FUIGO_DEFAULT_MODEL_TEST_NONEXISTENT",
        config_default,
...
fn config_models_default_custom_model_is_in_resolved_model_list() {
    ...
    assert!(
        models.contains_key("acme-fuigo"),
        "user-defined model must be in the resolved model list"
    );
    let model = models.get("acme-fuigo").unwrap();
    assert_eq!(model.info.model, "grok-4.5");
    assert_eq!(model.info.base_url, "https://inference.example.com/v1");
}
#[test]
...
fn e2e_default_model_with_external_api_key_routes_to_api_fuigo() {
    ...
    let model = models
        .get(crate::models::default_model())
        .expect("default model should exist");
    unsafe { std::env::set_var("FUIGO_API_KEY", "fuigo-external-key") };
    let sampling = resolve_sampling(model, None);
...
fn e2e_credential_priority_model_key_beats_session_beats_env() {
    ...
    unsafe { std::env::set_var("FUIGO_API_KEY", "env-key") };
...
fn e2e_duplicate_model_field_both_entries_survive() {
    ...
    let user = models.get("acme-fuigo").unwrap();
...
fn e2e_enterprise_custom_endpoint_skips_fuigo_defaults() {
    ...
    assert!(
        ...
        "Ferrox Labs default must not leak into enterprise model list"
    );
...
fn e2e_acp_model_info_no_dedup_on_model_field() {
    ...
    assert!(
        acp_models.contains_key(&acp::ModelId::new("default-fuigo")),
        "default entry should be addressable by map key"
    );
    assert!(
        acp_models.contains_key(&acp::ModelId::new("acme-fuigo")),
...
fn e2e_enterprise_endpoints_plus_partial_model_override() {
    ...
    let (_, models) = resolve_models_from_toml(
        &format!(
            ...
            fuigo_api_base_url = "https://enterprise-api.acme.com/v1"
...
fn e2e_enterprise_endpoints_only_no_model_override() {
    ...
    assert_eq!(
        model.api_base_url.as_deref(),
        Some("https://enterprise-api.acme.com/v1"),
        "default model should use enterprise fuigo_api_base_url"
    );
}
...
/// INVARIANT: auxiliary-service resolvers resolve to the cli-chat-proxy, never `fuigo_api_base_url`.
...
fn aux_endpoints_resolve_to_proxy_never_inference() {
    ...
    let cfg = EndpointsConfig {
        fuigo_api_base_url: inference.to_string(),
...
/// REGRESSION: the managed-config URL never follows `fuigo_api_base_url` through the full loader `Config::new_from_toml_cfg`.
...
fn resolve_title_refresh_defaults_to_turn_summary_but_decouples() {
    unsafe { std::env::remove_var("FUIGO_TITLE_REFRESH") };
    unsafe { std::env::remove_var("FUIGO_TURN_SUMMARY") };
    ...
    unsafe { std::env::set_var("FUIGO_TITLE_REFRESH", "0") };
    let r = decoupled.resolve_title_refresh();
    assert!(!r.value, "FUIGO_TITLE_REFRESH env wins");
    assert_eq!(r.source, ConfigSource::Env);
    unsafe { std::env::remove_var("FUIGO_TITLE_REFRESH") };
}
/// A `turn_summary` pin lands in the title's default slot, so it moves the title with it.
/// Only the default slot, so `FUIGO_TITLE_REFRESH` still outranks it and a user can turn the title back on.
/// Pinning `title_refresh` is what closes that.
#[test]
#[serial]
fn a_turn_summary_pin_moves_the_title_default_and_the_environment_lifts_it() {
    let _env = EnvGuard::set("FUIGO_TURN_SUMMARY", "1");
    let mut cfg = Config::default();
    ...
    {
        let _title = EnvGuard::unset("FUIGO_TITLE_REFRESH");
        assert!(
            !cfg.resolve_title_refresh().value,
            "the pin outranks FUIGO_TURN_SUMMARY, and the title default follows the pin"
        );
    }
    let _title = EnvGuard::set("FUIGO_TITLE_REFRESH", "1");
...
fn a_title_refresh_pin_outranks_the_environment() {
    let _env = EnvGuard::set("FUIGO_TITLE_REFRESH", "1");
    let mut cfg = Config::default();
    cfg.requirements
        .title_refresh
        .pin(false, crate::config::RequirementSource::Unknown);
    let r = cfg.resolve_title_refresh();
    assert!(!r.value, "the pin lost to FUIGO_TITLE_REFRESH");
...
fn resolve_doom_loop_recovery_precedence() {
    ...
    unsafe { std::env::remove_var("FUIGO_DOOM_LOOP_RECOVERY") };
    ...
    unsafe { std::env::set_var("FUIGO_DOOM_LOOP_RECOVERY", "0") };
    assert!(
...
fn worktree_auto_gc_section_parses_from_toml() {
    unsafe { fuigo_fast_worktree::clear_auto_gc_env_for_test() };
    let raw: toml::Value = toml::from_str(
        r#"
            [worktree.auto_gc]
            enabled = true
    ...
    assert_eq!(
        p.max_age_by_kind
            .get(&fuigo_fast_worktree::WorktreeKind::Subagent),
        Some(&Some(3600))
    );
    assert_eq!(
        p.max_age_by_kind
            .get(&fuigo_fast_worktree::WorktreeKind::Manual),
        Some(&None)
    );
}
...
fn resolve_trace_upload_disabled_when_telemetry_off_despite_remote_flag() {
    unsafe { std::env::remove_var("FUIGO_TELEMETRY_ENABLED") };
    unsafe { std::env::remove_var("FUIGO_TELEMETRY_TRACE_UPLOAD") };
    let mut cfg = Config::default();
...
fn resolve_goal_defaults_to_true_when_unset() {
    unsafe { std::env::remove_var("FUIGO_GOAL") };
    let cfg = Config::default();
...
fn resolve_goal_env_overrides_config_without_remote_kill_switch() {
    unsafe { std::env::set_var("FUIGO_GOAL", "1") };
    let mut cfg = Config::default();
...
fn background_workflows_default_on_without_affecting_goal() {
    unsafe { std::env::remove_var("FUIGO_WORKFLOWS") };
    let cfg = Config::default();
...
fn resolve_workflows_remote_false_kills_local_opt_in() {
    unsafe { std::env::set_var("FUIGO_WORKFLOWS", "1") };
    let mut cfg = Config::default();
...
fn resolve_workflows_env_wins() {
    unsafe { std::env::set_var("FUIGO_WORKFLOWS", "0") };
    let cfg = Config::default();
    let r = cfg.resolve_workflows();
    assert_eq!(r.source, ConfigSource::Env);
    assert!(
...
fn resolve_image_gen_model_override_remote_settings_or_config() {
    unsafe { std::env::remove_var("FUIGO_IMAGE_GEN_MODEL_OVERRIDE") };
    ...
    assert_eq!(
        with(None, Some("fuigo-imagine-image")).resolve_image_gen_model_override(),
        Some("fuigo-imagine-image".to_owned())
    );
    assert_eq!(
        with(Some("fuigo-imagine-image-pro"), Some("fuigo-imagine-image"))
            .resolve_image_gen_model_override(),
        Some("fuigo-imagine-image-pro".to_owned())
    );
}
#[test]
#[serial]
fn resolve_image_edit_model_override_remote_settings_or_config() {
    unsafe { std::env::remove_var("FUIGO_IMAGE_EDIT_MODEL_OVERRIDE") };
    let with = |config: Option<&str>, gb: Option<&str>| Config {
        features: Features {
    ...
    assert_eq!(
        with(None, Some("fuigo-imagine-image")).resolve_image_edit_model_override(),
        Some("fuigo-imagine-image".to_owned())
    );
    assert_eq!(
        with(Some("fuigo-imagine-image-pro"), Some("fuigo-imagine-image"))
    ...
    let gen_only = Config {
        remote_settings: Some(crate::util::config::RemoteSettings {
            image_gen_model_override: Some("fuigo-imagine-image".to_owned()),
            ..Default::default()
        }),
        ..Default::default()
    };
...
fn imagine_tools_disabled_gates_image_edit() {
    unsafe { std::env::remove_var("FUIGO_IMAGE_EDIT") };
    ...
    unsafe { std::env::set_var("FUIGO_IMAGE_EDIT", "1") };
...
fn resolve_image_gen_gates() {
    unsafe { std::env::remove_var("FUIGO_IMAGE_GEN") };
    ...
    unsafe { std::env::set_var("FUIGO_IMAGE_GEN", "1") };
    let denied = Config {
        remote_settings: Some(crate::util::config::RemoteSettings {
...
fn resolve_video_gen_gates() {
    unsafe { std::env::remove_var("FUIGO_VIDEO_GEN") };
...
fn clear_goal_envs() {
    unsafe {
        std::env::remove_var("FUIGO_GOAL");
        std::env::remove_var("FUIGO_GOAL_CLASSIFIER");
        std::env::remove_var("FUIGO_GOAL_PLANNER");
        std::env::remove_var("FUIGO_GOAL_SUMMARY");
        std::env::remove_var("FUIGO_GOAL_VERIFIER_N");
        std::env::remove_var("FUIGO_GOAL_CLASSIFIER_MAX");
        std::env::remove_var("FUIGO_GOAL_STRATEGIST_EVERY");
        std::env::remove_var("FUIGO_GOAL_REVERIFY_AFTER");
    }
}
...
fn resolve_goal_classifier_env_overrides_default_and_remote() {
    clear_goal_envs();
    unsafe { std::env::set_var("FUIGO_GOAL_CLASSIFIER", "0") };
    let r = cfg_with_goal_and_remote(true, remote_classifier(true))
        .resolve_goal_classifier_enabled(true);
    assert!(!r.value);
    assert_eq!(r.source, ConfigSource::Env);
    unsafe { std::env::set_var("FUIGO_GOAL_CLASSIFIER", "1") };
...
fn resolve_goal_summary_env_overrides_default_and_remote() {
    clear_goal_envs();
    unsafe { std::env::set_var("FUIGO_GOAL_SUMMARY", "0") };
...
const GOAL_USE_CURRENT_ENV: &str = "FUIGO_GOAL_USE_CURRENT_MODEL_ONLY";
...
[goal]
enabled = true
planner_model = { model = "fuigo-build", agent_type = "fuigo-build-plan" }
...
"#;
    ...
    assert_eq!(cfg.goal.skeptic_models[0].model, "fuigo-build");
    assert_eq!(
...
planner_model = { agent_type = "fuigo-build-plan" }
"#;
    let raw: toml::Value = toml::from_str(toml_str).unwrap();
...
"#,
    ...
    let fuigo_build = crate::util::config::GoalRoleModel {
        model: "fuigo-build".into(),
        agent_type: "fuigo-build-plan".into(),
    };
    ...
    assert_eq!(
        planner.value,
        GoalRoleModelChoice::Explicit(fuigo_build.clone())
    );
    ...
    assert_eq!(
        ...
        vec![
            GoalRoleModelChoice::Explicit(fuigo_build),
...
fn config_warns_on_section_typo() {
    let raw: toml::Value = toml::from_str(
        r#"
            [endpoint]
            deployment_key = "fuigo-token-test"
        "#,
    )
    .unwrap();
...
fn config_accepts_all_known_sections() {
    let unused = unused_keys_from_toml(
        r#"
            ...
            command = "/opt/bin/fuigo-identity"
            [repo_changes_dedup]
            enabled = false
            [relay]
            enabled = false
...
fn internal_otlp_test_config() -> EndpointsConfig {
    EndpointsConfig {
        ...
        fuigo_internal_otlp_traces_endpoint: None,
        fuigo_internal_otlp_headers: None,
        external_otel_master_switch: false,
        ..Default::default()
    }
}
/// `fuigo_internal_otlp_traces_endpoint` wins over the legacy `OTEL_*` fields regardless of the master switch.
#[test]
fn internal_otlp_endpoint_fuigo_internal_wins_regardless_of_switch() {
    for switch in [false, true] {
        let cfg = EndpointsConfig {
            fuigo_internal_otlp_traces_endpoint: Some(
        ...
        assert_eq!(
            ...
            "switch={switch}: FUIGO_INTERNAL_OTLP_TRACES_ENDPOINT must win verbatim (trailing / trimmed)"
        );
    }
}
...
fn internal_otlp_consumed_standard_vars_cases() {
    ...
    for case in cases {
        let cfg = EndpointsConfig {
            ...
            fuigo_internal_otlp_traces_endpoint: case
                .internal_ep
                .then(|| "https://internal.example/traces".to_string()),
            fuigo_internal_otlp_headers: case.internal_headers.then(|| "ik=iv".to_string()),
            ..internal_otlp_test_config()
        };
        assert_eq!(
...
/// Headers precedence: `fuigo_internal_otlp_headers` wins; legacy `otel_exporter_otlp_headers` only when the master switch is unset.
#[test]
fn internal_otlp_headers_precedence() {
    for switch in [false, true] {
        let cfg = EndpointsConfig {
            fuigo_internal_otlp_headers: Some("x-debug=1".to_string()),
...
fn ext_client() -> fuigo_telemetry::external::config::ExternalClientInfo {
    fuigo_telemetry::external::config::ExternalClientInfo::default()
}
#[test]
...
fn external_otel_requirements_pin_wins_over_env() {
    ...
    assert!(
        resolve_external_otel_config_with(
            None,
            Some(&req),
            ext_env(&[("FUIGO_EXTERNAL_OTEL", "1"), ("OTEL_LOGS_EXPORTER", "otlp"),]),
            ext_client(),
            false,
        )
        .is_none()
...
/// That path is managed config / requirements, with no `FUIGO_EXTERNAL_OTEL` env var set.
...
fn clear_runtime_env_vars() {
    unsafe {
        std::env::remove_var("FUIGO_SUBAGENTS");
        std::env::remove_var("FUIGO_RESPECT_GITIGNORE");
        std::env::remove_var("FUIGO_WEB_SEARCH_MODEL");
        std::env::remove_var("FUIGO_SESSION_SUMMARY_MODEL");
        std::env::remove_var("FUIGO_CURSOR_SKILLS_ENABLED");
        std::env::remove_var("FUIGO_CURSOR_RULES_ENABLED");
        std::env::remove_var("FUIGO_CURSOR_AGENTS_ENABLED");
        std::env::remove_var("FUIGO_CLAUDE_SKILLS_ENABLED");
        std::env::remove_var("FUIGO_CLAUDE_RULES_ENABLED");
        std::env::remove_var("FUIGO_CLAUDE_AGENTS_ENABLED");
    }
}
fn clear_managed_mcp_env_vars() {
    unsafe {
        std::env::remove_var("FUIGO_MANAGED_MCPS_ENABLED");
        std::env::remove_var("FUIGO_MANAGED_MCP_GATEWAY_TOOLS_ENABLED");
    }
}
...
fn resolve_raw_compat_sessions_load_failure_allows_env_override() {
    let _env = isolate_compat_env();
    let _codex = EnvGuard::set("FUIGO_CODEX_SESSIONS_ENABLED", "true");
    let resolved = resolve_compat_sessions_from_raw(Err(()), None);
    assert!(!resolved.cursor.sessions);
    assert!(!resolved.claude.sessions);
    assert!(resolved.codex.sessions);
...
fn resolve_compat_env_sessions_disable_independently() {
    ...
    for (vendor, env_var) in [
        (CompatVendor::Cursor, "FUIGO_CURSOR_SESSIONS_ENABLED"),
        (CompatVendor::Claude, "FUIGO_CLAUDE_SESSIONS_ENABLED"),
        (CompatVendor::Codex, "FUIGO_CODEX_SESSIONS_ENABLED"),
    ] {
...
fn resolve_compat_precedence_and_reserved_codex_hook() {
    ...
    let _session = EnvGuard::set("FUIGO_CURSOR_SESSIONS_ENABLED", "true");
    let _hook = EnvGuard::set("FUIGO_CODEX_HOOKS_ENABLED", "true");
    let resolved = resolve_compat_config(&config, Some(&remote));
    assert!(resolved.cursor.sessions);
    assert!(resolved.codex.hooks);
}
...
fn resolve_runtime_fields_compat_asymmetric_sources() {
    let _env = isolate_compat_env();
    let _cursor = EnvGuard::set("FUIGO_CURSOR_SESSIONS_ENABLED", "false");
    let raw: toml::Value =
...
fn resolve_runtime_fields_gitignore_from_env() {
    clear_runtime_env_vars();
    unsafe { std::env::set_var("FUIGO_RESPECT_GITIGNORE", "0") };
    let raw = empty_config();
    let mut cfg = Config::new_from_toml_cfg(&raw).unwrap();
    cfg.resolve_runtime_fields(&RuntimeResolutionContext {
        raw_config: &raw,
...
fn is_telemetry_explicitly_disabled_sync_env_signals() {
    unsafe { std::env::set_var("FUIGO_TELEMETRY_ENABLED", "0") };
    unsafe { std::env::remove_var("DISABLE_TELEMETRY") };
    assert!(is_telemetry_explicitly_disabled_sync());
    unsafe { std::env::set_var("FUIGO_TELEMETRY_ENABLED", "1") };
    assert!(!is_telemetry_explicitly_disabled_sync());
    unsafe { std::env::remove_var("FUIGO_TELEMETRY_ENABLED") };
...
"#,
    ...
    fuigo_config::apply_version_overrides(&mut value, &v).unwrap();
    let cfg = Config::new_from_toml_cfg(&value).unwrap();
    assert_eq!(cfg.models.default.as_deref(), Some("grok-4.5"));
}
/// Reproduce the enterprise managed config bug: [model.fuigo-build] sets context_window=500k for model="grok-4.5".
/// [models].default="grok-4.5" still resolves to the bare prefetched entry (256k).
/// Layer 3 only overrides key "fuigo-build", not key "grok-4.5".
///
...
fn slug_propagation_enterprise_managed_config_key_mismatch() {
    ...
    let by_key = resolved
        .get("fuigo-build")
        .expect("fuigo-build key must exist");
    ...
    assert_eq!(
        ...
        "BUG: prefetched 'grok-4.5' should inherit 500k from \
         sibling 'fuigo-build' (same model slug), not stay at {default_cw}"
    );
}
...
fn resolve_model_list_config_reasoning_efforts_beats_remote() {
    ...
    .unwrap();
    let cfg = Config::new_from_toml_cfg(&raw_config).expect("config should parse");
    let mut entry = prefetch_model_entry("fuigo-x", 200_000, ApiBackend::default());
    entry.info.reasoning_efforts = vec![ReasoningEffortOption {
        id: "high".to_string(),
        value: ReasoningEffort::High,
        label: "High".to_string(),
    ...
    let mut prefetched = IndexMap::new();
    prefetched.insert("fuigo-x".to_owned(), entry);
    let resolved = resolve_model_list(&cfg, Some(prefetched));
...
fn mcp_liveness_watchers_default_is_true() {
    unsafe { std::env::remove_var("FUIGO_MCP_LIVENESS_WATCHERS") };
...
fn mcp_liveness_watchers_requirement_wins_over_everything() {
    unsafe { std::env::set_var("FUIGO_MCP_LIVENESS_WATCHERS", "true") };
    let r =
...
fn mcp_liveness_watchers_env_wins_over_config_and_below() {
    unsafe { std::env::set_var("FUIGO_MCP_LIVENESS_WATCHERS", "false") };
...
fn mcp_auto_restart_default_is_true() {
    unsafe { std::env::remove_var("FUIGO_MCP_AUTO_RESTART") };
...
fn mcp_auto_restart_requirement_wins_over_everything() {
    unsafe { std::env::set_var("FUIGO_MCP_AUTO_RESTART", "false") };
...
fn mcp_auto_restart_env_wins_over_config_and_below() {
    unsafe { std::env::set_var("FUIGO_MCP_AUTO_RESTART", "true") };
...
fn turn_transient_retry_default_is_true() {
    unsafe { std::env::remove_var("FUIGO_TURN_TRANSIENT_RETRY") };
...
fn turn_transient_retry_env_wins_over_config() {
    unsafe { std::env::set_var("FUIGO_TURN_TRANSIENT_RETRY", "false") };
...
fn mcp_push_server_status_default_is_true() {
    unsafe { std::env::remove_var("FUIGO_MCP_PUSH_SERVER_STATUS") };
...
fn mcp_push_server_status_requirement_wins_over_everything() {
    unsafe { std::env::set_var("FUIGO_MCP_PUSH_SERVER_STATUS", "true") };
    let r =
...
fn mcp_push_server_status_env_wins_over_config_and_below() {
    unsafe { std::env::set_var("FUIGO_MCP_PUSH_SERVER_STATUS", "false") };
...
fn mcp_recursive_config_watch_default_is_true() {
    unsafe { std::env::remove_var("FUIGO_MCP_RECURSIVE_CONFIG_WATCH") };
...
fn mcp_recursive_config_watch_requirement_wins_over_everything() {
    unsafe { std::env::set_var("FUIGO_MCP_RECURSIVE_CONFIG_WATCH", "true") };
...
fn mcp_recursive_config_watch_env_wins_over_config_and_below() {
    unsafe { std::env::set_var("FUIGO_MCP_RECURSIVE_CONFIG_WATCH", "false") };
...
#[test]
fn a_status_line_the_parser_could_not_read_in_full_reaches_fuigo_inspect() {
    use super::super::config_model_override_parse::{ConfigWarningKind, WarningTarget};
    let raw_config: toml::Value = toml::from_str(
        r#"
            [ui]
