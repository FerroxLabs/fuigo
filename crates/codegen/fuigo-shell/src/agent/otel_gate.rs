...
//! It drives the process-global flag in [`fuigo_telemetry::external`]:
//!
...
pub(crate) fn suppress() {
    fuigo_telemetry::external::set_settings_gate_max_wait(SETTINGS_GATE_MAX_WAIT);
...
pub(crate) fn is_session_pending(
    ...
    !has_session
        && (fuigo_com_config.auth_provider_command.is_some()
...
mod tests {
    ...
    fn policy_channel_reports_every_structural_reason() {
        assert_eq!(
            ...
            "remote_fetch off: the deployment declared it never calls Ferrox Labs"
        );
        assert_eq!(
