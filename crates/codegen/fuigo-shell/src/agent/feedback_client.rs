...
const CLIENT_VERSION_HEADER: &str = "x-fuigo-client-version";

// ============================================================================
// Turn delta wire types (local to fuigo-shell until cli-chat-proxy catches up)
// ============================================================================
...
impl FeedbackClient {
    ...
    async fn send_json<T: DeserializeOwned>(
        ...
        let request = fuigo_file_utils::trace_context::inject_trace_context_into_request(request);
        let req = request.build().context(context)?;
        let (response, stamp) = fuigo_auth::execute_with_stamp(&self.client, req)
            .await
            .context(context)?;
