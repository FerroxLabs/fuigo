...
impl ReplayBuffer {
    ...
    fn merge(
        ...
        match (prev, new) {
            ...
            (Some(SessionNotification::Fuigo(prev)), SessionNotification::Fuigo(new)) => {
                let (force, first, second) = merge_fuigo_chunks(*prev, *new);
                (
                    force,
                    SessionNotification::Fuigo(Box::new(first)),
                    second.map(|s| SessionNotification::Fuigo(Box::new(s))),
                )
            }
...
/// Merge two consecutive Ferrox Labs notifications.
fn merge_fuigo_chunks(
...
fn estimate_payload_bytes(n: &SessionNotification) -> u64 {
    match n {
        ...
        SessionNotification::Fuigo(n) => match &n.update {
...
mod tests {
    ...
    // ── Fuigo / ToolCallDeltaChunk tests ──────────────────────────────
    ...
    #[test]
    fn fuigo_same_id_deltas_merge_args_and_preserve_name() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        ...
        let n = match flushed {
            SessionNotification::Fuigo(n) => *n,
            _ => panic!("expected Fuigo"),
        };
        match n.update {
            crate::extensions::notification::SessionUpdate::ToolCallDeltaChunk {
                tool_call_id,
    ...
    #[test]
    fn fuigo_different_tool_call_id_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        ...
        assert!(matches!(flushed, SessionNotification::Fuigo(_)));
        assert!(rest.is_some());
        assert!(buf.pending.is_none());
    }

    #[test]
    fn fuigo_tool_call_delta_is_bufferable() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
    ...
    #[test]
    fn fuigo_after_acp_forces_flush() {
        let mut buf = ReplayBuffer::new(Some(settings(100, 1_000_000)));
        let acp_chunk = msg_chunk("s", 1, "thinking...");
        let fuigo_chunk = delta_chunk("s", Some("call_1"), 0, Some("bash"), Some("args"));

        assert!(buf.consume_chunk(acp_chunk).is_none());
        let (flushed, rest) = buf.consume_chunk(fuigo_chunk).expect("should force-flush");

        // Both emitted immediately; different kinds can't merge
        assert!(matches!(flushed, SessionNotification::Acp(_)));
        assert!(matches!(rest, Some(SessionNotification::Fuigo(_))));
        assert!(buf.pending.is_none());
    }
}
