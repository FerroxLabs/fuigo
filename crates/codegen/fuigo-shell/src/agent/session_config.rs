...
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct FuigoSessionDetail {
...
mod tests {
    ...
    fn options_have_one_selected_model_and_a_mode_per_effort() {
        ...
        assert_eq!(selected_models[0].id, "fuigo-build");
    ...
    fn none_effort_is_not_a_user_selectable_mode() {
        ...
        let models = [model("fuigo-build", "Fuigo")];
        let current = acp::ModelId::from("fuigo-build");
        let opts = build_session_config_options(
            &models,
            &current,
    ...
    fn model_label_falls_back_to_id_when_name_empty() {
        let models = [model("fuigo-build", "")];
        let current = acp::ModelId::from("fuigo-build");
        let opts = build_session_config_options(&models, &current, &[], None);
        assert_eq!(opts[0].label, "fuigo-build");
    }

    #[test]
    fn session_config_option_serializes_camel_case() {
        let opt = SessionConfigOption {
            id: "fuigo-build".to_string(),
            category: "model".to_string(),
            label: "Fuigo".to_string(),
            description: None,
            selected: true,
        };
        let v = serde_json::to_value(&opt).expect("serialize");
        assert_eq!(v["id"], "fuigo-build");
        assert_eq!(v["category"], "model");
        assert_eq!(v["label"], "Fuigo");
        assert_eq!(v["selected"], true);
        assert!(v.get("description").is_none());
    }

    #[test]
    fn fuigo_session_detail_serializes_camel_case() {
        let detail = FuigoSessionDetail::build(
            "sess-1".to_string(),
            "/Users/me/fuigo".to_string(),
        ...
        assert_eq!(v["cwd"], "/Users/me/fuigo");
        assert_eq!(v["currentModelId"], "fuigo-build");
