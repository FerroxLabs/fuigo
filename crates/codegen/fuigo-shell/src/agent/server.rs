...
//!
//! Remote TUI clients connect here to a fuigo agent running on a different machine.
//!
...
fn persistent_agent_thread(
    ...
    let rt = match fuigo_tty_utils::runtime::build_with_blocking_pool(builder.enable_all()) {
        Ok(rt) => {
...
async fn run_persistent_agent(
    ...
    crate::agent::app::apply_otel_config(&auth_manager, &agent_config.fuigo_com_config);
    let agent = Rc::new(
