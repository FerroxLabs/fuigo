...
use crate::auth::{AuthManager, FuigoAuth, FuigoComConfig, run_auth_flow};
    ...
    W: tokio::io::AsyncWrite + Unpin + Send + 'static,
{
    ...
    let workspace_user_dir = fuigo_agent::prompt::workspace_user::optional_workspace_user_dir();
...
fn register_fs_watch_runtime() {
    fuigo_fsnotify::set_runtime_handle(tokio::runtime::Handle::current());
}
#[tracing::instrument(level = "debug", skip_all)]
pub async fn run_stdio_agent(
    ...
    fuigo_file_utils::queue::cleanup_orphaned_uploads(
        ...
        fuigo_file_utils::queue::DEFAULT_MAX_AGE,
    );
    if let Ok(version) = std::env::var("FUIGO_CLIENT_VERSION") {
        crate::unified_log::info(
            "FUIGO_CLIENT_VERSION",
            None,
...
pub async fn run_headless(
    ...
    let (mut auth, did_browser_flow) = if reauthenticate {
        ...
        let auth_manager = Arc::new(AuthManager::new(&fuigo_home::fuigo_home(), ctx.clone()));
        if crate::agent::auth_method::has_fuigo_api_key_env()
        ...
        .run_until(async move {
            let _agent_handle = tokio::task::spawn_local(async move {
                ...
                tokio::task::spawn_local(
                    GatewayReceiver::new(gw_rx, conn)
                        .with_on_meta(
                            fuigo_file_utils::trace_context::span_from_meta_traceparent,
                        )
                        .run(),
                );
...
fn should_seed_shared_session(existing: Option<&FuigoAuth>, session: &FuigoAuth) -> bool {
    match existing {
        None => true,
        Some(existing) => {
...
///
/// * `relay_on_demand == false` (default, an explicit `fuigo agent leader` invocation: devbox / systemd / nohup): connect **eagerly**, right now.
...
impl DeferredRelayArm {
    ...
    fn arm_if_eligible(self, session: &FuigoAuth, auth_manager: &Arc<AuthManager>) -> Option<Self> {
...
pub fn apply_otel_config(auth_manager: &AuthManager, fuigo_com_config: &FuigoComConfig) {
    suppress_otel();
    ...
    if crate::agent::otel_gate::should_open_at_startup(crate::agent::otel_gate::StartupGate {
        ...
        has_session,
        session_pending: crate::agent::otel_gate::is_session_pending(has_session, fuigo_com_config),
    }) {
...
pub async fn run_leader(
    ...
    let ctx = &agent_config.fuigo_com_config;
    suppress_otel();
    let auth: Option<FuigoAuth> = crate::auth::try_noninteractive_auth_no_mint(ctx).await;
    let has_session = auth.is_some()
        || agent_config
            ...
            .is_some();
    let session_pending =
        crate::agent::otel_gate::is_session_pending(has_session, &agent_config.fuigo_com_config);
    let policy_channel =
                ...
                tokio::task::spawn_local(async move {
                    tokio::select! {
                        ...
                        minted = crate::auth::mint_session_noninteractive(&mint_auth_manager)
                            => match minted {
                            Some(session) => info!(
                                is_fuigo = session.is_fuigo_auth(),
...
mod tests {
    ...
    fn oidc_session(key: &str, create_time: chrono::DateTime<chrono::Utc>) -> FuigoAuth {
        FuigoAuth {
            key: key.into(),
    ...
    async fn deferred_arm_connects_relay_when_auth_appears() {
        ...
        local
            .run_until(async {
                let ineligible = FuigoAuth::test_default();
                let arm = arm
