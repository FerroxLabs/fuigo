...
//!
//! Warnings are retained on `Config::config_warnings` and surfaced by `fuigo inspect`.
...
/// Serialize-only: `fuigo inspect --json` emits it, nothing deserializes it back.
...
pub(crate) fn log_config_warnings(warnings: &[ConfigWarning]) {
    ...
    if !warnings.is_empty() {
        tracing::warn!(
            ...
            "config: parsed with warnings; run `fuigo inspect` for details"
        );
    }
}
...
mod tests {
    ...
    fn fully_populated_override() -> ConfigModelOverride {
        ConfigModelOverride {
            ...
            auth_scheme: Some(fuigo_sampler::config::AuthScheme::XApiKey),
