//! `fuigo inspect`: configuration introspection.
//!
//! Shows everything Fuigo discovers in the current directory.
...
use fuigo_tools::util::truncate::estimate_tokens;
...
pub(crate) struct PermissionsReport {
    ...
    /// A Claude managed-settings `disableBypassPermissionsMode` request, which fuigo
    /// deliberately does not enforce
    /// ([`yolo_disabled_by_policy`](fuigo_workspace::permission::resolution::yolo_disabled_by_policy)).
...
/// Enterprise login-hardening policy resolved from `[fuigo_com_config]` (TOML and env).
...
pub(crate) struct LoginPolicyReport {
    /// Raw `disable_api_key_auth` knob (env `FUIGO_DISABLE_API_KEY_AUTH`).
...
/// A single config layer entry for `fuigo inspect`.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
...
async fn build_report(cwd: &Path) -> InspectReport {
    ...
    InspectReport {
        fuigo_version: fuigo_version::VERSION.to_string(),
...
fn instruction_file_type(
    ...
    if path
        .parent()
        .is_some_and(|parent| parent == fuigo_home.join("rules"))
        || has_rules_directory(file_path, ".fuigo")
...
async fn list_instructions(cwd: &Path) -> Vec<InstructionFile> {
    ...
    let vendor_homes = fuigo_dirs::home_dir()
        .map(|home_dir| {
            vec![
    ...
    // `fuigo_agent::prompt::agents_md::read_agents_config_with_paths`
...
fn permission_policy_rows(
    ...
    yolo_lock: Option<&fuigo_workspace::permission::resolution::YoloPolicyLock>,
...
/// Resolves the enterprise login-hardening knobs from the merged config (`[fuigo_com_config]`, the `[auth]` alias, and env overrides).
...
fn login_policy_report(config: Option<&crate::agent::config::Config>) -> LoginPolicyReport {
    let fuigo_com_config = config
        .map(|c| c.fuigo_com_config.clone())
        .unwrap_or_default();
    LoginPolicyReport {
        api_key_auth_disabled: fuigo_com_config.api_key_auth_disabled(),
        disable_api_key_auth: fuigo_com_config.disable_api_key_auth,
        force_login_team_uuid: fuigo_com_config.force_login_team_uuid,
    }
}
...
fn list_hooks(
    ...
    discovered_plugins: &[fuigo_agent::plugins::DiscoveredPlugin],
) -> Vec<HookEntry> {
    let all_on = fuigo_tools::types::compat::CompatConfig::default();
    ...
    let mut entries: Vec<HookEntry> = registry
        ...
        .map(|h| {
            ...
            use fuigo_hooks::config::HookOrigin as O;
            ...
            let source = match fuigo_hooks::config::hook_origin(h) {
                O::SystemManaged | O::Managed => ConfigSource::Managed {
                    path: Some(config_file(fuigo_config::MANAGED_CONFIG_FILENAME)),
                },
                O::Requirements => ConfigSource::Managed {
                    path: Some(config_file(fuigo_config::REQUIREMENTS_FILENAME)),
                },
                O::UserConfig => ConfigSource::ConfigToml {
                    path: config_file(fuigo_config::USER_CONFIG_FILENAME),
                },
...
fn slash_name_counts(skills: &[fuigo_agent::prompt::skills::SkillInfo]) -> HashMap<String, usize> {
    ...
    for skill in skills.iter().filter(|s| s.user_invocable && s.enabled) {
        ...
        *counts
            .entry(
                fuigo_tools::implementations::skills::skill::format_skill_name(skill)
                    .to_lowercase(),
            )
            .or_default() += 1;
    }
...
fn slash_collision(
    skill: &fuigo_agent::prompt::skills::SkillInfo,
    ...
    let qualified =
        fuigo_tools::implementations::skills::skill::format_skill_name(skill).to_lowercase();
...
fn skill_entry_source(s: &fuigo_agent::prompt::skills::SkillInfo) -> ConfigSource {
...
fn list_agents(
    ...
    let agents = fuigo_agent::discovery::all_subagents_with_plugins(
        cwd,
        &HashMap::new(),
        Some(plugin_registry),
    );
    ...
    discovered
        ...
        .map(|p| {
            let scope = match p.scope {
                fuigo_agent::plugins::PluginScope::CliOverride => Scope::Cli,
                fuigo_agent::plugins::PluginScope::Project => Scope::Project,
                fuigo_agent::plugins::PluginScope::User => Scope::User,
                fuigo_agent::plugins::PluginScope::ConfigPath => Scope::Config,
            };
            PluginEntry {
                ...
                provides: PluginProvides {
                    ...
                    skills: fuigo_agent::plugins::registry::skill_md_paths(&p.skill_dirs).len(),
...
fn list_marketplaces(git_root: Option<&Path>) -> Vec<MarketplaceEntry> {
    ...
    fuigo_agent::plugins::marketplace::resolve(root)
        .into_iter()
...
fn list_lsp_servers(
    ...
    let servers = fuigo_tools::implementations::lsp::config::load_servers_with_plugins_sourced(
        cwd,
        &plugin_lsp_paths,
        &inline_values,
        &plugin_names,
    ...
    // `remote = None` mirrors `fuigo mcp doctor` (no loaded RemoteSettings in a standalone command)
    crate::agent::folder_trust::resolve_and_record(cwd, None, false);
...
/// Project `.fuigo/config.toml` files come via `find_project_configs`.
...
fn list_config_sources(cwd: &Path) -> ConfigSources {
    ...
    // User managed
    if let Some(home) = crate::config::user_fuigo_home() {
    ...
    let inline_env = crate::config::FUIGO_CONFIG_ENV;
    let path_env = crate::config::FUIGO_CONFIG_PATH_ENV;
...
fn claude_bypass_advisory_message(p: &PermissionsReport) -> Option<&'static str> {
    p.claude_bypass_lock_advisory.then_some(
        "Claude disableBypassPermissionsMode: advisory only -- not enforced for fuigo \
...
fn print_human(r: &InspectReport, out: &mut impl Write) -> std::io::Result<()> {
    ...
    if r.mcp_servers.is_empty() {
        ...
        writeln!(out, "  {TREE} (none) \u{2014} see `fuigo mcp add --help`")?;
    } else {
        print_columns(
            out,
            "MCP Servers",
...
mod tests {
    use super::*;
    use fuigo_agent::prompt::skills::{SkillInfo, SkillsConfig};
    use fuigo_tools::implementations::skills::types::SkillScope;
    ...
    fn vendor_rule_paths_select_rules_compatibility_cells() {
        ...
        for (vendor, path) in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.fuigo"), false, &[]);
        ...
        for path in [
            ...
            assert_eq!(
                instruction_file_type(path, Path::new("/home/user/.fuigo"), true, &[]),
                "rules"
            );
        }
        for path in [
            ...
            let file_type = instruction_file_type(path, Path::new("/home/user/.fuigo"), true, &[]);
    ...
    #[test]
    fn fuigo_home_nested_in_workspace_keeps_direct_surfaces_global() {
        let fuigo_home = Path::new("/repo/config");
        let workspace = Path::new("/repo");
        for path in ["/repo/config/AGENTS.md", "/repo/config/rules/global.md"] {
            assert!(matches!(
                instruction_scope(path, fuigo_home, &[], workspace),
                Scope::Global
            ));
        }
        for path in [
            "/repo/config/.fuigo/rules/project.md",
    ...
    fn vendor_home_nested_in_workspace_keeps_direct_surfaces_global() {
        ...
        for path in ["/repo/.claude/rules/global.md", "/repo/.claude/CLAUDE.md"] {
            assert!(matches!(
                instruction_scope(path, Path::new("/other/fuigo"), &vendor_homes, workspace),
                Scope::Global
            ));
        }
        for path in [
    ...
    #[test]
    fn workspace_scope_wins_inside_fuigo_home() {
        let fuigo_home = Path::new("/custom/fuigo");
        let workspace = Path::new("/custom/fuigo/worktrees/repo");
        for path in [
            "/custom/fuigo/worktrees/repo/.cursor/rules/project.md",
            "/custom/fuigo/worktrees/repo/src/AGENTS.md",
        ] {
            assert!(matches!(
                instruction_scope(path, fuigo_home, &[], workspace),
                Scope::Project
        ...
        assert!(matches!(
            instruction_scope("/custom/fuigo/rules/global.md", fuigo_home, &[], workspace,),
            Scope::Global
        ));
    }

    #[test]
    fn custom_fuigo_home_rules_are_classified_as_rules() {
        assert_eq!(
    ...
    fn permissions_report_pins_advisory_and_enforced_wire_contract() {
        ...
        let row = EnforcedPolicy {
            setting: EnforcedSetting::AlwaysApprove,
            enabled: false,
            source: "/etc/fuigo/requirements.toml".to_string(),
        };
        ...
        assert_eq!(
            ...
            serde_json::json!([{
                ...
                "enabled": false,
                "source": "/etc/fuigo/requirements.toml",
            }])
        );
    }
    ...
    ) -> fuigo_workspace::permission::resolution::ManagedSettingsFeatures {
        fuigo_workspace::permission::resolution::ManagedSettingsFeatures {
            disable_telemetry,
            disable_feedback,
            disable_yolo,
    ...
    /// The enforced alwaysApprove row comes from fuigo's own requirements lock,
    ...
    fn requirements_lock_reports_always_approve_enforced() {
        let lock = fuigo_workspace::permission::resolution::YoloPolicyLock {
            source_label: "/etc/fuigo/requirements.toml".to_string(),
            reason: fuigo_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS,
        };
        ...
        assert_eq!(enforced[0].source, "/etc/fuigo/requirements.toml");
    ...
    fn skill_entry_source_maps_scopes() {
        let s = skill_fixture("a", "/repo/.fuigo/skills/a/SKILL.md", SkillScope::Local);
        assert!(matches!(
            skill_entry_source(&s),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("b", "/repo/.fuigo/skills/b/SKILL.md", SkillScope::Repo);
        assert!(matches!(
            skill_entry_source(&s),
            ConfigSource::Project { .. }
        ));

        let s = skill_fixture("c", "/home/u/.fuigo/skills/c/SKILL.md", SkillScope::User);
        assert!(matches!(skill_entry_source(&s), ConfigSource::User { .. }));

        let s = skill_fixture(
            "d",
            "/home/u/.fuigo/server-skills/d/SKILL.md",
            SkillScope::Server,
        );
        assert!(matches!(
            skill_entry_source(&s),
    ...
    async fn list_skills_honors_skills_config() {
        ...
        // Test-unique names: discovery also reads this machine's real ~/.fuigo dirs.
