...
pub use fuigo_sampling_types::types::*;

// `CreateResponseWrapper` and `MessagesRequestWrapper` live in `fuigo-sampling-types::types`, re-exported above via the wildcard
// That placement lets the `fuigo-sampler` crate reference them without a circular dep on `fuigo-shell`

// Tests for the types live in the fuigo-sampling-types crate

use fuigo_tools::types::output::ImageContent as ToolsImageContent;
...
///
/// Lives in the shell (rather than `fuigo-sampling-types` or `fuigo-tools`) so neither crate needs a dep on `agent-client-protocol`.
