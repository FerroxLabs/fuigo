...
pub mod fuigo_auth_credentials;
pub mod hooks;
pub mod limits;
...
// The foundation utilities live in `fuigo-shell-base` (upstream of this crate so they build in parallel)
// Re-exported at the original paths so existing `crate::util::…` and `fuigo_shell::util::…` users compile unchanged
pub use fuigo_shell_base::util::*;
...
pub(crate) fn is_user_instruction_path(
    ...
    let fuigo_rules = fuigo_home.join("rules");
    let is_exact_home_surface = parent
        .is_some_and(|parent| parent == fuigo_home || parent == fuigo_rules)
...
mod expand_home_tests {
    ...
    #[test]
    fn bare_tilde() {
        let home = fuigo_dirs::home_dir().expect("home_dir required for this test");
...
mod is_user_instruction_path_tests {
    ...
    #[test]
    fn fuigo_home_named_file_nested_in_workspace_is_user_scoped() {
    ...
    #[test]
    fn workspace_descendants_under_fuigo_home_stay_project_scoped() {
        assert!(!is_user_instruction_path(
            Path::new("/custom/fuigo/worktrees/repo/src/AGENTS.md"),
