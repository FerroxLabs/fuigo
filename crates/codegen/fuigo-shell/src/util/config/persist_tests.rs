...
/// First-run `ensure` creates a 0-byte `$FUIGO_HOME/config.toml`.
...
fn merge_section_preserves_unmodeled_fields_inside_a_nested_table() {
    ...
    let cfg = crate::agent::config::UiConfig {
        status_line: fuigo_status_line::test_support::StatusLineConfigFixture::from_kind(
            fuigo_status_line::StatusLineType::Command,
        )
...
fn merge_section_updates_modeled_fields_preserving_unmodeled() {
    ...
    ui.insert(
        ...
        TomlValue::String("fuigoday".into()),
    );
    table.insert("ui".into(), TomlValue::Table(ui));
    let cfg = crate::agent::config::UiConfig {
        yolo: true,
...
"#;
    ...
    assert_eq!(cfg.ui.auto_light_theme.as_deref(), Some("fuigoday"));
...
fn merge_section_models_only_updates_set_fields_preserves_others() {
    ...
    let cfg = crate::agent::config::ModelsConfig {
        default: Some("fuigo-new".to_string()),
        ..Default::default()
    };
    merge_section(&mut table, "models", &cfg);
    let m = table.get("models").unwrap().as_table().unwrap();
    assert_eq!(m.get("default").and_then(|v| v.as_str()), Some("fuigo-new"));
    assert_eq!(
...
fn persist_preferred_model_flow_roundtrips_via_load_and_new_from_toml_cfg() {
    ...
    let root: TomlValue = toml::from_str(original).unwrap();
    let mut cfg = load_config_from_toml(&root);
    cfg.models.default = Some("fuigo-persisted".to_string());
    ...
    assert_eq!(reloaded.models.default.as_deref(), Some("fuigo-persisted"));
    let cfg2 =
        crate::agent::config::Config::new_from_toml_cfg(&reloaded_root).expect("new_from_toml_cfg");
    assert_eq!(cfg2.models.default.as_deref(), Some("fuigo-persisted"));
}
#[test]
...
mod resolve_auto_compact {
    ...
    /// Serialize tests that mutate `FUIGO_AUTO_COMPACT_THRESHOLD_PERCENT`.
...
fn settings_helpers_target_correct_ui_fields() {
    ...
    let cfg = apply(|cfg| cfg.ui.auto_light_theme = Some("fuigoday".to_string()));
    assert_eq!(cfg.ui.auto_light_theme, Some("fuigoday".to_string()));
