...
pub use fuigo_telemetry::enums::PermissionMode;
...
pub(crate) const ENV_DEFAULT_PERMISSION_MODE: &str = "FUIGO_DEFAULT_PERMISSION_MODE";
...
use fuigo_workspace::permission::resolution::yolo_disabled_by_policy;
...
mod tests {
    ...
    fn campaign_cannot_arm_permission_mode_gate() {
        ...
        layers.campaigns.managed = vec![fuigo_config::CampaignEntry {
            id: "c1".into(),
    ...
    fn effective_auto_for_launch_cli_auto_not_yolo() {
        ...
        unsafe { std::env::set_var("FUIGO_AUTO_PERMISSION_MODE", "1") };
        ...
        unsafe { std::env::remove_var("FUIGO_AUTO_PERMISSION_MODE") };
    }
    ...
    /// With the gate forced OFF (`FUIGO_AUTO_PERMISSION_MODE=0`), `--permission-mode auto` or config auto is inert, so the classifier never launches.
    ...
    fn effective_auto_for_launch_inert_when_gate_off() {
        let _g = crate::util::config::resolve::AUTO_PERMISSION_MODE_ENV_LOCK
            .lock()
            .unwrap_or_else(|p| p.into_inner());
        unsafe { std::env::set_var("FUIGO_AUTO_PERMISSION_MODE", "0") };
        assert!(
    ...
    fn interactive_default_slot() {
        ...
        for (raw, expected) in [
            ...
            assert_eq!(
                default_interactive_permission_mode(),
                expected,
                "FUIGO_DEFAULT_PERMISSION_MODE={raw}"
            );
        }
    ...
    // Pure tests for the policy predicate itself live next to its canonical definition in `fuigo_workspace::permission::claude_compat`

    #[test]
    fn resolve_launch_yolo_policy_pin_neutralizes_requested_bypass() {
        let warning = fuigo_workspace::permission::resolution::YOLO_PIN_REASON_REQUIREMENTS;
