...
pub(crate) fn reenableable_disabled_stubs(
    ...
    let settings = fuigo_workspace::permission::resolution::managed_settings();
...
mod tests {
    ...
    /// Git repo with project `.fuigo/config.toml` so discovery is bounded to cwd.
    fn project_repo(toml: &str) -> tempfile::TempDir {
        let tmp = tempfile::tempdir().unwrap();
        git2::Repository::init(tmp.path()).unwrap();
        // Pin trust so ambient FUIGO_CLI_VERSION cannot drop project MCP names.
        crate::agent::folder_trust::record_for_test(tmp.path(), true);
        std::fs::create_dir_all(tmp.path().join(".fuigo")).unwrap();
        std::fs::write(tmp.path().join(".fuigo").join("config.toml"), toml).unwrap();
        tmp
    }

    /// Isolate HOME/FUIGO_HOME so ambient user MCP config cannot pad discovery.
    fn isolated_home() -> (
        tempfile::TempDir,
        ...
        std::fs::write(fuigo_home.join("config.toml"), "").unwrap();
        let home_guard = fuigo_test_support::EnvGuard::set("HOME", home.path());
        let fuigo_guard = fuigo_test_support::EnvGuard::set("FUIGO_HOME", &fuigo_home);
        (home, home_guard, fuigo_guard)
    }
    ...
    fn orphan_fuigo_com_name_is_not_a_definition() {
        ...
        assert!(!index.contains("fuigo_com_slack"));
        let disabled = HashSet::from(["fuigo_com_slack".into(), "totally_orphan_xyz".into()]);
        let stubs = index.reenableable_for_list(&disabled, &HashSet::new(), &unrestricted());
        assert!(!stubs.contains("fuigo_com_slack"));
