...
/// Per tip, the env master `FUIGO_CONTEXTUAL_HINTS` (all-on/off) beats user config `[ui.contextual_hints].X`.
...
pub fn resolve_contextual_hints(
    ...
    let resolve_tip = |user: Option<bool>, feature_flag: Option<bool>| -> bool {
        BoolFlag::env("FUIGO_CONTEXTUAL_HINTS")
            .config(user)
...
mod tests {
    ...
    const ENV_CONTEXTUAL_HINTS: &str = "FUIGO_CONTEXTUAL_HINTS";

    // `FUIGO_CONTEXTUAL_HINTS` is process-global
