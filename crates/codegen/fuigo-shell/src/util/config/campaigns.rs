...
use fuigo_config::config_override::{PatchPath, patch_touches_any};
use fuigo_config::{
    CampaignsState, ConfigLayers, campaigns_state_path, load_dismissed_ids_from_home,
    user_fuigo_home,
};
use fuigo_config_types::{CampaignOverride, RemoteSettings};
...
pub(crate) fn dismiss_campaign_ids(ids: impl IntoIterator<Item = String>) {
    let Some(home) = user_fuigo_home() else {
        return;
    };
...
/// `FUIGO_CAMPAIGNS_OVERRIDE` JSON array replaces all sources (`[]` means none; it beats the kill switch).
...
pub(crate) fn campaigns_override() -> Option<Vec<CampaignEntry>> {
    let json = std::env::var("FUIGO_CAMPAIGNS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<CampaignOverride>>(&json) {
        ...
        Err(e) => {
            tracing::warn!(error = %e, "invalid FUIGO_CAMPAIGNS_OVERRIDE JSON; suppressing all campaigns");
            Some(Vec::new())
        }
    }
}
...
/// The single campaign-resolution path: `FUIGO_CAMPAIGNS_OVERRIDE` first (it replaces all sources and beats the kill switch).
...
/// Env-free resolution core of [`campaign_driven_models_default`] (unit-testable without touching `FUIGO_HOME` or the process-global cache).
...
pub async fn persist_models_default(
    value: Option<String>,
    reasoning_effort: Option<fuigo_sampling_types::ReasoningEffort>,
) -> anyhow::Result<()> {
...
mod tests {
    ...
    /// `FUIGO_CAMPAIGNS_OVERRIDE` applies despite the kill switch; without it the kill switch (`features.campaigns = false`) wins.
    #[test]
    #[serial]
    fn override_beats_kill_switch() {
        ...
        let _env = EnvGuard::unset("FUIGO_CAMPAIGNS_OVERRIDE");
        let active = resolve_active_campaigns_from_layers(&layers, &base, &[], &HashSet::new());
        assert!(
            active.is_empty(),
    ...
    /// Invalid `FUIGO_CAMPAIGNS_OVERRIDE` JSON fails toward *no campaigns*.
    ...
    fn invalid_override_json_suppresses_all_campaigns() {
        let _env = EnvGuard::set("FUIGO_CAMPAIGNS_OVERRIDE", "{ not json");
    ...
    /// A model pick made while `FUIGO_CAMPAIGNS=0` must still record the dismissal.
    /// Otherwise a later re-enabled campaign would override the user's explicit choice.
    #[test]
    #[serial]
    fn dismiss_resolution_ignores_kill_switch() {
        let _over = EnvGuard::unset("FUIGO_CAMPAIGNS_OVERRIDE");
        let _kill = EnvGuard::set("FUIGO_CAMPAIGNS", "0");
    ...
    fn campaign_driven_models_default_tracks_remote_and_dismissals() {
        let _over = EnvGuard::unset("FUIGO_CAMPAIGNS_OVERRIDE");
        let _kill = EnvGuard::unset("FUIGO_CAMPAIGNS");
    ...
    /// `FUIGO_CAMPAIGNS_OVERRIDE="[]"` replaces all sources with nothing: even layer and remote campaigns resolve to empty.
    #[test]
    #[serial]
    fn override_empty_means_none() {
        let _env = EnvGuard::set("FUIGO_CAMPAIGNS_OVERRIDE", "[]");
