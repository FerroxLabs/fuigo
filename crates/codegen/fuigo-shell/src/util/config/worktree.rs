...
use fuigo_fast_worktree::CreationMode;
use serde::{Deserialize, Serialize};
use toml::Value as TomlValue;

/// Mirrors the internal `CreationMode` enum from fuigo-fast-worktree but uses config-friendly naming (lowercase strings in TOML).
...
pub const ENV_WORKTREE_TYPE: &str = "FUIGO_WORKTREE_TYPE";
...
/// (`fuigo-fast-worktree`'s `resolve_worktree_auto_gc_from_layers` owns and tests precedence and clamping; this only maps settings to layers.)
pub(crate) fn resolve_worktree_auto_gc_from_settings(
...
) -> fuigo_fast_worktree::ResolvedWorktreeAutoGc {
    use fuigo_workspace::worktree::worktree_auto_gc_layer_from_settings;
    ...
    fuigo_fast_worktree::resolve_worktree_auto_gc_from_layers(
        local_layer.as_ref(),
        remote_layer.as_ref(),
    )
}
