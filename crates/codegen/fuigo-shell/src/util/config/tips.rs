...
/// `FUIGO_TIPS_OVERRIDE` env var overrides everything (debug builds only).
...
pub fn resolve_tips(
    ...
    if let Ok(raw) = std::env::var("FUIGO_TIPS_OVERRIDE") {
...
/// Parse a `FUIGO_SLASH_COMMAND_TAGS` payload (a JSON object of string values) into a name-to-tag map.
fn parse_slash_command_tags_json(raw: Option<&str>) -> std::collections::HashMap<String, String> {
    ...
    match serde_json::from_str::<std::collections::BTreeMap<String, String>>(raw) {
        ...
        Err(e) => {
            tracing::warn!(
                error = %e,
                "ignoring malformed FUIGO_SLASH_COMMAND_TAGS; expected a JSON object of string values"
            );
...
fn slash_command_tags_from_env() -> std::collections::HashMap<String, String> {
    parse_slash_command_tags_json(std::env::var("FUIGO_SLASH_COMMAND_TAGS").ok().as_deref())
}
...
/// Remote settings are the base, local `[slash_command_tags]` overrides them, and the `FUIGO_SLASH_COMMAND_TAGS` env var wins.
...
mod tests {
    ...
    // Hermetic: drive the resolver through `_with_env` with an EXPLICIT env map so ambient `FUIGO_SLASH_COMMAND_TAGS` can't affect these assertions
    #[test]
