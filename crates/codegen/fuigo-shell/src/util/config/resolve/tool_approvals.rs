...
pub(crate) const ENV_REMEMBER_TOOL_APPROVALS: &str = "FUIGO_REMEMBER_TOOL_APPROVALS";
...
/// Precedence: requirements > env (`FUIGO_REMEMBER_TOOL_APPROVALS`) > `[ui].remember_tool_approvals` > managed > remote settings.
...
mod remember_tool_approvals_gate_tests {
    ...
    // `FUIGO_REMEMBER_TOOL_APPROVALS` is process-global
