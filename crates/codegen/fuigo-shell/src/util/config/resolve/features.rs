...
pub fn resolve_zdr_access_enabled(
    ...
    BoolFlag::env("FUIGO_ZDR_ACCESS_ENABLED")
...
/// Spawn-time kill switch: `[features] turn_transient_retry`, `FUIGO_TURN_TRANSIENT_RETRY`, remote key (below local), default on.
...
/// Whether model-catalog (`/v1/models`) and remote-settings (`/v1/settings`) fetches from Ferrox Labs backends are allowed.
...
/// Overlay-free: the `FUIGO_CONFIG` / `FUIGO_CONFIG_PATH` overlay is deliberately excluded, matching `ConfigLayers::env_overlay`'s contract.
...
fn remote_fetch_enabled_from_layers(layers: &crate::config::ConfigLayers) -> bool {
    ...
    // `env_overlay` is deliberately NOT in the walk: the `FUIGO_CONFIG` overlay is soft, user-tier input, and this is an egress gate
