pub const ENV_SYSTEM_PROMPT_LABEL: &str = "FUIGO_SYSTEM_PROMPT_LABEL";

pub const DEFAULT_SYSTEM_PROMPT_LABEL: &str = fuigo_agent::DEFAULT_SYSTEM_PROMPT_LABEL;

/// Precedence: env > config per-model > `[agent]` > GB per-model > GB global > `"Fuigo"`.
...
mod system_prompt_label_tests {
    ...
    /// Serialize access to `FUIGO_SYSTEM_PROMPT_LABEL` and clear it for tier tests.
