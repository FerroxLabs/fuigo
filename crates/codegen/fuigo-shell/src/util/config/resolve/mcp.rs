...
/// | env          | `FUIGO_MCP_LIVENESS_WATCHERS` (handled by `BoolFlag::env`)       |
/// | config       | `[features] mcp_liveness_watchers` in `~/.fuigo/config.toml`     |
...
/// | cli          | (none — no CLI flag)                                            |
/// | env          | `FUIGO_MCP_AUTO_RESTART` (handled by `BoolFlag::env`)            |
/// | config       | `[features] mcp_auto_restart` in `~/.fuigo/config.toml`          |
...
/// | cli          | (none — no CLI flag)                                            |
/// | env          | `FUIGO_MCP_PUSH_SERVER_STATUS` (handled by `BoolFlag::env`)      |
/// | config       | `[features] mcp_push_server_status` in `~/.fuigo/config.toml`    |
...
/// | cli          | (none — no CLI flag)                                                |
/// | env          | `FUIGO_MCP_RECURSIVE_CONFIG_WATCH` (handled by `BoolFlag::env`)      |
/// | config       | `[features] mcp_recursive_config_watch` in `~/.fuigo/config.toml`    |
...
/// Kept in sync with `fuigo_mcp::servers`'s standalone fallback.
...
const ENV_MCP_STARTUP_TIMEOUT_SECS: &str = "FUIGO_MCP_STARTUP_TIMEOUT_SECS";
...
/// `MCP_TIMEOUT` (ms, rounded up so a sub-second value never becomes 0s) > `FUIGO_MCP_STARTUP_TIMEOUT_SECS` (secs).
...
pub const DEFAULT_MAX_MCP_OUTPUT_BYTES: usize = fuigo_tools::MCP_MAX_OUTPUT_BYTES;
...
pub(crate) fn cache_remote_max_mcp_output_bytes(remote: Option<u64>) {
    fuigo_tools::set_mcp_max_output_bytes(resolve_max_mcp_output_bytes(remote));
}
...
///   2. env `FUIGO_MAX_MCP_OUTPUT_BYTES` / `MAX_MCP_OUTPUT_BYTES` (Fuigo-native wins when both set)
...
pub(crate) fn resolve_max_mcp_output_bytes(remote: Option<u64>) -> usize {
    ...
    resolve_max_mcp_output_bytes_precedence(
        requirements,
        fuigo_tools::mcp_max_output_bytes_from_env(),
...
/// Project tier of the MCP output cap: `[mcp] max_output_bytes` from the `.fuigo/config.toml` chain (`cwd` up to the git root), deepest file wins.
///
...
fn project_max_mcp_output_bytes(cwd: &std::path::Path) -> Option<usize> {
    ...
    for config_path in crate::config::find_project_configs(cwd) {
        if let Ok(toml_val) = fuigo_config::load_config_file(&config_path)
...
pub(crate) fn resolve_max_mcp_output_bytes_for_cwd(cwd: &std::path::Path) -> Option<usize> {
    let requirements = crate::config::load_merged_requirements()
        .as_ref()
        .and_then(max_mcp_output_bytes_from_toml);
    if requirements.is_some() || fuigo_tools::mcp_max_output_bytes_from_env().is_some() {
        return None;
    }
...
mod max_mcp_output_bytes_tests {
    ...
    fn project_chain_deepest_file_wins() {
        ...
        std::fs::create_dir_all(sub.join(".fuigo")).unwrap();
        std::fs::create_dir_all(root.join(".fuigo")).unwrap();
        std::fs::write(
            root.join(".fuigo/config.toml"),
        ...
        std::fs::write(sub.join(".fuigo/config.toml"), "[ui]\nvim_mode = true\n").unwrap();
        assert_eq!(super::project_max_mcp_output_bytes(&sub), Some(30_000));

        // No .fuigo file sets the key anywhere, so the walk returns None
        std::fs::remove_file(root.join(".fuigo/config.toml")).unwrap();
