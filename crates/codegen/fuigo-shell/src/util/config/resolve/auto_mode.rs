...
pub(crate) const ENV_AUTO_PERMISSION_MODE: &str = "FUIGO_AUTO_PERMISSION_MODE";
...
/// Crate-wide serialization lock for tests that mutate `FUIGO_AUTO_PERMISSION_MODE`.
...
/// requirements > env (`FUIGO_AUTO_PERMISSION_MODE`) > `[auto_mode] enabled` > managed > remote `auto_mode.enabled` > default (`true`).
...
pub(crate) fn auto_mode_classifier_defaults(
    ...
    fuigo_workspace::permission::ClassifierPromptType,
    Option<fuigo_sampling_types::ReasoningEffort>,
) {
    let prompt_type = cfg
        .prompt_type
        .unwrap_or(fuigo_workspace::permission::ClassifierPromptType::Full);
    let reasoning_effort = cfg.reasoning_effort.or_else(|| {
        effective_supports_reasoning_effort.then_some(fuigo_sampling_types::ReasoningEffort::Low)
    });
...
mod auto_permission_mode_gate_tests {
    ...
    // `FUIGO_AUTO_PERMISSION_MODE` is process-global; serialize every test that reads it (all of them, via `BoolFlag::env`)
