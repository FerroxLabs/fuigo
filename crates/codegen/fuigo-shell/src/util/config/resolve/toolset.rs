...
/// `~/.fuigo/requirements.toml` — is honored. Returns `(find_bfs, grep_ugrep)`,
/// which the caller bakes into a [`fuigo_tools::computer::local::SearchShadowConfig`] on the local terminal backend.
pub(crate) fn resolve_search_tools_enabled(
    requirements: Option<&TomlValue>,
    user: Option<&TomlValue>,
    managed: Option<&TomlValue>,
) -> (bool, bool) {
    let disable = fuigo_config::env_bool("DISABLE_EMBEDDED_SEARCH_TOOLS");
    ...
    let resolve = |primary: &str, alias: &str, key: &str| -> bool {
        let env = fuigo_config::env_bool(primary).or_else(|| fuigo_config::env_bool(alias));
    ...
    (
        resolve("FUIGO_TOOLS_FIND_BFS", "FUIGO_FIND_BFS", "find_bfs"),
        resolve("FUIGO_TOOLS_GREP_UGREP", "FUIGO_GREP_UGREP", "grep_ugrep"),
    )
}
...
const ENV_LOGIN_SHELL_CAPTURE: &str = "FUIGO_LOGIN_ENV";
...
/// Precedence (highest first): requirements/MDM (clamp, via [`crate::config::load_merged_requirements`]) > `FUIGO_LOGIN_ENV` env
/// > `FUIGO_CONFIG` overlay > user `config.toml` > managed layers > remote > default `true`.
/// `login_shell_capture` is a soft key, so the overlay is merged just above user config, mirroring its place in the disk merge.
/// That lets a `FUIGO_CONFIG` toggle reach it while requirements/MDM still clamp the value.
...
mod login_shell_capture_tests {
    ...
    fn env_beats_overlay() {
        let _g = guard();
        // `FUIGO_LOGIN_ENV` outranks the overlay
        unsafe { std::env::set_var(ENV_LOGIN_SHELL_CAPTURE, "1") };
...
const ENV_SCHEDULER_BACKGROUND_LOOPS: &str = "FUIGO_SCHEDULER_BACKGROUND_LOOPS";
...
///
/// Precedence: requirements > env (`FUIGO_SCHEDULER_BACKGROUND_LOOPS`) > user `config.toml` `[scheduler] background_loops`
...
const ENV_ASK_USER_QUESTION_TIMEOUT_ENABLED: &str = "FUIGO_ASK_USER_QUESTION_TIMEOUT_ENABLED";
...
/// Precedence: requirements > env (`FUIGO_ASK_USER_QUESTION_TIMEOUT_ENABLED`) > user `config.toml` > managed (user-level `managed_config.toml`
...
fn resolve_ask_user_question_timeout_secs_from_tiers(
    ...
    requirement
        ...
        .or(remote)
        .unwrap_or(
            fuigo_tools::implementations::fuigo_build::ask_user_question::RESPONSE_TIMEOUT
                .as_secs(),
        )
}

/// Precedence: requirements > env (`FUIGO_ASK_USER_QUESTION_TIMEOUT_SECS`, parsed by the tools crate's canonical parser) > user `config.toml`
...
fn resolve_ask_user_question_timeout_secs(
    ...
    resolve_ask_user_question_timeout_secs_from_tiers(
        ...
        fuigo_tools::implementations::fuigo_build::ask_user_question::response_timeout_env_secs(),
...
) -> fuigo_tools::implementations::fuigo_build::ask_user_question::AskUserQuestionParams {
    let requirements = crate::config::load_merged_requirements();
    let layers = match crate::config::ConfigLayers::load() {
        Ok(l) => Some(l),
        Err(e) => {
...
fn cap_web_search_domains(list: Option<Vec<String>>, field: &str) -> Option<Vec<String>> {
    const MAX: usize = fuigo_sampling_types::MAX_WEB_SEARCH_DOMAINS;
...
-> Option<fuigo_sampling_types::WebSearchOptions> {
...
fn web_search_options_from_section(
    ...
    let opts = fuigo_sampling_types::WebSearchOptions {
    ...
    match opts.validate() {
        ...
        Err(e) => {
            ...
            Some(fuigo_sampling_types::WebSearchOptions {
...
mod web_search_domains_tests {
    ...
    // Cross-layer precedence and allow/exclude atomicity live in ConfigLayers (see `fuigo_config::loader` normalization tests)
...
mod ask_user_question_timeout_tests {
    ...
    use fuigo_tools::implementations::fuigo_build::ask_user_question::RESPONSE_TIMEOUT_ENV;
    ...
    fn timeout_secs_tier_precedence() {
        let d = fuigo_tools::implementations::fuigo_build::ask_user_question::RESPONSE_TIMEOUT
            .as_secs();
...
mod tests {
    ...
    // Assumes FUIGO_TOOLS_* / DISABLE_EMBEDDED_SEARCH_TOOLS are unset in the test env.
    #[test]
