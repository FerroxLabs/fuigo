...
/// Walks from cwd up to the git repo root checking `.fuigo/config.toml` at each level.
/// Project-scoped `.fuigo/config.toml` entries override global `~/.fuigo/config.toml`
...
/// Scope an MCP server resolves at: project when any project-scoped `.fuigo/config.toml` defines it, otherwise user.
...
/// Load MCP servers with project-scoped overrides from `.fuigo/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.fuigo/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.fuigo/config.toml` at each level (matching skills and AGENTS.md discovery)
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .fuigo/config.toml"
                );
...
pub(crate) fn mcp_preferences_path() -> PathBuf {
    fuigo_config::fuigo_home().join("mcp_preferences.json")
}
    ...
    if let Some(registry) = plugin_registry {
        ...
        for plugin in registry.active_plugins() {
            ...
            if let Some(ref inline_value) = plugin.inline_mcp_servers {
                let normalized =
                    fuigo_agent::plugins::manifest::normalize_inline_mcp_servers(inline_value);
...
/// Upsert an MCP server entry in `~/.fuigo/config.toml`.
...
///
/// Same behavior as [`save_mcp_server_config`] but targets an explicit config file, e.g. a project-scoped `.fuigo/config.toml`.
...
///
/// Same behavior as [`delete_mcp_server_config`] but targets an explicit config file, e.g. a project-scoped `.fuigo/config.toml`.
...
/// The transport-less case is steered to `disabled_mcp_servers`, Fuigo's real disable mechanism.
    ...
    let message = if !has_command && !has_url {
        format!(
            "`mcp_servers.{name}` has no transport. To run it, set `command = \"...\"` or \
             ...
             See ~/.fuigo/docs/user-guide/07-mcp-servers.md"
        )
    } else {
        format!(
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` has an unrecognized field `{field}`; it is \
                             ignored. See ~/.fuigo/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                }
                if config.enabled
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` is enabled but its `{field}` is blank. \
                             ...
                             off. See ~/.fuigo/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                    continue;
                }
...
// `.mcp.json` discovery moved to `fuigo-workspace` (client-side, shared with the folder-trust gate)
// The re-export keeps `crate::util::config::*` paths working
pub use fuigo_workspace::project_config::{
...
///
/// Overlays project-scoped `.fuigo/config.toml` files from `cwd` up to the repo root onto the user-tier config, nearest definition winning.
...
/// MCP config problems across the same layers as [`load_mcp_server_configs_with_project`], for `fuigo inspect`.
...
/// Names `fuigo mcp enable`/`disable` may target.
...
/// `fuigo_com_*` is known only when a TOML / disabled / compat / plugin definition exists, not by prefix.
...
pub fn load_cli_plugin_registry(cwd: &std::path::Path) -> fuigo_agent::plugins::PluginRegistry {
    ...
    fuigo_agent::plugins::PluginRegistry::from_discovered(
        discovered,
...
/// Path to the user-level config file (`~/.fuigo/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.fuigo/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".fuigo").join("config.toml")
}
...
/// Overrides `[cli] session_registry`; usable before `~/.fuigo/config.toml` exists.
pub const SESSION_REGISTRY_ENV_VAR: &str = "FUIGO_SESSION_REGISTRY";

pub(crate) fn session_registry_from_env_opt() -> Option<bool> {
    fuigo_config::env_bool(SESSION_REGISTRY_ENV_VAR)
}
...
mod tests {
    ...
    fn session_registry_local_override_precedence() {
        ...
        {
            let _g = fuigo_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "false");
            assert_eq!(
        ...
        {
            let _g = fuigo_test_support::EnvGuard::set(SESSION_REGISTRY_ENV_VAR, "bogus");
            assert_eq!(
                session_registry_local_override_sourced(Some(&toml_true)),
        ...
        {
            let _g = fuigo_test_support::EnvGuard::unset(SESSION_REGISTRY_ENV_VAR);
    ...
    fn load_cli_plugin_registry_includes_project_config_path_plugins() {
        ...
        let agents = fuigo_agent::discovery::plugin_agents(&registry);
        assert!(
            agents
                .iter()
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.fuigo/skills", "~/.fuigo/skills/special/SKILL.md"]
ignore = ["~/.fuigo/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.fuigo/skills", "~/.fuigo/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.fuigo/skills/noisy/SKILL.md"]);
    }
    ...
    fn mcp_json_env_var_default_value() {
        ...
        std::fs::write(
            ...
            r#"{
                "mcpServers": {
                    "api": {
                        "url": "${FUIGO_TEST_MCP_UNSET_VAR_12345:-https://fallback.example.com}/mcp"
                    }
                }
            }"#,
        )
...
"#,
    ...
    fn apply_mcp_server_enabled_managed_name_only_updates_array() {
        ...
        apply_mcp_server_enabled(table, "fuigo_com_slack", false);
        let disabled = table
            .get("disabled_mcp_servers")
            .and_then(|v| v.as_array())
            .expect("disabled_mcp_servers array");
        assert_eq!(disabled.len(), 1);
        assert_eq!(disabled[0].as_str(), Some("fuigo_com_slack"));
        assert!(table.get("mcp_servers").is_none());

        apply_mcp_server_enabled(table, "fuigo_com_slack", true);
        assert!(table.get("disabled_mcp_servers").is_none());
        assert!(table.get("mcp_servers").is_none());
    }
...
"#,
    ...
    async fn enable_unstick_only_touches_nearest_project_definition() {
        ...
        std::fs::create_dir_all(nested.join(".fuigo")).unwrap();
...
enabled = false
"#;
        let ancestor = tmp.path().join(".fuigo").join("config.toml");
        let nearer = nested.join(".fuigo").join("config.toml");
    ...
    async fn restore_mcp_server_enabled_after_enable_scopes_tiers() {
        ...
        // Do not call save_mcp_server_enabled_in (that reads ambient config_path / fuigo_home)
