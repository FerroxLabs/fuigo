...
pub use fuigo_announcements::RemoteAnnouncement;
...
/// Dev/test override for announcements via `FUIGO_ANNOUNCEMENTS_OVERRIDE` (a JSON array of announcements).
...
pub(crate) fn announcements_override() -> Option<Vec<RemoteAnnouncement>> {
    let raw = std::env::var("FUIGO_ANNOUNCEMENTS_OVERRIDE").ok()?;
    match serde_json::from_str::<Vec<RemoteAnnouncement>>(&raw) {
        Ok(list) => Some(list),
        Err(_) => {
            tracing::warn!("invalid FUIGO_ANNOUNCEMENTS_OVERRIDE JSON; ignoring");
            None
        }
    }
}
...
/// The `FUIGO_ANNOUNCEMENTS_OVERRIDE` env var overrides everything (dev only).
