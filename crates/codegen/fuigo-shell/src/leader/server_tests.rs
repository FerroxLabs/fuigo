...
async fn workspace_start_errors_when_cancelled_before_auth() {
    ...
    let state = default_test_control_state(Path::new("/tmp/fuigo-ws-auth-test.sock"));
    let cancel = CancellationToken::new();
    cancel.cancel();
...
/// The first Headless registration (the devbox / `fuigo agent headless` flow) flips the watch so `run_leader` starts the deferred relay connection.
#[tokio::test]
...
async fn initialize_gets_client_identifier_injected() {
    ...
    // Register with client_type "fuigo-tui"
    write_message(
        &mut writer,
        &ClientMessage::Register {
    ...
    assert_eq!(
        json["params"]["_meta"]["clientIdentifier"], "fuigo-tui",
...
async fn initialize_preserves_existing_client_identifier() {
    ...
    let payload = r#"{"jsonrpc":"2.0","method":"initialize","id":1,"params":{"protocolVersion":"0.1","_meta":{"clientIdentifier":"fuigo-web"}}}"#;
    write_message(
        &mut writer,
        &ClientMessage::Acp {
            payload: payload.into(),
    ...
    let json: serde_json::Value = serde_json::from_str(&received).unwrap();
    assert_eq!(
        json["params"]["_meta"]["clientIdentifier"], "fuigo-web",
...
fn is_scheduled_task_inject_prompt_detects_only_inject() {
    assert!(is_scheduled_task_inject_prompt(&pv(
        r#"{"method":"fuigo/scheduled_task_inject_prompt","params":{"sessionId":"s1","taskId":"t1","prompt":"echo hi"}}"#
    )));
    ...
    assert!(is_scheduled_task_inject_prompt(&pv(
        r#"{"method":"_fuigo/scheduled_task_inject_prompt","params":{"method":"fuigo/scheduled_task_inject_prompt","params":{"sessionId":"s1","taskId":"t1","prompt":"echo hi"}}}"#
    )));
    // The sibling informational notification is NOT driver-routed (it fans out so every dashboard updates its tasks pane)
    assert!(!is_scheduled_task_inject_prompt(&pv(
        r#"{"method":"fuigo/scheduled_task_fired","params":{"sessionId":"s1"}}"#
    )));
    assert!(!is_scheduled_task_inject_prompt(&pv(
...
fn is_interaction_request_detects_only_interaction_methods() {
    ...
    assert!(!is_interaction_request(&pv(
        r#"{"jsonrpc":"2.0","method":"fuigo/sessions/changed","params":{}}"#
    )));
}
...
fn extract_interaction_tool_call_id_handles_direct_and_nested() {
    ...
    assert_eq!(
        extract_interaction_tool_call_id(&pv(
            r#"{"id":1,"method":"fuigo/ask_user_question","params":{"sessionId":"s","toolCallId":"tc-q"}}"#
        ))
        .as_deref(),
        Some("tc-q")
    );
    ...
    assert_eq!(
        extract_interaction_tool_call_id(&pv(
            r#"{"id":1,"method":"_fuigo/ask_user_question","params":{"method":"fuigo/ask_user_question","params":{"sessionId":"s","toolCallId":"tc-w"}}}"#
        ))
        .as_deref(),
        Some("tc-w")
    );
...
fn extract_interaction_resolved_tool_call_id_matches_only_resolved() {
    assert_eq!(
        extract_interaction_resolved_tool_call_id(&pv(
            r#"{"method":"fuigo/session_notification","params":{"sessionId":"s","update":{"sessionUpdate":"interaction_resolved","tool_call_id":"tc-r"}}}"#
        ))
        .as_deref(),
        Some("tc-r")
    );
    ...
    assert_eq!(
        extract_interaction_resolved_tool_call_id(&pv(
            r#"{"method":"_fuigo/session_notification","params":{"method":"fuigo/session_notification","params":{"sessionId":"s","update":{"sessionUpdate":"interaction_resolved","tool_call_id":"tc-rw"}}}}"#
        ))
        .as_deref(),
        Some("tc-rw")
    );
    ...
    assert_eq!(
        extract_interaction_resolved_tool_call_id(&pv(
            r#"{"method":"fuigo/session_notification","params":{"sessionId":"s","update":{"sessionUpdate":"pending_interaction","tool_call_id":"tc-r","kind":"permission"}}}"#
        )),
        None
    );
}
...
fn event_seq_of_parses_acp_and_ext_and_handles_missing() {
    ...
    // ExtNotification (Ferrox Labs): nested under params.params._meta.eventId.
    let ext = pv(
        r#"{"params":{"method":"fuigo/session/update","params":{"sessionId":"019e-aa","_meta":{"eventId":"019e-aa-7"}}}}"#,
    );
    assert_eq!(event_seq_of(&ext), Some(7));
    // No eventId (Ferrox Labs one-shot / older shell) yields None, so dedup cannot drop the event
...
fn inject_capabilities_adds_status_line_when_it_is_the_only_capability() {
    ...
    assert_eq!(
        json["params"]["_meta"][fuigo_status_line::CLIENT_STATUS_LINE_META],
        true,
...
fn inject_capabilities_omits_user_message_echo_when_false() {
    ...
    assert!(
        json["params"]["_meta"]
            ...
            .is_none(),
        "fuigo agent (echo false) must not inject clientUserMessageEcho=false over initialize"
    );
}
...
fn extract_yolo_mode_change_returns_value() {
    let payload =
        r#"{"jsonrpc":"2.0","method":"fuigo/yolo_mode_changed","params":{"yolo_mode":true}}"#;
    assert_eq!(extract_yolo_mode_change(&pv(payload)), Some(true));

    let payload =
        r#"{"jsonrpc":"2.0","method":"fuigo/yolo_mode_changed","params":{"yolo_mode":false}}"#;
...
fn extract_auto_mode_change_explicit_flag_wins() {
    let payload =
        r#"{"jsonrpc":"2.0","method":"fuigo/yolo_mode_changed","params":{"auto_mode":true}}"#;
    assert_eq!(extract_auto_mode_change(&pv(payload)), Some(true));

    let payload =
        r#"{"jsonrpc":"2.0","method":"fuigo/yolo_mode_changed","params":{"auto_mode":false}}"#;
    assert_eq!(extract_auto_mode_change(&pv(payload)), Some(false));

    // Explicit flag wins even when permission_mode would say otherwise.
    let payload = r#"{"jsonrpc":"2.0","method":"fuigo/yolo_mode_changed","params":{"auto_mode":false,"permission_mode":"auto"}}"#;
    assert_eq!(extract_auto_mode_change(&pv(payload)), Some(false));
}
...
fn extract_auto_mode_change_derives_from_permission_mode() {
    let payload = r#"{"jsonrpc":"2.0","method":"fuigo/yolo_mode_changed","params":{"permission_mode":"auto"}}"#;
    assert_eq!(extract_auto_mode_change(&pv(payload)), Some(true));

    for mode in ["ask", "always-approve", "default"] {
        let payload = format!(
            r#"{{"jsonrpc":"2.0","method":"fuigo/yolo_mode_changed","params":{{"permission_mode":"{mode}"}}}}"#
        );
        assert_eq!(
...
fn extract_session_id_from_nested_params_works() {
    ...
    let payload = r#"{"jsonrpc":"2.0","method":"_fuigo/session_notification","params":{"method":"fuigo/session_notification","params":{"sessionId":"sess-nested"}}}"#;
    assert_eq!(
        extract_session_id(&pv(payload)),
    ...
    let payload = r#"{"jsonrpc":"2.0","method":"_fuigo/fs_notify","params":{"method":"fuigo/fs_notify","params":{"session_id":"sess-nested-2","event":{}}}}"#;
    assert_eq!(
        extract_session_id(&pv(payload)),
...
fn extract_session_id_from_prompt_complete_works() {
    let payload = r#"{"jsonrpc":"2.0","method":"fuigo/session/prompt_complete","params":{"sessionId":"sess-prompt"}}"#;
    assert_eq!(
...
fn extract_session_id_from_prompt_complete_ignores_other_methods() {
    let payload = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-prompt"}}"#;
    assert_eq!(extract_session_id_from_prompt_complete(&pv(payload)), None);
}

#[test]
fn extract_child_session_event_spawned() {
    let payload = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-1"}}}"#;
...
fn extract_child_session_event_finished() {
    let payload = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-2"}}}"#;
    match extract_child_session_event(&pv(payload)) {
...
fn extract_child_session_event_nested_ext_notification() {
    let payload = r#"{"jsonrpc":"2.0","method":"_fuigo/session_notification","params":{"method":"fuigo/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-3"}}}}"#;
    match extract_child_session_event(&pv(payload)) {
...
fn extract_child_session_event_nested_ext_notification_finished() {
    let payload = r#"{"jsonrpc":"2.0","method":"_fuigo/session_notification","params":{"method":"fuigo/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-4"}}}}"#;
    match extract_child_session_event(&pv(payload)) {
...
fn extract_child_session_event_none_for_other_updates() {
    let payload = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"message_delta","content":"hello"}}}"#;
    assert!(extract_child_session_event(&pv(payload)).is_none());
}

#[test]
fn extract_child_session_event_none_without_child_id() {
    let payload = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"parent","update":{"sessionUpdate":"subagent_spawned"}}}"#;
    assert!(extract_child_session_event(&pv(payload)).is_none());
}
...
fn inject_capabilities_does_not_override_existing_client_identifier() {
    ...
    let mut json = pv(&payload);
    inject_session_request_context(&mut json, &caps, "fuigo-tui", ClientId(1));
...
fn inject_capabilities_adds_leader_client_id_to_session_load() {
    ...
    let mut json = pv(&payload);
    inject_session_request_context(&mut json, &caps, "fuigo-tui", ClientId(42));
    // The unique ClientId is stamped so the agent can echo it onto replay notifications for leader unicast routing
    assert_eq!(
        json["params"]["_meta"]["fuigo/leaderClientId"].as_u64(),
        Some(42)
    );
}
...
fn inject_capabilities_does_not_override_existing_leader_client_id() {
    let payload = format!(
        r#"{{"jsonrpc":"2.0","method":"{}","id":1,"params":{{"sessionId":"sess-1","_meta":{{"fuigo/leaderClientId":7}}}}}}"#,
        AGENT_METHOD_NAMES.session_load
    );
    let caps = ClientCapabilities::default();
...
fn extract_target_client_id_some_when_meta_present() {
    ...
    let direct = r#"{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"sess-1","_meta":{"fuigo/leaderClientId":9}}}"#;
    ...
    let nested = r#"{"jsonrpc":"2.0","method":"_fuigo/session/update","params":{"params":{"sessionId":"sess-1","_meta":{"fuigo/leaderClientId":11}}}}"#;
...
fn inject_yolo_notification_skips_non_yolo_methods() {
    let mut json = pv(r#"{"jsonrpc":"2.0","method":"fuigo/other","params":{"data":1}}"#);
    let before = json.clone();
...
fn inject_client_identity_adds_identifier_to_initialize() {
    ...
    let (mutated, was_initialize) = inject_client_identity_into_initialize(&mut json, "fuigo-tui");
    assert!(was_initialize, "should have detected an initialize message");
    assert!(mutated, "should have injected the identifier");
    assert_eq!(json["params"]["_meta"]["clientIdentifier"], "fuigo-tui");
}

#[test]
fn inject_client_identity_does_not_override_existing() {
    let mut json = pv(
        r#"{"jsonrpc":"2.0","method":"initialize","id":1,"params":{"protocolVersion":"0.1","_meta":{"clientIdentifier":"fuigo-web"}}}"#,
    );

    let (mutated, was_initialize) = inject_client_identity_into_initialize(&mut json, "fuigo-tui");
    assert!(was_initialize, "should have detected an initialize message");
    assert!(!mutated, "existing identifier means nothing was injected");
    assert_eq!(json["params"]["_meta"]["clientIdentifier"], "fuigo-web");
}
...
fn inject_client_identity_preserves_existing_meta() {
    ...
    let (mutated, was_initialize) =
        inject_client_identity_into_initialize(&mut json, "fuigo-code-extension");
    assert!(was_initialize, "should have detected an initialize message");
    assert!(mutated);
    assert_eq!(json["params"]["_meta"]["foo"], "bar");
    assert_eq!(
...
async fn set_model_updates_default_model_for_next_session_new() {
    ...
    // Register with an initial default_model of "fuigo-original"
    write_message(
        &mut writer,
        &ClientMessage::Register {
            client_type: "test".into(),
            ...
            capabilities: ClientCapabilities {
                yolo_mode: false,
                default_model: Some("fuigo-original".to_string()),
                ..Default::default()
            },
        },
    )
...
async fn ext_notification_with_nested_session_id_routes_correctly() {
    ...
    response_tx
        .send(r#"{"jsonrpc":"2.0","method":"_fuigo/session_notification","params":{"method":"fuigo/session_notification","params":{"sessionId":"sess-A","update":{"sessionUpdate":"retry_state","attempt":1,"maxRetries":3,"reason":"transient"}}}}"#.into())
        .unwrap();
    ...
    match msg {
        ServerMessage::Acp { payload } => {
            let json: serde_json::Value = serde_json::from_str(&payload).unwrap();
            assert_eq!(json["method"], "_fuigo/session_notification");
        }
        other => panic!("Expected Acp message, got {:?}", other),
    }
...
async fn high_throughput_replay_no_drops() {
    ...
    let (mut reader, mut writer) = connect_and_register(&sock_path, "fuigo-tui").await;
...
/// When a client disconnects after interacting with a session, the server sends an `fuigo/internal/evict_sessions` notification through acp_tx.
...
/// A `fuigo/scheduled_task_inject_prompt` (cron `/loop` fire) must be routed to the SINGLE session driver, not fanned out to every subscriber.
...
async fn scheduled_task_inject_prompt_routes_to_driver_only() {
    ...
    // That form (`_fuigo/...` top-level, nested method and params) is the shape that previously fell through to broadcast
    let inject = r#"{"method":"_fuigo/scheduled_task_inject_prompt","params":{"method":"fuigo/scheduled_task_inject_prompt","params":{"sessionId":"sess-cron","taskId":"task-1","prompt":"echo hello","humanSchedule":"every 1m"}}}"#;
...
async fn interaction_request_broadcasts_to_all_subscribers() {
    ...
    // The agent raises an `ask_user_question` reverse-request in the gateway-WRAPPED wire form (`_fuigo/...` top-level, nested method and params)
    // This is the shape that previously fell through to driver-only.
    let req = r#"{"jsonrpc":"2.0","id":501,"method":"_fuigo/ask_user_question","params":{"method":"fuigo/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-q","questions":[]}}}"#;
    response_tx.send(req.to_string()).unwrap();
...
async fn pending_interaction_replayed_to_late_joiner() {
    ...
    let req = r#"{"jsonrpc":"2.0","id":601,"method":"_fuigo/ask_user_question","params":{"method":"fuigo/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-late","questions":[]}}}"#;
    response_tx.send(req.to_string()).unwrap();
...
async fn reattached_client_backfilled_into_child_routes() {
    ...
    let spawned_live = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-sub","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-sub"}}}"#;
    ...
    let child_reverse = r#"{"jsonrpc":"2.0","id":777,"method":"fuigo/child_thing","params":{"sessionId":"child-sub"}}"#;
...
async fn replayed_spawn_registers_child_route_for_loading_client() {
    ...
    let spawned_replay = format!(
        r#"{{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{{"sessionId":"sess-fresh","_meta":{{"isReplay":true,"fuigo/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-fresh"}}}}}}"#,
        a_id.0
    );
...
async fn late_attacher_backfilled_into_existing_child_routes() {
    ...
    let spawned_live = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-sub2","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-sub2"}}}"#;
    response_tx.send(spawned_live.to_string()).unwrap();
...
async fn replayed_finished_does_not_tear_down_live_child_route() {
    ...
    let _ = next_acp_payload(&mut reader_a).await;
    tokio::time::sleep(Duration::from_millis(30)).await;
    let spawned_live = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-tear","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-tear"}}}"#;
    response_tx.send(spawned_live.to_string()).unwrap();
    let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
    ...
    let finished_replay = format!(
        r#"{{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{{"sessionId":"sess-tear","_meta":{{"isReplay":true,"fuigo/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-tear"}}}}}}"#,
        b_id.0
    );
...
async fn backfill_covers_nested_children() {
    ...
    let spawned_child = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-nest","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-nest"}}}"#;
    response_tx.send(spawned_child.to_string()).unwrap();
    let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
    let spawned_grandchild = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"child-nest","update":{"sessionUpdate":"subagent_spawned","child_session_id":"grandchild-nest"}}}"#;
...
async fn intermediate_finish_reparents_live_grandchild_for_root_backfill() {
    ...
    let spawned_a = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-rep","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-a"}}}"#;
    response_tx.send(spawned_a.to_string()).unwrap();
    let _ = next_acp_payload_matching(&mut reader_a, "child-a").await;
    let spawned_b = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"child-a","update":{"sessionUpdate":"subagent_spawned","child_session_id":"grandchild-b"}}}"#;
    ...
    let finished_a = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-rep","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-a"}}}"#;
...
async fn live_finished_prunes_index_so_reattach_skips_dead_child() {
    ...
    let spawned_live = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-dead","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-dead"}}}"#;
    response_tx.send(spawned_live.to_string()).unwrap();
    let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
    let finished_live = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-dead","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-dead"}}}"#;
...
async fn detached_live_finished_prunes_index_so_reattach_skips_dead_child() {
    ...
    let spawned_live = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-detach","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-detach"}}}"#;
    response_tx.send(spawned_live.to_string()).unwrap();
    let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
    ...
    let finished_live = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-detach","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-detach"}}}"#;
    response_tx.send(finished_live.to_string()).unwrap();
    tokio::time::sleep(Duration::from_millis(30)).await;
...
async fn mid_burst_disconnect_still_prunes_dead_child_route() {
    ...
    let spawned_replay = format!(
        r#"{{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{{"sessionId":"sess-leak","_meta":{{"isReplay":true,"fuigo/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-leak"}}}}}}"#,
        a_id.0
    );
    response_tx.send(spawned_replay).unwrap();
    let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
    ...
    tokio::time::sleep(Duration::from_millis(50)).await;
    let finished_replay = format!(
        r#"{{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{{"sessionId":"sess-leak","_meta":{{"isReplay":true,"fuigo/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-leak"}}}}}}"#,
        a_id.0
    );
    response_tx.send(finished_replay).unwrap();
    tokio::time::sleep(Duration::from_millis(30)).await;
...
async fn orphaned_replayed_finished_leaves_held_route_untouched() {
    ...
    let _ = next_acp_payload(&mut reader_a).await;
    tokio::time::sleep(Duration::from_millis(30)).await;
    let spawned_live = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-hold","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-hold"}}}"#;
    response_tx.send(spawned_live.to_string()).unwrap();
    let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
    ...
    tokio::time::sleep(Duration::from_millis(50)).await;
    let finished_replay = format!(
        r#"{{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{{"sessionId":"sess-hold","_meta":{{"isReplay":true,"fuigo/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-hold"}}}}}}"#,
        b_id.0
    );
    response_tx.send(finished_replay).unwrap();
    tokio::time::sleep(Duration::from_millis(30)).await;
...
async fn replayed_spawn_unions_into_existing_live_route() {
    ...
    let _ = next_acp_payload(&mut reader_a).await;
    tokio::time::sleep(Duration::from_millis(30)).await;
    let spawned_live = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-union","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-union"}}}"#;
    response_tx.send(spawned_live.to_string()).unwrap();
    let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
    ...
    let spawned_replay = format!(
        r#"{{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{{"sessionId":"sess-union","_meta":{{"isReplay":true,"fuigo/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-union"}}}}}}"#,
        b_id.0
    );
    response_tx.send(spawned_replay).unwrap();
...
async fn replayed_finished_last_subscriber_prunes_dead_child() {
    ...
    let spawned_replay = format!(
        r#"{{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{{"sessionId":"sess-last","_meta":{{"isReplay":true,"fuigo/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_spawned","child_session_id":"child-last"}}}}}}"#,
        a_id.0
    );
    response_tx.send(spawned_replay).unwrap();
    let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
    let finished_replay = format!(
        r#"{{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{{"sessionId":"sess-last","_meta":{{"isReplay":true,"fuigo/leaderClientId":{}}},"update":{{"sessionUpdate":"subagent_finished","child_session_id":"child-last"}}}}}}"#,
        a_id.0
    );
    response_tx.send(finished_replay).unwrap();
    let _ = next_acp_payload_matching(&mut reader_a, "subagent_finished").await;
...
async fn mid_load_child_delta_reaches_loader_via_request_side_backfill() {
    ...
    let _ = next_acp_payload(&mut reader_a).await;
    tokio::time::sleep(Duration::from_millis(30)).await;
    let spawned_live = r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-midload","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-midload"}}}"#;
    response_tx.send(spawned_live.to_string()).unwrap();
    let _ = next_acp_payload_matching(&mut reader_a, "subagent_spawned").await;
    drop(reader_a);
    drop(writer_a);
...
async fn pending_interaction_survives_disconnect_and_replays_on_reconnect() {
    ...
    let req = r#"{"jsonrpc":"2.0","id":801,"method":"_fuigo/ask_user_question","params":{"method":"fuigo/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-reconnect","questions":[]}}}"#;
    response_tx.send(req.to_string()).unwrap();
    let _ = next_acp_payload_matching(&mut reader_a, "ask_user_question").await;
    tokio::time::sleep(Duration::from_millis(30)).await;
...
async fn interaction_raised_with_no_subscriber_is_cached_and_replayed_on_first_attach() {
    ...
    let req = r#"{"jsonrpc":"2.0","id":901,"method":"_fuigo/ask_user_question","params":{"method":"fuigo/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-nosub","questions":[]}}}"#;
    response_tx.send(req.to_string()).unwrap();
...
async fn resolved_interaction_not_replayed_to_late_joiner() {
    ...
    let req = r#"{"jsonrpc":"2.0","id":701,"method":"_fuigo/ask_user_question","params":{"method":"fuigo/ask_user_question","params":{"sessionId":"sess-int","toolCallId":"tc-ev","questions":[]}}}"#;
    response_tx.send(req.to_string()).unwrap();
    let _ = next_acp_payload_matching(&mut reader_a, "ask_user_question").await;

    let resolved = r#"{"method":"_fuigo/session_notification","params":{"method":"fuigo/session_notification","params":{"sessionId":"sess-int","update":{"sessionUpdate":"interaction_resolved","tool_call_id":"tc-ev"}}}}"#;
...
/// `fuigo/sessions/changed` is a machine-wide roster notification with no sessionId.
...
async fn roster_changed_broadcasts_to_all_clients() {
    ...
    let changed = r#"{"jsonrpc":"2.0","method":"fuigo/sessions/changed","params":{"upserted":[{"sessionId":"sess-roster","cwd":"/repo","isWorktree":false,"yolo":false,"activity":"working","resident":true,"lastChangeUnixMs":1,"origin":{"kind":"local"}}],"removed":[]}}"#;
...
/// `fuigo/models/update` is a machine-wide catalog notification with no sessionId.
...
/// Uses the production wire form: agent ext notifications arrive `_`-prefixed (`_fuigo/models/update`).
#[tokio::test]
async fn models_update_broadcasts_to_all_clients() {
    ...
    let update = r#"{"jsonrpc":"2.0","method":"_fuigo/models/update","params":{"currentModelId":"fuigo-new","availableModels":[{"modelId":"fuigo-new","name":"Fuigo New"}]}}"#;
    response_tx.send(update.to_string()).unwrap();

    let got_a = next_acp_payload(&mut reader_a).await;
    let got_b = next_acp_payload(&mut reader_b).await;
    assert!(
        got_a.as_deref().is_some_and(|p| p.contains("fuigo-new")),
        "client A must receive the models broadcast, got {got_a:?}"
    );
    assert!(
        got_b.as_deref().is_some_and(|p| p.contains("fuigo-new")),
...
/// `fuigo/mcp/servers_updated` is a machine-wide MCP-catalog notification with no sessionId (session-agnostic by design).
...
async fn mcp_servers_updated_broadcasts_to_all_clients() {
    ...
    let update = r#"{"jsonrpc":"2.0","method":"_fuigo/mcp/servers_updated","params":{"method":"fuigo/mcp/servers_updated","params":{"mcpServers":[{"name":"fuigo_com_slack","source":"managed"}]}}}"#;
    response_tx.send(update.to_string()).unwrap();
    ...
    assert!(
        got_a
            .as_deref()
            .is_some_and(|p| p.contains("fuigo_com_slack")),
...
fn machine_wide_broadcast_classifier_matches_both_wire_forms() {
    ...
    assert!(is_machine_wide_broadcast_notification(&pv(
        r#"{"jsonrpc":"2.0","method":"fuigo/models/update","params":{}}"#
    )));
    assert!(is_machine_wide_broadcast_notification(&pv(
        r#"{"jsonrpc":"2.0","method":"fuigo/mcp/servers_updated","params":{}}"#
    )));
    assert!(is_machine_wide_broadcast_notification(&pv(
        r#"{"jsonrpc":"2.0","method":"fuigo/announcements/update","params":{}}"#
    )));
    // `_`-prefixed production ext-notification forms.
    assert!(is_machine_wide_broadcast_notification(&pv(
        r#"{"jsonrpc":"2.0","method":"_fuigo/sessions/changed","params":{}}"#
    )));
    assert!(is_machine_wide_broadcast_notification(&pv(
        r#"{"jsonrpc":"2.0","method":"_fuigo/models/update","params":{}}"#
    )));
    assert!(is_machine_wide_broadcast_notification(&pv(
        r#"{"jsonrpc":"2.0","method":"_fuigo/mcp/servers_updated","params":{"method":"fuigo/mcp/servers_updated","params":{"mcpServers":[]}}}"#
    )));
    assert!(is_machine_wide_broadcast_notification(&pv(
        r#"{"jsonrpc":"2.0","method":"_fuigo/announcements/update","params":{"method":"fuigo/announcements/update","params":{"gen":2,"announcements":[]}}}"#
    )));
    // Non-broadcast methods. `fuigo/settings/update` must stay unicast: it carries auth/gate state resolved for the requesting client.
    assert!(!is_machine_wide_broadcast_notification(&pv(
        r#"{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"s"}}"#
    )));
    assert!(!is_machine_wide_broadcast_notification(&pv(
        r#"{"jsonrpc":"2.0","method":"fuigo/settings/update","params":{}}"#
    )));
}
...
fn inject_capabilities_sets_code_nav_enabled_true() {
    ...
    let mut json = pv(payload);
    inject_session_request_context(&mut json, &caps, "fuigo-web", ClientId(1));
    assert_eq!(
...
fn inject_capabilities_sets_code_nav_enabled_false() {
    ...
    let payload = r#"{"jsonrpc":"2.0","method":"session/new","id":1,"params":{"cwd":"/repo","_meta":{"clientIdentifier":"fuigo-tui"}}}"#;
    let mut json = pv(payload);
    inject_session_request_context(&mut json, &caps, "fuigo-tui", ClientId(1));
    assert_eq!(
        json["params"]["_meta"]["codeNavEnabled"],
...
fn inject_capabilities_two_clients_stay_isolated() {
    ...
    inject_session_request_context(&mut web_json, &web_caps, "fuigo-web", ClientId(1));
    let mut tui_json = pv(session_new);
    inject_session_request_context(&mut tui_json, &tui_caps, "fuigo-tui", ClientId(2));
...
async fn subagent_child_session_routed_after_spawned() {
    ...
    response_tx
        .send(r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-123"}}}"#.into())
        .unwrap();
    let _: ServerMessage =
        tokio::time::timeout(Duration::from_millis(200), read_message(&mut reader))
            .await
    ...
    response_tx
        .send(r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"child-123","update":{"sessionUpdate":"message_delta","content":"hello"}}}"#.into())
        .unwrap();
...
async fn subagent_child_session_cleaned_up_on_finished() {
    ...
    response_tx
        .send(r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-456"}}}"#.into())
        .unwrap();
    let _: ServerMessage =
        tokio::time::timeout(Duration::from_millis(200), read_message(&mut reader))
            .await
    ...
    response_tx
        .send(r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_finished","child_session_id":"child-456"}}}"#.into())
        .unwrap();
    let _: ServerMessage =
        tokio::time::timeout(Duration::from_millis(200), read_message(&mut reader))
            .await
    ...
    response_tx
        .send(r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"child-456","update":{"sessionUpdate":"message_delta"}}}"#.into())
        .unwrap();
    let timeout_result: Result<Result<ServerMessage, _>, _> =
...
async fn subagent_child_session_not_leaked_to_other_client() {
    ...
    response_tx
        .send(r#"{"jsonrpc":"2.0","method":"_fuigo/session_notification","params":{"method":"fuigo/session_notification","params":{"sessionId":"sess-parent","update":{"sessionUpdate":"subagent_spawned","child_session_id":"child-789"}}}}"#.into())
        .unwrap();
    let _: ServerMessage =
        tokio::time::timeout(Duration::from_millis(200), read_message(&mut reader_a))
            .await
    ...
    response_tx
        .send(r#"{"jsonrpc":"2.0","method":"fuigo/session_notification","params":{"sessionId":"child-789","update":{"sessionUpdate":"message_delta"}}}"#.into())
        .unwrap();
...
async fn leader_client_id_unicasts_to_target_only() {
    // The agent stamps `_meta["fuigo/leaderClientId"]` onto every session/load replay line
    ...
    response_tx
        .send(format!(
            r#"{{"jsonrpc":"2.0","method":"session/update","params":{{"sessionId":"sess-1","update":{{"sessionUpdate":"agent_message_chunk"}},"_meta":{{"fuigo/leaderClientId":{}}}}}}}"#,
            id_a
        ))
        .unwrap();
...
async fn leader_client_id_dropped_when_target_disconnected() {
    ...
    // The direct ACP shape carries `params._meta`; the nested ext/notification shape (the Ferrox Labs envelope) carries `params.params._meta`
    response_tx
        .send(format!(
            r#"{{"jsonrpc":"2.0","method":"session/update","params":{{"sessionId":"sess-1","update":{{"sessionUpdate":"agent_message_chunk"}},"_meta":{{"isReplay":true,"fuigo/leaderClientId":{}}}}}}}"#,
            id_a
        ))
        .unwrap();
    response_tx
        .send(format!(
            r#"{{"jsonrpc":"2.0","method":"_fuigo/session/update","params":{{"params":{{"sessionId":"sess-1","update":{{"sessionUpdate":"hook_annotation","message":"m"}},"_meta":{{"isReplay":true,"fuigo/leaderClientId":{}}}}}}}}}"#,
            id_a
        ))
        .unwrap();
