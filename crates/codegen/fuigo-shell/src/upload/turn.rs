...
pub(crate) enum TurnMessages {
    Captured(fuigo_chat_state::TurnCapture),
...
impl From<Option<fuigo_chat_state::TurnCapture>> for TurnMessages {
    fn from(capture: Option<fuigo_chat_state::TurnCapture>) -> Self {
        match capture {
...
pub(crate) use fuigo_telemetry::session_metrics::TraceUploadReason;
#[derive(Clone)]
    ...
    F: Future<Output = ()> + Send + 'static,
{
    ...
    let span = match attach {
        ...
        UploadSpanAttach::Link {
            ...
            fuigo_file_utils::trace_context::link_span_to_current(&root);
            root
        }
    };
...
pub(crate) fn parse_agent_profile_from_meta(
...
) -> Option<fuigo_agent::AgentDefinition> {
    let value = meta?.get("agentProfile")?;
    if value.is_object() {
        return match fuigo_agent::AgentDefinition::from_json(value) {
            Ok(def) => {
                tracing::info!(
                    agent_name = %def.name,
    ...
    if let Some(name) = value.as_str() {
        ...
        return fuigo_agent::discovery::by_name(name);
    }
    tracing::warn!(
