//! Built-in files extracted to `~/.fuigo/` on startup.

const BUILTIN_FILES: &[(&str, &str)] = &[("README.md", include_str!("../README.md"))];

/// Extract built-in metadata files to `~/.fuigo/` on startup.
///
/// User skills under `~/.fuigo/skills/` are never managed here. Platform skills
/// are delivered separately through the bundled skill cache.
pub fn extract_builtin_files(fuigo_home: &std::path::Path) {
    let version = fuigo_version::VERSION;
    let marker = fuigo_home.join(".metadata_version");
    ...
    for stale in &["CHANGELOG.json", "CHANGELOG.md"] {
        let _ = std::fs::remove_file(fuigo_home.join(stale));
    }

    for &(filename, content) in BUILTIN_FILES {
        if let Err(e) = std::fs::write(fuigo_home.join(filename), content) {
...
/// `(name, sha256)` of every `SKILL.md` body ever extracted into `$FUIGO_HOME/skills/`; `help` rows hash the pre-substitution bytes.
...
/// Remove platform-skill leftovers extracted into `$FUIGO_HOME/skills/` by pre-bundle binaries, where they shadow `bundled/skills/`.
...
pub fn purge_stale_extracted_skills(fuigo_home: &std::path::Path) {
    purge_skill_dirs_matching(fuigo_home, FORMER_PLATFORM_SKILL_HASHES);
}

fn purge_skill_dirs_matching(fuigo_home: &std::path::Path, known: &[(&str, &str)]) {
    // For reversing the extract-time `~/.fuigo/` -> home rewrite (help only).
    let home_prefix = format!("{}/", fuigo_home.to_string_lossy());

    let mut names: Vec<&str> = known.iter().map(|&(name, _)| name).collect();
    names.dedup();

    for name in names {
        let dir = fuigo_home.join("skills").join(name);
        ...
        let unrewritten = sha256_hex(content.replace(&home_prefix, "~/.fuigo/").as_bytes());
        let managed = known
            .iter()
...
mod tests {
    ...
    fn same_version_does_not_restore_missing_or_delete_legacy_skills() {
        ...
        std::fs::write(home.join(".metadata_version"), fuigo_version::VERSION).unwrap();
    ...
    fn purge_reverses_home_rewrite_before_hashing() {
        ...
        let raw = EXTRACTED_BODY.replace("platform", "read ~/.fuigo/docs/user-guide then platform");
        let body = raw.replace("~/.fuigo/", &format!("{}/", home.to_string_lossy()));
