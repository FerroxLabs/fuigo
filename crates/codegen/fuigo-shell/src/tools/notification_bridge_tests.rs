...
fn make_test_config_full() -> (
    NotificationBridgeConfig,
    mpsc::UnboundedReceiver<fuigo_acp_lib::AcpClientMessage>,
...
fn make_test_config_full_raw() -> (
    ...
    let gateway = fuigo_acp_lib::AcpAgentGatewaySender::new(gateway_tx);
...
fn task_completed_will_wake(
    gateway_rx: &mut mpsc::UnboundedReceiver<fuigo_acp_lib::AcpClientMessage>,
) -> Option<bool> {
    while let Ok(msg) = gateway_rx.try_recv() {
        if let fuigo_acp_lib::AcpClientMessage::ExtNotification(args) = msg
            && args.request.method.as_ref() == "fuigo/task_completed"
...
async fn monitor_event_skipped_after_task_completed_auto_wake() {
    ...
    handle_notification(
        &config,
        ToolNotification::MonitorEvent(fuigo_tools::notification::types::MonitorEvent {
...
async fn scheduled_task_created_is_persisted() {
    ...
    let notification = ToolNotification::ScheduledTaskCreated(
        fuigo_tools::notification::types::ScheduledTaskCreated {
            task_id: "loop-1".into(),
    ...
    match msg {
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Fuigo(notif)) => {
            ...
            assert!(
                ...
                "persisted Ferrox Labs bridge lines must carry an eventId"
            );
        }
        _ => panic!("expected PersistenceMsg::Update(Fuigo(ScheduledTaskCreated))"),
    }
}
...
async fn bash_output_chunk_forwards_live_without_persisting() {
    ...
    let notification =
        ToolNotification::BashOutputChunk(fuigo_tools::notification::types::BashOutputChunk {
...
async fn live_in_progress_event_id_is_not_a_prepare_replay_cursor() {
    ...
    let live_id = match gateway_rx.try_recv().unwrap() {
        fuigo_acp_lib::AcpClientMessage::SessionNotification(args) => args
            .request
            .meta
            .as_ref()
            .and_then(|m| m.get("eventId"))
...
async fn scheduled_task_removed_is_persisted() {
    ...
    let (config, _gateway_rx, mut persistence_rx, _cmd_rx) = make_test_config_full();
    let removed = fuigo_tools::notification::ScheduledTaskRemoved::new(
        "loop-1".into(),
        fuigo_tools::notification::ScheduledTaskRemovedReason::Expired,
        "generation-a".into(),
        2,
    );
        ...
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Fuigo(notif)) => {
            assert!(matches!(
                ...
                crate::extensions::notification::SessionUpdate::ScheduledTaskDeleted {
                    reason: fuigo_tools::notification::ScheduledTaskRemovedReason::Expired,
                    ..
                }
            ));
            assert!(
                fuigo_persisted_event_id(&notif).is_some(),
...
async fn acknowledged_scheduler_removal_appends_before_ack_and_broadcast() {
    ...
    let removed = fuigo_tools::notification::ScheduledTaskRemoved::new(
        "loop-ack".into(),
        fuigo_tools::notification::ScheduledTaskRemovedReason::Deleted,
        "generation-a".into(),
        17,
    );
    ...
    let persistence = async {
        let PersistenceMsg::AppendUpdateDurablyAndAck {
            update: crate::session::storage::SessionUpdate::Fuigo(notification),
            respond_to,
    ...
    assert!(matches!(
        gateway_rx.try_recv(),
        Ok(fuigo_acp_lib::AcpClientMessage::ExtNotification(_))
    ));
}

fn fuigo_persisted_event_id(
...
async fn task_backgrounded_persisted_line_is_stamped() {
    ...
    match persistence_rx.try_recv().expect("must persist") {
        PersistenceMsg::Update(crate::session::storage::SessionUpdate::Fuigo(notif)) => {
            assert!(fuigo_persisted_event_id(&notif).is_some());
        }
        _ => panic!("expected Fuigo update"),
    }
}
...
async fn current_mode_update_persisted_line_is_stamped() {
    ...
    emit_current_mode_update(&config, fuigo_tools::types::SessionMode::Plan).await;
...
async fn scheduled_task_fired_is_not_persisted() {
    ...
    let notification = ToolNotification::ScheduledTaskFired(
        fuigo_tools::notification::types::ScheduledTaskFired {
            task_id: "loop-1".into(),
            prompt: "check deploy".into(),
            human_schedule: "every 5 minutes".into(),
            next_fire_at: Some("2026-01-01T00:00:00Z".into()),
    ...
    let fuigo_acp_lib::AcpClientMessage::ExtNotification(fired) = fired else {
...
async fn legacy_monitor_event_without_owner_is_injected() {
    // Legacy / non-fuigo-build backends record no owner; such events must pass through unchanged for backwards compatibility
    let (config, mut cmd_rx) = make_test_config();
...
async fn plan_mode_exited_emits_current_mode_update_default() {
    ...
    *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Plan;
    ...
    let notification =
        ToolNotification::PlanModeExited(fuigo_tools::notification::types::PlanModeExited {
...
/// With the default (fuigo) configuration, the exit_plan_mode tool result is the model's only exit signal.
...
async fn plan_mode_exited_does_not_arm_exit_reminder_by_default() {
    ...
    let notification =
        ToolNotification::PlanModeExited(fuigo_tools::notification::types::PlanModeExited {
            tool_call_id: "tc-exit-fuigo".into(),
            plan_content: Some("- step 1".into()),
            plan_file_path: "/tmp/test-session/plan.md".into(),
        });
...
async fn plan_mode_entered_emits_current_mode_update_plan() {
    ...
    let notification =
        ToolNotification::PlanModeEntered(fuigo_tools::notification::types::PlanModeEntered {
...
async fn task_completed_notification_is_frame_bounded() {
    ...
    while let Ok(msg) = persistence_rx.try_recv() {
        if let PersistenceMsg::Update(crate::session::storage::SessionUpdate::Fuigo(saved)) = msg {
