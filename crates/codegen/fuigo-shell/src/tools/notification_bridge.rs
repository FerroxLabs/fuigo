//! Translates `fuigo-tools` `ToolNotification` events into `fuigo-shell`'s native systems (ACP gateway, hunk tracker, file state tracker).
...
pub(crate) struct NotificationBridgeConfig {
    ...
    pub task_completion_reservations:
        fuigo_tools::reminders::task_completion::TaskCompletionReservations,
    pub task_wake_suppressed: fuigo_tools::reminders::task_completion::TaskWakeSuppressed,
    ...
    /// Fuigo-build leaves this `false`: its exit-plan tool result already informs the model, and a deferred reminder would arrive stale.
...
async fn handle_scheduled_task_removed(
    config: &NotificationBridgeConfig,
    removed: fuigo_tools::notification::ScheduledTaskRemoved,
    ...
    let result: Result<Box<serde_json::value::RawValue>, String> = async {
        ...
        let update = crate::session::storage::SessionUpdate::Fuigo(Box::new(notification));
...
async fn emit_current_mode_update(
    config: &NotificationBridgeConfig,
    mode: fuigo_tools::types::SessionMode,
) {
        ...
        ToolNotification::TaskCompleted(task_snapshot) => {
            ...
            if task_snapshot.is_auto_wake_suppressed() {
                ...
                let body = if is_monitor {
                    fuigo_tools::reminders::task_completion::format_monitor_completion(
                        &task_snapshot,
                        tool_name,
                    )
                } else {
                    fuigo_tools::reminders::task_completion::format_bash_completion(
                        &task_snapshot,
                        tool_name,
                        read_name,
                    )
                };
                let message = fuigo_tools::reminders::wrap_reminder(&body);
                ...
                let enqueued = config
                    ...
                    .send(SessionCommand::Prompt {
                        ...
                        screen_mode: None,
                        verbatim: true,
                        traceparent: fuigo_file_utils::trace_context::current_traceparent(),
                        json_schema: None,
                        send_now: false,
            ...
            if deactivated {
                ...
                *config.turn_prompt_mode.lock() = crate::session::plan_mode::PromptMode::Agent;
                ...
                let _ = config
                    ...
                    .tx
                    .send(PersistenceMsg::PlanModeState(snapshot));
                emit_current_mode_update(config, fuigo_tools::types::SessionMode::Default).await;
            }
            tracing::info!(
