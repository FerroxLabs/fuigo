...
impl BashToolConfig {
    ...
    pub(crate) fn to_bash_params_json(
        ...
        // Production fuigo-build opts up to 10h by sending it explicitly (overridable via config.toml)
...
pub struct WebFetchToolConfig {
    ...
    /// Resolution: TOML > `FUIGO_WEB_FETCH_PROXY` env > remote settings > None.
    ...
    /// Resolution: TOML > `FUIGO_WEB_FETCH_ALLOW_LOCAL` env > false.
...
impl WebFetchToolConfig {
    ...
    pub(crate) fn resolve_params(
    ...
    ) -> fuigo_tools::implementations::fuigo_build::web_fetch::WebFetchParams {
        ...
        let proxy_endpoint = self
            ...
            .as_ref()
            .cloned()
            .or_else(|| env_string("FUIGO_WEB_FETCH_PROXY"))
        ...
        let allow_local = self
            .allow_local
            .or_else(|| fuigo_config::env_bool("FUIGO_WEB_FETCH_ALLOW_LOCAL"));
...
/// It is distinct from `fuigo_tools::registry::types::ToolsetConfig` which holds tool-implementation-level config (bash limits, web search mode).
        ...
        match self {
            ...
            Self::Hashline => {
                ...
                Ok(vec![
                    ...
                    ToolConfig {
                        id: "FuigoBuildHashline:hashline_edit".to_owned(),
                        params: params_map.clone(),
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
                    ...
                    ToolConfig {
                        id: "FuigoBuildHashline:hashline_grep".to_owned(),
                        params: params_map,
                        name_override: None,
                        params_name_overrides: None,
                        description_override: None,
...
mod tests {
    ...
    fn standard_toolset_configs() {
        ...
        assert!(ids.contains(&"FuigoBuild:read_file"));
        assert!(ids.contains(&"FuigoBuild:search_replace"));
        assert!(ids.contains(&"FuigoBuild:grep"));
    }
    ...
    fn hashline_toolset_configs() {
        ...
        assert_eq!(configs.len(), 3);
        let ids: Vec<&str> = configs.iter().map(|c| c.id.as_str()).collect();
        assert!(ids.contains(&"FuigoBuildHashline:hashline_read"));
        assert!(ids.contains(&"FuigoBuildHashline:hashline_edit"));
        assert!(ids.contains(&"FuigoBuildHashline:hashline_grep"));
    }
    ...
    fn file_toolset_override_never_grants_edit_to_read_only_toolsets() {
        ...
        for mut def in [
            fuigo_agent::config::AgentDefinition::plan(),
            fuigo_agent::config::AgentDefinition::explore(),
        ] {
            ...
            assert!(
                !def.tool_config
                    .tools
                    .iter()
                    .any(|t| t.id == "FuigoBuild:search_replace"),
            ...
            assert!(
                ids.contains(&"FuigoBuildHashline:hashline_read"),
                "{name}: {ids:?}"
            );
            assert!(!ids.contains(&"FuigoBuild:read_file"), "{name}: {ids:?}");
            // ...but never grants the edit slot.
            assert!(
                !ids.contains(&"FuigoBuildHashline:hashline_edit"),
    ...
    fn max_timeout_secs_defaults_to_production_10h() {
        ...
        assert_eq!(
            ...
            "production fuigo-build must set the 10h foreground ceiling"
        );
    }
