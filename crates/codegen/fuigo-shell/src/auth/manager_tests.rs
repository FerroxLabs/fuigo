...
fn make_auth(expires_at: Option<DateTime<Utc>>, create_time: DateTime<Utc>) -> FuigoAuth {
    FuigoAuth {
...
fn manager_collection_predicates_fail_directions() {
    ...
    mgr.hot_swap(FuigoAuth::test_default());
    assert!(!mgr.is_data_collection_disabled());
...
fn try_use_disk_token_accepts_different_key_on_server_rejected() {
    ...
    let cfg = FuigoComConfig::default();
    let mgr = Arc::new(AuthManager::new(dir.path(), cfg));
    let mem_auth = FuigoAuth {
        key: "old-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    mgr.hot_swap(mem_auth);
    let disk_auth = FuigoAuth {
        key: "new-key".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
...
fn try_use_disk_token_skips_disk_token_older_than_memory_mint() {
    ...
    let cfg = FuigoComConfig::default();
    let mgr = Arc::new(AuthManager::new(dir.path(), cfg));
    let fresh_mint = FuigoAuth {
        key: "fresh-mint".into(),
        ..make_auth(Some(Utc::now() + Duration::hours(1)), Utc::now())
    };
    mgr.hot_swap(fresh_mint);
    let lagging_disk = FuigoAuth {
        key: "stale-disk".into(),
        ..make_auth(
...
async fn refresh_chain_demotes_when_attributed_tried_rt_differs_from_disk() {
    ...
    write_auth_json(&dir.path().join("auth.json"), &store).unwrap();
    struct AttributedRejection(FuigoAuth);
    #[async_trait::async_trait]
    ...
    assert!(
        ...
        "demotion must not record a sticky verdict that locks out every \
         sibling process until the user re-runs `fuigo login`",
    );
}
...
async fn permanent_other_retains_credentials() {
    ...
    let mut store = AuthStore::new();
    store.insert(FuigoComConfig::default().auth_scope(), session);
    write_auth_json(&dir.path().join("auth.json"), &store).unwrap();
...
async fn sticky_permanent_allows_refresh_when_attempted_key_differs() {
    ...
    impl crate::auth::refresh::TokenRefresher for CountingOk {
        async fn refresh(
            ...
            self.0.fetch_add(1, Ordering::SeqCst);
            crate::auth::refresh::RefreshOutcome::Success(Box::new(FuigoAuth {
...
/// `key`, `refresh_token`, `expires_at`, `oidc_issuer`, `oidc_client_id`, `auth_mode`, `create_time`, `has_fuigo_code_access`.
/// The `&mut FuigoAuth` signature enforces this at the type level (you cannot construct a fresh auth from a `UserInfo`; there's no `From` impl).
/// A unit test pins the exact list so a contributor adding a token-like field to both `FuigoAuth` and `UserInfo` is forced to look here.
#[test]
fn apply_user_info_enrichment_preserves_token_fields() {
    let mut disk = FuigoAuth {
        key: "ROT_KEY".into(),
        ...
        auth_mode: AuthMode::Oidc,
        create_time: Utc::now() - Duration::minutes(10),
        has_fuigo_code_access: Some(true),
    ...
    assert_eq!(disk.has_fuigo_code_access, snapshot.has_fuigo_code_access);
...
async fn current_api_key_async_drives_refresh_chain() {
    ...
    let _fuigo = EnvGuard::unset("FUIGO_API_KEY");
...
async fn refresh_chain_server_rejected_bypasses_valid_token_double_check() {
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), FuigoComConfig::default()));
    let valid_but_rejected = FuigoAuth {
...
fn oidc_session_for_team(principal_id: &str) -> FuigoAuth {
    FuigoAuth {
...
async fn cached_api_key_session_rejected_when_api_key_auth_disabled() {
    let api_key_session = || FuigoAuth {
        key: "fuigo-cached-key".into(),
        auth_mode: AuthMode::ApiKey,
        expires_at: Some(Utc::now() + Duration::hours(1)),
        ..FuigoAuth::test_default()
    };
    ...
    let mgr2 = Arc::new(AuthManager::new(dir2.path(), FuigoComConfig::default()));
    mgr2.hot_swap(api_key_session());
    assert_eq!(
        mgr2.current().map(|a| a.key),
        Some("fuigo-cached-key".to_string()),
...
/// With no OAuth session, voice/tools fall back to env or the auth.json `fuigo::api_key`.
#[tokio::test]
#[serial_test::serial]
async fn shared_api_key_provider_static_fallthrough() {
    ...
    {
        let _legacy = EnvGuard::unset("FUIGO_CODE_API_KEY");
        let _key = EnvGuard::set("FUIGO_API_KEY", "env-only-key");
        assert_eq!(
    ...
    {
        let _key = EnvGuard::set("FUIGO_API_KEY", "env-should-lose");
        mgr.hot_swap(FuigoAuth {
...
async fn shared_api_key_provider_kill_switch_blocks_static() {
    use fuigo_test_support::EnvGuard;
    let _key = EnvGuard::set("FUIGO_API_KEY", "blocked");
    let dir = tempfile::tempdir().unwrap();
...
async fn shared_api_key_provider_oidc_preferred_blocks_static() {
    use fuigo_test_support::EnvGuard;
    let _key = EnvGuard::set("FUIGO_API_KEY", "should-not-use");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(
        dir.path(),
        FuigoComConfig {
...
async fn shared_api_key_provider_api_key_preferred_skips_session() {
    use fuigo_test_support::EnvGuard;
    let _legacy = EnvGuard::unset("FUIGO_CODE_API_KEY");
    let _key = EnvGuard::set("FUIGO_API_KEY", "static-preferred");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(
        dir.path(),
        FuigoComConfig {
...
async fn shared_api_key_provider_sync_falls_through_when_session_expired() {
    use fuigo_test_support::EnvGuard;
    let _legacy = EnvGuard::unset("FUIGO_CODE_API_KEY");
    let _key = EnvGuard::set("FUIGO_API_KEY", "static-after-expiry");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), FuigoComConfig::default()));
    mgr.hot_swap(FuigoAuth {
        key: "expired-oidc".into(),
...
async fn shared_api_key_provider_sync_buffered_session_beats_static() {
    ...
    use fuigo_tools::types::ApiKeyProvider;
    let _legacy = EnvGuard::unset("FUIGO_CODE_API_KEY");
    let _key = EnvGuard::set("FUIGO_API_KEY", "leftover-static");
    let dir = tempfile::tempdir().unwrap();
    let mgr = Arc::new(AuthManager::new(dir.path(), FuigoComConfig::default()));
    mgr.hot_swap(FuigoAuth {
...
async fn process_key_precedence() {
    ...
    {
        let _key = EnvGuard::set("FUIGO_API_KEY", "env");
        assert_eq!(
            provider.current_api_key_async().await.as_deref(),
            Some("env")
        );
...
/// `FUIGO_AUTH_FORCE_DARK_WAKE` forces the dark-wake answer for manual and integration testing.
...
fn is_dark_wake_env_override_forces_both_states() {
    ...
    let mgr = AuthManager::new(dir.path(), FuigoComConfig::default());
    {
        let _g = EnvGuard::set("FUIGO_AUTH_FORCE_DARK_WAKE", "1");
        assert!(
            mgr.is_dark_wake(),
    ...
    {
        let _g = EnvGuard::set("FUIGO_AUTH_FORCE_DARK_WAKE", "0");
        assert!(!mgr.is_dark_wake(), "=0 must force full wake");
    }
    {
        let _g = EnvGuard::set("FUIGO_AUTH_FORCE_DARK_WAKE", "yes");
...
fn manual_auth_reason_maps_terminal_and_skips_non_forcing() {
    ...
    use fuigo_telemetry::events::ManualAuthReason as R;
...
async fn manual_auth_capture_attributes_and_recorder_debounces() {
    ...
    use fuigo_telemetry::events::{AuthTokenKind, ManualAuthSurface};
    let auth = FuigoAuth {
        key: "dead-token".into(),
        user_id: "user-1".into(),
        auth_mode: AuthMode::Oidc,
...
async fn manual_auth_emits_only_for_user_facing_source() {
    ...
    fn mgr_with(dir: &std::path::Path, key: &str, mode: AuthMode) -> Arc<AuthManager> {
        let mgr = Arc::new(AuthManager::new(dir, FuigoComConfig::default()));
...
fn external_provider_config() -> FuigoComConfig {
    FuigoComConfig {
