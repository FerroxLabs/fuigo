...
/// Parse stdout into a session-credential `FuigoAuth`.
pub(crate) fn parse_output(output: &std::process::Output) -> anyhow::Result<FuigoAuth> {
...
mod tests {
    ...
    #[tokio::test]
    async fn sets_fuigo_auth_expired_env_on_refresh() {
        let auth = run_external_refresh("echo $FUIGO_AUTH_EXPIRED")
            .await
            .unwrap();
