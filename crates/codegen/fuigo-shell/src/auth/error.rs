...
#[non_exhaustive]
pub enum AuthError {
    #[error("Not logged in. Run `fuigo login`.")]
    NotLoggedIn,

    /// The token expired and no refresh authority is available.
    #[error("Token expired. Run `fuigo login` to re-authenticate.")]
    TokenExpiredNoRefresh,

    /// Server rejected the token (401) with no recovery path.
    #[error("Authentication rejected by server. Run `fuigo login` to re-authenticate.")]
    ...
    #[error("{message} Run `fuigo login` to sign in with the required team.")]
    PinnedTeamMismatch { message: String },

    /// The cached API-key session was rejected because API-key auth is disabled.
    #[error("API-key auth is disabled by your administrator. Run `fuigo login` to authenticate.")]
    ApiKeyAuthDisabled,
...
pub enum RefreshTokenFailedReason {
    ...
    ClientRejected,
    /// The operator's `auth_provider_command` could not produce a credential in a headless run (`FUIGO_AUTH_EXPIRED=1`).
    ...
    pub(crate) fn user_message(self) -> Cow<'static, str> {
        match self {
            Self::RefreshTokenRejected => {
                "Your session has expired. Run `fuigo login` to sign in again.".into()
            }
            Self::ClientRejected => {
                "Authentication is temporarily unavailable. Run `fuigo login` if this persists."
                    .into()
            }
