...
async fn provider_refresh_sets_expired_env() {
    ...
    assert_eq!(
        ...
        Some("tok-0"),
        "first mint runs without FUIGO_AUTH_EXPIRED"
    );
    test_expire_provider_token("test-expired-env");
    assert_eq!(
        provider.ensure_fresh_token(None).await.rotated().as_deref(),
        Some("tok-1"),
        "re-mints run with FUIGO_AUTH_EXPIRED=1"
    );
}
...
/// On an in-session re-mint, the prior credential is handed back to the command via `FUIGO_AUTH_PROVIDER_*`.
