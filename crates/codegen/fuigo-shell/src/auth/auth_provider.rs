...
/// That keeps the `FUIGO_AUTH_PROVIDER_*` credentials in their env from outliving the reported timeout.
...
async fn run_capped(
    ...
    let mut group = fuigo_tools::util::ProcessGroup::new()
...
async fn mint_provider_token(
    ...
    if let Some(prev) = previous {
        cmd.env("FUIGO_AUTH_PROVIDER_ACCESS_TOKEN", &prev.token);
        if let Some(refresh) = &prev.refresh_token {
            cmd.env("FUIGO_AUTH_PROVIDER_REFRESH_TOKEN", refresh);
        }
        if let Some(expires_at) = prev.expires_at {
            cmd.env("FUIGO_AUTH_PROVIDER_EXPIRES_AT", expires_at.to_rfc3339());
        }
    }
