...
impl RejectedAuth {
    pub(crate) fn capture(auth: Option<&FuigoAuth>) -> Self {
        Self {
...
impl UnauthorizedRecovery {
    ...
    pub(crate) async fn next(&mut self) -> Result<FuigoAuth, AuthError> {
    ...
    async fn resolve_next(&mut self) -> Result<FuigoAuth, AuthError> {
    ...
    async fn next_step_loop(&mut self) -> Result<FuigoAuth, AuthError> {
        loop {
            match self.step {
                ...
                RecoveryStep::DevboxRecovery => {
                    ...
                    if !self.auth_manager.fuigo_com_config().blocks_automatic_oidc()
    ...
    async fn try_reload_from_disk(&self) -> Option<FuigoAuth> {
        let _lock = self
            .auth_manager
        ...
        let Some(disk_auth) = self.auth_manager.read_disk_auth() else {
            ...
            fuigo_telemetry::unified_log::debug("auth recovery: no disk entry", None, None);
            return None;
        };
    ...
    fn fresh_mint_guard(&self) -> Option<FuigoAuth> {
    ...
    async fn try_refresh_from_authority(&self) -> Result<FuigoAuth, AuthError> {
    ...
    fn is_different_token(&self, candidate: &FuigoAuth) -> bool {
...
mod tests {
    ...
    use crate::auth::model::{AuthMode, FuigoAuth};
    ...
    fn rejected_cred() -> Option<FuigoAuth> {
        Some(FuigoAuth {
    ...
    fn mgr() -> (tempfile::TempDir, Arc<AuthManager>) {
        ...
        let m = Arc::new(AuthManager::new(dir.path(), FuigoComConfig::default()));
        (dir, m)
    }
    ...
    async fn reload_from_disk_picks_up_different_token() {
        ...
        let scope = m.fuigo_com_config().auth_scope();
        let fresh = FuigoAuth {
    ...
    async fn reload_from_disk_rejects_expired_different_token() {
        ...
        let scope = m.fuigo_com_config().auth_scope();
        let expired_different = FuigoAuth {
