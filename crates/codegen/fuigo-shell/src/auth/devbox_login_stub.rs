...
//! The entry points that can still be reached directly (`fuigo login --devbox`) return a descriptive error.

use super::manager::AuthManager;
use super::model::FuigoAuth;
...
pub(crate) async fn mint_devbox_auth(_auth_manager: &AuthManager) -> anyhow::Result<FuigoAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// Unreachable in practice (guarded by [`is_devbox_environment`]); errors defensively if called.
pub(super) async fn mint_devbox_auth_raw() -> anyhow::Result<FuigoAuth> {
    anyhow::bail!(UNAVAILABLE)
}

/// `fuigo login --devbox` entry point: always errors in this build.
