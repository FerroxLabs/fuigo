...
use crate::auth::{AuthChannels, AuthManager, AuthMode, AuthUrlInfo, AuthUrlMode, FuigoAuth};
...
pub(crate) enum DeviceCodeError {
    #[error(
        "Device-code login is not available for this deployment. \
         Try `fuigo login` or set FUIGO_API_KEY instead."
    )]
    NotEnabled,
}
...
/// Low-cardinality hint sent to the OAuth2 provider as the `x-fuigo-client-surface` header.
...
///
/// On success, persists credentials to `~/.fuigo/auth.json` and returns
/// the authenticated `FuigoAuth`.
///
...
pub(crate) async fn complete_device_code_login(
    ...
    loop {
        ...
        if tokio::time::Instant::now() > deadline {
            anyhow::bail!("Device code expired. Run `fuigo login --device-auth` again.");
        }
...
pub(crate) mod tests {
    ...
    use crate::auth::{AuthMode, FuigoComConfig};
    ...
    fn auth_manager_with_fuigo_home(
        fuigo_home: &std::path::Path,
        proxy_base_url: &str,
    ) -> Arc<AuthManager> {
        Arc::new(
            AuthManager::new(fuigo_home, FuigoComConfig::default())
    ...
    fn build_auth_persists_credentials_without_proxy_fetch() {
        ...
        let fuigo_home = temp_dir.path().join(".fuigo");
        std::fs::create_dir_all(&fuigo_home).unwrap();
        let auth_manager = auth_manager_with_fuigo_home(&fuigo_home, "http://127.0.0.1:9");
    ...
    fn assert_build_auth_rejected(cfg: FuigoComConfig, token_principal: &str, expected_err: &str) {
        ensure_crypto_provider();
        let temp_dir = tempfile::tempdir().unwrap();
        let fuigo_home = temp_dir.path().join(".fuigo");
        std::fs::create_dir_all(&fuigo_home).unwrap();
        let auth_manager =
            Arc::new(AuthManager::new(&fuigo_home, cfg).with_proxy_base_url("http://127.0.0.1:9"));
        ...
        assert!(
            !fuigo_home.join("auth.json").exists(),
    ...
    async fn run_poll(
        responses: Vec<(u16, serde_json::Value)>,
    ) -> anyhow::Result<(super::FuigoAuth, bool)> {
        let (issuer, server) = spawn_token_server(responses).await;
        let temp_dir = tempfile::tempdir().unwrap();
        let auth_manager = auth_manager_with_fuigo_home(temp_dir.path(), "http://127.0.0.1:9");
