...
fn is_cached_credential_compatible(auth: &FuigoAuth, fuigo_com_config: &FuigoComConfig) -> bool {
    let expected_issuer = fuigo_com_config
        .oidc
        .as_ref()
        .map(|c| c.issuer.as_str())
        .or_else(|| fuigo_com_config.oauth2.as_ref().map(|c| c.issuer.as_str()));
    ...
    if let Some(policy) = crate::auth::oidc::login_principal_policy(fuigo_com_config) {
...
fn resolve_device_flow(
    ...
    crate::agent::config::BoolFlag::env("FUIGO_LOGIN_DEVICE_FLOW")
...
/// Whether `run_cli_login` should use the device flow for `config`: only the Ferrox Labs OAuth2 provider supports it.
...
async fn cli_should_use_device(
    config: &FuigoComConfig,
    login_override: LoginTransportOverride,
) -> bool {
    !crate::auth::oidc::is_configured(config) && should_use_device_flow(login_override).await
}
/// Whether interactive Ferrox Labs OAuth2 login uses the RFC 8628 device flow (vs loopback).
///
/// Precedence, highest first: the `--oauth`/`--device-auth` CLI flags, `FUIGO_LOGIN_DEVICE_FLOW` env, then `[auth] login_device_flow` config.
/// Below those, the `fuigo_build_login_device_flow` remote feature flag decides; loopback is the default.
async fn should_use_device_flow(login_override: LoginTransportOverride) -> bool {
    ...
    let resolved = if login_override.as_cli_bool().is_some() {
        ...
        let env = crate::agent::config::env_bool("FUIGO_LOGIN_DEVICE_FLOW");
...
/// Sets no `FUIGO_AUTH_EXPIRED`: operator binaries, which live outside this repo, read that variable as "headless, don't prompt" and decline the run.
...
pub(super) async fn run_auth_flow_steps(
    ...
    tracing::info!(
        has_oidc = fuigo_com_config.oidc.is_some(),
        has_oauth2 = fuigo_com_config.oauth2.is_some(),
        has_external_auth = fuigo_com_config.auth_provider_command.is_some(),
        reauth,
    ...
    if !force_interactive && let Some(auth) = auth_manager.current() {
        if is_cached_credential_compatible(&auth, fuigo_com_config) {
    ...
    if !force_interactive && !reauth && auth_manager.is_expired() {
        ...
        if disk_auth.as_ref().is_some_and(|d| {
            !crate::auth::is_expired(d) && is_cached_credential_compatible(d, fuigo_com_config)
        }) {
            fuigo_telemetry::unified_log::info(
    ...
    if let Some(ref cmd) = fuigo_com_config.auth_provider_command {
    ...
    if !fuigo_com_config.blocks_automatic_oidc()
    ...
    if crate::auth::oidc::is_configured(fuigo_com_config) {
        return crate::auth::oidc::run_login_flow(fuigo_com_config, auth_manager, channels).await;
    }
    if let Some(ref oauth2_cfg) = fuigo_com_config.oauth2 {
...
pub async fn try_ensure_fresh_auth(fuigo_com_config: &FuigoComConfig) -> Option<FuigoAuth> {
    try_ensure_fresh_auth_with(&build_startup_auth_manager(fuigo_com_config)).await
}
...
fn build_startup_auth_manager(fuigo_com_config: &FuigoComConfig) -> Arc<AuthManager> {
    let auth_manager = Arc::new(AuthManager::new(
        &fuigo_home::fuigo_home(),
        fuigo_com_config.clone(),
    ));
    auth_manager.configure_refresher(fuigo_com_config.auth_provider_command.clone(), None);
    auth_manager
}
/// Uses cached valid credentials, else a silent refresh; never interactive login.
async fn try_ensure_fresh_auth_with(auth_manager: &Arc<AuthManager>) -> Option<FuigoAuth> {
    match auth_manager.auth().await {
        Ok(auth) => Some(auth),
        Err(e) => {
...
pub(crate) async fn try_noninteractive_auth_no_mint(
    ...
    try_noninteractive_auth_no_mint_with(&build_startup_auth_manager(fuigo_com_config)).await
}
...
fn expired_refreshable_session(auth_manager: &AuthManager) -> Option<FuigoAuth> {
    auth_manager
        .current_or_expired()
        .filter(|a| a.is_fuigo_auth() && a.refresh_token.is_some())
}
...
pub(crate) async fn mint_session_noninteractive(
    ...
    let fuigo_com_config = auth_manager.fuigo_com_config();
    if !ActiveAuthBackend::default().is_fuigo_authority() {
        return None;
    }
    if fuigo_com_config.blocks_automatic_oidc() {
        tracing::debug!(
    ...
    if let Some(cmd) = fuigo_com_config.auth_provider_command.as_deref() {
...
async fn persist_or_use_minted(auth_manager: &AuthManager, new_auth: FuigoAuth) -> FuigoAuth {
...
pub async fn ensure_authenticated_with_override(
    ...
    let fuigo_home = fuigo_home::fuigo_home();
    let auth_manager = Arc::new(AuthManager::new(&fuigo_home, fuigo_com_config.clone()));
...
pub async fn ensure_authenticated_or_noninteractive(
    ...
    message_prefix: Option<&str>,
) -> anyhow::Result<Option<FuigoAuth>> {
    if has_noninteractive_auth {
        Ok(try_ensure_fresh_auth(fuigo_com_config).await)
    } else {
        ensure_authenticated(fuigo_com_config, false, message_prefix)
            .await
            .map(Some)
    }
}
/// Unified `fuigo login` handler for CLI entry points (tui, pager).
///
/// Precedence: `--oauth` forces loopback, `--device-auth` forces device.
/// Otherwise `FUIGO_LOGIN_DEVICE_FLOW` env, then `[auth] login_device_flow` config, then the loopback default.
...
pub async fn run_cli_login(
    ...
    if devbox {
        if !ActiveAuthBackend::default().is_fuigo_authority() {
            anyhow::bail!("--devbox mints an Ferrox Labs credential, which this build cannot use");
        }
...
/// `fuigo setup` reports the no-config case.
pub(crate) async fn apply_post_login_config(authenticated: FuigoAuth) -> anyhow::Result<()> {
...
pub struct LogoutResult {
    ...
    /// `true` if `FUIGO_API_KEY` / `FUIGO_CODE_API_KEY` env var is set.
...
pub fn perform_logout(
    ...
    Ok(LogoutResult {
        was_logged_in,
        email,
        api_key_still_set: crate::agent::auth_method::has_fuigo_api_key_env(),
    })
}
pub fn run_cli_logout(config: &crate::agent::config::Config) -> anyhow::Result<()> {
    let fuigo_home = fuigo_home::fuigo_home();
    let auth_manager = AuthManager::new(&fuigo_home, config.fuigo_com_config.clone());
...
mod tests {
    ...
    use chrono::Utc;
    use std::path::Path;
    /// `os_error` and the reqwest classification are covered in `fuigo-http`.
    ...
    /// That breaks unrelated auth tests, so classification is covered in `fuigo-http`.
    #[test]
    ...
    /// Run `f` with `FUIGO_LOGIN_DEVICE_FLOW` set to `value` (unset for `None`).
    ...
    fn with_device_flow_env<T>(value: Option<bool>, f: impl FnOnce() -> T) -> T {
        let _guard = match value {
            Some(true) => EnvVarGuard::set("FUIGO_LOGIN_DEVICE_FLOW", "true"),
            Some(false) => EnvVarGuard::set("FUIGO_LOGIN_DEVICE_FLOW", "false"),
            None => EnvVarGuard::remove("FUIGO_LOGIN_DEVICE_FLOW"),
        };
        f()
    }
    fn oidc_session(key: &str, refresh: Option<&str>) -> FuigoAuth {
        FuigoAuth {
            key: key.into(),
    ...
    async fn mint_session_noninteractive_uses_external_provider() {
        ...
        let cfg = FuigoComConfig {
            auth_provider_command: Some("printf '%s' fuigo-ext-token".to_string()),
        ...
        assert_eq!(auth.map(|a| a.key), Some("fuigo-ext-token".to_string()));
    }
    #[tokio::test]
    async fn interactive_login_carries_no_expired_flag_even_over_a_stale_credential() {
        let echo_env = "printf '%s' \"e=${FUIGO_AUTH_EXPIRED:-unset}\"";
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
    ...
    async fn a_provider_written_to_the_published_contract_can_sign_in_after_an_expiry() {
        let conforming =
            r#"if [ "$FUIGO_AUTH_EXPIRED" = "1" ]; then exit 1; else printf '%s' sso-token; fi"#;
        let dir = tempfile::tempdir().unwrap();
        let mgr = Arc::new(
            AuthManager::new(dir.path(), FuigoComConfig::default())
                .with_proxy_base_url(&dead_proxy_url()),
    ...
    async fn preresolved_bypasses_resolver_and_is_never_cli() {
        {
            let _guard = EnvVarGuard::set("FUIGO_LOGIN_DEVICE_FLOW", "false");
            assert!(
        ...
        {
            let _guard = EnvVarGuard::set("FUIGO_LOGIN_DEVICE_FLOW", "true");
            assert!(
    ...
    fn oidc_auth(issuer: &str) -> FuigoAuth {
        FuigoAuth {
    ...
    fn pinned_cfg(team: &str) -> FuigoComConfig {
        FuigoComConfig {
    ...
    fn extract_url_from_external_provider_stderr() {
        ...
        assert_eq!(
            extract(
                "Visit the following link to sign into Fuigo: https://auth.example.com/login?code=abc"
            ),
    ...
    /// CLI `fuigo login` passes `on_stderr=None`; stderr must be inherited so sign-in URLs appear in real time.
    ...
    async fn no_mint_readiness_auth_is_bounded() {
        ...
        assert!(
            result.is_none(),
            "a non-Ferrox Labs expired session is no first-party fallback and no mint runs on this path, so no auth is produced"
        );
        server.abort();
    }
    const _: () = assert!(
