...
pub(crate) enum BoundedRefresh {
    ...
    /// Boxed like [`SilentRefresh::Renewed`]: `FuigoAuth` is large and the other variant is unit-sized.
    Resolved(Box<Result<FuigoAuth, AuthError>>),
    ...
    pub(crate) fn auth_remedy(&self) -> AuthRemedy {
        ...
        match (user_must_act, provider_mints_sessions) {
            ...
            (true, true) => AuthRemedy::ProviderLogin {
                label: self.fuigo_com_config().auth_provider_label.clone(),
            },
...
mod tests {
    ...
    use crate::auth::{FuigoComConfig, error::RefreshTokenFailedReason};
    ...
    fn external_credential(expires_at: chrono::DateTime<Utc>) -> FuigoAuth {
        FuigoAuth {
            key: "external".into(),
    ...
    fn provider_manager(dir: &std::path::Path, credential: FuigoAuth) -> Arc<AuthManager> {
