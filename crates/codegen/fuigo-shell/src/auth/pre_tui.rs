...
//!
//! `auth_provider_command` already works via `fuigo login` because stderr is inherited.
...
//! This module is the first-launch equivalent of `fuigo login`.
...
use super::{AuthManager, FuigoAuth, FuigoComConfig, try_ensure_fresh_auth};
...
pub async fn maybe_run_pre_tui_external_login(
    ...
    let Some(cmd) = fuigo_com_config.auth_provider_command.as_deref() else {
        return Ok(PreTuiLoginOutcome::Skipped);
    };
    if !should_attempt_pre_tui_external_login(true, stdin_is_tty) {
        return Ok(PreTuiLoginOutcome::Skipped);
    }
    if !force_login && try_ensure_fresh_auth(fuigo_com_config).await.is_some() {
        return Ok(PreTuiLoginOutcome::Skipped);
    }
...
/// The `AuthManager` is injected so tests can use a temp fuigo-home instead of the process-cached [`fuigo_home::fuigo_home`].
...
mod tests {
    ...
    async fn provider_echo_token_signs_in() {
        let (_dir, mgr) = isolated_manager(Some("printf '%s' fuigo-ext-token"));
        let outcome = run_pre_tui_external_login_with(&mgr, "printf '%s' fuigo-ext-token", false)
            .await
            .expect("provider must mint");
        match outcome {
            PreTuiLoginOutcome::SignedIn(auth) => {
                assert_eq!(auth.key, "fuigo-ext-token");
                assert!(mgr.current().is_some());
            }
