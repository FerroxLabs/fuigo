...
use crate::auth::{AuthManager, AuthUrlInfo, FuigoAuth, FuigoComConfig, LoginTransportOverride};
...
pub(crate) struct LoginRequest<'a> {
    ...
    pub(crate) fuigo_com_config: &'a FuigoComConfig,
...
pub(crate) trait AuthBackend {
    ...
    fn owns(&self, auth: &FuigoAuth) -> bool;
    ...
    /// Whether Ferrox Labs issued this backend's credentials and may therefore receive them.
    /// Gates every request that carries the bearer to an Ferrox Labs host, and every Ferrox Labs-only policy.
...
pub(crate) type ActiveAuthBackend = fuigo::FuigoAuthBackend;
