...
use crate::auth::config::FuigoComConfig;
use crate::auth::error::AuthError;
use crate::auth::token_type::TokenType;
use fuigo_telemetry::events::ManualAuthSurface;

#[cfg(test)]
use super::model::UserInfo;
use super::model::{
    AuthMode, FuigoAuth, early_invalidation, is_expired, is_expired_with_buffer, lookup_auth,
};
...
pub struct AuthManager {
    ...
    inner: Arc<RwLock<Option<FuigoAuth>>>,
    path: PathBuf,
    scope: String,
    ...
    power_listener: parking_lot::Mutex<Option<fuigo_system_power::SystemPowerListener>>,
...
impl AuthManager {
    pub fn new(fuigo_home: &Path, fuigo_com_config: FuigoComConfig) -> Self {
        ...
        fuigo_telemetry::unified_log::info(
            ...
            Some(serde_json::json!({
                "scope": &scope,
                "fuigo_home": fuigo_home.display().to_string(),
                "HOME": std::env::var("HOME").unwrap_or_else(|_| "(unset)".into()),
                "FUIGO_HOME": std::env::var("FUIGO_HOME").unwrap_or_else(|_| "(unset)".into()),
                "FUIGO_AUTH_PATH": std::env::var("FUIGO_AUTH_PATH").unwrap_or_else(|_| "(unset)".into()),
                "FUIGO_AUTH": std::env::var("FUIGO_AUTH").map(|_| "(set)".to_string()).unwrap_or_else(|_| "(unset)".into()),
            })),
        );

        // FUIGO_AUTH_PATH: custom file path (overrides default $FUIGO_HOME/auth.json).
        ...
        if let Ok(inline_json) = std::env::var("FUIGO_AUTH") {
            if let Ok(auth) = serde_json::from_str::<FuigoAuth>(&inline_json) {
    ...
    pub(crate) fn cached_token_policy_error(&self, auth: &FuigoAuth) -> Option<AuthError> {
        ...
        // The pin reads an Ferrox Labs-issued claim, and a token minted elsewhere has none.
        ...
        let policy = crate::auth::oidc::login_principal_policy(&self.fuigo_com_config)?;
    ...
    fn vet_cached(&self, auth: FuigoAuth) -> Option<FuigoAuth> {
    ...
    pub(crate) fn current(&self) -> Option<FuigoAuth> {
    ...
    #[inline]
    pub(crate) fn with_inner_read<R>(&self, f: impl FnOnce(Option<&FuigoAuth>) -> R) -> R {
    ...
    pub(crate) fn current_or_expired(&self) -> Option<FuigoAuth> {
    ...
    pub(crate) fn current_wire_valid(&self) -> Option<FuigoAuth> {
        let auth = self
            .owned_inner()
    ...
    pub(crate) fn expired_auth(&self) -> Option<FuigoAuth> {
        let auth = self.owned_inner().filter(|a| self.is_token_expired(a))?;
        self.vet_cached(auth)
    }
    ...
    fn is_token_expired(&self, auth: &FuigoAuth) -> bool {
    ...
    fn is_token_hard_expired(&self, auth: &FuigoAuth) -> bool {
        self.token_expired_with_buffer(auth, Duration::zero())
    }

    fn token_expired_with_buffer(&self, auth: &FuigoAuth, buffer: Duration) -> bool {
        if auth.expires_at.is_some() {
            return is_expired_with_buffer(auth, buffer);
        }
        if auth.auth_mode == AuthMode::External
            && let Some(ttl) = self.fuigo_com_config.auth_token_ttl
        {
    ...
    pub(crate) async fn update(self: &Arc<Self>, auth: FuigoAuth) -> std::io::Result<FuigoAuth> {
        ...
        match &write_result {
            Ok(()) => fuigo_telemetry::unified_log::info(
            ...
            Err(e) => fuigo_telemetry::unified_log::error(
    ...
    /// `/user` lives on the Ferrox Labs proxy, so a build pointed elsewhere would send its bearer to the wrong host.
    /// That would happen on every login and every refresh.
    fn spawn_user_info_enrichment(self: &Arc<Self>, auth: FuigoAuth) {
        if !ActiveAuthBackend::default().is_fuigo_authority() {
            return;
        }
    ...
    pub(crate) async fn enrich_auth_inline(&self, auth: &mut FuigoAuth) {
        enrichment::enrich_inline(self, auth).await;
    }

    /// Path to the `auth.json` this manager reads/writes (respects `FUIGO_AUTH_PATH` / constructor home).
    /// Prefer this over `fuigo_home()/auth.json` so temp-home tests and custom stores stay isolated.
    pub(crate) fn auth_json_path(&self) -> &Path {
        &self.path
    }

    pub(crate) fn fuigo_com_config(&self) -> &FuigoComConfig {
        &self.fuigo_com_config
    }
    ...
    pub(crate) async fn run_external_refresh_command(&self, command: &str) -> Option<FuigoAuth> {
    ...
    pub(crate) fn hot_swap(&self, new_auth: FuigoAuth) {
    ...
    pub(crate) fn try_use_disk_token(
        &self,
        disk_auth: Option<&FuigoAuth>,
        reason: RefreshReason,
    ) -> Result<FuigoAuth, DiskTokenDecline> {
    ...
    fn try_adopt_disk_token(&self, reason: RefreshReason, msg: &str) -> Option<FuigoAuth> {
    ...
    fn inner_auth_or_external_default(&self) -> FuigoAuth {
        self.owned_inner().unwrap_or_else(|| FuigoAuth {
    ...
    #[cfg(test)]
    fn persist_and_swap(&self, auth: FuigoAuth) -> Option<FuigoAuth> {
    ...
    pub(crate) fn read_disk_auth(&self) -> Option<FuigoAuth> {
    ...
    pub(crate) fn read_disk_auth_with_state(&self) -> (Option<FuigoAuth>, DiskAuthState) {
    ...
    fn observe_disk_state(
        ...
        match new_state {
            // Recovery (or first observation in FUIGO_AUTH mode).
            DiskAuthState::Ok => {
                fuigo_telemetry::unified_log::info(
    ...
    pub async fn auth(self: &Arc<Self>) -> Result<FuigoAuth, AuthError> {
    ...
    async fn auth_dispatch(self: &Arc<Self>) -> Result<FuigoAuth, AuthError> {
        ...
        if let Some(err) = self.permanent_failure() {
            ...
            if !self.fuigo_com_config.blocks_automatic_oidc()
        ...
        let result = match token_type {
            ...
            TokenType::LegacySession => {
                ...
                // A sibling process (`fuigo login` from another shell, the desktop app, etc.) may have refreshed the on-disk credentials
        ...
        // preferred_method=api_key forbids automatic OIDC mint.
        if result.is_err()
            && !self.fuigo_com_config.blocks_automatic_oidc()
            && self.is_devbox_environment()
            && let Ok(auth) = self.try_devbox_recovery(snapshot_key.as_deref()).await
        {
            return Ok(auth);
    ...
    pub(crate) async fn try_devbox_recovery(
        ...
        // Devbox mints an Ferrox Labs credential and clears auth.json to do it.
        // Under another authority, doing that would evict the sibling login and hand an Ferrox Labs refresh token to a foreign host
        if !ActiveAuthBackend::default().is_fuigo_authority() {
        ...
        fuigo_telemetry::unified_log::info("auth: devbox recovery starting", None, None);
    ...
    pub(crate) fn is_different_token(&self, candidate: &FuigoAuth) -> bool {
        let current_key = self.inner.read().as_ref().map(|a| a.key.clone());
    ...
    fn is_external_provider_refresh_authority(&self) -> bool {
        self.fuigo_com_config.auth_provider_command.is_some()
    ...
    #[cfg(test)]
    pub(crate) fn manual_auth_last_emit(&self) -> Option<fuigo_telemetry::events::ManualAuth> {
...
pub(crate) fn compute_proactive_sleep(this: &AuthManager) -> StdDuration {
    ...
    match this.inner.read().as_ref().and_then(|a| a.expires_at) {
        ...
        // Operators wanting tighter feedback set `[fuigo_com] auth_token_ttl` to drive a real schedule
...
impl fuigo_tools::types::ApiKeyProvider for SharedAuthKeyProvider {
...
fn prefers_static_api_key(am: &AuthManager) -> bool {
    matches!(
        am.fuigo_com_config.preferred_method,
...
fn resolve_static_api_key(am: &AuthManager) -> Option<String> {
    if am.fuigo_com_config.api_key_auth_disabled() {
        return None;
    }
    if matches!(
        am.fuigo_com_config.preferred_method,
...
impl AuthManager {
    /// `fuigo::api_key` from this manager's auth file, memoized on [`AuthFileStamp`].
