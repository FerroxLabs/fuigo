...
pub(crate) trait ExternalCommandRunner: Send + Sync {
    ...
    async fn run_external_command(&self, command: &str) -> Option<FuigoAuth>;
}
#[async_trait::async_trait]
...
impl RefreshOutcome {
    ...
    pub(crate) fn success(auth: FuigoAuth) -> Self {
    ...
    /// Any refresher holding the [`FuigoAuth`] it sent must use [`Self::permanent_for`] instead.
    ...
    /// Prefer this wherever the attempted [`FuigoAuth`] is in hand: it captures both the AT key (verdict scope) and the RT (sibling-rotation check).
