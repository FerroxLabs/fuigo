//! Handler for fuigo/git/worktree/* extension methods.
...
const WORKTREE_EXT_LOG: &str = "fuigo_worktree";
...
pub async fn handle(
    ...
    match args.method.as_ref() {
        ...
        "fuigo/git/worktree/create_from_worktree" => {
            ...
            let result = ops
                .dispatch(
                    &fuigo_workspace::workspace_ops::PrepareWorktreeFromWorktreeReq {
                        inner: req.clone(),
                    },
                    None,
                )
        ...
        "fuigo/git/worktree/create_from_worktree_sync" => {
            ...
            let resolved_root = ops
                .dispatch(
                    &fuigo_workspace::workspace_ops::GitResolveRootReq {
            ...
            if let Some(git_root) = resolved_root {
                let vcs_kind = ops
                    .dispatch(
                        &fuigo_workspace::workspace_ops::DetectVcsKindReq {
                            path: git_root.clone(),
                        },
                        None,
                    )
            ...
            let result = ops
                .dispatch(
                    &fuigo_workspace::workspace_ops::CreateWorktreeFromWorktreeSyncReq {
                        inner: req.into_wire(),
                    },
                    None,
                )
        ...
        "fuigo/git/worktree/resume_session" => {
            ...
            let agent_id = fuigo_telemetry::id::agent_id();
        ...
        "fuigo/git/worktree/db/stats" => {
            let result = ops
                .dispatch(
                    &fuigo_workspace::workspace_ops::WorktreeDbStatsReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
        ...
        "fuigo/git/worktree/db/rebuild" => {
            let result = ops
                .dispatch(
                    &fuigo_workspace::workspace_ops::WorktreeDbRebuildReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
        ...
        "fuigo/git/worktree/db/path" => {
            let result = ops
                .dispatch(
                    &fuigo_workspace::workspace_ops::WorktreeDbPathReq {},
                    None,
                )
                .await
                .map_err(|e| acp::Error::internal_error().data(e.to_string()))?;
...
mod tests {
    ...
    fn list_request_with_filters() {
        let json = r#"{"repo": "fuigo", "type": ["session", "fork"], "includeAll": true}"#;
        let req: ListWorktreeRequest = serde_json::from_str(json).unwrap();
        assert_eq!(req.repo.as_deref(), Some("fuigo"));
    ...
    fn db_path_response_serializes() {
        let resp = WorktreeDbPathResponse {
            path: "/home/user/.fuigo/worktrees.db".into(),
        };
        let json = serde_json::to_string(&resp).unwrap();
        assert!(json.contains("\"path\":\"/home/user/.fuigo/worktrees.db\""));
    }
