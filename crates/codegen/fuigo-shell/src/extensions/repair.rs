...
//!
//! Repairs via [`fuigo_chat_state::compaction_utils::repair_history`].
...
/// `fuigo_root` is injectable for tests.
async fn repair_on_disk(fuigo_root: &std::path::Path, session_id: &str, dry_run: bool) -> ExtResult {
    ...
    let storage = JsonlStorageAdapter::with_root(fuigo_root.to_path_buf());
    ...
    let report = fuigo_chat_state::compaction_utils::repair_history(&mut chat_history);
