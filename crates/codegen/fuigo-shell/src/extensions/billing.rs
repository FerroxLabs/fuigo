...
/// Current billing configuration for Fuigo coding credits.
///
/// Carries the newer credits-config fields (`credit_usage_percent`, `current_period`).
/// It also carries the deprecated `FuigoBuildBillingConfig` fields (`monthly_limit`, `used`, `billing_period_*`).
/// Consumers should prefer the new fields and fall back to the deprecated ones.
/// The same struct then works against both the new `GetFuigoCreditsConfig` and the legacy `GetFuigoBuildBillingConfig` responses.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct BillingConfig {
    ...
    /// It comes from the credits config (`GetFuigoCreditsConfig.prepaid_balance`) and is absent in the legacy billing shape.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub prepaid_balance: Option<Cent>,
    /// Whether this user is on unified usage billing (a shared weekly/monthly pool).
    /// It comes from `FuigoCreditsConfig.is_unified_billing_user`, which billing sets from the remote setting `unified_consumer_billing_enabled`.
    /// `None` when absent (legacy `GetFuigoBuildBillingConfig` shape or older servers).
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Top-level response (primarily from `GET /rest/fuigo/credits` and the auto-topup-rule fetch).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
///
/// Keeps history to a count + the most recent period so `~/.fuigo/logs/unified.jsonl`
...
mod tests {
    ...
    fn billing_config_deserializes_credits_config_shape() {
        // Newer `GetFuigoCreditsConfig` response: percentage-based usage, a typed current period, and history keyed by `period`
        let json = serde_json::json!({
            "config": {
                ...
                "productUsage": [
                    {"product": "PRODUCT_FUIGO_BUILD", "usagePercent": 61.2}
                ],
                "history": [
                    {
                        "period": {
