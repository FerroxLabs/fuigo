...
/// Ferrox Labs-specific session notification (parallel to acp::SessionNotification).
/// This wraps an FuigoSessionUpdate with session context for persistence and replay.
...
impl PromptUsage {
    ...
    pub(crate) fn project_from_ledger(
        ledger: Option<&fuigo_chat_state::UsageLedger>,
        incomplete: bool,
    ) -> Option<Self> {
...
impl From<&fuigo_chat_state::UsageTotals> for PromptUsageModel {
    fn from(t: &fuigo_chat_state::UsageTotals) -> Self {
        ...
        let fuigo_chat_state::UsageTotals {
            input_tokens,
            output_tokens,
            cached_read_tokens,
...
impl From<&fuigo_chat_state::UsageLedger> for PromptUsage {
    fn from(ledger: &fuigo_chat_state::UsageLedger) -> Self {
        let mut usage = Self {
...
pub enum SessionUpdate {
    ...
    HooksChanged {
        hooks: Vec<fuigo_hooks_plugins_types::HookInfo>,
    ...
    SessionStatus(Box<fuigo_status_line::StatusLineContext>),
...
pub struct CompactionSegmentFile {
    pub items: Vec<fuigo_sampling_types::ConversationItem>,
    /// Curated summary, analysis tags already stripped.
    pub summary: String,
    pub detail: fuigo_chat_state::CompactionDetail,
...
pub struct CompactionRequestFile {
    ...
    /// This is a per-artifact ID, not the model API's `x_fuigo_req_id` (which is generated per-attempt inside the sampling layer).
    pub request_id: String,
    ...
    /// `"short"` is the concise self-summarization; `"detailed"` is the 10-section structured prompt for fuigo-build and similar agents.
    ...
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub attempt_details: Vec<fuigo_chat_state::compaction_utils::CompactionAttempt>,
}
...
pub struct RecapRequestFile {
    ...
    pub schema_version: u32,
    /// Unique artifact identifier (filename stem).
    /// Distinct from the model API's `x_fuigo_req_id` (also recorded below for proxy correlation).
    pub request_id: String,
    ...
    pub model: String,
    /// Sampling request id sent to the proxy (`fuigo-recap-{uuid}`).
    pub x_fuigo_req_id: String,
    /// Sampling conversation id (`recap-{uuid}`).
    pub x_fuigo_conv_id: String,
...
mod tests {
    ...
    fn recap_request_file_roundtrips() {
        let artifact = RecapRequestFile {
            ...
            x_fuigo_req_id: "fuigo-recap-abc".into(),
            x_fuigo_conv_id: "recap-abc".into(),
        ...
        assert_eq!(parsed.x_fuigo_req_id, "fuigo-recap-abc");
        assert_eq!(
    ...
    fn compaction_request_file_v2_roundtrips_attempt_details() {
        use fuigo_chat_state::compaction_utils::CompactionAttempt;
    ...
    fn memory_flush_completed_with_path_roundtrips() {
        let update = SessionUpdate::MemoryFlushCompleted {
            result: "written".into(),
            path: Some("/home/user/.fuigo/memory/ws/sessions/log.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_dream_completed_roundtrips() {
        let update = SessionUpdate::MemoryDreamCompleted {
            ...
            path: Some("/home/user/.fuigo/memory/ws/MEMORY.md".into()),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_session_saved_roundtrips() {
        let update = SessionUpdate::MemorySessionSaved {
            path: "/home/user/.fuigo/memory/ws/sessions/2026-01-15-fix-auth-abc12345.md".into(),
        };
        let json_str = serde_json::to_string(&update).unwrap();
        let parsed: SessionUpdate = serde_json::from_str(&json_str).unwrap();
        assert_eq!(update, parsed);
    ...
    fn memory_files_variant_roundtrips_through_json() {
        let update = SessionUpdate::MemoryFiles {
            files: vec![
                MemoryFileInfo {
                    path: "/home/user/.fuigo/memory/MEMORY.md".into(),
                    source: "global".into(),
                    size_bytes: 1024,
                    modified_epoch_secs: Some(1_700_000_000),
                },
                MemoryFileInfo {
                    path: "/project/.fuigo/memory/MEMORY.md".into(),
