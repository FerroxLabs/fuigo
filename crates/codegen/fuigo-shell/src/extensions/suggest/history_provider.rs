...
/// Priority order: local fuigo bash history, then shell history, then cross-CWD history.
...
fn scan_cross_cwd_prompts() -> Vec<String> {
    let sessions_dir = crate::util::fuigo_home::fuigo_home().join("sessions");
    ...
    for (dir, _) in dirs.iter().take(MAX_CROSS_CWD_DIRS) {
        ...
        let cwd = match crate::util::fuigo_home::decode_cwd_from_dirname(dir) {
