...
//! Scanning and install logic live in the `fuigo-plugin-marketplace` crate.
...
fn load_filtered_marketplace_sources() -> Vec<fuigo_plugin_marketplace::MarketplaceSource> {
    ...
    let source_names: Vec<String> = sources
        ...
        .map(|s| {
            let url = match &s.kind {
                fuigo_plugin_marketplace::SourceKind::Git { url, .. } => url.as_str(),
...
async fn handle_action(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let outcome = match req.action {
        MarketplaceAction::Refresh { source_url_or_path } => {
            ...
            {
                Ok(outcome) => outcome,
                Err(e) => fuigo_hooks_plugins_types::ActionOutcome {
                    status: fuigo_hooks_plugins_types::OutcomeStatus::InternalError,
...
fn refresh_sources(
    ...
    for source in sources {
        ...
        if let fuigo_plugin_marketplace::SourceKind::Git { url, branch } = &source.kind {
            let cache_root = fuigo_plugin_marketplace::git::default_cache_root();
            if let Err(e) = fuigo_plugin_marketplace::git::force_sync_source_cache(
                url,
                branch.as_deref(),
                &cache_root,
            ) {
    ...
    fuigo_hooks_plugins_types::ActionOutcome {
        status: fuigo_hooks_plugins_types::OutcomeStatus::Success,
        message: msg,
        requires_reload: false,
        requires_restart: false,
    }
...
async fn handle_update(
    ...
    let source_identity = |s: &fuigo_plugin_marketplace::MarketplaceSource| -> String {
        match &s.kind {
    ...
    let marketplace_root = match &source.kind {
        ...
        fuigo_plugin_marketplace::SourceKind::Git { url, branch } => {
            let cache_root = fuigo_plugin_marketplace::git::default_cache_root();
            match fuigo_plugin_marketplace::git::sync_source_cache_with_mode(
                url,
                branch.as_deref(),
                &cache_root,
                fuigo_plugin_marketplace::git::SyncMode::Force,
            ) {
                Ok(lease) => {
    ...
    let scan = fuigo_plugin_marketplace::scan_marketplace(&marketplace_root);
    ...
    let provenance = fuigo_agent::plugins::install_registry::MarketplaceProvenance {
    ...
    match update_result {
        Ok(result) => {
            ...
            let reload_outcome = agent
                .execute_plugins_action(sid, fuigo_hooks_plugins_types::PluginsAction::Reload)
                .await;
            let mut msg = format!(
                "Updated {} ({} -> {})",
                result.repo_key,
        ...
        Err(fuigo_agent::plugins::install_registry::InstallError::PluginNotFound { .. }) => {
            ActionOutcome {
                status: OutcomeStatus::NotFound,
                message: format!(
...
fn scan_source(
    ...
    let scan = fuigo_plugin_marketplace::scan_marketplace(&root);
    let catalog_loaded = scan.catalog_loaded;
    let discovered = scan.entries;
    drop(lease);

    // Cross-reference with install registry.
    let registry = fuigo_agent::plugins::install_registry::InstallRegistry::load();
    let plugins = discovered
        .into_iter()
        .map(|p| {
            let (install_status, installed_version) =
                match fuigo_plugin_marketplace::installer::find_installed_marketplace_plugin(
                    &registry,
                    &source_url_or_path,
                    &p.relative_path,
                ) {
...
fn to_plugin_entry(
    p: fuigo_plugin_marketplace::MarketplaceEntry,
...
/// Add a new git or local-path marketplace source to `~/.fuigo/config.toml`.
async fn handle_add_source(url: &str) -> fuigo_hooks_plugins_types::ActionOutcome {
    ...
    if let MarketplaceAddInput::GitUrl(git_url) = &input {
        ...
        let probe = tokio::task::spawn_blocking(move || {
            fuigo_plugin_marketplace::git::probe_git_remote(&probe_url)
        })
        .await;
        match probe {
            Ok(Ok(())) => {}
                ...
                return ActionOutcome {
                    ...
                    message: format!(
                        "{e}. Not a reachable git repository — to add it anyway (e.g. a \
                         VPN-gated host), run: fuigo plugin marketplace add {url} --force"
                    ),
                    requires_reload: false,
                    requires_restart: false,
                };
    ...
    let config_path = fuigo_config::fuigo_home().join("config.toml");
...
fn acquire_init_lock(fuigo_home: &std::path::Path) -> std::io::Result<std::fs::File> {
    use fs2::FileExt;
    let _ = std::fs::create_dir_all(fuigo_home);
    let lock_path = fuigo_home.join(".config-init.lock");
...
fn default_skills_repo_keys<'a>(
    repos: impl IntoIterator<
        Item = (
            &'a str,
            &'a fuigo_agent::plugins::install_registry::InstalledRepo,
        ),
    >,
) -> Vec<&'a str> {
    repos
...
pub(crate) fn purge_default_skills_installs(fuigo_home: &std::path::Path) {
    purge_default_skills_installs_impl(fuigo_home, || {
        fuigo_agent::plugins::install_registry::InstallRegistry::try_load_from(
            fuigo_agent::plugins::install_registry::InstallRegistry::resolve_install_dir(),
        )
    });
}
...
fn purge_default_skills_installs_impl(
    ...
    load_registry: impl FnOnce() -> Result<
        fuigo_agent::plugins::install_registry::InstallRegistry,
        fuigo_agent::plugins::install_registry::InstallError,
    >,
) {
    let config_path = fuigo_home.join("config.toml");
...
mod official_source_tests {
    ...
    fn read_sources(
        config_path: &std::path::Path,
    ) -> Vec<fuigo_plugin_marketplace::MarketplaceSource> {
        let raw = std::fs::read_to_string(config_path).unwrap_or_default();
        let parsed: toml::Value =
            toml::from_str(&raw).unwrap_or_else(|_| toml::Value::Table(Default::default()));
        fuigo_plugin_marketplace::load_sources(&parsed)
    }
    ...
    fn add_marketplace_source_local_path_writes_path_entry() {
        ...
        assert!(matches!(
            &sources[0].kind,
            fuigo_plugin_marketplace::SourceKind::Local { path } if path == &dir
        ));
    ...
    fn pre_existing_official_source_in_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"fuigo-official":{{"source":{{"source":"git","url":"{}"}}}}}}"#,
                fuigo_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
    ...
    fn pre_existing_official_source_in_extra_known_marketplaces_skips_append() {
        ...
        std::fs::write(
            ...
            format!(
                r#"{{"extraKnownMarketplaces":{{"fuigo-official":{{"source":{{"source":"git","url":"{}"}}}}}}}}"#,
                fuigo_plugin_marketplace::OFFICIAL_SOURCE_GIT_URL,
            ),
        )
        .unwrap();
...
mod default_skills_purge_tests {
    use super::*;
    use fuigo_agent::plugins::install_registry::{
...
mod conversion_tests {
    ...
    fn to_plugin_entry_carries_homepage_and_keywords() {
        let entry = fuigo_plugin_marketplace::MarketplaceEntry {
            name: "demo".into(),
            version: Some("1.0.0".into()),
            description: Some("demo".into()),
            category: Some("dev".into()),
            author: Some("fuigo".into()),
            tags: vec!["cli".into()],
            ...
            components: Some(fuigo_hooks_plugins_types::PluginComponents {
                skills: vec![fuigo_hooks_plugins_types::ComponentItem::new(
                    "code-review",
