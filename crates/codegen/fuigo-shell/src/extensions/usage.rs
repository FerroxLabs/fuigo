//! `fuigo/session/usage`: cumulative session token and cost totals as [`PromptUsage`].
//!
//! Reads the in-memory [`fuigo_chat_state::UsageLedger`] (main-loop and folded subagent spend).
...
/// Wire response for `fuigo/session/usage`.
...
mod tests {
    ...
    fn response_serializes_ledger_as_prompt_usage_wire_shape() {
        ...
        ledger.record_main_loop_call("fuigo-build", &usage(100, 10), Some(50), Some(20_000_000));
        ...
        assert_eq!(v["usage"]["modelUsage"]["fuigo-build"]["inputTokens"], 100);
