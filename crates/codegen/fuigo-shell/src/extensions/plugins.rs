...
pub(crate) fn loaded_plugin_to_info(plugin: &fuigo_agent::plugins::LoadedPlugin) -> PluginInfo {
    use fuigo_agent::plugins::discovery::PluginScope as AgentScope;
...
fn origin_to_dto(origin: &fuigo_agent::plugins::PluginOrigin) -> PluginOrigin {
    use fuigo_agent::plugins::PluginOrigin as AgentOrigin;
    match origin {
        AgentOrigin::CliOverride => PluginOrigin::CliOverride,
        AgentOrigin::ProjectFuigo => PluginOrigin::ProjectFuigo,
        AgentOrigin::ProjectClaude => PluginOrigin::ProjectClaude,
        AgentOrigin::UserFuigo => PluginOrigin::UserFuigo,
...
mod tests {
    use super::*;
    use fuigo_agent::plugins::PluginOrigin as AgentOrigin;
    use fuigo_agent::plugins::discovery::{PluginId, PluginScope as AgentScope};

    fn make_loaded_plugin(origin: AgentOrigin) -> fuigo_agent::plugins::LoadedPlugin {
        let root = std::path::PathBuf::from("/tmp/test-plugin");
        fuigo_agent::plugins::LoadedPlugin {
    ...
    fn info_carries_origin_and_marketplace_display_name() {
        let plugin = make_loaded_plugin(AgentOrigin::MarketplaceInstall {
            source_name: Some("Ferrox Labs Official".to_string()),
