...
async fn handle_session_delete(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    // Mirrored by the `fuigo sessions delete <id>` CLI path.
...
/// Called by the config hot-reload watcher when `<cwd>/.fuigo/config.toml`, `<cwd>/.mcp.json`, or `<cwd>/.claude.json` changes.
///
...
/// Hot-reload the model catalog from `~/.fuigo/models_cache.json` after an
...
async fn handle_commands_list(agent: &MvpAgent, args: &acp::ExtRequest) -> ExtResult {
    ...
    let plugin_reg = if let Some(cwd_str) = &req.cwd {
        ...
        // Fresh discovery for *this* cwd (includes .fuigo/plugins under it, plus the cli --plugin-dir dirs)
