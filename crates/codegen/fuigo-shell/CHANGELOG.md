...
- **Windows users** can now correctly open ~/.fuigo and worktree sessions.
...
- **fuigo clone** now reuses matching local checkouts as linked worktrees for faster session creation.
...
- **MCP servers** now receive a fuigo-cli User-Agent header so providers can attribute traffic.
...
- **fuigo clone** now fetches only the selected branch tip by default, with --full-history for legacy full clones.
- **fuigo clone** can now reuse a matching local checkout as a linked worktree instead of always fetching from the network.
...
- **fuigo mcp add** now correctly treats bare http(s):// URLs as HTTP servers instead of stdio commands.
...
- **Interactive fuigo sessions** now start in ask mode by default again instead of auto.
...
- Users hitting startup timeouts can now raise the connect budget with the `FUIGO_CONNECT_UI_TIMEOUT_SECS` environment variable.
...
- **fuigo clone** can now fetch a repo into a content store and mount a projected working tree.
...
- **FUIGO_CONFIG** and **FUIGO_CONFIG_PATH** environment variables now let launchers override selected config settings without editing config.toml.
- **Worktrees** under ~/.fuigo/worktrees are now automatically reclaimed when safe, with strong safeguards that never delete a user's last copy.
...
- **FUIGO_FORCE_LOGIN_TEAM_ID** environment variable now lets launchers restrict interactive login to one or more teams.
...
- **fuigo inspect** no longer crashes when its output is piped into a command that closes the pipe early.
...
- **Session search index** can now be disabled via FUIGO_SESSION_SEARCH or [features] session_search for hosts sharing $FUIGO_HOME.
...
- Tool commands and MCP servers now receive a FUIGO_SESSION_ID environment variable matching the current session.
...
- **Keystrokes typed while Fuigo is starting** are now preserved in the composer instead of being lost.
...
- **`fuigo du`** and worktree commands now work on Windows when only USERPROFILE is set.
...
- **Subagent spawning** is dramatically faster when you have many sessions in ~/.fuigo.
...
- New `fuigo du` command shows disk usage of ~/.fuigo including worktrees and sessions.
...
- **fuigo trace** exports now bundle memory trace files for easier debugging.
...
- **Failed alpha/enterprise updates** now suggest the matching FUIGO_CHANNEL reinstall command.
- **On Apple Silicon**, fuigo now installs the native arm64 build even from a Rosetta shell or x86_64 updater.
...
- **Non-interactive sessions** (`fuigo -p`) no longer fail when the agent asks for user input or plan approval.
...
- Fuigo skips the project-directory prompt when launched from home or other non-project directories
...
- Sandboxed Fuigo starts on large directories with many deny-glob matches
...
- Fuigo retries more server errors during outages
...
- **fuigo doctor** now warns when tmux is reducing colors and can fix the config.
...
- **FUIGO_EXTRA_CA_BUNDLE** env var allows adding custom TLS root certificates.
...
- **Fuigo** no longer crashes on startup when the host machine has no free threads.
...
- **MCP servers** can now be enabled or disabled directly from the CLI with `fuigo mcp enable <name>` and `fuigo mcp disable <name>`.
...
- **Invalid MCP server entries** in config.toml no longer prevent Fuigo from starting; problems are shown in `fuigo inspect`.
...
- **Fixed credential loss** across multiple fuigo processes sharing the same auth file.
...
- **New /tutorial slash command** opens an opt-in nine-topic onboarding tour of Fuigo.
...
- **`fuigo doctor fix`** can now repair common tmux clipboard and passthrough problems.
- **Per-provider auth helpers** now work on Windows and can run from a configurable working directory.
- **/resume** now shows only native Fuigo sessions by default and shows a hint when external sessions are hidden.
- **`fuigo --resume`** can now resume a session by its title as well as by ID.
...
- You can now run `fuigo doctor fix` commands directly from inside the TUI instead of only from the CLI.
...
- **npm package** now installs the native binary under `$FUIGO_HOME/bin` (honoring the same override as the Rust CLI).
...
- Fuigo now automatically stops a turn that keeps repeating the exact same tool call many times in a row.
...
- When the auto-permission classifier times out or fails, Fuigo now shows a normal permission prompt instead of silently denying.
...
- **fuigo doctor fix ssh-wrap** can set up `fuigo wrap ssh` automatically for Bash, zsh, and fish.
...
- **fuigo doctor** checks terminal, tmux, clipboard, and keyboard setup without opening the TUI.

## Bug Fixes

- **Image paste** over fuigo wrap now works on headless remotes.
...
- **Added FUIGO_CLIPBOARD_NO_OSC52** env var to stop clipboard sequences from appearing as garbage in unsupported terminals.
...
- **/btw** now works inside `fuigo --minimal`, showing answers in the live area and committing them to scrollback on Esc.
...
- **Global rules** from ~/.fuigo/rules and compatible vendor homes are now discovered correctly.
...
- **SSH sessions** now show a one-time tip recommending `fuigo wrap ssh <host>` for clipboard and terminal restore.
...
- **fuigo** and the agent binary now stay in sync even when no update is installed.
- **Copying** a multiline queued prompt now copies the complete text instead of a collapsed summary.
- **fuigo wrap** now restores the terminal after SSH disconnects or other abrupt child exits.
- **Voice speech-to-text** now works with per-model API keys in config.toml without requiring `fuigo login`.
...
- **fuigo login** now requests Fuigo Projects scopes so workspace listing works after consent.
...
- **`fuigo plugin install`** now accepts a marketplace's registered name as a qualifier.
...
- **Fixed background task tracking** when using fuigo -p --no-wait-for-background so tasks are properly reaped on exit.
...
- **fuigo inspect** now shows effective compatibility settings for Cursor, Claude, and Codex sessions.
...
- **Resume hint** after quitting minimal mode now prints the correct fuigo --minimal --resume command.
...
- **Fuigo no longer crashes** when printing resume hints after the terminal pane has closed.
...
- **fuigo setup --json** prints your team's managed configuration without installing it.
...
- **How-to Guides** modal now shows a tip linking to Ask Fuigo above the footer shortcuts.
...
- **`fuigo worktree ls`** now works as a short alias for `fuigo worktree list`.
- **MCP tool output truncation** can now be set per-repo in `.fuigo/config.toml`.
...
- **File links and media** now work for worktree sessions under ~/.fuigo/worktrees/.
...
- **fuigo inspect** now lists skills from [skills].paths and correctly labels bundled vs user skills.
- **--minimal** and **--fullscreen** choices are now remembered for future plain fuigo launches.

## Bug Fixes

- **Queued bash commands** promoted at turn end now render their output instead of disappearing.
- **Xcode / Foundation ACP clients** can now drive fuigo agent stdio without silent parse drops on session/* calls.
...
- **fuigo --minimal** now supports native drag-select on classic Windows conhost terminals.
...
- **fuigo --minimal** now aligns the prompt, status bar, and messages flush-left with the welcome card.
...
- **fuigo update** on Windows no longer fails when a previous .old executable is still running.
...
- **fuigo models** banner now correctly reports per-model API keys and deployment keys.
...
- You can now add a local directory as a plugin marketplace source with `fuigo plugin marketplace add`.
...
- **Plan files** now default to .fuigo/plan.md to match Fuigo conventions.
...
- **--minimal** flag now shows in `fuigo --help`.
...
- **Tool result previews** no longer paint opaque panels in `fuigo --minimal`.
- **fuigo wrap** now correctly handles quoted strings and shell aliases.
...
- **Fuigo.com chat sessions** now integrate fully with the gateway bridge for model catalog and resume.
...
- **`fuigo update`** downloads have a longer timeout.
...
- **Pasting images** from the local clipboard now works when running commands through `fuigo wrap`.
...
- **fuigo agent stdio** now checks for updates in the background like other modes.
...
- **fuigo wrap** now shows copy success over SSH and suggests native drag-select when paste fails.
...
- **Fixed `fuigo agent stdio` hangs** on Windows when used with persistent clients such as VS Code.
...
- **Added `fuigo wrap`** to run any command with local clipboard support.
...
- **Fixed `fuigo agent stdio` startup hangs** on Windows when used with persistent clients such as VS Code or fuigo-desktop.
...
- You can now pass --json-schema to fuigo -p and receive a validated JSON object instead of free text.
...
- **Agent-run terminal commands** now set `FUIGO_AGENT=1` so host tools can tell them apart from interactive shells.
...
- **OIDC sessions** with FUIGO_API_KEY present no longer lose refresh on idle.
...
- **fuigo -w --ref <branch>** now creates worktrees based on the specified ref instead of HEAD.
...
- **--disable-web-search** now honored in fuigo -p and fuigo agent; auxiliary model routing respects catalog overrides.
...
- The legacy `agent` command is now kept in sync with `fuigo` after running `fuigo update`.
- Headless (`fuigo -p`) runs now wait for background tasks and subagents to finish before exiting.
...
- **`fuigo plugin install <name>`** now resolves plugins from registered marketplaces instead of only local paths.
...
- **fuigo sessions delete <id>** command now lets you permanently remove a session from the CLI.
...
- **fuigo quit** no longer hangs when background git or network tasks are slow.
...
- Session lists (welcome screen, /resume, fuigo sessions list) are now sorted by the same activity time shown in the UI.
...
- **Added option** to fully disable the hunk tracker via --hunk-tracker-mode, FUIGO_HUNK_TRACKER, or config.
...
- **Fuigo** can now export usage metrics and events to your own OpenTelemetry collector when enabled.
...
- **`fuigo dashboard`** now reliably opens the dashboard instead of silently falling through to a normal session.
...
- **`fuigo mcp add`** now accepts positional arguments (e.g. `fuigo mcp add filesystem -- npx ...`), supports --scope project, and adds -e/-H flags for env/headers.
...
- **`fuigo update`** no longer downloads the same binary twice when multiple updaters or leader checks run concurrently.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.fuigo.
...
- **MCP servers menu** and slash commands now work when starting fuigo outside a project directory.
...
- **fuigo --resume** now correctly finds the real session instead of failing on empty image-only folders.
...
- **Fixed `fuigo --resume`** failing on empty image-only session folders left by cross-directory pastes.
...
- **Ctrl+C** now cancels a blocking `fuigo update` cleanly instead of leaving an orphaned download repainting the terminal.
...
- **`fuigo --debug`** now produces per-session log files under ~/.fuigo/debug/ even with a leader process.
...
- **`fuigo update`** now restarts any older running leader so all clients get the new binary.
...
- **`fuigo login`** now defaults to device code flow, which works reliably in SSH, WSL, VPN, and browser-restricted environments.
...
- **API-key users** can now run `fuigo agent --leader` without forced interactive login or timeouts.
...
- **Official Ferrox Labs plugin marketplace** now appears automatically in the Marketplace tab on first launch.
...
- **image_gen** can now be enabled via the harness model using [features] in config.toml or the FUIGO_IMAGE_GEN_HARNESS env var.
...
- **fuigo inspect** now shows the resolved on/off state and source for every Claude/Cursor compatibility toggle.
- **Cursor MCP servers and hooks** are now discovered and can be disabled independently via FUIGO_CURSOR_MCPS_ENABLED / FUIGO_CURSOR_HOOKS_ENABLED.
...
- **Cursor skills and rules** are now discovered alongside Fuigo and Claude directories.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the fuigo-build model instead of falling back to the active session model.
