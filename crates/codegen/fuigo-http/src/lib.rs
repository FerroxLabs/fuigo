...
//!
//! Sampling traffic uses the process-wide shared clients owned by `fuigo_sampler::shared_http`.
//! Every `SamplingClient` shares one HTTP/2 pooled client and one pool-less HTTP/1.1 fallback.
//! The sampler reads `FUIGO_POOL_*` and `FUIGO_CONNECT_TIMEOUT_SECS` once, when its shared client is first built.
//! `FUIGO_SAMPLER_SHARED_CLIENT=0` falls back to a fresh client per `SamplingClient`.
//!
//! TLS policy (backend pin, roots, provider) lives in `fuigo_extra_ca`.
...
///
/// Replaces `fuigo_shell::instrumentation_timer!`, which cannot be referenced here (the shell crate depends on this one).
/// It routes to the same `fuigo_telemetry::instrumentation` API and keeps the Chrome trace span for these startup timings.
macro_rules! startup_timer {
    ($name:literal) => {{
        use fuigo_telemetry::instrumentation::{
...
// `OriginClientInfo` is owned by `fuigo-sampler` so `SamplerConfig` can use it without taking a circular dependency on `fuigo-shell`
// It is re-exported here so callers keep the `crate::http::OriginClientInfo` path
// The telemetry engine in `fuigo-telemetry` consumes the same type via `fuigo_sampler::OriginClientInfo`
// The constructors that take `ClientType` (a shell-only type) are free functions below
pub use fuigo_sampler::OriginClientInfo;

/// Construct an [`OriginClientInfo`] from the `FUIGO_CLIENT_NAME` and `FUIGO_CLIENT_VERSION` env vars.
...
/// Process-level client identifier (`FUIGO_CLIENT_NAME` env var, default `"fuigo-shell"`).
pub fn process_client_identifier() -> String {
    std::env::var("FUIGO_CLIENT_NAME").unwrap_or_else(|_| "fuigo-shell".to_string())
}

/// Header telling cli-chat-proxy whether this process is a single-prompt (`fuigo -p`) run or an interactive session.
/// The value feeds the `client_mode` metric label.
pub const CLIENT_MODE_HEADER: &str = "x-fuigo-client-mode";

/// Set to `"headless"` at startup by the non-TUI entry points (`run_single_turn` for `fuigo -p`, `run_headless_inner` for `fuigo agent [headless]`).
...
pub fn shared_startup_blocking_client() -> reqwest::blocking::Client {
    ...
    BLOCKING_CLIENT
        .get_or_init(|| {
            ...
            fuigo_extra_ca::build_blocking_reqwest_client(|builder| {
                builder
...
mod tests {
    ...
    fn origin_client_info_from_meta_extracts_identifier_and_version() {
        ...
        assert_eq!(
            ...
            Some(OriginClientInfo {
                product: "fuigo-desktop".to_string(),
    ...
    fn session_user_agent_string_renders_expected_variants() {
        ...
        assert!(with_version.starts_with("fuigo-desktop/1.2.3 fuigo-shell/"));
        ...
        assert!(without_version.starts_with("fuigo-web fuigo-shell/"));
        assert!(!without_version.starts_with("fuigo-web/"));
    }
    ...
    fn user_agent_render_collapses_duplicate_origin_and_agent_identity() {
        ...
        assert_eq!(ua.render(), "fuigo-shell/0.1.171 (macos; aarch64)");
    }
