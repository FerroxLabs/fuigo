//! TODO: Move from fuigo-shell/src/sampling/error.rs
...
use thiserror::Error;
use fuigo_circuit_breaker::RetryPolicy;
...
/// Flat error from the Fuigo proxy/gateway: `{"code": "...", "error": "..."}`.
...
pub fn status_user_message(status: StatusCode) -> String {
    match status.as_u16() {
        code @ 502..=504 => {
            format!("Fuigo is temporarily unavailable. Please try again in a moment. (HTTP {code}).")
        }
        // Upstream capacity, not an edge failure; see [`SamplingError::is_overloaded`]
        code @ 529 => {
            format!("Fuigo is temporarily overloaded. Please try again in a moment. (HTTP {code}).")
        }
        // Cloudflare edge: origin unreachable or timed out (520-524), or an edge-side 1xxx failure (530)
        code @ 520..=524 | code @ 530 => {
            format!(
                "Connection to Fuigo timed out or was interrupted. Please try again. (HTTP {code})."
            )
        }
        // Cloudflare origin TLS (handshake / invalid certificate); not transient
        code @ 525 | code @ 526 => {
            format!("Secure connection to Fuigo failed. (HTTP {code}).")
        }
...
pub use fuigo_compaction::is_context_length_error;
...
mod tests {
    ...
    // The canonical wording table lives beside the detector in fuigo-compaction; tests here pin only crate-local couplings
    #[test]
