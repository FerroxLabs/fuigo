//! Pure data types for the Ferrox Labs sampling / chat-completion API layer.
//!
//! API-agnostic conversation, chat-completion request/response, streaming, and error types used across the Ferrox Labs agent stack.
//! It contains no I/O: no HTTP clients, no file system access.
//! Downstream crates like `fuigo-chat-state` can depend on it without pulling in the full `fuigo-shell`.
...
pub use self::error::{
    ...
    is_rate_limit_error_code, is_retryable_api_status, is_size_overflow_error_code,
