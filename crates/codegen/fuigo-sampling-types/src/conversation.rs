...
/// Public because `fuigo-shell` calls it.
...
impl LengthPolicy {
    ...
    /// The Ferrox Labs server never emits a completed `tool_calls` entry for a truncated call.
...
pub struct ConversationRequest {
    ...
    /// Custom headers for Ferrox Labs tracking
    pub x_fuigo_conv_id: Option<String>,
    pub x_fuigo_req_id: Option<String>,
    pub x_fuigo_session_id: Option<String>,
    pub x_fuigo_turn_idx: Option<String>,
    /// Turn-level resubmit attempt (absent on first submissions); sent as `x-fuigo-transient-retry` so the proxy can count retry traffic.
    pub x_fuigo_transient_retry: Option<String>,
    pub x_fuigo_agent_id: Option<String>,
    pub x_fuigo_deployment_id: Option<String>,
    pub x_fuigo_user_id: Option<String>,
    ...
    /// Sticky routing key for prompt-cache reuse; overrides `x_fuigo_conv_id` for routing.
...
pub struct ConversationResponse {
    ...
    /// Server-reported doom-loop triggers for this response (Responses API only, opt-in via the `x-fuigo-doom-loop-check` header).
    ...
    /// The Ferrox Labs Messages API reports a context cut as `max_tokens`, while the Responses mapping distinguishes it.
    /// `None` when unreported.
...
// ---------------------------------------------------------------------------
//
// Lets the shared engine in `crates/common/fuigo-compaction` operate over fuigo-build's `ConversationItem` without depending on this crate
...
impl fuigo_compaction::CompactionItem for ConversationItem {
    fn role(&self) -> fuigo_compaction::CompactionRole {
        use fuigo_compaction::CompactionRole;
        // fuigo-build has no distinct `Developer` role; everything maps onto the four `Role` variants `ConversationItem::role()` already returns
        match self.role() {
    ...
    fn is_compaction_summary(&self) -> bool {
        // fuigo-build has no structural marker that uniquely identifies a prior compaction summary
        ...
        // Revisit (add a dedicated marker) before routing fuigo-build history through the shared `history`/`inter` filter
        false
    }

    fn attachment_refs(&self) -> Vec<fuigo_compaction::CompactionFileRef> {
        // fuigo-build `UserItem`s carry only `Text`/`Image { url }` content parts
        // There is no id-and-name attachment-ref concept like the chat harness's `FuigoTurn` has
        // The full-replace path does not read this; revisit if image attachments need to survive into the `<fuigo_user_queries>` preamble
        Vec::new()
    }
}

impl fuigo_compaction::CompactionItemFactory for ConversationItem {
...
/// ([stream/messages.rs:340](crates/codegen/fuigo-sampler/src/stream/messages.rs))
...
impl ConversationRequest {
    ...
    pub fn with_conv_id(mut self, conv_id: impl Into<String>) -> Self {
        self.x_fuigo_conv_id = Some(conv_id.into());
        self
    }

    /// Set request ID header
    pub fn with_req_id(mut self, req_id: impl Into<String>) -> Self {
        self.x_fuigo_req_id = Some(req_id.into());
        self
    }
...
mod compaction_item_bridge_tests {
    use super::*;
    use fuigo_compaction::{CompactionItem, CompactionItemFactory, CompactionRole};
    ...
    fn metadata_accessors_are_conservative() {
        // fuigo-build has no structural compaction-summary marker and no id-and-name attachment refs, so both return empty/false
...
mod tests {
    ...
    fn test_transform_cwd_transforms_tool_call_arguments() {
        ...
        let worktree = "/home/user/.fuigo/worktrees/project/ab-uuid-a";
    ...
    fn test_transform_cwd_worktree_to_root_syncback() {
        ...
        let worktree = "/home/user/.fuigo/worktrees/myproject/fork-a";
    ...
    fn test_transform_cwd_assistant_only_tool_calls_no_content() {
        ...
        let worktree = "/home/user/.fuigo/worktrees/proj/fork-a";
    ...
    //   2. v1 assistant with singular `reasoning: ReasoningContent` (earlier fuigo-build / chat-completions written as v1)
    ...
    #[test]
    fn upgrade_legacy_reasoning_singular_fuigo_build_shape() {
