...
// The canonical definitions live in fuigo-tools; the re-export keeps existing `crate::sampling::types::ToolDefinition` imports working
pub use fuigo_tools::types::definition::{FunctionTool, ToolDefinition};
...
pub struct Usage {
    ...
    /// Ferrox Labs extension: request price in USD ticks (1 USD = 1e10 ticks).
...
pub struct SamplingConfig {
    ...
    pub temperature: Option<f32>,
    pub top_p: Option<f32>,
    /// Model-resolved general retry budget paired with the rate-limit ceiling below; a spawned subagent inherits it from the parent's live config.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Wrapper around `async_openai::types::responses::CreateResponse` that adds custom header fields for Ferrox Labs request tracking.
...
/// Wrapper around `MessagesRequest` that adds custom header fields for Ferrox Labs request tracking, analogous to `CreateResponseWrapper`.
#[derive(Debug, Clone, Default)]
...
mod tests {
    ...
    fn parse_reasoning_efforts_meta_absent_is_none() {
        ...
        assert!(
            parse_reasoning_efforts_meta(Some(json!({ "agentType": "fuigo" }).as_object().unwrap()))
                .is_none()
        );
    }
